...
/** A marker trait for objects that support structural selection via
  ...
  * `applyDynamic` is invoked for selections with arguments `v.m(...)`. If
  * there's only one kind of selection, the method supporting the other may be
  * omitted. The `applyDynamic` can also have a second parameter list of
  * `java.lang.Class` arguments, i.e. it may alternatively have the signature
  * {{{
...
object Selectable:
  ...
  implicit def reflectiveSelectableFromLangReflectiveCalls(x: Any)(using
      scala.languageFeature.reflectiveCalls
  ): scala.reflect.Selectable =
    scala.reflect.Selectable.reflectiveSelectable(x)

  /** A marker trait for subclasses of `Selectable` indicating that precise
    * parameter types are not needed for method dispatch. That is, a class
    * inheriting from this trait and implementing
    * {{{
    ...
    * the additional restriction that the signatures of the refinement and the
    * definition that implements the refinement must match.
    */
