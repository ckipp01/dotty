...
/** An immutable array. An `IArray[T]` has the same representation as an
  * `Array[T]`, but it cannot be updated. Unlike regular arrays, immutable
...
object IArray:
  ...
  extension (arr: IArray[Byte])
    def apply(n: Int): Byte = arr.asInstanceOf[Array[Byte]].apply(n)
  extension (arr: IArray[Short])
    def apply(n: Int): Short = arr.asInstanceOf[Array[Short]].apply(n)
  extension (arr: IArray[Char])
    def apply(n: Int): Char = arr.asInstanceOf[Array[Char]].apply(n)
  extension (arr: IArray[Int])
    def apply(n: Int): Int = arr.asInstanceOf[Array[Int]].apply(n)
  extension (arr: IArray[Long])
    def apply(n: Int): Long = arr.asInstanceOf[Array[Long]].apply(n)
  extension (arr: IArray[Float])
    def apply(n: Int): Float = arr.asInstanceOf[Array[Float]].apply(n)
  extension (arr: IArray[Double])
    def apply(n: Int): Double = arr.asInstanceOf[Array[Double]].apply(n)
  ...
  /** The number of elements in an immutable array
    ...
    */
  extension (arr: IArray[Byte])
    def length: Int = arr.asInstanceOf[Array[Byte]].length
  extension (arr: IArray[Short])
    def length: Int = arr.asInstanceOf[Array[Short]].length
  extension (arr: IArray[Char])
    def length: Int = arr.asInstanceOf[Array[Char]].length
  extension (arr: IArray[Int])
    def length: Int = arr.asInstanceOf[Array[Int]].length
  extension (arr: IArray[Long])
    def length: Int = arr.asInstanceOf[Array[Long]].length
  extension (arr: IArray[Float])
    def length: Int = arr.asInstanceOf[Array[Float]].length
  extension (arr: IArray[Double])
    def length: Int = arr.asInstanceOf[Array[Double]].length
  extension (arr: IArray[Object])
    def length: Int = arr.asInstanceOf[Array[Object]].length
  extension [T](arr: IArray[T])
    def length: Int = arr.asInstanceOf[Array[T]].length
  ...
  extension [T](arr: IArray[T])
    def copyToArray[U >: T](xs: Array[U]): Int =
      genericArrayOps(arr).copyToArray(xs)

  /** Copy elements of this array to another array. */
  extension [T](arr: IArray[T])
    def copyToArray[U >: T](xs: Array[U], start: Int): Int =
      genericArrayOps(arr).copyToArray(xs, start)

  /** Copy elements of this array to another array. */
  extension [T](arr: IArray[T])
    def copyToArray[U >: T](xs: Array[U], start: Int, len: Int): Int =
  ...
  /** Finds index of first occurrence of some value in this array after or at
    * some start index.
    */
  extension [T](arr: IArray[T])
  ...
  /** Finds index of last occurrence of some value in this array before or at a
    * given end index.
    */
  extension [T](arr: IArray[T])
  ...
  extension [T](arr: IArray[T])
    def ++[U >: T: ClassTag](suffix: IArray[U]): IArray[U] =
      genericArrayOps(arr) ++ suffix.toSeq
    def ++[U >: T: ClassTag](suffix: IterableOnce[U]): IArray[U] =
      genericArrayOps(arr) ++ suffix
    ...
    def appended[U >: T: ClassTag](x: U): IArray[U] =
      genericArrayOps(arr).appended(x)
    def appendedAll[U >: T: ClassTag](suffix: IArray[U]): IArray[U] =
      genericArrayOps(arr).appendedAll(suffix)
    def appendedAll[U >: T: ClassTag](suffix: IterableOnce[U]): IArray[U] =
      genericArrayOps(arr).appendedAll(suffix)
    def collect[U: ClassTag](pf: PartialFunction[T, U]): IArray[U] =
      genericArrayOps(arr).collect(pf)
    def collectFirst[U](f: PartialFunction[T, U]): Option[U] =
      genericArrayOps(arr).collectFirst(f)
    def combinations(n: Int): Iterator[IArray[T]] =
      genericArrayOps(arr).combinations(n)
    def concat[U >: T: ClassTag](suffix: IArray[U]): IArray[U] =
      genericArrayOps(arr).concat(suffix)
    def concat[U >: T: ClassTag](suffix: IterableOnce[U]): IArray[U] =
      genericArrayOps(arr).concat(suffix)
    def diff[U >: T](that: IArray[U]): IArray[T] =
      genericArrayOps(arr).diff(that.toSeq)
    ...
    def startsWith[U >: T](that: IArray[U]): Boolean =
      genericArrayOps(arr).startsWith(that, 0)
    def startsWith[U >: T](that: IArray[U], offset: Int): Boolean =
      genericArrayOps(arr).startsWith(that, offset)
    def startsWith[U >: T](that: IterableOnce[U]): Boolean =
      genericArrayOps(arr).startsWith(that, 0)
    def startsWith[U >: T](that: IterableOnce[U], offset: Int): Boolean =
      genericArrayOps(arr).startsWith(that, offset)
    def endsWith[U >: T](that: IArray[U]): Boolean =
      genericArrayOps(arr).endsWith(that)
    def endsWith[U >: T](that: Iterable[U]): Boolean =
      genericArrayOps(arr).endsWith(that)
    def groupBy[K](f: T => K): Map[K, IArray[T]] =
      genericArrayOps(arr).groupBy(f)
    def groupMap[K, U: ClassTag](key: T => K)(f: T => U): Map[K, IArray[U]] =
      genericArrayOps(arr).groupMap(key)(f)
    def grouped(size: Int): Iterator[IArray[T]] =
      genericArrayOps(arr).grouped(size)
    def inits: Iterator[IArray[T]] = genericArrayOps(arr).inits
    def intersect[U >: T](that: IArray[U]): IArray[T] =
      genericArrayOps(arr).intersect(that)
    def intersect[U >: T](that: Seq[U]): IArray[T] =
      genericArrayOps(arr).intersect(that)
    def lazyZip[U](that: IArray[U]): LazyZip2[T, U, IArray[T]] =
      genericArrayOps(arr)
        .lazyZip[U](that)
        .asInstanceOf[LazyZip2[T, U, IArray[T]]]
    def lazyZip[U](that: Iterable[U]): LazyZip2[T, U, IArray[T]] =
      genericArrayOps(arr)
        .lazyZip[U](that)
        .asInstanceOf[LazyZip2[T, U, IArray[T]]]
    def lengthCompare(len: Int): Int = genericArrayOps(arr).lengthCompare(len)
    def padTo[U >: T: ClassTag](len: Int, elem: U): IArray[U] =
      genericArrayOps(arr).padTo(len, elem)
    def partitionMap[T1: ClassTag, T2: ClassTag](
        f: T => Either[T1, T2]
    ): (IArray[T1], IArray[T2]) = genericArrayOps(arr).partitionMap(f)
    def patch[U >: T: ClassTag](
        from: Int,
        other: IterableOnce[U],
        replaced: Int
    ): IArray[U] = genericArrayOps(arr).patch(from, other, replaced)
    def permutations: Iterator[IArray[T]] = genericArrayOps(arr).permutations
    def prepended[U >: T: ClassTag](x: U): IArray[U] =
      genericArrayOps(arr).prepended(x)
    def prependedAll[U >: T: ClassTag](prefix: IterableOnce[U]): IArray[U] =
      genericArrayOps(arr).prependedAll(prefix)
    def reverseIterator: Iterator[T] = genericArrayOps(arr).reverseIterator
    def search[U >: T](elem: U)(using Ordering[U]): Searching.SearchResult =
      arr.toSeq.search(elem)
    def search[U >: T](elem: U, from: Int, to: Int)(using
        Ordering[U]
    ): Searching.SearchResult = arr.toSeq.search(elem, from, to)
    ...
    def sizeCompare(otherSize: Int): Int =
      genericArrayOps(arr).sizeCompare(otherSize)
    def sliding(size: Int, step: Int = 1): Iterator[IArray[T]] =
      genericArrayOps(arr).sliding(size, step)
    def stepper[S <: Stepper[?]](using StepperShape[T, S]): S = genericArrayOps(
      arr
    ).stepper[S]
    ...
    def unzip[T1, T2](using
        asPair: T => (T1, T2),
        ct1: ClassTag[T1],
        ct2: ClassTag[T2]
    ): (IArray[T1], IArray[T2]) = genericArrayOps(arr).unzip
    def unzip3[T1, T2, T3](using
        asTriple: T => (T1, T2, T3),
        ct1: ClassTag[T1],
        ct2: ClassTag[T2],
        ct3: ClassTag[T3]
    ): (IArray[T1], IArray[T2], IArray[T3]) = genericArrayOps(arr).unzip3
    def updated[U >: T: ClassTag](index: Int, elem: U): IArray[U] =
      genericArrayOps(arr).updated(index, elem)
    ...
    def zip[U](that: IterableOnce[U]): IArray[(T, U)] =
      genericArrayOps(arr).zip(that)
    def zipAll[T1 >: T, U](
        that: IArray[U],
        thisElem: T1,
        thatElem: U
    ): IArray[(T1, U)] = genericArrayOps(arr).zipAll(that, thisElem, thatElem)
    def zipAll[T1 >: T, U](
        that: Iterable[U],
        thisElem: T1,
        thatElem: U
  ...
  implicit def wrapRefArray[T <: AnyRef](arr: IArray[T]): ArraySeq.ofRef[T] =
    ...
    mapNull(
      ...
      if arr.length == 0 then
        ArraySeq.empty[AnyRef].asInstanceOf[ArraySeq.ofRef[T]]
  ...
  /** Convert an array into an immutable array without copying, the original
    * array must _not_ be mutated after this or the guaranteed immutablity of
    * IArray will be violated.
    */
  ...
  /** Returns an immutable array that contains the results of some element
    * computation a number of times. Each element is determined by a separate
    * computation.
    *
    * @param n
  ...
  /** Returns a two-dimensional immutable array that contains the results of
    * some element computation a number of times. Each element is determined by
  ...
  /** Returns a three-dimensional immutable array that contains the results of
    * some element computation a number of times. Each element is determined by
    * a separate computation.
    *
    * @param n1
  ...
  def fill[T: ClassTag](n1: Int, n2: Int, n3: Int)(
      elem: => T
  ): IArray[IArray[IArray[T]]] =
    Array.fill(n1, n2, n3)(elem)

  /** Returns a four-dimensional immutable array that contains the results of
    * some element computation a number of times. Each element is determined by
    * a separate computation.
    *
    * @param n1
  ...
  def fill[T: ClassTag](n1: Int, n2: Int, n3: Int, n4: Int)(
      elem: => T
  ): IArray[IArray[IArray[IArray[T]]]] =
    Array.fill(n1, n2, n3, n4)(elem)

  /** Returns a five-dimensional immutable array that contains the results of
    * some element computation a number of times. Each element is determined by
    * a separate computation.
    *
    * @param n1
  ...
  def fill[T: ClassTag](n1: Int, n2: Int, n3: Int, n4: Int, n5: Int)(
      elem: => T
  ): IArray[IArray[IArray[IArray[IArray[T]]]]] =
    Array.fill(n1, n2, n3, n4, n5)(elem)

  /** Returns an immutable array containing values of a given function over a
    * range of integer values starting from 0.
    *
    * @param n
    *   The number of elements in the array
    * @param f
  ...
  /** Returns a two-dimensional immutable array containing values of a given
  ...
  /** Returns a three-dimensional immutable array containing values of a given
    * function over ranges of integer values starting from `0`.
    *
    * @param n1
    *   the number of elements in the 1st dimension
  ...
  /** Returns a four-dimensional immutable array containing values of a given
    * function over ranges of integer values starting from `0`.
    *
    * @param n1
    *   the number of elements in the 1st dimension
  ...
  /** Returns a five-dimensional immutable array containing values of a given
    * function over ranges of integer values starting from `0`.
    *
    * @param n1
    *   the number of elements in the 1st dimension
  ...
  /** Returns an immutable array containing a sequence of increasing integers in
    * a range.
    *
    * @param start
    ...
    *   value '''not''' returned)
    * @return
    *   the immutable array with values in range `start, start + 1, ..., end -
    *   1` up to, but excluding, `end`.
    */
  def range(start: Int, end: Int): IArray[Int] = Array.range(start, end)

  /** Returns an immutable array containing equally spaced values in some
    * integer interval.
    *
    * @param start
    *   the start value of the array
    ...
    * @return
    *   the immutable array with values in `start, start + step, ...` up to, but
    *   excluding `end`
    */
  def range(start: Int, end: Int, step: Int): IArray[Int] =
    Array.range(start, end, step)

  /** Returns an immutable array containing repeated applications of a function
    * to a start value.
    *
    * @param start
    *   the start value of the array
    ...
    * @return
    *   the immutable array returning `len` values in the sequence `start,
    *   f(start), f(f(start)), ...`
    */
  def iterate[T: ClassTag](start: T, len: Int)(f: T => T): IArray[T] =
    Array.iterate(start, len)(f)
  ...
  /** Returns a decomposition of the array into a sequence. This supports a
    * pattern match like `{ case IArray(x,y,z) => println('3 elements')}`.
    *
    * @param x
    *   the selector value
    * @return
  ...
  class WithFilter[T](p: T => Boolean, xs: IArray[T]):
    ...
    /** Apply `f` to each element for its side effects. Note: [U] parameter
      * needed to help scalac's type inference.
      */
    def foreach[U](f: T => U): Unit =
      val len = xs.length
      var i = 0
    ...
    /** Builds a new array by applying a function to all elements of this array.
      ...
      * @return
      *   a new array resulting from applying the given function `f` to each
    ...
    def flatMap[U: ClassTag](f: T => IterableOnce[U]): IArray[U] =
      ...
      while i < xs.length do
        val x = xs(i)
        if p(x) then b ++= f(xs(i))
        i += 1
      b.result()

    def flatMap[BS, U](
        f: T => BS
    )(using asIterable: BS => Iterable[U], m: ClassTag[U]): IArray[U] =
