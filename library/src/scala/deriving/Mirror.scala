...
/** Mirrors allows typelevel access to enums, case classes and objects, and
  * their sealed parents.
  */
sealed trait Mirror:
...
object Mirror:
  ...
  trait Sum extends Mirror:
    ...
    /** The ordinal number of the case class of `x`. For enums, `ordinal(x) ==
      * x.ordinal`
      */
  ...
  extension [T](p: ProductOf[T])
    /** Create a new instance of type `T` with elements taken from product `a`.
      */
    def fromProductTyped[A <: scala.Product, Elems <: p.MirroredElemTypes](
        a: A
    )(using m: ProductOf[A] { type MirroredElemTypes = Elems }): T =
      p.fromProduct(a)
