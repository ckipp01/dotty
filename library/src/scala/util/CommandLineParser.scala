...
object CommandLineParser:
  ...
  /** Parse command line argument `s`, which has index `n`, as a value of type
    * `T`
    * @throws ParseError
  ...
  /** Parse `n`'th argument in `args` (counting from 0) as a value of type `T`
    ...
    */
  def parseArgument[T](args: Array[String], n: Int)(using
      fs: FromString[T]
  ): T =
    if n < args.length then parseString(args(n), n)
    else throw ParseError(n, "more arguments expected")

  /** Parse all arguments from `n`'th one (counting from 0) as a list of values
    * of type `T`
    * @throws ParseError
    *   if some of the arguments cannot be converted to type `T`.
    */
  def parseRemainingArguments[T](args: Array[String], n: Int)(using
      fs: FromString[T]
  ): List[T] =
    if n < args.length then
      parseString(args(n), n) :: parseRemainingArguments(args, n + 1)
    else Nil
