...
trait FromDigits[T]:
  ...
  /** Convert `digits` string to value of type `T` `digits` can contain
    ...
    *
    * @throws MalformedNumber
    *   if digit string is not legal for the given type
    * @throws NumberTooLarge
    *   if value of result does not fit into `T`'s range
    * @throws NumberTooSmall
    *   in case of numeric underflow (e.g. a non-zero floating point literal
    *   that produces a zero value)
    */
...
object FromDigits:
  ...
  /** A subclass of `FromDigits` that also allows to convert whole number
    * literals with a radix other than 10
    */
  trait WithRadix[T] extends FromDigits[T]:
    def fromDigits(digits: String): T = fromDigits(digits, 10)

    /** Convert digits string with given radix to number of type `T`. E.g. if
      * radix is 16, digits `a..f` and `A..F` are also allowed.
      */
    def fromDigits(digits: String, radix: Int): T

  /** A subclass of `FromDigits` that also allows to convert number literals
    * containing a decimal point ".".
    */
  trait Decimal[T] extends FromDigits[T]

  /** A subclass of `FromDigits`that allows also to convert number literals
    * containing a decimal point "." or an exponent `('e' | 'E')['+' | '-']digit
    * digit*`.
    */
  ...
  abstract class FromDigitsException(msg: String)
      extends NumberFormatException(msg)

  /** Thrown if value of result does not fit into result type's range */
  class NumberTooLarge(msg: String = "number too large")
      extends FromDigitsException(msg)

  /** Thrown in case of numeric underflow (e.g. a non-zero floating point
    * literal that produces a zero value)
    */
  class NumberTooSmall(msg: String = "number too small")
      extends FromDigitsException(msg)

  /** Thrown if digit string is not legal for the given type */
  class MalformedNumber(msg: String = "malformed number literal")
      extends FromDigitsException(msg)

  /** Convert digits and radix to integer value (either int or Long) This is
    * tricky because of the max negative value. Note: We cannot use
    * java.lang.Integer.valueOf or java.lang.Long.valueOf since these do not
    * handle unsigned hex numbers greater than the maximal value correctly.
    */
  private def integerFromDigits(digits: String, radix: Int, limit: Long): Long =
    var value: Long = 0
    val divider = if radix == 10 then 1 else 2
    var i = 0
    var negated = false
    val len = digits.length
    if 0 < len && (digits(0) == '-' || digits(0) == '+') then
      negated = digits(0) == '-'
      i += 1
    if i == len then throw MalformedNumber()
    while i < len do
      val ch = digits(i)
      val d =
        if ch <= '9' then ch - '0'
      ...
      if d < 0 || radix <= d then throw MalformedNumber()
      if value < 0 ||
        limit / (radix / divider) < value ||
        limit - (d / divider) < value * (radix / divider) &&
        !(negated && limit == value * radix - 1 + d)
      then throw NumberTooLarge()
      value = value * radix + d
      i += 1
    if negated then -value else value
  end integerFromDigits
  ...
  /** Convert digit string to Int number
    ...
    * @throws MalformedNumber
    *   if digits is not a legal digit string. Legal strings consist only of
    *   digits conforming to radix, possibly preceded by a "-" sign.
    */
  ...
  /** Convert digit string to Float number
    ...
    * @throws NumberTooSmall
    *   if the resulting number is 0.0f, yet the digits string contains non-zero
    *   digits before the exponent.
    * @throws MalformedNumber
  ...
  def floatFromDigits(digits: String): Float =
    val x: Float =
      ...
      catch case ex: NumberFormatException => throw MalformedNumber()
    if x.isInfinite then throw NumberTooLarge()
    if x == 0.0f && !zeroFloat.pattern.matcher(digits).nn.matches then
  ...
  /** Convert digit string to Double number
    ...
    *   if the resulting number is infinite
    * @throws NumberTooSmall
    *   if the resulting number is 0.0d, yet the digits string contains non-zero
    *   digits before the exponent.
    * @throws MalformedNumber
    *   if digits is not a legal digit string for floating point numbers..
    */
  def doubleFromDigits(digits: String): Double =
    ...
    if x.isInfinite then throw NumberTooLarge()
    if x == 0.0d && !zeroFloat.pattern.matcher(digits).nn.matches then
      throw NumberTooSmall()
    x
