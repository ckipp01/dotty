...
/** A boundary that can be exited by `break` calls. `boundary` and `break`
  * represent a unified and superior alternative for the
  ...
  *   - Integration with exceptions. `break`s are logically non-fatal
  *     exceptions. The `Break` exception class extends `RuntimeException` and
  *     is optimized so that stack trace generation is suppressed.
  *   - Better performance: breaks to enclosing scopes in the same method can be
  *     rewritten to jumps.
  *
  * Example usage:
  *
  ...
  *
  * def firstIndex[T](xs: List[T], elem: T): Int = boundary: for (x, i) <-
  * xs.zipWithIndex do if x == elem then break(i)
  * -1
  */
object boundary:
  ...
  final class Break[T] private[boundary] (val label: Label[T], val value: T)
      extends RuntimeException(
        /*message*/ null, /*cause*/ null, /*enableSuppression=*/ false,
  ...
  /** Labels are targets indicating which boundary will be exited by a `break`.
    */
  @implicitNotFound(
    "explain=A Label is generated from an enclosing `scala.util.boundary` call.\nMaybe that boundary is missing?"
  )
  final class Label[-T]
  ...
  /** Abort current computation and instead continue after the `boundary` call
    * that created `label`.
    */
