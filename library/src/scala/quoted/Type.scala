...
/** Type (or type constructor) `T` needed contextually when using `T` in a
  * quoted expression `'{... T ...}`
  */
...
object Type:
  ...
  /** Show a source code like representation of this type without syntax
    * highlight
    */
  ...
  @compileTimeOnly(
    "Reference to `scala.quoted.Type.of` was not handled by PickleQuotes"
  )
  given of[T <: AnyKind](using Quotes): Type[T] = ???

  /** Extracts the value of a singleton constant type. Returns Some of the value
    * of the type if it is a singleton constant type. Returns None if the type
    * is not a singleton constant type.
    *
    * Example usage:
    * ```scala
    * //{
  ...
  /** Extracts the value of a tuple of singleton constant types. Returns Some of
    * the tuple type if it is a tuple singleton constant types. Returns None if
    * the type is not a tuple singleton constant types.
    *
    * Example usage:
    * ```scala
    * //{
    ...
    *     case '{
    *           type label <: Tuple;
    *           $mirrorExpr: Mirror.Sum { type MirroredElemLabels = `label` }
    *         } =>
  ...
  private def valueOfTuple(using Quotes)(
    ...
    def rec(tpe: TypeRepr): Option[Tuple] =
      tpe.widenTermRefByName.dealias match
        ...
        case AppliedType(tp, List(ValueOf(headValue), tail))
            if tp.derivesFrom(cons) =>
          rec(tail) match
            case Some(tailValue) => Some(headValue *: tailValue)
            case None            => None
        case tpe =>
          if tpe.derivesFrom(Symbol.classSymbol("scala.EmptyTuple")) then
            Some(EmptyTuple)
          else None
    rec(tpe)
