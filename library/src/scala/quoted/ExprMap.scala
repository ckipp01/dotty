...
trait ExprMap:
  ...
  def transformChildren[T](e: Expr[T])(using Type[T])(using Quotes): Expr[T] =
    ...
    final class MapChildren():
      ...
      def transformTermChildren(tree: Term, tpe: TypeRepr)(
          owner: Symbol
      ): Term = tree match
        case Ident(name) =>
          tree
        case Select(qualifier, name) =>
          Select.copy(tree)(
            transformTerm(qualifier, qualifier.tpe)(owner),
            name
          )
        case This(qual) =>
          tree
        ...
        case tree @ Apply(fun, args) =>
          ...
          Apply.copy(tree)(
            transformTerm(fun, TypeRepr.of[Any])(owner),
            transformTerms(args, tpes1)(owner)
          )
        ...
        case Typed(expr, tpt) =>
          val tp = tpt.tpe match
            case AppliedType(
                  TypeRef(ThisType(TypeRef(NoPrefix(), "scala")), "<repeated>"),
                  List(tp0: TypeRepr)
                ) =>
              TypeRepr.of[Seq].appliedTo(tp0)
            case tp => tp
          Typed.copy(tree)(
            transformTerm(expr, tp)(owner),
        ...
        case Match(selector, cases) =>
          Match.copy(tree)(
            transformTerm(selector, selector.tpe)(owner),
            transformCaseDefs(cases, tpe)(owner)
          )
        ...
        case Repeated(elems, elemtpt) =>
          Repeated.copy(tree)(
            transformTerms(elems, elemtpt.tpe)(owner),
            elemtpt
          )
        case Inlined(call, bindings, expansion) =>
          Inlined.copy(tree)(
            call,
            transformDefinitions(bindings)(owner),
            transformTerm(expansion, tpe)(owner)
          )
      ...
      def transformCaseDef(tree: CaseDef, tpe: TypeRepr)(
          owner: Symbol
      ): CaseDef =
        CaseDef.copy(tree)(
          tree.pattern,
      ...
      def transformDefinitions(trees: List[Definition])(
          owner: Symbol
      ): List[Definition] =
        trees.mapConserve(x => transformDefinition(x)(owner))

      def transformTerms(trees: List[Term], tpes: List[TypeRepr])(
          owner: Symbol
      ): List[Term] =
      ...
      def transformTerms(trees: List[Term], tpe: TypeRepr)(
          owner: Symbol
      ): List[Term] =
      ...
      def transformCaseDefs(trees: List[CaseDef], tpe: TypeRepr)(
          owner: Symbol
      ): List[CaseDef] =
