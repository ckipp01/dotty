...
object Expr:
  ...
  /** `e.betaReduce` returns an expression that is functionally equivalent to
    * `e`, however if `e` is of the form `((y1, ..., yn) => e2)(e1, ..., en)`
    * then it optimizes this the top most call by returning the result of
    * beta-reducing the application. Otherwise returns `expr`.
    *
  ...
  /** Returns an expression containing a block with the given statements and
    * ending with the expression Given list of statements `s1 :: s2 :: ... ::
    * Nil` and an expression `e` the resulting expression will be equivalent to
    * `'{ $s1; $s2; ...; $e }`.
    */
  def block[T](statements: List[Expr[Any]], expr: Expr[T])(using
      Quotes
  ): Expr[T] =
    import quotes.reflect.*
  ...
  /** Get `Some` of a copy of the value if the expression contains a literal
    * constant or constructor of `T`. Otherwise returns `None`.
    *
    * Usage:
    * ```scala sc:nocompile
    ...
    * ```
    *
    * To directly get the value of an expression `expr: Expr[T]` consider using
    * `expr.value`/`expr.valueOrError` instead.
    */
  ...
  /** Creates an expression that will construct a copy of this sequence
    *
    * Transforms a sequence of expression `Seq(e1, e2, ...)` where `ei: Expr[T]`
    * to an expression equivalent to `'{ Seq($e1, $e2, ...) }` typed as an
    * `Expr[Seq[T]]`
    */
  ...
  /** Creates an expression that will construct a copy of this list
    *
    * Transforms a list of expression `List(e1, e2, ...)` where `ei: Expr[T]` to
    * an expression equivalent to `'{ List($e1, $e2, ...) }` typed as an
    * `Expr[List[T]]`
    */
  ...
  /** Creates an expression that will construct a copy of this tuple
    *
    * Transforms a sequence of expression `Seq(e1, e2, ...)` where `ei:
    * Expr[Any]` to an expression equivalent to `'{ ($e1, $e2, ...) }` typed as
    * an `Expr[Tuple]`
    */
  ...
  private def ofTupleFromSeqXXL(seq: Seq[Expr[Any]])(using
      Quotes
  ): Expr[Tuple] =
  ...
  private def tupleTypeFromSeq(seq: Seq[Expr[Any]])(using
      Quotes
  ...
  /** Given a tuple of the form `(Expr[A1], ..., Expr[An])`, outputs a tuple
    * `Expr[(A1, ..., An)]`.
    */
  def ofTuple[T <: Tuple: Tuple.IsMappedBy[Expr]: Type](tup: T)(using
      Quotes
  ): Expr[Tuple.InverseMap[T, Expr]] =
    val elems: Seq[Expr[Any]] = tup
      .asInstanceOf[Product]
      .productIterator
      .toSeq
      .asInstanceOf[Seq[Expr[Any]]]
    ofTupleFromSeq(elems).asExprOf[Tuple.InverseMap[T, Expr]]

  /** Find a given instance of type `T` in the current scope. Return `Some`
    * containing the expression of the implicit or `None` if implicit resolution
    * failed.
    *
    * @tparam T
  ...
  def summon[T](using Type[T])(using Quotes): Option[Expr[T]] =
    ...
    Implicits.search(TypeRepr.of[T]) match
      case iss: ImplicitSearchSuccess =>
        Some(iss.tree.asExpr.asInstanceOf[Expr[T]])
