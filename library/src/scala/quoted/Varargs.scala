...
/** Expression representation of literal sequence of expressions.
  *
  * `Varargs` can be used to create the an expression `args` that will be used
  * as varargs `'{ f($args: _*) }` or it can be used to extract all the
  * arguments of the a varargs.
  */
object Varargs:
  ...
  def apply[T](xs: Seq[Expr[T]])(using Type[T])(using Quotes): Expr[Seq[T]] =
    import quotes.reflect.*
    Repeated(xs.map(_.asTerm).toList, TypeTree.of[T]).asExpr
      .asInstanceOf[Expr[Seq[T]]]
  ...
  def unapply[T](expr: Expr[Seq[T]])(using Quotes): Option[Seq[Expr[T]]] =
    ...
    def rec(tree: Term): Option[Seq[Expr[T]]] = tree match
      case Repeated(elems, _) =>
        Some(elems.map(x => x.asExpr.asInstanceOf[Expr[T]]))
