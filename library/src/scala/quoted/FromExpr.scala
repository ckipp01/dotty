...
/** A type class for types that can convert a `quoted.Expr[T]` to a `T`.
  ...
  *     - This expression must be some kind of data structure (`Some`, `List`,
  *       `Either`, ...)
...
object FromExpr:
  ...
  /** Default implementation of `FromExpr[String]`
    *   - Transform `'{str}` into `Some(str)` for a literal `str` of type
    *     `String`
    *   - Otherwise returns `None`
    */
  ...
  private class PrimitiveFromExpr[
  ...
  /** Default implementation of `FromExpr[Option]`
    *   - Transform `'{Some(x)}` into `Some(Some(x))` if `x` can be transformed
    *     using `FromExpr[T]`
  ...
  /** Default implementation of `FromExpr[StringContext]`
    *   - Transform `'{StringContext(args: _*)}` into `Some(StringContext(args:
    *     _*))` if `args` is explicit and each one is liftable
    *   - Otherwise returns `None`
    */
  given StringContextFromExpr: FromExpr[StringContext] with
    def unapply(x: Expr[StringContext])(using Quotes) = x match
      case '{ new StringContext(${ Varargs(Exprs(args)) }*) } =>
        Some(StringContext(args*))
      case '{ StringContext(${ Varargs(Exprs(args)) }*) } =>
        Some(StringContext(args*))
      case _ => None
  ...
  /** Default implementation of `FromExpr[Tuple1[...]]`
    *   - Transform `'{Tuple1(x1)}` into `Some(Tuple1(x1))` if `x1` can be
    *     transformed using `FromExpr[T]`
    *   - Otherwise returns `None`
    */
  given Tuple1FromExpr[T1](using Type[T1], FromExpr[T1]): FromExpr[Tuple1[T1]]
  with
  ...
  /** Default implementation of `FromExpr[Tuple2[...]]`
    *   - Transform `'{Tuple2(x1, x2)}` into `Some(Tuple2(x1, x2))` if all `xi`
    *     can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple2FromExpr[T1, T2](using
      Type[T1],
      Type[T2],
      FromExpr[T1],
      FromExpr[T2]
  ): FromExpr[Tuple2[T1, T2]] with
    def unapply(x: Expr[Tuple2[T1, T2]])(using Quotes) = x match
      case '{ new Tuple2[T1, T2](${ Expr(y1) }, ${ Expr(y2) }) } =>
        Some(Tuple2(y1, y2))
      case '{ Tuple2[T1, T2](${ Expr(y1) }, ${ Expr(y2) }) } =>
        Some(Tuple2(y1, y2))
      case '{ (${ Expr(y1) }: T1) -> (${ Expr(y2) }: T2) } =>
        Some(Tuple2(y1, y2))
      case _ => None

  /** Default implementation of `FromExpr[Tuple3[...]]`
    *   - Transform `'{Tuple3(x1, x2, x3)}` into `Some(Tuple3(x1, x2, x3))` if
    *     all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple3FromExpr[T1, T2, T3](using
      Type[T1],
      Type[T2],
      Type[T3],
      FromExpr[T1],
      FromExpr[T2],
      FromExpr[T3]
  ): FromExpr[Tuple3[T1, T2, T3]] with
    def unapply(x: Expr[Tuple3[T1, T2, T3]])(using Quotes) = x match
      case '{
            new Tuple3[T1, T2, T3](${ Expr(y1) }, ${ Expr(y2) }, ${ Expr(y3) })
          } =>
        Some(Tuple3(y1, y2, y3))
      case '{
            Tuple3[T1, T2, T3](${ Expr(y1) }, ${ Expr(y2) }, ${ Expr(y3) })
          } =>
        Some(Tuple3(y1, y2, y3))
      case _ => None

  /** Default implementation of `FromExpr[Tuple4[...]]`
    *   - Transform `'{Tuple4(x1, ..., x4)}` into `Some(Tuple4(x1, ..., x4))` if
    *     all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple4FromExpr[T1, T2, T3, T4](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T3],
      FromExpr[T4]
  ): FromExpr[Tuple4[T1, T2, T3, T4]] with
  ...
  /** Default implementation of `FromExpr[Tuple5[...]]`
    *   - Transform `'{Tuple5(x1, ..., x5)}` into `Some(Tuple5(x1, ..., x5))` if
    *     all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple5FromExpr[T1, T2, T3, T4, T5](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T4],
      FromExpr[T5]
  ): FromExpr[Tuple5[T1, T2, T3, T4, T5]] with
  ...
  /** Default implementation of `FromExpr[Tuple6[...]]`
    *   - Transform `'{Tuple6(x1, ..., x6)}` into `Some(Tuple6(x1, ..., x6))` if
    *     all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple6FromExpr[T1, T2, T3, T4, T5, T6](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T5],
      FromExpr[T6]
  ): FromExpr[Tuple6[T1, T2, T3, T4, T5, T6]] with
  ...
  /** Default implementation of `FromExpr[Tuple7[...]]`
    *   - Transform `'{Tuple7(x1, ..., x7)}` into `Some(Tuple7(x1, ..., x7))` if
    *     all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple7FromExpr[T1, T2, T3, T4, T5, T6, T7](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T6],
      FromExpr[T7]
  ): FromExpr[Tuple7[T1, T2, T3, T4, T5, T6, T7]] with
    def unapply(x: Expr[Tuple7[T1, T2, T3, T4, T5, T6, T7]])(using Quotes) =
      x match
        case '{
          ...
          Some(Tuple7(y1, y2, y3, y4, y5, y6, y7))
        case '{
  ...
  /** Default implementation of `FromExpr[Tuple8[...]]`
    *   - Transform `'{Tuple8(x1, ..., x8)}` into `Some(Tuple8(x1, ..., x8))` if
    *     all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple8FromExpr[T1, T2, T3, T4, T5, T6, T7, T8](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T7],
      FromExpr[T8]
  ): FromExpr[Tuple8[T1, T2, T3, T4, T5, T6, T7, T8]] with
    def unapply(x: Expr[Tuple8[T1, T2, T3, T4, T5, T6, T7, T8]])(using Quotes) =
      x match
        case '{
          ...
          Some(Tuple8(y1, y2, y3, y4, y5, y6, y7, y8))
        case '{
  ...
  /** Default implementation of `FromExpr[Tuple9[...]]`
    *   - Transform `'{Tuple9(x1, ..., x9)}` into `Some(Tuple9(x1, ..., x9))` if
    *     all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple9FromExpr[T1, T2, T3, T4, T5, T6, T7, T8, T9](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T8],
      FromExpr[T9]
  ): FromExpr[Tuple9[T1, T2, T3, T4, T5, T6, T7, T8, T9]] with
    def unapply(x: Expr[Tuple9[T1, T2, T3, T4, T5, T6, T7, T8, T9]])(using
        Quotes
    ) = x match
      case '{
        ...
        Some(Tuple9(y1, y2, y3, y4, y5, y6, y7, y8, y9))
      case '{
  ...
  /** Default implementation of `FromExpr[Tuple10[...]]`
    *   - Transform `'{Tuple0(x1, ..., x10)}` into `Some(Tuple0(x1, ..., x10))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple10FromExpr[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
    ...
    def unapply(x: Expr[Tuple10[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10]])(using
        Quotes
    ) = x match
      case '{
        ...
        Some(Tuple10(y1, y2, y3, y4, y5, y6, y7, y8, y9, y10))
      case '{
  ...
  /** Default implementation of `FromExpr[Tuple11[...]]`
    *   - Transform `'{Tuple1(x1, ..., x11)}` into `Some(Tuple1(x1, ..., x11))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple11FromExpr[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T10],
      FromExpr[T11]
  ): FromExpr[Tuple11[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11]] with
    def unapply(x: Expr[Tuple11[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11]])(
        using Quotes
    ) = x match
      case '{
        ...
        Some(Tuple11(y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11))
      case '{
  ...
  /** Default implementation of `FromExpr[Tuple12[...]]`
    *   - Transform `'{Tuple2(x1, ..., x12)}` into `Some(Tuple2(x1, ..., x12))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple12FromExpr[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12](using
      Type[T1],
      Type[T2],
      Type[T3],
      Type[T4],
      ...
      FromExpr[T11],
      FromExpr[T12]
  ): FromExpr[Tuple12[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12]] with
    def unapply(
        x: Expr[Tuple12[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12]]
    )(using Quotes) = x match
      case '{
        ...
        Some(Tuple12(y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12))
      case '{
  ...
  /** Default implementation of `FromExpr[Tuple13[...]]`
    *   - Transform `'{Tuple3(x1, ..., x13)}` into `Some(Tuple3(x1, ..., x13))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple13FromExpr[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13](
      using
      Type[T1],
      Type[T2],
      Type[T3],
      ...
      FromExpr[T12],
      FromExpr[T13]
  ): FromExpr[Tuple13[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13]]
  with
    def unapply(
        x: Expr[Tuple13[T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13]]
    )(using Quotes) = x match
      case '{
        ...
        Some(Tuple13(y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13))
      case '{
  ...
  /** Default implementation of `FromExpr[Tuple14[...]]`
    *   - Transform `'{Tuple4(x1, ..., x14)}` into `Some(Tuple4(x1, ..., x14))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple14FromExpr[
      T1,
      T2,
      T3,
      T4,
    ...
    def unapply(
      ...
      case '{
        ...
        Some(
          Tuple14(y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14)
        )
      case '{
            Tuple14[
              T1,
  ...
  /** Default implementation of `FromExpr[Tuple15[...]]`
    *   - Transform `'{Tuple5(x1, ..., x15)}` into `Some(Tuple5(x1, ..., x15))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple15FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Tuple16[...]]`
    *   - Transform `'{Tuple6(x1, ..., x16)}` into `Some(Tuple6(x1, ..., x16))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple16FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Tuple17[...]]`
    *   - Transform `'{Tuple7(x1, ..., x17)}` into `Some(Tuple7(x1, ..., x17))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple17FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Tuple18[...]]`
    *   - Transform `'{Tuple8(x1, ..., x18)}` into `Some(Tuple8(x1, ..., x18))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple18FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Tuple19[...]]`
    *   - Transform `'{Tuple9(x1, ..., x19)}` into `Some(Tuple9(x1, ..., x19))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple19FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Tuple20[...]]`
    *   - Transform `'{Tuple0(x1, ..., x20)}` into `Some(Tuple0(x1, ..., x20))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple20FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Tuple21[...]]`
    *   - Transform `'{Tuple1(x1, ..., x21)}` into `Some(Tuple1(x1, ..., x21))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple21FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Tuple22[...]]`
    *   - Transform `'{Tuple2(x1, ..., x22)}` into `Some(Tuple2(x1, ..., x22))`
    *     if all `xi` can be transformed using `FromExpr[Ti]`
    *   - Otherwise returns `None`
    */
  given Tuple22FromExpr[
      T1,
      T2,
      T3,
      T4,
  ...
  /** Default implementation of `FromExpr[Seq]`
    *   - Transform `'{Seq(x1, ..., xn)}` into `Some(Seq(x1, ..., xn))` if all
    *     `xi` can be transformed using `FromExpr[Ti]`
  ...
  given SeqFromExpr[T](using Type[T], FromExpr[T]): FromExpr[Seq[T]] with
    def unapply(x: Expr[Seq[T]])(using Quotes) = x match
      ...
      case '{
            scala.collection.immutable.Seq[T](${ Varargs(Exprs(elems)) }*)
          } =>
        Some(elems)
  ...
  /** Default implementation of `FromExpr[List]`
    *   - Transform `'{List(x1, ..., xn)}` into `Some(List(x1, ..., xn))` if all
    *     `xi` can be transformed using `FromExpr[Ti]`
  ...
  given ListFromExpr[T](using Type[T], FromExpr[T]): FromExpr[List[T]] with
    def unapply(x: Expr[List[T]])(using Quotes) = x match
      ...
      case '{ Nil }                                        => Some(Nil)
      case '{
            scala.collection.immutable.List[T](${ Varargs(Exprs(elems)) }*)
          } =>
        Some(elems.toList)
  ...
  /** Default implementation of `FromExpr[Set]`
    *   - Transform `'{Set(x1, ..., xn)}` into `Some(Set(x1, ..., xn))` if all
    *     `xi` can be transformed using `FromExpr[Ti]`
  ...
  given SetFromExpr[T](using Type[T], FromExpr[T]): FromExpr[Set[T]] with
    def unapply(x: Expr[Set[T]])(using Quotes) = x match
      ...
      case '{
            scala.collection.immutable.Set[T](${ Varargs(Exprs(elems)) }*)
          } =>
        Some(elems.toSet)
  ...
  /** Default implementation of `FromExpr[Map]`
    *   - Transform `'{Map(x1, ..., xn)}` into `Some(Map(x1, ..., xn))` if all
    *     `xi` can be transformed using `FromExpr[Ti]`
    *   - Transform `'{Map.empty}` into `Some(Map())`
    *   - Otherwise returns `None`
    */
  given MapFromExpr[T, U](using
      Type[T],
      Type[U],
      FromExpr[T],
      FromExpr[U]
  ): FromExpr[Map[T, U]] with
    def unapply(x: Expr[Map[T, U]])(using Quotes) = x match
      ...
      case '{
            scala.collection.immutable.Map[T, U](${ Varargs(Exprs(elems)) }*)
          } =>
        Some(elems.toMap)
  ...
  /** Default implementation of `FromExpr[Either]`
    *   - Transform `'{Left(x)}` into `Some(Left(x))` if `x` can be transformed
    *     using `FromExpr[L]`
    *   - Transform `'{Right(x)}` into `Some(Right(x))` if `x` can be
    *     transformed using `FromExpr[R]`
    *   - Otherwise returns `None`
    */
  given EitherFromExpr[L, R](using
      Type[L],
      Type[R],
      FromExpr[L],
      FromExpr[R]
  ): FromExpr[Either[L, R]] with
  ...
  /** Default implementation of `FromExpr[Left]`
    ...
    *   - Otherwise returns `None`
    */
  given LeftFromExpr[L, R](using
      Type[L],
      Type[R],
      FromExpr[L]
  ): FromExpr[Left[L, R]] with
  ...
  /** Default implementation of `FromExpr[Right]`
    ...
    *   - Otherwise returns `None`
    */
  given RightFromExpr[L, R](using
      Type[L],
      Type[R],
      FromExpr[R]
  ): FromExpr[Right[L, R]] with
