...
object long:
  /** Successor of a natural number where zero is the type 0 and successors are
    * reduced as if the definition was:
    *
    * ```scala
  ...
  /** Number of zero bits preceding the highest-order ("leftmost") one-bit in
    * the two's complement binary representation of the specified `Long`
    * singleton type. Returns 64 if the specified singleton type has no one-bits
    * in its two's complement representation, in other words if it is equal to
    * zero.
    * ```scala
    * //{
    * import compiletime.ops.long.*
