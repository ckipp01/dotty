...
/** Use this method when you have a type, do not have a value for it but want to
  ...
  * //}
  * ```
  * This value can only be used in an inline match and the value cannot be used
  * in the branches.
  * @syntax
  *   markdown
  */
...
/** Used as the initializer of a mutable class or object field, like this:
  ...
  * ```
  *
  * This signifies that the field is not initialized on its own. It is still
  * initialized as part of the bulk initialization of the object it belongs to,
  * which assigns zero values such as `null`, `0`, `0.0`, `false` to all object
  * fields.
  */
@compileTimeOnly(
...
/** The error method is used to produce user-defined compile errors during
  * inline expansion. If an inline expansion results in a call error(msgStr) the
  * compiler produces an error message containing the given msgStr.
  *
  * ```scala sc:fail
...
/** Returns the string representation of argument code:
  ...
  *
  * @note
  *   only `inline` arguments will be displayed as "code". Other values may
  *   display unintutively.
  *
  * @syntax
  *   markdown
  */
...
/** Checks at compiletime that the provided values is a constant after inlining
  ...
  *   requireConst(
  *     n
  *   ) // compile-time assertion that the parameter `n` is a constant
  *   n + n
  *
  * twice(1)
  * val m: Int = ???
  ...
  *   markdown
  */
inline def requireConst(
    inline x: Boolean | Byte | Short | Int | Long | Float | Double | Char |
      String
): Unit =
  // implemented in dotty.tools.dotc.typer.Inliner
  error("Compiler bug: `requireConst` was not evaluated by the compiler")

/** Same as `constValue` but returns a `None` if a constant value cannot be
  * constructed from the provided type. Otherwise returns that value wrapped in
  * `Some`.
  */
...
/** Given a constant, singleton type `T`, convert it to a value of the same
  * singleton type. For example: `assert(constValue[1] == 1)`.
  */
transparent inline def constValue[T]: T =
  // implemented in dotty.tools.dotc.typer.Inliner
  error("Compiler bug: `constValue` was not evaluated by the compiler")

/** Given a tuple type `(X1, ..., Xn)`, returns a tuple value `(constValue[X1],
  * ..., constValue[Xn])`.
  */
...
/** Summon a given value of type `T`. Usually, the argument is not passed
  * explicitly. The summoning is delayed until the call has been fully inlined.
  *
  * @tparam T
...
/** Given a tuple T, summons each of its member types and returns them in a
  * Tuple.
  *
  * @tparam T
...
/** Casts a value to be `Matchable`. This is needed if the value's type is an
  * unconstrained type parameter and the value is the scrutinee of a match
  * expression. This is normally disallowed since it violates parametricity and
  * allows to uncover implementation details that were intended to be hidden.
  * The `asMatchable` escape hatch should be used sparingly. It's usually better
  * to constrain the scrutinee type to be `Matchable` in the first place.
  */
extension [T](x: T)
