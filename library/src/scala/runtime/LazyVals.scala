...
object LazyVals:
  ...
  /** Used to indicate the state of a lazy val that is being evaluated and of
    * which other threads await the result.
    */
  final class Waiting extends CountDownLatch(1) with LazyValControlState

  /** Used to indicate the state of a lazy val that is currently being evaluated
    * with no other thread awaiting its result.
    */
  ...
  def STATE(cur: Long, ord: Int): Long =
    ...
    if debug then println(s"STATE($cur, $ord) = $r")
    r

  def CAS(t: Object, offset: Long, e: Long, v: Int, ord: Int): Boolean =
    if debug then println(s"CAS($t, $offset, $e, $v, $ord)")
  ...
  def wait4Notification(t: Object, offset: Long, cur: Long, ord: Int): Unit =
    ...
    while retry do
      ...
      if state == 1 then CAS(t, offset, cur, 2, ord)
      else if state == 2 then
        val monitor = getMonitor(t, ord)
        monitor.synchronized {
          if STATE(get(t, offset), ord) == 2
          then // make sure notification did not happen yet.
            monitor.wait()
        }
      else retry = false

  def get(t: Object, off: Long): Long =
    if debug then println(s"get($t, $off)")
  ...
  def getOffset(clz: Class[?], name: String): Long =
    ...
    if debug then println(s"getOffset($clz, $name) = $r")
    r
  ...
  def getStaticFieldOffset(field: java.lang.reflect.Field): Long =
    ...
    if debug then
      println(
        s"getStaticFieldOffset(${field.getDeclaringClass}, ${field.getName}) = $r"
      )
    r
