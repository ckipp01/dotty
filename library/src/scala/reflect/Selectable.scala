...
/** A class that implements structural selections using Java reflection.
  *
  * It can be used as a supertrait of a class or be made available as an
  * implicit conversion via `reflectiveSelectable`.
  *
  * In Scala.js, it is implemented using a separate Scala.js-specific mechanism,
  * since Java reflection is not available.
  */
trait Selectable extends scala.Selectable:

  /** The value from which structural members are selected. By default this is
    * the Selectable instance itself, but it can be overridden.
    */
...
object Selectable:
  ...
  /** An implicit conversion that turns a value into a Selectable such that
    * structural selections are performed on that value.
    */
  implicit def reflectiveSelectable(x: Any): Selectable =
    new DefaultSelectable(x)

  @inline // important for Scala.js
  private final class DefaultSelectable(
      override protected val selectedValue: Any
  ) extends Selectable
end Selectable
