...
/** MainAnnotation provides the functionality for a compiler-generated main
  * class. It links a compiler-generated main method (call it compiler-main) to
  * a user written main method (user-main). The protocol of calls from
  * compiler-main is as follows:
  *
  *   - create a `command` with the command line arguments,
  *   - for each parameter of user-main, a call to `command.argGetter`, or
  *     `command.varargGetter` if is a final varargs parameter,
  *   - a call to `command.run` with the closure of user-main applied to all
  *     arguments.
  *
  * Example:
  * ```scala sc:nocompile
  ...
  *   */
  * @myMain def sum(first: Int, second: Int = 0, rest: Int*): Int =
  ...
  *           hasDefault = false,
  *           isVarargs = true,
  *           "The rest of the numbers to sum"
  *         )
  *       )
  *     )
  ...
  *       val args0 = mainAnnot.argGetter[Int](
  *         info.parameters(0),
  *         mainArgs(0),
  *         None
  *       ) // using parser Int
  *       val args1 = mainAnnot.argGetter[Int](
  *         info.parameters(1),
  *         mainArgs(1),
  *         Some(() => sum$default$1())
  *       ) // using parser Int
  *       val args2 = mainAnnot.varargGetter[Int](
  *         info.parameters(2),
  *         mainArgs.drop(2)
  *       ) // using parser Int
  ...
  * @param Result
  *   The required result type of the main method. If this type is Any or Unit,
...
trait MainAnnotation[Parser[_], Result] extends StaticAnnotation:
  ...
  /** Process the command arguments before parsing them.
    *
    * Return `Some` of the sequence of arguments that will be parsed to be
    * passed to the main method. This sequence needs to have the same length as
    * the number of parameters of the main method (i.e. `info.parameters.size`).
    * If there is a varags parameter, then the sequence must be at least of
    * length `info.parameters.size - 1`.
    *
    * Returns `None` if the arguments are invalid and parsing and run should be
    * stopped.
    *
    * @param info
    *   The information about the command (name, documentation and info about
    *   parameters)
    * @param args
  ...
  /** Run `program` if all arguments are valid if all arguments are valid
    *
    * @param program
    *   A function containing the call to the main method and instantiation of
    *   its arguments
    */
...
object MainAnnotation:
  ...
  /** Information about the main method
    ...
    * @param documentation
    *   The documentation of the main method without the `@param` documentation
    *   (see Parameter.documentaion)
    * @param parameters
  ...
  /** Information about a parameter of a main method
    ...
    * @param isVarargs
    *   If the parameter is a varargs parameter (can only be true for the last
    *   parameter)
    * @param documentation
    *   The documentation of the parameter (from `@param` documentation in the
    *   main method)
    * @param annotations
  ...
  /** Marker trait for annotations that will be included in the Parameter
    * annotations.
    */
  @experimental // MiMa does not check scope inherited @experimental
