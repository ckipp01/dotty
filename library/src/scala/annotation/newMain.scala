...
/** The annotation that designates a main function. Main functions are entry
  * points for Scala programs. They can be called through a command line
  * interface by using the `scala` command, followed by their name and,
  * optionally, their parameters.
  *
  * The parameters of a main function may have any type `T`, as long as there
  * exists a `given util.CommandLineParser.FromString[T]` in the scope. It will
  * be used for parsing the string given as input into the correct argument
  * type. These types already have parsers defined:
  *   - String,
  *   - Boolean,
  *   - Byte, Short, Int, Long, Float, Double.
  *
  * The parameters of a main function may be passed either by position, or by
  * name. Passing an argument positionally means that you give the arguments in
  * the same order as the function's signature. Passing an argument by name
  * means that you give the argument right after giving its name. Considering
  * the function `@newMain def foo(i: Int, str: String)`, we may have arguments
  * passed:
  *   - by position: `scala foo 1 abc`,
  *   - by name: `scala foo -i 1 --str abc` or `scala foo --str abc -i 1`.
  *
  * A mixture of both is also possible: `scala foo --str abc 1` is equivalent to
  * all previous examples.
  *
  * Note that main function overloading is not currently supported, i.e. you
  * cannot define two main methods that have the same name in the same project.
  *
  * Special arguments are used to display help regarding a main function:
  * `--help` and `-h`. If used as argument, the program will display some useful
  * information about the main function. This help directly uses the ScalaDoc
  * comment associated with the function, more precisely its description and the
  * description of the parameters documented with `@param`. Note that if a
  * parameter is named `help` or `h`, or if one of the parameters has as alias
  * one of those names, the help displaying will be disabled for that argument.
  * For example, for `@newMain def foo(help: Boolean)`, `scala foo -h` will
  * display the help, but `scala foo --help` will fail, as it will expect a
  * Boolean value after `--help`.
  *
  * Parameters may be given annotations to add functionalities to the main
  * function:
  *   - `main.alias` adds other names to a parameter. For example, if a
  *     parameter `node` has as aliases `otherNode` and `n`, it may be addressed
  *     using `--node`, `--otherNode` or `-n`.
  *
  * Here is an example of a main function with annotated parameters: `@newMain
  * def foo(@newMain.alias("x") number: Int, @newMain.alias("explanation") s:
  * String)`. The following commands are equivalent:
  *   - `scala foo --number 1 -s abc`
  *   - `scala foo -x 1 -s abc`
  *   - `scala foo --number 1 --explanation abc`
  *   - `scala foo -x 1 --explanation abc`
  *
  * Boolean parameters are considered flags that do not require the "true" or
  * "false" value to be passed. For example, `@newMain def foo(i: Boolean)` can
  * be called as `foo` (where `i=false`) or `foo -i` (where `i=true`).
  *
  * The special `--` marker can be used to indicate that all following arguments
  * are passed verbatim as positional parameters. For example, `@newMain def
  * foo(args: String*)` can be called as `scala foo a b -- -c -d` which implies
  * that `args=Seq("a", "b", "-c", "-d")`.
  */
@experimental
final class newMain extends MainAnnotation[FromString, Any]:
  ...
  extension (param: Parameter)
    private def aliasNames: Seq[String] =
      param.annotations.collect { case alias: alias =>
        getNameWithMarker(alias.name)
      }
  ...
  private def preProcessArgs(
    ...
    val orderedArgs: List[String] =
      def rec(
        ...
        params match
          case Nil =>
            for remainingArg <- remainingArgs do
              error(s"unused argument: $remainingArg")
            acc.reverse
          case param :: tailParams =>
            if param.isVarargs then // also last arguments
              byNameArgsMap.get(param.name) match
                case Some(byNameVarargs) =>
                  acc.reverse ::: byNameVarargs.toList ::: remainingArgs
                case None => acc.reverse ::: remainingArgs
            else
              byNameArgsMap.get(getNameWithMarker(param.name)) match
                case Some(argValues) =>
                  assert(
                    argValues.nonEmpty,
                    s"${param.name} present in byNameArgsMap, but it has no argument value"
                  )
                  if argValues.length > 1 then
                    error(
                      s"more than one value for ${param.name}: ${argValues.mkString(", ")}"
                    )
  ...
  private def parse[T](param: Parameter, arg: String)(using
      p: FromString[T]
  ): () => T =
    p.fromStringOption(arg) match
      ...
      case None =>
        ...
        println(
          s"Error: could not parse argument for `${param.name}` of type ${param.typeName.split('.').last}: $arg"
        )
        hasParseErrors = true
  ...
  private object Help:
    ...
    /** The name of the special argument to display the method's help. If one of
      * the method's parameters is called the same, will be ignored.
      */
    private inline val helpArg = "--help"

    /** The short name of the special argument to display the method's help. If
      * one of the method's parameters uses the same short name, will be
      * ignored.
      */
    ...
    def printUsage(info: Info): Unit =
      def argsUsage: Seq[String] =
        for (param <- info.parameters)
          yield
            ...
            val namesPrint =
              (canonicalName +: param.aliasNames).mkString("[", " | ", "]")
            val shortTypeName = param.typeName.split('.').last
            if param.isVarargs then
              s"[<$shortTypeName> [<$shortTypeName> [...]]]"
      ...
      def wrapArgumentUsages(
          argsUsage: Seq[String],
          maxLength: Int
      ): Seq[String] =
        def recurse(
            args: Seq[String],
          ...
          (args, currentLine) match
            ...
            case (arg +: t, l) if l.length + 1 + arg.length <= maxLength =>
              recurse(t, s"$l $arg", acc)
      ...
      val printUsages =
        wrapArgumentUsages(argsUsage, maxUsageLineLength - argsOffset)

      println(
        printUsageBeginning + printUsages.mkString("\n" + " " * argsOffset)
      )
    end printUsage

    def printExplain(info: Info): Unit =
      def shiftLines(s: Seq[String], shift: Int): String =
        s.map(" " * shift + _).mkString("\n")
      ...
      if info.documentation.nonEmpty then
        println(
          wrapLongLine(info.documentation, maxUsageLineLength).mkString("\n")
        )
      if info.parameters.nonEmpty then
        val argNameShift = 2
          ...
          if param.documentation.nonEmpty then
            val shiftedDoc =
              param.documentation
                ...
                .map(line =>
                  shiftLines(
                    wrapLongLine(line.nn, maxUsageLineLength - argDocShift),
                    argDocShift
                  )
                )
                .mkString("\n")
  ...
  private class Names(info: Info):
    ...
    private lazy val namesToCanonicalName: Map[String, String] =
      info.parameters
        .flatMap(param =>
          val canonicalName = getNameWithMarker(param.name)
          (canonicalName -> canonicalName) +: param.aliasNames
            .map(_ -> canonicalName)
        )
        .toMap
    ...
    def canonicalName(name: String): Option[String] =
      namesToCanonicalName.get(name)

    def isFlagName(name: String): Boolean =
      namesToCanonicalName
        .get(name)
        .map(canonicalFlagsNames.contains)
        .contains(true)
    ...
    private def checkNames(): Unit =
      def checkDuplicateNames() =
        val nameAndCanonicalName = info.parameters.flatMap { paramInfo =>
          (getNameWithMarker(paramInfo.name) +: paramInfo.aliasNames)
            .map(_ -> paramInfo.name)
        }
        val nameToNames = nameAndCanonicalName.groupMap(_._1)(_._2)
        for (name, canonicalNames) <- nameToNames if canonicalNames.length > 1
        do
          throw IllegalArgumentException(
            s"$name is used for multiple parameters: ${canonicalNames.mkString(", ")}"
          )
      def checkValidNames() =
        ...
        for param <- info.parameters do
          if !isValidArgName(param.name) then
            throw IllegalArgumentException(
              s"The following argument name is invalid: ${param.name}"
            )
          for annot <- param.annotations do
            annot match
              case alias: alias if !isValidArgName(alias.name) =>
                throw IllegalArgumentException(
                  s"The following alias is invalid: ${alias.name}"
                )
              case _ =>
    ...
    private def checkFlags(): Unit =
      for param <- info.parameters if param.isFlag && param.hasDefault do
        throw IllegalArgumentException(
          s"@newMain flag parameters cannot have a default value. `${param.name}` has a default value."
        )
...
object newMain:
  ...
  /** Alias name for the parameter.
    *
    * If the name has one character, then it is a short name (e.g. `-i`). If the
    * name has more than one characters, then it is a long name (e.g.
    * `--input`).
    */
  @experimental
