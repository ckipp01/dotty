...
/** Base trait for macro annotation implementation. Macro annotations can
  * transform definitions and add new definitions.
  *
...
trait MacroAnnotation extends StaticAnnotation:
  ...
  /** Transform the `tree` definition and add new definitions
    *
    * This method takes as argument the annotated definition. It returns a
    * non-empty list containing the modified version of the annotated
    * definition. The new tree for the definition must use the original symbol.
    * New definitions can be added to the list before or after the transformed
    * definitions, this order will be retained. New definitions will not be
    * visible from outside the macro expansion.
    *
    * #### Restrictions
    *   - All definitions in the result must have the same owner. The owner can
    *     be recovered from `Symbol.spliceOwner`.
    *     - Special case: an annotated top-level `def`, `val`, `var`, `lazy val`
    *       can return a `class`/`object` definition that is owned by the
    *       package or package object.
    *   - Can not return a `type`.
    *   - Annotated top-level `class`/`object` can not return top-level `def`,
    *     `val`, `var`, `lazy val`.
    ...
    *     (`Symbol.freshName`) to avoid collisions.
    *   - New top-level definitions should use a fresh name (`Symbol.freshName`)
    *     that includes the name of the annotated member as a prefix to avoid
    *     collisions of definitions added in other files.
    *
    * **IMPORTANT**: When developing and testing a macro annotation, you must
    * enable `-Xcheck-macros` and `-Ycheck:all`.
    *
    * #### Example 1 This example shows how to modify a `def` and add a `val`
    ...
    *               val cacheRefExpr =
    *                 Ref(cacheSymbol).asExprOf[mutable.Map[t, u]]
    ...
    * ```
    *
    * #### Example 2 This example shows how to modify a `class` using a macro
    * annotation. It shows how to override inherited members and add new ones.
    * ```scala
    ...
    *         val cls = tree.symbol
    *
    *         val constructorParameters = ctr.paramss.collect {
    *           case clause: TermParamClause => clause
    *         }
    *         if constructorParameters.size != 1 || constructorParameters.head.params.isEmpty
    *         then
    *           report.errorAndAbort(
    *             "@equals class must have a single argument list with at least one argument",
    *             ctr.pos
    *           )
    *         def checkNotOverridden(sym: Symbol): Unit =
    *           if sym.overridingSymbol(cls).exists then
    *             report.error(
    *               s"Cannot override ${sym.name} in a @equals class"
    *             )
    *
    ...
    *         val equalsOverrideSym = Symbol.newMethod(
    *           cls,
    *           "equals",
    *           equalsSym.info,
    *           Flags.Override,
    ...
    *         val equalsOverrideDef =
    *           DefDef(equalsOverrideSym, equalsOverrideDefBody)
    *
    *         val hashSym = Symbol.newVal(
    *           cls,
    *           Symbol.freshName("hash"),
    *           TypeRepr.of[Int],
    *           Flags.Private | Flags.Lazy,
    *           Symbol.noSymbol
    *         )
    *         val hashVal = ValDef(
    *           hashSym,
    *           Some(
    *             hashCodeExpr(className, fields)(using hashSym.asQuotes).asTerm
    *           )
    *         )
    *
    *         val hashCodeSym =
    *           Symbol.requiredMethod("java.lang.Object.hashCode")
    *         checkNotOverridden(hashCodeSym)
    *         val hashCodeOverrideSym = Symbol.newMethod(
    *           cls,
    *           "hashCode",
    *           hashCodeSym.info,
    *           Flags.Override,
    *           Symbol.noSymbol
    *         )
    *         val hashCodeOverrideDef =
    *           DefDef(hashCodeOverrideSym, _ => Some(Ref(hashSym)))
    *
    *         val newBody =
    *           equalsOverrideDef :: hashVal :: hashCodeOverrideDef :: body
    ...
    *   end transform
    *
    *   private def equalsExpr[T: Type](
    *       that: Expr[Any],
    *       thisFields: List[Expr[Any]]
    ...
    *     }
    *
    *   private def hashCodeExpr(
    *       className: String,
    *       thisFields: List[Expr[Any]]
    *   )(using Quotes): Expr[Int] =
    *     '{
    *       var acc: Int = ${
    *         Expr(scala.runtime.Statics.mix(-889275714, className.hashCode))
    *       }
    *       ${
    *         Expr.block(
    *           thisFields
    *             .map {
    *               case '{ $field: Boolean } =>
    *                 '{ if $field then 1231 else 1237 }
    *               case '{ $field: Byte }  => '{ $field.toInt }
    *               case '{ $field: Char }  => '{ $field.toInt }
    *               case '{ $field: Short } => '{ $field.toInt }
    *               case '{ $field: Int }   => field
    *               case '{ $field: Long } =>
    *                 '{ scala.runtime.Statics.longHash($field) }
    *               case '{ $field: Double } =>
    *                 '{ scala.runtime.Statics.doubleHash($field) }
    *               case '{ $field: Float } =>
    *                 '{ scala.runtime.Statics.floatHash($field) }
    *               case '{ $field: Null } => '{ 0 }
    *               case '{ $field: Unit } => '{ 0 }
    *               case field => '{ scala.runtime.Statics.anyHash($field) }
    *             }
    *             .map(hash =>
    *               '{ acc = scala.runtime.Statics.mix(acc, $hash) }
    *             ),
    *           '{
    *             scala.runtime.Statics
    *               .finalizeHash(acc, ${ Expr(thisFields.size) })
    *           }
    *         )
    *       }
    *     }
    ...
    * ```
    * and the macro will modify the class definition to generate the following
    * code
    * ```scala
    ...
    *     var acc =
    *       515782504 // scala.runtime.Statics.mix(-889275714, "User".hashCode)
    *     acc =
    *       scala.runtime.Statics.mix(acc, scala.runtime.Statics.anyHash(name))
