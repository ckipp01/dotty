...
class TupleOps:
  ...
  def setup(): Unit =
    tuple1 = Tuple()
    for i <- 1 until 15 do tuple1 = s"elem$i" *: tuple1

    tuple2 = Tuple()
    for i <- 1 until 10 do tuple2 = s"elem$i" *: tuple2
  ...
  def tupleMerge(tuple1: Tuple, tuple2: Tuple): Tuple = (tuple1, tuple2) match
    ...
    case (Tuple(), _) => tuple2
    case (x *: xs, y *: ys) =>
      if x.asInstanceOf[Int] <= y.asInstanceOf[Int] then
        x *: tupleMerge(xs, tuple2)
