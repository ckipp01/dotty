...
object Bench:
  def main(args: Array[String]): Unit =
    ...
    def getIntArg(i: Int, default: Int): Int =
      if intArgs.length > i then intArgs(i).toInt else default
    ...
    if args1.length > 1 && args1(1) != "--" then
      for (param, values) <- paramsFromFile("inputs/" ++ args1(1)) do
        builder = builder.param(param, values*)

    if args1.length > 2 then
  ...
  def printUsage(): Unit =
    ...
    println()
    println(
      "dotty-bench-run/jmh:run [<warmup>] [<iterations>] [<forks>] <regexp> [<input>|--] [<output>]"
    )
    println()
    ...
    println(
      "input: input vector file. each line should have format \'<paramName>: <comma-separated-values>\'"
    )
