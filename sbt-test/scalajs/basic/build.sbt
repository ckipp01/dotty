...
testIRPositions := {
  ...
  val classDefFuture =
    ...
    Future
      .traverse(ir.data) { irFile =>
        val ir = IRFileImpl.fromIRFile(irFile)
        ir.entryPointsInfo
          .map { i =>
            if i.className == classNameToTest then Success(Some(ir))
            else Success(None)
          }
      ...
      .flatMap { irs =>
        irs
          ...
          .getOrElse {
            val t = new MessageOnlyException(
              s"class ${classNameToTest.nameString} not found on classpath"
            )
  ...
  def testPos(pos: Position, expectedLine: Int, expectedColumn: Int): Unit =
    if !pos.source.getPath.endsWith(
        "/Main.scala"
      ) || pos.line != expectedLine || pos.column != expectedColumn
    then
      throw new MessageOnlyException(
        s"Expected Main.scala@$expectedLine:$expectedColumn but got $pos"
      )
