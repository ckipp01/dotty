...
import xsbti.compile.analysis.{Compilation as XCompilation}

previousCompile in Compile := {
  val previous = (previousCompile in Compile).value
  if !CompileState.isNew then
    val res =
      PreviousResult.of(none[CompileAnalysis].asJava, none[MiniSetup].asJava)
...
TaskKey[Unit]("checkCompilations") := {
  val analysis = (compile in Compile).value match
    case a: Analysis => a
  val srcDir = (scalaSource in Compile).value
  def findFile(className: String): VirtualFileRef =
    analysis.relations.definesClass(className).head
  val allCompilations = analysis.compilations.allCompilations
  val recompiledFiles: Seq[Set[VirtualFileRef]] = allCompilations map {
    c: XCompilation =>
      val recompiledFiles = analysis.apis.internal.collect {
        case (cn, api) if api.compilationTimestamp == c.getStartTime =>
          findFile(cn)
      }
  ...
  def recompiledFilesInIteration(iteration: Int, fileNames: Set[String]) =
    assert(
      recompiledFiles(iteration).map(_.name) == fileNames,
  ...
  assert(
    allCompilations.size == 2,
    s"All compilations is ${allCompilations.size}"
  )
