...
class DivideZero extends PluginPhase with StandardPlugin:
  ...
  private def isNumericDivide(sym: Symbol)(implicit ctx: Context): Boolean =
    ...
    test("scala.Int") || test("scala.Long") || test("scala.Short") || test(
      "scala.Float"
    ) || test("scala.Double")

  override def transformApply(tree: tpd.Apply)(implicit
      ctx: Context
  ): tpd.Tree = tree match
    case tpd.Apply(fun, tpd.Literal(Constants.Constant(v)) :: Nil)
        if isNumericDivide(fun.symbol) && v == 0 =>
