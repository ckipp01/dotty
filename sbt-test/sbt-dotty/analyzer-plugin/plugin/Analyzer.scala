...
class InitPlugin extends StandardPlugin:
  ...
  override val description: String =
    "checks that under -Yretain-trees we may get tree for all symbols"
...
class InitChecker extends PluginPhase:
  ...
  private def checkDef(tree: Tree)(implicit ctx: Context): Tree =
    if tree.symbol.defTree.isEmpty then
  ...
  private def checkable(sym: Symbol)(implicit ctx: Context): Boolean =
    sym.exists && !sym.isOneOf(Flags.Package) && !sym.isOneOf(Flags.Param) &&
      (sym.isClass || !sym.isOneOf(
        Flags.Case,
        butNot = Flags.Enum
      )) // pattern-bound symbols
  ...
  private def checkRef(tree: Tree)(implicit ctx: Context): Tree =
    if !checkable(tree.symbol) then tree
      ...
      if enclosingPkg == helloPkgSym then // source code
        checkDef(tree)
        report.warning("tree: " + tree.symbol.defTree.show)
      else if enclosingPkg == libPkgSym then // tasty from library
        checkDef(tree)
        ...
        report.warning(
          s"${tree.symbol} is neither in lib nor hello, owner = $enclosingPkg",
          tree.sourcePos
        )
      tree
