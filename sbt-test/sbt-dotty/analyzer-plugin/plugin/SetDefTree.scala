...
/** Set the `defTree` property of symbols for compile plugins that perform
  * "whole-program" analysis.
  *
  * All plugins that depend on `symbol.defTree` should sit between the phase
  * `SetDefTree` and `SetDefTreeOff`.
  */
