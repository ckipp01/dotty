...
package lib {
  object NewMirrors:
    val mFoo =
      summon[Mirror.Of[Foo]] // we can access the constructor of Foo here.
...
package app {
  object Main:
    ...
    def testFoo(): Unit =
      ...
      val oldMirrorFooObj: Mirror.ProductOf[lib.Foo.type] =
        lib.OldMirrors.mFooObj

      assert(
        oldMirrorFoo eq oldMirrorFooObj
      ) // - not good as oldMirrorFoo is really the mirror for `Foo.type`
      ...
      val newMirrorFooObj: Mirror.ProductOf[lib.Foo.type] =
        lib.NewMirrors.mFooObj

      assert(
        oldMirrorFooObj eq newMirrorFooObj
      ) // mirror for Foo.type has not changed.

      assert(newMirrorFoo ne lib.Foo) // anonymous mirror for Foo
      assert(
        newMirrorFoo.fromProduct(Tuple(23)).x == 23
      ) // mirror for Foo behaves as expected

    def testBar(): Unit =
      val oldMirrorBar: Mirror.ProductOf[lib.Bar] = lib.OldMirrors.SubBar.mBar
      val oldMirrorBarObj: Mirror.ProductOf[lib.Bar.type] =
        lib.OldMirrors.SubBar.mBarObj

      assert(
        oldMirrorBar eq oldMirrorBarObj
      ) // - not good as oldMirrorBar is really the mirror for `Bar.type`
      ...
      val newMirrorBarObj: Mirror.ProductOf[lib.Bar.type] =
        lib.NewMirrors.SubBar.mBarObj

      assert(
        oldMirrorBarObj eq newMirrorBarObj
      ) // mirror for Bar.type has not changed.

      assert(newMirrorBar ne lib.Bar) // anonymous mirror for Bar
      assert(
        newMirrorBar.fromProduct(Tuple(23)).x == 23
      ) // mirror for Bar behaves as expected
