...
abstract class SnippetsE2eTest(testName: String, flag: SCFlags)
    extends ScaladocTest(testName):

  import SnippetsE2eTest.*

  val source = Source.fromFile(
    s"${BuildInfo.test_testcasesSourceRoot}/tests/$testName.scala"
  )
  ...
  override def args = Scaladoc.Args(
    ...
    snippetCompiler =
      List(s"${BuildInfo.test_testcasesSourceRoot}/tests=${flag.flagName}")
  )

  override def withModule(op: DocContext ?=> Module => Unit) =
    given DocContext =
      DocContext(args, testContext.fresh.setReporter(new StoreReporter))
  ...
  end checkWrappedSnippet

  private def checkMessages(
      compilationMessages: Seq[SnippetCompilerMessage],
    ...
    def checkRelativeLines(
        msg: Message,
        cmsg: SnippetCompilerMessage
    ): Seq[String] =
      val pos = cmsg.position.get
      if !(pos.relativeLine == pos.srcPos.line - ws.outerLineOffset) then
        Seq(
    ...
    val mResult = compilationMessagesWithPos.flatMap { cmsg =>
      messages
        ...
        .fold(
          Seq(
            s"Unexpected compilation message: ${cmsg.message} at relative line: ${cmsg.position
                .fold(-1)(_.relativeLine)}"
          )
        )(_ => Seq())
    }
    ...
    val result = mResult ++ messages.flatMap { msg =>
      compilationMessagesWithPos
        ...
        .fold(
          Seq(
            s"Expected ${msg.level.text} message at ${msg.offset.line}:${msg.offset.column}."
          )
        ) { resp =>
    ...
    if !result.isEmpty then
      ...
      val foundMessages = compilationMessages
        .map(m =>
          s"${m.level} at ${m.position.get.srcPos.line}:${m.position.get.srcPos.column}"
        )
        .mkString("\n")
      throw AssertionError(
        Seq("Errors:", errors, "Found:", foundMessages)
          .mkString("\n", "\n", "\n")
      )
  end checkMessages
  ...
  def moduleTestingFunc: DocContext ?=> Module => Unit = (m: Module) =>
    ...
    assertTrue(
      report(s"Expected $snippetsCount snippets but found ${snippets.size}"),
      snippets.size == snippetsCount
    )
    ...
    snippets.foreach { snippet =>
      val configStrs = (snippet.getPrevious() match
        ...
        case _ =>
          throw AssertionError(
            s"Not found info for snippet ${snippet.codeBlock.getContentChars.toString}"
          )
      ).toList
...
object SnippetsE2eTest:
  ...
  object SnippetInfo:
    def apply(str: String): SnippetInfo = str match
      case snippetInfoRegex(ol, oc, il, ic) =>
        SnippetInfo(
  ...
  object Message:
    def apply(str: String): Message = str match
      case errorRegex(ln, cl) =>
        Message(MessageLevel.Error, Offset(ln.toInt, cl.toInt))
      case warningRegex(ln, cl) =>
        Message(MessageLevel.Warning, Offset(ln.toInt, cl.toInt))
