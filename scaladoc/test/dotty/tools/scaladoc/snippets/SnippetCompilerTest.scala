...
class SnippetCompilerTest:
  ...
  def wrapFn: String => WrappedSnippet = (str: String) =>
    WrappedSnippet(
      str,
      Some("test"),
      0,
  ...
  def runTest(str: String) = compiler.compile(
    wrapFn(str),
    SnippetCompilerArg(SCFlags.Compile),
  ...
  private def assertSuccessfulCompilation(res: SnippetCompilationResult): Unit =
    res match
      case r @ SnippetCompilationResult(_, isSuccessful, _, messages) =>
        assert(isSuccessful, r.messages.map(_.message).mkString("\n"))
  ...
  def assertSuccessfulCompilation(str: String): Unit =
    assertSuccessfulCompilation(runTest(str))

  def assertFailedCompilation(str: String): Unit = assertFailedCompilation(
    runTest(str)
  )

  def assertMessageLevelPresent(str: String, level: MessageLevel): Unit =
    assertMessageLevelPresent(runTest(str), level)

  def assertMessageLevelPresent(
      res: SnippetCompilationResult,
      level: MessageLevel
  ): Unit = res match
    case r @ SnippetCompilationResult(_, isSuccessful, _, messages) =>
      assertTrue(
