...
case class ReportedDiagnostics(
...
extension (c: CompilerContext)
  def reportedDiagnostics: ReportedDiagnostics =
    val t = c.reporter.asInstanceOf[TestReporter]
    ReportedDiagnostics(t.errors.result, t.warnings.result, t.infos.result)

def assertNoWarning(diag: ReportedDiagnostics) =
  assertEquals("Warnings should be empty", Nil, diag.warningMsgs)
def assertNoErrors(diag: ReportedDiagnostics) =
  assertEquals("Erros should be empty", Nil, diag.errorMsgs)
def assertNoInfos(diag: ReportedDiagnostics) =
  assertEquals("Infos should be empty", Nil, diag.infoMsgs)
...
def testArgs(files: Seq[File] = Nil, dest: File = new File("notUsed")) =
  Scaladoc.Args(
...
def testDocContext(files: Seq[File] = Nil) =
  DocContext(testArgs(files), testContext)

def tastyFiles(
    name: String,
    allowEmpty: Boolean = false,
    rootPck: String = "tests"
) =
  def listFilesSafe(dir: File) = Option(dir.listFiles).getOrElse {
    throw AssertionError(
      s"$dir not found. The test name is incorrect or scaladoc-testcases were not recompiled."
    )
  }
