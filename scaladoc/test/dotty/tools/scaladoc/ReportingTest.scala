...
class ReportingTest:
  ...
  private def checkReportedDiagnostics(
    ...
    try
      ...
      Scaladoc.run(newArgs(testArgs(tastyFiles("nested"), dest.toFile)))(using
        ctx
      )
  ...
  def errorsInCaseOfIncompletClasspath =
    ...
    try
      ...
      checkReportedDiagnostics(a =>
        a.copy(tastyFiles = notTasty.toFile +: a.tastyFiles)
      ) { diag =>
        assertMessagesAbout(diag.errorMsgs)(
          "File extension is not `tasty` or `jar`"
        )
      }
  ...
  def testErrorInCaseOfAssetShadowing =
    ...
    checkReportedDiagnostics(
      ...
      Seq("conflict", "api", "resource", "resources/tests/adoc.html").foreach(
        word =>
          Assert.assertTrue(
            s"Error message: $msg should contains $word",
            msg.contains(word)
          )
      )
    }
