...
class SiteGeneratationTest extends BaseHtmlTest:
  ...
  def checkFile(path: String)(
    ...
    withHtmlFile(path) { content =>
      content.assertTextsIn(
        ".project-name",
        projectName,
        projectName
      ) // There are two elements with project name: one for desktop, one for mobile
  ...
  def testDocPages()(using ProjectContext) =
    checkFile("docs/Adoc.html")(
      title = "Adoc",
      header = "Header in Adoc",
      parents = Seq(projectName)
    )
    checkFile("docs/dir/index.html")(
      title = "A directory",
      header = "A directory",
      parents = Seq(projectName)
    )
    checkFile("docs/dir/nested.html")(
      ...
      header = "Nested in a directory",
      parents = Seq(projectName, "A directory")
    )

  def testDocIndexPage()(using ProjectContext) =
    checkFile("docs/index.html")(
      title = projectName,
      header = s"$projectName in header"
    )

  def testApiPages(
      mainTitle: String = projectName,
      parents: Seq[String] = Seq.empty,
      hasToplevelIndexIndex: Boolean = false
  )(using ProjectContext) =
  ...
  @Test
  def noGlobalIndexTest() =
    withGeneratedSite(testDocPath.resolve("noGlobalIndex")) {
      testDocPages()
      testDocIndexPage()
