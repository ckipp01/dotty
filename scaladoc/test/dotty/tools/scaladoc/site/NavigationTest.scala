...
class NavigationTest extends BaseHtmlTest:
  ...
  case class NavMenuTestEntry(
      name: String,
      link: String,
      nested: Seq[NavMenuTestEntry]
  )

  def testNavMenu(page: String, topLevel: Seq[NavMenuTestEntry])(using
      ProjectContext
  ): Unit =
    ...
    val apiNav = Seq(
      NavMenuTestEntry(
        ...
        Seq(
          NavMenuTestEntry(
            "some.other",
            "site/some/other.html",
            Seq(
              NavMenuTestEntry(
                "SomeOtherPackage",
                "site/some/other/SomeOtherPackage.html",
                Nil
              )
            )
          ),
          ...
          NavMenuTestEntry(
            "OtherPackageLink",
            "site/OtherPackageLink.html",
            Nil
          ),
          NavMenuTestEntry(
            "OtherPackageLinkWiki",
            "site/OtherPackageLinkWiki.html",
            Nil
          ),
          NavMenuTestEntry("SamePackageLink", "site/SamePackageLink.html", Nil),
          NavMenuTestEntry(
            "SamePackageLinkWiki",
            "site/SamePackageLinkWiki.html",
            Nil
          ),
