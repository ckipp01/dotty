...
def getScalaLibraryPath: String =
  val classpath: List[String] = System
    .getProperty("java.class.path")
    .split(java.io.File.pathSeparatorChar)
    .toList
  classpath
    .find(_.contains("scala-library-2"))
    .getOrElse(
      "foobarbazz"
    ) // If we don't find the scala 2 library, the test will fail

class Scaladoc2LegacyExternalLocationProviderIntegrationTest
    extends LegacyExternalLocationProviderIntegrationTest(
      "externalScaladoc2",
...
end ExternalLocationProviderIntegrationTest
