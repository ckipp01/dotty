...
abstract class SignatureTest(
  ...
  def runTest =
    org.junit.Assume
      .assumeTrue("Running on Windows", java.io.File.separatorChar == '/');
    afterRendering {
      ...
      val allSignaturesFromSources = sources
        .map { file =>
          Source.fromFile(
            s"${BuildInfo.test_testcasesSourceRoot}/tests/$file.scala"
          )
        }
        .flatMap(signaturesFromSources(_, signatureKinds))
        .toList
      val expectedFromSources: Map[String, List[String]] =
        allSignaturesFromSources
          .collect { case e: Expected => e }
          .groupMap(_.name)(_.signature)
      val unexpectedFromSources: Set[String] =
        allSignaturesFromSources.collect { case Unexpected(name) => name }.toSet

      val actualSignatures: Map[String, Seq[String]] =
        signaturesFromDocumentation()
          .flatMap { signature =>
      ...
      val unexpected =
        unexpectedFromSources.flatMap(actualSignatures.get).flatten
      val expectedButNotFound = expectedFromSources.flatMap { case (k, v) =>
        findMissingSignatures(v, actualSignatures.getOrElse(k, Nil))
      }

      val missingReport = Option.when(
        !ignoreMissingSignatures && !expectedButNotFound.isEmpty
      ...
      if !reports.isEmpty then
        ...
        """.stripMargin
        val errorMessage = (reports ++ Some(allSignaturesMessage)).mkString(
          start = "\n",
          sep = "\n\n",
          end = "\n"
        )
  ...
  private val identifierRegex =
    raw"^\s*(`.*`|(?:\w+)(?:_[^\[\(\s]+)|\w+|[^\[\(\s]+)".r

  private def findMissingSignatures(
      expected: Seq[String],
      actual: Seq[String]
  ): Set[String] =
  ...
  private def findName(signature: String, kinds: Seq[String]): Option[String] =
    for
      kindMatch <- kinds
        .flatMap(k => s"\\b$k\\b".r.findFirstMatchIn(signature))
        .headOption
      kind <- Option(kindMatch.group(0)) // to filter out nulls
      afterKind <- Option(kindMatch.after(0)) // to filter out nulls
      name <-
        if kind.contains("extension") then
          Some(
            signature
          ) // The name of an extension will always be the signature itself
        else identifierRegex.findFirstMatchIn(afterKind).map(_.group(1))
    yield name

  private def signaturesFromSources(
      source: Source,
      kinds: Seq[String]
  ): Seq[SignatureRes] =
    source.getLines
      ...
      .toSeq
      .flatMap {
        case unexpectedRegex(signature) =>
          findName(signature, kinds).map(Unexpected(_))
        ...
        case signature =>
          findName(signature, kinds).map(
            Expected(
              _,
              commentRegex
                .replaceAllIn(signature, "")
                .compactWhitespaces
                .reverse
                .dropWhile(List('{', ':').contains(_))
                .reverse
            )
          )
      }
  ...
  private def signaturesFromDocumentation()(using DocContext): Seq[String] =
    ...
    def processFile(path: Path): Unit = if filterFunc(path) then
      ...
      val content = document.select(".documentableElement").forEach { elem =>
        val annotations =
          elem.select(".annotations").eachText.asScala.mkString("")
        val other =
          elem.select(".header .other-modifiers").eachText.asScala.mkString("")
        val kind = elem.select(".header .kind").eachText.asScala.mkString("")
        val name =
          elem.select(".header .documentableName").eachText.asScala.mkString("")
        val signature =
          elem.select(".header .signature").eachText.asScala.mkString("")
        val sigPrefix = elem.select(".header .signature").textNodes match
          case list
              if list.size > 0 && list.get(0).getWholeText().startsWith(" ") =>
            " "
          case _ => ""
    ...
    signatures.result
  end signaturesFromDocumentation
end SignatureTest

object SignatureTest:
  val classlikeKinds =
    Seq("class", "object", "trait", "enum") // TODO add docs for packages
