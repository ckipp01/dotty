...
class BaseHtmlTest:
  ...
  def withGeneratedDoc(
    ...
    val dest =
      customArgs.fold(Files.createTempDirectory("test-doc").toFile)(_.output)
    try
      val args = customArgs.getOrElse(
        Scaladoc.Args(
          name = projectName,
  ...
  class DocumentContext(d: Document, path: Path):
    ...
    def assertTextsIn(selector: String, expected: String*) =
      assertFalse(
        niceMsg(s"Selector not found for '$selector'"),
        d.select(selector).isEmpty
      )
      val found = d.select(selector).eachText.asScala
      assertEquals(
        niceMsg(s"Content does not match for '$selector'"),
        expected.toList,
        found.toList
      )
    ...
    def assertAttr(selector: String, attr: String, expected: String*) =
      assertFalse(
        niceMsg(s"Selector '$selector' not found"),
        d.select(selector).isEmpty
      )
      val found = d.select(selector).eachAttr(attr).asScala
      assertEquals(
        niceMsg(s"Attribute $attr does not match for '$selector'"),
        expected.toList,
        found.toList
      )
  ...
  def withHtmlFile(pathStr: String)(op: DocumentContext => Unit)(using
      ProjectContext
  ) =
