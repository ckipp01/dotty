...
def jsonString(s: String): JSON =
  ...
  @tailrec def firstToBeEncoded(ix: Int = 0): Int =
    if ix == s.length then -1
    else if requiresEncoding(s.charAt(ix)) then ix
      ...
      @tailrec def append(ix: Int): Unit =
        if ix < s.length then
          s.charAt(ix) match
            ...
            case '\t'                      => sb.append("\\t")
            case x if x <= 0xf =>
              sb.append("\\u000").append(Integer.toHexString(x))
            case x if x <= 0xff =>
              sb.append("\\u00").append(Integer.toHexString(x))
            case x if x <= 0xfff =>
              sb.append("\\u0").append(Integer.toHexString(x))
