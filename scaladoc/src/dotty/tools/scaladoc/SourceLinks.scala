...
trait SourceLink:
  ...
  def render(
      ...
      operation: String,
      line: Option[Int],
      optionalRevision: Option[String]
  ): String
...
case class TemplateSourceLink(val urlTemplate: String) extends SourceLink:
  ...
  override def render(
    ...
    val mapping = Map(
      ...
      "\\{\\{ ext \\}\\}".r -> Some(pathString)
        .filter(_.lastIndexOf(".") != -1)
        .fold("")(p => p.substring(p.lastIndexOf("."))),
      "\\{\\{ path_no_ext \\}\\}".r -> Some(pathString)
        .filter(_.lastIndexOf(".") != -1)
        .fold(pathString)(p => p.substring(0, p.lastIndexOf("."))),
      "\\{\\{ name \\}\\}".r -> memberName
    )
    mapping.foldLeft(urlTemplate) { case (sourceLink, (regex, value)) =>
      regex.replaceAllIn(sourceLink, Regex.quoteReplacement(value))
    }
  override def repoSummary = UnknownRepoSummary

sealed class RepoSummary
case class DefinedRepoSummary(origin: String, org: String, repo: String)
    extends RepoSummary
case object UnknownRepoSummary
    extends RepoSummary // we cannot easily get that information from template source links

case class WebBasedSourceLink(
  ...
  override def render(
      ...
      operation: String,
      line: Option[Int],
      optionalRevision: Option[String] = None
  ): String =
...
class SourceLinkParser(revision: Option[String]) extends ArgParser[SourceLink]:
  ...
  val ScalaDocPatten =
    raw"€\{(TPL_NAME|TPL_OWNER|FILE_PATH|FILE_EXT|FILE_LINE|FILE_PATH_EXT)\}".r
  ...
  def gitlabPrefix(org: String, repo: String) =
    s"https://gitlab.com/$org/$repo/-"
  ...
  def parse(string: String): Either[String, SourceLink] =
    val res = string match
      case scaladocSetting
          if ScalaDocPatten.findFirstIn(scaladocSetting).nonEmpty =>
        val all = ScalaDocPatten.findAllIn(scaladocSetting)
        val (supported, unsupported) =
          all.partition(SupportedScalaDocPatternReplacements.contains)
        if unsupported.nonEmpty then
          Left(
            s"Unsupported patterns from scaladoc format are used: ${unsupported.mkString(" ")}"
          )
        else
          Right(
            TemplateSourceLink(
              supported.foldLeft(string)((template, pattern) =>
                template.replace(
                  pattern,
      ...
      case KnownProvider(name, organization, repo, rawRevision, rawSubPath) =>
        ...
        name match
          case "github" =>
            withRevision(rev =>
              WebBasedSourceLink(
                DefinedRepoSummary("github", organization, repo),
                githubPrefix(organization, repo),
                rev,
                subPath
              )
            )
          case "gitlab" =>
            withRevision(rev =>
              WebBasedSourceLink(
                DefinedRepoSummary("gitlab", organization, repo),
                gitlabPrefix(organization, repo),
                rev,
                subPath
              )
            )
      ...
      case BrokenKnownProvider("gitlab" | "github") =>
        Left(
          s"Does not match known provider syntax: `<name>://organization/repository`"
        )
      case other =>
...
class SourceLinks(private val sourceLinks: PathBased[SourceLink]):
  def pathTo(
    ...
    sourceLinks
      .get(rawPath)
      .map(res =>
        res.elem.render(memberName, res.path, operation, line, optionalRevision)
      )

  def pathTo(member: Member): Option[String] =
    member.sources.flatMap(s =>
      pathTo(s.path, member.name, Option(s.lineNumber).map(_ + 1))
    )
  ...
  def fullPath(path: Path): Option[Path] =
    sourceLinks.get(path).map { case PathBased.Result(path, elem) =>
      elem match
