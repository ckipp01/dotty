...
// (val classpath: String, val bootclasspath: String, val tastyFiles: Seq[File], isScalajs: Boolean, useJavaCp: Boolean):
  ...
  private val fullClasspath = List(
    args.tastyFiles
      ...
      .flatMap(t =>
        try TastyFileUtil.getClassPath(t)
        catch case e: AssertionError => Seq()
      )
      .distinct
      .mkString(sep),
    args.classpath
  ...
  private val snippetCompilerSettings: Seq[SnippetCompilerSetting[?]] =
    cctx.settings
      .userSetSettings(cctx.settingsState)
  ...
  private val compiler: SnippetCompiler =
    SnippetCompiler(snippetCompilerSettings = snippetCompilerSettings)
...
object SnippetChecker:
  type LineOffset = Int
  type SnippetCheckingFunc =
    (String, LineOffset, Option[SCFlags]) => Option[SnippetCompilationResult]
