...
class InheritanceInformationTransformer(using DocContext)
  ...
  override def apply(original: Module): Module =
    ...
    original.updateMembers { m =>
      val edges =
        getEdges(m.asLink.copy(kind = bareClasslikeKind(m.kind)), subtypes)
  ...
  private def getEdges(
      ltt: LinkToType,
      subtypes: Map[DRI, Seq[LinkToType]]
  ): Seq[(LinkToType, LinkToType)] =
  ...
  private def getSupertypes(c: Member): Seq[(DRI, LinkToType)] =
    val selfMapping =
      if !c.kind.isInstanceOf[Classlike] && !c.kind.isInstanceOf[Kind.EnumCase]
      then Nil
      else c.directParents.map(p => p.dri -> c.asLink)
    c.members.flatMap(getSupertypes) ++ selfMapping
end InheritanceInformationTransformer
