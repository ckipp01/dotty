...
import dotty.tools.scaladoc.util.*
...
object Inkuire:
    ...
    e.result.typ match
      case t: Type if t.name.name == s"Function${t.params.size - 1}" =>
        curry(
          e.copy(
            arguments =
              e.arguments ++ t.params.init.map(_.typ).map(Contravariance(_)),
  ...
  def functions: Seq[ExternalSignature] = Inkuire.db.functions.flatMap { func =>
    val fromConversions = Inkuire.implicitConversions
      .filter { ic =>
        func.signature.receiver.nonEmpty && matchingICTypes(
          ic._2,
          func.signature.receiver.get.typ
        )
      }
      .map {
        case (Some(from), to) =>
  ...
  def matchingICTypes(a: TypeLike, b: TypeLike): Boolean = (a, b) match
    case (a: Type, b: Type)
        if a.params.size == 0 && b.params.size == 0 && a.itid == b.itid =>
      true
    case (a: Type, b: Type)
        if a.params.size == 1 && b.params.size == 1 && a.itid == b.itid =>
      a.params.head.typ.isInstanceOf[Type] && a.params.head.typ
        .asInstanceOf[Type]
        .isVariable &&
      b.params.head.typ
        .isInstanceOf[Type] && b.params.head.typ.asInstanceOf[Type].isVariable
    case _ => false
      ...
      case (a: Type, b: Type)
          if a.params.size == 1 && b.params.size == 1 && a.itid == b.itid
            && a.params.head.typ.isInstanceOf[Type] && a.params.head.typ
              .asInstanceOf[Type]
              .isVariable &&
            b.params.head.typ.isInstanceOf[Type] && b.params.head.typ
              .asInstanceOf[Type]
              .isVariable =>
        if from.isInstanceOf[Type] && from
            .asInstanceOf[Type]
            .params
            .size == 1 && from
            .asInstanceOf[Type]
        ...
        else if from.isInstanceOf[Type] && from.asInstanceOf[Type].isVariable
        then a.params.head.typ.asInstanceOf[Type]
        else from
      case _ => old
  ...
  case class Signature(
    ...
    def typesWithVariances: Seq[Variance] =
      receiver.toSeq ++ arguments ++ Seq(result)
  ...
  object Signature:
    def apply(
      ...
      Signature(
        receiver.map(Contravariance(_)),
        arguments.map(Contravariance(_)),
        Covariance(result),
        context
      )
  ...
  object EngineModelSerializers:
    def serialize(db: InkuireDb): JSON =
      jsonObject(
        ...
        (
          "implicitConversions",
          jsonList(db.implicitConversions.map(serializeConversion))
        ),
    ...
    private def serializeConstraints(
      ...
      jsonObject(
        (
          constraints.toList.map { case (name, vs) =>
            (name, jsonList(vs.map(serialize)))
          }
        )*
      )
  end EngineModelSerializers
end Inkuire
