...
class HtmlRenderer(rootPackage: Member, members: Map[DRI, Member])(using
    ctx: DocContext
      ...
      case ResolvedTemplate(loadedTemplate, _) =>
        ...
        ctx.sourceLinks.repoSummary(path) match
          case Some(DefinedRepoSummary("github", org, repo)) =>
            ctx.sourceLinks.fullPath(relativePath(path)).fold(Nil) {
              contributorsFilename =>
                List[AppliedAttr](
                  Attr(
                    "data-githubContributorsUrl"
                  ) := s"https://api.github.com/repos/$org/$repo",
                  Attr(
                    "data-githubContributorsFilename"
                  ) := s"$contributorsFilename"
                )
            }
          case _ => Nil
      case _ => Nil
  ...
  private def renderResources(): Seq[String] =
        ...
        resourceFile.listFiles.toSeq.map(_.toPath).flatMap { file =>
          Using(Files.walk(file)) { stream =>
            stream
              ...
              .map(from =>
                Resource
                  .File(resourceFile.toPath.relativize(from).toString, from)
              )
          }.fold(
            { t =>
    ...
    val resources =
      staticSiteResources ++ allResources(allPages) ++ onlyRenderedResources
  ...
  def mkHead(page: Page): Seq[TagArg] =
    val resources = page.content match
      case t: ResolvedTemplate =>
        t.resolved.resources ++ (if t.hasFrame then
                                   commonResourcesPaths ++ staticSiteOnlyResourcesPaths
                                 else Nil)
      case _ =>
    ...
    val earlyResources = page.content match
      case t: ResolvedTemplate =>
        if t.hasFrame then earlyCommonResourcePaths else Nil
      case _ => earlyCommonResourcePaths

    Seq(
      meta(charset := "utf-8"),
      meta(
        util.HTML.name := "viewport",
      ...
      ctx.args.versionsDictionaryUrl match
        case Some(url) =>
          script(raw(s"""var versionsDictionaryUrl = "$url";"""))
        case None => ""
    )
  end mkHead

  private def buildNavigation(
      pageLink: Link
  ): (Option[(Boolean, Seq[AppliedTag])], Option[(Boolean, Seq[AppliedTag])]) =
    def navigationIcon(member: Member) = member match
      case m if m.needsOwnPage =>
        Seq(span(cls := s"micon ${member.kind.name.take(2)}"))
      case _ => Nil
    ...
    def renderNested(
      ...
      def linkHtml(expanded: Boolean = false, withArrow: Boolean = false) =
        ...
        Seq(
          span(cls := s"nh " + attrs.mkString(" "))(
            if withArrow then
              Seq(button(cls := s"ar icon-button ${
                  if isSelected || expanded then "expanded" else ""
                }"))
            else Nil,
            a(
              href := (if isSelected then "#"
                       else pathToPage(pageLink.dri, nav.link.dri))
            )(icon, span(name))
          )
        )
      ...
      nav.children.filterNot(_.hidden) match
        case Nil =>
          isSelected -> div(
            cls := s"ni n$nestLevel ${if isSelected then "expanded" else ""}"
          )(linkHtml())
        case children =>
          val nested = children.map(renderNested(_, nestLevel + 1, newPrefix))
          val expanded = nested.exists(_._1)
          val attr =
            if expanded || isSelected then
              Seq(cls := s"ni n$nestLevel expanded")
            else Seq(cls := s"ni n$nestLevel")
    ...
    val isRootApiPageSelected =
      rootApiPage.fold(false)(_.link.dri == pageLink.dri)
    val isDocsApiPageSelected =
      rootDocsPage.fold(false)(_.link.dri == pageLink.dri)
    val apiNav = rootApiPage.map { p =>
      p.children.filterNot(_.hidden).map(renderNested(_, 0)) match
        case entries =>
          (entries.exists(_._1) || isRootApiPageSelected, entries.map(_._2))
    }
    val docsNav = rootDocsPage.map { p =>
      p.children.filterNot(_.hidden).map(renderNested(_, 0)) match
        case entries =>
          (entries.exists(_._1) || isDocsApiPageSelected, entries.map(_._2))
    }
  ...
  private def renderTableOfContents(toc: Seq[TocEntry]): Option[AppliedTag] =
    def renderTocRec(level: Int, rest: Seq[TocEntry]): Seq[AppliedTag] =
      rest match
        ...
        case head :: tail if head.level == level =>
          ...
          val nestedList =
            if nested.nonEmpty then Seq(ul(renderTocRec(level + 1, nested)))
            else Nil
          li(a(href := head.anchor)(head.content), nestedList) +: renderTocRec(
            level,
            rest
          )
    ...
    if toc.nonEmpty then
      ...
      Some(
        nav(cls := "toc-nav")(
          ul(cls := "toc-list")(renderTocRec(minLevel, toc))
        )
      )
    else None
  ...
  private def mkFrame(
    ...
    val projectLogoElem =
      projectLogo.flatMap {
        case Resource.File(path, _) =>
          Some(
            span(id := "project-logo", cls := "project-logo")(
              img(src := resolveRoot(link.dri, path))
            )
          )
        case _ => None
      }
    ...
    val darkProjectLogoElem =
      darkProjectLogo.orElse(projectLogo).flatMap {
        case Resource.File(path, _) =>
          Some(
            span(id := "dark-project-logo", cls := "project-logo")(
              img(src := resolveRoot(link.dri, path))
            )
          )
        case _ => None
    ...
    val (apiNavOpt, docsNavOpt): (
        Option[(Boolean, Seq[AppliedTag])],
        Option[(Boolean, Seq[AppliedTag])]
    ...
    div(id := "")(
      div(id := "header", cls := "body-small")(
        div(cls := "header-container-left")(
          ...
          span(
            onclick := "dropdownHandler(event)",
            cls := "text-button with-arrow",
            id := "dropdown-trigger"
          )(
            a(
              args.projectVersion
                .map(v => div(cls := "projectVersion")(v))
                .toSeq
            )
          ),
      ...
      div(id := "mobile-menu")(
        ...
        div(cls := "mobile-menu-container body-medium")(
          input(
            id := "mobile-scaladoc-searchbar-input",
      ...
      div(id := "leftColumn", cls := "body-small")(
        Seq(
              ...
              case Some(isDocsActive, docsNav) =>
                Seq(
                  a(
                    id := "docs-nav-button",
                    cls := s"switcher h100 ${
                        if isDocsActive then "selected" else ""
                      }",
                    href := pathToPage(link.dri, rootDocsPage.get.link.dri)
                  )("Docs")
                )
              case _ => Nil
            ,
              ...
              case Some(isApiActive, apiNav) =>
                Seq(
                  a(
                    id := "api-nav-button",
                    cls := s"switcher h100 ${
                        if isApiActive then "selected" else ""
                      }",
                    href := pathToPage(link.dri, rootApiPage.get.link.dri)
                  )("API")
                )
              case _ => Nil
          ),
          apiNavOpt
            ...
            .orElse(
              docsNavOpt.map(docsNav =>
                nav(id := "docs-nav", cls := s"side-menu")(docsNav._2)
              )
            )
            .get
        )
