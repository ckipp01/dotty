...
class MemberRenderer(signatureRenderer: SignatureRenderer)(using DocContext)
    extends DocRender(signatureRenderer):
  import signatureRenderer.*

  val signatureProvider: ScalaSignatureProvider = ScalaSignatureProvider()

  def doc(m: Member): Seq[AppliedTag] =
    m.docs.fold(Nil)(d => Seq(renderDocPart(d.body)))

  def tableRow(name: String, content: TagArg*) =
    Seq(dt(cls := "body-small")(name), dd(cls := "body-medium")(content*))
  ...
  def defintionClasses(m: Member) = m.origin match
    case Origin.Overrides(defs) =>
      ...
      val headNode = m.inheritedFrom.map(form =>
        signatureRenderer.renderLink(form.name, form.dri)
      )
  ...
  def inheritedFrom(m: Member) = m.inheritedFrom match
    case Some(InheritedFrom(name, dri, isSourceSuperclassHidden)) =>
      val hiddenNameSuffix =
        if isSourceSuperclassHidden then " (hidden)" else ""
      tableRow(
        "Inherited from:",
        signatureRenderer.renderLink(name + hiddenNameSuffix, dri)
      )
    case _ => Nil

  def flattenedDocPart(on: SortedMap[String, DocPart]): Seq[AppliedTag] =
    on.flatMap { case (name, value) =>
      tableRow(name, renderDocPart(value))
    }.toSeq

  def docAttributes(m: Member): Seq[AppliedTag] =

    def list(name: String, on: List[DocPart]): Seq[AppliedTag] =
      if on.isEmpty then Nil
      else tableRow(name, div(on.map(e => div(renderDocPart(e)))))

    def opt(name: String, on: Option[DocPart]): Seq[AppliedTag] =
      if on.isEmpty then Nil else tableRow(name, renderDocPart(on.get))

    def authors(authors: List[DocPart]) =
      if summon[DocContext].args.includeAuthors then list("Authors:", authors)
      else Nil
  ...
  def companion(m: Member): Seq[AppliedTag] = m.companion.fold(Nil) {
    (kind, dri) =>
    ...
    summon[DocContext].sourceLinks.pathTo(m).fold(Nil) { link =>
      tableRow(
        ...
        a(href := link, target := "_blank")(
          m.sources.fold("(source)")(_.path.getFileName().toString())
        )
      )
    }
  ...
  def experimental(m: Member) =
    m.experimental.fold(Nil)(_ => tableRow("Experimental", Seq("true")))

  def typeParams(m: Member): Seq[AppliedTag] =
    m.docs.fold(Nil)(d => flattenedDocPart(d.typeParams))
  def valueParams(m: Member): Seq[AppliedTag] =
    m.docs.fold(Nil)(d => flattenedDocPart(d.valueParams))

  def memberInfo(
      m: Member,
      withBrief: Boolean = false,
      full: Boolean = false
  ): Seq[AppliedTag] =
    val comment = m.docs
    ...
    Seq(
      Option.when(withBrief && comment.flatMap(_.short).nonEmpty)(
        div(cls := "documentableBrief doc")(
          comment.flatMap(_.short).fold("")(renderDocPart)
        )
      ),
          ...
          Option
            .when(full)(
              section(id := "attributes")(
                Option
                  .when(memberTypeParams.nonEmpty)(
                    Seq(
                  ...
                  .flatten,
                Option
                  .when(memberValueParams.nonEmpty)(
                    Seq(
  ...
  def memberSignature(member: Member) =
    ...
    if !isSubtype then
      ...
      val (beforeExtends, afterExtends) =
        signature.suffix.splitAt(signature.suffix.indexOf(Keyword("extends")))
      val (shortSuffix, longSuffix) =
        splitTypeSuffixSignature(beforeExtends, afterExtends)
      Seq(
        div(cls := "signature")(
  ...
  def splitTypeSuffixSignature(
      shortAcc: List[SignaturePart],
      tail: List[SignaturePart],
      nestedTypeLevel: Int = 0
  ): (List[SignaturePart], List[SignaturePart]) =
    tail match
      case Nil =>
        (shortAcc, Nil)
  ...
  def memberIcon(member: Member) = member.kind match
    case _ =>
      ...
      val iconSpan = span(cls := "icon")(
        ...
        member.companion
          .map((kind, _) =>
            span(cls := s"micon companion ${kind.name.take(2)}")
          )
          .toList
      )
      Seq(
        member.companion
          .flatMap((_, dri) => link(dri))
          .fold(iconSpan)(link => a(href := link)(iconSpan))
      )
  ...
  def annotations(member: Member): Option[TagArg] =
    ...
    Option.when(rendered.nonEmpty)(
      span(cls := "annotations monospace")(rendered)
    )

  def member(member: Member) =
    val filterAttributes = FilterAttributes.attributesFor(member)
    val anchor =
      if member.dri.anchor.isEmpty then Nil else Seq(id := member.dri.anchor)
    ...
    div(topLevelAttr*)(
      div(cls := "documentableElement-expander")(
        Option
          .when(annots.nonEmpty || originInf.nonEmpty || memberInf.nonEmpty)(
            button(cls := "icon-button ar show-content")
          )
          .toList,
      ...
      Option
        .when(originInf.nonEmpty || memberInf.nonEmpty)(
          div(cls := "docs")(
  ...
  private def actualGroup(
      name: String,
      members: Seq[Member | MGroup],
      headerConstructor: String => AppliedTag = makeSubgroupHeader,
      wrapInSection: Boolean = true
  ): Seq[AppliedTag] =
    if members.isEmpty then Nil
    else
  ...
  private def isAbstract(m: Member | MGroup): Boolean = m match
    case m: Member =>
      m.modifiers.exists(Set(Modifier.Abstract, Modifier.Deferred).contains)
  ...
  private def buildGroup(name: String, subgroups: Seq[SubGroup]): Tab =
    val all = subgroups.map { case (name, members) =>
      ...
      val definedWithGroup =
        if Set("methods", "fields").contains(normalizedName) then
  ...
  def grouppedMember(m: Member, membersInGroups: Seq[Member]): Tab =
    if membersInGroups.isEmpty then emptyTab
    else
      ...
      val content =
        rawGroups.toSeq.sortBy(_._1.prio).flatMap { case (group, members) =>
          Seq(
    ...
    val extensions =
      rest
        ...
        .collect { case (Some(on), members) =>
          ...
          MGroup(
            span(cls := "groupHeader")(sig.map(renderElement(_))),
            members.sortBy(_.name).toSeq,
            on.name
          ) -> on.position
        }
        .toSeq
      ...
      renderTabs(
        ...
        Tab(
          ...
          actualGroup(
            ...
            name => h3(cls := "groupHeader h400")(name),
            false
          ),
          "expand"
        ),
        ...
        buildGroup(
          ...
          Seq(
            (
              "Constructors",
              rest.filter(_.kind.isInstanceOf[Kind.Constructor])
            ),
            ("Methods", rest.filter(_.kind.isInstanceOf[Kind.Def])),
            (
              "Fields",
              rest.filter(m => m.kind == Kind.Val || m.kind == Kind.Var)
            )
          )
        ),
        buildGroup(
  ...
  private def renderTabs(
    ...
    if tabs.isEmpty then Nil
    else
      Seq(
        div(cls := (if singleSelection then "tabs single" else "tabs"))(
          div(cls := "contents")(
            tabs.map(t =>
  ...
  def classLikeParts(m: Member, full: Boolean = true): TagArg =
    if !m.kind.isInstanceOf[Classlike] then Nil
    else
      val graphHtml = m.graph match
        case graph if graph.edges.nonEmpty && full =>
          Seq(
            div(id := "inheritance-diagram", cls := "diagram-class showGraph")(
              button(
                `type` := "button",
                cls := "label-only-button",
                onclick := "zoomOut()"
              )("Reset zoom"),
              button(
                `type` := "button",
      ...
      def signatureList(
        ...
        if list.isEmpty then Nil
          ...
          Seq(
            div(cls := s"mono-small-block $className")(
              list.map(link =>
                div(link.kind.name, " ", link.signature.map(renderElement(_)))
              ),
              if expandable then
                span(cls := "show-all-code show-content text-button h100")(
                  "Show all"
                )
              else span()
            )
      ...
      val supertypes =
        signatureList(m.parents, "supertypes", m.parents.length > 5)
      val subtypes =
        signatureList(m.knownChildren, "subtypes", m.knownChildren.length > 5)
  ...
  private def buildDocumentableFilter = div(cls := "documentableFilter")(
    ...
    input(
      cls := "filterableInput",
      placeholder := "Filter by any phrase",
      testId := "filterBarInput",
      `type` := "search"
    ),
    button(
      cls := "clearButton label-only-button",
      testId := "filterBarClearButton"
    )("Clear all")
  )

  private def companionBadge(m: Member): Seq[AppliedTag] =
    m.companion.fold(Nil) { companion =>
      Seq(
        div(cls := "companion-badge body-small")(
          span(
            "See the",
  ...
  def fullMember(m: Member): PageContent =
    ...
    val intro = m.kind match
      case Kind.RootPackage =>
        Seq(h1(cls := s"h600")(summon[DocContext].args.name))
      case _ =>
        Seq(
