...
case class Page(
    link: Link,
    content: Member | ResolvedTemplate | String,
    children: Seq[Page],
    hidden: Boolean = false
):
  def withNewChildren(newChildren: Seq[Page]) =
    copy(children = children ++ newChildren)
...
abstract class Renderer(
    rootPackage: Member,
    val members: Map[DRI, Member],
    protected val extension: String = "html"
)(using ctx: DocContext)
    extends SiteRenderer,
      Resources,
      Locations,
      Writer:
  ...
  val rootApiPage: Option[Page] = Some(memberPage(rootPackage))
    .filter(_.children.nonEmpty)
    .map(_.withTitle(ctx.args.name))
  ...
  val rootDocsPage: Option[Page] = staticSite match
    ...
    case Some(siteContext) =>
      ...
      // We add dummy guards
      val notHidden: Seq[Option[LoadedTemplate]] =
        None +: siteContext.allTemplates
          .filterNot(_.hidden)
          .map(Some(_)) :+ None

      // Let's gather the list of maps for each template with its in-order neighbours
      val newSettings: List[Map[String, Object]] = notHidden
        ...
        .map {
          ...
          case prev :: mid :: next :: Nil =>
            def link(sibling: Option[LoadedTemplate]): Option[String] =
              def realPath(path: Path) = if Files.isDirectory(path) then
                Paths.get(path.toString, "index.html")
              else path
              sibling.map { n =>
                val realMidPath = realPath(mid.get.file.toPath)
                val realSiblingPath = realPath(n.file.toPath)
                realMidPath
                  .relativize(realSiblingPath)
                  .toString
                  .stripPrefix("../")
              }
            List(
      ...
      def updateSettings(
          templates: Seq[LoadedTemplate],
          additionalSettings: ListBuffer[Map[String, Object]]
      ): List[LoadedTemplate] =
        ...
        for template <- templates do
          ...
          val current: Map[String, Object] = template.templateFile.settings
            .getOrElse("page", Map.empty)
            .asInstanceOf[Map[String, Object]]
          val updatedTemplateFile = template.templateFile.copy(settings =
            template.templateFile.settings.updated("page", head ++ current)
          )
      ...
      val newTemplates =
        updateSettings(Seq(rootTemplate), newSettings.to(ListBuffer))
      val templatePages = newTemplates.map(templateToPage(_, siteContext))

      val newRoot = newTemplates.head

      Some(newRoot)
        .filter(r => r.children.nonEmpty || r.templateFile.rawCode.nonEmpty)
        .map(templateToPage(_, siteContext))

  val redirectPages: Seq[Page] = staticSite.fold(Seq.empty)(siteContext =>
    siteContext.redirectTemplates.map { case (template, driFrom, driTo) =>
      val redirectTo = pathToPage(driFrom, driTo)
      templateToPage(
        template.copy(templateFile =
          template.templateFile.copy(settings =
            template.templateFile.settings ++ Map("redirectTo" -> redirectTo)
          )
        ),
        siteContext
      )
  ...
  /** Here we have to retrive index pages from hidden pages and replace fake
    * index pages in navigable page tree.
    */
      ...
      def walk(page: Page): Unit =
        if page.link.dri.isStaticFile then
          ...
          if apiPaths.contains(dest) then
            report.error(
              s"Conflict between static page and API member for $dest. $pathsConflictResoultionMsg"
            )
      ...
      val signatureRenderer = new SignatureRenderer:
        ...
        def link(dri: DRI): Option[String] =
          dri.externalLink.orElse(
            Some(pathToPage(currentDri, dri)).filter(
              _ != UnresolvedLocationLink
            )
          )
  ...
  /** Handler to prepare the content to be rendered. It's a good place to
    * organize frame, footers, front-matter, etc.
    */
  ...
  def renderPage(page: Page, parents: Vector[Link]): Seq[String] =
    ...
    write(page.link.dri, content, extension) +: page.children.flatMap(
      renderPage(_, newParents)
    )
end Renderer
