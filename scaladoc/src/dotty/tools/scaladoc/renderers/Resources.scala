...
trait Resources(using ctx: DocContext) extends Locations, Writer:
  ...
  lazy val darkProjectLogo = ctx.args.projectLogo
    .map(p => Paths.get(p))
    .map { p =>
    ...
    for res <- resources yield fileExtension(res) match
      ...
      case "js" =>
        script(
          ...
          if deferJs then Seq(defer := "true") else Nil
        )
      case _ => raw("")
  ...
  val commonResources: Seq[Resource] =
    ...
    fromResources ++ urls ++ projectLogo ++ darkProjectLogo ++ Seq(
      scaladocVersionFile,
      dynamicJsData
    )
  end commonResources
  ...
  val commonResourcesPaths = Seq(searchDataPath) ++ Seq(
    scastieConfigurationPath
  ) ++ commonResources.map(_.path)
  ...
  val staticSiteOnlyResourcesPaths: Seq[String] =
    staticSiteOnlyResources.map(_.path)
  ...
  def searchData(pages: Seq[Page]) =
    ...
    def mkEntry(
      ...
      "e" -> (if dri.externalLink.isDefined then rawJSON("true")
              else rawJSON("false")),
    ...
    def docPartRenderPlain(d: DocPart): String =
      import dotty.tools.scaladoc.tasty.comments.wiki.*
      ...
      d match
        ...
        case _ => ""
    end docPartRenderPlain

    def processPage(
        page: Page,
        pageFQName: List[String]
    ): Seq[(JSON, Seq[String])] =
        ...
        case m: Member if m.kind != Kind.RootPackage =>
          def processMember(
            ...
            val signature: MemberSignature =
              signatureProvider.rawSignature(member)()
            val sig = Signature(Plain(member.name)) ++ signature.suffix
            val descr =
              if member.kind == Kind.Package then "" else fqName.mkString(".")
            val extraDescr =
              member.docs.map(d => docPartRenderPlain(d.body)).getOrElse("")
            val entry = mkEntry(
              member.dri,
              member.name,
              flattenToText(sig),
            ...
            val children = member
              ...
              .filter(m =>
                m.origin == Origin.RegularlyDefined && m.inheritedFrom
                  .fold(true)(_.isSourceSuperclassHidden)
              )
            val updatedFqName =
              if member.kind == Kind.Package then List(member.name)
              else fqName :+ member.name
            Seq((entry, updatedFqName)) ++ children.flatMap(
              processMember(_, updatedFqName)
            )
          end processMember
  ...
  def scastieConfiguration() =
    Resource.Text(
      scastieConfigurationPath,
      s"""scastieConfiguration = "${ctx.args.scastieConfiguration
          .replace('"'.toString, """\"""")}""""
    )
  ...
  def renderResource(resource: Resource): Seq[String] =
    ...
    if normalizedPath.endsWith(".html") && apiPaths.contains(normalizedPath)
    then
      report.error(
        s"Conflict between resource and API member for $normalizedPath. $pathsConflictResoultionMsg"
      )
      Nil
    else
      resource match
