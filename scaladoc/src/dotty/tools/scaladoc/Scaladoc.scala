...
object Scaladoc:
  ...
  def run(args: Array[String], rootContext: CompilerContext): Reporter =
    ...
    parsedArgsOrNone.map { parsedArgs =>
      ...
      if !ctx.reporter.hasErrors then
        val updatedArgs = parsedArgs.copy(
          tastyDirs = parsedArgs.tastyDirs,
          tastyFiles = tastyFiles
        )

        if parsedArgs.output.exists() then util.IO.delete(parsedArgs.output)

        run(updatedArgs)
        report.inform("Done")
      else report.error("Failure")

      if parsedArgs.generateInkuire then
        dumpInkuireDB(parsedArgs.output.getAbsolutePath, parsedArgs)
    }
  ...
  def dumpInkuireDB(output: String, parsedArgs: Args) =
    ...
    configWriter.write(
      Inkuire.generateInkuireConfig(
        parsedArgs.externalMappings.map(_.documentationUrl.toString)
      )
    )
    configWriter.close()
  ...
  def extract(
    ...
    if !other.isEmpty then
      report.echo(
        s"Skipping unused scalacOptions: ${other.map(_.name).mkString(", ")}"
      )
    ...
    argumentFilesOrNone.fold((None, newContext)) { argumentFiles =>
      ...
      val legacySourceLinkList =
        if legacySourceLink.get.nonEmpty then List(legacySourceLink.get)
        else Nil
