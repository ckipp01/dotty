...
class StaticSiteLoader(val root: File, val args: Scaladoc.Args)(using
    StaticSiteContext,
    CompilerContext
):
  ...
  /** Method loading static site structure based on YAML configuration file.
    ...
    *   - If the subsection object contains location of index and doesn't
    *     contain any item, items are loaded using file system from the
    *     directory of the index file.
    *   - By default, directory name is a subsection title converted to kebab
    *     case. However, you can override default name by setting "directory"
    *     property of the subsection object.
    */
  def loadBasedOnYaml(yamlRoot: Sidebar.Category): StaticSiteRoot =
    ...
    val rootIndex = yamlRoot.indexPath
      ...
      .fold(emptyTemplate(rootDest, "index")) { f =>
        ...
        then
          report.warn("Property `title` will be overridden by project name", f)
        loaded
      }
    ...
    def loadChild(pathFromRoot: Path): Sidebar => LoadedTemplate = {
      case Sidebar.Category(optionTitle, optionIndexPath, nested, dir) =>
        ...
        val title = (
          ...
          report.error(
            s"Title for subsection needs to be set in YAML config or in index file"
          )
          TemplateName.FilenameDefined("unnamed_section")
        }
        val categoryPath = dir.fold(
          pathFromRoot.resolve(toKebabCase(title.name))
        )(pathFromRoot.resolve(_))
        val indexPage = indexPageOpt.getOrElse(
          emptyTemplate(categoryPath.resolve("index.html").toFile, title.name)
        )

        val children = optionIndexPath
          .filter(_ => nested.isEmpty)
          .fold(
            nested.map(child => loadChild(categoryPath)(child))
          ) { indexPath =>
            val indexPathDirectory = Paths.get(indexPath).getParent
            val filesInDirectory =
              Option(root.toPath.resolve(indexPathDirectory).toFile.listFiles)
            filesInDirectory.fold(List.empty) { files =>
              val mappingFunc: File => File = file =>
                val relativeFile = root.toPath
                  .resolve(indexPathDirectory)
        ...
        LoadedTemplate(
          indexPage,
          children,
          categoryPath.resolve("index.html").toFile
        )
      case Sidebar.Page(optionTitle, pagePath, hidden) =>
        ...
        LoadedTemplate(
          templateFile,
          List.empty,
          pathFromRoot.resolve(file.getName).toFile,
          hidden
        )
    }
    val rootTemplate = LoadedTemplate(
      rootIndex,
      yamlRoot.nested.map(c => loadChild(ctx.docsPath)(c)) ++ loadBlog(),
      rootDest
    )
  ...
  def loadBasedOnFileSystem(): StaticSiteRoot =
    val rootTemplate =
      loadRecursively(ctx.docsPath.toFile).getOrElse(
        LoadedTemplate(
          emptyTemplate(ctx.docsPath.resolve("index.html").toFile, "index"),
          List.empty,
          ctx.docsPath.resolve("index.html").toFile
        )
      )

    if rootTemplate.templateFile.title.name != "index" then
      report.warn(
        "Property `title` will be overridden by project name",
        rootTemplate.templateFile.file
      )

    val withChangedTitle =
      rootTemplate.copy(templateFile =
        rootTemplate.templateFile.copy(title =
          TemplateName.FilenameDefined(args.name)
        )
      )

    val withBlog = loadBlog().fold(withChangedTitle)(blog =>
      withChangedTitle.copy(children = withChangedTitle.children :+ blog)
    )

    val mappings = createMapping(withBlog)
    StaticSiteRoot(withBlog, mappings)
  end loadBasedOnFileSystem

  def loadBlog(): Option[LoadedTemplate] =
    val blogConfig = BlogParser.readYml(root)
    val rootPath = Option(blogConfig.input)
      .map(input => ctx.resolveNewBlogPath(input))
    ...
    if !Files.exists(rootPath) || blogConfig.hidden then None
      ...
      val indexPageOpt = Seq(
        ...
        case list =>
          report.warning(
            s"Multiple index pages for $rootPath found in ${list.map(_.toFile)}. Defaulting to first."
          )
          list.headOption
      val indexTemplateOpt = indexPageOpt.map(p => loadTemplateFile(p.toFile))

      val indexPage = indexTemplateOpt.getOrElse(
        emptyTemplate(rootPath.resolve("index.html").toFile, "Blog")
      )
      val indexDest =
        ctx.docsPath.resolve(defaultDirectory).resolve("index.html")
      val regex = raw"(\d*)-(\d*)-(\d*)-(.*)".r
      def splitDateName(tf: TemplateFile): (Date, String) =
        tf.file.getName match
          case regex(year, month, day, name) => ((year, month, day), name)
          case name =>
            report.warn(
              "Incorrect file name for blog post. Post file name should be in format <year>-<month>-<day>-<name>",
              tf.file
            )
            (("1900", "01", "01"), name)

      def dateFrom(tf: TemplateFile, default: String = "1900-01-01"): String =
        val pageSettings = tf.settings.get("page").collect {
          case m: Map[String @unchecked, ?] => m
        }
        pageSettings
          .flatMap(_.get("date").collect { case s: String => s })
          .getOrElse(default) // blogs without date are last
  ...
  def loadRecursively(
      currRoot: File,
      destMappingFunc: File => File = identity
  ): Option[LoadedTemplate] =
    ...
    then
      val indexPageOpt = Seq(
        ...
        case list =>
          report.warning(
            s"Multiple index pages for $currRoot found in ${list.map(_.toFile)}. Defaulting to first."
          )
          list.headOption
      val templateFileOpt = indexPageOpt.map(p => loadTemplateFile(p.toFile))

      val indexPage = templateFileOpt.getOrElse(
        emptyTemplate(rootPath.resolve("index.html").toFile, "index")
      )
      ...
      Some(
        LoadedTemplate(
          indexPage,
          children
            .flatMap(loadRecursively(_, destMappingFunc))
            .sortBy(_.templateFile.title.name),
          destMappingFunc(indexPage.file)
        )
      )
    else if currRoot.exists && ctx.siteExtensions.exists(ext =>
        currRoot.getName.endsWith(ext)
      )
    then
      ...
      Some(
        LoadedTemplate(
          templateFile,
          List.empty,
          destMappingFunc(templateFile.file)
        )
      )
    else None
    end if
  ...
  def createMapping(root: LoadedTemplate): Map[Path, Path] =
    Map((root.templateFile.file.toPath, root.file.toPath)) ++ root.children
      .map(createMapping)
      .fold(Map.empty)(_ ++ _)
