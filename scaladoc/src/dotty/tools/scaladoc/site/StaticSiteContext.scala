...
class StaticSiteContext(
    ...
    val snippetChecker: snippets.SnippetChecker
)(using val outerCtx: CompilerContext):
  ...
  def relativize(path: Path): Path =
    if args.apiSubdirectory then docsPath.relativize(path)
    else
  ...
  lazy val layouts: Map[String, TemplateFile] =
    ...
    dirs
      .map { it => loadTemplateFile(it)(using this) }
      .map { it => it.name -> it }
      .toMap

  lazy val staticSiteRoot: StaticSiteRoot =
    StaticSiteLoader(root, args)(using this, outerCtx).load()
  ...
  /** Handles redirecting from multiple locations to one page
    *
    * For each entry in redirectFrom setting, create a page which contains code
    * that redirects you to the page where the redirectFrom is defined.
    */
  lazy val redirectTemplates: Seq[(LoadedTemplate, DRI, DRI)] =
    allTemplates.flatMap { loadedTemplate =>
      val redirectFrom = loadedTemplate.templateFile.settings
      ...
      def redirectToTemplate(redirectFrom: String) =
        ...
        (
          LoadedTemplate(layouts("redirect"), List.empty, path.toFile),
          driFrom,
          driTo
        )
      redirectFrom
        .map {
          ...
          case redirects: List[?] =>
            redirects.asInstanceOf[List[String]].map(redirectToTemplate)
        }
        .getOrElse(Nil)
    }
  ...
  def driForLink(loadedTemplateFile: File, link: String): Seq[DRI] =
    def possibleLinks(link: String): Seq[Path] =
      ...
      val templateSourceLocation =
        staticSiteRoot.reverseSiteMappings.get(templateDestLocation)

      // Check if link is relative or absolute
      if link.startsWith("/")
      then Seq(root.toPath.resolve(link.drop(1)))
      else
        Seq(templateDestLocation.getParent.resolve(link).normalize) ++
    ...
    val fileNames =
      if siteExtensions.exists(link.endsWith(_))
      then
        val withoutExt = siteExtensions
          .find(link.endsWith(_))
          .fold(link)(ext => link.stripSuffix(ext))
    ...
    val links = baseFiles.flatMap(p =>
      Option
        .when(staticSiteRoot.sources.contains(p))(p)
        .map(p => staticSiteRoot.siteMappings(p))
