...
object FlexmarkSectionWrapper:
  ...
  def getNewChildren(
      finished: List[mdu.Node],
      current: Option[(mda.Heading, List[mdu.Node])],
      rest: List[mdu.Node]
  ): List[mdu.Node] = rest match
    case Nil => current.fold(finished)(finished :+ Section(_, _))
    case (h: mda.Heading) :: rest =>
      current.fold(getNewChildren(finished, Some(h, Nil), rest))((head, b) =>
        getNewChildren(finished :+ Section(head, b), Some(h, Nil), rest)
      )
    case (n: mdu.Node) :: rest =>
      current.fold(getNewChildren(finished :+ n, None, rest))((head, b) =>
        getNewChildren(finished, Some(head, b :+ n), rest)
      )
