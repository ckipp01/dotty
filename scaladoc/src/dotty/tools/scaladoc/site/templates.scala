...
/** RenderingContext stores information about defined properties, layouts and
  * sites being resolved
  *
  * @param properties
  ...
  * @param resolving
  *   Set containing names of sites being resolved in this context. This
  *   information is useful for cycle detection
  * @param resources
...
case class TemplateFile(
  ...
  def isIndexPage() =
    file.isFile && (file.getName == "index.md" || file.getName == "index.html")

  private[site] def resolveInner(ctx: RenderingContext)(using
      ssctx: StaticSiteContext
  ): ResolvedPage =
    ...
    lazy val snippetCheckingFunc: SnippetChecker.SnippetCheckingFunc =
      ...
      val sourceFile = dotty.tools.dotc.util.SourceFile(
        dotty.tools.io.AbstractFile.getFile(path.get),
        scala.io.Codec.UTF8
      )
      (
          str: String,
          lineOffset: SnippetChecker.LineOffset,
          argOverride: Option[SCFlags]
      ) =>
        ...
        ssctx.snippetChecker
          .checkSnippet(str, Some(compilerData), arg, lineOffset, sourceFile)
          .collect {
    ...
    if ctx.resolving.contains(file.getAbsolutePath) then
      throw new RuntimeException(
        s"Cycle in templates involving $file: ${ctx.resolving}"
      )

    val layoutTemplate = layout.map(name =>
      ctx.layouts.getOrElse(
        name,
    ...
    def asJavaElement(o: Object): Object = o match
      case m: Map[?, ?] =>
        m.transform { case (k: String, v: Object) =>
          asJavaElement(v)
        }.asJava
      case l: List[?] =>
        l.map(x => asJavaElement(x.asInstanceOf[Object])).asJava
      case other => other

    // Library requires mutable maps..
    val mutableProperties = new JHashMap(
      ctx.properties.transform((_, v) => asJavaElement(v)).asJava
    )

    val parseSettings =
      ParseSettings.Builder().withFlavor(Flavor.JEKYLL).build()

    val rendered =
      Template.parse(this.rawCode, parseSettings).render(mutableProperties)

    // We want to render markdown only if next template is html
    val code =
      if isHtml || layoutTemplate.exists(!_.isHtml) then rendered
      else
        ...
        val parsedMd = parser
          .parse(rendered)
          .pipe { md =>
            FlexmarkSnippetProcessor.processSnippets(
              md,
              None,
              snippetCheckingFunc
            )(using ssctx.outerCtx)
          }
          .pipe { md =>
