...
class ScalaSignatureProvider:
  ...
  def rawSignature(documentable: Member)(
      kind: Kind = documentable.kind
  ): MemberSignature =
    kind match
  ...
  private def classLikeSignature(member: Member, kind: Kind & Classlike)(
      showKind: Kind = kind
  ): MemberSignature =
    MemberSignature(
  ...
  private def enumEntrySignature(
      member: Member,
      cls: Kind.Class
  ): MemberSignature =
  ...
  private def givenClassSignature(
      member: Member,
      cls: Kind.Class
  ): MemberSignature =
    ...
    member.kind match
      case Kind.Given(_, Some(instance), _) =>
        initialSignature
          .copy(suffix =
            initialSignature.suffix ++ builder.plain(": ").signature(instance)
          )
  ...
  private def methodLikeSignature(
      method: Member,
      kind: Kind.Def,
      instance: Option[Signature] = None
  )(showKind: Kind = kind): MemberSignature =
    MemberSignature(
  ...
  private def extensionSignature(
      extension: Member,
      fun: Kind.Def
  ): MemberSignature =
    methodLikeSignature(extension, fun, Some(extension.signature))()

  private def givenMethodSignature(
      method: Member,
      body: Kind.Def
  ): MemberSignature = method.kind match
  ...
  private def fieldLikeSignature(
      member: Member,
      kind: Kind,
      instance: Option[Signature] = None
  ): MemberSignature =
  ...
  private def givenValSignature(field: Member): MemberSignature =
    field.kind match
      case Kind.Given(_, iOpt @ Some(instance), _) =>
