  ...
  def tryContinue(state: ParserState): BlockContinue =
    ...
    if matches then
      ...
      if matcher.find then
        ...
        then // closing fence - we're at end of line, so we can finalize now
    ...
    if lines.size > 0 then
      ...
      if lines.size > 1 then // have more lines
        ...
        if this.codeContentBlock then
          ...
          val codeBlock = new Text(
            SegmentedSequence.create(segments.asScala.toSeq*)
          )
          block.appendChild(codeBlock)
