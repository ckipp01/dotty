...
case class ExternalDocLink(
  ...
  def withPackageList(url: URL): ExternalDocLink =
    copy(packageListUrl = Some(url))
...
object ExternalDocLink:
  private def fail(mapping: String, msg: String) = Left(
    s"Unable to process external mapping $mapping. $msg"
  )

  private def tryParse[T](mapping: String, descr: String)(
      op: => T
  ): Either[String, T] = Try(op) match
    case Success(v) => Right(v)
    case Failure(e) =>
      fail(mapping, s"Unable to parse $descr. Exception $e occured")

  private def stripIndex(url: String): String =
    url.stripSuffix("index.html").stripSuffix("/") + "/"

  def parseLegacy(mapping: String): Either[String, ExternalDocLink] =
    mapping.split("#").toList match
      case path :: apiUrl :: Nil =>
        for url <- tryParse(mapping, "url")(URI(stripIndex(apiUrl)).toURL)
      ...
      case _ =>
        fail(
          mapping,
          "Wrong format of legacy external mapping. path#apiUrl format is accepted."
        )
  ...
  def parse(mapping: String): Either[String, ExternalDocLink] =
    ...
    def parsePackageList(elements: List[String]) = elements match
      case List(urlStr) =>
        tryParse(mapping, "packageList")(Some(URI(urlStr).toURL))
      case Nil   => Right(None)
