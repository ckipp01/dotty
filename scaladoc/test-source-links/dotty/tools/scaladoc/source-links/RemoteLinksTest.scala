...
class RemoteLinksTest:
  ...
  val mtslAll =
    membersToSourceLinks(using testDocContext()).filter(_._1 != "Predef")

  @Test
  def scala213XSourceLink =
    assertTrue(
      mtslAll.find((k, _) => k == "AbstractMap").isDefined
    ) // source link to Scala2.13.X stdlib class

  @Test
  def scala3SourceLink =
    assertTrue(
      mtslAll.find((k, _) => k == "PolyFunction").isDefined
    ) // source link to Scala3 stdlib class

  @Test
  def tastySourceLink =
    assertTrue(
      mtslAll.find((k, _) => k == "TastyBuffer").isDefined
    ) // source link to Scala3 tasty class

  @Test
  def runTest =
    assertTrue(mtslAll.nonEmpty)
    val mtsl =
      randomGenerator.shuffle(mtslAll).take(80) // take 80 random entries
    val pageToMtsl: Map[String, List[(String, Int)]] =
      mtsl.groupMap(_._2.split("#L").head)(v =>
        (v._1, v._2.split("#L").last.toInt)
      )
    pageToMtsl.toSeq.sortBy(_._1).foreach { case (link, members) =>
      try
        ...
        members.foreach { case (member, expectedLine) =>
          if !member.startsWith("given_")
          then // TODO: handle synthetic givens, for now we disable them from testing
            val toLine = expectedLine + 3
            val memberToMatch = member.replace("`", "")
            val lineCorrectlyDefined =
              (expectedLine until toLine).exists { line =>
            ...
            assertTrue(
              s"Expected to find $memberToMatch at $link at lines $expectedLine-$toLine",
              lineCorrectlyDefined
            )
        }
      catch
        case e: java.lang.IllegalArgumentException =>
          report.error(s"Could not open link for $link - invalid URL")(using
            testContext
          )
        case e: TimeoutException =>
          report.error(s"Tried to open link $link 16 times but with no avail")(
            using testContext
          )
        case e: org.jsoup.HttpStatusException =>
          e.getStatusCode match
            ...
            case n =>
              report.warning(s"Could not open link for $link, return code $n")(
                using testContext
              )
    }
          ...
          if dElem.select("span.kind").first.text != "package" then
            dElem.select("dt").forEach { elem =>
              ...
              if content == "Source" then
                mtsl += dElem
                  .select(".documentableName")
                  .first
                  .text -> elem.nextSibling.childNode(0).attr("href")
            }
        }
