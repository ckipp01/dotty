...
/** This plugin provides automatic dependency overrides for projects in the
  * community build. In doing so, we permit the projects in the build to depend
  * on arbitrary versions of other projects in the build, and the version
...
object CommunityBuildPlugin extends AutoPlugin:
  ...
  override val projectSettings: Seq[Setting[?]] = Seq(
    publishLocal := Def.taskDyn {
      ...
      Def.task {
        if artifacts.value.nonEmpty && !(publish / skip).value then
...
object CommunityBuildDependencies:
  ...
  /** Publish dependency override data for a module in the community build. This
    * appends a single entry to the tracking file.
    */
  ...
  private def sanitized(m: ModuleID): ModuleID =
    ModuleID(m.organization, m.name, m.revision).withCrossVersion(
      m.crossVersion
    )
end CommunityBuildDependencies
