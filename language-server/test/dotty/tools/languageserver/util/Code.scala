...
import dotty.tools.languageserver.util.embedded.*

/** Helper object to create virtual source files and extract markers from the
  * source files, using the `code` interpolator.
  *
  * The markers can then be used to perform tests at different positions within
  * the source file.
  */
object Code:
  ...
  implicit class CodeHelper(val sc: StringContext) extends AnyVal:
    ...
    /** An interpolator that lets set marker inside a virtual source file.
      ...
      * ```
      *
      * This will define a source file where the markers `m1` and `m2` enclose
      * the identifier `Foo`, and `m3` and `m4` enclose the identifier `Hello`.
      * These positions can then be used to ask to perform actions such as
      * finding all references, etc.
      */
    ...
    /** An interpolator similar to `code`, but used for defining a source that
      * will be unpickled from TASTY.
      *
      * @see
      *   code
      */
    ...
    private def textAndPositions(
        args: Embedded*
    ): (String, List[(CodeMarker, Int, Int)]) =
      ...
      if pi.hasNext then stringBuilder.append(pi.next().stripMargin)

      (stringBuilder.result(), positions.result())
    end textAndPositions
  end CodeHelper

  /** A new `CodeTester` working with a single project containing `sources`. */
  def withSources(sources: SourceWithPositions*): CodeTester = withProjects(
    Project(sources.toList)
  )

  /** A new `CodeTester` working with `projects`. */
  def withProjects(projects: Project*): CodeTester = new CodeTester(
    projects.toList
  )
  ...
  object SourceWithPositions:
    ...
    implicit def sourceWithPositionsToCodeTester(
        scalaSource: SourceWithPositions
    ): CodeTester =
  ...
  case class ScalaSourceWithPositions(
      text: String,
      positions: List[(CodeMarker, Int, Int)]
  ) extends SourceWithPositions:
  ...
  /** A virtual worksheet where several markers have been set.
    ...
    *   The positions of the markers that have been set.
    */
  case class WorksheetWithPositions(
      text: String,
      positions: List[(CodeMarker, Int, Int)]
  ) extends SourceWithPositions:
    def sourceName(index: Int): String = s"Worksheet$index.sc"

  /** A virtual source file that will not be opened in the IDE, but instead
  ...
  /** A group of sources belonging to the same project.
    *
    * @param sources
    *   The sources that this project holds.
    * @param name
    *   The name of this project
    * @param dependsOn
    *   The other projects on which this project depend.
    */
  case class Project(
      sources: List[SourceWithPositions],
    ...
    /** Add `sources` to the sources of this project.
      */
    def withSources(sources: SourceWithPositions*): Project =
      copy(sources = this.sources ::: sources.toList)
  ...
  object Project:
    ...
    /** Creates a new project that depends on `projects`.
      *
      * @param projects
      *   The dependencies of the new project.
      * @return
      *   An empty project with a dependency on the specified projects.
      */
    def dependingOn(projects: Project*) =
      new Project(Nil, dependsOn = projects.toList)

    /** Create a new project with the given sources.
      *
      * @param sources
      *   The sources to add to this project.
      * @return
      *   a new project containing the specified sources.
      */
    def withSources(sources: SourceWithPositions*): Project = new Project(
      sources.toList
    )
end Code
