...
class TestServer(testFolder: Path, projects: List[Project]):
  ...
  private[this] def init(): InitializeResult =
    ...
    def compileProjectAndDependencies(project: Project): Unit =
      if !compiledProjects.contains(project) then
    ...
    /** Set up given project, return JSON config.
      *
      * If the project has dependencies, these dependencies are compiled. The
      * classfiles of the dependent projects are put on the classpath of this
      * project.
      *
      * @param project
      *   The project to configure.
      * @return
      *   A JSON object representing the configuration for this project.
      */
    def projectSetup(project: Project): String =
      def showSeq[T](lst: collection.Seq[T]): String =
        lst
          .map(elem =>
            '"'.toString + elem.toString.replace('\\', '/') + '"'.toString
          )
          .mkString("[ ", ", ", " ]")

      if project.sources.exists(_.isInstanceOf[TastyWithPositions]) then
      ...
      s"""{
         ...
         |  "compilerArguments" : ${showSeq(
          BuildInfo.ideTestsCompilerArguments
        )},
         |  "sourceDirectories" : ${showSeq(
          sourceDirectory(project, wipe = false) :: Nil
        )},
         |  "dependencyClasspath" : ${showSeq(dependencyClasspath(project))},
         |  "classDirectory" : "${classDirectory(project, wipe = false).toString
          .replace('\\', '/')}",
  ...
  /** Open the code in the given file and returns the file.
    ...
    *   code in file
    * @param fileName
    *   file path in the source directory
    * @param openInIDE
    *   If true, send `textDocument/didOpen` to the server.
    * @return
    *   the file opened
    */
  def openCode(
  ...
  private def classDirectory(project: Project, wipe: Boolean): Path =
    ...
    if wipe then Directory(path).deleteRecursively()
  ...
  /** Sets up the sources of the given project, creates the necessary
    * directories and compile the sources.
    *
    * @param project
  ...
  private def compileProject(project: Project): Unit =
    ...
    val compileOptions =
      sources.toArray ++
        Array(
          "-classpath",
          dependencyClasspath(project).mkString(pathSeparator),
          "-d",
          classDirectory(project, wipe = true).toString
        )
