  ...
  override def execute(): Exec[Unit] =
    ...
    expected.zip(logs).foreach {
      case ((expectedCodeRange, expectedOutput), (actualRange, actualOutput)) =>
        ...
        if strict then assertEquals(expectedOutput, actualOutput)
        else
          assertTrue(
            s"'$actualOutput' didn't start with '$expectedOutput'",
            actualOutput.startsWith(expectedOutput)
          )
    }
