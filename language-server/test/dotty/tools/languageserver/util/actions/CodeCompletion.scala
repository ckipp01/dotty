...
/** An action requesting for code completion at `marker`, expecting `expected`.
  * This action corresponds to the `textDocument/completion` method of the
  * Language Server Protocol.
  *
  * @param marker
  *   The marker indicating the position where completion should be requested.
  * @param checkResults
  *   A function that takes the results and verifies that they match
  *   expectations.
  */
class CodeCompletion(
  ...
  override def execute(): Exec[Unit] =
    ...
    assertFalse(
      s"Completion results were 'incomplete': $result",
...
object CodeCompletion:
  ...
  def simplifyResults(
      items: Set[CompletionItem]
  ): Set[(String, CompletionItemKind, String)] =
