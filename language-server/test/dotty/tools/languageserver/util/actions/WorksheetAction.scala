...
import dotty.tools.languageserver.worksheet.{
  WorksheetRunOutput,
  WorksheetRunParams,
  WorksheetRunResult
}
...
abstract class WorksheetAction extends Action:
  ...
  def triggerRun(
      marker: CodeMarker
  ): Exec[CompletableFuture[WorksheetRunResult]] =
  ...
  def worksheetOutput(marker: CodeMarker): Exec[List[WorksheetRunOutput]] =
    ...
    client.worksheetOutput.get.filter(
      _.textDocument.getUri == textDocument.getUri
    )
