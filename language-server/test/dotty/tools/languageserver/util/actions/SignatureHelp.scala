...
/** An action requesting for signature help at `marker`. This action corresponds
  * to the `textDocument/signatureHelp` method of the Language Server Protocol.
  *
  * @param marker
  *   The marker indicating the position where the signature help should be
  *   requested.
  * @param expected
  *   The results that are expected for this query.
  * @param activeSignature
  *   The expected index of the active signature.
  * @param activeParam
  *   The expected index of the active paremeter.
  */
class SignatureHelp(
  ...
  val expectedSignatures =
    expected.map(DottyLanguageServer.signatureToSignatureInformation)

  override def execute(): Exec[Unit] =
    val results =
      server.signatureHelp(marker.toTextDocumentPositionParams).get()
    val resultSignatures = results.getSignatures.asScala

    assertEquals(
      "Number of signatures",
      expected.length,
      resultSignatures.length
    )
...
object SignatureHelp:
  ...
  implicit val signatureInformationOrdering: Ordering[SignatureInformation] =
