...
trait ActionOnRange extends Action:
  ...
  override def execute(): Exec[Unit] =
    ...
    assert(
      startLine <= endLine,
      "Start of range should be before end " + range.show
    )
    assert(
    ...
    assert(
      startLine == endLine,
      "multiline ranges not supported"
    ) // TODO implement multiline
    (startCharacter until endCharacter).foreach { char =>
      val marker =
        new CodeMarker(start.name + "-with-offset=" + (char - startCharacter))
      implicit def posCtx2: PositionContext =
        posCtx.withPos(marker, (file, startLine, char))
      onMarker(marker)
    }
end ActionOnRange
