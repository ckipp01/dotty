...
class WorksheetTest:
  ...
  @Test def runExpression: Unit =
    ws"${m1}2 + 2${m2}"
      .run(m1, ((m1 to m2), "val res0: Int = 4"))

  @Test def runSimpleVal: Unit =
    ws"${m1}val foo = 123${m2}"
      .run(m1, ((m1 to m2), "val foo: Int = 123"))
  ...
  @Test def defineAnonymousClass0: Unit =
    ws"""${m1}new {
      ...
      .run(m1, ((m1 to m2), "val res0: Object = Foo"))
  ...
  @Test def patternMatching0: Unit =
    ws"""${m1}1 + 2 match {
      ...
      .run(m1, ((m1 to m2), "val res0: String = odd"))
  ...
  @Test def worksheetHighlight(): Unit =
    ws"""class ${m1}Foo${m2} { def ${m3}bar${m4} = 123  }
      ...
      .highlight(
        m1 to m2,
        (m1 to m2, DocumentHighlightKind.Read),
        (m5 to m6, DocumentHighlightKind.Read)
      )
      .highlight(
        m3 to m4,
        (m3 to m4, DocumentHighlightKind.Read),
        (m7 to m8, DocumentHighlightKind.Read)
      )
      .highlight(
        m5 to m6,
        (m1 to m2, DocumentHighlightKind.Read),
    ...
    ws"""${m1}class Foo {
      ...
      .documentSymbol(
        ...
        (m1 to m2).symInfo(
          "Foo",
          SymbolKind.Class,
          WorksheetWrapper.moduleClassName.stripModuleClassSuffix.toString
        ),
