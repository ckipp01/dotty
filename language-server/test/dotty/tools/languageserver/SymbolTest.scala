...
class SymbolTest:
  ...
  @Test def symbol0: Unit =
    ...
    withSources(code"class $Foo")
      .symbol("Foo", Foo.range.symInfo("Foo", SymbolKind.Class))
  ...
  @Test def symbol1: Unit =
    ...
    withSources(
      ...
      "Foo",
      Foo.range.symInfo("Foo", SymbolKind.Class),
      fooFoo.range.symInfo("Foo", SymbolKind.Class, "foo")
    ).symbol(
      "Bar",
      Bar.range.symInfo("Bar", SymbolKind.Class),
      (m7 to m8).symInfo("Bar", SymbolKind.Class, "Foo")
    )
