...
import Comments.*, Constants.*, Contexts.*, Flags.*, Names.*, NameOps.*,
  Symbols.*, SymDenotations.*, Trees.*, Types.*
import classpath.ClassPathEntries
import reporting.*
import typer.Typer
import util.*
import interactive.*, interactive.InteractiveDriver.*
...
import lsp4j.services.*

/** An implementation of the Language Server Protocol for Dotty.
  *
  * You should not have to directly this class, instead see
  * `dotty.tools.languageserver.Main`.
  *
  * For more information see:
  *   - The LSP is defined at
  *     https://github.com/Microsoft/language-server-protocol
  *   - This implementation is based on the LSP4J library:
  *     https://github.com/eclipse/lsp4j
  */
class DottyLanguageServer
    extends LanguageServer
    with TextDocumentService
    with WorkspaceService
    with WorksheetService
    with TastyDecompilerService:
  thisServer =>
  import ast.tpd.*

  import DottyLanguageServer.*
  import InteractiveDriver.*

  import lsp4j.jsonrpc.{CancelChecker, CompletableFutures}
  import lsp4j.jsonrpc.messages.{Either as JEither}
  import lsp4j.*
  ...
  private[this] var myDependentProjects
      : mutable.Map[ProjectConfig, mutable.Set[ProjectConfig]] = _
  ...
  def drivers: Map[ProjectConfig, InteractiveDriver] = thisServer.synchronized {
    if myDrivers == null then
      ...
      val configs: List[ProjectConfig] = (new ObjectMapper)
        .readValue(configFile, classOf[Array[ProjectConfig]])
        .toList

      val defaultFlags =
        List("-color:never" /*, "-Yplain-printer","-Yprint-pos"*/ )
        ...
        val settings =
          defaultFlags ++
            config.compilerArguments.toList
              ...
              .update(
                "-classpath",
                (config.classDirectory +: config.dependencyClasspath)
                  .mkString(File.pathSeparator)
              )
              .update(
                "-sourcepath",
                config.sourceDirectories.mkString(File.pathSeparator)
              )
  ...
  private def restart() = thisServer.synchronized {
    ...
    val driverConfigs =
      for ((config, driver) <- myDrivers.toList)
        yield (
          config,
          new InteractiveDriver(driver.settings),
          driver.openedFiles
        )
    for (config, driver, _) <- driverConfigs do myDrivers(config) = driver
    System.gc()
    for (_, driver, opened) <- driverConfigs; (uri, source) <- opened do
      driver.run(uri, source)
    if Memory.isCritical() then
      println(
        s"WARNING: Insufficient memory to run Scala language server on these projects."
      )
  }
  ...
  def configFor(uri: URI): ProjectConfig = thisServer.synchronized {
    val config =
      drivers.keys.find(config =>
        config.sourceDirectories.exists(sourceDir =>
      ...
      val settings =
        defaultFlags ++
          config.compilerArguments.toList
            .update(
              "-classpath",
              (classPath +: config.dependencyClasspath).mkString(
                File.pathSeparator
              )
            )
  ...
  /** A mapping from project `p` to the set of projects that transitively depend
    * on `p`.
    */
    ...
    thisServer.synchronized {
      if myDependentProjects == null then
        ...
        def transitiveDependencies(config: ProjectConfig): Set[ProjectConfig] =
          val dependencies =
            config.projectDependencies.flatMap(idToConfig.get).toSet
          dependencies ++ dependencies.flatMap(transitiveDependencies)

        myDependentProjects =
          new mutable.HashMap().withDefaultValue(mutable.Set.empty)
        for
          project <- allProjects
          dependency <- transitiveDependencies(project)
        do myDependentProjects(dependency) += project
      myDependentProjects
    }
      ...
      def computation(): R =
        ...
        try fun(cancelToken)
          ...
          case NonFatal(ex) =>
            ex.printStackTrace
            throw ex
      if synchronize then thisServer.synchronized { computation() }
      else computation()
    }

  override def initialize(params: InitializeParams) = computeAsync {
    cancelToken =>
      rootUri = params.getRootUri
      assert(rootUri != null)

      class DottyServerCapabilities(
          val worksheetRunProvider: Boolean = true,
          val tastyDecompiler: Boolean = true
      ) extends lsp4j.ServerCapabilities
      ...
      c.setSignatureHelpProvider(
        new SignatureHelpOptions( /* triggerCharacters = */ List("(").asJava)
      )
  ...
  override def didOpen(params: DidOpenTextDocumentParams): Unit =
    thisServer.synchronized {
      ...
      val text =
        if worksheetMode then wrapWorksheet(document.getText)
  ...
  override def didChange(params: DidChangeTextDocumentParams): Unit =
    ...
    thisServer.synchronized {
      ...
      val text =
        if worksheetMode then wrapWorksheet(change.getText)
        else change.getText
  ...
  override def completion(params: CompletionParams) = computeAsync {
    cancelToken =>
  ...
  /** If cursor is on a reference, show its definition and all overriding
    * definitions. If cursor is on a definition, show this definition together
    * with all overridden and overriding definitions.
    */
  override def definition(params: TextDocumentPositionParams) = computeAsync {
    cancelToken =>
      val uri = new URI(params.getTextDocument.getUri)
      val driver = driverFor(uri)
      implicit def ctx: Context = driver.currentCtx
  ...
  override def references(params: ReferenceParams) = computeAsync {
    cancelToken =>
      val uri = new URI(params.getTextDocument.getUri)
      val driver = driverFor(uri)
      ...
      val includes =
        ...
        Include.references | Include.overriding | Include.imports | Include.local |
          (if includeDeclaration then Include.definitions else Include.empty)
      ...
      val references = {
        ...
        val perProjectInfo =
          inProjectsSeeing(driver, definitions, originalSymbols)
        ...
        perProjectInfo.flatMap { (remoteDriver, ctx, definitions) =>
          definitions.flatMap { definition =>
            ...
            val matches =
              Interactive.findTreesMatching(trees, includes, definition)(using
                ctx
              )
  ...
  override def rename(params: RenameParams) = computeAsync { cancelToken =>
    ...
    def findRenamedReferences(
      ...
      syms.flatMap { sym =>
        Interactive.findTreesMatching(
          trees,
          Include.all,
          sym,
          t => Interactive.sameName(t.name, withName)
        )
      }
    ...
    val refs =
      path match
        ...
        case untpd.ImportSelector(
              _,
              rename: untpd.Ident,
              _
            ) :: (_: Import) :: rest if rename.span.contains(pos.span) =>
        ...
        case _ =>
          val (include, allSymbols) =
            if syms.exists(_.allOverriddenSymbols.nonEmpty) then
              showMessageRequest(
                MessageType.Info,
                ...
                List(
                  RENAME_OVERRIDDEN -> (() =>
                    (
                      Include.all,
                      syms.flatMap(s => s :: s.allOverriddenSymbols.toList)
                    )
                  ),
                  RENAME_NO_OVERRIDDEN -> (() =>
                    (Include.all.except(Include.overridden), syms)
                  )
                )
              ).get.getOrElse((Include.empty, Nil))
            else (Include.all, syms)

          val names = allSymbols.map(_.name.sourceModuleName).toSet
          val definitions = Interactive.findDefinitions(
            allSymbols,
            driver,
            include.isOverridden,
    ...
    val changes =
      refs
        .groupBy(ref => toUriOption(ref.source))
        .flatMap { case (uriOpt, refs) =>
          uriOpt.map(uri => (uri.toString, refs))
        }
        .transform((_, refs) =>
          refs
            .flatMap(ref =>
              range(ref.namePos)
                .map(nameRange => new TextEdit(nameRange, newName))
            )
            .distinct
            .asJava
        )
  ...
  override def hover(params: TextDocumentPositionParams) = computeAsync {
    cancelToken =>
      val uri = new URI(params.getTextDocument.getUri)
      val driver = driverFor(uri)
      implicit def ctx: Context = driver.currentCtx
      ...
      if tp.isError || tpw == NoType then
        null // null here indicates that no response should be sent
      else
  ...
  override def documentSymbol(params: DocumentSymbolParams) = computeAsync {
    cancelToken =>
      val uri = new URI(params.getTextDocument.getUri)
      val driver = driverFor(uri)
      implicit def ctx: Context = driver.currentCtx
      ...
      val defs = Interactive.namedTrees(
        uriTrees,
        Include.local,
        excludeParamsFromSyntheticSymbols
      )
  ...
  override def symbol(params: WorkspaceSymbolParams) = computeAsync {
    cancelToken =>
      ...
      drivers.values.toList.flatMap { driver =>
        ...
        val defs = Interactive.namedTrees(
          trees,
          Include.empty,
          _.name.toString.contains(query)
        )
      ...
      val implementations = {
        ...
        perProjectInfo.flatMap { (remoteDriver, ctx, definitions) =>
          ...
          val predicate: NameTree => Boolean =
            val predicates =
              definitions.map(Interactive.implementationFilter(_)(using ctx))
            tree => predicates.exists(_(tree))
          val matches =
            Interactive.namedTrees(trees, Include.local, predicate)(using ctx)
          matches.map(tree => location(tree.namePos(using ctx)))
        }
      }.toList
  ...
  override def signatureHelp(params: TextDocumentPositionParams) =
    computeAsync { canceltoken =>
      ...
      val trees = driver.openedTrees(uri)
      val path = Interactive.pathTo(trees, pos)
      val (paramN, callableN, signatures) =
        Signatures.signatureHelp(path, pos.span)
      new SignatureHelp(
        signatures.map(signatureToSignatureInformation).asJava,
        callableN,
        paramN
      )
  ...
  /** Find the set of projects that have any of `definitions` on their
    * classpath.
    *
    * @param definitions
    *   The definitions to consider when looking for projects.
    * @return
    *   The set of projects that have any of `definitions` on their classpath.
    */
  private def projectsSeeing(
      definitions: List[SourceTree]
  )(implicit ctx: Context): Set[ProjectConfig] =
    if definitions.isEmpty then drivers.keySet
    else
      for
  ...
  /** Finds projects that can see any of `definitions`, translate `symbols` in
    * their universe.
    *
    * @param baseDriver
    *   The driver responsible for the trees in `definitions` and `symbol`.
    * @param definitions
    *   The definitions to consider when looking for projects.
    * @param symbol
    *   Symbols to translate in the universes of the remote projects.
    * @return
    *   A list consisting of the remote drivers, their context, and the
    *   translation of `symbol` into their universe.
    */
  private def inProjectsSeeing(
      baseDriver: InteractiveDriver,
      definitions: List[SourceTree],
      symbols: List[Symbol]
  ): List[(InteractiveDriver, Context, List[Symbol])] =
    ...
    projects.toList.map { config =>
      ...
      val definitions =
        symbols.map(Interactive.localize(_, baseDriver, remoteDriver))
      (remoteDriver, ctx, definitions)
    }

  /** Send a `window/showMessageRequest` to the client, asking to choose between
    * `choices`, and perform the associated operation.
    *
    * @param tpe
    *   The type of the request
    * @param message
    *   The message accompanying the request
    * @param choices
    *   The choices and their associated operation
    * @return
    *   A future that will complete with the result of executing the action
    *   corresponding to the user's response.
    */
  private def showMessageRequest[T](
      tpe: MessageType,
      message: String,
      choices: List[(String, () => T)]
  ): CompletableFuture[Option[T]] =
    val options = choices.map((title, _) => new MessageActionItem(title))
    val request = new ShowMessageRequestParams(options.asJava)
    request.setMessage(message)
    request.setType(tpe)

    client.showMessageRequest(request).thenApply {
...
end DottyLanguageServer

object DottyLanguageServer:

  final val IDE_CONFIG_FILE = ".dotty-ide.json"

  final val RENAME_OVERRIDDEN_QUESTION =
    "Do you want to rename the base member, or only this member?"
  ...
  def sourcePosition(
    ...
    val actualPosition =
      if isWorksheet(uri) then toWrappedPosition(pos)
      else pos
    val source = driver.openedFiles(uri)
    if source.exists then
      source
        .lineToOffsetOpt(actualPosition.getLine)
        .map(_ + actualPosition.getCharacter) match
  ...
  def range(p: SourcePosition): Option[lsp4j.Range] =
    if p.exists then
      ...
      Some(
        new lsp4j.Range(
          new lsp4j.Position(
            mappedPosition.startLine,
            mappedPosition.startColumn
          ),
  ...
  def diagnostic(
    ...
    if !dia.pos.exists then
      ...
      val message = dia.msg
      if displayMessage(message, dia.pos.source) then
        val code = message.errorId.errorNumber.toString
        range(dia.pos).map(r =>
          new lsp4j.Diagnostic(
            r,
            dia.message,
            severity(dia.level), /*source =*/ "",
            code
          )
        )
      else None
  ...
  /** Check whether `message` should be displayed in the IDE.
    *
    * Currently we only filter out the warning about pure expressions in
    * statement position when they are immediate children of the worksheet
    * wrapper.
    *
    * @param message
    *   The message to filter.
    * @param sourceFile
    *   The sourcefile from which `message` originates.
    * @return
    *   true if the message should be displayed in the IDE, false otherwise.
    */
  private def displayMessage(message: Message, sourceFile: SourceFile)(implicit
      ctx: Context
  ): Boolean =
    if isWorksheet(sourceFile) then
      message match
        case msg: PureExpressionInStatementPosition =>
          val ownerSym =
            if msg.exprOwner.isLocalDummy then msg.exprOwner.owner
            else msg.exprOwner
  ...
  /** Map `position` in a wrapped worksheet to the same position in the
    * unwrapped source.
    *
    * Because worksheet are wrapped in an `object`, the positions in the source
    * are one line above from what the compiler sees.
    *
    * @see
    *   wrapWorksheet
    * @param position
    *   The position as seen by the compiler (after wrapping)
    * @return
    *   The position in the actual source file (before wrapping).
    */
  ...
  /** Map `position` in an unwrapped worksheet to the same position in the
    * wrapped source.
    *
    * Because worksheet are wrapped in an `object`, the positions in the source
    * are one line above from what the compiler sees.
    ...
    *   wrapWorksheet
    * @param position
    *   The position as seen by VSCode (before wrapping)
    * @return
    *   The position as seen by the compiler (after wrapping)
    */
  ...
  /** Returns the position mapper necessary to unwrap positions for
    * `sourcefile`. If `sourcefile` is not a worksheet, no mapper is necessary.
    * Otherwise, return `toUnwrappedPosition`.
    */
  private def positionMapperFor(
      sourcefile: SourceFile
  ): Option[SourcePosition => SourcePosition] =
    if isWorksheet(sourcefile) then Some(toUnwrappedPosition _)
    else None
  ...
  /** Is `sourceTree` the wrapper object that we put around worksheet sources?
    ...
    *   wrapWorksheet
    */
  def isWorksheetWrapper(sourceTree: SourceTree)(implicit
      ctx: Context
  ): Boolean =
  ...
  /** Is this symbol the wrapper object for top level definitions?
    */
  def isTopLevelWrapper(sourceTree: SourceTree)(implicit
      ctx: Context
  ): Boolean =
  ...
  def completionItem(completion: Completion)(implicit
      ctx: Context
  ): lsp4j.CompletionItem =
    def completionItemKind(sym: Symbol)(implicit
        ctx: Context
    ): lsp4j.CompletionItemKind =
      import lsp4j.{CompletionItemKind as CIK}
      ...
      else if sym.isConstructor then CIK.Constructor
      else if sym.isClass then CIK.Class
      else if sym.is(Mutable) then CIK.Variable
      else if sym.is(Method) then CIK.Method
      else CIK.Field
    ...
    item.setDeprecated(
      completion.symbols.forall(_.hasAnnotation(defn.DeprecatedAnnot))
    )
    completion.symbols.headOption.foreach(s =>
      item.setKind(completionItemKind(s))
    )
    item
  end completionItem
  ...
  def symbolInfo(sym: Symbol, pos: SourcePosition)(implicit
      ctx: Context
  ): Option[lsp4j.SymbolInformation] =
    def symbolKind(sym: Symbol)(implicit ctx: Context): lsp4j.SymbolKind =
      import lsp4j.{SymbolKind as SK}

      if sym.is(Package) then SK.Package
      else if sym.isConstructor then SK.Constructor
      else if sym.is(Module) then SK.Module
      else if sym.isAllOf(EnumCase) || sym.isAllOf(EnumValue) then SK.EnumMember
      else if sym.is(Enum) then SK.Enum
      else if sym.is(Trait) then SK.Interface
      else if sym.isClass then SK.Class
      else if sym.is(Mutable) then SK.Variable
      else if sym.is(Method) then SK.Method
      else if sym.is(TypeParam) || sym.isAbstractOrAliasType then
        SK.TypeParameter
      else SK.Field
    def containerName(sym: Symbol): String =
      val owner =
        if sym.owner.exists && sym.owner.isPackageObject then sym.owner.owner
        else sym.owner
      if owner.exists && !owner.isEmptyPackage then
    ...
    location(pos).map(l =>
      new lsp4j.SymbolInformation(name, symbolKind(sym), l, containerName(sym))
    )
  end symbolInfo
