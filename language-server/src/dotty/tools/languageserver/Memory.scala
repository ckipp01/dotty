...
object Memory:
  ...
  /** Turn this flag on to stress test restart capability in compiler. It will
    * restart the presentation compiler after every 10 editing actions
    */
  ...
  def isCritical(): Boolean =
    if stressTest then
      stressTestCounter += 1
      if stressTestCounter % 10 == 0 then return true
