...
trait TastyDecompilerService:
  ...
  @JsonRequest
  def decompile(
      params: TastyDecompileParams
  ): CompletableFuture[TastyDecompileResult] =
    computeAsync(
      synchronize = false,
      fun = cancelChecker =>
        ...
        try
          ...
          TastyFileUtil.getClassPath(tastyFile) match
            ...
            case _ =>
              TastyDecompileResult(error =
                TastyDecompileResult.ErrorClassNotFound
              )
        catch
