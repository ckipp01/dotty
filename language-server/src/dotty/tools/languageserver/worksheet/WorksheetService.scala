...
import org.eclipse.lsp4j.jsonrpc.services.* //{JsonSegment, JsonRequest}
...
trait WorksheetService:
      ...
      fun = cancelChecker =>
        ...
        try
          ...
          val sendMessage = (pos: SourcePosition, msg: String) =>
            client.publishOutput(
              WorksheetRunOutput(params.textDocument, range(pos).get, msg)
            )

          runWorksheet(driver, uri, sendMessage, cancelChecker)(
            driver.currentCtx
          )
  ...
  /** Run the worksheet at `uri`.
    *
    * @param driver
    *   The driver for the project that contains the worksheet.
    * @param uri
    *   The URI of the worksheet.
    * @param sendMessage
    *   A mean of communicating the results of evaluation back.
    * @param cancelChecker
    *   Token to check whether evaluation was cancelled
    */
  private def runWorksheet(
    ...
    treeOpt.foreach(tree =>
      Worksheet.run(tree, thisServer, sendMessage, cancelChecker)
    )
end WorksheetService
