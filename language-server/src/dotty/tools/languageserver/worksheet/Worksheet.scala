...
object Worksheet:
      ...
      Evaluator.get(cancelChecker) match
        ...
        case Some(evaluator) =>
          ...
          queries.foreach { (line, code) =>
            ...
            cancelChecker.checkCanceled()
            if res.nonEmpty then sendMessage(line, res)
          }
    }
  ...
  /** Extract the position and source code corresponding to this tree
    *
    * @param evaluator
    *   The JVM that runs the REPL.
    * @param tree
    *   The compiled tree to evaluate.
    * @param sourcefile
    *   The sourcefile of the worksheet.
    */
  private def query(tree: Tree, sourcefile: SourceFile)(implicit
      ctx: Context
  ): (SourcePosition, String) =
    val source =
      sourcefile.content.slice(tree.span.start, tree.span.end).mkString
