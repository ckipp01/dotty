...
private object Evaluator:
  ...
  private val javaExec: Option[String] =
    ...
    val java =
      new File(bin, if scala.util.Properties.isWin then "java.exe" else "java")

    if java.exists() then Some(java.getAbsolutePath())
    else None

  /** The most recent Evaluator that was used. It can be reused if the user
    * classpath hasn't changed between two calls.
    */
  ...
  /** Get a (possibly reused) Evaluator and set cancel checker.
    *
    * @param cancelChecker
    *   The token that indicates whether evaluation has been cancelled.
    * @return
    *   A JVM running the REPL.
    */
  def get(
      cancelChecker: CancelChecker
  )(implicit ctx: Context): Option[Evaluator] = synchronized {
    ...
    previousEvaluator match
      ...
      case _ =>
        ...
        val newEvaluator = javaExec.map(
          new Evaluator(_, ctx.settings.classpath.value, cancelChecker)
        )
...
/** Represents a JVM running the REPL, ready for evaluation.
  ...
  *   The token that indicates whether evaluation has been cancelled.
  */
private class Evaluator private (
    javaExec: String,
  ...
  private val process =
    new ProcessBuilder(
      javaExec,
      "-classpath",
      scala.util.Properties.javaClassPath,
  ...
  locally {
    val task = new TimerTask:
      def run(): Unit =
        if !isAlive then cancellationTimer.cancel()
        else
