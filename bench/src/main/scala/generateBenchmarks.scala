...
def generateBenchmarks(genDirName: String) =
  ...
  def generateBenchmark(
    ...
    if !Files.exists(file) ||
      Files.getLastModifiedTime(file).toMillis() <
        Files.getLastModifiedTime(thisFile).toMillis()
    then
  ...
  generateBenchmark(
    ...
    () =>
      val innerSum =
        (1 to 50) // Limited to 50 to avoid stackoverflows in the compiler.
          .map(i => f"$i")
          .mkString(" + ")
...
object Test:
  ...
  generateBenchmark(
    "compiletime-ops",
    "sum-applications.scala",
    () =>
      def makeOp(depth: Int, last: Boolean, k: Int): String =
        if depth == 0 then
          f"Op[one.type, ${if last then k.toString else "n.type"}]"
        else
          f"Op[${makeOp(depth - 1, false, k)}, ${makeOp(depth - 1, last, k)}]"
