...
class ExtractAPISpecification:
  ...
  def extractNestedClasses =
    ...
    val compilerForTesting = new ScalaCompilerForUnitTesting
    val apis =
      compilerForTesting.extractApiFromSrc(src).map(c => c.name -> c).toMap
  ...
  /** Checks if representation of the inherited Namer class (with a declared
    * self variable) in Global.Foo is stable between compiling from source and
    * unpickling. We compare extracted APIs of Global when Global is compiled
    * together with Namers or Namers is compiled first and then Global refers to
    * Namers by unpickling types from class files.
    */
  @Ignore
  ...
  def extractDifferentRepresentationForAnInheritedClass =
    ...
    val compilerForTesting = new ScalaCompilerForUnitTesting
    val apis =
      compilerForTesting.extractApiFromSrc(src).map(a => a.name -> a).toMap
  ...
  def handlePackageObjectsAndTypeCompanions =
    ...
    assertEquals(
      Set(
        "abc.package",
        "abc.package$.BuildInfoKey",
        "abc.package$.BuildInfoKey$.Entry"
      ),
      apis.keySet
    )

  /** Checks if self type is properly extracted in various cases of declaring a
    * self type with or without a self variable.
    */
  @Ignore
  def representASelfTypeCorrectly =
    ...
    assertEquals(
      Set("X", "Y", "C1", "C2", "C8"),
      withoutSelfType.map(_.name).toSet
    )
  end representASelfTypeCorrectly
end ExtractAPISpecification
