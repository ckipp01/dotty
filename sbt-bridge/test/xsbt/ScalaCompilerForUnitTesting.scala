/** Adapted from
  * https://github.com/sbt/sbt/blob/0.13/compile/interface/src/test/scala/xsbt/ScalaCompilerForUnitTesting.scala
  */
package xsbt
...
class ScalaCompilerForUnitTesting:
  ...
  /** Compiles given source code using Scala compiler and returns API
    * representation extracted by ExtractAPI class.
    */
  ...
  /** Extract used names from src provided as the second argument. If
    * `assertDefaultScope` is set to true it will fail if there is any name used
    * in scope other then Default
    *
  ...
  def extractUsedNamesFromSrc(
    ...
    val (Seq(_, tempSrcFile), analysisCallback) =
      compileSrcs(definitionSrc, actualSrc)

    if assertDefaultScope then
      for
    ...
    classesInActualSrc
      .map(className => className -> analysisCallback.usedNames(className))
      .toMap
  end extractUsedNamesFromSrc
  ...
  /** Compiles given source code snippets (passed as Strings) using Scala
    * compiler and returns extracted dependencies between snippets. Source code
    * snippets are identified by symbols. Each symbol should be associated with
    * one snippet only.
    *
    * Snippets can be grouped to be compiled together in the same compiler run.
    * This is useful to compile macros, which cannot be used in the same
    * compilation run that defines them.
    *
    * Symbols are used to express extracted dependencies between source code
    * snippets. This way we have file system-independent way of testing
    * dependencies between source code "files".
    */
  def extractDependenciesFromSrcs(
  ...
  end extractDependenciesFromSrcs

  def extractDependenciesFromSrcs(srcs: String*): ExtractedClassDependencies =
    extractDependenciesFromSrcs(List(srcs.toList))

  /** Compiles given source code snippets written to temporary files. Each
    * snippet is written to a separate temporary file.
    *
    * Snippets can be grouped to be compiled together in the same compiler run.
    * This is useful to compile macros, which cannot be used in the same
    * compilation run that defines them.
    *
    * The sequence of temporary files corresponding to passed snippets and
    * analysis callback is returned as a result.
    */
  def compileSrcs(groupedSrcs: List[List[String]]): (Seq[File], TestCallback) =
    ...
    val files =
      for ((compilationUnit, unitId) <- groupedSrcs.zipWithIndex) yield
        val srcFiles = compilationUnit.toSeq.zipWithIndex.map { (src, i) =>
          val fileName = s"Test-$unitId-$i.scala"
          prepareSrcFile(temp, fileName, src)
        }

        val virtualSrcFiles =
          srcFiles.map(file => TestVirtualFile(file.toPath)).toArray
  ...
  private def prepareSrcFile(
    ...
    srcFile
end ScalaCompilerForUnitTesting
