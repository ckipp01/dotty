...
object SourceCode:
  ...
  )(syntaxHighlight: SyntaxHighlight, fullNames: Boolean): String =
    new SourceCodePrinter[quotes.type](syntaxHighlight, fullNames)
      .printTree(tree)
      .result()
  ...
  def showSymbol(using Quotes)(symbol: quotes.reflect.Symbol)(
      syntaxHighlight: SyntaxHighlight
  ): String =
    symbol.fullName

  def showFlags(using Quotes)(flags: quotes.reflect.Flags)(
      syntaxHighlight: SyntaxHighlight
  ): String =
    import quotes.reflect.*
    val flagList = List.newBuilder[String]
    if flags.is(Flags.Abstract) then flagList += "abstract"
    if flags.is(Flags.Artifact) then flagList += "artifact"
    if flags.is(Flags.Case) then flagList += "case"
    if flags.is(Flags.CaseAccessor) then flagList += "caseAccessor"
    if flags.is(Flags.Contravariant) then flagList += "contravariant"
    if flags.is(Flags.Covariant) then flagList += "covariant"
    if flags.is(Flags.Deferred) then flagList += "deferred"
    if flags.is(Flags.Enum) then flagList += "enum"
    if flags.is(Flags.Erased) then flagList += "erased"
    if flags.is(Flags.Exported) then flagList += "exported"
    if flags.is(Flags.ExtensionMethod) then flagList += "extension"
    if flags.is(Flags.FieldAccessor) then flagList += "accessor"
    if flags.is(Flags.Final) then flagList += "final"
    if flags.is(Flags.HasDefault) then flagList += "hasDefault"
    if flags.is(Flags.Implicit) then flagList += "implicit"
    if flags.is(Flags.Infix) then flagList += "infix"
    if flags.is(Flags.Inline) then flagList += "inline"
    if flags.is(Flags.JavaDefined) then flagList += "javaDefined"
    if flags.is(Flags.JavaStatic) then flagList += "static"
    if flags.is(Flags.Lazy) then flagList += "lazy"
    if flags.is(Flags.Local) then flagList += "local"
    if flags.is(Flags.Macro) then flagList += "macro"
    if flags.is(Flags.Method) then flagList += "method"
    if flags.is(Flags.Module) then flagList += "object"
    if flags.is(Flags.Mutable) then flagList += "mutable"
    if flags.is(Flags.NoInits) then flagList += "noInits"
    if flags.is(Flags.Override) then flagList += "override"
    if flags.is(Flags.Package) then flagList += "package"
    if flags.is(Flags.Param) then flagList += "param"
    if flags.is(Flags.ParamAccessor) then flagList += "paramAccessor"
    if flags.is(Flags.Private) then flagList += "private"
    if flags.is(Flags.PrivateLocal) then flagList += "private[this]"
    if flags.is(Flags.Protected) then flagList += "protected"
    if flags.is(Flags.Scala2x) then flagList += "scala2x"
    if flags.is(Flags.Sealed) then flagList += "sealed"
    if flags.is(Flags.StableRealizable) then flagList += "stableRealizable"
    if flags.is(Flags.Synthetic) then flagList += "synthetic"
    if flags.is(Flags.Trait) then flagList += "trait"
    if flags.is(Flags.Transparent) then flagList += "transparent"
    flagList.result().mkString("/*", " ", "*/")
  end showFlags

  private class SourceCodePrinter[Q <: Quotes & Singleton](
      syntaxHighlight: SyntaxHighlight,
      fullNames: Boolean
  )(using val quotes: Q):
    ...
    def printTree(tree: Tree)(using
        elideThis: Option[Symbol] = None
    ): this.type = tree match
      case PackageObject(body) =>
        printTree(body) // Print package object

      case PackageClause(Ident(name), (inner @ PackageClause(_, _)) :: Nil)
          if name != "<empty>" && PackageObject.unapply(inner).isEmpty =>
      ...
      case tree @ PackageClause(name, stats) =>
        val stats1 = stats.collect {
          ...
          case stat: Definition
              if !(stat.symbol.flags
                .is(Flags.Module) && stat.symbol.flags.is(Flags.Lazy)) =>
            stat
      ...
      case cdef @ ClassDef(
        ...
        if flags.is(Flags.Implicit) then this += highlightKeyword("implicit ")
        if flags.is(Flags.Sealed) then this += highlightKeyword("sealed ")
        if flags.is(Flags.Final) && !flags.is(Flags.Module) then
          this += highlightKeyword("final ")
        if flags.is(Flags.Case) then this += highlightKeyword("case ")

        if name == "package$" then
          this += highlightKeyword("package object ") += highlightTypeDef(
            cdef.symbol.owner.name.stripSuffix("$")
          )
        else if flags.is(Flags.Module) then
          this += highlightKeyword("object ") += highlightTypeDef(
            name.stripSuffix("$")
          )
        else if flags.is(Flags.Trait) then
          this += highlightKeyword("trait ") += highlightTypeDef(name)
        else if flags.is(Flags.Abstract) then
          this += highlightKeyword("abstract class ") += highlightTypeDef(name)
          ...
          for paramClause <- paramss do
            paramClause match
              ...
              case TypeParamClause(params) =>
                printTargsDefs(
                  stats
                    .collect { case targ: TypeDef => targ }
                    .filter(_.symbol.isTypeParam)
                    .zip(params)
                )

        val parents1 = parents.filter {
          case Apply(Select(New(tpt), _), _) =>
            tpt.tpe.typeSymbol != Symbol.requiredClass("java.lang.Object")
          case TypeSelect(Select(Ident("_root_"), "scala"), "Product") => false
          case TypeSelect(Select(Ident("_root_"), "scala"), "Serializable") =>
            false
          case _ => true
        }
        if parents1.nonEmpty then this += highlightKeyword(" extends ")

        def printParent(
            parent: Tree /* Term | TypeTree */,
            needEmptyParens: Boolean = false
        ): Unit = parent match
          ...
          case Apply(fun @ Apply(_, _), args) =>
            ...
            if !args.isEmpty || needEmptyParens then
        ...
        def printSeparated(list: List[Tree /* Term | TypeTree */ ]): Unit =
          list match
            case Nil      =>
        ...
        def keepDefinition(d: Definition): Boolean =
          ...
          def isUndecompilableCaseClassMethod: Boolean =
            ...
            d.symbol.flags.is(Flags.Synthetic) &&
              (d match
                case DefDef("apply" | "unapply" | "writeReplace", _, _, _)
                    if d.symbol.owner.flags.is(Flags.Module) =>
                  true
                case DefDef(n, _, _, _)
                    if d.symbol.owner.flags.is(Flags.Case) =>
                  n == "copy" ||
                  n.matches(
                    "copy\\$default\\$[1-9][0-9]*"
                  ) || // default parameters for the copy method
                  n.matches("_[1-9][0-9]*") || // Getters from Product
                  n == "productElementName"
                case _ => false
              )
          def isInnerModuleObject =
            d.symbol.flags.is(Flags.Lazy) && d.symbol.flags.is(Flags.Module)
          !flags.is(Flags.Param) && !flags.is(Flags.ParamAccessor) && !flags.is(
            Flags.FieldAccessor
          ) && !isUndecompilableCaseClassMethod && !isInnerModuleObject
        val stats1 = stats.collect {
        ...
        def printBody(printSelf: Boolean) =
          ...
          indented {
            if printSelf then
              ...
              indented {
                val name1 = if name == "_" then "this" else name
        ...
        self match
          case Some(ValDef(_, Singleton(_), _)) =>
            if stats1.nonEmpty then printBody(printSelf = false)
      ...
      case vdef @ ValDef(name, tpt, rhs) =>
        ...
        if flags.is(Flags.Implicit) then this += highlightKeyword("implicit ")
        if flags.is(Flags.Override) then this += highlightKeyword("override ")
        if flags.is(Flags.Final) && !flags.is(Flags.Module) then
          this += highlightKeyword("final ")

        printProtectedOrPrivate(vdef)

        if flags.is(Flags.Lazy) then this += highlightKeyword("lazy ")
        if vdef.symbol.flags.is(Flags.Mutable) then
          this += highlightKeyword("var ")
      ...
      case While(cond, body) =>
        (cond, body) match
          case (
                Block(Block(Nil, body1) :: Nil, Block(Nil, cond1)),
      ...
      case ddef @ DefDef(name, paramss, tpt, rhs) =>
        ...
        if flags.is(Flags.Implicit) then this += highlightKeyword("implicit ")
        if flags.is(Flags.Inline) then this += highlightKeyword("inline ")
        if flags.is(Flags.Override) then this += highlightKeyword("override ")
        if flags.is(Flags.Final) && !flags.is(Flags.Module) then
          this += highlightKeyword("final ")
        ...
        val name1: String =
          ...
          else splicedName(ddef.symbol).getOrElse(name)
      ...
      case Select(qual, name) =>
        printQualTree(qual)
        if name != "<init>" && name != "package" then this += "." += name
        this
      ...
      case Apply(fn, args)
          if fn.symbol == Symbol.requiredMethod("scala.quoted.runtime.quote") =>
        args.head match
      ...
      case Apply(fn, arg :: Nil)
          if fn.symbol == Symbol.requiredMethod(
            "scala.quoted.runtime.splice"
          ) =>
        this += "${"
        printTree(arg)
        this += "}"
      ...
      case Apply(fn, args) =>
        var argsPrefix = ""
        fn match
          case Select(This(_), "<init>") =>
            this += "this" // call to constructor inside a constructor
          case Select(qual, "apply") =>
            if qual.tpe.isContextFunctionType then argsPrefix += "using "
            if qual.tpe.isErasedFunctionType then argsPrefix += "erased "
            printQualTree(fn)
          case _ => printQualTree(fn)
        val args1 = args match
          case init :+ Typed(Repeated(Nil, _), _) =>
            init // drop empty var args at the end
          case _ => args
      ...
      case Super(qual, idOpt) =>
        qual match
          ...
          case This(None)       =>
        this += "super"
        for id <- idOpt do inSquare(this += id)
        this
      ...
      case Typed(term, tpt) =>
        tpt.tpe match
          ...
          case _ =>
            inParens {
              printTree(term)
              this += (if dotty.tools.dotc.util.Chars.isOperatorPart(sb.last)
                       then " : "
                       else ": ")
    ...
    private def flatBlock(
      ...
      def extractFlatStats(stat: Statement): Unit = stat match
        ...
        case Block(stats1, expr1) =>
          ...
          while it.hasNext do extractFlatStats(it.next())
    ...
    private def printFlatBlock(stats: List[Statement], expr: Term)(using
        elideThis: Option[Symbol]
    ): this.type =
      val (stats1, expr1) = flatBlock(stats, expr)
      val splicedTypeAnnot = Symbol
        .requiredClass("scala.quoted.runtime.SplicedType")
        .primaryConstructor
    ...
    private def printStats(stats: List[Tree], expr: Tree)(using
      ...
      def printSeparator(next: Tree): Unit =
        ...
        def rec(next: Tree): Unit = next match
          ...
          case Inlined(_, bindings, _) if bindings.nonEmpty =>
            this += doubleLineBreak()
      ...
      def printSeparated(list: List[Tree]): Unit = list match
        ...
        case x :: xs =>
          printTree(x)
          printSeparator(if xs.isEmpty then expr else xs.head)
          printSeparated(xs)
    ...
    private def printTrees(trees: List[Tree], sep: String)(using
        elideThis: Option[Symbol]
    ): this.type =
      printList(trees, sep, (t: Tree) => printTree(t))

    private def printTypeTrees(trees: List[TypeTree], sep: String)(using
        elideThis: Option[Symbol] = None
    ): this.type =
      printList(trees, sep, (t: TypeTree) => printTypeTree(t))

    private def printTypes(trees: List[TypeRepr], sep: String)(using
        elideThis: Option[Symbol]
    ): this.type =
    ...
    private def printTypesOrBounds(types: List[TypeRepr], sep: String)(using
        elideThis: Option[Symbol]
    ): this.type =
      def printSeparated(list: List[TypeRepr]): Unit = list match
        case Nil      =>
    ...
    private def printTargsDefs(
        targs: List[(TypeDef, TypeDef)],
        isDef: Boolean = true
    )(using elideThis: Option[Symbol]): Unit =
    ...
    private def printTargDef(
        arg: (TypeDef, TypeDef),
        isMember: Boolean = false,
        isDef: Boolean = true
    )(using elideThis: Option[Symbol]): this.type =
      val (argDef, argCons) = arg

      if isDef then
        if argDef.symbol.flags.is(Flags.Covariant) then
          this += highlightValDef("+")
        else if argDef.symbol.flags.is(Flags.Contravariant) then
          this += highlightValDef("-")
    ...
    private def printArgsDefs(args: List[ValDef])(using
        elideThis: Option[Symbol]
    ): Unit =
      val argFlags = args match
        case Nil      => Flags.EmptyFlags
        case arg :: _ => arg.symbol.flags
      if argFlags.is(Flags.Erased | Flags.Given) then
        if argFlags.is(Flags.Given) then this += " given"
        if argFlags.is(Flags.Erased) then this += " erased"
        this += " "
      inParens {
        if argFlags.is(Flags.Implicit) && !argFlags.is(Flags.Given) then
          this += "implicit "
    ...
    private def printAnnotations(trees: List[Term])(using
        elideThis: Option[Symbol]
    ): this.type =
    ...
    private def printParamDef(arg: ValDef)(using
        elideThis: Option[Symbol]
    ): Unit =
      ...
      if sym.isDefDef && sym.name == "<init>" then
        ...
        body.collectFirst {
          case vdef @ ValDef(`name`, _, _)
              if vdef.symbol.flags.is(Flags.ParamAccessor) =>
            if !vdef.symbol.flags.is(Flags.Local) then
              var printedPrefix = false
              if vdef.symbol.flags.is(Flags.Override) then
                this += "override "
                printedPrefix = true
              printedPrefix |= printProtectedOrPrivate(vdef)
              if vdef.symbol.flags.is(Flags.Mutable) then
                this += highlightValDef("var ")
              else if printedPrefix || !vdef.symbol.flags.is(Flags.CaseAccessor)
              then this += highlightValDef("val ")
        }
      end if
    ...
    def printConstant(const: Constant): this.type = const match
      ...
      case CharConstant(v) =>
        this += highlightString(s"${qc}${escapedChar(v)}${qc}")
      case StringConstant(v) =>
        this += highlightString(s"${qSc}${escapedString(v)}${qSc}")
      case ClassOfConstant(v) =>
        this += "classOf"
        inSquare(printType(v))

    private def printTypeOrBoundsTree(tpt: Tree)(using
        elideThis: Option[Symbol] = None
    ): this.type = tpt match
      case TypeBoundsTree(lo, hi) =>
        this += "_ >: "
      ...
      case Inferred() =>
        ...
        def printTypeAndAnnots(tpe: TypeRepr): this.type = tpe match
          case AnnotatedType(tp, annot) =>
            ...
            printAnnotation(annot)
          case tpe: TypeRef
              if tpe.typeSymbol == Symbol.requiredClass(
                "scala.runtime.Null$"
              ) || tpe.typeSymbol == Symbol.requiredClass(
                "scala.runtime.Nothing$"
              ) =>
            // scala.runtime.Null$ and scala.runtime.Nothing$ are not modules, those are their actual names
            printType(tpe)
          case tpe: TermRef
              if tpe.termSymbol.isClassDef && tpe.termSymbol.name.endsWith(
                "$"
              ) =>
            printType(tpe)
            this += ".type"
          case tpe: TypeRef
              if tpe.typeSymbol.isClassDef && tpe.typeSymbol.name.endsWith(
                "$"
              ) =>
            printType(tpe)
            this += ".type"
    ...
    /** Print type
      ...
      * prefix `C.this` to avoid type checking errors.
      */
    def printType(tpe: TypeRepr)(using
        elideThis: Option[Symbol] = None
    ): this.type = tpe match
      ...
      case tpe: TypeRef =>
        ...
        if fullNames then
          tpe.qualifier match
            case ThisType(tp)
                if tp.typeSymbol == defn.RootClass || tp.typeSymbol == defn.EmptyPackageClass =>
            case NoPrefix() =>
              if sym.owner.flags.is(Flags.Package) then
                // TODO should these be in the prefix? These are at least `scala`, `java` and `scala.collection`.
                val packagePath = sym.owner.fullName
                  .stripPrefix("<root>")
                  .stripPrefix("<empty>")
                  .stripPrefix(".")
                if packagePath != "" then this += packagePath += "."
            ...
            case prefix: TypeRef if prefix.typeSymbol.isClassDef =>
              printType(prefix)
              this += "#"
            case ThisType(TermRef(cdef, _))
                if elideThis.nonEmpty && cdef == elideThis.get =>
            case ThisType(TypeRef(cdef, _))
                if elideThis.nonEmpty && cdef == elideThis.get =>
      ...
      case TermRef(prefix, name) =>
        if fullNames then
          prefix match
            ...
            case _ =>
              printType(prefix)
              if name != "package" then this += "." += highlightTypeDef(name)
              this
      ...
      case AppliedType(tp, args) =>
        tp match
          ...
          case tp: TypeRef
              if tp.typeSymbol == Symbol.requiredClass("scala.<repeated>") =>
            this += "_*"
          case _ =>
            printType(tp)
            inSquare(printTypesOrBounds(args, ", "))
      ...
      case tpe: MethodType =>
        this += "("
        printList(
          tpe.paramNames.zip(tpe.paramTypes),
          ", ",
      ...
      case tpe: PolyType =>
        this += "["
        printList(
          tpe.paramNames.zip(tpe.paramBounds),
          ", ",
    ...
    private def printAnnotation(annot: Term)(using
        elideThis: Option[Symbol]
    ): this.type =
      val Annotation(ref, args) = annot: @unchecked
      this += "@"
      ...
      else inParens(printTrees(args, ", "))

    private def printDefAnnotations(definition: Definition)(using
        elideThis: Option[Symbol]
    ): this.type =
      ...
      if annots.nonEmpty then this += " "
      else this

    private def printRefinement(tpe: TypeRepr)(using
        elideThis: Option[Symbol]
    ): this.type =
    ...
    private def printMethodicTypeParams(
    ...
    private def printBounds(bounds: TypeBounds)(using
        elideThis: Option[Symbol]
    ): this.type =
      this += " >: "
    ...
    private def +=(x: Boolean): this.type =
      sb.append(x); this
    private def +=(x: Byte): this.type =
      sb.append(x); this
    private def +=(x: Short): this.type =
      sb.append(x); this
    private def +=(x: Int): this.type =
      sb.append(x); this
    private def +=(x: Long): this.type =
      sb.append(x); this
    private def +=(x: Float): this.type =
      sb.append(x); this
    private def +=(x: Double): this.type =
      sb.append(x); this
    private def +=(x: Char): this.type =
      sb.append(x); this
    private def +=(x: String): this.type =
      sb.append(x); this
    ...
    private def escapedChar(ch: Char): String = (ch: @switch) match
      ...
      case '\\' => "\\\\"
      case _ =>
        if ch.isControl then f"${"\\"}u${ch.toInt}%04x"
        else String.valueOf(ch).nn
    ...
    private object SpecialOp:
      def unapply(arg: Tree): Option[(String, List[Term])] = arg match
        case arg @ Apply(fn, args) =>
          fn.tpe match
            case tpe @ TermRef(ThisType(TypeRef(_, name)), name2)
                if name == "<special-ops>" =>
              Some((name2, args))
            case _ => None
        case _ => None
    ...
    private object Annotation:
      def unapply(arg: Tree): Option[(TypeTree, List[Term])] = arg match
        ...
        case Apply(TypeApply(Select(New(annot), "<init>"), targs), args) =>
          Some((annot, args))
        case _ => None
    ...
    private object Types:
      ...
      object Sequence:
        def unapply(tpe: TypeRepr): Option[TypeRepr] = tpe match
          case AppliedType(seq, (tp: TypeRepr) :: Nil)
              if seq.typeSymbol == Symbol.requiredClass(
                "scala.collection.Seq"
              ) || seq.typeSymbol == Symbol.requiredClass(
      ...
      object Repeated:
        def unapply(tpe: TypeRepr): Option[TypeRepr] = tpe match
          case AppliedType(rep, (tp: TypeRepr) :: Nil)
              if rep.typeSymbol == Symbol.requiredClass("scala.<repeated>") =>
            Some(tp)
          case _ => None

    private object PackageObject:
      def unapply(tree: Tree): Option[Tree] = tree match
        case PackageClause(_, ValDef("package", _, _) :: body :: Nil) =>
          Some(body)
        case _ => None
  end SourceCodePrinter
end SourceCode
