...
object NoScope extends Scope:
  ...
  override def outer: Scope = throw UnsupportedOperationException(
    "NoScope.outer"
  )

class SpliceScope(val pos: SourcePosition, override val outer: Scope)
    extends Scope:
...
object SpliceScope:
  ...
  /** A key to be used in a context property that tracks current splices we are
    * evaluating
    */
