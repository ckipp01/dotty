...
/** Matches a quoted tree against a quoted pattern tree. A quoted pattern tree
  * may have type and term holes in addition to normal terms.
  *
  * Semantics:
  *
  * We use `'{..}` for expression, `'[..]` for types and `⟨..⟩` for patterns
  * nested in expressions. The semantics are defined as a list of reduction
  * rules that are tried one by one until one matches.
  *
  * Operations:
  *   - `s =?= p` checks if a scrutinee `s` matches the pattern `p` while
  *     accumulating extracted parts of the code.
  *   - `isClosedUnder(x1, .., xn)('{e})` returns true if and only if all the
  *     references in `e` to names defined in the pattern are contained in the
  *     set `{x1, ... xn}`.
  *   - `lift(x1, .., xn)('{e})` returns `(y1, ..., yn) => [xi = $yi]'{e}` where
  *     `yi` is an `Expr` of the type of `xi`.
  *   - `withEnv(x1 -> y1, ..., xn -> yn)(matching)` evaluates matching
  *     recording that `xi` is equivalent to `yi`.
  *   - `matched` denotes that the the match succeeded and `matched('{e})`
  *     denotes that a match succeeded and extracts `'{e}`
  *   - `&&&` matches if both sides match. Concatenates the extracted
  *     expressions of both sides.
  *
...
object QuoteMatcher:
  ...
  /** Sequence of matched expressions. These expressions are part of the
    * scrutinee and will be bound to the quote pattern term splices.
    */
  type MatchingExprs = Seq[MatchResult]

  /** A map relating equivalent symbols from the scrutinee and the pattern For
    * example in
    * ```
    * '{val a = 4; a * a} match case '{ val x = 4; x * x }
    * ```
    * when matching `a * a` with `x * x` the environment will contain `Map(a ->
    * x)`.
    */
  ...
  /** Evaluate the result of pattern matching against a quote pattern.
    * Implementation of the runtime of `QuoteMatching.{ExprMatch,
    * TypeMatch}.unapply`.
    */
    ...
    inContext(ctx1) {
      optional {
        ...
        val matchedTypes = typeHoleApproximations.map(tpe =>
          new TypeImpl(TypeTree(tpe), spliceScope)
        )
        val matchedExprs =
          val typeHoleMap: Type => Type =
            if typeHoles.isEmpty then identity
              ...
              new TypeMap:
                private val typeHoleMapping =
                  Map(typeHoles.zip(typeHoleApproximations)*)
                def apply(tp: Type): Type = tp match
                  case TypeRef(NoPrefix, _) =>
                    typeHoleMapping.getOrElse(tp.typeSymbol, tp)
                  case _ => mapOver(tp)
  ...
  def instrumentTypeHoles(pat: Tree)(using
      Context
  ): (Tree, List[Symbol], Context) =
    ...
    pat match
      ...
      case tpd.Block(stats @ ((typeHole: TypeDef) :: _), expr)
          if isTypeHoleDef(typeHole) =>
  ...
  /** Type approximation of a quote pattern type variable. Should only be
    * approximated after matching the tree.
    */
  def typeHoleApproximation(sym: Symbol)(using Context): Type =
    val fromAboveAnnot =
      sym.hasAnnotation(defn.QuotedRuntimePatterns_fromAboveAnnot)
  ...
  /** Check that all trees match with `mtch` and concatenate the results with
    * &&&
    */
  private def matchLists[T](l1: List[T], l2: List[T])(
      mtch: (T, T) => MatchingExprs
  ): optional[MatchingExprs] = (l1, l2) match
  ...
  extension (scrutinees: List[Tree])
    private def =?=(
        patterns: List[Tree]
    )(using Env, Context): optional[MatchingExprs] =
      matchLists(scrutinees, patterns)(_ =?= _)

  extension (scrutinee0: Tree)

    /** Check that the trees match and return the contents from the pattern
      * holes. Return a sequence containing all the contents in the holes. If it
      * does not match, continues to the `optional` with `None`.
      *
      * @param scrutinee
      *   The tree being matched
      * @param pattern
      *   The pattern tree that the scrutinee should match. Contains
      *   `patternHole` holes.
      * @param `summon[Env]`
      *   Set of tuples containing pairs of symbols (s, p) where s defines a
      *   symbol in `scrutinee` which corresponds to symbol p in `pattern`.
      * @return
      *   The sequence with the contents of the holes of the matched expression.
      */
    private def =?=(
        pattern0: Tree
    )(using Env, Context): optional[MatchingExprs] =
      ...
      def checkValFlags(): Boolean =
        ...
        sFlags.is(Lazy) == pFlags.is(Lazy) && sFlags.is(Mutable) == pFlags.is(
          Mutable
        )
      ...
      val res = pattern match
        ...
        case Apply(
              TypeApply(Ident(_), List(TypeTree())),
              SeqLiteral(args, _) :: Nil
            )
          ...
          withEnv(captureEnv) {
            scrutinee match
              case ClosedPatternTerm(scrutinee) =>
                matchedOpen(scrutinee, pattern.tpe, args, env)
              case _ => notMatched
          }
        ...
        case _ =>
          scrutinee match
              ...
              pattern match
                ...
                case Select(qual2, _) if symbolMatch(scrutinee, pattern) =>
                  ref match
                    ...
                    case ref: Ident =>
                      if qual2.existsSubTree(
                          _.symbol == defn.QuotedRuntimePatterns_patternHole
                        )
                      then
                ...
                /* Match type */
                case TypeTreeTypeTest(pattern)
                    if scrutinee.tpe <:< pattern.tpe =>
                  matched
                case _ => notMatched
                ...
                case Block(stat2 :: stats2, expr2) =>
                  val newEnv = (stat1, stat2) match
                    ...
                    case _ =>
                      summon[Env]
                  withEnv(newEnv) {
                    stat1 =?= stat2 &&& Block(stats1, expr1) =?= Block(
                      stats2,
                      expr2
                    )
                  }
            ...
            case New(tpt1) =>
              pattern match
                case New(tpt2) if tpt1.tpe.typeSymbol == tpt2.tpe.typeSymbol =>
                  matched
                case _ => notMatched
            ...
            case SeqLiteral(elems1, _) =>
              pattern match
                case SeqLiteral(elems2, _) if elems1.size == elems2.size =>
                  elems1 =?= elems2
                case _ => notMatched
                ...
                case pattern @ DefDef(_, paramss2, tpt2, _) =>
                  def rhsEnv: Env =
                    val paramSyms: List[(Symbol, Symbol)] =
                      ...
                      yield param1.symbol -> param2.symbol
                    val oldEnv: Env = summon[Env]
                    val newEnv: List[(Symbol, Symbol)] =
                      (scrutinee.symbol -> pattern.symbol) :: paramSyms
                    oldEnv ++ newEnv
            ...
            case Closure(_, _, tpt1) =>
              pattern match
                case Closure(_, _, tpt2) =>
                  matched // TODO match tpt1 with tpt2?
                case _ => notMatched
      ...
      if debug && res == notMatched then
        ...
        println(s""">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
              ...
              |
              |Scrutinee: ${quotes.reflect.Printer.TreeStructure
                    .show(scrutinee.asInstanceOf)}
              |Pattern: ${quotes.reflect.Printer.TreeStructure.show(
  ...
  /** Does the scrutinee symbol match the pattern symbol? It matches if:
    ...
    */
  private def symbolMatch(scrutineeTree: Tree, patternTree: Tree)(using
      Env,
      Context
  ): Boolean =
  ...
  private object ClosedPatternTerm:
    /** Matches a term that does not contain free variables defined in the
      * pattern (i.e. not defined in `Env`)
      */
    def unapply(term: Tree)(using Env, Context): Option[term.type] =
      if freePatternVars(term).isEmpty then Some(term) else None

    /** Return all free variables of the term defined in the pattern (i.e.
      * defined in `Env`)
      */
  ...
  enum MatchResult:
    /** Closed pattern extracted value
      * @param tree
      *   Scrutinee sub-tree that matched
      */
    ...
    /** HOAS pattern extracted value
      ...
      *   Scrutinee sub-tree that matched
      * @param patternTpe
      *   Type of the pattern hole (from the pattern)
      * @param args
      *   HOAS arguments (from the pattern)
      * @param env
      *   Mapping between scrutinee and pattern variables
      */
    case OpenTree(tree: Tree, patternTpe: Type, args: List[Tree], env: Env)

    /** Return the expression that was extracted from a hole.
      *
      * If it was a closed expression it returns that expression. Otherwise, if
      * it is a HOAS pattern, the surrounding lambda is generated using
      * `mapTypeHoles` to create the signature of the lambda.
      *
      * This expression is assumed to be a valid expression in the given splice
      * scope.
      */
    def toExpr(mapTypeHoles: Type => Type, spliceScope: Scope)(using
        Context
    ): Expr[Any] = this match
      ...
      case MatchResult.OpenTree(tree, patternTpe, args, env) =>
        val names: List[TermName] = args.map {
          case Block(
                List(DefDef(nme.ANON_FUN, _, _, Apply(Ident(name), _))),
                _
              ) =>
            name.asTermName
        ...
        def bodyFn(lambdaArgss: List[List[Tree]]): Tree =
          ...
          val body = new TreeMap:
            override def transform(tree: Tree)(using Context): Tree =
              tree match
                case tree: Ident =>
                  env.get(tree.symbol).flatMap(argsMap.get).getOrElse(tree)
  ...
  private def matchedOpen(
      tree: Tree,
      patternTpe: Type,
      args: List[Tree],
      env: Env
  )(using Context): MatchingExprs =
