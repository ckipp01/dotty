...
class ExtractDependencies extends Phase:
  ...
  override def run(using Context): Unit =
    ...
    if ctx.settings.YdumpSbtInc.value then
      ...
      val names = collector.usedNames
        .map { case (clazz, names) => s"$clazz: $names" }
        .toArray[Object]
      Arrays.sort(deps)
      Arrays.sort(names)

      val pw = io
        .File(unit.source.file.jpath)
        .changeExtension("inc")
        .toFile
        .printWriter()
      // val pw = Console.out
    ...
    if ctx.sbtCallback != null then
      collector.usedNames.foreach { case (clazz, usedNames) =>
        val className = classNameAsString(clazz)
        usedNames.names.foreach { case (usedName, scopes) =>
  ...
  def recordDependency(dep: ClassDependency)(using Context): Unit =
    ...
    def processExternalDependency(
      ...
      depFile match
        ...
        case pf: PlainFile => // The dependency comes from a class file
          ...
          if pf.file != null then binaryDependency(pf.file, binaryClassName)
        case _ =>
          internalError(
            s"Ignoring dependency $depFile of unknown class ${depFile.getClass}}",
            dep.from.srcPos
          )
    ...
    if depFile != null then
      ...
      def allowLocal =
        dep.context == DependencyByInheritance || dep.context == LocalDependencyByInheritance
      if depFile.extension == "class" then
        // Dependency is external -- source is undefined
        processExternalDependency(depFile, dep.to.binaryClassName)
      else if allowLocal || depFile.file != sourceFile then
...
object ExtractDependencies:
  ...
  def internalError(msg: => String, pos: SrcPos = NoSourcePosition)(using
      Context
  ): Unit =
    report.error(
      em"Internal error in the incremental compiler while compiling ${ctx.compilationUnit.source}: $msg",
      pos
    )

private case class ClassDependency(
    from: Symbol,
    to: Symbol,
...
/** Extract the dependency information of a compilation unit.
  ...
  * algorithm" in
  * http://www.scala-sbt.org/0.13/docs/Understanding-Recompilation.html To
  * understand why we need to track dependencies introduced by inheritance
  * specially, see the subsection "Dependencies introduced by member reference
  * and inheritance" in the "Name hashing algorithm" section.
  */
private class ExtractDependenciesCollector extends tpd.TreeTraverser:
  thisTreeTraverser =>
  import tpd.*
  ...
  /** Top level import dependencies are registered as coming from a first top
    * level class/trait/object declared in the compilation unit. If none exists,
    * issue warning.
    */
  ...
  private def responsibleForImports(using Context) =
    ...
    if _responsibleForImports == null then
      val tree = ctx.compilationUnit.tpdTree
      _responsibleForImports = firstClassOrModule(tree)
      if !_responsibleForImports.exists then
        report.warning(
  ...
  /** Resolves dependency source (that is, the closest non-local enclosing class
    * from a given `ctx.owner`
    */
  private def resolveDependencySource(using Context): Symbol =
    def nonLocalEnclosingClass =
      ...
      while !owner.is(PackageClass) do
    ...
    if lastOwner != ctx.owner then
      lastOwner = ctx.owner
      val source = nonLocalEnclosingClass
      lastDepSource =
        if source.is(PackageClass) then responsibleForImports else source

    lastDepSource

  private def addUsedName(
      fromClass: Symbol,
      name: Name,
      scope: UseScope
  ): Unit =
  ...
  private def addUsedName(name: Name, scope: UseScope)(using Context): Unit =
    ...
    if fromClass.exists then // can happen when visiting imports
      assert(fromClass.isClass)
      addUsedName(fromClass, name, scope)

  private def addMemberRefDependency(sym: Symbol)(using Context): Unit =
    if !ignoreDependency(sym) then
      val enclOrModuleClass =
        if sym.is(ModuleVal) then sym.moduleClass else sym.enclosingClass
  ...
  private def ignoreDependency(sym: Symbol)(using Context) =
    try
      !sym.exists ||
        sym.isAbsent(canForce =
          false
        ) || // ignore dependencies that have a symbol but do not exist.
  ...
  /** Traverse the tree of a source file and record the dependencies and used
    * names which can be retrieved using `dependencies` and`usedNames`.
    */
  ...
  /** Traverse a used type and record all the dependencies we need to keep track
    ...
    *
    * type T >: L <: H type L <: A1 type H <: B1 class A1 extends A0 class B1
    * extends B0
    *
    ...
    */
  private abstract class TypeDependencyTraverser(using Context)
    ...
    def traverse(tp: Type): Unit = if !seen.contains(tp) then
      seen += tp
      tp match
        case tp: NamedType =>
          val sym = tp.symbol
  ...
  def addPatMatDependency(tpe: Type)(using Context): Unit =
    val traverser = new TypeDependencyTraverser:
      def addDependency(symbol: Symbol) =
        if !ignoreDependency(symbol) && symbol.is(Sealed) then
