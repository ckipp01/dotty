...
object ShowAPI:
  ...
  private def truncateDecls(
      decls: Array[ClassDefinition]
  ): Array[ClassDefinition] =
    if numDecls <= 0 then decls else decls.take(numDecls)
  ...
  def showDefinition(d: Definition)(implicit nesting: Int): String = d match
    ...
    case d: Def =>
      showPolyDef(d, "def") + showValueParams(
        d.valueParameters.toIndexedSeq
      ) + ": " + showType(d.returnType)
    case ta: TypeAlias => showPolyDef(ta, "type") + " = " + showType(ta.tpe)
    case td: TypeDeclaration =>
      showPolyDef(td, "type") + showBounds(td.lowerBound, td.upperBound)
    case cl: ClassLike =>
      showMonoDef(d, showDefinitionType(cl.definitionType)) +
        showTypeParameters(
          cl.typeParameters.toIndexedSeq
        ) + " extends " + showTemplate(cl)
    case cl: ClassLikeDef =>
      showPolyDef(cl, showDefinitionType(cl.definitionType))
  ...
  private def showTemplate(cl: ClassLike)(implicit nesting: Int) =
    if nesting <= 0 then "<nesting level reached>"
      ...
      cl.structure.parents
        .map(showNestedType)
        .mkString("", " with ", " {") + showSelf +
        lines(
        ...
        lines(
          truncateDecls(cl.structure.declared).toIndexedSeq
  ...
  def showType(t: Type)(implicit nesting: Int): String = t match
    ...
    case p: Parameterized =>
      showType(p.baseType) + p.typeArguments
        .map(showType)
        .mkString("[", ", ", "]")
    case c: Constant => showType(c.baseType) + "(" + c.value + ")"
    case a: Annotated =>
      showAnnotations(a.annotations.toIndexedSeq) + " " + showType(a.baseType)
    case s: Structure =>
      s.parents.map(showType).mkString(" with ") + (
        if nesting <= 0 then "{ <nesting level reached> }"
        else
          truncateDecls(s.declared)
            .map(showNestedDefinition)
            .mkString(" {", "\n", "}")
      )
    case e: Existential =>
      showType(e.baseType) + (
        if nesting <= 0 then " forSome { <nesting level reached> }"
        else
          e.clause
            .map(t => "type " + showNestedTypeParameter(t))
            .mkString(" forSome { ", "; ", " }")
      )
    case p: Polymorphic =>
      showType(p.baseType) + (
  ...
  private def showPath(p: Path): String =
    p.components.map(showPathComponent).mkString(".")
  private def showPathComponent(pc: PathComponent) = pc match
    case s: Super => "super[" + showPath(s.qualifier) + "]"
    case _: This  => "this"
    case i: Id    => i.id

  private def space(s: String) = if s.isEmpty then s else s + " "
  private def showMonoDef(d: Definition, label: String)(implicit
      nesting: Int
  ): String =
    space(showAnnotations(d.annotations.toIndexedSeq)) + space(
      showAccess(d.access)
    ) + space(showModifiers(d.modifiers)) + space(label) + d.name
  ...
  private def showTypeParameters(
      tps: Seq[TypeParameter]
  )(implicit nesting: Int): String =
    if tps.isEmpty then ""
    else tps.map(showTypeParameter).mkString("[", ", ", "]")

  private def showTypeParameter(
      tp: TypeParameter
  )(implicit nesting: Int): String =
  ...
  private def showAnnotation(a: Annotation)(implicit nesting: Int) =
    "@" + showType(a.base) + (
      if a.arguments.isEmpty then ""
      else
        a.arguments.map(a => a.name + " = " + a.value).mkString("(", ", ", ")")
    )

  private def showBounds(lower: Type, upper: Type)(implicit
      nesting: Int
  ): String = ">: " + showType(lower) + " <: " + showType(upper)

  private def showValueParams(
      ps: Seq[ParameterList]
  )(implicit nesting: Int): String =
    ps.map(pl =>
      pl.parameters
        .map(mp =>
          mp.name + ": " + showParameterModifier(
            showType(mp.tpe),
            mp.modifier
          ) + (if mp.hasDefault then "= ..." else "")
        )
        .mkString(if pl.isImplicit then "(implicit " else "(", ", ", ")")
    ).mkString("")

  private def showParameterModifier(
      base: String,
  ...
  private def showNestedType(tp: Type)(implicit nesting: Int) =
    showType(tp)(nesting - 1)
  private def showNestedTypeParameter(tp: TypeParameter)(implicit
      nesting: Int
  ) = showTypeParameter(tp)(nesting - 1)
  private def showNestedTypeParameters(tps: Seq[TypeParameter])(implicit
      nesting: Int
