...
/** Create and hold thunks. A thunk is a (potentially) unevaluated value that
...
private[sbt] trait ThunkHolder:
  ...
  @tailrec protected final def forceThunks(): Unit = if !thunks.isEmpty then
  ...
  /** Store the by-name parameter `s` in a `Lazy` container without evaluating
    * it. It will be forced by the next call to `forceThunks()`
    */
