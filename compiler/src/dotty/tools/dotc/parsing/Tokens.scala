...
abstract class TokensCommon:
  ...
  def enter(token: Int, str: String, debugStr: String = ""): Unit =
    ...
    debugString(token) = if debugStr.isEmpty then str else debugStr

  /** special tokens */
  inline val EMPTY = 0;
  enter(EMPTY, "<empty>") // a missing token, used in lookahead
  ...
  enter(STRINGPART, "string literal", "string literal part")
  ...
  enter(COLONop, ":") // a stand-alone `:`, see also COLONfollow
  ...
  def buildKeywordArray(keywords: TokenSet): (Int, Array[Int]) =
    ...
    for kw <- sourceKeywords do arr(start(kw)) = kw
...
object Tokens extends TokensCommon:
  ...
  enter(QUOTEID, "quoted identifier") // TODO: deprecate

  inline val BACKQUOTED_IDENT = 15;
  enter(BACKQUOTED_IDENT, "identifier", "backquoted ident")
  ...
  enter(SELFARROW, "=>") // reclassified ARROW following self-type
  ...
  final val literalTokens: TokenSet = simpleLiteralTokens | BitSet(
    INTERPOLATIONID,
    QUOTEID,
    NULL
  ) // TODO: drop QUOTEID when symbol literals are gone

  final val atomicExprTokens: TokenSet =
    literalTokens | identifierTokens | BitSet(
      USCORE,
      NULL,
      THIS,
      SUPER,
  ...
  final val canStartInfixTypeTokens: TokenSet =
    literalTokens | identifierTokens | BitSet(
      THIS,
      SUPER,
      USCORE,
  ...
  final val canStartTypeTokens: TokenSet =
    canStartInfixTypeTokens | BitSet(LBRACE)

  final val templateIntroTokens: TokenSet =
    BitSet(CLASS, TRAIT, OBJECT, ENUM, CASECLASS, CASEOBJECT)
  ...
  final val localModifierTokens: TokenSet =
    BitSet(ABSTRACT, FINAL, SEALED, IMPLICIT, LAZY)

  final val accessModifierTokens: TokenSet = BitSet(PRIVATE, PROTECTED)

  final val modifierTokens: TokenSet =
    localModifierTokens | accessModifierTokens | BitSet(OVERRIDE)
  ...
  final val mustStartStatTokens: TokenSet =
    defIntroTokens | modifierTokens | BitSet(IMPORT, EXPORT, PACKAGE)

  final val canStartStatTokens2: TokenSet =
    canStartExprTokens2 | mustStartStatTokens | BitSet(
      AT,
      CASE,
      END
    ) // END is included since it might be tested before being converted back to IDENTIFIER
  final val canStartStatTokens3: TokenSet =
    canStartExprTokens3 | mustStartStatTokens | BitSet(AT, CASE, END)

  final val canEndStatTokens: TokenSet =
    atomicExprTokens | BitSet(TYPE, GIVEN, RPAREN, RBRACE, RBRACKET, OUTDENT)

  /** Tokens that stop a lookahead scan search for a `<-`, `then`, or `do`. Used
    * for disambiguating between old and new syntax.
    */
  ...
  final val numericLitTokens: TokenSet =
    BitSet(INTLIT, DECILIT, EXPOLIT, LONGLIT, FLOATLIT, DOUBLELIT)

  final val statCtdTokens: BitSet =
    BitSet(THEN, ELSE, DO, CATCH, FINALLY, YIELD, MATCH)

  final val closingRegionTokens =
    BitSet(RBRACE, RPAREN, RBRACKET, CASE) | statCtdTokens

  final val canStartIndentTokens: BitSet =
    statCtdTokens | BitSet(
      COLONeol,
      WITH,
      EQUALS,
      ARROW,
  ...
  final val endMarkerTokens = identifierTokens | BitSet(
    IF,
    WHILE,
    FOR,
    MATCH,
  ...
  final val colonEOLPredecessors =
    BitSet(RPAREN, RBRACKET, BACKQUOTED_IDENT, THIS, SUPER, NEW)

  final val closingParens = BitSet(RPAREN, RBRACKET, RBRACE)

  final val softModifierNames =
    Set(nme.inline, nme.into, nme.opaque, nme.open, nme.transparent, nme.infix)

  def showTokenDetailed(token: Int): String = debugString(token)

  def showToken(token: Int): String =
    val str = tokenString(token)
    if isKeyword(token) || token == COLONfollow || token == COLONeol then
      s"'$str'"
    else str
end Tokens
