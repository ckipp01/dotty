...
/** This class builds instance of `Tree` that represent XML.
  ...
  * positions to be transparent. Not sure this is a good idea for navigating XML
  * trees in the IDE but it's the best I can do right now. If someone who
  * understands this part better wants to give it a shot, please do!
  *
  * @author
  *   Burak Emir
  * @version 1.0
...
class SymbolicXMLBuilder(parser: Parser, preserveWS: Boolean)(using Context):
  ...
  // convenience methods
  private def LL[A](x: A*): List[List[A]] = List(List(x*))
    ...
    def nonpat = New(
      ...
      List(
        List(
          ...
          attrs,
          scope,
          if empty then Literal(Constant(true)) else Literal(Constant(false))
        ) ::: starArgs
      )
    )

    atSpan(span) { if isPattern then pat else nonpat }
  end mkXML
  ...
  final def text(span: Span, txt: String): Tree = atSpan(span) {
    if isPattern then makeTextPat(const(txt))
  ...
  def comment(span: Span, text: String): Tree =
    atSpan(span)(Comment(const(text)))
  def charData(span: Span, txt: String): Tree =
    atSpan(span)(makeText1(const(txt)))
  ...
  /** could optimize if args.length == 0, args.length == 1 AND args(0) is <:
    * Node.
    */
  ...
  def element(
    ...
    def mkAttributeTree(pre: String, key: String, value: Tree) =
      atSpan(span.toSynthetic) {
        ...
        val (clazz, attrArgs) =
          if pre == null then (_scala_xml_UnprefixedAttribute, baseArgs)
    ...
    lazy val scopeDef =
      ValDef(_scope, _scala_xml_NamespaceBinding, Ident(_tmpscope))
    lazy val tmpScopeDef =
      ValDef(_tmpscope, _scala_xml_NamespaceBinding, Ident(_scope)).withFlags(
        Mutable
      )
    lazy val metadataDef =
      ValDef(_md, _scala_xml_MetaData, _scala_xml_Null).withFlags(Mutable)
    val makeSymbolicAttrs =
      if !attributes.isEmpty then Ident(_md) else _scala_xml_Null
    ...
    atSpan(span.toSynthetic)(
      new XMLBlock(nsResult, new XMLBlock(attrResult, body))
    )
  end element
