...
object MarkupParsers:
  ...
  class MarkupParser(parser: Parser, final val preserveWS: Boolean)(using
      Context
    ...
    def xHandleError(that: Char, msg: String): Unit =
      if ch == SU then throw TruncatedXMLControl
    ...
    import parser.{symbXMLBuilder as handle}
    ...
    def appendText(span: Span, ts: Buffer[Tree], txt: String): Unit =
      ...
      if preserveWS then append(txt)
        ...
        txt foreach { c =>
          if !isSpace(c) then sb append c
          else if sb.isEmpty || !isSpace(sb.last) then sb append ' '
        }
    ...
    def content_BRACE(p: Span, ts: ArrayBuffer[Tree]): Unit =
      if xCheckEmbeddedBlock then ts append xEmbeddedExpr
      else appendText(p, ts, xText)

    /** Returns true if it encounters an end tag (without consuming it), appends
      * trees to ts as side-effect.
      *
      * @param ts
      *   ...
      * @return
    ...
    private def content_LT(ts: ArrayBuffer[Tree]): Boolean =
      if ch == '/' then return true // end tag

      val toAppend = ch match
        case '!' =>
          nextch();
          if ch == '[' then xCharData else xComment // CDATA or Comment
      ...
      while true do
        if xEmbeddedBlock then ts append xEmbeddedExpr
          ...
          ch match
            ...
            case '<' => nextch(); if content_LT(ts) then return ts
    ...
    private def xText: String =
      ...
      while ch != SU do
        ...
        buf append ch
        nextch()
        if xCheckEmbeddedBlock || ch == '<' || ch == '&' then return done
      done

    /** Some try/catch/finally logic used by xLiteral and xLiteralPattern. */
    inline private def xLiteralCommon(
        f: () => Tree,
      ...
      try output = f()
        ...
        case _: ArrayIndexOutOfBoundsException =>
          parser.syntaxError(
            em"missing end tag in XML literal for <$debugLastElem>",
            debugLastPos
          )
      finally parser.in.resume(saved)

      if output == null then parser.errorTermTree(parser.in.offset)
      else output

    /** Use a lookahead parser to run speculative body, and return the first
      * char afterward.
      */
    ...
    def xLiteral: Tree = xLiteralCommon(
      () =>
        ...
        // parse more XML?
        if charComingAfter(xSpaceOpt()) == '<' then
          while {
            xSpaceOpt()
            nextch()
            ts.append(element)
    ...
    def escapeToScala[A](op: => A, kind: String): A =
      ...
      if parser.in.token != RBRACE then
    ...
    def reportSyntaxError(offset: Int, str: String): Unit =
      parser.syntaxError(str.toMessage, offset)
    ...
    def xPattern: Tree =
      ...
      if !isEmptyTag then
        ...
        def doPattern: Boolean =
          ...
          if xEmbeddedBlock then ts ++= xScalaPatterns
          else
            ch match
              case '<' => // tag
                nextch()
                if ch != '/' then ts append xPattern // child
        ...
        while doPattern do {} // call until false
        xEndTag(qname)
        debugLastStartElement = debugLastStartElement.tail
      end if
