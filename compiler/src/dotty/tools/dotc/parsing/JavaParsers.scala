...
object JavaParsers:
  ...
  class JavaParser(source: SourceFile)(using Context)
      extends ParserCommon(source):
      ...
      while true do
        in.token match
          case EOF =>
            return
          case SEMI =>
            if nparens == 0 && nbraces == 0 then return
          case RPAREN =>
            nparens -= 1
          case RBRACE =>
            if nbraces == 0 then return
            nbraces -= 1
          case LPAREN =>
            nparens += 1
          case LBRACE =>
    ...
    def errorTypeTree: TypeTree =
      TypeTree().withType(UnspecifiedErrorType).withSpan(Span(in.offset))

    // --------- tree building -----------------------------

    def scalaAnnotationDot(name: Name): Select =
    ...
    def makeTemplate(
      ...
      def pullOutFirstConstr(stats: List[Tree]): (Tree, List[Tree]) =
        stats match
          case (meth: DefDef) :: rest if meth.name == nme.CONSTRUCTOR =>
            (meth, rest)
          case first :: rest =>
      ...
      if needsDummyConstr then
        if constr1 == EmptyTree then constr1 = makeConstructor(List(), Nil)
        stats1 = constr1 :: stats1
        constr1 = makeConstructor(
          List(scalaDot(tpnme.Unit)),
          tparams,
          Flags.JavaDefined | Flags.PrivateLocal
        )
      else if constr1 == EmptyTree then
    ...
    def makeParam(name: TermName, tpt: Tree): ValDef =
      ValDef(name, tpt, EmptyTree).withMods(
        Modifiers(Flags.JavaDefined | Flags.Param)
      )

    def makeConstructor(
        formals: List[Tree],
        tparams: List[TypeDef],
        flags: FlagSet = Flags.JavaDefined
    ): DefDef =
      val vparams = formals.zipWithIndex.map { case (p, i) =>
        makeSyntheticParam(i + 1, p)
      }
      DefDef(
        nme.CONSTRUCTOR,
        joinParams(tparams, List(vparams)),
        TypeTree(),
        EmptyTree
      ).withMods(Modifiers(flags))
    ...
    def skipTo(tokens: Int*): Unit =
      while !(tokens contains in.token) && in.token != EOF do
    ...
    def acceptClosingAngle(): Unit =
      ...
      if closers isDefinedAt in.token then in.token = closers(in.token)
      else accept(GT)
    ...
    def qualId(): RefTree =
      ...
      while in.token == DOT && in.lookaheadToken == IDENTIFIER do
        in.nextToken()
    ...
    def basicType(): Tree =
      atSpan(in.offset) {
        in.token match
          ...
          case _ =>
            syntaxError(em"illegal start of type", skipIt = true); errorTypeTree
      }

    def typ(): Tree =
      annotations()
      optArrayBrackets {
        if in.token == FINAL then in.nextToken()
        if in.token == IDENTIFIER then
          ...
          while in.token == DOT do
            in.nextToken()
            t =
              typeArgs(atSpan(t.span.start, in.offset)(typeSelect(t, ident())))
          convertToTypeId(t)
        else basicType()
      }
    ...
    def typeArgs(t: Tree): Tree =
      var wildnum = 0
      def typeArg(): Tree =
        if in.token == QMARK then
          val offset = in.offset
          in.nextToken()
          val hi = if in.token == EXTENDS then
            in.nextToken(); typ()
          else javaLangObject()
          val lo = if in.token == SUPER then
            in.nextToken(); typ()
          else EmptyTree
          atSpan(offset) {
            /*
    ...
    /** Annotation ::= TypeName [`(` [AnnotationArgument {`,`
      ...
      * parsing error, but instead we skip entire annotation silently.
      */
    def annotation(): Option[Tree] =
      def classOrId(): Tree =
        ...
        if in.lookaheadToken == CLASS then
          ...
          accept(CLASS)
          TypeApply(
            Select(scalaDot(nme.Predef), nme.classOf),
      ...
      def annArg(): Option[Tree] =
        val name = if in.token == IDENTIFIER && in.lookaheadToken == EQUALS then
          val n = ident()
          accept(EQUALS)
          n
        else nme.value
        ...
        in.token match
          ...
          case _ =>
            val privateWithin: TypeName =
              if isPackageAccess && !inInterface then thisPackageName
              else tpnme.EMPTY
    ...
    def typeParams(
        flags: FlagSet = Flags.JavaDefined | Flags.PrivateLocal | Flags.Param
    ): List[TypeDef] =
      if in.token == LT then
        in.nextToken()
    ...
    def bound(): Tree =
      atSpan(in.offset) {
        ...
        while in.token == AMP do
          in.nextToken()
          buf += typ()
        val ts = buf.toList
        if ts.tail.isEmpty then ts.head
    ...
    def formalParams(): List[ValDef] =
      accept(LPAREN)
      val vparams =
        if in.token == RPAREN then List()
        else repsep(() => formalParam(), COMMA)
      accept(RPAREN)
      vparams
    ...
    def definesInterface(token: Int): Boolean =
      token == INTERFACE || token == AT

    /** If the next token is the identifier "record", convert it into the RECORD
      * token. This makes it easier to handle records in various parts of the
      * code, in particular when a `parentToken` is passed to some functions.
      */
    ...
    def termDecl(
        ...
        mods: Modifiers,
        parentToken: Int,
        parentTParams: List[TypeDef]
    ): List[Tree] =
      val inInterface = definesInterface(parentToken)
      val tparams =
        if in.token == LT then typeParams(Flags.JavaDefined | Flags.Param)
        else List()
      ...
      val rtptName = rtpt match
        ...
        case _           => nme.EMPTY
      if in.token == LPAREN && rtptName != nme.EMPTY && !inInterface then
      ...
      then
        ...
        if in.token == LPAREN then
          // method declaration
          val vparams = formalParams()
          if !isVoid then rtpt = optArrayBrackets(rtpt)
          optThrows()
          val bodyOk = !inInterface || mods.isOneOf(
            Flags.DefaultMethod | Flags.JavaStatic | Flags.Private
          )
          val body =
            if bodyOk && in.token == LBRACE then methodBody()
            else if parentToken == AT && in.token == DEFAULT then
              val annot =
                atSpan(nameOffset) {
                  New(
                    Select(
                      Select(scalaDot(nme.annotation), nme.internal),
    ...
    /** Parse a sequence of field declarations, separated by commas. This one is
      * tricky because a comma might also appear in an initializer. Since we
      * don't parse initializers we don't know what the comma signifies. We
      * solve this with a second list buffer `maybe` which contains potential
      * variable definitions. Once we have reached the end of the statement, we
      * know whether these potential definitions are real or not.
      */
    def fieldDecls(
        start: Offset,
      ...
      val buf = ListBuffer[Tree](atSpan(start, firstNameOffset) {
      ...
      while in.token == COMMA do
        ...
        then // if there's an ident after the comma ...
          val nextNameOffset = in.offset
          val name = ident()
          if in.token == EQUALS || in.token == SEMI
          then // ... followed by a `=` or `;`, we know it's a real variable definition
            buf ++= maybe
            buf += atSpan(start, nextNameOffset) {
              varDecl(mods, tpt, name.toTermName)
            }
            maybe.clear()
          else if in.token == COMMA
          then // ... if there's a comma after the ident, it could be a real vardef or not.
    ...
    def varDecl(mods: Modifiers, tpt: Tree, name: TermName): ValDef =
      ...
      /** Tries to detect final static literals syntactically and returns a
        * constant type replacement
        */
        ...
        def forConst(const: Constant): Tree =
          if in.token != SEMI then tpt1
            ...
            def isStringTyped = tpt1 match
              ...
              case _                  => false
            if const.tag == Constants.StringTag && isStringTyped then
              constantTpe(const)
            else
              tpt1 match
                case TypedSplice(tpt2) =>
                  if const.tag == Constants.BooleanTag || const.isNumeric then
                    // for example, literal 'a' is ok for float. 127 is ok for byte, but 128 is not.
                    val converted = const.convertTo(tpt2.tpe)
                    if converted == null then tpt1
      ...
      val tpt2: Tree =
        if in.token == EQUALS && !mods.is(Flags.Param) then
          val res = optConstantTpe()
          skipTo(COMMA, SEMI)
          res
        else tpt1

      val mods1 = if mods.is(Flags.Final) then mods else mods | Flags.Mutable
      ValDef(
        name,
        tpt2,
        if mods.is(Flags.Param) then EmptyTree else unimplementedExpr
      ).withMods(mods1)
    end varDecl
    ...
    def memberDecl(
        ...
        parentToken: Int,
        parentTParams: List[TypeDef]
    ): List[Tree] = in.token match
      case CLASS | ENUM | RECORD | INTERFACE | AT =>
        typeDecl(
          start,
          if definesInterface(parentToken) then mods | Flags.JavaStatic
          else mods
        )
      case _ =>
    ...
    def makeCompanionObject(cdef: TypeDef, statics: List[Tree]): Tree =
      atSpan(cdef.span) {
        ...
        ).withMods((cdef.mods & Flags.RetainedModuleClassFlags).toTermFlags)
      }
...
//        }
    ...
    def classDecl(start: Offset, mods: Modifiers): List[Tree] =
      ...
      val cls = atSpan(start, nameOffset) {
        TypeDef(
          name,
          makeTemplate(superclass :: interfaces, body, tparams, true)
        ).withMods(mods)
      }
    ...
    def recordDecl(start: Offset, mods: Modifiers): List[Tree] =
      ...
      val superclass =
        javaLangRecord() // records always extend java.lang.Record
      ...
      var fieldsByName =
        header.map(v => (v.name, (v.tpt, v.mods.annotations))).to(LinkedHashMap)

      for
        case DefDef(name, paramss, _, _) <- body
        if paramss.isEmpty && fieldsByName.contains(name)
      do fieldsByName -= name
      end for

      val accessors =
        (for (name, (tpt, annots)) <- fieldsByName
        yield DefDef(name, Nil, tpt, unimplementedExpr)
          .withMods(
            Modifiers(Flags.JavaDefined | Flags.Method | Flags.Synthetic)
          )).toList
      ...
      val canonicalConstructor =
        DefDef(
          nme.CONSTRUCTOR,
          joinParams(tparams, List(header)),
          TypeTree(),
          EmptyTree
        )
          .withMods(
            Modifiers(Flags.JavaDefined | Flags.Synthetic, mods.privateWithin)
          )
    ...
    def interfaceDecl(start: Offset, mods: Modifiers): List[Tree] =
      ...
      val iface = atSpan(start, nameOffset) {
        TypeDef(name, makeTemplate(parents, body, tparams, false)).withMods(
          mods | Flags.JavaInterface
        )
      }
    ...
    def typeBodyDecls(
      ...
      while in.token != RBRACE && in.token != EOF do
        val start = in.offset
    ...
    def annotationDecl(start: Offset, mods: Modifiers): List[Tree] =
      ...
      val constructorParams = body.collect { case dd: DefDef =>
        makeParam(dd.name, dd.tpt)
      }
      val constr =
        DefDef(nme.CONSTRUCTOR, List(constructorParams), TypeTree(), EmptyTree)
          .withMods(Modifiers(Flags.JavaDefined))
      val templ = makeTemplate(annotationParents, constr :: body, List(), true)
      val annot = atSpan(start, nameOffset) {
        TypeDef(name, templ).withMods(
          mods | Flags.JavaInterface | Flags.JavaAnnotation
        )
      }
    ...
    def enumDecl(start: Offset, mods: Modifiers): List[Tree] =
      ...
      def parseEnumConsts(): Unit =
        if in.token != RBRACE && in.token != SEMI && in.token != EOF then
      ...
      val predefs = List(
        DefDef(nme.values, ListOfNil, arrayOf(enumType), unimplementedExpr)
          .withMods(
            Modifiers(Flags.JavaDefined | Flags.JavaStatic | Flags.Method)
          ),
        DefDef(
          nme.valueOf,
       ...
       */
      val superclazz = Apply(
        TypeApply(
          Select(New(javaLangDot(tpnme.Enum)), nme.CONSTRUCTOR),
          List(enumType)
        ),
        Nil
      )
      val enumclazz = atSpan(start, nameOffset) {
        ...
        ).withMods(mods | Flags.JavaEnumTrait)
      }
    ...
    def enumConst(enumType: Tree): ValDef =
      ...
      atSpan(in.offset) {
        ...
        if in.token == LBRACE then
          ...
          skipAhead()
          accept(RBRACE)
        ValDef(name.toTermName, enumType, unimplementedExpr).withMods(
          Modifiers(
            Flags.JavaEnumTrait | Flags.StableRealizable | Flags.JavaDefined | Flags.JavaStatic
          )
        )
      }
    ...
    def compilationUnit(): Tree =
      val start = in.offset
      val pkg: RefTree =
        if in.token == AT || in.token == PACKAGE then
          annotations()
          accept(PACKAGE)
          val pkg = qualId()
          accept(SEMI)
      ...
      thisPackageName = convertToTypeName(pkg) match
        ...
        case _       => tpnme.EMPTY
      val buf = new ListBuffer[Tree]
      while in.token == IMPORT do buf ++= importDecl()
      while in.token != EOF && in.token != RBRACE do
  ...
  /** OutlineJavaParser parses top-level declarations in `source` to find
    * declared classes, ignoring their bodies (which must only have balanced
    * braces). This is used to map class names to defining sources. This is
    * necessary even for Java, because the filename defining a non-public
    * classes cannot be determined from the classname alone.
    */
  class OutlineJavaParser(source: SourceFile)(using Context)
      extends JavaParser(source)
      with OutlineParserCommon:
    override def skipBracesHook(): Option[Tree] = None
    override def typeBody(
        leadingToken: Int,
        parentName: Name,
        parentTParams: List[TypeDef]
    ): (List[Tree], List[Tree]) =
