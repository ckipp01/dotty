...
object Parsers:
  ...
  enum Location(
      val inParens: Boolean,
      val inPattern: Boolean,
      val inArgs: Boolean
  ):
  ...
  /** The parse starting point depends on whether the source file is
    * self-contained: if not, the AST will be supplemented.
    */
  ...
  abstract class ParserCommon(val source: SourceFile)(using Context):
    ...
    /** Positions tree. If `t` does not have a span yet, set its span to the
      * given one.
      */
    def atSpan[T <: Positioned](span: Span)(t: T): T =
      if t.span.isSourceDerived then t else t.withSpan(span.union(t.span))

    def atSpan[T <: Positioned](start: Offset, point: Offset, end: Offset)(
        t: T
    ): T =
      atSpan(Span(start, end, point))(t)

    /** If the last read offset is strictly greater than `start`, assign tree
      * the span from `start` to last read offset, with given point. If the last
      * offset is less than or equal to start, the tree `t` did not consume any
      * source for its construction. In this case, don't assign a span yet, but
      * wait for its span to be determined by `setChildSpans` when the parent
      * node is positioned.
      */
    def atSpan[T <: Positioned](start: Offset, point: Offset)(t: T): T =
      if in.lastOffset > start then atSpan(start, point, in.lastOffset)(t)
      else t
    ...
    def startOffset(t: Positioned): Int =
      if t.span.exists then t.span.start else in.offset

    def pointOffset(t: Positioned): Int =
      if t.span.exists then t.span.point else in.offset

    def endOffset(t: Positioned): Int =
      if t.span.exists then t.span.end else in.lastOffset

    def nameStart: Offset =
      if in.token == BACKQUOTED_IDENT then in.offset + 1 else in.offset
    ...
    def expectedOffset: Int =
      ...
      if current.line != last.line then in.lastOffset else in.offset
    ...
    /** Issue an error at given offset if beyond last error offset and update
      * lastErrorOffset.
      */
    def syntaxError(msg: Message, offset: Int = in.offset): Unit =
      if offset > lastErrorOffset then
        val length =
          if offset == in.offset && in.name != null then in.name.show.length
          else 0
        syntaxError(msg, Span(offset, offset + length))
        lastErrorOffset = in.offset

    /** Unconditionally issue an error at given span, without updating
      * lastErrorOffset.
      */
  ...
  trait OutlineParserCommon extends ParserCommon:
    ...
    def skipBraces(): Unit =
      accept(if in.token == INDENT then INDENT else LBRACE)
      var openBraces = 1
      while in.token != EOF && openBraces > 0 do
        skipBracesHook() getOrElse {
          if in.token == LBRACE || in.token == INDENT then openBraces += 1
          else if in.token == RBRACE || in.token == OUTDENT then openBraces -= 1
          in.nextToken()
        }
  ...
  class Parser(source: SourceFile)(using Context) extends ParserCommon(source):
    ...
    def isPureArrow(name: Name): Boolean =
      isIdent(name) && Feature.pureFunsEnabled
    def isPureArrow: Boolean =
      isPureArrow(nme.PUREARROW) || isPureArrow(nme.PURECTXARROW)
    ...
    def isSimpleLiteral =
      simpleLiteralTokens.contains(in.token)
        || isIdent(nme.raw.MINUS) && numericLitTokens.contains(
          in.lookahead.token
        )
    ...
    def isModifier: Boolean =
      modifierTokens.contains(in.token) || in.isSoftModifier
    ...
    def isDefIntro(
        allowedMods: BitSet,
        excludedSoftModifiers: Set[TermName] = Set.empty
    ): Boolean =
      in.token == AT
        || defIntroTokens.contains(in.token)
        || allowedMods.contains(in.token)
        || in.isSoftModifierInModifierPosition && !excludedSoftModifiers
          .contains(in.name)

    def isStatSep: Boolean = in.isStatSep

    /** A '$' identifier is treated as a splice if followed by a `{`. A longer
      * identifier starting with `$` is treated as a splice/id combination in a
      * quoted block '{...'
      */
    ...
    /** Is offset1 less or equally indented than offset2? This is the case if
      * the characters between the preceding end-of-line and offset1 are a
      * prefix of the characters between the preceding end-of-line and offset2.
      */
    def isLeqIndented(offset1: Int, offset2: Int): Boolean =
      def recur(idx1: Int, idx2: Int): Boolean =
        idx1 == offset1 ||
          idx2 < offset2 && source(idx1) == source(idx2) && recur(
            idx1 + 1,
            idx2 + 1
          )
    ...
    /** If at end of file, issue an incompleteInputError. Otherwise issue a
      * syntax error and skip to next safe point.
      */
    def syntaxErrorOrIncomplete(msg: Message, offset: Int = in.offset): Unit =
      if in.token == EOF then incompleteInputError(msg)
      else
    ...
    /** Consume one token of the specified type, or signal an error if it is not
      * there.
      *
      * @return
    ...
    def accept(name: Name): Int =
      val offset = in.offset
      if !isIdent(name) then syntaxErrorOrIncomplete(em"`$name` expected")
      if isIdent(name) then in.nextToken()
      offset
    ...
    /** Parse statement separators and end markers. Ensure that there is at
      * least one statement separator unless the next token terminates a
      * statement´sequence.
      * @param stats
      ...
      * @param altEnd
      *   a token that is also considered as a terminator of the statement
      ...
      */
    def statSepOrEnd[T <: Tree](
        stats: ListBuffer[T],
        noPrevStat: Boolean = false,
      ...
      def recur(sepSeen: Boolean, endSeen: Boolean): Boolean =
        ...
        else if isStatSeqEnd || in.token == altEnd then false
        else if sepSeen || endSeen then true
        else
          val found = in.token
          val statFollows = mustStartStatTokens.contains(found)
          syntaxError(
            if noPrevStat then
              IllegalStartOfStatement(what, isModifier, statFollows)
    ...
    def rewriteNotice(
        version: SourceVersion = `3.0-migration`,
    ...
    def syntaxVersionError(option: String, span: Span) =
      syntaxError(
        em"""This construct is not allowed under $option.${rewriteNotice(
            `3.0-migration`,
            option
          )}""",
        span
    ...
    def rewriteToOldSyntax(span: Span = Span(in.offset)): Boolean =
      if in.oldSyntax then
        if in.rewrite then return true
        syntaxVersionError("-old-syntax", span)
      false

    def errorTermTree(start: Offset): Literal =
      atSpan(start, in.offset, in.offset) { Literal(Constant(null)) }
    ...
    /** A flag indicating we are parsing in the annotations of a primary class
      * constructor
      */
    ...
    def convertToParam(tree: Tree, mods: Modifiers): ValDef =
      def fail() =
        syntaxError(
          em"not a legal formal parameter for a function literal",
          tree.span
        )
    ...
    def checkNonParamTuple(t: Tree) = t match
      case Tuple(ts) =>
        ts.collectFirst { case param: ValDef =>
          syntaxError(
            em"invalid parameter definition syntax in tuple value",
            param.span
          )
        }
      case _ =>
    ...
    def isWildcard(t: Tree): Boolean = t match
      case Ident(name1) =>
        placeholderParams.nonEmpty && name1 == placeholderParams.head.name
    ...
    /** The markup parser. The first time this lazy val is accessed, we assume
      * we were trying to parse an XML literal. The current position is recorded
      * for later error reporting if it turns out that we don't have scala-xml
    ...
    object symbXMLBuilder
        extends xml.SymbolicXMLBuilder(this, true) // DEBUG choices
    ...
    def inBracesOrIndented[T](
      ...
      else if in.rewriteToIndent then bracesToIndented(body, rewriteWithColon)
      else inBraces(body)
    ...
    def inSepRegion[T](f: Region => Region)(op: => T): T =
      ...
      try op
      finally in.currentRegion = cur

    /** Parse `body` while checking (under -no-indent) that a `{` is not missing
      * before it. This is done as follows: If the next token S is indented
      * relative to the current region, and the end of `body` is followed by a
      * new line and another statement, check that that other statement is
      * indented less than S
      */
            ...
            warning(
              if startIndentWidth <= nextIndentWidth then
                em"""Line is indented too far to the right, or a `{` is missing before:
                  |
                  |${t.tryToShow}"""
              else in.spaceTabMismatchMsg(startIndentWidth, nextIndentWidth),
              in.next.offset
            )
          t
        else body()
    ...
    /** Check that this is not the start of a statement that's indented relative
    ...
    def checkNextNotIndented(): Unit =
      if in.isNewLine then
        ...
        if in.currentRegion.indentWidth < nextIndentWidth then
          warning(
            em"Line is indented too far to the right, or a `{` or `:` is missing",
            in.next.offset
          )

    /* -------- REWRITES ----------------------------------------------------------- */

    /** The last offset where a colon at the end of line would be required if a
      * subsequent { ... } block would be converted to an indentation region.
      */
    ...
    def skipBlanks(idx: Int, step: Int = 1): Int =
      if testChar(idx, c => c == ' ' || c == '\t' || c == Chars.CR) then
        skipBlanks(idx + step, step)
      else idx
    ...
    def indentedToBraces[T](body: => T): T =
      ...
      /** Is `expr` a tree that lacks a final `else`? Put such trees in `{...}`
        * to make sure we don't accidentally merge them with a following `else`.
        */
      ...
      /** Is `expr` a (possibly curried) function that has a multi-statement
        * block as body? Put such trees in `{...}` since we don't enclose
        * statements following a `=>` in braces.
        */
      ...
      def closingOffset(lineStart: Offset): Offset =
        if in.lineOffset >= 0 && lineStart >= in.lineOffset then in.lineOffset
          ...
          if testChar(commentStart, '/') && indentWidth < in.indentWidth(
              commentStart
            )
      ...
      if needsBraces(t) then
        ...
        if closedByEndMarker then patch(source, Span(next.offset), "} // ")
          ...
          patch(
            source,
            Span(closingOffset(source.nextLine(in.lastOffset))),
            indentWidth.toPrefix ++ "}\n"
          )
      t
    end indentedToBraces

    /** The region to eliminate when replacing an opening `(` or `{` that ends a
      * line. The `(` or `{` is at in.offset.
      */
    def startingElimRegion(colonRequired: Boolean): (Offset, Offset) =
      val skipped = skipBlanks(in.offset + 1)
      if in.isAfterLineEnd then
        if testChar(skipped, Chars.LF) && !colonRequired then
          (in.lineOffset, skipped + 1) // skip the whole line
        else (in.offset, skipped)
    ...
    /** The region to eliminate when replacing a closing `)` or `}` that starts
      * a new line The `)` or `}` precedes in.lastOffset.
      */
    def closingElimRegion(): (Offset, Offset) =
      val skipped = skipBlanks(in.lastOffset)
      if testChar(skipped, Chars.LF)
      then // if `)` or `}` is on a line by itself
        (source.startOfLine(in.lastOffset), skipped + 1) //   skip the whole line
      else // else
        (
          in.lastOffset - 1,
          skipped
        ) //   move the following text up to where the `)` or `}` was

    /** Parse brace-enclosed `body` and rewrite it to be an indentation region
      * instead, if possible. If possible means:
      *   1. not inside (...), [...], case ... => 2. opening brace `{` is at end
      *      of line 3. closing brace `}` is at start of line 4. there is at
      *      least one token between the braces 5. the closing brace is also at
      *      the end of the line, or it is followed by one of `then`, `else`,
      *      `do`, `catch`, `finally`, `yield`, or `match`. 6. the opening brace
      *      does not follow a `=>`. The reason for this condition is that
      *      rewriting back to braces does not work after `=>` (since in most
      *      cases braces are omitted after a `=>` it would be annoying if
    ...
    def bracesToIndented[T](body: => T, rewriteWithColon: Boolean): T =
      ...
      canRewrite &= (in.isAfterLineEnd || statCtdTokens.contains(
        in.token
      )) // test (5)
    ...
    def dropParensOrBraces(start: Offset, endStr: String): Unit =
      if testChar(start + 1, Chars.isLineBreakChar) then
        patch(
          source,
          Span(
            if testChar(start - 1, ' ') then start - 1 else start,
            start + 1
          ),
          ""
        )
        ...
        patch(
          source,
          Span(start, start + 1),
          if testChar(start - 1, Chars.isIdentifierPart) then " " else ""
        )
      val closingStartsLine =
        testChar(skipBlanks(in.lastOffset - 2, -1), Chars.LF)
      val preFill = if closingStartsLine || endStr.isEmpty then "" else " "
      val postFill = if in.lastOffset == in.offset then " " else ""
      val (startClosing, endClosing) =
        if closingStartsLine && endStr.isEmpty then closingElimRegion()
    ...
    def widenIfWholeLine(span: Span): Span =
      ...
      if testChar(start, Chars.LF) && testChar(end, Chars.LF) then
        Span(start, end)
      else span
    ...
    def dropTerminator(): Unit =
      if in.token == THEN || in.token == DO then
        ...
        if in.isAfterLineEnd then
          if testChar(endOffset, ' ') then endOffset += 1
        else if testChar(startOffset - 1, ' ') &&
    ...
    /** In the tokens following the current one, does `query` precede any of the
      * tokens that
    ...
    def followedByToken(query: Token): Boolean =
      ...
      while true do
        ...
        if braces == 0 then
          if token == query then return true
          if stopScanTokens.contains(token) || lookahead.isNestedEnd then
            return false
        else if token == EOF then return false
        else if lookahead.isNestedEnd then braces -= 1
        if lookahead.isNestedStart then braces += 1
        lookahead.nextToken()
      false
    ...
    def followingIsEnclosedGenerators(): Boolean =
      ...
      var parens = 1
      lookahead.nextToken()
      while parens != 0 && lookahead.token != EOF do
        val token = lookahead.token
        if token == LPAREN then parens += 1
        else if token == RPAREN then parens -= 1
        lookahead.nextToken()
      if lookahead.token == LARROW then false // it's a pattern
      else if lookahead.isIdent then
    ...
    /** Is current ident a `*`, and is it followed by a `)`, `, )`, `,EOF`? The
      * latter two are not syntactically valid, but we need to include them here
      * for error recovery.
      */
    ...
    /** When encountering a `:`, is that in the binding of a lambda?
      * @pre
      *   location of the enclosing expression is `InParens`, so there is an
      *   open `(`.
      */
    ...
    /** Is the token sequence following the current `:` token classified as a
      * lambda? This is the case if the input starts with an identifier, a
      * wildcard, or something enclosed in (...) or [...], and this is followed
      * by a `=>` or `?=>` and an INDENT.
      */
    ...
    def checkAssoc(
        ...
        op1: Name,
        op2: Name,
        op2LeftAssoc: Boolean
    ): Unit =
      if op1.isRightAssocOperatorName == op2LeftAssoc then
        syntaxError(
          MixedLeftAndRightAssociativeOps(op1, op2, op2LeftAssoc),
          offset
        )
    ...
    def reduceStack(
      ...
      if opStack != base && precedence(opStack.head.operator.name) == prec then
        checkAssoc(
          opStack.head.offset,
          opStack.head.operator.name,
          op2,
          leftAssoc
        )
      def recur(top: Tree): Tree =
        if opStack == base then top
        else
          ...
          if prec < opPrec || leftAssoc && prec == opPrec then
            ...
            recur {
              atSpan(
                opInfo.operator.span union opInfo.operand.span union top.span
              ) {
    ...
    def infixOps(
      ...
      def recur(top: Tree): Tree =
        ...
        if isIdent && isOperator then
          ...
          val top1 = reduceStack(
            base,
            top,
            precedence(op.name),
            !op.name.isRightAssocOperatorName,
            op.name,
            isType
          )
          ...
          if isColonLambda then
            ...
            opStack = opStack.tail
            val od =
              reduceStack(base, topInfo.operand, 0, true, in.name, isType)
          ...
          val t =
            reduceStack(base, top, minPrec, leftAssoc = true, in.name, isType)
    ...
    private def makeIdent(tok: Token, offset: Offset, name: Name) =
      val tree = Ident(name)
      if tok == BACKQUOTED_IDENT then tree.pushAttachment(Backquoted, ())
      ...
      if tree.name == nme.ERROR && tree.span == NoSpan then
        tree.withSpan(Span(errorOffset, errorOffset))
    ...
    def dotSelectors(t: Tree): Tree =
      if in.token == DOT then
        in.nextToken(); dotSelectors(selector(t))
      else t

    private val id: Tree => Tree = x => x

    /** SimpleRef ::= id \| [id ‘.’] ‘this’ \| [id ‘.’] ‘super’ [ClassQualifier]
      * ‘.’ id
      */
    def simpleRef(): Tree =
      val start = in.offset
    ...
    def mixinQualifierOpt(): Ident =
      if in.token == LBRACKET then inBrackets(atSpan(in.offset) { typeIdent() })
      else EmptyTypeIdent
    ...
    /** Singleton ::= SimpleRef \| SimpleLiteral \| Singleton ‘.’ id
    ...
    /** SimpleLiteral ::= [‘-’] integerLiteral \| [‘-’] floatingPointLiteral \|
    ...
    def simpleLiteral(): Tree =
      ...
      else literal(inTypeOrSingleton = true)

    /** Literal ::= SimpleLiteral \| processedStringLiteral \| symbolLiteral \|
      * ‘null’
      *
      * @param negOffset
      *   The offset of a preceding `-' sign, if any. If the literal is not
      *   negated, negOffset == in.offset.
      */
    def literal(
        negOffset: Int = in.offset,
        inPattern: Boolean = false,
        inTypeOrSingleton: Boolean = false,
        inStringInterpolation: Boolean = false
    ): Tree =
      ...
      if inStringInterpolation then
        val t = in.token match
          case STRINGLIT | STRINGPART =>
            val value = in.strVal
            atSpan(negOffset, negOffset, negOffset + value.length) {
              Literal(Constant(value))
            }
          case _ =>
            syntaxErrorOrIncomplete(IllegalLiteral())
        ...
        atSpan(negOffset) {
          if in.token == QUOTEID then
            if (staged & StageKind.Spliced) != 0 && Chars.isIdentifierStart(
                in.name(0)
              )
            then
              ...
              if !in.featureEnabled(Feature.symbolLiterals) then
                val name =
                  in.name // capture name (not `in`) in the warning message closure
                report.errorOrMigrationWarning(
          ...
          else if in.token == INTERPOLATIONID then interpolatedString(inPattern)
          else
      ...
      atSpan(in.offset) {
        ...
        def nextSegment(literalOffset: Offset) =
          segmentBuf += Thicket(
            ...
            atSpan(in.offset) {
              if in.token == IDENTIFIER then termIdent()
        ...
        if in.token == STRINGLIT then
          segmentBuf += literal(
            inPattern = inPattern,
            negOffset = in.offset + offsetCorrection,
            inStringInterpolation = true
          )
    ...
    def newLinesOptWhenFollowedBy(name: Name): Unit =
      if in.isNewLine && in.next.token == IDENTIFIER && in.next.name == name
      then in.nextToken()

    def newLineOptWhenFollowing(p: Int => Boolean): Unit =
      // note: next is defined here because current == NEWLINE
      if in.token == NEWLINE && p(in.next.token) then newLineOpt()
    ...
    def possibleTemplateStart(isNew: Boolean = false): Unit =
      ...
      else newLineOptWhenFollowedBy(LBRACE)
    ...
    def checkEndMarker[T <: Tree](stats: ListBuffer[T]): Unit =
      ...
      def endName = if in.token == IDENTIFIER then in.name.toString
      else tokenString(in.token)

      def matchesAndSetEnd(last: T): Boolean =
        val didMatch = matches(last)
        if didMatch then updateSpanOfLast(last)
        didMatch
      ...
      if in.token == END then
        ...
        in.token =
          IDENTIFIER // Leaving it as the original token can confuse newline insertion
        in.nextToken()
    end checkEndMarker

    /* ------------- TYPES ------------------------------------------------------ */

    /** Same as [[typ]], but if this results in a wildcard it emits a syntax
      * error and returns a tree for type `Any` instead.
      */
    ...
    private def checkFunctionNotErased(f: Function, context: String) =
      def fail(span: Span) =
        syntaxError(
          em"Implementation restriction: erased parameters are not supported in $context",
          span
        )
      ...
      val hasErasedParam = f match
        ...
        case _                   => false
      if hasErasedParam then fail(f.span)
    ...
    /** CaptureSet ::= `{` CaptureRef {`,` CaptureRef} `}` -- under
      * captureChecking
      */
    ...
    /** Type ::= FunType \| HkTypeParamClause ‘=>>’ Type \| FunParamClause ‘=>>’
      * Type \| MatchType \| InfixType FunType ::= (MonoFunType | PolyFunType)
      * MonoFunType ::= FunTypeArgs (‘=>’ | ‘?=>’) Type \| (‘->’ | ‘?->’ )
      * [CaptureSet] Type -- under pureFunctions PolyFunType ::=
      * HKTypeParamClause '=>' Type \| HKTypeParamClause ‘->’ [CaptureSet] Type
      * -- under pureFunctions FunTypeArgs ::= InfixType \| `(' [ [ ‘[using]’
      * ‘['erased'] FunArgType {`,' FunArgType } ] `)' \| '(' [ ‘[using]’
      ...
      def functionRest(params: List[Tree]): Tree =
        ...
        atSpan(start, in.offset) {
          ...
          if token == CTXARROW then
            in.nextToken()
            imods |= Given
          else if token == ARROW || token == TLARROW then in.nextToken()
          else accept(ARROW)
          ...
          if token == TLARROW then
            for case ValDef(_, tpt, _) <- params do
              if isByNameType(tpt) then
                syntaxError(
                  em"parameter of type lambda may not be call-by-name",
                  tpt.span
                )
            TermLambdaTypeTree(params.asInstanceOf[List[ValDef]], resultType)
          else if imods.isOneOf(Given | Impure) || erasedArgs.contains(true)
          then
            if imods.is(Given) && params.isEmpty then
              syntaxError(
                em"context function types require at least one parameter",
                paramSpan
              )
            FunctionWithMods(params, resultType, imods, erasedArgs.toList)
          else if !ctx.settings.YkindProjector.isDefault then
            val (newParams :+ newResultType, tparams) =
              replaceKindProjectorPlaceholders(params :+ resultType): @unchecked
            lambdaAbstract(tparams, Function(newParams, newResultType))
          else Function(params, resultType)
        }
      end functionRest
      ...
      val t =
        if in.token == LPAREN then
          ...
          if in.token == RPAREN then
            ...
            accept(RPAREN)
            if isValParamList || in.isArrow || isPureArrow then functionRest(ts)
            else
              ...
              infixTypeRest(
                refinedTypeRest(
                  withTypeRest(annotTypeRest(simpleTypeRest(tuple)))
                )
              )
          end if
          ...
          if in.token == TLARROW then
            ...
            atSpan(start, arrowOffset) {
              getFunction(body) match
                ...
                case None =>
                  syntaxError(
                    em"Implementation restriction: polymorphic function types must have a value parameter",
                    arrowOffset
                  )
    ...
    private def makeKindProjectorTypeDef(name: TypeName): TypeDef =
      ...
      val unannotatedName =
        if isVarianceAnnotated then name.mapLast(_.drop(1)) else name
      TypeDef(unannotatedName, WildcardTypeBoundsTree()).withFlags(Param)

    /** Replaces kind-projector's `*` in a list of types arguments with
      * synthetic names, returning the new argument list and the synthetic type
      * definitions.
      */
    private def replaceKindProjectorPlaceholders(
        params: List[Tree]
    ): (List[Tree], List[TypeDef]) =
      ...
      val newParams = params.mapConserve {
        case param @ Ident(
              tpnme.raw.STAR | tpnme.raw.MINUS_STAR | tpnme.raw.PLUS_STAR
            ) =>
          addParam()
        case param @ Ident(
              tpnme.USCOREkw | tpnme.raw.MINUS_USCORE | tpnme.raw.PLUS_USCORE
            ) if uscores =>
          addParam()
        case other => other
      }
    ...
    /** Disambiguation: a `^` is treated as a postfix operator meaning `^{cap}`
      * if followed by `{`, `->`, or `?->`, or followed by a new line
      * (significant or not), or followed by a token that cannot start an infix
      * type. Otherwise it is treated as an infix operator.
      */
    ...
    def withTypeRest(t: Tree): Tree =
      if in.token == WITH then
        ...
        in.nextToken()
        if in.token == LBRACE || in.token == INDENT then t
        else
    ...
    /** ExprSplice ::= ‘$’ spliceId -- if inside quoted block \| ‘$’ ‘{’ Block
      * ‘}’ -- unless inside quoted pattern \| ‘$’ ‘{’ Pattern ‘}’ -- when
      * inside quoted pattern TypeSplice ::= ‘$’ spliceId -- if inside quoted
      * type \| ‘$’ ‘{’ Block ‘}’ -- unless inside quoted type pattern \| ‘$’
    ...
    def splice(isType: Boolean): Tree =
      ...
      atSpan(in.offset) {
        ...
        val expr =
          if in.name.length == 1 then
            in.nextToken()
            withinStaged(StageKind.Spliced)(
              if inPattern then inBraces(pattern()) else stagedBlock()
            )
          else
            atSpan(in.offset + 1) {
        ...
        if isType then
          ...
          Ident(nme.ERROR.toTypeName)
        else if inPattern then SplicePattern(expr, Nil)
        else Splice(expr)
      }
    end splice

    /** SimpleType ::= SimpleLiteral \| ‘?’ SubtypeBounds \| SimpleType1 \|
      * SimpleType ‘(’ Singletons ‘)’ -- under language.experimental.dependent,
      * checked in Typer Singletons ::= Singleton {‘,’ Singleton}
      */
    def simpleType(): Tree =
      if isSimpleLiteral then SingletonTypeTree(simpleLiteral())
        ...
        if ctx.settings.YkindProjector.value == "underscores" then
          ...
          if sourceVersion.isAtLeast(future) then
            deprecationWarning(
              em"`_` is deprecated for wildcard arguments of types: use `?` instead"
            )
      ...
      else if (isIdent(nme.MINUS) || isIdent(
          nme.PLUS
        )) && in.lookahead.token == USCORE && ctx.settings.YkindProjector.value == "underscores"
      then
        ...
        def singletonArgs(t: Tree): Tree =
          ...
          then
            singletonArgs(
              AppliedTypeTree(t, inParens(commaSeparated(singleton)))
            )
          else t
        singletonArgs(simpleType1())

    /** SimpleType1 ::= id \| Singleton `.' id \| Singleton `.' type \| ‘(’
      * ArgTypes ‘)’ \| Refinement \| TypeSplice -- deprecated syntax (since
      * 3.0.0) \| SimpleType1 TypeArgs \| SimpleType1 `#' id
      */
    def simpleType1() = simpleTypeRest {
      if in.token == LPAREN then
        ...
        atSpan(in.offset) {
          RefinedTypeTree(EmptyTree, refinement(indentOK = false))
        }
      else if isSplice then splice(isType = true)
      else
        def singletonCompletion(t: Tree): Tree =
          if in.token == DOT then
            ...
            else singletonCompletion(selector(t))
    ...
    private def simpleTypeRest(t: Tree): Tree = in.token match
      ...
      case LBRACKET =>
        simpleTypeRest(atSpan(startOffset(t)) {
              ...
              case Ident(tpnme.raw.LAMBDA) =>
                args match
                  case List(Function(params, body)) =>
                    val typeDefs = params.collect { case param @ Ident(name) =>
                      makeKindProjectorTypeDef(name.toTypeName).withSpan(
                        param.span
                      )
                    }
                    if typeDefs.length != params.length then fail()
          ...
          else AppliedTypeTree(applied, args)
        })
      case _ =>
        if !ctx.settings.YkindProjector.isDefault then
          t match
            case Tuple(params) =>
              val (newParams, tparams) = replaceKindProjectorPlaceholders(
                params
              )

              if tparams.isEmpty then t
              else LambdaTypeTree(tparams, Tuple(newParams))
            case _ => t
        else t
    ...
    def argTypes(namedOK: Boolean, wildOK: Boolean): List[Tree] =
      ...
      if namedOK && in.token == IDENTIFIER then
        in.currentRegion.withCommasExpected {
          argType() match
    ...
    /** ParamType ::= ParamValueType \| `=>' ParamValueType \| `->' [CaptureSet]
      * ParamValueType
      */
    ...
    /** TypeArgs ::= `[' Type {`,' Type} `]' NamedTypeArgs ::= `[' NamedTypeArg
      * {`,' NamedTypeArg} `]'
      */
    def typeArgs(namedOK: Boolean, wildOK: Boolean): List[Tree] = inBrackets(
      argTypes(namedOK, wildOK)
    )
    ...
    /** Does the current conditional expression continue after the initially
      * parsed (...) region?
      */
    def toBeContinued(altToken: Token): Boolean =
      inline def canContinue =
        !in.canStartStatTokens.contains(
          in.token
        ) // not statement, so take as continued expr
          || followedByToken(
            altToken
          ) // scan ahead to see whether we find a `then` or `do`
    ...
    def condExpr(altToken: Token): Tree =
      val t: Tree =
        ...
        else inSepRegion(InCond)(expr())
      if rewriteToOldSyntax(t.span.startPos) then revertToParens(t)
      accept(altToken)
      t

    /** Expr ::= [`implicit'] FunParams (‘=>’ | ‘?=>’) Expr \| HkTypeParamClause
      * ‘=>’ Expr \| Expr1 FunParams ::= Bindings \| id \| `_' ExprInParens ::=
      * PostfixExpr `:' Type \| Expr BlockResult ::= [‘implicit’] FunParams
      * (‘=>’ | ‘?=>’) Block \| HkTypeParamClause ‘=>’ Block \| Expr1 Expr1 ::=
      * [‘inline’] `if' `(' Expr `)' {nl} Expr [[semi] else Expr] \| [‘inline’]
      * `if' Expr `then' Expr [[semi] else Expr] \| `while' `(' Expr `)' {nl}
      * Expr \| `while' Expr `do' Expr \| `try' Expr Catches [`finally' Expr] \|
      * `try' Expr [`finally' Expr] \| `throw' Expr \| `return' [Expr] \|
      * ForExpr \| [SimpleExpr `.'] id `=' Expr \| PrefixOperator SimpleExpr `='
      * Expr \| SimpleExpr1 ArgumentExprs `=' Expr \| PostfixExpr [Ascription]
      * \| ‘inline’ InfixExpr MatchClause Bindings ::= `(' [Binding {`,'
      * Binding}] `)' Binding ::= (id | `_') [`:' Type] Ascription ::= `:'
      * InfixType \| `:' Annotation {Annotation} \| `:' `_' `*' Catches ::=
      ...
      in.token match
        ...
        case LBRACKET =>
          ...
          atSpan(start, arrowOffset) {
            getFunction(body) match
              ...
              case None =>
                syntaxError(
                  em"Implementation restriction: polymorphic function literals must have a value parameter",
                  arrowOffset
                )
        ...
        case _ =>
          ...
          def wrapPlaceholders(t: Tree) = try
            if placeholderParams.isEmpty then t
          ...
          if in.isArrow then
            placeholderParams =
              Nil // don't interpret `_' to the left of `=>` as placeholder
    ...
    def expr1(location: Location = Location.ElseWhere): Tree = in.token match
      ...
      case DO =>
        ...
        atSpan(start) {
          val body = expr()
          if isStatSep then in.nextToken()
      ...
      case RETURN =>
        atSpan(in.skipToken()) {
          Return(if isExprIntro then expr() else EmptyTree, EmptyTree)
        }
      case FOR =>
        forExpr()
      case _ =>
          ...
          in.token match
            ...
            case _ =>
              postfixExpr() match
                ...
                case t =>
                  syntaxError(
                    em"`inline` must be followed by an `if` or a `match`",
                    start
                  )
                  t
    ...
    def ascription(t: Tree, location: Location): Tree = atSpan(startOffset(t)) {
      in.token match
        case USCORE if in.lookahead.isIdent(nme.raw.STAR) =>
          ...
          else syntaxError(SeqWildcardPatternPos(), uscoreStart)
        ...
        case _ =>
          ...
          if isWildcard(t) && !location.inPattern then
            val vd :: rest = placeholderParams: @unchecked
            placeholderParams = cpy
              .ValDef(vd)(tpt = tpt)
              .withSpan(vd.span.union(tpt.span)) :: rest
          Typed(t, tpt)
    }

    /** `if' `(' Expr `)' {nl} Expr [[semi] else Expr] `if' Expr `then' Expr
      * [[semi] else Expr]
      */
    def ifExpr(start: Offset, mkIf: (Tree, Tree, Tree) => If): If =
      atSpan(start, in.skipToken()) {
        ...
        newLinesOpt()
        val thenp = subExpr()
        val elsep = if in.token == ELSE then
          in.nextToken(); subExpr()
        else EmptyTree
    ...
    def matchType(t: Tree): MatchTypeTree =
      atSpan(startOffset(t), accept(MATCH)) {
        MatchTypeTree(
          EmptyTree,
          t,
          inBracesOrIndented(caseClauses(typeCaseClause))
        )
      }

    /** FunParams ::= Bindings \| id \| `_' Bindings ::= `(' [[‘erased’] Binding
      * {`,' Binding}] `)'
      */
      ...
      atSpan(start, in.offset) {
        if in.token == CTXARROW then
          if params.isEmpty then
            syntaxError(
              em"context function literals require at least one formal parameter",
        ...
        val body =
          ...
          else if location == Location.InColonArg && in.token == INDENT then
            blockExpr()
          else expr()
    ...
    /** PostfixExpr ::= InfixExpr [id [nl]] InfixExpr ::= PrefixExpr \|
      * InfixExpr id [nl] InfixExpr \| InfixExpr id ColonArgument \| InfixExpr
      * MatchClause
      */
    ...
    val prefixExpr: Location => Tree = location =>
      ...
      then
        val start = in.offset
        val op = termIdent()
        if op.name == nme.raw.MINUS && isNumericLit then
          simpleExprRest(literal(start), location, canApply = true)
        else atSpan(start) { PrefixOp(op, simpleExpr(location)) }
      else simpleExpr(location)

    /** SimpleExpr ::= ‘new’ ConstrApp {`with` ConstrApp} [TemplateBody] \|
      * ‘new’ TemplateBody \| BlockExpr \| ExprSplice \| Quoted \| quoteId \|
      * SimpleExpr1 [`_`] SimpleExpr1 ::= literal \| xmlLiteral \| SimpleRef \|
      * `(` [ExprsInParens] `)` \| SimpleExpr `.` id \| SimpleExpr `.`
      * MatchClause \| SimpleExpr (TypeArgs | NamedTypeArgs) \| SimpleExpr1
      * ArgumentExprs \| SimpleExpr1 ColonArgument ColonArgument ::= colon
      * [LambdaStart] indent (CaseClauses | Block) outdent LambdaStart ::=
      * FunParams (‘=>’ | ‘?=>’) \| HkTypeParamClause ‘=>’ ColonArgBody ::=
      * indent (CaseClauses | Block) outdent Quoted ::= ‘'’ ‘{’ Block ‘}’ \| ‘'’
      * ‘[’ Type ‘]’
      */
    def simpleExpr(location: Location): Tree =
      ...
      val t = in.token match
        case XMLSTART =>
          xmlLiteral()
        case IDENTIFIER =>
          if isSplice then splice(isType = false)
          else simpleRef()
        ...
        case USCORE =>
          ...
          val param = ValDef(pname, TypeTree(), EmptyTree)
        ...
        case LPAREN =>
          atSpan(in.offset) {
            makeTupleOrParens(inParens(exprsInParensOrBindings()))
          }
        case LBRACE | INDENT =>
          canApply = false
          blockExpr()
        case QUOTE =>
          atSpan(in.skipToken()) {
            withinStaged(
              StageKind.Quoted | (if location.inPattern then
                                    StageKind.QuotedPattern
                                  else 0)
            ) {
              val body =
                if in.token == LBRACKET then inBrackets(typ())
                else stagedBlock()
              Quote(body, Nil)
            }
          }
        ...
        case _ =>
          if isLiteral then literal()
            ...
            syntaxErrorOrIncomplete(
              IllegalStartSimpleExpr(tokenString(in.token)),
              expectedOffset
            )
            errorTermTree(start)
    ...
    def simpleExprRest(
      ...
      if canApply then argumentStart()
      in.token match
        case DOT =>
          in.nextToken()
    ...
    def newExpr(): Tree =
      ...
      parents match
        case parent :: Nil if !in.isNestedStart =>
          reposition(
            if parent.isType then ensureApplied(wrapNew(parent)) else parent
          )
        case _ =>
    ...
    def parArgumentExprs(): (List[Tree], Boolean) = inParens {
      ...
      else (commaSeparated(argumentExpr), false)
    }

    /** ArgumentExprs ::= ParArgumentExprs \| [nl] BlockExpr
      */
    def argumentExprs(): (List[Tree], Boolean) =
      if in.isNestedStart then (blockExpr() :: Nil, false)
      else parArgumentExprs()
    ...
    /** ParArgumentExprss ::= {ParArgumentExprs}
      ...
      * Furthermore, `()` is considered a annotation argument only if it comes
      * first.
      */
    def parArgumentExprss(fn: Tree): Tree =
      def isLegalAnnotArg: Boolean =
        ...
        (lookahead.token == LPAREN) && {
          lookahead.nextToken()
          if lookahead.token == RPAREN then
            !fn.isInstanceOf[Trees.Apply[
              ?
            ]] // allow one () as annotation argument
          else if lookahead.token == IDENTIFIER then
            lookahead.nextToken()
            !lookahead.isColon
          else in.canStartExprTokens.contains(lookahead.token)
        }
      if in.token == LPAREN && (!inClassConstrAnnots || isLegalAnnotArg) then
        parArgumentExprss(
    ...
    def blockExpr(): Tree = atSpan(in.offset) {
      ...
      inDefScopeBraces {
        if in.token == CASE then
          Match(EmptyTree, caseClauses(() => caseClause()))
        else block(simplify)
      }
    }
    ...
    def block(simplify: Boolean = false): Tree =
      ...
      if stats.nonEmpty && isExpr(stats.last) then
        val inits = stats.init
        val last = stats.last
        if inits.isEmpty && (simplify || last.isInstanceOf[Block]) then last
    ...
    def guard(): Tree =
      if in.token == IF then
        in.nextToken(); postfixExpr(Location.InGuard)
      else EmptyTree
    ...
    def enumeratorsRest(): List[Tree] =
      if isStatSep then
        in.nextToken()
        if in.token == DO || in.token == YIELD || in.token == RBRACE then Nil
        else enumerator() :: enumeratorsRest()
      else if in.token == IF then guard() :: enumeratorsRest()
      else Nil
    ...
    /** Generator ::= [‘case’] Pattern `<-' Expr
      */
    def generator(): Tree =
      val casePat = if in.token == CASE then
        in.nextToken(); true
      else false
    ...
    def generatorRest(pat: Tree, casePat: Boolean): GenFrom =
      atSpan(startOffset(pat), accept(LARROW)) {
        val checkMode =
          ...
          else if sourceVersion.isAtLeast(`3.2`) then
            GenCheckMode.CheckAndFilter
          else
            GenCheckMode.FilterNow // filter on source version < 3.2, for backward compat
        GenFrom(pat, subExpr(), checkMode)
      }

    /** ForExpr ::= ‘for’ ‘(’ Enumerators ‘)’ {nl} [‘do‘ | ‘yield’] Expr \|
      * ‘for’ ‘{’ Enumerators ‘}’ {nl} [‘do‘ | ‘yield’] Expr \| ‘for’
      * Enumerators (‘do‘ | ‘yield’) Expr
      */
    def forExpr(): Tree =
      atSpan(in.skipToken()) {
        ...
        val enums =
          ...
          then
            in.nextToken()
            val res =
              if leading == LBRACE || in.token == CASE then enumerators()
              else
                val pats = patternsOpt()
                val pat =
                  if in.token == RPAREN || pats.length > 1 then
                    wrappedEnums = false
                    accept(RPAREN)
                    atSpan(start) {
                      makeTupleOrParens(pats)
                    } // note: alternatives `|' need to be weeded out by typer.
                  else pats.head
                generatorRest(pat, casePat = false) :: enumeratorsRest()
            if wrappedEnums then
              ...
              if in.newSyntax && in.rewrite && (leading == LBRACE || !hasMultiLineEnum)
              then
                // Don't rewrite if that could change meaning of newlines
                newLinesOpt()
                dropParensOrBraces(
                  start,
                  if in.token == YIELD || in.token == DO then "" else "do"
                )
            in.observeIndented()
            res
          else
            ...
            if in.token == INDENT then inBracesOrIndented(enumerators())
            else
              val ts = inSepRegion(InFor)(enumerators())
              if rewriteToOldSyntax(Span(start)) && ts.nonEmpty then
              ...
              ts
        newLinesOpt()
        if in.token == YIELD then
          in.nextToken()
          ForYield(enums, subExpr())
        else if in.token == DO then
          if rewriteToOldSyntax() then dropTerminator()
          in.nextToken()
          ForDo(enums, subExpr())
        else
          if !wrappedEnums then
            syntaxErrorOrIncomplete(YieldOrDoExpectedInForComprehension())
          ForDo(enums, expr())
      }

    /** CaseClauses ::= CaseClause {CaseClause} TypeCaseClauses ::=
      * TypeCaseClause {TypeCaseClause}
      */
    def caseClauses(clause: () => CaseDef): List[CaseDef] =
      val buf = new ListBuffer[CaseDef]
      buf += clause()
      while in.token == CASE do buf += clause()
      buf.toList

    /** CaseClause ::= ‘case’ Pattern [Guard] `=>' Block ExprCaseClause ::=
    ...
    def pattern(location: Location = Location.InPattern): Tree =
      ...
      if isIdent(nme.raw.BAR) then
        atSpan(startOffset(pat)) { Alternative(pat :: patternAlts(location)) }
      else pat

    def patternAlts(location: Location): List[Tree] =
      if isIdent(nme.raw.BAR) then
        in.nextToken(); pattern1(location) :: patternAlts(location)
      else Nil

    /** Pattern1 ::= PatVar Ascription \| [‘-’] integerLiteral Ascription \|
      * [‘-’] floatingPointLiteral Ascription \| Pattern2
      */
    ...
    /** SimplePattern ::= PatVar \| Literal \| Quoted \| XmlPattern \| `('
      * [Patterns] `)' \| SimplePattern1 [TypeArgs] [ArgumentPatterns]
      * SimplePattern1 ::= SimpleRef \| SimplePattern1 `.' id PatVar ::= id \|
      * `_'
      */
    def simplePattern(): Tree = in.token match
      case IDENTIFIER | BACKQUOTED_IDENT | THIS | SUPER =>
        simpleRef() match
          case id @ Ident(nme.raw.MINUS) if isNumericLit =>
            literal(startOffset(id))
      ...
      case _ =>
        if isLiteral then literal(inPattern = true)
        else
          val start = in.lastOffset
    ...
    def simplePatternRest(t: Tree): Tree =
      if in.token == DOT then
        ...
        if in.token == LBRACKET then
          p = atSpan(startOffset(t), in.offset) {
            TypeApply(p, typeArgs(namedOK = false, wildOK = false))
          }
        if in.token == LPAREN then
    ...
    def patternsOpt(location: Location = Location.InPattern): List[Tree] =
      if in.token == RPAREN then Nil else patterns(location)
    ...
    private def normalize(mods: Modifiers): Modifiers =
      if mods.is(Private) && mods.hasPrivateWithin then
        normalize(mods &~ Private)
      else if mods.isAllOf(AbstractOverride) then
    ...
    private def addModifier(mods: Modifiers): Modifiers =
      ...
      if mods.isOneOf(mod.flags) then
        syntaxError(RepeatedModifier(mod.flags.flagsString))
      addMod(mods, mod)

    def addFlag(mods: Modifiers, flag: FlagSet): Modifiers =
      mods.withAddedFlags(flag, Span(in.offset))

    /** Always add the syntactic `mod`, but check and conditionally add semantic
      * `mod.flags`
      */
    ...
    def accessQualifierOpt(mods: Modifiers): Modifiers =
      if in.token == LBRACKET then
        if mods.is(Local) || mods.hasPrivateWithin then
    ...
    /** {Annotation} {Modifier} Modifiers ::= {Modifier} LocalModifiers ::=
      * {LocalModifier} AccessModifier ::= (private | protected)
      * [AccessQualifier] Modifier ::= LocalModifier \| AccessModifier \|
      * override \| opaque LocalModifier ::= abstract | final | sealed | open |
      * implicit \| lazy | erased | inline | transparent
      */
    def modifiers(
        allowed: BitSet = modifierTokens,
        start: Modifiers = Modifiers()
    ): Modifiers =
      @tailrec
      def loop(mods: Modifiers): Modifiers =
        if allowed.contains(in.token)
          || in.isSoftModifier
          && localModifierTokens.subsetOf(
            allowed
          ) // soft modifiers are admissible everywhere local modifiers are
          && !in.lookahead.isColon
        then
          val isAccessMod = accessModifierTokens contains in.token
          val mods1 = addModifier(mods)
          loop(if isAccessMod then accessQualifierOpt(mods1) else mods1)
        else if in.isNewLine && (mods.hasFlags || mods.hasAnnotations) then
          in.nextToken()
          loop(mods)
        else mods
    ...
    /** Adjust start of annotation or constructor to offset of preceding @ or
      * new
      */
    def adjustStart(start: Offset)(tree: Tree): Tree =
      val tree1 = tree match
        ...
        case Select(qual, name) =>
          cpy.Select(tree)(adjustStart(start)(qual), name)
        case _ => tree
      if tree1.span.exists && start < tree1.span.start then
    ...
    def annotations(skipNewLines: Boolean = false): List[Tree] =
      if skipNewLines then newLinesOptWhenFollowedBy(AT)
      if in.token == AT then annot() :: annotations(skipNewLines)
      else Nil
    ...
    /** DefParamClauses ::= DefParamClause { DefParamClause } -- and two
      * DefTypeParamClause cannot be adjacent DefParamClause ::=
      * DefTypeParamClause \| DefTermParamClause \| UsingParamClause
      */
    def typeOrTermParamClauses(
      ...
      def recur(
          firstClause: Boolean,
          numLeadParams: Int,
          prevIsTypeClause: Boolean
      ): List[List[TypeDef] | List[ValDef]] =
        newLineOptWhenFollowedBy(LPAREN)
        newLineOptWhenFollowedBy(LBRACKET)
        if in.token == LPAREN then
          val paramsStart = in.offset
          val params = termParamClause(numLeadParams, firstClause = firstClause)
          val lastClause =
            params.nonEmpty && params.head.mods.flags.is(Implicit)
          params :: (if lastClause then Nil
                     else
                       recur(
                         firstClause = false,
                         numLeadParams + params.length,
                         prevIsTypeClause = false
                       )
          )
        else if in.token == LBRACKET then
          ...
          typeParamClause(ownerKind) :: recur(
            firstClause,
            numLeadParams,
            prevIsTypeClause = true
          )
        else Nil
      end recur
      ...
      recur(
        firstClause = true,
        numLeadParams = numLeadParams,
        prevIsTypeClause = false
      )
    end typeOrTermParamClauses

    /** ClsTypeParamClause::= ‘[’ ClsTypeParam {‘,’ ClsTypeParam} ‘]’
      * ClsTypeParam ::= {Annotation} [‘+’ | ‘-’] id [HkTypeParamClause]
      * TypeParamBounds
      *
      * DefTypeParamClause::= ‘[’ DefTypeParam {‘,’ DefTypeParam} ‘]’
      * DefTypeParam ::= {Annotation} [`sealed`] -- under captureChecking id
      * [HkTypeParamClause] TypeParamBounds
      *
      * TypTypeParamClause::= ‘[’ TypTypeParam {‘,’ TypTypeParam} ‘]’
      * TypTypeParam ::= {Annotation} id [HkTypePamClause] TypeBounds
      *
      * HkTypeParamClause ::= ‘[’ HkTypeParam {‘,’ HkTypeParam} ‘]’ HkTypeParam
      * ::= {Annotation} [‘+’ | ‘-’] (id [HkTypePamClause] | ‘_’) TypeBounds
      */
    def typeParamClause(ownerKind: ParamOwner): List[TypeDef] = inBrackets {

      def checkVarianceOK(): Boolean =
        val ok =
          ownerKind != ParamOwner.Def && ownerKind != ParamOwner.TypeParam
      ...
      def typeParam(): TypeDef =
        val isAbstractOwner =
          ownerKind == ParamOwner.Type || ownerKind == ParamOwner.TypeParam
        val start = in.offset
        ...
        if Feature.ccEnabled && in.token == SEALED then
          if ownerKind == ParamOwner.Def then mods |= Sealed
            ...
            syntaxError(
              em"`sealed` modifier only allowed for method type parameters"
            )
          in.nextToken()
        if isIdent(nme.raw.PLUS) && checkVarianceOK() then mods |= Covariant
        else if isIdent(nme.raw.MINUS) && checkVarianceOK() then
          mods |= Contravariant
        atSpan(start, nameStart) {
          val name =
            if isAbstractOwner && in.token == USCORE then
              in.nextToken()
              WildcardParamName.fresh().toTypeName
            else ident().toTypeName
          val hkparams = typeParamClauseOpt(ParamOwner.Type)
          val bounds =
            if isAbstractOwner then typeBounds() else typeParamBounds(name)
    ...
    def typeParamClauseOpt(ownerKind: ParamOwner): List[TypeDef] =
      if in.token == LBRACKET then typeParamClause(ownerKind) else Nil
    ...
    def contextTypes(
      ...
      tps.map(
        makeSyntheticParameter(
          nextIdx,
          _,
          paramFlags | Synthetic | impliedMods.flags
        )
      )

    /** ClsTermParamClause ::= ‘(’ ClsParams ‘)’ | UsingClsTermParamClause
      * UsingClsTermParamClause::= ‘(’ ‘using’ [‘erased’] (ClsParams |
      * ContextTypes) ‘)’ ClsParams ::= ClsParam {‘,’ ClsParam} ClsParam ::=
      * {Annotation}
      *
      * TypelessClause ::= DefTermParamClause \| UsingParamClause
      *
      * DefTermParamClause::= [nl] ‘(’ [DefTermParams] ‘)’ UsingParamClause ::=
      * ‘(’ ‘using’ (DefTermParams | ContextTypes) ‘)’ DefImplicitClause ::=
      * [nl] ‘(’ ‘implicit’ DefTermParams ‘)’ DefTermParams ::= DefTermParam
      * {‘,’ DefTermParam} DefTermParam ::= {Annotation} [‘erased’] [‘inline’]
      * Param
      *
    ...
    def termParamClause(
        ...
        firstClause: Boolean =
          false // clause is the first in regular list of clauses
    ): List[ValDef] =
      var impliedMods: Modifiers = EmptyModifiers

      def addParamMod(mod: () => Mod) = impliedMods =
        addMod(impliedMods, atSpan(in.skipToken()) { mod() })

      def paramMods() =
        if in.token == IMPLICIT then addParamMod(() => Mod.Implicit())
        else if isIdent(nme.using) then addParamMod(() => Mod.Given())

      def param(): ValDef =
        val start = in.offset
        var mods = impliedMods.withAnnotations(annotations())
        if isErasedKw then mods = addModifier(mods)
        if ofClass then
          mods = addFlag(modifiers(start = mods), ParamAccessor)
          mods = if in.token == VAL then
            in.nextToken()
            mods
            ...
            then syntaxError(em"`val` or `var` expected")
            if firstClause && ofCaseClass then mods
            else mods | PrivateLocal
        else
          if isIdent(nme.inline) && in.isSoftModifierInParamModifierPosition
          then mods = addModifier(mods)
          mods |= Param
        atSpan(start, nameStart) {
          val name = ident()
          acceptColon()
          if in.token == ARROW && ofClass && !mods.is(Local) then
            syntaxError(
              VarValParametersMayNotBeCallByName(name, mods.is(Mutable))
            )
          ...
          if impliedMods.mods.nonEmpty then
            impliedMods = impliedMods.withMods(
              Nil
            ) // keep only flags, so that parameter positions don't overlap
      ...
      inParens {
        if in.token == RPAREN && !prefix && !impliedMods.is(Given) then Nil
          ...
          val clause =
            if prefix && !isIdent(nme.using) && !isIdent(nme.erased) then
              param() :: Nil
            else
              paramMods()
              ...
              val (firstParamMod, isParams) =
                ...
                if in.lookahead.isColon then (mods, true)
                else
    ...
    /** ClsTermParamClauses ::= {ClsTermParamClause} [[nl] ‘(’ [‘implicit’]
      * ClsParams ‘)’] TypelessClauses ::= TypelessClause {TypelessClause}
      *
      * @return
      ...
      def recur(firstClause: Boolean, numLeadParams: Int): List[List[ValDef]] =
        ...
        if in.token == LPAREN then
          ...
          params :: (if lastClause then Nil
                       ...
                       recur(firstClause = false, numLeadParams + params.length)
          )
        else Nil
      end recur
    ...
    /** Import ::= `import' ImportExpr {‘,’ ImportExpr} Export ::= `export'
    ...
    def importOrExportClause(leading: Token, mkTree: ImportConstr): List[Tree] =
      ...
      commaSeparated(importExpr(mkTree)) match
        case t :: rest =>
          ...
          val firstPos =
            if t.span.exists then t.span.withStart(offset)
    ...
    def mkImport(outermost: Boolean = false): ImportConstr =
      (tree, selectors) =>
        val imp = Import(tree, selectors)
        languageImport(tree) match
          case Some(prefix) =>
            in.languageImportContext =
              in.languageImportContext.importContext(imp, NoSymbol)
            for case ImportSelector(
                id @ Ident(imported),
                EmptyTree,
                _
              ) <- selectors
            do
              if Feature.handleGlobalLanguageImport(
                  prefix,
                  imported
                ) && !outermost
              then
                syntaxError(
                  em"this language import is only allowed at the toplevel",
                  id.span
                )
              if allSourceVersionNames.contains(imported) && prefix.isEmpty then
                if !outermost then
                  syntaxError(
                    em"source version import is only allowed at the toplevel",
                    id.span
                  )
                  ...
                  val candidate =
                    val nonMigration =
                      imported.toString.replace("-migration", "")
                  ...
                  val msg = candidate match
                    case Some(member) =>
                      baseMsg.append(i", did you mean language.`$member`?")
                    case _ => baseMsg
                  syntaxError(msg, id.span)
                else
                  ctx.compilationUnit.sourceVersion = Some(
                    SourceVersion.valueOf(imported.toString)
                  )
            end for
          case None =>
        end match
    ...
    /** ImportExpr ::= SimpleRef {‘.’ id} ‘.’ ImportSpec \| SimpleRef ‘as’ id
      * ImportSpec ::= NamedSelector \| WildcardSelector \| ‘{’ ImportSelectors
      * ‘}’ ImportSelectors ::= NamedSelector [‘,’ ImportSelectors] \|
      * WildCardSelector {‘,’ WildCardSelector} NamedSelector ::= id [‘as’ (id |
      * ‘_’)] WildCardSelector ::= ‘*' | ‘given’ [InfixType]
      */
    def importExpr(mkTree: ImportConstr): () => Tree =
      ...
      def givenSelector() =
        ImportSelector(
          ...
          bound =
            if canStartInfixTypeTokens.contains(in.token) then
              rejectWildcardType(infixType())
            else EmptyTree
        )
      ...
      def namedSelector(from: Ident) =
        if in.token == ARROW || isIdent(nme.as) then
          if in.token == ARROW && sourceVersion.isAtLeast(future) then
            ...
            patch(
              source,
              Span(in.offset, in.offset + 2),
              if testChar(in.offset - 1, ' ') && testChar(in.offset + 2, ' ')
              then "as"
              else " as "
            )
          ...
          in.token match
            ...
            case _ =>
              if isIdent(nme.raw.STAR) then wildcardSelector()
                ...
                if !idOK then
                  syntaxError(em"named imports cannot follow wildcard imports")
      ...
      def importSelectors(): List[ImportSelector] =
        ...
        commaSeparated { () =>
          val isWildcard =
            in.token == USCORE || in.token == GIVEN || isIdent(nme.raw.STAR)
        ...
        if in.isIdent(nme.as) && qual.isInstanceOf[RefTree] then
          qual match
            case Select(qual1, name) =>
              val from =
                Ident(name).withSpan(Span(qual.span.point, qual.span.end, 0))
          ...
          in.token match
            ...
            case _ =>
              if isIdent(nme.raw.STAR) then
                ...
                val name = ident()
                if in.token == DOT then
                  importSelection(atSpan(startOffset(qual), start) {
                    Select(qual, name)
                  })
                else
                  mkTree(
                    qual,
                    namedSelector(atSpan(start) { Ident(name) }) :: Nil
                  )
      end importSelection
    ...
    /** Def ::= val PatDef \| var VarDef \| def DefDef \| type {nl} TypeDcl \|
      * TmplDef Dcl ::= val ValDcl \| var ValDcl \| def DefDcl \| type {nl}
      * TypeDcl EnumCase ::= `case' (id ClassConstr [`extends' ConstrApps]] |
      * ids)
      */
    ...
    /** PatDef ::= ids [‘:’ Type] ‘=’ Expr \| Pattern2 [‘:’ Type] ‘=’ Expr
      * VarDef ::= PatDef \| id {`,' id} `:' Type `=' `_' (deprecated in 3.x)
      * ValDcl ::= id {`,' id} `:' Type VarDcl ::= id {`,' id} `:' Type
      */
      ...
      atSpan(start, nameStart) {
        ...
        val rhs =
          if tpt.isEmpty || in.token == EQUALS then
            ...
            subExpr() match
              case rhs0 @ Ident(name)
                  if placeholderParams.nonEmpty && name == placeholderParams.head.name
                    && !tpt.isEmpty && mods
                      .is(Mutable) && lhs.forall(_.isInstanceOf[Ident]) =>
                if sourceVersion.isAtLeast(future) then
                  deprecationWarning(
        ...
        lhs match
          case IdPattern(id, t) :: Nil if t.isEmpty =>
            ...
            if isBackquoted(id) then vdef.pushAttachment(Backquoted, ())
          ...
          case _ =>
            ...
            val rhs2 =
              if rhs.isEmpty && !isAllIds then
                val start = in.lastOffset
                syntaxError(
                  ExpectedTokenButFound(EQUALS, in.token),
                  Span(in.lastOffset)
                )
                errorTermTree(start)
              else rhs
    ...
    /** DefDef ::= DefSig [‘:’ Type] ‘=’ Expr \| this TypelessClauses
      * [DefImplicitClause] `=' ConstrExpr DefDcl ::= DefSig `:' Type DefSig ::=
      * id [DefTypeParamClause] DefTermParamClauses
      *
      * if clauseInterleaving is enabled: DefSig ::= id [DefParamClauses]
      * [DefImplicitClause]
      */
    def defDefOrDcl(
        start: Offset,
      ...
      if in.token == THIS then
        ...
        if vparamss.isEmpty || vparamss.head
            .take(1)
            .exists(_.mods.isOneOf(GivenOrImplicit))
        then
          in.token match
            case LBRACKET => syntaxError(em"no type parameters allowed here")
            case EOF =>
              incompleteInputError(AuxConstructorNeedsNonImplicitParameter())
            case _ =>
              syntaxError(AuxConstructorNeedsNonImplicitParameter(), nameStart)
        if migrateTo3 then newLineOptWhenFollowedBy(LBRACE)
        val rhs =
          if !(in.token == LBRACE && scala2ProcedureSyntax("")) then
            accept(EQUALS)
          atSpan(in.offset) { subPart(constrExpr) }
        makeConstructor(Nil, vparamss, rhs)
          .withMods(mods)
          .setComment(in.getDocComment(start))
      else
        ...
        val rhs =
          if in.token == EQUALS then
            in.nextToken()
            subExpr()
          else if !tpt.isEmpty then EmptyTree
          else if scala2ProcedureSyntax(": Unit") then
            tpt = scalaUnit
            if in.token == LBRACE then expr()
            else EmptyTree
          else
            if !isExprIntro then syntaxError(MissingReturnType(), in.lastOffset)
            accept(EQUALS)
            expr()

        val ddef = DefDef(name, paramss, tpt, rhs)
        if isBackquoted(ident) then ddef.pushAttachment(Backquoted, ())
    ...
    val constrExpr: () => Tree = () =>
      if in.isNestedStart then
        atSpan(in.offset) {
          inBracesOrIndented {
            val stats = selfInvocation() :: (if isStatSep then
    ...
    def typeDefOrDcl(start: Offset, mods: Modifiers): Tree =
      ...
      atSpan(start, nameStart) {
        ...
        def makeTypeDef(rhs: Tree): Tree =
          ...
          if nameIdent.isBackquoted then tdef.pushAttachment(Backquoted, ())
              ...
              rhs match
                case mtt: MatchTypeTree =>
                  bounds match
                    ...
                    case _ =>
                      syntaxError(
                        em"cannot combine lower bound and match type alias",
                        eqOffset
                      )
                case _ =>
                  if mods.is(Opaque) then
                    rhs = TypeBoundsTree(bounds.lo, bounds.hi, rhs)
                  else syntaxError(em"cannot combine bound and alias", eqOffset)
              makeTypeDef(rhs)
    ...
    /** TmplDef ::= ([‘case’] ‘class’ | ‘trait’) ClassDef \| [‘case’] ‘object’
      * ObjectDef \| ‘enum’ EnumDef \| ‘given’ GivenDef
      */
    ...
    /** ClassConstr ::= [ClsTypeParamClause] [ConstrMods] ClsTermParamClauses
      */
    def classConstr(isCaseClass: Boolean = false): DefDef =
      atSpan(in.lastOffset) {
        val tparams = typeParamClauseOpt(ParamOwner.Class)
        val cmods = fromWithinClassConstr(constrModsOpt())
        val vparamss =
          termParamClauses(ofClass = true, ofCaseClass = isCaseClass)
      ...
      atSpan(start, nameStart) {
        ...
        if in.token == COMMA then
          ...
          val caseDef =
            ...
            else ModuleDef(id.name.toTermName, caseTemplate(emptyConstructor))
    ...
    def checkExtensionMethod(
      ...
      case stat: DefDef =>
        if stat.mods.is(ExtensionMethod) && vparamss.nonEmpty then
          syntaxError(
            em"no extension method allowed here since leading parameter was already given",
            stat.span
          )
          ...
          syntaxError(
            em"extension method cannot have type parameters since some were already given previously",
    ...
    /** GivenDef ::= [GivenSig] (AnnotType [‘=’ Expr] | StructuralInstance)
      ...
      */
    def givenDef(start: Offset, mods: Modifiers, givenMod: Mod) =
      atSpan(start, nameStart) {
        var mods1 = addMod(mods, givenMod)
        val nameStart = in.offset
        val name =
          if isIdent && followingIsGivenSig() then ident() else EmptyTermName
        ...
        val gdef =
          ...
          if in.token == EQUALS && parentsIsType then
            ...
            val vparamss1 = vparamss.map(
              _.map(vparam =>
                vparam.withMods(
                  vparam.mods &~ Param | ParamAccessor | Protected
                )
              )
            )
    ...
    def extension(): ExtMethods =
      ...
      val methods: List[Tree] =
        if in.token == EXPORT then exportClause()
        else if isDefIntro(modifierTokens) then extMethod(numLeadParams) :: Nil
        else
          in.observeIndented()
          newLineOptWhenFollowedBy(LBRACE)
          if in.isNestedStart then inDefScopeBraces(extMethods(numLeadParams))
          else
            syntaxErrorOrIncomplete(em"Extension without extension methods");
            Nil
    ...
    def extMethods(numLeadParams: Int): List[Tree] =
      checkNoEscapingPlaceholders {
        ...
        while
          val start = in.offset
          if in.token == EXPORT then meths ++= exportClause()
          else
            val mods = defAnnotsMods(modifierTokens)
    ...
    /** Template ::= InheritClauses [TemplateBody] InheritClauses ::= [‘extends’
      * ConstrApps] [‘derives’ QualId {‘,’ QualId}]
      */
    def template(constr: DefDef, isEnum: Boolean = false): Template =
      val parents =
        if in.token == EXTENDS then
          in.nextToken()
          if in.token == LBRACE || in.token == COLONeol then
            report.errorOrMigrationWarning(
    ...
    def templateOpt(constr: DefDef): Template =
      newLinesOptWhenFollowedBy(nme. derives)
      if in.token == EXTENDS || isIdent(nme. derives) then template(constr)
      else
        possibleTemplateStart()
        if in.isNestedStart then template(constr)
        else
          checkNextNotIndented()
          Template(constr, Nil, Nil, EmptyValDef, Nil)

    /** TemplateBody ::= [nl] `{' TemplateStatSeq `}' EnumBody ::= [nl] ‘{’
    ...
    def templateBodyOpt(
      ...
      val (self, stats) =
        if in.isNestedStart then templateBody(parents)
        else
          checkNextNotIndented()
          (EmptyValDef, Nil)
      Template(constr, parents, derived, self, stats)
    ...
    def withTemplate(constr: DefDef, parents: List[Tree]): Template =
      ...
      Template(constr, parents, Nil, self, stats)
        .withSpan(
          Span(constr.span.orElse(parents.head.span).start, in.lastOffset)
        )
    ...
    /** TopStatSeq ::= TopStat {semi TopStat} TopStat ::= Import \| Export \|
      * Annotations Modifiers Def \| Packaging \| package object objectDef \|
      * Extension \|
      */
    def topStatSeq(outermost: Boolean = false): List[Tree] =
      ...
      while
        ...
        if in.token == PACKAGE then
          val start = in.skipToken()
          if in.token == OBJECT then
            in.nextToken()
            stats += objectDef(start, Modifiers(Package))
          else stats += packaging(start)
        else if in.token == IMPORT then stats ++= importClause(outermost)
        else if in.token == EXPORT then stats ++= exportClause()
      ...
      then
        atSpan(in.offset) {
          ...
          if in.token == ARROW then
            ...
            in.nextToken()
          else syntaxError(em"`=>` expected after self type")
          makeSelfDef(selfName, selfTpt)
        }
      else EmptyValDef

    /** TemplateStatSeq ::= [SelfType] TemplateStat {semi TemplateStat}
      * TemplateStat ::= Import \| Export \| Annotations Modifiers Def \|
      * Annotations Modifiers Dcl \| Extension \| Expr1 \| EnumStat ::=
      * TemplateStat \| Annotations Modifiers EnumCase
      */
    def templateStatSeq(): (ValDef, List[Tree]) = checkNoEscapingPlaceholders {
      ...
      while
        var empty = false
        if in.token == IMPORT then stats ++= importClause()
        else if in.token == EXPORT then stats ++= exportClause()
        else if isIdent(nme.extension) && followingIsExtension() then
          stats += extension()
        else if isDefIntro(modifierTokensOrCase) then
          stats +++= defOrDcl(in.offset, defAnnotsMods(modifierTokens))
        else if isExprIntro then stats += expr1()
        else empty = true
    ...
    /** RefineStatSeq ::= RefineStat {semi RefineStat} RefineStat ::= ‘val’
      * VarDcl \| ‘def’ DefDcl \| ‘type’ {nl} TypeDcl (in reality we admit Defs
      * and vars and filter them out afterwards in `checkLegal`)
      */
    def refineStatSeq(): List[Tree] =
      ...
      def checkLegal(tree: Tree): List[Tree] =
        ...
        tree match
          case tree: ValDef if tree.mods.is(Mutable) =>
            fail(
              em"""refinement cannot be a mutable var.
          ...
          case tree: MemberDef if !(tree.mods.flags & ModifierFlags).isEmpty =>
            fail(
              em"refinement cannot be ${(tree.mods.flags & ModifierFlags).flagStrings().mkString("`", "`, `", "`")}"
            )
      ...
      while
        ...
        if dclFound then stats ++= checkLegal(defOrDcl(in.offset, Modifiers()))
        var what = "declaration"
        if inFunReturnType then
          what += " (possible cause: missing `=` in front of current method body)"
    ...
    def localDef(start: Int, implicitMods: Modifiers = EmptyModifiers): Tree =
      ...
      for imod <- implicitMods.mods do mods = addMod(mods, imod)
      if mods.is(Final) then
        // A final modifier means the local definition is "class-like".  // FIXME: Deal with modifiers separately
        tmplDef(start, mods)
      else defOrDcl(start, mods)

    /** BlockStatSeq ::= { BlockStat semi } [Expr] BlockStat ::= Import \|
      * Annotations [implicit] [lazy] Def \| Annotations LocalModifiers TmplDef
      * \| Extension \| Expr1 \|
      */
    def blockStatSeq(): List[Tree] = checkNoEscapingPlaceholders {
      ...
      while
        var empty = false
        if in.token == IMPORT then stats ++= importClause()
        else if isExprIntro then stats += expr(Location.InBlock)
        else if in.token == IMPLICIT && !in.inModifierPosition() then
          stats += closure(
            in.offset,
            Location.InBlock,
            modifiers(BitSet(IMPLICIT))
          )
        else if isIdent(nme.extension) && followingIsExtension() then
          stats += extension()
        else if isDefIntro(
            localModifierTokens,
            excludedSoftModifiers = Set(nme.`opaque`)
          )
        then stats +++= localDef(in.offset)
        else empty = true
    ...
    def compilationUnit(): Tree = checkNoEscapingPlaceholders {
      def topstats(): List[Tree] =
        ...
        while in.token == SEMI do in.nextToken()
        val start = in.offset
        if in.token == PACKAGE then
          in.nextToken()
          if in.token == OBJECT then
            ...
            var continue = false
            possibleTemplateStart()
            if in.token == EOF then ts += makePackaging(start, pkg, List())
            else if in.isNestedStart then
              ts += inDefScopeBraces(
                makePackaging(start, pkg, topStatSeq()),
                rewriteWithColon = true
              )
              continue = true
            else
              acceptStatSep()
            ...
            if continue then
              statSepOrEnd(ts, what = "toplevel definition")
              ts ++= topStatSeq()
        else ts ++= topStatSeq(outermost = true)
      ...
      topstats() match
        ...
        case Nil =>
          EmptyTree // without this case we'd get package defs without positions
  ...
  /** OutlineParser parses top-level declarations in `source` to find declared
    * classes, ignoring their bodies (which must only have balanced braces).
    * This is used to map class names to defining sources.
    */
  class OutlineParser(source: SourceFile)(using Context)
      extends Parser(source)
      with OutlineParserCommon:

    def skipBracesHook(): Option[Tree] =
      if in.token == XMLSTART then Some(xmlLiteral()) else None
