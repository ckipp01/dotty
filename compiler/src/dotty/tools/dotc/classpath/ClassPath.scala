...
case class ClassPathEntries(
    packages: scala.collection.Seq[PackageEntry],
    classesAndSources: scala.collection.Seq[ClassRepresentation]
):
  def toTuple: (
      scala.collection.Seq[PackageEntry],
      scala.collection.Seq[ClassRepresentation]
  ) = (packages, classesAndSources)
...
case class PackageName(dottedString: String):
  val dirPathTrailingSlashJar: String =
    FileUtils.dirPathInJar(dottedString) + "/"

  val dirPathTrailingSlash: String =
    if java.io.File.separatorChar == '/' then dirPathTrailingSlashJar
    else FileUtils.dirPath(dottedString) + java.io.File.separator
...
private[dotty] case class ClassFileEntryImpl(file: AbstractFile)
...
private[dotty] case class SourceFileEntryImpl(file: AbstractFile)
...
private[dotty] case class ClassAndSourceFilesEntry(
