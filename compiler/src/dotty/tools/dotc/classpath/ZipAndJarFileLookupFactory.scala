...
/** A trait providing an optional cache for classpath entries obtained from zip
  * and jar files. It allows us to e.g. reduce significantly memory used by
  * PresentationCompilers in Scala IDE when there are a lot of projects having a
  * lot of common dependencies.
  */
sealed trait ZipAndJarFileLookupFactory:
  ...
  def create(zipFile: AbstractFile)(using Context): ClassPath =
    val release =
      Option(ctx.settings.javaOutputVersion.value).filter(_.nonEmpty)
    if ctx.settings.YdisableFlatCpCaching.value || zipFile.file == null then
      createForZipFile(zipFile, release)
    else createUsingCache(zipFile, release)

  protected def createForZipFile(
...
object ZipAndJarClassPathFactory extends ZipAndJarFileLookupFactory:
  private case class ZipArchiveClassPath(
      zipFile: File,
      override val release: Option[String]
    ...
    override private[dotty] def classes(
        inPackage: PackageName
    ): Seq[ClassFileEntry] = files(inPackage)

    override protected def createFileEntry(
        file: FileZipArchive#Entry
    ): ClassFileEntryImpl = ClassFileEntryImpl(file)
    override protected def isRequiredFileType(file: AbstractFile): Boolean =
      file.isClass

  /** This type of classpath is closely related to the support for JSR-223. Its
    * usage can be observed e.g. when running: jrunscript -classpath
    * scala-compiler.jar;scala-reflect.jar;scala-library.jar -l scala with a
    * particularly prepared scala-library.jar. It should have all classes listed
    * in the manifest like e.g. this entry: Name: scala/Function2$mcFJD$sp.class
    */
  private case class ManifestResourcesClassPath(file: ManifestResources)
      extends ClassPath
      with NoSourcePaths:
    override def findClassFile(className: String): Option[AbstractFile] =
      val (pkg, simpleClassName) =
    ...
    /** A cache mapping package name to abstract file for package directory and
      * subpackages of given package.
      *
      * ManifestResources can iterate through the collections of entries from
      * e.g. remote jar file. We can't just specify the path to the concrete
      * directory etc. so we can't just 'jump' into given package, when it's
      * needed. On the other hand we can iterate over entries to get
      * AbstractFiles, iterate over entries of these files etc.
      *
      * Instead of traversing a tree of AbstractFiles once and caching all
      * entries or traversing each time, when we need subpackages of a given
      * package or its classes, we traverse once and cache only packages.
      * Classes for given package can be then easily loaded when they are
      * needed.
      */
    private lazy val cachedPackages: util.HashMap[String, PackageFileInfo] =
      ...
      def traverse(
          ...
          subpackagesQueue: collection.mutable.Queue[PackageInfo]
      ): Unit = filesForPrefix match
    ...
    override private[dotty] def packages(
        inPackage: PackageName
    ): Seq[PackageEntry] = cachedPackages.get(inPackage.dottedString) match
      case None => Seq.empty
      case Some(PackageFileInfo(_, subpackages)) =>
        subpackages.map(packageFile =>
          PackageEntryImpl(inPackage.entryName(packageFile.name))
        )

    override private[dotty] def classes(
        inPackage: PackageName
    ): Seq[ClassFileEntry] = cachedPackages.get(inPackage.dottedString) match
      case None => Seq.empty
    ...
    override private[dotty] def hasPackage(pkg: PackageName) =
      cachedPackages.contains(pkg.dottedString)
  ...
  end ManifestResourcesClassPath
  ...
  override protected def createForZipFile(
    ...
    if zipFile.file == null then createWithoutUnderlyingFile(zipFile)
...
object ZipAndJarSourcePathFactory extends ZipAndJarFileLookupFactory:
  private case class ZipArchiveSourcePath(zipFile: File)
    ...
    override private[dotty] def sources(
        inPackage: PackageName
    ): Seq[SourceFileEntry] = files(inPackage)
  ...
  override protected def createForZipFile(
      zipFile: AbstractFile,
      release: Option[String]
  ): ClassPath = ZipArchiveSourcePath(zipFile.file)

final class FileBasedCache[T]:
  private case class Stamp(lastModified: FileTime, fileKey: Object)
  private val cache =
    collection.mutable.Map.empty[java.nio.file.Path, (Stamp, T)]
