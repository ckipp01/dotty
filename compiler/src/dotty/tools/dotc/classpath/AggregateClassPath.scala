...
/** A classpath unifying multiple class- and sourcepath entries. The Classpath
  * can obtain entries for classes and sources independently so it tries to do
  * operations quite optimally - iterating only these collections which are
  * needed in the given moment and only as far as it's necessary.
  *
  * @param aggregates
...
case class AggregateClassPath(aggregates: Seq[ClassPath]) extends ClassPath:
  override def findClassFile(className: String): Option[AbstractFile] =
    ...
    aggregatesForPackage(PackageName(pkg)).iterator
      .map(_.findClassFile(className))
      .collectFirst { case Some(x) =>
        x
      }
  private val packageIndex: collection.mutable.Map[String, Seq[ClassPath]] =
    collection.mutable.Map()
  private def aggregatesForPackage(pkg: PackageName): Seq[ClassPath] =
    packageIndex.synchronized {
      packageIndex.getOrElseUpdate(
        pkg.dottedString,
        aggregates.filter(_.hasPackage(pkg))
      )
    }
  ...
  override def findClass(className: String): Option[ClassRepresentation] =
    ...
    (classEntry, sourceEntry) match
      case (Some(c: ClassFileEntry), Some(s: SourceFileEntry)) =>
        Some(ClassAndSourceFilesEntry(c.file, s.file))
      case (c @ Some(_), _) => c
      case (_, s)           => s
  ...
  override private[dotty] def hasPackage(pkg: PackageName): Boolean =
    aggregates.exists(_.hasPackage(pkg))
  override private[dotty] def list(inPackage: PackageName): ClassPathEntries =
    ...
    val classesAndSourcesBuffer =
      collection.mutable.ArrayBuffer[ClassRepresentation]()
  ...
  /** Returns only one entry for each name. If there's both a source and a class
    * entry, it creates an entry containing both of them. If there would be more
    * than one class or source entries for the same class it always would use
    * the first entry of each type found on a classpath.
    */
  private def mergeClassesAndSources(
      entries: scala.collection.Seq[ClassRepresentation]
    ...
    do
      val name = entry.name
      if indices.contains(name) then
        val index = indices(name)
        val existing = mergedEntries(index)

        if existing.binary.isEmpty && entry.binary.isDefined then
          mergedEntries(index) =
  ...
  private def getDistinctEntries[EntryType <: ClassRepresentation](
      getEntries: ClassPath => Seq[EntryType]
  ): Seq[EntryType] =
