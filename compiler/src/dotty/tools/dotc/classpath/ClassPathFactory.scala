...
/** Provides factory methods for classpath. When creating classpath instances
  * for a given path, it uses proper type of classpath depending on a types of
  * particular files containing sources or classes.
  */
class ClassPathFactory:
  ...
  def classesInExpandedPath(path: String)(using
      Context
  ): IndexedSeq[ClassPath] =
    classesInPathImpl(path, expand = true).toIndexedSeq

  def classesInPath(path: String)(using Context): List[ClassPath] =
    classesInPathImpl(path, expand = false)

  def classesInManifest(
      useManifestClassPath: Boolean
  )(using Context): List[ClassPath] =
    if useManifestClassPath then
      dotty.tools.io.ClassPath.manifests.map(url =>
        newClassPath(AbstractFile getResources url)
      )
    else Nil

  // Internal
  protected def classesInPathImpl(path: String, expand: Boolean)(using
      Context
  ): List[ClassPath] =
    for
      file <- expandPath(path, expand)
      dir <-
        def asImage = if file.endsWith(".jimage") then
          Some(AbstractFile.getFile(file))
        else None
  ...
  private def createSourcePath(file: AbstractFile)(using Context): ClassPath =
    if file.isJarOrZip then ZipAndJarSourcePathFactory.create(file)
    else if file.isDirectory then new DirectorySourcePath(file.file)
    else sys.error(s"Unsupported sourcepath element: $file")
end ClassPathFactory
...
object ClassPathFactory:
  def newClassPath(file: AbstractFile)(using Context): ClassPath = file match
    ...
    case _ =>
      if file.isJarOrZip then ZipAndJarClassPathFactory.create(file)
      else if file.isDirectory then new DirectoryClassPath(file.file)
