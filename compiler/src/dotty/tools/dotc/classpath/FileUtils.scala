...
import java.io.{File as JFile, FileFilter}
import java.net.URL
import dotty.tools.io.AbstractFile

/** Common methods related to Java files and abstract files used in the context
  * of classpath
  */
object FileUtils:
  extension (file: AbstractFile)
    ...
    def isClass: Boolean = !file.isDirectory && file.hasExtension(
      "class"
    ) && !file.name.endsWith("$class.class")
    ...
    /** Safe method returning a sequence containing one URL representing this
      * file, when underlying file exists, and returning given default value in
      * other case
      */
    def toURLs(default: => Seq[URL] = Seq.empty): Seq[URL] =
      if file.file == null then default else Seq(file.toURL)

  extension (file: JFile)
    def isPackage: Boolean = file.isDirectory && mayBeValidPackage(file.getName)

    def isClass: Boolean =
      file.isFile && file.getName.endsWith(".class") && !file.getName.endsWith(
        "$class.class"
      )
    // FIXME: drop last condition when we stop being compatible with Scala 2.11
  ...
  def stripSourceExtension(fileName: String): String =
    if endsScala(fileName) then stripClassExtension(fileName)
    else if endsJava(fileName) then stripJavaExtension(fileName)
    else throw new FatalError("Unexpected source file ending: " + fileName)

  def dirPath(forPackage: String): String =
    forPackage.replace('.', JFile.separatorChar)

  def dirPathInJar(forPackage: String): String = forPackage.replace('.', '/')

  inline private def ends(filename: String, suffix: String) =
    filename.endsWith(suffix) && filename.length > suffix.length
  ...
  def stripJavaExtension(fileName: String): String =
    fileName.substring(
      0,
      fileName.length - 5
    ) // equivalent of fileName.length - SUFFIX_JAVA.length
