...
/** A trait allowing to look for classpath entries in directories. It provides
  * common logic for classes handling class and source files. It makes use of
  * the fact that in the case of nested directories it's easy to find a file
  * when we have a name of a package. It abstracts over the file representation
  * to work with both JFile and AbstractFile.
  */
...
trait JFileDirectoryLookup[FileEntryType <: ClassRepresentation]
  ...
  protected def getSubDir(packageDirName: String): Option[JFile] =
    ...
    if packageDir.exists && packageDir.isDirectory then Some(packageDir)
    else None
  protected def listChildren(
      dir: JFile,
      filter: Option[JFile => Boolean]
  ): Array[JFile] =
      ...
      release match
        case Some(v) if v.toInt < currentMajorVersion =>
          try
            ...
            if Files.notExists(ctSym) then None
...
/** Implementation `ClassPath` based on the JDK 9 encapsulated runtime modules
  * (JEP-220)
  *
  * https://bugs.openjdk.java.net/browse/JDK-8066492 is the most up to date
  * reference for the structure of the jrt:// filesystem.
  *
  ...
  private val packageToModuleBases: Map[String, Seq[Path]] =
    ...
    def lookup(pack: Path): Seq[Path] =
      Files
        ...
        .asScala
        .map(l =>
          if Files.isSymbolicLink(l) then Files.readSymbolicLink(l) else l
        )
        .toList
    ...
    if !className.contains(".") then None
      ...
      packageToModuleBases
        ...
        .flatMap { x =>
          ...
          if Files.exists(file) then file.toPlainFile :: Nil else Nil
        }
        .take(1)
        .toList
        .headOption
...
final class CtSymClassPath(ctSym: java.nio.file.Path, release: Int)
  ...
  private def codeFor(major: Int): String =
    if major < 10 then major.toString else ('A' + (major - 10)).toChar.toString
  ...
  private val rootsForRelease: List[Path] =
    ...
    if !className.contains(".") then None
      ...
      packageIndex
        ...
        .flatMap { p =>
          ...
          if Files.exists(path) then path.toPlainFile :: Nil else Nil
        }
        .take(1)
        .toList
        .headOption
...
case class DirectoryClassPath(dir: JFile)
    extends JFileDirectoryLookup[ClassFileEntryImpl]
    with NoSourcePaths:
...
case class DirectorySourcePath(dir: JFile)
    extends JFileDirectoryLookup[SourceFileEntryImpl]
    with NoClassPaths:
