...
/** A dense map from some `Key` type to `Int. Dense means: All keys and values
  * are stored in arrays from 0 up to the size of the map. Keys and values can
  * be obtained by index using `key(index)` and `value(index)`. Values can also
  * be stored using `setValue(index, value)`.
  *
...
final class IntMap[Key](initialCapacity: Int = 8, capacityMultiple: Int = 2)
  ...
  def update(k: Key, v: Int): Unit =
    val idx = add(
      k
    ) // don't merge the two statements, `add` might change `values`.
    values(idx) = v
