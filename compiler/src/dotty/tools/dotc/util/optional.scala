...
/** Return type that indicates that the method returns a T or aborts to the
  * enclosing boundary with a `None`
  */
type optional[T] = boundary.Label[None.type] ?=> T

/** A prompt for `Option`, which establishes a boundary which `_.?` on `Option`
  * can return
  */
object optional:
