...
object NameTransformer:
  ...
  /** Replace operator symbols by corresponding expansion strings, and replace
    ...
    * character's unicode expansion. Note that no attempt is made to escape the
    * use of '$' in `name`: blindly escaping them might make it impossible to
    * call some platform APIs. This unfortunately means that
    * `decode(encode(name))` might not be equal to `name`, this is considered
    * acceptable since '$' is a reserved character in the Scala spec as well as
    * the Java spec.
    */
  def encode(name: SimpleName): SimpleName =
    ...
    while i < len do
      val c = name(i)
      if c < nops && (op2code(c.toInt) ne null) then
        if buf eq null then
          buf = new StringBuilder()
          buf.append(name.sliceToString(0, i))
        buf.append(op2code(c.toInt))
        /* Handle glyphs that are not valid Java/JVM identifiers */
      else if !Character.isJavaIdentifierPart(c) then
        if buf eq null then
          buf = new StringBuilder()
          buf.append(name.sliceToString(0, i))
        buf.append("$u%04X".format(c.toInt))
      else if buf ne null then buf.append(c)
      i += 1
    if buf eq null then name else termName(buf.toString)

  /** Replace operator expansions by the operators themselves, and decode
    * `$u....` expansions into unicode characters.
    */
    ...
    while i < len do
      ...
      if c == '$' && i + 2 < len then
        ...
        if 'a' <= ch1 && ch1 <= 'z' then
          ...
          if 'a' <= ch2 && ch2 <= 'z' then
            ...
            while (ops ne null) && !name.startsWith(ops.code, i) do
              ops = ops.next
            if ops ne null then
              if buf eq null then
                buf = new StringBuilder()
      ...
      /* If we didn't see an opcode or encoded Unicode glyph, and the
        buffer is non-empty, write the current character and advance
         one */
      if (ops eq null) && !unicode then
        if buf ne null then buf.append(c)
        i += 1
    end while
