...
object StackTraceOps:
  ...
  extension (t: Throwable)
    ...
    /** Format a stack trace, taking the prefix span satisfying a predicate.
      *
      * The format is similar to the typical case described in the Javadoc for
      * [[java.lang.Throwable#printStackTrace()*]]. If a stack trace is
      * truncated, it will be followed by a line of the form `... 3 elided`, by
      * analogy to the lines `... 3 more` which indicate shared stack trace
      * segments.
      * @param p
    ...
    def formatStackTracePrefix(p: StackTraceElement => Boolean): String =
      ...
      def header(e: Throwable): String =
        def because = e.getCause match
          case null => null; case c => header(c)
        def msg = e.getMessage match
          case null => because; case s => s
        def txt = msg match
          case null => ""; case s => s": $s"
        s"${e.getClass.getName}$txt"

      val seen = mutable.Set.empty[Throwable]
      def unseen(e: Throwable): Boolean =
        (e != null && !seen(e)).tap(if _ then seen += e)
      ...
      def print(
          ...
          indents: Int
      ): Unit = if unseen(e) then
        val trace = e.getStackTrace
        val frames =
          if share.isEmpty then trace
          else
        ...
        if traceFramesLenDiff > 0 then
          if framesPrefixLenDiff > 0 then
            lines += s"$margin  ... $framesPrefixLenDiff elided and $traceFramesLenDiff more"
          else lines += s"$margin  ... $traceFramesLenDiff more"
        else if framesPrefixLenDiff > 0 then
