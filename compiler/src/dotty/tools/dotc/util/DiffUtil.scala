...
object DiffUtil:
  ...
  @tailrec private def splitTokens(
      str: String,
      acc: List[String] = Nil
  ): List[String] =
      ...
      val (token, rest) =
        if head == ansiColorToken then // ansi color token
          ...
          str.span { c =>
            ...
            !Character.isMirrored(c) && !Character.isWhitespace(
              c
            ) && c != ansiColorToken
          }
  ...
  /** Return a colored diff between the tokens of every line in `expected` and
    * `actual`. Each line of output contains the expected value on the left and
    * the actual value on the right.
    *
    * @param expected
    *   The expected lines
    * @param actual
    *   The actual lines
    * @return
    *   A string with one element of `expected` and `actual` on each lines,
    *   where differences are highlighted.
    */
  ...
  end mkColoredLineDiff

  def mkColoredCodeDiff(
      code: String,
      lastCode: String,
      printDiffDel: Boolean
  ): String =
  ...
  private def nwScore(x: Array[String], y: Array[String]): Array[Int] =
    ...
    for j <- 1 to y.length do score(0)(j) = score(0)(j - 1) + ins(y(j - 1))
  ...
  private def needlemanWunsch(
    ...
    var j = y.length
    while i > 0 || j > 0 do
      if i > 0 && j > 0 && score(i)(j) == score(i - 1)(j - 1) + similarity(
          x(i - 1),
          y(j - 1)
        )
      then
