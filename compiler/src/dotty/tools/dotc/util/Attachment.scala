...
/** A class inheriting from Attachment.Container supports adding, removing and
  * lookup of attachments. Attachments are typed key/value pairs.
  *
  * Attachments whose key is an instance of `StickyKey` will be kept when the
  * attachments are copied using `withAttachmentsFrom`.
  */
object Attachment:
  import Property.{Key, StickyKey}

  /** An implementation trait for attachments. Clients should inherit from
    * Container instead.
    */
  trait LinkSource:
    ...
    final def getAttachment[V](key: Key[V]): Option[V] =
      val nx = next
      if nx == null then None
      else if nx.key eq key then Some(nx.value.asInstanceOf[V])
      else nx.getAttachment[V](key)

    /** Does an attachment corresponding to `key` exist? */
    final def hasAttachment[V](key: Key[V]): Boolean =
      val nx = next
      if nx == null then false
      else if nx.key eq key then true
    ...
    final def attachment[V](key: Key[V]): V =
      val nx = next
      if nx == null then throw new NoSuchElementException
      else if nx.key eq key then nx.value.asInstanceOf[V]
      else nx.attachment(key)

    /** The attachment corresponding to `key`, or `default` if no attachment
      * with `key` exists.
      */
    final def attachmentOrElse[V](key: Key[V], default: V): V =
      val nx = next
      if nx == null then default
      else if nx.key eq key then nx.value.asInstanceOf[V]
    ...
    /** Add attachment with given `key` and `value`.
      ...
      *   The new attachment is added at the position of the old one, or at the
      *   end if no attachment with same `key` existed.
      */
    ...
    /** Remove attachment with given `key`, if it exists.
      * @return
      *   Optionally, the removed attachment with given `key` if one existed
      *   before.
      */
    ...
    final def allAttachments: List[(Key[?], Any)] =
      val nx = next
      if nx == null then Nil else (nx.key, nx.value) :: nx.allAttachments
  end LinkSource

  /** A private, concrete implementation class linking attachments.
    */
  private[Attachment] class Link[+V](
      val key: Key[V],
      val value: V,
      var next: Link[?] | Null
  ) extends LinkSource
  ...
  trait Container extends LinkSource:
    ...
    final def withAttachmentsFrom(container: Container): this.type =
      ...
      while current != null do
        if current.key.isInstanceOf[StickyKey[?]] then
          putAttachment(current.key, current.value)
    ...
    final def pushAttachment[V](key: Key[V], value: V)(using
        ctx: Context
    ): Unit =
      assert(
        !hasAttachment(key) || ctx.base.errorsToBeReported,
        s"duplicate attachment for key $key"
      )
