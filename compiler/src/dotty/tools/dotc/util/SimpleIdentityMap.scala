...
/** A simple linked map with `eq` as the key comparison, optimized for small
  * maps. It has linear complexity for `apply`, `updated`, and `remove`.
  */
abstract class SimpleIdentityMap[K <: AnyRef, +V <: AnyRef]
    extends (K => V | Null):
  ...
  def mapValuesNow[V1 >: V <: AnyRef](
      f: (K, V1) => V1
  ): SimpleIdentityMap[K, V1]
...
object SimpleIdentityMap:
  ...
  def empty[K <: AnyRef]: SimpleIdentityMap[K, Nothing] =
    myEmpty.asInstanceOf[SimpleIdentityMap[K, Nothing]]

  class Map1[K <: AnyRef, +V <: AnyRef](k1: K, v1: V)
      extends SimpleIdentityMap[K, V]:
    def size: Int = 1
    def apply(k: K): V | Null =
      if k eq k1 then v1
      else null
  ...
  class Map2[K <: AnyRef, +V <: AnyRef](k1: K, v1: V, k2: K, v2: V)
      extends SimpleIdentityMap[K, V]:
    def size: Int = 2
    def apply(k: K): V | Null =
      if k eq k1 then v1
  ...
  class MapMore[K <: AnyRef, +V <: AnyRef](bindings: Array[AnyRef])
      extends SimpleIdentityMap[K, V]:
    ...
    def remove(k: K): SimpleIdentityMap[K, V] =
      ...
      while i < bindings.length do
        if bindings(i) eq k then
          return if size == CompactifyThreshold then
    ...
    def mapValuesNow[V1 >: V <: AnyRef](
      ...
      while i < bindings.length do
        val v = value(i)
        val v1 = f(key(i), v)
        if (v1 ne v) && (bindings1 eq bindings) then bindings1 = bindings.clone
    ...
    def forallBinding(f: (K, V) => Boolean): Boolean =
      var i = 0
      while i < bindings.length do
        if !f(key(i), value(i)) then return false
        i += 2
      return true
  end MapMore
end SimpleIdentityMap
