...
object ShowPickled:
  ...
  case class PickleBufferEntry(
    ...
    def hasName: Boolean = tag match
      ...
      case _ => false
    def readName: String =
      if isName then new String(bytes, StandardCharsets.UTF_8)
      else sys.error("%s is no name" format tagName)
    def nameIndex: Int =
      if hasName then readNat(bytes, 0)
  ...
  case class PickleBufferEntryList(entries: IndexedSeq[PickleBufferEntry]):
    def nameAt(idx: Int): String =
      ...
      if entry.isName then entry.readName
      else if entry.hasName then entries(entry.nameIndex).readName
      else "?"
  ...
  def printFile(buf: PickleBuffer, out: PrintStream = System.out.nn): Unit =
    ...
    def printSymInfo(end: Int, isType: Boolean): Unit =
      ...
      def printFlags(privateWithin: Option[Int]) =
        ...
        val flagString =
          PickleBuffer.unpickleScalaFlags(pflags, isType).toString
      ...
      tag match
        ...
        case TYPEsym | ALIASsym | CLASSsym | MODULEsym | VALsym =>
          printSymInfo(
            ...
            tag == TYPEsym || tag == ALIASsym || tag == CLASSsym
          )
          if tag == CLASSsym && (buf.readIndex < end) then printTypeRef()
        case EXTref | EXTMODCLASSref =>
          printNameRef()
          if buf.readIndex < end then printSymbolRef()
        case THIStpe =>
          printSymbolRef()
        case SINGLEtpe =>
        ...
        case LITERALboolean =>
          out.print(if buf.readLong(len) == 0L then " false" else " true")
        case LITERALbyte =>
    ...
    for i <- 0 until index.length do printEntry(i)
  end printFile
end ShowPickled
