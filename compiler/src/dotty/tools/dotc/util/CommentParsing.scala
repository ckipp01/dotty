...
object CommentParsing:
  ...
  /** Returns index of string `str` following `start` skipping longest sequence
    * of whitespace characters characters (but no newlines)
    */
  def skipWhitespace(str: String, start: Int): Int =
    if start < str.length && isWhitespace(str charAt start) then
      skipWhitespace(str, start + 1)
    else start

  /** Returns index of string `str` following `start` skipping sequence of
    * identifier characters.
    */
  def skipIdent(str: String, start: Int): Int =
    if start < str.length && isIdentifierPart(str charAt start) then
  ...
  def skipTag(str: String, start: Int): Int =
    if start < str.length && (str charAt start) == '@' then
      skipIdent(str, start + 1)
    else start

  /** Returns index of string `str` after `start` skipping longest sequence of
    * space and tab characters, possibly also containing a single `*` character
    * or the `/``**` sequence.
    * @pre
  ...
  /** Returns first index following `start` and starting a line (i.e. after
    * skipLineLead) or starting the comment which satisfies predicate `p`.
    */
  ...
  /** Return first index following `start` and starting a line (i.e. after
    * skipLineLead) which satisfies predicate `p`.
    */
  ...
  /** Produces a string index, which is a list of `sections`, i.e pairs of
    * start/end positions of all tagged sections in the string. Every section
    * starts with an at sign and extends to the next at sign, or to the end of
    * the comment string, but excluding the final two characters which terminate
    * the comment.
    *
    * Also take usecases into account - they need to expand until the next
    * usecase or the end of the string, as they might include other sections of
    * their own
    */
  def tagIndex(
      str: String,
  ...
  /** Merge sections following an usecase into the usecase comment, so they can
    * override the parent symbol's sections
    */
  ...
  /** The first start tag of a list of tag intervals, or the end of the whole
    * comment string - 2 if list is empty
    */
  ...
  /** A map from parameter names to start/end indices describing all parameter
    * sections in `str` tagged with `tag`, where `sections` is the index of
    * `str`.
    */
  def paramDocs(
      str: String,
  ...
  /** Returns index following variable, or start index if no variable was
    * recognized
    */
  ...
  def cleanupSectionText(str: String): String =
    ...
    while result.endsWith("\n") do result = result.substring(0, str.length - 1)
    result
