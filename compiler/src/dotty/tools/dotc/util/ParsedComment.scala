...
class ParsedComment(val comment: Comment):
  ...
  /** The bounds of a section that represents the [start; end[ char offset of
    * the section within this comment's `content`.
    */
  ...
  private lazy val paramDocs: Map[String, Bounds] =
    CommentParsing.paramDocs(content, "@param", tagIndex)

  /** The "main" documentation for this comment. That is, the comment before any
    * section starts.
    */
  ...
  /** Renders this comment as markdown.
    *
    * The different sections are formatted according to the mapping in
    * `knownTags`.
    */
  def renderAsMarkdown(using Context): String =
    ...
    val sections = for
      ...
      texts = boundss.map { case (start, end) =>
        clean(content.slice(start, end))
      }
  ...
  def paramDoc(name: TermName): Option[String] =
    paramDocs.get(name.toString).map { case (start, end) =>
...
object ParsedComment:
  ...
  def docOf(symbol: Symbol)(using Context): Option[ParsedComment] =
    val documentedSymbol =
      if symbol.isPrimaryConstructor then symbol.owner else symbol
    for
      docCtx <- ctx.docCtx
  ...
  /** Formats a list of items into a list describing them.
    *
    * Each element is assumed to consist of a first word, which is the item
    * being described. The rest is the description of the item.
    *
    * @param items
    *   The items to format into a list.
    * @return
    *   A markdown list of descriptions.
    */
  ...
  /** Formats a list of items into a markdown list.
    *
    * @param items
    *   The items to put in a list.
    * @return
    *   The list of items, in markdown.
    */
  private def toMarkdownList(ctx: Context, items: List[String]): String =
    val formattedItems =
      items.map(_.linesIterator.mkString(System.lineSeparator + "   "))
    formattedItems.mkString(" - ", System.lineSeparator + " - ", "")

  /** If the color is enabled, add syntax highlighting to each of `snippets`,
    * otherwise wrap each of them in a markdown code fence. The results are put
    * into a markdown list.
    *
    * @param language
    *   The language to use for the code fences
    * @param snippets
    *   The list of snippets to format.
    * @return
    *   A markdown list of code fences.
    * @see
    *   toCodeFence
    */
  private def toCodeFences(
      language: String
  )(ctx: Context, snippets: List[String]): String =
    toMarkdownList(ctx, snippets.map(toCodeFence(language)(ctx, _)))

  /** Formats `snippet` for display. If the color is enabled, the syntax is
    * highlighted, otherwise the snippet is wrapped in a markdown code fence.
    *
    * @param language
    *   The language to use.
    * @param snippet
    *   The code snippet
    * @return
    *   `snippet`, wrapped in a code fence.
    */
  private def toCodeFence(
      language: String
  )(ctx: Context, snippet: String): String = inContext(ctx) {
    if colorEnabled then SyntaxHighlighting.highlight(snippet)
    else s"""```$language
         |$snippet
         |```""".stripMargin
  }

  /** Format the elements of documentation associated with a given tag using
    * `fn`, and starts the section with `title`.
    *
    * @param title
    *   The title to give to the formatted items.
    * @param fn
    *   The formatting function to use.
    */
  private case class TagFormatter(
      title: String,
      fn: (Context, List[String]) => String
  ):
    ...
    /** Format `item` using `fn` if `items` is not empty.
      ...
      *   The items to format
      * @return
      *   If items is not empty, the items formatted using `fn`.
      */
  ...
  private def bold(str: String)(using Context): String =
    if colorEnabled then s"$BOLD$str$RESET"
    else s"**$str**"
end ParsedComment
