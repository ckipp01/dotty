...
/** A HashSet where the elements are stored weakly. Elements in this set are
  * eligible for GC if no other hard references are associated with them. Its
  * primary use case is as a canonical reference identity holder (aka
  * "hash-consing") via findEntryOrUpdate
  *
  * This Set implementation cannot hold null. Any attempt to put a null in it
  * will result in a NullPointerException
  *
  * This set implementation is not in general thread safe without external
  * concurrency control. However it behaves properly when GC concurrently
  * collects elements in this set.
  */
abstract class WeakHashSet[A <: AnyRef](
    initialCapacity: Int = 8,
    loadFactor: Double = 0.5
  ...
  /** queue of Entries that hold elements scheduled for GC the
    * removeStaleEntries() method works through the queue to remove stale
  ...
  /** from a specified initial capacity compute the capacity we'll use as being
    * the next power of two equal to or greater than the specified initial
    * capacity
    */
  ...
  protected def resize(): Unit =
    ...
    @tailrec
    def tableLoop(oldBucket: Int): Unit = if oldBucket < oldTable.size then
      @tailrec
  ...
  protected def addEntryAt(
      bucket: Int,
      elem: A,
      elemHash: Int,
      oldHead: Entry[A] | Null
  ): A =
  ...
  def put(elem: A): A = (elem: A | Null) match
    ...
    case _ =>
      ...
      def linkedListLoop(entry: Entry[A] | Null): A = entry match
        ...
        case _ =>
          val entryElem = entry.get
          if entryElem != null && isEqual(elem, entryElem) then
            entryElem.uncheckedNN
          else linkedListLoop(entry.tail)
  ...
  def clear(resetToInitial: Boolean): Unit =
    ...
    @tailrec def queueLoop(): Unit = if queue.poll() != null then queueLoop()
    queueLoop()
  ...
  override def iterator: Iterator[A] =
    ...
    new collection.AbstractIterator[A]:
      ...
      /** the bucket currently being examined. Initially it's set past the last
        * bucket and will be decremented
        */
  ...
  /** Diagnostic information about the internals of this set. Not normally
    * needed by ordinary code, but may be useful for diagnosing performance
    * problems
    */
  private[util] class Diagnostics:
    /** Verify that the internal structure of this hash set is fully consistent.
      ...
      * the entries must be stable. If any are garbage collected during
      * validation then an assertion may inappropriately fire.
      */
    def fullyValidate(): Unit =
      ...
      while bucket < table.size do
        ...
        while entry != null do
          assert(
            entry.get != null,
            s"$entry had a null value indicated that gc activity was happening during diagnostic validation or that a null value was inserted"
          )
          computedCount += 1
          ...
          assert(
            cachedHash == realHash,
            s"for $entry cached hash was $cachedHash but should have been $realHash"
          )
          val computedBucket = index(realHash)
          assert(
            computedBucket == bucket,
            s"for $entry the computed bucket was $computedBucket but should have been $bucket"
          )
      ...
      assert(
        computedCount == count,
        s"The computed count was $computedCount but should have been $count"
      )
    end fullyValidate
    ...
    /** Produces a diagnostic dump of the table that underlies this hash set.
      */
    def dump: String =
      java.util.Arrays.toString(table.asInstanceOf[Array[AnyRef | Null]])

    /** Number of buckets that hold collisions. Useful for diagnosing
      * performance issues.
      */
...
/** Companion object for WeakHashSet
  */
object WeakHashSet:
  /** A single entry in a WeakHashSet. It's a WeakReference plus a cached hash
    * code and a link to the next Entry in the same bucket
    */
  class Entry[A](
