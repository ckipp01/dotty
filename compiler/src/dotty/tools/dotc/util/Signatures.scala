...
object Signatures:
  ...
  /** Extract (current parameter index, function index, functions) method call
    * for given position.
    *
    * @param path
    ...
    *
    * @return
    *   A triple containing the index of the parameter being edited, the index
    *   of functeon being called, the list of overloads of this function).
    */
  def signatureHelp(path: List[tpd.Tree], pos: Span)(using
      Context
  ): (Int, Int, List[Signature]) =
    computeSignatureHelp(path, pos)

  /** Extract (current parameter index, function index, functions) out of a
    * method call.
    *
    * @param path
    *   The path to the function application
    ...
    * @return
    *   A triple containing the index of the parameter being edited, the index
    *   of the function being called, the list of overloads of this function).
    */
  @deprecated(
  ...
  def callInfo(path: List[tpd.Tree], span: Span)(using
      Context
  ...
  /** Computes call info (current parameter index, function index, functions)
    * for a method call.
    *
    * @param path
    *   The path to the function application
    ...
    *   of the function being called, the list of overloads of this function).
    */
  def computeSignatureHelp(path: List[tpd.Tree], span: Span)(using
      Context
  ): (Int, Int, List[Signature]) =
    findEnclosingApply(path, span) match
      ...
      case appliedTypeTree @ AppliedTypeTree(_, types) =>
        appliedTypeTreeCallInfo(appliedTypeTree, types)
  ...
  /** Finds enclosing application from given `path` for `span`.
    ...
    *
    * @return
    *   Tree which encloses closest application containing span. In case if
    *   cursor is pointing on closing parenthesis and next subsequent
    *   application exists, it returns the latter
    */
  private def findEnclosingApply(path: List[tpd.Tree], span: Span)(using
      Context
  ): tpd.Tree =
    path.filterNot {
      case apply @ Apply(fun, _) => fun.span.contains(span) || isValid(apply)
      case unapply @ UnApply(fun, _, _) =>
        fun.span.contains(span) || isValid(unapply)
      case typeTree @ AppliedTypeTree(fun, _) =>
        fun.span.contains(span) || isValid(typeTree)
      case typeApply @ TypeApply(fun, _) =>
        fun.span.contains(span) || isValid(typeApply)
      case _ => true
    } match
      case Nil => tpd.EmptyTree
      case direct :: enclosing :: _
          if isClosingSymbol(direct.source(span.end - 1)) =>
        enclosing
  ...
  /** Extracts call information for applied type tree:
    *
    * @param types
    *   Currently applied function type parameters
    * @param fun
    *   Function tree which is being applied
    */
  private def appliedTypeTreeCallInfo(
    ...
    val signature =
      Signature(typeName, typeParams, Nil, Some(typeName), None, Some(denot))
  ...
  private def applyCallInfo(
    ...
    val (alternativeIndex, alternatives) = fun.tpe match
      case err: ErrorType =>
        val (alternativeIndex, alternatives) =
          alternativesFromError(err, params) match
            ...
            case (_, Nil) =>
              val denot = fun.denot
              if denot.exists then (0, List(denot.asSingleDenotation))
              else (0, Nil)
            case other => other
    ...
    if alternativeIndex < alternatives.length then
      ...
      val typeParamsShift =
        if !isTypeApply then
          fun.symbol.denot.paramSymss.flatten.filter(_.isType).length
        else 0
      val paramIndex = params.indexWhere(_.span.contains(span)) match
        case -1 =>
          (params.length - 1 max 0) + curriedArguments + typeParamsShift
        case n => n + curriedArguments + typeParamsShift

      val pre = treeQualifier(fun)
      val alternativesWithTypes =
        alternatives.map(_.asSeenFrom(pre.tpe.widenTermRefExpr))
      val alternativeSignatures =
        alternativesWithTypes.flatMap(toApplySignature)
  ...
  private def unapplyCallInfo(
    ...
    val paramTypes =
      extractParamTypess(resultType, denot, patterns.size).flatten
        .map(stripAllAnnots)
    val paramNames = extractParamNamess(resultType, denot).flatten

    val activeParameter =
      unapplyParameterIndex(patterns, span, paramTypes.length)
    val unapplySignature = toUnapplySignature(
  ...
  /** Extract parameter names from `resultType` only if `resultType` is case
    * class and `denot` is synthetic.
    *
    * @param resultType
    ...
    *
    * @return
    *   List of lists of names of parameters if `resultType` is case class
    *   without overriden unapply
    */
  private def extractParamNamess(resultType: Type, denot: Denotation)(using
      Context
  ): List[List[Name]] =
    if resultType.typeSymbol.flags
        .is(Flags.CaseClass) && denot.symbol.flags.is(Flags.Synthetic)
    then resultType.typeSymbol.primaryConstructor.paramInfo.paramNamess
    else Nil
  ...
  /** Extract parameter types from `resultType` in unapply context.
    ...
    *   Function denotation
    * @param patternsSize
    *   Number of pattern trees present in function tree
    *
    * @return
    *   List of lists of types present in unapply clause
    */
  private def extractParamTypess(
    ...
    resultType match
      ...
      case ref: TypeRef if !ref.symbol.isPrimitiveValueClass =>
        mapOptionLessUnapply(ref, patternsSize, isUnapplySeq(denot))
      // unapply(_$1: Any): Option[T[_]]
      case AppliedType(
            TypeRef(_, cls),
            (appliedType @ AppliedType(tycon, args)) :: Nil
          )
          if (cls == ctx.definitions.OptionClass || cls == ctx.definitions.SomeClass) =>
        tycon match
          // unapply[T](_$1: Any): Option[(T1, T2 ... Tn)]
          case typeRef: TypeRef
              if ctx.definitions.isTupleClass(typeRef.symbol) =>
            List(args)
          case _ => List(List(appliedType))
      // unapply[T](_$1: Any): CustomClass[T]
      case appliedType: AppliedType =>
        mapOptionLessUnapply(appliedType, patternsSize, isUnapplySeq(denot))
      case _ => Nil

  /** Recursively strips annotations from given type
    *
    * @param tpe
    *   Type to strip annotations from
    * @return
    *   Type with stripped annotations
    */
  private def stripAllAnnots(tpe: Type)(using Context): Type = tpe match
    case AppliedType(t, args) =>
      AppliedType(stripAllAnnots(t), args.map(stripAllAnnots))
    case other => other.stripAnnots

  /** Get index of currently edited parameter in unapply context.
    *
    * @param patterns
    *   Currently applied patterns for unapply method
    * @param span
    *   The position of the cursor
    * @param maximumParams
    *   Number of parameters taken by unapply method
    * @return
    *   Index of currently edited parameter
    */
  private def unapplyParameterIndex(
      patterns: List[tpd.Tree],
      span: Span,
      maximumParams: Int
  )(using Context): Int =
    ...
    (patternPosition, patterns.length) match
      ...
      case (-1, pos) =>
        -1 // there are patterns, we must be outside range so we set no active parameter
      case _ =>
        (maximumParams - 1) min patternPosition max 0 // handle unapplySeq to always highlight Seq[A] on elements

  /** Checks if tree is valid for signatureHelp. Skipped trees are either tuple
  ...
  /** Get unapply method result type omiting unknown types and another method
    * calls.
    *
    * @param fun
    *   Unapply tree
    *
    * @return
    *   Proper unapply method type after extracting result from method types and
    *   omiting unknown types.
    */
  ...
  /** Maps type by checking if given match is single match, name-based match,
    * sequence match or product sequence match. The precedence in higher
    * priority - higher index order for fixed-arity extractors is:
    *   1. Single match 2. Name based match For variadic extractors:
    *   1. Sequence match 2. Product sequence match
    *
    * @see
    *   [[https://docs.scala-lang.org/scala3/reference/changed-features/pattern-matching.html]]
    *
    * @param resultType
    *   Final result type for unapply
    * @param patternCount
    *   Currently applied patterns to unapply function
    * @param isUnapplySeq
    *   true if unapply name equals "unapplySeq", false otherwise
    *
    * @return
    *   List of List of types dependent on option less extractor type.
    */
  private def mapOptionLessUnapply(
    ...
    val availableExtractors =
      if isUnapplySeq && dropMethod.exists then List(dropMethod)
      else if getMethod.exists && patternCount <= 1 then List(getMethod)
      else productAccessors

    List(availableExtractors.map(_.info.finalResultType).toList)
  end mapOptionLessUnapply

  /** Filter returning only members starting with underscore followed with
    * number
    */
  ...
  /** Creates signature for apply method.
    *
    * @param denot
    *   Function denotation for which apply signature is returned.
    *
    * @return
    *   Signature if denot is a function, None otherwise
    */
  private def toApplySignature(denot: SingleDenotation)(using
      Context
  ): Option[Signature] =
    ...
    def toParamss(tp: Type)(using Context): List[List[Param]] =
      val rest = tp.resultType match
        case res: MethodType =>
          if isDummyImplicit(res) then Nil else toParamss(res)
        case _ => Nil
    ...
    def isSyntheticEvidence(name: String) =
      if !name.startsWith(NameKinds.EvidenceParamName.separator) then false
      else
        symbol.paramSymss.flatten
          .find(_.name.show == name)
          .exists(_.flags.is(Flags.Implicit))

    denot.info.stripPoly match
      case tpe: (MethodType | AppliedType | TypeRef | TypeParamRef) =>
        val paramss = toParamss(tpe).map(
          _.filterNot(param => isSyntheticEvidence(param.name))
        )
        val evidenceParams =
          (tpe.paramNamess.flatten zip tpe.paramInfoss.flatten).flatMap {
            case (name, AppliedType(tpe, (ref: TypeParamRef) :: _))
                if isSyntheticEvidence(name.show) =>
        ...
        val typeParams = denot.info match
          case poly: PolyType =>
            ...
            tparams.map { (name, info) =>
              evidenceParams
                .find((evidenceName: TypeName, _: Type) => name == evidenceName)
                .flatMap { case (_, tparam) =>
                  tparam.show.split('.').lastOption
                } match
                case Some(evidenceTypeName) =>
                  s"${name.show}: ${evidenceTypeName}"
                case None => name.show + info.show
            }
          case _ => Nil
        val (name, returnType) =
          if symbol.isConstructor then (symbol.owner.name.show, None)
          else
  ...
  end toApplySignature

  @deprecated(
    "Deprecated in favour of `signatureHelp` which now returns Signature along SingleDenotation",
    "3.1.3"
  )
  def toSignature(denot: SingleDenotation)(using Context): Option[Signature] =
    if denot.name.isUnapplyName then
      ...
      val paramTypes =
        extractParamTypess(resultType, denot, 0).flatten.map(stripAllAnnots)
      val paramNames = extractParamNamess(resultType, denot).flatten

      toUnapplySignature(denot.asSingleDenotation, paramNames, paramTypes)
    else toApplySignature(denot)

  /** Creates signature for unapply method. It is different from apply one as it
    * should not show function name, return type and type parameters. Instead we
    * show function in the following pattern (_$1: T1, _$2: T2, _$n: Tn), where
    * _$n is only present for synthetic product extractors such as case classes.
    * In rest of cases signature skips them resulting in pattern (T1, T2, T3,
    * Tn)
    *
    * @param denot
    *   Unapply denotation
    * @param paramNames
    *   Parameter names for unapply final result type. It non empty only when
    *   unapply returns synthetic product as for case classes.
    * @param paramTypes
    *   Parameter types for unapply final result type.
    *
    * @return
    *   Signature if paramTypes is non empty, None otherwise
    */
  private def toUnapplySignature(
    ...
    val params =
      if paramNames.length == paramTypes.length then
        (paramNames zip paramTypes).map((name, info) =>
          Param(name.show, info.show)
        )
      else paramTypes.map(info => Param("", info.show))
  ...
  /** The number of parameters before `tree` application. It is necessary to
    ...
    * one.
    *
    * This handles currying, so for an application such as `foo(1, 2)(3)`, the
    * result of `countParams` should be 3. It also takes into considerations
    * unapplied arguments so for `foo(1)(3)` we will still get 3, as first
    * application `foo(1)` takes 2 parameters with currently only 1 applied.
    *
    * @param tree
    *   The tree to inspect.
    * @param denot
    *   Denotation of function we are trying to apply
    * @param alreadyCurried
    *   Number of subsequent Apply trees before current tree
    *
    * @return
    *   The number of parameters that are passed.
    */
  private def countParams(
      tree: tpd.Tree,
      denot: SingleDenotation,
      alreadyCurried: Int = 0
  )(using Context): Int =
    tree match
      case Apply(fun, params) =>
        countParams(fun, denot, alreadyCurried + 1) + denot.symbol
          .paramSymss(alreadyCurried)
          .length
      case _ => 0

  /** Inspect `err` to determine, if it is an error related to application of an
    * overloaded function, what were the possible alternatives.
    *
    * If several alternatives are found, determines what is the best suited
    * alternatives given the parameters `params`: The alternative that has the
    * most formal parameters matching the given arguments is chosen.
    *
    * @param err
    *   The error message to inspect.
    * @param params
    *   The parameters that were given at the call site.
    *
    * @return
    *   A pair composed of the index of the best alternative (0 if no
    *   alternatives were found), and the list of alternatives.
    */
