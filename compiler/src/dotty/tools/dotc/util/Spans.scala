...
/** The offsets part of a full position, consisting of 2 or 3 entries:
  ...
  *
  * Spans are encoded according to the following format in little endian: Start:
  * unsigned 26 Bits (works for source files up to 64M) End: unsigned 26 Bits
  * Point: unsigned 12 Bits relative to start NoSpan encoded as -1L (this is a
  * normally invalid span because point would lie beyond end).
  */
object Spans:
  ...
  /** Convert offset `x` to an integer by sign extending the original field of
    * `StartEndBits` width.
    */
  def offsetToInt(x: Int): Int =
    x << (32 - StartEndBits) >> (32 - StartEndBits)

  /** A span indicates a range between a start offset and an end offset. Spans
    * can be synthetic or source-derived. A source-derived span has in addition
    * a point. The point lies somewhere between start and end. The point is
    * roughly where the `^` would go if an error was diagnosed at that position.
  ...
  class Span(val coords: Long) extends AnyVal:
    ...
    /** The union of two spans. This is the least range that encloses both
      * spans. It is always a synthetic span.
      */
    ...
    /** Does the range of this span overlap with the range of that span at more
      * than a single point?
      */
    def overlaps(that: Span): Boolean =
      def containsInner(span: Span, offset: Int) =
    ...
    /** A span where all components are shifted by a given `offset` relative to
      * this span.
      */
    ...
    def withStart(start: Int): Span =
      if exists then
        fromOffsets(
          start,
          this.end,
          if isSynthetic then SyntheticPointDelta else this.point - start
        )
      else this
    ...
    override def toString: String =
      ...
      if exists then
        s"$left$start..${if point == start then "" else s"$point.."}$end$right"
  ...
  def Span(start: Int, end: Int, point: Int): Span =
    ...
    fromOffsets(
      ...
      if pointDelta >= SyntheticPointDelta then 0 else pointDelta
    )
  ...
  class Coord(val encoding: Int) extends AnyVal:
    ...
    override def toString =
      if isSpan then s"$toSpan" else s"Coord(idx=$toIndex)"
