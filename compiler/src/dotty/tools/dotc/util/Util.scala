...
object Util:
  ...
  /** The index `i` in `candidates.indices` such that `candidates(i) <= x` and
    * `candidates(i)` is closest to `x`, determined by binary search, or -1 if
    * `x < candidates(0)`.
    * @param hint
  ...
  def bestFit(
    ...
    def recur(lo: Int, hi: Int, mid: Int): Int =
      if x < candidates(mid) then recur(lo, mid - 1, (lo + mid - 1) / 2)
      else if mid + 1 < length && x >= candidates(mid + 1) then
        recur(mid + 1, hi, (mid + 1 + hi) / 2)
      else mid
    val initMid = if 0 <= hint && hint < length then hint else length / 2
    if length == 0 || x < candidates(0) then -1
