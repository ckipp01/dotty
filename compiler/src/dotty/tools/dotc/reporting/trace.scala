...
/** Exposes the {{{trace("question") { op }}}} syntax.
  *
  * Traced operations will print indented messages if enabled. Tracing depends
  * on [[Config.tracingEnabled]] and
  * [[dotty.tools.dotc.config.ScalaSettings.Ylog]]. Tracing can be forced by
  * replacing [[trace]] with [[trace.force]] or [[trace.log]] (see below).
  */
...
/** This module is carefully optimized to give zero overhead if
  * Config.tracingEnabled is false. The `trace` operation is called in various
  * hotspots, so every tiny bit of overhead is unacceptable: boxing, closures,
  * additional method calls are all out.
  */
trait TraceSyntax:
  ...
  inline def onDebug[TD](inline question: String)(inline op: TD)(using
      Context
  ): TD =
    conditionally(ctx.settings.YdebugTrace.value, question, false)(op)

  inline def conditionally[TC](
      inline cond: Boolean,
      inline question: String,
      inline show: Boolean
  )(inline op: TC)(using Context): TC =
    inline if isEnabled then
      apply(
        question,
        if cond then Printers.default else Printers.noPrinter,
        show
      )(op)
    else op

  inline def apply[T, U >: T](
      inline question: String,
      inline printer: Printers.Printer,
      inline showOp: U => String
  )(inline op: T)(using Context): T =
    inline if isEnabled then doTrace[T](question, printer, showOp)(op)
    else op
  ...
  inline def apply[T](inline question: String, inline show: Boolean)(
      inline op: T
  )(using Context): T =
  ...
  private def doTrace[T](
    ...
    if ctx.mode.is(
        Mode.Printing
      ) || !isForced && (printer eq Printers.noPrinter)
    then op
    else
      ...
      def doLog(s: String) =
        if isForced then println(s) else report.log(s)(using logctx)
