...
/** This trait implements `isHidden` so that multiple messages per position are
  * suppressed, unless they are of increasing severity.
  */
trait UniqueMessagePositions extends Reporter:
  ...
  /** Logs a position and returns true if it was already logged.
    * @note
    *   Two positions are considered identical for logging if they have the same
    *   point.
    */
  ...
  override def markReported(dia: Diagnostic)(using Context): Unit =
    if dia.pos.exists then
      for pos <- dia.pos.start to dia.pos.end do
