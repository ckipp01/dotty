...
/** This interface provides methods to issue information, warning and error
  * messages.
  */
abstract class Reporter extends interfaces.ReporterResult:
  ...
  private def isIncompleteChecking =
    incompleteHandler ne defaultIncompleteHandler
  ...
  /** The number of warnings reported by this reporter (ignoring outer
    * reporters)
    */
  ...
  /** Have errors been reported by this reporter, or in the case where this is a
    * StoreReporter, by an outer reporter?
    */
  ...
  def reportNewFeatureUseSite(featureTrait: Symbol): Unit =
    reportedFeaturesUseSites += featureTrait
  ...
  /** Issue the diagnostic, ignoring `-Wconf` and `@nowarn` configurations, but
  ...
  def issueUnconfigured(dia: Diagnostic)(using Context): Unit = dia match
    ...
    case _ =>
      ...
      val d = dia match
        ...
        case _                                               => dia
      if !isHidden(d)
      then // avoid isHidden test for summarized warnings so that message is not forced
        markReported(d)
  ...
  def issueIfNotSuppressed(dia: Diagnostic)(using Context): Unit =
    ...
    dia match
      case w: Warning if run != null =>
        ...
        if !isIncompleteChecking then sup.addSuspendedMessage(w)
      case _ => go()
  ...
  def summary: String =
    ...
    if errorCount > 0 then b += countString(errorCount, "error") + " found"
    b.mkString("\n")
  ...
  def summarizeUnreportedWarnings()(using Context): Unit =
    for (settingName, count) <- unreportedWarnings do
      ...
      val msg =
        em"there $were ${countString(count, settingName.tail + " warning")}; re-run with $settingName for details"
      report(Warning(msg, NoSourcePosition))

  /** Print the summary of warnings and errors */
  def printSummary()(using Context): Unit =
    val s = summary
    if s != "" then report(new Info(s, NoSourcePosition))
  ...
  /** Does this reporter contain errors that have yet to be reported by its
    * outer reporter ? Note: this is always false when there is no outer
    * reporter.
    */
  def hasUnreportedErrors: Boolean = false

  /** Does this reporter contain any message that have yet to be reported by its
    * outer reporter ? This includes any warning stored in `unreportedWarnings`
    * which need to be propagated to get an accurate count of unreported
    * warnings in the outer reporter.
    */
  def hasUnreportedMessages(using Context): Boolean =
    pendingMessages.nonEmpty || unreportedWarnings.nonEmpty

  /** If this reporter buffers messages, remove and return all buffered
    * messages.
    */
  def removeBufferedMessages(using Context): List[Diagnostic] = Nil

  /** Issue all messages in this reporter to next outer one, or make sure they
    * are written.
    */
