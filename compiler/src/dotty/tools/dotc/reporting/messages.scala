...
abstract class SyntaxMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId):
  def kind = MessageKind.Syntax

abstract class TypeMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId):
...
abstract class TypeMismatchMsg(found: Type, expected: Type)(
    errorId: ErrorMessageID
)(using Context)
    extends Message(errorId),
      ShowMatchTrace(found, expected):
  def kind = MessageKind.TypeMismatch
  def explain(using Context) = err.whyNoMatchStr(found, expected)
  override def canExplain = true

abstract class NamingMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId),
      NoDisambiguation:
  def kind = MessageKind.Naming

abstract class DeclarationMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId):
  def kind = MessageKind.Declaration

/** A simple not found message (either for idents, or member selection. Messages
  * of this class are sometimes dropped in favor of other, more specific
  * messages.
  */
abstract class NotFoundMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId):
  def kind = MessageKind.NotFound
  def name: Name

abstract class PatternMatchMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId):
  def kind = MessageKind.PatternMatch

abstract class CyclicMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId):
  def kind = MessageKind.Cyclic

abstract class ReferenceMsg(errorId: ErrorMessageID)(using Context)
    extends Message(errorId):
  def kind = MessageKind.Reference

abstract class EmptyCatchOrFinallyBlock(
    tryBody: untpd.Tree,
    errNo: ErrorMessageID
)(using Context)
  ...
  def explain(using Context) =
    ...
    i"""|A ${hl(
        "try"
      )} expression should be followed by some mechanism to handle any exceptions
        |thrown. Typically a ${hl("catch")} expression follows the ${hl(
        "try"
      )} and pattern matches
        ...
        |$code1
        |
        |It is also possible to follow a ${hl("try")} immediately by a ${hl(
        "finally"
      )} - letting the
        |exception propagate - but still allowing for some clean up in ${hl(
        "finally"
      )}:
        |
        |$code2
        |
        |It is recommended to use the ${hl(
        "NonFatal"
      )} extractor to catch all exceptions as it
        |correctly handles transfer functions like ${hl("return")}."""
  end explain
end EmptyCatchOrFinallyBlock
...
class EmptyCatchAndFinallyBlock(tryBody: untpd.Tree)(using Context)
  ...
  def msg(using Context) =
    i"""|A ${hl("try")} without ${hl("catch")} or ${hl(
        "finally"
...
class DeprecatedWithOperator()(using Context)
  ...
  def msg(using Context) =
    i"""${hl("with")} as a type operator has been deprecated; use ${hl(
        "&"
      )} instead"""
  def explain(using Context) =
    i"""|Dotty introduces intersection types - ${hl(
        "&"
      )} types. These replace the
...
class CaseClassMissingParamList(cdef: untpd.TypeDef)(using Context)
  ...
  def explain(using Context) =
    i"""|${cdef.name} must have at least one parameter list, if you would rather
        |have a singleton representation of ${cdef.name}, use a "${hl(
        "case object"
      )}".
  ...
  def msg(using Context) =
    val ofFun =
      if param.name.is(WildcardParamName)
        || (MethodType.syntheticParamNames(
          tree.args.length + 1
        ) contains param.name)
...
class MissingIdent(tree: untpd.Ident, treeKind: String, val name: Name)(using
    Context
...
class TypeMismatch(
  ...
  def msg(using Context) =
    ...
    val (found2, expected2) =
      if (found1 frozen_<:< expected1) || reported.fbounded then
        (found, expected)
...
class NotAMember(
  ...
  def msg(using Context) =
    ...
    def closest: List[(Int, Symbol)] = candidates.toList
      .map(sym => (distance(sym.name.show, missing), sym))
      .filter((d, sym) =>
        d <= maxDist && d < missing.length && d < sym.name.show.length
      )
      .sortBy((d, sym) =>
        (d, sym.name.show)
      ) // sort by distance first, alphabetically second

    val enumClause =
      if ((name eq nme.values) || (name eq nme.valueOf)) && site.classSymbol.companionClass.isEnumClass
      then
        val kind =
          if name eq nme.values then i"${nme.values} array"
          else i"${nme.valueOf} lookup method"
...
end EarlyDefinitionsNotSupported

class TopLevelImplicitClass(cdef: untpd.TypeDef)(using Context)
    extends SyntaxMsg(TopLevelImplicitClassID):
  def msg(using Context) = i"""An ${hl(
      "implicit class"
    )} may not be top-level"""

  def explain(using Context) =
    val TypeDef(name, impl @ Template(constr0, parents, self, _)) =
      cdef: @unchecked
    val exampleArgs =
      if constr0.termParamss.isEmpty then "..."
      else
        constr0
          .termParamss(0)
          .map(_.withMods(untpd.Modifiers()).show)
          .mkString(", ")
    def defHasBody[T] = impl.body.exists(!_.isEmpty)
    val exampleBody = if defHasBody then "{\n ...\n }" else ""
    i"""|There may not be any method, member or object in scope with the same name as
        ...
        |were allowed.
        |           |
        |To resolve the conflict declare ${cdef.name} inside of an ${hl(
        "object"
...
class ObjectMayNotHaveSelfType(mdef: untpd.ModuleDef)(using Context)
  ...
  def msg(using Context) = i"""${hl("object")}s must not have a self ${hl(
      "type"
    )}"""
...
class UnboundPlaceholderParameter()(implicit ctx: Context)
  ...
  def explain(using Context) =
    i"""|The ${hl("_")} placeholder syntax was used where it could not be bound.
        ...
        |${hl("x => { x }")}
        |
        |Another common occurrence for this error is defining a val with ${hl(
        "_"
      )}:
        |
        |${hl("val a = _")}
        ...
        |${hl("trait A { this: B => ... ")}
        |"""
end UnboundPlaceholderParameter
...
class MissingReturnType()(implicit ctx: Context)
  ...
  def explain(using Context) =
    i"""|An abstract declaration must have a return type. For example:
        ...
        |  ${hl(
        "def area: Double"
      )} // abstract declaration returning a Double"""
...
class MissingReturnTypeWithReturnStatement(method: Symbol)(using Context)
  ...
  def msg(using Context) =
    i"$method has a return statement; it needs a result type"
  def explain(using Context) =
...
class YieldOrDoExpectedInForComprehension()(using Context)
  ...
  def explain(using Context) =
    i"""|When the enumerators in a for comprehension are not placed in parentheses or
        |braces, a ${hl("do")} or ${hl(
        "yield"
      )} statement is required after the enumerators
        ...
        |can also be written without parentheses but a ${hl(
        "do"
      )} keyword has to be
...
class ProperDefinitionNotFound()(using Context)
  ...
  def explain(using Context) =
    ...
    i"""|Usecases are only supported for ${hl(
        "def"
      )}s. They exist because with Scala's
        |advanced type-system, we sometimes end up with seemingly scary signatures.
        |The usage of these methods, however, needs not be - for instance the ${hl(
        "map"
      )}
        |function
        |
        ...
        |$noUsecase
        |
        |to mitigate this and ease the usage of such functions we have the ${hl(
        "@usecase"
      )}
...
class WrongNumberOfTypeArgs(
  ...
  private val msgPrefix =
    if actualCount > expectedCount then "Too many" else "Not enough"
...
class AuxConstructorNeedsNonImplicitParameter()(implicit ctx: Context)
  ...
  def explain(using Context) =
    i"""|Only the primary constructor is allowed an ${hl(
        "implicit"
      )} parameter list;
        ...
        | - Forgotten parenthesis on ${hl("this")} (${hl(
        "def this() = { ... }"
      )})
        | - Auxiliary constructors specify the implicit value
        |"""

class IllegalLiteral()(using Context) extends SyntaxMsg(IllegalLiteralID):
...
class LossyWideningConstantConversion(sourceType: Type, targetType: Type)(using
    Context
...
class PatternMatchExhaustivity(uncoveredFn: => String, hasMore: Boolean)(using
    Context
  ...
  def explain(using Context) =
    i"""|There are several ways to make the match exhaustive:
        ...
        | - If an extractor always return ${hl("Some(...)")}, write ${hl(
        "Some[X]"
      )} for its return type
...
class UncheckedTypePattern(msgFn: => String)(using Context)
  ...
  def explain(using Context) =
    i"""|Type arguments and type refinements are erased during compile time, thus it's
        ...
        |
        |You can either replace the type arguments by ${hl(
        "_"
      )} or use `@unchecked`.
        |"""
...
class MatchCaseOnlyNullWarning()(using Context)
  ...
  def msg(using Context) = i"""Unreachable case except for ${hl(
      "null"
    )} (if this is intentional, consider writing ${hl(
      "case null =>"
    )} instead)."""
  def explain(using Context) = ""
...
class UnboundWildcardType()(using Context)
  ...
  def explain(using Context) =
    i"""|The wildcard type syntax (${hl(
        "_"
      )}) was used where it could not be bound.
        |Replace ${hl(
        "_"
      )} with a non-wildcard type. If the type doesn't matter,
...
class OverridesNothingButNameExists(
    member: Symbol,
    existing: List[Denotations.SingleDenotation]
)(using Context)
...
class OverrideError(
    ...
    extends DeclarationMsg(OverrideErrorID),
      NoDisambiguation:
  def msg(using Context) =
    val isConcreteOverAbstract =
      (other.owner isSubClass member.owner) && other.is(Deferred) && !member.is(
        Deferred
      )
    def addendum =
      if isConcreteOverAbstract then
        i"""|
            ...
            |and is therefore overridden by concrete ${err
            .infoStringWithLocation(member, base)})"""
      else ""
    i"""error overriding ${err.infoStringWithLocation(other, base)};
        |  ${err.infoString(
        member,
        base,
        showLocation = member.owner != base.typeSymbol
      )} $core$addendum"""
...
class ForwardReferenceExtendsOverDefinition(value: Symbol, definition: Symbol)(
    using Context
) extends ReferenceMsg(ForwardReferenceExtendsOverDefinitionID):
  def msg(using Context) =
    i"${definition.name} is a forward reference extending over the definition of ${value.name}"
...
class ExpectedTokenButFound(expected: Token, found: Token)(using Context)
  ...
  def explain(using Context) =
    if Tokens.isIdentifier(expected) && Tokens.isKeyword(found) then
      s"""
        |If you want to use $foundText as identifier, you may put it in backticks: `${Tokens
          .tokenString(found)}`.""".stripMargin
    else ""

class MixedLeftAndRightAssociativeOps(
    op1: Name,
    op2: Name,
  ...
  def msg(using Context) =
    val op1Asso: String =
      if op2LeftAssoc then "which is right-associative"
      else "which is left-associative"
    val op2Asso: String =
      if op2LeftAssoc then "which is left-associative"
  ...
  def explain(using Context) =
    s"""|The operators ${op1} and ${op2} are used as infix operators in the same expression,
        ...
        |${op1} is applied to the operand to its ${
         if op2LeftAssoc then "right" else "left"
       }
        |${op2} is applied to the operand to its ${
         if op2LeftAssoc then "left" else "right"
       }
        ...
        |""".stripMargin
end MixedLeftAndRightAssociativeOps

class CantInstantiateAbstractClassOrTrait(cls: Symbol, isTrait: Boolean)(using
    Context
) extends TypeMsg(CantInstantiateAbstractClassOrTraitID):
  private val traitOrAbstract = if isTrait then "a trait" else "abstract"
  def msg(using Context) =
    i"""${cls.name} is ${traitOrAbstract}; it cannot be instantiated"""
  def explain(using Context) =
...
class UnreducibleApplication(tycon: Type)(using Context)
    extends TypeMsg(UnreducibleApplicationID):
  def msg(using Context) =
    i"unreducible application of higher-kinded type $tycon to wildcard arguments"
  def explain(using Context) =
...
class OverloadedOrRecursiveMethodNeedsResultType(cycleSym: Symbol)(using
    Context
) extends CyclicMsg(OverloadedOrRecursiveMethodNeedsResultTypeID):
  def msg(using Context) =
    i"""Overloaded or recursive $cycleSym needs return type"""
  def explain(using Context) =
...
class SkolemInInferred(tree: tpd.Tree, pt: Type, argument: tpd.Tree)(using
    Context
...
class SuperQualMustBeParent(qual: untpd.Ident, cls: ClassSymbol)(using Context)
  ...
  def explain(using Context) =
    val parents: Seq[String] =
      (cls.info.parents map (_.typeSymbol.name.show)).sorted
    i"""|When a qualifier ${hl("T")} is used in a ${hl(
        "super"
      )} prefix of the form ${hl("C.super[T]")},
...
class VarArgsParamMustComeLast()(using Context)
  ...
  def explain(using Context) =
    i"""|The ${hl(
        "varargs"
      )} field must be the last field in the method signature.
        |Attempting to define a field in a method signature after a ${hl(
        "varargs"
      )} field is an error.
        |"""
...
class ConstrProxyShadows(
    proxy: TermRef,
    shadowed: Type,
    shadowedIsApply: Boolean
)(using Context)
    extends ReferenceMsg(ConstrProxyShadowsID),
      NoDisambiguation:
...
class AmbiguousReference(
  ...
  /** A string which explains how something was bound; Depending on `prec` this
    * is either imported by <tree> or defined in <symbol>
    */
  private def bindingString(
      prec: BindingPrec,
      whereFound: Context,
      qualifier: String = ""
  )(using Context) =
    ...
    if prec.isImportPrec then
      i"""$howVisible$qualifier by ${whereFound.importInfo}"""
    else i"""$howVisible$qualifier in ${whereFound.owner}"""
...
class MethodDoesNotTakeParameters(tree: tpd.Tree)(using Context)
  ...
  def msg(using Context) =
    val more = if tree.isInstanceOf[tpd.Apply] then " more" else ""
    val meth = methodSymbol
    val methStr = if meth.exists then meth.showLocated else "expression"
...
class AmbiguousOverload(
    tree: tpd.Tree,
    val alternatives: List[SingleDenotation],
    pt: Type,
    addendum: String = ""
)(implicit ctx: Context)
    extends ReferenceMsg(AmbiguousOverloadID),
      NoDisambiguation:
  private def all = if alternatives.length == 2 then "both" else "all"
  def msg(using Context) =
  ...
  def explain(using Context) =
    i"""|There are ${alternatives.length} methods that could be referenced as the compiler knows too little
        ...
        |- adding a type ascription as in ${hl(
        "instance.myMethod: String => Int"
      )}
        |"""

class AmbiguousExtensionMethod(
    tree: untpd.Tree,
    expansion1: tpd.Tree,
    expansion2: tpd.Tree
)(using Context)
    extends ReferenceMsg(AmbiguousExtensionMethodID),
      NoDisambiguation:
  def msg(using Context) =
...
class TypeDoesNotTakeParameters(tpe: Type, params: List[untpd.Tree])(using
    Context
  ...
  def explain(using Context) =
    val ps =
      if params.size == 1 then s"a type parameter ${params.head}"
      else s"type parameters ${params.map(_.show).mkString(", ")}"
    i"""You specified ${NoColor(ps)} for $tpe, which is not
        |declared to take any.
        |"""

class VarValParametersMayNotBeCallByName(name: TermName, mutable: Boolean)(using
    Context
  ...
  def explain(using Context) =
    i"""${hl("var")} and ${hl(
        "val"
      )} parameters of classes and traits may no be call-by-name. In case you
...
class MissingTypeParameterInTypeApp(tpe: Type)(using Context)
  ...
  def parameters = if numParams == 1 then "parameter" else "parameters"
  def msg(using Context) = i"Missing type $parameters for $tpe"
  def explain(using Context) =
    i"A fully applied type is expected but $tpe takes $numParams $parameters"
...
class MissingArgumentList(method: String, sym: Symbol)(using Context)
  ...
  def msg(using Context) =
    val symDcl =
      if sym.exists then "\n\n  " + hl(sym.showDcl(using ctx.withoutColors))
      else ""
...
class DoesNotConformToBound(tpe: Type, which: String, bound: Type)(using
    Context
) extends TypeMismatchMsg(
      if which == "lower" then bound else tpe,
      if which == "lower" then tpe else bound
  ...
  def msg(using Context) =
    ...
    else i"Type argument ${tpe} does not conform to $which bound $bound"

class DoesNotConformToSelfType(
    category: String,
    selfType: Type,
    cls: Symbol,
    otherSelf: Type,
  ...
  def msg(using Context) =
    i"""$category: self type $selfType of $cls does not conform to self type $otherSelf
                |of $relation $other"""

class DoesNotConformToSelfTypeCantBeInstantiated(tp: Type, selfType: Type)(
    implicit ctx: Context
) extends TypeMismatchMsg(tp, selfType)(
      DoesNotConformToSelfTypeCantBeInstantiatedID
    ):
  def msg(using Context) =
    i"""$tp does not conform to its self type $selfType; cannot be instantiated"""

class IllegalParameterInit(
    found: Type,
    expected: Type,
    param: Symbol,
    cls: Symbol
...
class AbstractMemberMayNotHaveModifier(sym: Symbol, flag: FlagSet)(implicit
  ...
  def msg(using Context) = i"""${hl(
      "abstract"
    )} $sym may not have `${flag.flagsString}` modifier"""
  def explain(using Context) = ""
...
class TypesAndTraitsCantBeImplicit()(using Context)
  ...
  def msg(using Context) = i"""${hl(
      "implicit"
    )} modifier cannot be used for types or traits"""
  def explain(using Context) = ""

class OnlyClassesCanBeAbstract(sym: Symbol)(implicit ctx: Context)
    extends SyntaxMsg(OnlyClassesCanBeAbstractID):
  def explain(using Context) = ""
  def msg(using Context) = i"""${hl(
      "abstract"
    )} modifier can be used only for classes; it should be omitted for abstract members"""

class AbstractOverrideOnlyInTraits(sym: Symbol)(implicit ctx: Context)
    extends SyntaxMsg(AbstractOverrideOnlyInTraitsID):
  def msg(using Context) = i"""${hl(
      "abstract override"
    )} modifier only allowed for members of traits"""
  def explain(using Context) = ""

class TraitsMayNotBeFinal(sym: Symbol)(implicit ctx: Context)
...
class NativeMembersMayNotHaveImplementation(sym: Symbol)(implicit ctx: Context)
    extends SyntaxMsg(NativeMembersMayNotHaveImplementationID):
  def msg(using Context) = i"""${hl(
      "@native"
    )} members may not have an implementation"""
  def explain(using Context) = ""

class TraitMayNotDefineNativeMethod(sym: Symbol)(implicit ctx: Context)
    extends SyntaxMsg(TraitMayNotDefineNativeMethodID):
  def msg(using Context) = i"""A trait cannot define a ${hl(
      "@native"
    )} method."""
  def explain(using Context) = ""

class OnlyClassesCanHaveDeclaredButUndefinedMembers(sym: Symbol)(implicit
    ctx: Context
) extends SyntaxMsg(OnlyClassesCanHaveDeclaredButUndefinedMembersID):

  def msg(using Context) =
    i"""Declaration of $sym not allowed here: only classes can have declared but undefined members"""
  def explain(using Context) =
...
class CannotExtendAnyVal(sym: Symbol)(using Context)
  ...
  def explain(using Context) =
    i"""Only classes (not traits) are allowed to extend ${hl(
        "AnyVal"
      )}, but traits may extend
        |${hl("Any")} to become ${Green(
        "\"universal traits\""
      )} which may only have ${hl("def")} members.
...
class CannotExtendJavaEnum(sym: Symbol)(using Context)
  ...
  def msg(using Context) = i"""$sym cannot extend ${hl(
      "java.lang.Enum"
    )}: only enums defined with the ${hl("enum")} syntax can"""
  def explain(using Context) = ""
...
class JavaEnumParentArgs(parent: Type)(using Context)
  ...
  def msg(using Context) = i"""not enough arguments for constructor Enum: ${hl(
      "(name: String, ordinal: Int)"
    )}: ${hl(parent.show)}"""
  def explain(using Context) = ""

class CannotHaveSameNameAs(
    sym: Symbol,
    cls: Symbol,
    reason: CannotHaveSameNameAs.Reason
)(using Context)
  ...
  def msg(using Context) =
    i"""$sym cannot have the same name as ${cls.showLocated} -- """ + reasonMessage
  def explain(using Context) = ""
...
class ValueClassesMayNotDefineASecondaryConstructor(
  ...
  def msg(using Context) =
    i"""Value classes may not define a secondary constructor"""
  def explain(using Context) = ""
...
class ValueClassesMayNotContainInitalization(valueClass: Symbol)(using Context)
  ...
  def msg(using Context) =
    i"""Value classes may not contain initialization statements"""
  def explain(using Context) = ""
...
class ValueClassesMayNotBeContainted(valueClass: Symbol)(using Context)
  ...
  private def localOrMember =
    if valueClass.owner.isTerm then "local class" else "member of another class"
...
class ValueClassParameterMayNotBeAVar(valueClass: Symbol, param: Symbol)(using
    Context
) extends SyntaxMsg(ValueClassParameterMayNotBeAVarID):
  def msg(using Context) = i"""A value class parameter may not be a ${hl(
      "var"
    )}"""
  def explain(using Context) =
...
class ValueClassParameterMayNotBeCallByName(valueClass: Symbol, param: Symbol)(
    using Context
) extends SyntaxMsg(ValueClassParameterMayNotBeCallByNameID):
  def msg(using Context) =
    s"Value class parameter `${param.name}` may not be call-by-name"
  def explain(using Context) = ""
...
class NotAPath(tp: Type, usage: String)(using Context)
    extends TypeMsg(NotAPathID):
  def msg(using Context) =
    i"$tp is not a valid $usage, since it is not an immutable path"
  def explain(using Context) =
...
class NoReturnFromInlineable(owner: Symbol)(using Context)
  ...
  def msg(using Context) =
    i"No explicit ${hl("return")} allowed from inlineable $owner"
  def explain(using Context) =
    i"""Methods marked with ${hl("inline")} modifier may not use ${hl(
        "return"
      )} statements.
...
class ExpectedTypeBoundOrEquals(found: Token)(using Context)
  ...
  def msg(using Context) =
    i"${hl("=")}, ${hl(">:")}, or ${hl("<:")} expected, but ${Tokens.showToken(found)} found"
  ...
  def explain(using Context) =
    i"""Type parameters and abstract types may be constrained by a type bound.
        ...
        |
        |A lower type bound ${hl(
        "B >: A"
      )} expresses that the type variable ${hl("B")}
        |refers to a supertype of type ${hl("A")}.
        |
        |An upper type bound ${hl("T <: A")} declares that type variable ${hl(
        "T"
      )}
...
class ClassAndCompanionNameClash(cls: Symbol, other: Symbol)(using Context)
  ...
  def explain(using Context) =
    i"""|A ${cls.kindString} and its companion object cannot both define a ${hl(
        "class"
...
class TailrecNotApplicable(symbol: Symbol)(using Context)
  ...
  def msg(using Context) =
    val reason =
      ...
      else if !symbol.isEffectivelyFinal then
        i"$symbol is neither ${hl("private")} nor ${hl("final")} so can be overridden"
      else i"$symbol contains no recursive calls"

    s"TailRec optimisation not applicable, $reason"
  def explain(using Context) = ""

class FailureToEliminateExistential(
    tp: Type,
    tp1: Type,
    tp2: Type,
    boundSyms: List[Symbol],
    classRoot: Symbol
)(using Context)
...
class OnlyFunctionsCanBeFollowedByUnderscore(tp: Type)(using Context)
  ...
  def msg(using Context) =
    i"Only function types can be followed by ${hl("_")} but the current expression has type $tp"
  def explain(using Context) =
    i"""The syntax ${hl("x _")} is no longer supported if ${hl(
        "x"
      )} is not a function.
        |To convert to a function value, you need to explicitly write ${hl(
        "() => x"
      )}"""
...
class UndefinedNamedTypeParameter(
    undefinedName: Name,
    definedNames: List[Name]
)(using Context)
    extends SyntaxMsg(UndefinedNamedTypeParameterID):
  def msg(using Context) =
    i"Type parameter $undefinedName is undefined. Expected one of ${definedNames.map(_.show).mkString(", ")}."
  def explain(using Context) = ""

class IllegalStartOfStatement(
    what: String,
    isModifier: Boolean,
    isStat: Boolean
)(using Context)
    extends SyntaxMsg(IllegalStartOfStatementID):
  def msg(using Context) =
    if isStat then "this kind of statement is not allowed here"
    else
      val addendum =
        if isModifier then ": this modifier is not allowed here" else ""
...
class TraitIsExpected(symbol: Symbol)(using Context)
    extends SyntaxMsg(TraitIsExpectedID):
...
class TraitRedefinedFinalMethodFromAnyRef(method: Symbol)(using Context)
    extends SyntaxMsg(TraitRedefinedFinalMethodFromAnyRefID):
  def msg(using Context) =
    i"Traits cannot redefine final $method from ${hl("class AnyRef")}."
  def explain(using Context) = ""

class AlreadyDefined(name: Name, owner: Symbol, conflicting: Symbol)(using
    Context
) extends NamingMsg(AlreadyDefinedID):
  def msg(using Context) =
    def where: String =
      if conflicting.effectiveOwner.is(
          Package
        ) && conflicting.associatedFile != null
      then i" in ${conflicting.associatedFile}"
    ...
    else i"$name is already defined as $conflicting$where$note"
  def explain(using Context) = ""

class PackageNameAlreadyDefined(pkg: Symbol)(using Context)
    extends NamingMsg(PackageNameAlreadyDefinedID):
  def msg(using Context) =
    def where =
      if pkg.associatedFile == null then "" else s" in ${pkg.associatedFile}"
    i"""${pkg.name} is the name of $pkg$where.
        |It cannot be used at the same time as the name of a package."""
  def explain(using Context) =
    def or = if pkg.associatedFile == null then ""
    else " or delete the containing class file"
    i"""An ${hl(
        "object"
      )} or other toplevel definition cannot have the same name as an existing ${hl(
        "package"
      )}.
        |Rename either one of them$or."""

class UnapplyInvalidNumberOfArguments(qual: untpd.Tree, argTypes: List[Type])(
    using Context
) extends SyntaxMsg(UnapplyInvalidNumberOfArgumentsID):
  def msg(using Context) =
    i"Wrong number of argument patterns for $qual; expected: ($argTypes%, %)"
  def explain(using Context) =
...
class UnapplyInvalidReturnType(unapplyResult: Type, unapplyName: Name)(using
    Context
  ...
  def msg(using Context) =
    ...
    i"""| ${Red(
        i"$unapplyResult"
      )} is not a valid result type of an $unapplyName method of an ${Magenta(
        "extractor"
      )}.$addendum"""
  def explain(using Context) = if unapplyName.show == "unapply" then
    i"""
        |To be used as an extractor, an unapply method has to return a type that either:
        | - has members ${Magenta("isEmpty: Boolean")} and ${Magenta(
        "get: S"
      )} (usually an ${Green("Option[S]")})
        | - is a ${Green("Boolean")}
        | - is a ${Green("Product")} (like a ${Magenta(
        "Tuple2[T1, T2]"
      )}) of arity i with i >= 1, and has members _1 to _i
        |
    ...
    i"""
        ...
        |${Magenta("isEmpty: Boolean")} and ${Magenta(
        "get: S"
      )} where ${Magenta("S <: Seq[V]")} (usually an ${Green(
        "Option[Seq[V]]"
      )}):
        |
        |object CharList {
        |  def unapplySeq(s: String): ${Green(
        "Option[Seq[Char]"
      )} = Some(s.toList)
        |
        |  "example" match {
        ...
        |}
      """
end UnapplyInvalidReturnType

class StaticFieldsOnlyAllowedInObjects(member: Symbol)(using Context)
    extends SyntaxMsg(StaticFieldsOnlyAllowedInObjectsID):
  def msg(using Context) =
    i"${hl("@static")} $member in ${member.owner} must be defined inside a static ${hl("object")}."
  def explain(using Context) =
    i"${hl("@static")} members are only allowed inside objects."

class StaticFieldsShouldPrecedeNonStatic(member: Symbol, defns: List[tpd.Tree])(
    using Context
) extends SyntaxMsg(StaticFieldsShouldPrecedeNonStaticID):
  def msg(using Context) =
    i"${hl("@static")} $member in ${member.owner} must be defined before non-static fields."
  def explain(using Context) =
    val nonStatics = defns
      .takeWhile(_.symbol != member)
      .take(3)
      .filter(_.isInstanceOf[tpd.ValDef])
    ...
    i"""The fields annotated with @static should precede any non @static fields.
      ...
      |
      |The definition of ${member.name} should have been before the non ${hl(
        "@static val"
      )}s:
      |$codeExample
      |"""

class CyclicInheritance(symbol: Symbol, addendum: => String)(using Context)
    extends SyntaxMsg(CyclicInheritanceID):
  def msg(using Context) =
    i"Cyclic inheritance: $symbol extends itself$addendum"
  def explain(using Context) =
...
class BadSymbolicReference(denot: SymDenotation)(using Context)
  ...
  def msg(using Context) =
    ...
    val denotationName = ctx.fresh
      .setSetting(ctx.settings.YdebugNames, true)
      .printer
      .nameString(denot.name)
    val file = denot.symbol.associatedFile
    val (location, src) =
      if file != null then (s" in $file", file.toString)
      else ("", "the signature")
...
class UnableToExtendSealedClass(pclazz: Symbol)(using Context)
    extends SyntaxMsg(UnableToExtendSealedClassID):
  def msg(using Context) =
    i"Cannot extend ${hl("sealed")} $pclazz in a different source file"
  def explain(using Context) =
    "A sealed class or trait can only be extended in the same file as its declaration"

class SymbolHasUnparsableVersionNumber(symbol: Symbol, errorMessage: String)(
    using Context
) extends SyntaxMsg(SymbolHasUnparsableVersionNumberID):
  def msg(using Context) =
    i"${symbol.showLocated} has an unparsable version number: $errorMessage"
  def explain(using Context) =
    i"""The ${symbol.showLocated} is marked with ${hl(
        "@migration"
      )} indicating it has changed semantics
        |between versions and the ${hl(
        "-Xmigration"
      )} settings is used to warn about constructs
...
class SymbolChangedSemanticsInVersion(
    ...
    extends SyntaxMsg(SymbolChangedSemanticsInVersionID):
  def msg(using Context) =
    i"${symbol.showLocated} has changed semantics in version $migrationVersion: $migrationMessage"
  def explain(using Context) =
    i"""The ${symbol.showLocated} is marked with ${hl(
        "@migration"
      )} indicating it has changed semantics
...
class UnableToEmitSwitch()(using Context)
  ...
  def msg(using Context) =
    i"Could not emit switch for ${hl("@switch")} annotated match"
  def explain(using Context) =
    val codeExample =
      """val ConstantB = 'B'
    ...
    i"""If annotated with ${hl("@switch")}, the compiler will verify that the match has been compiled to a
        ...
        |- the matched value is not of type ${hl("Int")}, ${hl("Byte")}, ${hl(
        "Short"
      )} or ${hl("Char")}
...
class PolymorphicMethodMissingTypeInParent(rsym: Symbol, parentSym: Symbol)(
    using Context
) extends SyntaxMsg(PolymorphicMethodMissingTypeInParentID):
  def msg(using Context) =
    i"Polymorphic refinement $rsym without matching type in parent $parentSym is no longer allowed"
  def explain(using Context) =
...
class ParamsNoInline(owner: Symbol)(using Context)
  ...
  def msg(using Context) = i"""${hl(
      "inline"
    )} modifier can only be used for parameters of inline methods"""
  def explain(using Context) = ""

class JavaSymbolIsNotAValue(symbol: Symbol)(using Context)
    extends TypeMsg(JavaSymbolIsNotAValueID):
  def msg(using Context) =
    val kind =
...
class DoubleDefinition(decl: Symbol, previousDecl: Symbol, base: Symbol)(using
    Context
) extends NamingMsg(DoubleDefinitionID):
  def msg(using Context) =
    def nameAnd = if decl.name != previousDecl.name then " name and" else ""
    def erasedType = if ctx.erasedTypes then i" ${decl.info}" else ""
    def details(using Context): String =
      if decl.isRealMethod && previousDecl.isRealMethod then
    ...
    def symLocation(sym: Symbol) =
      val lineDesc =
        if sym.span.exists && sym.span != sym.owner.span then
          s" at line ${sym.srcPos.line + 1}"
        else ""
      i"in ${sym.owner}${lineDesc}"
    val clashDescription =
      if decl.owner eq previousDecl.owner then "Double definition"
      else if (decl.owner eq base) || (previousDecl eq base) then
...
class ImportRenamedTwice(ident: untpd.Ident)(using Context)
    extends SyntaxMsg(ImportRenamedTwiceID):
  def msg(using Context) =
    s"${ident.show} is renamed twice on the same import line."
  def explain(using Context) = ""

class TypeTestAlwaysDiverges(scrutTp: Type, testTp: Type)(using Context)
    extends SyntaxMsg(TypeTestAlwaysDivergesID):
  def msg(using Context) =
...
class TermMemberNeedsResultTypeForImplicitSearch(cycleSym: Symbol)(using
    Context
) extends CyclicMsg(TermMemberNeedsNeedsResultTypeForImplicitSearchID):
  def msg(using Context) =
    i"""$cycleSym needs result type because its right-hand side attempts implicit search"""
  def explain(using Context) =
...
class ClassCannotExtendEnum(cls: Symbol, parent: Symbol)(using Context)
    extends SyntaxMsg(ClassCannotExtendEnumID):
  def msg(using Context) =
    i"""$cls in ${cls.owner} extends enum ${parent.name}, but extending enums is prohibited."""
  def explain(using Context) = ""

class NotAnExtractor(tree: untpd.Tree)(using Context)
    extends SyntaxMsg(NotAnExtractorID):
  def msg(using Context) =
    i"$tree cannot be used as an extractor in a pattern because it lacks an unapply or unapplySeq method"
  def explain(using Context) =
    i"""|An ${hl("unapply")} method should be defined in an ${hl(
        "object"
      )} as follow:
        |  - If it is just a test, return a ${hl("Boolean")}. For example ${hl(
        "case even()"
      )}
        |  - If it returns a single sub-value of type T, return an ${hl(
        "Option[T]"
      )}
        |  - If it returns several sub-values T1,...,Tn, group them in an optional tuple ${hl(
        "Option[(T1,...,Tn)]"
      )}
        |
        |Sometimes, the number of sub-values isn't fixed and we would like to return a sequence.
        |For this reason, you can also define patterns through ${hl(
        "unapplySeq"
      )} which returns ${hl("Option[Seq[T]]")}.
        |This mechanism is used for instance in pattern ${hl(
        "case List(x1, ..., xn)"
      )}"""
...
class MemberWithSameNameAsStatic()(using Context)
  ...
  def msg(using Context) =
    i"Companion classes cannot define members with same name as a ${hl("@static")} member"
  def explain(using Context) = ""

class PureExpressionInStatementPosition(
    stat: untpd.Tree,
    val exprOwner: Symbol
)(using Context)
...
class UnqualifiedCallToAnyRefMethod(stat: untpd.Tree, method: Symbol)(using
    Context
) extends Message(UnqualifiedCallToAnyRefMethodID):
  def kind = MessageKind.PotentialIssue
  def msg(using Context) =
    i"Suspicious top-level unqualified call to ${hl(method.name.toString)}"
  def explain(using Context) =
    i"""Top-level unqualified calls to ${hl("AnyRef")} or ${hl(
        "Any"
      )} methods such as ${hl(method.name.toString)} are
       |resolved to calls on ${hl(
        "Predef"
      )} or on imported methods. This might not be what
       |you intended."""

class TraitCompanionWithMutableStatic()(using Context)
    extends SyntaxMsg(TraitCompanionWithMutableStaticID):
  def msg(using Context) =
    i"Companion of traits cannot define mutable @static fields"
  def explain(using Context) = ""

class LazyStaticField()(using Context) extends SyntaxMsg(LazyStaticFieldID):
...
class StaticOverridingNonStaticMembers()(using Context)
  ...
  def msg(using Context) =
    i"${hl("@static")} members cannot override or implement non-static ones"
  def explain(using Context) = ""
...
class IllegalSuperAccessor(
    ...
    extends DeclarationMsg(IllegalSuperAccessorID):
  def msg(using Context) =
    ...
    val staticSuperCall =
      val staticSuper = accMixin.asClass.info.parents.reverse
        .find(
          _.nonPrivateMember(memberName)
            .matchingDenotation(accMixin.thisType, acc.info, targetName)
            .exists
        )
...
class IllegalCyclicTypeReference(sym: Symbol, where: String, lastChecked: Type)(
    using Context
...
class ErasedTypesCanOnlyBeFunctionTypes()(using Context)
  ...
  def msg(using Context) =
    "Types with erased keyword can only be function types `(erased ...) => ...`"
  def explain(using Context) = ""
...
class CaseClassMissingNonImplicitParamList(cdef: untpd.TypeDef)(using Context)
  ...
  def msg(using Context) =
    i"""|A ${hl(
        "case class"
      )} must have at least one leading non-implicit parameter list"""
  ...
  def explain(using Context) =
    i"""|${cdef.name} must have at least one leading non-implicit parameter list,
        ...
        | add an explicit ${hl(
        "()"
      )} as the first parameter list to ${cdef.name}."""
...
class TypedCaseDoesNotExplicitlyExtendTypedEnum(
    enumDef: Symbol,
    caseDef: untpd.TypeDef
)(using Context)
    extends SyntaxMsg(TypedCaseDoesNotExplicitlyExtendTypedEnumID):
  def msg(using Context) =
    i"explicit extends clause needed because both enum case and enum class have type parameters"
  ...
  def explain(using Context) =
    i"""Enumerations where the enum class as well as the enum case have type parameters need
        ...
        |  ${hl("case")} ${caseDef.name}[U](u: U) ${hl(
        "extends"
      )} ${enumDef.name}[U]
        | }
        |"""

class IllegalRedefinitionOfStandardKind(kindType: String, name: Name)(using
    Context
...
class NoExtensionMethodAllowed(mdef: untpd.DefDef)(using Context)
  ...
  def msg(using Context) =
    i"No extension method allowed here, since collective parameters are given"
  def explain(using Context) =
    i"""|Extension method:
        |  `${mdef}`
...
class ExtensionMethodCannotHaveTypeParams(mdef: untpd.DefDef)(using Context)
  ...
  def msg(using Context) =
    i"Extension method cannot have type parameters since some were already given previously"

  def explain(using Context) =
    i"""|Extension method:
        |  `${mdef}`
        |has type parameters `[${mdef.leadingTypeParams
        .map(_.show)
        .mkString(",")}]`, while the extension clause has
...
class ExtensionCanOnlyHaveDefs(mdef: untpd.Tree)(using Context)
  ...
  def msg(using Context) =
    i"Only methods allowed here, since collective parameters are given"
  def explain(using Context) =
...
class UnexpectedPatternForSummonFrom(tree: Tree[?])(using Context)
  ...
  def msg(using Context) =
    i"Unexpected pattern for summonFrom. Expected ${hl("`x: T`")} or ${hl("`_`")}"
  def explain(using Context) =
    i"""|The pattern "${tree.show}" provided in the ${hl(
        "case"
      )} expression of the ${hl("summonFrom")},
...
class AnonymousInstanceCannotBeEmpty(impl: untpd.Template)(using Context)
  ...
  def msg(using Context) =
    i"anonymous instance must implement a type or have at least one extension method"
  def explain(using Context) =
...
class ModifierNotAllowedForDefinition(flag: Flag)(using Context)
  ...
  def msg(using Context) =
    i"Modifier ${hl(flag.flagsString)} is not allowed for this definition"
  def explain(using Context) = ""

class RedundantModifier(flag: Flag)(using Context)
    extends SyntaxMsg(RedundantModifierID):
  def msg(using Context) =
    i"Modifier ${hl(flag.flagsString)} is redundant for this definition"
  def explain(using Context) = ""

class InvalidReferenceInImplicitNotFoundAnnotation(
    typeVar: String,
    owner: String
)(using Context)
    extends ReferenceMsg(InvalidReferenceInImplicitNotFoundAnnotationID):
  def msg(using Context) = i"""|Invalid reference to a type variable ${hl(
      typeVar
    )} found in the annotation argument.
                |The variable does not occur as a parameter in the scope of ${hl(
      owner
    )}.
                |"""
  def explain(using Context) = ""
...
class CaseClassInInlinedCode(tree: tpd.Tree)(using Context)
  ...
  def msg(using Context) =
    s"Case $defKind definitions are not allowed in inline methods or quoted code. Use a normal $defKind instead."
  def explain(using Context) =
...
class ImplicitSearchTooLargeWarning(
    limit: Int,
    openSearchPairs: List[(Candidate, Type)]
)(using Context)
...
class TargetNameOnTopLevelClass(symbol: Symbol)(using Context)
  ...
  def msg(using Context) =
    i"${hl("@targetName")} annotation not allowed on top-level $symbol"
  def explain(using Context) =
    val annot = symbol.getAnnotation(defn.TargetNameAnnot).get
    i"""The @targetName annotation may be applied to a top-level ${hl(
        "val"
      )} or ${hl("def")}, but not
        ...
        |
        |If your use case requires @targetName, consider wrapping $symbol in an ${hl(
        "object"
      )}
...
class NotClassType(tp: Type)(using Context)
    extends TypeMsg(NotClassTypeID),
      ShowMatchTrace(tp):
...
class MissingImplicitArgument(
    ...
    extends TypeMsg(MissingImplicitArgumentID),
      ShowMatchTrace(pt):
  ...
  /** Format `raw` implicitNotFound or implicitAmbiguous argument, replacing all
    * occurrences of `${X}` where `X` is in `paramNames` with the corresponding
    * shown type in `args`.
    */
  def userDefinedErrorString(
      raw: String,
    ...
    def translate(name: String): Option[String] =
      ...
      if idx >= 0 then Some(i"${args(idx)}") else None
    """\$\{\s*([^}\s]+)\s*\}""".r.replaceAllIn(
      raw,
      (_: Regex.Match) match
  ...
  /** @param rawMsg
    ...
    * @param sym
    *   Symbol of the annotated type or of the method whose parameter was
    *   annotated
    * @param substituteType
    *   Function substituting specific types for abstract types associated with
    *   variables, e.g A -> Int
    */
  def formatAnnotationMessage(
    ...
    userDefinedErrorString(
      rawMsg,
      paramNames =
        substitutableTypesSymbols.map(_.name.unexpandedName.toString),
      args = substitutableTypesSymbols.map(_.typeRef).map(substituteType)
    )

  /** Extract a user defined error message from a symbol `sym` with an
    * annotation matching the given class symbol `cls`.
    */
  ...
  /** Extracting the message from a method parameter, e.g. in
    ...
    * trait Foo
    *
    * def foo(implicit @annotation.implicitNotFound("Foo is missing") foo: Foo):
    * Any = ???
    */
    ...
    paramSymWithMethodCallTree.flatMap: (sym, applTree) =>
      userDefinedMsg(sym, defn.ImplicitNotFoundAnnot).map: rawMsg =>
        ...
        val substituteType = (_: Type)
          .asSeenFrom(methodOwnerType, methodOwner)
          .subst(methodTypeParams, methodTypeArgs)
  ...
  /** The implicitNotFound annotation on the parameter, or else on the type.
    ...
    * @param explain
    *   The message is used for an additional explanation, not the message
    *   proper.
    */
  def userDefinedImplicitNotFoundMessage(explain: Boolean)(using
      Context
  ): Option[String] =
  ...
  def msg(using Context): String =
    ...
    def location(preposition: String) =
      if where.isEmpty then "" else s" $preposition $where"
    ...
    /** Construct a custom error message given an ambiguous implicit candidate
      * `alt` and a user defined message `raw`.
      */
    def userDefinedAmbiguousImplicitMsg(alt: SearchSuccess, raw: String) =
      ...
      def resolveTypes(targs: List[tpd.Tree])(using Context) =
        targs.map(a =>
          Inferencing.fullyDefinedType(a.tpe, "type argument", a.srcPos)
        )
    ...
    /** Extracting the message from a type, e.g. in
      *
      * @annotation.implicitNotFound("Foo
      *   is missing") trait Foo
      *
  ...
  def msg(using Context) =
    ...
    val whatCanNot = alts match
      case Nil =>
        i"$name cannot"
      case sym :: Nil =>
        i"${if sym.owner == pre.typeSymbol then sym.show else sym.showLocated} cannot"
      case _ =>
        i"none of the overloaded alternatives named $name can"
    val where =
      if ctx.owner.exists then s" from ${ctx.owner.enclosingClass}" else ""
...
class MatchTypeScrutineeCannotBeHigherKinded(tp: Type)(using Context)
  ...
  def msg(using Context) =
    i"the scrutinee of a match type cannot be higher-kinded"
  def explain(using Context) = ""
