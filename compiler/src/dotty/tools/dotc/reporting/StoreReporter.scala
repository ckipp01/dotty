...
/** This class implements a Reporter that stores all messages
  ...
  */
class StoreReporter(
    outer: Reporter | Null = Reporter.NoReporter,
    fromTyperState: Boolean = false
) extends Reporter:
  ...
  def doReport(dia: Diagnostic)(using Context): Unit =
    ...
    if infos == null then infos = new mutable.ListBuffer
    infos.uncheckedNN += dia

  override def hasUnreportedErrors: Boolean =
    outer != null && infos != null && infos.uncheckedNN.exists(
      _.isInstanceOf[Error]
    )
  ...
  override def removeBufferedMessages(using Context): List[Diagnostic] =
    if infos != null then
      try infos.uncheckedNN.toList
      finally infos = null
    else Nil

  override def pendingMessages(using Context): List[Diagnostic] =
    if infos != null then infos.uncheckedNN.toList else Nil

  override def errorsReported: Boolean =
    hasErrors || (outer != null && outer.errorsReported)
