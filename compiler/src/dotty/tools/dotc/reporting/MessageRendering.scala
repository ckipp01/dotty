...
trait MessageRendering:
  ...
  def inlinePosStack(pos: SourcePosition): List[SourcePosition] =
    if pos.outer != null && pos.outer.exists then
      pos :: inlinePosStack(pos.outer)
    else Nil
  ...
  private def sourceLines(
      pos: SourcePosition
  )(using Context, Level, Offset): (List[String], List[String], Int) =
  ...
  private def newBox(
    ...
    val line =
      if soft then ("- " * ((lineWidth + 1) / 2)).trim else "-" * lineWidth
    hl(s"$prefix|$line")
  ...
  /** The error message (`msg`) aligned under `pos`
    ...
    */
  private def errorMsg(pos: SourcePosition, msg: String)(using
      Context,
      Level,
      Offset
  ): String =
    val padding = msg.linesIterator.foldLeft(pos.startColumnPadding) {
      (pad, line) =>
        val lineLength = stripColor(line).length
        val maxPad = math.max(
          0,
          ctx.settings.pageWidth.value - offset - lineLength
        ) - offset

        if maxPad < pad.length then " " * maxPad
        else pad
    }
  ...
  /** The source file path, line and column numbers from the given
    * SourcePosition
    */
  ...
  private def posStr(
    ...
    if pos.source != NoSourcePosition.source then
      hl({
        ...
        val title =
          if fileAndPos.isEmpty then
            s"$errId$kind:" // this happens in dotty.tools.repl.ScriptedTests // TODO add name of source or remove `:` (and update test files)
  ...
  def explanation(m: Message)(using Context): String =
    ...
    if !m.explanation.endsWith(EOL) then sb.append(EOL)
    sb.toString

  private def appendFilterHelp(
      dia: Diagnostic,
    ...
    if hasId || category.nonEmpty then
      ...
      if hasId then
        ...
        sb.append(EOL)
          .append("  - name=")
          .append(msg.errorId.productPrefix.stripSuffix("ID"))
  ...
  def messageAndPos(dia: Diagnostic)(using Context): String =
    ...
    if Diagnostic.shouldExplain(dia) then
      ...
      sb.append(EOL)
        .append(offsetBox)
        .append(" longer explanation available when compiling with `-explain`")
