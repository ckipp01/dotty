...
import java.util.{Collections, Optional, List as JList}
...
object Diagnostic:
  ...
  def shouldExplain(dia: Diagnostic)(using Context): Boolean =
    ctx.settings.explain.value && dia.msg.canExplain
      || ctx.settings.explainTypes.value && dia.msg
        .isInstanceOf[TypeMismatchMsg]
  ...
  /** A sticky error is an error that should not be hidden by backtracking and
    * trying some alternative path. Typically, errors issued after catching a
    * TypeError exception are sticky.
    */
  class StickyError(
      msg: Message,
      pos: SourcePosition
  ...
  class Warning(
    ...
    def toError: Error =
      new Error(msg, pos).tap(e => if isVerbose then e.setVerbose())
    def toInfo: Info =
      new Info(msg, pos).tap(e => if isVerbose then e.setVerbose())
...
class Diagnostic(
  ...
  override def position: Optional[interfaces.SourcePosition] =
    if pos.exists && pos.source.exists then Optional.of(pos)
    else Optional.empty()
  override def message: String =
    msg.message.replaceAll("\u001B\\[[;\\d]*m", "")
  override def diagnosticRelatedInformation
      : JList[interfaces.DiagnosticRelatedInformation] =
