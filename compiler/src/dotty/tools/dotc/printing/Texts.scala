...
object Texts:
  ...
  sealed abstract class Text:
    ...
    def remaining(width: Int): Int = this match
      ...
      case Fluid(last :: prevs) =>
        ...
        if r < 0 then r else Fluid(prevs) remaining r
      case Vertical(_) =>
        -1
    ...
    def appendToLastLine(that: Text): Text = that match
      case Str(s2, lines1) =>
        this match
          ...
          case Fluid(Str(s1, lines2) :: prev) =>
            Fluid(Str(s1 + s2, lines1 union lines2) :: prev)
          case Fluid(relems) => Fluid(that :: relems)
          case Vertical(_) =>
            throw new IllegalArgumentException(
              "Unexpected Vertical.appendToLastLine"
            )
      case Fluid(relems) =>
        relems.reverse.foldLeft(this)(_ appendToLastLine _)
      case Vertical(_) =>
        throw new IllegalArgumentException(
          "Unexpected Text.appendToLastLine(Vertical(...))"
        )
    ...
    private def append(width: Int)(that: Text): Text =
      ...
      else if that.remaining(width - lengthWithoutAnsi(lastLine)) >= 0 then
        appendToLastLine(that)
      else if that.isSplittable then
        that.relems.reverse.foldLeft(this)(_.append(width)(_))
    ...
    def stripPrefix(pre: String): Text = this match
      ...
      case Vertical(relems) =>
        val elems = relems.reverse
        val head = elems.head.stripPrefix(pre)
        if head eq elems.head then this
        else Vertical((head :: elems.tail).reverse)
    ...
    def mkString(width: Int, withLineNumbers: Boolean): String =
      ...
      val numberWidth =
        if withLineNumbers then (2 * maxLine.toString.length) + 2 else 0
    ...
    def ~(that: Text): Text =
      if this.isEmpty then that
        ...
        this match
          case Fluid(relems1) if !isClosed =>
            that match
  ...
  object Text:
    ...
    /** A concatenation of elements in `xs` and interspersed with separator
      * strings `sep`.
      */
    ...
    extension (text: => Text)
      def provided(cond: Boolean): Text = if cond then text else Str("")
  ...
  case class LineRange(start: Int, end: Int):
    def union(that: LineRange): LineRange =
      LineRange(start min that.start, end max that.end)
    def show: String =
      if start == end then (start + 1).toString
      else if start < end then s"${start + 1}-${end + 1}"
      else "" // empty range
