...
import core.*
import Texts.*, Types.*, Flags.*, Names.*, Symbols.*, NameOps.*, Constants.*,
  Denotations.*
import StdNames.*
import Contexts.*
...
class PlainPrinter(_ctx: Context) extends Printer:
  ...
  /** The context of all public methods in Printer and subclasses. Overridden in
    * RefinedPrinter.
    */
  ...
  protected def showUniqueIds =
    ctx.settings.uniqid.value || Printer.debugPrintUnique
    ...
    if homogenizedView then
      tp match
        ...
        case tp @ AppliedType(tycon, args) =>
          if defn.isCompiletimeAppliedType(tycon.typeSymbol) then
  ...
  private def homogenizeArg(tp: Type) = tp match
    case TypeBounds(lo, hi) if homogenizedView && sameBound(lo, hi) =>
      homogenize(hi)
    case _ => tp
  ...
  /** If the name of the symbol's owner should be used when you care about
    ...
    * "package", etc. The kind string, if non-empty, will be phrased relative to
  ...
  /** String representation of a name used in a refinement In refined printing
    * this undoes type parameter expansion
    */
  protected def refinementNameString(tp: RefinedType): String = nameString(
    tp.refinedName
  )
  ...
  protected def argText(arg: Type, isErased: Boolean = false): Text =
    keywordText("erased ").provided(isErased) ~ (homogenizeArg(arg) match
      case arg: TypeBounds => "?" ~ toText(arg)
      case arg             => toText(arg)
    )

  /** Pretty-print comma-separated type arguments for a constructor to be
    * inserted among parentheses or brackets (hence with `GlobalPrec`
    * precedence).
    */
  protected def argsText(args: List[Type]): Text =
    atPrec(GlobalPrec) { Text(args.map(arg => argText(arg)), ", ") }

  /** The longest sequence of refinement types, starting at given type and
    * following parents.
    */
  ...
  /** Direct references to these symbols are printed without their prefix for
    * convenience. They are either aliased in scala.Predef or in the scala
    * package object, as well as `Object`
    */
  private lazy val printWithoutPrefix: Set[Symbol] =
    (defn.ScalaPredefModule.termRef.typeAliasMembers
      ++ defn.ScalaPackageObject.termRef.typeAliasMembers)
      .map(_.info.classSymbol)
      .toSet
      + defn.ObjectClass
  ...
  def toText(tp: Type): Text = controlled {
    homogenize(tp) match
      ...
      case tp: TypeRef =>
        if printWithoutPrefix.contains(tp.symbol) then toText(tp.name)
        else toTextPrefix(tp.prefix) ~ selectionString(tp)
      ...
      case tp: TypeParamRef =>
        val suffix =
          if showNestingLevel then
            ...
            if tvar.exists then
              s"#${tvar.asInstanceOf[TypeVar].nestingLevel.toString}"
            else ""
          else ""
      ...
      case tp: RefinedType =>
        ...
        toTextLocal(parent) ~ "{" ~ Text(
          refined map toTextRefinement,
          "; "
        ).close ~ "}"
      case tp: RecType =>
        try
          openRecs = tp :: openRecs
          "{" ~ selfRecName(openRecs.length) ~ " => " ~ toTextGlobal(
            tp.parent
          ) ~ "}"
        finally openRecs = openRecs.tail
      case AndType(tp1, tp2) =>
        changePrec(AndTypePrec) {
          toText(tp1) ~ " & " ~ atPrec(AndTypePrec + 1) { toText(tp2) }
        }
      case OrType(tp1, tp2) =>
        changePrec(OrTypePrec) {
          toText(tp1) ~ " | " ~ atPrec(OrTypePrec + 1) { toText(tp2) }
        }
      ...
      case tp @ EventuallyCapturingType(parent, refs) =>
        ...
        val refsText =
          if refs.isUniversal then rootSetText else toTextCaptureSet(refs)
      ...
      case tp: WildcardType =>
        if tp.optBounds.exists then "<?" ~ toTextRHS(tp.bounds) ~ ">" else "<?>"
      case NoType =>
        "<notype>"
      case NoPrefix =>
        "<noprefix>"
        ...
        changePrec(GlobalPrec) {
          "("
            ...
            ~ keywordText("implicit ").provided(
              tp.isImplicitMethod && !tp.isContextualMethod
            )
            ~ paramsText(tp)
            ~ ")"
      ...
      case ExprType(restp) =>
        def arrowText: Text = restp match
          case ct @ EventuallyCapturingType(parent, refs)
              if ct.annot.symbol == defn.RetainsByNameAnnot =>
            if refs.isUniversal then Str("=>")
            else Str("->") ~ toTextCaptureSet(refs)
          case _ =>
            if Feature.pureFunsEnabled then "->" else "=>"
        changePrec(GlobalPrec)(arrowText ~ " " ~ toText(restp))
      case tp: HKTypeLambda =>
        changePrec(GlobalPrec) {
          "[" ~ paramsText(tp) ~ "]" ~ lambdaHash(tp) ~ Str(
            " =>> "
      ...
      case tp: TypeVar =>
        ...
        if tp.isInstantiated then toTextCaret(tp.instanceOpt)
          ...
          if bounds.isTypeAlias then toTextCaret(bounds.lo)
  ...
  protected def paramsText(lam: LambdaType): Text =
    ...
    def paramText(name: Name, tp: Type, erased: Boolean) =
      keywordText("erased ").provided(erased) ~ toText(name) ~ lambdaHash(
        lam
      ) ~ toTextRHS(tp, isParameter = true)
    Text(
      lam.paramNames
        .lazyZip(lam.paramInfos)
        .lazyZip(erasedParams)
        .map(paramText),
      ", "
    )
  ...
  protected def idString(sym: Symbol): String =
    (if showUniqueIds || Printer.debugPrintUnique then "#" + sym.id else "") +
      (if showNestingLevel then "%" + sym.nestingLevel else "")

  def nameString(sym: Symbol): String =
    simpleNameString(sym) + idString(
      sym
    ) // + "<" + (if (sym.exists) sym.owner else "") + ">"
  ...
  def fullNameString(sym: Symbol): String =
    if (sym eq defn.FromJavaObjectSymbol) && !printDebug then
      ...
      nameString(sym)
    else fullNameString(fullNameOwner(sym)) + "." + nameString(sym)

  protected def fullNameOwner(sym: Symbol): Symbol =
    sym.effectiveOwner.enclosingClass
  ...
  protected def selectionString(tp: NamedType): String =
    val sym = if homogenizedView then tp.symbol else tp.currentSymbol
    if sym.exists then nameString(sym) else nameString(tp.name)
  ...
  def toTextRef(tp: SingletonType): Text = controlled {
    tp match
      ...
      case pref: TermParamRef =>
        nameString(pref.binder.paramNames(pref.paramNum)) ~ lambdaHash(
          pref.binder
        )
      case tp: RecThis =>
        val idx = openRecs.reverse.indexOf(tp.binder)
        if idx >= 0 then selfRecName(idx + 1)
        else
          "{...}.this" // TODO move underlying type to an addendum, e.g. ... z3 ... where z3: ...
      case tp: SkolemType =>
        if homogenizedView then toText(tp.info)
        else if ctx.settings.XprintTypes.value then
          "<" ~ toText(tp.repr) ~ ":" ~ toText(tp.info) ~ ">"
        else toText(tp.repr)
  }

  /** The string representation of this type used as a prefix, including
    * separator
    */
  ...
  /** String representation of a definition's type following its name, if symbol
      ...
      case lam: HKTypeLambda =>
        val names =
          if lam.isDeclaredVarianceLambda then
            lam.paramNames
              .lazyZip(lam.declaredVariances)
              .map((name, v) => varianceSign(v) + name)
  ...
  protected def toTextRHS(tp: Type, isParameter: Boolean = false): Text =
    controlled {
      homogenize(tp) match
        case tp: TypeBounds =>
          ...
          val binder = rhs match
            ...
            case TypeBounds(lo, hi) =>
              (if lo.isExactlyNothing then Text() else " >: " ~ toText(lo))
                ~ (if hi.isExactlyAny || (!printDebug && hi.isFromJavaObject)
                   then Text()
                   else " <: " ~ toText(hi))
          tparamStr ~ binder
        case tp @ ClassInfo(pre, cls, cparents, decls, selfInfo) =>
          ...
          val tparamsText =
            if tparams.isEmpty then Text()
            else ("[" ~ dclsText(tparams) ~ "]").close
          ...
          val declsText =
            if trueDecls.isEmpty || !ctx.settings.Ydebug.value then Text()
            else dclsText(trueDecls)
          tparamsText ~ " extends " ~ toTextParents(
            tp.parents
          ) ~~ "{" ~ selfText ~ declsText ~
            "} at " ~ preText
        case mt: MethodType =>
        ...
        case tp: ExprType =>
          ...
          (if isParameter then ": => " else ": ") ~ toTextGlobal(tp.widenExpr)
        case tp: PolyType =>
          "[" ~ paramsText(tp) ~ "]"
            ~ (Str(": ") provided !tp.resultType.isInstanceOf[MethodOrPoly])
            ~ toTextGlobal(tp.resultType)
  ...
  protected def toTextParents(parents: List[Type]): Text =
    Text(parents.map(toTextLocal), " with ")
  ...
  def kindString(sym: Symbol): String =
    ...
    if flags.is(PackageClass) then "package class"
    else if flags.is(PackageVal) then "package"
    else if sym.isPackageObject then
      if sym.isClass then "package object class"
      else "package object"
    else if sym.isAnonymousClass then "anonymous class"
    else if flags.is(ModuleClass) then "object class"
    else if flags.is(ModuleVal) then "object"
    else if flags.is(Trait) then "trait"
    else if sym.isClass then "class"
    else if sym.isType then "type"
    else if sym.isGetter then "getter"
    else if sym.isSetter then "setter"
    else if sym.is(Param) then "parameter"
    else if sym.is(Given) then "given instance"
    else if flags.is(Lazy) then "lazy value"
    else if flags.is(Mutable) then "variable"
    else if sym.isClassConstructor && sym.isPrimaryConstructor then
      "primary constructor"
    else if sym.isClassConstructor then "constructor"
    else if sym.is(Method) then "method"
    else if sym.isTerm then "value"
    else ""

  /** String representation of symbol's definition keyword */
  protected def keyString(sym: Symbol): String =
    val flags = sym.flagsUNSAFE
    if flags.isAllOf(JavaInterface) then "interface"
    else if flags.is(Trait) then "trait"
    else if flags.is(Module) then "object"
    else if sym.isClass then "class"
    else if sym.isType then "type"
    else if flags.is(Mutable) then "var"
    else if flags.is(Package) then "package"
    else if sym.is(Method) then "def"
    else if sym.isTerm && !flags.is(Param) then "val"
    else ""

  protected def privateWithinString(sym: Symbol): String =
    if sym.exists && sym.privateWithin.exists then
  ...
  protected def toTextFlags(sym: Symbol): Text =
    toTextFlags(sym, sym.flagsUNSAFE)

  protected def toTextFlags(sym: Symbol, flags: FlagSet): Text =
    Text(
      flags
        .flagStrings(privateWithinString(sym))
        .map(flag => stringToText(keywordStr(flag))),
      " "
    )
  ...
  def dclText(d: SingleDenotation): Text =
    dclTextWithInfo(d.symbol, Some(d.info))
  ...
  def toText(sym: Symbol): Text =
    (kindString(sym) ~~ {
      ...
      else if hasMeaninglessName(sym) && !printDebug then
    ...
    if !sym.exists then ""
      ...
      def recur(ownr: Symbol, innerLocation: String): Text =
        def nextOuter(innerKind: String): Text =
          recur(
            ownr.effectiveOwner,
            if !innerLocation.isEmpty then innerLocation
            else s" in an anonymous $innerKind"
          )
        def showLocation(ownr: Symbol, where: String): Text =
          innerLocation ~ " " ~ where ~ " " ~ toText(ownr)
        if ownr.isAnonymousClass then nextOuter("class")
        else if ownr.isAnonymousFunction then nextOuter("function")
        else if isEmptyPrefix(ownr) then ""
        else if ownr.isLocalDummy then
          showLocation(ownr.owner, "locally defined in")
        else if ownr.isTerm && !ownr.isOneOf(Module | Method) then
  ...
  private def escapedChar(ch: Char): String = (ch: @switch) match
    ...
    case '\\' => "\\\\"
    case _ =>
      if ch.isControl then f"${"\\"}u${ch.toInt}%04x" else String.valueOf(ch).nn

  def toText(const: Constant): Text = const.tag match
    case StringTag =>
      stringText("\"" + escapedString(const.value.toString) + "\"")
  ...
  def toText[T <: Untyped](tree: Tree[T]): Text = {
    ...
    nodeName ~ "(" ~ elems ~ tpSuffix ~ ")" ~ (Str(
      tree.sourcePos.toString
    ) provided printDebug)
  }.close

  def toText(pos: SourcePosition): Text =
    if !pos.exists then "<no position>"
    else if pos.source.exists then s"${pos.source.file.name}:${pos.line + 1}"
    ...
    case result: SearchFailure =>
      result.reason match
        ...
        case result: AmbiguousImplicits =>
          "Ambiguous Implicit: " ~ toText(result.alt1.ref) ~ " and " ~ toText(
            result.alt2.ref
          )
        case _ =>
  ...
  def toText(importInfo: ImportInfo): Text =
    ...
    val exprStr =
      if siteStr.endsWith(".type") then siteStr.dropRight(5) else siteStr
  ...
  def toText(c: OrderingConstraint): Text =
    ...
    finally ctx.typerState.constraint = savedConstraint
    end try
  end toText
  ...
  protected def keywordStr(text: String): String =
    coloredStr(text, SyntaxHighlighting.KeywordColor)
  protected def keywordText(text: String): Text =
    coloredStr(text, SyntaxHighlighting.KeywordColor)
  protected def valDefText(text: Text): Text =
    coloredText(text, SyntaxHighlighting.ValDefColor)
  protected def typeText(text: Text): Text =
    coloredText(text, SyntaxHighlighting.TypeColor)
  protected def literalText(text: Text): Text =
    coloredText(text, SyntaxHighlighting.LiteralColor)
  protected def stringText(text: Text): Text =
    coloredText(text, SyntaxHighlighting.StringColor)

  protected def coloredStr(text: String, color: String): String =
    if ctx.useColors then color + text + SyntaxHighlighting.NoColor else text
