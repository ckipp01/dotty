...
object Formatting:
  ...
  object ShownDef:
    /** Represents a value that has been "shown" and can be consumed by
      * StringFormatter. Not just a string because it may be a Seq that
      * StringFormatter will intersperse with the trailing separator. It may
      * also be a CtxShow, which allows the Show instance to finish showing the
      * value with the string interpolator's correct context, that is with
      * non-sensical tagging, message limiting, explanations, etc.
      */
    ...
    /** The base implementation, passing the argument to StringFormatter which
      * will try to `.show` it.
      */
    ...
    object Show extends ShowImplicits1:
      ...
      given [H: Show, T <: Tuple: Show]: Show[H *: T] with
        def show(x: H *: T) = new CtxShow:
          def run(using Context) =
            show1(x.head) *: Show[T].show(x.tail).ctxShow.asInstanceOf[Tuple]
      ...
      given Show[TypeComparer.ApproxState] with
        def show(x: TypeComparer.ApproxState) =
          TypeComparer.ApproxState.Repr.show(x)
      ...
      private def show1[A: Show](x: A)(using Context) = show2(
        Show[A].show(x).ctxShow
      )
  ...
  /** General purpose string formatter, with the following features:
    *
    *   1. Invokes the `show` extension method on the interpolated arguments. 2.
    *      Sequences can be formatted using the desired separator between two
    *      `%` signs, eg `i"myList = (${myList}%, %)"` 3. Safe handling of
    *      multi-line margins. Left margins are stripped on the parts of the
    *      string context *before* inserting the arguments. That way, we guard
  ...
  class StringFormatter(protected val sc: StringContext):
    ...
    private def treatArg(arg: Shown, suffix: String)(using
        Context
    ): (String, String) = arg.ctxShow match
      case arg: Seq[?]
          if suffix.indexOf('%') == 0 && suffix.indexOf('%', 1) != -1 =>
    ...
    def assemble(args: Seq[Shown])(using Context): String =
      ...
      new StringContext(prefix :: suffixes1.toList: _*).s(args1*)
  end StringFormatter

  /** This method will produce a colored type diff from the given arguments. The
    * idea is to do this for known cases that are useful and then fall back on
    * regular syntax highlighting for the cases which are unhandled.
    *
    ...
    *
    * @return
    *   the (found, expected, changePercentage) with coloring to highlight the
    *   difference
    */
  def typeDiff(found: Type, expected: Type)(using Context): (String, String) =
    ...
    DiffUtil.mkColoredTypeDiff(fnd, exp) match
      case (fnd1, exp1, change)
          if change < 0.5 && ctx.settings.color.value != "never" =>
        (fnd1, exp1)
      case _ => (fnd, exp)
  ...
  /** Explicitly highlight a string with the same formatting as used for
    * keywords
    */
