  ...
  def highlight(in: String)(using Context): String =
    ...
    if in.isEmpty || ctx.settings.color.value == "never" then in
      ...
      given Context = freshCtx
        .setCompilationUnit(
          CompilationUnit(source, mustExist = false)(using freshCtx)
        )
      ...
      def highlightPosition(span: Span, color: String) = if span.exists then
        if span.start < 0 || span.end > in.length then
          if debug then
            println(
              s"Trying to highlight erroneous position $span. Input size: ${in.length}"
            )
        else highlightRange(span.start, span.end, color)
      ...
      for span <- scanner.commentSpans do highlightPosition(span, CommentColor)
      ...
      object TreeHighlighter extends untpd.UntypedTreeTraverser:
        ...
        def highlightAnnotations(tree: MemberDef): Unit =
          for annotation <- tree.rawMods.annotations do
      ...
      for idx <- colorAt.indices do
        val prev = if idx == 0 then NoColor else colorAt(idx - 1)
        val curr = colorAt(idx)
        if curr != prev then highlighted.append(curr)
