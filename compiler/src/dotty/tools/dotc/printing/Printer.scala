...
import Types.{Type, SingletonType, LambdaParam}, Symbols.Symbol, Scopes.Scope,
  Constants.Constant, Names.Name, Denotations.*, Annotations.Annotation,
  Contexts.Context
...
abstract class Printer:
  ...
  /** The current precedence level. When pretty-printing arguments of operator
    * `op`, `currentPrecedence` must equal `op`'s precedence level, so that
    * pretty-printing expressions using lower-precedence operators can insert
    * parentheses automatically by calling `changePrec`.
    */
  ...
  /** Generate text using `op`, assuming a given precedence level `prec`.
    ...
    *
    * This is to be used when changing precedence inside some sort of
    * parentheses: for instance, to print `T[A]` use `toText(T) ~ '[' ~
    * atPrec(GlobalPrec) { toText(A) } ~ ']'`.
    *
    * If the presence of the parentheses depends on precedence, inserting them
    * manually is most certainly a bug. Use `changePrec` instead to generate
    * them exactly when needed.
    */
  ...
  /** Generate text using `op`, assuming a given precedence level `prec`. If new
    * level `prec` is lower than previous level, put text in parentheses.
    *
    * ### `atPrec` vs `changePrec`
    *
    * To pretty-print `A op B`, you need something like
    * `changePrec(parsing.precedence(op, isType)) { toText(a) ~ op ~ toText(b)
    * }` // BUGGY that will insert parentheses around `A op B` if, for instance,
    * the preceding operator has higher precedence.
    *
    * But that does not handle infix operators with left- or right-
    * associativity.
    *
    * If op and op' have the same precedence and associativity, A op B op' C
    * parses as (A op B) op' C if op and op' are left-associative, and as A op
    * (B op' C) if they're right-associative, so we need respectively
    * ```scala
    * val prec = parsing.precedence(op)
    * // either:
    * changePrec(prec) {
    *   toText(a) ~ op ~ atPrec(prec + 1) { toText(b) }
    * } // for left-associative op and op'
    * // or:
    * changePrec(prec) {
    *   atPrec(prec + 1) { toText(a) } ~ op ~ toText(b)
    * } // for right-associative op and op'
    * ```
    */
  def changePrec(prec: Precedence)(op: => Text): Text =
    if prec < this.prec then atPrec(prec)("(" ~ op ~ ")") else atPrec(prec)(op)

  /** The name, possibly with with namespace suffix if debugNames is set: /L for
    * local names, /V for other term names, /T for type names
    */
  def nameString(name: Name): String

  /** The name of the given symbol. If !settings.debug, the original name where
    * expansions of operators are translated back to operator symbol. E.g. $eq
  ...
  /** Textual representation, including symbol's kind e.g., "class Foo", "method
    * Bar". If hasMeaninglessName is true, uses the owner's name to disambiguate
    * identity.
    */
  ...
  /** Textual representation of all symbols in given list, using `dclText` for
    * displaying each.
    */
  def dclsText(syms: List[Symbol], sep: String = "\n"): Text

  /** Textual representation of all definitions in a scope using `dclText` for
    * each
    */
...
object Printer:
  ...
  /** Debug hook; set to true if you want to see unique ids but cannot run with
    * option
    * -uniqid. A typical use case is for further exploration after a
    * -Ytest-pickler failure.
    */
