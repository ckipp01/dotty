...
object NameOps:
  ...
  object compactify:
    ...
    def apply(s: String): String =
      ...
      if s.length <= MaxNameLength then s else toMD5(s, MaxNameLength / 4)
    end apply
  end compactify
  ...
  extension [N <: Name](name: N)
    ...
    def isConstructorName: Boolean =
      name == CONSTRUCTOR || name == TRAIT_CONSTRUCTOR
    ...
    def isReplWrapperName: Boolean =
      name.toString contains str.REPL_SESSION_LINE
    def isReplAssignName: Boolean =
      name.toString contains str.REPL_ASSIGN_SUFFIX
    def isSetterName: Boolean =
      name.endsWith(str.SETTER_SUFFIX) || name.is(SyntheticSetterName)
    def isScala2LocalSuffix: Boolean = testSimple(_.endsWith(" "))
    def isSelectorName: Boolean =
      testSimple(n => n.startsWith("_") && n.drop(1).forall(_.isDigit))
        ...
        n.length > 0 && {
          def isLowerLetterSupplementary: Boolean =
            ...
            isHighSurrogate(n(0)) && n.length > 1 && isLowSurrogate(n(1)) && {
              ...
              isValidCodePoint(codepoint) && isLetter(codepoint) && isLowerCase(
                codepoint
              )
            }
          val first = n.head
    ...
    def isOpAssignmentName: Boolean = name match
      ...
      case name: SimpleName =>
        name.length > 0 && name.last == '=' && name.head != '=' && isOperatorPart(
          name.firstCodePoint
        )
      case _ =>
        false
    ...
    def isPackageObjectName: Boolean = name match
      case name: TermName =>
        name == nme.PACKAGE || name.endsWith(str.TOPLEVEL_SUFFIX)
      case name: TypeName =>
    ...
    /** If name ends in module class suffix, drop it. This method needs to work
      * on mangled as well as unmangled names because it is also called from the
      * backend.
      */
    ...
    def adjustIfModuleClass(flags: FlagSet): N = likeSpacedN {
      if flags.is(ModuleClass, butNot = Package) then
        name.asTypeName.moduleClassName
      else name.toTermName
    }

    /** The expanded name. This is the fully qualified name of `base` with
      * `ExpandPrefixName` as separator, followed by `kind` and the name.
      */
    def expandedName(base: Symbol, kind: QualifiedNameKind = ExpandedName)(using
        Context
    ): N =
    ...
    def unexpandedName: N = likeSpacedN {
      name.replaceDeep { case ExpandedName(_, unexp) =>
        unexp
      }
    }
    ...
    /** The arity of a name ending in the suffix `Function{\d}`, but -1 if the
      * number is larger than Int.MaxValue / 10.
      * @param suffixStart
    ...
    private def isFunctionPrefix(suffixStart: Int, mustHave: String = "")(using
        Context
    ): Boolean =
      suffixStart >= 0
        && {
    ...
    /** Same as `funArity`, except that it returns -1 if the prefix is not one
      * of a (possibly empty) concatenation of a subset of "Impure" (only under
      * pureFunctions), "Erased" and "Context" (in that order).
      */
    ...
    /** Is a function name, i.e one of FunctionXXL, FunctionN, ContextFunctionN,
      * ImpureFunctionN, ImpureContextFunctionN for N >= 0
      */
    ...
    def isContextFunction(using Context): Boolean = isSpecificFunction(
      "Context"
    )
    ...
    def isSyntheticFunction(using Context): Boolean =
      ...
      if suffixStart == 0 then
        funArity(suffixStart) > MaxImplementedFunctionArity
    ...
    /** The name of the generic runtime operation corresponding to an array
      * operation
      */
    ...
    /** The name of the primitive runtime operation corresponding to an array
      * operation
      */
    ...
    /** This method is to be used on **type parameters** from a class, since
      ...
      */
    def specializedFor(
        classTargs: List[Type],
        classTargsNames: List[Name],
        methodTargs: List[Type],
        methodTarsNames: List[Name]
    )(using Context): N =
      val methodTags: Seq[Name] = (methodTargs zip methodTarsNames)
        .sortBy(_._2)
        .map(x => defn.typeTag(x._1))
      val classTags: Seq[Name] = (classTargs zip classTargsNames)
        .sortBy(_._2)
        .map(x => defn.typeTag(x._1))

      likeSpacedN(
        name ++ nme.specializedTypeNames.prefix ++
          methodTags.fold(nme.EMPTY)(
            _ ++ _
          ) ++ nme.specializedTypeNames.separator ++
          classTags.fold(nme.EMPTY)(_ ++ _) ++ nme.specializedTypeNames.suffix
      )

    /** Determines if the current name is the specialized name of the given base
      * name. For example
      * `typeName("Tuple2$mcII$sp").isSpecializedNameOf(tpnme.Tuple2) == true`
      */
    def isSpecializedNameOf(base: N)(using Context): Boolean =
      var i = 0
      inline def nextString(str: String) = name.startsWith(str, i) && {
        i += str.length; true
      }
    ...
    /** Returns the name of the class specialised to the provided types, in the
      * given order. Used for the specialized tuple classes.
      */
    ...
    def compactified(using Context): TermName = termName(
      compactify(name.toString)
    )

    def unmangleClassName: N = name.toTermName match
      case name: SimpleName
          if name.endsWith(str.MODULE_SUFFIX) && !nme.falseModuleClassNames
            .contains(name) =>
        ...
        case _ =>
          name replace {
            ...
            case ExpandedName(prefix, last) =>
              kind.unmangle(last) replace { case kernel: SimpleName =>
    ...
    def unmangle(kinds: List[NameKind]): N =
      ...
      if unmangled eq name then name else unmangled.unmangle(kinds)
    ...
    def firstCodePoint: Int =
      ...
      if isHighSurrogate(first(0)) && first.length > 1 && isLowSurrogate(
          first(1)
        )
      then
  ...
  extension (name: TermName)
    ...
    def getterName: TermName =
      ...
      if name1.is(SyntheticSetterName) then name1.exclude(SyntheticSetterName)
        ...
        name1.mapLast(n =>
          if n.endsWith(str.SETTER_SUFFIX) then
            n.take(n.length - str.SETTER_SUFFIX.length).asSimpleName
          else n
        )

    def fieldName: TermName =
      if name.isSetterName then
        if name.is(SyntheticSetterName) then
          name
            .exclude(SyntheticSetterName)
    ...
    def stripScala2LocalSuffix: TermName =
      if name.isScala2LocalSuffix then name.asSimpleName.dropRight(1) else name
