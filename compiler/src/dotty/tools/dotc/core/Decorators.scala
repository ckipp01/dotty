...
object Decorators:
  ...
  extension (s: String)
    def splitWhere(
      ...
      def splitAt(idx: Int, doDropIndex: Boolean): Option[(String, String)] =
        if idx == -1 then None
        else Some((s.take(idx), s.drop(if doDropIndex then idx + 1 else idx)))
      splitAt(s.indexWhere(f), doDropIndex)

    /** Create a term name from a string slice, using a common buffer. This
      * avoids some allocation relative to `termName(s)`
      */
  ...
  /** Convert lazy string to message. To be with caution, since no
    * message-defined formatting will be done on the string.
    */
  extension (str: => String)
    def toMessage: Message = NoExplanation(str)(using NoContext)

  /** Implements a findSymbol method on iterators of Symbols that works like
    * find but avoids Option, replacing None with NoSymbol.
    */
  extension (it: Iterator[Symbol])
    final def findSymbol(p: Symbol => Boolean): Symbol =
      while it.hasNext do
        val sym = it.next()
        if p(sym) then return sym
      NoSymbol

  inline val MaxFilterRecursions = 10

  /** Implements filterConserve, zipWithConserve methods on lists that avoid
    * duplication of list nodes where feasible.
    */
  extension [T](xs: List[T])
    ...
    def filterConserve(p: T => Boolean): List[T] =
      ...
      def addAll(
        ...
        if from eq until then buf
        else addAll(buf += from.head, from.tail, until)
      ...
      def loop(
        ...
        explore match
          case x :: rest =>
            if p(x) then loop(keep, rest, keepCount + 1, recCount)
              ...
              keepCount match
                ...
                case 3 =>
                  val tl = keep.tail;
                  keep.head :: tl.head :: tl.tail.head :: rest1
            else loopWithBuffer(addAll(new ListBuffer[T], keep, explore), rest)
          case nil =>
            keep
    ...
    /** Like `xs.lazyZip(ys).map(f)`, but returns list `xs` itself
      ...
      *     themselves. Also, it is required that `ys` is at least as long as
      *     `xs`.
      */
    ...
    def mapWithIndexConserve[U <: T](f: (T, Int) => U): List[U] =
      ...
      def loopWithBuffer(
          ...
          explore: List[T],
          idx: Int
      ): List[U] = explore match
        case Nil       => buf.toList
        case t :: rest => loopWithBuffer(buf += f(t, idx), rest, idx + 1)

      @tailrec
      def loop(keep: List[T], explore: List[T], idx: Int): List[U] =
        explore match
          ...
          case t :: rest =>
            ...
            if u.asInstanceOf[AnyRef] eq t.asInstanceOf[AnyRef] then
              ...
              val buf = addAll(new ListBuffer[T], keep, explore)
                .asInstanceOf[ListBuffer[U]]
    ...
    /** True if two lists have the same length. Since calling length on linear
      * sequences is Θ(n), it is an inadvisable way to test length equality.
      * This method is Θ(n min m).
      */
    final def hasSameLengthAs[U](ys: List[U]): Boolean =
      @tailrec def loop(xs: List[T], ys: List[U]): Boolean =
        if xs.isEmpty then ys.isEmpty
    ...
    /** Reduce left with `op` as long as list `xs` is not longer than
      * `seqLimit`. Otherwise, split list in two half, reduce each, and combine
      * with `op`.
      */
    def reduceBalanced(op: (T, T) => T, seqLimit: Int = 100): T =
      ...
      if len > seqLimit then
        ...
        op(
          leading.reduceBalanced(op, seqLimit),
          trailing.reduceBalanced(op, seqLimit)
        )
      else xs.reduceLeft(op)
  end extension
  ...
  extension [T, U](xss: List[List[T]])
    ...
    def nestedZipWithConserve(yss: List[List[U]])(
        f: (T, U) => T
    ): List[List[T]] =
  ...
  /** Test whether a list of strings representing phases contains a given phase.
  ...
  extension (names: List[String])
    def containsPhase(phase: Phase): Boolean =
      names.nonEmpty && {
        phase match
          case phase: MegaPhase =>
            phase.miniPhases.exists(x => names.containsPhase(x))
          case _ =>
            names exists { name =>
              name == "all" || {
  ...
  extension [T](x: T)
    def showing[U](
        ...
        printer: config.Printers.Printer = config.Printers.default
    )(using c: Conversion[T, U] | Null = null): T =
    ...
    /** Instead of `toString` call `show` on `Showable` values, falling back to
      * `toString` if an exception is raised.
      */
  ...
  extension (sc: StringContext)
    ...
    /** Interpolator yielding an error message, which undergoes the formatting
      * defined in Message.
      */
  ...
  extension [T <: AnyRef](arr: Array[T])
    def binarySearch(x: T | Null): Int =
      java.util.Arrays.binarySearch(arr.asInstanceOf[Array[Object | Null]], x)
end Decorators
