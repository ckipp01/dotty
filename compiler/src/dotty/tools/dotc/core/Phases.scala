...
object Phases:
  ...
  trait PhasesBase:
    ...
    def allPhases: Array[Phase] =
      (if fusedPhases.nonEmpty then fusedPhases else phases).tail
    ...
    final def setPhasePlan(phasess: List[List[Phase]]): Unit = this.phasesPlan =
      phasess

    /** Squash TreeTransform's beloning to same sublist to a single
      * TreeTransformer Each TreeTransform gets own period, whereas a combined
      * TreeTransformer gets period equal to union of periods of it's
      * TreeTransforms
      */
    final def fusePhases(
      ...
      val filteredPhases = phasess.map(_.filter { p =>
        try isEnabled(p)
          ...
          stop |= stopBeforePhases.contains(p.phaseName) | stopAfterPhases
            .contains(p.phaseName)
      })

      var i = 0

      while i < filteredPhases.length do
        if filteredPhases(i).nonEmpty then // could be empty due to filtering
          val filteredPhaseBlock = filteredPhases(i)
          val phaseToAdd =
            if filteredPhaseBlock.length > 1 then
              for phase <- filteredPhaseBlock do
                phase match
                  case p: MiniPhase =>
                  ...
                  case _ =>
                    assert(
                      false,
                      s"Only tree transforms can be fused, ${phase.phaseName} can not be fused"
                    )
              val superPhase = new MegaPhase(
                filteredPhaseBlock.asInstanceOf[List[MiniPhase]].toArray
              )
          ...
          val shouldAddYCheck = filteredPhases(i).exists(
            _.isCheckable
          ) && YCheckAfter.containsPhase(phaseToAdd)
          if shouldAddYCheck then
    ...
    /** Use the following phases in the order they are given. The list should
      * never contain NoPhase. if fusion is enabled, phases in same subgroup
      * will be fused to single phase.
      */
    final def usePhases(phasess: List[Phase], fuse: Boolean = true): Unit =
      ...
      phases =
        (NoPhase :: flatPhases.toList ::: new TerminalPhase :: Nil).toArray
      setSpecificPhases()
      ...
      def checkRequirements(p: Phase) =
        ...
        assert(
          unmetPrecedeRequirements.isEmpty,
      ...
      if fuse then this.fusedPhases = (NoPhase :: phasess).toArray
      else this.fusedPhases = this.phases

      config.println(s"Phases = ${phases.toList}")
      config.println(
        s"nextDenotTransformerId = ${nextDenotTransformerId.toList}"
      )
    end usePhases

    /** Unlink `phase` from Denot transformer chain. This means that any
      * denotation transformer defined by the phase will not be executed.
      */
    ...
    private def setSpecificPhases() =
      def phaseOfClass(pclass: Class[?]) =
        phases.find(pclass.isInstance).getOrElse(NoPhase)
      ...
      mySbtExtractDependenciesPhase = phaseOfClass(
        classOf[sbt.ExtractDependencies]
      )
      ...
      myCollectNullableFieldsPhase = phaseOfClass(
        classOf[CollectNullableFields]
      )
  ...
  abstract class Phase:
    ...
    /** If set, allow missing or superfluous arguments in applications and type
      * applications.
      */
    ...
    /** @pre `isRunnable` returns true */
    def runOn(units: List[CompilationUnit])(using
        Context
    ): List[CompilationUnit] =
      units.map { unit =>
        val unitCtx = ctx.fresh
          .setPhase(this.start)
        ...
        try run(using unitCtx)
          ...
          case ex: Throwable if !ctx.run.enrichedErrorMessage =>
            println(
              ctx.run.enrichErrorMessage(
                s"unhandled exception while running $phaseName on $unit"
              )
            )
            throw ex
    ...
    /** Check what the phase achieves, to be called at any point after it is
      * finished.
      */
    def checkPostCondition(tree: tpd.Tree)(using Context): Unit = ()

    /** Is this phase the standard typerphase? True for TyperPhase, but not for
      * other first phases (such as FromTasty or Parser). The predicate is
      * tested in some places that perform checks and corrections. It's
    ...
    /** The sequence position of this phase in the given context where 0 is
      * reserved for NoPhase and the first real phase is at position 1.
    ...
    final def lambdaLifted: Boolean =
      myLambdaLifted // Phase is after LambdaLift
    final def patternTranslated: Boolean =
      myPatternTranslated // Phase is after PatternMatcher
    ...
    protected[Phases] def init(base: ContextBase, start: Int, end: Int): Unit =
      if start >= FirstPhaseId then
        assert(
          myPeriod == Periods.InvalidPeriod,
          s"phase $this has already been used once; cannot be reused"
        )
      assert(
        start <= Periods.MaxPossiblePhaseId,
        s"Too many phases, Period bits overflow"
      )
      myBase = base
      ...
      myPatternTranslated =
        prev.getClass == classOf[PatternMatcher] || prev.patternTranslated
      mySameMembersStartId =
        if changesMembers then id else prev.sameMembersStartId
      mySameParentsStartId =
        if changesParents then id else prev.sameParentsStartId
      mySameBaseTypesStartId =
        if changesBaseTypes then id else prev.sameBaseTypesStartId
    ...
    final def prev: Phase =
      if id > FirstPhaseId then myBase.phases(start - 1) else NoPhase

    final def prevMega(using Context): Phase =
      ctx.base.fusedContaining(ctx.phase.prev)

    final def next: Phase =
      if hasNext then myBase.phases(end + 1) else NoPhase

    final def hasNext: Boolean =
      start >= FirstPhaseId && end + 1 < myBase.phases.length
  ...
  def sbtExtractDependenciesPhase(using Context): Phase =
    ctx.base.sbtExtractDependenciesPhase
  ...
  def extensionMethodsPhase(using Context): Phase =
    ctx.base.extensionMethodsPhase
  ...
  def elimErasedValueTypePhase(using Context): Phase =
    ctx.base.elimErasedValueTypePhase
  ...
  /** Replace all instances of `oldPhaseClass` in `current` phases by the result
    * of `newPhases` applied to the old phase.
    */
  private def replace(
      oldPhaseClass: Class[? <: Phase],
      newPhases: Phase => List[Phase],
      current: List[List[Phase]]
  ): List[List[Phase]] =
    current.map(
      _.flatMap(phase =>
        if oldPhaseClass.isInstance(phase) then newPhases(phase)
        else phase :: Nil
      )
    )
end Phases
