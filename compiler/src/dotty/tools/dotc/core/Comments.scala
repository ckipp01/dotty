...
object Comments:
  ...
  class ContextDocstrings:
    ...
    private val _docstrings: MutableSymbolMap[Comment] =
      MutableSymbolMap[Comment](
        512
      ) // FIXME: 2nd [Comment] needed or "not a class type"
  ...
  /** A `Comment` contains the unformatted docstring, it's position and
    * potentially more information that is populated when the comment is
    * "cooked".
    *
    * @param span
  ...
  final case class Comment(
    ...
    /** Expands this comment by giving its content to `f`, and then parsing the
      * `@usecase` sections. Typically, `f` will take care of expanding the
      * variables.
      *
      * @param f
  ...
  object Comment:
    ...
    private def parseUsecases(expandedComment: String, span: Span)(using
        Context
    ): List[UseCase] =
      if !isDocComment(expandedComment) then Nil
      else
        tagIndex(expandedComment)
          .filter { startsWithTag(expandedComment, _, "@usecase") }
          .map { case (start, end) =>
            decomposeUseCase(expandedComment, span, start, end)
          }
  ...
  final case class UseCase(
      code: String,
      codePos: Span,
      untpdCode: untpd.Tree,
      tpdCode: Option[tpd.DefDef]
  ):
  ...
  object UseCase:
    def apply(code: String, codePos: Span)(using Context): UseCase =
      val tree =
        val tree = new Parser(SourceFile.virtual("<usecase>", code))
          .localDef(codePos.start)
        tree match
          case tree: untpd.DefDef =>
            val newName = ctx.compilationUnit.freshNames
              .newName(tree.name, NameKinds.DocArtifactName)
  ...
  class CommentExpander:
    ...
    /** The cooked doc comment of symbol `sym` after variable expansion, or ""
      * if missing.
      *
      * @param sym
      ...
      * @throws ExpansionLimitExceeded
      *   when more than 10 successive expansions of the same string are done,
      *   which is interpreted as a recursive variable definition.
      */
    def expandedDocComment(sym: Symbol, site: Symbol, docStr: String = "")(using
      ...
      val parent =
        if (sym.is(Flags.Module) || sym.isClass) && site.is(Flags.Package) then
          sym
        else site
    ...
    def cookedDocComment(sym: Symbol, docStr: String = "")(using
        Context
    ): String = cookedDocComments.getOrElseUpdate(
      sym, {
        var ownComment =
          if docStr.length == 0 then
            ctx.docCtx
              .flatMap(_.docstring(sym).map(c => template(c.raw)))
              .getOrElse("")
        ...
        superComment(sym) match
          case None =>
            ...
            if ownComment.indexOf("@inheritdoc") != -1 && !sym.isSetter then
              scaladoc.println(
                s"${sym.span}: the comment for ${sym} contains @inheritdoc, but no parent comment is available to inherit from."
              )
            ownComment.replace("@inheritdoc", "<invalid inheritdoc annotation>")
          case Some(sc) =>
            if ownComment == "" then sc
    ...
    /** Expand inheritdoc tags
      ...
      *   - for the param, tparam and throws sections we must replace comments
      *     on the spot
      *
      * This is done separately, for two reasons:
      *   1. It takes longer to run compared to merge 2. The inheritdoc
      *      annotation should not be used very often, as building the comment
      *      from pieces severely impacts performance
      *
      * @param parent
    ...
    def expandInheritdoc(parent: String, child: String, sym: Symbol): String =
      if child.indexOf("@inheritdoc") == -1 then child
        ...
        def replaceInheritdoc(childSection: String, parentSection: => String) =
          if childSection.indexOf("@inheritdoc") == -1 then childSection
          else childSection.replace("@inheritdoc", parentSection)
        ...
        def getParentSection(section: (Int, Int)): String =
          ...
          def sectionString(
            ...
            paramMap.get(param) match
              ...
              case None =>
                scaladoc.println(
                  s"""${sym.span}: the """" + getSectionHeader + "\" annotation of the " + sym +
    ...
    protected def expandVariables(
      ...
      def expandInternal(str: String, depth: Int): String =
        if depth >= expandLimit then throw new ExpansionLimitExceeded(str)
            ...
            variableName(str.substring(vstart + 1, idx)) match
              case "super" =>
                superComment(sym) foreach { sc =>
                  ...
                  for sec @ (start, end) <- superSections do
                    if !isMovable(sc, sec) then
    ...
    /** Maps symbols to the variable -> replacement maps that are defined in
      * their doc comments
      */
    private val defs =
    ...
    def lookupVariable(vble: String, site: Symbol)(using
        Context
    ): Option[String] = site match
      case NoSymbol => None
      case _ =>
        val searchList =
          if site.flags.is(Flags.Module) then site :: site.info.baseClasses
