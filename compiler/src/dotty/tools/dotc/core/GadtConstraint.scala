...
class GadtConstraint private (
  ...
  /** Full bounds of `sym`, including TypeRefs to other lower/upper symbols.
    *
    * @note
    *   this performs subtype checks between ordered symbols. Using this in
    *   isSubType can lead to infinite recursion. Consider `bounds` instead.
    */
  def fullBounds(sym: Symbol)(using Context): TypeBounds | Null = mapping(
    sym
  ) match
    case null => null
    case tv: TypeVar =>
      fullBounds(tv.origin) // .ensuring(containsNoInternalTypes(_))

  /** Immediate bounds of `sym`. Does not contain lower/upper symbols (see
    * [[fullBounds]]).
    */
  def bounds(sym: Symbol)(using Context): TypeBounds | Null =
    mapping(sym) match
      case null => null
      case tv: TypeVar =>
        def retrieveBounds: TypeBounds = externalize(
          constraint.bounds(tv.origin)
        ).bounds
        retrieveBounds
  ...
  /** Is the symbol registered in the constraint?
    *
    * @note
    *   this is true even if the symbol is constrained to be equal to another
    *   type, unlike [[Constraint.contains]].
    */
  ...
  def fullLowerBound(param: TypeParamRef)(using Context): Type =
    ...
    constraint.minLower(param).foldLeft(nonParamBounds(param).lo) { (acc, p) =>
      externalize(p) match
        ...
        case tp: TypeRef
            if tp.symbol.isPatternBound && self =:= tp.info.hiBound =>
          acc
        case tp => acc | tp
    }
  ...
  def fullUpperBound(param: TypeParamRef)(using Context): Type =
    ...
    constraint.minUpper(param).foldLeft(nonParamBounds(param).hi) { (acc, u) =>
      externalize(u) match
        case tp: TypeRef
            if tp.symbol.isPatternBound && self =:= tp.info.loBound =>
          acc // like fullLowerBound
        case tp =>
  ...
  def externalize(tp: Type, theMap: TypeMap | Null = null)(using
      Context
  ): Type = tp match
  ...
  def tvarOrError(sym: Symbol)(using Context): TypeVar =
    mapping(sym)
      .ensuring(_ != null, i"not a constrainable symbol: $sym")
      .uncheckedNN
  ...
  private def containsNoInternalTypes(
      tp: Type,
      theAcc: TypeAccumulator[Boolean] | Null = null
    ...
    case tp =>
      (if theAcc != null then theAcc
       else new ContainsNoInternalTypesAccumulator()).foldOver(true, tp)

  private class ContainsNoInternalTypesAccumulator(using Context)
      extends TypeAccumulator[Boolean]:
    override def apply(x: Boolean, tp: Type): Boolean =
      x && containsNoInternalTypes(tp, this)

  override def toText(printer: Printer): Texts.Text = printer.toText(this)

  /** Provides more information than toText, by showing the underlying
    * Constraint details.
    */
  ...
  private def copy(
    ...
    GadtConstraint(myConstraint, mapping, reverseMapping, wasConstrained)
end GadtConstraint
...
sealed trait GadtState:
  ...
  override protected def legalBound(
      param: TypeParamRef,
      rawBound: Type,
      isUpper: Boolean
  )(using Context): Type =
  ...
  /** Add symbols to constraint, correctly handling inter-dependencies.
    *
    * @see
    *   [[ConstraintHandling.addToConstraint]]
    */
  def addToConstraint(sym: Symbol)(using Context): Boolean = addToConstraint(
    sym :: Nil
  )
  def addToConstraint(params: List[Symbol])(using Context): Boolean =
    import NameKinds.DepParamName

    val poly1 = PolyType(params.map { sym =>
      DepParamName.fresh(sym.name.toTypeName)
    })(
      pt =>
        params.map { param =>
          ...
          def substDependentSyms(tp: Type, isUpper: Boolean)(using
              Context
          ): Type =
  ...
  def addBound(sym: Symbol, bound: Type, isUpper: Boolean)(using
    ...
    val symTvar: TypeVar =
      gadt.stripInternalTypeVar(gadt.tvarOrError(sym)) match
        case tv: TypeVar => tv
        case inst =>
  ...
  def approximation(
    ...
    approximation(gadt.tvarOrError(sym).origin, fromBelow, maxLevel)
      .match
  ...
  override protected def constraint_=(c: Constraint) = gadt =
    gadt.withConstraint(c)
...
private class ProperGadtState(private var myGadt: GadtConstraint)
