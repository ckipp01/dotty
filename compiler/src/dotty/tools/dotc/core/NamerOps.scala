...
import Contexts.*, Symbols.*, Types.*, Flags.*, Scopes.*, Decorators.*, Names.*,
  NameOps.*
...
object NamerOps:
  ...
  /** The type of the constructed instance is returned
    ...
    *   the constructor
    */
  def effectiveResultType(ctor: Symbol, paramss: List[List[Symbol]])(using
      Context
  ): Type =
    paramss match
  ...
  /** If isConstructor, make sure it has at least one non-implicit parameter
    * list This is done by adding a () in front of a leading old style implicit
    * parameter, or by adding a () as last -- or only -- parameter list if the
    * constructor has only using clauses as parameters.
    */
  def normalizeIfConstructor(
      paramss: List[List[Symbol]],
      isConstructor: Boolean
  )(using Context): List[List[Symbol]] =
    if !isConstructor then paramss
    else
      paramss match
        case TypeSymbols(tparams) :: paramss1 =>
          tparams :: normalizeIfConstructor(paramss1, isConstructor)
        case TermSymbols(vparam :: _) :: _ if vparam.is(Implicit) =>
          Nil :: paramss
        case _ =>
          if paramss.forall {
              case TermSymbols(vparams) =>
                vparams.nonEmpty && vparams.head.is(Given)
              case _ => true
            }
          then paramss :+ Nil
          else paramss
  ...
  def methodType(
    ...
    def recur(paramss: List[List[Symbol]]): Type = (paramss: @unchecked) match
      ...
      case TermSymbols(params) :: paramss1 =>
        ...
        val make = MethodType.companion(
          isContextual = isContextual,
          isImplicit = isImplicit
        )
        if isJava then
          for param <- params do
            if param.info.isDirectRef(defn.ObjectClass) then
              param.info = defn.AnyType
  ...
  /** Add moduleClass or sourceModule functionality to completer for a module or
    * module class
    */
  def adjustModuleCompleter(completer: LazyType, name: Name)(using
      Context
  ): LazyType =
  ...
  def findModuleBuddy(
      name: Name,
      scope: Scope,
      alternate: Name = EmptyTermName
  )(using Context): Symbol =
    var it = scope.lookupAll(name).filter(_.is(Module))
    if !alternate.isEmpty then
      it ++= scope.lookupAll(alternate).filter(_.is(Module))
  ...
  /** If a class has one of these flags, it does not get a constructor companion
    */
  private val NoConstructorProxyNeededFlags =
    Abstract | Trait | Case | Synthetic | Module | Invisible
  ...
  /** If this is a reference to a class and the reference has a stable prefix,
    * the reference otherwise NoType
    */
  private def underlyingStableClassRef(tp: Type)(using
      Context
  ): TypeRef | NoType.type = tp match
  ...
  /** Add constructor proxy apply methods to `scope`. Proxies are for
    * constructors in `cls` and they reside in `modcls`.
    */
  def addConstructorApplies(
      scope: MutableScope,
      cls: ClassSymbol,
      modcls: ClassSymbol
  ...
  /** The completer of a constructor companion for class `cls`, where `modul` is
    * the companion symbol and `modcls` is its class.
    */
  def constructorCompanionCompleter(
      cls: ClassSymbol
    ...
    new LazyType with SymbolLoaders.SecondCompleter:
      def complete(denot: SymDenotation)(using Context): Unit =
        ...
        denot.info = ClassInfo(
          ...
          modcls,
          defn.AnyType :: Nil,
          addConstructorApplies(newScope, cls, modcls),
          TermRef(prefix, modul)
        )
  ...
  def typeConstructorCompanion(tsym: Symbol, prefix: Type, proxy: Symbol)(using
      Context
  ): TermSymbol =
    newSymbol(
      tsym.owner,
      tsym.name.toTermName,
      ConstructorCompanionFlags | StableRealizable | Method,
      ExprType(prefix.select(proxy)),
      coord = tsym.coord
    )

  /** Add all necesssary constructor proxy symbols for members of class `cls`.
    * This means:
    *
    *   - if a member is a class, or type alias, that needs a constructor
    *     companion, add one, provided no member with the same name exists.
    *   - if `cls` is a companion object of a class that needs a constructor
    *     companion, and `cls` does not already define or inherit an `apply`
    *     method, add `apply` methods for all constructors of the companion
    *     class.
    */
      ...
      if needsConstructorProxies(mbr) then
        mbr match
          case mbr: ClassSymbol =>
            ...
            then classConstructorCompanion(mbr).entered
          case _ =>
            underlyingStableClassRef(mbr.info.loBound): @unchecked
        match
          case ref: TypeRef =>
            val proxy = ref.symbol.registeredCompanion
            if proxy
                .is(ConstructorProxy) && !memberExists(cls, mbr.name.toTermName)
            then typeConstructorCompanion(mbr, ref.prefix, proxy).entered

    if cls.is(Module)
      && needsConstructorProxies(cls.linkedClass)
      && !memberExists(cls, nme.apply)
    then
      addConstructorApplies(
        cls.info.decls.openForMutations,
        cls.linkedClass.asClass,
        cls
      )
  end addConstructorProxies

  /** Turn `modul` into a constructor companion for class `cls` */
  def makeConstructorCompanion(modul: TermSymbol, cls: ClassSymbol)(using
      Context
  ): Unit =
  ...
  /** For secondary constructors, make it known in the context that their type
    * parameters are aliases of the class type parameters.
    * @return
    *   if `sym` is a secondary constructor, a fresh context that contains GADT
    *   constraints linking the type parameters.
    */
  ...
  /** For secondary constructor `sym`, make it known in the given context
    * `rhsCtx` that their type parameters are aliases of the class type
    * parameters. This is done by (ab?)-using GADT constraints. See
    * pos/i941.scala.
    */
  def linkConstructorParams(
      sym: Symbol,
      tparams: List[Symbol],
      rhsCtx: Context
  )(using Context): Unit =
