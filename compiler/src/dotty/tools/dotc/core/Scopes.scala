...
object Scopes:
  ...
  /** A hashtable is created once current size exceeds MinHash * FillFactor The
    * initial hash table has twice that size (i.e 16). This value must be a
    * power of two, so that the index of an element can be computed as
    * element.hashCode & (hashTable.length - 1)
    */
  ...
  /** A function that optionally produces synthesized symbols with the given
    * name in the given context. Returns `NoSymbol` if the no symbol should be
    * synthesized for the given name.
    */
  type SymbolSynthesizer = Name => Context ?=> Symbol

  class ScopeEntry private[Scopes] (
      val name: Name,
      _sym: Symbol,
      val owner: Scope
  ):
  ...
  /** A scope contains a set of symbols. It can be an extension of some outer
    * scope, from which it inherits all symbols. This class does not have any
    * methods to add symbols to a scope or to delete them. These methods are
    * provided by subclass MutableScope.
    */
  abstract class Scope extends printing.Showable:
    ...
    /** The number of symbols in this scope (including inherited ones from outer
      * scopes).
      */
    def size: Int
    ...
    /** The symbols in this scope in the order they were entered; inherited from
      * outer ones first.
      */
    def toList(using Context): List[Symbol]

    /** Return all symbols as an iterator in the order they were entered in this
      * scope.
      */
    ...
    def filter(p: Symbol => Boolean)(using Context): List[Symbol] =
      ...
      while (e != null) && e.owner == this do
        val sym = e.sym
        if p(sym) then syms = sym :: syms
        e = e.prev
      syms

    /** Tests whether a predicate holds for at least one Symbol of this Scope.
      */
    def exists(p: Symbol => Boolean)(using Context): Boolean = filter(
      p
    ).nonEmpty
    ...
    final def lookup(name: Name)(using Context): Symbol =
      ...
      if e == null then NoSymbol else e.sym
    ...
    final def lookupAll(name: Name)(using Context): Iterator[Symbol] =
      new Iterator[Symbol]:
        ...
        def next(): Symbol =
          val r = e.nn.sym; e = lookupNextEntry(e.uncheckedNN); r
    ...
    /** The denotation set of all the symbols with given name in this scope
      * Symbols occur in the result in reverse order relative to their
      * occurrence in `this.toList`.
      */
    ...
    /** The scope that keeps only those symbols from this scope that match the
      * given predicates. If all symbols match, returns the scope itself,
      * otherwise a copy with the matching symbols.
      */
    ...
    /** Ensure that all elements of this scope have been entered. Overridden by
      * SymbolLoaders.PackageLoader#PackageScope, where it makes sure that all
      * names with `$`'s have been added.
      */
    protected def ensureComplete()(using Context): Unit = ()
  end Scope

  /** A subclass of Scope that defines methods for entering and unlinking
    * entries. Note: constructor is protected to force everyone to use the
    * factory methods newScope or newNestedScope instead. This is necessary
    * because when run from reflection every scope needs to have a
    * SynchronizedScope as mixin.
    */
  class MutableScope protected[Scopes] (
      initElems: ScopeEntry | Null,
      initSize: Int,
    ...
    /** The synthesizer to be used, or `null` if no synthesis is done on this
      * scope
      */
    ...
    /** Clone scope, taking care not to force the denotations of any symbols in
      * the scope.
      */
    def cloneScope(using Context): MutableScope =
      ...
      for i <- entries.length - 1 to 0 by -1 do
        val e = entries(i)
    ...
    protected def newScopeEntry(name: Name, sym: Symbol)(using
        Context
    ): ScopeEntry =
      ensureCapacity(
        if hashTable != null then hashTable.uncheckedNN.length
        else MinHashedScopeSize
      )
      val e = new ScopeEntry(name, sym, this)
      e.prev = lastEntry
      lastEntry = e
      if hashTable != null then enterInHash(e)
      size += 1
      elemsCache = null
      e
    ...
    final def enter[T <: Symbol](sym: T)(using Context): T =
      if sym.isType && ctx.phaseId <= typerPhase.id then
        assert(
          lookup(sym.name) == NoSymbol,
          s"duplicate ${sym.debugString}; previous was ${lookup(sym.name).debugString}"
        ) // !!! DEBUG
      enter(sym.name, sym)
    ...
    final def enterUnique(sym: Symbol)(using Context): Unit =
      assert(
        lookup(sym.name) == NoSymbol,
        (sym.showLocated, lookup(sym.name).showLocated)
      )
      enter(sym)
    ...
    final def unlink(sym: Symbol, name: Name)(using Context): Unit =
      var e = lookupEntry(name)
      while e != null do
        if e.sym == sym then unlink(e)
        e = lookupNextEntry(e)

    /** Replace symbol `prev` (if it exists in current scope) by symbol
      * `replacement`.
      * @pre
    ...
    final def replace(prev: Symbol, replacement: Symbol)(using Context): Unit =
      ...
      while e != null do
        if e.sym == prev then e.sym = replacement
        e = lookupNextEntry(e)
      elemsCache = null
    ...
    /** lookup next entry with same name as this one */
    override final def lookupNextEntry(entry: ScopeEntry)(using
        Context
    ): ScopeEntry | Null =
    ...
    /** Returns all symbols as a list in the order they were entered in this
      * scope. Does _not_ include the elements of inherited scopes.
      */
    override final def toList(using Context): List[Symbol] =
      if elemsCache == null then
        ensureComplete()
        elemsCache = Nil
        var e = lastEntry
        while (e != null) && e.owner == this do
    ...
    override def implicitDecls(using Context): List[TermRef] =
      ...
      var e = lastEntry
      while e != null do
        if e.sym.isOneOf(GivenOrImplicitVal) then
          val d = e.sym.denot
    ...
    /** Check that all symbols in this scope are in their correct hashtable
      * buckets.
      */
    override def checkConsistent()(using Context): Unit =
      ...
      while e != null do
        ...
        while e1 != e && e1 != null do e1 = lookupNextEntry(e1)
  ...
  /** Create a new scope nested in another one with which it shares its elements
    */
  def newNestedScope(outer: Scope)(using Context): MutableScope =
    new MutableScope(outer)
  ...
  /** Transform scope of members of `owner` using operation `op` This is
    * overridden by the reflective compiler to avoid creating new scopes for
    * packages
    */
  ...
  object EmptyScope extends Scope:
    ...
    override def cloneScope(using Context): MutableScope = newScope(
      nestingLevel
    )
    override def lookupEntry(name: Name)(using Context): ScopeEntry | Null =
      null
    override def lookupNextEntry(entry: ScopeEntry)(using Context): ScopeEntry |
      Null = null
end Scopes
