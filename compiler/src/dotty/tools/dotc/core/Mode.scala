...
object Mode:
  ...
  /** This mode bit is set if we collect information without reference to a
    * valid context with typerstate and constraint. This is typically done when
    * we cache the eligibility of implicits. Caching needs to be done across
    * different constraints. Therefore, if TypevarsMissContext is set, subtyping
    * becomes looser, and assumes that TypeParamRefs can be sub- and supertypes
    * of anything. See TypeComparer.
    */
  ...
  /** Make subtyping checks instead infer constraints necessarily following from
    * given subtyping relation.
    *
    * This enables changing [[GadtConstraint]] and alters how [[TypeComparer]]
    * approximates constraints.
    */
  ...
  /** We are currently printing something: avoid producing more logs about the
    * printing.
    */
  ...
  /** Signifies one of two possible situations:
    *   1. We are currently checking bounds to be non-empty, so we should not do
    *      any widening when computing members of refined types. 2. We are
    *      currently checking self type conformance, so we should not ignore
    *      capture sets added to otherwise pure classes (only needed for capture
    *      checking).
    */
  ...
  /** Treat CapturingTypes as plain AnnotatedTypes even in phase CheckCaptures.
    * Reuses the value of OldOverloadingResolution to save Mode bits. This is OK
    * since OldOverloadingResolution only affects implicit search, which is done
    * during phases Typer and Inlinig, and IgnoreCaptures only has an effect
    * during phase CheckCaptures.
    */
  val IgnoreCaptures = OldOverloadingResolution

  /** Allow hk applications of type lambdas to wildcard arguments; used for
    * checking that such applications do not normally arise
    */
  ...
  /** We are elaborating the fully qualified name of a package clause. In this
    * case, identifiers should never be imported.
    */
  ...
  val SynthesizeExtMethodReceiver: Mode =
    newMode(23, "SynthesizeExtMethodReceiver")
  ...
  /** We are typing the body of the condition of an `inline if` or the scrutinee
    * of an `inline match` This mode forces expansion of inline calls in those
    * positions even during typing.
    */
  val ForceInline: Mode = newMode(29, "ForceInline")

  /** This mode is enabled when we check Java overriding in explicit nulls. Type
    * `Null` becomes a subtype of non-primitive value types in TypeComparer.
    */
