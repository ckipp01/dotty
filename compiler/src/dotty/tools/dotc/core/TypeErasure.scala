...
object SourceLanguage:
  ...
  def apply(sym: Symbol)(using Context): SourceLanguage =
    ...
    else if sym.isClass && sym
        .is(Scala2x) || (sym.maybeOwner.is(Scala2x) && !sym.is(Inline))
    then SourceLanguage.Scala2
...
/** Erased types are:
  *
  * ErasedValueType TypeRef(prefix is ignored, denot is ClassDenotation)
  * TermRef(prefix is ignored, denot is SymDenotation) JavaArrayType
  * AnnotatedType MethodType ThisType SuperType ClassInfo (NoPrefix, ...) NoType
  * NoPrefix WildcardType ErrorType
  *
...
object TypeErasure:
  ...
  def normalizeClass(cls: ClassSymbol)(using Context): ClassSymbol =
    if cls.owner == defn.ScalaPackageClass then
      if defn.specialErasure.contains(cls) then
        return defn.specialErasure(cls).uncheckedNN
      if cls == defn.UnitClass then return defn.BoxedUnitClass
    cls

  /** A predicate that tests whether a type is a legal erased type. Only
    * asInstanceOf and isInstanceOf may have types that do not satisfy the
    * predicate. ErasedValueType is considered an erased type because it is
    * valid after Erasure (it is eliminated by ElimErasedValueType).
    */
  def isErasedType(tp: Type)(using Context): Boolean = tp match
    ...
    case tp @ ClassInfo(pre, _, parents, decls, _) =>
      isErasedType(pre) && parents.forall(
        isErasedType
      ) // && decls.forall(sym => isErasedType(sym.info)) && isErasedType(tp.selfType)
  ...
  /** A type representing the semi-erasure of a derived value class, see SIP-15
    * where it's called "C$unboxed" for a class C. Derived value classes are
    * erased to this type during Erasure (when semiEraseVCs = true) and
    * subsequently erased to their underlying type during ElimErasedValueType.
    * This type is outside the normal Scala class hierarchy: it is a subtype of
    * no other type and is a supertype only of Nothing. This is because this
    * type is only useful for type adaptation (see
    * [[Erasure.Boxing#adaptToType]]).
    *
    * @param tycon
    *   A TypeRef referring to the value class symbol
    * @param erasedUnderlying
  ...
  /** Produces an erasure function. See the documentation of the class
    * [[TypeErasure]] for a description of each parameter.
    */
  private def erasureFn(
      sourceLanguage: SourceLanguage,
      semiEraseVCs: Boolean,
      isConstructor: Boolean,
      isSymbol: Boolean,
  ...
  def preErasureCtx(using Context) =
    if ctx.erasedTypes then ctx.withPhase(erasurePhase) else ctx

  /** The standard erasure of a Scala type. Value classes are erased as normal
    * classes.
    *
    * @param tp
    *   The type to erase.
  ...
  def erasure(tp: Type)(using Context): Type =
    erasureFn(
      sourceLanguage = SourceLanguage.Scala3,
      semiEraseVCs = false,
      isConstructor = false,
      isSymbol = false,
      wildcardOK = false
    )(tp)(using preErasureCtx)

  /** The value class erasure of a Scala type, where value classes are
    * semi-erased to ErasedValueType (they will be fully erased in
    * [[ElimErasedValueType]]).
    *
    * @param tp
    *   The type to erase.
  ...
  def scala2Erasure(tp: Type)(using Context): Type =
    erasureFn(
      sourceLanguage = SourceLanguage.Scala2,
      semiEraseVCs = true,
      isConstructor = false,
      isSymbol = false,
      wildcardOK = false
  ...
  /** Like value class erasure, but value classes erase to their underlying type
    * erasure
    */
  ...
  def sigName(tp: Type, sourceLanguage: SourceLanguage)(using
      Context
  ): TypeName =
    val normTp = tp.translateFromRepeated(toArray = sourceLanguage.isJava)
    val erase = erasureFn(
      sourceLanguage,
      semiEraseVCs = !sourceLanguage.isJava,
      isConstructor = false,
      isSymbol = false,
      wildcardOK = true
    )
  ...
  def erasedRef(tp: Type)(using Context): Type = tp match
    case tp: TermRef =>
      ...
      if tp1 ne tp then erasedRef(tp1)
  ...
  /** The symbol's erased info. This is the type's erasure, except for the
    ...
    *
    * `sourceLanguage`, `isConstructor` and `semiEraseVCs` are set based on the
    * symbol.
    */
  def transformInfo(sym: Symbol, tp: Type)(using Context): Type =
    val sourceLanguage = SourceLanguage(sym)
    val semiEraseVCs =
      !sourceLanguage.isJava // Java sees our value classes as regular classes.
    val erase = erasureFn(
      sourceLanguage,
      semiEraseVCs,
      sym.isConstructor,
    ...
    def eraseParamBounds(tp: PolyType): Type =
      tp.derivedLambdaType(
        tp.paramNames,
        tp.paramNames map (Function.const(TypeBounds.upper(defn.ObjectType))),
        tp.resultType
      )

    if defn.isPolymorphicAfterErasure(sym) then
      eraseParamBounds(sym.info.asInstanceOf[PolyType])
    else if sym.isAbstractType then TypeAlias(WildcardType)
    else if sym.is(ConstructorProxy) then NoType
    else if sym.isConstructor then
      outer.addParam(sym.owner.asClass, erase(tp)(using preErasureCtx))
    else if sym.is(Label) then erase.eraseResult(sym.info)(using preErasureCtx)
    else
      erase.eraseInfo(tp, sym)(using preErasureCtx) match
        case einfo: MethodType =>
          if sym.isGetter && einfo.resultType.isRef(defn.UnitClass) then
          ...
          then
            MethodType(
              nme.ALLARGS :: Nil,
              JavaArrayType(defn.ObjectType) :: Nil,
              einfo.resultType
            )
          else if sym.name == nme.apply && sym.owner.derivesFrom(
              defn.PolyFunctionClass
            )
          then
  ...
  /** Is `Array[tp]` a generic Array that needs to be erased to `Object`? This
    * is true if among the subtypes of `Array[tp]` there is either:
    *   - both a reference array type and a primitive array type (e.g. `Array[_
    *     <: Int | String]`, `Array[_ <: Any]`)
    *   - or two different primitive array types (e.g. `Array[_ <: Int |
    *     Double]`) In both cases the erased lub of those array types on the JVM
    *     is `Object`.
    *
    ...
    */
  def isGenericArrayElement(tp: Type, isScala2: Boolean)(using
      Context
  ): Boolean =
    /** A symbol that represents the sort of JVM array that values of type `t`
      * can be stored in:
      *   - If we can always store such values in a reference array, return
      *     Object
      *   - If we can always store them in a specific primitive array, return
    ...
    def arrayUpperBound(t: Type): Symbol = t.dealias match
      case t: TypeRef if t.symbol.isClass =>
        ...
        if (sym eq defn.AnyClass) || (sym eq defn.AnyValClass) || (sym eq defn.MatchableClass) || (sym eq defn.SingletonClass)
          || isScala2 && !(t.derivesFrom(defn.ObjectClass) || t.isNullType)
        then NoSymbol
        // We only need to check for primitives because derived value classes in arrays are always boxed.
        else if sym.isPrimitiveValueClass then sym
      ...
      case tp: AndOrType =>
        ...
        else if tp.isAnd then repr1.orElse(repr2)
        else NoSymbol
      case _ =>
        NoSymbol
    ...
    tp.dealias match
      case tp: TypeRef if !tp.symbol.isOpaqueAlias =>
        ...
        !tp.symbol.is(
          JavaDefined
        ) && // In Java code, Array[T] can never erase to Object
        !fitsInJVMArray(tp)
      ...
      case tp: AndType =>
        isGenericArrayElement(tp.tp1, isScala2) && isGenericArrayElement(
          tp.tp2,
          isScala2
        )
      case tp: OrType =>
  ...
  /** Is `tp` an abstract type or polymorphic type parameter, or another
  ...
  /** The erased least upper bound of two erased types is computed as follows
    *   - if both argument are arrays of objects, an array of the erased lub of
    *     the element types
    *   - if both arguments are arrays of same primitives, an array of this
    *     primitive
    *   - if one argument is array of primitives and the other is array of
    *     objects, Object
    *   - if one argument is an array, Object
    *   - otherwise a common superclass or trait S of the argument classes, with
    *     the following two properties: S is minimal: no other common superclass
    *     or trait derives from S S is last : in the linearization of the first
    *     argument type `tp1` there are no minimal common superclasses or traits
    *     that come after S. The reason to pick last is that we prefer classes
    *     over traits that way, which leads to more predictable bytecode and (?)
    *     faster dynamic dispatch.
    */
  def erasedLub(tp1: Type, tp2: Type)(using Context): Type =
    ...
    if tp1.isRef(defn.NothingClass) || (tp1.isRef(defn.NullClass) && tp2
        .derivesFrom(defn.ObjectClass))
    then
      tp2 // After erasure, Nothing | T is just T and Null | C is just C, if C is a reference type.
    else if tp2.isRef(defn.NothingClass) || (tp2.isRef(defn.NullClass) && tp1
        .derivesFrom(defn.ObjectClass))
    then
        ...
        case JavaArrayType(elem1) =>
          ...
          tp2 match
            case JavaArrayType(elem2) =>
              if elem1.isPrimitiveValueType || elem2.isPrimitiveValueType then
                if elem1.classSymbol eq elem2.classSymbol then // same primitive
                  JavaArrayType(elem1)
  ...
  /** The erased greatest lower bound of two erased type picks one of the two
    * argument types.
    *
    * This operation has the following the properties:
    *   - Associativity and commutativity, because this method acts as the
    *     minimum of the total order induced by `compareErasedGlb`.
    */
  ...
  /** A comparison function that induces a total order on erased types, where `A
    * <= B` implies that the erasure of `A & B` should be A.
    *
  ...
  private def compareErasedGlb(tp1: Type, tp2: Type)(using Context): Int =
    ...
    if isEVT1 && isEVT2 then
      return compareErasedGlb(
        tp1.asInstanceOf[ErasedValueType].tycon,
        tp2.asInstanceOf[ErasedValueType].tycon
      )
    else if isEVT1 then return -1
    else if isEVT2 then return 1

    val isArray1 = tp1.isInstanceOf[JavaArrayType]
    val isArray2 = tp2.isInstanceOf[JavaArrayType]
    if isArray1 && isArray2 then
      return compareErasedGlb(
        tp1.asInstanceOf[JavaArrayType].elemType,
        tp2.asInstanceOf[JavaArrayType].elemType
      )
    else if isArray1 then return -1
    else if isArray2 then return 1

    val sym1 = tp1.classSymbol
    val sym2 = tp2.classSymbol
    def compareClasses: Int =
      if sym1.isSubClass(sym2) then -1
      ...
      else sym1.fullName.toString.compareTo(sym2.fullName.toString)

    val isPrimitive1 = sym1.isPrimitiveValueClass
    val isPrimitive2 = sym2.isPrimitiveValueClass
    if isPrimitive1 && isPrimitive2 then return compareClasses
    else if isPrimitive1 then return -1
    else if isPrimitive2 then return 1

    val isRealClass1 = sym1.isRealClass
    val isRealClass2 = sym2.isRealClass
    if isRealClass1 && isRealClass2 then return compareClasses
    else if isRealClass1 then return -1
    else if isRealClass2 then return 1
...
/** @param sourceLanguage
  ...
  * @param semiEraseVCs
  *   If true, value classes are semi-erased to ErasedValueType (they will be
  *   fully erased in [[ElimErasedValueType]]). If false, they are erased like
  *   normal classes.
  * @param isConstructor
  ...
  * @param wildcardOK
  *   Wildcards are acceptable (true when using the erasure for computing a
  *   signature name).
  */
class TypeErasure(
    sourceLanguage: SourceLanguage,
  ...
  /** The erasure |T| of a type T. This is:
    ...
    *   - For a refined type scala.PolyFunction { def apply[...](x_1, ..., x_N):
    *     R }, scala.FunctionN
    *   - For a typeref scala.Any, scala.AnyVal, scala.Singleton, scala.Tuple,
    *     or scala.*: : |java.lang.Object|
    *   - For a typeref scala.Unit, |scala.runtime.BoxedUnit|.
    *   - For a typeref scala.FunctionN, where N > MaxImplementedFunctionArity,
    *     scala.FunctionXXL
    *   - For a typeref scala.ContextFunctionN, | scala.FunctionN |
    *   - For a typeref P.C where C refers to a class, <noprefix> # C.
    *   - For a typeref P.C where C refers to an alias type, the erasure of C's
    *     alias.
    *   - For a typeref P.C where C refers to an abstract type, the erasure of
    *     C's upper bound.
    ...
    *   - For T1 | T2, the first base class in the linearization of T which is
    *     also a base class of T2
    *   - For => T, ()T
    *   - For a method type (Fs)scala.Unit, (|Fs|)scala.Unit.
    *   - For any other uncurried method type (Fs)T, (|Fs|)|T|.
    *   - For a curried method type (Fs1)(Fs2)T, (|Fs1|,Es2)ET where (Es2)ET =
    *     \|(Fs2)T|.
    *   - For a polymorphic type [Ts](Ps)T, |(Ps)T| _ For a polymorphic type
    *     [Ts]T where T is not a method type, ()|T|
    *   - For the class info type of java.lang.Object, the same type without any
    *     parents.
    *   - For a class info type of a value class, the same type without any
    *     parents.
  ...
  private def apply(tp: Type)(using Context): Type = tp match
    ...
    case tp: TypeRef =>
      ...
      else if semiEraseVCs && isDerivedValueClass(sym) then
        eraseDerivedValueClass(tp)
      else if defn.isSyntheticFunctionClass(sym) then
        defn.functionTypeErasure(sym)
      else eraseNormalClassRef(tp)
    case tp: AppliedType =>
      val tycon = tp.tycon
      if tycon.isRef(defn.ArrayClass) then eraseArray(tp)
      else if tycon.isRef(defn.PairClass) then erasePair(tp)
      else if tp.isRepeatedParam then
        apply(tp.translateFromRepeated(toArray = sourceLanguage.isJava))
      else if semiEraseVCs && isDerivedValueClass(tycon.classSymbol) then
        eraseDerivedValueClass(tp)
    ...
    case RefinedType(parent, nme.apply, refinedInfo: MethodType)
        if defn.isErasedFunctionType(parent) =>
      eraseErasedFunctionApply(refinedInfo)
    case tp: TypeProxy =>
      this(tp.underlying)
    case tp @ AndType(tp1, tp2) =>
      if sourceLanguage.isJava then this(tp1)
      else if sourceLanguage.isScala2 then
        this(
          Scala2Erasure.intersectionDominator(
            Scala2Erasure.flattenedParents(tp)
          )
        )
      else erasedGlb(this(tp1), this(tp2))
    case OrType(tp1, tp2) =>
      ...
      else TypeComparer.orType(this(tp1), this(tp2), isErased = true)
    case tp: MethodType =>
    ...
    case tp @ ClassInfo(pre, cls, parents, decls, _) =>
      if cls.is(Package) then tp
        ...
        val erasedParents: List[Type] =
          if (cls eq defn.ObjectClass) || cls.isPrimitiveValueClass then Nil
          else
            parents.mapConserve(eraseParent) match
              case tr :: trs1 =>
                assert(
                  !tr.classSymbol.is(Trait),
                  i"$cls has bad parents $parents%, %"
                )
                val tr1 = if cls.is(Trait) then defn.ObjectType else tr
                tr1 :: trs1.filterNot(_.isAnyRef)
              case nil => nil
        var erasedDecls = decls
          .filteredScope(sym => !sym.isType || sym.isClass)
          .openForMutations
        for dcl <- erasedDecls.iterator do
          if dcl.lastKnownDenotation
              .unforcedAnnotation(defn.TargetNameAnnot)
              .isDefined
  ...
  /** Widen term ref, skipping any `()` parameter of an eventual getter. Used to
    * erase a TermRef. Since getters are introduced after erasure, one would
    * think that erasing a TermRef could just use `widen`. However, it's
    * possible that the TermRef got read from a class file after Getters (i.e.
    * in the backend). In that case, the reference will not get an earlier
    * denotation even when time travelling forward to erasure. Hence, we need to
    * take the extra precaution of going from nullary method types to their
    * resuls. A test case where this is needed is pos/i15649.scala, which fails
    * non-deterministically if `underlyingOfTermRef` is replaced by `widen`.
    */
  ...
  private def erasePair(tp: Type)(using Context): Type =
    ...
    if arity < 0 then defn.ProductClass.typeRef
  ...
  /** The erasure of a symbol's info. This is different from `apply` in the way
    * `ExprType`s and `PolyType`s are treated. `eraseInfo` maps them them to
    * method types, whereas `apply` maps them to the underlying type.
    */
      ...
      case ExprType(rt) =>
        ...
        // See doc comment for ElimByName for speculation how we could improve this.
          ...
          MethodType(
            ...
            Nil,
            eraseResult(
              rt.translateFromRepeated(toArray = sourceLanguage.isJava)
            )
          )
      case tp1: PolyType =>
  ...
  private def eraseDerivedValueClass(tp: Type)(using Context): Type =
    ...
    if unbox.exists then
      ...
      val erasedValueClass =
        if erasedUnderlying.isPrimitiveValueType && !genericUnderlying.isPrimitiveValueType
        then defn.boxedType(erasedUnderlying)
        else if genericUnderlying.derivesFrom(defn.ArrayClass) then
          erasure(genericUnderlying)
        else erasedUnderlying

      if erasedValueClass.exists then
        ErasedValueType(cls.typeRef, erasedValueClass)
      else
        assert(ctx.reporter.errorsReported, i"no erasure for $underlying")
        NoType
    else NoType
    end if
  end eraseDerivedValueClass

  private def eraseNormalClassRef(tref: TypeRef)(using Context): Type =
    val cls = tref.symbol.asClass
    (if cls.owner.is(Package) then normalizeClass(cls) else cls).typeRef
  ...
  def eraseResult(tp: Type)(using Context): Type =
    ...
    if semiEraseVCs && isConstructor && !tp.isInstanceOf[MethodOrPoly] then
      ...
      tp match
        case tp: TypeRef =>
          val sym = tp.symbol
          if sym eq defn.UnitClass then sym.typeRef
          else this(tp)
        case tp: AppliedType =>
          val sym = tp.tycon.typeSymbol
          if sym.isClass && !erasureDependsOnArgs(sym) then
            eraseResult(tp.tycon)
          else this(tp)
        case _ =>
          this(tp)

  /** The name of the type as it is used in `Signature`s. Need to ensure
    * correspondence with erasure!
    */
  private def sigName(tp: Type)(using Context): TypeName = try
    tp match
      case tp: TypeRef =>
        if !tp.denot.exists then
        ...
        if !sym.isClass then
          ...
          if !info.exists then assert(false, i"undefined: $tp with symbol $sym")
          return sigName(info)
        if semiEraseVCs && isDerivedValueClass(sym) then
          val erasedVCRef = eraseDerivedValueClass(tp)
          if erasedVCRef.exists then return sigName(erasedVCRef)
        if defn.isSyntheticFunctionClass(sym) then
      ...
      case tp: AppliedType =>
        ...
        sigName( // todo: what about repeatedParam?
          if erasureDependsOnArgs(sym) then this(tp)
          else if sym.isClass then tp.underlying
      ...
      case tp: TypeVar =>
        ...
        if inst.exists then sigName(inst) else tpnme.Uninstantiated
      case tp @ RefinedType(parent, nme.apply, _)
          if parent.typeSymbol eq defn.PolyFunctionClass =>
        ...
        sigName(this(tp))
      case tp @ RefinedType(parent, nme.apply, refinedInfo)
          if defn.isErasedFunctionType(parent) =>
        sigName(this(tp))
      case tp: TypeProxy =>
