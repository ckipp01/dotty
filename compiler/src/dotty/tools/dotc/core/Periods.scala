...
object Periods:
  ...
  /** The period containing the current period where denotations do not change.
    ...
    * the current phase that has the same "nextTransformerId". As last phase we
    * take the next transformer id following the current phase.
    */
  def currentStablePeriod(using Context): Period =
    var first = ctx.phaseId
    val nxTrans = ctx.base.nextDenotTransformerId(first)
    while first - 1 > NoPhaseId && (ctx.base.nextDenotTransformerId(
        first - 1
      ) == nxTrans)
    do first -= 1
    Period(ctx.runId, first, nxTrans)

  /** Are all base types in the current period guaranteed to be the same as in
    * period `p`?
    */
  ...
  /** A period is a contiguous sequence of phase ids in some run. It is coded as
    * follows:
    *
    * sign, always 0 1 bit runid 17 bits last phase id: 7 bits #phases before
    * last: 7 bits
    *
    * // Dmitry: sign == 0 isn't actually always true, in some cases phaseId ==
    * -1 is used for shifts, that easily creates code < 0
    */
  class Period(val code: Int) extends AnyVal:
    ...
    def containsPhaseId(id: PhaseId): Boolean =
      firstPhaseId <= id && id <= lastPhaseId
    ...
    override def toString: String =
      s"Period($firstPhaseId..$lastPhaseId, run = $runId)"
