...
object Denotations:
  ...
  /** A PreDenotation represents a group of single denotations or a single
    * multi-denotation It is used as an optimization to avoid forming
    * MultiDenotations too eagerly.
    */
  abstract class PreDenotation:
    ...
    /** Keep only those denotations in this group which have a signature that's
      * not already defined by `denots`.
      */
    def filterDisjoint(denots: PreDenotation)(using Context): PreDenotation

    /** Keep only those inherited members M of this predenotation for which the
      * following is true
      *   - M is not marked Private
      *   - If M has a unique symbol, it does not appear in `prevDenots`.
      *   - M's signature as seen from prefix `pre` does not appear in
      *     `ownDenots` Return the denotation as seen from `pre`. Called from
      *     SymDenotations.computeMember. There, `ownDenots` are the denotations
      *     found in the base class, which shadow any inherited denotations with
      *     the same signature. `prevDenots` are the denotations that are
      *     defined in the class or inherited from a base type which comes
      *     earlier in the linearization.
      */
    def mapInherited(
    ...
    /** Keep only those denotations in this group that have all of the flags in
      * `required`, but none of the flags in `excluded`.
      */
    def filterWithFlags(required: FlagSet, excluded: FlagSet)(using
        Context
    ): PreDenotation
    ...
    def asSeenFrom(pre: Type)(using Context): AsSeenFromResult =
      if Config.cacheAsSeenFrom then
        if (cachedPrefix ne pre) || ctx.period != validAsSeenFrom then
          cachedAsSeenFrom = computeAsSeenFrom(pre)
          cachedPrefix = pre
          validAsSeenFrom = if pre.isProvisional then Nowhere else ctx.period
        cachedAsSeenFrom
  ...
  /** A denotation is the result of resolving a name (either simple identifier
    * or select) during a given period.
    *
    * Denotations can be combined with `&` and `|`. & is conjunction, | is
    * disjunction.
    *
    * `&` will create an overloaded denotation from two non-overloaded
    * denotations if their signatures differ. Analogously `|` of two denotations
    * with different signatures will give an empty denotation `NoDenotation`.
    *
    ...
    * Say, we have:
    *
    * class A { def f: A } class B { def f: B } val x: A | B = if (test) new A
    * else new B val y = x.f
    *
  ...
  abstract class Denotation(
    ...
    /** The type info. The info is an instance of TypeType iff this is a type
      * denotation Uncompleted denotations set myInfo to a LazyType.
      */
    final def info(using Context): Type =
      def completeInfo = // Written this way so that `info` is small enough to be inlined
        this
          .asInstanceOf[SymDenotation]
          .completeFrom(myInfo.asInstanceOf[LazyType]); info
      if myInfo.isInstanceOf[LazyType] then completeInfo else myInfo

    /** The type info, or, if this is a SymDenotation where the symbol is not
      * yet completed, the completer
      */
    ...
    /** Denotation points to unique symbol; false for overloaded denotations and
      * JointRef denotations.
      */
    ...
    /** The variant of this denotation that's current in the given context. If
      * no such denotation exists, returns the denotation with each alternative
    ...
    /** The unique alternative of this denotation that satisfies the predicate
      * `p`, or NoDenotation if no satisfying alternative exists.
      * @throws TypeError
    ...
    /** Does this denotation have an alternative that satisfies the predicate
      * `p`?
      */
    def hasAltWith(p: SingleDenotation => Boolean): Boolean

    /** The denotation made up from the alternatives of this denotation that are
      * accessible from prefix `pre`, or NoDenotation if no accessible
      * alternative exists.
      */
    def accessibleFrom(pre: Type, superAccess: Boolean = false)(using
        Context
    ): Denotation

    /** Find member of this denotation with given `name`, all `required` flags
      * and no `excluded` flag, and produce a denotation that contains the type
      * of the member as seen from given prefix `pre`.
      */
    def findMember(name: Name, pre: Type, required: FlagSet, excluded: FlagSet)(
        using Context
    ): Denotation =
      info.findMember(name, pre, required, excluded)

    /** If this denotation is overloaded, filter with given predicate. If result
      * is still overloaded throw a TypeError. Note: disambiguate is slightly
      * different from suchThat in that single-denotations that do not satisfy
      * the predicate are left alone (whereas suchThat would map them to
      * NoDenotation).
      */
    inline def disambiguate(inline p: Symbol => Boolean)(using
        Context
    ): SingleDenotation = this match
      case sdenot: SingleDenotation => sdenot
      case mdenot => suchThat(p) orElse NoQualifyingRef(alternatives)

    /** Return symbol in this denotation that satisfies the given predicate. if
      * generateStubs is specified, return a stubsymbol if denotation is a
      * missing ref. Throw a `TypeError` if predicate fails to disambiguate
      * symbol or no alternative matches.
      */
    def requiredSymbol(
        kind: String,
        name: Name,
    ...
    )(p: Symbol => Boolean)(using Context): Symbol =
      disambiguate(p) match
        ...
        case NoDenotation | _: NoQualifyingRef | _: MissingRef =>
          def argStr = if args.isEmpty then "" else i" matching ($args%, %)"
          val msg =
            if site.exists then
              em"$site does not have a member $kind $name$argStr"
    ...
    def requiredMethod(pname: PreName)(using Context): TermSymbol =
      ...
      info
        .member(name)
        .requiredSymbol("method", name, this)(_.is(Method))
        .asTerm
    def requiredMethodRef(name: PreName)(using Context): TermRef =
      requiredMethod(name).termRef

    def requiredMethod(pname: PreName, argTypes: List[Type])(using
        Context
    ): TermSymbol =
      val name = pname.toTermName
      info
        .member(name)
        .requiredSymbol("method", name, this, argTypes) { x =>
          x.is(Method) && {
            x.info.paramInfoss match
              case paramInfos :: Nil =>
                paramInfos.corresponds(argTypes)(_ =:= _)
              case _ => false
          }
        }
        .asTerm
    def requiredMethodRef(name: PreName, argTypes: List[Type])(using
        Context
    ): TermRef =
    ...
    def requiredValue(pname: PreName)(using Context): TermSymbol =
      ...
      info
        .member(name)
        .requiredSymbol("field or getter", name, this)(_.info.isParameterless)
        .asTerm
    ...
    /** The alternative of this denotation that has a type matching `targetType`
      * when seen as a member of type `site` and that has a target name matching
      * `targetName`, or `NoDenotation` if none exists.
      */
    def matchingDenotation(site: Type, targetType: Type, targetName: Name)(using
        Context
    ): SingleDenotation =
      def qualifies(sym: Symbol) =
        site.memberInfo(sym).matchesLoosely(targetType) && sym.hasTargetName(
          targetName
        )
      if isOverloaded then
        atSignature(
          ...
          case sd: SingleDenotation =>
            sd.matchingDenotation(site, targetType, targetName)
          case md => md.suchThat(qualifies(_))
      else if exists && !qualifies(symbol) then NoDenotation
      else asSingleDenotation

    /** Form a denotation by conjoining with denotation `that`.
      *
      * NoDenotations are dropped. MultiDenotations are handled by merging parts
      * with same signatures. SingleDenotations with equal signatures are joined
      * by following this sequence of steps:
      *
      *   1. If exactly one the denotations has an inaccessible symbol, pick the
      *      other one. 2. Otherwise, if one of the infos overrides the other
      *      one, and the associated symbol does not score strictly lower than
      *      the other one, pick the associated denotation. 3. Otherwise, if the
      *      two infos can be combined with `infoMeet`, pick that as result
      *      info, and pick the symbol that scores higher as result symbol, or
      *      pick `sym1` as a tie breaker. The picked info and symbol are
      *      combined in a JointDenotation. 4. Otherwise, if one of the two
      *      symbols scores strongly higher than the other one, pick the
      *      associated denotation. 5. Otherwise return a multi-denotation
      *      consisting of both denotations.
      *
      * Symbol scoring is determined according to the following ranking where
      * earlier criteria trump later ones. Cases marked with (*) give a strong
      * score advantage, the others a weak one.
      *
      *   1. The symbol exists, and the other one does not. (*) 2. The symbol is
      *      not a bridge, but the other one is. (*) 3. The symbol is concrete,
      *      and the other one is deferred 4. The symbol appears before the
      *      other in the linearization of `pre` 5. The symbol's visibility is
      *      strictly greater than the other one's. 6. The symbol is a method,
      *      but the other one is not.
      */
    def meet(that: Denotation, pre: Type, safeIntersection: Boolean = false)(
        using Context
    ): Denotation =
      /** Try to merge denot1 and denot2 without adding a new signature. */
      def mergeDenot(denot1: Denotation, denot2: SingleDenotation): Denotation =
        denot1 match
      ...
      def mergeSingleDenot(
          denot1: SingleDenotation,
          denot2: SingleDenotation
      ): Denotation =
        ...
        /** Similar to SymDenotation#accessBoundary, but without the special
          * cases.
          */
        def accessBoundary(sym: Symbol) =
          if sym.is(Private) then sym.owner
          else
            sym.privateWithin.orElse(
              if sym.is(Protected) then sym.owner.enclosingPackageClass
              else defn.RootClass
            )
        ...
        if hidden1 && !hidden2 then denot2
          ...
          val relaxedOverriding =
            ctx.explicitNulls && (sym1.is(JavaDefined) || sym2.is(JavaDefined))
          val matchLoosely =
            sym1.matchNullaryLoosely || sym2.matchNullaryLoosely

          if symScore <= 0 && info2.overrides(
              info1,
              relaxedOverriding,
              matchLoosely,
              checkClassInfo = false
            )
          then denot2
          ...
          then denot1
            ...
            if jointInfo.exists then
              ...
              JointRefDenotation(
                ...
                pre,
                denot1.isRefinedMethod || denot2.isRefinedMethod
              )
            else if symScore == 2 then denot1
            else if symScore == -2 then denot2
            else
              overload.println(
                i"overloaded with same signature: ${sym1.showLocated}: $info1 / ${sym2.showLocated}: $info2, info = ${info1.getClass}, ${info2.getClass}, $jointInfo"
              )
  ...
  /** Merge parameter names of lambda types. If names in corresponding positions
    * match, keep them, otherwise generate new synthetic names.
    */
  private def mergeParamNames(
      tp1: LambdaType,
      tp2: LambdaType
  ): List[tp1.ThisName] =
    (for (
      (name1, name2, idx) <- tp1.paramNames
        .lazyZip(tp2.paramNames)
        .lazyZip(tp1.paramNames.indices)
    )
      yield
        if name1 == name2 then name1
        else tp1.companion.syntheticParamName(idx)).toList

  /** Normally, `tp1 & tp2`, with extra care taken to return `tp1` or `tp2`
    * directly if that's a valid answer. Special cases for matching methods and
    * classes, with the possibility of returning NoType. Special handling of
    * ExprTypes, where mixed intersections widen the ExprType away.
    */
  def infoMeet(tp1: Type, tp2: Type, safeIntersection: Boolean)(using
      Context
  ): Type =
    if tp1 eq tp2 then tp1
      ...
      tp1 match
        case tp1: TypeBounds =>
          tp2 match
            case tp2: TypeBounds =>
              if safeIntersection then tp1 safe_& tp2 else tp1 & tp2
        ...
        case tp1: PolyType =>
          tp2 match
            case tp2: PolyType
                if tp1.paramNames.hasSameLengthAs(tp2.paramNames) =>
              val resType = infoMeet(
                tp1.resType,
                tp2.resType.subst(tp2, tp1),
                safeIntersection
  ...
  abstract class SingleDenotation(
      ...
      initInfo: Type,
      isType: Boolean
  ) extends Denotation(symbol, initInfo, isType):
    ...
    /** For SymDenotation, this is NoPrefix. For other denotations this is the
      * prefix under which the denotation was constructed.
      *
    ...
    /** For SymDenotations, the language-specific signature of the info,
      * depending on where the symbol is defined. For non-SymDenotations, the
      * Scala 3 signature.
      *
      * Invariants:
    ...
    final def signature(using Context): Signature =
      signature(sourceLanguage =
        if isType || !this.isInstanceOf[SymDenotation] then
          SourceLanguage.Scala3
        else SourceLanguage(symbol)
      )

    /** Overload of `signature` which lets the caller pick the language used to
      * compute the signature of the info. Useful to match denotations defined
      * in different classes (see `matchesLoosely`).
      */
    def signature(sourceLanguage: SourceLanguage)(using Context): Signature =
      if isType then
        Signature.NotAMethod // don't force info if this is a type denotation
      else
        info match
    ...
    def derivedSingleDenotation(
        symbol: Symbol,
        info: Type,
        pre: Type = this.prefix,
        isRefinedMethod: Boolean = this.isRefinedMethod
    )(using Context): SingleDenotation =
      if (symbol eq this.symbol) && (info eq this.info) && (pre eq this.prefix) && (isRefinedMethod == this.isRefinedMethod)
      then this
    ...
    def altsWith(p: Symbol => Boolean): List[SingleDenotation] =
      if exists && p(symbol) then this :: Nil else Nil

    def suchThat(p: Symbol => Boolean)(using Context): SingleDenotation =
      if exists && p(symbol) then this else NoDenotation

    def hasAltWith(p: SingleDenotation => Boolean): Boolean =
      exists && p(this)

    def accessibleFrom(pre: Type, superAccess: Boolean)(using
        Context
    ): Denotation =
      if !symbol.exists || symbol.isAccessibleFrom(pre, superAccess) then this
      else NoDenotation
    ...
    def atSignature(
      ...
      val sigMatches =
        try
          ...
          case ex: MissingType =>
            false
      if sigMatches && symbol.hasTargetName(targetName) then this
      else NoDenotation
    ...
    def matchesImportBound(bound: Type)(using Context): Boolean =
      if bound.isRef(defn.NothingClass) then false
        ...
        NoViewsAllowed.normalizedCompatible(info, bound, keepConstraint = false)

    // ------ Transformations -----------------------------------------

    /** The next SingleDenotation in this run, with wrap-around from last to
      * first.
      *
      * There may be several `SingleDenotation`s with different validity
      * representing the same underlying definition at different phases. These
      * are called a "flock". Flock members are generated by
      * @See
      *   current. Flock members are connected in a ring with their `nextInRun`
      *   fields.
      *
      * There are the following invariants concerning flock members
      *
      * 1) validity periods are non-overlapping 2) the union of all validity
      * periods is a contiguous interval.
      */
    ...
    /** Invalidate all caches and fields that depend on base classes and their
      * contents
      */
    ...
    private def updateValidity()(using Context): this.type =
      assert(
        ctx.runId >= validFor.runId
          ...
          || ctx.mode.is(
            Mode.Printing
          ) // no use to be picky when printing error messages
      ...
      while {
        d.validFor =
          Period(ctx.runId, d.validFor.firstPhaseId, d.validFor.lastPhaseId)
    ...
    /** Move validity period of this denotation to a new run. Throw a
      * StaleSymbol error if denotation is no longer valid. However, StaleSymbol
      * error is not thrown in the following situations:
      *
      *   - If acceptStale returns true (e.g. because we are in the IDE), update
      *     the symbol to the new version if it exists, or return the old
      *     version otherwise.
      *   - If the symbol did not have a denotation that was defined at the
      *     current phase return a NoDenotation instead.
      */
    private def bringForward()(using Context): SingleDenotation =
      this match
        case symd: SymDenotation =>
          if stillValid(symd) then return updateValidity()
          if acceptStale(
              symd
            ) && symd.initial.validFor.firstPhaseId <= ctx.lastPhaseId
          then
        ...
        case _ =>
      if !symbol.exists then return updateValidity()
      if !coveredInterval.containsPhaseId(ctx.phaseId) then return NoDenotation
      if ctx.debug then traceInvalid(this)
      staleSymbolError
    ...
    private def nextDefined: SingleDenotation =
      var p1 = this
      var p2 = nextInRun
      while p1.validFor == Nowhere && (p1 ne p2) do
        p1 = p1.nextInRun
    ...
    def skipRemoved(using Context): SingleDenotation =
      if validFor.code <= 0 then nextDefined else this

    /** Produce a denotation that is valid for the given context. Usually called
      * when !(validFor contains ctx.period) (even though this is not a
      * precondition). If the runId of the context is the same as runId of this
      * denotation, the right flock member is located, or, if it does not exist
      * yet, created by invoking a transformer (@See Transformers). If the
      * runId's differ, but this denotation is a SymDenotation and its toplevel
      * owner class or module is still a member of its enclosing package, then
      * the whole flock is brought forward to be valid in the new runId.
      * Otherwise the symbol is stale, which constitutes an internal error.
      */
    def current(using Context): SingleDenotation =
      ...
      def assertNotPackage(d: SingleDenotation, transformer: DenotTransformer) =
        d match
          case d: ClassDenotation =>
            assert(
              !d.is(Package),
              s"illegal transformation of package denotation by transformer $transformer"
            )
          case _ =>
      ...
      def goForward =
        ...
        if next.validFor.code > valid.code then
          ...
          val nextTransformerId =
            ctx.base.nextDenotTransformerId(cur.validFor.lastPhaseId)
          if currentPeriod.lastPhaseId <= nextTransformerId then
            ...
            finally mutCtx.setPeriod(savedPeriod)
            if next eq cur then startPid = cur.validFor.firstPhaseId
            else
              assertNotPackage(next, transformer)
              next.insertAfter(cur)
              cur = next
            cur.validFor =
              Period(currentPeriod.runId, startPid, transformer.lastPhaseId)
            // printPeriods(cur)
      ...
      if valid.code <= 0 then
        ...
        nextDefined
      else if valid.runId != currentPeriod.runId then toNewRun
      else if currentPeriod.code > valid.code then goForward
      else goBack
    end current
    ...
    /** Install this denotation to be the result of the given denotation
      * transformer. This is the implementation of the same-named method in
      * SymDenotations. It's placed here because it needs access to private
      * fields of SingleDenotation.
      * @pre
      *   Can only be called in `phase.next`.
      */
    ...
    protected def transformAfter(
      ...
      do current = current.nextInRun
      var hasNext = true
      while (current.validFor.firstPhaseId >= phase.id) && hasNext do
        val current1: SingleDenotation = f(current.asSymDenotation)
        if current1 ne current then
    ...
    /** Insert this denotation instead of `old`. Also ensure that `old` refers
      * with `nextInRun` to this denotation and set its `validFor` field to
      * `Nowhere`. This is necessary so that references to the old denotation
      * can be brought forward via `current` to a valid denotation.
      *
      * The code to achieve this is subtle in that it works correctly whether
      * the replaced denotation is the only one in its cycle or not.
      */
    private[dotc] def replaceWith(newd: SingleDenotation): Unit =
      var prev = this
      while prev.nextInRun ne this do prev = prev.nextInRun
    ...
    /** The period (interval of phases) for which there exists a valid
      * denotation in this flock.
      */
    ...
    /** Show declaration string; useful for showing declarations as seen from
      * subclasses.
      */
    def showDcl(using Context): String = ctx.printer.dclText(this).show

    override def toString: String =
      if symbol == NoSymbol then symbol.toString
    ...
    /** `matches` without a target name check.
      ...
      */
    def matchesLoosely(
        other: SingleDenotation,
        alwaysCompareTypes: Boolean = false
      ...
      if isType then true
        ...
        val commonLanguage =
          SourceLanguage.commonLanguage(thisLanguage, otherLanguage)
        ...
        sig.matchDegree(otherSig) match
          ...
          case MethodNotAMethodMatch =>
            !ctx.erasedTypes && {
              ...
              if !thisLanguage.isJava && !otherLanguage.isJava then true
              // Java allows defining both a field and a zero-parameter method with the same name,
              // so they must not match.
              else if thisLanguage.isJava && otherLanguage.isJava then false
              // A Java field never matches a Scala method.
              else if thisLanguage.isJava then symbol.is(Method)
    ...
    def filterWithPredicate(p: SingleDenotation => Boolean): SingleDenotation =
      if p(this) then this else NoDenotation
    def filterDisjoint(denots: PreDenotation)(using Context): SingleDenotation =
      if denots.exists && denots.matches(this) then NoDenotation else this
    def filterWithFlags(required: FlagSet, excluded: FlagSet)(using
        Context
    ): SingleDenotation =
      val realExcluded =
        if ctx.isAfterTyper then excluded else excluded | Invisible
      def symd: SymDenotation = this match
        case symd: SymDenotation => symd
        case _                   => symbol.denot
      if !required.isEmpty && !symd.isAllOf(required)
        || symd.isOneOf(realExcluded)
      then NoDenotation
      else this
    ...
    protected def computeAsSeenFrom(pre: Type)(using
        Context
    ): SingleDenotation =
      val symbol = this.symbol
      val owner = this match
        case thisd: SymDenotation => thisd.owner
        case _ => if symbol.exists then symbol.owner else NoSymbol

      /** The derived denotation with the given `info` transformed with
        * `asSeenFrom`.
        *
        * As a performance hack, we might reuse an existing SymDenotation,
        * instead of creating a new denotation with a given `prefix`, see
      ...
      def derived(info: Type) =
        ...
        if Config.reuseSymDenotations && this.isInstanceOf[SymDenotation]
          && (derivedInfo eq info) && !needsPrefix
        then this
        else derivedSingleDenotation(symbol, derivedInfo, pre)
      end derived
      ...
      if !owner.membersNeedAsSeenFrom(
          pre
      ...
      then this
        ...
        if arg.exists
        then
          derivedSingleDenotation(symbol, normalizedArgBounds(arg.bounds), pre)
        else derived(symbol.info)
      else derived(symbol.info)
    end computeAsSeenFrom

    /** The argument bounds, possibly intersected with the parameter's info
      * TypeBounds, if the latter is not F-bounded and does not refer to other
      * type parameters of the same class, and the intersection is provably
      * nonempty.
      */
    private def normalizedArgBounds(
        argBounds: TypeBounds
    ...
    private def hasBoundsDependingOnParamsOf(cls: Symbol)(using
        Context
    ): Boolean =
      val acc = new TypeAccumulator[Boolean]:
        def apply(x: Boolean, tp: Type): Boolean = tp match
          case _: LazyRef => true
          case tp: TypeRef
              if tp.symbol.isAllOf(ClassTypeParam) && tp.symbol.owner == cls =>
            true
          case _ => foldOver(x, tp)
      acc(false, symbol.info)
  end SingleDenotation

  abstract class NonSymSingleDenotation(
      symbol: Symbol,
      initInfo: Type,
  ...
  class UniqueRefDenotation(
      ...
      initValidFor: Period,
      prefix: Type
  ) extends NonSymSingleDenotation(symbol, initInfo, prefix):
  ...
  class JointRefDenotation(
      ...
      initValidFor: Period,
      prefix: Type,
      override val isRefinedMethod: Boolean
  ) extends NonSymSingleDenotation(symbol, initInfo, prefix):
    validFor = initValidFor
  ...
  class ErrorDenotation(using Context)
      extends NonSymSingleDenotation(NoSymbol, NoType, NoType):
  ...
  /** An error denotation that provides more info about the missing reference.
    ...
    */
  case class MissingRef(val owner: SingleDenotation, name: Name)(using Context)
      extends ErrorDenotation:
    val ex: Exception = new Exception // DEBUG

  /** An error denotation that provides more info about alternatives that were
    * found but that do not qualify. Produced by staticRef, consumed by
    * requiredSymbol.
    */
  case class NoQualifyingRef(alts: List[SingleDenotation])(using Context)
      extends ErrorDenotation
  ...
  trait MultiPreDenotation extends PreDenotation:
    ...
    assert(
      denot1.exists && denot2.exists,
      s"Union of non-existing denotations ($denot1) and ($denot2)"
    )
    ...
    def mapInherited(
        owndenot: PreDenotation,
        prevdenot: PreDenotation,
        pre: Type
    )(using Context): PreDenotation =
      derivedUnion(
        denot1.mapInherited(owndenot, prevdenot, pre),
        denot2.mapInherited(owndenot, prevdenot, pre)
      )
    def filterWithPredicate(p: SingleDenotation => Boolean): PreDenotation =
    ...
    def filterWithFlags(required: FlagSet, excluded: FlagSet)(using
      ...
      derivedUnion(
        denot1.filterWithFlags(required, excluded),
        denot2.filterWithFlags(required, excluded)
      )
    def aggregate[T](f: SingleDenotation => T, g: (T, T) => T): T =
      g(denot1.aggregate(f, g), denot2.aggregate(f, g))
    protected def derivedUnion(denot1: PreDenotation, denot2: PreDenotation) =
      if (denot1 eq this.denot1) && (denot2 eq this.denot2) then this
      else denot1 union denot2
  end MultiPreDenotation

  final case class DenotUnion(denot1: PreDenotation, denot2: PreDenotation)
      extends MultiPreDenotation:
    def exists: Boolean = true
    def toDenot(pre: Type)(using Context): Denotation =
  ...
  /** An overloaded denotation consisting of the alternatives of both given
    * denotations.
    */
  case class MultiDenotation(denot1: Denotation, denot2: Denotation)
      extends Denotation(NoSymbol, NoType, isType = false)
      with MultiPreDenotation:
    ...
    def atSignature(
      ...
      if sig eq Signature.OverloadedSignature then this
      else
        derivedUnionDenotation(
    ...
    def suchThat(p: Symbol => Boolean)(using Context): SingleDenotation =
      ...
      if sd1.exists then
        if sd2.exists then
          throw TypeError(em"""Failure to disambiguate overloaded reference with
    ...
    override def filterWithPredicate(
      ...
      derivedUnionDenotation(
        denot1.filterWithPredicate(p),
        denot2.filterWithPredicate(p)
      )
    def hasAltWith(p: SingleDenotation => Boolean): Boolean =
    ...
    def accessibleFrom(pre: Type, superAccess: Boolean)(using
      ...
      if !d1.exists then d2
      else if !d2.exists then d1
    ...
    def derivedUnionDenotation(d1: Denotation, d2: Denotation): Denotation =
      if (d1 eq denot1) && (d2 eq denot2) then this
      else if !d1.exists then d2
      else if !d2.exists then d1
  ...
  /** The current denotation of the static reference given by path, or a
    * MissingRef or NoQualifyingRef instance, if it does not exist. if
    * generateStubs is set, generates stubs for missing top-level symbols
    */
  def staticRef(
      path: Name,
      generateStubs: Boolean = true,
    ...
    def select(prefix: Denotation, selector: Name): Denotation =
      ...
      if owner.exists then
        val result =
          if isPackage then owner.info.decl(selector)
          else owner.info.member(selector)
      ...
      path match
        ...
        case path: SimpleName =>
          def recurSimple(len: Int, wrap: TermName => Name): Denotation =
            ...
            val prefix =
              if point > 0 then recurSimple(point, identity)
              else if selector.isTermName then defn.RootClass.denot
  ...
  /** If we are looking for a non-existing term name in a package, assume it is
    * a package for which we do not have a directory and enter it.
    */
  def missingHook(owner: Symbol, name: Name)(using Context): Symbol =
    if owner.is(Package) && name.isTermName then
