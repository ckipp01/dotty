...
object Symbols:
  ...
  class Symbol private[Symbols] (
    ...
    /** Set the coordinate of this class, this is only useful when the
      * coordinate is not known at symbol creation. This is the case for root
      * symbols unpickled from TASTY.
      *
      * @pre
      *   coord == NoCoord
      */
    ...
    /** The tree defining the symbol at pickler time, EmptyTree if none was
      * retained
      */
    def defTree: Tree =
      if myDefTree == null then tpd.EmptyTree else myDefTree.nn

    /** Set defining tree if this symbol retains its definition tree */
    def defTree_=(tree: Tree)(using Context): Unit =
      if retainsDefTree then myDefTree = tree

    /** Does this symbol retain its definition tree? A good policy for this
      * needs to balance costs and benefits, where costs are mainly memoty
      * leaks, in particular across runs.
      */
    ...
    /** Set the denotation of this symbol `denot` should always be initialized
      * when a new Symbol is created.
      */
    ...
    private def computeDenot(lastd: SymDenotation)(using
        Context
    ): SymDenotation =
    ...
    protected def recomputeDenot(lastd: SymDenotation)(using
        Context
    ): SymDenotation =
    ...
    /** The last known denotation of this symbol, without going through
      * `current`
      */
    ...
    private inline def associatedFileMatches(
        inline filter: AbstractFile => Boolean
    ...
    final def isDefinedInCurrentRun(using Context): Boolean =
      span.exists && defRunId == ctx.runId && associatedFileMatches(
        ctx.run.nn.files.contains
      )

    /** Is this symbol valid in the current run and has an associated file that
      * is not a binary file. e.g. This will return true for symbols defined by
      * the user in a prior run of the REPL, that are still valid.
      */
    final def isDefinedInSource(using Context): Boolean =
      span.exists && isValidInCurrentRun && associatedFileMatches(
        _.extension != "class"
      )
    ...
    final def isTerm(using Context): Boolean =
      (if defRunId == ctx.runId then lastDenot else denot).isTerm
    final def isType(using Context): Boolean =
      (if defRunId == ctx.runId then lastDenot else denot).isType
    ...
    /** Test whether symbol is private. This conservatively returns `false` if
      * symbol does not yet have a denotation, or denotation is a class that is
      * not yet read.
      */
    ...
    /** The symbol's signature if it is completed or a method, NotAMethod
      * otherwise.
      */
    final def signature(using Context): Signature =
      if lastDenot.uncheckedNN.isCompleted || lastDenot.uncheckedNN.is(Method)
      then denot.signature
      else Signature.NotAMethod

    /** Special cased here, because it may be used on naked symbols in
      * substituters
      */
    ...
    final def entered(using Context): this.type =
      if this.owner.isClass then
        ...
        if this.is(Module) then this.owner.asClass.enter(this.moduleClass)
      this
        ...
        this.owner match
          case owner: ClassSymbol =>
            if owner.is(Package) then
              ...
              if this.is(Module) then this.moduleClass.validFor |= InitialPeriod
            else owner.ensureFreshScopeAfter(phase)
            assert(
              isPrivate || phase.changesMembers,
              i"$this entered in $owner at undeclared phase $phase"
            )
            entered
          case _ => this
    ...
    final def drop()(using Context): Unit =
      ...
      if this.is(Module) then this.owner.asClass.delete(this.moduleClass)

    /** Remove symbol from scope of owning class after given `phase`. Create a
      * fresh denotation for its owner class if the class does not already have
      * one that starts being valid after `phase`.
      * @pre
      *   Symbol is a class member
      */
    def dropAfter(phase: DenotTransformer)(using Context): Unit =
      if ctx.phaseId != phase.next.id then atPhase(phase.next)(dropAfter(phase))
      else
        assert(!this.owner.is(Package))
        this.owner.asClass.ensureFreshScopeAfter(phase)
        assert(
          isPrivate || phase.changesMembers,
          i"$this deleted in ${this.owner} at undeclared phase $phase"
        )
        drop()
    ...
    def filter(p: Symbol => Boolean): Symbol =
      if p(this) then this else NoSymbol
    ...
    /** The source or class file from which this class or the class containing
      * this symbol was generated, null if not applicable. Note that this the
      * returned classfile might be the top-level class containing this symbol
      * instead of the directly enclosing class. Overridden in ClassSymbol
      */
    def associatedFile(using Context): AbstractFile | Null =
      lastDenot.topLevelClass.associatedFile

    /** The class file from which this class was generated, null if not
      * applicable.
      */
    ...
    /** A trap to avoid calling x.symbol on something that is already a symbol.
      ...
      * the same as `x`. With the given setup, all such calls will give
      * implicit-not found errors
      */
    final def symbol(implicit ev: DontUseSymbolOnSymbol): Nothing = unsupported(
      "symbol"
    )
    ...
    final def sourceSymbol(using Context): Symbol =
      if !denot.exists then this
      else if denot.is(ModuleVal) then
        ...
        else denot.owner.sourceSymbol
      else if denot.isPrimaryConstructor then denot.owner.sourceSymbol
      else this

    /** The position of this symbol, or NoSpan if the symbol was not loaded from
      * source or from TASTY. This is always a zero-extent position.
      */
    final def span: Span = if coord.isSpan then coord.toSpan else NoSpan

    final def sourcePos(using Context): SourcePosition =
      val src = source
      (if src.exists then src else ctx.source).atSpan(span)

    /** This positioned item, widened to `SrcPos`. Used to make clear we only
      * need the position, typically for error reporting.
      */
    ...
    /** Copy a symbol, overriding selective fields. Note that `coord` and
      * `associatedFile` will be set from the fields in `owner`, not the fields
      * in `sym`.
      */
    def copy(using Context)(
        ...
        associatedFile: AbstractFile | Null =
          null // Can be `= owner.associatedFile` once we bootstrap
    ): Symbol =
      val coord1 = if coord == NoCoord then owner.coord else coord
      val associatedFile1 =
        if associatedFile == null then owner.associatedFile else associatedFile
      ...
      else newSymbol(owner, name, flags, info, privateWithin, coord1)

// -------- Printing --------------------------------------------------------

    /** The prefix string to be used when displaying this symbol without
      * denotation
      */
    ...
    def showExtendedLocation(using Context): String =
      ctx.printer.extendedLocationText(this).show
  ...
  class ClassSymbol private[Symbols] (
      coord: Coord,
      val assocFile: AbstractFile | Null,
      id: Int,
      nestingLevel: Int
    ...
    /** If this is a top-level class and `-Yretain-trees` (or `-from-tasty`) is
      * set. Returns the TypeDef tree (possibly wrapped inside PackageDefs) for
      * this class, otherwise EmptyTree. This will force the info of the class.
      */
    def rootTree(using Context): Tree = rootTreeContaining("")

    /** Same as `tree` but load tree only if `id == ""` or the tree might
      * contain `id`. For Tasty trees this means consulting whether the name
      * table defines `id`. For already loaded trees, we maintain the referenced
      * ids in an attachment.
      */
    ...
    private[dotc] def rootTreeOrProvider_=(t: TreeOrProvider)(using
        Context
    ): Unit =
      myTree = t
        ...
        case None =>
          ...
          tree.foreachSubTree {
            case tree: tpd.NameTree
                if tree.name.toTermName.isInstanceOf[SimpleName] =>
    ...
    /** The source or class file from which this class was generated, null if
      * not applicable.
      */
    override def associatedFile(using Context): AbstractFile | Null =
      if assocFile != null || this.is(Package) || this.owner.is(Package) then
        assocFile
          ...
          if !mySource.exists then
            mySource = atPhaseNoLater(flattenPhase) {
              denot.topLevelClass.unforcedAnnotation(defn.SourceFileAnnot) match
                case Some(sourceAnnot) =>
                  sourceAnnot.argumentConstant(0) match
  ...
  @sharable object NoSymbol extends Symbol(NoCoord, 0, 0):
    override def associatedFile(using Context): AbstractFile | Null =
      NoSource.file
    override def recomputeDenot(lastd: SymDenotation)(using
        Context
    ): SymDenotation = NoDenotation
  ...
  /** The current class */
  def currentClass(using Context): ClassSymbol =
    ctx.owner.enclosingClass.asClass
  ...
  def MutableSymbolMap[T](initialCapacity: Int): EqHashMap[Symbol, T] =
    EqHashMap[Symbol, T](initialCapacity)
...
// ---- Symbol creation methods ----------------------------------
  ...
  ): Symbol { type ThisName = N } =
    val sym = new Symbol(coord, ctx.base.nextSymId, nestingLevel)
      .asInstanceOf[Symbol { type ThisName = N }]
    val denot = SymDenotation(sym, owner, name, flags, info, privateWithin)
    sym.denot = denot
    sym

  /** Create a class symbol from its non-info fields and a function producing
    * its info (the produced info may be lazy).
    */
  def newClassSymbol(
      owner: Symbol,
      name: TypeName,
  ...
  )(using Context): ClassSymbol =
    val cls =
      new ClassSymbol(coord, assocFile, ctx.base.nextSymId, ctx.nestingLevel)
    val denot =
      SymDenotation(cls, owner, name, flags, infoFn(cls), privateWithin)
    cls.denot = denot
    cls
  ...
  /** Same as `newCompleteClassSymbol` except that `parents` can be a list of
    * arbitrary types which get normalized into type refs and parameter
    * bindings.
    */
  ...
  end newNormalizedClassSymbol

  def newRefinedClassSymbol(
      coord: Coord = NoCoord
  )(using Context): ClassSymbol =
  ...
  /** Create a module symbol with associated module class from its non-info
    * fields and a function producing the info of the module class (this info
    * may be lazy).
    */
  def newModuleSymbol(
      owner: Symbol,
      ...
      infoFn: (
          TermSymbol,
          ClassSymbol
      ) => Type, // typically a ModuleClassCompleterWithDecls
      privateWithin: Symbol = NoSymbol,
      coord: Coord = NoCoord,
      assocFile: AbstractFile | Null = null
  )(using Context): TermSymbol =
    ...
    val module = newSymbol(
      owner,
      name,
      modFlags | ModuleValCreationFlags,
      NoCompleter,
      privateWithin,
      coord
    )
    ...
    module.info =
      if modcls.isCompleted then TypeRef(owner.thisType, modcls)
      else new ModuleCompleter(modcls)
    module
  end newModuleSymbol

  /** Create a module symbol with associated module class from its non-info
    * fields and the fields of the module class info.
    * @param flags
    *   The combined flags of the module and the module class These are masked
    *   with RetainedModuleValFlags/RetainedModuleClassFlags.
    */
  ...
  /** Same as `newCompleteModuleSymbol` except that `parents` can be a list of
    * arbitrary types which get normalized into type refs and parameter
    * bindings.
    */
  ...
  end newNormalizedModuleSymbol

  /** Create a package symbol with associated package class from its non-info
    * fields and a lazy type for loading the package's members.
    */
  ...
  def newCompletePackageSymbol(
    ...
    newCompleteModuleSymbol(
      owner,
      name,
      modFlags | PackageCreationFlags,
      clsFlags | PackageCreationFlags,
      Nil,
      decls
    )

  /** Define a new symbol associated with a Bind or pattern wildcard and, by
    * default, make it gadt narrowable.
    */
  def newPatternBoundSymbol(
    ...
    if addToGadt && name.isTypeName then ctx.gadtState.addToConstraint(sym)
    sym

  /** Create a stub symbol that will issue a missing reference error when
    * attempted to be completed.
    */
  def newStubSymbol(
      owner: Symbol,
      name: Name,
      file: AbstractFile | Null = null
  )(using Context): Symbol =
    def stubCompleter = new StubInfo()
    val normalizedOwner =
      if owner.is(ModuleVal) then owner.moduleClass else owner
    typr.println(
      s"creating stub for ${name.show}, owner = ${normalizedOwner.denot.debugString}, file = $file"
    )
    typr.println(
      s"decls = ${normalizedOwner.unforcedDecls.toList.map(_.debugString).mkString("\n  ")}"
    ) // !!! DEBUG
  ...
  /** Create the local template dummy of given class `cls`. In a template
    ...
    *
    * the owner of `x` is the local dummy of the template. The owner of the
    * local dummy is then the class of the template itself. By contrast, the
    * owner of `y` would be `fld2`. There is a single local dummy per template.
    */
  def newLocalDummy(cls: Symbol, coord: Coord = NoCoord)(using
      Context
  ): TermSymbol =
    newSymbol(cls, nme.localDummyName(cls), NonMember, NoType)

  /** Create an import symbol pointing back to given qualifier `expr`. */
  def newImportSymbol(owner: Symbol, expr: Tree, coord: Coord = NoCoord)(using
      Context
  ): TermSymbol =
    newImportSymbol(owner, ImportType(expr), coord = coord)

  /** Create an import symbol with given `info`. */
  def newImportSymbol(owner: Symbol, info: Type, coord: Coord)(using
      Context
  ): TermSymbol =
  ...
  def newConstructor(
    ...
    newSymbol(
      ...
      nme.CONSTRUCTOR,
      flags | Method,
      MethodType(paramNames, paramTypes, cls.typeRef),
      privateWithin,
      coord
    )

  /** Create an anonymous function symbol */
  def newAnonFun(owner: Symbol, info: Type, coord: Coord = NoCoord)(using
      Context
  ): TermSymbol =
  ...
  def newLazyImplicit(info: Type, coord: Coord = NoCoord)(using
      Context
  ): TermSymbol =
    newSymbol(
      ctx.owner,
      LazyImplicitName.fresh(),
      EmptyFlags,
      info,
      coord = coord
    )
  ...
  def newSelfSym(
    ...
    newSymbol(
      ...
      name,
      SelfSymFlags,
      selfInfo orElse cls.classInfo.selfType,
      coord = cls.coord
    )

  /** Create new type parameters with given owner, names, and flags.
    * @param boundsFn
    *   A function that, given type refs to the newly created parameters returns
    *   a list of their bounds.
    */
  def newTypeParams(
      owner: Symbol,
      names: List[TypeName],
    ...
    for name <- names do
      val tparam = newSymbol(
        owner,
        name,
        flags | owner.typeParamCreationFlags,
        NoType,
        coord = owner.coord
      )
      tparamBuf += tparam
    ...
    for (tparam, bound) <- tparams.lazyZip(bounds) do tparam.info = bound
    tparams

  /** Create a new skolem symbol. This is not the same as SkolemType, even
    * though the motivation (create a singleton referencing to a type) is
    * similar.
    */
  def newSkolem(tp: Type)(using Context): TermSymbol =
    newSymbol(
      defn.RootClass,
      nme.SKOLEM,
      SyntheticArtifact | NonMember | Permanent,
      tp
    )

  def newErrorSymbol(owner: Symbol, name: Name, msg: Message)(using
      Context
  ): Symbol =
    ...
    newSymbol(
      ...
      name,
      SyntheticArtifact,
      if name.isTypeName then TypeAlias(errType) else errType
    )

  /** Map given symbols, subjecting their attributes to the mappings defined in
    * the given TreeTypeMap `ttmap`. Cross symbol references are brought over
    * from originals to copies. Do not copy any symbols if all attributes of all
    * symbols stay the same.
    */
  def mapSymbols(
      originals: List[Symbol],
      ttmap: TreeTypeMap,
      mapAlways: Boolean = false
      ...
      originals.lazyZip(copies) foreach { (original, copy) =>
            ...
            val oinfo = original.info match
              case ClassInfo(pre, _, parents, decls, selfInfo) =>
                ...
                if otypeParams.isEmpty then
                  ...
                  val decls1 = newScope
                  val newTypeParams =
                    mapSymbols(original.typeParams, ttmap1, mapAlways = true)
                  newTypeParams.foreach(decls1.enter)
                  for sym <- decls do
                    if !sym.is(TypeParam) then decls1.enter(sym)
                  val parents2 =
                    parents1.map(_.substSym(otypeParams, newTypeParams))
                  val selfInfo1 = selfInfo match
                    case selfInfo: Type =>
                      selfInfo.substSym(otypeParams, newTypeParams)
                    case _ => selfInfo
                  ClassInfo(pre, copy.asClass, parents2, decls1, selfInfo1)
              case oinfo => oinfo

            denot.info =
              oinfo // needed as otherwise we won't be able to go from Sym -> parents & etc
        ...
        copy.denot match
          case cd: ClassDenotation =>
            cd.registeredCompanion =
              original.registeredCompanion.subst(originals, copies)
          case _ =>
      }
  ...
  /** Matches lists of term symbols, including the empty list. All symbols in
    ...
    */
  object TermSymbols:
    def unapply(xs: List[Symbol])(using Context): Option[List[TermSymbol]] =
      xs match
        case (x: Symbol) :: _ if x.isType => None
        case _ => Some(xs.asInstanceOf[List[TermSymbol]])

  /** Matches lists of type symbols, excluding the empty list. All symbols in
    * the list are assumed to be of the same kind.
    */
  object TypeSymbols:
    def unapply(xs: List[Symbol])(using Context): Option[List[TypeSymbol]] =
      xs match
        case (x: Symbol) :: _ if x.isType =>
          Some(xs.asInstanceOf[List[TypeSymbol]])
        case _ => None
...
// ----- Locating predefined symbols ----------------------------------------
  ...
  def requiredPackage(path: PreName)(using Context): TermSymbol =
    ...
    staticRef(name, isPackage = true)
      .requiredSymbol("package", name)(_.is(Package))
      .asTerm

  def requiredPackageRef(path: PreName)(using Context): TermRef =
    requiredPackage(path).termRef
  ...
  def requiredClassRef(path: PreName)(using Context): TypeRef = requiredClass(
    path
  ).typeRef

  /** Get ClassSymbol if class is either defined in current compilation run or
    * present on classpath. Returns NoSymbol otherwise.
    */
  def getClassIfDefined(path: PreName)(using Context): Symbol =
    staticRef(path.toTypeName, generateStubs = false)
      .disambiguate(_.isClass)
      .symbol

  /** Get a List of ClassSymbols which are either defined in current compilation
    * run or present on classpath.
    */
  def getClassesIfDefined(
      paths: List[PreName]
  )(using Context): List[ClassSymbol] =
    paths
      .map(getClassIfDefined)
      .filter(_.exists)
      .map(_.asInstanceOf[ClassSymbol])

  /** Get ClassSymbol if package is either defined in current compilation run or
    * present on classpath. Returns NoSymbol otherwise.
    */
  def getPackageClassIfDefined(path: PreName)(using Context): Symbol =
    staticRef(path.toTypeName, isPackage = true, generateStubs = false)
      .disambiguate(_ is PackageClass)
      .symbol
  ...
  /** Get module symbol if the module is either defined in current compilation
    * run or present on classpath. Returns NoSymbol otherwise.
    */
  def getModuleIfDefined(path: PreName)(using Context): Symbol =
    staticRef(path.toTermName, generateStubs = false)
      .disambiguate(_.is(Module))
      .symbol

  def requiredModuleRef(path: PreName)(using Context): TermRef = requiredModule(
    path
  ).termRef
  ...
  def requiredMethodRef(path: PreName)(using Context): TermRef = requiredMethod(
    path
  ).termRef
end Symbols
