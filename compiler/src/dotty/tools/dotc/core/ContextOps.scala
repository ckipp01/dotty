...
/** Extension methods for contexts where we want to keep the ctx.<methodName>
  * syntax
  */
object ContextOps:
  ...
  extension (ctx: Context)
    ...
    /** Enter symbol into current class, if current class is owner of current
      * context, or into current scope, if not. Should always be called instead
      * of scope.enter in order to make sure that updates to class members are
      * reflected in finger prints.
      */
    ...
    /** The denotation with the given `name` and all `required` flags in current
      * context
      */
    def denotNamed(
        name: Name,
      ...
      inContext(ctx) {
        if ctx.owner.isClass then
          if ctx.outer.owner == ctx.owner
          then // inner class scope; check whether we are referring to self
            if ctx.scope.size == 1 then
              val elem = ctx.scope.lastEntry.nn
              if elem.name == name then return elem.sym.denot // return self
          ...
          ctx.scope
            .denotsNamed(name)
            .filterWithFlags(required, excluded)
            .toDenot(NoPrefix)
      }

    final def javaFindMember(
        name: Name,
        pre: Type,
        required: FlagSet = EmptyFlags,
        excluded: FlagSet = EmptyFlags
      ...
      inContext(ctx) {
        ...
        def searchCompanionClass = if preSym.is(Flags.Module) then
          preSym.companionClass.thisType.findMember(
            name,
            pre,
            required,
            excluded
    ...
    /** A fresh local context with given tree and owner. Owner might not exist
      * (can happen for self valdefs), in which case no owner is set in result
      * context
      */
    ...
    def packageContext(tree: untpd.PackageDef, pkg: Symbol): Context =
      inContext(ctx) {
        if pkg.is(Package) then
          ctx.fresh.setOwner(pkg.moduleClass).setTree(tree)
        else ctx
      }
  end extension
end ContextOps
