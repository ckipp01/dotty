...
/** Constraint over undetermined type parameters. Constraints are built over
  * values of the following types:
  *
  *   - TypeLambda A constraint constrains the type parameters of a set of
  *     TypeLambdas
...
abstract class Constraint extends Showable:
  ...
  /** Does this constraint contain the type variable `tvar` and is it
    * uninstantiated?
    */
  def contains(tvar: TypeVar): Boolean

  /** The constraint entry for given type parameter `param`, or NoType if
    * `param` is not part of the constraint domain. Note: Low level,
    * implementation dependent.
    */
  def entry(param: TypeParamRef): Type

  /** The type variable corresponding to parameter `param`, or NoType, if
  ...
  /** The lower dominator set.
    *
    * This is like `lower`, except that each parameter returned is no smaller
    * than every other returned parameter.
    */
  def minLower(param: TypeParamRef): List[TypeParamRef]

  /** The upper dominator set.
    *
    * This is like `upper`, except that each parameter returned is no greater
    * than every other returned parameter.
    */
  ...
  /** The constraint bounds for given type parameter `param`. Poly params that
    * are known to be smaller or greater than `param` are not contained in the
    * return bounds.
    * @pre
    *   `param` is not part of the constraint domain.
    */
  ...
  /** A new constraint which is derived from this constraint by adding entries
    * for all type parameters of `poly`.
    * @param tvars
    *   A list of type variables associated with the params, or Nil if the
    *   constraint will just be checked for satisfiability but will solved to
    *   give instances of type variables.
    */
  def add(poly: TypeLambda, tvars: List[TypeVar])(using Context): This

  /** A new constraint which is derived from this constraint by updating the
    * entry for parameter `param` to `tp`. `tp` can be one of the following:
    *
    ...
    *   `this contains param`.
    * @pre
    *   `tp` does not contain top-level references to `param` (see
    *   `validBoundsFor`)
    */
  def updateEntry(param: TypeParamRef, tp: Type)(using Context): This

  /** A constraint that includes the relationship `p1 <: p2`. `<:` relationships
    * between parameters ("edges") are propagated, but non-parameter bounds are
    * left alone.
    *
    * @param direction
    *   Must be set to `KeepParam1` or `KeepParam2` when `p2 <: p1` is already
    *   true depending on which parameter the caller intends to keep. This will
    *   avoid propagating bounds that will be redundant after `p1` and `p2` are
    *   unified.
    */
  def addLess(
      p1: TypeParamRef,
      p2: TypeParamRef,
      direction: UnificationDirection = UnificationDirection.NoUnification
  )(using Context): This
  ...
  /** A new constraint which is derived from this constraint by removing the
    ...
    * occurrences of the parameter elsewhere in the constraint by type `tp`, or
    * a conservative approximation of it if that is needed to avoid cycles.
  ...
  /** Is entry associated with `tl` removable? This is the case if all type
    * parameters of the entry are associated with type variables which have
    * their `inst` fields set.
    */
  ...
  /** A new constraint with entry `from` replaced with `to` Rerences to `from`
    * from within other constraint bounds are updated to `to`. Type variables
    * are left alone.
    */
  ...
  /** Gives for each instantiated type var that does not yet have its `inst`
    * field set, the instance value stored in the constraint. Storing instances
    * in constraints is done only in a temporary way for contexts that may be
    * retracted without also retracting the type var as a whole.
    */
  def instType(tvar: TypeVar): Type

  /** The given `tl` in case it is not contained in this constraint, a fresh
    * copy of `tl` otherwise.
    */
  ...
  /** Perform operation `op` on all typevars that do not have their `inst` field
    * set.
    */
  def foreachTypeVar(op: TypeVar => Unit): Unit

  /** The uninstantiated typevars of this constraint, which still have a bounds
    * constraint
    */
  ...
  /** Does `param` occur at the toplevel in `tp` ? Toplevel means: the type
    * itself or a factor in some combination of `&` or `|` types.
    */
  ...
  /** Does the constraint restricted to variables outside `except` depend on
    * `tv` in the given direction `co`?
    * @param `co`
    *   If true, test whether the constraint would change if the variable is
    *   made larger otherwise, test whether the constraint would change if the
    *   variable is made smaller.
    */
  def dependsOn(tv: TypeVar, except: TypeVars, co: Boolean)(using
      Context
  ): Boolean

  /** Depending on Config settngs:
    *   - Under `checkConstraintsNonCyclic`, check that no constrained parameter
    *     contains itself as a bound.
