...
object Scala2Erasure:
  /** Is this a supported Scala 2 refinement or parent of such a type?
    *
    * We do not allow types that look like: ((A with B) @foo) with C or: (A {
    * type X <: ... })#X with C`
    *
  ...
  def checkSupported(tp: Type)(using Context): Unit = tp match
    ...
    case AnnotatedType(parent, _)
        if parent.dealias.isInstanceOf[Scala2RefinedType] =>
      throw TypeError(
        em"Unsupported Scala 2 type: Component $parent of intersection is annotated."
      )
    case tp @ TypeRef(prefix, _)
        if !tp.symbol.exists && prefix.dealias
          .isInstanceOf[Scala2RefinedType] =>
      throw TypeError(
        em"Unsupported Scala 2 type: Prefix $prefix of intersection component is an intersection or refinement."
      )
    case _ =>

  /** A type that would be represented as a RefinedType in Scala 2.
    *
    * The `RefinedType` of Scala 2 contains both a list of parents and a list of
    * refinements, intersections are represented as a RefinedType with no
    * refinements.
    */
  ...
  /** The equivalent of a Scala 2 type symbol.
    ...
    *
    * To emulate this, we simply use the type itself as a stand-in for its
    * symbol.
    *
    * See also `sameSymbol` which determines if two pseudo-symbols are really
    * the same.
    */
  ...
  /** The pseudo symbol of `tp`, see `PseudoSymbol`.
    ...
    * `isNonBottomSubClass` behaves like it would in Scala 2, in particular this
    * lets us strip all aliases.
    */
  def pseudoSymbol(tp: Type)(using Context): PseudoSymbol =
  ...
  extension (psym: PseudoSymbol)(using Context)
    /** Would these two pseudo-symbols be represented with the same symbol in
      * Scala 2?
      */
    def sameSymbol(other: PseudoSymbol): Boolean =
      // Pattern match on (psym1, psym2) desugared by hand to avoid allocating a tuple
      if psym.isInstanceOf[StructuralRef] && other.isInstanceOf[StructuralRef]
      then
        ...
        (tp1.name eq tp2.name) && pseudoSymbol(tp1.prefix).sameSymbol(
          pseudoSymbol(tp2.prefix)
        )
      else
    ...
    /** Is this a class symbol? Also returns true for refinements since they get
      * a class symbol in Scala 2.
      */
    ...
    /** An emulation of `Symbol#isNonBottomSubClass` from Scala 2.
      ...
      *
      * > Is this class symbol a subclass of that symbol, > and is this class
      * symbol also different from Null or Nothing?
      *
      * Which sounds fine, except that it is also used with non-class symbols,
      * so what does it do then? Its implementation delegates to
      * `Type#baseTypeSeq` whose documentation states:
      *
      * > The base type sequence of T is the smallest set of [...] class types
      * Ti, so that [...]
      *
        ...
        psym.sameSymbol(that) ||
          ...
          psym.match
            case sym1: Symbol =>
              that match
                case sym2: Symbol =>
                  if sym1.isClass && sym2.isClass then sym1.derivesFrom(sym2)
  ...
  def flattenedParents(tp: AndType)(using Context): List[Type] =
    ...
    def collect(parent: Type, parents: ListBuffer[Type]): Unit =
      parent.dealiasKeepAnnots match
        case AndType(tp1, tp2) =>
          collect(tp1, parents)
          collect(tp2, parents)
        case _ =>
