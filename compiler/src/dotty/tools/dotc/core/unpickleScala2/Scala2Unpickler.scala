...
import Contexts.*, Symbols.*, Types.*, Scopes.*, SymDenotations.*, Names.*,
  NameOps.*
import StdNames.*, Denotations.*, NameOps.*, Flags.*, Constants.*,
  Annotations.*, Phases.*
...
object Scala2Unpickler:
  ...
  case class TempPolyType(tparams: List[TypeSymbol], tpe: Type)
      extends UncachedGroundType:
    override def fallbackToText(printer: Printer): Text =
      "[" ~ printer.dclsText(tparams, ", ") ~ "]" ~ printer.toText(tpe)

  /** Temporary type for classinfos, will be decomposed on completion of the
    * class
    */
  ...
  def translateTempPoly(tp: Type)(using Context): Type = tp match
    case TempPolyType(tparams, restpe) =>
      ...
      (if tparams.head.owner.is(Method) then PolyType else HKTypeLambda)
  ...
  def ensureConstructor(
      cls: ClassSymbol,
      clsDenot: ClassDenotation,
      scope: Scope
  )(using Context): Unit =
  ...
  def setClassInfo(
    ...
    val ost =
      if (selfInfo eq NoType) && denot.is(ModuleClass) then
        val sourceModule = denot.sourceModule.orElse {
          ...
          val alternate =
            if cls.privateWithin.exists && cls.owner.is(Trait) then
              modName.expandedName(cls.owner)
            else EmptyTermName
    ...
    val parents1 =
      if parents.isEmpty then defn.ObjectType :: Nil else parents.map(_.dealias)
    ...
    val normalizedParents =
      if fromScala2 then defn.adjustForTuple(cls, tparams, parents1)
      else defn.adjustForBoxedUnit(cls, parents1)
    for tparam <- tparams do
      val tsym = decls.lookup(tparam.name)
      if tsym.exists then tsym.setFlag(TypeParam)
      else denot.enter(tparam, decls)
    if !denot.flagsUNSAFE.isAllOf(JavaModule) then
      ensureConstructor(cls, denot, decls)
    ...
    if denot.flagsUNSAFE.is(Module) then
      registerCompanionPair(denot.classSymbol, scalacCompanion)
    else registerCompanionPair(scalacCompanion, denot.classSymbol)
...
class Scala2Unpickler(
  ...
  def showPickled(): Unit =
    atReadPos(
      ...
      () =>
        println(
          s"classRoot = ${classRoot.debugString}, moduleClassRoot = ${moduleClassRoot.debugString}"
        )
  ...
  val moduleRoot: SymDenotation = inContext(ictx) {
    moduleClassRoot.sourceModule.denot
  }
  ...
  private val loadingMirror = defn(using
    ictx
  ) // was: mirrorThatLoaded(classRoot)
  ...
  private val paramsOfMethodType =
    new java.util.IdentityHashMap[MethodType, List[Symbol]]

  protected def errorBadSignature(
      msg: String,
      original: Option[RuntimeException] = None
  )(using Context): Nothing =
    val ex = new BadSignature(
      i"""error reading Scala signature of $classRoot from $source:
         |error occurred at position $readIndex: $msg"""
    )
    if ctx.settings.YdebugMissingRefs.value then
      original.getOrElse(ex).printStackTrace()
    throw ex

  protected def handleRuntimeException(ex: RuntimeException)(using
      Context
  ): Nothing = ex match
  ...
  def run()(using Context): Unit =
    try
      ...
      while i < index.length do
        if entries(i) == null && isSymbolEntry(i) then
      ...
      while i < index.length do
        if entries(i) == null then
          if isSymbolAnnotationEntry(i) then
            val savedIndex = readIndex
            readIndex = index(i)
            readSymbolAnnotation()
            readIndex = savedIndex
          else if isChildrenEntry(i) then
            val savedIndex = readIndex
            readIndex = index(i)
            readChildren()
            readIndex = savedIndex
        i += 1
    catch case ex: RuntimeException => handleRuntimeException(ex)

  def source(using Context): AbstractFile =
    val f = classRoot.symbol.associatedFile
    if f != null then f else moduleClassRoot.symbol.associatedFile
  ...
  private def checkVersion(using Context): Unit =
    ...
    if major != MajorVersion || minor > MinorVersion then
      throw new IOException(
        "Scala signature " + classRoot.fullName +
          " has wrong version\n expected: " +
  ...
  private def checkScala2Stdlib(using Context): Unit =
    assert(
      ...
      "No Scala 2 libraries should be unpickled under -Yscala2-stdlib"
    )

  /** The `decls` scope associated with given symbol */
  protected def symScope(sym: Symbol): Scope =
    symScopes.getOrElseUpdate(sym, newScope(0))
  ...
  protected def isLocal(sym: Symbol)(using Context): Boolean = isUnpickleRoot(
    sym.topLevelClass
  )
  ...
  /** If entry at <code>i</code> is undefined, define it by performing operation
    * <code>op</code> with <code>readIndex at start of i'th entry. Restore
    * <code>readIndex</code> afterwards.
    */
  ...
  /** Read a symbol */
  protected def readSymbol()(using Context): Symbol =
    readDisambiguatedSymbol(alwaysTrue)()

  /** Read a symbol, with possible disambiguation */
  protected def readDisambiguatedSymbol(p: Symbol => Boolean)()(using
      Context
  ): Symbol =
    ...
    def readExtSymbol(): Symbol =
      ...
      if tag != EXTMODCLASSref then name = name.unmangle(Scala2MethodNameKinds)

      val owner = if atEnd then loadingMirror.RootClass else readSymbolRef()

      def adjust(denot: Denotation) =
        val denot1 = denot.disambiguate(p)
        val sym = denot1.symbol
        if denot.exists && !denot1.exists then // !!!DEBUG
          val alts =
            denot.alternatives map (d => s"$d:${d.info}/${d.signature}")
          System.err.println(s"!!! disambiguation failure: $alts")
          val members =
            denot.alternatives.head.symbol.owner.info.decls.toList map (d =>
              s"$d:${d.info}/${d.signature}"
            )
          System.err.println(s"!!! all members: $members")
        if tag == EXTref then sym else sym.moduleClass
      ...
      def fromName(name: Name): Symbol = name.toTermName match
        ...
        case _ =>
          ...
          if sym.exists || owner.ne(defn.ObjectClass) then sym
          else declIn(defn.AnyClass)
        ...
        fromName(name.toTermName.expandedName(owner)) orElse {
          ...
          nestedObjectSymbol orElse {
            ...
            adjust(missingHook(owner, name)) orElse {
              ...
              System.err.println(
                i"***** missing reference, looking for ${name.debugString} in $owner"
              )
              System.err.println(i"decls = ${owner.info.decls}")
              owner.info.decls.checkConsistent()
              if slowSearch(name).exists then
                System.err.println(
                  i"**** slow search found: ${slowSearch(name)}"
                )
              if ctx.settings.YdebugMissingRefs.value then Thread.dumpStack()
    ...
    if (name eq nme.getClass_) && defn.hasProblematicGetClass(owner.name)
      ...
      || tag == TYPEsym && flags
        .is(TypeParam) && symScope(owner).lookup(name.asTypeName).exists
    then
      // skip this member
      return NoSymbol
    ...
    def completeRoot(
      ...
      if owner == defn.ScalaPackageClass && (name eq tpnme.Product) then
    ...
    def finishSym(sym: Symbol): Symbol =
      if sym.isClass then sym.setFlag(Scala2x)
      if !(isRefinementClass(sym) || isUnpickleRoot(sym) || sym.is(
          Scala2Existential
        ))
      then
        val owner = sym.owner
        ...
        if canEnter then owner.asClass.enter(sym, symScope(owner))
      sym
    end finishSym
    ...
    finishSym(tag match
      case TYPEsym | ALIASsym =>
        ...
        var flags1 = flags
        if flags.is(TypeParam) then flags1 |= owner.typeParamCreationFlags
        newSymbol(
          owner,
          name1,
          flags1,
      ...
      case CLASSsym =>
        if isClassRoot then
          completeRoot(
            classRoot,
            rootClassUnpickler(start, classRoot.symbol, NoSymbol, infoRef),
            privateWithin
          )
        else if isModuleClassRoot then
          completeRoot(
            moduleClassRoot,
            rootClassUnpickler(
              start,
              moduleClassRoot.symbol,
              moduleClassRoot.sourceModule,
              infoRef
            ),
            privateWithin
          )
          ...
          def completer(cls: Symbol) =
            ...
            if flags.is(ModuleClass) then
              unpickler.withSourceModule(
                cls.owner.info.decls
                  .lookup(cls.name.sourceModuleName)
                  .suchThat(_.is(Module))
                  .symbol
              )
            else unpickler
          newClassSymbol(
  ...
  class LocalUnpickler extends LazyType:
    ...
    def complete(denot: SymDenotation)(using Context): Unit = try
      def parseToCompletion(denot: SymDenotation)(using Context) =
        ...
        var inforef = readNat()
        if isSymbolRef(inforef) then inforef = readNat()
        ...
        denot match
          case denot: ClassDenotation if !isRefinementClass(denot.symbol) =>
            val selfInfo = if atEnd then NoType else readTypeRef()
          ...
          case denot =>
            ...
            denot.info =
              if tag == ALIASsym then TypeAlias(tp1)
              else if denot.isType then
                checkNonCyclic(denot.symbol, tp1, reportErrors = false)
              // we need the checkNonCyclic call to insert LazyRefs for F-bounded cycles
              else if !denot.is(Param) then
                tp1.translateFromRepeated(toArray = false)
              else tp1
            if denot.isConstructor then
            ...
            if atEnd then assert(!denot.symbol.isSuperAccessor, denot)
              ...
              def disambiguate(alt: Symbol) = // !!! DEBUG
                trace.onDebug(
                  s"disambiguating ${denot.info} =:= ${denot.owner.thisType
                      .memberInfo(alt)} ${denot.owner}"
                ) {
  ...
  class ClassUnpickler(infoRef: Int)
      extends LocalUnpickler
    ...
    private def readTypeParams()(using Context): Unit =
      val tag = readByte()
      val end = readNat() + readIndex
      myTypeParams = if tag == POLYtpe then
        val unusedRestpeRef = readNat()
        until(end, () => readSymbolRef()(using ctx))
    ...
    /** Force reading type params early, we need them in setClassInfo of
      * subclasses.
      */
  ...
  /** Convert tp { type name = sym } forSome { sym >: L <: H } to tp { name >: L
    * <: H } and tp { name: sym } forSome { sym <: T with Singleton } to tp {
    * name: T }
    */
  def elimExistentials(boundSyms: List[Symbol], tp: Type)(using Context): Type =
    ...
    val isBoundAccumulator =
      new ExistsAccumulator(isBound, StopAt.Static, forceLazy = true):
    ...
    def elim(tp: Type): Type = tp match
        ...
        rinfo match
          ...
          case info: TypeRef if isBound(info) =>
            ...
            RefinedType(
              parent1,
              name,
              info1.mapReduceAnd(removeSingleton)(_ & _)
            )
          case info =>
            tp.derivedRefinedType(parent1, name, info)
      case tp @ AppliedType(tycon, args) =>
        val tycon1 = tycon.safeDealias
        if tycon1 ne tycon then elim(tycon1.appliedTo(args))
      ...
      case _ =>
        tp
    val tp1 = elim(tp)
    if isBoundAccumulator(false, tp1) then
  ...
  /** Read type ref, mapping a TypeRef to a package to the package's ThisType
    * Package references should be TermRefs or ThisTypes but it was observed
    * that nsc sometimes pickles them as TypeRefs instead.
    */
  ...
  /** Read a type
    *
    * @param forceProperType
    *   is used to ease the transition to NullaryMethodTypes (commentmarker:
    *   NMT_TRANSITION) the flag say that a type of kind * is expected, so that
    *   PolyType(tps, restpe) can be disambiguated to PolyType(tps,
    *   NullaryMethodType(restpe)) (if restpe is not a ClassInfoType, a
    *   MethodType or a NullaryMethodType, which leaves TypeRef/SingletonType --
    *   the latter would make the polytype a type constructor)
    */
  protected def readType()(using Context): Type =
    def select(pre: Type, sym: Symbol): Type =
      ...
      if !pre.isInstanceOf[ThisType] && isRefinementClass(sym.owner) then
        pre.select(sym.name)
      else pre.select(sym)
    ...
    (tag: @switch) match
      ...
      case TYPEREFtpe =>
        ...
        pre match
          case thispre: ThisType =>
            ...
            if sym.owner != thispre.cls then
              val overriding = thispre.cls.info.decls.lookup(sym.name)
              if overriding.exists && overriding != sym then
        ...
        if sym == defn.ByNameParamClass2x then
          ExprType(args.head.adaptByNameArgUnderPureFuns)
        ...
        then
          ...
          tycon
            .safeAppliedTo(
              EtaExpandIfHK(sym.typeParams, args.map(translateTempPoly))
            )
            .adaptFunctionTypeUnderPureFuns
        else if sym.typeParams.nonEmpty then tycon.EtaExpand(sym.typeParams)
        else tycon
      case TYPEBOUNDStpe =>
        val lo = readTypeRef()
        val hi = readTypeRef()
      ...
      case METHODtpe | IMPLICITMETHODtpe =>
        ...
        val maker = MethodType.companion(
          isImplicit =
            tag == IMPLICITMETHODtpe || params.nonEmpty && params.head.is(
              Implicit
            )
        )
  ...
  protected def readSymbolRef()(using
      Context
  ...
  protected def readDisambiguatedSymbolRef(p: Symbol => Boolean)(using
      Context
  ): Symbol =
    at(readNat(), () => readDisambiguatedSymbol(p)())

  protected def readNameRef()(using Context): Name =
    at(readNat(), () => readName())
  protected def readTypeRef()(using Context): Type = at(
    readNat(),
    () => readType()
  ) // after the NMT_TRANSITION period, we can leave off the () => ... ()
  protected def readConstantRef()(using Context): Constant | TermRef =
    at(readNat(), () => readConstant())

  protected def readTypeNameRef()(using Context): TypeName =
    readNameRef().toTypeName
  protected def readTermNameRef()(using Context): TermName =
    readNameRef().toTermName

  protected def readAnnotationRef()(using Context): Annotation =
    at(readNat(), () => readAnnotation())

  protected def readModifiersRef(isType: Boolean)(using Context): Modifiers =
    at(readNat(), () => readModifiers(isType))
  protected def readTreeRef()(using Context): Tree =
    at(readNat(), () => readTree())

  /** Read an annotation argument, which is pickled either as a Constant or a
    * Tree.
    */
  protected def readAnnotArg(i: Int)(using Context): untpd.Tree =
    untpd.TypedSplice(bytes(index(i)) match
  ...
  private def readAnnotInfoArg()(using Context): untpd.Tree =
    untpd.TypedSplice {
  ...
  protected def readClassfileAnnotArg(i: Int)(using Context): untpd.Tree =
    bytes(index(i)) match
  ...
  /** Read an annotation and as a side effect store it into the symbol it
    * requests. Called at top-level, for all (symbol, annotInfo) entries.
    */
  protected def readSymbolAnnotation()(using Context): Unit =
    val tag = readByte()
    if tag != SYMANNOT then
      errorBadSignature("symbol annotation expected (" + tag + ")")
    val end = readNat() + readIndex
    val target = readSymbolRef()
    target.addAnnotation(deferredAnnot(end))

  /** Read an annotation and return it. Used when unpickling an
    * ANNOTATED(WSELF)tpe or a NestedAnnotArg
    */
  protected def readAnnotation()(using Context): Annotation =
    val tag = readByte()
    if tag != ANNOTINFO then
      errorBadSignature("annotation expected (" + tag + ")")
    val end = readNat() + readIndex
    deferredAnnot(end)

  /** A deferred annotation that can be completed by reading the bytes between
    * `readIndex` and `end`.
    */
  ...
  protected def readTree()(using Context): Tree =
    ...
    if outerTag != TREE then
      errorBadSignature("tree expected (" + outerTag + ")")
    val end = readNat() + readIndex
    val tag = readByte()
    val tpe = if tag == EMPTYtree then NoType else readTypeRef()
    ...
    tag match
      ...
      case DEFDEFtree =>
        ...
        val vparamss =
          times(readNat(), () => times(readNat(), () => readValDefRef()))
        val tpt = readTreeRef()
        val rhs = readTreeRef()
      ...
      case LABELtree =>
        ...
        if isCaseLabel(symbol) then ldef
      ...
      case TEMPLATEtree =>
        ...
        untpd
          .Template(
            ...
            self,
            body
          ) // !!! TODO: pull out primary constructor
      ...
      case FUNCTIONtree =>
        ...
        val applyType =
          MethodType(vparams map (_.name), vparams map (_.tpt.tpe), body.tpe)
        val applyMeth = newSymbol(symbol.owner, nme.apply, Method, applyType)
        Closure(
          applyMeth,
          Function.const(body.changeOwner(symbol, applyMeth)) _
        )
