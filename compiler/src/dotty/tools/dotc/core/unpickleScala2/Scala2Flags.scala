...
/** Scala2 flags, adapted from
  * https://github.com/scala/scala/blob/2.11.x/src/reflect/scala/reflect/internal/Flags.scala
  */
object Scala2Flags:
  ...
  inline val INTERFACE =
    1 << 7 // symbol is an interface (i.e. a trait which defines only abstract methods)
  ...
  inline val LAZY =
    1L << 31 // symbol is a lazy val. can't have MUTABLE unless transformed by typer
  inline val PRESUPER = 1L << 37 // value is evaluated before super call
  inline val DEFAULTINIT =
    1L << 41 // symbol is initialized to the default value: used by -Xcheckinit
  inline val ARTIFACT =
  ...
  inline val CAPTURED =
    1 << 16 // variable is accessed from nested function.  Set by LambdaLift.
  inline val LABEL = 1 << 17 // method symbol is a label. Set by TailCall
  inline val INCONSTRUCTOR =
    1 << 17 // class symbol is defined in this/superclass constructor.
  inline val SYNTHETIC =
    1 << 21 // symbol is compiler-generated (compare with ARTIFACT)
  ...
  inline val ACCESSOR =
    1 << 27 // a value or variable accessor (getter or setter)

  inline val SUPERACCESSOR = 1 << 28 // a super accessor
  inline val MODULEVAR =
  ...
  inline val EXISTENTIAL =
    1L << 35 // type is an existential parameter or skolem
  inline val EXPANDEDNAME = 1L << 36 // name has been expanded with class suffix
  inline val TRANS_FLAG =
  ...
  inline val SYNCHRONIZED =
    1L << 45 // symbol is a method which should be marked ACC_SYNCHRONIZED
