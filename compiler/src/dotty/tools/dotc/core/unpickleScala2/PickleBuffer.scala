...
class PickleBuffer(data: Array[Byte], from: Int, to: Int):
  ...
  def ensureCapacity(capacity: Int): Unit =
    while bytes.length < writeIndex + capacity do dble()

  // -- Basic output routines --------------------------------------------

  /** Write a byte of data */
  def writeByte(b: Int): Unit =
    if writeIndex == bytes.length then dble()
    bytes(writeIndex) = b.toByte
    writeIndex += 1

  /** Write a natural number in big endian format, base 128. All but the last
    * digits have bit 0x80 set.
    */
  def writeNat(x: Int): Unit =
    writeLongNat(x.toLong & 0x00000000ffffffffL)

  /** Like writeNat, but for longs. This is not the same as writeLong, which
    * writes in base 256. Note that the binary representation of LongNat is
    * identical to Nat if the long value is in the range Int.MIN_VALUE to
    * Int.MAX_VALUE.
    */
  def writeLongNat(x: Long): Unit =
    def writeNatPrefix(x: Long): Unit =
      val y = x >>> 7
      if y != 0L then writeNatPrefix(y)
      writeByte(((x & 0x7f) | 0x80).toInt)
    val y = x >>> 7
    if y != 0L then writeNatPrefix(y)
    writeByte((x & 0x7f).toInt)

  /** Write a natural number <code>x</code> at position <code>pos</code>. If
    * number is more than one byte, shift rest of array to make space.
    *
    * @param pos
    *   ...
    * @param x
  ...
  /** Read a natural number in big endian format, base 128. All but the last
    * digits have bit 0x80 set.
    */
  ...
  /** Returns the buffer as a sequence of (Int, Array[Byte]) representing (tag,
    * data) of the individual entries. Saves and restores buffer state.
    */
  ...
  /** Perform operation <code>op</code> until the condition <code>readIndex ==
    * end</code> is satisfied. Concatenate results into a list.
    *
    * @param end
    *   ...
    * @param op
  ...
  /** Perform operation <code>op</code> the number of times specified.
    * Concatenate the results into a list.
    */
  def times[T](n: Int, op: () => T): List[T] =
    if n == 0 then List() else op() :: times(n - 1, op)

  /** Pickle = majorVersion_Nat minorVersion_Nat nbEntries_Nat {Entry} Entry =
    * type_Nat length_Nat [actual entries]
    *
    * Assumes that the ..Version_Nat are already consumed.
    *
    * @return
    *   an array mapping entry numbers to locations in the byte array where the
    *   entries start.
    */
...
object PickleBuffer:
  ...
  private val (scalaTermFlagMap, scalaTypeFlagMap) =
    ...
    for idx <- 0 until ScalaFlagEnd do
      corr get (1L << idx) match
        case Some(
              (termFlag: FlagSet @unchecked, typeFlag: FlagSet @unchecked)
            ) =>
    ...
    def chunkMap(xs: Array[Long]): FlagMap =
      ...
      while i < size do
        ...
        while j < ChunkSize do
          var k = 0
          while k < ChunkBits do
            if (j & (1 << k)) != 0 then chunked(i)(j) |= xs(i * ChunkBits + k)
            k += 1
          j += 1
        i += 1
      chunked
  ...
  def unpickleScalaFlags(sflags: Long, isType: Boolean): FlagSet =
    val map: FlagMap = if isType then scalaTypeFlagMap else scalaTermFlagMap
    val shift = ChunkBits
