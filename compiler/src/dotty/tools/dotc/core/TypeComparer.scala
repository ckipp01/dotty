...
import Types.*, Contexts.*, Symbols.*, Flags.*, Names.*, NameOps.*,
  Denotations.*
import Decorators.*
...
/** Provides methods to compare types.
  */
class TypeComparer(@constructorOnly initctx: Context)
    extends ConstraintHandling,
  ...
  private var canCompareAtoms: Boolean =
    true // used for internal consistency checking
  ...
  /** Is a subtype check in progress? In that case we may not permanently
    * instantiate type variables, because the corresponding constraint might
    * still be retracted and the instantiation should then be reversed.
    */
  ...
  /** For statistics: count how many isSubTypes are part of successful
    * comparisons
    */
  ...
  /** Record that GADT bounds of `sym` were used in a subtype check. But exclude
    * constructor type parameters, as these are aliased to the corresponding
    * class parameters, which does not constitute a true usage of a GADT symbol.
    */
  private def GADTusage(sym: Symbol): true = recordGadtUsageIf(
    !sym.owner.isConstructor
  )

  private def recordGadtUsageIf(cond: Boolean): true =
    if cond then GADTused = true
    true
  ...
  protected def gadtAddBound(sym: Symbol, b: Type, isUpper: Boolean): Boolean =
    ctx.gadtState.addBound(sym, b, isUpper)
  ...
  def topLevelSubType(tp1: Type, tp2: Type): Boolean =
    if tp2 eq NoType then return false
    if (tp2 eq tp1) || (tp2 eq WildcardType) then return true
  ...
  /** The original left-hand type of the comparison. Gets reset every time we
    * compare components of the previous pair of types. This type is used for
    * capture conversion in `isSubArgs`.
    */
  ...
  /** A map that records successful isSameType comparisons. Used together with
    * `sameLevel` to avoid exponential blowUp of isSameType comparisons for
    * deeply nested invariant applied types.
    */
  private var sames: util.EqHashMap[Type, Type] | Null = null

  /** The `sameLevel` nesting depth from which on we want to keep track of
    * isSameTypes suucesses using `sames`
    */
  ...
  private inline def inFrozenGadtIf[T](cond: Boolean)(inline op: T): T =
    ...
    frozenGadt ||= cond
    try op
    finally frozenGadt = savedFrozenGadt
  ...
  private inline def comparingTypeLambdas(tl1: TypeLambda, tl2: TypeLambda)(
      op: => Boolean
  ): Boolean =
  ...
  protected def isSubType(tp1: Type, tp2: Type, a: ApproxState): Boolean =
    ...
    try recur(tp1, tp2)
      ...
      case ex: Throwable =>
        handleRecursive("subtype", i"$tp1 <:< $tp2", ex, weight = 2)
    finally
      this.approx = savedApprox
      this.leftRoot = savedLeftRoot

  def isSubType(tp1: Type, tp2: Type): Boolean =
    isSubType(tp1, tp2, ApproxState.Fresh)

  override protected def isSub(tp1: Type, tp2: Type)(using Context): Boolean =
    isSubType(tp1, tp2)

  /** The inner loop of the isSubType comparison. Recursive calls from recur
    * should go to recur directly if the two types compared in the callee are
    * essentially the same as the types compared in the caller. "The same"
    * means: represent essentially the same sets of values. `recur` should not
    * be used to compare components of types. In this case one should use
    * `isSubType(_, _)`. `recur` should also not be used to compare approximated
    * versions of the original types (as when we go from an abstract type to one
    * of its bounds). In that case one should use `isSubType(_, _, a)` where `a`
    * defines the kind of approximation.
    *
    * Note: Logically, `recur` could be nested in `isSubType`, which would avoid
    * the instance state consisting `approx` and `leftRoot`. But then the
    * implemented code would have two extra parameters for each of the many
    * calls that go from one sub-part of isSubType to another.
    */
  protected def recur(tp1: Type, tp2: Type): Boolean =
    trace(s"isSubType ${traceInfo(tp1, tp2)}${approx.show}", subtyping) {

      def monitoredIsSubType =
        if pendingSubTypes == null then
          pendingSubTypes = util.HashSet[(Type, Type)]()
          report.log(
            s"!!! deep subtype recursion involving ${tp1.show} <:< ${tp2.show}, constraint = ${state.constraint.show}"
          )
          ...
          if Config.traceDeepSubTypeRecursions && !this
              .isInstanceOf[ExplainingTypeComparer]
          then report.log(explained(_.isSubType(tp1, tp2, approx)))
        ...
        !pendingSubTypes.nn.contains(p) && {
          try
            ...
            firstTry
          finally pendingSubTypes.nn -= p
        }
      end monitoredIsSubType
      ...
      def firstTry: Boolean = tp2 match
        case tp2: NamedType =>
          def compareNamed(tp1: Type, tp2: NamedType): Boolean =
            ...
            /** Does `tp2` have a stable prefix? If that's not the case,
              * following an alias via asSeenFrom could be lossy so we should
              * not conclude `false` if comparing aliases fails. See
              * pos/i17064.scala for a test case
              */
            ...
            info2 match
              case info2: TypeAlias =>
                ...
                if tp2.asInstanceOf[TypeRef].canDropAlias && hasStablePrefix(
                    tp2
                  )
                then return false
              case _ =>
                ...
                tp1.info match
                  case info1: TypeAlias =>
                    ...
                    if tp1
                        ...
                        .canDropAlias && hasStablePrefix(tp2)
                    then return false
                  case _ =>
                val sym2 = tp2.symbol
                var sym1 = tp1.symbol
                if sym1.is(ModuleClass) && sym2.is(ModuleVal) then
          ...
          def compareThis =
            ...
            tp1 match
              case tp1: ThisType =>
                tp1.cls eq cls2
              case tp1: NamedType
                  if cls2.is(Module) && cls2.eq(tp1.typeSymbol) =>
                cls2.isStaticOwner ||
        ...
        case OrType(tp21, tp22) =>
          if tp21.stripTypeVar eq tp22.stripTypeVar then recur(tp1, tp21)
          else secondTry
        case TypeErasure.ErasedValueType(tycon1, underlying2) =>
          def compareErasedValueType = tp1 match
            case TypeErasure.ErasedValueType(tycon2, underlying1) =>
              (tycon1.symbol eq tycon2.symbol) && isSubType(
                underlying1,
                underlying2
              )
            case _ =>
        ...
        case ConstantType(v2) =>
          tp1 match
            case ConstantType(v1) =>
              v1.value == v2.value && recur(v1.tpe, v2.tpe)
            case _ => secondTry
        case tp2: AnyConstantType =>
          if tp2.tpe.exists then recur(tp1, tp2.tpe)
          else
            tp1 match
      ...
      def secondTry: Boolean = tp1 match
        ...
        case tp1: TypeParamRef =>
          def flagNothingBound =
            ...
            then
              def msg =
                s"!!! instantiated to Nothing: $tp1, constraint = ${constraint.show}"
              if Config.failOnInstantiationToNothing then assert(false, msg)
              else report.log(msg)
            true
        ...
        case tp1: SkolemType =>
          tp2 match
            case tp2: SkolemType
                if !ctx.phase.isTyper && recur(tp1.info, tp2.info) =>
              true
            case _ => thirdTry
        case tp1: TypeVar =>
        ...
        case tp1 @ OrType(tp11, tp12) =>
          ...
          /** Mark toplevel type vars in `tp2` as hard in the current constraint
            */
          def hardenTypeVars(tp2: Type): Unit =
            tp2.dealiasKeepRefiningAnnots match
        ...
        case tp1: MatchType =>
          ...
          if reduced.exists then
            recur(reduced, tp2) && recordGadtUsageIf {
              MatchType.thatReducesUsingGadt(tp1)
            }
          else thirdTry
        case _: FlexType =>
          true
      ...
      def thirdTryNamed(tp2: NamedType): Boolean = tp2.info match
        case info2: TypeBounds =>
              ...
              isSubTypeWhenFrozen(tp1, gbounds2.lo)
                || tp1.match
                  case tp1: NamedType if ctx.gadt.contains(tp1.symbol) =>
                    ...
                    ctx.gadt.isLess(tp1.symbol, tp2.symbol) && GADTusage(
                      tp1.symbol
          ...
          isSubApproxHi(
            tp1,
            info2.lo.boxedIfTypeParam(tp2.symbol)
        ...
        case _ =>
          val cls2 = tp2.symbol
          if cls2.isClass then
            if cls2.typeParams.isEmpty then
              if cls2 eq AnyKindClass then return true
              if isBottom(tp1) then return true
              if tp1.isLambdaSub then return false
        ...
        assumedTrue(tp2)
          || {
            val alwaysTrue =
              ...
              if frozenConstraint then recur(tp1, bounds(tp2).lo.boxed)
      ...
      def thirdTry: Boolean = tp2 match
          ...
          def compareRefined: Boolean =
              ...
              def isSubInfo(info1: Type, info2: Type): Boolean =
                (info1, info2) match
                  case (info1: PolyType, info2: PolyType) =>
                    ...
                    && isSubInfo(
                      info1.resultType,
                      info2.resultType.subst(info2, info1)
                    )
            ...
            if (skipped2 eq tp2) || !Config.fastPathForRefinedSubtype then
              if containsAnd(tp1) then
                ...
                fourthTry
              else compareRefinedSlow || fourthTry
        ...
        case tp2: RecType =>
          def compareRec = tp1.safeDealias match
            ...
            case _ =>
              ...
              recur(
                fixRecs(tp1stable, tp1stable.widenExpr),
                tp2.parent.substRecThis(tp2, tp1stable)
              )
          compareRec
        case tp2: HKTypeLambda =>
          def compareTypeLambda: Boolean = tp1.stripTypeVar match
            case tp1: HKTypeLambda =>
              ...
              def boundsOK =
                migrateTo3 ||
                  tp1.typeParams.corresponds(tp2.typeParams)(
                    (tparam1, tparam2) =>
                      isSubType(
                        tparam2.paramInfo.subst(tp2, tp1),
                        tparam1.paramInfo
                      )
                  )
              comparingTypeLambdas(tp1, tp2) {
                ...
                variancesOK && boundsOK && isSubType(
                  tp1.resType,
                  tp2.resType.subst(tp2, tp1)
                )
              }
            case _ =>
              val tparams1 = tp1.typeParams
              if tparams1.nonEmpty then
                return recur(tp1.EtaExpand(tparams1), tp2) || fourthTry
              tp2 match
                case EtaExpansion(tycon2: TypeRef)
                    if tycon2.symbol.isClass && tycon2.symbol.is(JavaDefined) =>
        ...
        case tp2 @ OrType(tp21, tp22) =>
          ...
          // as in `1 <:< 1 | 2`.
          val tp1w = tp1.widen
          if (tp1w ne tp1) && recur(tp1w, tp2) then return true
          ...
          tp21 match
            case AndType(tp211, tp212) =>
              return recur(tp1, OrType(tp211, tp22, tp2.isSoft)) && recur(
                tp1,
                OrType(tp212, tp22, tp2.isSoft)
              )
            case _ =>
          tp22 match
            case AndType(tp221, tp222) =>
              return recur(tp1, OrType(tp21, tp221, tp2.isSoft)) && recur(
                tp1,
                OrType(tp21, tp222, tp2.isSoft)
              )
            case _ =>
        ...
        case tp2: MatchType =>
          ...
          if reduced.exists then
            recur(tp1, reduced) && recordGadtUsageIf {
              MatchType.thatReducesUsingGadt(tp2)
            }
          else fourthTry
        case tp2: MethodType =>
          ...
          def compareCapturing =
            ...
            try
              ...
              case ex: AssertionError =>
                println(
                  i"assertion failed while compare captured $tp1 <:< $tp2"
                )
                throw ex
          compareCapturing || fourthTry
        case tp2: AnnotatedType if tp2.isRefining =>
          (tp1.derivesAnnotWith(
            tp2.annot.sameAnnotation
          ) || tp1.isBottomType) &&
          recur(tp1, tp2.parent)
      ...
      def tryBaseType(cls2: Symbol) =
        ...
        if base.exists && (base ne tp1)
          && (!caseLambda.exists || canWidenAbstract || tp1.widen
            .underlyingClassRef(refinementOK = true)
            .exists)
        then
          isSubType(
            base,
            tp2,
            if tp1.isRef(cls2) then approx else approx.addLow
          )
      ...
      def fourthTry: Boolean = tp1 match
        case tp1: TypeRef =>
          tp1.info match
            case info1 @ TypeBounds(lo1, hi1) =>
              ...
              && isSubType(
                hi1.boxedIfTypeParam(tp1.symbol),
                tp2,
                approx.addLow
        ...
        case tp1: HKTypeLambda =>
          def compareHKLambda = tp1 match
            case EtaExpansion(tycon1: TypeRef)
                if tycon1.symbol.isClass && tycon1.symbol.is(JavaDefined) =>
            ...
            case _ =>
              tp2 match
                case tp2: HKTypeLambda =>
                  false // this case was covered in thirdTry
                case _ =>
                  tp2.typeParams.hasSameLengthAs(tp1.paramRefs) && isSubType(
                    tp1.resultType,
                    tp2.appliedTo(tp1.paramRefs)
                  )
          compareHKLambda
        case AndType(tp11, tp12) =>
          val tp2a = tp2.dealiasKeepRefiningAnnots
          if tp2a ne tp2
          then // Follow the alias; this might avoid truncating the search space in the either below
          ...
          tp11 match
            case OrType(tp111, tp112) =>
              return recur(AndType(tp111, tp12), tp2) && recur(
                AndType(tp112, tp12),
                tp2
              )
            case _ =>
          tp12 match
            case OrType(tp121, tp122) =>
              return recur(AndType(tp11, tp121), tp2) && recur(
                AndType(tp11, tp122),
                tp2
              )
            case _ =>
      ...
      /** When called from `pre1.A <:< pre2.A` does `pre1` relate to `pre2` so
        * that the subtype test is true? This is the case if
        *
        *   1. `pre1 <:< pre2`, or 2. One of `pre1` and `pre2` refers to a
        *      package and the other to a package object in that package, or 3.
        *      `pre1` and `pre2` are both this-types of related classes.
        *
        * Here, two classes are related if each of them has a self type that
        * derives from the other. The third criterion is a bit dubious. I.e. in
        * the test
        *
        * A.this.T <:< B.this.T
        *
        * where `T` is the same type, what relationship must exist between A and
        * B for the test to be guaranteed true? The problem is we can't tell
        * without additional info. One could be an outer this at the point where
        * we do the test, but that location is unknown to us.
        *
        * The conservative choice would be to require A == B, but then some
        * tests involving self types fail. Specifically, t360, t361 and pat_iuli
        * fail the pickling test, and Namer fails to compile. At line 203, we
        * get
        *
        * val Deriver : Property.Key[typer.Deriver] = new Property.Key ^ value
        * Deriver in class Namer is not a legal implementation of `Deriver` in
        * class Namer. its type
        * dotty.tools.dotc.util.Property.Key[Namer.this.Deriver]
        * \|* does not conform to
        * dotty.tools.dotc.util.Property.Key[Typer.this.Deriver &
        * Namer.this.Deriver]
        */
      def isSubPrefix(pre1: Type, pre2: Type): Boolean =
        def samePkg(sym1: Symbol, sym2: Symbol) =
          sym2.is(
            Package
          ) && sym1.isPackageObject && sym1.owner == sym2.moduleClass
            || sym1.is(
              Package
            ) && sym2.isPackageObject && sym2.owner == sym1.moduleClass
        pre1 match
          case pre1: ThisType =>
            pre2 match
              case pre2: ThisType =>
      ...
      /** Compare `tycon[args]` with `other := otherTycon[otherArgs]`, via `>:>`
        * if fromBelow is true, `<:<` otherwise (we call this relationship `~:~`
        * in the rest of this comment).
        *
        * This method works by:
        *
        *   1. Choosing an appropriate type constructor `adaptedTycon` 2.
        *      Constraining `tycon` such that `tycon ~:~ adaptedTycon` 3.
        *      Recursing on `adaptedTycon[args] ~:~ other`
        *
        * So, how do we pick `adaptedTycon`? When `args` and `otherArgs` have
        * the same length the answer is simply:
        *
        ...
        *
        * where the bounds of `T_i` are set based on the bounds of
        * `otherTycon.typeParams(d+i)` after substituting type parameter
        * references by the corresponding argument in `bodyArgs` (see
        * `adaptedBounds` in the implementation).
        *
        * Historical note: this strategy is known in Scala as "partial
        * unification" (even though the type constructor variable isn't actually
        * unified but only has one of its bounds constrained), for background
        * see:
        *   - The infamous SI-2712: https://github.com/scala/bug/issues/2712
        *   - The PR against Scala 2.12 implementing -Ypartial-unification:
        *     https://github.com/scala/scala/pull/5102
        *   - Some explanations on how this impacts API design:
        *     https://gist.github.com/djspiewak/7a81a395c461fd3a09a6941d4cd040f2
        */
      def compareAppliedTypeParamRef(
          tycon: TypeParamRef,
          args: List[Type],
          other: AppliedType,
          fromBelow: Boolean
              ...
              if d > 0 then
                ...
                def adaptedBounds(tl: HKTypeLambda) =
                  ...
                  remainingTparams
                    .map(_.paramInfo)
                    .mapconserve(
                      _.substTypeParams(otherTycon, bodyArgsComputed).bounds
                    )
      ...
      def compareAppliedType2(
        ...
        if tparams.isEmpty then
          return false // can happen for ill-typed programs, e.g. neg/tcpoly_overloaded.scala

        /** True if `tp1` and `tp2` have compatible type constructors and their
          * corresponding arguments are subtypes relative to their variance (see
          * `isSubArgs`).
          */
              ...
              case tycon1: TypeRef =>
                tycon2 match
                  case tycon2: TypeRef =>
                    ...
                    extension (sym: Symbol)
                      inline def byGadtBounds(
                          inline op: TypeBounds => Boolean
                      ): Boolean =
                        touchedGADTs = true
                        sym.onGadtBounds(b =>
                          op(b) && {
                            gadtIsInstantiated = b.isInstanceOf[TypeAlias]; true
                          }
                        )
                    ...
                    val res = (
                      tycon1sym == tycon2sym && isSubPrefix(
                        ...
                        || tycon1sym.byGadtBounds(b =>
                          isSubTypeWhenFrozen(b.hi, tycon2)
                        )
                        || tycon2sym.byGadtBounds(b =>
                      ...
                      inFrozenGadtIf(!tyconIsInjective) {
                        if tycon1sym == tycon2sym && tycon1sym.isAliasType then
                          ...
                          && tryAlso(
                            preConstraint,
                            recur(
                              tp1.superTypeNormalized,
                              tp2.superTypeNormalized
                            )
                          )
                        else isSubArgs(args1, args2, tp1, tparams)
                      }
                    }
        ...
        /** `param2` can be instantiated to a type application prefix of the LHS
          * or to a type application prefix of one of the LHS base class
          * instances and the resulting type application is a supertype of
          * `tp1`.
          */
        def canInstantiate(tycon2: TypeParamRef): Boolean =
          ...
          appOK(tp1w) || tp1w.typeSymbol.isClass && {
            ...
            def liftToBase(bcs: List[ClassSymbol]): Boolean = bcs match
              case bc :: bcs1 =>
                classBounds.exists(bc.derivesFrom) && appOK(
                  nonExprBaseType(tp1, bc)
                )
                || liftToBase(bcs1)
              case _ =>
                false
        ...
        /** Fall back to comparing either with `fourthTry` or against the lower
          ...
          * @param tyconLo
          *   The type constructor's lower approximation.
          */
        def fallback(tyconLo: Type) =
          either(
            fourthTry,
            isSubApproxHi(tp1, tyconLo.applyIfParameterized(args2))
          )

        /** Let `tycon2bounds` be the bounds of the RHS type constructor
          * `tycon2`. Let `app2 = tp2` where the type constructor of `tp2` is
          * replaced by `tycon2bounds.lo`. If both bounds are the same, continue
          * with `tp1 <:< app2`. otherwise continue with either
          *
          ...
          */
        def compareLower(
            tycon2bounds: TypeBounds,
            tyconIsTypeRef: Boolean
        ): Boolean =
          if (tycon2bounds.lo `eq` tycon2bounds.hi) && !tycon2bounds
              .isInstanceOf[MatchAlias]
          then
            if tyconIsTypeRef then
              recur(tp1, tp2.superTypeNormalized) && recordGadtUsageIf(
                MatchType.thatReducesUsingGadt(tp2)
              )
            else isSubApproxHi(tp1, tycon2bounds.lo.applyIfParameterized(args2))
          else fallback(tycon2bounds.lo)
        ...
        tycon2 match
          ...
          case tv: TypeVar =>
            if tv.isInstantiated then recur(tp1, tp2.superType)
            else compareAppliedType2(tp2, tv.origin, args2)
      ...
      def compareAppliedType1(
        ...
        tycon1 match
          case param1: TypeParamRef =>
            ...
            isSubType(
              bounds(param1).hi.applyIfParameterized(args1),
              tp2,
              approx.addLow
            )
          case tycon1: TypeRef =>
            ...
            def byGadtBounds: Boolean =
              sym.onGadtBounds { bounds1 =>
                inFrozenGadt {
                  isSubType(
                    bounds1.hi.applyIfParameterized(args1),
                    tp2,
                    approx.addLow
                  )
                }
            ...
            !sym.isClass && {
              defn.isCompiletimeAppliedType(
                sym
              ) && compareCompiletimeAppliedType(
                tp1,
                tp2,
                fromBelow = false
              ) || {
                recur(tp1.superTypeNormalized, tp2) && recordGadtUsageIf(
                  MatchType.thatReducesUsingGadt(tp1)
                )
              } ||
              tryLiftedToThis1
      ...
      /** Compare `tp` of form `S[arg]` with `other`, via ">:>" if fromBelow is
        * true, "<:<" otherwise. If `arg` is a Nat constant `n`, proceed with
        * comparing `n + 1` and `other`. Otherwise, if `other` is a Nat constant
        * `n`, proceed with comparing `arg` and `n - 1`.
        */
      def compareS(tp: AppliedType, other: Type, fromBelow: Boolean): Boolean =
        tp.args match
          case arg :: Nil =>
            natValue(arg) match
      ...
      /** Compare `tp` of form `tycon[...args]`, where `tycon` is a
        * scala.compiletime type, with `other` via ">:>" if fromBelow is true,
        * "<:<" otherwise. Delegates to compareS if `tycon` is
        * scala.compiletime.S. Otherwise, constant folds if possible.
        */
      def compareCompiletimeAppliedType(
          tp: AppliedType,
          other: Type,
          fromBelow: Boolean
      ): Boolean =
        if defn.isCompiletime_S(tp.tycon.typeSymbol) then
      ...
      /** Like tp1 <:< tp2, but returns false immediately if we know that the
        * case was covered previously during subtyping.
        */
      def isNewSubType(tp1: Type): Boolean =
        if isCovered(tp1) && isCovered(tp2) then
      ...
      def isSubApproxHi(tp1: Type, tp2: Type): Boolean =
        tp1
          .eq(tp2) || tp2.ne(NothingType) && isSubType(tp1, tp2, approx.addHigh)
      ...
      if tp2 eq NoType then false
        ...
        try
          recCount += 1
          if recCount >= Config.LogPendingSubTypesThreshold then
            monitored = true
  ...
  /** If `tp` is an external reference to an enclosing module M that contains
    * opaque types, convert to M.this. Note: It would be legal to do the lifting
    * also if M does not contain opaque types, but in this case the retries in
    * tryLiftedToThis would be redundant.
    */
  private def liftToThis(tp: Type): Type =

    def findEnclosingThis(moduleClass: Symbol, from: Symbol): Type =
      if (from.owner eq moduleClass) && from.isPackageObject && from.is(Opaque)
      then from.thisType
      else if from.is(Package) then tp
      else if (from eq moduleClass) && from.is(Opaque) then from.thisType
      else if from eq NoSymbol then tp
    ...
    tp match
      ...
      case tp: TypeRef =>
        ...
        if (pre1 ne tp.prefix) && pre1.exists then tp.withPrefix(pre1) else tp
      case tp: ThisType if tp.cls.is(Package) =>
        findEnclosingThis(tp.cls, ctx.owner)
      case tp: AppliedType =>
        val tycon1 = liftToThis(tp.tycon)
        if tycon1 ne tp.tycon then tp.derivedAppliedType(tycon1, tp.args)
        else tp
      case tp: TypeVar if tp.isInstantiated =>
        liftToThis(tp.inst)
      case tp: AnnotatedType =>
        val parent1 = liftToThis(tp.parent)
        if parent1 ne tp.parent then tp.derivedAnnotatedType(parent1, tp.annot)
        else tp
      case _ =>
        tp
  end liftToThis
  ...
  def constValue(tp: Type): Option[Constant] =
    ...
    if isSubTypeWhenFrozen(tp, ct) then
      ct.tpe match
        case ConstantType(c) => Some(c)
        case _               => None
    else None

  /** If both `tp1` and `tp2` have atoms information, compare the atoms in a
    * Some, otherwise None.
    * @param knownSingletons
    *   If true, we are coming from a comparison of two singleton types This
    *   influences the comparison as shown below:
    *
    * Say you have singleton types p.type and q.type the atoms of p.type are
    * `{p.type}..{p.type}`, and the atoms of `q.type` are `{}..{p.type}`.
    * Normally the atom comparison between p's atoms and q's atoms gives false.
    * But in this case we know that `q.type` is an alias of `p.type` so we are
    * still allowed to conclude that `p.type <:< q.type`. A situation where this
    * happens is in i6635.scala. Here,
    *
  ...
  def compareAtoms(
      tp1: Type,
      tp2: Type,
      knownSingletons: Boolean = false
  ): Option[Boolean] =

    /** Check whether we can compare the given set of atoms with another to
      * determine a subtype test between OrTypes. There is one situation where
      * this is not the case, which has to do with SkolemTypes. TreeChecker
      * sometimes expects two types to be equal that have different skolems. To
      * account for this, we identify two different skolems in all phases `p`,
      * where `p.isTyper` is false. But in that case comparing two sets of atoms
      * that contain skolems for equality would give the wrong result, so we
      * should not use the sets for comparisons.
      */
      ...
      case Atoms.Range(lo2, hi2) if canCompareAtoms && canCompare(hi2) =>
        tp1.atoms match
          case Atoms.Range(lo1, hi1) =>
            if hi1.subsetOf(
                lo2
              ) || knownSingletons && hi2.size == 1 && hi1 == hi2
            then Some(verified(true))
  ...
  def isSubArgs(
      ...
      args2: List[Type],
      tp1: Type,
      tparams2: List[ParamInfo]
  ): Boolean =
    /** The bounds of parameter `tparam`, where all references to type
      * paramneters are replaced by corresponding arguments (or their
      * approximations in the case of wildcard arguments).
      */
    ...
    def recurArgs(
      ...
      if args1.isEmpty then args2.isEmpty
        ...
        args2.nonEmpty && tparams2.nonEmpty && {
          val tparam = tparams2.head
          val v = tparam.paramVarianceSign

          /** Try a capture conversion: If the original left-hand type
            * `leftRoot` is a path `p.type`, and the current widened left type
            * is an application with wildcard arguments such as `C[?]`, where
            * `X` is `C`'s type parameter corresponding to the `_` argument,
            * compare with `C[p.X]` instead. Otherwise approximate based on
            * variance. Also do a capture conversion in either of the following
            * cases:
            *
            *   - If we are after typer. We generally relax soundness
            *     requirements then. We need the relaxed condition to correctly
            *     compute overriding relationships. Missing this case led to
            *     AbstractMethod errors in the bootstrap.
            *
            *   - If we are in mode TypevarsMissContext, which means we test
            *     implicits for eligibility. In this case, we can be more
            *     permissive, since it's just a pre-check. This relaxation is
            *     needed since the full implicit typing might perform an
            *     adaptation that skolemizes the type of a synthesized tree
            *     before comparing it with an expected type. But no such
            *     adaptation is applied for implicit eligibility testing, so we
            *     have to compensate.
            *
            * Note: Doing the capture conversion on path types is actually not
            * necessary since we can already deal with the situation through
            * skolemization in Typer#captureWildcards. But performance tests
            * indicate that it's better to do it, since we avoid skolemizations,
            * which are more expensive . And, besides, capture conversion on
          ...
          def compareCaptured(arg1: TypeBounds, arg2: Type) = tparam match
            case tparam: Symbol =>
              ...
              if (leftr.isStable || ctx.isAfterTyper || ctx.mode.is(
                  Mode.TypevarsMissContext
                ))
                && leftr.isValueType
              ...
              val arg1norm = arg1 match
                case arg1: TypeBounds =>
                  tparam match
                    ...
                    case _ =>
                      arg1 // This case can only arise when a hk-type is illegally instantiated with a wildcard
                case _ => arg1
          ...
          isSubArg(
            args1.head.boxedUnlessFun(tp1),
            args2.head.boxedUnlessFun(tp1)
          )
  ...
  /** Test whether `tp1` has a base type of the form `B[T1, ..., Tn]` where
    ...
    *   - the type parameters of `B` match one-by-one the variances of
    *     `tparams`,
    *   - `B` satisfies predicate `p`.
    */
  private def testLifted(
      tp1: Type,
      tp2: Type,
      tparams: List[TypeParamInfo],
      p: Type => Boolean
  ): Boolean =
  ...
  /** Replace any top-level recursive type `{ z => T }` in `tp` with `[z :=
    * anchor]T`.
    */
  private def fixRecs(anchor: SingletonType, tp: Type): Type =
    def fix(tp: Type): Type = tp.stripTypeVar match
      ...
      case tp @ RefinedType(parent, rname, rinfo) =>
        tp.derivedRefinedType(fix(parent), rname, rinfo)
  ...
  /** Returns true iff the result of evaluating either `op1` or `op2` is true
    * and approximates resulting constraints.
    *
    * If `necessaryConstraintsOnly` is true, we keep the smaller constraint if
    * any, or no constraint at all.
    *
    * Otherwise, we infer _sufficient_ constraints: we try to keep the smaller
    * of the two constraints, but if never is smaller than the other, we just
    * pick the first one.
    */
  protected def either(op1: => Boolean, op2: => Boolean): Boolean =
    Stats.record("TypeComparer.either")
    if necessaryConstraintsOnly then necessaryEither(op1, op2)
    else sufficientEither(op1, op2)

  /** Returns true iff the result of evaluating either `op1` or `op2` is true,
    * trying at the same time to keep the constraint as wide as possible. E.g,
    * if
    *
    * tp11 <:< tp12 = true with post-constraint c1 tp12 <:< tp22 = true with
    * post-constraint c2
    *
    * and c1 subsumes c2, then c2 is kept as the post-constraint of the result,
    * otherwise c1 is kept.
    *
    * This method is used to approximate a solution in one of the following
    * cases
    *
    * T1 & T2 <:< T3 T1 <:< T2 | T3
    *
    * In the first case (the second one is analogous), we have a choice whether
    * we want to establish the subtyping judgement using
    *
    * T1 <:< T3 or T2 <:< T3
    *
    * as a precondition. Either precondition might constrain type variables. The
    * purpose of this method is to pick the precondition that constrains less.
    * The method is not complete, because sometimes there is no best solution.
    * Example:
    *
    * A? & B? <: T
    *
    ...
    *
    * Note that to be complete when it comes to typechecking, we would instead
    * need to backtrack and attempt to typecheck with the other constraint.
    *
    * Method name comes from the notion that we are keeping a constraint which
    * is sufficient to satisfy one of subtyping relationships.
    */
  private def sufficientEither(op1: => Boolean, op2: => Boolean): Boolean =
    val preConstraint = constraint
    if op1 then tryAlso(preConstraint, op2) else op2
  end sufficientEither

  /** Check whether `op` generates a weaker constraint than the current
    * constraint if we run it starting with `preConstraint`. If that's the case,
    * replace the current constraint with the constraint generated by `op`.
    */
  private def tryAlso(preConstraint: Constraint, op: => Boolean): true =
    if constraint ne preConstraint then
      ...
      if !(op && subsumes(leftConstraint, constraint, preConstraint)) then
        if constr != noPrinter && !subsumes(
            constraint,
            leftConstraint,
            preConstraint
          )
        then constr.println(i"CUT - prefer $leftConstraint over $constraint")
        constraint = leftConstraint
    true

  /** Returns true iff the result of evaluating either `op1` or `op2` is true,
    * keeping the smaller constraint if any. E.g., if
    *
    * tp11 <:< tp12 = true with constraint c1 and GADT constraint g1 tp12 <:<
    * tp22 = true with constraint c2 and GADT constraint g2
    *
    * We keep:
    ...
    *
    * Like [[sufficientEither]], this method is used to approximate a solution
    * in one of the following cases:
    *
    * T1 & T2 <:< T3 T1 <:< T2 | T3
    *
    * But this method is used when `useNecessaryEither` is true, like when we
    * are attempting to infer GADT constraints that necessarily follow from the
    * subtyping relationship. For instance, if we have
    *
    * enum Expr[T] { case IntExpr(i: Int) extends Expr[Int] case StrExpr(s:
    * String) extends Expr[String] }
    *
    ...
    *
    * Since we don't know which branch is true, we need to give up and not keep
    * either constraint. OTOH, if one constraint pair is subsumed by the other,
    * we know that it is necessary for both cases and therefore we can keep it.
    *
    * Like [[sufficientEither]], this method is not complete because sometimes,
    * the necessary constraint is neither of the pairs. For instance, if
    *
    * g1 = { A = Int, B = String } g2 = { A = Int, B = Int }
    *
    * then the necessary constraint is { A = Int }, but correctly inferring that
    * is, as far as we know, too expensive.
    *
    * This method is also used in ConstrainResult mode to avoid inference
    * getting stuck due to lack of backtracking, see or-inf.scala and
    * and-inf.scala for examples.
    *
    * Method name comes from the notion that we are keeping the constraint which
    * is necessary to satisfy both subtyping relationships.
    */
  private def necessaryEither(op1: => Boolean, op2: => Boolean): Boolean =
    ...
    def allSubsumes(
        leftGadt: GadtConstraint,
        rightGadt: GadtConstraint,
        left: Constraint,
        right: Constraint
    ): Boolean =
    ...
    if op1 then
      ...
      if op2 then
        if allSubsumes(op1Gadt, ctx.gadt, op1Constraint, constraint) then
          ...
          gadts.println(
            i"GADT CUT - no constraint is preferable, reverting to $preGadt"
          )
          constr.println(
            i"CUT - no constraint is preferable, reverting to $preConstraint"
          )
          constraint = preConstraint
          ctx.gadtState.restore(preGadt)
      else
  ...
  /** Decompose into conjunction of types each of which has only a single
    * refinement
    */
  def decomposeRefinements(tp: Type, refines: List[(Name, Type)]): Type =
    tp match
      ...
      case AndType(tp1, tp2) =>
        AndType(
          decomposeRefinements(tp1, refines),
          decomposeRefinements(tp2, refines)
        )
      case _ =>
  ...
  def containsAnd(tp: Type): Boolean = tp match
    ...
    case tp: TypeRef =>
      containsAnd(tp.info.hiBound) || tp.symbol.onGadtBounds(gbounds =>
        containsAnd(gbounds.hi)
      )
    case tp: TypeProxy => containsAnd(tp.superType)
    case _             => false

  /** Does type `tp1` have a member with name `name` whose normalized type is a
    * subtype of the normalized type of the refinement of `tp2`? Normalization
    * is as follows: If `tp2` contains a skolem to its refinement type, rebase
    * both itself and the member info of `tp` on a freshly created skolem type.
    */
  def hasMatchingMember(name: Name, tp1: Type, tp2: RefinedType): Boolean =
    trace(
      i"hasMatchingMember($tp1 . $name :? ${tp2.refinedInfo}), mbr: ${tp1.member(name).info}",
      subtyping
    ) {
  ...
  /** Skip refinements in `tp2` which match corresponding refinements in `tp1`.
    ...
    * @return
    *   The parent type of `tp2` after skipping the matching refinements.
    */
  ...
  /** Are refinements in `tp1` pairwise subtypes of the refinements of `tp2` up
    * to parent type `limit`?
    * @pre
    *   `tp1` has the necessary number of refinements, they are type aliases,
    *   and their names match the corresponding refinements in `tp2`. Further,
    *   no refinement refers back to the refined type via a refined this. The
    *   precondition is established by `skipMatching`.
    */
  private def isSubRefinements(
      tp1: RefinedType,
      tp2: RefinedType,
      limit: Type
  ): Boolean =
    isSubType(tp1.refinedInfo, tp2.refinedInfo)
      && ((tp2.parent eq limit)
        || isSubRefinements(
          ...
          tp2.parent.asInstanceOf[RefinedType],
          limit
        ))

  /** A type has been covered previously in subtype checking if it is some
    * combination of TypeRefs that point to classes, where the combiners are
    * AppliedTypes, RefinedTypes, RecTypes, And/Or-Types or AnnotatedTypes.
    */
  private def isCovered(tp: Type): Boolean =
    tp.dealiasKeepRefiningAnnots.stripTypeVar match
      case tp: TypeRef =>
        tp.symbol.isClass && tp.symbol != NothingClass && tp.symbol != NullClass
  ...
  /** Narrow gadt.bounds for the type parameter referenced by `tr` to include
    * `bound` as an upper or lower bound (which depends on `isUpper`). Test that
    * the resulting bounds are still satisfiable.
    */
  private def narrowGADTBounds(
      tr: NamedType,
      bound: Type,
      approx: ApproxState,
      isUpper: Boolean
    ...
    ctx.mode.is(
      Mode.GadtConstraintInference
    ) && !frozenGadt && !frozenConstraint && !boundImprecise && {
      val tparam = tr.symbol
      gadts.println(i"narrow gadt bound of $tparam: ${tparam.info} from ${
          if isUpper then "above" else "below"
        } to $bound ${bound.toString} ${bound.isRef(tparam)}")
      if bound.isRef(tparam) then false
      else
  ...
  /** Do the parameter types of `tp1` and `tp2` match in a way that allows `tp1`
    * to override `tp2` ? Two modes: precise or not. If `precise` is set (which
    * is the default) this is the case if they're pairwise `=:=`. Otherwise
    * parameters in `tp2` must be subtypes of corresponding parameters in `tp1`.
    */
  def matchingMethodParams(
      tp1: MethodType,
      tp2: MethodType,
      precise: Boolean = true
  ): Boolean =
    def loop(formals1: List[Type], formals2: List[Type]): Boolean =
      formals1 match
        case formal1 :: rest1 =>
          formals2 match
            case formal2 :: rest2 =>
              val formal2a =
                if tp2.isParamDependent then formal2.subst(tp2, tp1)
                else formal2
              val paramsMatch =
    ...
    val erasedValid =
      (!tp1.hasErasedParams && !tp2.hasErasedParams) || (tp1.erasedParams == tp2.erasedParams)
  ...
  /** Two types are the same if they are mutual subtypes of each other. To avoid
    * exponential blowup for deeply nested invariant applied types, we cache
    * successes once the stack of outstanding isSameTypes reaches depth
    * `startSameTypeTrackingLevel`. See pos/i15525.scala, where this matters.
    */
  ...
  override protected def isSame(tp1: Type, tp2: Type)(using Context): Boolean =
    isSameType(tp1, tp2)
  ...
  /** If the range `tp1..tp2` consist of a single type, that type, otherwise
    * NoType`. This is the case if `tp1 =:= tp2`, but also if `tp1 <:< tp2`,
    * `tp1` is a singleton type, and `tp2` derives from `scala.Singleton` (or
    * vice-versa). Examples of the latter case:
    *
    * "name".type .. Singleton "name".type .. String & Singleton Singleton ..
    * "name".type String & Singleton .. "name".type
    *
  ...
  def singletonInterval(tp1: Type, tp2: Type): Type =
    def isSingletonBounds(lo: Type, hi: Type) =
      lo.isSingleton && hi.derivesFrom(
        defn.SingletonClass
      ) && isSubTypeWhenFrozen(lo, hi)
    if isSameTypeWhenFrozen(tp1, tp2) then tp1
    else if isSingletonBounds(tp1, tp2) then tp1
    else if isSingletonBounds(tp2, tp1) then tp2
    else NoType

  /** The greatest lower bound of two types */
  def glb(tp1: Type, tp2: Type): Type = /*>|>*/ trace(
    s"glb(${tp1.show}, ${tp2.show})",
    subtyping,
    show = true
  ) /*<|<*/ {
    ...
    else if tp1.isAny && !tp2.isLambdaSub || tp1.isAnyKind || isBottom(tp2) then
      tp2
    else if tp2.isAny && !tp1.isLambdaSub || tp2.isAnyKind || isBottom(tp1) then
      tp1
    else
      tp2 match
        case tp2: LazyRef =>
            ...
            case _ =>
              ...
              if tp1a ne tp1 then glb(tp1a, tp2)
                ...
                if tp2a ne tp2 then glb(tp1, tp2a)
                  ...
                  tp2 match // normalize to disjunctive normal form if possible.
                    case tp2 @ OrType(tp21, tp22) =>
  ...
  def lub(
    ...
    if tp1 eq tp2 then tp1
      ...
      def mergedLub(tp1: Type, tp2: Type): Type =
        ...
        val tp1w = widen(tp1)
        val tp2w = widen(tp2)
        if (tp1 ne tp1w) || (tp2 ne tp2w) then
          lub(tp1w, tp2w, canConstrain = canConstrain, isSoft = isSoft)
        else
          orType(tp1w, tp2w, isSoft = isSoft) // no need to check subtypes again
      mergedLub(tp1.stripLazyRef, tp2.stripLazyRef)
  }

  /** Try to produce joint arguments for a lub `A[T_1, ..., T_n] | A[T_1', ...,
    * T_n']` using the following strategies:
    *
  ...
  /** Try to produce joint arguments for a glb `A[T_1, ..., T_n] & A[T_1', ...,
    * T_n']` using the following strategies:
    *
    *   - if arguments are the same, that argument.
    *   - if corresponding parameter variance is co/contra-variant, the glb/lub.
    *   - if at least one of the arguments if a TypeBounds, the union of the
    *     bounds.
    *   - if homogenizeArgs is set, and arguments can be unified by
    *     instantiating type parameters, the unified argument.
    *   - otherwise NoType
    *
      ...
      case tparam :: tparamsRest =>
        ...
        val glbArg =
          if common.exists then common
            ...
            TypeBounds(
              lub(arg1.loBound, arg2.loBound),
              glb(arg1.hiBound, arg2.hiBound)
            )
          else if homogenizeArgs && !frozenConstraint && isSameType(arg1, arg2)
          then arg1
          else NoType
  ...
  /** If some (&-operand of) `tp` is a supertype of `sub` replace it with
    * `NoType`.
    */
  ...
  private def mergeIfSub(tp1: Type, tp2: Type): Type =
    if isSubTypeWhenFrozen(tp1, tp2) then tp1
    else
      tp2 match
        case tp2 @ AndType(tp21, tp22) =>
          val lower1 = mergeIfSub(tp1, tp21)
          if lower1 eq tp21 then tp2
          else if lower1.exists then lower1 & tp22
          else {
            val lower2 = mergeIfSub(tp1, tp22)
            if lower2 eq tp22 then tp2
            else if lower2.exists then tp21 & lower2
            else NoType
          }
        case _ =>
          NoType
  ...
  /** Merge `tp1` into `tp2` if tp1 is a supertype of some |-summand of tp2.
    * @param canConstrain
    *   If true, new constraints might be added to make the merge possible.
    */
  private def mergeIfSuper(tp1: Type, tp2: Type, canConstrain: Boolean): Type =
    if isSubType(tp2, tp1, whenFrozen = !canConstrain) then tp1
    else
      tp2 match
        case tp2 @ OrType(tp21, tp22) =>
          val higher1 = mergeIfSuper(tp1, tp21, canConstrain)
          if higher1 eq tp21 then tp2
          else if higher1.exists then lub(higher1, tp22, isSoft = tp2.isSoft)
          else {
            val higher2 = mergeIfSuper(tp1, tp22, canConstrain)
            if higher2 eq tp22 then tp2
            else if higher2.exists then lub(tp21, higher2, isSoft = tp2.isSoft)
            else NoType
          }
        case _ =>
          NoType

  private def andTypeGen(
      tp1: Type,
      tp2: Type,
      op: (Type, Type) => Type,
      original: (Type, Type) => Type = _ & _,
      isErased: Boolean = ctx.erasedTypes
  ): Type =
    trace(s"andTypeGen(${tp1.show}, ${tp2.show})", subtyping, show = true) {
  ...
  /** Form a normalized conjunction of two types. Note: For certain types, `&`
    * is distributed inside the type. This holds for all types which are not
    * value types (e.g. TypeBounds, ClassInfo, ExprType, LambdaType). Also, when
    * forming an `&`, instantiated TypeVars are dereferenced and annotations are
    * stripped. Finally, refined types with the same refined name are
  ...
  final def simplifyAndTypeWithFallback(
      tp1: Type,
      tp2: Type,
      fallback: Type
  ): Type =
  ...
  /** Form a normalized conjunction of two types. Note: For certain types, `|`
    ...
    * @param isErased
    *   Apply erasure semantics. If erased is true, instead of creating an
    *   OrType, the lub will be computed using TypeCreator#erasedLub.
    */
  final def orType(
      tp1: Type,
      tp2: Type,
    ...
    if t1.exists then t1
      ...
      if t2.exists then t2
      else if isErased then erasedLub(tp1, tp2)
      else
        liftIfHK(tp1, tp2, OrType.balanced(_, _, soft = isSoft), _ | _, _ & _)
    }

  /** `op(tp1, tp2)` unless `tp1` and `tp2` are type-constructors. In the latter
    * case, combine `tp1` and `tp2` under a type lambda like this:
    *
  ...
  def liftIfHK(
      ...
      tp2: Type,
      op: (Type, Type) => Type,
      original: (Type, Type) => Type,
      combineVariance: (Variance, Variance) => Variance
  ) =
    val tparams1 = tp1.typeParams
    val tparams2 = tp2.typeParams
    def applied(tp: Type) =
      tp.appliedTo(tp.typeParams.map(_.paramInfoAsSeenFrom(tp)))
    if tparams1.isEmpty then
      if tparams2.isEmpty then op(tp1, tp2)
      else original(tp1, applied(tp2))
    else if tparams2.isEmpty then original(applied(tp1), tp2)
    else if tparams1.hasSameLengthAs(tparams2) then
      HKTypeLambda(
        ...
        variances =
          if tp1.isDeclaredVarianceLambda && tp2.isDeclaredVarianceLambda then
            tparams1
              .lazyZip(tparams2)
              .map((p1, p2) =>
                combineVariance(p1.paramVariance, p2.paramVariance)
              )
          else Nil
      )(
        paramInfosExp = tl =>
  ...
  /** Try to distribute `&` inside type, detect and handle conflicts
    * @pre
    *   !(tp1 <: tp2) && !(tp2 <:< tp1) -- these cases were handled before
    */
    ...
    case tp1 @ AppliedType(tycon1, args1) =>
      tp2 match
        case AppliedType(tycon2, args2)
          ...
          if jointArgs.forall(_.exists) then
            (tycon1 & tycon2).appliedTo(jointArgs)
          else NoType
        case _ =>
          NoType
    case tp1: RefinedType =>
      ...
      tp2 match
        case tp2: RefinedType if tp1.refinedName == tp2.refinedName =>
          val jointInfo = Denotations.infoMeet(
            tp1.refinedInfo,
            tp2.refinedInfo,
            safeIntersection = false
          )
  ...
  /** Try to distribute `|` inside type, detect and handle conflicts Note that,
    * unlike for `&`, a disjunction cannot be pushed into a refined or applied
    * type. Example:
    *
    ...
    */
  private def distributeOr(tp1: Type, tp2: Type, isSoft: Boolean = true): Type =
    tp1 match
      case ExprType(rt1) =>
        tp2 match
          case ExprType(rt2) =>
      ...
      tp2 match
        case tp2: ClassInfo =>
          isSubTypeWhenFrozen(
            tp1.prefix,
            tp2.prefix
          ) || (tp1.cls.owner derivesFrom tp2.cls.owner)
        case _ =>
          false
    case tp1: PolyType =>
      tp2 match
  ...
  protected def subCaptures(
      refs1: CaptureSet,
      refs2: CaptureSet,
      frozen: Boolean
  )(using Context): CaptureSet.CompareResult =
    refs1.subCaptures(refs2, frozen)

  /** Is the boxing status of tp1 and tp2 the same, or alternatively, is the
    * capture sets `refs1` of `tp1` a subcapture of the empty set? In the latter
    * case, boxing status does not matter.
    */
  protected def sameBoxed(tp1: Type, tp2: Type, refs1: CaptureSet)(using
      Context
  ): Boolean =
    (tp1.isBoxedCapturing == tp2.isBoxedCapturing)
    ...
    s"${tp1.show} <:< ${tp2.show}" + {
      if ctx.settings.verbose.value || Config.verboseExplainSubtype then
        s" ${tp1.getClass}, ${tp2.getClass}" +
          ...
          (if ctx.mode is Mode.TypevarsMissContext then " tvars-miss-ctx"
           else "")
      else ""
    }
  ...
  def showGoal(tp1: Type, tp2: Type)(using Context): Unit =
    try
      report.echo(
        i"assertion failure for ${show(tp1)} <:< ${show(tp2)}, frozen = $frozenConstraint"
      )
      def explainPoly(tp: Type) = tp match
        case tp: TypeParamRef =>
          report.echo(s"TypeParamRef ${tp.show} found in ${tp.binder.show}")
        case tp: TypeRef if tp.symbol.exists =>
          report.echo(s"typeref ${tp.show} found in ${tp.symbol.owner.show}")
        case tp: TypeVar =>
          report.echo(s"typevar ${tp.show}, origin = ${tp.origin}")
        case _ => report.echo(s"${tp.show} is a ${tp.getClass}")
      if Config.verboseExplainSubtype then
        explainPoly(tp1)
        explainPoly(tp2)
    catch
      case NonFatal(ex) =>
        report.echo(
          s"assertion failure [[cannot display since $ex was thrown]]"
        )

  /** Record statistics about the total number of subtype checks and the number
    * of "successful" subtype checks, i.e. checks that form part of a subtype
    * derivation tree that's ultimately successful.
    */
  def recordStatistics(result: Boolean, prevSuccessCount: Int): Unit =
    // Stats.record(s"isSubType ${tp1.show} <:< ${tp2.show}")
    totalCount += 1
    if result then successCount += 1 else successCount = prevSuccessCount
    if recCount == 0 then
  ...
  /** Does `tycon` have a field with type `tparam`? Special cased for `scala.*:`
    ...
    */
  private def typeparamCorrespondsToField(
      tycon: Type,
      tparam: TypeParamInfo
  ): Boolean =
  ...
  /** Are `tp1` and `tp2` provablyDisjoint types?
    ...
    *
    *   1. Single inheritance of classes 2. Final classes cannot be extended 3.
    *      ConstantTypes with distinct values are non intersecting 4. TermRefs
    *      with distinct values are non intersecting 5. There is no value of
    *      type Nothing
    *
    * Note on soundness: the correctness of match types relies on on the
    * property that in all possible contexts, the same match type expression is
    * either stuck or reduces to the same case.
    */
  def provablyDisjoint(tp1: Type, tp2: Type)(using Context): Boolean =
    trace(i"provable disjoint $tp1, $tp2", matchTypes) {
        ...
        override def apply(x: Boolean, t: Type) =
          x && {
            t.dealias match
              ...
              case _: SkolemType | _: TypeVar | _: TypeParamRef |
                  _: TypeBounds =>
                false
              case _ => foldOver(x, t)
          }
      ...
      (tp1.dealias, tp2.dealias) match
        ...
        case (tp1: TypeRef, tp2: TypeRef)
            if tp1.symbol.isClass && tp2.symbol.isClass =>
          ...
          if cls1.derivesFrom(cls2) || cls2.derivesFrom(cls1) then false
        ...
        case (AppliedType(tycon1, args1), AppliedType(tycon2, args2))
          ...
          def covariantDisjoint(
              tp1: Type,
              tp2: Type,
              tparam: TypeParamInfo
          ): Boolean =
            provablyDisjoint(tp1, tp2) && typeparamCorrespondsToField(
              tycon1,
              tparam
            )
          ...
          def invariantDisjoint(
            ...
            provablyDisjoint(tp1, tp2) ||
              ...
              fullyInstantiated(
                tp1
              ) && // We can only trust a "no" from `isSameType` when
              fullyInstantiated(
                tp2
              ) // both `tp1` and `tp2` are fully instantiated.

          args1.lazyZip(args2).lazyZip(tycon1.typeParams).exists {
            (arg1, arg2, tparam) =>
              val v = tparam.paramVarianceSign
              if v > 0 then covariantDisjoint(arg1, arg2, tparam)
              else if v < 0 then
        ...
        case (tp1: TermRef, tp2: TermRef)
            if isEnumValueOrModule(tp1) && isEnumValueOrModule(tp2) =>
          tp1.termSymbol != tp2.termSymbol
        case (tp1: TermRef, tp2: TypeRef) if isEnumValue(tp1) =>
          fullyInstantiated(tp2) && !tp1.classSymbols.exists(
            _.derivesFrom(tp2.symbol)
          )
        case (tp1: TypeRef, tp2: TermRef) if isEnumValue(tp2) =>
          fullyInstantiated(tp1) && !tp2.classSymbols.exists(
            _.derivesFrom(tp1.symbol)
          )
        case (tp1: RefinedType, tp2: RefinedType)
            if tp1.refinedName == tp2.refinedName =>
          provablyDisjoint(tp1.parent, tp2.parent) || provablyDisjoint(
            tp1.refinedInfo,
            tp2.refinedInfo
          )
        ...
        case (tp1: TypeProxy, tp2: TypeProxy) =>
          provablyDisjoint(tp1.superTypeNormalized, tp2) || provablyDisjoint(
            tp1,
            tp2.superTypeNormalized
          )
  ...
  private def inSubComparer[T, Cmp <: TypeComparer](comparer: Cmp)(
      op: Cmp => T
  ): T =
  ...
  def explained[T](
      op: ExplainingTypeComparer => T,
      header: String = "Subtype trace:"
...
object TypeComparer:
  ...
  def matchesType(tp1: Type, tp2: Type, relaxed: Boolean)(using
      Context
  ): Boolean =
    comparing(_.matchesType(tp1, tp2, relaxed))

  def hasMatchingMember(name: Name, tp1: Type, tp2: RefinedType)(using
      Context
  ): Boolean =
    comparing(_.hasMatchingMember(name, tp1, tp2))

  def matchingMethodParams(tp1: MethodType, tp2: MethodType)(using
      Context
  ): Boolean =
  ...
  def andType(using
      Context
  )(tp1: Type, tp2: Type, isErased: Boolean = ctx.erasedTypes): Type =
  ...
  def addToConstraint(tl: TypeLambda, tvars: List[TypeVar])(using
      Context
  ): Boolean =
    comparing(_.addToConstraint(tl, tvars))

  def widenInferred(inst: Type, bound: Type, widenUnions: Boolean)(using
      Context
  ): Type =
  ...
  def constrainPatternType(
      pat: Type,
      scrut: Type,
      forceInvariantRefinement: Boolean = false
...
object TrackingTypeComparer:
  ...
  enum MatchResult extends Showable:
    ...
    def toText(p: Printer): Text = this match
      ...
      case Stuck       => "Stuck"
      case NoInstance(fails) =>
        "NoInstance(" ~ Text(fails.map(p.toText(_) ~ p.toText(_)), ", ") ~ ")"
...
class TrackingTypeComparer(initctx: Context) extends TypeComparer(initctx):
  ...
  override def bounds(param: TypeParamRef)(using Context): TypeBounds =
    if param.binder `ne` caseLambda then footprint += param
    super.bounds(param)

  override def addOneBound(param: TypeParamRef, bound: Type, isUpper: Boolean)(
      using Context
  ): Boolean =
    if param.binder `ne` caseLambda then footprint += param
  ...
  override def gadtBounds(sym: Symbol)(using Context): TypeBounds | Null =
    if sym.exists then footprint += sym.typeRef
  ...
  def matchCases(scrut: Type, cases: List[Type])(using Context): Type =
    ...
    /** Match a single case. */
    def matchCase(cas: Type): MatchResult = trace(
      i"$scrut match ${MatchTypeTrace.caseText(cas)}",
      matchTypes,
      show = true
    ) {
      val cas1 = cas match
      ...
      def redux(canApprox: Boolean): MatchResult =
        caseLambda match
          case caseLambda: HKTypeLambda =>
            val instances = paramInstances(canApprox)(
              Array.fill(caseLambda.paramNames.length)(NoType),
              pat
            )
      ...
      else if matches(canWidenAbstract = true) then redux(canApprox = false)
      else if provablyDisjoint(scrut, pat) then
    ...
    inFrozenConstraint {
      ...
      if provablyEmpty(scrut) then
        MatchTypeTrace.emptyScrutinee(scrut)
        NoType
      else recur(cases)
    }
  end matchCases
end TrackingTypeComparer
...
class ExplainingTypeComparer(initctx: Context) extends TypeComparer(initctx):
  ...
  override def recur(tp1: Type, tp2: Type): Boolean =
    ...
    traceIndented(
      s"${show(tp1)}  <:  ${show(tp2)}$moreInfo${approx.show}$frozenNotice"
    ) {
      super.recur(tp1, tp2)
    }

  override def hasMatchingMember(
      name: Name,
      tp1: Type,
      tp2: RefinedType
  ): Boolean =
    traceIndented(
      s"hasMatchingMember(${show(tp1)} . $name, ${show(tp2.refinedInfo)}), member = ${show(tp1.member(name).info)}"
    ) {
  ...
  override def lub(
    ...
    traceIndented(
      s"lub(${show(tp1)}, ${show(tp2)}, canConstrain=$canConstrain, isSoft=$isSoft)"
    ) {
  ...
  override def addConstraint(
    ...
    traceIndented(
      s"add constraint ${show(param)} ${if fromBelow then ">:" else "<:"} ${show(
          bound
        )} $frozenNotice, constraint = ${show(ctx.typerState.constraint)}"
    ) {
      super.addConstraint(param, bound, fromBelow)
    }

  override def gadtAddBound(sym: Symbol, b: Type, isUpper: Boolean): Boolean =
    traceIndented(s"add GADT constraint ${show(sym)} ${
        if isUpper then "<:" else ">:"
      } ${show(b)} $frozenNotice, GADT constraint = ${show(ctx.gadt)}") {
      super.gadtAddBound(sym, b, isUpper)
    }

  override def subCaptures(
      refs1: CaptureSet,
      refs2: CaptureSet,
      frozen: Boolean
  )(using Context): CaptureSet.CompareResult =
    traceIndented(
      i"subcaptures $refs1 <:< $refs2 ${if frozen then "frozen" else ""}"
    ) {
      super.subCaptures(refs1, refs2, frozen)
    }

  def lastTrace(header: String): String = header + {
    try b.toString
    finally b.clear()
  }
end ExplainingTypeComparer
