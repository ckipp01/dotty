...
object StdNames:
  ...
  object str:
    ...
    def sanitize(str: String): String =
      disallowed.matcher(str).nn.replaceAll("""\$""").nn
  end str
...
// Keywords, need to come first -----------------------
    ...
    val BITMAP_PREFIX: N =
      "bitmap$" // @darkdimius: $bitmap? Also, the next 4 names are unused.
    val BITMAP_NORMAL: N =
      BITMAP_PREFIX // initialization bitmap for public/protected lazy vals
    val BITMAP_TRANSIENT: N =
      s"${BITMAP_PREFIX}trans$$" // initialization bitmap for transient lazy vals
    val BITMAP_CHECKINIT: N =
      s"${BITMAP_PREFIX}init$$" // initialization bitmap for checkinit values
    val BITMAP_CHECKINIT_TRANSIENT: N =
      s"${BITMAP_PREFIX}inittrans$$" // initialization bitmap for transient checkinit values
...
// ----- Type names -----------------------------------------
    ...
    final val RuntimeVisibleAnnotationATTR: N =
      "RuntimeVisibleAnnotations" // RetentionPolicy.RUNTIME
    final val RuntimeInvisibleAnnotationATTR: N =
      "RuntimeInvisibleAnnotations" // RetentionPolicy.CLASS
    final val RuntimeParamAnnotationATTR: N =
      "RuntimeVisibleParameterAnnotations" // RetentionPolicy.RUNTIME (annotations on parameters)
...
// ----- Term names -----------------------------------------
    ...
    val falseModuleClassNames: Set[N] =
      Set(nothingClass, nullClass, nothingRuntimeClass, nullRuntimeClass)
    ...
    val ConversionNames: Set[Name] =
      Set(toByte, toChar, toDouble, toFloat, toInt, toLong, toShort)
  ...
  class ScalaTypeNames extends ScalaNames[TypeName]:
    ...
    val JFunctionPrefix: Seq[TypeName] =
      (0 to 2).map(i => s"scala.runtime.java8.JFunction${i}")
    val JProcedure: Seq[TypeName] =
      (0 to 22).map(i => s"scala.runtime.function.JProcedure${i}")
  ...
  abstract class JavaNames[N <: Name] extends DefinedNames[N]:
    ...
    final val InvTargetException: N =
