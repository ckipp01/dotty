...
object TypeEval:
  ...
  def tryCompiletimeConstantFold(tp: AppliedType)(using Context): Type =
    tp.tycon match
      case tycon: TypeRef if defn.isCompiletimeAppliedType(tycon.symbol) =>
        extension (tp: Type)
          def fixForEvaluation: Type =
        ...
        def expectArgsNum(expectedNum: Int): Unit =
          ...
          assert(
            tp.args.length == expectedNum,
            s"Type operation expects $expectedNum arguments but found ${tp.args.length}"
          )

        def natValue(tp: Type): Option[Int] =
          intValue(tp).filter(n => n >= 0 && n < Int.MaxValue)
        ...
        trace(i"compiletime constant fold $tp", typr, show = true) {
          ...
          val constantType =
            ...
            else if owner == defn.CompiletimeOpsAnyModuleClass then
              name match
                ...
                case tpnme.IsConst =>
                  isConst(tp.args.head).map(b => ConstantType(Constant(b)))
                case _ => None
            else if owner == defn.CompiletimeOpsIntModuleClass then
              name match
                ...
                case tpnme.NumberOfLeadingZeros =>
                  constantFold1(intValue, Integer.numberOfLeadingZeros(_))
                ...
                case _              => None
            else if owner == defn.CompiletimeOpsLongModuleClass then
              name match
                ...
                case tpnme.NumberOfLeadingZeros =>
                  constantFold1(
                    longValue,
                    java.lang.Long.numberOfLeadingZeros(_)
                  )
                ...
                case _              => None
            else if owner == defn.CompiletimeOpsFloatModuleClass then
              name match
                ...
                case _              => None
            else if owner == defn.CompiletimeOpsDoubleModuleClass then
              name match
                ...
                case _             => None
            else if owner == defn.CompiletimeOpsStringModuleClass then
              name match
                ...
                case tpnme.Substring =>
                  constantFold3(
                    ...
                    intValue,
                    intValue,
                    (s, b, e) => s.substring(b, e)
                  )
                case tpnme.CharAt =>
                  constantFold2AB(stringValue, intValue, _ charAt _)
                case _ => None
            else if owner == defn.CompiletimeOpsBooleanModuleClass then
              name match
