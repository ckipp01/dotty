...
object NameKinds:
  ...
  @sharable private val simpleNameKinds =
    util.HashMap[Int, ClassifiedNameKind]()
  @sharable private val qualifiedNameKinds =
    util.HashMap[Int, QualifiedNameKind]()
  @sharable private val numberedNameKinds =
    util.HashMap[Int, NumberedNameKind]()
  ...
  /** An abstract base class of classes that define the kind of a derived name
    * info
    */
  abstract class NameKind(val tag: Int):
    ...
    class Info extends NameInfo:
      ...
      def mkString(underlying: TermName): String =
        self.mkString(underlying, this)
      override def toString: String = infoString

    /** Does this kind define logically a new name (respectively qualified
      * name)? Tested by the `replace` and `collect` combinators of class
      * `Name`.
      */
    def definesNewName: Boolean = false
    def definesQualifiedName: Boolean = false

    /** Unmangle simple name `name` into a name of this kind, or return original
      * name if this is not possible.
      */
    def unmangle(name: SimpleName): TermName = name

    /** Turn a name of this kind consisting of an `underlying` prefix and the
      * given `info` into a string. Used to turn structured into simple name.
      */
  ...
  object SimpleNameKind extends NameKind(UTF8):
    ...
    def mkString(underlying: TermName, info: ThisInfo): Nothing = unsupported(
      "mkString"
    )
  ...
  /** The kind of names that add a simple classification to an underlying name.
    */
  abstract class ClassifiedNameKind(tag: Int, val infoString: String)
  ...
  class PrefixNameKind(tag: Int, prefix: String, optInfoString: String = "")
      extends ClassifiedNameKind(
        tag,
        if optInfoString.isEmpty then s"Prefix $prefix" else optInfoString
      ):
    def mkString(underlying: TermName, info: ThisInfo): String =
    ...
    override def unmangle(name: SimpleName): TermName =
      if name.startsWith(prefix) then
        apply(name.drop(prefix.length).asSimpleName)
      else name

  /** The kind of names that get formed by appending a suffix to an underlying
    * name
    */
  class SuffixNameKind(tag: Int, suffix: String, optInfoString: String = "")
      extends ClassifiedNameKind(
        tag,
        if optInfoString.isEmpty then s"Suffix $suffix" else optInfoString
      ):
    def mkString(underlying: TermName, info: ThisInfo): String =
      underlying.qualToString(_.toString, n => n.toString + suffix)
    override def unmangle(name: SimpleName): TermName =
      if name.endsWith(suffix) then
        apply(name.take(name.length - suffix.length).asSimpleName)
      else name
  ...
  /** The kind of qualified names, consisting of an underlying name as a prefix,
    ...
    *
    * A qualified names always constitutes a new name, different from its
    * underlying name.
    */
  class QualifiedNameKind(tag: Int, val separator: String)
    ...
    case class QualInfo(name: SimpleName) extends Info with QualifiedInfo:
      override def map(f: SimpleName => SimpleName): NameInfo = new QualInfo(
        f(name)
      )
      override def toString: String = s"$infoString $name"
      override def hashCode =
        scala.runtime.ScalaRunTime._hashCode(this) * 31 + kind.hashCode
  ...
  /** The kind of numbered names consisting of an underlying name and a number
    */
  abstract class NumberedNameKind(tag: Int, val infoString: String)
      extends NameKind(tag):
    self =>
    type ThisInfo = NumberedInfo
    case class NumberedInfo(val num: Int)
        extends Info
        with NameKinds.NumberedInfo:
    ...
    def unapply(name: DerivedName): Option[(TermName, Int)] = name match
      case DerivedName(underlying, info: this.NumberedInfo) =>
        Some((underlying, info.num))
      case _ => None
    protected def skipSeparatorAndNum(
        name: SimpleName,
        separator: String
    ): Int =
      var i = name.length
  ...
  /** The kind of unique names that consist of an underlying name (can be
    * empty), a separator indicating the class of unique name, and a unique
    * number.
    *
    * A unique names always constitutes a new name, different from its
    * underlying name.
    */
  ...
  object AnyUniqueName:
    def unapply(name: DerivedName): Option[(TermName, TermName, Int)] =
      name match
        case DerivedName(qual, info: NumberedInfo) =>
          info.kind match
            case unique: UniqueNameKind =>
              Some((qual, unique.separatorName, info.num))
            case _ => None
        case _ => None
  ...
  /** Names of the form `prefix $ name` that are constructed as a result of
    * flattening
    */
  ...
  val ExpandPrefixName: QualifiedNameKind =
    new QualifiedNameKind(EXPANDPREFIX, "$")

  /** Expanded names of the form `prefix $$ name`. */
  val ExpandedName: QualifiedNameKind =
    new QualifiedNameKind(EXPANDED, str.EXPAND_SEPARATOR):
      ...
      override def unmangle(name: SimpleName): TermName =
        ...
        if i < 0 then name
          ...
          // instead of `x$$super`.
          if i > FalseSuperLength && name.slice(
              i - FalseSuperLength,
              i
            ) == FalseSuper
          then i -= FalseSuper.length
  ...
  val TraitSetterName: QualifiedNameKind =
    new QualifiedNameKind(TRAITSETTER, str.TRAIT_SETTER_SEPARATOR)
  ...
  val UniqueName: UniqueNameKind = new UniqueNameKind("$"):
    override def mkString(underlying: TermName, info: ThisInfo) =
      if underlying.isEmpty then "$" + info.num + "$"
      else super.mkString(underlying, info)
  ...
  val LazyBitMapName: UniqueNameKind = new UniqueNameKind(
    nme.BITMAP_PREFIX.toString
  )
  val NonLocalReturnKeyName: UniqueNameKind = new UniqueNameKind(
    "nonLocalReturnKey"
  )
  ...
  /** A kind of unique extension methods; Unlike other unique names, these can
    * be unmangled.
    */
  ...
  val DefaultGetterName: NumberedNameKind =
    new NumberedNameKind(DEFAULTGETTER, "DefaultGetter"):
      def mkString(underlying: TermName, info: ThisInfo) =
        val prefix =
          if underlying.isConstructorName then nme.DEFAULT_GETTER_INIT
          else underlying
      ...
      override def unmangle(name: SimpleName): TermName =
        ...
        if i > 0 then
          ...
          if original == nme.DEFAULT_GETTER_INIT then original = nme.CONSTRUCTOR
          apply(original, index)
        else name

  /** Names of the form N_<outer>. Emitted by inliner, replaced by outer path in
    * ExplicitOuter.
    */
  val OuterSelectName: NumberedNameKind =
    new NumberedNameKind(OUTERSELECT, "OuterSelect"):
      def mkString(underlying: TermName, info: ThisInfo) =
        assert(underlying.isEmpty)
        s"${info.num}_<outer>"

  val SuperAccessorName: PrefixNameKind =
    new PrefixNameKind(SUPERACCESSOR, "super$")
  val InitializerName: PrefixNameKind =
    new PrefixNameKind(INITIALIZER, "initial$")
  val ProtectedAccessorName: PrefixNameKind =
    new PrefixNameKind(PROTECTEDACCESSOR, "protected$")
  val InlineAccessorName: PrefixNameKind =
    new PrefixNameKind(INLINEACCESSOR, "inline$")

  /** See `ConstraintHandling#LevelAvoidMap`. */
  enum AvoidNameKind(tag: Int, prefix: String)
      extends PrefixNameKind(tag, prefix):
  ...
  val BodyRetainerName: SuffixNameKind =
    new SuffixNameKind(BODYRETAINER, "$retainedBody")
  val FieldName: SuffixNameKind = new SuffixNameKind(FIELD, "$$local"):
    override def mkString(underlying: TermName, info: ThisInfo) =
      underlying.toString
  val ExplicitFieldName: SuffixNameKind =
    new SuffixNameKind(EXPLICITFIELD, "$field")
  val ExtMethName: SuffixNameKind = new SuffixNameKind(EXTMETH, "$extension")
  val ParamAccessorName: SuffixNameKind =
    new SuffixNameKind(PARAMACC, "$accessor")
  val ModuleClassName: SuffixNameKind =
    new SuffixNameKind(OBJECTCLASS, "$", optInfoString = "ModuleClass")
  val DirectMethName: SuffixNameKind = new SuffixNameKind(DIRECT, "$direct")
  val AdaptedClosureName: SuffixNameKind =
    new SuffixNameKind(ADAPTEDCLOSURE, "$adapted"):
      override def definesNewName = true
  ...
  object SignedName extends NameKind(SIGNED):
    ...
    def unapply(name: DerivedName): Option[(TermName, Signature, TermName)] =
      name match
        case DerivedName(underlying, info: SignedInfo) =>
          Some((underlying, info.sig, info.target))
        case _ => None

    def mkString(underlying: TermName, info: ThisInfo): String =
      s"$underlying[with sig ${info.sig}]"
    def infoString: String = "Signed"

  /** Possible name kinds of a method that comes from Scala2 pickling info. and
    * that need to be unmangled. Note: Scala2 protected accessors and setters
  ...
  def simpleNameKindOfTag: util.ReadOnlyMap[Int, ClassifiedNameKind] =
    simpleNameKinds
  def qualifiedNameKindOfTag: util.ReadOnlyMap[Int, QualifiedNameKind] =
    qualifiedNameKinds
  def numberedNameKindOfTag: util.ReadOnlyMap[Int, NumberedNameKind] =
    numberedNameKinds
  def uniqueNameKindOfSeparator: util.ReadOnlyMap[String, UniqueNameKind] =
    uniqueNameKinds
end NameKinds
