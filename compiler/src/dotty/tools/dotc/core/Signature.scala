...
/** The signature of a denotation.
  ...
  *
  * Signature( List(2, "scala.Int".toTypeName,
  * "scala.collection.immutable.List".toTypeName),
...
case class Signature(paramsSig: List[ParamSig], resSig: TypeName):
  ...
  /** Two names are consistent if they are the same or one of them is
  ...
  /** Does this signature coincide with that signature on their parameter parts?
    ...
    * are either equal or on of them is tpnme.Uninstantiated.
    */
  final def consistentParams(that: Signature)(using Context): Boolean =
    @tailrec def loop(names1: List[ParamSig], names2: List[ParamSig]): Boolean =
      if names1.isEmpty then names2.isEmpty
      else
        !names2.isEmpty && consistent(names1.head, names2.head) && loop(
          names1.tail,
          names2.tail
        )
  ...
  /** The degree to which this signature matches `that`. If parameter signatures
    ...
    * is a wildcard), the result is `FullMatch`. If only the parameter
    * signatures are consistent, the result is either `MethodNotAMethodMatch`
    * (if one side is a method signature and the other isn't), or `ParamMatch`.
  ...
  final def matchDegree(that: Signature)(using Context): MatchDegree =
    if consistentParams(that) then
      if resSig == that.resSig || isWildcard(resSig) || isWildcard(that.resSig)
      then FullMatch
  ...
  /** Construct a signature by prepending the signature names of the given
    * `params` to the parameter part of this signature.
    *
    * Like Signature#apply, the result is only cacheable if `isUnderDefined ==
    * false`.
    */
  def prependTermParams(params: List[Type], sourceLanguage: SourceLanguage)(
      using Context
  ): Signature =
  ...
  /** Construct a signature by prepending the length of a type parameter section
    ...
    * false`.
    */
  def prependTypeParams(typeParamSigsSectionLength: Int)(using
      Context
  ): Signature =
  ...
  /** A signature is under-defined if its paramsSig part contains at least one
    * `tpnme.Uninstantiated`. Under-defined signatures arise when taking a
    * signature of a type that still contains uninstantiated type variables.
    */
...
object Signature:
  /** A parameter signature, see the documentation of `Signature` for more
    * information.
    */
  ...
  /** The signature of everything that's not a method, i.e. that has a type
    * different from PolyType or MethodType.
    */
  ...
  val OverloadedSignature: Signature =
    Signature(List(tpnme.OVERLOADED), EmptyTypeName)
  ...
  /** The signature of a method with no parameters and result type `resultType`.
    ...
    * been instantiated.
    */
  def apply(resultType: Type, sourceLanguage: SourceLanguage)(using
      Context
  ): Signature =
  ...
  val lexicographicOrdering: Ordering[Signature] = new Ordering[Signature]:
    val paramSigOrdering: Ordering[Signature.ParamSig] =
      new Ordering[Signature.ParamSig]:
        def compare(x: ParamSig, y: ParamSig): Int =
          x match // `(x, y) match` leads to extra allocations
            case x: TypeName =>
              y match
                case y: TypeName =>
    ...
    def compare(x: Signature, y: Signature): Int =
      ...
      val paramsOrdering =
        seqOrdering(paramSigOrdering).compare(x.paramsSig, y.paramsSig)
      if paramsOrdering != 0 then paramsOrdering
