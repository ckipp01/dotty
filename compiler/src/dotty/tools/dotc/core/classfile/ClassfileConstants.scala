...
object ClassfileConstants:
  ...
  /** (see http://java.sun.com/docs/books/jvms/second_edition/jvms-clarify.html)
    ...
    * set (ch. 2.13.1).
    *
    * A class file cannot have both its `ACC_FINAL` and `ACC_ABSTRACT` flags set
    * (ch. 2.8.2).
    *
    ...
    * `ACC_SYNCHRONIZED` flags set (ch. 2.13.3.2).
    *
    * All interface methods must have their `ACC_ABSTRACT` and `ACC_PUBLIC`
    * flags set.
    *
    * Note for future reference: see this thread on ACC_SUPER and how its
    * enforcement differs on the android vm.
  ...
  abstract class FlagTranslation:
    ...
    private def translateFlag(jflag: Int): FlagSet = (jflag: @switch) match
      ...
      case JAVA_ACC_ABSTRACT   => if isClass then Abstract else Deferred
    ...
    private def addFlag(base: FlagSet, jflag: Int): FlagSet =
      if jflag == 0 then base else base | translateFlag(jflag)
  ...
  val fieldTranslation: FlagTranslation = new FlagTranslation:
    override def baseFlags(jflags: Int) =
      if (jflags & JAVA_ACC_FINAL) == 0 then Mutable else EmptyFlags
  val methodTranslation: FlagTranslation = new FlagTranslation:
    override def baseFlags(jflags: Int) =
      if (jflags & JAVA_ACC_BRIDGE) != 0 then Bridge else EmptyFlags
end ClassfileConstants
