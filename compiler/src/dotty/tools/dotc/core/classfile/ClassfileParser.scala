...
import Contexts.*, Symbols.*, Types.*, Names.*, StdNames.*, NameOps.*, Scopes.*,
  Decorators.*
import SymDenotations.*, unpickleScala2.Scala2Unpickler.*, Constants.*,
  Annotations.*, util.Spans.*
import Phases.*
...
object ClassfileParser:
  ...
  /** Indicate that there is nothing to unpickle and the corresponding symbols
    * can be invalidated.
    */
  ...
  def cook(using Context): TypeMap = new TypeMap:
    def apply(tp: Type): Type = tp match
      case tp: TypeRef if tp.symbol.typeParams.nonEmpty =>
        AppliedType(
          tp,
          tp.symbol.typeParams.map(Function.const(TypeBounds.empty))
        )
      ...
      case tp @ TempPolyType(_, tpe) =>
        val tpe1 = this(tpe)
        if tpe1 eq tpe then tp else tp.copy(tpe = tpe1)
      case tp @ TempClassInfoType(parents, _, _) =>
        val parents1 = parents.mapConserve(this)
        if parents eq parents1 then tp else tp.copy(parentTypes = parents1)
      case _ =>
        mapOver(tp)
...
class ClassfileParser(
  ...
  protected val instanceScope: MutableScope = newScope(
    0
  ) // the scope of all instance definitions
  protected val staticScope: MutableScope = newScope(
    0
  ) // the scope of all static definitions
  protected var pool: ConstantPool = _ // the classfile's constant pool

  protected var currentClassName: SimpleName =
    _ // JVM name of the current class
  ...
  moduleRoot.info =
    NoLoader().withDecls(staticScope).withSourceModule(staticModule)

  private def currentIsTopLevel(using Context) =
    classRoot.owner.is(Flags.PackageClass)

  private def mismatchError(className: SimpleName) =
    throw new IOException(
      s"class file '${classfile.canonicalPath}' has location not matching its contents: contains class $className"
    )
  ...
  def run()(using Context): Option[Embedded] = try
    ...
    case e: RuntimeException =>
      if ctx.debug then e.printStackTrace()
      throw new IOException(
  ...
  private def parseHeader()(using in: DataReader): Unit =
    ...
    if magic != JAVA_MAGIC then
      throw new IOException(
        s"class file '${classfile}' has wrong magic number 0x${toHexString(magic)}, should be 0x${toHexString(JAVA_MAGIC)}"
      )
  ...
  def classNameToSymbol(name: Name)(using Context): Symbol =
    ...
    innerClasses.get(nameStr) match
      ...
      case None =>
        ...
        if split < 0 || split >= nameStr.length - 1 then lookupTopLevel()
        else
            ...
            case _ =>
              ...
              val r =
                if outerSym eq classRoot.symbol then
                  instanceScope
                    .lookup(innerName)
                    .orElse(staticScope.lookup(innerName))
                else
                  outerSym.info
                    .member(innerName)
                    .orElse(
                      outerSym.asClass.companionModule.info.member(innerName)
                    )
                    .symbol
              r.orElse(lookupTopLevel())
    end match
  end classNameToSymbol
  ...
  def parseClass()(using ctx: Context, in: DataReader): Option[Embedded] =
    ...
    /** Parse parents for Java classes. For Scala, return AnyRef, since the real
      * type will be unpickled. Updates the read pointer of 'in'.
      */
    def parseParents: List[Type] =
      ...
      var ifaces =
        for (i <- (0 until ifaceCount).toList)
          yield pool.getSuperClass(in.nextChar).typeRef
      // Dotty deviation: was
    ...
    if !result.isDefined then
      var classInfo: Type =
        TempClassInfoType(parseParents, instanceScope, classRoot.symbol)
      ...
      for i <- 0 until in.nextChar do parseMember(method = false)
      for i <- 0 until in.nextChar do parseMember(method = true)
  ...
  def addEnclosingTParams()(using Context): Unit =
    ...
    while sym.isClass && !sym.is(Flags.ModuleClass) do
      for tparam <- sym.typeParams do
  ...
  def parseMember(method: Boolean)(using ctx: Context, in: DataReader): Unit =
    ...
    val jflags = in.nextChar
    val sflags =
      if method then Flags.Method | methodTranslation.flags(jflags)
      else fieldTranslation.flags(jflags)
    val preName = pool.getName(in.nextChar)
    if !sflags.isOneOf(
        Flags.PrivateOrArtifact
      ) || preName.name == nme.CONSTRUCTOR
    then
  ...
  class MemberCompleter(name: SimpleName, jflags: Int, sig: String)
    ...
    def complete(denot: SymDenotation)(using Context): Unit =
      ...
      def normalizeConstructorParams() =
        innerClasses.get(currentClassName.toString) match
          case Some(entry) if !isStatic(entry.jflags) =>
            val mt @ MethodTpe(paramNames, paramTypes, resultType) =
              denot.info: @unchecked
            var normalizedParamNames = paramNames.tail
            var normalizedParamTypes = paramTypes.tail
            if (jflags & JAVA_ACC_SYNTHETIC) != 0 then
            ...
            denot.info = mt.derivedLambdaType(
      ...
      /** Make return type of constructor be the enclosing class type, and make
        * constructor type polymorphic in the type parameters of the class
        */
      def normalizeConstructorInfo() =
        val rt =
          classRoot.typeRef appliedTo (classRoot.typeParams map (_.typeRef))

        def resultType(tpe: Type): Type = tpe match
          case mt @ MethodType(paramNames) =>
            mt.derivedLambdaType(paramNames, mt.paramInfos, rt)
          case pt: PolyType =>
            pt.derivedLambdaType(
              pt.paramNames,
              pt.paramInfos,
              resultType(pt.resType)
            )
      ...
      if isConstructor then normalizeConstructorParams()
      if isNative then
        attrCompleter.annotations ::= Annotation.deferredSymAndTree(
          defn.NativeAnnot
        )(New(defn.NativeAnnot.typeRef, Nil))
      if isTransient then
        attrCompleter.annotations ::= Annotation.deferredSymAndTree(
          defn.TransientAnnot
        )(New(defn.TransientAnnot.typeRef, Nil))
      if isVolatile then
        attrCompleter.annotations ::= Annotation.deferredSymAndTree(
          defn.VolatileAnnot
        )(New(defn.VolatileAnnot.typeRef, Nil))
      denot.info = translateTempPoly(
        attrCompleter.complete(denot.info, isVarargs)
      )
      if isConstructor then normalizeConstructorInfo()

      if ctx.explicitNulls then
        denot.info =
          JavaNullInterop.nullifyMember(denot.symbol, denot.info, isEnum)
  ...
  /** As specified in
    ...
    * represented as a CONSTANT_INTEGER, so we need to convert it to produce a
    * correctly-typed tree. We need to do this each time the constant is
    * accessed instead of storing the result of the conversion in the `values`
    * cache, because the same constant might be used for annotation arguments of
    * different type.
    */
  def convertTo(ct: Constant, pt: Type)(using Context): Constant =
    if (pt eq defn.BooleanType) && ct.tag == IntTag then Constant(ct.value != 0)
    else ct.convertTo(pt)

  private def sigToType(
      sig: String,
      owner: Symbol = null,
    ...
    def subName(isDelimiter: Char => Boolean): SimpleName =
      val start = index
      while !isDelimiter(sig(index)) do index += 1
    ...
    def sig2type(tparams: immutable.Map[Name, Symbol], skiptvs: Boolean)(using
        Context
    ): Type =
      val tag = sig(index); index += 1
      (tag: @switch) match
        case 'L' =>
          /** A type representation where inner classes become `A#B` instead of
            * `A.this.B` (like with `typeRef`)
            *
            * Note: the symbol must not be nested in a generic class.
            */
          def innerType(symbol: Symbol): Type =
            if symbol.is(Flags.Package) then symbol.thisType
            else if symbol.isType then TypeRef(innerType(symbol.owner), symbol)
            else throw new RuntimeException("unexpected term symbol " + symbol)

          def processTypeArgs(tp: Type): Type = tp match
            case tp: TypeRef =>
              if sig(index) == '<' then
                accept('<')
                val argsBuf = if skiptvs then null else new ListBuffer[Type]
                while sig(index) != '>' do
                    ...
                    case variance @ ('+' | '-' | '*') =>
                      ...
                      variance match
                        ...
                        case '-' =>
                          ...
                          if argTp.typeSymbol == defn.AnyClass then
                            TypeBounds.upper(defn.FromJavaObjectType)
                  ...
                  if argsBuf != null then argsBuf += arg
                accept('>')
                if skiptvs then tp else AppliedType(tp, argsBuf.toList)
              else tp
            case tp =>
              assert(sig(index) != '<', tp)
              tp
          ...
          val classTpe =
            if classSym eq defn.ObjectClass then defn.FromJavaObjectType
            else innerType(classSym)
          ...
          tpe
        case ARRAY_TAG =>
          while '0' <= sig(index) && sig(index) <= '9' do index += 1
        ...
        case '(' =>
          ...
          /** Is this a repeated parameter type? This is true if we're in a
            * vararg method and this is the last parameter.
            */
          ...
          while !isMethodEnd(index) do
            ...
            paramtypes += {
              ...
              else sig2type(tparams, skiptvs = false)
            }
        ...
        case 'T' =>
          ...
          if skiptvs then defn.AnyType else tparams(n).typeRef
        case tag =>
    ...
    def sig2typeBounds(tparams: immutable.Map[Name, Symbol], skiptvs: Boolean)(
        using Context
    ): Type =
      val ts = new ListBuffer[Type]
      while sig(index) == ':' do
        index += 1
        if sig(index) != ':' then // guard against empty class bound
          val tp = sig2type(tparams, skiptvs)
          if !skiptvs then ts += cook(tp)
      if !skiptvs then
        val bound =
          if ts.isEmpty then defn.AnyType else ts.reduceLeft(AndType.apply)
    ...
    if sig(index) == '<' then
      ...
      while sig(index) != '>' do
        ...
        val s = newSymbol(
          ...
          typeParamCompleter(index),
          coord = indexCoord(index)
        )
        if owner.isClass then owner.asClass.enter(s)
  ...
  def parseAnnotArg(
      skip: Boolean = false
  )(using ctx: Context, in: DataReader): Option[untpd.Tree | EnumTag] =
    ...
    tag match
      ...
      case CLASS_TAG =>
        if skip then None else Some(lit(Constant(pool.getType(index))))
      case ENUM_TAG =>
        val sig = pool.getExternalName(index).value
        val enumCaseName = pool.getName(in.nextChar)
        if skip then None else Some(EnumTag(sig, enumCaseName))
      case ARRAY_TAG =>
  ...
  class ClassfileAnnotation(
      annotType: Type,
      lazyArgs: List[(NameOrString, untpd.Tree | EnumTag)]
  ) extends LazyAnnotation:
    private def args(using Context): List[untpd.Tree] =
      lazyArgs.map {
        case (name, tree: untpd.Tree) =>
          untpd.NamedArg(name.name, tree).withSpan(NoSpan)
        case (name, tag: EnumTag) =>
          untpd.NamedArg(name.name, tag.toTree).withSpan(NoSpan)
      }
  ...
  /** Parse and return a single annotation. If it is malformed, return None.
    */
  def parseAnnotation(attrNameIndex: Char, skip: Boolean = false)(using
      ctx: Context,
      in: DataReader
  ): Option[ClassfileAnnotation] = try
    ...
    attrType match
      case tp: TypeRef if tp.denot.infoOrCompleter.isInstanceOf[StubInfo] =>
        ...
        if ctx.debug then
          report.warning(
            em"Error while parsing annotations in ${classfile}: annotation class $tp not present on classpath"
          )
        None
      case _ =>
        if hasError || skip then None
  ...
  /** A completer for attributes
    *
    * Top-level classes complete attributes eagerly, while members complete
    * lazily.
    *
    * @note
    *   We cannot simply store the bytes of attributes, as the bytes may contain
    *   references to the constant pool, where the constants are loaded lazily.
    */
      ...
      def fillInParamNames(t: Type): Type = t match
        case mt @ MethodType(oldp) if namedParams.nonEmpty =>
          mt.derivedLambdaType(
            List.tabulate(oldp.size)(n => namedParams.getOrElse(n, oldp(n)))
          )
        case pt: PolyType if namedParams.nonEmpty =>
          pt.derivedLambdaType(
            pt.paramNames,
            pt.paramInfos,
            fillInParamNames(pt.resultType)
          )
        case _ => t

      cook.apply(fillInParamNames(newType))
    end complete
  end AttributeCompleter

  def parseAttributes(
      sym: Symbol
  )(using ctx: Context, in: DataReader): AttributeCompleter =
    ...
    def parseAttribute(): Unit =
      ...
      attrName match
        ...
        case tpnme.DeprecatedATTR =>
          val msg = Literal(
            Constant("see corresponding Javadoc for more information.")
          )
          val since = Literal(Constant(""))
          res.annotations ::= Annotation.deferredSymAndTree(
            defn.DeprecatedAnnot
          ) {
        ...
        case tpnme.ConstantValueATTR =>
          ...
          if c ne null then res.constant = c
          else
            report.warning(
              em"Invalid constant in attribute of ${sym.showLocated} while parsing ${classfile}"
            )
        ...
        case tpnme.AnnotationDefaultATTR =>
          sym.addAnnotation(
            Annotation(defn.AnnotationDefaultAnnot, Nil, sym.span)
          )
        ...
        case tpnme.CodeATTR =>
          ...
          if sym.owner.flagsUNSAFE.isAllOf(Flags.JavaInterface) then
    ...
    /** Parse the "Exceptions" attribute which denotes the exceptions thrown by
      * a method.
      */
    def parseExceptions(len: Int): Unit =
      val nClasses = in.nextChar
      for n <- 0 until nClasses do
        // FIXME: this performs an equivalent of getExceptionTypes instead of getGenericExceptionTypes (SI-7065)
        val cls = pool.getClassName(in.nextChar.toInt)
        res.exceptions ::= cls

    /** Parse a sequence of annotations and attaches them to the current symbol
      * sym, except for the ScalaSignature annotation that it returns, if it is
      * available.
      */
    def parseAnnotations(len: Int): Unit =
      val nAttr = in.nextChar
      for n <- 0 until nAttr do
    ...
    for i <- 0 until in.nextChar do parseAttribute()

    res
  end parseAttributes

  /** Annotations in Scala are assumed to get all their arguments as constructor
    * parameters. For Java annotations we need to fake it by making up the
    * constructor.
    */
  def addAnnotationConstructor(
      classInfo: TempClassInfoType
  )(using Context): Unit =
    newSymbol(
  ...
  class AnnotConstructorCompleter(classInfo: TempClassInfoType)
      extends LazyType:
    def complete(denot: SymDenotation)(using Context): Unit =
      val attrs = classInfo.decls.toList.filter(sym =>
        sym.isTerm && sym != denot.symbol && sym.name != nme.CONSTRUCTOR
      )
  ...
  /** Enter own inner classes in the right scope. It needs the scopes to be set
    * up, and implicitly current class' superclasses.
    */
  private def enterOwnInnerClasses()(using Context, DataReader): Unit =
    ...
    for entry <- innerClasses.valuesIterator do
      ...
      if entry.outer.name == currentClassName then
        val file = ctx.platform.classPath.findClassFile(
          entry.externalName.toString
        ) getOrElse {
  ...
  /** Parse inner classes. Expects `in.bp` to point to the superclass entry.
    * Restores the old `bp`.
    * @return
    *   Some(unpickler) iff classfile is from Scala, so no Java info needs to be
    *   read.
    */
  def unpickleOrParseInnerClasses()(using
      ctx: Context,
      in: DataReader
  ): Option[Embedded] =
    val oldbp = in.bp
    try
      ...
      def unpickleScala(bytes: Array[Byte]): Some[Embedded] =
        ...
        if allowed != "always" then
          ...
          failUnless(
            file == null || allowedList.exists(path =>
              file.toString.contains(path)
            )
          )

        val unpickler =
          new unpickleScala2.Scala2Unpickler(bytes, classRoot, moduleRoot)(ctx)
      ...
      def unpickleTASTY(bytes: Array[Byte]): Some[Embedded] =
        ...
        unpickler.enter(roots =
          Set(classRoot, moduleRoot, moduleRoot.sourceModule)
        )(using ctx.withSource(util.NoSource))
        Some(unpickler)
      ...
      if scan(tpnme.TASTYATTR) then
        val attrLen = in.nextInt
        val bytes = in.nextBytes(attrLen)
        if attrLen == 16
        then // A tasty attribute with that has only a UUID (16 bytes) implies the existence of the .tasty file
          val tastyBytes: Array[Byte] =
            classfile match // TODO: simplify when #3552 is fixed
              case classfile: io.ZipArchive#Entry => // We are in a jar
                val path = classfile.parent.lookupName(
                  classfile.name.stripSuffix(".class") + ".tasty",
                  directory = false
                )
                if path != null then
              ...
              case _ =>
                ...
                if tastyFileOrNull == null then
                  report.error(em"Could not find TASTY file $name under $dir")
                  Array.empty
                else tastyFileOrNull.toByteArray
          if tastyBytes.nonEmpty then
            ...
            if expectedUUID != tastyUUID then
              report.warning(
                s"$classfile is out of sync with its TASTy file. Loaded TASTy file. Try cleaning the project to fix this issue",
                NoSourcePosition
              )
      ...
      if scan(tpnme.ScalaATTR) && !scalaUnpickleWhitelist.contains(
          classRoot.name
        )
        && !(classRoot.name.startsWith("Tuple") && classRoot.name.endsWith(
          "$sp"
        ))
        && !(classRoot.name.startsWith("Product") && classRoot.name.endsWith(
          "$sp"
        ))
      then
      ...
      if scan(tpnme.ScalaSignatureATTR) && scan(
          tpnme.RuntimeVisibleAnnotationATTR
        )
      then
        val attrLen = in.nextInt
        ...
        while i < nAnnots do
          ...
          while j < nArgs do
            ...
            if argName.name == nme.bytes then
      ...
      if scan(tpnme.InnerClassesATTR) then
        val attrLen = in.nextInt
        ...
        for i <- 0 until entries do
          ...
          val jflags = in.nextChar
          if innerIndex != 0 && outerIndex != 0 && nameIndex != 0 then
  ...
  end unpickleOrParseInnerClasses

  /** An entry in the InnerClasses attribute of this class file. */
  case class InnerClassEntry(
      external: NameOrString,
      outer: NameOrString,
      name: NameOrString,
      jflags: Int
  ):
  ...
  private object innerClasses extends util.HashMap[String, InnerClassEntry]:
    /** Return the Symbol of the top level class enclosing `name`, or 'name's
      * symbol if no entry found for `name`.
      */
    def topLevelClass(name: String)(using Context): Symbol =
      val tlName = if contains(name) then
        var entry = this(name)
        while contains(entry.outerName) do entry = this(entry.outerName)
        entry.outerName
      else name
  ...
  def skipAttributes()(using in: DataReader): Unit =
    ...
    for i <- 0 until attrCount do
      in.skip(2); in.skip(in.nextInt)

  def skipMembers()(using in: DataReader): Unit =
    val memberCount = in.nextChar
    for i <- 0 until memberCount do
  ...
  protected def getOwner(flags: Int): Symbol =
    if isStatic(flags) then moduleRoot.symbol else classRoot.symbol

  protected def getScope(flags: Int): MutableScope =
    if isStatic(flags) then staticScope else instanceScope

  private def getPrivateWithin(jflags: Int)(using Context): Symbol =
    if (jflags & (JAVA_ACC_PRIVATE | JAVA_ACC_PUBLIC)) == 0 then
  ...
  protected class NameOrString(val value: String):
    ...
    def name: SimpleName =
      if _name eq null then _name = termName(value)
      _name
  ...
  class ConstantPool(using in: DataReader):
    ...
    def getName(index: Int)(using in: DataReader): NameOrString =
      if index <= 0 || len <= index then errorBadIndex(index)

      values(index) match
        case name: NameOrString => name
        case null =>
          val start = starts(index)
          if in.getByte(start).toInt != CONSTANT_UTF8 then errorBadTag(start)
          val len = in.getChar(start + 1).toInt
          val name = new NameOrString(in.getUTF(start + 1, len + 2))
          values(index) = name
          name

    /** Return the name found at given index in the constant pool, with '/'
      * replaced by '.'.
      */
    def getExternalName(index: Int)(using in: DataReader): NameOrString =
      if index <= 0 || len <= index then errorBadIndex(index)

      if internalized(index) == null then
        internalized(index) = new NameOrString(
          getName(index).value.replace('/', '.')
        )
    ...
    def getClassSymbol(index: Int)(using ctx: Context, in: DataReader): Symbol =
      ...
      if c eq null then
        val start = starts(index)
        if in.getByte(start).toInt != CONSTANT_CLASS then errorBadTag(start)
    ...
    /** Return the type of a class constant entry. Since arrays are considered
      * to be class types, they might appear as entries in 'newarray' or 'cast'
      * opcodes.
      */
    def getClassOrArrayType(
        index: Int
    )(using ctx: Context, in: DataReader): Type =
      if index <= 0 || len <= index then errorBadIndex(index)
    ...
    def getType(index: Int, isVarargs: Boolean = false)(using
        Context,
        DataReader
    ): Type =
    ...
    def getBytes(indices: List[Int])(using in: DataReader): Array[Byte] =
      ...
      if value eq null then
        ...
        for index <- indices do
          if index <= 0 || ConstantPool.this.len <= index then
            errorBadIndex(index)
          val start = starts(index)
          if in.getByte(start).toInt != CONSTANT_UTF8 then errorBadTag(start)
          val len = in.getChar(start + 1)
    ...
    private def errorBadIndex(index: Int)(using in: DataReader) =
      throw new RuntimeException(
        "bad constant pool index: " + index + " at pos: " + in.bp
      )

    /** Throws an exception signaling a bad tag at given address. */
    private def errorBadTag(start: Int)(using in: DataReader) =
      throw new RuntimeException(
        "bad constant pool tag " + in.getByte(start) + " at byte " + start
      )
  end ConstantPool
end ClassfileParser
