...
final class ReusableDataReader() extends DataReader:
  ...
  private[this] val reader: DataInputStream =
    val stream = new InputStream:
      override def read(): Int = try bb.get & 0xff
      catch case _: BufferUnderflowException => -1
  ...
  private def nextPositivePowerOfTwo(target: Int): Int =
    1 << -Integer.numberOfLeadingZeros(target - 1)
  ...
  def reset(file: dotty.tools.io.AbstractFile): this.type =
    ...
    file.sizeOption match
      case Some(size) =>
        ...
        else java.util.Arrays.fill(data, 0.toByte)
