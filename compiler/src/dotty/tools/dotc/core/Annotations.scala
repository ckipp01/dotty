...
object Annotations:
  ...
  def annotClass(tree: Tree)(using Context) =
    if tree.symbol.isConstructor then tree.symbol.owner
  ...
  abstract class Annotation extends Showable:
    ...
    def derivedAnnotation(tree: Tree)(using Context): Annotation =
      if tree eq this.tree then this else Annotation(tree)
    ...
    def argumentConstant(i: Int)(using Context): Option[Constant] =
      for (
        case ConstantType(c) <- argument(
          i
        ) map (_.tpe.widenTermRefExpr.normalized)
      ) yield c
    ...
    /** Normally, applies a type map to all tree nodes of this annotation, but
      * can be overridden. Returns EmptyAnnotation if type type map produces a
      * range type, since ranges cannot be types of trees.
      */
    ...
    def hasOneOfMetaAnnotation(
        metaSyms: Set[Symbol],
        orNoneOf: Set[Symbol] = Set.empty
    )(using Context): Boolean = atPhaseNoLater(erasurePhase) {
      def go(metaSyms: Set[Symbol]) =
        def recTp(tp: Type): Boolean = tp.dealiasKeepAnnots match
          case AnnotatedType(parent, metaAnnot) =>
            metaSyms.exists(metaAnnot.matches) || recTp(parent)
          case _ => false
        def rec(tree: Tree): Boolean = methPart(tree) match
          case New(tpt)        => rec(tpt)
          case Select(qual, _) => rec(qual)
          case Annotated(arg, metaAnnot) =>
            metaSyms.exists(metaAnnot.tpe.classSymbol.derivesFrom) || rec(arg)
  ...
  class DeferredSymAndTree(symFn: Context ?=> Symbol, treeFn: Context ?=> Tree)
    ...
    protected var mySym: Symbol | (Context ?=> Symbol) | Null = ctx ?=>
      symFn(using ctx)
    protected var myTree: Tree | (Context ?=> Tree) | Null = ctx ?=>
      treeFn(using ctx)

  /** An annotation indicating the body of a right-hand side, typically of an
    * inline method. Treated specially in pickling/unpickling and TypeTreeMaps
    */
  abstract class BodyAnnotation extends Annotation:
    ...
    override def derivedAnnotation(tree: Tree)(using Context): Annotation =
      if tree eq this.tree then this else ConcreteBodyAnnotation(tree)
  ...
  object Annotation:
    ...
    def apply(cls: ClassSymbol, arg: Tree, span: Span)(using
        Context
    ): Annotation =
      apply(cls, arg :: Nil, span)

    def apply(cls: ClassSymbol, args: List[Tree], span: Span)(using
        Context
    ): Annotation =
    ...
    def apply(atp: Type, args: List[Tree], span: Span)(using
        Context
    ): Annotation =
    ...
    /** Create an annotation where the symbol and the tree are computed lazily.
      */
    def deferredSymAndTree(symFn: Context ?=> Symbol)(
        treeFn: Context ?=> Tree
    ): Annotation =
    ...
    object Child:
      ...
      def later(delayedSym: Context ?=> Symbol, span: Span)(using
          Context
      ): Annotation =
        def makeChildLater(using Context) =
          val sym = delayedSym
          New(
            defn.ChildAnnot.typeRef.appliedTo(
              sym.owner.thisType.select(sym.name, sym)
            ),
            Nil
          )
            .withSpan(span)
      ...
      def unapply(ann: Annotation)(using Context): Option[Symbol] =
        if ann.symbol == defn.ChildAnnot then
  ...
  /** Extracts the type of the thrown exception from an annotation.
    *
    * Supports both "old-style" `@throws(classOf[Exception])` as well as
    * "new-style" `@throws[Exception]("cause")` annotations.
    */
  object ThrownException:
    def unapply(a: Annotation)(using Context): Option[Type] =
      if a.symbol ne defn.ThrowsAnnot then None
      else
        a.argumentConstant(0) match
