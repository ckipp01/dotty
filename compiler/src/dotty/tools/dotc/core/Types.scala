...
object Types:
  ...
  /** Main class representing types.
    ...
    * ```
    *
    * Note: please keep in sync with copy in
...
//    }
    ...
    /** A cache indicating whether the type was still provisional, last time we
      * checked
      */
    @sharable private var mightBeProvisional = true

    /** Is this type still provisional? This is the case if the type contains,
      * or depends on, uninstantiated type variables or type symbols that have
      * the Provisional flag set. This is an antimonotonic property - once a
      * type is not provisional, it stays so forever.
      */
    def isProvisional(using Context): Boolean =
      mightBeProvisional && testProvisional
    ...
    final def isValueTypeOrLambda: Boolean =
      isValueType || this.isInstanceOf[TypeLambda]

    /** Is this a value type or a wildcard? */
    final def isValueTypeOrWildcard: Boolean =
      isValueType || this.isInstanceOf[WildcardType]

    /** Does this type denote a stable reference (i.e. singleton type)?
      *
      * Like in isStableMember, "stability" means idempotence. Rationale: If an
      * expression has a stable type, the expression must be idempotent, so
      * stable types must be singleton types of stable expressions.
      */
    final def isStable(using Context): Boolean = stripTypeVar match
      case tp: TermRef =>
        tp.symbol.isStableMember && tp.prefix.isStable || tp.info.isStable
    ...
    /** Is this type a (possibly refined, applied, aliased or annotated) type
      * reference to the given type symbol?
      * @sym
      *   The symbol to compare to. It must be a class symbol or abstract type.
      *   It makes no sense for it to be an alias type because isRef would
      *   always return false in that case.
      */
    def isRef(sym: Symbol, skipRefined: Boolean = true)(using
        Context
    ): Boolean = this match
      case this1: TypeRef =>
      ...
      case this1: AppliedType =>
        ...
        if this2 ne this1 then this2.isRef(sym, skipRefined)
    ...
    /** Is this type a (neither aliased nor applied nor annotated) reference to
      * class `sym`?
      */
    ...
    def isAny(using Context): Boolean =
      isRef(defn.AnyClass, skipRefined = false)
    def isAnyRef(using Context): Boolean =
      isRef(defn.ObjectClass, skipRefined = false)
    def isAnyKind(using Context): Boolean =
      isRef(defn.AnyKindClass, skipRefined = false)
    ...
    /** Is this type exactly Nothing (no vars, aliases, refinements etc
      * allowed)?
      */
    ...
    def isBottomType(using Context): Boolean =
      if ctx.mode.is(Mode.SafeNulls) && !ctx.phase.erasedTypes then
        hasClassSymbol(defn.NothingClass)
    ...
    /** True if this type is an instance of the given `cls` or an instance of a
      * non-bottom subclass of `cls`.
      */
    final def derivesFrom(cls: Symbol)(using Context): Boolean =
      ...
      def loop(tp: Type): Boolean = tp match
        case tp: TypeRef =>
          val sym = tp.symbol
          if sym.isClass then sym.derivesFrom(cls) else loop(tp.superType)
        case tp: AppliedType =>
        ...
        case tp: OrType =>
          ...
          if isLowerBottomType(tp.tp1) then loop(tp.tp2)
          else if isLowerBottomType(tp.tp2) then loop(tp.tp1)
          else loop(tp.tp1) && loop(tp.tp2)
    ...
    def containsFromJavaObject(using Context): Boolean = this match
      case tp: OrType =>
        tp.tp1.containsFromJavaObject || tp.tp2.containsFromJavaObject
      case tp: AndType =>
        tp.tp1.containsFromJavaObject && tp.tp2.containsFromJavaObject
      case _ => isFromJavaObject

    /** True iff `symd` is a denotation of a class type parameter and the
      * reference `<pre> . <symd>` is an actual argument reference, i.e. `pre`
      * is not the ThisType of `symd`'s owner, or a reference to `symd`'s
      * owner.'
      */
    def isArgPrefixOf(symd: SymDenotation)(using Context): Boolean =
      symd.exists
        && !symd.owner.is(
          Package
        ) // Early exit if possible because the next check would force SymbolLoaders
    ...
    def isSingleton(using Context): Boolean =
      dealias.isInstanceOf[SingletonType]
    ...
    def hasAnyKind(using Context): Boolean =
      @tailrec def loop(tp: Type): Boolean = tp match
        case tp: TypeRef =>
          val sym = tp.symbol
          if sym.isClass then sym == defn.AnyKindClass
          else loop(tp.translucentSuperType)
        case tp: TypeProxy =>
          loop(
            tp.underlying
          ) // underlying OK here since an AnyKinded type cannot be a type argument of another type
        case _ =>
          false
      loop(this)
    ...
    final def isNotNull(using Context): Boolean = this match
      ...
      case tp: ClassInfo =>
        !tp.cls.isNullableClass && tp.cls != defn.NothingClass
    ...
    final def isError(using Context): Boolean =
      stripTypeVar.isInstanceOf[ErrorType]

    /** Is some part of the widened version of this type produced as a repair
      * for an error?
      */
    ...
    /** Is this type unusable for implicit search or overloading resolution
      * since it has embedded errors that can match anything? This is weaker and
      * more ad-hoc than isErroneous. The main differences are that we always
      * consider aliases (since these are relevant for inference or resolution)
      * but never consider prefixes (since these often do not constrain the
      * search space anyway).
      */
    def unusableForInference(using Context): Boolean = widenDealias match
      case AppliedType(tycon, args) =>
        tycon.unusableForInference || args.exists(_.unusableForInference)
      case RefinedType(parent, _, rinfo) =>
        parent.unusableForInference || rinfo.unusableForInference
      case TypeBounds(lo, hi) =>
        lo.unusableForInference || hi.unusableForInference
      case tp: AndOrType =>
        tp.tp1.unusableForInference || tp.tp2.unusableForInference
      case tp: LambdaType =>
        tp.resultType.unusableForInference || tp.paramInfos.exists(
          _.unusableForInference
        )
      case WildcardType(optBounds) => optBounds.unusableForInference
      case CapturingType(parent, refs) =>
        parent.unusableForInference || refs.elems.exists(_.unusableForInference)
      case _: ErrorType => true
      case _            => false

    /** Does the type carry an annotation that is an instance of `cls`? */
    @tailrec final def hasAnnotation(cls: ClassSymbol)(using Context): Boolean =
      stripTypeVar match
        case AnnotatedType(tp, annot) =>
          (annot matches cls) || (tp hasAnnotation cls)
        case _ => false

    /** Does this type have a supertype with an annotation satisfying given
      * predicate `p`?
      */
    def derivesAnnotWith(p: Annotation => Boolean)(using Context): Boolean =
      this match
    ...
    /** Is this the type of a method that has a repeated parameter type as last
      * parameter type?
      */
    def isVarArgsMethod(using Context): Boolean = stripPoly match
      case mt: MethodType =>
        mt.paramInfos.nonEmpty && mt.paramInfos.last.isRepeatedParam
      case _ => false
    ...
    /** Is this a Method or PolyType which has implicit or contextual
      * parameters?
      */
    def isImplicitMethod: Boolean = false

    /** Is this a Method or PolyType which has contextual parameters as first
    ...
    def underlyingMatchType(using Context): Type = stripped match
      ...
      case tp: AppliedType if tp.isMatchAlias =>
        tp.superType.underlyingMatchType
      case _ => NoType

    /** Is this a higher-kinded type lambda with given parameter variances?
      * These lambdas are used as the RHS of higher-kinded abstract types or
      * type aliases. The variance info is strictly needed only for abstract
      * types. For type aliases we allow the user to write the variance, and we
      * use it to check that the structural variance of the type lambda is
      * compatible with the declared variance, and that the declared variance is
      * compatible with any abstract types that are overridden.
      *
      * But it's important to note that the variance of a type parameter in a
      * type lambda is strictly determined by how it occurs in the body of the
      * lambda. Declared variances have no influence here. For instance the
      * following two lambdas are variant, even though no parameter variance is
      * indicated:
      *
      * [X] =>> List[X] // covariant [X] =>> X => Unit // contravariant
      *
      * Why store declared variances in lambdas at all? It's because type
      * symbols are just normal symbols, and there is no field in a Symbol that
      * keeps a list of variances. Generally we have the design that we store
      * all info that applies to some symbols but not others in the symbol's
      * types.
      */
...
// ----- Higher-order combinators -----------------------------------
    ...
    /** Returns true if there is a part of this type that satisfies predicate
      * `p`.
      */
    final def existsPart(
        p: Type => Boolean,
    ...
    final def foreachPart(p: Type => Unit, stopAt: StopAt = StopAt.None)(using
        Context
    ): Unit =
      new ForeachAccumulator(p, stopAt).apply((), this)

    /** The parts of this type which are type or term refs and which satisfy
      * predicate `p`.
      *
      * @param p
      *   The predicate to satisfy
      */
    def namedPartsWith(p: NamedType => Boolean)(using
        Context
    ): List[NamedType] =
      new NamedPartsAccumulator(p).apply(Nil, this)

    /** Map function `f` over elements of an AndType, rebuilding with function
      * `g`
      */
    def mapReduceAnd[T](f: Type => T)(g: (T, T) => T)(using Context): T =
      stripTypeVar match
        case AndType(tp1, tp2) =>
          g(tp1.mapReduceAnd(f)(g), tp2.mapReduceAnd(f)(g))
        case _ => f(this)

    /** Map function `f` over elements of an OrType, rebuilding with function
      * `g`
      */
...
// ----- Associated symbols ----------------------------------------------
    ...
    /** The least class or trait of which this type is a subtype or
      * parameterized instance, or NoSymbol if none exists (either because this
      * type is not a value type, or because superclasses are ambiguous).
      */
    final def classSymbol(using Context): Symbol = this match
      case tp: TypeRef =>
        val sym = tp.symbol
        if sym.isClass then sym else tp.superType.classSymbol
      case tp: TypeProxy =>
      ...
      case tp: OrType =>
        if tp.tp1.hasClassSymbol(defn.NothingClass) then tp.tp2.classSymbol
        else if tp.tp2.hasClassSymbol(defn.NothingClass) then tp.tp1.classSymbol
        else
          ...
          if ctx.erasedTypes && (tp1Null || tp2Null) then
            val otherSide =
              if tp1Null then tp.tp2.classSymbol else tp.tp1.classSymbol
    ...
    /** The least (wrt <:<) set of symbols satisfying the `include` predicate of
      * which this type is a subtype
      */
    final def parentSymbols(include: Symbol => Boolean)(using
        Context
    ): List[Symbol] = this match
      case tp: TypeRef =>
        val sym = tp.symbol
        if include(sym) then sym :: Nil else tp.superType.parentSymbols(include)
      case tp: TypeProxy =>
      ...
      case OrType(l, r) =>
        l.parentSymbols(include) intersect r.parentSymbols(
          include
        ) // TODO does not conform to spec
      case _ =>
        Nil
    ...
    /** Same as hasClassSmbol(MatchableClass), except that we also follow the
      * constraint bounds of type variables in the constraint.
      */
    ...
    /** The base classes of this type as determined by ClassDenotation in
      * linearization order, with the class itself as first element. Inherited
      * by all type proxies. Overridden for And and Or types. `Nil` for all
      * other types.
      */
...
// ----- Member access -------------------------------------------------
    ...
    /** The scope of all declarations of this type. Defined by ClassInfo,
      * inherited by type proxies. Empty scope for all other types.
      */
    ...
    /** A denotation containing the declaration(s) in this type with the given
      * name. The result is either a SymDenotation or a MultiDenotation of
      * SymDenotations. The info(s) are the original symbol infos, no
    ...
    /** A denotation containing the non-private declaration(s) in this type with
      * the given name
      */
    ...
    /** A denotation containing the declaration(s) in this type with the given
      * name, as seen from prefix type `pre`. Declarations that have a flag in
      * `excluded` are omitted.
      */
    @tailrec final def findDecl(name: Name, excluded: FlagSet)(using
        Context
    ): Denotation = this match
      case tp: ClassInfo =>
        tp.decls
          .denotsNamed(name)
          .filterWithFlags(EmptyFlags, excluded)
          .toDenot(NoPrefix)
      case tp: TypeProxy =>
    ...
    final def memberBasedOnFlags(
        name: Name,
        required: FlagSet = EmptyFlags,
        excluded: FlagSet = EmptyFlags
    )(using Context): Denotation =
    ...
    /** Find member of this type with given `name`, all `required` flags and no
      * `excluded` flag and produce a denotation that contains the type of the
      * member as seen from given prefix `pre`.
      */
    final def findMember(
        name: Name,
        pre: Type,
        required: FlagSet = EmptyFlags,
        excluded: FlagSet = EmptyFlags
      ...
      @tailrec def go(tp: Type): Denotation = tp match
        case tp: TermRef =>
          go(tp.underlying match
            case mt: MethodType
                if mt.paramInfos.isEmpty && tp.symbol.is(StableRealizable) =>
              mt.resultType
            case tp1 => tp1
          )
        case tp: TypeRef =>
        ...
        case tp: AppliedType =>
          tp.tycon match
            case tc: TypeRef =>
              if tc.symbol.isClass then go(tc)
              else {
                val normed = tp.tryNormalize
                go(if normed.exists then normed else tp.superType)
              }
        ...
        case tp: ThisType => // ??? inline
          goThis(tp)
        case tp: RefinedType =>
          if name eq tp.refinedName then goRefined(tp) else go(tp.parent)
        case tp: RecType =>
          goRec(tp)
        case tp: TypeParamRef =>
          goParam(tp)
        ...
        case tp: MatchType =>
          val normed = tp.tryNormalize
          go(if normed.exists then normed else tp.underlying)
        case tp: TypeProxy =>
          go(tp.underlying)
        case tp: ClassInfo =>
      ...
      def goRec(tp: RecType) =
        ...
        if (tp.parent: Type | Null) == null then NoDenotation
        else if tp eq pre then go(tp.parent)
        else
      ...
      def goRefined(tp: RefinedType) =
        ...
        then // simplified case that runs more efficiently
          val jointInfo =
            if rinfo.isInstanceOf[TypeAlias] && !ctx.mode.is(
          ...
          pdenot.asSingleDenotation.derivedSingleDenotation(
            pdenot.symbol,
            jointInfo
          )
        else
      ...
      def goApplied(tp: AppliedType, tycon: HKTypeLambda) =
        go(tycon.resType).mapInfo(info =>
          tycon
            .derivedLambdaAbstraction(tycon.paramNames, tycon.paramInfos, info)
            .appliedTo(tp.args)
        )
          ...
          if underlying.isInstanceOf[AndType] then
            ...
            d match
              case d: SingleDenotation =>
                if qualifies(d) then d else NoDenotation
      ...
      def goSuper(tp: SuperType) = go(tp.underlying) match
        case d: JointRefDenotation =>
          typr.println(
            i"redirecting super.$name from $tp to ${d.symbol.showLocated}"
          )
          new UniqueRefDenotation(
            d.symbol,
            tp.memberInfo(d.symbol),
            d.validFor,
            pre
          )
        case d => d
      ...
      if recCount >= Config.LogPendingFindMemberThreshold then
      ...
      try go(this)
        ...
        case ex: Throwable =>
          core.println(
            s"findMember exception for $this member $name, pre = $pre, recCount = $recCount"
          )

          def showPrefixSafely(pre: Type)(using Context): String =
            pre.stripTypeVar match
              case pre: TermRef   => i"${pre.symbol.name}."
              case pre: TypeRef   => i"${pre.symbol.name}#"
              case pre: TypeProxy => showPrefixSafely(pre.superType)
              case _ =>
                if pre.typeSymbol.exists then i"${pre.typeSymbol.name}#"
                else "."
    ...
    /** The set of names of members of this type that pass the given name filter
      * when seen as members of `pre`. More precisely, these are all of members
      * `name` such that `keepOnly(pre, name)` is `true`.
      * @note:
      *   OK to use a Set[Name] here because Name hashcodes are replayable,
      *   hence the Set will always give the same names in the same order.
      */
    final def memberNames(keepOnly: NameFilter, pre: Type = this)(using
        Context
    ): Set[Name] = this match
      case tp: ClassInfo =>
      ...
      case tp: RefinedType =>
        ...
        if keepOnly(pre, tp.refinedName) then ns + tp.refinedName else ns
      case tp: TypeProxy =>
    ...
    def memberDenots(
        keepOnly: NameFilter,
        f: (Name, mutable.Buffer[SingleDenotation]) => Unit
    )(using Context): Seq[SingleDenotation] =
      val buf = mutable.ListBuffer[SingleDenotation]()
      for name <- memberNames(keepOnly) do f(name, buf)
      buf.toList
    ...
    /** Returns the set of methods that are abstract and do not overlap with any
      * of [[java.lang.Object]] methods.
      *
      * Conceptually, a SAM (functional interface) has exactly one abstract
      * method. If an interface declares an abstract method overriding one of
      * the public methods of [[java.lang.Object]], that also does not count
      ...
      *
      * @return
      *   the set of methods that are abstract and do not match any of
      *   [[java.lang.Object]]
      */
    ...
    final def implicitMembers(using Context): List[TermRef] =
      ...
      memberDenots(
        implicitFilter,
        (name, buf) =>
          buf ++= member(name).altsWith(_.isOneOf(GivenOrImplicitVal))
    ...
    /** The set of members of this type that have all of `required` flags but
      * none of `excluded` flags set.
      */
    final def membersBasedOnFlags(required: FlagSet, excluded: FlagSet)(using
        Context
    ): Seq[SingleDenotation] =
      record("membersBasedOnFlags")
      memberDenots(
        takeAllFilter,
        (name, buf) =>
          buf ++= memberBasedOnFlags(name, required, excluded).alternatives
      )
    ...
    final def allMembers(using Context): Seq[SingleDenotation] =
      ...
      memberDenots(
        takeAllFilter,
        (name, buf) => buf ++= member(name).alternatives
      )
...
// ----- Subtype-related --------------------------------------------
    ...
    /** Is this type a primitive value type which can be widened to the
      * primitive value type `that`?
      */
    ...
    /** Is this type a legal type for member `sym1` that overrides another
      ...
      * @param relaxedCheck
      *   if true type `Null` becomes a subtype of non-primitive value types in
      *   TypeComparer.
      * @param matchLoosely
      *   if true the types `=> T` and `()T` are seen as overriding each other.
      * @param checkClassInfo
    ...
    final def overrides(
        that: Type,
        relaxedCheck: Boolean,
        matchLoosely: => Boolean,
        checkClassInfo: Boolean = true,
        isSubType: (Type, Type) => Context ?=> Boolean = (tp1, tp2) =>
          tp1 frozen_<:< tp2
    )(using Context): Boolean =
      ...
      inContext(overrideCtx) {
        ...
        || matchLoosely && {
          ...
          ((this1 `ne` this) || (that1 `ne` that)) && this1.overrides(
            that1,
            relaxedCheck,
            false,
            checkClassInfo
    ...
    /** Is this type close enough to that type so that members with the two
      * types would override each other? This means:
      *   - Either both types are polytypes with the same number of type
      *     parameters and their result types match after renaming corresponding
      *     type parameters
      *   - Or both types are method types with =:=-equivalent(*) parameter
      *     types and matching result types after renaming corresponding
      *     parameter types if the method types are dependent.
      *   - Or both types are =:=-equivalent
      *   - Or phase.erasedTypes is false, and neither type takes term or type
      *     parameters.
      *
      * (*) when matching with a Java method, we also regard Any and Object as
      * equivalent parameter types.
      *
      * Under explicit nulls, this function will always use unsafe-nulls
      * semamtics to check the types. This is because we are using a relaxed
      * rule (ignoring `Null` types) to check overriding Java methods.
      */
    def matches(that: Type)(using Context): Boolean =
      record("matches")
      val overrideCtx =
        if ctx.explicitNulls then ctx.relaxedOverrideContext else ctx
      TypeComparer.matchesType(this, that, relaxed = !ctx.phase.erasedTypes)(
        using overrideCtx
      )

    /** This is the same as `matches` except that it also matches => T with T
      * and vice versa.
      */
    ...
    /** The basetype of this type with given class symbol, NoType if `base` is
      * not a class.
      */
    ...
    /** Safer version of `&`.
      *
      * This version does not simplify the bounds of the intersection of two
      * TypeBounds. The simplification done by `&` requires subtyping checks
      ...
      * (which will be masked unless `-Yno-deep-subtypes` is enabled). pos/i536
      * demonstrates that the infinite loop can also involve lower bounds.
      */
...
// ----- Unwrapping types -----------------------------------------------
    ...
    /** Map a TypeVar to either its instance if it is instantiated, or its
      * origin, if not, until the result is no longer a TypeVar. Identity on all
      * other types.
      */
    ...
    /** Remove all AnnotatedTypes wrapping this type.
      */
    def stripAnnots(keep: Annotation => Context ?=> Boolean)(using
        Context
    ): Type = this
    ...
    /** Widen from singleton type to its underlying non-singleton base type by
      * applying one or more `underlying` dereferences, Also go from => T to T.
      * Identity for all other types. Example:
      *
      * class Outer { class C ; val x: C } def o: Outer <o.x.type>.widen = o.C
      */
    final def widen(using Context): Type = this match
      case _: TypeRef | _: MethodOrPoly =>
        this // fast path for most frequent cases
    ...
    /** Widen from singleton type to its underlying non-singleton base type by
      * applying one or more `underlying` dereferences.
      */
    final def widenSingleton(using Context): Type = stripped match
      case tp: SingletonType if !tp.isOverloaded => tp.underlying.widenSingleton
      case _                                     => this

    /** Widen from TermRef to its underlying non-termref base type, while also
      * skipping Expr types.
      */
    final def widenTermRefExpr(using Context): Type = stripTypeVar match
      case tp: TermRef if !tp.isOverloaded =>
        tp.underlying.widenExpr.widenTermRefExpr
      case _ => this

    /** Widen from ExprType type to its result type. (Note: no stripTypeVar
      * needed because TypeVar's can't refer to ExprTypes.)
      */
    final def widenExpr: Type = this match
      case tp: ExprType => tp.resType
      case _            => this

    /** Widen type if it is unstable (i.e. an ExprType, or TermRef to unstable
      * symbol
      */
    final def widenIfUnstable(using Context): Type = stripTypeVar match
      case tp: ExprType => tp.resultType.widenIfUnstable
      case tp: TermRef if tp.symbol.exists && !tp.symbol.isStableMember =>
        tp.underlying.widenIfUnstable
      case _ => this
    ...
    /** Widen this type and if the result contains embedded soft union types,
      * replace them by their joins. "Embedded" means: inside type lambdas,
      * intersections or recursive types, in prefixes of refined types, or in
      * hard union types. If an embedded soft union is found, we first try to
      * simplify or eliminate it by re-lubbing it while allowing type parameters
      * to be constrained further. Any remaining union types are replaced by
      * their joins.
      *
      ...
      *
      * is approximated by constraining `A` to be =:= to `Int` and returning
      * `ArrayBuffer[Int]` instead of `ArrayBuffer[? >: Int | A <: Int & A]`
      *
      * Exception (if `-YexplicitNulls` is set): if this type is a nullable
      * union (i.e. of the form `T | Null`), then the top-level union isn't
      * widened. This is needed so that type inference can infer nullable types.
      */
    ...
    def widenUnionWithoutNull(using Context): Type = widen match
      case tp: AndType =>
        tp.derivedAndType(
          tp.tp1.widenUnionWithoutNull,
          tp.tp2.widenUnionWithoutNull
        )
      case tp: RefinedType =>
        tp.derivedRefinedType(
          tp.parent.widenUnion,
          tp.refinedName,
          tp.refinedInfo
        )
      case tp: RecType =>
    ...
    /** Widen all top-level singletons reachable by dealiasing and going to the
      * operands of & and |. Overridden and cached in OrType.
      */
    def widenSingletons(using Context): Type = dealias match
      ...
      case tp: OrType =>
        ...
        if tp1w eq tp then this else tp1w
      case tp: AndType =>
        val tp1w = tp.tp1.widenSingletons
        val tp2w = tp.tp2.widenSingletons
        if (tp.tp1 eq tp1w) && (tp.tp2 eq tp2w) then this else tp1w & tp2w
      case _ =>
        this
    ...
    def atoms(using Context): Atoms =
      ...
      dealias match
        case tp: SingletonType =>
          tp.underlying.atoms match
            case as @ Atoms.Range(lo, hi) =>
              if hi.size == 1 then
                as // if there's just one atom, there's no uncertainty which one it is
        ...
        case tp: TypeRef if tp.symbol.is(ModuleClass) =>
          ...
          single(
            tp.symbol.companionModule.termRef
              .asSeenFrom(tp.prefix, tp.symbol.owner)
          )
        case tp: TypeProxy =>
    ...
    private def dealias1(
        keep: AnnotatedType => Context ?=> Boolean,
        keepOpaques: Boolean
    )(using Context): Type = this match
      case tp: TypeRef =>
      ...
      case app @ AppliedType(tycon, _) =>
        ...
        if tycon1 ne tycon then app.superType.dealias1(keep, keepOpaques)
        else this
      case tp: TypeVar =>
        val tp1 = tp.instanceOpt
        if tp1.exists then tp1.dealias1(keep, keepOpaques) else tp
      case tp: AnnotatedType =>
    ...
    /** Follow aliases and dereferences LazyRefs, annotated types and
      * instantiated TypeVars until type is no longer alias type, annotated
      * type, LazyRef, or instantiated type variable.
      */
    final def dealias(using Context): Type =
      dealias1(keepNever, keepOpaques = false)

    /** Follow aliases and dereferences LazyRefs and instantiated TypeVars until
      * type is no longer alias type, LazyRef, or instantiated type variable.
      * Goes through annotated types and rewraps annotations on the result.
      */
    final def dealiasKeepAnnots(using Context): Type =
      dealias1(keepAlways, keepOpaques = false)

    /** Like `dealiasKeepAnnots`, but keeps only refining annotations */
    final def dealiasKeepRefiningAnnots(using Context): Type =
      dealias1(keepIfRefining, keepOpaques = false)

    /** Follow non-opaque aliases and dereferences LazyRefs, annotated types and
      * instantiated TypeVars until type is no longer alias type, annotated
      * type, LazyRef, or instantiated type variable.
      */
    final def dealiasKeepOpaques(using Context): Type =
      dealias1(keepNever, keepOpaques = true)
    ...
    final def deskolemized(using Context): Type =
      val deskolemizer = new ApproximatingTypeMap:
        def apply(
            tp: Type
        ) = /*trace(i"deskolemize($tp) at $variance", show = true)*/
          tp match
            case tp: SkolemType =>
    ...
    final def normalized(using Context): Type =
      ...
      if normed.exists then normed else this
    ...
    private def widenDealias1(keep: AnnotatedType => Context ?=> Boolean)(using
        Context
    ): Type =
      val res = this.widen.dealias1(keep, keepOpaques = false)
      if res eq this then res else res.widenDealias1(keep)

    /** Perform successive widenings and dealiasings until none can be applied
      * anymore
      */
    final def widenDealias(using Context): Type = widenDealias1(keepNever)

    /** Perform successive widenings and dealiasings while rewrapping
      * annotations, until none can be applied anymore
      */
    final def widenDealiasKeepAnnots(using Context): Type = widenDealias1(
      keepAlways
    )

    /** Perform successive widenings and dealiasings while rewrapping refining
      * annotations, until none can be applied anymore
      */
    final def widenDealiasKeepRefiningAnnots(using Context): Type =
      widenDealias1(keepIfRefining)
    ...
    final def dropDependentRefinement(using Context): Type = dealias match
      case RefinedType(parent, nme.apply, mt)
    ...
    /** If this is a (possibly aliased, annotated, and/or parameterized)
      * reference to a class, the class type ref, otherwise NoType.
      * @param refinementOK
    ...
    def underlyingClassRef(refinementOK: Boolean)(using Context): Type =
      dealias match
        case tp: TypeRef =>
          if tp.symbol.isClass then tp
          else if tp.symbol.isAliasType then
            tp.underlying.underlyingClassRef(refinementOK)
          else NoType
        case tp: AppliedType =>
          if tp.tycon.isLambdaSub then NoType
          else tp.superType.underlyingClassRef(refinementOK)
        case tp: AnnotatedType =>
          tp.parent.underlyingClassRef(refinementOK)
        case tp: RefinedType =>
          if refinementOK then tp.underlying.underlyingClassRef(refinementOK)
          else NoType
        case tp: RecType =>
          tp.underlying.underlyingClassRef(refinementOK)
        case _ =>
    ...
    /** The iterator of underlying types as long as type is a TypeProxy. Useful
      * for diagnostics
      */
    def underlyingIterator(using Context): Iterator[Type] = new Iterator[Type]:
      ...
      def next =
        ...
        if hasNext then current = current.asInstanceOf[TypeProxy].underlying
        res

    /** A prefix-less refined this or a termRef to a new skolem symbol that has
      * the given type as info.
      */
    def narrow(using Context): TermRef =
      TermRef(NoPrefix, newSkolem(this))

    /** Useful for diagnostics: The underlying type if this type is a type
      * proxy, otherwise NoType
      */
    ...
    def repeatedToSingle(using Context): Type = this match
      ...
      case _ => if isRepeatedParam then this.argTypesHi.head else this
    ...
    /** If this normalizes* to a refinement type that has a refinement for
      * `name` (which might be followed by other refinements), and the refined
      * info is a type alias, return the alias, otherwise return NoType. Used to
      * reduce types of the form
      *
      * P { ... type T = / += / -= U ... } # T
      *
      * to just U. Does not perform the reduction if the resulting type would
      * contain a reference to the "this" of the current refined type, except in
        ...
        case pre: RefinedType =>
          pre.refinedInfo match
            case tp: AliasingBounds =>
              if pre.refinedName ne name then loop(pre.parent) else tp.alias
            case _ =>
              loop(pre.parent)
        case pre: RecType =>
          val candidate = pre.parent.lookupRefined(name)
          if candidate.exists && !pre.isReferredToBy(candidate) then
    ...
    def select(name: TermName, sig: Signature, target: Name)(using
        Context
    ): TermRef =
      TermRef(
        this,
        name,
        member(name).atSignature(sig, target, relaxed = !ctx.erasedTypes)
      )

// ----- Access to parts --------------------------------------------

    /** The normalized prefix of this type is: For an alias type, the normalized
      * prefix of its alias For all other named type and class infos: the
      * prefix. Inherited by all other type proxies. `NoType` for all other
      * types.
      */
    @tailrec final def normalizedPrefix(using Context): Type = this match
      case tp: NamedType =>
        if tp.symbol.info.isTypeAlias then tp.info.normalizedPrefix
        else tp.prefix
      case tp: ClassInfo =>
        tp.prefix
      case tp: TypeProxy =>
    ...
    def parents(using Context): List[Type] = this match
      ...
      case tp: TypeRef =>
        if tp.info.isInstanceOf[TempClassInfo] then
          tp.recomputeDenot()
    ...
    /** The parameter types in the first parameter section of a generic type or
      * MethodType, Empty list for others
      */
    final def firstParamTypes(using Context): List[Type] = stripPoly match
      case mt: MethodType => mt.paramInfos
      case _              => Nil

    /** The parameter names in the first parameter section of a generic type or
      * MethodType, Empty list for others
      */
    ...
    /** The final result type of a PolyType, MethodType, or ExprType, after
      * skipping all parameter sections, the type itself for all others.
      */
    def finalResultType(using Context): Type = resultType.stripPoly match
      case mt: MethodType => mt.resultType.finalResultType
      case _              => resultType

    /** Determine the expected function type from the prototype. If no function
      * type is found, NoType is returned. If multiple function types are found
      * in an intersection, their intersection is returned. This works since `&`
      * invokes `TypeComparer.distributeAnd`, which ensures that `(A1 => B1) &
      * (A2 => B2)` simplifies to `(A1 | A2) => (B1 & B2)`, so the result is
      * again a function type. An analogous distribution mechanism does not
      * exist for `|`. Therefore, a union of function types also yields
      * `NoType`, since we cannot determine a single expected function type.
      */
    ...
    /** The type parameter with given `name`. This tries first `decls` in order
      * not to provoke a cycle by forcing the info. If that yields no symbol it
      * tries `member` as an alternative.
      */
    ...
    /** If this is an ignored proto type, its underlying type, otherwise the
      * type itself
      */
    ...
    /** If this is a OrType, the number of factors if that match `soft`, 1 for
      * all other types.
      */
...
// ----- Substitutions -----------------------------------------------------
    ...
    /** Substitute all types that refer in their symbol attribute to one of the
      * symbols in `from` by the corresponding types in `to`.
      */
    final def subst(from: List[Symbol], to: List[Type])(using Context): Type =
      if from.isEmpty then this
      else {
        val from1 = from.tail
        if from1.isEmpty then
          Substituters.subst1(this, from.head, to.head, null)
        else {
          val from2 = from1.tail
          if from2.isEmpty then
            Substituters.subst2(
              this,
              from.head,
              to.head,
    ...
    /** As substThis, but only is class is a static owner (i.e. a globally
      * accessible object)
      */
    final def substThisUnlessStatic(cls: ClassSymbol, tp: Type)(using
        Context
    ): Type =
      if cls.isStaticOwner then this
      else Substituters.substThis(this, cls, tp, null)
    ...
    final def substParams(from: BindingType, to: List[Type])(using
        Context
    ): Type =
      Substituters.substParams(this, from, to, null)

    /** Substitute all occurrences of symbols in `from` by references to
      * corresponding symbols in `to`
      */
    final def substSym(from: List[Symbol], to: List[Symbol])(using
        Context
    ): Type =
      Substituters.substSym(this, from, to, null)

    /** Substitute all occurrences of symbols in `from` by corresponding types
      * in `to`. Unlike for `subst`, the `to` types can be type bounds. A
      * TypeBounds target will be replaced by range that gets absorbed in an
      * approximating type map.
      */
    final def substApprox(from: List[Symbol], to: List[Type])(using
        Context
    ): Type =
...
// ----- misc -----------------------------------------------------------
    ...
    /** Turn type into a function type.
      * @pre
      *   this is a method type without parameter dependencies.
      * @param dropLast
      *   the number of trailing parameters that should be dropped when forming
      *   the function type.
      * @param alwaysDependent
      *   if true, always create a dependent function type.
      */
    def toFunctionType(
        isJava: Boolean,
        dropLast: Int = 0,
        alwaysDependent: Boolean = false
    )(using Context): Type = this match
      case mt: MethodType if !mt.isParamDependent =>
        val formals1 =
          if dropLast == 0 then mt.paramInfos
          else mt.paramInfos dropRight dropLast
    ...
    /** The signature of this type. This is by default NotAMethod, but is
      * overridden for PolyTypes, MethodTypes, and TermRef types. (the reason
      * why we deviate from the "final-method-with-pattern-match-in-base-class"
      * pattern is that method signatures use caching, so encapsulation is
      * improved using an OO scheme).
      */
    ...
    def dropAnnot(cls: Symbol)(using Context): Type = stripTypeVar match
      case self @ AnnotatedType(pre, annot) =>
        if annot.symbol eq cls then pre
    ...
    /** A translation from types of original parameter ValDefs to the types of
      * parameters in MethodTypes. Translates `Seq[T] @repeated` or `Array[T]
      * \@repeated` to `<repeated>[T]`. That way, repeated arguments are made
      * manifest without risk of dropped annotations.
      */
    def annotatedToRepeated(using Context): Type = this match
      ...
      case self @ AnnotatedType(tp, annot)
          if annot matches defn.RetainsByNameAnnot =>
    ...
    /** A type capturing the capture set `cs`. If this type is already a
      * capturing type the two capture sets are combined.
      */
    ...
    /** The set of distinct symbols referred to by this type, after all aliases
      * are expanded
      */
    def coveringSet(using Context): Set[Symbol] =
      (new CoveringSetAccumulator).apply(Set.empty[Symbol], this)

    /** The number of applications and refinements in this type, after all
    ...
    def showWithUnderlying(n: Int = 1)(using Context): String = this match
      case tp: TypeProxy if n > 0 =>
        s"$show with underlying ${tp.underlying.showWithUnderlying(n - 1)}"
      case _ => show

    /** A simplified version of this type which is equivalent wrt =:= to this
      * type. This applies a typemap to the type which (as all typemaps) follows
      * type variable instances and reduces typerefs over refined types. It also
      *
      *   - re-evaluates all occurrences of And/OrType with &/| because what was
      *     a union or intersection of type variables might be a simpler type
      *     after the type variables are instantiated.
      *   - maps poly params in the current constraint set back to their type
      *     vars.
      ...
      * example, the Java erasure of `Object & Serializable` is `Object`, but
      * its simplification is `Serializable`). This means that simplification
      * should never be used in a `MethodicType`, because that could lead to a
      * different `signature`. Since this isn't very useful anyway, this method
      * handles this by never simplifying inside a `MethodicType`, except for
      * replacing type parameters with associated type variables.
      */
    def simplified(using Context): Type = TypeOps.simplify(this, null)

    /** Compare `this == that`, assuming corresponding binders in `bs` are
      * equal. The normal `equals` should be equivalent to `equals(that,
      * null`)`. We usually override `equals` when we override `iso` except if
      * the `equals` comes from a case class, so it already has the right
      * definition anyway.
      */
    ...
    /** Equality used for hash-consing; uses `eq` on all recursive invocations,
      * except where a BindingType is involved. The latter demand a deep
      * isomorphism check.
      */
    def eql(that: Type): Boolean = this.equals(that)

    /** customized hash code of this type. NotCached for uncached types. Cached
      * types compute hash and use it as the type's hashCode.
      */
    def hash: Int
    ...
    /** Is the `hash` of this type the same for all possible sequences of
      * enclosing binders?
      */
...
// ----- Type categories ----------------------------------------------
  ...
  /** A marker trait for type proxies. Each implementation is expected to
    * redefine the `underlying` method.
    */
  abstract class TypeProxy extends Type:
    ...
    /** The closest supertype of this type. This is the same as `underlying`,
      ...
      *   - for applied types it returns the upper bound of the constructor
      *     re-applied to the arguments.
      */
    ...
    /** Same as superType, except for two differences:
      ...
      *
      * Note: the reason to reduce match type aliases here and not in
      * `superType` is that `superType` is context-independent and cached,
      * whereas matchtype reduction depends on context and should not be cached
      * (at least not without the very specific cache invalidation condition for
      * matchtypes).
      */
  ...
  abstract class CachedGroundType extends Type with CachedType:
    ...
    final def hash: Int =
      if myHash == HashUnknown then
        myHash = computeHash(null)
        assert(myHash != HashUnknown)
      myHash
    override final def hashCode: Int =
      if hash == NotCached then System.identityHashCode(this) else hash
  ...
  /** A marker trait for types that apply only to term symbols or that represent
    * higher-kinded types.
    */
  trait TermType extends Type

  /** A marker trait for types that can be types of values or prototypes of
    * value types
    */
  trait ValueTypeOrProto extends TermType

  /** A marker trait for types that can be types of values or that are
    * higher-kinded
    */
  trait ValueType extends ValueTypeOrProto

  /** A marker trait for types that are guaranteed to contain only a single
    * non-null value (they might contain null in addition).
    */
  trait SingletonType extends TypeProxy with ValueType:
    def isOverloaded(using Context): Boolean = false

  /** A trait for references in CaptureSets. These can be NamedTypes, ThisTypes
    * or ParamRefs
    */
  trait CaptureRef extends SingletonType:
    ...
    /** Can the reference be tracked? This is true for all ThisTypes or
      * ParamRefs but only for some NamedTypes.
      */
    def canBeTracked(using Context): Boolean

    /** Is the reference tracked? This is true if it can be tracked and the
      * capture set of the underlying type is not always empty.
      */
    final def isTracked(using Context): Boolean =
      canBeTracked && !captureSetOfInfo.isAlwaysEmpty
    ...
    def captureSetOfInfo(using Context): CaptureSet =
      if ctx.runId == myCaptureSetRunId then myCaptureSet.nn
        ...
        if ctx.phase != Phases.checkCapturesPhase || underlying.isProvisional
        then myCaptureSet = null
        else
  ...
  /** A trait for types that bind other types that refer to them. Instances are:
    * LambdaType, RecType.
    */
  trait BindingType extends Type:
    ...
    override def identityHash(bs: Binders): Int =
      def recur(n: Int, tp: BindingType, rest: Binders): Int =
        if this `eq` tp then finishHash(hashing.mix(hashSeed, n), 1)
        else if rest == null then System.identityHashCode(this)
        else recur(n + 1, rest.tp, rest.next)
      avoidSpecialHashes(
        if bs == null then System.identityHashCode(this)
  ...
  trait ProtoType extends Type:
    def isMatchedBy(tp: Type, keepConstraint: Boolean = false)(using
        Context
    ): Boolean
    def fold[T](x: T, ta: TypeAccumulator[T])(using Context): T
    def map(tm: TypeMap)(using Context): ProtoType

    /** If this prototype captures a context, the same prototype except that the
      * result captures the given context `ctx`.
      */
  ...
  trait NarrowCached extends Type:
    ...
    override def narrow(using Context): TermRef =
      if myNarrow == null then myNarrow = super.narrow
      myNarrow.nn

// --- NamedTypes ------------------------------------------------------------------

  abstract class NamedType extends CachedProxyType, ValueType, Product:
    self =>
    ...
    /** If designator is a name, this name. Otherwise, the original name of the
      * designator symbol.
      */
    final def name(using Context): ThisName =
      if myName == null then myName = computeName
    ...
    final override def signature(using Context): Signature =
      /** The signature computed from the last known denotation with
        * `sigFromDenot`, or if there is none, the signature of the symbol.
        * Signatures are always computed before erasure, since some symbols
        * change their signature at erasure.
        */
    ...
    /** The signature computed from the current denotation with `sigFromDenot`
      * if it is known without forcing. Otherwise the signature of the current
      * symbol if it is known without forcing. Otherwise NotAMethod. Signatures
      * are always computed before erasure, since some symbols change their
      * signature at erasure.
      */
    ...
    final def symbol(using Context): Symbol =
      ...
      if checkedPeriod.code == ctx.period.code then
    ...
    private def computeSymbol(using Context): Symbol =
      val result = designator match
        ...
        case name =>
          (if denotationIsCurrent then lastDenotation.asInstanceOf[Denotation]
           else denot).symbol
      if checkedPeriod.code != NowhereCode then checkedPeriod = ctx.period
      result

    /** There is a denotation computed which is valid (somewhere in) the current
      * run.
      */
    def denotationIsCurrent(using Context): Boolean =
      lastDenotation != null && lastDenotation.uncheckedNN.validFor.runId == ctx.runId

    /** If the reference is symbolic or the denotation is current, its symbol,
      * otherwise NoDenotation.
      *
      * Note: This operation does not force the denotation, and is therefore
      * timing dependent. It should only be used if the outcome of the essential
      * computation does not depend on the symbol being present or not. It's
      * currently used to take an optimized path in substituters and type
      * accumulators, as well as to be safe in diagnostic printing. Normally,
      * it's better to use `symbol`, not `currentSymbol`.
      */
    final def currentSymbol(using Context): Symbol = designator match
      case sym: Symbol => sym
      case _ =>
        if denotationIsCurrent then lastDenotation.nn.symbol else NoSymbol

    /** Retrieves currently valid symbol without necessarily updating
      * denotation. Assumes that symbols do not change between periods in the
      * same run. Used to get the class underlying a ThisType.
      */
    private[Types] def stableInRunSymbol(using Context): Symbol =
      if checkedPeriod.runId == ctx.runId then lastSymbol.nn
      else symbol

    def info(using Context): Type = denot.info

    /** The denotation currently denoted by this type. Extremely hot. Carefully
    ...
    final def denot(using Context): Denotation =
      ...
      if checkedPeriod.code != NowhereCode && lastd.validFor.contains(
          ctx.period
        )
      then lastd
      else computeDenot
    ...
    private def computeDenot(using Context): Denotation =
      ...
      def fromDesignator = designator match
        case name: Name =>
          val sym = lastSymbol
          val allowPrivate =
            sym == null || (sym == NoSymbol) || sym.lastKnownDenotation.flagsUNSAFE
              .is(Private)
          finish(memberDenot(name, allowPrivate))
        case sym: Symbol =>
          val symd = sym.lastKnownDenotation
          if symd.validFor.runId != ctx.runId && !stillValid(symd) then
            finish(memberDenot(symd.initial.name, allowPrivate = false))
          else if prefix.isArgPrefixOf(symd) then finish(argDenot(sym.asType))
          else if infoDependsOnPrefix(symd, prefix) then
            finish(
              memberDenot(symd.initial.name, allowPrivate = symd.is(Private))
            )
          else finish(symd.current)

      lastDenotation match
        case lastd0: SingleDenotation =>
          val lastd = lastd0.skipRemoved
          if lastd.validFor.runId == ctx.runId && checkedPeriod.code != NowhereCode
          then finish(lastd.current)
          else
            lastd match
              case lastd: SymDenotation =>
                if stillValid(lastd) && checkedPeriod.code != NowhereCode then
                  finish(lastd.current)
                else
                  finish(memberDenot(lastd.initial.name, allowPrivate = false))
              case _ =>
                fromDesignator
    ...
    private def memberDenot(name: Name, allowPrivate: Boolean)(using
        Context
    ): Denotation =
      var d = memberDenot(prefix, name, allowPrivate)
      if !d.exists && !allowPrivate && ctx.mode.is(Mode.Interactive) then
        // In the IDE we might change a public symbol to private, and would still expect to find it.
        d = memberDenot(prefix, name, true)
      if !d.exists && ctx.isAfterTyper && lastDenotation
          .isInstanceOf[SymDenotation]
      then
        // name has changed; try load in earlier phase and make current
        d = atPhase(ctx.phaseId - 1)(memberDenot(name, allowPrivate)).current
      if d.isOverloaded then d = disambiguate(d)
      d

    private def memberDenot(prefix: Type, name: Name, allowPrivate: Boolean)(
        using Context
    ): Denotation =
      if allowPrivate then prefix.member(name)
    ...
    private def argDenot(param: TypeSymbol)(using Context): Denotation =
      ...
      if 0 <= idx && idx < args.length then
        val argInfo = args(idx) match
          case arg: TypeBounds =>
            val v = param.paramVarianceSign
            val pbounds = param.paramInfo
            if v > 0 && pbounds.loBound.dealiasKeepAnnots.isExactlyNothing then
              TypeAlias(arg.hiBound & rebase(pbounds.hiBound))
            else if v < 0 && pbounds.hiBound.dealiasKeepAnnots.isExactlyAny then
              TypeAlias(arg.loBound | rebase(pbounds.loBound))
            else arg recoverable_& rebase(pbounds)
          case arg => TypeAlias(arg)
        param.derivedSingleDenotation(param, argInfo)
      else {
        if !ctx.reporter.errorsReported then
          throw TypeError(em"""bad parameter reference $this at ${ctx.phase}
    ...
    /** Reload denotation by computing the member with the reference's name as
      * seen from the reference's prefix.
      */
    def recomputeDenot()(using Context): Unit =
      setDenot(
        memberDenot(name, allowPrivate = !symbol.exists || symbol.is(Private))
      )

    private def setDenot(denot: Denotation)(using Context): Unit =
      if ctx.base.checkNoDoubleBindings then checkSymAssign(denot.symbol)

      lastDenotation = denot
      lastSymbol = denot.symbol
      checkedPeriod = if prefix.isProvisional then Nowhere else ctx.period
      designator match
        case sym: Symbol if designator ne lastSymbol.nn =>
          designator = lastSymbol
            .asInstanceOf[Designator { type ThisName = self.ThisName }]
        case _ =>
      checkDenot()
    ...
    private def checkSymAssign(sym: Symbol)(using Context) =
      def selfTypeOf(sym: Symbol) =
        if sym.isClass then sym.asClass.givenSelfType else NoType
      ...
      assert(
        (lastSym == null)
          ...
          else s"""data race? overwriting $lastSym with $sym in type $this,
    ...
    /** A reference with the initial symbol in `symd` has an info that might
      * depend on the given prefix. Note: If M is an abstract type or non-final
      * term member in trait or class C, its info depends even on C.this if
      * class C has a self type that refines the info of M.
      */
    private def infoDependsOnPrefix(symd: SymDenotation, prefix: Type)(using
        Context
    ): Boolean =
    ...
    /** Is this a reference to a class or object member with an info that might
    ...
    /** Reduce a type-ref `T { X = U; ... } # X` to `U` provided `U` does not
      * refer with a RecThis to the refinement type `T { X = U; ... }`
      */
    ...
    /** Guard against cycles that can arise if given `op` follows info. The
      * problematic cases are a type alias to itself or bounded by itself or a
      * val typed as itself:
      *
      * type T <: T val x: x.type
      *
      * These are errors but we have to make sure that operations do not loop
    ...
    final def controlled[T](op: => T)(using Context): T = try
      ...
      then op
        ...
        try
          ...
          op
        finally ctx.pendingUnderlying -= this
    finally ctx.base.underlyingRecursions -= 1
    ...
    /** The argument corresponding to class type parameter `tparam` as seen from
      ...
      * or prefix is an & or | type and parameter is non-variant. Otherwise, a
      * typebounds argument is dropped and the original type parameter reference
      * is returned.
      */
    def argForParam(pre: Type, widenAbstract: Boolean = false)(using
        Context
    ): Type =
      val tparam = symbol
      ...
      base.stripped match
        case AppliedType(tycon, allArgs) =>
          ...
          while tparams.nonEmpty && args.nonEmpty do
            if tparams.head.eq(tparam) then
        ...
        case _ =>
          if pre.termSymbol.is(Package) then
            argForParam(pre.select(nme.PACKAGE))
    ...
    /** A selection of the same kind, but with potentially a different prefix.
      ...
      * --> S#A & T#A otherwise (S | T)#A --> S#A | T#A
      */
    def derivedSelect(prefix: Type)(using Context): Type =
      if prefix eq this.prefix then this
        ...
        if isType then
          val res =
            if currentSymbol.isAllOf(ClassTypeParam) then argForParam(prefix)
            else prefix.lookupRefined(name)
          if res.exists then return res
          if Config.splitProjections then
            prefix match
              case prefix: AndType =>
                ...
                return (if isMissing(derived1) then derived2
                        else if isMissing(derived2) then derived1
                        else prefix.derivedAndType(derived1, derived2)) case prefix: OrType =>
                val derived1 = derivedSelect(prefix.tp1)
                val derived2 = derivedSelect(prefix.tp2)
    ...
    /** A reference like this one, but with the given denotation, if it exists.
      * Returns a new named type with the denotation's symbol as designator if
      * that symbol exists and it is different from the current designator.
      * Returns a new named type with the denotations's name as designator if
      * the denotation is overloaded and its name is different from the current
      * designator.
      */
      ...
      if denot.exists then
        ...
        denot match
          case denot: SymDenotation
            ...
            core.println(
              i"overwrite ${adapted.toString} / ${adapted.lastDenotation}, ${adapted.lastDenotation.getClass} with $denot at ${ctx.phaseId}"
            )
            adapted.setDenot(
              UniqueRefDenotation(
                denot.symbol,
    ...
    final def withPrefix(prefix: Type)(using Context): Type =
      def reload(): NamedType =
        ...
        var d = memberDenot(prefix, name, allowPrivate)
        if d.isOverloaded && lastSym.exists then
          d = disambiguate(
            d,
            if lastSym.signature == Signature.NotAMethod then
      ...
      else if lastDenotation == null then NamedType(prefix, designator)
      else
        designator match
          case sym: Symbol =>
            ...
            then
              ...
              val falseOverride =
                sym.isClass && candidate.symbol.exists && candidate.symbol != symbol
              ...
              if falseOverride then
                NamedType(prefix, sym.name, denot.asSeenFrom(prefix))
              else candidate
    ...
    override def hashIsStable: Boolean =
      if myStableHash == 0 then
        myStableHash = if prefix.hashIsStable then 1 else -1
      myStableHash > 0

    override def eql(that: Type): Boolean =
      this eq that // safe because named types are hash-consed separately
  end NamedType
  ...
  abstract case class TermRef(
    ...
    override def underlying(using Context): Type =
      val d = denot
      if d.isOverloaded then NoType else d.info
    ...
    /** A term reference can be tracked if it is a local term ref to a value or
      * a method term parameter. References to term parameters of classes cannot
      * be tracked individually. They are subsumed in the capture sets of the
      * enclosing class. TODO: ^^^ What about call-by-name?
      */
    def canBeTracked(using Context) =
      ((prefix eq NoPrefix)
        || symbol.is(ParamAccessor) && (prefix eq symbol.owner.thisType)
        || isRootCapability) && !symbol.isOneOf(UnstableValueFlags)
  ...
  abstract case class TypeRef(
    ...
    /** Given an alias type `type A = B` where a recursive comparison with `B`
      * yields `false`, can we conclude that the comparison is definitely false?
      * This could not be the case if `A` overrides some abstract type. Example:
      *
      * class C { type A } class D { type A = Int } val c: C val d: D & c.type
      * c.A <:< d.A ?
      *
      * The test should return true, by performing the logic in the bottom half
      * of firstTry (where we check the names of types). But just following the
      * alias from d.A to Int reduces the problem to `c.A <:< Int`, which
      * returns `false`. So we can't drop the alias here, we need to do the
      * backtracking to the name- based tests.
      */
    def canDropAlias(using Context) =
      if myCanDropAliasPeriod != ctx.period then
        myCanDropAlias = !symbol.canMatchInheritedSymbols
    ...
    override def translucentSuperType(using Context) = info match
      ...
      case TypeBounds(_, hi) =>
        if symbol.isOpaqueAlias then
          symbol.opaqueAlias
            .asSeenFrom(prefix, symbol.owner)
            .orElse(hi) // orElse can happen for malformed input
        else hi
      case _ => underlying
  ...
  final class CachedTermRef(prefix: Type, designator: Designator, hc: Int)
      extends TermRef(prefix, designator):
    assert((prefix ne NoPrefix) || designator.isInstanceOf[Symbol])
    myHash = hc

  final class CachedTypeRef(prefix: Type, designator: Designator, hc: Int)
      extends TypeRef(prefix, designator):
    assert((prefix ne NoPrefix) || designator.isInstanceOf[Symbol])
    myHash = hc

  /** Assert current phase does not have erasure semantics */
  private def assertUnerased()(using Context) =
    if Config.checkUnerased then assert(!ctx.phase.erasedTypes)

  /** The designator to be used for a named type creation with given prefix,
    * name, and denotation. This is the denotation's symbol, if it exists and
    * the prefix is not the this type of the class owning the symbol. The reason
    * for the latter qualification is that when re-computing the denotation of a
    * `this.<symbol>` reference we read the type directly off the symbol. But
    * the given denotation might contain a more precise type than what can be
    * computed from the symbol's info. We have to create in this case a
    * reference with a name as designator so that the denotation will be
    * correctly updated in the future. See also NamedType#withDenot. Test case
    * is neg/opaque-self-encoding.scala.
    */
  private def designatorFor(prefix: Type, name: Name, denot: Denotation)(using
      Context
  ): Designator =
    ...
    if sym.exists && (prefix.eq(NoPrefix) || !ownerIsPrefix(sym.owner)) then sym
    else name
  ...
  object NamedType:
    ...
    def apply(prefix: Type, designator: Designator)(using Context): NamedType =
      if isType(designator) then TypeRef.apply(prefix, designator)
      else TermRef.apply(prefix, designator)
    def apply(prefix: Type, designator: Name, denot: Denotation)(using
        Context
    ): NamedType =
      if designator.isTermName then
        TermRef.apply(prefix, designator.asTermName, denot)
      else TypeRef.apply(prefix, designator.asTypeName, denot)
    def unapply(tp: NamedType): NamedType = tp

    def validPrefix(prefix: Type): Boolean =
  ...
  object TermRef:
    ...
    def apply(prefix: Type, desig: Designator)(using Context): TermRef =
      ctx.uniqueNamedTypes
        .enterIfNew(prefix, desig, isTerm = true)
        .asInstanceOf[TermRef]

    /** Create a term ref with given initial denotation. The name of the
      * reference is taken from the denotation's symbol if the latter exists, or
      * else it is the given name.
      */
    def apply(prefix: Type, name: TermName, denot: Denotation)(using
        Context
    ): TermRef =
  ...
  object TypeRef:
    ...
    def apply(prefix: Type, desig: Designator)(using Context): TypeRef =
      ctx.uniqueNamedTypes
        .enterIfNew(prefix, desig, isTerm = false)
        .asInstanceOf[TypeRef]

    /** Create a type ref with given initial denotation. The name of the
      * reference is taken from the denotation's symbol if the latter exists, or
      * else it is the given name.
      */
    def apply(prefix: Type, name: TypeName, denot: Denotation)(using
        Context
    ): TypeRef =
      apply(prefix, designatorFor(prefix, name, denot)).withDenot(denot)
  ...
  /** The type cls.this
    * @param tref
    *   A type ref which indicates the class `cls`. Note: we do not pass a class
    *   symbol directly, because symbols do not survive runs whereas typerefs
    *   do.
    */
  abstract case class ThisType(tref: TypeRef)
    ...
    def cls(using Context): ClassSymbol = tref.stableInRunSymbol match
      ...
      case _ if ctx.mode.is(Mode.Interactive) =>
        defn.AnyClass // was observed to happen in IDE mode

    override def underlying(using Context): Type =
      if ctx.erasedTypes then tref
      else
        cls.info match
          case cinfo: ClassInfo => cinfo.selfType
          case _: ErrorType | NoType if ctx.mode.is(Mode.Interactive) =>
            cls.info
    ...
    /** Check that the rhs is a ThisType that refers to the same class.
      */
    def sameThis(that: Type)(using Context): Boolean =
  ...
  /** The type of a super reference cls.super where `thistpe` is cls.this and
    * `supertpe` is the type of the value referenced by `super`.
    */
  abstract case class SuperType(thistpe: Type, supertpe: Type)
    ...
    def derivedSuperType(thistpe: Type, supertpe: Type)(using Context): Type =
      if (thistpe eq this.thistpe) && (supertpe eq this.supertpe) then this
    ...
    override def eql(that: Type): Boolean = that match
      case that: SuperType =>
        thistpe.eq(that.thistpe) && supertpe.eq(that.supertpe)
      case _ => false

  final class CachedSuperType(thistpe: Type, supertpe: Type)
      extends SuperType(thistpe, supertpe)
  ...
  abstract case class ConstantType(value: Constant)
      extends CachedProxyType
      with SingletonType:
  ...
  case class LazyRef(private var refFn: (Context => (Type | Null)) | Null)
      extends UncachedProxyType
      with ValueType:
    ...
    override def toString: String =
      s"LazyRef(${if computed then myRef else "..."})"
  ...
  abstract case class RefinedType(
    ...
    if refinedName.isTermName then assert(refinedInfo.isInstanceOf[TermType])
    ...
    def derivedRefinedType(parent: Type, refinedName: Name, refinedInfo: Type)(
        using Context
    ): Type =
    ...
    /** Add this refinement to `parent`, provided `refinedName` is a member of
      * `parent`.
      */
    def wrapIfMember(parent: Type)(using Context): Type =
      if parent.member(refinedName).exists then
        derivedRefinedType(parent, refinedName, refinedInfo)
      else parent

    override def computeHash(bs: Binders): Int =
      doHash(bs, refinedName, refinedInfo, parent)
    override def hashIsStable: Boolean =
      refinedInfo.hashIsStable && parent.hashIsStable
  ...
  object RefinedType:
    @tailrec def make(parent: Type, names: List[Name], infos: List[Type])(using
        Context
    ): Type =
      if names.isEmpty then parent
      else
        make(
          RefinedType(parent, names.head, infos.head),
          names.tail,
          infos.tail
        )

    def apply(parent: Type, name: Name, info: Type)(using
        Context
    ): RefinedType =
      assert(!ctx.erasedTypes)
      unique(new CachedRefinedType(parent, name, info)).checkInst

  /** A recursive type. Instances should be constructed via the companion
    * object.
    *
    * @param parentExp
    *   A function that, given a recursive type R, produces a type that can
    *   refer to R via a `RecThis(R)` node. This is used to "tie the knot".
    *
    * For example, in class C { type T1; type T2 } type C2 = C { type T1; type
    * T2 = T1 }
    *
    * The type of `C2` is a recursive type `{(x) => C{T1; T2 = x.T1}}`, written
    * as
    *
    * RecType( RefinedType( RefinedType( TypeRef(...,class C), T1,
    * TypeBounds(...)), T2, TypeBounds( TypeRef(RecThis(...),T1),
    * TypeRef(RecThis(...),T1))))
    *
  ...
  class RecType(parentExp: RecType => Type)
    ...
    def recThis: RecThis =
      if myRecThis == null then myRecThis = new RecThisImpl(this)
      myRecThis.nn
    ...
    override def computeHash(bs: Binders): Int =
      doHash(new SomeBinders(this, bs), parent)
  ...
  object RecType:
    ...
    /** Create a RecType, normalizing its contents. This means:
      ...
      *      2. Any refinement of the form `type T = z.T` on the spine of the
      *      type where `z` refers to the created rec-type is replaced by `type
      *      T`. This avoids infinite recursions later when we try to follow
      *      these references. TODO: Figure out how to guarantee absence of
      *      cycles of length > 1
      */
      ...
      def normalize(tp: Type): Type = tp.stripTypeVar match
        ...
        case tp @ RefinedType(parent, rname, rinfo) =>
          val rinfo1 = rinfo match
            case TypeAlias(ref @ TypeRef(RecThis(`rt`), _))
                if ref.name == rname =>
              TypeBounds.empty
            case _ => rinfo
    ...
    /** Create a `RecType`, but only if the type generated by `parentExp` is
      * indeed recursive.
      */
    def closeOver(parentExp: RecType => Type)(using Context): Type =
      val rt = this(parentExp)
      if rt.isReferredToBy(rt.parent) then rt else rt.parent
  end RecType
  ...
  abstract case class AndType(tp1: Type, tp2: Type) extends AndOrType:
    ...
    override final def baseClasses(using Context): List[ClassSymbol] =
      if myBaseClassesPeriod != ctx.period then
          ...
          case bc2 :: bcs2rest =>
            if bcs1set contains bc2 then
              if bc2.is(Trait) then recur(bcs2rest)
                ...
                bcs1 // common class, therefore rest is the same in both sequences
  ...
  object AndType:
    ...
    /** Make an AndType using `op` unless clearly unnecessary (i.e. without
      * going through `&`).
      */
    def make(tp1: Type, tp2: Type, checkValid: Boolean = false)(using
        Context
    ): Type =
      if (tp1 eq tp2) || (tp2 eq defn.AnyType) then tp1
      else if tp1 eq defn.AnyType then tp2
      else if checkValid then apply(tp1, tp2)
      else unchecked(tp1, tp2)

    /** Like `make`, but also supports higher-kinded types as argument */
    def makeHk(tp1: Type, tp2: Type)(using Context): Type =
      TypeComparer.liftIfHK(
        tp1,
        tp2,
        AndType.make(_, _, checkValid = false),
        makeHk,
        _ | _
      )
  end AndType
  ...
  abstract case class OrType protected (tp1: Type, tp2: Type) extends AndOrType:
    ...
    override final def baseClasses(using Context): List[ClassSymbol] =
      if myBaseClassesPeriod != ctx.period then
        ...
        def recur(bcs2: List[ClassSymbol]): List[ClassSymbol] = bcs2 match
          case bc2 :: bcs2rest =>
            if bcs1set contains bc2 then
              if bc2.is(Trait) then bc2 :: recur(bcs2rest)
              else bcs2
            else recur(bcs2rest)
          case nil =>
            bcs2
    ...
    def join(using Context): Type =
      if myJoinPeriod != ctx.period then
    ...
    override def widenUnionWithoutNull(using Context): Type =
      if myUnionPeriod != ctx.period then
        myUnion =
          if isSoft then
            TypeComparer.lub(
              tp1.widenUnionWithoutNull,
              tp2.widenUnionWithoutNull,
              canConstrain = true,
              isSoft = isSoft
            ) match
    ...
    private def computeWidenSingletons()(using Context): Type =
      ...
      if (tp1 eq tp1w) && (tp2 eq tp2w) then this
      else TypeComparer.lub(tp1w, tp2w, isSoft = isSoft)
    ...
    def derivedOrType(tp1: Type, tp2: Type, soft: Boolean = isSoft)(using
        Context
    ): Type =
      if (tp1 eq this.tp1) && (tp2 eq this.tp2) && soft == isSoft then this
    ...
    override def eql(that: Type): Boolean = that match
      case that: OrType =>
        tp1.eq(that.tp1) && tp2.eq(that.tp2) && isSoft == that.isSoft
      case _ => false
  end OrType

  final class CachedOrType(tp1: Type, tp2: Type, override val isSoft: Boolean)
      extends OrType(tp1, tp2)
  ...
  def expectValueTypeOrWildcard(tp: Type, where: => String)(using
      Context
  ): Unit =
  ...
  /** An extractor object to pattern match against a nullable union. e.g.
    *
    * (tp: Type) match case OrNull(tp1) => // tp had the form `tp1 | Null` case
    * _ => // tp was not a nullable union
    */
  object OrNull:
  ...
  /** A by-name parameter type of the form `=> T`, or the type of a method with
    * no parameter list.
    */
  abstract case class ExprType(resType: Type)
    ...
    def derivedExprType(resType: Type)(using Context): ExprType =
      if resType eq this.resType then this else ExprType(resType)
  ...
  /** The lambda type square:
    ...
    * -------------+-------------------+------------------ HKLambda |
    * HKTermLambda | HKTypeLambda MethodOrPoly | MethodType | PolyType
    */
  trait LambdaType extends BindingType with TermType:
    ...
    /** Like `paramInfos` but substitute parameter references with the given
      * arguments
      */
    final def instantiateParamInfos(
        argTypes: => List[Type]
    )(using Context): List[Type] =
      if isParamDependent then
        paramInfos.mapConserve(_.substParams(this, argTypes))
      else paramInfos

    /** Like `resultType` but substitute parameter references with the given
      * arguments
      */
    final def instantiate(argTypes: => List[Type])(using Context): Type =
      if isResultDependent then resultType.substParams(this, argTypes)
      else resultType
    ...
    /** The type `[tparams := paramRefs] tp`, where `tparams` can be either a
      * list of type parameter symbols or a list of lambda parameters
      *
      * @pre
      *   If `tparams` is a list of lambda parameters, then it must be the full,
      *   in-order list of type parameters of some type constructor, as can be
      *   obtained using `TypeApplications#typeParams`.
      */
    def integrate(tparams: List[ParamInfo], tp: Type)(using Context): Type =
      (tparams: @unchecked) match
        case LambdaParam(lam, _) :: _ =>
          tp.subst(lam, this) // This is where the precondition is necessary.
    ...
    override def toString: String =
      s"$prefixString($paramNames, $paramInfos, $resType)"
  end LambdaType

  abstract class HKLambda extends CachedProxyType with LambdaType:
    final override def underlying(using Context): Type = resType
    final override def hashIsStable: Boolean =
      resType.hashIsStable && paramInfos.hashIsStable
    final override def equals(that: Any): Boolean = equals(that, null)

  /** The superclass of MethodType and PolyType. */
  sealed abstract class MethodOrPoly
      extends UncachedGroundType
      with LambdaType
      with MethodicType:
    ...
    /** If `isJava` is false, the Scala signature of this method. Otherwise, its
      * Java signature.
      *
      * This distinction is needed because the same method type might be part of
      * both a Java and Scala class and each language has different type erasure
      * rules.
      *
      * Invariants:
    ...
    def signature(sourceLanguage: SourceLanguage)(using Context): Signature =
      def computeSignature(using Context): Signature =
        val resultSignature = resultType match
          ...
          case tp =>
            if tp.isRef(defn.UnitClass) then
              Signature(Nil, defn.UnitClass.fullName.asTypeName)
            else Signature(tp, sourceLanguage)
        this match
          case tp: MethodType =>
            val params =
              if hasErasedParams then
                tp.paramInfos
                  .zip(tp.erasedParams)
      ...
      sourceLanguage match
        case SourceLanguage.Java =>
          if ctx.runId != myJavaSignatureRunId then
            ...
            if !myJavaSignature.isUnderDefined then
              myJavaSignatureRunId = ctx.runId
          myJavaSignature
        case SourceLanguage.Scala2 =>
          if ctx.runId != myScala2SignatureRunId then
            myScala2Signature = computeSignature
            if !myScala2Signature.isUnderDefined then
              myScala2SignatureRunId = ctx.runId
          myScala2Signature
  ...
  trait TermLambda extends LambdaType:
    ...
    override def resultType(using Context): Type =
      if dependencyStatus == FalseDeps then // dealias all false dependencies
        val dealiasMap = new TypeMap with IdentityCaptRefMap:
          def apply(tp: Type) = tp match
            case tp @ TypeRef(pre, _) =>
              tp.info match
                case TypeAlias(alias) if depStatus(NoDeps, pre) == TrueDeps =>
                  apply(alias)
                case _ => mapOver(tp)
            case _ =>
              mapOver(tp)
    ...
    private def depStatus(initial: DependencyStatus, tp: Type)(using
      ...
      def compute(
          ...
          tp: Type,
          theAcc: TypeAccumulator[DependencyStatus] | Null
      ): DependencyStatus =
        def applyPrefix(tp: NamedType) =
          if tp.isInstanceOf[SingletonType] && tp.currentSymbol.isStatic
          then
            status // Note: a type ref with static symbol can still be dependent since the symbol might be refined in the enclosing type. See pos/15331.scala.
        ...
        if status == TrueDeps then status
          ...
          tp match
            ...
            case tp: AnnotatedType =>
              tp match
                case CapturingType(parent, refs) =>
                  (compute(status, parent, theAcc) /: refs.elems) { (s, ref) =>
                    ref match
                      case tp: TermParamRef if tp.binder eq thisLambdaType =>
                        combine(s, CaptureDeps)
                      case _ => s
                  }
                case _ =>
    ...
    /** The dependency status of this method. Some examples:
      *
      * class C extends { type S; type T = String } def f(x: C)(y: Boolean) //
      * dependencyStatus = NoDeps def f(x: C)(y: x.S) // dependencyStatus =
      * TrueDeps def f(x: C)(y: x.T) // dependencyStatus = FalseDeps, i.e. //
      * dependency can be eliminated by dealiasing.
      */
    private def dependencyStatus(using Context): DependencyStatus =
      if myDependencyStatus != Unknown then myDependencyStatus
      else {
        val result = depStatus(NoDeps, resType)
        if (result & Provisional) == 0 then myDependencyStatus = result
        (result & StatusMask).toByte
      }

    /** The parameter dependency status of this method. Analogous to
      * `dependencyStatus`, but tracking dependencies in same parameter list.
      */
    private def paramDependencyStatus(using Context): DependencyStatus =
      if myParamDependencyStatus != Unknown then myParamDependencyStatus
        ...
        val result =
          if paramInfos.isEmpty then NoDeps
          else paramInfos.tail.foldLeft(NoDeps)(depStatus(_, _))
        if (result & Provisional) == 0 then myParamDependencyStatus = result
        (result & StatusMask).toByte
      }
        ...
        val dropDependencies = new ApproximatingTypeMap
          ...
          def apply(tp: Type) = tp match
            ...
            case AnnotatedType(parent, ann)
                if ann.refersToParamOf(thisLambdaType) =>
              val parent1 = mapOver(parent)
              if ann.symbol == defn.RetainsAnnot || ann.symbol == defn.RetainsByNameAnnot
              then
                range(
                  AnnotatedType(
                    parent1,
                    CaptureSet.empty.toRegularAnnotation(ann.symbol)
                  ),
                  AnnotatedType(
                    parent1,
                    CaptureSet.universal.toRegularAnnotation(ann.symbol)
                  )
                )
              else parent1
            case _ => mapOver(tp)
  ...
  abstract class LambdaTypeCompanion[
    ...
    def syntheticParamNames(n: Int): List[N] = synchronized {
      memoizedNames.getOrElseUpdate(
        n,
        (0 until n).map(syntheticParamName).toList
      )
    }

    def apply(
        paramNames: List[N]
    )(paramInfosExp: LT => List[PInfo], resultTypeExp: LT => Type)(using
        Context
    ): LT
    def apply(paramNames: List[N], paramInfos: List[PInfo], resultType: Type)(
        using Context
    ): LT =
      apply(paramNames)(_ => paramInfos, _ => resultType)
    def apply(
        paramInfos: List[PInfo]
    )(resultTypeExp: LT => Type)(using Context): LT =
      apply(syntheticParamNames(paramInfos.length))(
        _ => paramInfos,
        resultTypeExp
      )
    ...
    /** If `tparam` is a sealed type parameter symbol of a polymorphic method,
      * add a @caps.Sealed annotation to the upperbound in `tp`.
      */
    protected def addSealed(tparam: ParamInfo, tp: Type)(using Context): Type =
      tp

    def fromParams[PI <: ParamInfo.Of[N]](params: List[PI], resultType: Type)(
        using Context
    ): Type =
      if params.isEmpty then resultType
      else
        apply(params.map(_.paramName))(
          tl =>
            params.map(param =>
              toPInfo(addSealed(param, tl.integrate(params, param.paramInfo)))
            ),
          tl => tl.integrate(params, resultType)
        )
  end LambdaTypeCompanion
  ...
  abstract class MethodTypeCompanion(val prefixString: String)
      extends TermLambdaCompanion[MethodType]:
    self =>

    /** Produce method type from parameter symbols, with special mappings for
      * repeated and inline parameters:
      *   - replace @repeated annotations on Seq or Array types by <repeated>
      *     types
      *   - add @inlineParam to inline parameters
      *   - add @erasedParam to erased parameters
      *   - wrap types of parameters that have an @allowConversions annotation
      *     with Into[_]
      */
    def fromSymbols(params: List[Symbol], resultType: Type)(using
        Context
    ): MethodType =
      def addAnnotation(tp: Type, cls: ClassSymbol, param: Symbol): Type =
        tp match
      ...
      /** Add `Into[..] to the type itself and if it is a function type, to all
        * its curried result type(s) as well.
        */
      def addInto(tp: Type): Type = tp match
        case tp @ AppliedType(tycon, args)
            if tycon.typeSymbol == defn.RepeatedParamClass =>
          tp.derivedAppliedType(tycon, addInto(args.head) :: Nil)
        case tp @ AppliedType(tycon, args) if defn.isFunctionType(tp) =>
          wrapConvertible(
            tp.derivedAppliedType(tycon, args.init :+ addInto(args.last))
          )
        case tp @ RefinedType(parent, rname, rinfo)
            if defn.isFunctionOrPolyType(tp) =>
    ...
    def apply(paramNames: List[TermName])(
        paramInfosExp: MethodType => List[Type],
        resultTypeExp: MethodType => Type
    )(using Context): MethodType =
      checkValid(
        unique(
          new CachedMethodType(paramNames)(paramInfosExp, resultTypeExp, self)
        )
      )

    def checkValid(mt: MethodType)(using Context): mt.type =
      if Config.checkMethodTypes then
        for (paramInfo, idx) <- mt.paramInfos.zipWithIndex do
  ...
  object MethodType extends MethodTypeCompanion("MethodType"):
    def companion(
      ...
      if isContextual then ContextualMethodType
      else if isImplicit then ImplicitMethodType
      else MethodType

  object ContextualMethodType
      extends MethodTypeCompanion("ContextualMethodType")
  object ImplicitMethodType extends MethodTypeCompanion("ImplicitMethodType")

  /** A ternary extractor for MethodType */
  object MethodTpe:
    def unapply(mt: MethodType)(using
        Context
    ): Some[(List[TermName], List[Type], Type)] =
  ...
  trait TypeLambda extends LambdaType:
    ...
    def derivedLambdaAbstraction(
        paramNames: List[TypeName],
        paramInfos: List[TypeBounds],
        resType: Type
    )(using Context): Type =
      resType match
        case resType: AliasingBounds =>
          resType.derivedAlias(
            newLikeThis(paramNames, paramInfos, resType.alias)
          )
        case resType @ TypeBounds(lo, hi) =>
          resType.derivedTypeBounds(
            if lo.isRef(defn.NothingClass) then lo
            else newLikeThis(paramNames, paramInfos, lo),
  ...
  /** A type lambda of the form `[X_0 B_0, ..., X_n B_n] => T`
    ...
    * @param variances
    *   The variances of the type parameters, if the type lambda carries
    *   variances, i.e. it is a bound of an abstract type or the rhs of a match
    *   alias or opaque alias. The parameter is Nil for all other lambdas.
    *
    * Variances are stored in the `typeParams` list of the lambda.
    */
  class HKTypeLambda(
      val paramNames: List[TypeName],
      @constructorOnly variances: List[Variance]
  )(
      paramInfosExp: HKTypeLambda => List[TypeBounds],
      resultTypeExp: HKTypeLambda => Type
  ) extends HKLambda
      with TypeLambda:
    ...
    private def setVariances(
        tparams: List[LambdaParam],
        vs: List[Variance]
    ): Unit =
    ...
    override def computeHash(bs: Binders): Int =
      doHash(
        new SomeBinders(this, bs),
        declaredVariances ::: paramNames,
        resType,
        paramInfos
      )
    ...
    final override def iso(that: Any, bs: BinderPairs): Boolean = that match
      case that: HKTypeLambda =>
        ...
        && {
          ...
          (paramInfos: List[TypeBounds] | Null) != null && (resType: Type |
            Null) != null &&
          paramInfos.equalElements(that.paramInfos, bs1) &&
          resType.equals(that.resType, bs1)
        }
    ...
    final override def toString: String =
      if isDeclaredVarianceLambda then
        s"HKTypeLambda($paramNames, $paramInfos, $resType, ${declaredVariances
            .map(_.flagsString)})"
      else super.toString
  ...
  class PolyType(val paramNames: List[TypeName])(
    ...
    /** Merge nested polytypes into one polytype. nested polytypes are normally
      * not supported but can arise as temporary data structures.
      */
    def flatten(using Context): PolyType = resType match
      case that: PolyType =>
        val shiftedSubst = (x: PolyType) =>
          new TypeMap:
        ...
        PolyType(paramNames ++ that.paramNames)(
          x =>
            this.paramInfos.mapConserve(_.subst(this, x).bounds) ++
  ...
  object HKTypeLambda extends TypeLambdaCompanion[HKTypeLambda]:
    ...
    def apply(paramNames: List[TypeName], variances: List[Variance])(
      ...
      unique(
        new HKTypeLambda(paramNames, variances)(paramInfosExp, resultTypeExp)
      )
    ...
    def any(n: Int)(using Context): HKTypeLambda =
      apply(syntheticParamNames(n))(
        pt => List.fill(n)(TypeBounds.empty),
        pt => defn.AnyType
      )

    override def fromParams[PI <: ParamInfo.Of[TypeName]](
        params: List[PI],
        resultType: Type
    )(using Context): Type =
      resultType match
    ...
    /** Distributes Lambda inside type bounds. Examples:
      *
      * type T[X] = U becomes type T = [X] -> U type T[X] <: U becomes type T >:
      * Nothing <: ([X] -> U) type T[X] >: L <: U becomes type T >: ([X] -> L)
      * <: ([X] -> U)
      *
      ...
      * `params`. The variances of other type aliases are determined from the
      * given parameters only if one of these parameters carries a `+` or `-`
      * variance annotation. Type aliases without variance annotation are
      * treated structurally. That is, their parameter variances are determined
      * by how the parameter(s) appear in the result type.
      *
      * Examples:
      *
      * type T[X] >: A // X is invariant type T[X] <: List[X] // X is invariant
      * type T[X] = List[X] // X is covariant (determined structurally) opaque
      * type T[X] = List[X] // X is invariant opaque type T[+X] = List[X] // X
      * is covariant type T[A, B] = A => B // A is contravariant, B is covariant
      * (determined structurally) type T[A, +B] = A => B // A is invariant, B is
      * covariant
      */
    def boundsFromParams[PI <: ParamInfo.Of[TypeName]](
        params: List[PI],
        bounds: TypeBounds
    )(using Context): TypeBounds =
      def expand(tp: Type, useVariances: Boolean) =
        if params.nonEmpty && useVariances then
          apply(params.map(_.paramName), params.map(_.paramVariance))(
            tl =>
              params.map(param =>
                toPInfo(tl.integrate(params, param.paramInfo))
              ),
            tl => tl.integrate(params, tp)
          )
        else super.fromParams(params, tp)
  ...
  object PolyType extends TypeLambdaCompanion[PolyType]:
    def apply(paramNames: List[TypeName])(
        paramInfosExp: PolyType => List[TypeBounds],
        resultTypeExp: PolyType => Type
    )(using Context): PolyType =
      unique(new PolyType(paramNames)(paramInfosExp, resultTypeExp))

    override protected def addSealed(tparam: ParamInfo, tp: Type)(using
        Context
    ): Type =
      tparam match
          ...
          tp match
            case tp @ TypeBounds(lo, hi) =>
              tp.derivedTypeBounds(
                ...
                AnnotatedType(
                  hi,
                  Annotation(defn.Caps_SealedAnnot, tparam.span)
                )
              )
            case _ => tp
        case _ => tp
  ...
  private object DepStatus:
    ...
    final val FalseDeps: DependencyStatus =
      2 // all dependent parameters are prefixes of non-depended alias types
    final val CaptureDeps: DependencyStatus =
      3 // dependencies in capture sets under captureChecking, otherwise only false dependencoes
    final val TrueDeps: DependencyStatus =
      4 // some truly dependent parameters exist
    final val StatusMask: DependencyStatus =
      7 // the bits indicating actual dependency status
    final val Provisional: DependencyStatus =
      8 // set if dependency status can still change due to type variable instantiations

  // ----- Type application: LambdaParam, AppliedType ---------------------

  /** The parameter of a type lambda */
  case class LambdaParam(tl: TypeLambda, n: Int)
      extends ParamInfo,
        printing.Showable:
    type ThisName = TypeName
    ...
    /** Set the declared variance of this parameter.
      * @pre
      *   the containing lambda is a isDeclaredVarianceLambda
      */
  ...
  abstract case class AppliedType(tycon: Type, args: List[Type])
    ...
    def isGround(acc: TypeAccumulator[Boolean])(using Context): Boolean =
      if myGround == 0 then
        myGround = if acc.foldOver(true, this) then 1 else -1
      myGround > 0
    ...
    private def computeIsStable(using Context): Boolean = tycon match
      case tycon: TypeRef
          if defn.isCompiletimeAppliedType(tycon.symbol) && args.forall(
            _.isStable
          ) =>
        true
      case _ => false
    ...
    override def superType(using Context): Type =
      if ctx.period != validSuper then
        validSuper = if tycon.isProvisional then Nowhere else ctx.period
    ...
    /** Is this an unreducible application to wildcard arguments? This is the
      * case if tycon is higher-kinded. This means it is a subtype of a
      * hk-lambda, but not a match alias. (normal parameterized aliases are
      * removed in `appliedTo`). Applications of higher-kinded type constructors
      * to wildcard arguments are equivalent to existential types, which are not
      * supported.
      */
    ...
    def lowerBound(using Context): Type = tycon.stripTypeVar match
      case tycon: TypeRef =>
        tycon.info match
          case TypeBounds(lo, hi) =>
            if lo eq hi then
              superType // optimization, can profit from caching in this case
    ...
    def tyconTypeParams(using Context): List[ParamInfo] =
      ...
      if tparams.isEmpty then HKTypeLambda.any(args.length).typeParams
      else tparams
    ...
    override def hashIsStable: Boolean =
      if myStableHash == 0 then
        myStableHash = if tycon.hashIsStable && args.hashIsStable then 1 else -1
      myStableHash > 0

    override def eql(that: Type): Boolean =
      this `eq` that // safe because applied types are hash-consed separately

    // equals comes from case class; no matching override is needed

    final override def iso(that: Any, bs: BinderPairs): Boolean = that match
      case that: AppliedType =>
        tycon.equals(that.tycon, bs) && args.equalElements(that.args, bs)
      case _ => false
  end AppliedType

  final class CachedAppliedType(tycon: Type, args: List[Type], hc: Int)
  ...
  abstract class ParamRef extends BoundType:
    ...
    override def underlying(using Context): Type =
      ...
      if infos == null then
        NoType // this can happen if the referenced generic type is not initialized yet
      else infos(paramNum)

    override def computeHash(bs: Binders): Int =
      doHash(paramNum, binder.identityHash(bs))

    override def equals(that: Any): Boolean = equals(that, null)

    override def iso(that: Any, bs: BinderPairs): Boolean = that match
      case that: ParamRef =>
        paramNum == that.paramNum && binder.equalBinder(that.binder, bs)
      case _ => false
  ...
  private final class TermParamRefImpl(binder: TermLambda, paramNum: Int)
      extends TermParamRef(binder, paramNum)
  ...
  abstract case class TypeParamRef(binder: TypeLambda, paramNum: Int)
    ...
    /** Optimized version of occursIn, avoid quadratic blowup when solving
      ...
      */
    override def occursIn(that: Type)(using Context): Boolean =
      !that.isGround && super.occursIn(that)

    /** Looking only at the structure of `bound`, is one of the following true?
      *   - fromBelow and param <:< bound
      *   - !fromBelow and param >:> bound
      */
    def occursIn(bound: Type, fromBelow: Boolean)(using Context): Boolean =
      bound.stripTypeVar match
        case bound: ParamRef => bound == this
        case bound: AndType =>
          occursIn(bound.tp1, fromBelow) && occursIn(bound.tp2, fromBelow)
        case bound: OrType =>
          occursIn(bound.tp1, fromBelow) || occursIn(bound.tp2, fromBelow)
        case _ => false

  private final class TypeParamRefImpl(binder: TypeLambda, paramNum: Int)
      extends TypeParamRef(binder, paramNum)

  /** a self-reference to an enclosing recursive type. The only creation method
    * is `binder.recThis`, returning `RecThis(binder)`.
    */
  abstract case class RecThis(binder: RecType)
      extends BoundType
      with SingletonType:
    type BT = RecType
    ...
    override def computeHash(bs: Binders): Int =
      addDelta(binder.identityHash(bs), 41)
    ...
    override def toString: String =
      ...
      catch case ex: NullPointerException => s"RecThis(<under construction>)"
  ...
  /** A skolem type reference with underlying type `info`.
    *
    * For Dotty, a skolem type is a singleton type of some unknown value of type
    * `info`. Note that care is needed when creating them, since not all types
    * need to be inhabited. A skolem is equal to itself and no other type.
    */
  case class SkolemType(info: Type)
      extends CachedProxyType
      with ValueType
      with SingletonType:
    override def underlying(using Context): Type = info
    def derivedSkolemType(info: Type)(using Context): SkolemType =
      if info eq this.info then this else SkolemType(info)
    ...
    def repr(using Context): Name =
      if myRepr == null then myRepr = SkolemName.fresh()
      myRepr.nn
  ...
  /** In a TypeApply tree, a TypeVar is created for each argument type to be
    * inferred. Every type variable is referred to by exactly one inferred type
    * parameter of some TypeApply tree.
    *
    * A type variable is essentially a switch that models some part of a
    * substitution. It is first linked to `origin`, a poly param that's in the
    * current constraint set. It can then be (once) instantiated to some other
    * type. The instantiation is recorded in the type variable itself, or else,
    * if the current type state is different from the variable's creation state
    * (meaning unrolls are possible) in the current typer state.
    *
    * @param origin
    *   the parameter that's tracked by the type variable.
    * @param creatorState
    *   the typer state in which the variable was created.
    * @param initNestingLevel
  ...
  final class TypeVar private (
    ...
    /** Set origin to new parameter. Called if we merge two conflicting
      * constraints. See OrderingConstraint#merge, OrderingConstraint#rename
      */
    def setOrigin(p: TypeParamRef) = currentOrigin = p

    /** The permanent instance type of the variable, or NoType is none is given
      * yet
      */
    ...
    private[core] def setInst(tp: Type): Unit =
      ...
      if tp.exists && owningState != null then
        ...
        if owningState1 != null then
          ...
          owningState =
            null // no longer needed; null out to avoid a memory leak
    ...
    /** The state owning the variable. This is at first `creatorState`, but it
      * can be changed to an enclosing state on a commit.
      */
    private[core] var owningState: WeakReference[TyperState] | Null =
      if creatorState == null then null else new WeakReference(creatorState)

    /** The nesting level of this type variable in the current typer state. This
      * is usually the same as `initNestingLevel`, but can be decremented by
      * calling `TyperState#setNestingLevel`. Symbols with a nestingLevel
      * strictly greater than this level will not appear in the instantiation of
      * this type variable. This is enforced in `ConstraintHandling`, dependig
      * on the Config flags setting `checkLevelsOnConstraints` and
      * `checkLevelsOnInstantiation`.
      *
      * Under `checkLevelsOnConstraints` we maintain the invariant that the
      * `nonParamBounds` of a type variable never refer to a type with a greater
      * `nestingLevel` (see `legalBound` for the reason why this cannot be
      * delayed until instantiation). Then, on instantiation, we replace any
      * param in the param bound with a level greater than nestingLevel (see
      * `fullLowerBound`).
      *
      * Under `checkLevelsOnInstantiation`, we avoid incorrect levels only when
      * a type variable is instantiated, see `ConstraintHandling$fixLevels`.
    ...
    /** The instance type of this variable, or NoType if the variable is
      * currently uninstantiated
      */
    def instanceOpt(using Context): Type =
      if inst.exists then inst else ctx.typerState.constraint.instType(this)
    ...
    def instantiateWith(tp: Type)(using Context): Type =
      assert(
        tp ne this,
        i"self instantiation of $origin, constraint = ${ctx.typerState.constraint}"
      )
      assert(
        !myInst.exists,
        i"$origin is already instantiated to $myInst but we attempted to instantiate it to $tp"
      )
      typr.println(i"instantiating $this with $tp")

      if Config.checkConstraintsSatisfiable then
        assert(
          currentEntry.bounds.contains(tp),
    ...
    /** Instantiate variable from the constraints over its `origin`. If
      * `fromBelow` is true, the variable is instantiated to the lub of its
      * lower bounds in the current constraint; otherwise it is instantiated to
      * the glb of its upper bounds. However, a lower bound instantiation can be
      * a singleton type only if the upper bound is also a singleton type.
      */
    def instantiate(fromBelow: Boolean)(using Context): Type =
      val tp =
        TypeComparer.instanceType(origin, fromBelow, widenUnions, nestingLevel)
      if myInst.exists
      then // The line above might have triggered instantiation of the current type variable
        myInst
      else instantiateWith(tp)

    /** Widen unions when instantiating this variable in the current context? */
    def widenUnions(using Context): Boolean =
      !ctx.typerState.constraint.isHard(this)

    /** For uninstantiated type variables: the entry in the constraint (either
      * bounds or provisional instance value)
      */
    private def currentEntry(using Context): Type =
      ctx.typerState.constraint.entry(origin)

    /** For uninstantiated type variables: Is the lower bound different from
      * Nothing?
      */
    def hasLowerBound(using Context): Boolean =
      !currentEntry.loBound.isExactlyNothing

    /** For uninstantiated type variables: Is the upper bound different from
      * Any?
      */
    def hasUpperBound(using Context): Boolean =
      !currentEntry.hiBound.finalResultType.isExactlyAny

    /** Unwrap to instance (if instantiated) or origin (if not), until result is
      * no longer a TypeVar
      */
    override def stripTypeVar(using Context): Type =
      val inst = instanceOpt
      if inst.exists then inst.stripTypeVar else origin
    ...
    override def underlying(using Context): Type =
      val inst = instanceOpt
      if inst.exists then inst else origin

    override def computeHash(bs: Binders): Int = identityHash(bs)
    override def equals(that: Any): Boolean = this.eq(that.asInstanceOf[AnyRef])

    override def toString: String =
      def instStr = if inst.exists then s" -> $inst" else ""
  ...
  /** scrutinee match { case_1 ... case_n }
    ...
    */
  abstract case class MatchType(bound: Type, scrutinee: Type, cases: List[Type])
      extends CachedProxyType
      with ValueType:
    def derivedMatchType(bound: Type, scrutinee: Type, cases: List[Type])(using
        Context
    ): MatchType =
      if bound.eq(this.bound) && scrutinee.eq(this.scrutinee) && cases
          .eqElements(this.cases)
      then this
    ...
    def reduced(using Context): Type =
      ...
      def contextInfo(tp: Type): Type = tp match
        case tp: TypeParamRef =>
          ...
          if constraint.entry(tp).exists then TypeComparer.fullBounds(tp)
          else NoType
        case tp: TypeRef =>
          val bounds = ctx.gadt.fullBounds(tp.symbol)
          if bounds == null then NoType else bounds
        case tp: TypeVar =>
          tp.underlying

      def updateReductionContext(footprint: collection.Set[Type]): Unit =
        reductionContext = util.HashMap()
        for tp <- footprint do reductionContext(tp) = contextInfo(tp)
        typr.println(i"footprint for $this $hashCode: ${footprint.toList
            .map(x => (x, contextInfo(x)))}%, %")
      ...
      then
        ...
        if myReduced != null then record("MatchType.reduce cache miss")
        myReduced =
    ...
    override def computeHash(bs: Binders): Int =
      doHash(bs, scrutinee, bound :: cases)

    override def eql(that: Type): Boolean = that match
      case that: MatchType =>
        bound.eq(that.bound) && scrutinee.eq(that.scrutinee) && cases
          .eqElements(that.cases)
      case _ => false
  end MatchType

  class CachedMatchType(bound: Type, scrutinee: Type, cases: List[Type])
      extends MatchType(bound, scrutinee, cases)

  object MatchType:
    def apply(bound: Type, scrutinee: Type, cases: List[Type])(using
        Context
    ): MatchType =
  ...
  /** Roughly: the info of a class during a period.
    ...
    * @param cls
    *   The class symbol.
    * @param declaredParents
    *   The parent types of this class. These are all normalized to be TypeRefs
    *   by moving any refinements to be member definitions of the class itself.
    *   Unlike `parents`, the types are not seen as seen from `prefix`.
    * @param decls
    *   The symbols defined directly in this class.
    * @param selfInfo
    *   The type of `this` in this class, if explicitly given, NoType otherwise.
    *   If class is compiled from source, can also be a reference to the self
    *   symbol containing the type.
    */
  abstract case class ClassInfo(
    ...
    /** The self type of a class is the conjunction of
      *   - the explicit self type if given (or the info of a given self
      *     symbol), and
      *   - the fully applied reference to the class itself.
      */
    def selfType(using Context): Type =
      val clsd = cls.classDenot
      if selfTypeCache == null then
        selfTypeCache =
          val givenSelf = clsd.givenSelfType
          if !givenSelf.isValueType then appliedRef
          else if clsd.is(Module) then givenSelf
          else if ctx.erasedTypes then appliedRef
          else
            givenSelf.dealiasKeepAnnots match
              case givenSelf1 @ EventuallyCapturingType(tp, _) =>
                givenSelf1.derivedAnnotatedType(
                  tp & appliedRef,
                  givenSelf1.annot
                )
              case _ =>
    ...
    def appliedRef(using Context): Type =
      if appliedRefCache == null then
        appliedRefCache = TypeRef(prefix, cls).appliedTo(
          cls.classDenot.typeParams.map(_.typeRef)
        )
      appliedRefCache.nn
    ...
    override def parents(using Context): List[Type] =
      if parentsCache == null then
        parentsCache =
          declaredParents.mapConserve(_.asSeenFrom(prefix, cls.owner))
      parentsCache.nn

    protected def newLikeThis(
        prefix: Type,
        declaredParents: List[Type],
        decls: Scope,
        selfInfo: TypeOrSymbol
    )(using Context): ClassInfo =
    ...
    def derivedClassInfo(
        prefix: Type = this.prefix,
        declaredParents: List[Type] = this.declaredParents,
        decls: Scope = this.decls,
        selfInfo: TypeOrSymbol = this.selfInfo
    )(using Context): ClassInfo =
      if (prefix eq this.prefix) && (declaredParents eq this.declaredParents) && (decls eq this.decls) && (selfInfo eq this.selfInfo)
      then this
      else newLikeThis(prefix, declaredParents, decls, selfInfo)

    /** If this class has opaque type alias members, a new class info with their
      * aliases added as refinements to the self type of the class. Otherwise,
      * this classInfo. If there are opaque alias members, updates `cls` to have
      * `Opaque` flag as a side effect.
      */
      ...
      decls.toList.foldLeft(this) { (cinfo, sym) =>
        if sym.isOpaqueAlias then
          ...
          def force(using Context) =
            if sym.isOpaqueAlias
            then // could have been reset because of a syntax error
          ...
          def refineSelfType(selfType: Type) =
            RefinedType(
              selfType,
              sym.name,
              TypeAlias(withMode(Mode.CheckCyclic)(LazyRef.of(force)))
            )
          cinfo.selfInfo match
            case self: Type =>
    ...
    override def hashIsStable: Boolean =
      prefix.hashIsStable && declaredParents.hashIsStable
    ...
    override def toString: String =
      s"ClassInfo($prefix, $cls, $declaredParents)"
  end ClassInfo
  ...
  object ClassInfo:
    def apply(
        ...
        declaredParents: List[Type],
        decls: Scope,
        selfInfo: TypeOrSymbol = NoType
    )(using Context): ClassInfo =
      unique(new CachedClassInfo(prefix, cls, declaredParents, decls, selfInfo))

  /** Type bounds >: lo <: hi */
  abstract case class TypeBounds(lo: Type, hi: Type)
      extends CachedProxyType
      with TypeType:
      ...
      case tp: ClassInfo =>
        ...
        contains(
          TypeRef(tp.prefix, cls)
            .withDenot(
              new UniqueRefDenotation(cls, tp, cls.validFor, tp.prefix)
            )
        )
      case _ =>
        lo <:< tp && tp <:< hi
    ...
    def &(that: TypeBounds)(using Context): TypeBounds =
      ...
      then
        ...
        && (this.lo frozen_<:< that.lo)
      then
        // Similarly, prefer tp2 if FromJavaObject is in its hi
        that
      else if (this.lo frozen_<:< that.lo) && (that.hi frozen_<:< this.hi) then
        that
      else if (that.lo frozen_<:< this.lo) && (this.hi frozen_<:< that.hi) then
        this
      else TypeBounds(this.lo | that.lo, this.hi & that.hi)

    def |(that: TypeBounds)(using Context): TypeBounds =
      if (this.lo frozen_<:< that.lo) && (that.hi frozen_<:< this.hi) then this
      else if (that.lo frozen_<:< this.lo) && (this.hi frozen_<:< that.hi) then
        that
      else TypeBounds(this.lo & that.lo, this.hi | that.hi)
  ...
  abstract class AliasingBounds(val alias: Type)
      extends TypeBounds(alias, alias):
    ...
    override def iso(that: Any, bs: BinderPairs): Boolean = that match
      case that: AliasingBounds =>
        this.isTypeAlias == that.isTypeAlias && alias.equals(that.alias, bs)
      case _ => false
    ...
    override def eql(that: Type): Boolean = that match
      case that: AliasingBounds =>
        this.isTypeAlias == that.isTypeAlias && alias.eq(that.alias)
      case _ => false
  ...
  class TypeAlias(alias: Type) extends AliasingBounds(alias):
    def derivedAlias(alias: Type)(using Context): AliasingBounds =
      if alias eq this.alias then this else TypeAlias(alias)

  /** \= T where `T` is a `MatchType`
    *
    * Match aliases are treated differently from type aliases. Their sides are
    * mutually subtypes of each other but one side is not generally
    * substitutable for the other. If we assumed full substitutivity, we would
    * have to reject all recursive match aliases (or else take the jump and
    * allow full recursive types).
    */
  class MatchAlias(alias: Type) extends AliasingBounds(alias):
    def derivedAlias(alias: Type)(using Context): AliasingBounds =
      if alias eq this.alias then this else MatchAlias(alias)
  ...
  object TypeBounds:
    ...
    def emptyPolyKind(using Context): TypeBounds =
      apply(defn.NothingType, defn.AnyKindType)
  ...
  object TypeAlias:
    def apply(alias: Type)(using Context): TypeAlias = unique(
      new TypeAlias(alias)
    )
    def unapply(tp: TypeAlias): Option[Type] = Some(tp.alias)

  object MatchAlias:
    def apply(alias: Type)(using Context): MatchAlias = unique(
      new MatchAlias(alias)
    )
  ...
  abstract case class AnnotatedType(parent: Type, annot: Annotation)
      extends CachedProxyType,
        ValueType:

    override def underlying(using Context): Type = parent

    def derivedAnnotatedType(parent: Type, annot: Annotation)(using
        Context
    ): AnnotatedType =
      if (parent eq this.parent) && (annot eq this.annot) then this
      else AnnotatedType(parent, annot)

    override def stripTypeVar(using Context): Type =
      derivedAnnotatedType(parent.stripTypeVar, annot)

    override def stripAnnots(keep: Annotation => (Context) ?=> Boolean)(using
        Context
    ): Type =
    ...
    override def eql(that: Type): Boolean = that match
      case that: AnnotatedType =>
        (parent eq that.parent) && (annot eql that.annot)
      case _ => false

    override def iso(that: Any, bs: BinderPairs): Boolean = that match
      case that: AnnotatedType =>
        parent.equals(that.parent, bs) && (annot eql that.annot)
      case _ => false
  end AnnotatedType

  class CachedAnnotatedType(parent: Type, annot: Annotation)
      extends AnnotatedType(parent, annot)
  ...
  abstract case class JavaArrayType(elemType: Type)
      extends CachedGroundType
      with ValueType:
    def derivedJavaArrayType(elemtp: Type)(using Context): JavaArrayType =
      if elemtp eq this.elemType then this else JavaArrayType(elemtp)
    ...
    override def eql(that: Type): Boolean = that match
      ...
      case _                   => false
  final class CachedJavaArrayType(elemType: Type)
      extends JavaArrayType(elemType)
  object JavaArrayType:
    def apply(elemType: Type)(using Context): JavaArrayType = unique(
      new CachedJavaArrayType(elemType)
    )
  ...
  /** A common superclass of `ErrorType` and `TryDynamicCallSite`. Instances of
    * this class are at the same time subtypes and supertypes of every other
    * type.
    */
  ...
  abstract case class WildcardType(optBounds: Type)
      extends CachedGroundType
      with TermType:
    ...
    def derivedWildcardType(optBounds: Type)(using Context): WildcardType =
      if optBounds eq this.optBounds then this
      else if !optBounds.exists then WildcardType
  ...
  final class CachedWildcardType(optBounds: Type)
      extends WildcardType(optBounds)
  ...
  /** An extractor for single abstract method types. A type is a SAM type if it
    * is a reference to a class or trait, which
    *
    *   - has a single abstract method with a method type (ExprType and PolyType
    *     not allowed!) whose result type is not an implicit function type and
    *     which is not marked inline.
    *   - can be instantiated without arguments or with just () as argument.
    *
    * The pattern `SAMType(sam)` matches a SAM type, where `sam` is the type of
    ...
    def zeroParamClass(tp: Type)(using Context): Type = tp match
      case tp: ClassInfo =>
        def zeroParams(tp: Type): Boolean = tp.stripPoly match
          case mt: MethodType =>
            mt.paramInfos.isEmpty && !mt.resultType.isInstanceOf[MethodType]
          case et: ExprType => true
          case _            => false
        // `ContextFunctionN` does not have constructors
        val ctor = tp.cls.primaryConstructor
        if !ctor.exists || zeroParams(ctor.info) then tp
        else NoType
      case tp: AppliedType =>
      ...
      case _ =>
        NoType
    def isInstantiatable(tp: Type)(using Context): Boolean = zeroParamClass(
      tp
    ) match
      ...
      if isInstantiatable(tp) then
        ...
        if absMems.size == 1 then
          absMems.head.info match
                    ...
                    tp.info match
                      ...
                      case _ =>
                        range(
                          defn.NothingType,
                          defn.AnyType
                        ) // should happen only in error cases
                  case _ =>
                    mapOver(tp)
              val approx =
            ...
            case _ =>
              None
        else if tp isRef defn.PartialFunctionClass then
          ...
          // case clauses.
          absMems
            .find(_.symbol.name == nme.apply)
            .map(_.info.asInstanceOf[MethodType])
        else None
        end if
      else None
  end SAMType
  ...
  abstract class VariantTraversal:
    ...
    protected def stopBecauseStaticOrLocal(
        tp: NamedType
    )(using Context): Boolean =
      (tp.prefix eq NoPrefix)
        || {
          val stop = stopAt
          stop == StopAt.Static && tp.currentSymbol.isStatic && isStaticPrefix(
            tp.prefix
          )
          || stop == StopAt.Package && tp.currentSymbol.is(Package)
        }

    /** The type parameters of the constructor of this applied type. Overridden
      * in OrderingConstraint's ConstraintAwareTraversal to take account of
      * instantiations in the constraint that are not yet propagated to the
      * instance types of type variables.
      */
    protected def tyconTypeParams(tp: AppliedType)(using
        Context
    ): List[ParamInfo] =
      tp.tyconTypeParams
  end VariantTraversal

  /** A supertrait for some typemaps that are bijections. Used for capture
    * checking. BiTypeMaps should map capture references to capture references.
    */
  trait BiTypeMap extends TypeMap:
    ...
    /** The inverse of the type map as a BiTypeMap map, which has the original
      * type map as its own inverse.
      */
  ...
  abstract class TypeMap(implicit protected var mapCtx: Context)
    ...
    protected def derivedRefinedType(
        tp: RefinedType,
        parent: Type,
        info: Type
    ): Type =
    ...
    protected def derivedSuperType(
        tp: SuperType,
        thistp: Type,
        supertp: Type
    ): Type =
      tp.derivedSuperType(thistp, supertp)
    protected def derivedAppliedType(
        tp: AppliedType,
        tycon: Type,
        args: List[Type]
    ): Type =
    ...
    protected def derivedMatchType(
        tp: MatchType,
        bound: Type,
        scrutinee: Type,
        cases: List[Type]
    ...
    protected def derivedCapturingType(
        tp: Type,
        parent: Type,
        refs: CaptureSet
    ): Type =
    ...
    protected def derivedLambdaType(
        tp: LambdaType
    )(formals: List[tp.PInfo], restpe: Type): Type =
      tp.derivedLambdaType(tp.paramNames, formals, restpe)

    protected def mapArgs(
        args: List[Type],
        tparams: List[ParamInfo]
    ): List[Type] = args match
      case arg :: otherArgs if tparams.nonEmpty =>
        val arg1 = arg match
          case arg: TypeBounds => this(arg)
          case arg =>
            atVariance(variance * tparams.head.paramVarianceSign)(this(arg))
    ...
    protected def mapCapturingType(
        tp: Type,
        parent: Type,
        refs: CaptureSet,
        v: Int
      ...
      tp match
        case tp: NamedType =>
          if stopBecauseStaticOrLocal(tp) then tp
            ...
            val prefix1 = atVariance(variance max 0)(
              this(tp.prefix)
        ...
        case tp: AppliedType =>
          derivedAppliedType(
            tp,
            this(tp.tycon),
            mapArgs(tp.args, tyconTypeParams(tp))
          )
        ...
        case tp: TypeVar =>
          ...
          if inst.exists then apply(inst) else tp
    ...
    def mapOver(syms: List[Symbol]): List[Symbol] =
      mapSymbols(syms, treeTypeMap)

    def mapOver(scope: Scope): Scope =
      val elems = scope.toList
      val elems1 = mapOver(elems)
      if elems1 eq elems then scope
  ...
  /** A type map that approximates TypeBounds types depending on variance.
    *
    * if variance > 0 : approximate by upper bound variance < 0 : approximate by
    * lower bound variance = 0 : propagate bounds to next outer level
    */
  abstract class ApproximatingTypeMap(using Context) extends TypeMap:
    thisMap =>
    ...
    /** Expand parameter reference corresponding to prefix `pre`; If the
      * expansion is a wildcard parameter reference, convert its underlying
      * bounds to a range, otherwise return the expansion.
      */
    ...
    /** Derived selection.
      * @pre
      *   the (upper bound of) prefix `pre` has a member named `tp.name`.
      */
      ...
      if pre eq tp.prefix then tp
        ...
        pre match
          case Range(preLo, preHi) =>
            val forwarded =
              if tp.symbol.isAllOf(ClassTypeParam) then expandParam(tp, preHi)
              else tryWiden(tp, preHi)
            forwarded.orElse(
              range(
                super.derivedSelect(tp, preLo).loBound,
                super.derivedSelect(tp, preHi).hiBound
              )
            )
          case _ =>
    ...
    override protected def derivedRefinedType(
        tp: RefinedType,
        parent: Type,
        info: Type
    ): Type =
      if (parent eq tp.parent) && (info eq tp.refinedInfo) then tp
        ...
        parent match
          case Range(parentLo, parentHi) =>
            range(
              derivedRefinedType(tp, parentLo, info),
              derivedRefinedType(tp, parentHi, info)
            )
          case _ =>
            def propagate(lo: Type, hi: Type) =
              range(
                derivedRefinedType(tp, parent, lo),
                derivedRefinedType(tp, parent, hi)
              )
            if parent.isExactlyNothing then parent
              ...
              info match
                case Range(infoLo: TypeBounds, infoHi: TypeBounds) =>
                  assert(variance == 0)
                  if !infoLo.isTypeAlias && !infoHi.isTypeAlias then
    ...
    override protected def derivedAlias(tp: AliasingBounds, alias: Type): Type =
      if alias eq tp.alias then tp
        ...
        alias match
          case Range(lo, hi) =>
            if variance > 0 then TypeBounds(lo, hi)
            else range(tp.derivedAlias(lo), tp.derivedAlias(hi))
          case _ => tp.derivedAlias(alias)

    override protected def derivedTypeBounds(
        tp: TypeBounds,
        lo: Type,
        hi: Type
    ): Type =
      if (lo eq tp.lo) && (hi eq tp.hi) then tp
      else if isRange(lo) || isRange(hi) then
        if variance > 0 then TypeBounds(lower(lo), upper(hi))
        else
          range(
            TypeBounds(upper(lo), lower(hi)),
            TypeBounds(lower(lo), upper(hi))
          )
      else tp.derivedTypeBounds(lo, hi)

    override protected def derivedSuperType(
        tp: SuperType,
        thistp: Type,
        supertp: Type
    ): Type =
    ...
    override protected def derivedAppliedType(
        tp: AppliedType,
        tycon: Type,
        args: List[Type]
    ): Type =
      tycon match
        case Range(tyconLo, tyconHi) =>
          range(
            derivedAppliedType(tp, tyconLo, args),
            derivedAppliedType(tp, tyconHi, args)
          )
        case _ =>
          if args.exists(isRange) then
            if variance > 0 then
              tp.derivedAppliedType(tycon, args.map(rangeToBounds)) match
                case tp1: AppliedType
                    if tp1.isUnreducibleWild && ctx.phase != checkCapturesPhase =>
            ...
            if distributeArgs(args, tyconTypeParams(tp)) then
              range(
                tp.derivedAppliedType(tycon, loBuf.toList),
    ...
    private def isRangeOfNonTermTypes(tp: Type): Boolean = tp match
      case Range(lo, hi) =>
        !lo.isInstanceOf[TermType] || !hi.isInstanceOf[TermType]
      case _ => false

    override protected def derivedAndType(
        tp: AndType,
        tp1: Type,
        tp2: Type
    ): Type =
      if isRange(tp1) || isRange(tp2) then
        range(lower(tp1) & lower(tp2), upper(tp1) & upper(tp2))
      else tp.derivedAndType(tp1, tp2)

    override protected def derivedOrType(
        tp: OrType,
        tp1: Type,
        tp2: Type
    ): Type =
    ...
    override protected def derivedAnnotatedType(
        tp: AnnotatedType,
        underlying: Type,
        annot: Annotation
    ): Type =
      underlying match
        case Range(lo, hi) =>
          range(
            tp.derivedAnnotatedType(lo, annot),
            tp.derivedAnnotatedType(hi, annot)
          )
        case _ =>
          if underlying.isExactlyNothing then underlying
          else tp.derivedAnnotatedType(underlying, annot)
    override protected def derivedCapturingType(
        tp: Type,
        parent: Type,
        refs: CaptureSet
    ): Type =
      parent match // TODO ^^^ handle ranges in capture sets as well
        case Range(lo, hi) =>
          range(
            derivedCapturingType(tp, lo, refs),
            derivedCapturingType(tp, hi, refs)
          )
        case _ =>
          tp.derivedCapturingType(parent, refs)

    override protected def derivedWildcardType(
        tp: WildcardType,
        bounds: Type
    ): WildcardType =
      tp.derivedWildcardType(rangeToBounds(bounds))

    override protected def derivedMatchType(
        tp: MatchType,
        bound: Type,
        scrutinee: Type,
        cases: List[Type]
      ...
      bound match
        case Range(lo, hi) =>
          range(
            derivedMatchType(tp, lo, scrutinee, cases),
            derivedMatchType(tp, hi, scrutinee, cases)
          )
        case _ =>
          scrutinee match
            ...
            case _ =>
              ...
              else tp.derivedMatchType(bound, scrutinee, cases)
    ...
    override protected def derivedLambdaType(
        tp: LambdaType
    )(formals: List[tp.PInfo], restpe: Type): Type =
      restpe match
        case Range(lo, hi) =>
          range(
            derivedLambdaType(tp)(formals, lo),
            derivedLambdaType(tp)(formals, hi)
          )
        case _ =>
          if formals.exists(isRange) then
            range(
              derivedLambdaType(tp)(
                formals.map(upper(_).asInstanceOf[tp.PInfo]),
                restpe
              ),
              derivedLambdaType(tp)(
                formals.map(lower(_).asInstanceOf[tp.PInfo]),
                restpe
              )
            )
          else tp.derivedLambdaType(tp.paramNames, formals, restpe)

    /** Overridden in TypeOps.avoid */
    protected def needsRangeIfInvariant(refs: CaptureSet): Boolean = true

    override def mapCapturingType(
        tp: Type,
        parent: Type,
        refs: CaptureSet,
        v: Int
      ...
      if v == 0 && needsRangeIfInvariant(refs) then
        range(
          mapCapturingType(tp, parent, refs, -1),
          mapCapturingType(tp, parent, refs, 1)
        )
      else super.mapCapturingType(tp, parent, refs, v)

    protected def reapply(tp: Type): Type = apply(tp)
  end ApproximatingTypeMap
  ...
  abstract class TypeAccumulator[T](implicit protected val accCtx: Context)
    ...
    protected def applyToAnnot(x: T, annot: Annotation): T =
      x // don't go into annotations

    /** A prefix is never contravariant. Even if say `p.A` is used in a
      * contravariant context, we cannot assume contravariance for `p` because
      * `p`'s lower bound might not have a binding for `A`, since the lower
      * bound could be `Nothing`. By contrast, covariance does translate to the
      * prefix, since we have that if `p <: q` then `p.A <: q.A`, and
      * well-formedness requires that `A` is a member of `p`'s upper bound.
    ...
    def foldOver(x: T, tp: Type): T =
      ...
      tp match
        ...
        case tp @ AppliedType(tycon, args) =>
          @tailrec def foldArgs(
              x: T,
              tparams: List[ParamInfo],
              args: List[Type]
          ): T =
            if args.isEmpty || tparams.isEmpty then x
            else {
              val tparam = tparams.head
              val acc = args.head match
                case arg: TypeBounds => this(x, arg)
                case arg =>
                  atVariance(variance * tparam.paramVarianceSign)(this(x, arg))
  ...
  class ExistsAccumulator(
      ...
      extends TypeAccumulator[Boolean]:
    def apply(x: Boolean, tp: Type): Boolean =
      x || p(tp) || (forceLazy || !tp.isInstanceOf[LazyRef]) && foldOver(x, tp)

  class ForeachAccumulator(p: Type => Unit, override val stopAt: StopAt)(using
      Context
  ) extends TypeAccumulator[Unit]:
    def apply(x: Unit, tp: Type): Unit = foldOver(p(tp), tp)

  class NamedPartsAccumulator(p: NamedType => Boolean)(using Context)
      extends TypeAccumulator[List[NamedType]]:
    def maybeAdd(xs: List[NamedType], tp: NamedType): List[NamedType] =
      if p(tp) then tp :: xs else xs
  ...
  class CoveringSetAccumulator(using Context)
      extends TypeAccumulator[Set[Symbol]]:
    var seen = util.HashSet[Type](initialCapacity = 8)
    def apply(cs: Set[Symbol], tp: Type): Set[Symbol] =
      if seen.contains(tp) then cs
        ...
        tp match
          ...
          case tp: TermRef =>
            val tsym =
              if tp.termSymbol.is(Param) then tp.underlying.typeSymbol
              else tp.termSymbol
            foldOver(cs + tsym, tp)
          case tp: TypeParamRef =>
  ...
  /** A name filter selects or discards a member name of a type `pre`. To enable
    * efficient caching, name filters have to satisfy the following invariant:
    * If `keep` is a name filter, and `pre` has class `C` as a base class, then
    *
  ...
  abstract class NameFilter:
    ...
    /** Filter does not need to be rechecked with full prefix, if it has been
      * already checked for the class denotation of the prefix
      */
  ...
  object abstractTermNameFilter extends NameFilter:
    def apply(pre: Type, name: Name)(using Context): Boolean =
      name.isTermName && pre
        .nonPrivateMember(name)
        .hasAltWith(_.symbol.is(Deferred))
    def isStable = false
  ...
  object implicitFilter extends NameFilter:
    /** A dummy filter method. Implicit filtering is handled specially in
      * computeMemberNames, so no post-filtering is needed.
      */
  ...
  extension (tps1: List[Type])
    ...
    @tailrec def equalElements(tps2: List[Type], bs: BinderPairs): Boolean =
      (tps1 `eq` tps2) || {
        if tps1.isEmpty then tps2.isEmpty
        else
          tps2.nonEmpty && tps1.head.equals(tps2.head, bs) && tps1.tail
            .equalElements(tps2.tail, bs)
      }

  private val keepAlways: AnnotatedType => Context ?=> Boolean = _ => true
  private val keepNever: AnnotatedType => Context ?=> Boolean = _ => false
  private val keepIfRefining: AnnotatedType => Context ?=> Boolean =
    _.isRefining
