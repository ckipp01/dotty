...
object TypeApplications:
  ...
  object EtaExpansion:
    ...
    /** Test that the parameter bounds in a hk type lambda `[X1,...,Xn] => C[X1,
      * ..., Xn]` contain the bounds of the type parameters of `C`. This is
      * necessary to be able to contract the hk lambda to `C`.
      */
    private def weakerBounds(tp: HKTypeLambda, tparams: List[ParamInfo])(using
        Context
    ): Boolean =
      val onlyEmptyBounds =
        tp.typeParams.forall(_.paramInfo == TypeBounds.empty)
      onlyEmptyBounds
      ...
      || {
        ...
        tp.typeParams.corresponds(tparams) { (param1, param2) =>
          param2.paramInfo frozen_<:< param1.paramInfo.substParams(
            tp,
            paramRefs
          )
        }
    ...
    def unapply(tp: Type)(using Context): Option[Type] = tp match
      case tp @ HKTypeLambda(tparams, AppliedType(fn: Type, args))
          if fn.typeSymbol.isClass
            ...
            && args
              .lazyZip(tparams)
              .forall((arg, tparam) => arg == tparam.paramRef)
            && weakerBounds(tp, fn.typeParams) =>
        Some(fn)
      case _ => None

  end EtaExpansion

  /** Adapt all arguments to possible higher-kinded type parameters using
    * etaExpandIfHK
    */
  def EtaExpandIfHK(tparams: List[TypeParamInfo], args: List[Type])(using
      Context
  ): List[Type] =
    if tparams.isEmpty then args
    else
      args.zipWithConserve(tparams)((arg, tparam) =>
        arg.EtaExpandIfHK(tparam.paramInfoOrCompleter)
      )

  /** A type map that tries to reduce (part of) the result type of the type
    * lambda `tycon` with the given `args`(some of which are wildcard arguments
    * represented by type bounds). Non-wildcard arguments are substituted
    * everywhere as usual. A wildcard argument `>: L <: H` is substituted for a
    * type lambda parameter `X` only under certain conditions.
    *
    *   1. If Mode.AllowLambdaWildcardApply is set: The wildcard argument is
    *      substituted only if `X` appears in a toplevel application of the form
    *
    * C[..., X, ...]
    *
    * and there are no other occurrences of `X` in the reduced type. In that
    * case the refinement above is replaced by
    *
    * C[..., ? >: L <: H, ...]
    *
    * The `allReplaced` field indicates whether all occurrences of type lambda
    * parameters in the reduced type have been replaced with arguments.
    *
    * 2. If Mode.AllowLambdaWildcardApply is not set: All `X` arguments are
    * replaced by:
    *
    * ? >: L <: H
    *
    * Any other occurrence of `X` in `tycon` is replaced by `U`, if the
    * occurrence of `X` in `tycon` is covariant, or nonvariant, or by `L`, if
    * the occurrence is contravariant.
    *
    * The idea is that the `AllowLambdaWildcardApply` mode is used to check
    * whether a type can be soundly reduced, and to give an error or warning if
    * that is not the case. By contrast, the default mode, with
    * `AllowLambdaWildcardApply` not set, reduces all applications even if this
    * yields a different type, so its postcondition is that no type parameters
    * of `tycon` appear in the result type. Using this mode, we can guarantee
    * that `appliedTo` will never produce a higher-kinded application with a
    * type lambda as type constructor.
    */
  class Reducer(tycon: TypeLambda, args: List[Type])(using Context)
      extends TypeMap:
    ...
    def canReduceWildcard(p: TypeParamRef): Boolean =
      !ctx.mode.is(Mode.AllowLambdaWildcardApply) || available.contains(
        p.paramNum
      )
      ...
      case p: TypeParamRef if p.binder == tycon =>
        args(p.paramNum) match
          case TypeBounds(lo, hi) =>
            if ctx.mode.is(Mode.AllowLambdaWildcardApply) then
              allReplaced = false; p
...
class TypeApplications(val self: Type) extends AnyVal:
  ...
  /** The type parameters of this type are: For a ClassInfo type, the type
    * parameters of its class. For a typeref referring to a class, the type
    * parameters of the class. For a refinement type, the type parameters of its
    * parent, dropping any type parameter that is-rebound by the refinement.
    */
    ...
    try
      self match
        case self: TypeRef =>
          ...
          if tsym.isClass then tsym.typeParams
          else
            tsym.infoOrCompleter match
              case info: LazyType if isTrivial(self.prefix, tsym) =>
                info.completerTypeParams(tsym)
              case _ => self.info.typeParams
        case self: AppliedType =>
          if self.tycon.typeSymbol.isClass then Nil
  ...
  final def hkTypeParams(using Context): List[TypeParamInfo] =
    if isLambdaSub then typeParams else Nil
  ...
  /** The top type with the same kind as `self`. This is largest type capturing
    ...
    *   - A kind like (* -> *) -> * is represented by the top type [X1 <: [X2]
    *     \=>> Any] =>> Any
    */
  def topType(using Context): Type =
    if self.hasSimpleKind then defn.AnyType
    else
      EtaExpand(self.typeParams) match
        case tp: HKTypeLambda =>
          tp.derivedLambdaType(resType = tp.resultType.topType)
        case _ =>
          defn.AnyKindType

  /** If self type is higher-kinded, its result type, otherwise NoType. Note:
    * The hkResult of an any-kinded type is again AnyKind.
    */
  def hkResult(using Context): Type = self.dealias match
    case self: TypeRef =>
      if self.symbol == defn.AnyKindClass then self else self.info.hkResult
    case self: AppliedType =>
      if self.tycon.typeSymbol.isClass then NoType else self.superType.hkResult
  ...
  /** Convert a type constructor `TC` which has type parameters `X1, ..., Xn` to
    * `[X1, ..., Xn] -> TC[X1, ..., Xn]`.
    */
  ...
  def ensureLambdaSub(using Context): Type =
    if isLambdaSub then self else EtaExpansion(self)

  /** Eta expand if `self` is a (non-lambda) class reference and `bound` is a
    * higher-kinded type
    */
  def EtaExpandIfHK(bound: Type)(using Context): Type =
    val hkParams = bound.hkTypeParams
    if hkParams.isEmpty then self
    else
      self match
        case self: TypeRef
            if self.symbol.isClass && self.typeParams.length == hkParams.length =>
          EtaExpansion(self)
        case _ => self
  ...
  final def appliedTo(args: List[Type])(using Context): Type =
    ...
    if args.isEmpty || ctx.erasedTypes then self
    else
      dealiased match
        case dealiased: HKTypeLambda =>
          def tryReduce =
            if !args.exists(isBounds) then
              ...
              if (dealiased eq stripped) || followAlias then
                try
                  val instantiated = dealiased.instantiate(
                    args.mapConserve(_.boxedUnlessFun(self))
                  )
                  if followAlias then instantiated.normalized else instantiated
                catch
              ...
              dealiased.resType match
                case AppliedType(tycon, args1) if tycon.safeDealias ne tycon =>
                  ...
                  dealiased
                    .derivedLambdaType(resType =
                      tycon.safeDealias.appliedTo(args1)
                    )
                    .appliedTo(args)
                case _ =>
        ...
        case dealiased: AndType =>
          dealiased.derivedAndType(
            dealiased.tp1.appliedTo(args),
            dealiased.tp2.appliedTo(args)
          )
        case dealiased: OrType =>
          dealiased.derivedOrType(
            dealiased.tp1.appliedTo(args),
            dealiased.tp2.appliedTo(args)
          )
        ...
        case dealiased: TypeBounds =>
          dealiased.derivedTypeBounds(
            dealiased.lo.appliedTo(args),
            dealiased.hi.appliedTo(args)
          )
        case dealiased: LazyRef =>
          LazyRef.of(dealiased.ref.appliedTo(args))
        case dealiased: WildcardType =>
          WildcardType(
            dealiased.optBounds.orElse(TypeBounds.empty).appliedTo(args).bounds
          )
  ...
  final def appliedTo(arg1: Type, arg2: Type)(using Context): Type = appliedTo(
    arg1 :: arg2 :: Nil
  )

  final def applyIfParameterized(args: List[Type])(using Context): Type =
    if typeParams.nonEmpty then appliedTo(args) else self

  /** A cycle-safe version of `appliedTo` where computing type parameters do not
    * force the typeconstructor. Instead, if the type constructor is completing,
    * we make up hk type parameters matching the arguments. This is needed when
    * unpickling Scala2 files such as `scala.collection.generic.Mapfactory`.
    */
  ...
  /** Turns non-bounds types to type bounds. A (possible lambda abstracted)
    * match type is turned into a match alias. Every other type is turned into a
    * type alias
    */
  final def toBounds(using Context): TypeBounds = self match
    case self: TypeBounds => self // this can happen for wildcard args
    case _ => if self.isMatch then MatchAlias(self) else TypeAlias(self)

  /** Translate a type of the form From[T] to either To[T] or To[? <: T] (if
    * `wildcardArg` is set). Keep other types as they are. `from` and `to` must
    * be static classes, both with one type parameter, and the same variance. Do
    * the same for by name types => From[T] and => To[T]
    */
  def translateParameterized(
      from: ClassSymbol,
      to: ClassSymbol,
      wildcardArg: Boolean = false
  )(using Context): Type = self match
    ...
    case _ =>
      if self.derivesFrom(from) then
        def elemType(tp: Type): Type = tp.widenDealias match
          ...
          case tp: AndType =>
            tp.derivedAndType(elemType(tp.tp1), elemType(tp.tp2))
          case _ =>
            tp.baseType(from).argInfos.headOption.getOrElse(defn.NothingType)
        val arg = elemType(self)
        val arg1 = if wildcardArg then TypeBounds.upper(arg) else arg
        to.typeRef.appliedTo(arg1)
      else self

  /** If this is a repeated parameter `*T`, translate it to either `Seq[T]` or
    * `Array[? <: T]` depending on the value of `toArray`. Additionally, if
    * `translateWildcard` is true, a wildcard type will be translated to `*<?>`.
    * Other types are kept as-is.
    */
  def translateFromRepeated(
      toArray: Boolean,
      translateWildcard: Boolean = false
  )(using Context): Type =
    val seqClass = if toArray then defn.ArrayClass else defn.SeqClass
  ...
  /** Translate `T` to `T & Object` in the situations where an `Array[T]` coming
    * from Java would need to be interpreted as an `Array[T & Object]` to be
    * erased correctly.
    *
  ...
  def translateJavaArrayElementType(using Context): Type =
    ...
    if self.typeSymbol.isAbstractOrParamType && (self.classSymbol eq defn.ObjectClass)
    then
      // The order is important here since Java intersections erase to their first operand
      AndType(defn.ObjectType, self)
    else self

  /** If this is an encoding of a (partially) applied type, return its
    * arguments, otherwise return Nil. Existential types in arguments are
    * returned as TypeBounds instances.
    */
  final def argInfos(using Context): List[Type] = self.stripped match
    case AppliedType(tycon, args) => args.boxedUnlessFun(tycon)
    case _                        => Nil

  /** If this is an encoding of a function type, return its arguments, otherwise
    * return Nil. Handles `ErasedFunction`s and poly functions gracefully.
    */
  ...
  /** Argument types where existential types in arguments are approximated by
    * their lower bound
    */
