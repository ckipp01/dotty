...
/** Methods for adding constraints and solving them.
  ...
  *
  * Constraint code is purely functional: Operations get constraints and produce
  * new ones. Constraint code does not have access to a type-comparer. Anything
  * regarding lubs and glbs has to be done elsewhere.
  *
  * By comparison: Constraint handlers are parts of type comparers and can use
  * their functionality. Constraint handlers update the current constraint as a
  * side effect.
  */
trait ConstraintHandling:
  ...
  /** Potentially a type lambda that is still instantiatable, even though the
    * constraint is generally frozen.
    */
  protected var caseLambda: Type = NoType

  /** If set, align arguments `S1`, `S2`when taking the glb `T1 { X = S1 } & T2
    * { X = S2 }` of a constraint upper bound for some type parameter. Aligning
    * means computing `S1 =:= S2` which may change the current constraint. See
    * note in TypeComparer#distributeAnd.
    */
  protected var homogenizeArgs: Boolean = false

  /** We are currently comparing type lambdas. Used as a flag for optimization:
    * when `false`, no need to do an expensive `pruneLambdaParams`
    */
  protected var comparedTypeLambdas: Set[TypeLambda] = Set.empty

  /** Used for match type reduction: If false, we don't recognize an abstract
    * type to be a subtype type of any of its base classes. This is in place
    * only at the toplevel; it is turned on again when we add parts of the
    * scrutinee to the constraint.
    */
  protected var canWidenAbstract: Boolean = true

  protected var myNecessaryConstraintsOnly = false

  /** When collecting the constraints needed for a particular subtyping judgment
    * to be true, we sometimes need to approximate the constraint set (see
    * `TypeComparer#either` for example).
    *
  ...
  /** If `trustBounds = false` we perform comparisons in a pessimistic way as
    * follows: Given an abstract type `A >: L <: H`, a subtype comparison of any
    * type with `A` will compare against both `L` and `H`. E.g.
    *
    * T <:< A if T <:< L and T <:< H A <:< T if L <:< T and H <:< T
    *
    ...
    * the following example from neg/i8900.scala that @smarter came up with: We
    * have a type variable X with constraints
    *
    * X >: 1, X >: x.M
    *
    ...
    * Then even if we correct levels on instantiation to eliminate the local
    * `x`, it is alreay too late, we'd get `Int & String` as instance, which
    * does not satisfy the original constraint `X >: 1`.
    *
    * But if `trustBounds` is false, we do not conclude the `x.M >: 1` since we
    * compare both bounds and the upper bound `Int & String` is not a supertype
  ...
  def nestingLevel(param: TypeParamRef)(using Context) =
  ...
  /** If `param` is nested deeper than `maxLevel`, try to instantiate it to a
    * fresh type variable of level `maxLevel` and return the new variable. If
    * this isn't possible, throw a TypeError.
    */
  def atLevel(maxLevel: Int, param: TypeParamRef)(using Context): TypeParamRef =
    if levelOK(nestingLevel(param), maxLevel) then return param
  ...
  def nonParamBounds(param: TypeParamRef)(using Context): TypeBounds =
    constraint.nonParamBounds(param)

  /** The full lower bound of `param` includes both the `nonParamBounds` and the
    * params in the constraint known to be `<: param`, except that params with a
    * `nestingLevel` higher than `param` will be instantiated to a fresh param
    * at a legal level. See the documentation of `TypeVar` for details.
    */
  ...
  /** The full upper bound of `param`, see the documentation of
    * `fullLowerBounds` above.
    */
  ...
  /** Full bounds of `param`, including other lower/upper params.
    *
    * Note that underlying operations perform subtype checks - for this reason,
    * recursing on `fullBounds` of some param when comparing types might lead to
    * infinite recursion. Consider `bounds` instead.
    */
  def fullBounds(param: TypeParamRef)(using Context): TypeBounds =
    nonParamBounds(param).derivedTypeBounds(
      fullLowerBound(param),
      fullUpperBound(param)
    )
  ...
  /** An approximating map that prevents types nested deeper than maxLevel as
    ...
    */
  class LevelAvoidMap(topLevelVariance: Int, maxLevel: Int)(using Context)
      extends TypeOps.AvoidMap:
    variance = topLevelVariance

    def toAvoid(tp: NamedType): Boolean =
      tp.prefix == NoPrefix && !tp.symbol.isStatic && !levelOK(
        tp.symbol.nestingLevel,
        maxLevel
      )

    /** Return a (possibly fresh) type variable of a level no greater than
      * `maxLevel` which is:
      *   - lower-bounded by `tp` if variance >= 0
      *   - upper-bounded by `tp` if variance <= 0 If this isn't possible,
    ...
    def legalVar(tp: TypeVar): Type =
      ...
      /** If it exists, return the first param in the list created in a previous
        * call to `legalVar(tp)` with the appropriate level and variance.
        */
      ...
      findParam(constraint.lower(oldParam))
        .orElse(findParam(constraint.upper(oldParam))) match
        case Some(param) =>
        ...
        case _ =>
          ...
          val freshVar = newTypeVar(
            TypeBounds.upper(tp.topType),
            name,
            nestingLevel = maxLevel,
            represents = oldParam
          )
          val ok =
            if variance < 0 then addLess(freshVar.origin, oldParam)
            else if variance > 0 then addLess(oldParam, freshVar.origin)
            else unify(freshVar.origin, oldParam)
          if ok then freshVar else emptyRange
    end legalVar

    override def apply(tp: Type): Type = tp match
      case tp: TypeVar
          if !tp.isInstantiated && !levelOK(tp.nestingLevel, maxLevel) =>
        legalVar(tp)
    ...
    override def mapWild(t: WildcardType) =
      if ctx.mode.is(Mode.TypevarsMissContext) then super.mapWild(t)
        ...
        val tvar =
          newTypeVar(apply(t.effectiveBounds).toBounds, nestingLevel = maxLevel)
        tvar
  end LevelAvoidMap

  /** Approximate `rawBound` if needed to make it a legal bound of `param` by
    * avoiding cycles, wildcards and types with a level strictly greater than
    * its `nestingLevel`.
    *
  ...
  protected def legalBound(
    ...
    val approx = new LevelAvoidMap(variance, nestingLevel(param)):
      override def legalVar(tp: TypeVar): Type =
        ...
        val v =
          if necessaryConstraintsOnly then -this.variance else this.variance
        atVariance(v)(super.legalVar(tp))
    constraint.validBoundFor(param, approx(rawBound), isUpper)
  end legalBound

  protected def addOneBound(
      param: TypeParamRef,
      rawBound: Type,
      isUpper: Boolean
  ...
  protected def addBoundTransitively(
      param: TypeParamRef,
      rawBound: Type,
      isUpper: Boolean
  )(using Context): Boolean =
    ...
    /** Adjust the bound `tp` in the following ways:
      *
      *   1. Toplevel occurrences of TypeRefs that are instantiated in the
      *      current constraint are also dereferenced. 2. Toplevel occurrences
      *      of ExprTypes lead to a `NoType` return, which causes the
      *      addOneBound operation to fail.
      *
    ...
    def description =
      i"constraint $param ${if isUpper then "<:" else ":>"} $rawBound to\n$constraint"
    constr.println(i"adding $description$location")
    if isUpper && rawBound.isRef(
        defn.NothingClass
    ...
    def others =
      if isUpper then constraint.lower(param) else constraint.upper(param)
    val bound = adjust(rawBound)
    bound.exists
    && addOneBound(param, bound, isUpper) && others
      .forall(addOneBound(_, bound, isUpper))
      .showing(i"added $description = $result$location", constr)
  end addBoundTransitively

  protected def addLess(p1: TypeParamRef, p2: TypeParamRef)(using
      Context
  ): Boolean =
    ...
    val res =
      if constraint.isLess(p2, p1) then unify(p2, p1)
        ...
        constr.println(
          i"adding $description down1 = $down1, up2 = $up2$location"
        )
  ...
  def location(using Context) =
    "" // i"in ${ctx.typerState.stateChainStr}" // use for debugging

  /** Unify p1 with p2: one parameter will be kept in the constraint, the other
    * will be removed and its bounds transferred to the remaining one.
    *
    ...
    */
  private def unify(p1: TypeParamRef, p2: TypeParamRef)(using
      Context
  ): Boolean =
    constr.println(s"unifying $p1 $p2")
    if !constraint.isLess(p1, p2) then constraint = constraint.addLess(p1, p2)
    ...
    constraint = constraint.addLess(
      p2,
      p1,
      direction = if pKept eq p1 then KeepParam2 else KeepParam1
    )

    val boundKept = constraint.validBoundsFor(
      pKept,
      constraint.nonParamBounds(pKept).substParam(pRemoved, pKept).bounds
    )
    var boundRemoved = constraint.validBoundsFor(
      pKept,
    ...
    if level1 != level2 then
      ...
      if !isSub(lo, hi) then boundRemoved = TypeBounds(lo & hi, hi)

    val newBounds = (boundKept & boundRemoved).bounds
    constraint =
      constraint.updateEntry(pKept, newBounds).replace(pRemoved, pKept)
  ...
  protected def isSubType(tp1: Type, tp2: Type, whenFrozen: Boolean)(using
      Context
  ): Boolean =
    if whenFrozen then isSubTypeWhenFrozen(tp1, tp2)
    else isSub(tp1, tp2)
  ...
  /** Test whether the lower bounds of all parameters in this constraint are a
  ...
  /** Fix instance type `tp` by avoidance so that it does not contain references
    ...
    * @param fromBelow
    *   whether type was obtained from lower bound
    * @param maxLevel
    *   the maximum level of references allowed
    * @param param
    *   the parameter that was instantiated
    */
  private def fixLevels(
      tp: Type,
      fromBelow: Boolean,
      maxLevel: Int,
      param: TypeParamRef
    ...
    /** An accumulator that determines whether levels need to be fixed and
      * computes on the side sets of nested type variables that need to be
      * instantiated.
      */
        ...
        need || tp.match
          ...
          case tp: TypeVar =>
            ...
            if inst.exists then apply(need, inst)
              ...
              // of `tp` would fail.
              constr.println(
                i"widening nesting level of type variable $tp from ${tp.nestingLevel} to $maxLevel"
              )
    ...
    if Config.checkLevelsOnInstantiation && !ctx.isAfterTyper && needsLeveling(
        false,
        tp
      )
    then
  ...
  /** Solve constraint set for given type parameter `param`. If `fromBelow` is
    * true the parameter is approximated by its lower bound, otherwise it is
    * approximated by its upper bound, unless the upper bound contains a
    * reference to the parameter itself (such occurrences can arise for
    * F-bounded types, `addOneBound` ensures that they never occur in the lower
    * bound). The solved type is not allowed to contain references to types
    * nested deeper than `maxLevel`. Wildcard types in bounds are approximated
    * by their upper or lower bounds. The constraint is left unchanged.
    * @return
    *   the instantiating type
    * @pre
    *   `param` is in the constraint's domain.
    */
  final def approximation(
      param: TypeParamRef,
      fromBelow: Boolean,
      maxLevel: Int
  )(using Context): Type =
    constraint.entry(param) match
      case entry: TypeBounds =>
        ...
        typr.println(
          i"approx $param, from below = $fromBelow, inst = $levelInst"
        )
        levelInst
      case inst =>
  ...
  private def isTransparent(tp: Type, traitOnly: Boolean)(using
      Context
  ): Boolean = tp match
  ...
  /** If `tp` is an intersection such that some operands are transparent trait
    * instances and others are not, replace as many transparent trait instances
    * as possible with Any as long as the result is still a subtype of `bound`.
    * But fall back to the original type if the resulting widened type is a
    * supertype of all dropped types (since in this case the type was not a true
    * intersection of transparent traits and other types to start with).
    */
  def dropTransparentTraits(tp: Type, bound: Type)(using Context): Type =
    var kept: Set[Type] =
      Set() // types to keep since otherwise bound would not fit
    var dropped: List[Type] =
      List() // the types dropped so far, last one on top
  ...
  /** If `tp` is an applied match type alias which is also an unreducible
    * application of a higher-kinded type to a wildcard argument, widen to the
    * match type's bound, in order to avoid an unreducible application of
    * higher-kinded type ... in inferred type" error in PostTyper. Fixes #11246.
    */
  ...
  /** Widen inferred type `inst` with upper `bound`, according to the following
    * rules:
    *   1. If `inst` is a singleton type, or a union containing some singleton
    *      types, widen (all) the singleton type(s), provided the result is a
    *      subtype of `bound` (i.e. `inst.widenSingletons <:< bound` succeeds
    *      with satisfiable constraint) and is not transparent according to
    *      `isTransparent`. 2a. If `inst` is a union type and `widenUnions` is
    *      true, approximate the union type from above by an intersection of all
    *      common base types, provided the result is a subtype of `bound`. 2b.
    *      If `inst` is a union type and `widenUnions` is false, turn it into a
    *      hard union type (except for unions | Null, which are kept in the
    *      state they were). 3. Widen some irreducible applications of
    *      higher-kinded types to wildcard arguments (see @widenIrreducible). 4.
    *      Drop transparent traits from intersections (see
    *      \@dropTransparentTraits).
    *
    * Don't do these widenings if `bound` is a subtype of `scala.Singleton`.
    * Also, if the result of these widenings is a TypeRef to a module class, and
    * this type ref is different from `inst`, replace by a TermRef to its source
    * module instead.
    *
    * At this point we also drop the @Repeated annotation to avoid inferring
    * type arguments with it, as those could leak the annotation to users (see
    * run/inferred-repeated-result).
    */
  def widenInferred(inst: Type, bound: Type, widenUnions: Boolean)(using
    ...
    def isSingleton(tp: Type): Boolean = tp match
      case WildcardType(optBounds) =>
        optBounds.exists && isSingleton(optBounds.bounds.hi)
  ...
  extension (tp: Type)
    private def hardenUnions(using Context): Type = tp.widen match
      case tp: AndType =>
        tp.derivedAndType(tp.tp1.hardenUnions, tp.tp2.hardenUnions)
      case tp: RefinedType =>
        tp.derivedRefinedType(
          tp.parent.hardenUnions,
          tp.refinedName,
          tp.refinedInfo
        )
      case tp: RecType =>
  ...
  /** The instance type of `param` in the current constraint (which contains
    * `param`). If `fromBelow` is true, the instance type is the lub of the
    * parameter's lower bounds; otherwise it is the glb of its upper bounds.
    * However, a lower bound instantiation can be a singleton type only if the
    * upper bound is also a singleton type. The instance type is not allowed to
    * contain references to types nested deeper than `maxLevel`.
    */
  def instanceType(
      param: TypeParamRef,
      fromBelow: Boolean,
  ...
  /** Constraint `c1` subsumes constraint `c2`, if under `c2` as constraint we
    * have for all poly params `p` defined in `c2` as `p >: L2 <: U2`:
    *
    * c1 defines p with bounds p >: L1 <: U1, and L2 <: L1, and U1 <: U2
    *
    * Both `c1` and `c2` are required to derive from constraint `pre`, without
    * adding any new type variables but possibly narrowing already registered
    * ones with further bounds.
    */
  protected final def subsumes(c1: Constraint, c2: Constraint, pre: Constraint)(
      using Context
  ): Boolean =
  ...
  def bounds(param: TypeParamRef)(using Context): TypeBounds =
    constraint.bounds(param)

  /** Add type lambda `tl`, possibly with type variables `tvars`, to current
    * constraint and propagate all bounds.
    * @param tvars
    *   See Constraint#add
    */
  ...
  final def canConstrain(param: TypeParamRef): Boolean =
    (!frozenConstraint || (caseLambda `eq` param.binder)) && constraint
      .contains(param)

  /** Is `param` assumed to be a sub- and super-type of any other type? This
    * holds if `TypeVarsMissContext` is set unless `param` is a part of a
    * MatchType that is currently normalized.
    */
  ...
  /** Add constraint `param <: bound` if `fromBelow` is false, `param >: bound`
    * otherwise. `bound` is assumed to be in normalized form, as specified in
    * `firstTry` and `secondTry` of `TypeComparer`. In particular, it should not
    * be an alias type, lazy ref, typevar, wildcard type, error type. In
    * addition, upper bounds may not be AndTypes and lower bounds may not be
    * OrTypes. This is assured by the way isSubType is organized.
    */
  protected def addConstraint(
    ...
    /** When comparing lambdas we might get constraints such as `A <: X0` or `A
      * \= List[X0]` where `A` is a constrained parameter and `X0` is a lambda
      * parameter. The constraint for `A` is not allowed to refer to such a
      * lambda parameter because the lambda parameter is not visible where `A`
      * is defined. Consequently, we need to approximate the bound so that the
      * lambda parameter does not appear in it. If `tp` is an upper bound, we
      * need to approximate with something smaller, otherwise something larger.
      * Test case in pos/i94-nada.scala. This test crashes with an illegal
      * instance error in Test2 when the rest of the SI-2712 fix is applied but
      * `pruneLambdaParams` is missing.
      */
      ...
      if comparedTypeLambdas.nonEmpty then
        val approx = new ApproximatingTypeMap:
          ...
          def apply(t: Type): Type = t match
            case t @ TypeParamRef(tl: TypeLambda, n)
                if comparedTypeLambdas contains tl =>
    ...
    def addParamBound(bound: TypeParamRef) =
      constraint.entry(param) match
        case _: TypeBounds =>
          if fromBelow then addLess(bound, param) else addLess(param, bound)
        case tp =>
          if fromBelow then isSub(bound, tp) else isSub(tp, bound)

    def kindCompatible(tp1: Type, tp2: Type): Boolean =
      val tparams1 = tp1.typeParams
      val tparams2 = tp2.typeParams
      tparams1.corresponds(tparams2)((p1, p2) =>
        kindCompatible(p1.paramInfo, p2.paramInfo)
      )
    ...
    def description =
      i"constr $param ${if fromBelow then ">:" else "<:"} $bound:\n$constraint"
      ...
      try
        bound match
          ...
          case _ =>
            ...
            kindCompatible(param, pbound) && addBoundTransitively(
              param,
              pbound,
              !fromBelow
            )
  ...
  /** Check that constraint is fully propagated. See comment in
    * Config.checkConstraintsPropagated
    */
  def checkPropagated(msg: => String)(result: Boolean)(using Context): Boolean =
    if Config.checkConstraintsPropagated && result && addConstraintInvocations == 0
    then
      inFrozenConstraint {
        for p <- constraint.domainParams do
          def check(
              cond: => Boolean,
              q: TypeParamRef,
              ordering: String,
            ...
            assert(
              cond,
              i"propagation failure for $p $ordering $q: $explanation\n$msg"
            )
          for u <- constraint.upper(p) do
            check(
              bounds(p).hi <:< bounds(u).hi,
              u,
              "<:",
              "upper bound not propagated"
            )
          for l <- constraint.lower(p) do
            check(
              bounds(l).lo <:< bounds(p).hi,
              l,
              ">:",
              "lower bound not propagated"
            )
            check(
              constraint.isLess(l, p),
              l,
              ">:",
              "reverse ordering (<:) missing"
            )
      }
    end if
    result
  end checkPropagated
end ConstraintHandling
