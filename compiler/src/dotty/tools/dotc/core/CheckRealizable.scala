...
object CheckRealizable:
  ...
  sealed abstract class Realizability(val msg: String):
    def andAlso(other: => Realizability): Realizability =
      if this == Realizable then other else this
    def mapError(f: Realizability => Realizability): Realizability =
      if this == Realizable then this else f(this)
  ...
  class HasProblemBounds(name: Name, info: Type)(using Context)
      extends Realizability(
        i" has a member $name with possibly conflicting bounds ${info.bounds.lo} <: ... <: ${info.bounds.hi}"
      )

  class HasProblemBaseArg(typ: Type, argBounds: TypeBounds)(using Context)
      extends Realizability(
        i" has a base type $typ with possibly conflicting parameter bounds ${argBounds.lo} <: ... <: ${argBounds.hi}"
      )
  ...
  class HasProblemField(fld: SingleDenotation, problem: Realizability)(using
      Context
  ) extends Realizability(
        i" has a member $fld which is not a legal path\nsince ${fld.symbol.name}: ${fld.info}${problem.msg}"
      )
...
/** Compute realizability status.
  *
  * A type T is realizable iff it is inhabited by non-null values. This ensures
  * that its type members have good bounds (in the sense from DOT papers). A
  * type projection T#L is legal if T is realizable, and can be understood as
  * Scala 2's `v.L forSome { val v: T }`.
  *
  * In general, a realizable type can have multiple inhabitants, hence it need
  * not be stable (in the sense of Type.isStable).
  */
class CheckRealizable(using Context):
  import CheckRealizable.*

  /** A set of all fields that have already been checked. Used to avoid infinite
    * recursions when analyzing recursive types.
    */
  private val checkedFields: mutable.Set[Symbol] =
    mutable.LinkedHashSet[Symbol]()

  /** Is symbol's definitition a lazy or erased val? (note we exclude modules
    * here, because their realizability is ensured separately)
    */
  private def isLateInitialized(sym: Symbol) =
    sym.isOneOf(LateInitializedFlags, butNot = Module)
  ...
  def realizability(tp: Type): Realizability = tp.dealias match
      ...
      if sym.is(StableRealizable) then realizability(tp.prefix)
        ...
        val r =
          if sym.isStableMember && !isLateInitialized(sym) then
            ...
            Realizable
          else if !sym.isEffectivelyFinal then
        ...
        r andAlso {
          if sym.isStableMember then
            sym.setFlag(
              StableRealizable
            ) // it's known to be stable and realizable
          realizability(tp.prefix)
        } mapError { r =>
          // A mutable path is in fact stable and realizable if it has a realizable singleton type.
          if tp.info.isStable && tpInfoRealizable == Realizable then
    ...
    case tp =>
      def isConcrete(tp: Type): Boolean = tp.dealias match
        ...
        case _                => false
      if !isConcrete(tp) then NotConcrete
  ...
  /** `Realizable` if `tp` has good bounds, a `HasProblem...` instance pointing
    * to a bad bounds member otherwise. "Has good bounds" means:
    *
    *   - all type members have good bounds (except for opaque helpers)
    *   - all refinements of the underlying type have good bounds (except for
    *     opaque companions)
    *   - all base types are class types, and if their arguments are wildcards
    *     they have good bounds.
    *   - base types do not appear in multiple instances with different
    *     arguments. (depending on the simplification scheme for AndTypes
    *     employed, this could also lead to base types with bad bounds).
    */
  private def boundsRealizability(tp: Type) =
    ...
    baseProblems.foldLeft(
      refinementProblems.foldLeft(
        memberProblems.foldLeft(Realizable: Realizability)(_ andAlso _)
      )(_ andAlso _)
    )(_ andAlso _)
  end boundsRealizability

  /** `Realizable` if all of `tp`'s non-strict fields have realizable types, a
  ...
  private def memberRealizability(tp: Type) =
    def checkField(sofar: Realizability, fld: SingleDenotation): Realizability =
      sofar andAlso {
        if checkedFields.contains(fld.symbol) || fld.symbol.isOneOf(
            Private | Mutable | LateInitializedFlags
          )
        then
