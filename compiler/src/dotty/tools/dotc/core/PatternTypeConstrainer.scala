...
trait PatternTypeConstrainer:
  ...
  def constrainPatternType(
  ...
  ): Boolean = trace(i"constrainPatternType($scrut, $pat)", gadts) {
    ...
    def classesMayBeCompatible: Boolean =
      ...
      !patCls.exists || !scrCls.exists || {
        if patCls.is(Final) then patCls.derivesFrom(scrCls)
        else if scrCls.is(Final) then scrCls.derivesFrom(patCls)
        else if !patCls.is(Flags.Trait) && !scrCls.is(Flags.Trait) then
    ...
    def constrainUpcasted(scrut: Type): Boolean =
      trace(i"constrainUpcasted($scrut)", gadts) {
        // Fold a list of types into an AndType
        def buildAndType(xs: List[Type]): Type =
          @annotation.tailrec
          def recur(acc: Type, rem: List[Type]): Type = rem match
            case Nil      => acc
        ...
        scrut match
          ...
          case scrut @ AppliedType(tycon: TypeRef, _) if tycon.symbol.isClass =>
            ...
            def allParentsSharedWithPat(
                tp: Type,
                tpClassSym: ClassSymbol
            ): List[Symbol] =
              var parents = tpClassSym.info.parents
              if parents.nonEmpty && parents.head.classSymbol == defn.ObjectClass
              then parents = parents.tail
              parents flatMap { tp =>
            ...
            def trySuperType =
              val upcasted: Type = scrut match
                case scrut: TypeProxy =>
                  scrut.superType
                case _ => NoType
              if upcasted.exists then
                tryConstrainSimplePatternType(
                  pat,
                  upcasted
    ...
    dealiasDropNonmoduleRefs(scrut) match
      case OrType(scrut1, scrut2) =>
        either(
          constrainPatternType(pat, scrut1),
          constrainPatternType(pat, scrut2)
        )
      ...
      case scrut =>
        dealiasDropNonmoduleRefs(pat) match
          case OrType(pat1, pat2) =>
            either(
              constrainPatternType(pat1, scrut),
              constrainPatternType(pat2, scrut)
            )
          case AndType(pat1, pat2) =>
            constrainPatternType(pat1, scrut) && constrainPatternType(
              pat2,
              scrut
            )
  ...
  /** Constrain "simple" patterns (see `constrainPatternType`).
    ...
    * constraints for the applied types' arguments that follow from both types
    * being inhabited by one value (the scrutinee).
    *
    * Importantly, note that the pattern type may contain type variables, which
    * are used to infer type arguments to Unapply trees.
    *
    * ## Invariant refinement Essentially, we say that `D[B] extends C[B]` s.t.
    * refines parameter `A` of `trait C[A]` invariantly if when `c: C[T]` and
    * `c` is instance of `D`, then necessarily `c: D[T]`. This is violated if
    * `A` is variant:
    *
    * trait C[+A] trait D[+B](val b: B) extends C[B] trait E extends D[Any](0)
    * with C[String]
    *
    * `E` is a counter-example to the above - if `e: E`, then `e: C[String]` and
    * `e` is instance of `D`, but it is false that `e: D[String]`! This is a
    * problem if we're constraining a pattern like the below:
    *
    * def foo[T](c: C[T]): T = c match { case d: D[t] => d.b }
    *
    * It'd be unsound for us to say that `t <: T`, even though that follows from
    * `D[t] <: C[T]`. Note, however, that if `D` was a final class, we *could*
    * rely on that relationship. To support typical case classes, we also assume
    * that this relationship holds for them and their parent traits. This is
    * enforced by checking that classes inheriting from case classes do not
    * extend the parent traits of those case classes without also appropriately
    * extending the relevant case class (see
    * `RefChecks#checkCaseClassInheritanceInvariant`).
    */
  def constrainSimplePatternType(
      patternTp: Type,
      scrutineeTp: Type,
      forceInvariantRefinement: Boolean
  ): Boolean =
    ...
    def widenVariantParams(tp: Type) = tp match
      case tp @ AppliedType(tycon, args) =>
        val args1 = args.zipWithConserve(tycon.typeParams)((arg, tparam) =>
          if tparam.paramVarianceSign != 0 then TypeBounds.empty else arg
        )
    ...
    val pt =
      if upcastPattern then patternTp.baseType(scrutineeCls) else patternTp
    val tp =
      if !upcastPattern then scrutineeTp.baseType(patternCls) else scrutineeTp

    val assumeInvariantRefinement =
      migrateTo3 || forceInvariantRefinement || refinementIsInvariant(patternTp)

    trace(
      i"constraining simple pattern type $tp >:< $pt",
      gadts,
      (res: Boolean) => i"$res gadt = ${ctx.gadt}"
    ) {
      (tp, pt) match
        case (AppliedType(tyconS, argsS), AppliedType(tyconP, argsP)) =>
          ...
          if !result then constraint = saved
          result
        case _ =>
  ...
  end constrainSimplePatternType
end PatternTypeConstrainer
