...
object MatchTypeTrace:
  ...
  /** Execute `op` and if it involves a failed match type reduction return the
    * trace of that reduction. Otherwise return the empty string.
    */
  ...
  /** Record a failure that scrutinee `scrut` does not match any case in
    * `cases`. Only the first failure is recorded.
    */
  ...
  /** Record a failure that scrutinee `scrut` does not match `stuckCase` but is
    * not disjoint from it either, which means that the remaining cases
    * `otherCases` cannot be visited. Only the first failure is recorded.
    */
  def stuck(scrut: Type, stuckCase: Type, otherCases: List[Type])(using
      Context
  ) =
    matchTypeFail(Stuck(scrut, stuckCase, otherCases))

  def noInstance(scrut: Type, stuckCase: Type, fails: List[(Name, TypeBounds)])(
      using Context
  ) =
    matchTypeFail(NoInstance(scrut, stuckCase, fails))

  /** Record a failure that scrutinee `scrut` is provably empty. Only the first
    * failure is recorded.
    */
  def emptyScrutinee(scrut: Type)(using Context) =
    matchTypeFail(EmptyScrutinee(scrut))

  /** Record in the trace that we are trying to reduce `scrut` when performing
    * `op` If `op` succeeds the entry is removed after exit. If `op` fails, it
    * stays.
    */
  ...
  private def explainEntry(entry: TraceEntry)(using Context): String =
    entry match
      case TryReduce(scrut: Type) =>
