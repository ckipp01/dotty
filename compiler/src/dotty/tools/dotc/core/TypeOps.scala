...
object TypeOps:
  ...
  /** The type `tp` as seen from prefix `pre` and owner `cls`. See the spec for
    * what this means.
    */
  final def asSeenFrom(tp: Type, pre: Type, cls: Symbol)(using Context): Type =
    pre match
      case pre: QualSkolemType =>
        ...
        if widenedAsf.approxCount == 0 then return ret
  ...
  class AsSeenFromMap(pre: Type, cls: Symbol)(using Context)
      extends ApproximatingTypeMap,
        IdempotentCaptRefMap:

    /** The number of range approximations in invariant or contravariant
      * positions performed by this TypeMap.
      *   - Incremented each time we produce a range.
      *   - Decremented each time we drop a prefix range by forwarding to a type
      *     alias or singleton type.
      */
    private[TypeOps] var approxCount: Int = 0

    def apply(tp: Type): Type =

      /** Map a `C.this` type to the right prefix. If the prefix is unstable,
        * and the current variance is <= 0, return a range.
        * @param pre
        *   The prefix
        * @param cls
        *   The class in which the `C.this` type occurs
        * @param thiscls
        *   The prefix `C` of the `C.this` type.
        */
      def toPrefix(pre: Type, cls: Symbol, thiscls: ClassSymbol): Type =
        /*>|>*/ trace.conditionally(
          track,
          s"toPrefix($pre, $cls, $thiscls)",
          show = true
        ) /*<|<*/ {
            ...
            pre match
              ...
              case _ =>
                if thiscls.derivesFrom(cls) && pre.baseType(thiscls).exists then
                  if variance <= 0 && !isLegalPrefix(pre) then
                    approxCount += 1
                  ...
                  toPrefix(
                    pre.baseType(cls).normalizedPrefix,
                    cls.owner,
                    thiscls
                  )
        }
      ...
      trace.conditionally(
        track,
        s"asSeen ${tp.show} from (${pre.show}, ${cls.show})",
        show = true
      ) { // !!! DEBUG
  ...
  def simplify(tp: Type, theMap: SimplifyMap | Null)(using Context): Type =
    def mapOver =
      (if theMap != null then theMap else new SimplifyMap).mapOver(tp)
    tp match
      case tp: NamedType =>
        if tp.symbol.isStatic || (tp.prefix `eq` NoPrefix) then tp
        else
          tp.derivedSelect(simplify(tp.prefix, theMap)) match
      ...
      case _: MatchType =>
        val normed = tp.tryNormalize
        if normed.exists then normed else mapOver
  ...
  /** Approximate union type by intersection of its dominators. That is, replace
    * a union type Tn | ... | Tn by the smallest intersection type of accessible
    * base-class instances of T1,...,Tn. Example: Given
    *
    * trait C[+T] trait D class A extends C[A] with D class B extends C[B] with
    * D with E
    *
    * we approximate `A | B` by `C[A | B] with D`.
    *
    * Before we do that, we try to find a common non-class supertype of T1 | ...
    * \| Tn in a "best effort", ad-hoc way by selectively widening types in `T1,
    * ..., Tn` and stopping if the resulting union simplifies to a type that is
    * not a disjunction.
    */
  def orDominator(tp: Type)(using Context): Type =
    ...
    /** The minimal set of classes in `cs` which derive all other classes in
      * `cs`
      */
    def dominators(
        cs: List[ClassSymbol],
        accu: List[ClassSymbol]
    ): List[ClassSymbol] = (cs: @unchecked) match
      case c :: rest =>
        val accu1 = if accu exists (_ derivesFrom c) then accu else c :: accu
        if cs == c.baseClasses then accu1 else dominators(rest, accu1)
    ...
    def mergeRefinedOrApplied(tp1: Type, tp2: Type): Type =
      def fail = throw new AssertionError(
        i"Failure to join alternatives $tp1 and $tp2"
      )
          ...
          tp2 match
            case RefinedType(parent2, `name1`, rinfo2) =>
              tp1.derivedRefinedType(
                mergeRefinedOrApplied(parent1, parent2),
                name1,
                rinfo1 | rinfo2
              )
            case _ => fallback
    ...
    def approximateOr(tp1: Type, tp2: Type): Type =
      ...
      val tp2w = tp2 match
        ...
        case _                                  => tp2
      if (tp1w ne tp1) || (tp2w ne tp2) then
        val isSingle1 = tp1.isInstanceOf[SingletonType]
        val isSingle2 = tp2.isInstanceOf[SingletonType]
        return if tp2w eq tp2 then orDominator(tp1w | tp2) // 2.1
        else if tp1w eq tp1 then orDominator(tp1 | tp2w) // 2.1
        else if tp1w frozen_<:< tp2w then orDominator(tp1w | tp2) // 2.2
        else if tp2w frozen_<:< tp1w then orDominator(tp1 | tp2w) // 2.2
        else if isSingle1 && !isSingle2 then orDominator(tp1w | tp2) // 2.3
        else if isSingle2 && !isSingle1 then orDominator(tp1 | tp2w) // 2.3
        else if tp1 frozen_<:< tp2w then tp2w // 2.4
        else if tp2 frozen_<:< tp1w then tp1w // 2.5
    ...
    tp match
      case tp: OrType =>
        (tp.tp1.dealias, tp.tp2.dealias) match
          case (
                tp1 @ AppliedType(tycon1, args1),
                tp2 @ AppliedType(tycon2, args2)
              )
  ...
  /** An abstraction of a class info, consisting of
    ...
    *   - where all references to `this` of the class are closed over in a
    *     RecType.
    */
  def classBound(info: ClassInfo)(using Context): Type =
    ...
    val (refinableDecls, missingDecls) =
      info.decls.toList.partition(isRefinable)
    ...
    def addRefinement(parent: Type, decl: Symbol) =
      ...
      val isPolyFunctionApply =
        decl.name == nme.apply && (parent <:< defn.PolyFunctionType)
      val needsRefinement =
        isPolyFunctionApply
          || {
            ...
            else parent.derivesFrom(defn.SelectableClass)
          }
  ...
  /** An approximating map that drops NamedTypes matching `toAvoid` and wildcard
    * types.
    */
  abstract class AvoidMap(using Context)
      extends AvoidWildcardsMap,
        IdempotentCaptRefMap:
    ...
    override def apply(tp: Type): Type =
      try
        ...
        case ex: Throwable =>
          handleRecursive(
            "traversing for avoiding local references",
            s"${tp.show}",
            ex
          )
    end apply
    ...
    /** Three deviations from standard derivedSelect:
      *   1. We first try a widening conversion to the type's info with the
      *      original prefix. Since the original prefix is known to be a subtype
      *      of the returned prefix, this can improve results. 2. Then, if the
      *      approximation result is a singleton reference C#x.type, we replace
      *      by the widened type, which is usually more natural. 3. Finally, we
      *      need to handle the case where the prefix type does not have a
      *      member named `tp.name` anymmore. In that case, we need to fall back
      *      to Bot..Top.
      */
    override def derivedSelect(tp: NamedType, pre: Type) =
      if pre eq tp.prefix then tp
      else
        (if pre.isSingleton then NoType else tryWiden(tp, tp.prefix)).orElse {
          if tp.isTerm && variance > 0 && !pre.isSingleton then
            apply(tp.info.widenExpr)
          else if upper(pre).member(tp.name).exists then
            super.derivedSelect(tp, pre)
          else range(defn.NothingType, defn.AnyType)
        }
  end AvoidMap

  /** An upper approximation of the given type `tp` that does not refer to any
    * symbol in `symsToAvoid` and does not contain any WildcardType. We need to
    * approximate with ranges:
    *
    * term references to symbols in `symsToAvoid`, term references that have a
    * widened type of which some part refers to a symbol in `symsToAvoid`, type
    * references to symbols in `symsToAvoid`,
    *
    * Type variables that would be interpolated to a type that needs to be
    * widened are replaced by the widened interpolation instance.
    *
  ...
  def avoid(tp: Type, symsToAvoid: => List[Symbol])(using Context): Type =
    val widenMap = new AvoidMap:
      ...
      /** We need to split the set into upper and lower approximations only if
        * it contains a local element. The idea here is that at the time we
        * perform an `avoid` all local elements are already accounted for and no
        * further elements will be added afterwards. So we can just keep the set
        * as it is. See comment by @linyxus on #16261.
        */
  ...
  /** If `tpe` is of the form `p.x` where `p` refers to a package but `x` is not
    * owned by a package, expand it to
    *
    * p.package.x
    */
  def makePackageObjPrefixExplicit(tpe: NamedType)(using Context): Type =
    def tryInsert(pkgClass: SymDenotation): Type = pkgClass match
      ...
      case _ =>
        tpe
    if tpe.symbol.isRoot then tpe
    else
      tpe.prefix match
        case pre: ThisType if pre.cls.is(Package) => tryInsert(pre.cls)
        case pre: TermRef if pre.symbol.is(Package) =>
          tryInsert(pre.symbol.moduleClass)
        case _ => tpe
  ...
  /** The list of violations where arguments are not within bounds.
    ...
    * @param instantiate
    *   A function that maps a bound type and the list of argument types to a
    *   resulting type. Needed to handle bounds that refer to other bounds.
    * @param app
    *   The applied type whose arguments are checked, or NoType if arguments are
    *   for a TypeApply.
    *
    * This is particularly difficult for F-bounds that also contain wildcard
    * arguments (see below). In fact the current treatment for this sitiuation
    * can so far only be classified as "not obviously wrong", (maybe it still
    * needs to be revised).
    */
  def boundsViolations(
    ...
    withMode(Mode.CheckBoundsOrSelfType) {
      ...
      /** Replace all wildcards in `tps` with `<app>#<tparam>` where `<tparam>`
        * is the type parameter corresponding to the wildcard.
        */
      def skolemizeWildcardArgs(tps: List[Type], app: Type) = app match
        case AppliedType(tycon: TypeRef, args)
          ...
          tps.zipWithConserve(tycon.typeSymbol.typeParams) { (tp, tparam) =>
            tp match
      ...
      def checkOverlapsBounds(
        ...
        if argTypes ne skolemizedArgTypes
        then // some of the arguments are wildcards

          /** Is there a `LazyRef(TypeRef(_, sym))` reference in `tp`? */
          def isLazyIn(sym: Symbol, tp: Type): Boolean =
            def isReference(tp: Type) = tp match
              case tp: LazyRef =>
                tp.ref.isInstanceOf[TypeRef] && tp.ref.typeSymbol == sym
              case _ => false
            tp.existsPart(isReference, forceLazy = false)

          /** The argument types of the form `TypeRef(_, sym)` which appear as a
            * LazyRef in `bounds`. This indicates that the application is used
            * as an F-bound for the symbol referred to in the LazyRef.
            */
          ...
          for sym <- lazyRefs do
            ...
            def narrowBound(bound: Type, fromBelow: Boolean): Unit =
              ...
              if bound1 ne bound then
                if checkCtx eq ctx then checkCtx = ctx.fresh.setFreshGADTBounds
                if !checkCtx.gadt.contains(sym) then
                  checkCtx.gadtState.addToConstraint(sym)
                checkCtx.gadtState.addBound(sym, bound1, fromBelow)
                typr.println(
                  "install GADT bound $bound1 for when checking F-bounded $sym"
                )
        ...
        def check(using Context) =
          if !(lo <:< hiBound) then violations += ((arg, "upper", hiBound))
          if !(loBound <:< hi) then violations += ((arg, "lower", loBound))
        check(using checkCtx)
      end checkOverlapsBounds
        ...
        case arg :: args1 =>
          boundss match
            case bounds :: boundss1 =>
              arg.tpe match
                case TypeBounds(lo, hi) =>
                  checkOverlapsBounds(lo, hi, arg, bounds)
  ...
  /** Refine child based on parent
    ...
    *
    * class Child[Ts] extends path.Parent[Us] with Es object Child extends
    * path.Parent[Us] with Es val child = new path.Parent[Us] with Es // enum
    * values
    *
    * Given a parent type `parent` and a child symbol `child`, we infer the
    * prefix and type parameters for the child:
    *
    * prefix.child[Vs] <:< parent
    *
    * where `Vs` are fresh type variables and `prefix` is the symbol prefix with
    * all non-module and non-package `ThisType` replaced by fresh type
    * variables.
    *
    * If the subtyping is true, the instantiated type `p.child[Vs]` is returned.
    * Otherwise, `NoType` is returned.
    */
  def refineUsingParent(parent: Type, child: Symbol, mixins: List[Type] = Nil)(
      using Context
  ): Type =
    ...
    //
    if child.name == tpnme.LOCAL_CHILD then return child.typeRef

    val childTp = if child.isTerm then child.termRef else child.typeRef

    inContext(
      ctx.fresh
        .setExploreTyperState()
        .setFreshGADTBounds
        .addMode(Mode.GadtConstraintInference)
    ) {
      instantiateToSubType(childTp, parent, mixins).dealias
    }
  end refineUsingParent

  /** Instantiate type `tp1` to be a subtype of `tp2`
    *
    * Return the instantiated type if type parameters in this type in `tp1` can
    * be instantiated such that `tp1 <:< tp2`.
    *
    * Otherwise, return NoType.
    */
  private def instantiateToSubType(
      tp1: NamedType,
      tp2: Type,
      mixins: List[Type]
  )(using Context): Type =
    trace(i"instantiateToSubType($tp1, $tp2, $mixins)", typr) {
        ...
        def apply(tp: Type): Type = tp.dealias match
          case tp: MatchType =>
            ...
            if reduced.exists then tp // break cycles
              ...
              mapOver(tp.bound) // if the match type doesn't statically reduce
          ...
          case tp @ AppliedType(tycon: TypeRef, _)
              if !tycon.dealias.typeSymbol.isClass && !tp.isMatchAlias =>
      ...
      object TraverseTp2 extends TypeTraverser:
        ...
        def traverse(tp: Type) =
          ...
          if tpd ne tp then traverse(tpd)
            ...
            tp match
              case tp: ThisType
                  if !tp.tref.symbol.isStaticOwner && !thisTypes.contains(tp) =>
                thisTypes += tp
              ...
              case tp: TypeRef if tp.symbol.isAbstractOrParamType =>
                ...
                traverseChildren(tp)
                val owners =
                  Iterator.iterate(tp.symbol)(_.maybeOwner).takeWhile(_.exists)
                for sym <- owners do
                  ...
                  if sym.isClass && !sym.isAnonymousClass && !sym.isStaticOwner
                  then traverse(sym.thisType)
              case _ =>
                traverseChildren(tp)
        ...
        def apply(tp: Type): Type = tp match
          case tp @ ThisType(tref) if !tref.symbol.isStaticOwner =>
            ...
            if thisTypes.contains(tp) then
              ...
              val tvars = tref.typeParams.map { tparam =>
                newTypeVar(
                  tparam.paramInfo.bounds,
                  DepParamName.fresh(tparam.paramName)
                )
              }
              val tref2 = this(tref.applyIfParameterized(tvars))
              prefixTVar = newTypeVar(
                TypeBounds.upper(tref2),
      ...
      val tvars = tp1.typeParams.map { tparam =>
        newTypeVar(
          tparam.paramInfo.bounds,
          DepParamName.fresh(tparam.paramName)
        )
      ...
      if gadtSyms.nonEmpty then ctx.gadtState.addToConstraint(gadtSyms)
      ...
      def parentQualify(tp1: Type, tp2: Type) =
        tp1.classSymbol.info.parents.exists { parent =>
  ...
  def nestedPairs(ts: List[Type])(using Context): Type =
    ts.foldRight(defn.EmptyTupleModule.termRef: Type)(
      defn.PairClass.typeRef.appliedTo(_, _)
    )
  ...
  /** computes a prefix for `child`, derived from its common prefix with `pre`
    ...
    *   - `child` is assumed to be the sealed child of `parent`, and reachable
    *     according to `whyNotGenericSum`.
    */
  def childPrefix(pre: Type, parent: Symbol, child: Symbol)(using
      Context
  ): Type =
    ...
    /** unwind the prefix into a sequence of sub-prefixes, selecting the one at
      * `limit`
      * @return
      *   `NoType` if there is an unrecognised prefix type.
      */
      ...
      def go(pre: Type, limit: Int): Type =
        if limit == 0 then pre // EXIT: No More prefix
          ...
          pre match
            ...
            case _: SuperType | NoPrefix =>
              pre.ensuring(limit == 1) // EXIT: can't rewind further than this
            case _ => NoType // EXIT: unrecognized prefix
      go(pre, limit)
    end subPrefixAt

    /** Successively select each symbol in the `suffix` from `pre`, such that
      * they are reachable.
      */
    ...
    def stripCommonPrefix(
        xs: List[Symbol],
        ys: List[Symbol]
    ): (List[Symbol], List[Symbol]) = (xs, ys) match
    ...
    val commonPrefix = subPrefixAt(
      pre,
      parentRest.size
    ) // unwind parent owners up to common prefix
