...
object Constants:
  ...
  class Constant(val value: Any, val tag: Int)
    ...
    def convertTo(pt: Type)(using Context): Constant | Null =
        ...
        case param: TypeParamRef =>
          ctx.typerState.constraint.entry(param) match
            case TypeBounds(lo, hi) =>
              if hi.classSymbol.isPrimitiveValueClass then
                hi // constrain further with high bound
              else classBound(lo)
            case NoType =>
              classBound(param.binder.paramInfos(param.paramNum).lo)
      ...
      pt match
        ...
        case _ =>
          ...
          else if (target == defn.ByteClass) && isByteRange then
            Constant(byteValue)
          else if target == defn.ShortClass && isShortRange then
            Constant(shortValue)
          else if target == defn.CharClass && isCharRange then
            Constant(charValue)
          else if target == defn.IntClass && isIntRange then Constant(intValue)
          else if target == defn.LongClass && isLongRange then
            Constant(longValue)
          else if target == defn.FloatClass && isFloatRange then
            Constant(floatValue)
          else if target == defn.DoubleClass && isNumeric then
    ...
    /** Consider two `NaN`s to be identical, despite non-equality Consider -0d
      * to be distinct from 0d, despite equality
      *
      * We use the raw versions (i.e. `floatToRawIntBits` rather than
      * `floatToIntBits`) to avoid treating different encodings of `NaN` as the
      * same constant. You probably can't express different `NaN` varieties as
      * compile time constants in regular Scala code, but it is conceivable that
      * you could conjure them with a macro.
      */
