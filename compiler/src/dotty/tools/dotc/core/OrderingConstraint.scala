...
object OrderingConstraint:
  ...
  /** If true, use reverse dependencies in `replace` to avoid checking the
    * bounds of all parameters in the constraint. This can speed things up, but
    * there are some rare corner cases where reverse dependencies miss a
    * parameter. Specifically, if a constraint contains a free reference to
    * TypeParam P and afterwards the same P is added as a bound variable to the
    * constraint, a backwards link would then become necessary at this point but
    * is missing. This causes two CB projects to fail when reverse dependencies
    * are checked (parboiled2 and perspective). In these rare cases `replace`
    * could behave differently when optimized. However, no deviation was found
    * in the two projects. It is not clear what the "right" behavior of
    * `replace` should be in these cases. Normally, PolyTypes added to
    * constraints are supposed to be fresh, so that would mean that the behavior
    * with optimizeReplace = true would be correct. But the previous behavior
    * without reverse dependency checking corresponds to `optimizeReplace =
    * false`. This behavior makes sense if we assume that the added polytype was
    * simply added too late, so we want to establish the link between newly
    * bound variable and pre-existing reference.
    */
  ...
  /** A lens for updating a single entry array in one of the three constraint
    * maps
    */
  private abstract class ConstraintLens[T <: AnyRef: ClassTag]:
    ...
    def updateEntries(
        ...
        poly: TypeLambda,
        entries: Array[T]
    )(using Context): OrderingConstraint
    def initial: T

    def apply(c: OrderingConstraint, poly: TypeLambda, idx: Int): T =
      val es = entries(c, poly)
      if es == null then initial else es(idx)

    /** The `current` constraint but with the entry for `param` updated to
      * `entry`. `current` is used linearly. If it is different from `prev` then
      * `current` is known to be dead after the call. Hence it is OK to update
      * destructively parts of `current` which are not shared by `prev`.
      */
    def update(
        prev: OrderingConstraint,
  ...
  private val lowerLens: ConstraintLens[List[TypeParamRef]] =
    new ConstraintLens[List[TypeParamRef]]:
      def entries(
          c: OrderingConstraint,
          poly: TypeLambda
      ): Array[List[TypeParamRef]] | Null =
        c.lowerMap(poly)
      def updateEntries(
          c: OrderingConstraint,
          poly: TypeLambda,
          entries: Array[List[TypeParamRef]]
      )(using Context): OrderingConstraint =
        c.newConstraint(lowerMap = c.lowerMap.updated(poly, entries))
      def initial = Nil

  private val upperLens: ConstraintLens[List[TypeParamRef]] =
    new ConstraintLens[List[TypeParamRef]]:
      def entries(
          c: OrderingConstraint,
          poly: TypeLambda
  ...
  @sharable
  val empty = new OrderingConstraint(
    SimpleIdentityMap.empty,
    SimpleIdentityMap.empty,
    SimpleIdentityMap.empty,
    SimpleIdentitySet.empty
  )
end OrderingConstraint
...
/** Constraint over undetermined type parameters that keeps separate maps to
  ...
  * @param boundsMap
  *   a map from TypeLambda to arrays. Each array contains twice the number of
  *   entries as there a type parameters in the TypeLambda. The first half of
  *   the array contains the type bounds that constrain the lambda's type
  *   parameters. The second half might contain type variables that track the
  *   corresponding parameters, or is left empty (filled with nulls). An
  *   instantiated type parameter is represented by having its instance type in
  *   the corresponding array entry. The dual use of arrays for poly params and
  *   typevars is to save space and hopefully gain some speed.
  *
  * @param lowerMap
  *   a map from TypeLambdas to arrays. Each array entry corresponds to a
  *   parameter P of the type lambda; it contains all constrained parameters Q
  *   that are known to be smaller than P, i.e. Q <: P.
  * @param upperMap
  *   a map from TypeLambdas to arrays. Each array entry corresponds to a
  *   parameter P of the type lambda; it contains all constrained parameters Q
  *   that are known to be greater than P, i.e. P <: Q.
  * @param hardVars
  *   a set of type variables that are marked as hard and therefore will not
...
class OrderingConstraint(
  ...
  private def newConstraint(
    ...
    if boundsMap.isEmpty && lowerMap.isEmpty && upperMap.isEmpty then empty
    else
      val result =
        new OrderingConstraint(boundsMap, lowerMap, upperMap, hardVars)
      if ctx.run != null then
...
// ----------- Basic indices --------------------------------------------------
  ...
  /** The type variable corresponding to parameter numbered `n`, null if none
    * was created
    */
...
// ----------- Contains tests --------------------------------------------------
  ...
  def contains(tvar: TypeVar): Boolean =
    ...
    entries != null && isBounds(entries(pnum)) && (typeVar(
      entries,
      pnum
    ) eq tvar)
...
// ------------- Type parameter dependencies ----------------------------------------
  ...
  private type ReverseDeps =
    SimpleIdentityMap[TypeParamRef, SimpleIdentitySet[TypeParamRef]]

  /** A map that associates type parameters of this constraint with all other
    * type parameters that refer to them in their bounds covariantly, such that,
    * if the type parameter is instantiated to a larger type, the constraint
    * would be narrowed (i.e. solution set changes other than simply being made
    * larger).
    */
  ...
  /** A map that associates type parameters of this constraint with all other
    * type parameters that refer to them in their bounds covariantly, such that,
    * if the type parameter is instantiated to a smaller type, the constraint
    * would be narrowed. (i.e. solution set changes other than simply being made
    * larger).
    */
  ...
  /** Null-safe indexing */
  extension (deps: ReverseDeps)
    def at(param: TypeParamRef): SimpleIdentitySet[TypeParamRef] =
  ...
  override def dependsOn(tv: TypeVar, except: TypeVars, co: Boolean)(using
      Context
  ): Boolean =
  ...
  /** Modify traversals in two respects:
    *   - when encountering an application C[Ts], where C is a type variable or
    *     parameter that has an instantiation in this constraint, assume the
    *     type parameters of the instantiation instead of the type parameters of
    *     C when traversing the arguments Ts. That can make a difference for the
    *     variance in which an argument is traversed. Example constraint:
    *
    * constrained types: C[X], A A >: C[B] C := Option
    *
    * Here, B is traversed with variance +1 instead of 0. Test case:
    * pos/t3152.scala
    *
    *   - When typing a prefx, don't avoid negative variances. This matters only
    *     for the corner case where a parameter is instantiated to Nothing (see
    *     comment in TypeAccumulator#applyToPrefix). When determining
    *     instantiation directions in interpolations (which is what dependency
    *     variances are for), it can be ignored.
    */
  private trait ConstraintAwareTraversal[T] extends TypeAccumulator[T]:

    /** Does `param` have bounds in the current constraint? */
    protected def hasBounds(param: TypeParamRef): Boolean =
      entry(param).isInstanceOf[TypeBounds]

    override def tyconTypeParams(tp: AppliedType)(using
        Context
    ): List[ParamInfo] =
  ...
  /** A type traverser that adjust dependencies originating from a given type
    * @param ignoreBinding
    *   if not null, a parameter that is assumed to be still uninstantiated.
    *   This is necessary to handle replacements.
    */
  private class Adjuster(
      srcParam: TypeParamRef,
      ignoreBinding: TypeParamRef | Null
  )(using Context)
      extends TypeTraverser,
        ConstraintAwareTraversal[Unit]:
  ...
  def adjustDeps(
      ...
      prevEntry: Type | Null,
      srcParam: TypeParamRef,
      ignoreBinding: TypeParamRef | Null = null
  )(using Context): this.type =
    ...
    /** Adjust reverse dependencies of all type parameters referenced by `bound`
      ...
      * @param add
      *   if true, add referenced variables to dependencoes, otherwise drop
      *   them.
      */
    ...
    /** Use an optimized strategy to adjust dependencies to account for the
      * delta of previous bound `prevBound` and new bound `bound`: If
      * `prevBound` is some and/or prefix of `bound`, and `baseCase` is true,
      * just add the new parts of `bound`.
      * @param isLower
      *   `bound` and `prevBound` are lower bounds
      * @return
      *   true iff the delta strategy succeeded, false if it failed in which
      *   case the constraint is left unchanged.
      */
    def adjustDelta(
        bound: Type,
        prevBound: Type,
      ...
      if bound eq prevBound then baseCase
      else
        bound match
    ...
    /** Add or remove depenencies referenced in `bounds`.
      * @param add
      *   if true, dependecies are added, otherwise they are removed
      */
    ...
    entry match
      ...
      case _ =>
        ...
        dropDeps(
          srcParam
        ) // srcParam is instantiated, so its dependencies can be dropped
    this
  end adjustDeps

  /** Adjust dependencies to account for adding or dropping all `entries`
    * associated with `poly`.
    * @param add
    *   if true, entries is added, otherwise it is dropped
    */
  def adjustDeps(poly: TypeLambda, entries: Array[Type], add: Boolean)(using
      Context
  ): this.type =
  ...
  def depsToString(using Context): String =
    def depsStr(deps: ReverseDeps): String =
      def depStr(param: TypeParamRef) =
        i"$param --> ${deps.at(param).toList}%, %"
...
// ---------- Adding TypeLambdas --------------------------------------------------
  ...
  /** The bound type `tp` without constrained parameters which are clearly
    * dependent. A parameter in an upper bound is clearly dependent if it
    * appears in a hole of a context H given by:
    *
    * H = [] H & T T & H
    *
    * (the idea is that a parameter P in a H context is guaranteed to be a
    * supertype of the bounded parameter.) Analogously, a parameter in a lower
    * bound is clearly dependent if it appears in a hole of a context H given
    * by:
    *
    * L = [] L | T T | L
    *
    * "Clearly dependent" is not synonymous with "dependent" in the sense it is
    * defined in `dependentParams`. Dependent parameters are handled in
    * `updateEntry`. The idea of stripping off clearly dependent parameters and
    * to handle them separately is for efficiency, so that type expressions used
    * as bounds become smaller.
    *
    * TODO: try to do without stripping? It would mean it is more efficient to
    * pull out full bounds from a constraint.
    *
    * @param isUpper
    *   If true, `bound` is an upper bound, else a lower bound.
    */
  private def stripParams(
      tp: Type,
      todos: mutable.ListBuffer[
        (OrderingConstraint, TypeParamRef) => OrderingConstraint
      ],
      isUpper: Boolean
  )(using Context): Type = tp match
  ...
  /** Split dependent parameters off the bounds for parameters in `poly`. Update
    * all bounds to be normalized and update ordering to account for dependent
    * parameters.
    */
  private def init(poly: TypeLambda)(using Context): This =
    var current = this
    val todos = new mutable.ListBuffer[
      (OrderingConstraint, TypeParamRef) => OrderingConstraint
    ]
    var i = 0
    val dropWildcards = AvoidWildcardsMap()
    while i < poly.paramNames.length do
      ...
      i += 1
    current
      .adjustDeps(poly, current.boundsMap(poly).nn, add = true)
      .checkWellFormed()

// ---------- Updates ------------------------------------------------------------

  def validBoundFor(param: TypeParamRef, bound: Type, isUpper: Boolean)(using
      Context
  ): Type =
    def recur(tp: Type): Type = tp match
      ...
      case tp: TypeParamRef =>
        if tp eq param then if isUpper then defn.AnyType else defn.NothingType
        else
          entry(tp) match
            case NoType             => tp
  ...
  def validBoundsFor(param: TypeParamRef, bounds: TypeBounds)(using
      Context
  ): Type =
  ...
  def order(
      ...
      param1: TypeParamRef,
      param2: TypeParamRef,
      direction: UnificationDirection = NoUnification
  )(using Context): This =
    ...
    if param1 == param2 || current.isLess(param1, param2) then current
      ...
      val newUpper =
        ...
        if unifying then
          ...
          if direction == KeepParam2 then param2 :: filtered
          else filtered
        else param2 :: up
      val newLower =
        ...
        if unifying then
          ...
          if direction == KeepParam1 then param1 :: filtered
          else filtered
        else param1 :: lower
      val current1 =
        newLower.foldLeft(current)(upperLens.map(this, _, _, newUpper ::: _))
      val current2 =
        newUpper.foldLeft(current1)(lowerLens.map(this, _, _, newLower ::: _))
      current2
    end if
  end order
  ...
  /** The list of parameters P such that, for a fresh type parameter Q:
    ...
    * isUpper = false
    */
  private def dependentParams(tp: Type, isUpper: Boolean)(using
      Context
  ): List[TypeParamRef] = tp match
    case param: TypeParamRef if contains(param) =>
      param :: (if isUpper then upper(param) else lower(param))
    case tp: AndType if isUpper =>
      dependentParams(tp.tp1, isUpper).setUnion(
        dependentParams(tp.tp2, isUpper)
      )
    case tp: OrType if !isUpper =>
      dependentParams(tp.tp1, isUpper).intersect(
        dependentParams(tp.tp2, isUpper)
      )
  ...
  private def updateEntry(current: This, param: TypeParamRef, newEntry: Type)(
      using Context
  ): This =
    if Config.checkNoWildcardsInConstraint then
      assert(!newEntry.containsWildcardTypes)
    val oldEntry = current.entry(param)
    var current1 = boundsLens
      .update(this, current, param, newEntry)
...
// ---------- Replacements and Removals -------------------------------------
  ...
  /** A new constraint which is derived from this constraint by removing the
    * type parameter `param` from the domain and replacing all top-level
    * occurrences of the parameter elsewhere in the constraint by type `tp`.
    */
  def replace(param: TypeParamRef, tp: Type)(using
      Context
  ): OrderingConstraint =
    ...
    if param == replacement then this.checkWellFormed()
      ...
      if optimizeReplace then
        current.foreachParam { (p, i) =>
          ...
          entry(other) match
            case _: TypeBounds =>
              if coDepsOfParam.contains(other) || contraDepsOfParam.contains(
                  other
                )
              then replaceParamIn(other)
  ...
  def remove(pt: TypeLambda)(using Context): This =
    def removeFromOrdering(po: ParamOrdering) =
      def removeFromBoundss(
          key: TypeLambda,
          bndss: Array[List[TypeParamRef]]
      ): Array[List[TypeParamRef]] =
        val bndss1 = bndss.map(_.filterConserve(_.binder ne pt))
        if bndss.corresponds(bndss1)(_ eq _) then bndss else bndss1
      po.remove(pt).mapValuesNow(removeFromBoundss)
    val hardVars1 = pt.paramRefs.foldLeft(hardVars)((hvs, param) =>
      hvs - typeVarOfParam(param)
    )
    newConstraint(
      boundsMap.remove(pt),
      removeFromOrdering(lowerMap),
      removeFromOrdering(upperMap),
      hardVars1
    )
...
// ----------- Joins -----------------------------------------------------
  ...
  def hasConflictingTypeVarsFor(tl: TypeLambda, that: Constraint): Boolean =
    contains(tl) && that.contains(tl) &&
      ...
      (this.typeVarOfParam(tl.paramRefs(0)) ne that.typeVarOfParam(
        tl.paramRefs(0)
      ))

  def subst(from: TypeLambda, to: TypeLambda)(using
      Context
  ): OrderingConstraint =
    ...
    var current =
      newConstraint(swapKey(boundsMap), swapKey(lowerMap), swapKey(upperMap))
  ...
  def ensureFresh(tl: TypeLambda)(using Context): TypeLambda =
    if contains(tl) then
      ...
      if tl.isInstanceOf[HKLambda] then
  ...
  def occursAtToplevel(param: TypeParamRef, inst: Type)(using
      Context
  ): Boolean =
...
// ---------- Exploration --------------------------------------------------------
  ...
  def forallParams(p: TypeParamRef => Boolean): Boolean =
    boundsMap.forallBinding { (poly, entries) =>
      !0.until(paramCount(entries))
        .exists(i => isBounds(entries(i)) && !p(poly.paramRefs(i)))
    }
  ...
  def uninstVars: collection.Seq[TypeVar] =
    ...
    then
      ...
      boundsMap.foreachBinding { (poly, entries) =>
        for i <- 0 until paramCount(entries) do
          typeVar(entries, i) match
            case tv: TypeVar if !tv.inst.exists && isBounds(entries(i)) =>
              myUninstVars.uncheckedNN += tv
            case _ =>
      }
...
// ---------- Checking -----------------------------------------------
  ...
  def checkWellFormed()(using Context): this.type =
    ...
    /** Check that each dependency A -> B in coDeps and contraDeps corresponds
      * to a reference to A at the right variance in the entry of B.
      */
    def checkBackward(deps: ReverseDeps, depsName: String, v: Int)(using
        Context
    ): Unit =
      deps.foreachBinding { (param, params) =>
        for srcParam <- params do
          assert(
            contains(srcParam) && occursAtVariance(
              param,
              v,
              in = entry(srcParam)
            ),
      ...
      new TypeTraverser with ConstraintAwareTraversal[Unit]:
        ...
        def traverse(t: Type): Unit = t match
          case param: TypeParamRef if param ne srcParam =>
            def check(
                deps: ReverseDeps,
                directDeps: List[TypeParamRef],
                depsName: String
            ) =
              assert(
                deps.at(param).contains(srcParam) || directDeps.contains(
                  srcParam
                ),
    ...
    /** Does `param` occur at variance `v` or else at variance 0 in entry `in`?
      */
    def occursAtVariance(param: TypeParamRef, v: Int, in: Type)(using
        Context
    ): Boolean =
      val test = new TypeAccumulator[Boolean]
        with ConstraintAwareTraversal[Boolean]:
    ...
    if Config.checkConstraintsNonCyclic then
      domainParams.foreach { param =>
        ...
        assert(
          !occursAtToplevel(param, inst),
          s"cyclic bound for $param: ${inst.show} in ${this.show}"
        )
      }
    if Config.checkConstraintDeps || ctx.settings.YcheckConstraintDeps.value
    then
      checkBackward(coDeps, "co", -1)
      checkBackward(contraDeps, "contra", +1)
      domainParams.foreach(p =>
        if contains(p) then checkForward(p).traverse(entry(p))
      )
  ...
  override def checkClosed()(using Context): Unit =
    ...
    def checkClosedType(tp: Type | Null, where: String) =
      if tp != null then
        assert(
          !tp.existsPart(isFreeTypeParamRef),
          i"unclosed constraint: $this refers to $tp in $where"
        )

    boundsMap.foreachBinding((_, tps) =>
      tps.foreach(checkClosedType(_, "bounds"))
    )
    lowerMap.foreachBinding((_, paramss) =>
      paramss.foreach(_.foreach(checkClosedType(_, "lower")))
    )
    upperMap.foreachBinding((_, paramss) =>
      paramss.foreach(_.foreach(checkClosedType(_, "upper")))
    )
  end checkClosed
      ...
      "\n bounds = " + {
        val assocs =
          for (param <- domainParams)
            yield s"${param.binder.paramNames(param.paramNum)}: ${entryText(entry(param))}"
        assocs.mkString("\n")
      }
    val depsText =
      "\n coDeps = " + coDeps +
