...
object Flags:
  ...
  object opaques:
    ...
    /** A FlagSet represents a set of flags. Flags are encoded as follows: The
      * first two bits indicate whether a flag set applies to terms, to types,
  ...
  extension (x: FlagSet)
    ...
    /** The union of the given flag sets. Combining two FlagSets with `|` will
      * give a FlagSet that has the intersection of the applicability to
      * terms/types of the two flag sets. It is checked that the intersection is
      * not empty.
      */
    ...
    def &~(y: FlagSet): FlagSet =
      ...
      if (tbits & y.bits) == 0 then x
      else FlagSet(tbits | ((x.bits & ~y.bits) & ~KINDFLAGS))

    def ^(y: FlagSet) =
      FlagSet((x.bits | y.bits) & KINDFLAGS | (x.bits ^ y.bits) & ~KINDFLAGS)

    /** Does the given flag set contain the given flag? This means that both the
      * kind flags and the carrier bits have non-empty intersection.
      */
    def is(flag: Flag): Boolean =
      val fs = x.bits & flag.bits
      (fs & KINDFLAGS) != 0 && (fs & ~KINDFLAGS) != 0

    /** Does the given flag set contain the given flag and at the same time
      * contain none of the flags in the `butNot` set?
      */
    def is(flag: Flag, butNot: FlagSet): Boolean =
      x.is(flag) && !x.isOneOf(butNot)

    /** Does the given flag set have a non-empty intersection with another flag
      * set? This means that both the kind flags and the carrier bits have
    ...
    /** Does the given flag set have a non-empty intersection with another flag
      * set, and at the same time contain none of the flags in the `butNot` set?
      */
    def isOneOf(flags: FlagSet, butNot: FlagSet): Boolean =
      x.isOneOf(flags) && !x.isOneOf(butNot)

    /** Does a given flag set have all of the flags of another flag set? Pre:
      * The intersection of the term/type flags of both sets must be non-empty.
      */
    ...
    /** Does a given flag set have all of the flags in another flag set and at
      * the same time contain none of the flags in the `butNot` set? Pre: The
      * intersection of the term/type flags of both sets must be non-empty.
      */
    def isAllOf(flags: FlagSet, butNot: FlagSet): Boolean =
      x.isAllOf(flags) && !x.isOneOf(butNot)
    ...
    def firstBit: Int =
      java.lang.Long.numberOfTrailingZeros(x.bits & ~KINDFLAGS)

    /** The list of non-empty names of flags with given index idx that are set
    ...
    def flagStrings(privateWithin: String = ""): Seq[String] =
      var rawStrings = (2 to MaxFlag).flatMap(
        x.flagString(_)
      ) // DOTTY problem: cannot drop with (_)
      if !privateWithin.isEmpty && !x.is(Protected) then
        rawStrings = rawStrings :+ "private"
      val scopeStr = if x.is(Local) then "this" else privateWithin
      if scopeStr != "" then
  ...
  /** The flag set containing all defined flags of either kind whose bits lie in
    * the given range
    */
  ...
  def union(flagss: FlagSet*): FlagSet =
    var flag = EmptyFlags
    for f <- flagss do flag |= f
    flag

  def commonFlags(flagss: FlagSet*): FlagSet = union(
    flagss.map(_.toCommonFlags)*
  )
  ...
  /** Three flags with given index between 2 and 63. The first applies to both
    * terms and types. the second is a term flag, and the third is a type flag.
    * Installs given name(s) as the name(s) of the flags.
    * @param name
    *   The name to be used for the term flag
    * @param typeName
    *   The name to be used for the type flag, if it is different from `name`.
    */
  private def newFlags(
      index: Int,
      name: String,
      typeName: String = ""
  ): (Flag, Flag, Flag) =
    flagName(index)(TERMindex) = name
    flagName(index)(TYPEindex) = if typeName.isEmpty then name else typeName
    val bits = 1L << index
    (
  ...
  val (Deferred @ _, DeferredTerm @ _, DeferredType @ _) =
    newFlags(5, "<deferred>")
  ...
  val (AccessorOrSealed @ _, Accessor @ _, Sealed @ _) =
    newFlags(11, "<accessor>", "sealed")

  /** A mutable var, an open class */
  val (MutableOrOpen @ __, Mutable @ _, Open @ _) =
    newFlags(12, "mutable", "open")
  ...
  /** A field generated for a primary constructor parameter (no matter if it's a
    * 'val' or not), or an accessor of such a field.
    */
  ...
  val (Package @ _, PackageVal @ _, PackageClass @ _) =
    newFlags(16, "<package>")

  /** A case class or its companion object Note: Case is also used to indicate
    * that a symbol is bound by a pattern.
    */
  val (Case @ _, CaseVal @ _, CaseClass @ _) = newFlags(17, "case")

  /** A compiler-generated symbol, which is visible for type-checking (compare
    * with artifact)
    */
  ...
  val (OuterOrCovariant @ _, OuterAccessor @ _, Covariant @ _) =
    newFlags(20, "<outer accessor>", "<covariant>")

  /** The label of a labeled block / a contravariant type variable */
  val (LabelOrContravariant @ _, Label @ _, Contravariant @ _) =
    newFlags(21, "<label>", "<contravariant>")

  /** Labeled with of abstract & override / A trait that has only abstract
    * methods as members and therefore can be represented by a Java interface.
    * Warning: PureInterface is set during regular typer pass, should be tested
    * only after typer.
    */
  val (_, AbsOverride @ _, PureInterface @ _) =
    newFlags(22, "abstract override", "interface")

  /** Labeled with `abstract` modifier (an abstract class) Note: You should
    * never see Abstract on any symbol except a class. Note: the flag counts as
    * common, because it can be combined with OVERRIDE in a term.
    */
  ...
  /** Lazy val or method is known or assumed to be stable and realizable.
    ...
    *
    *   - the purity analysis used by the inliner to decide whether it is safe
    *     to elide, and
    *   - the TASTy reader of Scala 2.13, to determine whether there is a $init$
    *     method.
    *
    * StableRealizable is
  ...
  val (SuperParamAliasOrScala2x @ _, SuperParamAlias @ _, Scala2x @ _) =
    newFlags(26, "<super-param-alias>", "<scala-2.x>")
  ...
  val (JavaStatic @ _, JavaStaticTerm @ _, JavaStaticType @ _) =
    newFlags(31, "<static>")

  /** Variable is accessed from nested function / Trait does not have own fields
    * or initialization code or class does not have own or inherited
    * initialization code.
    *
    * Warning: NoInits is set during regular typer pass, should be tested only
    * after typer.
    */
  val (_, Captured @ _, NoInits @ _) = newFlags(32, "<captured>", "<noinits>")

  /** Symbol should be ignored when typechecking; will be marked ACC_SYNTHETIC
    * in bytecode
    */
  ...
  val (_, JavaVarargs @ _, JavaAnnotation @ _) =
    newFlags(37, "<varargs>", "<java-annotation>")
  ...
  /** Denotation is in train of being loaded and completed, used to catch cyclic
    * dependencies
    */
  val (Touched @ _, _, _) = newFlags(50, "<touched>")

  /** Class has been lifted out to package level, local value has been lifted
    * out to class level
    */
  val (Lifted @ _, _, _) = newFlags(
    51,
    "<lifted>"
  ) // only used from lambda-lift (could be merged with ConstructorProxy)
  ...
  /** A Scala 2 superaccessor (only needed during Scala2Unpickling) / an
    * existentially bound symbol (Scala 2.x only)
    */
  val (Scala2SpecialFlags @ _, Scala2SuperAccessor @ _, Scala2Existential @ _) =
    newFlags(55, "<existential>")

  /** Children were queried on this class */
  val (_, _, ChildrenQueried @ _) = newFlags(56, "<children-queried>")

  /** A module variable (Scala 2.x only) (re-used as a flag for private
    * parameter accessors in Recheck)
    */
  ...
  /** Translation of Scala2's EXPANDEDNAME flag. This flag is never stored in
    ...
    * It's therefore safe to share the code with `HasDefaultParams` because the
    * latter is never present in Scala2 unpickle info. / A method that is known
    * to have (defined or inherited) default parameters
    */
  val (Scala2ExpandedName @ _, HasDefaultParams @ _, _) =
    newFlags(59, "<has-default-params>")

  /** A method that is known to have no default parameters / A type symbol with
    * provisional empty bounds
    */
  val (_, NoDefaultParams @ _, Provisional @ _) =
    newFlags(60, "<no-default-params>", "<provisional>")
  ...
  val (ConstructorProxy @ _, _, _) =
    newFlags(62, "<constructor proxy>") // (could be merged with Lifted)
...
// --------- Combined Flag Sets and Conjunctions ----------------------
  ...
  /** Flags that are not (re)set when completing the denotation TODO: Should
    * check that FromStartFlags do not change in completion
    */
  ...
  /** Flags that are not (re)set when completing the denotation, or, if symbol
    * is a top-level class or object, when completing the denotation once the
    * class file defining the symbol is loaded (which is generally before the
    * denotation is completed)
    */
...
// ----- Creation flag sets ----------------------------------
  ...
  val ModuleValCreationFlags: FlagSet =
    ModuleVal | Lazy | Final | StableRealizable
...
// ----- Retained flag sets ----------------------------------
  ...
  /** Flags that are passed from a type parameter of a class to a refinement
    * symbol that sets the type parameter
    */
  val RetainedTypeArgFlags: FlagSet = VarianceFlags | Protected | Local

  /** Flags that can apply to both a module val and a module class, except those
    * that are added at creation anyway
    */
...
// ------- Other flag sets -------------------------------------
  ...
  val JavaOrPrivateOrSynthetic: FlagSet =
    Artifact | JavaDefined | Private | Synthetic
  ...
  val ContravariantLocal: FlagSet =
    Contravariant | Local // A contravariant type parameter
  val EffectivelyErased = ConstructorProxy | Erased
  val ConstructorProxyModule: FlagSet = ConstructorProxy | Module
  val DefaultParameter: FlagSet =
    HasDefault | Param // A Scala 2x default parameter
  ...
  val DeferredOrTermParamOrAccessor: FlagSet =
    Deferred | ParamAccessor | TermParam // term symbols without right-hand sides
  val DeferredOrTypeParam: FlagSet =
    Deferred | TypeParam // type symbols without right-hand sides
  val EnumValue: FlagSet = Enum | StableRealizable // A Scala enum value
  val FinalOrInline: FlagSet = Final | Inline
  val FinalOrModuleClass: FlagSet =
    Final | ModuleClass // A module class or a final class
  val EffectivelyFinalFlags: FlagSet = Final | Private
  val ExcludedForwarder: Flags.FlagSet =
    Specialized | Lifted | Protected | JavaStatic | Private | Macro | ConstructorProxy
  ...
  val InlineOrProxy: FlagSet =
    Inline | InlineProxy // An inline method or inline argument proxy */
  ...
  val StaticProtected: FlagSet =
    JavaDefined | JavaStatic | Protected // Java symbol which is `protected` and `static`
  ...
  val ParamForwarder: FlagSet =
    Method | ParamAccessor | StableRealizable // A parameter forwarder
  ...
  val NoInitsTrait: FlagSet =
    NoInits | Trait // A trait that does not need to be initialized
