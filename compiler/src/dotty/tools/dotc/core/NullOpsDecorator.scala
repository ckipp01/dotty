...
object NullOpsDecorator:
  ...
  extension (self: Type)
    /** Syntactically strips the nullability from this type. If the type is `T1
      * \| ... | Tn`, and `Ti` references to `Null`, then return `T1 | ... |
      * Ti-1 | Ti+1 | ... | Tn`. If this type isn't (syntactically) nullable,
      * then returns the type unchanged. The type will not be changed if
      * explicit-nulls is not enabled.
      */
      ...
      def strip(tp: Type): Type =
        ...
        val tpStripped = tpWiden match
          ...
          case tp @ AndType(tp1, tp2) =>
            ...
            val tp1s = strip(tp1)
            val tp2s = strip(tp2)
            if (tp1s ne tp1) && (tp2s ne tp2) then tp.derivedAndType(tp1s, tp2s)
            else tp
