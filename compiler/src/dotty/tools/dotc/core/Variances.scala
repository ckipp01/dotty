...
object Variances:
  ...
  def flip(v: Variance): Variance =
    if v == Covariant then Contravariant
    else if v == Contravariant then Covariant
    else v
  ...
  /** Does variance `v1` conform to variance `v2`? This is the case if the
    * variances are the same or `sym` is nonvariant.
    */
  def varianceConforms(v1: Int, v2: Int): Boolean =
    v1 == v2 || v2 == 0

  /** Does the variance of type parameter `tparam1` conform to the variance of
    * type parameter `tparam2`?
    */
  def varianceConforms(tparam1: TypeParamInfo, tparam2: TypeParamInfo)(using
      Context
  ): Boolean =
    tparam1.paramVariance.isAllOf(tparam2.paramVariance)

  /** Do the variances of type parameters `tparams1` conform to the variances of
    * corresponding type parameters `tparams2`? This is only the case if
    * `tparams1` and `tparams2` have the same length.
    */
  def variancesConform(
      tparams1: List[TypeParamInfo],
      tparams2: List[TypeParamInfo]
