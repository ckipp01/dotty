...
object Contexts:
  ...
  private val (compilerCallbackLoc, store1) =
    Store.empty.newLocation[CompilerCallback]()
  private val (sbtCallbackLoc, store2) = store1.newLocation[AnalysisCallback]()
  private val (printerFnLoc, store3) =
    store2.newLocation[Context => Printer](new RefinedPrinter(_))
  private val (settingsStateLoc, store4) = store3.newLocation[SettingsState]()
  private val (compilationUnitLoc, store5) =
    store4.newLocation[CompilationUnit]()
  private val (runLoc, store6) = store5.newLocation[Run | Null]()
  private val (profilerLoc, store7) = store6.newLocation[Profiler]()
  private val (notNullInfosLoc, store8) =
    store7.newLocation[List[NotNullInfo]]()
  private val (importInfoLoc, store9) = store8.newLocation[ImportInfo | Null]()
  private val (typeAssignerLoc, store10) =
    store9.newLocation[TypeAssigner](TypeAssigner)
  ...
  inline def atPeriod[T](pd: Period)(inline op: Context ?=> T)(using
      Context
  ): T =
    op(using ctx.fresh.setPeriod(pd))

  /** Execute `op` at given phase id */
  inline def atPhase[T](pid: PhaseId)(inline op: Context ?=> T)(using
      Context
  ): T =
    op(using ctx.withPhase(pid))

  /** Execute `op` at given phase */
  inline def atPhase[T](phase: Phase)(inline op: Context ?=> T)(using
      Context
  ): T =
  ...
  /** Execute `op` at the current phase if it's before the first transform
    * phase, otherwise at the last phase before the first transform phase.
    *
    * Note: this should be used instead of `atPhaseNoLater(ctx.picklerPhase)`
    * because the later won't work if the `Pickler` phase is not present (for
    * example, when using `QuoteCompiler`).
    */
  inline def atPhaseBeforeTransforms[T](inline op: Context ?=> T)(using
      Context
  ): T =
    atPhaseNoLater(firstTransformPhase.prev)(op)

  inline def atPhaseNoLater[T](
      limit: Phase
  )(inline op: Context ?=> T)(using Context): T =
    op(using
      if !limit.exists || ctx.phase <= limit then ctx else ctx.withPhase(limit)
    )

  inline def atPhaseNoEarlier[T](
      limit: Phase
  )(inline op: Context ?=> T)(using Context): T =
    op(using
      if !limit.exists || limit <= ctx.phase then ctx else ctx.withPhase(limit)
    )

  inline def inMode[T](mode: Mode)(inline op: Context ?=> T)(using
      ctx: Context
  ): T =
    op(using if mode != ctx.mode then ctx.fresh.setMode(mode) else ctx)

  inline def withMode[T](mode: Mode)(inline op: Context ?=> T)(using
      ctx: Context
  ): T =
    inMode(ctx.mode | mode)(op)

  inline def withoutMode[T](mode: Mode)(inline op: Context ?=> T)(using
      ctx: Context
  ): T =
    inMode(ctx.mode &~ mode)(op)

  /** A context is passed basically everywhere in dotc. This is convenient but
    * carries the risk of captured contexts in objects that turn into space
    * leaks. To combat this risk, here are some conventions to follow:
    *
    ...
    *     parameter named `initctx`. They pass initctx to all positions where it
    *     is needed (and these positions should all be part of the intialization
  ...
  abstract class Context(val base: ContextBase):
    ...
    def outersIterator: Iterator[Context] = new Iterator[Context]:
      ...
      def next =
        val c = current; current = current.outer; c
    ...
    /** A map in which more contextual properties can be stored Typically used
      * for attributes that are read and written only in special situations.
      */
    ...
    /** A store that can be used by sub-components. Typically used for
      * attributes that are defined only once per compilation unit. Access to
      * store entries is much faster than access to properties, and only
      * slightly slower than a normal field access would be.
      */
    def store: Store

    /** The compiler callback implementation, or null if no callback will be
      * called.
      */
    ...
    def implicits: ContextualImplicits =
      if implicitsCache == null then
        implicitsCache =
          val implicitRefs: List[ImplicitRef] =
            if isClassDefContext then
              try owner.thisType.implicitMembers
              catch case ex: CyclicReference => Nil
            else if isImportContext then importInfo.nn.importedImplicits
            else if isNonEmptyScopeContext then scope.implicitDecls
            else Nil
          val outerImplicits =
            if isImportContext && importInfo.nn.unimported.exists then
              outer.implicits exclude importInfo.nn.unimported
            else outer.implicits
          if implicitRefs.isEmpty then outerImplicits
          else
            new ContextualImplicits(
              implicitRefs,
              outerImplicits,
              isImportContext
            )(this)
    ...
    def getSource(
        file: AbstractFile,
        codec: => Codec = Codec(settings.encoding.value)
    ) =
    ...
    /** The next outer context whose tree is a template or package definition
      * Note: Currently unused def enclTemplate: Context = { var c = this while
      * (c != NoContext && !c.tree.isInstanceOf[Template[?]] &&
      * !c.tree.isInstanceOf[PackageDef[?]]) c = c.outer c }
      */

    /** The context for a supercall. This context is used for elaborating the
      * parents of a class and their arguments. The context is computed from the
      * current class context. It has
      *
      ...
      *
      *   - The constructor must be the owner, because that's where any local
      *     methods or closures should go.
      *   - The context may not see any class members (inherited or defined),
      *     and should instead see definitions defined in the outer context
      *     which might be shadowed by such class members. That's why the outer
      *     context must be the outer context of the class.
      *   - At the same time the context should see the parameter accessors of
      *     the current class, that's why they get added to the local scope. An
      *     alternative would have been to have the context see the constructor
      *     parameters instead, but then we'd need a final substitution step
      *     from constructor parameters to class parameter accessors.
      */
    def superCallContext: Context =
      val locals = newScopeWith(
        owner.typeParams ++ owner.asClass.paramAccessors: _*
      )
      superOrThisCallContext(owner.primaryConstructor, locals)

    /** The context for the arguments of a this(...) constructor call. The
      * context is computed from the local auxiliary constructor context. It has
      *
    ...
    private def superOrThisCallContext(
        owner: Symbol,
        locals: Scope
    ): FreshContext =
      var classCtx = outersIterator.dropWhile(!_.isClassDefContext).next()
      classCtx.outer.fresh
        .setOwner(owner)
        .setScope(locals)
        .setMode(classCtx.mode)

    /** The context of expression `expr` seen as a member of a statement
      * sequence
      */
    def exprContext(stat: Tree[?], exprOwner: Symbol): Context =
      if exprOwner == this.owner then this
      else if untpd.isSuperConstrCall(stat) && this.owner.isClass then
        superCallContext
    ...
    final def withOwner(owner: Symbol): Context =
      if owner ne this.owner then fresh.setOwner(owner) else this

    final def withTyperState(typerState: TyperState): Context =
      if typerState ne this.typerState then fresh.setTyperState(typerState)
      else this
    ...
    final def withProperty[T](key: Key[T], value: Option[T]): Context =
      if property(key) == value then this
      else
        value match
    ...
    /** Reuse this context as a fresh context nested inside `outer` But keep the
      * typerstate, this one has to be set explicitly if needed.
      */
    def reuseIn(outer: Context): this.type
  end Context

  /** A condensed context provides only a small memory footprint over a Context
    * base, and therefore can be stored without problems in long-lived objects.
    * abstract class CondensedContext extends Context { override def condensed =
    * this }
    */

  /** A fresh context allows selective modification of its attributes using the
    * with... methods.
    */
  class FreshContext(base: ContextBase) extends Context(base):
    ...
    /** Initialize all context fields, except typerState, which has to be set
      * separately
      * @param outer
      *   The outer context
      * @param origin
      *   The context from which fields are copied
      */
    ...
    private def setMoreProperties(
        moreProperties: Map[Key[Any], Any]
    ): this.type =
    ...
    def setTypeAssigner(typeAssigner: TypeAssigner): this.type =
      updateStore(typeAssignerLoc, typeAssigner)
    ...
    def setPhase(phase: Phase): this.type = setPeriod(
      Period(runId, phase.start, phase.end)
    )
  ...
  object FreshContext:
    /** Defines an initial context with given context base and possible
      * settings.
      */
  ...
  extension (c: Context)
    final def withModeBits(mode: Mode): Context =
      if mode != c.mode then c.fresh.setMode(mode) else c
  ...
  /** Run `op` with a pool-allocated FreshContext that has an ExporeTyperState.
    */
  inline def exploreInFreshCtx[T](inline op: FreshContext ?=> T)(using
      Context
  ): T =
  ...
  /** Run `op` with a pool-allocated context that has a fresh typer state.
    ...
    */
  inline def withFreshTyperState[T](
      inline op: Context ?=> T,
      inline commit: T => Context ?=> Boolean
  )(using Context): T =
  ...
  inline def runWithOwner[T](
    ...
    else op(using ctx.fresh.setOwner(owner))

  /** The type comparer of the kind created by `maker` to be used. This is the
    * currently active type comparer CMP if
    *   - CMP is associated with the current context, and
    *   - CMP is of the kind created by maker or maker creates a plain type
    *     comparer. Note: plain TypeComparers always take on the kind of the
    *     outer comparer if they are in the same context. In other words:
    *     tracking or explaining is a sticky property in the same context.
    */
  private def comparer(using Context): TypeComparer =
    ...
    then base.comparers(base.comparersInUse - 1).currentInstance
    else
      val result =
  ...
  @sharable val NoContext: Context = new FreshContext(
    (null: ContextBase | Null).uncheckedNN
  ):
    override val implicits: ContextualImplicits =
      new ContextualImplicits(Nil, null, false)(this: @unchecked)
    setSource(NoSource)
  ...
  class ContextBase extends ContextState with Phases.PhasesBase with Plugins:
    ...
    /** Initializes the `ContextBase` with a starting context. This initializes
      * the `platform` and the `definitions`.
      */
  ...
  class ContextPool:
    ...
    def next()(using Context): FreshContext =
      ...
      import base.*
      val nestedCtx =
        if inUse < pool.size then pool(inUse).reuseIn(ctx)
        else
          val c = fresh()
          pool += c
          c
  ...
  /** The essential mutable state of a context base, collected into a common
    * class
    */
  class ContextState:
    // Symbols state
    ...
    def nextSymId: Int =
      _nextSymId += 1; _nextSymId

    /** Sources and Files that were loaded */
    val sources: util.HashMap[AbstractFile, SourceFile] =
      util.HashMap[AbstractFile, SourceFile]()
    ...
    /** List of names which have a findMemberCall on stack, after
      * Config.LogPendingFindMemberThreshold is reached.
      */
    private[core] var pendingMemberSearches: List[Name] = Nil

    /** The number of recursive invocation of underlying on a NamedType during a
      * controlled operation.
      */
    private[core] var underlyingRecursions: Int = 0

    /** The set of named types on which a currently active invocation of
      * underlying during a controlled operation exists.
      */
    private[core] val pendingUnderlying: util.HashSet[Type] =
      util.HashSet[Type]()

    /** A map from ErrorType to associated message. We use this map instead of
      * storing messages directly in ErrorTypes in order to avoid space leaks -
      * the message usually captures a context.
      */
    private[core] val errorTypeMsg: mutable.Map[Types.ErrorType, Message] =
      mutable.Map()
    ...
    /** Phases with consecutive Transforms grouped into a single phase, Empty
      * array if fusion is disabled
      */
    ...
    /** Cached -Yno-double-bindings setting. This is accessed from `setDenot`,
      * which is fairly hot, so we don't want to lookup the setting each time it
      * is called.
      */
    private[dotc] var checkNoDoubleBindings = false

    /** A variable that records that some error was reported in a globally
      * committable context. The error will not necessarlily be emitted, since
      * it could still be that the enclosing context will be aborted. The
      * variable is used as a smoke test to turn off assertions that might be
      * wrong if the program is erroneous. To just test for
      * `ctx.reporter.errorsReported` is not always enough, since it could be
      * that the context in which the assertion is tested is a completer context
      * that's different from the context where the error was reported. See
      * i13218.scala for a test.
      */
    ...
    private[core] val reusableDataReader = ReusableInstance(
    ...
    def reset(): Unit =
      ...
      files.clear()
      comparers
        .clear() // forces re-evaluation of top and bottom classes in TypeComparer
    ...
    def checkSingleThreaded(): Unit =
      if thread == null then thread = Thread.currentThread()
      else
        assert(
          thread == Thread.currentThread(),
          "illegal multithreaded access to ContextBase"
        )
  end ContextState
end Contexts
