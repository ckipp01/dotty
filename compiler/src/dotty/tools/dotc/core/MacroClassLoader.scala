...
object MacroClassLoader:
  ...
  /** A key to be used in a context property that caches the class loader used
    * for macro expansion
    */
  ...
  /** Context with a cached macro class loader that can be accessed with
    * `macroClassLoader`
    */
  ...
  private def makeMacroClassLoader(using Context): ClassLoader =
    trace("new macro class loader") {
      ...
      val urls =
        entries.map(cp => java.nio.file.Paths.get(cp).toUri.toURL).toArray
      val out = Option(
