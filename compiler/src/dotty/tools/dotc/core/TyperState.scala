...
class TyperState():
  ...
  /** A fresh type state with the same constraint as this one and the given
    * reporter
    */
  def setReporter(reporter: Reporter): this.type =
    myReporter = reporter; this
  ...
  def constraint_=(c: Constraint)(using Context): Unit =
    if Config.debugCheckConstraintsClosed && isGlobalCommittable then
      c.checkClosed()
    myConstraint = c
  ...
  /** The set of uninstantiated type variables which have this state as their
    * owning state.
    *
    * Invariant: if `tstate.isCommittable` then
  ...
  /** Initializes all fields except reporter, isCommittable, which need to be
    * set separately.
    */
  private[core] def init(
      previous: TyperState | Null,
  ...
  def fresh(
      reporter: Reporter = StoreReporter(this.reporter, fromTyperState = true),
      committable: Boolean = this.isCommittable
  ): TyperState =
    util.Stats.record("TyperState.fresh")
    val ts = TyperState()
      .init(this, this.constraint)
  ...
  /** Set the nestingLevel of `tv` in this typer state
    * @pre
    *   this level must be smaller than `tv.initNestingLevel`
    */
  def setNestingLevel(tv: TypeVar, level: Int) =
    assert(level < tv.initNestingLevel)
    upLevels = upLevels.updated(tv, level)

  /** The closest ancestor of this typer state (including possibly this typer
    * state itself) which is not yet committed, or which does not have a parent.
    */
  def uncommittedAncestor: TyperState =
    if isCommitted && previous != null then
      previous.uncheckedNN.uncommittedAncestor
    else this

  /** Commit typer state so that its information is copied into current typer
    * state In addition (1) the owning state of undetermined or temporarily
    * instantiated type variables changes from this typer state to the current
    * one. (2) Variables that were temporarily instantiated in the current typer
    * state are permanently instantiated instead.
    *
    * A note on merging: An interesting test case is isApplicableSafe.scala. It
    * turns out that this requires a context merge using the new `&' operator.
    * Sequence of actions: 1) Typecheck argument in typerstate 1. 2) Cache
    * argument. 3) Evolve same typer state (to typecheck other arguments, say)
    * leading to a different constraint. 4) Take typechecked argument in same
    * state.
    *
    * It turns out that the merge is needed not just for isApplicableSafe but
    * also for (e.g. erased-lubs.scala) as well as many parts of dotty itself.
    */
  def commit()(using Context): Unit =
    ...
    val nothingToCommit =
      (constraint eq targetState.constraint) && !reporter.hasUnreportedMessages
    assert(
      !targetState.isCommitted || nothingToCommit ||
    ...
    if constraint ne targetState.constraint then
      ...
      constr.println(
        i"committing $this to $targetState, fromConstr = $constraint, toConstr = ${targetState.constraint}"
      )
  ...
  /** Ensure that this constraint does not associate different TypeVars for the
    * same type lambda than the `other` constraint. Do this by renaming type
    * lambdas in this constraint and its predecessors where necessary.
    */
  def ensureNotConflicting(other: Constraint)(using Context): Unit =
    val conflicting = constraint.domainLambdas.filter(
      constraint.hasConflictingTypeVarsFor(_, other)
    )
    for tl <- conflicting do
      val tl1 = constraint.ensureFresh(tl)
      for case (tvar: TypeVar, pref1) <- tl.paramRefs
          .map(constraint.typeVarOfParam)
          .lazyZip(tl1.paramRefs)
      do tvar.setOrigin(pref1)
      var ts = this
  ...
  /** Integrate the constraints from `that` into this TyperState.
    *
    * @pre
    *   If `this` and `that` are committable, `that` must not contain any type
    *   variable which does not exist in `this` (in other words, all its type
    *   variables must be owned by a common parent of `this` and `that`).
    */
  def mergeConstraintWith(that: TyperState)(using Context): this.type =
    ...
    inContext(comparingCtx)(comparing(typeComparer =>
      ...
      val res = other.domainLambdas.forall(tl =>
        ...
        constraint.contains(tl) || other.isRemovable(tl) || {
          ...
          if this.isCommittable then
            tvars.foreach(tvar =>
              if !tvar.inst.exists && !isOwnedAnywhere(this, tvar) then
                includeVar(tvar)
            )
      ...
      assert(
        res || ctx.reporter.errorsReported,
        i"cannot merge $constraint with $other."
      )
    ))
  ...
  /** Take ownership of `tvar`.
    *
    * @pre
    *   `tvar` is not owned by a committable TyperState. This ensures each tvar
    *   can only be instantiated by one TyperState.
    */
  private def includeVar(tvar: TypeVar)(using Context): Unit =
    val oldState = tvar.owningState.nn.get
    assert(
      oldState == null || !oldState.isCommittable,
  ...
  private def isOwnedAnywhere(ts: TyperState, tvar: TypeVar): Boolean =
    ts.ownedVars.contains(tvar) || ts.previous != null && isOwnedAnywhere(
      ts.previous.uncheckedNN,
      tvar
    )
  ...
  def gc()(using Context): Unit =
    if !ownedVars.isEmpty then
      ...
      for tvar <- ownedVars do
        ...
        assert(
          tvarState eqn this,
          s"Inconsistent state in $this: it owns $tvar whose owningState is ${tvarState}"
        )
        assert(
          !tvar.inst.exists,
          s"Inconsistent state in $this: it owns $tvar which is already instantiated"
        )
      ...
      for tl <- toCollect do constraint = constraint.remove(tl)

  override def toString: String =
    def ids(state: TyperState): List[String] =
      s"${state.id}${if state.isCommittable then "" else "X"}" ::
        (if state.previous == null then Nil
         else ids(state.previous.uncheckedNN))
    s"TS[${ids(this).mkString(", ")}]"

  def stateChainStr: String =
    s"$this${if previous == null then "" else previous.uncheckedNN.stateChainStr}"
end TyperState
