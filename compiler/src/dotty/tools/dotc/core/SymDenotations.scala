...
import Periods.*, Contexts.*, Symbols.*, Denotations.*, Names.*, NameOps.*,
  Annotations.*
...
object SymDenotations:
  ...
  class SymDenotation private[SymDenotations] (
  ...
  ) extends SingleDenotation(symbol, initInfo, name.isTypeName):

    // assert(symbol.id != 4940, name)

    override def hasUniqueSym: Boolean = exists

    /** Debug only override def validFor_=(p: Period) = { super.validFor_=(p) }
      */
    if Config.checkNoSkolemsInInfo then assertNoSkolems(initInfo)
    ...
    final def flags(using Context): FlagSet =
      ensureCompleted(); myFlags

    /** The flag set without forcing symbol completion. Should be used only for
      * printing.
      */
    ...
    private def adaptFlags(flags: FlagSet) =
      if isType then flags.toTypeFlags else flags.toTermFlags
    ...
    final def setNoInitsFlags(parentFlags: FlagSet, bodyFlags: FlagSet): Unit =
      setFlag(
        if myFlags.is(Trait) then
          NoInitsInterface & bodyFlags // no parents are initialized from a trait
    ...
    final def relevantFlagsFor(fs: FlagSet)(using Context) =
      if isCurrent(fs) then myFlags else flags

    /** Has this denotation one of given flag set? */
    final def is(flag: Flag)(using Context): Boolean =
      (if isCurrent(flag) then myFlags else flags).is(flag)

    /** Has this denotation one of the flags in `fs` set? */
    final def isOneOf(fs: FlagSet)(using Context): Boolean =
      (if isCurrent(fs) then myFlags else flags).isOneOf(fs)

    /** Has this denotation the given flag set, whereas none of the flags in
      * `butNot` are set?
      */
    final def is(flag: Flag, butNot: FlagSet)(using Context): Boolean =
      (if isCurrent(flag) && isCurrent(butNot) then myFlags else flags)
        .is(flag, butNot)

    /** Has this denotation one of the flags in `fs` set, whereas none of the
      * flags in `butNot` are set?
      */
    final def isOneOf(fs: FlagSet, butNot: FlagSet)(using Context): Boolean =
      (if isCurrent(fs) && isCurrent(butNot) then myFlags else flags)
        .isOneOf(fs, butNot)

    /** Has this denotation all of the flags in `fs` set? */
    final def isAllOf(fs: FlagSet)(using Context): Boolean =
      (if isCurrent(fs) then myFlags else flags).isAllOf(fs)

    /** Has this denotation all of the flags in `fs` set, whereas none of the
      * flags in `butNot` are set?
      */
    ...
    final def completeFrom(completer: LazyType)(using Context): Unit =
      if completer.needsCompletion(this) then
        if Config.showCompletions then
          println(
            i"${"  " * indent}completing ${if isType then "type" else "val"} $name"
          )
          indent += 1

          if myFlags.is(Touched) then throw CyclicReference(this)
          myFlags |= Touched
    ...
    protected[dotc] def info_=(tp: Type): Unit =
       ...
       */
      if Config.checkNoSkolemsInInfo then assertNoSkolems(tp)
      myInfo = tp
    ...
    def effectiveName(using Context): Name =
      if this.is(ModuleClass) then name.stripModuleClassSuffix
      else name
    ...
    /** Set privateWithin, prefer setting it at symbol-creation time instead if
      * possible.
      * @pre
      *   `isCompleting` is false, or this is a ModuleCompleter or SymbolLoader
      */
    protected[dotc] final def setPrivateWithin(pw: Symbol)(using
        Context
    ): Unit =
      if isCompleting then
        assert(
          myInfo.isInstanceOf[ModuleCompleter | SymbolLoader],
    ...
    /** The annotations without ensuring that the symbol is completed. Used for
      * diagnostics where we don't want to force symbols.
      */
    ...
    final def transformAnnotations(f: Annotation => Annotation)(using
        Context
    ): Unit =
    ...
    def annotationsCarrying(
        meta: Set[Symbol],
        orNoneOf: Set[Symbol] = Set.empty
    )(using Context): List[Annotation] =
      annotations.filterConserve(
        _.hasOneOfMetaAnnotation(meta, orNoneOf = orNoneOf)
      )

    def keepAnnotationsCarrying(
        phase: DenotTransformer,
        meta: Set[Symbol],
        orNoneOf: Set[Symbol] = Set.empty
    )(using Context): Unit =
      updateAnnotationsAfter(
        phase,
        annotationsCarrying(meta, orNoneOf = orNoneOf)
      )

    def updateAnnotationsAfter(
        phase: DenotTransformer,
    ...
    /** The same as getAnnotation, but without ensuring that the symbol carrying
      * the annotation is completed
      */
    final def unforcedAnnotation(cls: Symbol)(using
        Context
    ): Option[Annotation] =
    ...
    /** Add the given annotation without parameters to the annotations of this
      * denotation
      */
    ...
    final def addAnnotations(annots: IterableOnce[Annotation])(using
        Context
    ): Unit =
      annots.iterator.foreach(addAnnotation)

    @tailrec
    private def dropOtherAnnotations(anns: List[Annotation], cls: Symbol)(using
        Context
    ): List[Annotation] = anns match
      case ann :: rest =>
        if ann matches cls then anns else dropOtherAnnotations(rest, cls)
      case Nil => Nil

    /** If this is a method, the parameter symbols, by section. Both type and
      * value parameters are included. Empty sections are skipped.
      */
    ...
    final def setParamssFromDefs(paramss: List[tpd.ParamClause])(using
        Context
    ): Unit =
    ...
    /** The symbols of each type parameter list and value parameter list of this
      ...
      *
      * Makes use of `rawParamss` when present, or constructs fresh parameter
      * symbols otherwise. This method can be allocation-heavy.
      */
    final def paramSymss(using Context): List[List[Symbol]] =

      def recurWithParamss(
          info: Type,
          paramss: List[List[Symbol]]
      ): List[List[Symbol]] =
        info match
          case info: LambdaType =>
            if info.paramNames.isEmpty then
              Nil :: recurWithParamss(info.resType, paramss)
      ...
      def recurWithoutParamss(info: Type): List[List[Symbol]] = info match
        case info: LambdaType =>
          val params = info.paramNames
            .lazyZip(info.paramInfos)
            .map((pname, ptype) =>
    ...
    /** The extension parameter of this extension method
      * @pre
      *   this symbol is an extension method
      */
    ...
    /** The denotation is completed: info is not a lazy type and attributes have
      * defined values
      */
    ...
    /** If this denotation is not completed, run the completer. The resulting
      * info might be another completer.
      *
      * @see
      *   ensureCompleted
      */
    ...
    /** The symbols defined in this class or object. Careful! This does not
      * force the type, so is compilation order dependent. This method should be
      * used only in the following circumstances:
      *
      *   1. When accessing type parameters or type parameter accessors (both
      *      are entered before completion). 2. When obtaining the current scope
      *      in order to enter, rename or delete something there. 3. When
      *      playing it safe in order not to raise CylicReferences, e.g. for
      *      printing things or taking more efficient shortcuts (e.g. the
      *      stillValid test).
      */
    ...
    /** If this is a package class, the symbols entered in it before it is
      * completed. (this is needed to eagerly enter synthetic aliases such as
      * AnyRef into a package class without forcing it. Right now, the only
      * usage is for the AnyRef alias in Definitions.
      */
    final private[core] def currentPackageDecls(using Context): MutableScope =
      myInfo match
        case pinfo: SymbolLoaders.PackageLoader => pinfo.currentDecls
        case _ => unforcedDecls.openForMutations

    /** If this is an opaque alias, replace the right hand side `info` by
      * appropriate bounds and store `info` in the refinement of the self type
      * of the enclosing class. Otherwise return `info`
      *
      * @param info
      *   Is assumed to be a (lambda-abstracted) right hand side TypeAlias of
      *   the opaque type definition.
      * @param rhs
      *   The right hand side tree of the type definition
      * @param tparams
      *   The type parameters with which the right-hand side bounds should be
      *   abstracted
      */
    def opaqueToBounds(info: Type, rhs: tpd.Tree, tparams: List[TypeSymbol])(
        using Context
    ): Type =

      def setAlias(tp: Type) =
        def recur(self: Type): Unit = self match
          case RefinedType(parent, name, rinfo) =>
            rinfo match
              case TypeAlias(lzy: LazyRef) if name == this.name =>
                if !lzy.completed then lzy.update(tp)
                else throw CyclicReference(this)
              case _ =>
                recur(parent)
    ...
    /** The encoded full path name of this denotation, where outer names and
      * inner names are separated by `separator` strings as indicated by the
      * given name kind. Drops package objects. Represents each term in the
      * owner chain by a simple `_$`.
      */
    def fullNameSeparated(kind: QualifiedNameKind)(using Context): Name =
      maybeOwner.fullNameSeparated(kind, kind, name)

    /** The encoded full path name of this denotation (separated by
      * `prefixKind`), followed by the separator implied by `kind` and the given
      * `name`. Drops package objects. Represents each term in the owner chain
      * by a simple `_$`.
      */
    def fullNameSeparated(
      ...
      if symbol == NoSymbol || isEffectiveRoot || kind == FlatName && is(
          PackageClass
        )
      then name
      else
        ...
        def qualify(n: SimpleName) =
          val qn = kind(
            prefix.toTermName,
            if filler.isEmpty then n else termName(filler + n)
          )
          if kind == FlatName && !encl.is(JavaDefined) then qn.compactified
          else qn
        val fn = name.replaceDeep { case n: SimpleName =>
          qualify(n)
        }
    ...
    /** The fully qualified name on the JVM of the class corresponding to this
      * symbol.
      */
    def binaryClassName(using Context): String =
      ...
      val clsFlatName =
        if isOneOf(JavaDefined | ConstructorProxy) then
          flatName.stripModuleClassSuffix
        else flatName
    ...
    private def computeTargetName(targetNameAnnot: Option[Annotation])(using
        Context
    ): Name =
    ...
    /** The name given in a `@targetName` annotation if one is present, `name`
      * otherwise
      */
    def targetName(using Context): Name =
      if myTargetName == null then
        ...
        if name.is(SuperAccessorName) then
          myTargetName = myTargetName.nn.unmangle(
            List(ExpandedName, SuperAccessorName, ExpandPrefixName)
          )
    ...
    /** Is symbol known to not exist?
      * @param canForce
      *   If this is true, the info may be forced to avoid a false-negative
      *   result
      */
    @tailrec
    final def isAbsent(canForce: Boolean = true)(using Context): Boolean =
      myInfo match
        case myInfo: ModuleCompleter =>
    ...
    /** Is this a synthetic method that represents conversions between
      * representations of a value class These methods are generated in
      * ExtensionMethods and used in ElimErasedValueType.
      */
    ...
    def isPrimitiveValueClass(using Context): Boolean =
      maybeOwner == defn.ScalaPackageClass && defn
        .ScalaValueClasses()
        .contains(symbol)
    ...
    def isNotRuntimeClass(using Context): Boolean =
      defn.NotRuntimeClasses contains symbol

    /** Is this symbol a class representing a refinement? These classes are used
      * only temporarily in Typer and Unpickler as an intermediate step for
    ...
    final def isAliasType(using Context): Boolean =
      isAbstractOrAliasType && !this.is(Deferred)

    /** Is this symbol an abstract or alias type? */
    final def isAbstractOrAliasType: Boolean = isType & !isClass

    /** Is this symbol an abstract type or type parameter? */
    final def isAbstractOrParamType(using Context): Boolean =
      this.isOneOf(DeferredOrTypeParam)
    ...
    /** Is this the denotation of a self symbol of some class? This is the case
      * if one of two conditions holds:
      *   1. It is the symbol referred to in the selfInfo part of the ClassInfo
      *      which is the type of this symbol's owner. 2. This symbol is owned
      *      by a class, it's selfInfo field refers to a type (indicating the
      *      self definition does not introduce a name), and the symbol's name
      *      is "_". TODO: Find a more robust way to characterize self symbols,
      *      maybe by spending a Flag on them?
      */
    ...
    final def isContainedIn(boundary: Symbol)(using Context): Boolean =
      def recur(sym: Symbol): Boolean =
        if sym eq boundary then true
        else if sym eq NoSymbol then false
        else if sym.is(PackageClass) && !boundary.is(PackageClass) then false
    ...
    final def isStatic(using Context): Boolean =
      (if maybeOwner eq NoSymbol then isRoot
       else maybeOwner.originDenotation.isStaticOwner) ||
    ...
    /** Is this denotation defined in the same scope and compilation unit as
      * that symbol?
      */
    ...
    /** Do this symbol and `cls` represent a pair of a given or implicit method
      * and its associated class that were defined by a single definition? This
      * can mean one of two things:
      *   - the method and class are defined in a structural given instance, or
      *   - the class is an implicit class and the method is its implicit
      *     conversion.
      */
    ...
    /** Is this a denotation of a stable term (or an arbitrary type)? Terms are
      * stable if they are idempotent (as in TreeInfo.Idempotent): that is, they
      * always return the same value, if any.
      *
      * A *member* is stable, basically, if it behaves like a field projection:
      * that is, it projects a constant result out of its owner.
      *
      * However, a stable member might not yet be initialized (if it is an
      * object or anyhow lazy). So the first call to a stable member might fail
      * and/or produce side effects.
      */
    final def isStableMember(using Context): Boolean =
      def isUnstableValue =
        isOneOf(UnstableValueFlags) || info.isInstanceOf[ExprType] || isAllOf(
          InlineParam
        )
      isType || is(StableRealizable) || exists && !isUnstableValue

    /** Is this a denotation of a real class that does not have - either direct
      * or inherited - initialization code?
      */
    def isNoInitsRealClass(using Context): Boolean =
      isRealClass &&
        (asClass.baseClasses.forall(_.is(NoInits)) || defn.isAssuredNoInits(
          symbol
        ))
    ...
    final def isGetter(using Context): Boolean =
      this.is(
        Accessor
      ) && !originalName.isSetterName && !(originalName.isScala2LocalSuffix && symbol.owner
        .is(Scala2x))
    ...
    /** Is this a subclass of `base`, and is the denoting symbol also different
      * from `Null` or `Nothing`?
      * @note
      *   erroneous classes are assumed to derive from all other classes and all
    ...
    /** Is this symbol a class that extends `AnyVal`? Overridden in
      * ClassDenotation
      */
    ...
    final def isNullableClass(using Context): Boolean =
      ...
      then
        symbol == defn.NullClass || symbol == defn.AnyClass || symbol == defn.MatchableClass
      else isNullableClassAfterErasure

    /** Is this symbol a class of which `null` is a value after erasure? For
      * example, if `-Yexplicit-nulls` is set, `String` is not nullable before
      * erasure, but it becomes nullable after erasure.
      */
    final def isNullableClassAfterErasure(using Context): Boolean =
      isClass && !isValueClass && !is(
        ModuleClass
    ...
    /** Is this definition accessible as a member of tree with type `pre`?
      ...
      * @param superAccess
      *   Access is via super Everything is accessible if `pre` is `NoPrefix`. A
      *   symbol with type `NoType` is not accessible for any other prefix.
      *
      * As a side effect, drop Local flags of members that are not accessed via
      * the ThisType of their owner.
      */
    final def isAccessibleFrom(
        pre: Type,
        superAccess: Boolean = false,
        whyNot: StringBuffer | Null = null
    )(using Context): Boolean =

      /** Are we inside definition of `boundary`? If this symbol is Java
        * defined, package structure is interpreted to be flat.
        */
      def accessWithin(boundary: Symbol) =
        ctx.owner.isContainedIn(boundary)
          && !(is(JavaDefined) && boundary.is(
            PackageClass
          ) && ctx.owner.enclosingPackageClass != boundary)
      ...
      /** Is `pre` the same as C.thisThis, where C is exactly the owner of this
        * symbol, or, if this symbol is protected, a subclass of the owner?
        */
      ...
      def isProtectedAccessOK: Boolean =
        ...
        if !cls.exists then
          val encl =
            if ctx.owner.isConstructor then
              ctx.owner.enclosingClass.owner.enclosingClass
            else ctx.owner.enclosingClass
          fail(
            i"""
        ...
        else if isType || pre.derivesFrom(cls) || isConstructor || owner.is(
            ModuleClass
          )
        then
      ...
      if pre eq NoPrefix then true
        ...
        (superAccess
          ...
          || isProtectedAccessOK))
    end isAccessibleFrom

    /** Do members of this symbol need translation via asSeenFrom when accessed
      * via prefix `pre`?
      */
    ...
    /** Does this symbol have defined or inherited default parameters? Default
      * parameters are recognized until erasure.
      */
    ...
    def isEffectivelyErased(using Context): Boolean =
      isOneOf(EffectivelyErased)
        || is(Inline) && !isRetainedInline && !hasAnnotation(
          defn.ScalaStaticAnnot
        )

    /** ()T and => T types should be treated as equivalent for this symbol.
      * Note: For the moment, we treat Scala-2 compiled symbols as loose
      * matching, because the Scala library does not always follow the right
      * conventions. Examples are: isWhole(), toInt(), toDouble() in BigDecimal,
      * Numeric, RichInt, ScalaNumberProxy.
      */
     ...
     * the completers.
     */
    /** If this a module, return the corresponding class, if this is a module,
      * return itself, otherwise NoSymbol
      */
    final def moduleClass(using Context): Symbol =
      def notFound =
        if Config.showCompletions then
          println(s"missing module class for $name: $myInfo")
        NoSymbol
      if this.is(ModuleVal) then
        myInfo match
          case info: TypeRef => info.symbol
          case ExprType(info: TypeRef) =>
            info.symbol // needed after uncurry, when module terms might be accessor defs
          ...
          case t: MethodType =>
            t.resultType match
              ...
              case _             => notFound
          case _ => notFound
      else if this.is(ModuleClass) then symbol
      else NoSymbol

    /** If this a module class, return the corresponding module, if this is a
      * module, return itself, otherwise NoSymbol
      */
    final def sourceModule(using Context): Symbol =
      if this.is(ModuleClass) then
        myInfo match
          ...
          case _ =>
            NoSymbol
      else if this.is(ModuleVal) then symbol
      else NoSymbol

    /** The field accessed by this getter or setter, or if it does not exist,
      * the getter
      */
    def accessedFieldOrGetter(using Context): Symbol =
      val fieldName = if isSetter then name.asTermName.getterName else name
    ...
    /** The field accessed by a getter or setter, or if it does not exists, the
      * getter of a setter, or if that does not exist the symbol itself.
      */
    def underlyingSymbol(using Context): Symbol =
      if is(Accessor) then accessedFieldOrGetter orElse symbol else symbol

    /** The chain of owners of this denotation, starting with the denoting
      * symbol itself
      */
    ...
    /** The class containing this denotation. If this denotation is already a
      * class, return itself Definitions flagged with JavaStatic are treated
      * specially. Their enclosing class is not the lexically enclosing class,
      * but in turn the enclosing class of the latter. This reflects the context
      * created by `Context#superCallContext`, `Context#thisCallArgContext` for
      * these definitions.
      *
      * Note, that as packages have ClassSymbols, top level classes will have an
      * `enclosingClass` with Package flag set.
      */
    final def enclosingClass(using Context): Symbol =
      def enclClass(sym: Symbol, skip: Boolean): Symbol =
        ...
        if !sym.exists then NoSymbol
          ...
          if skip then enclClass(sym.owner, newSkip) else sym
        else enclClass(sym.owner, skip || newSkip)
    ...
    final def lexicallyEnclosingClass(using Context): Symbol =
      if !exists || isClass then symbol else owner.lexicallyEnclosingClass

    /** A class is extensible if it is not final, nor a module class, nor an
      * anonymous class.
      */
    ...
    /** A class is effectively sealed if has the `final` or `sealed` modifier,
      * or it is defined in Scala 3 and is neither abstract nor open.
      */
    ...
    final def enclosingClassNamed(name: Name)(using Context): Symbol =
      ...
      if cls.effectiveName == name || !cls.exists then cls
      else cls.owner.enclosingClassNamed(name)

    /** The closest enclosing method containing this definition. A local dummy
      * owner is mapped to the primary constructor of the class.
      */
    final def enclosingMethod(using Context): Symbol =
      if this.is(Method) then symbol
      else if this.isClass then primaryConstructor
      else if this.exists then owner.enclosingMethod
      else NoSymbol
    ...
    /** The top-level class containing this denotation, except for a toplevel
      * module, where its module class is returned.
      */
    final def topLevelClass(using Context): Symbol =
      ...
      if sym.isClass then sym else sym.moduleClass

    final def isTopLevelClass(using Context): Boolean =
      !this.exists || this.isEffectiveRoot || this.is(
        PackageClass
      ) || this.owner.is(PackageClass)

    /** The package class containing this denotation */
    final def enclosingPackageClass(using Context): Symbol =
      if this.is(PackageClass) then symbol else owner.enclosingPackageClass
    ...
    /** The module object with the same (term-) name as this class or module
      * class, and which is also defined in the same scope and compilation unit.
      * NoSymbol if this module does not exist.
      */
    final def companionModule(using Context): Symbol =
      if is(Module) then sourceModule
    ...
    private def companionType(using Context): Symbol =
      if is(Package) then NoSymbol
      else if is(ModuleVal) then moduleClass.denot.companionType
      else if registeredCompanion.isAbsent() then NoSymbol
      else registeredCompanion

    /** The class with the same (type-) name as this module or module class, and
      * which is also defined in the same scope and compilation unit. NoSymbol
    ...
    final def scalacLinkedClass(using Context): Symbol =
      if this.is(ModuleClass) then companionNamed(effectiveName.toTypeName)
      else if this.isClass then companionNamed(effectiveName.moduleClassName)
      else NoSymbol

    /** Find companion class symbol with given name, or NoSymbol if none exists.
      * Three alternative strategies:
      *   1. If owner is a class, look in its members, otherwise 2. If current
      *      compilation unit has a typed tree, determine the defining statement
      *      sequence and search its trees, otherwise 3. If context has an
      *      enclosing scope which defines this symbol, lookup its companion in
      *      the same scope.
      */
    private def companionNamed(name: TypeName)(using Context): Symbol =
      ...
      if owner.isClass then
        owner.unforcedDecls
          .lookup(name)
          .suchThat(_.isCoDefinedWith(symbol))
          .symbol
      else if !owner.exists || (unit eq NoCompilationUnit) then NoSymbol
      else if !unit.tpdTree.isEmpty then
        tpd
          .definingStats(symbol)
          .iterator
          .map(tpd.definedSym)
          .find(_.name == name)
          .getOrElse(NoSymbol)
      else if ctx.scope eq EmptyScope then NoSymbol
      else if ctx.scope.lookup(this.name) == symbol then ctx.scope.lookup(name)
      else
        companionNamed(name)(using
          ctx.outersIterator.dropWhile(_.scope eq ctx.scope).next()
        )
    ...
    /** If this is a class, the module class of its companion object. If this is
      * a module class, its companion class. NoSymbol otherwise.
      */
    final def linkedClass(using Context): Symbol =
      if this.is(ModuleClass) then companionClass
      else if this.isClass then companionModule.moduleClass
      else NoSymbol

    /** The class that encloses the owner of the current context and that is a
      * subclass of this class. NoSymbol if no such class exists.
      */
    ...
    /** The non-private symbol whose name and type matches the type of this
      * symbol in the given class.
      * @param inClass
    ...
    final def matchingDecl(inClass: Symbol, site: Type)(using Context): Symbol =
      ...
      if denot.isTerm then // types of the same name always match
        denot = denot.matchingDenotation(
          site,
          site.memberInfo(symbol),
          symbol.targetName
        )
      denot.symbol

    /** The non-private member of `site` whose name and type matches the type of
      * this symbol
      */
    ...
    /** If false, this symbol cannot possibly participate in an override, either
      * as overrider or overridee.
      */
    ...
    /** The symbol, in class `inClass`, that is overridden by this denotation in
      * class `siteClass`.
      */
    final def overriddenSymbol(
        inClass: ClassSymbol,
        siteClass: ClassSymbol = owner.asClass
    )(using Context): Symbol =
      if !canMatchInheritedSymbols && (owner ne inClass) then NoSymbol
    ...
    /** Equivalent to `allOverriddenSymbols.headOption.getOrElse(NoSymbol)` but
      * more efficient.
      */
    final def nextOverriddenSymbol(using Context): Symbol =
      val overridden = allOverriddenSymbols
      if overridden.hasNext then overridden.next
      else NoSymbol
    ...
    private def overriddenFromType(tp: Type)(using Context): Iterator[Symbol] =
      tp.baseClasses match
        case _ :: inherited =>
          inherited.iterator.map(overriddenSymbol(_)).filter(_.exists)
        case Nil => Iterator.empty

    /** The symbol overriding this symbol in given subclass `inClass`.
      *
      * @pre
      *   `inClass` is a subclass of this symbol's owner
      */
    final def overridingSymbol(inClass: ClassSymbol)(using Context): Symbol =
      if canMatchInheritedSymbols then matchingDecl(inClass, inClass.thisType)
      else NoSymbol

    /** The symbol accessed by a super in the definition of this symbol when
      * seen from class `base`. This symbol is always concrete. pre:
      ...
      @tailrec def loop(bcs: List[ClassSymbol]): Symbol = bcs match
        case bc :: bcs1 =>
          val sym = matchingDecl(bcs.head, base.thisType)
            .suchThat(alt => !alt.is(Deferred))
            .symbol
          if sym.exists then sym else loop(bcs.tail)
        case _ =>
          NoSymbol
      loop(base.info.baseClasses.dropWhile(owner != _).tail)

    /** A member of class `base` is incomplete if (1) it is declared deferred or
    ...
    /** The class or term symbol up to which this symbol is accessible, or
      * RootClass if it is public. As java protected statics are otherwise
      * completely inaccessible in scala, they are treated as public.
      * @param base
      *   The access boundary to assume if this symbol is protected
      */
    final def accessBoundary(base: Symbol)(using Context): Symbol =
      if this.is(Private) then owner
      else if this.isAllOf(StaticProtected) then defn.RootClass
      else if privateWithin.exists && (!ctx.phase.erasedTypes || this.is(
          JavaDefined
        ))
      then privateWithin
      else if this.is(Protected) then base
      else defn.RootClass
    ...
    /** The current declaration in this symbol's class owner that has the same
      * name as this one, and, if there are several, also has the same
      * signature.
      */
    def currentSymbol(using Context): Symbol =
      val candidates = owner.info.decls.lookupAll(name)
      def test(sym: Symbol): Symbol =
        if sym == symbol || sym.signature == signature then sym
        else if candidates.hasNext then test(candidates.next)
        else NoSymbol
      if candidates.hasNext then
        val sym = candidates.next
        if candidates.hasNext then test(sym) else sym
      else NoSymbol
    ...
    /** The type This(cls), where cls is this class, NoPrefix for all other
      * symbols
      */
    ...
    /** The NamedType representing this denotation at its original location.
      * Same as either `typeRef` or `termRef` depending whether this denotes a
      * type or not.
      */
    def namedType(using Context): NamedType =
      if isType then typeRef else termRef

    /** Like typeRef, but objects in the prefix are represented by their
      * singleton type, this means we output `pre.O.member` rather than
      * `pre.O$.this.member`.
      *
      * This is required to avoid owner crash in ExplicitOuter. See
    ...
    /** Like termRef, if it is addressable from the current context, but objects
      * in the prefix are represented by their singleton type, this means we
      * output `pre.O.member` rather than `pre.O$.this.member`.
      *
      * This is required to avoid owner crash in ExplicitOuter. See
      * tests/pos/i10769.scala
      *
      * If the reference is to an object that is not accessible from the current
      * context since the object is nested in a class that is not an outer class
      * of the current context, fall back to a TypeRef to the module class. Test
      * case is tests/pos/i17556.scala. If the reference is to some other
      * inaccessible object, throw an AssertionError.
      */
    def reachableTermRef(using Context): Type = owner.reachablePrefix match
      ...
      case _ =>
        throw AssertionError(
          i"cannot compute path to TermRef $this from ${ctx.owner}"
        )

    /** Like thisType, but objects in the type are represented by their
      * singleton type, this means we output `pre.O.member` rather than
      * `pre.O$.this.member`.
      */
    def reachableThisType(using Context): Type =
      if this.is(Package) then symbol.thisType
      else if this.isTerm then NoPrefix
      else if this.is(Module) then
        TermRef(owner.reachableThisType, this.sourceModule)
      else ThisType.raw(TypeRef(owner.reachableThisType, symbol.asType))

    /** Like `reachableThisType`, except if that would refer to a class where
      * the `this` cannot be accessed. In that case, fall back to the rawTypeRef
      * of the class. E.g. instead of `A.this.X` where `A.this` is inaccessible,
      * use `A#X`.
      */
    ...
    def hasSkolems(tp: Type): Boolean = tp match
      ...
      case tp: RefinedType =>
        hasSkolems(tp.parent) || hasSkolems(tp.refinedInfo)
      ...
      case tp: LambdaType =>
        tp.paramInfos.exists(hasSkolems) || hasSkolems(tp.resType)
    ...
    def assertNoSkolems(tp: Type): Unit =
      if !this.isSkolem then
        assert(
          !hasSkolems(tp),
          s"assigning type $tp containing skolems to $this"
        )
    ...
    final def copySymDenotation(
      ...
      val initFlags1 =
        (if initFlags != UndefinedFlags then initFlags else this.flags)
      val info1 = if info != null then info else this.info
      if ctx.isAfterTyper && changedClassParents(
          info,
          info1,
          completersMatter = false
        )
      then
        assert(
          ctx.phase.changesParents,
          i"undeclared parent change at ${ctx.phase} for $this, was: $info, now: $info1"
        )
      val privateWithin1 =
        if privateWithin != null then privateWithin else this.privateWithin
      val annotations1 =
        if annotations != null then annotations else this.annotations
      val rawParamss1 =
        if rawParamss != null then rawParamss else this.rawParamss
      val d =
        SymDenotation(symbol, owner, name, initFlags1, info1, privateWithin1)
      d.annotations = annotations1
      d.rawParamss = rawParamss1
      d.registeredCompanion = registeredCompanion
      d
    end copySymDenotation

    /** Copy mamberNames and baseData caches from given denotation, provided
      * they are valid at given `phase`.
      */
    def copyCaches(from: SymDenotation, phase: Phase)(using
        Context
    ): this.type = this

    /** Are `info1` and `info2` ClassInfo types with different parents?
      * @param completersMatter
      *   if `true`, consider parents changed if `info1` or `info2 `is a type
      *   completer
      */
    protected def changedClassParents(
        info1: Type | Null,
        info2: Type | Null,
        completersMatter: Boolean
    ): Boolean =
      info2 match
        case info2: ClassInfo =>
          info1 match
            case info1: ClassInfo =>
              info1.declaredParents ne info2.declaredParents
            case _ => completersMatter
        case _ => completersMatter

    override def initial: SymDenotation = super.initial.asSymDenotation

    /** Install this denotation as the result of the given denotation
      * transformer.
      */
    ...
    /** Apply a transformation `f` to all denotations in this group that start
      * at or after given phase. Denotations are replaced while keeping the same
      * validity periods.
      */
    override def transformAfter(
        phase: DenotTransformer,
        f: SymDenotation => SymDenotation
    )(using Context): Unit =
      super.transformAfter(phase, f)

    /** Set flag `flags` in current phase and in all phases that follow */
    def setFlagFrom(phase: DenotTransformer, flags: FlagSet)(using
        Context
    ): Unit =
      setFlag(flags)
      transformAfter(
    ...
    /** If denotation is private, remove the Private flag and expand the name if
      * necessary
      */
    ...
    /** If this is a sealed class, its known children in the order of textual
      * occurrence
      */
    def children(using Context): List[Symbol] =
      ...
      def completeChildrenIn(owner: Symbol)(using Context) =
        ...
        def maybeChild(sym: Symbol) =
          (sym.isClass && !this.is(JavaDefined) || sym.originDenotation.is(
            EnumVal
          ))
            && (!owner.is(Package)
              || sym.originDenotation.infoOrCompleter.match
                case _: SymbolLoaders.SecondCompleter =>
                  sym.associatedFile == this.symbol.associatedFile
                case _ => false
            )
    ...
    /** Recursively assemble all children of this symbol, Preserves order of
      * insertion.
      */
    final def sealedStrictDescendants(using Context): List[Symbol] =
      ...
      def findLvlN(
        ...
        if explore.isEmpty then acc.toList
        else
          ...
          if notSeen.isEmpty then findLvlN(explore1, seen, acc)
          else
            findLvlN(
              explore1 ++= notSeen,
              { seen ++= notSeen; seen },
      ...
      /** Scans through `explore` to see if there are recursive children. If a
        * symbol in `explore` has children that are not contained in `lvl1`,
        * fallback to `findLvlN`, or else return `lvl1`.
        */
      @tailrec
      def findLvl2(
          lvl1: List[Symbol],
          explore: List[Symbol],
          seenOrNull: util.HashSet[Symbol] | Null
        ...
        case sym :: explore1 =>
          ...
          if lvl2.isEmpty then // no children, scan rest of explore1
            ...
            if notSeen.isEmpty
            then // we found children, but we had already seen them, scan the rest of explore1
    ...
    final def sealedDescendants(using Context): List[Symbol] =
      this.symbol :: sealedStrictDescendants
  end SymDenotation
  ...
  class ClassDenotation private[SymDenotations] (
    ...
    private var fullNameCache: SimpleIdentityMap[QualifiedNameKind, Name] =
    ...
    private def memberCache(using Context): EqHashMap[Name, PreDenotation] =
      if myMemberCachePeriod != ctx.period then
    ...
    override def copyCaches(from: SymDenotation, phase: Phase)(using
        Context
    ): this.type =
      from match
        case from: ClassDenotation =>
          if from.memberNamesCache.isValidAt(phase) then
            memberNamesCache = from.memberNamesCache
          if from.baseDataCache.isValidAt(phase) then
    ...
    if initFlags.is(Module, butNot = Package) then
      assert(
        name.is(ModuleClassName),
        s"module naming inconsistency: ${name.debugString}"
      )
    ...
    /** The type parameters in this class, in the order they appear in the
      * current scope `decls`. This might be temporarily the incorrect order
      * when reading Scala2 pickled info. The problem is fixed by
      * `ensureTypeParamsInCorrectOrder`, which is called once an unpickled
      * symbol has been completed.
      */
    private def typeParamsFromDecls(using Context) =
      unforcedDecls
        .filter(sym => sym.is(TypeParam) && sym.owner == symbol)
        .asInstanceOf[List[TypeSymbol]]
    ...
    override protected[dotc] final def info_=(tp: Type): Unit =
      if changedClassParents(infoOrCompleter, tp, completersMatter = true) then
        invalidateBaseDataCache()
      invalidateMemberNamesCache()
      myTypeParams =
        null // changing the info might change decls, and with it typeParams
      super.info_=(tp)
    ...
    /** The this-type depends on the kind of class:
      ...
      *   - for all other classes `c` with owner `o`:
      *     ThisType(TypeRef(o.thisType, c))
      */
    override def thisType(using Context): Type =
      if myThisType == null then myThisType = computeThisType
      myThisType.nn

    private def computeThisType(using Context): Type =
      val cls = symbol.asType
      val pre = if this.is(Package) then NoPrefix else owner.thisType
      ThisType.raw(TypeRef(pre, cls))

    private var myTypeRef: TypeRef | Null = null

    override def typeRef(using Context): TypeRef =
      if myTypeRef == null then myTypeRef = super.typeRef
      myTypeRef.nn

    override def appliedRef(using Context): Type = classInfo.appliedRef

    private def baseData(implicit
        onBehalf: BaseData,
        ctx: Context
    ): (List[ClassSymbol], BaseClassSet) =
      if !baseDataCache.isValid then baseDataCache = BaseData.newCache()
      baseDataCache(this)

    /** The base classes of this class in linearization order, with the class
      * itself as first element.
      */
    def baseClasses(implicit
        onBehalf: BaseData,
        ctx: Context
    ): List[ClassSymbol] =
      baseData._1
    ...
    private def baseClassSet(implicit
        onBehalf: BaseData,
        ctx: Context
    ): BaseClassSet =
      baseData._2

    def computeBaseData(implicit
        onBehalf: BaseData,
        ctx: Context
    ): (List[ClassSymbol], BaseClassSet) =
      def emptyParentsExpected =
        is(
          Package
        ) || (symbol == defn.AnyClass) || ctx.erasedTypes && (symbol == defn.ObjectClass)
      val parents = parentTypes
      if parents.isEmpty && !emptyParentsExpected then
      ...
      def traverse(parents: List[Type]): Unit = parents match
        case p :: parents1 =>
          p.classSymbol match
            ...
            case _ =>
              assert(
                isRefinementClass || p.isError || ctx.mode.is(Mode.Interactive),
    ...
    /** Is it possible that a class inherits both `this` and `that`?
      *
      * @note
      *   The test is based on single-class inheritance and the closed hierarchy
      *   of final classes.
      *
      * @return
    ...
    final def mayHaveCommonChild(that: ClassSymbol)(using Context): Boolean =
      !this.is(Final) && !that.is(Final) && (this.is(Trait) || that.is(
        Trait
      )) ||
    ...
    protected def proceedWithEnter(sym: Symbol, mscope: MutableScope)(using
        Context
    ): Boolean = true
    ...
    /** Enter a symbol in current scope, and future scopes of same denotation.
      * Note: We require that this does not happen after the first time someone
      * does a findMember on a subclass.
      * @param scope
      *   The scope in which symbol should be entered. If this is EmptyScope,
      *   the scope is `decls`.
      */
    def enter(sym: Symbol, scope: Scope = EmptyScope)(using Context): Unit =
      val mscope = scope match
        ...
        case _                   => unforcedDecls.openForMutations
      if proceedWithEnter(sym, mscope) then
        enterNoReplace(sym, mscope)
        val nxt = this.nextInRun
        if nxt.validFor.code > this.validFor.code then
          this.nextInRun.asSymDenotation.asClass.enter(sym)

    /** Enter a symbol in given `scope` without potentially replacing the old
      * copy.
      */
    ...
    /** Replace symbol `prev` (if defined in current class) by symbol
      * `replacement`. If `prev` is not defined in current class, do nothing.
      * @pre
      *   `prev` and `replacement` have the same name.
      */
    def replace(prev: Symbol, replacement: Symbol)(using Context): Unit =
      unforcedDecls.openForMutations.replace(prev, replacement)
      if myMemberCache != null then
        myMemberCache.uncheckedNN.remove(replacement.name)

    /** Delete symbol from current scope. Note: We require that this does not
      * happen after the first time someone does a findMember on a subclass.
      */
    def delete(sym: Symbol)(using Context): Unit =
      ...
      if myMemberCache != null then myMemberCache.uncheckedNN.remove(sym.name)
      if !sym.flagsUNSAFE.is(Private) then invalidateMemberNamesCache()

    /** Make sure the type parameters of this class appear in the order given by
      * `typeParams` in the scope of the class. Reorder definitions in scope if
      * necessary.
      */
    def ensureTypeParamsInCorrectOrder()(using Context): Unit =
      val tparams = typeParams
      if !ctx.erasedTypes && !typeParamsFromDecls.corresponds(tparams)(
          _.name == _.name
        )
      then
        val decls = info.decls
        val decls1 = newScope
        for tparam <- typeParams do decls1.enter(decls.lookup(tparam.name))
        for sym <- decls do if !tparams.contains(sym) then decls1.enter(sym)
        info = classInfo.derivedClassInfo(decls = decls1)
        myTypeParams = null

    /** All members of this class that have the given name. The elements of the
      * returned pre-denotation all have existing symbols.
      */
    final def membersNamed(name: Name)(using Context): PreDenotation =
      ...
      if Config.cacheMembersNamed then
        ...
        if denots == null then
          ...
          assert(
            denots.exists == denots1.exists,
            s"cache inconsistency: cached: $denots, computed $denots1, name = $name, owner = $this"
          )
        denots
      else computeMembersNamed(name)

    /** All non-private members of this class that have the given name. The
      * elements of the returned pre-denotation all have existing symbols.
      */
    ...
    /** All members of this class that have the given name and match the
      * `required` and `excluded` flag sets; members excluded based on the flag
      * sets do not shadow inherited members that would not be excluded.
      *
      * The elements of the returned pre-denotation all have existing symbols.
      */
    final def membersNamedNoShadowingBasedOnFlags(
        name: Name,
        required: FlagSet = EmptyFlags,
        excluded: FlagSet = EmptyFlags
    )(using Context): PreDenotation =
    ...
    private[core] def computeMembersNamed(name: Name)(using
        Context
    ): PreDenotation =
      Stats.record("computeMembersNamed")
      val ownDenots = info.decls.denotsNamed(name)
      if debugTrace then println(s"$this.member($name), ownDenots = $ownDenots")
      addInherited(name, ownDenots)

    private def addInherited(
        name: Name,
        ownDenots: PreDenotation,
        required: FlagSet = EmptyFlags,
        excluded: FlagSet = EmptyFlags
    )(using Context): PreDenotation =
      def collect(denots: PreDenotation, parents: List[Type]): PreDenotation =
        parents match
          case p :: ps =>
            val denots1 = collect(denots, ps)
            p.classSymbol.denot match
              case parentd: ClassDenotation =>
                val inherited = parentd.membersNamedNoShadowingBasedOnFlags(
                  name,
                  required,
                  excluded | Private
                )
                denots1.union(
                  inherited.mapInherited(ownDenots, denots1, thisType)
                )
              case _ =>
                denots1
          case nil => denots
    ...
    override final def findMember(
      ...
      val raw =
        if excluded.is(Private) then nonPrivateMembersNamed(name)
        else membersNamed(name)
      val pre1 = pre match
        case pre: OrType => pre.widenUnion
        case _           => pre
      raw.filterWithFlags(required, excluded).asSeenFrom(pre1).toDenot(pre1)

    final def findMemberNoShadowingBasedOnFlags(
        name: Name,
        pre: Type,
        required: FlagSet = EmptyFlags,
        excluded: FlagSet = EmptyFlags
    )(using Context): Denotation =
      membersNamedNoShadowingBasedOnFlags(name, required, excluded)
        .asSeenFrom(pre)
        .toDenot(pre)
    ...
    final def baseTypeOf(tp: Type)(using Context): Type =
      ...
      def record(tp: CachedType, baseTp: Type) =
        if Stats.monitored then
          Stats.record("basetype cache entries")
          if !baseTp.exists then Stats.record("basetype cache NoTypes")
        if !tp.isProvisional && !CapturingType.isUncachable(tp) then
          btrCache(tp) = baseTp
        else btrCache.remove(tp) // Remove any potential sentinel value

      def ensureAcyclic(baseTp: Type) =
        if baseTp `eq` NoPrefix then throw CyclicReference(this)
        baseTp
      ...
      def recur(tp: Type): Type = try
        tp match
          case tp: CachedType =>
            ...
            if baseTp != null then return ensureAcyclic(baseTp)
          case _ =>
        if Stats.monitored then
          Stats.record("computeBaseType, total")
          Stats.record(s"computeBaseType, ${tp.getClass}")
        val normed = tp.tryNormalize
        if normed.exists then return recur(normed)
            ...
            def computeTypeRef =
              ...
              tpSym.denot match
                case clsd: ClassDenotation =>
                  ...
                  val baseTp =
                    if tpSym eq symbol then tp
                      ...
                      if clsd.baseClassSet.contains(symbol) then
                        if symbol.isStatic && symbol.typeParams.isEmpty then
                          symbol.typeRef
                ...
                case _ =>
                  ...
                  if inCache(superTp) then record(tp, baseTp)
            ...
            def computeAndOrType: Type =
              ...
              if !tp.isAnd then
                ...
                if tp2.isBottomType && (tp2 frozen_<:< tp1) then
                  return recur(tp1)
              val baseTp =
                if symbol.isStatic && tp.derivesFrom(
                    symbol
                ...
                then symbol.typeRef
                  ...
                  val combined =
                    if tp.isAnd then baseTp1 & baseTp2 else baseTp1 | baseTp2
                  combined match
                    case combined: AndOrType
                        if (combined.tp1 eq tp1) && (combined.tp2 eq tp2) && (combined.isAnd == tp.isAnd) =>
                      tp
                    case _ => combined

              if baseTp.exists && inCache(tp1) && inCache(tp2) then
                record(tp, baseTp)
              baseTp
    ...
    def memberNames(
      ...
      if this.is(PackageClass) || !Config.cacheMemberNames then
        computeMemberNames(
          keepOnly
    ...
    def computeMemberNames(
      ...
      var names = Set[Name]()
      def maybeAdd(name: Name) = if keepOnly(thisType, name) then names += name
      try
        for ptype <- parentTypes do
          ptype.classSymbol match
            case pcls: ClassSymbol =>
              for name <- pcls.memberNames(keepOnly) do maybeAdd(name)
        val ownSyms =
          if keepOnly eq implicitFilter then
            if this.is(Package) then Iterator.empty
        ...
        for sym <- ownSyms do maybeAdd(sym.name)
        names
      catch
        case ex: Throwable =>
          handleRecursive("member names", i"of $this", ex)

    override final def fullNameSeparated(kind: QualifiedNameKind)(using
        Context
    ): Name =
    ...
    override def primaryConstructor(using Context): Symbol =
      def constrNamed(cname: TermName) =
        info.decls.denotsNamed(cname).last.symbol
      // denotsNamed returns Symbols in reverse order of occurrence
      if this.is(Package) then NoSymbol
      else
        constrNamed(nme.CONSTRUCTOR).orElse(constrNamed(nme.TRAIT_CONSTRUCTOR))

    /** The term parameter accessors of this class. Both getters and setters are
    ...
    /** If this class has the same `decls` scope reference in `phase` and
      * `phase.next`, install a new denotation with a cloned scope in
      * `phase.next`.
      */
    def ensureFreshScopeAfter(phase: DenotTransformer)(using Context): Unit =
      if ctx.phaseId != phase.next.id then
        ...
        if prevClassInfo.decls eq decls then
          copySymDenotation(info =
            ClassInfo(pre, classSymbol, ps, decls.cloneScope, selfInfo)
          )
    ...
    override def registerCompanion(companion: Symbol)(using Context) =
      if companion.isClass && !isAbsent(canForce = false) && !companion
          .isAbsent(canForce = false)
      then myCompanion = companion

    private[core] def unforcedRegisteredCompanion: Symbol = myCompanion

    override def registeredCompanion(using Context) =
      if !myCompanion.exists then ensureCompleted()
      myCompanion
  ...
  /** The denotation of a package class. It overrides ClassDenotation to take
    * account of package objects when looking for members
    */
  final class PackageClassDenotation private[SymDenotations] (
    ...
    def packageObjs(using Context): List[ClassDenotation] =
      if packageObjsRunId != ctx.runId then
        packageObjsRunId = ctx.runId
        packageObjsCache =
          Nil // break cycle in case we are looking for package object itself
        packageObjsCache =
          val pkgObjBuf = new mutable.ListBuffer[ClassDenotation]
          for sym <- info.decls
          do // don't use filter, since that loads classes with `$`s in their name
            val denot =
              sym.lastKnownDenotation // don't use `sym.denot`, as this brings forward classes too early
            if denot.isType && denot.name.isPackageObjectName then
    ...
    def packageObjFor(sym: Symbol)(using Context): Symbol =
      ...
      if owner.is(Package) then NoSymbol
      else if owner.isPackageObject then owner.sourceModule
    ...
    /** Looks in both the package object and the package for members. The
      * precise algorithm is as follows:
      *
      * If this is the scala package look in the package first, and if nothing
      * is found there, look in the package object second. Otherwise, look in
      * the both the package object and the package and form a union of the
      * results.
      *
      * The reason for the special treatment of the scala package is that if we
      * complete it too early, we freeze its superclass Any, so that no members
      * can be entered in it. As a consequence, there should be no entry in the
      * scala package object that hides a class or object in the scala package
      * of the same name, because the behavior would then be unintuitive for
      * such members.
      */
    override def computeMembersNamed(name: Name)(using Context): PreDenotation =

      def recur(
          pobjs: List[ClassDenotation],
          acc: PreDenotation
      ): PreDenotation = pobjs match
        case pcls :: pobjs1 =>
          if pcls.isCompleting then recur(pobjs1, acc)
          else
            val pobjMembers =
              pcls.nonPrivateMembersNamed(name).filterWithPredicate { d =>
                !defn.topClasses
                  .contains(d.symbol.maybeOwner) // Drop members of top classes
              }
        ...
        case nil =>
          ...
          if !acc.exists then directMembers
            ...
            acc.union(
              directMembers.filterWithPredicate(!_.symbol.isAbsent())
            ) match
              case d: DenotUnion => dropStale(d)
              case d             => d

      /** Filter symbols making up a DenotUnion to remove alternatives from
        * stale classfiles. This proceeds as follow:
        *
        *   - prefer alternatives that are currently compiled over ones that
        *     have been compiled before.
        *   - if no alternative is compiled now, and they all come from the same
        *     file, keep all of them
        *   - if no alternative is compiled now, and they come from different
        *     files, keep the ones from the youngest file, but issue a warning
        *     that one of the class files should be removed from the classpath.
        */
      def dropStale(multi: DenotUnion): PreDenotation =
        ...
        if compiledNow.exists then compiledNow
          ...
          val assocFiles =
            multi.aggregate(d => Set(d.symbol.associatedFile.nn), _ union _)
          if assocFiles.size == 1 then
            ...
            if youngest.size > 1 then
              throw TypeError(
                em"""${ambiguousFilesMsg(youngest.tail.head)}
    ...
    override def memberNames(
        keepOnly: NameFilter
    )(implicit onBehalf: MemberNames, ctx: Context): Set[Name] =
      def recur(pobjs: List[ClassDenotation], acc: Set[Name]): Set[Name] =
        pobjs match
          case pcls :: pobjs1 =>
    ...
    /** If another symbol with the same name is entered, unlink it. If symbol is
      * a package object, invalidate the packageObj cache.
      * @return
      *   `sym` is not already entered
      */
    override def proceedWithEnter(sym: Symbol, mscope: MutableScope)(using
        Context
    ): Boolean =
      val entry = mscope.lookupEntry(sym.name)
      if entry != null then
        if entry.sym == sym then return false
        mscope.unlink(entry)
      if sym.name.isPackageObjectName then packageObjsRunId = NoRunId
      true
    ...
    def unlinkFromFile(file: AbstractFile)(using Context): Unit =
      ...
      for sym <- scope.toList.iterator do
        ...
        then scope.unlink(sym, sym.lastKnownDenotation.name)
  ...
  @sharable object NoDenotation
    ...
    override def computeAsSeenFrom(pre: Type)(using Context): SingleDenotation =
      this
    override def mapInfo(f: Type => Type)(using Context): SingleDenotation =
      this
    override def asSeenFrom(pre: Type)(using Context): AsSeenFromResult = this

    override def matches(other: SingleDenotation)(using Context): Boolean =
      false
    override def targetName(using Context): Name = EmptyTermName
    override def mapInherited(
        ownDenots: PreDenotation,
        prevDenots: PreDenotation,
        pre: Type
    )(using Context): SingleDenotation = this
    override def filterWithPredicate(
        p: SingleDenotation => Boolean
    ): SingleDenotation = this
    override def filterDisjoint(denots: PreDenotation)(using
        Context
    ): SingleDenotation = this
    override def filterWithFlags(required: FlagSet, excluded: FlagSet)(using
        Context
    ): SingleDenotation = this
  ...
  /** Can a private symbol with given name and flags be inferred to be local, if
    * all references to such symbols are via `this`? This holds for all symbols
    * except
    *   - constructors, since they can never be referred to as members of their
    *     own, fully elaborated `this`.
    *   - parameters and parameter accessors, since their Local status is
    *     already determined by whether they have a `val` or `var` or not.
    */
  def canBeLocal(name: Name, flags: FlagSet)(using Context) =
    !name.isConstructorName && !flags.is(Param) && !flags.is(ParamAccessor)

  /** Factory method for SymDenotion creation. All creations should be done via
    * this method.
    */
  def SymDenotation(
      symbol: Symbol,
  ...
  def stillValid(denot: SymDenotation)(using Context): Boolean =
    ...
    then true
      ...
      if firstPhaseId > ctx.lastPhaseId then false
  ...
  private[SymDenotations] def stillValidInOwner(denot: SymDenotation)(using
    ...
    && (!owner.isClass
      ...
      || (denot.is(Synthetic) && denot.is(ModuleClass) && stillValidInOwner(
        denot.companionClass
      ))
      || denot.isSelfSym
      || denot.isLocalDummy)
  ...
  def traceInvalid(denot: Denotation)(using Context): Boolean =
    ...
    denot match
      case denot: SymDenotation =>
        def explainSym(msg: String) = explain(
          s"$msg\ndefined = ${denot.definedPeriodsString}"
        )
        if denot.isOneOf(ValidForeverFlags) || denot.isRefinementClass then true
        else
          val initial = denot.initial
          if (initial ne denot) || ctx.phaseId != initial.validFor.firstPhaseId
          then atPhase(initial.validFor.firstPhaseId)(traceInvalid(initial))
          else
            try
              val owner = denot.owner.denot
              if !traceInvalid(owner) then explainSym("owner is invalid")
              else if !owner.isClass || owner.isRefinementClass || denot.isSelfSym
              then true
              else if owner.unforcedDecls.lookupAll(
                  denot.name
                ) contains denot.symbol
              then true
              else
                explainSym(
                  s"decls of ${show(owner)} are ${owner.unforcedDecls.lookupAll(denot.name).toList}, do not contain ${denot.symbol}"
                )
            catch case ex: StaleSymbol => explainSym(s"$ex was thrown")
      case _ =>
        explain("denotation is not a SymDenotation")
  end traceInvalid

  /** Configurable: Accept stale symbol with warning if in IDE Always accept
    * stale symbols when testing pickling.
    */
...
// ---- Completion --------------------------------------------------------
  ...
  /** Instances of LazyType are carried by uncompleted symbols. Note: LazyTypes
    * double up as (constant) functions from Symbol and from (TermSymbol,
    * ClassSymbol) to LazyType. That way lazy types can be directly passed to
    * symbol creation methods in Symbols that demand instances of these function
    * types.
    */
  abstract class LazyType
    ...
    /** The type parameters computed by the completer before completion has
      * finished
      */
    def completerTypeParams(sym: Symbol)(using Context): List[TypeParamInfo] =
      if sym.is(Touched) then
        Nil // return `Nil` instead of throwing a cyclic reference
    ...
    def withDecls(decls: Scope): this.type =
      myDecls = decls; this
    def withSourceModule(sourceModuleFn: Context ?=> Symbol): this.type =
      mySourceModuleFn = sourceModuleFn; this
    def withModuleClass(moduleClassFn: Context ?=> Symbol): this.type =
      myModuleClassFn = moduleClassFn; this

    override def toString: String = getClass.toString

    /** A hook that is called before trying to complete a symbol with its
      * associated cycle detection via the Touched flag. This is overridden for
      * Type definitions in Namer, where we make sure that owners are completed
      * before nested types.
      */
  ...
  /** A subtrait of LazyTypes where completerTypeParams yields a
    * List[TypeSymbol], which should be completed independently of the info.
    */
  trait TypeParamsCompleter extends LazyType:
    override def completerTypeParams(
        sym: Symbol
    )(using Context): List[TypeSymbol] =
      unsupported(
        "completerTypeParams"
      ) // should be abstract, but Scala-2 will then compute the wrong type for it

  /** A missing completer */
  trait NoCompleter extends LazyType:
    def complete(denot: SymDenotation)(using Context): Unit = unsupported(
      "complete"
    )

  @sharable object NoCompleter extends NoCompleter

  /** A lazy type for modules that points to the module class. Needed so that
    * `moduleClass` works before completion. Completion of modules is always
    * completion of the underlying module class, followed by copying the
    * relevant fields to the module.
    */
  ...
  class StubInfo() extends LazyType:
    ...
    def initializeToDefaults(denot: SymDenotation, errMsg: Message)(using
        Context
    ): Unit =
    ...
    def complete(denot: SymDenotation)(using Context): Unit =
      ...
      if ctx.debug then throw new scala.Error()
  ...
  /** A cache for baseclasses, as a sequence in linearization order and as a set
    * that can be queried efficiently for containment.
    */
  trait BaseData extends InheritedCache:
    def apply(clsd: ClassDenotation)(implicit
        onBehalf: BaseData,
        ctx: Context
    ): (List[ClassSymbol], BaseClassSet)
  ...
  object BaseData:
    implicit val None: BaseData = new InvalidCache with BaseData:
      def apply(
          clsd: ClassDenotation
      )(implicit onBehalf: BaseData, ctx: Context) = ???
      def signalProvisional() = ()
    def newCache()(using Context): BaseData = new BaseDataImpl(ctx.period)

  private abstract class InheritedCacheImpl(val createdAt: Period)
      extends InheritedCache:
    ...
    protected def invalidateDependents() =
      ...
      if dependent != null then
        val it = dependent.keySet.iterator()
        while it.hasNext() do it.next().invalidate()
      dependent = null

    protected def addDependent(dep: InheritedCache) =
      if dependent == null then dependent = new WeakHashMap
  ...
  private class MemberNamesImpl(createdAt: Period)
      extends InheritedCacheImpl(createdAt)
      with MemberNames:
    private var cache: SimpleIdentityMap[NameFilter, Set[Name]] | Null =
      SimpleIdentityMap.empty
    ...
    /** Computing parent member names might force parents, which could
      * invalidate the cache itself. In that case we should cancel invalidation
      * and proceed as usual. However, all cache entries should be cleared.
      */
    def invalidate(): Unit =
      if cache != null then
    ...
    def sameGroup(p1: Phase, p2: Phase) =
      p1.sameMembersStartId == p2.sameMembersStartId
  end MemberNamesImpl

  private class BaseDataImpl(createdAt: Period)
      extends InheritedCacheImpl(createdAt)
      with BaseData:
    ...
    def sameGroup(p1: Phase, p2: Phase) =
      p1.sameParentsStartId == p2.sameParentsStartId
  end BaseDataImpl
  ...
  class BaseClassSet(val classIds: Array[Int]) extends AnyVal:
    def contains(sym: Symbol, limit: Int): Boolean =
      val id = sym.id
      var i = 0
      while i < limit && classIds(i) != id do i += 1
      i < limit && {
        if i > 0 then
          val t = classIds(i)
  ...
  class BaseDataBuilder:
    ...
    private def add(sym: Symbol): Unit =
      if length == classIds.length then resize(length * 2)
    ...
    def addAll(bcs: List[ClassSymbol]): this.type =
      ...
      bcs match
        case bc :: bcs1 =>
          addAll(bcs1)
          if !new BaseClassSet(classIds).contains(bc, len) then
            add(bc)
            classes = bc :: classes
        case nil =>
      this
    ...
    def baseClassSet: BaseClassSet =
      if length != classIds.length then resize(length)
