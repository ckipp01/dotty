...
object Names:
  ...
  /** A name is either a term name or a type name. Term names can be simple or
    * derived. A simple term name is essentially an interned string stored in a
    * name table. A derived term name adds a tag, and possibly a number or a
    * further simple name to some other name.
    */
  abstract class Name extends Designator, Showable derives CanEqual:
    ...
    /** This name converted to a simple term name and in addition with all
      * symbolic operator characters expanded.
      */
    ...
    /** Apply rewrite rule given by `f` to some part of this name, skipping and
      * rewrapping other decorators. Stops at DerivedNames with infos of kind
      * QualifiedInfo. If `f` does not apply to any part, return name unchanged.
      */
    def replace(f: PartialFunction[Name, Name]): ThisName

    /** Same as replace, but does not stop at DerivedNames with infos of kind
      * QualifiedInfo.
      */
    def replaceDeep(f: PartialFunction[Name, Name]): ThisName =
      replace(f.orElse { case DerivedName(underlying, info: QualifiedInfo) =>
        underlying.replaceDeep(f).derived(info)
      })

    /** If partial function `f` is defined for some part of this name, apply it
      * in a Some, otherwise None. Stops at derived names whose kind has
      * `definesNewName = true`.
      */
    ...
    /** A name in the same (term or type) namespace as this name and with same
    ...
    /** A derived name consisting of this name and the added info, unless it is
      ...
      * @pre
      *   This name does not have a different info of the same kind as `info`.
      */
    ...
    /** This name without any info of the given `kind`. Excepted, as always, is
      * the underlying name part of a qualified name.
      */
    ...
    /** A string showing the internal structure of this name. By contrast,
    ...
    /** Does (the first part of) this name starting at index `start` starts with
      * `str`?
      */
    def startsWith(str: String, start: Int = 0): Boolean =
      firstPart.startsWith(str, start)
  ...
  abstract class TermName extends Name:
    ...
    override def asTypeName: Nothing = throw new ClassCastException(
      s"$this is not a type name"
    )
    ...
    private def rewrap(underlying: TermName) =
      if underlying eq this.underlying then this else underlying.add(info)
    ...
    override def exclude(kind: NameKind): TermName =
      val thisKind = this.info.kind
      if shadows(thisKind, kind) then this
      else if thisKind.tag > kind.tag then rewrap(underlying.exclude(kind))
      else underlying
    ...
    final def mangled: ThisName =
      if myMangled == null then myMangled = mangle
      myMangled.asInstanceOf[ThisName]

    final def mangledString: String =
      if myMangledString == null then
        myMangledString = qualToString(_.mangledString, _.mangled.toString)
      myMangledString.nn

    /** If this a qualified name, split it into underlying, last part, and
      * separator Otherwise return an empty name, the name itself, and "")
      */
    def split: (TermName, TermName, String)

    /** Convert to string as follows. If this is a qualified name `<first> <sep>
      * <last>`, the sanitized version of `f1(<first>) <sep> f2(<last>)`.
      * Otherwise `f2` applied to this name.
      */
    def qualToString(f1: TermName => String, f2: TermName => String): String =
      val (first, last, sep) = split
      if first.isEmpty then f2(last)
      else str.sanitize(f1(first) + sep + f2(last))
  ...
  final class SimpleName(val start: Int, val length: Int) extends TermName:
    ...
    def exists(p: Char => Boolean): Boolean =
      var i = 0
      while i < length && !p(chrs(start + i)) do i += 1
      i < length
    ...
    def contains(ch: Char): Boolean =
      var i = 0
      while i < length && chrs(start + i) != ch do i += 1
      i < length
    ...
    def sliceToString(from: Int, end: Int): String =
      if end <= from then "" else new String(chrs, start + from, end - from)

    def head: Char = apply(0)
    def last: Char = apply(length - 1)

    /** Copy character slice (from until end) to character array starting at
      * `dstStart`.
      * @pre
      *   Destination must have enough space to hold all characters of this
      *   name.
      */
    ...
    override def replace(f: PartialFunction[Name, Name]): ThisName =
      if f.isDefinedAt(this) then likeSpaced(f(this)) else this
    ...
    override def encode: SimpleName =
      ...
      if dontEncode then this else NameTransformer.encode(this)
    ...
    override def startsWith(str: String, start: Int): Boolean =
      var i = 0
      while i < str.length && start + i < length && apply(start + i) == str(i)
      do i += 1
      i == str.length

    override def endsWith(suffix: String): Boolean =
      var i = 1
      while i <= suffix.length && i <= length && apply(length - i) == suffix(
          suffix.length - i
        )
      do i += 1
      i > suffix.length
    ...
    override def replace(from: Char, to: Char): SimpleName =
      ...
      for i <- 0 until length do if cs(i) == from then cs(i) = to
  ...
  final class TypeName(val toTermName: TermName) extends Name:
    ...
    override def asTermName: Nothing = throw new ClassCastException(
      s"$this is not a term name"
    )
  ...
  /** A term name that's derived from an `underlying` name and that adds `info`
    * to it.
    */
  final case class DerivedName(
      override val underlying: TermName,
      override val info: NameInfo
  ) extends TermName:
    ...
    override def replace(f: PartialFunction[Name, Name]): ThisName =
      if f.isDefinedAt(this) then likeSpaced(f(this))
      else
        info match
    ...
    override def collect[T](f: PartialFunction[Name, T]): Option[T] =
      if f.isDefinedAt(this) then Some(f(this))
      else
        info match
    ...
    override def split: (TermName, TermName, String) = info match
      case info: QualifiedInfo =>
        (
          underlying,
          info.name,
          info.kind.asInstanceOf[QualifiedNameKind].separator
        )
      case _ =>
    ...
    override def encode: ThisName =
      underlying.encode.derived(info.map(_.encode))
    override def decode: ThisName =
      underlying.decode.derived(info.map(_.decode))
  ...
  private class NameTable
      extends HashSet[SimpleName](
        initialCapacity = 0x10000,
        capacityMultiple = 2
      ):
    import util.Stats

    override def hash(x: SimpleName) =
      hashValue(chrs, x.start, x.length) // needed for resize
    ...
    def enterIfNew(cs: Array[Char], offset: Int, len: Int): SimpleName =
      ...
      while name != null do
        if name.nn.length == len && Names.equals(name.nn.start, cs, offset, len)
        then return name.nn
  ...
  private def equals(
    ...
    var i = 0
    while (i < len) && (chrs(index + i) == cs(offset + i)) do i += 1
    i == len

  /** Create a term name from the characters in cs[offset..offset+len-1]. Assume
    * they are already encoded.
    */
  def termName(cs: Array[Char], offset: Int, len: Int): SimpleName =
    nameTable.enterIfNew(cs, offset, len)

  /** Create a type name from the characters in cs[offset..offset+len-1]. Assume
    * they are already encoded.
    */
  def typeName(cs: Array[Char], offset: Int, len: Int): TypeName =
    termName(cs, offset, len).toTypeName

  /** Create a term name from the UTF8 encoded bytes in
    * bs[offset..offset+len-1]. Assume they are already encoded.
    */
  def termName(bs: Array[Byte], offset: Int, len: Int): SimpleName =
    val chars = Codec.fromUTF8(bs, offset, len)
    termName(chars, 0, chars.length)

  /** Create a type name from the UTF8 encoded bytes in
    * bs[offset..offset+len-1]. Assume they are already encoded.
    */
  def typeName(bs: Array[Byte], offset: Int, len: Int): TypeName =
    termName(bs, offset, len).toTypeName

  /** Create a term name from a string. See `sliceToTermName` in `Decorators`
    * for a more efficient version which however requires a Context for its
    * operation.
    */
  ...
  implicit val NameOrdering: Ordering[Name] = new Ordering[Name]:
    private def compareInfos(x: NameInfo, y: NameInfo): Int =
      if x.kind.tag != y.kind.tag then x.kind.tag - y.kind.tag
      else
        x match
    ...
    private def compareSimpleNames(x: SimpleName, y: SimpleName): Int =
      ...
      var i = 0
      while i < until && x(i) == y(i) do i = i + 1
      if i < until then
        if x(i) < y(i) then -1
        else /*(x(i) > y(i))*/ 1
      ...
      case DerivedName(xPre, xInfo) =>
        y match
          case DerivedName(yPre, yInfo) =>
            ...
            if s == 0 then compareTermNames(xPre, yPre) else s
          case _ => 1
    def compare(x: Name, y: Name): Int =
      if x.isTermName && y.isTypeName then 1
      else if x.isTypeName && y.isTermName then -1
      else if x eq y then 0
