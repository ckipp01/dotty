...
object DenotTransformers:
  ...
  /** A transformer group contains a sequence of transformers, ordered by the
    * phase where they apply. Transformers are added to a group via `install`.
    */
  ...
  trait DenotTransformer extends Phase:
    ...
    def lastPhaseId(using Context): Int =
      ctx.base.nextDenotTransformerId(id + 1)
  ...
  trait InfoTransformer extends DenotTransformer:
    ...
    /** Denotations with a symbol where `infoMayChange` is false are guaranteed
      ...
      * This can save time, and more importantly, can help avoid forcing symbol
      * completers.
      */
    protected def infoMayChange(sym: Symbol)(using Context): Boolean = true

  /** A transformer that only transforms SymDenotations. Note: Infos of non-sym
    * denotations are left as is. So the transformer should be used before
    * erasure only if this is not a problem. After erasure, all denotations are
    * SymDenotations, so SymTransformers can be used freely.
    */
  ...
  /** A `DenotTransformer` trait that has the identity as its `transform`
    * method. You might want to inherit from this trait so that new denotations
    * can be installed using `installAfter` and `enteredAfter` at the end of the
    * phase.
    */
