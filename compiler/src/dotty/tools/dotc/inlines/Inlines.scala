...
/** Support for querying inlineable methods and for inlining calls to such
  * methods
  */
object Inlines:
  import tpd.*
  ...
  def bodyToInline(sym: SymDenotation)(using Context): Tree =
    if hasBodyToInline(sym) then sym.getAnnotation(defn.BodyAnnot).get.tree
    else EmptyTree
  ...
  /** Try to inline a call to an inline method. Fail with error if the maximal
    ...
    * @return
    *   An `Inlined` node that refers to the original call and the inlined
    *   bindings and body that replace it.
    */
  def inlineCall(tree: Tree)(using Context): Tree =
    ...
    then
      if tree.symbol == defn.CompiletimeTesting_typeChecks then
        return Intrinsics.typeChecks(tree)
      if tree.symbol == defn.CompiletimeTesting_typeCheckErrors then
        return Intrinsics.typeCheckErrors(tree)
    ...
    if tree.symbol.isConstructor then
      return tree // error already reported for the inline constructor definition

    /** Set the position of all trees logically contained in the expansion of
      * inlined call `call` to the position of `call`. This transform is
      * necessary when lifting bindings from the expansion to the outside of the
      * call.
      */
    ...
    /** Lift bindings around inline call or in its function part to the
      * `bindings` buffer. This is done as an optimization to keep inline call
      * expansions smaller.
      */
    def liftBindings(tree: Tree, liftPos: Tree => Tree): Tree = tree match
      ...
      case Inlined(call, stats, expr) =>
        ...
        cpy.Inlined(tree)(
          call,
          Nil,
          liftBindings(expr, liftFromInlined(call).transform(_))
        )
      case Apply(fn, args) =>
      ...
      case TypeApply(fn, args) =>
        fn.tpe.widenTermRefExpr match
          case tp: PolyType =>
            ...
            for
              case (arg, bounds: TypeBounds) <- args.zip(targBounds)
              if !bounds.contains(arg.tpe)
            do
              val boundsStr =
                if bounds == TypeBounds.empty then
                  " <: Any. Note that this type is higher-kinded."
                else bounds.show
              report.error(
                em"${arg.tpe} does not conform to bound$boundsStr",
                arg
              )
    ...
    val tree2 =
      ...
      else if enclosingInlineds.length < ctx.settings.XmaxInlines.value && !reachedInlinedTreesLimit
      then
        val body =
          try
            bodyToInline(
              tree.symbol
            ) // can typecheck the tree and thereby produce errors
          catch
            case _: MissingInlineInfo =>
              throw CyclicReference(ctx.owner)
        new InlineCall(tree).expand(body)
      else
        ctx.base.stopInlining = true
        val (reason, setting) =
          if reachedInlinedTreesLimit then
            ("inlined trees", ctx.settings.XmaxInlinedTrees)
  ...
  /** Try to inline a pattern with an inline unapply method. Fail with error if
    * the maximal inline depth is exceeded.
    *
    * @param unapp
  ...
  def inlinedUnapply(unapp: tpd.UnApply)(using Context): Tree =
    ...
    var unapplySym1: Symbol =
      NoSymbol // created from within AnonClass() and used afterwards

    val newUnapply = AnonClass(ctx.owner, List(defn.ObjectType), sym.coord) {
      cls =>
        ...
        val unapply = DefDef(
          ...
          argss =>
            ...
            if body.symbol.is(Transparent) then
              inlineCall(body)(using ctx.withOwner(unapplySym))
            else body
        )
  ...
  /** For a retained inline method, another method that keeps track of the body
    * that is kept at runtime. For instance, an inline method
    *
  ...
  def bodyRetainer(mdef: DefDef)(using Context): DefDef =
    ...
    val retainer = meth
      .copy(
        ...
        flags =
          (meth.flags &~ (Inline | Macro | Override | AbsOverride)) | Private,
        coord = mdef.rhs.span.startPos
      )
      .asTerm
      .entered
    retainer.deriveTargetNameAnnotation(
      meth,
      name => BodyRetainerName(name.asTermName)
    )
    DefDef(
      retainer,
      prefss =>
  ...
  def dropInlined(inlined: Inlined)(using Context): Tree =
    ...
    else reposition(tree1, inlined.span)
  ...
  def reposition(tree: Tree, callSpan: Span)(using Context): Tree =
    ...
    val cpyWithNewSource = new TypedTreeCopier:
      ...
      override protected val untpdCpy: untpd.UntypedTreeCopier =
        new untpd.UntypedTreeCopier:
          override protected def sourceFile(tree: untpd.Tree): SourceFile =
            curSource

    /** Removes all Inlined trees, replacing them with blocks. Repositions all
      * trees directly inside an inlined expansion of a non empty call to the
      * position of the call. Any tree directly inside an empty call (inlined in
      * the inlined code) retains their position.
      *
      * Until we implement JSR-45, we cannot represent in output positions in
      * other source files. So, reposition inlined code from other files with
      * the call position.
      */
    class Reposition extends TreeMap(cpyWithNewSource):
      ...
      override def transform(tree: Tree)(using Context): Tree =
        def fixSpan[T <: untpd.Tree](copied: T): T =
          copied.withSpan(
            if tree.source == curSource then tree.span else callSpan
          )
        ...
        inContext(ctx.withSource(curSource)) {
          tree match
            ...
            case tree: JavaSeqLiteral =>
              finalize(
                untpd.JavaSeqLiteral(
                  transform(tree.elems),
                  transform(tree.elemtpt)
                )(curSource)
              )
            ...
            case tree: Bind =>
              finalize(untpd.Bind(tree.name, transform(tree.body))(curSource))
  ...
  /** Leave only a call trace consisting of
    ...
    *   - the call's position in the call field of an Inlined node. The trace
    *     has enough info to completely reconstruct positions. Note: For macros
    *     it returns a Select and for other inline methods it returns an Ident
    *     (this distinction is only temporary to be able to run YCheckPositions)
    */
  def inlineCallTrace(callSym: Symbol, pos: SourcePosition)(using
      Context
  ): Tree =
    assert(ctx.source == pos.source)
    val topLevelCls = callSym.topLevelClass
    if callSym.is(Macro) then
      ref(topLevelCls.owner)
        .select(topLevelCls.name)(using ctx.withOwner(topLevelCls.owner))
        .withSpan(pos.span)
  ...
  private object Intrinsics:
    ...
    private def compileForErrors(tree: Tree)(using
        Context
    ): List[(ErrorKind, Error)] =
      assert(
        tree.symbol == defn.CompiletimeTesting_typeChecks || tree.symbol == defn.CompiletimeTesting_typeCheckErrors
      )
      ...
      val underlyingCodeArg =
        if Inlines.isInlineable(codeArg1.symbol) then
          stripTyped(Inlines.inlineCall(codeArg1))
        else codeArg1
      ...
      ConstFold(underlyingCodeArg).tpe.widenTermRefExpr match
            ...
            if ctx.reporter.allErrors.nonEmpty then
              ...
              ctx.base.postTyperPhase match
                case postTyper: PostTyper if ctx.reporter.allErrors.isEmpty =>
                  val tree4 = atPhase(postTyper) {
                    postTyper.newTransformer.transform(tree3)
                  }
                  ctx.base.inliningPhase match
                    case inlining: Inlining if ctx.reporter.allErrors.isEmpty =>
                      atPhase(inlining) {
                        inlining.newTransformer.transform(tree4)
                      }
                    case _ =>
                case _ =>
        ...
        case t =>
          report.error(
            em"argument to compileError must be a statically known String but was: $codeArg",
            codeArg1.srcPos
          )
          Nil
      end match
    end compileForErrors
    ...
    private def packErrors(errors: List[(ErrorKind, Error)], pos: SrcPos)(using
        Context
    ): Tree =
  ...
  private class InlineCall(call: tpd.Tree)(using Context) extends Inliner(call):
    ...
    def expand(rhsToInline: Tree): Tree =
          ...
          if inlinedMethod == defn.Compiletime_requireConst then
            arg match
              case ConstantValue(_) |
                  Inlined(_, Nil, Typed(ConstantValue(_), _)) => // ok
              case _ =>
                report.error(
                  em"expected a constant value but found: $arg",
                  arg.srcPos
                )
      ...
      if callTypeArgs.length == 1 then
        if inlinedMethod == defn.Compiletime_constValue then
          val constVal = tryConstValue
          if constVal.isEmpty then
            val msg =
              em"not a constant type: ${callTypeArgs.head}; cannot take constValue"
            return ref(defn.Predef_undefined)
              .withSpan(call.span)
              .withType(ErrorType(msg))
          else return constVal
        else if inlinedMethod == defn.Compiletime_constValueOpt then
          val constVal = tryConstValue
          return (
            if constVal.isEmpty then ref(defn.NoneModule.termRef)
            else
              New(
                defn.SomeClass.typeRef.appliedTo(constVal.tpe),
                constVal :: Nil
              )
          )
        else if inlinedMethod == defn.Compiletime_summonInline then
