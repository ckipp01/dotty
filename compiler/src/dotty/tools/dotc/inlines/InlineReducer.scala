...
class InlineReducer(inliner: Inliner)(using Context):
  ...
  extension (tp: Type)
    /** same as widenTermRefExpr, but preserves modules and singleton enum
      * values
      */
    private def widenInlineScrutinee(using Context): Type =
      tp.stripTypeVar match
        case tp: TermRef =>
          val sym = tp.termSymbol
          if sym.isAllOf(EnumCase, butNot = JavaDefined) || sym.is(Module) then
            tp
          else if !tp.isOverloaded then
            tp.underlying.widenExpr.widenInlineScrutinee
          else tp
        case _ => tp

  /** An extractor for terms equivalent to `new C(args)`, returning the class
    * `C`, a list of bindings, and the arguments `args`. Can see inside blocks
    * and Inlined nodes and can follow a reference to an inline value binding to
    * its right hand side.
    *
    * @return
    *   optionally, a triple consisting of
    *   - the class `C`
    *   - the arguments `args`
  ...
  private object NewInstance:
    def unapply(tree: Tree)(using
        Context
    ): Option[(Symbol, List[Tree], List[Tree], Boolean)] =
      def unapplyLet(bindings: List[Tree], expr: Tree) =
        unapply(expr) map { case (cls, reduced, prefix, precomputed) =>
          (cls, reduced, bindings ::: prefix, precomputed)
        }
      tree match
        case Apply(fn, args) =>
          fn match
            ...
            case _ =>
              ...
              then Some(meth.owner.linkedClass, args, Nil, false)
              else None
        case Typed(inner, _) =>
        ...
        case Ident(_) =>
          ...
          for ((cls, reduced, prefix, precomputed) <- unapply(binding))
            yield (
              ...
              reduced,
              prefix,
              precomputed || binding.isInstanceOf[ValDef]
            )
  ...
  def reduceProjection(tree: Tree)(using Context): Tree =
    if ctx.debug then inlining.println(i"try reduce projection $tree")
    tree match
      case Select(NewInstance(cls, args, prefix, precomputed), field)
          if cls.isNoInitsRealClass =>
        def matches(param: Symbol, selection: Symbol): Boolean =
          param == selection || {
            selection.name match
              case InlineAccessorName(underlying) =>
                param.name == underlying && selection.info
        ...
        if idx >= 0 && idx < args.length then
          def finish(arg: Tree) =
            new TreeTypeMap()
              .transform(
                arg
              ) // make sure local bindings in argument have fresh symbols
          ...
          if precomputed then
            ...
            def collectImpure(from: Int, end: Int) =
              (from until end)
                .filterNot(i => isElideableExpr(args(i)))
                .toList
                .map(args)
            val leading = collectImpure(0, idx)
            val trailing = collectImpure(idx + 1, args.length)
            val argInPlace =
              if trailing.isEmpty then arg
              else
                def argsSpan =
                  trailing.map(_.span).foldLeft(arg.span)(_.union(_))
                letBindUnless(TreeInfo.Pure, arg)(
                  Block(trailing, _).withSpan(argsSpan)
                )
            val blockSpan = (prefix ::: leading)
              .map(_.span)
  ...
  /** The result type of reducing a match. It consists optionally of a list of
    * bindings for the pattern-bound variables and the RHS of the selected case.
    * Returns `None` if no case was selected.
    */
  type MatchRedux = Option[(List[MemberDef], Tree)]

  /** Same as MatchRedux, but also includes a boolean that is true if the guard
  ...
  /** Reduce an inline match
    ...
    *   the match tree
    * @param scrutinee
    *   the scrutinee expression, assumed to be pure, or EmptyTree for a
    *   summonFrom
    * @param scrutType
    ...
    * @return
    *   optionally, if match can be reduced to a matching case: A pair of
    *   bindings for all pattern-bound variables and the RHS of the case.
    */
  def reduceInlineMatch(
      scrutinee: Tree,
      scrutType: Type,
      cases: List[CaseDef],
      typer: Typer
  )(using Context): MatchRedux =

    val isImplicit = scrutinee.isEmpty

    /** Try to match pattern `pat` against scrutinee reference `scrut`. If
      * successful add bindings for variables bound in this pattern to
      * `caseBindingMap`.
      */
    def reducePattern(
      ...
      def newTermBinding(sym: TermSymbol, rhs: Tree): Unit =
        val copied = sym
          .copy(
            info = rhs.tpe.widenInlineScrutinee,
            coord = sym.coord,
            flags = sym.flags &~ Case
          )
          .asTerm
        caseBindingMap += ((
          sym,
          ValDef(copied, constToLiteral(rhs)).withSpan(sym.span)
        ))
        ...
        inContext(evCtx) {
          ...
          evidence.tpe match
            case fail: Implicits.AmbiguousImplicits =>
              report.error(
                evTyper.missingArgMsg(evidence, tpt.tpe, ""),
                tpt.srcPos
              )
      ...
      def getTypeBindsMap(pat: Tree, tpt: Tree): TypeBindsMap =
        val getBinds = new TreeAccumulator[Set[TypeSymbol]]:
          def apply(syms: Set[TypeSymbol], t: Tree)(using
              Context
          ): Set[TypeSymbol] =
            val syms1 = t match
        ...
        val extractBindVariance = new TypeAccumulator[TypeBindsMap]:
          def apply(syms: TypeBindsMap, t: Type) =
            val syms1 = t match
              ...
              case tr: TypeRef
                  if tr.symbol.is(Case) && binds.contains(tr.symbol.asType) =>
                ...
                val wasToBeMinimized: Boolean =
                  val v = syms(trSym)
                  if v != null then v else false
                syms.updated(
                  trSym,
                  wasToBeMinimized || variance >= 0: java.lang.Boolean
                )
              case _ =>
                syms
            foldOver(syms1, t)
      ...
      def addTypeBindings(typeBinds: TypeBindsMap)(using Context): Unit =
        typeBinds.foreachBinding { case (sym, shouldBeMinimized) =>
          newTypeBinding(
            sym,
            ctx.gadtState.approximation(
              sym,
              fromBelow = shouldBeMinimized,
      ...
      def registerAsGadtSyms(typeBinds: TypeBindsMap)(using Context): Unit =
        if typeBinds.size > 0 then ctx.gadtState.addToConstraint(typeBinds.keys)
        ...
        case UnApply(unapp, _, pats) =>
          unapp.tpe.widen match
            case mt: MethodType if mt.paramInfos.length == 1 =>
              def reduceSubPatterns(
                  pats: List[Tree],
                  selectors: List[Tree]
              ): Boolean = (pats, selectors) match
                case (Nil, Nil) => true
                case (pat :: pats1, selector :: selectors1) =>
                  val elem = newSym(
                    InlineBinderName.fresh(),
                    Synthetic,
                    selector.tpe.widenInlineScrutinee
                  ).asTerm
                  val rhs = constToLiteral(selector)
                  elem.defTree = rhs
                  caseBindingMap += ((
                    NoSymbol,
                    ValDef(elem, rhs).withSpan(elem.span)
                  ))
              ...
              if paramCls.is(Case) && unapp.symbol.is(
                  Synthetic
                ) && scrut <:< paramType
              then
                val caseAccessors =
                  if paramCls.is(Scala2x) then
                    paramCls.caseAccessors.filter(_.is(Method))
                  else paramCls.asClass.paramAccessors
                val selectors =
                  for (accessor <- caseAccessors)
                    yield constToLiteral(
                      reduceProjection(
                        ref(scrut).select(accessor).ensureApplied
                      )
                    )
                caseAccessors.length == pats.length && reduceSubPatterns(
                  pats,
                  selectors
                )
              else false
    ...
    val scrutineeSym =
      newSym(InlineScrutineeName.fresh(), Synthetic, scrutType).asTerm
    ...
    def reduceCase(cdef: CaseDef): MatchReduxWithGuard =
      ...
      def substBindings(
          ...
          from: List[Symbol],
          to: List[Symbol]
      ): (List[MemberDef], List[Symbol], List[Symbol]) =
        bindings match
          case (sym, binding) :: rest =>
            bbuf += binding.subst(from, to).asInstanceOf[MemberDef]
            if sym.exists then
              substBindings(rest, bbuf, sym :: from, binding.symbol :: to)
            else substBindings(rest, bbuf, from, to)
          case Nil => (bbuf.toList, from, to)

      if !isImplicit then caseBindingMap += ((NoSymbol, scrutineeBinding))
      val gadtCtx =
        ctx.fresh.setFreshGADTBounds.addMode(Mode.GadtConstraintInference)
      if reducePattern(caseBindingMap, scrutineeSym.termRef, cdef.pat)(using
          gadtCtx
        )
      then
        val (caseBindings, from, to) =
          substBindings(caseBindingMap.toList, mutable.ListBuffer(), Nil, Nil)
        ...
        if guardOK then
          Some(
            (
              caseBindings.map(_.subst(from, to)),
              cdef.body.subst(from, to),
              canReduceGuard
            )
          )
    ...
    def recur(cases: List[CaseDef]): MatchRedux = cases match
      ...
      case cdef :: cases1 =>
        reduceCase(cdef) match
          ...
          case r @ Some((caseBindings, rhs, canReduceGuard))
              if canReduceGuard =>
            Some((caseBindings, rhs))
          case _ => None

    recur(cases)
  end reduceInlineMatch
end InlineReducer
