...
object Inliner:
  ...
  /** Very similar to TreeInfo.isPureExpr, but with the following inliner-only
    * exceptions:
    *   - synthetic case class apply methods, when the case class constructor is
    *     empty, are elideable but not pure. Elsewhere, accessing the apply
    *     method might cause the initialization of a containing object so they
    *     are merely idempotent.
    */
  object isElideableExpr:
    def isStatElideable(tree: Tree)(using Context): Boolean = unsplice(
      tree
    ) match
      case EmptyTree | TypeDef(_, _) | Import(_, _) | DefDef(_, _, _, _) =>
        true
    ...
    def apply(tree: Tree)(using Context): Boolean = unsplice(tree) match
      ...
      case Select(qual, _) =>
        if tree.symbol.is(Erased) then true
        else isPureRef(tree) && apply(qual)
      case New(_) | Closure(_, _, _) =>
        true
      case TypeApply(fn, _) =>
        if fn.symbol.is(Erased) || fn.symbol == defn.QuotedTypeModule_of then
          true
        else apply(fn)
      case Apply(fn, args) =>
        val isCaseClassApply =
          ...
          funPart(fn).match
            case Select(qual, _) =>
              qual.symbol.is(Synthetic) // e.g: disallow `{ ..; Foo }.apply(..)`
            case meth @ Ident(_) =>
              meth.symbol.is(
                Synthetic
              ) // e.g: allow `import Foo.{ apply => foo }; foo(..)`
            case _ => false
        if isPureApply(tree, fn) then apply(fn) && args.forall(apply)
        else if isCaseClassApply then args.forall(apply)
        else if fn.symbol.is(Erased) then true
        else false
      case Typed(expr, _) =>
        apply(expr)
  ...
  private class InlineCopier() extends TypedTreeCopier:
    override def Apply(
        tree: Tree
    )(fun: Tree, args: List[Tree])(using Context): Apply =
...
class Inliner(val call: tpd.Tree)(using Context):
  ...
  for arg <- callTypeArgs do isFullyDefined(arg.tpe, ForceDegree.flipBottom)

  /** A map from parameter names of the inlineable method to references of the
    * actual arguments. For a type argument this is the full argument type. For
    * a value argument, it is a reference to either the argument value (if the
    * argument is a pure expression of singleton type), or to `val` or `def`
    * acting as a proxy (if the argument is something else).
    */
  private val paramBinding = new mutable.HashMap[Name, Type]

  /** A map from parameter names of the inlineable method to spans of the actual
    * arguments
    */
  private val paramSpan = new mutable.HashMap[Name, Span]

  /** A map from references to (type and value) parameters of the inlineable
    * method to their corresponding argument or proxy references, as given by
    * `paramBinding`.
    */
  ...
  /** A map from the classes of (direct and outer) this references in
    * `rhsToInline` to references of their proxies. Note that we can't index by
    * the ThisType itself since there are several possible forms to express what
    * is logicaly the same ThisType. E.g.
    *
  ...
  private[inlines] def newSym(name: Name, flags: FlagSet, info: Type)(using
      Context
  ): Symbol =
    newSymbol(ctx.owner, name, flags, info, coord = call.span)

  /** A binding for the parameter of an inline method. This is a `val` def for
    * by-value parameters and a `def` def for by-name parameters. `val` defs
    * inherit inline annotations from their parameters. The generated `def` is
    * appended to `buf`.
    * @param name
    ...
    * @param buf
    *   the buffer to which the definition should be appended
    */
  private[inlines] def paramBindingDef(
      name: Name,
      formal: Type,
      arg0: Tree,
      buf: DefBuffer
  )(using Context): ValOrDefDef =
    ...
    val arg =
      ...
      arg0 match
        case Typed(seq, tpt) if tpt.tpe.isRepeatedParam =>
          if seq.tpe.derivesFrom(defn.ArrayClass) then
            wrapArray(dropNameArg(seq), arg0.tpe.elemType)
          else cpy.Typed(arg0)(dropNameArg(seq), tpt)
        case arg0 =>
          dropNameArg(arg0)
    val argtpe =
      arg.tpe.dealiasKeepAnnots.translateFromRepeated(toArray = false)
    ...
    if formal.widenExpr.hasAnnotation(defn.ErasedParamAnnot) then
      bindingFlags |= Erased
    if isByName then bindingFlags |= Method
    val boundSym = newSym(
      InlineBinderName.fresh(name.asTermName),
      bindingFlags,
      bindingType
    ).asTerm
    val binding = {
      ...
      if bindingFlags.is(Inline) && argIsBottom then
        newArg = Typed(
          newArg,
          TypeTree(formal.widenExpr)
        ) // type ascribe RHS to avoid type errors in expansion. See i8612.scala
    ...
    tp match
      case tp: PolyType =>
        ...
        computeParamBindings(
          tp.resultType,
          targs.drop(tp.paramNames.length),
          argss,
          formalss,
          buf
        )
      case tp: MethodType =>
        if argss.isEmpty then
          report.error(
            em"missing arguments for inline method $inlinedMethod",
            call.srcPos
          )
          false
        else
          tp.paramNames.lazyZip(formalss.head).lazyZip(argss.head).foreach {
            (name, formal, arg) =>
              paramSpan(name) = arg.span
  ...
  private def classNestingLevel(cls: Symbol) =
    cls.ownersIterator.count(_.isClass)
  ...
  private def computeThisBindings() =
    ...
    var lastLevel: Int = 0
    for (level, selfSym, cls) <- sortedProxies do
      val rhs = selfSym.info.dealias match
        case info: TermRef
            if info.isStable && (lastSelf.exists || isPureExpr(
              inlineCallPrefix
            )) =>
      ...
      val binding = accountForOpaques(
        ValDef(selfSym.asTerm, QuoteUtils.changeOwnerOfTree(rhs, selfSym))
          .withSpan(selfSym.span)
      )
      bindingsBuf += binding
  ...
  end computeThisBindings

  /** A list of pairs between TermRefs appearing in thisProxy bindings that
    * refer to objects with opaque type aliases and local proxy symbols that
    * contain refined versions of these TermRefs where the aliases are exposed.
    */
  private val opaqueProxies = new mutable.ListBuffer[(TermRef, TermRef)]

  protected def hasOpaqueProxies = opaqueProxies.nonEmpty

  /** Map first halfs of opaqueProxies pairs to second halfs, using =:= as
    * equality
    */
  ...
  /** If `tp` contains TermRefs that refer to objects with opaque type aliases,
    * add proxy definitions to `opaqueProxies` that expose these aliases.
    */
  private def addOpaqueProxies(tp: Type, span: Span, forThisProxy: Boolean)(
      using Context
  ): Unit =
    tp.foreachPart {
      case ref: TermRef =>
        for cls <- ref.widen.baseClasses do
          if cls.containsOpaques
            ...
            && mapRef(ref).isEmpty
          then
            def openOpaqueAliases(selfType: Type): List[(Name, Type)] =
              selfType match
            ...
            val refinedType =
              refinements.foldLeft(ref: Type)((parent, refinement) =>
                RefinedType(parent, refinement._1, TypeAlias(refinement._2))
              )
            val refiningSym =
              newSym(InlineBinderName.fresh(), Synthetic, refinedType).asTerm
            val refiningDef =
              ValDef(refiningSym, tpd.ref(ref).cast(refinedType)).withSpan(span)
            inlining.println(
              i"add opaque alias proxy $refiningDef for $ref in $tp"
            )
  ...
  /** If `binding` contains TermRefs that refer to objects with opaque type
    * aliases, add proxy definitions that expose these aliases and substitute
    * such TermRefs with theproxies. Example from pos/opaque-inline1.scala:
    *
    * object refined: opaque type Positive = Int inline def Positive(value:
    * Int): Positive = f(value) def f(x: Positive): Positive = x def run: Unit =
    * { val x = 9; val nine = refined.Positive(x) }
    *
    * This generates the following proxies:
    *
    * val $proxy1: refined.type{type Positive = Int} =
    * refined.$asInstanceOf$[refined.type{type Positive = Int}] val
    * refined$_this: ($proxy1 : refined.type{Positive = Int}) = $proxy1
    *
  ...
  private def accountForOpaques(binding: ValDef)(using Context): ValDef =
    ...
    if opaqueProxies.isEmpty then binding
      ...
      mapOpaques
        ...
        .asInstanceOf[ValDef]
        .showing(
          i"transformed this binding exposing opaque aliases: $result",
          inlining
        )
  end accountForOpaques

  /** If value argument contains references to objects that contain opaque
    * types, map them to their opaque proxies.
    */
  ...
  private def canElideThis(tpe: ThisType): Boolean =
    inlineCallPrefix.tpe == tpe && ctx.owner.isContainedIn(tpe.cls)
      ...
      || tpe.cls.is(Package)
      || tpe.cls.isStaticOwner && !(tpe.cls.seesOpaques && inlinedMethod
        .isContainedIn(tpe.cls))

  private def adaptToPrefix(tp: Type) =
    tp.asSeenFrom(inlineCallPrefix.tpe, inlinedMethod.owner)

  /** Populate `thisProxy` and `paramProxy` as follows:
    *
    * 1a. If given type refers to a static this, thisProxy binds it to
    * corresponding global reference, 1b. If given type refers to an instance
    * this to a class that is not contained in the inline method, create a proxy
    * symbol and bind the thistype to refer to the proxy. The proxy is not yet
    * entered in `bindingsBuf`; that will come later. 2. If given type refers to
    * a parameter, make `paramProxy` refer to the entry stored in `paramNames`
    * under the parameter's name. This roundabout way to bind parameter
    * references to proxies is done because we don't know a priori what the
    * parameter references of a method are (we only know the method's type, but
    * that contains TypeParamRefs and MethodParams, not TypeRefs or TermRefs.
    */
  ...
  private def registerLeaf(tree: Tree): Unit = tree match
    case _: This | _: Ident | _: TypeTree =>
      registerTypes.traverse(tree.typeOpt)
    case tree: Quote => registerTypes.traverse(tree.bodyType)
    case _           =>

  /** Make `tree` part of inlined expansion. This means its owner has to be
    * changed from its `originalOwner`, and, if it comes from outside the
    * inlined method itself, it has to be marked as an inlined argument.
    */
  private def integrate(tree: Tree, originalOwner: Symbol)(using
      Context
  ): Tree =
  ...
  def inlined(rhsToInline: tpd.Tree): (List[MemberDef], Tree) =
    ...
    def paramTypess(call: Tree, acc: List[List[Type]]): List[List[Type]] =
      call match
        case Apply(fn, args) =>
          fn.tpe.widen.match
            case mt: MethodType =>
              paramTypess(fn, mt.instantiateParamInfos(args.tpes) :: acc)
            case _ => Nil
        case TypeApply(fn, _) => paramTypess(fn, acc)
        case _                => acc

    val paramBindings =
      val mappedCallValueArgss =
        callValueArgss.nestedMapConserve(mapOpaquesInValueArg)
      if mappedCallValueArgss ne callValueArgss then
        inlining.println(
          i"mapped value args = ${mappedCallValueArgss.flatten}%, %"
        )
      ...
      if !computeParamBindings(
          inlinedMethod.info,
          callTypeArgs,
          mappedCallValueArgss,
          paramTypess(call, Nil),
          paramBindingsBuf
        )
      then return (Nil, EmptyTree)

      paramBindingsBuf.toList
    end paramBindings

    // make sure prefix is executed if it is impure
    if !isIdempotentExpr(inlineCallPrefix) then
      registerType(inlinedMethod.owner.thisType)
    ...
    def inlinedFromOutside(tree: Tree)(span: Span): Tree =
      Inlined(EmptyTree, Nil, tree)(using
        ctx.withSource(inlinedMethod.topLevelClass.source)
      ).withSpan(span)
    ...
    val inliner = new InlinerMap(
      ...
      treeMap = {
        ...
        case tree: Ident =>
          ...
          def argSpan =
            if tree.name == nme.WILDCARD then tree.span // From type match
            else if tree.symbol.isTypeParam && tree.symbol.owner.isClass then
              tree.span // TODO is this the correct span?
          ...
          paramProxy.get(tree.tpe) match
            ...
            case _ => tree
        case tree @ Select(qual: This, name)
            if tree.symbol.is(Private) && tree.symbol.isInlineMethod =>
    ...
    inlining.println(i"""inliner transform with
         |thisProxy = ${thisProxy.toList.map(_._1)}%, % --> ${thisProxy.toList
        .map(_._2)}%, %
         |paramProxy = ${paramProxy.toList.map(
        _._1.typeSymbol.showLocated
      )}%, % --> ${paramProxy.toList.map(_._2)}%, %""")
    ...
    def issueError() = callValueArgss match
      case (msgArg :: Nil) :: Nil =>
        val message = msgArg.tpe match
          ...
          case _ =>
            em"A literal string is expected as an argument to `compiletime.error`. Got $msgArg"
        ...
        val callToReport =
          if enclosingInlineds.nonEmpty then enclosingInlineds.last else call
        val ctxToReport =
          ctx.outersIterator.dropWhile(enclosingInlineds(using _).nonEmpty).next
    ...
    /** The number of nodes in this tree, excluding code in nested inline calls
      * and annotations of definitions.
      */
    ...
    trace(i"inlining $call", inlining, show = true) {
      ...
      val (finalBindings, finalExpansion) =
        dropUnusedDefs(bindingsBuf.toList, expansion1)

      if inlinedMethod == defn.Compiletime_error then issueError()
  ...
  private object InlineableArg:
    ...
    def unapply(tree: Trees.Ident[?])(using Context): Option[Tree] =
      ...
      if paramProxies.contains(tree.typeOpt) then
  ...
  private[inlines] def tryInlineArg(tree: Tree)(using Context): Tree =
    tree match
  ...
  /** A typer for inlined bodies. Beyond standard typing, an inline typer
    * performs the following functions:
    *
    *   1. Implement constant folding over inlined code 2. Selectively expand
    *      ifs with constant conditions 3. Inline arguments that are by-name
    *      closures 4. Make sure inlined code is type-correct. 5. Make sure that
    *      the tree's typing is idempotent (so that future -Ycheck passes
    *      succeed)
    */
  class InlineTyper(
      initialErrorCount: Int,
      @constructorOnly nestingLevel: Int = ctx.nestingLevel + 1
  ) extends ReTyper(nestingLevel):
    import reducer.*

    override def ensureAccessible(tpe: Type, superAccess: Boolean, pos: SrcPos)(
        using Context
    ): Type =
      tpe match
        case tpe: NamedType
            if tpe.symbol.exists && !tpe.symbol.isAccessibleFrom(
              tpe.prefix,
              superAccess
            ) =>
          tpe.info match
            case TypeAlias(alias) =>
              return ensureAccessible(alias, superAccess, pos)
    ...
    override def typedIdent(tree: untpd.Ident, pt: Type)(using Context): Tree =
      ...
      val tree0 = tryInlineArg(tree.asInstanceOf[tpd.Tree]) `orElse` super
        .typedIdent(tree, pt)
    ...
    override def typedSelect(tree: untpd.Select, pt: Type)(using
        Context
    ): Tree =
      val locked = ctx.typerState.ownedVars
      val qual1 =
        typed(tree.qualifier, shallowSelectionProto(tree.name, pt, this))
      val resNoReduce =
      ...
      if reducedProjection.isType then
        ...
        else if res.symbol.isInlineMethod then inlineIfNeeded(res, pt, locked)
        else
          ensureAccessible(
            res.tpe,
            tree.qualifier.isInstanceOf[untpd.Super],
            tree.srcPos
          )
          res
    ...
    override def typedIf(tree: untpd.If, pt: Type)(using Context): Tree =
      ...
      typed(tree.cond, defn.BooleanType)(using condCtx) match
        case cond1 @ ConstantValue(b: Boolean) =>
          val selected0 = if b then tree.thenp else tree.elsep
          val selected =
            if selected0.isEmpty then tpd.Literal(Constant(()))
            else typed(selected0, pt)
          if isIdempotentExpr(cond1) then selected
    ...
    override def typedValDef(vdef: untpd.ValDef, sym: Symbol)(using
        Context
    ): Tree =
      val vdef1 =
        if sym.is(Inline) then
          ...
          sym.info = rhs.tpe
          untpd.cpy.ValDef(vdef)(
            vdef.name,
            untpd.TypeTree(rhs.tpe),
            untpd.TypedSplice(rhs)
          )
        else vdef
    ...
    override def typedSplice(tree: untpd.Splice, pt: Type)(using
        Context
    ): Tree =
      super.typedSplice(tree, pt) match
        case tree1 @ Splice(expr) if level == 0 && !hasInliningErrors =>
          ...
          typedExpr(
            expanded
          ) // Inline calls and constant fold code generated by the macro
        case tree1 => tree1
    ...
    override def typedMatch(tree: untpd.Match, pt: Type)(using Context): Tree =
      val tree1 =
        if tree.isInline then
          ...
          val sel1 =
            typedExpr(tree.selector)(using ctx.addMode(Mode.ForceInline))
          untpd.cpy.Match(tree)(sel1, tree.cases)
        else tree
      super.typedMatch(tree1, pt)

    override def typedMatchFinish(
        tree: untpd.Match,
        sel: Tree,
        wideSelType: Type,
      ...
      if !tree.isInline || ctx.owner.isInlineMethod
      then // don't reduce match of nested inline method yet
        ...
        def selTyped(sel: Tree): Type = sel match
          ...
          case _                     => sel.tpe
        val selType = if sel.isEmpty then wideSelType else selTyped(sel)
        reduceInlineMatch(
          sel,
          selType,
          ...
          case Some((caseBindings, rhs0)) =>
            ...
            val rhs1 = rhs0 match
              case Block(stats, t) if t.span.isSynthetic =>
                t match
                  case Typed(expr, _) =>
                    Block(stats, expr)
                  case TypeApply(sel @ Select(expr, _), _)
                      if sel.symbol.isTypeCast =>
                    Block(stats, expr)
                  case _ =>
                    rhs0
              case _ => rhs0
            ...
            typedExpr(rhs, pt)
          case None =>
            def guardStr(guard: untpd.Tree) =
              if guard.isEmpty then "" else i" if $guard"
            def patStr(cdef: untpd.CaseDef) =
              i"case ${cdef.pat}${guardStr(cdef.guard)}"
            val msg =
              if tree.selector.isEmpty then
    ...
    override def newLikeThis(nestingLevel: Int): Typer =
      new InlineTyper(initialErrorCount, nestingLevel)

    /** True if this inline typer has already issued errors */
    override def hasInliningErrors(using Context) =
      ctx.reporter.errorCount > initialErrorCount

    private def inlineIfNeeded(tree: Tree, pt: Type, locked: TypeVars)(using
        Context
    ): Tree =
      ...
      if meth.isAllOf(DeferredInline) then
        ...
        Inlines.inlineCall(simplify(tree, pt, locked))
      else tree

    override def typedUnadapted(tree: untpd.Tree, pt: Type, locked: TypeVars)(
        using Context
    ): Tree =
  ...
  /** Drop any side-effect-free bindings that are unused in expansion or other
    * reachable bindings. Inline def bindings that are used only once.
    */
  private def dropUnusedDefs(bindings: List[MemberDef], tree: Tree)(using
      Context
  ): (List[MemberDef], Tree) =
    // inlining.println(i"drop unused $bindings%, % in $tree")
    val (termBindings, typeBindings) = bindings.partition(_.symbol.isTerm)
    if typeBindings.nonEmpty then
      val typeBindingsSet = typeBindings.foldLeft[SimpleIdentitySet[Symbol]](
      ...
      val inlineTypeBindings = new TreeTypeMap(
        typeMap = new TypeMap():
          override def apply(tp: Type): Type = tp match
            case tr: TypeRef
                if tr.prefix.eq(NoPrefix) && typeBindingsSet
                  .contains(tr.symbol) =>
        ...
        treeMap = {
          case ident: Ident
              if ident.isType && typeBindingsSet.contains(ident.symbol) =>
            val TypeAlias(r) = ident.symbol.info: @unchecked
            TypeTree(r).withSpan(ident.span)
          case tree => tree
        }
      )
      val Block(termBindings1, tree1) = inlineTypeBindings(
        Block(termBindings, tree)
      )
        ...
        tree.typeOpt.foreachPart {
          case ref: TermRef =>
            updateRefCount(
              ref.symbol,
              2
            ) // can't be inlined, so make sure refCount is at least 2
          case _ =>
        }
      ...
      countRefs(tree)
      for binding <- bindings do countRefs(binding)
      ...
      val inlineBindings = new TreeMap:
        override def transform(t: Tree)(using Context) = t match
          ...
          case t: Apply =>
            ...
            if t1 `eq` t then t else BetaReduce(t1)
          case Block(Nil, expr) =>
      ...
      if retained `eq` bindings then (bindings, tree)
      else {
    ...
    if dependencies.nonEmpty && !ctx.reporter.errorsReported then
      for sym <- dependencies do
        if ctx.compilationUnit.source.file == sym.associatedFile then
          report.error(
            em"Cannot call macro $sym defined in the same source file",
            call.srcPos
          )
        if suspendable && ctx.settings.XprintSuspension.value then
          report.echo(
            i"suspension triggered by macro call to ${sym.showLocated} in ${sym.associatedFile}",
            call.srcPos
          )
      if suspendable then
    ...
    val evaluatedSplice =
      inContext(quoted.MacroExpansion.context(inlinedFrom)) {
        Splicer.splice(
          ...
          splicePos,
          inlinedFrom.srcPos,
          MacroClassLoader.fromContext
        )
      }
    val inlinedNormailizer = new TreeMap:
      override def transform(tree: tpd.Tree)(using Context): tpd.Tree =
        tree match
          case Inlined(EmptyTree, Nil, expr) if enclosingInlineds.isEmpty =>
            transform(expr)
          case _ => super.transform(tree)
    val normalizedSplice = inlinedNormailizer.transform(evaluatedSplice)
    if normalizedSplice.isEmpty then normalizedSplice
    else normalizedSplice.withSpan(splicePos.span)
  end expandMacro

  /** Return the set of symbols that are referred at level -1 by the tree and
    * defined in the current run. This corresponds to the symbols that will need
    * to be interpreted.
    */
  private def macroDependencies(tree: Tree)(using Context) =
    new TreeAccumulator[List[Symbol]]:
      override def apply(syms: List[Symbol], tree: tpd.Tree)(using
          Context
      ): List[Symbol] =
        tree match
          case tree: RefTree
              if tree.isTerm && level == -1 && tree.symbol.isDefinedInCurrentRun && !tree.symbol.isLocal =>
