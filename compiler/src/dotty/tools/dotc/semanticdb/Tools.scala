...
object Tools:
  ...
  /** Converts a Path to a String that is URI encoded, without forcing absolute
    * paths.
    */
  ...
  def loadTextDocument(
    ...
    sdocs.documents.find(_.uri == reluri) match
      ...
      case Some(document) =>
        val text = new String(
          Files.readAllBytes(scalaAbsolutePath),
        ...
        if document.md5 != md5FingerprintOnDisk then
          throw new IllegalArgumentException(
            s"stale semanticdb: $scalaRelativePath"
          )
        else
  ...
  def loadTextDocumentUnsafe(
    ...
    docs.head.copy(text =
      new String(Files.readAllBytes(scalaAbsolutePath), StandardCharsets.UTF_8)
    )

  /** Parses SemanticDB text documents from an absolute path to a `*.semanticdb`
    * file.
    */
  private def parseTextDocuments(path: Path): TextDocuments =
    val bytes =
      Files
        .readAllBytes(path)
        .nn // NOTE: a semanticdb file is a TextDocuments message, not TextDocument
    TextDocuments.parseFrom(bytes)

  def metac(doc: TextDocument, realPath: Path)(using
      sb: StringBuilder
  ): StringBuilder =
    ...
    sb.append("Occurrences => ")
      .append(doc.occurrences.length)
      .append(" entries")
      .nl
    if doc.synthetics.nonEmpty then
      sb.append("Synthetics => ")
        .append(doc.synthetics.length)
        .append(" entries")
        .nl
    sb.nl
  ...
  private def processSymbol(
      ...
      printer: SymbolInformationPrinter
  )(using sb: StringBuilder): Unit =
    sb.append(printer.pprintSymbolInformation(info)).nl

  private def processSynth(synth: Synthetic, printer: SyntheticPrinter)(using
      sb: StringBuilder
  ): Unit =
    sb.append(printer.pprint(synth)).nl

  private def processOccurrence(
      occ: SymbolOccurrence
  )(using sb: StringBuilder, sourceFile: SourceFile): Unit =
    occ.range match
      case Some(range) =>
        ...
        then
          val line =
            sourceFile.lineContent(sourceFile.lineToOffset(range.startLine))
          assert(
            range.startCharacter <= line.length && range.endCharacter <= line.length,
            s"Line is only ${line.length} - start line was ${range.startLine} in source ${sourceFile.name}"
          )
          sb.append(" ")
            .append(line.substring(range.startCharacter, range.endCharacter))
      case _ =>
        sb.append("[):")
    end match
    sb.append(if occ.role.isReference then " -> " else " <- ")
      .append(occ.symbol)
      .nl
