...
class SyntheticsExtractor:
  ...
  def tryFindSynthetic(
      tree: Tree
  )(using Context, SemanticSymbolBuilder, TypeOps): Option[s.Synthetic] =
    extension (synth: s.Synthetic) def toOpt: Some[s.Synthetic] = Some(synth)
        ...
        case tree: TypeApply
            if tree.span.isSynthetic &&
              ...
              (tree.fun match
                ...
                case Select(_, name) if name.is(NameKinds.DefaultGetterName) =>
                  false
  ...
  end tryFindSynthetic

  private given TreeOps: AnyRef with
    extension (tree: Tree)
      def toSemanticTree(using
          Context,
        ...
        tree match
          ...
          case tree: TypeApply =>
            s.TypeApplyTree(
              tree.fun.toSemanticQualifierTree,
              tree.args.map { targ =>
                targ.tpe.toSemanticType(targ.symbol)(using
      ...
      def toSemanticQualifierTree(using
          Context,
          SemanticSymbolBuilder
      ): s.Tree = tree match
  ...
  private def isSyntheticName(select: Select): Boolean =
    select.span.toSynthetic == select.qualifier.span.toSynthetic && (select.name == nme.apply ||
