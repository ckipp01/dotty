...
import scala.annotation.{threadUnsafe as tu, tailrec}
import scala.PartialFunction.condOpt

import dotty.tools.dotc.{semanticdb as s}
import dotty.tools.io.{AbstractFile, JarArchive}

/** Extract symbol references and uses to semanticdb files. See
  * https://scalameta.org/docs/semanticdb/specification.html#symbol-1 for a
  * description of the format. TODO: Also extract type information
  */
class ExtractSemanticDB extends Phase:
  ...
  override def isRunnable(using Context) =
    ...
    def writesToOutputJar =
      semanticdbTarget.isEmpty && outputDirectory.isInstanceOf[JarArchive]
  ...
  class Extractor extends TreeTraverser:
    ...
    def extract(tree: Tree)(using Context): Unit =
      traverse(tree)
      val fakeSyms = converter.fakeSymbols.map(
        _.symbolInfo(Set.empty)(using LinkMode.SymlinkChildren, converter)
      )
    ...
    /** Uses of this symbol where the reference has given span should be
      * excluded from semanticdb
      */
    private def excludeUse(qualifier: Option[Symbol], sym: Symbol)(using
        Context
    ): Boolean =
      !sym.exists
        || excludeDefOrUse(sym)
    ...
    override def traverse(tree: Tree)(using Context): Unit =
      ...
      tree match
          ...
          if !tree.symbol.isAllOf(ModuleValCreationFlags) then
            tree match
              case tree: ValDef if tree.symbol.isAllOf(EnumValue) =>
                tree.rhs match
                  case Block(
                        TypeDef(_, template: Template) :: _,
                        _
                      ) => // simple case with specialised extends clause
                    template.parents
                      .filter(!_.span.isZeroExtent)
                      .foreach(traverse)
                  case _ => // calls $new
              case tree: ValDef if tree.symbol.isSelfSym =>
                if tree.tpt.span.hasLength then traverse(tree.tpt)
              case tree: DefDef
                ...
                tree match
                  case tree: DefDef =>
                    tree.paramss.foreach(
                      _.foreach(param => registerSymbolSimple(param.symbol))
                    )
              ...
              case tree =>
                if !excludeChildren(tree.symbol) then traverseChildren(tree)
            end match
            if !excludeDef(
                tree.symbol
              ) && (tree.span.hasLength || tree.symbol.isAnonymousClass)
            then
              registerDefinition(
                tree.symbol,
                tree.nameSpan,
              ...
              if privateWithin.exists then
                registerUseGuarded(
                  None,
                  privateWithin,
                  spanOfSymbol(privateWithin, tree.span, tree.source),
                  tree.source
                )
            else if !excludeSymbol(tree.symbol) then
              registerSymbol(tree.symbol, symbolKinds(tree))
        case tree: Template
            if tree.symbol != NoSymbol && tree.symbol.owner.is(Invisible) =>
        // do nothing
        ...
        case tree: Template =>
          ...
          if selfSpan.exists && selfSpan.hasLength then traverse(tree.self)
          if tree.symbol != NoSymbol && tree.symbol.owner.isEnumClass then
            tree.body
              .foreachUntilImport(traverse)
              .foreach(traverse) // the first import statement
          else tree.body.foreach(traverse)
          if !excludeDef(ctorSym) then
            traverseAnnotsOfDefinition(ctorSym)
            ctorParams(
              tree.constr.termParamss,
              tree.constr.leadingTypeParams,
              tree.body
            )
            registerDefinition(
              ctorSym,
              tree.constr.nameSpan.startPos,
              Set.empty,
              tree.source
            )
        case tree: Apply =>
          @tu lazy val genParamSymbol: Name => String =
            tree.fun.symbol.funParamSymbol
          traverse(tree.fun)
            ...
            arg match
              case tree @ NamedArg(name, arg) =>
                ...
                registerUse(
                  genParamSymbol(name),
                  tree.span.startPos
                    .withEnd(tree.span.start + name.toString.length),
                  tree.source
                )
              case _ => traverse(arg)
        case tree: Assign =>
          val qualSym = condOpt(tree.lhs) {
            case Select(qual, _) if qual.symbol.exists => qual.symbol
          }
          if !excludeUse(qualSym, tree.lhs.symbol) then
            ...
            tree.lhs match
              case tree: Select =>
                registerUse(setter, selectSpan(tree), tree.source)
        ...
        case tree: Select =>
          ...
          val sym = tree.symbol.adjustIfCtorTyparam
          if qualSpan.exists && qualSpan.hasLength then traverse(qual)
          if sym != NoSymbol then
            registerUseGuarded(
              qual.symbol.ifExists,
              sym,
              selectSpan(tree),
              tree.source
            )
            ...
            for sel <- tree.selectors do
              ...
              if imported != nme.WILDCARD then
                for alt <- tree.expr.tpe.member(imported).alternatives do
                  ...
                  try
                    if alt.symbol.companionClass.exists then
                      registerUseGuarded(
                        None,
                        alt.symbol.companionClass,
                        sel.imported.span,
                        tree.source
                      )
                  catch
        ...
        case tree: TypeTree =>
          tree.typeOpt match
            ...
            case Types.TypeRef(_, sym: Symbol)
                if namePresentInSource(sym, tree.span, tree.source) =>
              registerUseGuarded(None, sym, tree.span, tree.source)
            case _ => ()
      ...
      tree match
        case tree: WithEndMarker[t] =>
          ...
          if endSpan.exists &&
            namePresentInSource(tree.symbol, endSpan, tree.source)
          then
    ...
    private object PatternValDef:
      ...
      def unapply(tree: ValDef)(using Context): Option[(Tree, Tree)] =
        tree.rhs match

          case Match(
                Typed(selected: Tree, tpt: TypeTree),
                CaseDef(pat: Tree, _, _) :: Nil
              )
      ...
      extension (tpe: Types.Type)
        private inline def isAnnotatedByUnchecked(using Context) = tpe match
          case Types.AnnotatedType(_, annot) =>
            annot.symbol == defn.UncheckedAnnot
          case _ => false
      ...
      def collectPats(pat: Tree): List[Tree] =
        ...
        def impl(acc: List[Tree], pats: List[Tree]): List[Tree] = pats match
          ...
          case pat :: pats =>
            pat match
              case Typed(UnApply(fun: Tree, _, args), tpt: Tree) =>
                impl(fun :: tpt :: acc, args ::: pats)
              case Typed(obj: Ident, tpt: Tree) => impl(obj :: tpt :: acc, pats)
              case UnApply(fun: Tree, _, args) =>
                impl(fun :: acc, args ::: pats)
    ...
    private def registerSymbol(sym: Symbol, symkinds: Set[SymbolKind])(using
        Context
    ): Unit =
      val sname = sym.symbolName
      val isLocal = sname.isLocal
      if !isLocal || !localNames.contains(sname) then
        if isLocal then localNames += sname
        symbolInfos += sym.symbolInfo(symkinds)(using
          LinkMode.SymlinkChildren,
          converter
        )
    ...
    private def registerOccurrence(
        symbol: String,
        span: Span,
    ...
    private def registerUseGuarded(
        qualSym: Option[Symbol],
        sym: Symbol,
        span: Span,
        treeSource: SourceFile
    )(using Context) =
    ...
    private def registerUse(sym: Symbol, span: Span, treeSource: SourceFile)(
        using Context
    ): Unit =
      registerUse(sym.symbolName, span, treeSource)

    private def registerUse(symbol: String, span: Span, treeSource: SourceFile)(
        using Context
    ): Unit =
      registerOccurrence(
        symbol,
    ...
    private def registerDefinition(
        sym: Symbol,
        span: Span,
      ...
      val sname = sym.symbolName
      val finalSpan =
        if !span.hasLength || !sym
            .is(Given) || namePresentInSource(sym, span, treeSource)
        then span
        else Span(span.start)
      ...
      if !sym.is(Package) then registerSymbol(sym, symkinds)

    private def spanOfSymbol(sym: Symbol, span: Span, treeSource: SourceFile)(
        using Context
    ): Span =
      val contents =
        if treeSource.exists then treeSource.content() else Array.empty[Char]
    ...
    extension (tree: DefDef)
      private def isSetterDef(using Context): Boolean =
        tree.name.isSetterName && tree.mods.is(
          Accessor
        ) && tree.termParamss.isSingleArg

    private def findGetters(ctorParams: Set[Names.TermName], body: List[Tree])(
        using Context
    ): Map[Names.TermName, ValDef] =
      if ctorParams.isEmpty || body.isEmpty then Map.empty
      else
        body
          .collect({
            case tree: ValDef
    ...
    private def selectSpan(tree: Select) =
      ...
      val start =
        if limit < end then
          ...
          if tree.source.content()(end - 1) == '`' then end - len - 2
          else end - len
        else limit
    ...
    /** Consume head while not an import statement. Returns the rest of the list
      * after the first import, or else the empty list
      */
    extension (body: List[Tree])
      @tailrec private def foreachUntilImport(op: Tree => Unit): List[Tree] =
        body match
    ...
    private inline def matchingMemberType(
        ctorTypeParam: Symbol,
        classSym: Symbol
    )(using Context) =
      classSym.info.member(ctorTypeParam.name).symbol

    /** Necessary because not all of the eventual flags are propagated from the
      * Tree to the symbol yet.
      */
    private def symbolKinds(
        tree: NamedDefTree
    )(using Context): Set[SymbolKind] =
      if tree.symbol.isSelfSym then Set.empty
      else
        val symkinds = mutable.HashSet.empty[SymbolKind]
        tree match
          case tree: ValDef =>
            if !tree.symbol.is(Param) then
              symkinds += (if tree.mods is Mutable then SymbolKind.Var
                           else SymbolKind.Val)
            if tree.rhs.isEmpty && !tree.symbol.isOneOf(
                TermParam | CaseAccessor | ParamAccessor
              )
            then symkinds += SymbolKind.Abstract
          case tree: DefDef =>
            if tree.isSetterDef then symkinds += SymbolKind.Setter
            else if tree.rhs.isEmpty then symkinds += SymbolKind.Abstract
    ...
    private def ctorParams(
      ...
      @tu lazy val getters =
        findGetters(vparamss.flatMap(_.map(_.name)).toSet, body)
      for
        vparams <- vparamss
        vparam <- vparams
      do
        ...
        if !excludeSymbol(vparam.symbol) then
          ...
          val symkinds =
            getters
              .get(vparam.name)
              .fold(SymbolKind.emptySet)(getter =>
                if getter.mods.is(Mutable) then SymbolKind.VarSet
                else SymbolKind.ValSet
              )
