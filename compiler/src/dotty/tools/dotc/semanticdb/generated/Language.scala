...
sealed abstract class Language(val value: _root_.scala.Int)
  ...
  final def asRecognized
      : _root_.scala.Option[dotty.tools.dotc.semanticdb.Language.Recognized] =
    if isUnrecognized then _root_.scala.None
    else
      _root_.scala.Some(
        this.asInstanceOf[dotty.tools.dotc.semanticdb.Language.Recognized]
      )
...
object Language:
  ...
  final case class Unrecognized(unrecognizedValue: _root_.scala.Int)
      extends Language(unrecognizedValue)
      with SemanticdbUnrecognizedEnum
  lazy val values =
    scala.collection.immutable.Seq(UNKNOWN_LANGUAGE, SCALA, JAVA)
