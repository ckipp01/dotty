...
sealed abstract class Schema(val value: _root_.scala.Int)
  ...
  final def asRecognized
      : _root_.scala.Option[dotty.tools.dotc.semanticdb.Schema.Recognized] =
    if isUnrecognized then _root_.scala.None
    else
      _root_.scala.Some(
        this.asInstanceOf[dotty.tools.dotc.semanticdb.Schema.Recognized]
      )
...
object Schema:
  ...
  final case class Unrecognized(unrecognizedValue: _root_.scala.Int)
      extends Schema(unrecognizedValue)
      with SemanticdbUnrecognizedEnum
  lazy val values =
    scala.collection.immutable.Seq(LEGACY, SEMANTICDB3, SEMANTICDB4)
