...
object SemanticdbOutputStream:
  ...
  def computePreferredBufferSize(dataLength: Int): Int =
    if dataLength > DEFAULT_BUFFER_SIZE then return DEFAULT_BUFFER_SIZE
    dataLength
  ...
  def computeInt32SizeNoTag(value: Int): Int =
    if value >= 0 then computeRawVarint32Size(value)
    else 10
  ...
  def computeRawVarint32Size(value: Int): Int =
    if (value & (0xffffffff << 7)) == 0 then return 1
    if (value & (0xffffffff << 14)) == 0 then return 2
    if (value & (0xffffffff << 21)) == 0 then return 3
    if (value & (0xffffffff << 28)) == 0 then return 4
    5

  def computeRawVarint64Size(value: Long): Int =
    if (value & (0xffffffffffffffffL << 7)) == 0 then return 1
    if (value & (0xffffffffffffffffL << 14)) == 0 then return 2
    if (value & (0xffffffffffffffffL << 21)) == 0 then return 3
    if (value & (0xffffffffffffffffL << 28)) == 0 then return 4
    if (value & (0xffffffffffffffffL << 35)) == 0 then return 5
    if (value & (0xffffffffffffffffL << 42)) == 0 then return 6
    if (value & (0xffffffffffffffffL << 49)) == 0 then return 7
    if (value & (0xffffffffffffffffL << 56)) == 0 then return 8
    if (value & (0xffffffffffffffffL << 63)) == 0 then return 9
    10
...
class SemanticdbOutputStream(output: OutputStream, buffer: Array[Byte]):
  private def refreshBuffer(): Unit =
    if output == null then throw new OutOfSpaceException()
    output.write(buffer, 0, position)
    position = 0

  def flush(): Unit =
    if output != null then refreshBuffer()

  def spaceLeft(): Int =
    if output == null then limit - position
    else
  ...
  def writeInt32NoTag(value: Int): Unit =
    if value >= 0 then writeRawVarint32(value)
  ...
  def writeBoolNoTag(value: Boolean): Unit =
    writeRawByte(if value then 1 else 0)
  ...
  def writeRawByte(value: Byte): Unit =
    if position == limit then refreshBuffer()
