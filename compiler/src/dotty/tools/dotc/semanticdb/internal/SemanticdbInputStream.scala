...
object SemanticdbInputStream:
  ...
  def readRawVarint32(firstByte: Int, input: InputStream): Int =
    if (firstByte & 0x80) == 0 then return firstByte
    var result = firstByte & 0x7f
    var offset = 7
    while offset < 32 do
      val b = input.read()
      if b == -1 then throw InvalidProtocolBufferException.truncatedMessage()
...
class SemanticdbInputStream private (buffer: Array[Byte], input: InputStream):
  ...
  private def ensureAvailable(n: Int): Unit =
    if bufferSize - bufferPos < n then refillBuffer(n)
  ...
  private def tryRefillBuffer(n: Int): Boolean =
    if bufferPos + n <= bufferSize then
    ...
    if totalBytesRetired + bufferPos + n <= currentLimit && input != null then
      ...
      if bytesRead == 0 || bytesRead < -1 || bytesRead > buffer.length then
        throw new IllegalStateException(
          "InputStream#read(byte[]) returned invalid result: " + bytesRead + "\nThe InputStream implementation is buggy."
        )
      if bytesRead > 0 then
        bufferSize += bytesRead
        if totalBytesRetired + n - sizeLimit > 0 then
  ...
  /** Sets {@code currentLimit} to (current position) + {@code byteLimit}. This
    ...
    * SemanticdbInputStream} reads from an underlying {@code InputStream} when
  ...
  def pushLimit(byteLimit0: Int): Int =
    if byteLimit0 < 0 then throw InvalidProtocolBufferException.negativeSize()
  ...
  def skipMessage(): Unit =
    while true do
      val tag: Int = readTag()
      if tag == 0 || !skipField(tag) then return

  /** Reads and discards {@code size} bytes.
    */
  def skipRawBytes(size: Int): Unit =
    if size <= (bufferSize - bufferPos) && size >= 0 then bufferPos += size
    else skipRawBytesSlowPath(size)
  ...
  def readRawVarint32(): Int =
    ...
    then
      bufferPos = pos
      return x
    else if bufferSize - pos < 9 then return readRawVarint64SlowPath().toInt
    else if (({
        x ^= (buffer(({
          pos += 1; pos - 1
        })) << 7); x
  ...
  private def skipRawBytesSlowPath(size: Int): Unit =
    if size < 0 then throw InvalidProtocolBufferException.negativeSize()
  ...
  def readTag(): Int =
    ...
    if WireFormat.getTagFieldNumber(lastTag) == 0 then
      throw InvalidProtocolBufferException.invalidTag()
    lastTag

  def readString(): String =
    val size: Int = readRawVarint32()
    if size <= (bufferSize - bufferPos) && size > 0 then
      val result: String =
        new String(buffer, bufferPos, size, StandardCharsets.UTF_8)
      bufferPos += size
      return result
    else if size == 0 then return ""
    else return new String(readRawBytesSlowPath(size), StandardCharsets.UTF_8)

  def readStringRequireUtf8(): String =
    val size: Int = readRawVarint32()
    var bytes: Array[Byte] = Array()
    var pos = bufferPos;
    if size <= (bufferSize - pos) && size > 0 then
  ...
  def checkLastTagWas(value: Int): Unit =
    if lastTag != value then
      throw InvalidProtocolBufferException.invalidEndTag();

  def getBytesUntilLimit: Int =
    if currentLimit == Integer.MAX_VALUE then return -1
  ...
  def readRawVarint64(): Long =
    var pos: Int = bufferPos
    if bufferSize == pos then return readRawVarint64SlowPath()
    val buffer: Array[Byte] = this.buffer
    var x: Long = 0L
    var y: Int = 0
    if (({
    ...
    then
      bufferPos = pos
      return y
    else if bufferSize - pos < 9 then return readRawVarint64SlowPath()
    else if (({
        y ^= (buffer(({
          pos += 1; pos - 1
        })) << 7); y
    ...
    then x = y ^ ((~0 << 7) ^ (~0 << 14) ^ (~0 << 21))
    else if (({
        x = (y.toLong) ^ (buffer(({
          pos += 1; pos - 1
        })).toLong << 28); x
      })) >= 0L
    then x ^= (~0L << 7) ^ (~0L << 14) ^ (~0L << 21) ^ (~0L << 28)
    else if (({
        x ^= (buffer(({
          pos += 1; pos - 1
        })).toLong << 35); x
      })) < 0L
    then x ^= (~0L << 7) ^ (~0L << 14) ^ (~0L << 21) ^ (~0L << 28) ^ (~0L << 35)
    else if (({
        x ^= (buffer(({
          pos += 1; pos - 1
        })).toLong << 42); x
  ...
  def readRawByte(): Byte =
    if bufferPos == bufferSize then refillBuffer(1)
    buffer({
  ...
  def readRawBytes(size: Int): Array[Byte] =
    ...
    else readRawBytesSlowPath(size)
  ...
  private def readRawBytesSlowPath(size: Int): Array[Byte] =
    if size <= 0 then
      if size == 0 then return Internal.EMPTY_BYTE_ARRAY
      else throw InvalidProtocolBufferException.negativeSize()
    if totalBytesRetired + bufferPos + size > currentLimit then
      skipRawBytes(currentLimit - totalBytesRetired - bufferPos)
      throw InvalidProtocolBufferException.truncatedMessage()
