...
trait SemanticdbGeneratedOneof extends Any with Product with Serializable:
  ...
  def isEmpty: Boolean
  def value: ValueType
  def valueOption: Option[ValueType] = if isDefined then Some(value) else None
...
trait SemanticdbGeneratedSealedOneof extends Any with Product with Serializable:
