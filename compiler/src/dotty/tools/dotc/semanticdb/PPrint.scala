...
class SymbolInformationPrinter(symtab: PrinterSymtab):
  ...
  class InfoNotes:
    ...
    def enter(info: SymbolInformation) =
      if symtab.info(info.symbol).isEmpty && info.kind != UNKNOWN_KIND then
  ...
  class InfoPrinter(notes: InfoNotes):
    ...
    def pprint(info: SymbolInformation): String =
      ...
      if annotStr.nonEmpty then sb.append(annotStr + " ")
      ...
      sb.append(
        s"${info.displayName}${info.prefixBeforeTpe}${pprint(info.signature)}"
      )
    ...
    private def pprintDef(sym: String): String =
      pprint(sym, SymbolStyle.Definition)
    ...
    private def pprint(sig: Signature): String =
      sig match
        case ClassSignature(tparams, parents, self, decls) =>
          ...
          if self.isDefined || decls.infos.nonEmpty then
            val selfStr =
              if self.isDefined then s"self: ${pprint(self)} =>" else ""
            val declsStr =
              if decls.infos.nonEmpty then s"+${decls.infos.length} decls"
              else ""
        ...
        case TypeRef(pre, sym, args) =>
          val preStr = pre match
            ...
            case _ =>
              s"${prefix(pre)}#"
          val argsStr =
            if args.nonEmpty then args.map(normal).mkString("[", ", ", "]")
            else ""
    ...
    extension (scope: Scope)
      private def infos: List[SymbolInformation] =
        if scope.symlinks.nonEmpty then
          scope.symlinks
            .map(symbol => SymbolInformation(symbol = symbol))
            .toList
...
extension (info: SymbolInformation)
  def prefixBeforeTpe: String =
    info.kind match
      case LOCAL | FIELD | PARAMETER | SELF_PARAMETER | UNKNOWN_KIND |
          Unrecognized(_) =>
        ": "
      case METHOD | CONSTRUCTOR | MACRO | TYPE | TYPE_PARAMETER | OBJECT |
          PACKAGE | PACKAGE_OBJECT | CLASS | TRAIT | INTERFACE =>
        " "
...
def processRange(sb: StringBuilder, range: Range): Unit =
  sb.append('[')
    ...
    .append(':')
    .append(range.startCharacter)
    .append("..")
    .append(range.endLine)
    .append(':')
    .append(range.endCharacter)
    .append("):")

class SyntheticPrinter(symtab: PrinterSymtab, source: SourceFile)
    extends SymbolInformationPrinter(symtab):
  ...
  extension (source: SourceFile)
    ...
    private def substring(range: s.Range): String =
      ...
      def lineLength(line: Int): Int =
        val isLastLine = source.lineToOffsetOpt(line).nonEmpty && source
          .lineToOffsetOpt(line + 1)
          .isEmpty
        if isLastLine then source.content.length - source.lineToOffset(line) - 1
        else
          source.lineToOffset(line + 1) - source.lineToOffset(
            line
  ...
  class TreePrinter(
    ...
    private def rep[T](xs: Seq[T], seq: String)(f: T => Unit)(using
        sb: StringBuilder
    ): Unit =
