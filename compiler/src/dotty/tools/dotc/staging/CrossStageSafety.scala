...
/** Checks that staging level consistency holds and heals staged types.
  *
  * Local term references are level consistent if and only if they are used at
  * the same level as their definition.
  *
  * Local type references can be used at the level of their definition or lower.
  * If used used at a higher level, it will be healed if possible, otherwise it
  * is inconsistent.
  *
  * Healing a type consists in replacing locally defined types defined at
  * staging level 0 and used in higher levels. For each type local `T` that is
  * defined at level 0 and used in a quote, we summon a tag `t: Type[T]`. This
  * `t` tag must be defined at level 0. The tags will be listed in the `tags` of
  * the level 0 quote (`'<t>{ ... }`) and each reference to `T` will be replaced
  * by `t.Underlying` in the body of the quote.
  *
  * We delay the healing of types in quotes at level 1 or higher until those
  * quotes reach level 0. At this point more types will be statically known and
  * fewer types will need to be healed. This also keeps the nested quotes in
  * their original form, we do not want macro users to see any artifacts of this
  * phase in quoted expressions they might inspect.
  *
  * Type heal example:
  *
  * '{ val x: List[T] = List[T]() '{ .. T .. } () }
  *
  * is transformed to
  *
  * '<t>{ // where `t` is a given term of type `Type[T]` val x:
  * List[t.Underlying] = List[t.Underlying](); '{ .. t.Underlying .. } () }
  */
class CrossStageSafety extends TreeMapWithStages:
  ...
  override def transform(tree: Tree)(using Context): Tree =
    if tree.source != ctx.source && tree.source.exists then
      ...
      tree match
        ...
        case tree: Quote =>
          if ctx.property(InAnnotation).isDefined then
          ...
          val tree1 =
            ...
            val bodyType1 =
              healType(tree.srcPos)(stripAnnotationsDeep(tree.bodyType))
            tree.withBodyType(bodyType1)

          if level == 0 then
            val (tags, body1) = inContextWithQuoteTypeTags {
              transform(tree1.body)(using quoteContext)
            }
            cpy.Quote(tree1)(body1, tags)
          else super.transform(tree1)
        ...
        case tree @ QuotedTypeOf(body) =>
          ...
          if level == 0 then
            ...
            tags match
              case tag :: Nil
                  if body1.isType && body1.tpe =:= tag.tpe.select(
                    tpnme.Underlying
                  ) =>
    ...
    def apply(tp: Type): Type = tp match
      case tp: AnnotatedType =>
        ...
        derivedAnnotatedType(
          tp,
          apply(tp.parent),
          tp.annot.derivedAnnotation(newAnnotTree)
        )
      case _ =>
        mapOver(tp)
    .apply(tp)

  /** Check that annotations do not contain quotes and and that splices are
    * valid
    */
  private def checkAnnotations(tree: Tree)(using Context): Unit =
    tree match
      case tree: DefTree =>
        lazy val annotCtx =
          ctx.fresh.setProperty(InAnnotation, true).withOwner(tree.symbol)
        for annot <- tree.symbol.annotations do
          annot match
            case annot: BodyAnnotation =>
              annot // already checked in PrepareInlineable before the creation of the BodyAnnotation
  ...
  /** If the type refers to a locally defined symbol (either directly, or in a
    * pickled type), check that its staging level matches the current level.
    ...
    *     It cannot be healed because the term will not exist in any future
    *     stage.
    *
    * If `T` is a reference to a type at the wrong level, try to heal it by
    * replacing it with a type tag of type `quoted.Type[T]`. The tag is
    * generated by an instance of `QuoteTypeTags` directly if the splice is
    * explicit or indirectly by `tryHeal`.
    */
      ...
      def traverse(tp: Type): Unit =
        tp match
          case tp @ TermRef(NoPrefix, _)
              if !tp.symbol.isStatic && level != levelOf(tp.symbol) =>
  ...
  private def levelError(sym: Symbol, tp: Type, pos: SrcPos)(using
      Context
  ): tp.type =
    def symStr =
      if !tp.isInstanceOf[ThisType] then sym.show
      else if sym.is(ModuleClass) then sym.sourceModule.show
    ...
    report.error(
      em"""access to $symStr from wrong staging level:
          ...
          | - but the access is at level $level.$hint""",
      pos
    )
    tp
