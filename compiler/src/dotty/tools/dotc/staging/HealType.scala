...
class HealType(pos: SrcPos)(using Context) extends TypeMap:
  ...
  /** If the type refers to a locally defined symbol (either directly, or in a
    ...
    * replacing it with a type tag of type `quoted.Type[T]`. The tag is recorded
    * by an instance of `QuoteTypeTags` directly if the splice is explicit or
    * indirectly by `tryHeal`.
    */
  ...
  private def healTypeRef(tp: TypeRef): Type =
    tp.prefix match
      ...
      case _: NamedType | _: ThisType | NoPrefix =>
        if levelInconsistentRootOfPath(tp).exists then tryHeal(tp)
        else tp
      case _ =>
        mapOver(tp)
  ...
  private def checkNotWildcardSplice(splice: TypeRef): Unit =
    splice.prefix.termSymbol.info.argInfos match
      case (tb: TypeBounds) :: _ =>
        report.error(
          em"Cannot stage $splice because it is an alias to a wildcard type",
          pos
        )
      case _ =>

  /** Return the root of this path if it is a variable defined in a previous
    * level. If the path is consistent, return NoSymbol.
    */
  private def levelInconsistentRootOfPath(tp: Type)(using Context): Symbol =
    tp match
      case tp @ NamedType(NoPrefix, _) if level > levelOf(tp.symbol) =>
        tp.symbol
      case tp: NamedType if !tp.symbol.isStatic =>
        levelInconsistentRootOfPath(tp.prefix)
      case tp: ThisType if level > levelOf(tp.cls) => tp.cls
      case _                                       => NoSymbol

  /** Try to heal reference to type `T` used in a higher level than its
    * definition. Returns a reference to a type tag generated by `QuoteTypeTags`
    * that contains a reference to a type alias containing the equivalent of
    * `${summon[quoted.Type[T]]}.Underlying`. Emits an error if `T` cannot be
    * healed and returns `T`.
    */
  protected def tryHeal(tp: TypeRef): Type =
    ...
    tag.tpe match
      ...
      case _ =>
        report.error(
          em"""Reference to $tp within quotes requires a given $reqType in scope.
                      |
                      |""",
          pos
        )
  ...
  private def levelError(sym: Symbol, tp: Type, pos: SrcPos): tp.type =
    report.error(
      em"""access to $sym from wrong staging level:
          ...
          | - but the access is at level $level""",
      pos
    )
    tp
end HealType
