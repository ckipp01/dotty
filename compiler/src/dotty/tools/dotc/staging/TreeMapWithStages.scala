...
abstract class TreeMapWithStages extends TreeMapWithImplicits:
  ...
  override def transform(tree: Tree)(using Context): Tree =
    if tree.source != ctx.source && tree.source.exists then
      ...
      reporting.trace(
        i"TreeMapWithStages.transform $tree at $level",
        staging,
        show = true
      ) {
        tree match
          case Block(stats, _) =>
            val defSyms = stats.collect { case defTree: DefTree =>
              defTree.symbol
            }
          ...
          case LambdaTypeTree(tparams, body) =>
            super.transform(tree)(using
              symbolsInCurrentLevel(tparams.map(_.symbol))
            )
          ...
          case tree: DefTree =>
            val paramSyms = tree match
              ...
              case _            => Nil
            super.transform(tree)(using
              symbolsInCurrentLevel(tree.symbol :: paramSyms)
            )

          case _ =>
            super.transform(tree)
      }
end TreeMapWithStages
