...
/** One of the results of a completion query.
  *
  * @param label
  *   The label of this completion result, or the text that this completion
  *   result should insert in the scope where the completion request happened.
  * @param description
  *   The description of this completion result: the fully qualified name for
...
object Completion:
  ...
  def completions(pos: SourcePosition)(using Context): (Int, List[Completion]) =
    ...
    computeCompletions(pos, path)(using
      Interactive.contextOfPath(path).withPhase(Phases.typerPhase)
    )
  ...
  /** Inspect `path` to determine what kinds of symbols should be considered.
    ...
    *   - a renaming import, and the cursor is on the renamee, accept both terms
    *     and types;
  ...
  def completionMode(path: List[Tree], pos: SourcePosition): Mode =
    path match
      ...
      case (ref: RefTree) :: _ =>
        if ref.name.isTermName then Mode.Term
        else if ref.name.isTypeName then Mode.Type
        else Mode.None
  ...
  /** When dealing with <errors> in varios palces we check to see if they are
    ...
    *
    * @param content
    *   The source content that we'll check the positions for the prefix
    * @param start
    *   The start position we'll start to look for the prefix at
    * @param end
    *   The end position we'll look for the prefix at
    * @return
    *   Either the full prefix including the ` or an empty string
    */
  private def checkBacktickPrefix(
      content: Array[Char],
      start: Int,
      end: Int
  ): String =
  ...
  /** Inspect `path` to determine the completion prefix. Only symbols whose name
    * start with the returned prefix should be considered.
    */
  def completionPrefix(path: List[untpd.Tree], pos: SourcePosition)(using
      Context
  ): String =
    path match
      case (sel: untpd.ImportSelector) :: _ =>
      ...
      case (ident: untpd.Ident) :: _ if ident.name == nme.ERROR =>
        checkBacktickPrefix(
          ident.source.content(),
          ident.span.start,
          ident.span.end
        )

      case (ref: untpd.RefTree) :: _ =>
        if ref.name == nme.ERROR then ""
  ...
  /** Inspect `path` to determine the offset where the completion result should
    * be inserted.
    */
  ...
  private def computeCompletions(pos: SourcePosition, path: List[Tree])(using
      Context
  ): (Int, List[Completion]) =
    ...
    val backtickedCompletions =
      describedCompletions.map(completion =>
        backtickCompletions(completion, hasBackTick)
      )

    val offset = completionOffset(path)

    interactiv.println(
      i"""completion with pos     = $pos,
                          |                prefix  = ${completer.prefix},
                          |                term    = ${completer.mode.is(
          Mode.Term
        )},
                          |                type    = ${completer.mode.is(
          Mode.Type
        )}
                          |                results = $backtickedCompletions%, %"""
    )
    (offset, backtickedCompletions)
  end computeCompletions
  ...
  /** Return the list of code completions with descriptions based on a mapping
    * from names to the denotations they refer to. If several denotations share
    * the same name, each denotation will be transformed into a separate
    * completion item.
    */
  def describeCompletions(
      completions: CompletionMap
  )(using Context): List[Completion] =
    for
      (name, denots) <- completions.toList
      denot <- denots
    yield Completion(name.show, description(denot), List(denot.symbol))
  ...
  /** Computes code completions depending on the context in which completion is
    * requested
    * @param mode
    ...
    *
    * For the results of all `xyzCompletions` methods term names and type names
    * are always treated as different keys in the same map and they never
    * conflict with each other.
    */
  class Completer(val mode: Mode, val prefix: String, pos: SourcePosition):
    /** Completions for terms and types that are currently in scope: the members
      * of the current class, local definitions and the symbols that have been
      * imported, recursively adding completions from outer scopes. In case a
      * name is ambiguous, no completions are returned for it. This mimics the
      * logic for deciding what is ambiguous used by the compiler. In general in
      * case of a name clash symbols introduced in more deeply nested scopes
      * have higher priority and shadow previous definitions with the same name
      * although:
      *   - imports with the same level of nesting cause an ambiguity
      *   - members and local definitions with the same level of nesting are
      *     allowed for overloading
      *   - an import is ignored if there is a local definition or a member
      *     introduced in the same scope (even if the import follows it
      *     syntactically)
      *   - a more deeply nested import shadowing a member or a local definition
      *     causes an ambiguity
      */
    def scopeCompletions(using context: Context): CompletionMap =
      val mappings = collection.mutable.Map
        .empty[Name, List[ScopedDenotations]]
      ...
      ctx.outersIterator.foreach { case ctx @ given Context =>
        if ctx.isImportContext then
          ...
          accessibleMembers(ctx.owner.thisType).groupByName.foreach {
            (name, denots) =>
              addMapping(name, ScopedDenotations(denots, ctx))
          }
        else if ctx.scope ne EmptyScope then
          ctx.scope.toList
            .filter(symbol => include(symbol, symbol.name))
      ...
      mappings.foreach { (name, denotss) =>
        ...
        // import a.C
        // import a.C
        def isSameSymbolImportedDouble =
          denotss.forall(_.denots == first.denots)

        def isScalaPackage(scopedDenots: ScopedDenotations) =
          scopedDenots.denots.exists(
            _.info.typeSymbol.owner == defn.ScalaPackageClass
          )

        def isJavaLangPackage(scopedDenots: ScopedDenotations) =
          scopedDenots.denots.exists(
            _.info.typeSymbol.owner == defn.JavaLangPackageClass
          )
        ...
        def isJavaLangAndScala =
          try
            denotss.forall(denots =>
              isScalaPackage(denots) || isJavaLangPackage(denots)
            )
        ...
        denotss.find(!_.ctx.isImportContext) match
          ...
          case None
              if isSingleImport || isImportedInDifferentScope || isSameSymbolImportedDouble =>
            resultMappings += name -> first.denots
          case None if isJavaLangAndScala =>
            denotss.foreach { denots =>
    ...
    def widenQualifier(qual: Tree)(using Context): Tree =
      qual.tpe.widenDealias match
        case widenedType if widenedType.isExactlyNothing =>
          qual.withType(widenedType)
        case appliedType: AppliedType => qual.withType(appliedType)
        case _                        => qual

    /** Completions for selections from a term. Direct members take priority
      * over members from extensions and so do members from extensions over
      * members from implicit conversions
      */
    ...
    /** Completions for members of `qual`'s type. These include inherited
      * definitions but not members added by extensions or implicit conversions
      */
    def directMemberCompletions(qual: Tree)(using Context): CompletionMap =
      if qual.tpe.isExactlyNothing then Map.empty
      else accessibleMembers(qual.tpe).groupByName

    /** Completions introduced by imports directly in this context. Completions
      * from outer contexts are not included.
      */
    private def importedCompletions(using Context): CompletionMap =
      ...
      def fromImport(
          name: Name,
          nameInScope: Name
      ): Seq[(Name, SingleDenotation)] =
        imp.site
          .member(name)
          .alternatives
          .collect {
            case denot if include(denot, nameInScope) => nameInScope -> denot
          }

      if imp == null then Map.empty
      else
        val givenImports = imp.importedImplicits
          .map { ref =>
            (ref.implicitName: Name, ref.underlyingRef.denot.asSingleDenotation)
          }
        ...
        val wildcardMembers =
          if imp.selectors.exists(_.imported.name == nme.WILDCARD) then
            val denots = accessibleMembers(imp.site)
              .filter(mbr =>
                !mbr.symbol.is(Given) && !imp.excluded.contains(
                  mbr.name.toTermName
                )
              )
            denots.groupByName
        ...
        val explicitMembers =
          ...
          val duplicatedNames =
            importNamesInScope.diff(importNamesInScope.distinct)
          val discardedNames = duplicatedNames ++ imp.excluded
          imp.reverseMapping.toList
            .filter { (nameInScope, _) =>
              !discardedNames.contains(nameInScope)
            }
    ...
    /** Completions from implicit conversions including old style extensions
      * using implicit classes
      */
    private def implicitConversionMemberCompletions(
        qual: Tree
    )(using Context): CompletionMap =
      if qual.tpe.isExactlyNothing || qual.tpe.isNullType then Map.empty
      else
    ...
    private def extensionCompletions(qual: Tree)(using Context): CompletionMap =
      ...
      def tryApplyingReceiverToExtension(
          termRef: TermRef
      ): Option[SingleDenotation] =
        ctx.typer
          .tryApplyingExtensionMethod(termRef, qual)
          .map { tree =>
      ...
      def extractMemberExtensionMethods(
          types: Seq[Type]
      ): Seq[(TermRef, TermName)] =
        object DenotWithMatchingName:
          def unapply(
              denot: SingleDenotation
          ): Option[(SingleDenotation, TermName)] =
            denot.name match
        ...
        types.flatMap { tp =>
          ...
          tpe
            ...
            .collect { case DenotWithMatchingName(denot, name) =>
      ...
      val extMethodsInScope = termCompleter.scopeCompletions.toList.flatMap {
        case (name, denots) =>
          denots.collect {
            case d: SymDenotation if d.isTerm => (d.termRef, name.asTermName)
          }
      }

      // 2. The extension method is a member of some given instance that is visible at the point of the reference.
      val givensInScope =
        ctx.implicits.eligible(defn.AnyType).map(_.implicitRef.underlyingRef)
      val extMethodsFromGivensInScope = extractMemberExtensionMethods(
        givensInScope
      )

      // 3. The reference is of the form r.m and the extension method is defined in the implicit scope of the type of r.
      val implicitScopeCompanions =
        ctx.run.nn.implicitScope(qual.tpe).companionRefs.showAsList
      val extMethodsFromImplicitScope = extractMemberExtensionMethods(
      ...
      val givensInImplicitScope = implicitScopeCompanions
        .flatMap(
          _.membersBasedOnFlags(required = GivenVal, excluded = EmptyFlags)
        )
        .map(_.info)
      val extMethodsFromGivensInImplicitScope = extractMemberExtensionMethods(
        givensInImplicitScope
      )

      val availableExtMethods =
        extMethodsFromGivensInImplicitScope ++ extMethodsFromImplicitScope ++ extMethodsFromGivensInScope ++ extMethodsInScope
    ...
    end extensionCompletions

    /** Include in completion sets only symbols that
      *   1. start with given name prefix, and 2. is not absent (info is not
      *      NoType) 3. are not a primary constructor, 4. have an existing
      *      source symbol, 5. are the module class in case of packages, 6. are
      *      mutable accessors, to exclude setters for `var`, 7. symbol is not a
      *      package object 8. symbol is not an artifact of the compiler 9. have
      *      same term/type kind as name prefix given so far
      */
    private def include(denot: SingleDenotation, nameInScope: Name)(using
        Context
    ): Boolean =
    ...
    private def extractRefinements(site: Type)(using
        Context
    ...
    /** @param site
      *   The type to inspect.
      * @return
      *   The members of `site` that are accessible and pass the include filter.
      */
    private def accessibleMembers(site: Type)(using
        Context
    ): Seq[SingleDenotation] =
      def appendMemberSyms(
          name: Name,
          buf: mutable.Buffer[SingleDenotation]
      ): Unit =
        try
          val member = site.member(name)
          if member.symbol.is(ParamAccessor) && !member.symbol.isAccessibleFrom(
              site
            )
          then buf ++= site.nonPrivateMember(name).alternatives
          else buf ++= member.alternatives
        catch case ex: TypeError =>

      val members =
        site.memberDenots(completionsFilter, appendMemberSyms).collect {
          case mbr
              if include(mbr, mbr.name)
                && mbr.symbol.isAccessibleFrom(site) =>
            mbr
        }
      val refinements =
        extractRefinements(site).filter(mbr => include(mbr, mbr.name))

      members ++ refinements

    /** Given `qual` of type T, finds all the types S such that there exists an
      * implicit conversion from T to S. It then applies conversion method for
      * proper type parameter resolution.
      *
      * @param qual
      *   The argument to which the implicit conversion should be applied.
      * @return
      *   The set of types after `qual` implicit conversion.
      */
    private def implicitConversionTargets(qual: Tree)(using
        Context
    ): Set[Type] =
      val typer = ctx.typer
      val conversions =
        new typer.ImplicitSearch(defn.AnyType, qual, pos.span).allImplicits
      val targets = conversions.map(_.tree.tpe)

      interactiv.println(
        i"implicit conversion targets considered: ${targets.toList}%, %"
      )
      targets
    ...
    extension [N <: Name](namedDenotations: Seq[(N, SingleDenotation)])
      ...
      def groupByName: CompletionMap =
        namedDenotations.groupMap((name, denot) => name)((name, denot) => denot)
  end Completer

  private type CompletionMap = Map[Name, Seq[SingleDenotation]]

  /** Temporary data structure representing denotations with the same name
    * introduced in a given scope as a member of a type, by a local definition
    * or by an import clause
    */
  ...
  /** The completion mode: defines what kinds of symbols should be included in
