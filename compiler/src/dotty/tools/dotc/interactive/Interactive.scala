...
/** High-level API to get information out of typed trees, designed to be used by
  * IDEs.
  *
  * @see
...
object Interactive:
  ...
  /** The type of the closest enclosing tree with a type containing position
    * `pos`.
    */
  def enclosingType(trees: List[SourceTree], pos: SourcePosition)(using
      Context
  ): Type =
    val path = pathTo(trees, pos)
    if path.isEmpty then NoType
    else path.head.tpe

  /** The closest enclosing tree with a symbol containing position `pos`, or the
    * `EmptyTree`.
    */
  def enclosingTree(trees: List[SourceTree], pos: SourcePosition)(using
      Context
  ): Tree =
  ...
  /** The source symbols that are the closest to `path`.
    *
    * If this path ends in an import, then this returns all the symbols that are
    * imported by this import statement.
    *
    * @param path
    ...
    *   sourceSymbol
    */
  def enclosingSourceSymbols(path: List[Tree], pos: SourcePosition)(using
      Context
  ): List[Symbol] =
    val syms = path match
  ...
  end enclosingSourceSymbols

  /** Check if `tree` matches `sym`. This is the case if the symbol defined by
    * `tree` equals `sym`, or the source symbol of tree equals sym, or `include`
    * is `overridden`, and `tree` is overridden by `sym`, or `include` is
    * `overriding`, and `tree` overrides `sym`.
    */
  def matchSymbol(tree: Tree, sym: Symbol, include: Include.Set)(using
      Context
  ): Boolean =

    def overrides(sym1: Symbol, sym2: Symbol) =
      sym1.owner.derivesFrom(sym2.owner) && sym1.overriddenSymbol(
        sym2.owner.asClass
      ) == sym2
  ...
  /** Find named trees with a non-empty position whose symbol match `sym` in
    * `trees`.
    *
  ...
  def namedTrees(trees: List[SourceTree], include: Include.Set, sym: Symbol)(
    ...
    else namedTrees(trees, include, matchSymbol(_, sym, include))

  /** Find named trees with a non-empty position satisfying `treePredicate` in
    * `trees`.
    *
    * @param trees
      ...
      new untpd.TreeTraverser:
        private def handle(utree: untpd.NameTree): Unit =
          ...
          if tree.symbol.exists
            ...
            && treePredicate(tree)
          then buf += SourceTree(tree, source)
        override def traverse(tree: untpd.Tree)(using Context) =
          tree match
            ...
            case utree: untpd.ValOrDefDef if tree.hasType =>
              handle(utree)
              if include.isLocal then traverseChildren(tree)
  ...
  /** The reverse path to the node that closest encloses position `pos`, or
    * `Nil` if no such path exists. If a non-empty path is returned it starts
    * with the tree closest enclosing `pos` and ends with an element of `trees`.
    *
    * Note that if the given `pos` points out places for incomplete parses, this
    * method returns `errorTermTree` (`Literal(Consotant(null)`).
    *
    * @see
    *   https://github.com/lampepfl/dotty/issues/15294
    */
  def pathTo(trees: List[SourceTree], pos: SourcePosition)(using
      Context
  ): List[Tree] =
  ...
  private def pathTo(trees: List[Tree], span: Span)(using Context): List[Tree] =
    if trees.exists(_.span.contains(span)) then
      NavigateAST
        .pathTo(span, trees, skipZeroExtent = true)
        .collect { case t: untpd.Tree => t }
        .dropWhile(!_.hasType)
        .asInstanceOf[List[tpd.Tree]]
    else Nil
      ...
      try
        encl match
          ...
          case tree: DefDef =>
            ...
            localCtx
          case tree: MemberDef =>
            if tree.symbol.exists then outer.localContext(tree, tree.symbol)
            else outer
          ...
          case tree @ Template(constr, _, self, _) =>
            if (constr :: self :: tree.parentsOrDerived).contains(nested) then
              outer
            else
              contextOfStat(
                tree.body,
  ...
  /** Find the definitions of the symbol at the end of `path`. In the case of an
    * import node, all imported symbols will be considered.
    *
    * @param path
  ...
  /** Find the definitions of `symbols`.
    ...
    * @return
    *   The definitions for the symbols in `symbols`, and if `includeOverridden`
  ...
  def findDefinitions(
    ...
    val include = Include.definitions | Include.overriding |
      (if includeOverridden then Include.overridden else Include.empty)
    symbols.flatMap { sym =>
      val name = sym.name.sourceModuleName.toString
      val includeLocal =
        if sym.exists && sym.isLocal then Include.local else Include.empty
      val trees =
        if includeExternal then driver.allTreesContaining(name)
  ...
  def localize(
      symbol: Symbol,
      sourceDriver: InteractiveDriver,
      targetDriver: InteractiveDriver
  ): Symbol =
    ...
    if sourceDriver == targetDriver then symbol
      ...
      in(targetDriver) {
        ...
        owners.tail.foldLeft(base) { (prefix, symbolName) =>
          if prefix.exists then prefix.info.member(symbolName).symbol
          else NoSymbol
        }
      }
  end localize
  ...
  /** Return a predicate function that determines whether a given `NameTree` is
    * an implementation of `sym`.
    *
    * @param sym
    *   The symbol whose implementations to find.
    * @return
    *   A function that determines whether a `NameTree` is an implementation of
    *   `sym`.
    */
  def implementationFilter(sym: Symbol)(using Context): NameTree => Boolean =
    if sym.isClass then {
      case td: TypeDef =>
        ...
        (treeSym != sym || !treeSym.isOneOf(AbstractOrTrait)) && treeSym
          .derivesFrom(sym)
      case _ =>
        false
    }
  ...
  /** Is this tree using a renaming introduced by an import statement or an
    * alias for `this`?
    *
    * @param tree
    *   The tree to inspect
    * @return
    *   True, if this tree's name is different than its symbol's name,
    *   indicating that it uses a renaming introduced by an import statement or
    *   an alias for `this`.
    */
