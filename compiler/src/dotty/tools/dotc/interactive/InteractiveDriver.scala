...
import Contexts.*, Names.*, NameOps.*, Symbols.*, SymDenotations.*, Trees.*,
  Types.*
...
class InteractiveDriver(val settings: List[String]) extends Driver:
  ...
  private val myCompilationUnits =
    new mutable.LinkedHashMap[URI, CompilationUnit]
  ...
  private val (zipClassPaths, dirClassPaths) =
    currentCtx.platform.classPath(using currentCtx) match
  ...
  private val zipClassPathClasses: Seq[TypeName] =
    ...
    for cp <- zipClassPaths do classesFromZip(cp.zipFile, names)
    names
  ...
  /** The trees for all the source files in this project.
    *
    * This includes the trees for the buffers that are presently open in the
    * IDE, and the trees from the target directory.
    */
  ...
  def sourceTreesContaining(id: String)(using Context): List[SourceTree] =
    ...
    val fromCompilationOutput =
      ...
      if output.isDirectory then classesFromDir(output.jpath, classNames)
      else classesFromZip(output.file, classNames)
  ...
  /** All the trees for this project.
    *
    * This includes the trees of the sources of this project, along with the
    * trees that are found on this project's classpath.
    */
  ...
  def allTreesContaining(id: String)(using Context): List[SourceTree] =
    ...
    val fromClassPath = (dirClassPathClasses ++ zipClassPathClasses).flatMap {
      cls =>
        treesFromClassName(cls, id)
    }
  ...
  def run(uri: URI, sourceCode: String): List[Diagnostic] =
    run(uri, toSource(uri, sourceCode))
  ...
  def run(uri: URI, source: SourceFile): List[Diagnostic] =
    ...
    try
      ...
      val unit =
        if ctxRun.units.nonEmpty then ctxRun.units.head
        else ctxRun.suspendedUnits.head
      val t = unit.tpdTree
      cleanup(t)
  ...
  /** The `SourceTree`s that define the class `className` and/or module
    * `className`.
    *
    * @see
    *   SourceTree.fromSymbol
    */
  private def treesFromClassName(className: TypeName, id: String)(using
      Context
  ): List[SourceTree] =
  ...
  private def classesFromZip(
      file: File,
      buffer: mutable.ListBuffer[TypeName]
  ): Unit =
            ...
            if !attrs.isDirectory then
              ...
              if name.endsWith(tastySuffix) then
                buffer += dir
                  ...
                  .toString
                  .replace("/", ".")
                  .stripSuffix(tastySuffix)
                  .toTypeName
            FileVisitResult.CONTINUE
      )
    catch case _: NoSuchFileException =>

  private def topLevelTrees(
      topTree: Tree,
      source: SourceFile
  ): List[SourceTree] =
  ...
  /** Remove attachments and error out completers. The goal is to avoid having a
    * completer hanging in a typed tree which can capture the context of a
    * previous run. Note that typed trees can have untyped or partially typed
    * children if the source contains errors.
    */
      ...
      tree.foreachSubTree { t =>
        if t.symbol.exists && t.hasType then
          ...
          t.symbol.annotations.foreach { annot =>
             ...
             */
            if annot.isEvaluated && !seen(annot.tree) then
