...
import core.*
import ast.*
import Contexts.*, ContextOps.*, Constants.*, Types.*, Symbols.*, Names.*,
  Flags.*, Decorators.*
import ErrorReporting.*, Annotations.*, Denotations.*, SymDenotations.*,
  StdNames.*
import util.SrcPos
import NameOps.*
...
trait TypeAssigner:
  ...
  /** The qualifying class of a this or super with prefix `qual` (which might be
    * empty).
    * @param packageOK
    *   The qualifier may refer to a package.
    */
  def qualifyingClass(tree: untpd.Tree, qual: Name, packageOK: Boolean)(using
      Context
  ): Symbol =
    def qualifies(sym: Symbol) =
      sym.isClass && (qual.isEmpty ||
        sym.name == qual ||
      ...
      case _ =>
        report.error(
          if qual.isEmpty then
            em"$tree can be used only in a class, object, or template"
  ...
  def seqToRepeated(tree: Tree)(using Context): Tree =
    toRepeated(tree, defn.SeqClass)

  def arrayToRepeated(tree: Tree)(using Context): Tree =
    toRepeated(tree, defn.ArrayClass)

  /** A denotation exists really if it exists and does not point to a stale
    * symbol.
    */
  ...
  /** If `tpe` is a named type, return the type with those alternatives as
    * denotations which are accessible (or NoType, if no alternatives are
    * accessible).
    *
    * Also performs the following normalizations on the type `tpe`. (1) if the
    * owner of the denotation is a package object, it is assured that the
    * package object shows up as the prefix. (2) in Java compilation units,
    * `Object` is replaced by `defn.FromJavaObjectType`
    */
  ...
  def ensureAccessible(tpe: Type, superAccess: Boolean, pos: SrcPos)(using
      Context
  ): Type =
  ...
  /** Return a potentially skolemized version of `qualTpe` to be used as a
    * prefix when selecting `name`.
    *
    * @see
  ...
  /** The type of the selection `tree`, where `qual1` is the typed qualifier
    * part.
    */
  def selectionType(tree: untpd.RefTree, qual1: Tree)(using Context): Type =
    ...
    name match
      ...
      case p.arrayUpdate =>
        MethodType(defn.IntType :: arrayElemType :: Nil, defn.UnitType)
      ...
      case _ =>
        ...
        val mbr =
          if ctx.isJava then ctx.javaFindMember(name, pre)
          else qualType.findMember(name, pre)

        if reallyExists(mbr) then qualType.select(name, mbr)
        else if qualType.isErroneous || name.toTermName == nme.ERROR then
  ...
  def notAMemberErrorType(tree: untpd.Select, qual: Tree)(using
      Context
  ): ErrorType =
    ...
    val foundWithoutNull = qualType match
      ...
      case _ => false
    def addendum = err.selectErrorAddendum(
      tree,
      qual,
      qualType,
      importSuggestionAddendum,
      foundWithoutNull
    )
    val msg: Message =
      if tree.name == nme.CONSTRUCTOR then
        em"$qualType does not have a constructor"
      else NotAMember(qualType, tree.name, kind, addendum)
    errorType(msg, tree.srcPos)

  def inaccessibleErrorType(tpe: NamedType, superAccess: Boolean, pos: SrcPos)(
      using Context
  ): Type =
  ...
  def processAppliedType(tree: untpd.Tree, tp: Type)(using Context): Type =
    tp match
      case AppliedType(tycon, args) =>
        ...
        else if constr == defn.orType then
  ...
  def assignType(tree: untpd.Select, qual: Tree)(using Context): Select =
    ...
    val checkedType =
      ensureAccessible(rawType, qual.isInstanceOf[Super], tree.srcPos)
    val ownType = checkedType.orElse(notAMemberErrorType(tree, qual))
    assignType(tree, ownType)

  /** Normalize type T appearing in a new T by following eta expansions to avoid
    * higher-kinded types.
    */
    ...
    qualType match
      ...
      case qtype @ ThisType(_) =>
        ...
        def findMixinSuper(site: Type): Type =
          site.parents filter (_.typeSymbol.name == mix.name) match
            case p :: Nil =>
              p.typeConstructor
            case Nil =>
  ...
  /** Substitute argument type `argType` for parameter `pref` in type `tp`,
    * skolemizing the argument type if it is not stable and `pref` occurs in
    * `tp`.
    */
  def safeSubstParam(tp: Type, pref: ParamRef, argType: Type)(using
      Context
  ): Type =
  ...
  /** Substitute types of all arguments `args` for corresponding `params` in
    * `tp`. The number of parameters `params` may exceed the number of
    * arguments. In this case, only the common prefix is substituted.
    */
  def safeSubstParams(tp: Type, params: List[ParamRef], argTypes: List[Type])(
      using Context
  ): Type = argTypes match
  ...
  def safeSubstMethodParams(mt: MethodType, argTypes: List[Type])(using
      Context
  ): Type =
    if mt.isResultDependent then
      safeSubstParams(mt.resultType, mt.paramRefs, argTypes)
    else mt.resultType

  def assignType(tree: untpd.Apply, fn: Tree, args: List[Tree])(using
      Context
  ): Apply =
    val ownType = fn.tpe.widen match
      case fntpe: MethodType =>
        if fntpe.paramInfos.hasSameLengthAs(
            args
          ) || ctx.phase.prev.relaxedTyping
        then
          ...
          errorType(
            em"wrong number of arguments at ${ctx.phase.prev} for $fntpe: ${fn.tpe}, expected: ${fntpe.paramInfos.length}, found: ${args.length}",
            tree.srcPos
          )
      case t =>
        if ctx.settings.Ydebug.value then new FatalError("").printStackTrace()
        errorType(err.takesNoParamsMsg(fn, ""), tree.srcPos)
    ConstFold.Apply(tree.withType(ownType))

  def assignType(tree: untpd.TypeApply, fn: Tree, args: List[Tree])(using
      Context
  ): TypeApply =
    def fail =
      tree.withType(errorType(err.takesNoParamsMsg(fn, "type "), tree.srcPos))
      ...
      case pt: TypeLambda =>
        tree.withType {
          ...
          if hasNamedArg(args) then
            ...
            for case NamedArg(name, arg) <- args do
              if namedArgMap.contains(name) then
                report.error(DuplicateNamedTypeParameter(name), arg.srcPos)
              else if !paramNames.contains(name) then
                report.error(
                  UndefinedNamedTypeParameter(name, paramNames),
                  arg.srcPos
                )
              else namedArgMap(name) = arg.tpe
            ...
            val normArgs: List[Type] = paramNames.zipWithIndex.map {
              case (pname, idx) =>
  ...
  def assignType(tree: untpd.Block, stats: List[Tree], expr: Tree)(using
      Context
  ): Block =
    tree.withType(avoidingType(expr, stats))

  def assignType(tree: untpd.Inlined, bindings: List[Tree], expansion: Tree)(
      using Context
  ): Inlined =
  ...
  def assignType(tree: untpd.Closure, meth: Tree, target: Tree)(using
      Context
  ): Closure =
    tree.withType(
      if target.isEmpty then
        meth.tpe.widen
          .toFunctionType(isJava = meth.symbol.is(JavaDefined), tree.env.length)
      else target.tpe
    )

  def assignType(tree: untpd.CaseDef, pat: Tree, body: Tree)(using
      Context
  ): CaseDef =
    val ownType =
      if body.isType then
        val getParams = new TreeAccumulator[mutable.ListBuffer[TypeSymbol]]:
          def apply(ps: mutable.ListBuffer[TypeSymbol], t: Tree)(using
              Context
          ) = t match
            case t: Bind if t.symbol.isType =>
              foldOver(ps += t.symbol.asType, t)
            case _ => foldOver(ps, t)
        val params1 =
          getParams(new mutable.ListBuffer[TypeSymbol](), pat).toList
        ...
        val matchCase2 =
          if params2 eq params1 then matchCase1
          else matchCase1.substSym(params1, params2)
        HKTypeLambda.fromParams(params2, matchCase2)
      else body.tpe
    tree.withType(ownType)
  end assignType

  def assignType(tree: untpd.Match, scrutinee: Tree, cases: List[CaseDef])(using
      Context
  ): Match =
  ...
  def assignType(tree: untpd.WhileDo)(using Context): WhileDo =
    tree.withType(
      if tree.cond eq EmptyTree then defn.NothingType else defn.UnitType
    )

  def assignType(tree: untpd.Try, expr: Tree, cases: List[CaseDef])(using
      Context
  ): Try =
    if cases.isEmpty then tree.withType(expr.tpe)
    else tree.withType(TypeComparer.lub(expr.tpe :: cases.tpes))

  def assignType(tree: untpd.SeqLiteral, elems: List[Tree], elemtpt: Tree)(using
      Context
  ): SeqLiteral =
    tree.withType(seqLitType(tree, elemtpt.tpe))

  def assignType(tree: untpd.SingletonTypeTree, ref: Tree)(using
      Context
  ): SingletonTypeTree =
    tree.withType(ref.tpe)

  /** Assign type of RefinedType. Refinements are typed as if they were members
    * of refinement class `refineCls`.
    */
  def assignType(
      tree: untpd.RefinedTypeTree,
      parent: Tree,
      refinements: List[Tree],
      refineCls: ClassSymbol
  )(using Context): RefinedTypeTree =
    def addRefinement(parent: Type, refinement: Tree): Type =
      val rsym = refinement.symbol
      val rinfo = if rsym.is(Accessor) then rsym.info.resultType else rsym.info
      if rinfo.isError then rinfo
      else if !rinfo.exists then
        parent // can happen after failure in self type definition
      else RefinedType(parent, rsym.name, rinfo)
    val refined = refinements.foldLeft(parent.tpe)(addRefinement)
    tree.withType(
      RecType.closeOver(rt => refined.substThis(refineCls, rt.recThis))
    )

  def assignType(tree: untpd.AppliedTypeTree, tycon: Tree, args: List[Tree])(
      using Context
  ): AppliedTypeTree =
  ...
  def assignType(
      ...
      tparamDefs: List[TypeDef],
      body: Tree
  )(using Context): LambdaTypeTree =
    ...
    tree.withType(
      HKTypeLambda.fromParams(validParams.map(_.symbol.asType), body.tpe)
    )

  def assignType(
      tree: untpd.MatchTypeTree,
      bound: Tree,
      scrutinee: Tree,
      cases: List[CaseDef]
  )(using Context): MatchTypeTree =
    val boundType = if bound.isEmpty then defn.AnyType else bound.tpe
    tree.withType(MatchType(boundType, scrutinee.tpe, cases.tpes))

  def assignType(tree: untpd.ByNameTypeTree, result: Tree)(using
      Context
  ): ByNameTypeTree =
    tree.withType(ExprType(result.tpe))

  def assignType(tree: untpd.TypeBoundsTree, lo: Tree, hi: Tree, alias: Tree)(
      using Context
  ): TypeBoundsTree =
    tree.withType(
  ...
  def assignType(tree: untpd.Alternative, trees: List[Tree])(using
      Context
  ): Alternative =
  ...
  def assignType(tree: untpd.ValDef, sym: Symbol)(using Context): ValDef =
    tree.withType(if sym.exists then assertExists(sym.termRef) else NoType)
  ...
  def assignType(tree: untpd.Annotated, arg: Tree, annot: Tree)(using
      Context
  ): Annotated =
    assert(
      tree.isType
    ) // annotating a term is done via a Typed node, can't use Annotate directly
...
object TypeAssigner extends TypeAssigner:
  def seqLitType(tree: untpd.SeqLiteral, elemType: Type)(using Context) =
    tree match
      case tree: untpd.JavaSeqLiteral => defn.ArrayOf(elemType)
      case _ =>
        if ctx.erasedTypes then defn.SeqType
        else defn.SeqType.appliedTo(elemType)
