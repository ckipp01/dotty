...
object ImportInfo:
  ...
  def rootImport(ref: RootRef)(using Context): ImportInfo =
    var selectors =
      untpd.ImportSelector(
        untpd.Ident(nme.WILDCARD)
      ) // import all normal members...
        :: untpd.ImportSelector(
          untpd.Ident(nme.EMPTY)
        ) // ... and also all given members
        :: Nil
    if ref.isPredef then // do not import any2stringadd
      selectors = untpd.ImportSelector(
        untpd.Ident(nme.any2stringadd),
        untpd.Ident(nme.WILDCARD)
      )
        :: selectors
    ...
    def sym(using Context) =
      val expr = tpd.Ident(
        ref.refFn()
      ) // refFn must be called in the context of ImportInfo.sym
  ...
  extension (c: Context)
    def withRootImports(rootRefs: List[RootRef])(using Context): Context =
      rootRefs.foldLeft(c)((ctx, ref) =>
        ctx.fresh.setImportInfo(rootImport(ref))
      )
...
/** Info relating to an import clause
  ...
  * @param qualifier
  *   The import qualifier, or EmptyTree for root imports. Defined for all
  *   explicit imports from ident or select nodes.
  * @param isRootImport
  *   true if this is one of the implicit imports of scala, java.lang,
...
class ImportInfo(
    ...
    val isRootImport: Boolean = false
) extends Showable:
  ...
  def excluded: Set[TermName] =
    ensureInitialized(); myExcluded.nn

  /** A mapping from original to renamed names */
  def forwardMapping: SimpleIdentityMap[TermName, TermName] =
    ensureInitialized(); myForwardMapping.nn

  /** A mapping from renamed to original names */
  def reverseMapping: SimpleIdentityMap[TermName, TermName] =
    ensureInitialized(); myReverseMapping.nn

  /** Does the import clause contain wildcard selectors (both `_` and `given`
    * count)?
    */
  def isWildcardImport: Boolean =
    ensureInitialized(); myWildcardImport

  /** Does the import clause have at least one `given` selector? */
  def isGivenImport: Boolean =
    ensureInitialized(); myGivenImport
  ...
  private def ensureInitialized(): Unit = if myExcluded == null then
    ...
    for sel <- selectors do
      if sel.isWildcard then
        ...
        if sel.rename != sel.name then myExcluded = myExcluded.nn + sel.name
        if sel.rename != nme.WILDCARD then
          myForwardMapping =
            myForwardMapping.uncheckedNN.updated(sel.name, sel.rename)
          myReverseMapping =
            myReverseMapping.uncheckedNN.updated(sel.rename, sel.name)
  ...
  def importedImplicits(using Context): List[ImplicitRef] =
    ...
    if isWildcardImport then
          ...
          if renamed == null then // not explicitly named
            val isEligible =
              isGivenImport // a given import pulls in implicits and givens
                || ctx.mode.is(
                  Mode.FindHiddenImplicits
                ) // consider both implicits and givens for error reporting
                || ref.symbol.is(
                  Implicit
                ) // a wildcard `_` import only pulls in implicits
            val bound = if isGivenImport then givenBound else wildcardBound
            if isEligible && ref.denot.asSingleDenotation.matchesImportBound(
                bound
              )
            then ref :: Nil
            else Nil
      ...
      for
        ...
        denot <- pre
          .member(reverseMapping(renamed).nn)
          .altsWith(_.isOneOf(GivenOrImplicitVal))
      yield
  ...
  end importedImplicits

  /** The root import symbol hidden by this symbol, or NoSymbol if no such
    * symbol is hidden. Note: this computation needs to work even for
    * un-initialized import infos, and is not allowed to force initialization.
    *
    * TODO: Once we have fully bootstrapped, I would prefer if we expressed
    * unimport with an `override` modifier, and generalized it to all imports. I
    * believe this would be more transparent than the current set of conditions.
    * E.g.
    *
    * override import Predef.{any2stringAdd => _, StringAdd => _, _} // disables
    * String + override import java.lang.{} // disables all imports
    */
  def unimported(using Context): Symbol =
    if myUnimported == null then
      ...
      def maybeShadowsRoot = symNameOpt match
        ...
        case None          => false
      myUnimported =
        if maybeShadowsRoot && defn.rootImportTypes.exists(_.symbol == sym) then
          sym
        else NoSymbol
      assert(myUnimported != null)
    myUnimported.uncheckedNN

  private val isLanguageImport: Boolean =
    untpd.languageImport(qualifier).isDefined

  private var myUnimported: Symbol | Null = _

  private var featureCache: SimpleIdentityMap[TermName, java.lang.Boolean] =
  ...
  /** Does this import clause enable or disable `feature`?
    ...
    * @return
    *   Some(true) if `feature` is imported Some(false) if `feature` is excluded
