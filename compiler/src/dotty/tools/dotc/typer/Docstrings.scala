...
import core.*
import Contexts.*, Symbols.*, Decorators.*, Comments.{*, given}
import ast.tpd

object Docstrings:

  /** Expands or cooks the documentation for `sym` in class `owner`. The
    * expanded comment will directly replace the original comment in the doc
    * context.
    *
    * The expansion registers `@define` sections, and will replace `@inheritdoc`
    * and variable occurrences in the comments.
    *
  ...
  private def expand(sym: Symbol, owner: Symbol)(using
      Context
  )(using docCtx: ContextDocstrings): Option[Comment] =
        ...
        expandComment(sym).map { expanded =>
          val typedUsecases = expanded.usecases.map { usecase =>
            ...
            ctx.typer.typedStats(usecase.untpdCode :: Nil, owner)._1 match
              ...
              case _ =>
                report.error(
                  em"`@usecase` was not a valid definition",
                  ctx.source.atSpan(usecase.codePos)
                )
                usecase
          }
  ...
  private def expandComment(sym: Symbol, owner: Symbol, comment: Comment)(using
      Context
  )(using docCtx: ContextDocstrings): Comment =
  ...
  private def expandComment(
      sym: Symbol
  )(using Context)(using docCtx: ContextDocstrings): Option[Comment] =
    if sym eq NoSymbol then None
    else
      for
