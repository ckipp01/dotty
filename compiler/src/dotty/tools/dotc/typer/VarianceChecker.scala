...
/** Provides `check` method to check that all top-level definitions in tree are
  * variance correct. Does not recurse inside methods. The method should be
  * invoked once for each Template.
  */
object VarianceChecker:
  ...
  /** Check that variances of type lambda correspond to their occurrences in its
    * body. Note: this is achieved by a mechanism separate from checking class
    * type parameters. Question: Can the two mechanisms be combined in one?
    */
  def checkLambda(tree: tpd.LambdaTypeTree, bounds: TypeBounds)(using
      Context
  ): Unit =
      ...
      case tl: HKTypeLambda if tl.isDeclaredVarianceLambda =>
        val checkOK = new TypeAccumulator[Boolean]:
          ...
          def error(tref: TypeParamRef) =
            ...
            report.error(
              em"${varianceLabel(v)} type parameter $paramName occurs in ${varianceLabel(variance)} position in ${tl.resType}",
              pos
            )
...
class VarianceChecker(using Context):
  ...
  private object Validator extends TypeAccumulator[Option[VarianceError]]:
    ...
    /** The variance of a symbol occurrence of `tvar` seen at the level of the
      * definition of `base`. The search proceeds from `base` to the owner of
      * `tvar`. Initially the state is covariant, but it might change along the
      * search.
      */
    def relativeVariance(
        tvar: Symbol,
        base: Symbol,
        v: Variance = Covariant
    ): Variance = /*trace(i"relative variance of $tvar wrt $base, so far: $v")*/
      if base == tvar.owner then v
      else if base.is(Param) && base.owner.isTerm && !base.owner.isAllOf(
          PrivateLocal
        )
      then relativeVariance(tvar, paramOuter(base.owner), flip(v))
      else if base.owner.isTerm || base.owner.is(Package) || base.isAllOf(
          PrivateLocal
        )
      then Bivariant
    ...
    /** The next level to take into account when determining the relative
      * variance with a method parameter as base. The method is always skipped.
      * If the method is a constructor, we also skip its class owner, because
      * constructors are not checked for variance relative to the type
      * parameters of their own class. On the other hand constructors do count
      * for checking the variance of type parameters of enclosing classes. I
      * believe the Scala 2 rules are too lenient in that respect.
      */
    ...
    private def checkVarianceOfSymbol(tvar: Symbol): Option[VarianceError] =
      ...
      if relative == Bivariant then None
        ...
        def base_s = s"$base in ${base.owner}" + (if base.owner.isClass then ""
                                                  else
                                                    " in " + base.owner.enclosingClass
        )
        report.log(
          s"verifying $tvar_s is ${varianceLabel(required)} at $base_s"
        )
    ...
    def apply(status: Option[VarianceError], tp: Type): Option[VarianceError] =
      trace(s"variance checking $tp of $base at $variance", variances) {
        try
          if status.isDefined then status
          else
            tp match
              case tp: TypeRef =>
                val sym = tp.symbol
                if sym.isOneOf(VarianceFlags) && base.isContainedIn(sym.owner)
                then checkVarianceOfSymbol(sym)
                else
                  sym.info match
              ...
              case tp: MethodOrPoly =>
                this(
                  status,
                  tp.resultType
                ) // params will be checked in their TypeDef or ValDef nodes.
              case AnnotatedType(_, annot)
                  if annot.symbol == defn.UncheckedVarianceAnnot =>
                status
              case tp: ClassInfo =>
          ...
          case ex: Throwable =>
            handleRecursive("variance check of", tp.show, ex)
      }
  ...
  private object Traverser extends TreeTraverser:
    def checkVariance(sym: Symbol, pos: SrcPos) =
      Validator.validateDefinition(sym) match
        case Some(VarianceError(tvar, required)) =>
          def msg =
            val enumAddendum =
              val towner = tvar.owner
              if towner.isAllOf(EnumCase) && towner.isClass && tvar.is(
                  Synthetic
                )
              then
                val example =
            ...
            em"${varianceLabel(tvar.flags)} $tvar occurs in ${varianceLabel(
                required
              )} position in type ${sym.info} of $sym$enumAddendum"
          if migrateTo3 &&
            (sym.owner.isConstructor || sym.ownersIterator.exists(
              _.isAllOf(ProtectedLocal)
            ))
          then
    ...
    override def traverse(tree: Tree)(using Context) =
      ...
      def skip = !sym.exists
        || sym.name.is(
          InlineAccessorName
        ) // TODO: should we exclude all synthetic members?
        || sym.isAllOf(
          LocalParamAccessor
        ) // local class parameters are construction only
        || sym.is(
          TypeParam
        ) && sym.owner.isClass // already taken care of in primary constructor of class
      try
        tree match
          ...
          case _ =>
      catch case ex: TypeError => report.error(ex, tree.srcPos.focus)
    end traverse
  end Traverser
end VarianceChecker
