...
trait Deriving:
  ...
  /** A helper class to derive type class instances for one class or object
    ...
    * @param codePos
    *   The default position that should be given to generic synthesized
    *   infrastructure code that is not connected with a `derives` instance.
    */
  class Deriver(cls: ClassSymbol, codePos: SrcPos)(using Context):
    ...
    /** A version of Type#underlyingClassRef that works also for higher-kinded
      * types
      */
    ...
    /** Enter type class instance with given name and info in current scope,
      * provided an instance with the same name does not exist already.
      * @param reportErrors
      *   Report an error if an instance with the same name exists already
      */
    private def addDerivedInstance(
        clsName: Name,
        info: Type,
        pos: SrcPos
    ): Unit =
      val instanceName = "derived$".concat(clsName)
      if ctx.denotNamed(instanceName).exists then
        ...
        val flags =
          if info.isInstanceOf[MethodOrPoly] then GivenMethod else Given | Lazy
        synthetics +=
          newSymbol(
            ctx.owner,
            instanceName,
    ...
    /** Check derived type tree `derived` for the following well-formedness
      * conditions: (1) It must be a class type with a stable prefix (@see
      * checkClassTypeWithStablePrefix)
      *
      * (2) It must belong to one of the following three categories: (a) a
      * single parameter type class with a parameter which matches the kind of
      * the deriving ADT (b) a single parameter type class with a parameter of
      * kind * and an ADT with one or more type parameter of kind * (c) the
      * CanEqual type class
      *
      * See detailed descriptions in deriveSingleParameter and deriveCanEqual
      * below.
      *
      * If it passes the checks, enter a type class instance for it in the
      * current scope.
      *
      * See test run/typeclass-derivation2, run/poly-kinded-derives and
      * pos/derive-eq for examples that spell out what would be generated.
      *
      * Note that the name of the derived method contains the name in the
      * derives clause, not the underlying class name. This allows one to
      * disambiguate derivations of type classes that have the same name but
      * different prefixes through selective aliasing.
      */
    private def processDerivedInstance(derived: untpd.Tree): Unit =
      val originalTypeClassType =
        typedAheadType(derived, AnyTypeConstructorProto).tpe
      ...
      def cannotBeUnified =
        report.error(
          em"${cls.name} cannot be unified with the type argument of ${typeClass.name}",
          derived.srcPos
        )

      def addInstance(
          derivedParams: List[TypeSymbol],
          evidenceParamInfos: List[List[Type]],
          instanceTypes: List[Type]
      ): Unit =
        ...
        val monoInfo =
          if evidenceParamInfos.isEmpty then resultType
            ...
            ImplicitMethodType(
              evidenceParamInfos.map(typeClassType.appliedTo),
              resultType
            )
        val derivedInfo =
        ...
        addDerivedInstance(
          originalTypeClassType.typeSymbol.name,
          derivedInfo,
          derived.srcPos
        )
      ...
      def deriveSingleParameter: Unit =
        ...
        val alignedTypeClassParamInfos =
          typeClassParamInfos.takeRight(alignedClsParamInfos.length)

        if (instanceArity == clsArity || instanceArity > 0) && sameParamKinds(
            alignedClsParamInfos,
            alignedTypeClassParamInfos
          )
        then
        ...
        else cannotBeUnified
      end deriveSingleParameter
      ...
      def deriveCanEqual: Unit =
        ...
        val clsParamss: List[List[TypeSymbol]] = cls.typeParams.map { tparam =>
          typeClassParams.map(tcparam =>
            tparam
              .copy(name = s"${tparam.name}_$$_${tcparam.name}".toTypeName)
      ...
      if typeClassArity == 1 then deriveSingleParameter
      else if typeClass == defn.CanEqualClass then deriveCanEqual
      else if typeClassArity == 0 then
        report.error(
          em"type ${typeClass.name} in derives clause of ${cls.name} has no type parameters",
          derived.srcPos
        )
      else cannotBeUnified
    end processDerivedInstance

    /** Create symbols for derived instances and infrastructure, append them to
      * `synthetics` buffer, and enter them into class scope. Also, add generic
      * instances if needed.
      */
    ...
    def syntheticDefs: List[tpd.Tree] =
      ...
      def typeclassInstance(sym: Symbol)(using
          Context
      ): List[List[tpd.Tree]] => tpd.Tree =
        (paramRefss: List[List[tpd.Tree]]) =>
          ...
          val vparams =
            if vparamRefss.isEmpty then Nil
            else vparamRefss.head.map(_.symbol.asTerm)
          ...
          if companion.termSymbol.exists then typed(rhs, resultType)
            ...
            errorTree(
              rhs,
              em"$resultType cannot be derived since ${resultType.typeSymbol} has no companion object"
            )
      end typeclassInstance

      def syntheticDef(sym: Symbol): Tree =
        inContext(ctx.fresh.setOwner(sym).setNewScope) {
    ...
    def finalize(stat: tpd.TypeDef): tpd.Tree =
      ...
      tpd.cpy.TypeDef(stat)(rhs =
        tpd.cpy.Template(templ)(body = templ.body ++ syntheticDefs)
      )
  end Deriver
end Deriving
