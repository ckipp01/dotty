...
object Typer:
  ...
  /** The precedence of bindings which determines which of several bindings will
    * be accessed by an Ident.
    */
  enum BindingPrec:
    case NothingBound, PackageClause, WildImport, NamedImport, Inheritance,
      Definition
  ...
  def assertPositioned(tree: untpd.Tree)(using Context): Unit =
    if !tree.isEmpty && !tree
        .isInstanceOf[untpd.TypedSplice] && ctx.typerState.isGlobalCommittable
    then
      assert(
        tree.span.exists,
        i"position not set for $tree # ${tree.uniqueId} of ${tree.getClass} in ${tree.source}"
      )
  ...
  /** An attachment on a Select node with an `apply` field indicating that the
    * `apply` was inserted by the Typer.
    */
  ...
  /** An attachment on a tree `t` occurring as part of a `t()` where the `()`
    * was dropped by the Typer.
    */
  private val DroppedEmptyArgs = new Property.Key[Unit]

  /** An attachment that indicates a failed conversion or extension method
    * search was tried on a tree. This will in some cases be reported in error
    * messages
    */
  private[typer] val HiddenSearchFailure = new Property.Key[List[SearchFailure]]

  /** Is tree a compiler-generated `.apply` node that refers to the apply of a
    * function class?
    */
  private[typer] def isSyntheticApply(tree: tpd.Tree): Boolean = tree match
    ...
    case TypeApply(fn, targs) =>
      isSyntheticApply(fn) && targs.forall(_.isInstanceOf[tpd.InferredTypeTree])
    case _ => false
...
/** Typecheck trees, the main entry point is `typed`.
  ...
  */
class Typer(@constructorOnly nestingLevel: Int = 0)
    extends Namer
    with TypeAssigner
    with Applications
    with Implicits
  ...
  /** The scope of the typer. For nested typers (cf `Namer#nestedTyper`), this
    * is a place parameters are entered during completion and where they survive
    * until typechecking. A context with this typer also has this scope.
    */
  val scope: MutableScope = newScope(nestingLevel)

  /** A temporary data item valid for a single typed ident: The set of all root
    * import symbols that have been encountered as a qualifier of an import so
    * far. Note: It would be more proper to move importedFromRoot into
    * typedIdent. We should check that this has no performance degradation,
    * however.
    */
  private var unimported: Set[Symbol] = Set()

  /** Temporary data item for single call to typed ident: This symbol would be
    * found under Scala2 mode, but is not in dotty (because dotty conforms to
    * spec section 2 wrt to package member resolution but scalac doe not).
    */
  ...
  /** Find the type of an identifier with given `name` in given context `ctx`.
    ...
    * @param altImports
    *   a ListBuffer in which alternative imported references are collected in
    *   case `findRef` is called from an expansion of an extension method, i.e.
    *   when `e.m` is expanded to `m(e)` and a reference for `m` is searched.
  ...
  def findRef(
    ...
    /** A symbol qualifies if it really exists and is not a package class.
      * Package classes are part of their parent's scope, because otherwise we
      * could not reload them via `_.member`. On the other hand, accessing a
      * package as a type from source is always an error.
      *
      * In addition:
      *   - if we are in a constructor of a pattern, we ignore all definitions
      *     which are parameterized (including nullary) methods and not
      *     accessors (note: if we don't do that case x :: xs in class List
      *     would return the :: method).
      *   - Members of the empty package can be accessed only from within the
      *     empty package. Note: it would be cleaner to never nest package
      *     definitions in empty package definitions, but then we'd have to give
      *     up the fiction that a compilation unit consists of a single tree
      *     (because a source file may have both toplevel classes which go into
      *     the empty package and package definitions, which would have to stay
      *     outside). Since the principle of a single tree per compilation unit
      *     is assumed by many tools, we did not want to take that step.
      */
    def qualifies(denot: Denotation): Boolean =
      def isRealMethod(sd: SingleDenotation): Boolean =
        sd.symbol.is(Method, butNot = Accessor) && !sd.info.isParameterless
      reallyExists(denot)
      && (!pt.isInstanceOf[UnapplySelectionProto] || denot.hasAltWith(
        !isRealMethod(_)
      ))
    ...
    /** Find the denotation of enclosing `name` in given context `ctx`.
      * @param previous
      *   A denotation that was found in a more deeply nested scope, or else
      *   `NoDenotation` if nothing was found yet.
      * @param prevPrec
      *   The binding precedence of the previous denotation, or else
      *   `nothingBound` if nothing was found yet.
      * @param prevCtx
      *   The context of the previous denotation, or else `NoContext` if nothing
      *   was found yet.
      */
    def findRefRecur(previous: Type, prevPrec: BindingPrec, prevCtx: Context)(
        using Context
    ): Type =
      import BindingPrec.*

      /** Check that any previously found result from an inner context does
        * properly shadow the new one from an outer context.
        * @param found
        *   The newly found result
        * @param newPrec
        *   Its precedence
        * @param scala2pkg
        *   Special mode where we check members of the same package, but defined
        *   in different compilation units under Scala2. If set, and the
        *   previous and new contexts do not have the same scope, we select the
        *   previous (inner) definition. This models what scalac does.
        */
      def checkNewOrShadowed(
          found: Type,
          newPrec: BindingPrec,
          scala2pkg: Boolean = false
      )(using Context): Type =
      ...
      /** Assemble and check alternatives to an imported reference. This
        * implies:
        *   - If we expand an extension method (i.e. altImports != null), search
        *     imports on the same level for other possible resolutions of
        *     `name`. The result and altImports together then contain all
        *     possible imported references of the highest possible precedence,
        *     where `NamedImport` beats `WildImport`.
        *   - Find a posssibly shadowing reference in an outer context. If the
        *     result is the same as `previous`, check that it is new or
        *     shadowed. This order of checking is necessary since an outer
        *     package-level definition might trump two conflicting inner
        *     imports, so no error should be issued in that case. See
        *     i7876.scala.
        * @param previous
        *   the previously found reference (which is an import)
        * @param prevPrec
        *   the precedence of the reference (either NamedImport or WildImport)
        * @param prevCtx
        *   the context in which the reference was found
        * @param using_Context
        *   the outer context of `precCtx`
        */
      def checkImportAlternatives(
        ...
        def addAltImport(altImp: TermRef) =
          ...
          then altImports.uncheckedNN += altImp

        if Feature.enabled(
            Feature.relaxedExtensionImports
        ...
        then
          ...
          namedImportRef(curImport) match
            case altImp: TermRef =>
              if prevPrec == WildImport then
                ...
                altImports.clear()
                checkImportAlternatives(altImp, NamedImport, ctx)(using
                  ctx.outer
                )
              else
                addAltImport(altImp)
                checkImportAlternatives(previous, prevPrec, prevCtx)(using
                  ctx.outer
                )
            case _ =>
              if prevPrec == WildImport then
          ...
          if found eq previous then
            checkNewOrShadowed(found, prevPrec)(using prevCtx)
          else found
        end if
      ...
      def selection(imp: ImportInfo, name: Name, checkBounds: Boolean): Type =
        imp.importSym.info match
          case ImportType(expr) =>
            val pre = expr.tpe
            val denot0 = pre
              .memberBasedOnFlags(name, required, excluded)
            ...
            if denot0.exists then
              val denot =
                if checkBounds then
                  denot0.filterWithPredicate { mbr =>
                    mbr.matchesImportBound(
                      if mbr.symbol.is(Given) then imp.givenBound
                      else imp.wildcardBound
                    )
                  }
                else denot0
              ...
              if reallyExists(denot) && !isScalaJsPseudoUnion then
                if unimported.isEmpty || !unimported.contains(pre.termSymbol)
                then return pre.select(name, denot)
          case _ =>
          ...
          case selector :: rest =>
            def checkUnambiguous(found: Type) =
              ...
              if other.exists && found.exists && found != other then
                fail(
                  em"reference to `$name` is ambiguous; it is imported twice"
                )
              found

            if selector.rename == termName && selector.rename != nme.WILDCARD
            then
              val memberName =
      ...
      /** The type representing a wildcard import with enclosing name when
        * imported from given import info
        */
      def wildImportRef(imp: ImportInfo)(using Context): Type =
        if imp.isWildcardImport && !imp.excluded.contains(
            name.toTermName
          ) && name != nme.CONSTRUCTOR
        then selection(imp, name, checkBounds = true)
        else NoType

      /** Is (some alternative of) the given predenotation `denot` defined in
        * current compilation unit?
        */
      def isDefinedInCurrentUnit(denot: Denotation)(using Context): Boolean =
        denot match
          case MultiDenotation(d1, d2) =>
            isDefinedInCurrentUnit(d1) || isDefinedInCurrentUnit(d2)
          case denot: SingleDenotation =>
            (ctx.compilationUnit ne NoCompilationUnit) && denot.symbol.source == ctx.compilationUnit.source
      ...
      /** Would import of kind `prec` be not shadowed by a nested
      ...
      @tailrec def loop(lastCtx: Context)(using Context): Type =
        if ctx.scope eq EmptyScope then previous
        else
          var result: Type = NoType
          val curOwner = ctx.owner

          /** Is curOwner a package object that should be skipped? A package
            * object should always be skipped if we look for a term. That way we
            * make sure we consider all overloaded alternatives of a definition,
            * even if they are in different source files. If we are looking for
            * a type, a package object should be skipped only if it does not
            * contain opaque definitions. Package objects with opaque
            * definitions are significant, since opaque aliases are only seen if
            * the prefix is the this-type of the package object.
            */
          def isTransparentPackageObject =
            curOwner.isPackageObject && (name.isTermName || !curOwner.is(
              Opaque
            ))
          ...
          def isInherited(denot: Denotation) =
            def isCurrent(mbr: SingleDenotation): Boolean =
              !mbr.symbol.exists || mbr.symbol.owner == ctx.owner || ctx.owner
                .is(Package)
            denot match
                ...
                if owner.is(Package) then
                  owner.denot.asClass
                    ...
                    .filterWithPredicate(d =>
                      !d.symbol
                        .is(Package) && d.symbol.source == denot.symbol.source
                    )
                else
                  val scope =
                    if owner.isClass then owner.info.decls else outer.scope
                  scope.denotsNamed(name)
              val competing =
                found.filterWithFlags(required, excluded | Synthetic)
              if competing.exists then
                ...
                if !symsMatch && !suppressErrors then
                  report.errorOrMigrationWarning(
                    AmbiguousReference(name, Definition, Inheritance, prevCtx)(
                      using outer
                    ),
                    pos,
                    from = `3.0`
                  ...
                  if migrateTo3 then
                    patch(
                      Span(pos.span.start),
                      if prevCtx.owner == refctx.owner.enclosingClass then
                        "this."
                      else s"${prevCtx.owner.name}.this."
                    )
              else checkNoOuterDefs(denot, outer, prevCtx)
            end if
          end checkNoOuterDefs

          if isNewDefScope then
            val defDenot = ctx.denotNamed(name, required, excluded)
            if qualifies(defDenot) then
              val found =
                if isSelfDenot(defDenot) then curOwner.enclosingClass.thisType
                else if ctx.isJava && defDenot.symbol.isStatic then
                  defDenot.symbol.namedType
                else
                  val effectiveOwner =
                    if curOwner.isTerm && defDenot.symbol.maybeOwner.isType then
              ...
              if !curOwner.is(Package) || isDefinedInCurrentUnit(defDenot) then
                result = checkNewOrShadowed(
                  found,
                  Definition
                ) // no need to go further out, we found highest prec entry
                found match
                  case found: NamedType
                      if curOwner.isClass && isInherited(
                        found.denot
                ...
                if migrateTo3 && !foundUnderScala2.exists then
                  foundUnderScala2 =
                    checkNewOrShadowed(found, Definition, scala2pkg = true)
                if defDenot.symbol.is(Package) then
                  result =
                    checkNewOrShadowed(previous orElse found, PackageClause)
                else if prevPrec.ordinal < PackageClause.ordinal then
                  result =
                    findRefRecur(found, PackageClause, ctx)(using ctx.outer)
            end if
          end if
          ...
          if result.exists then result
            ...
            def updateUnimported() =
              if curImport.nn.unimported ne NoSymbol then
                unimported += curImport.nn.unimported
            if curOwner.is(
                Package
              ) && curImport != null && curImport.isRootImport && previous.exists
            then previous // no more conflicts possible in this case
            else if isPossibleImport(
                NamedImport
              ) && (curImport nen outer.importInfo)
            then
              val namedImp = namedImportRef(curImport.uncheckedNN)
              if namedImp.exists then
                checkImportAlternatives(namedImp, NamedImport, ctx)(using outer)
              else if isPossibleImport(
                  WildImport
                ) && !curImport.nn.importSym.isCompleting
              then
                val wildImp = wildImportRef(curImport.uncheckedNN)
                if wildImp.exists then
  ...
  /** If `tree`'s type is a `TermRef` identified by flow typing to be non-null,
    * then cast away `tree`s nullability. Otherwise, `tree` remains unchanged.
    *
    * Example: If x is a trackable reference and we know x is not null at this
    * point, (x: T | Null) => x.$asInstanceOf$[x.type & T]
    */
  ...
  /** Attribute an identifier consisting of a simple name or wildcard
    *
    * @param tree
    *   The tree representing the identifier. Transformations: (1) Prefix class
    *   members with this. (2) Change imported symbols to selections. (3) Change
    *   pattern Idents id (but not wildcards) to id @ _
    */
  def typedIdent(tree: untpd.Ident, pt: Type)(using Context): Tree =
    record("typedIdent")
    val name = tree.name
    def kind = if name.isTermName then "" else "type "
    typr.println(s"typed ident $kind$name in ${ctx.owner}")
    if ctx.mode.is(Mode.Pattern) then
      if name == nme.WILDCARD then return tree.withType(pt)
      if name == tpnme.WILDCARD then return tree.withType(defn.AnyType)
    ...
    // cyclic references.
    if name == nme.ROOTPKG then return tree.withType(defn.RootPackage.termRef)
        ...
        if foundUnderScala2.exists && !(foundUnderScala2 =:= found) then
          report.migrationWarning(
            em"""Name resolution will change.
              ...
              | in the future, without -source 3.0-migration: $found""",
            tree.srcPos
          )
          foundUnderScala2
        else found
    ...
    /** Normally, returns `ownType` except if `ownType` is a constructor proxy,
      * and there is another shadowed type accessible with the same name that is
      * not:
        ...
        case ownType: TermRef if ownType.symbol.is(ConstructorProxy) =>
          findRef(name, pt, EmptyFlags, ConstructorProxy, tree.srcPos) match
            case shadowed: TermRef
                if !shadowed.symbol.maybeOwner.isEmptyPackage =>
              pt match
                case pt: FunOrPolyProto =>
                  def err(shadowedIsApply: Boolean) =
                    report.error(
                      ConstrProxyShadows(ownType, shadowed, shadowedIsApply),
                      tree.srcPos
                    )
                  if shadowed.denot.hasAltWith(sd =>
                      sd.symbol.is(Method, butNot = Accessor)
                    )
                  then err(shadowedIsApply = false)
                  else if shadowed
                      .member(nme.apply)
                      .hasAltWith(_.symbol.is(Method, butNot = Accessor))
                  then err(shadowedIsApply = true)
                case _ =>
                  return shadowed
            case shadowed =>
        case _ =>
      ownType
    end checkNotShadowed
    ...
    else if name.toTermName == nme.ERROR then setType(UnspecifiedErrorType)
    ...
    else errorTree(tree, MissingIdent(tree, kind, name))
  end typedIdent

  /** (1) If this reference is neither applied nor selected, check that it does
    * not refer to a package or Java companion object. (2) Check that a stable
    * identifier pattern is indeed stable (SLS 8.1.5)
    */
  private def checkLegalValue(tree: Tree, pt: Type)(using Context): Unit =
    ...
    if ctx.mode.is(Mode.Pattern)
      ...
      && !tree.tpe.isStable
      && !isWildcardArg(tree)
    then report.error(StableIdentPattern(tree, pt), tree.srcPos)

  def typedSelect(tree0: untpd.Select, pt: Type, qual: Tree)(using
      Context
  ): Tree =
    ...
    def finish(tree: untpd.Select, qual: Tree, checkedType: Type): Tree =
      ...
      if selName.isTypeName then
        checkStable(qual.tpe, qual.srcPos, "type prefix")
      checkLegalValue(select, pt)
      ConstFold(select)

    if checkedType.exists then finish(tree, qual, checkedType)
      ...
      val tree1 = tryExtensionOrConversion(
        ...
        .orElse {
          if ctx.gadt.isNarrowing then
            ...
            gadts.println(
              i"Trying to heal member selection by GADT-approximating $wtp"
            )
      ...
      if !tree1.isEmpty then tree1
      else if canDefineFurther(qual.tpe.widen) then typedSelect(tree, pt, qual)
      ...
      then
        ...
        else typedDynamicSelect(tree2, Nil, pt)
      else
        assignType(
          tree,
          rawType match
  ...
  def typedSelect(tree: untpd.Select, pt: Type)(using Context): Tree =
    ...
    def typeSelectOnTerm(using Context): Tree =
      val qual =
        typedExpr(tree.qualifier, shallowSelectionProto(tree.name, pt, this))
    ...
    def javaSelectOnType(qual: Tree)(using Context) =
      ...
      else assignType(cpy.Select(tree)(qual, tree.name), qual)
    ...
    def tryJavaSelectOnType(using Context): Tree = tree.qualifier match
      ...
      case _ =>
        errorTree(
          tree,
          em"cannot convert to type selection"
        ) // will never be printed due to fallback

    def selectWithFallback(fallBack: Context ?=> Tree) =
      tryAlternatively(typeSelectOnTerm)(fallBack)

    if tree.qualifier.isType then
      val qual1 =
        typedType(tree.qualifier, shallowSelectionProto(tree.name, pt, this))
      assignType(cpy.Select(tree)(qual1, tree.name), qual1)
    else if ctx.isJava && tree.name.isTypeName then
      // SI-3120 Java uses the same syntax, A.B, to express selection from the
      // value A and from the type A. We have to try both.
      selectWithFallback(
        tryJavaSelectOnType
      ) // !!! possibly exponential bcs of qualifier retyping
    else typeSelectOnTerm
  end typedSelect
  ...
  def typedSuper(tree: untpd.Super, pt: Type)(using Context): Tree =
    ...
    val enclosingInlineable =
      ctx.owner.ownersIterator.findSymbol(_.isInlineMethod)
    if enclosingInlineable.exists && !PrepareInlineable.isLocal(
        qual1.symbol,
        enclosingInlineable
      )
    then
      report.error(
        SuperCallsNotAllowedInlineable(enclosingInlineable),
        tree.srcPos
      )
    pt match
    ...
    try
      ...
      then
        tree.kind match
          ...
          case _            =>
      else if target.isRef(defn.LongClass) then
        tree.kind match
          case Whole(radix) => return lit(longFromDigits(digits, radix))
          case _            =>
      else if target.isRef(defn.FloatClass) then
        tree.kind match
          ...
          case _ =>
            ...
            if digits.toIntOption.exists(_ != float.toInt) then
              report.warning(
                LossyWideningConstantConversion(defn.IntType, target),
                tree.srcPos
              )
            return lit(float)
      else if target.isRef(defn.DoubleClass) then
        tree.kind match
      ...
      then
        ...
        inferImplicit(
          fromDigitsCls.typeRef.appliedTo(target),
          EmptyTree,
          tree.span
        ) match
          case SearchSuccess(arg, _, _, _) =>
            val fromDigits = untpd
              .Select(untpd.TypedSplice(arg), nme.fromDigits)
              .withSpan(tree.span)
            ...
            if ctx.mode.is(Mode.Pattern) then app = untpd.Block(Nil, app)
            return typed(app, pt)
          case _ =>
      end if
  ...
  def typedLiteral(tree: untpd.Literal)(using Context): Tree =
    ...
    if ctx.mode.is(Mode.Type) then
      tpd.SingletonTypeTree(
        tree1
      ) // this ensures that tree is classified as a type tree
    else tree1
  ...
  def typedNew(tree: untpd.New, pt: Type)(using Context): Tree =
    tree.tpt match
      case templ: untpd.Template =>
        ...
        then templ1 = cpy.Template(templ)(parents = untpd.TypeTree(pt) :: Nil)
      ...
      case _ =>
        ...
        tpt1 = tpt1.withType(
          ensureAccessible(tpt1.tpe, superAccess = false, tpt1.srcPos)
        )
        tpt1 match
  ...
  def typedTyped(tree: untpd.Typed, pt: Type)(using Context): Tree =
     ...
     */
    def cases(ifPat: => Tree, ifExpr: => Tree, wildName: TermName) =
      tree.expr match
        case id: untpd.Ident
            if (ctx.mode is Mode.Pattern) && untpd.isVarPattern(id) =>
          if id.name == nme.WILDCARD || id.name == nme.WILDCARD_STAR then ifPat
          else
            import untpd.*
            typed(
              Bind(id.name, Typed(Ident(wildName), tree.tpt))
                .withSpan(tree.span),
              pt
            )
        case _ => ifExpr
    ...
    def ascription(tpt: Tree, isWildcard: Boolean) =
      val underlyingTreeTpe =
        if isRepeatedParamType(tpt) then
    ...
    if untpd.isWildcardStarArg(tree) then
      def typedWildcardStarArgExpr =
        ...
        val tpt1 = TypeTree(expr1.tpe.widen.translateToRepeated(fromCls))
          .withSpan(tree.tpt.span)
        assignType(cpy.Typed(tree)(expr1, tpt1), tpt1)
      end typedWildcardStarArgExpr
      cases(
        ifPat = ascription(
          TypeTree(defn.RepeatedParamType.appliedTo(pt)),
          isWildcard = true
        ),
      ...
      def typedTpt = checkSimpleKinded(
        typedType(tree.tpt, mapPatternBounds = true)
      )
  ...
  /** For a typed tree `e: T`, if `T` is an abstract type for which an implicit
    * type test or class tag `tt` exists, rewrite to `tt(e)`.
    * @pre
    *   We are in pattern-matching mode (Mode.Pattern)
    */
  def tryWithTypeTest(tree: Typed, pt: Type)(using Context): Tree =
        ...
        case SearchSuccess(clsTag, _, _, _) =>
          withMode(Mode.InTypeTest) {
            Some(
              typed(
                untpd.Apply(
                  untpd.TypedSplice(clsTag),
                  untpd.TypedSplice(tree.expr)
                ),
                pt
              )
            )
    ...
    def tagged(tpe: Type) =
      val tag = withTag(defn.TypeTestClass.typeRef.appliedTo(pt, tpe))
        ...
        .getOrElse(tree)
      if tag.symbol.maybeOwner == defn.ClassTagClass && config.Feature.sourceVersion
          .isAtLeast(config.SourceVersion.future)
      then
        report.warning(
          "Use of `scala.reflect.ClassTag` for type testing may be unsound. Consider using `scala.reflect.TypeTest` instead.",
          tree.srcPos
        )
      tag
    tree.tpt.tpe.dealias match
      case tpe @ AppliedType(tref: TypeRef, _)
          if !tref.symbol.isClass && !ctx.isAfterTyper && !(tpe =:= pt) =>
        tagged(tpe)
      case tref: TypeRef
          if !tref.symbol.isClass && !ctx.isAfterTyper && !(tref =:= pt) =>
        tagged(tref)
      case _ => tree
  end tryWithTypeTest
  ...
  def typedNamedArg(tree: untpd.NamedArg, pt: Type)(using Context): NamedArg =
    ...
    val arg1 = pt.stripNull match
      case AppliedType(a, typ :: Nil)
          if ctx.isJava && a.isRef(defn.ArrayClass) =>
  ...
  def typedAssign(tree: untpd.Assign, pt: Type)(using Context): Tree =
    tree.lhs match
      ...
      case untpd.TypedSplice(Apply(MaybePoly(Select(fn, app), targs), args))
          if app == nme.apply =>
        val rawUpdate: untpd.Tree =
          untpd.Select(untpd.TypedSplice(fn), nme.update)
        val wrappedUpdate =
          if targs.isEmpty then rawUpdate
          else untpd.TypeApply(rawUpdate, targs map (untpd.TypedSplice(_)))
        val appliedUpdate =
          untpd.Apply(
            wrappedUpdate,
            (args map (untpd.TypedSplice(_))) :+ tree.rhs
          )
      ...
      case lhs =>
        ...
        def reassignmentToVal =
          ...
          cpy
            .Assign(tree)(lhsCore, typed(tree.rhs, lhs1.tpe.widen))
            .withType(defn.UnitType)

        def canAssign(sym: Symbol) =
          sym.is(Mutable, butNot = Accessor) ||
            ctx.owner.isPrimaryConstructor && !sym.is(
              Method
            ) && sym.maybeOwner == ctx.owner.owner ||
          ...
          case Apply(fn, _) if fn.symbol.is(ExtensionMethod) =>
            def toSetter(fn: Tree): untpd.Tree = fn match
              ...
              case fn @ TypeApply(fn1, targs) =>
                untpd.cpy
                  .TypeApply(fn)(toSetter(fn1), targs.map(untpd.TypedSplice(_)))
          ...
          case _ =>
            lhsCore.tpe match
              case ref: TermRef =>
                ...
                if canAssign(lhsVal.symbol) then
                  ...
                  val lhsBounds =
                    TypeBounds
                      .lower(lhsVal.symbol.info)
                      .asSeenFrom(ref.prefix, lhsVal.symbol.owner)
                  assignType(
                    cpy.Assign(tree)(lhs1, typed(tree.rhs, lhsBounds.loBound))
                  )
                  ...
                  lhsCore match
                    case lhsCore: RefTree if setter.exists =>
                      ...
                      val setterType = ensureAccessible(
                        setterTypeRaw,
                        isSuperSelection(lhsCore),
                        tree.srcPos
                      )
                      val lhs2 =
                        untpd.rename(lhsCore, setterName).withType(setterType)
                      typedUnadapted(
                        untpd.Apply(untpd.TypedSplice(lhs2), tree.rhs :: Nil),
                        WildcardType,
                        locked
                      )
                    case _ =>
  ...
  def typedBlockStats(stats: List[untpd.Tree])(using
      Context
  ): (List[tpd.Tree], Context) =
    index(stats)
  ...
  def typedBlock(tree: untpd.Block, pt: Type)(using Context): Tree =
    ...
    if ctx.mode.is(Mode.SafeNulls)
      ...
      && pt.isValueType
      && !inContext(exprCtx.addMode(Mode.SafeNulls))(expr1.tpe <:< pt)
    then expr1 = expr1.cast(pt)

    ensureNoLocalRefs(
      cpy
        .Block(tree)(stats1, expr1)
        .withType(expr1.tpe)
        .withNotNullInfo(
          stats1.foldRight(expr1.notNullInfo)(_.notNullInfo.seq(_))
        ),
      pt,
      localSyms(stats1)
    )

  def escapingRefs(block: Tree, localSyms: => List[Symbol])(using
      Context
  ): List[NamedType] =
    lazy val locals = localSyms.toSet
    block.tpe.namedPartsWith(tp =>
      locals.contains(tp.symbol) && !tp.isErroneous
    )

  /** Ensure that an expression's type can be expressed without references to
    * locally defined symbols. This is done by adding a type ascription of a
    * widened type that does not refer to the locally defined symbols. The
    * widened type is computed using `TyperAssigner#avoid`. However, if the
    * expected type is fully defined and not a supertype of the widened type, we
    * ascribe with the expected type instead.
    *
    ...
    */
  protected def ensureNoLocalRefs(
      tree: Tree,
      pt: Type,
      localSyms: => List[Symbol]
  )(using Context): Tree =
    def ascribeType(tree: Tree, pt: Type): Tree = tree match
      case block @ Block(stats, expr) if !expr.isInstanceOf[Closure] =>
        ...
        cpy.Block(block)(
          stats,
          expr1
        ) withType expr1.tpe // no assignType here because avoid is redundant
      case _ =>
  ...
  def typedIf(tree: untpd.If, pt: Type)(using Context): Tree =
    ...
    val result =
      if tree.elsep.isEmpty then
        val thenp1 =
          typed(tree.thenp, branchPt)(using cond1.nullableContextIf(true))
        ...
        val thenp1 :: elsep1 :: Nil = harmonic(harmonize, pt) {
          val thenp0 =
            typed(tree.thenp, branchPt)(using cond1.nullableContextIf(true))
          val elsep0 =
            typed(tree.elsep, branchPt)(using cond1.nullableContextIf(false))
          thenp0 :: elsep0 :: Nil
        }: @unchecked
  ...
  /** Decompose function prototype into a list of parameter prototypes and a
    * result prototype tree, using WildcardTypes where a type is not known.
    * Note: parameter prototypes may be TypeBounds. For the result type we do
    * this even if the expected type is not fully defined, which is a bit of a
    * hack. But it's needed to make the following work (see typers.scala and
    * printers/PlainPrinter.scala for examples).
    *
    * def double(x: Char): String = s"$x$x" "abc" flatMap double
    */
  private def decomposeProtoFunction(pt: Type, defaultArity: Int, pos: SrcPos)(
      using Context
  ): (List[Type], untpd.Tree) =
    ...
    if (pt1 ne pt1.dropDependentRefinement)
      && defn.isContextFunctionType(
        pt1.nonPrivateMember(nme.apply).info.finalResultType
      )
    then
      report.error(
    ...
    pt1 match
      case tp: TypeParamRef =>
        decomposeProtoFunction(
          ctx.typerState.constraint.entry(tp).bounds.hi,
          defaultArity,
          pos
        )
      case _ =>
        pt1.findFunctionType match
          case pt1 if defn.isNonRefinedFunction(pt1) =>
            ...
            (
              pt1.argInfos.init,
              typeTree(interpolateWildcards(pt1.argInfos.last.hiBound))
            )
          case RefinedType(
                parent,
                nme.apply,
                mt @ MethodTpe(_, formals, restpe)
              )
              if (defn.isNonRefinedFunction(parent) || defn
                .isErasedFunctionType(
                  parent
                )) && formals.length == defaultArity =>
            (
              formals,
              untpd.DependentTypeTree(syms =>
                restpe.substParams(mt, syms.map(_.termRef))
              )
            )
          case SAMType(mt @ MethodTpe(_, formals, restpe)) =>
            (
              ...
              else typeTree(restpe)
            )
          case _ =>
            (List.tabulate(defaultArity)(alwaysWildcardType), untpd.TypeTree())
    end match
  end decomposeProtoFunction

  /** The parameter type for a parameter in a lambda that does not have an
    * explicit type given, and where the type is not known from the context. In
    * this case the parameter type needs to be inferred the "target type" T
    * known from the callee `f` if the lambda is of a form like `x => f(x)`. If
    * `T` exists, we know that `S <: I <: T`.
    *
    * The inference makes two attempts:
    *
    *   1. Compute the target type `T` and make it known that `S <: T`. If the
    *      expected type `S` can be fully defined under ForceDegree.flipBottom,
    *      pick this one (this might use the fact that S <: T for an upper
    *      approximation). 2. Otherwise, if the target type `T` can be fully
    *      defined under ForceDegree.flipBottom, pick this one.
    *
    ...
    val target = calleeType.widen match
      case mtpe: MethodType =>
        ...
        if pos < mtpe.paramInfos.length then
          ...
          if !isErased then tp.stripAnnots(_.symbol != defn.ErasedParamAnnot)
          else tp
        else NoType
      case _ => NoType
    if target.exists then formal <:< target
    if !formal.isExactlyNothing && isFullyDefined(
        formal,
        ForceDegree.flipBottom
      )
    then formal
    else if target.exists && isFullyDefined(target, ForceDegree.flipBottom) then
      target
    else NoType
  end inferredFromTarget

  def typedFunction(tree: untpd.Function, pt: Type)(using Context): Tree =
    if ctx.mode is Mode.Type then typedFunctionType(tree, pt)
  ...
  def typedFunctionType(tree: untpd.Function, pt: Type)(using Context): Tree =
    ...
    def typedDependent(params: List[untpd.ValDef])(using Context): Tree =
      ...
      val appDef0 = untpd
        .DefDef(nme.apply, List(params3), body, EmptyTree)
        .withSpan(tree.span)
      index(appDef0 :: Nil)
      val appDef = typed(appDef0).asInstanceOf[DefDef]
      val mt = appDef.symbol.info.asInstanceOf[MethodType]
      if mt.isParamDependent then
        report.error(
          em"$mt is an illegal function type because it has inter-parameter dependencies",
          tree.srcPos
        )
      // Restart typechecking if there are erased classes that we want to mark erased
      if mt.erasedParams
          .zip(mt.paramInfos.map(_.isErasedClass))
          .exists((paramErased, classErased) => classErased && !paramErased)
      then
        val newParams =
          params3.zipWithConserve(mt.paramInfos.map(_.isErasedClass)) {
            (arg, isErasedClass) =>
    ...
    args match
      case ValDef(_, _, _) :: _ =>
        typedDependent(args.asInstanceOf[List[untpd.ValDef]])(using
          ctx.fresh.setOwner(newRefinedClassSymbol(tree.span)).setNewScope
        )
      case _ =>
        if erasedParams.contains(true) then
          ...
          val funSym = defn.FunctionSymbol(numArgs, isContextual, isImpure)
          val result = typed(
            cpy.AppliedTypeTree(tree)(
              untpd.TypeTree(funSym.typeRef),
              args :+ body
            ),
            pt
          )
          ...
          result match
            case r: AppliedTypeTree if r.args.exists(_.tpe.isErasedClass) =>
              typedFunctionType(desugar.makeFunctionWithValDefs(tree, pt), pt)
            case _ => result
  end typedFunctionType

  def typedFunctionValue(tree: untpd.Function, pt: Type)(using Context): Tree =
    val untpd.Function(params: List[untpd.ValDef] @unchecked, _) =
      tree: @unchecked

    val (isContextual, isDefinedErased) = tree match
      case tree: untpd.FunctionWithMods =>
        (tree.mods.is(Given), tree.erasedParams)
      case _ => (false, tree.args.map(_ => false))

    /** The function body to be returned in the closure. Can become a
      * TypedSplice of a typed expression if this is necessary to infer a
      * parameter type.
      */
    ...
    def refersTo(arg: untpd.Tree, param: untpd.ValDef): Boolean = arg match
      ...
      case Typed(arg1, _) if untpd.isWildcardStarArg(arg) =>
        refersTo(arg1, param)
      case _ => false

    /** If parameter `param` appears exactly once as an argument in `args`, the
      * singleton list consisting of its position in `args`, otherwise `Nil`.
      */
    def paramIndices(param: untpd.ValDef, args: List[untpd.Tree]): List[Int] =
      def loop(args: List[untpd.Tree], start: Int): List[Int] = args match
        case arg :: args1 =>
          ...
          if refersTo(arg, param) then start :: others else others
        case _ => Nil
    ...
    /** Infer parameter type from the body of the function
      ...
      * (x1, ..., xN) => f(... x1, ..., XN, ...)
      *
      * where each `xi` occurs exactly once in the argument list of `f` (in any
      * order), the type of `f`, otherwise NoType.
      *
      * 2. If the function is of the form
      *
      * (using x1, ..., xN) => f
      *
      * where `f` is a contextual function type of the form `(T1, ..., TN) ?=>
      * T`, then `xi` takes the type `Ti`.
      *
      * Updates `fnBody` and `paramIndex` as a side effect.
      * @post:
      *   If result exists, `paramIndex` is defined for the name of every
      *   parameter in `params`.
      */
    lazy val calleeType: Type =
      untpd.stripAnnotated(untpd.unsplice(fnBody)) match
        case ident: untpd.Ident if isContextual =>
          val ident1 = typedIdent(ident, WildcardType)
          val tp = ident1.tpe.widen
          if defn.isContextFunctionType(tp) && params.size == defn
              .functionArity(tp)
          then
            ...
            expr match
              ...
              case _ =>
                ...
                inContext(nestedCtx) {
                  ...
                  val callProto =
                    FunProto(protoArgs, WildcardType)(this, app.applyKind)
    ...
    val (protoFormals, resultTpt) =
      decomposeProtoFunction(pt, params.length, tree.srcPos)

    /** Returns the type and whether the parameter is erased */
    def protoFormal(i: Int): (Type, Boolean) =
      if protoFormals.length == params.length then
        (protoFormals(i), isDefinedErased(i))
      else
        (
          errorType(WrongNumberOfParameters(protoFormals.length), tree.srcPos),
          false
        )

    /** Is `formal` a product type which is elementwise compatible with
      * `params`?
      */
    def ptIsCorrectProduct(formal: Type) =
      isFullyDefined(formal, ForceDegree.flipBottom) &&
        ...
        tupleComponentTypes(formal).corresponds(params) { (argType, param) =>
          param.tpt.isEmpty || argType.widenExpr <:< typedAheadType(
            param.tpt
          ).tpe
        }
    ...
    if protoFormals.length == 1 && params.length != 1 then
      ...
      fnBody match
        case untpd.Match(
              scrut,
              untpd.CaseDef(untpd.Tuple(elems), untpd.EmptyTree, rhs) :: Nil
            )
            if scrut.span.isSynthetic && isParamRef(scrut) && elems
              .hasSameLengthAs(protoFormals) =>
        ...
        for ((param, i) <- params.zipWithIndex) yield
          ...
          val param0 =
            if !param.tpt.isEmpty then param
              ...
              val isBottomFromWildcard =
                (formalBounds ne formal) && formal.isExactlyNothing
              ...
              val paramType =
                ...
                val formal0 =
                  if !isErased then
                    formal.stripAnnots(_.symbol != defn.ErasedParamAnnot)
                  else formal
                if knownFormal && !isBottomFromWildcard then formal0
                else
                  inferredFromTarget(
                    param,
                    formal,
                    ...
                    if knownFormal then formal0
                      ...
                      errorType(
                        AnonymousFunctionMissingParamType(param, tree, formal),
                        param.srcPos
                      )
                  )
            ...
            pt.findFunctionType match
              case pt @ SAMType(sam)
                ...
                def isWildcardClassSAM =
                  !pt.classSymbol.is(Trait) && pt.argInfos.exists(
                    _.isInstanceOf[TypeBounds]
                  )
                val targetTpe =
                  if isFullyDefined(pt, ForceDegree.all) && !isWildcardClassSAM
                  then pt
                    ...
                    defn.PartialFunctionOf(
                      mt.firstParamTypes.head,
                      mt.resultType
                    )
                  else
                    report.error(
                      em"result type of lambda is an underspecified SAM type $pt",
                      tree.srcPos
                    )
                    pt
                TypeTree(targetTpe)
              case _ =>
                ...
                then
                  ...
                  TypeTree(
                    defn.FunctionOf(Nil, mt.resType, isContextual = true)
                  )
          ...
          case tp =>
            if !tp.isErroneous then
              throw new java.lang.Error(
                i"internal error: closing over non-method $tp, pos = ${tree.span}"
              )
        ...
        if tree.isInline then
          ...
          val cases1 = tree.cases.mapconserve {
            case cdef @ CaseDef(pat @ Typed(Ident(nme.WILDCARD), _), _, _) =>
              ...
              cpy.CaseDef(cdef)(pat =
                untpd.Bind(EvidenceParamName.fresh(), pat)
              )
            case cdef => cdef
          }
          typedMatchFinish(
          ...
          typed(
            desugar
              .makeCaseLambda(tree.cases, checkMode, protoFormals.length)
              .withSpan(tree.span),
            pt
          )
      case _ =>
        ...
        val rawSelectorTpe =
          fullyDefinedType(sel1.tpe, "pattern selector", tree.srcPos)
        val selType = rawSelectorTpe match
          case c: ConstantType if tree.isInline => c
          case otherTpe                         => otherTpe.widen

        /** Does `tree` has the same shape as the given match type? We only
          * support typed patterns with empty guards, but that could potentially
              ...
              .forall {
                case (pat: untpd.Typed, pt) =>
                  ...
                  instantiateMatchTypeProto(pat1, pt) match
                    case defn.MatchCase(patternTp, _) =>
                      tpt.tpe frozen_=:= patternTp
                    case _ => false
                case (id @ Ident(nme.WILDCARD), pt) =>
                  pt match
                    case defn.MatchCase(patternTp, _) =>
                      defn.AnyType frozen_=:= patternTp
                    case _ => false
                case _ => false
              }
        ...
        /** Are some form of brackets necessary to annotate the tree `sel` as
          * `@unchecked`? If so, return a Some(opening bracket, closing
          * bracket), otherwise None.
          */
        def uncheckedBrackets(sel: untpd.Tree): Option[(String, String)] =
          sel match
            case _: untpd.If | _: untpd.Match | _: untpd.ForYield |
                _: untpd.ParsedTry | _: untpd.Try | _: untpd.Typed =>
              Some("(", ")")
        ...
        result match
          case result @ Match(sel, CaseDef(pat, _, _) :: _) =>
            tree.selector.removeAttachment(desugar.CheckIrrefutable) match
              case Some(checkMode)
                  if !sel.tpe.hasAnnotation(defn.UncheckedAnnot) =>
                ...
                then
                  if isPatDef then
                    uncheckedBrackets(tree.selector) match
                      case None =>
                ...
                val uncheckedTpe = AnnotatedType(
                  sel.tpe.widen,
                  Annotation(defn.UncheckedAnnot, tree.selector.span)
                )
  ...
  /** Special typing of Match tree when the expected type is a MatchType, and
    * the patterns of the Match tree and the MatchType correspond.
    */
  def typedDependentMatchFinish(
      tree: untpd.Match,
      sel: Tree,
      wideSelType: Type,
      cases: List[untpd.CaseDef],
      pt: MatchType
  )(using Context): Tree =
    var caseCtx = ctx
    val cases1 = tree.cases
      .zip(pt.cases)
      .map { case (cas, tpe) =>
  ...
  def typedMatchFinish(
    ...
    val cases1 = harmonic(harmonize, pt)(
      typedCases(cases, sel, wideSelType, pt.dropIfProto)
    )
      .asInstanceOf[List[CaseDef]]
  ...
  /**   - strip all instantiated TypeVars from pattern types.
    *     run/reducable.scala is a test case that shows stripping typevars is
    *     necessary.
      ...
      super.transform(trt.withType(stripTypeVars(trt.tpe))) match
        case b: Bind =>
          ...
          if !ctx.isAfterTyper then
            ...
            if bounds != null then sym.info = bounds
          b
        case t: UnApply if t.symbol.is(Inline) => Inlines.inlinedUnapply(t)
        case t                                 => t

  /** If the prototype `pt` is the type lambda (when doing a dependent typing of
    * a match), instantiate that type lambda with the pattern variables found in
    * the pattern `pat`.
    */
  def instantiateMatchTypeProto(pat: Tree, pt: Type)(using Context) = pt match
    case caseTp: HKTypeLambda =>
      ...
      val bindingsTps = bindingsSyms.collect {
        case sym if sym.isType => sym.typeRef
      }
      caseTp.appliedTo(bindingsTps)
    case pt => pt

  /** Type a case. */
  def typedCase(tree: untpd.CaseDef, sel: Tree, wideSelType: Type, pt: Type)(
      using Context
  ): CaseDef =
    val originalCtx = ctx
    ...
    def caseRest(pat: Tree)(using Context) =
      ...
      var body1 =
        ensureNoLocalRefs(typedExpr(tree.body, pt1), pt1, ctx.scope.toList)
      if ctx.gadt.isNarrowing then
      ...
      if pt1.isValueType
      then // insert a cast if body does not conform to expected type if we disregard gadt bounds
    ...
    caseRest(pat1)(using Nullables.caseContext(sel, pat1)(using gadtCtx))
  ...
  def typedTypeCase(cdef: untpd.CaseDef, selType: Type, pt: Type)(using
      Context
  ): CaseDef =
  ...
  def typedReturn(tree: untpd.Return)(using Context): Return =
    ...
    def enclMethInfo(cx: Context): (Tree, Type) =
      ...
      if owner.isType then
        ...
        if owner.isInlineMethod then
          ...
          (
            EmptyTree,
            errorType(MissingReturnTypeWithReturnStatement(owner), tree.srcPos)
          )
        else (Ident(TermRef(NoPrefix, owner.asTerm)), owner.returnProto)
    ...
    val (from, proto) =
      if tree.from.isEmpty then enclMethInfo(ctx)
        ...
        val proto =
          if ctx.erasedTypes then from.symbol.info.finalResultType
          else
            WildcardType // We cannot reliably detect the internal type view of polymorphic or dependent methods
        ...
        (from, proto)
    val expr1 =
      typedExpr(tree.expr orElse untpd.unitLiteral.withSpan(tree.span), proto)
  ...
  def typedWhileDo(tree: untpd.WhileDo)(using Context): Tree =
    inContext(Nullables.whileContext(tree.span)) {
      val cond1 =
        if tree.cond eq EmptyTree then EmptyTree
        else typed(tree.cond, defn.BooleanType)
      val body1 =
        typed(tree.body, defn.UnitType)(using cond1.nullableContextIf(true))
      assignType(cpy.WhileDo(tree)(cond1, body1))
        .withNotNullInfo(
          body1.notNullInfo.retractedInfo.seq(cond1.notNullInfoIf(false))
        )
    }

  /** Add givens reflecting `CanThrow` capabilities for all checked exceptions
    * matched by `cases`. The givens appear in nested blocks with earlier cases
    * leading to more deeply nested givens. This way, given priority will be the
    * same as pattern priority. The functionality is enabled if the
    * experimental.saferExceptions language feature is enabled.
    */
  def addCanThrowCapabilities(expr: untpd.Tree, cases: List[CaseDef])(using
      Context
  ): untpd.Tree =
  ...
  end addCanThrowCapabilities

  def typedTry(tree: untpd.Try, pt: Type)(using Context): Try =
    val expr2 :: cases2x = harmonic(harmonize, pt) {
      val cases1 =
        typedCases(tree.cases, EmptyTree, defn.ThrowableType, pt.dropIfProto)
      val expr1 =
        typed(addCanThrowCapabilities(tree.expr, cases1), pt.dropIfProto)
      expr1 :: cases1
    }: @unchecked
  ...
  def typedTry(tree: untpd.ParsedTry, pt: Type)(using Context): Try =
    val cases: List[untpd.CaseDef] = tree.handler match
      ...
      case handler =>
        val handler1 =
          typed(handler, defn.FunctionType(1).appliedTo(defn.ThrowableType, pt))
        desugar.makeTryCase(handler1) :: Nil
    typedTry(
      untpd.Try(tree.expr, cases, tree.finalizer).withSpan(tree.span),
      pt
    )
  ...
  def typedSeqLiteral(tree: untpd.SeqLiteral, pt: Type)(using
      Context
  ): SeqLiteral =
    ...
    if !tree.elemtpt.isEmpty then
      ...
      val elemtptType =
        if isFullyDefined(elemProto, ForceDegree.none) then elemProto
        else if tree.elems.isEmpty && tree.isInstanceOf[Trees.JavaSeqLiteral[?]]
        then defn.ObjectType // generic empty Java varargs are of type Object[]
        else TypeComparer.lub(elems1.tpes)
  ...
  def typedInlined(tree: untpd.Inlined, pt: Type)(using Context): Tree =
    ...
    val expansion1 =
      typed(tree.expansion, pt)(using inlineContext(tree.call)(using exprCtx))
    assignType(
      cpy.Inlined(tree)(
        tree.call,
        bindings1.asInstanceOf[List[MemberDef]],
        expansion1
      ),
      bindings1,
      expansion1
  ...
  def completeTypeTree(tree: untpd.TypeTree, pt: Type, original: untpd.Tree)(
      using Context
  ): TypeTree =
    tree
      .withSpan(original.span)
      .withAttachmentsFrom(original)
      .withType(
        if isFullyDefined(pt, ForceDegree.flipBottom) then pt
        else if ctx.reporter.errorsReported then UnspecifiedErrorType
        else
          errorType(
            em"cannot infer type; expected type $pt is not fully defined",
            tree.srcPos
          )
      )
      ...
      case tree: untpd.DerivedTypeTree =>
        ...
        tree.getAttachment(untpd.OriginalSymbol) match
          ...
          case None =>
            errorTree(
              tree,
              em"Something's wrong: missing original symbol for type tree"
            )
      case _ =>
        completeTypeTree(InferredTypeTree(), pt, tree)

  def typedSingletonTypeTree(tree: untpd.SingletonTypeTree)(using
      Context
  ): SingletonTypeTree =
  ...
  def typedRefinedTypeTree(tree: untpd.RefinedTypeTree)(using
      Context
  ): TypTree =
    val tpt1 =
      if tree.tpt.isEmpty then TypeTree(defn.ObjectType)
      else typedAheadType(tree.tpt)
    val refineClsDef =
      desugar.refinedTypeToClass(tpt1, tree.refinements).withSpan(tree.span)
    val refineCls = createSymbol(refineClsDef).asClass
    val TypeDef(_, impl: Template) = typed(refineClsDef): @unchecked
    val refinements1 = impl.body
    assert(
      tree.refinements.hasSameLengthAs(refinements1),
      i"${tree.refinements}%, % > $refinements1%, %"
    )
    val seen = mutable.Set[Symbol]()
    for refinement <- refinements1
    do // TODO: get clarity whether we want to enforce these conditions
      ...
      if !polymorphicRefinementAllowed && rsym.info
          .isInstanceOf[PolyType] && rsym.allOverriddenSymbols.isEmpty
      then
        report.error(
          PolymorphicMethodMissingTypeInParent(rsym, tpt1.symbol),
          refinement.srcPos
        )
      val member = refineCls.info.member(rsym.name)
      if member.isOverloaded then
        report.error(OverloadInRefinement(rsym), refinement.srcPos)
    assignType(
      cpy.RefinedTypeTree(tree)(tpt1, refinements1),
      tpt1,
      refinements1,
      refineCls
    )
  end typedRefinedTypeTree
  ...
  def typedAppliedTypeTree(tree: untpd.AppliedTypeTree)(using Context): Tree =
    tree.args match
      case arg :: _ if arg.isTerm =>
        ...
        else return errorTree(tree, dependentMsg)
      case _ =>
    ...
    if tpt1.tpe.isError then
      ...
      val args1 =
        ...
        def typedArg(arg: untpd.Tree, tparam: ParamInfo) =
          def tparamBounds =
            val bounds =
              tparam.paramInfoAsSeenFrom(
                tpt1.tpe.appliedTo(tparams.map(_ => TypeBounds.empty))
              )
          ...
          val (desugaredArg, argPt) =
            if ctx.mode.is(Mode.Pattern) then
              (
                if untpd.isVarPattern(arg) then desugar.patternVar(arg)
                else arg,
                tparamBounds
              )
            else if ctx.mode.is(Mode.QuotedPattern) then (arg, tparamBounds)
            else (arg, WildcardType)
          if tpt1.symbol.isClass then
            tparam match
              case tparam: Symbol =>
                tparam
                  .ensureCompleted() // This is needed to get the test `compileParSetSubset` to work
              case _ =>
          if desugaredArg.isType then
            arg match
      ...
      then
        report.error(
          em"throws clause cannot be defined for RuntimeException",
          checkedArgs(1).srcPos
        )
  ...
  def typedTermLambdaTypeTree(
      tree: untpd.TermLambdaTypeTree
  )(using Context): Tree =
    if Feature.dependentEnabled then
      errorTree(tree, em"Not yet implemented: (...) =>> ...")
    else errorTree(tree, dependentMsg)

  def typedMatchTypeTree(tree: untpd.MatchTypeTree, pt: Type)(using
      Context
  ): Tree =
    val bound1 =
      if tree.bound.isEmpty && isFullyDefined(pt, ForceDegree.none) then
        TypeTree(pt)
    ...
    val pt1 = if bound1.isEmpty then pt else bound1.tpe
    val cases1 = tree.cases.mapconserve(typedTypeCase(_, sel1Tpe, pt1))
    assignType(
      cpy.MatchTypeTree(tree)(bound1, sel1, cases1),
      bound1,
      sel1,
      cases1
    )

  def typedByNameTypeTree(tree: untpd.ByNameTypeTree)(using
      Context
  ): ByNameTypeTree = tree.result match
    case untpd.CapturesAndResult(refs, tpe) =>
      typedByNameTypeTree(
        cpy.ByNameTypeTree(tree)(
          untpd.makeRetaining(tpe, refs, tpnme.retainsByName)
        )
      )
    case _ =>
  ...
  def typedTypeBoundsTree(tree: untpd.TypeBoundsTree, pt: Type)(using
      Context
  ): Tree =
    ...
    val lo2 =
      if lo1.isEmpty then typed(untpd.TypeTree(defn.NothingType)) else lo1
    val hi2 = if hi1.isEmpty then typed(untpd.TypeTree(defn.AnyType)) else hi1
  ...
  def typedBind(tree: untpd.Bind, pt: Type)(using Context): Tree =
    ...
    body1 match
      case UnApply(fn, Nil, arg :: Nil)
          if fn.symbol.exists && (fn.symbol.owner.derivesFrom(
            defn.TypeTestClass
          ) || fn.symbol.owner == defn.ClassTagClass) && !body1.tpe.isError =>
        ...
        tpd.cpy.UnApply(body1)(
          fn,
          Nil,
          typed(
            untpd.Bind(tree.name, untpd.TypedSplice(arg)).withSpan(tree.span),
            arg.tpe
          ) :: Nil
        )
      case _ =>
        var name = tree.name
        if name == nme.WILDCARD && tree.mods.is(Given) then
          val Typed(_, tpt) = tree.body: @unchecked
          name = desugar.inventGivenOrExtensionName(tpt)
        if name == nme.WILDCARD then body1
        else
          ...
          if pt == defn.ImplicitScrutineeTypeRef || tree.mods.is(Given) then
            sym.setFlag(Given)
          if ctx.mode.is(Mode.InPatternAlternative) then
            report.error(
              IllegalVariableInPatternAlternative(sym.name),
              tree.srcPos
            )
  ...
  def typedAlternative(tree: untpd.Alternative, pt: Type)(using
      Context
  ): Alternative =
    ...
    val trees1 = tree.trees
      .mapconserve(typed(_, pt)(using nestedCtx))
      .mapconserve(ensureValueTypeOrWildcard)
    assignType(cpy.Alternative(tree)(trees1), trees1)

  /** The context to be used for an annotation of `mdef`. This should be the
    * context enclosing `mdef`, or if `mdef` defines a parameter the context
    * enclosing the owner of `mdef`. Furthermore, we need to make sure that
    * annotation trees are evaluated with an owner that is not the enclosing
    * class since otherwise locally defined symbols would be entered as class
    * members.
    */
  def annotContext(mdef: untpd.Tree, sym: Symbol)(using Context): Context =
    def isInner(owner: Symbol) =
      owner == sym || sym.is(Param) && owner == sym.owner
  ...
  def completeAnnotations(mdef: untpd.MemberDef, sym: Symbol)(using
      Context
  ): Unit =
    ...
    for annot <- mdef.mods.annotations do
  ...
  def registerNowarn(tree: Tree, mdef: untpd.Tree)(using Context): Unit =
    ...
    val filters = annot.argumentConstantString(0) match
      case None =>
        annot.argument(0) match
          ...
          case _ =>
            report.warning(
              s"filter needs to be a compile-time constant string",
              argPos
            )
    ...
    val sup =
      Suppression(tree.sourcePos, filters, range.start, range.end, verbose)
  ...
  def typedValDef(vdef: untpd.ValDef, sym: Symbol)(using Context): Tree =
    ...
    if sym.isOneOf(GivenOrImplicit) then checkImplicitConversionDefOK(sym)
  ...
  def typedDefDef(ddef: untpd.DefDef, sym: Symbol)(using Context): Tree =
    ...
    val paramss1 =
      paramss.nestedMapConserve(typed(_)).asInstanceOf[List[ParamClause]]
    for case ValDefs(vparams) <- paramss1 do checkNoForwardDependencies(vparams)
    if sym.isOneOf(GivenOrImplicit) then checkImplicitConversionDefOK(sym)
    val tpt1 = checkSimpleKinded(typedType(tpt))

    val rhsCtx = ctx.fresh
    val tparamss = paramss1.collect { case untpd.TypeDefs(tparams) =>
      tparams
    }
    ...
    if sym.isInlineMethod then
      ...
      if sym.is(Given)
        && untpd
          .stripBlock(untpd.unsplice(ddef.rhs))
          .isInstanceOf[untpd.Function]
      then report.warning(InlineGivenShouldNotBeFunction(), ddef.rhs.srcPos)
        ...
        if sym.owner.is(Case) then
          ...
          do
            if defn.isContextFunctionType(param.tpt.tpe) then
              report.error(
                "case class element cannot be a context function",
                param.srcPos
              )
      else
        if sym.targetName != sym.name then
          report.error(
            em"@targetName annotation may not be used on a constructor",
            ddef.srcPos
          )
    ...
    if sym.is(Method) && sym.owner.denot.isRefinementClass then
      for annot <- sym.paramSymss.flatten
          .filter(_.isTerm)
          .flatMap(_.getAnnotation(defn.ImplicitNotFoundAnnot))
      do
        report.warning(
  ...
  /** (1) Check that the signature of the class member does not return a
    * repeated parameter type (2) If info is an erased class, set erased flag of
    * member
    */
  private def postProcessInfo(sym: Symbol)(using Context): Unit =
    if !sym.isOneOf(
        Synthetic | InlineProxy | Param
      ) && sym.info.finalResultType.isRepeatedParam
    then
      report.error(
        em"Cannot return repeated parameter type ${sym.info.finalResultType}",
        sym.srcPos
      )
    if !sym.is(
        Module
      ) && !sym.isConstructor && sym.info.finalResultType.isErasedClass
    then sym.setFlag(Erased)
  ...
  def typedClassDef(cdef: untpd.TypeDef, cls: ClassSymbol)(using
      Context
  ): Tree =
    if !cls.info.isInstanceOf[ClassInfo] then
      return EmptyTree.assertingErrorsReported
    ...
    def typedParent(tree: untpd.Tree): Tree =
      ...
      if seenParents.contains(psym) && !cls.isRefinementClass then
        ...
        if !tree.span.isSourceDerived then return EmptyTree
        if !ctx.isAfterTyper then
          report.error(em"$psym is extended twice", tree.srcPos)
      ...
      if cls is Case then checkCaseInheritance(psym, cls, tree.srcPos)
      result

    def ensureCorrectSuperClass(): Unit =
      val parents0 = cls.classInfo.declaredParents
      parents0 match
        case AnnotatedType(sc, ann) :: rest
            if ann.symbol == defn.ProvisionalSuperClassAnnot =>
          val parents1 = ensureFirstIsClass(cls, rest)
          if parents1.head ne sc then
            typr.println(
              i"improved provisional superclass $sc to ${parents1.head}"
            )
          cls.info = cls.classInfo.derivedClassInfo(declaredParents = parents1)
        case _ =>

    /** Augment `ptrees` to have the same class symbols as `parents`. Generate
      * TypeTrees or New trees to fill in any parents for which no tree exists
      * yet.
      */
            ...
            ptrees match
              ...
              case _ =>
                val added: Tree = ensureConstrCall(
                  cls,
                  TypeTree(parent).withSpan(cdef.nameSpan.focus),
                  psym
                )(using superCtx)
    ...
    val self1 =
      typed(self)(using ctx.outer).asInstanceOf[
        ValDef
      ] // outer context where class members are not visible
    if self1.tpt.tpe.isError || classExistsOnSelf(cls.unforcedDecls, self1) then
      // fail fast to avoid typing the body with an error type
      cdef.withType(UnspecifiedErrorType)
    else
      val dummy = localDummy(cls, impl)
      val body1 = addAccessorDefs(
        cls,
      ...
      val impl1 = cpy
        .Template(impl)(constr1, parents1, Nil, self1, body1)
        .withType(dummy.termRef)
      if !cls.isOneOf(AbstractOrTrait) && !ctx.isAfterTyper then
      ...
      if reportDynamicInheritance then
        val isRequired = parents1.exists(_.tpe.isRef(defn.DynamicClass))
        report.featureWarning(
          nme.dynamics.toString,
        ...
        body1.foreach {
          case ddef: DefDef =>
            ddef.paramss.foreach { params =>
              val erasedParam = params.collectFirst {
                case vdef: ValDef if vdef.symbol.is(Erased) => vdef
              }
              erasedParam.foreach { p =>
                report.error(
                  em"Implementation restriction: erased classes are not allowed in a poly function definition",
                  p.srcPos
                )
              }
            }
      ...
      then
        report.error(
          em"$cls cannot be defined in universal $effectiveOwner",
          cdef.srcPos
        )
      ...
      if ctx.mode.is(Mode.Interactive) && ctx.settings.YretainTrees.value then
        cls.rootTreeOrProvider = cdef1

      for deriver <- cdef.removeAttachment(AttachedDeriver) do
  ...
  protected def addAccessorDefs(cls: Symbol, body: List[Tree])(using
      Context
  ): List[Tree] =
    PrepareInlineable.addAccessorDefs(cls, body)

  /** Turn a parent type into a constructor call where needed. This is the case
    * where
    *   - we are in a Scala class or module (not a Java class, nor a trait), and
    *     - the parent symbol is a non-trait class, or
    *     - the parent symbol is a trait that takes at least one (explicit or
    *       implicit) parameter and the parent symbol is directly extended by
    *       the current class (i.e. not extended by the superclass).
    */
  def ensureConstrCall(cls: ClassSymbol, parent: Tree, psym: Symbol)(using
      Context
  ): Tree =
    if parent.isType && !cls.is(Trait) && !cls.is(JavaDefined) && psym.isClass
      ...
      && (!psym.is(Trait)
        || psym.primaryConstructor.info.takesParams && !cls.superClass
          .isSubClass(psym))
    then typed(untpd.New(untpd.TypedSplice(parent), Nil))
    else parent

  def localDummy(cls: ClassSymbol, impl: untpd.Template)(using
      Context
  ): Symbol =
    newLocalDummy(cls, impl.span)

  inline private def typedSelectors(
      selectors: List[untpd.ImportSelector]
  )(using Context): List[untpd.ImportSelector] =
      ...
      if sel.bound.isEmpty then sel
        ...
        cpy
          .ImportSelector(sel)(
            sel.imported,
            sel.renamed,
            untpd.TypedSplice(typedType(sel.bound))
          )
          .asInstanceOf[untpd.ImportSelector]
    }

  def typedImportQualifier(imp: untpd.Import, typd: (untpd.Tree, Type) => Tree)(
      using Context
  ): Tree =
    if imp.expr == untpd.EmptyTree then
      assert(imp.selectors.length == 1, imp)
      val from = imp.selectors.head.imported
      val sel = tryAlternatively(typedIdent(from, AnySelectionProto))(
  ...
  def typedImport(imp: untpd.Import)(using Context): Tree =
    ...
    val expr1 =
      typedImportQualifier(imp, typedExpr(_, _)(using ctx.withOwner(sym)))
  ...
  def typedExport(exp: untpd.Export)(using Context): Tree =
    exp.expr.removeAttachment(TypedAhead) match
      ...
      case _ =>
        errorTree(
          exp,
          em"exports are only allowed from objects and classes, they can not belong to local blocks"
        )

  def typedPackageDef(tree: untpd.PackageDef)(using Context): Tree =
    val pid1 =
      withMode(Mode.InPackageClauseName)(typedExpr(tree.pid, AnySelectionProto))
    val pkg = pid1.symbol
    pid1 match
      case pid1: RefTree if pkg.is(Package) =>
        inContext(ctx.packageContext(tree, pkg)) {
          ...
          val topLevelClassName =
            desugar.packageObjectName(ctx.source).moduleClassName
  ...
  def typedAnnotated(tree: untpd.Annotated, pt: Type)(using Context): Tree =
    ...
    if ctx.mode is Mode.Type then
      ...
      val arg2 = arg1 match
        ...
        case _ => arg1
      val argType =
        if arg1.isInstanceOf[Bind] then
          arg1.tpe.widen // bound symbol is not accessible outside of Bind node
  ...
  def typedTypedSplice(tree: untpd.TypedSplice)(using Context): Tree =
    tree.splice match
      ...
      case tree1: Ident =>
        tree1 // ... or here, since these trees cannot contain bindings
      case tree1 =>
        if ctx.owner ne tree.owner then tree1.changeOwner(tree.owner, ctx.owner)
        else tree1
  ...
  def typedAsFunction(tree: untpd.PostfixOp, pt: Type)(using Context): Tree =
    ...
    val pt1 = if defn.isFunctionType(pt) then pt else AnyFunctionProto
    ...
    res match
      ...
      case _ =>
        ...
        report.errorOrMigrationWarning(
          OnlyFunctionsCanBeFollowedByUnderscore(recovered.tpe.widen),
          tree.srcPos,
          from = `3.0`
        )
        if migrateTo3 then
    ...
    if sourceVersion.isAtLeast(future) then
      lazy val (prefix, suffix) = res match
        ...
        case _ =>
          ("(() => ", ")")
      def remedy =
        if (prefix ++ suffix).isEmpty then "simply leave out the trailing ` _`"
  ...
  /** Translate infix operation expression `l op r` to
    *
    * l.op(r) if `op` is left-associative { val x = l; r.op(x) } if `op` is
    * right-associative call-by-value and `l` is impure r.op(l) if `op` is
    * right-associative call-by-name or `l` is pure
    *
    * Translate infix type `l op r` to `op[l, r]` Translate infix pattern `l op
    * r` to `op(l, r)`
    */
  ...
  def typedTuple(tree: untpd.Tuple, pt: Type)(using Context): Tree =
    ...
    if arity <= Definitions.MaxTupleArity then
      ...
      val elems = tree.trees
        ...
        .map(
          if ctx.mode.is(Mode.Type) then
            typedType(_, _, mapPatternBounds = true)
          else typed(_, _)
        )
      if ctx.mode.is(Mode.Type) then
        elems
          .foldRight(TypeTree(defn.EmptyTupleModule.termRef): Tree)(
            (elemTpt, elemTpts) =>
              AppliedTypeTree(
                TypeTree(defn.PairClass.typeRef),
                List(elemTpt, elemTpts)
              )
          )
          .withSpan(tree.span)
      else
        ...
        val app = untpd.cpy
          .Apply(tree)(tupleXXLobj, elems.map(untpd.TypedSplice(_)))
          .withSpan(tree.span)
        val app1 = typed(
          app,
          if ctx.mode.is(Mode.Pattern) then pt else defn.TupleXXLClass.typeRef
        )
  ...
  protected def retrieveSym(tree: untpd.Tree)(using Context): Symbol =
    tree.removeAttachment(SymOfTree) match
      case Some(sym) =>
        sym.ensureCompleted()
        sym
      case none =>
  ...
  def typedUnadapted(initTree: untpd.Tree, pt: Type = WildcardType)(using
      Context
  ): Tree =
  ...
  /** Typecheck tree without adapting it, returning a typed tree.
    ...
    * @param locked
    *   the set of type variables of the current typer state that cannot be
    *   interpolated at the present time
    */
  def typedUnadapted(initTree: untpd.Tree, pt: Type, locked: TypeVars)(using
      Context
  ): Tree =
      ...
      case none =>
        def typedNamed(tree: untpd.NameTree, pt: Type)(using Context): Tree =
          ...
          tree match
            ...
            case tree: untpd.ValDef =>
              if tree.isEmpty then tpd.EmptyValDef
              else typedValDef(tree, sym)(using ctx.localContext(tree, sym))
            case tree: untpd.DefDef =>
              val typer1 = localTyper(sym)
              typer1.typedDefDef(tree, sym)(using
                ctx.localContext(tree, sym).setTyper(typer1)
              )
            case tree: untpd.TypeDef =>
              ...
              def typedTypeOrClassDef: Tree =
                if tree.name eq tpnme.? then
                  val addendum =
                    if sym.owner.is(TypeParam)
                    then ", use `_` to denote a higher-kinded type parameter"
                    else ""
                  val namePos = tree.sourcePos.withSpan(tree.nameSpan)
                  report.errorOrMigrationWarning(
                    em"`?` is not a valid type name$addendum",
                    namePos,
                    from = `3.0`
                  )
                if tree.isClassDef then
                  typedClassDef(tree, sym.asClass)(using
                    ctx.localContext(tree, sym)
                  )
                else
                  typedTypeDef(tree, sym)(using
                    ctx.localContext(tree, sym).setNewScope
                  )
        ...
        def typedUnnamed(tree: untpd.Tree): Tree = tree match
          case tree: untpd.Apply =>
            if ctx.mode is Mode.Pattern then typedUnApply(tree, pt)
          ...
          case tree: untpd.Block =>
            typedBlock(desugar.block(tree), pt)(using ctx.fresh.setNewScope)
          ...
          case tree: untpd.LambdaTypeTree =>
            typedLambdaTypeTree(tree)(using
              ctx.localContext(tree, NoSymbol).setNewScope
            )
          case tree: untpd.TermLambdaTypeTree =>
            typedTermLambdaTypeTree(tree)(using
              ctx.localContext(tree, NoSymbol).setNewScope
            )
          ...
          case tree: untpd.DependentTypeTree =>
            completeTypeTree(untpd.InferredTypeTree(), pt, tree)
          case tree: untpd.InfixOp   => typedInfixOp(tree, pt)
          case tree: untpd.ParsedTry => typedTry(tree, pt)
          case tree @ untpd.PostfixOp(qual, Ident(nme.WILDCARD)) =>
            typedAsFunction(tree, pt)
          ...
          case tree: untpd.MacroTree =>
            report.error("Unexpected macro", tree.srcPos);
            tpd.nullLiteral // ill-formed code may reach here
        ...
        try
          ...
          val result =
            if ifpt.exists
              ...
              && !ctx.isAfterTyper
              && !ctx.isInlineContext
            then makeContextualFunction(xtree, ifpt)
            else
              xtree match
  ...
  protected def simplify(tree: Tree, pt: Type, locked: TypeVars)(using
      Context
  ): tree.type =
    if !tree.denot.isOverloaded
    then // for overloaded trees: resolve overloading before simplifying
      if !tree.tpe.widen.isInstanceOf[
          MethodOrPoly
        ] // wait with simplifying until method is fully applied
        || tree.isDef // ... unless tree is a definition
      then
        interpolateTypeVars(tree, pt, locked)
        val simplified = tree.tpe.simplified
        if !MatchType.thatReducesUsingGadt(tree.tpe)
        then // needs a GADT cast. i15743
          tree.overwriteType(simplified)
    tree

  protected def makeContextualFunction(tree: untpd.Tree, pt: Type)(using
      Context
  ): Tree =
    val defn.FunctionOf(formals, _, true) =
      pt.dropDependentRefinement: @unchecked
    ...
    val paramTypes =
      val hasWildcard = formals.exists(
        _.existsPart(_.isInstanceOf[WildcardType], StopAt.Static)
      )
  ...
  end makeContextualFunction

  /** Typecheck and adapt tree, returning a typed tree. Parameters as for
    * `typedUnadapted`
    */
  def typed(tree: untpd.Tree, pt: Type, locked: TypeVars)(using Context): Tree =
    trace(i"typing $tree, pt = $pt", typr, show = true) {
      ...
      record("typed total")
      if ctx.phase.isTyper then assertPositioned(tree)
      if tree.source != ctx.source && tree.source.exists then
        typed(tree, pt, locked)(using ctx.withSource(tree.source))
      else if ctx.run.nn.isCancelled then tree.withType(WildcardType)
  ...
  def typedStats(stats: List[untpd.Tree], exprOwner: Symbol)(using
      Context
  ): (List[Tree], Context) =
    val buf = new mutable.ListBuffer[Tree]
    var enumContexts: SimpleIdentityMap[Symbol, Context] =
      SimpleIdentityMap.empty
    val initialNotNullInfos = ctx.notNullInfos
    // A map from `enum` symbols to the contexts enclosing their definitions
    @tailrec def traverse(stats: List[untpd.Tree])(using
        Context
    ): (List[Tree], Context) = stats match
        ...
        mdef.removeAttachment(ExpandedTree) match
          ...
          case none =>
            ...
            typed(mdef)(using newCtx) match
              case mdef1: DefDef
                  if mdef1.symbol.is(Inline, butNot = Deferred) && !Inlines
                    .bodyToInline(mdef1.symbol)
                    .isEmpty =>
      ...
      case stat :: rest =>
        ...
        if !checkInterestingResultInStatement(stat1) then
    ...
    def finalize(stat: Tree)(using Context): Tree = stat match
      case stat: TypeDef if stat.symbol.is(Module) =>
        ...
        if enumContext != null then checkEnumCaseRefsLegal(stat, enumContext)
  ...
  /** Tries to adapt NotNullInfos from creation context to the DefTree, returns
    * whether the adaption took place. An adaption only takes place if the
    * DefTree has a symbol and it has not been completed (is not forward
    * referenced).
    */
  ...
  /** Given an inline method `mdef`, the method rewritten so that its body uses
    * accessors to access non-public members. Also, if the inline method is
    * retained, add a method to record the retained version of the body.
  ...
  protected def inlineExpansion(mdef: DefDef)(using Context): List[Tree] =
    tpd.cpy.DefDef(mdef)(rhs = Inlines.bodyToInline(mdef.symbol))
      :: (if mdef.symbol.isRetainedInlineMethod then
            Inlines.bodyRetainer(mdef) :: Nil
          else Nil)

  def typedExpr(tree: untpd.Tree, pt: Type = WildcardType)(using
      Context
  ): Tree =
  ...
  def typedType(
      ...
      mapPatternBounds: Boolean = false
  )(using Context): Tree =
    ...
    if mapPatternBounds && ctx.mode.is(Mode.Pattern) && !ctx.isAfterTyper then
      tree1 match
        case tree1: TypeBoundsTree =>
          ...
          val wildcardSym =
            newPatternBoundSymbol(boundName, tree1.tpe & pt, tree.span)
          untpd.Bind(boundName, tree1).withType(wildcardSym.typeRef)
        case tree1 =>
          tree1
    else tree1

  def typedPattern(tree: untpd.Tree, selType: Type = WildcardType)(using
      Context
  ): Tree =
    withMode(Mode.Pattern)(typed(tree, selType))

  def tryEither[T](op: Context ?=> T)(fallBack: (T, TyperState) => T)(using
      Context
  ): T =
    val nestedCtx = ctx.fresh.setNewTyperState()
    val result = op(using nestedCtx)
    if nestedCtx.reporter.hasErrors && !nestedCtx.reporter.hasStickyErrors then
  ...
  /** Try `op1`, if there are errors, try `op2`, if `op2` also causes errors,
    * fall back to errors and result of `op1`.
    */
  def tryAlternatively[T](
      op1: Context ?=> T
  )(op2: Context ?=> T)(using Context): T =
  ...
  /** Is `pt` a prototype of an `apply` selection, or a parameterless function
    * yielding one?
    */
  ...
  /** Potentially add apply node or implicit conversions. Before trying either,
    ...
    *
    * 0th strategy: If `tree` overrides a nullary method, mark the prototype so
    * that the argument is dropped and return `tree` itself. (but do this at
    * most once per tree).
    *
    * After that, two strategies are tried, and the first that is successful is
    * picked.
    *
    * 1st strategy: Try to insert `.apply` so that the result conforms to
    * prototype `pt`. This strategy is not tried if the prototype represents
    * already another `.apply` or `.apply()` selection.
    *
    * 2nd strategy: If tree is a select `qual.name`, try to insert an implicit
    * conversion around the qualifier part `qual` so that the result conforms to
    * the expected type with wildcard result type.
    *
    * If neither of the strategies are successful, continues with the `apply`
    * result if an apply insertion was tried and `tree` has an `apply` method,
    * or continues with `fallBack` otherwise. `fallBack` is supposed to always
    * give an error.
    */
  def tryInsertApplyOrImplicit(tree: Tree, pt: ProtoType, locked: TypeVars)(
    ...
    def isMethod(tree: Tree) = tree.tpe match
      case ref: TermRef =>
        ref.denot.alternatives.forall(_.info.widen.isInstanceOf[MethodicType])
      case _ => false
    ...
    def tryApply(using Context) =
      ...
      val sel =
        typedSelect(untpd.Select(untpd.TypedSplice(tree), nme.apply), pt1)
      ...
      if sel.tpe.isError then
        ...
        sel match
          case Select(qual, _) =>
            assumeApplyExists = qual
              .getAttachment(Typer.HiddenSearchFailure)
              .exists(
        ...
        try adapt(simplify(sel, pt1, locked), pt1, locked)
        finally sel.removeAttachment(InsertedApply)

    def tryImplicit(fallBack: => Tree) =
      tryInsertImplicitOnQualifier(tree, pt.withContext(ctx), locked)
        .getOrElse(fallBack)

    if ctx.mode.is(Mode.SynthesizeExtMethodReceiver) then
      // Suppress insertion of apply or implicit conversion on extension method receiver
      tree
    else
      pt match
        case pt @ FunProto(Nil, _)
            if tree.symbol.allOverriddenSymbols.exists(
              _.info.isNullaryMethod
            ) &&
          ...
          tree
        case _ =>
          if isApplyProto(pt) || isMethod(tree) || isSyntheticApply(tree) then
            tryImplicit(fallBack)
          else
            tryEither(tryApply) { (app, appState) =>
              tryImplicit {
  ...
  end tryInsertApplyOrImplicit

  /** If this tree is a select node `qual.name` (possibly applied to type
    * variables) that does not conform to `pt`, try two mitigations:
    *   1. Instantiate any TypeVars in the widened type of `tree` with their
    *      lower bounds. 2. Try to insert an implicit conversion `c` around
    *      `qual` so that `c(qual).name` conforms to `pt`.
    */
  def tryInsertImplicitOnQualifier(tree: Tree, pt: Type, locked: TypeVars)(using
      Context
  ): Option[Tree] = trace(i"try insert impl on qualifier $tree $pt") {
    tree match
      case tree @ Select(qual, name) if name != nme.CONSTRUCTOR =>
        if couldInstantiateTypeVar(qual.tpe.widen, applied = true)
        then Some(adapt(tree, pt, locked))
        else
          val selProto =
            SelectionProto(name, pt, NoViewsAllowed, privateOK = false)
          if selProto.isMatchedBy(qual.tpe) || tree.hasAttachment(
              InsertedImplicitOnQualifier
            )
          then None
          else
            tryEither {
      ...
      case TypeApply(fn, args)
          if args.forall(_.isInstanceOf[untpd.InferredTypeTree]) =>
  ...
  def tryExtensionOrConversion(
    ...
    def selectionProto =
      SelectionProto(tree.name, mbrProto, compat, privateOK = inSelect)
      ...
      findRef(
        ...
        case ref: TermRef =>
          def tryExtMethod(ref: TermRef)(using Context) =
            extMethodApply(
              untpd.TypedSplice(tpd.ref(ref).withSpan(tree.nameSpan)),
              qual,
              pt
            )
          if altImports.isEmpty then tryExtMethod(ref)
          else
            ...
            typr.println(
              i"multiple extensioin methods, success: ${successes.toList}, failure: ${failures.toList}"
            )
            ...
            successes.toList match
              ...
              case (expansion1, _) :: (expansion2, _) :: _ =>
                report.error(
                  AmbiguousExtensionMethod(tree, expansion1, expansion2),
                  tree.srcPos
                )
                expansion1
        case _ =>
    ...
    try
      ...
      for err <- remembered do
        rememberSearchFailure(
          qual,
          SearchFailure(
            app.withType(FailedExtension(app, selectionProto, err.msg))
          )
        )
    ...
    if ctx.mode.is(
        Mode.ImplicitsEnabled
      ) && !tree.name.isConstructorName && qual.tpe.isValueType
    then
      try
        val selProto = selectionProto
        trace(i"try insert impl on qualifier $tree $pt") {
          inferView(qual, selProto)
        } match
          case SearchSuccess(found, _, _, isExtension) =>
            if isExtension then return found
            else
              checkImplicitConversionUseOK(found, selProto)
              return withoutMode(Mode.ImplicitsEnabled)(
                typedSelect(tree, pt, found)
              )
          case failure: SearchFailure =>
            if failure.isAmbiguous then
              return if !inSelect // in a selection we will do the canDefineFurther afterwards
              ...
              then
                ...
                err.typeMismatch(
                  ...
                  selProto,
                  failure.reason
                ) // TODO: report NotAMember instead, but need to be aware of failure
  ...
  /** Perform the following adaptations of expression, pattern or type `tree`
    * wrt to given prototype `pt`: (1) Resolve overloading (2) Apply
    * parameterless functions (3) Apply polymorphic types to fresh instances of
    * their type parameters and store these instances in context.undetparams,
    * unless followed by explicit type application. (4) Do the following to
    * unapplied methods used as values: (4.1) If the method has only implicit
    * parameters pass implicit arguments (4.2) otherwise, if `pt` is a function
    * type and method is not a constructor, convert to function by
    * eta-expansion, (4.3) otherwise, if the method is nullary with a result
    * type compatible to `pt` and it is not a constructor, apply it to ()
    * otherwise issue an error (5) Convert constructors in a pattern as follows:
    * (5.1) If constructor refers to a case class factory, set tree's type to
    * the unique instance of its primary constructor that is a subtype of the
    * expected type. (5.2) If constructor refers to an extractor, convert to
    * application of unapply or unapplySeq method.
    *
    * (6) Convert all other types to TypeTree nodes. (7) When in TYPEmode but
    * not FUNmode or HKmode, check that types are fully parameterized (7.1) In
    * HKmode, higher-kinded types are allowed, but they must have the expected
    * kind-arity (8) When in both EXPRmode and FUNmode, add apply method calls
    * to values of object type. (9) If there are undetermined type variables and
    * not POLYmode, infer expression instance Then, if tree's type is not a
    * subtype of expected type, try the following adaptations: (10) If the
    * expected type is Byte, Short or Char, and the expression is an integer
    * fitting in the range of that type, convert it to that type. (11) Widen
    * numeric literals to their expected type, if necessary (12) When in mode
    * EXPRmode, convert E to { E; () } if expected type is scala.Unit. (13) When
    * in mode EXPRmode, apply AnnotationChecker conversion if expected type is
    * annotated. (14) When in mode EXPRmode, apply a view If all this fails,
    * error Parameters as for `typedUnadapted`.
    */
  ...
  private def adapt1(tree: Tree, pt: Type, locked: TypeVars)(using
      Context
  ): Tree =
    assert(
      pt.exists && !pt.isInstanceOf[ExprType] || ctx.reporter.errorsReported,
      i"tree: $tree, pt: $pt"
    )
    ...
    def adaptOverloaded(ref: TermRef) =
      ...
      val altDenots =
        ...
        if pt.isExtensionApplyProto then
          allDenots.filter(_.symbol.is(ExtensionMethod))
        else allDenots

      typr.println(
        i"adapt overloaded $ref with alternatives ${altDenots map (_.info)}%\n\n %"
      )

      /** Search for an alternative that does not take parameters. If there is
        * one, return it, otherwise emit an error.
        */
      ...
      resolveOverloaded(alts, pt) match
        ...
        case ambiAlts =>
          ...
          def errorAmbiguous =
            val remainingDenots =
              altDenots.filter(denot => ambiAlts.contains(altRef(denot)))
            val addendum =
              if ambiAlts.exists(!_.symbol.exists) then i"""|
                    |
                    |Note: Overloaded definitions introduced by refinements cannot be resolved"""
              else ""
            errorTree(
              tree,
              AmbiguousOverload(tree, remainingDenots, pt, addendum)
            )
          end errorAmbiguous
      ...
      case wtp: MethodOrPoly =>
        ...
        if matchingApply(wtp, pt) then
          ...
          then // Under future-migration, don't infer implicit arguments yet for parameters
            ...
            def rewriteMsg =
              Message.rewriteNotice("This code", `future-migration`)
            report.migrationWarning(
            ...
            tree
          else adaptNoArgs(wtp) // insert arguments implicitly
        else if tree.symbol.isPrimaryConstructor && tree.symbol.info.firstParamTypes.isEmpty
        then readapt(tree.appliedToNone) // insert () to primary constructors
        else errorTree(tree, em"Missing arguments for $methodStr")
      case _ =>
    ...
    def adaptNoArgsImplicitMethod(wtp: MethodType): Tree =
      ...
      def addImplicitArgs(using Context) =
          ...
          case formal :: formals1 =>
            ...
            def inferArgsAfter(leading: Tree) =
              val formals2 =
                if wtp.isParamDependent && leading.tpe.exists then
                  formals1.mapconserve(f1 =>
                    safeSubstParam(f1, wtp.paramRefs(argIndex), leading.tpe)
                  )
                else formals1
            ...
            arg.tpe match
              case failed: AmbiguousImplicits =>
                ...
                if (pt1 `ne` pt) && (pt1 ne sharpenedPt) && constrainResult(
                    tree.symbol,
                    wtp,
                    pt1
                  )
              ...
              case failed: SearchFailureType =>
                lazy val defaultArg =
                  ...
                  defaultArgument(appPart(tree), argIndex, testOnly = false)
                    .showing(
                      i"default argument: for $formal, $tree, $argIndex = $result",
                      typr
                    )
        ...
        def issueErrors(): Tree =
          ...
          wtp.paramNames.lazyZip(wtp.paramInfos).lazyZip(args).foreach {
            (paramName, formal, arg) =>
              arg.tpe match
                case failure: SearchFailureType =>
                  report.error(
                    missingArgMsg(
                      arg,
                      formal,
                      implicitParamString(paramName, methodStr, tree),
                      paramSymWithMethodTree(paramName)
                    ),
                    tree.srcPos.endPos
                  )
                case _ =>
        ...
        if propFail.exists then
          ...
          if hasDefaultParams && !propFail.isInstanceOf[AmbiguousImplicits] then
            val namedArgs =
              wtp.paramNames.lazyZip(args).flatMap { (pname, arg) =>
                if arg.tpe.isError then Nil
                else untpd.NamedArg(pname, untpd.TypedSplice(arg)) :: Nil
              }
            val app = cpy.Apply(tree)(untpd.TypedSplice(tree), namedArgs)
            if wtp.isContextualMethod then app.setApplyKind(ApplyKind.Using)
          ...
          tree match
            case tree: Block =>
              readaptSimplified(
                tpd.Block(tree.stats, tpd.Apply(tree.expr, args))
              )
            case tree: NamedArg =>
              readaptSimplified(
                tpd.NamedArg(tree.name, tpd.Apply(tree.arg, args))
              )
            case _ =>
    ...
    end adaptNoArgsImplicitMethod

    /** A synthetic apply should be eta-expanded if it is the apply of an
      * implicit function class, and the expected type is a function type. This
      * rule is needed so we can pass an implicit function to a regular function
      * type. So the following is OK
      *
      * val f: implicit A => B = ??? val g: A => B = f
      *
      * and the last line expands to
      *
      * val g: A => B = (x$0: A) => f.apply(x$0)
      *
      * One could be tempted not to eta expand the rhs, but that would violate
      * the invariant that expressions of implicit function types are always
      * implicit closures, which is exploited by ShortcutImplicits.
      *
      * On the other hand, the following would give an error if there is no
      * implicit instance of A available.
      *
      * val x: AnyRef = f
      *
      * That's intentional, we want to fail here, otherwise some unsuccessful
      * implicit searches would go undetected.
      *
      * Examples for these cases are found in run/implicitFuns.scala and
      * neg/i2006.scala.
      */
    def adaptNoArgsUnappliedMethod(
        wtp: MethodType,
      ...
      def isAutoApplied(sym: Symbol): Boolean =
        sym.isConstructor
          ...
          || Feature.warnOnMigration(
            MissingEmptyArgumentList(sym.show),
            tree.srcPos,
            version = `3.0`
          )
          && { patch(tree.span.endPos, "()"); true }

      /** If this is a selection prototype of the form `.apply(...): R`, return
        * the nested function prototype `(...)R`. Otherwise `pt`.
        */
      ...
      then
        ...
        if pt1 ne pt then
          ...
          if !defn.isFunctionType(pt) then
            pt match
              case SAMType(_)
                  if !pt.classSymbol.hasAnnotation(
                    defn.FunctionalInterfaceAnnot
                  ) =>
                report.warning(
                  em"${tree.symbol} is eta-expanded even though $pt does not have the @FunctionalInterface annotation.",
                  tree.srcPos
                )
              case _ =>
          simplify(typed(etaExpand(tree, wtp, arity), pt), pt, locked)
      else if wtp.paramInfos.isEmpty && isAutoApplied(tree.symbol) then
        readaptSimplified(tpd.Apply(tree, Nil))
      else if wtp.isImplicitMethod then err.typeMismatch(tree, pt)
      else missingArgs(wtp)
    end adaptNoArgsUnappliedMethod

    def adaptNoArgsOther(wtp: Type, functionExpected: Boolean): Tree =
      val implicitFun =
        defn.isContextFunctionType(wtp) && !untpd.isContextualClosure(tree)
      def caseCompanion =
        functionExpected &&
          ...
          !tree.tpe.baseClasses.exists(defn.isFunctionClass) && {
            report.warning(
              "The method `apply` is inserted. The auto insertion will be deprecated, please write `" + tree.show + ".apply` explicitly.",
              tree.sourcePos
            )
            true
          }
      ...
      then
        ...
        val sel = untpd
          .Select(untpd.TypedSplice(tree), nme.apply)
          .withAttachment(InsertedApply, ())
        try typed(sel, pt, locked)
        finally sel.removeAttachment(InsertedApply)
      else if ctx.mode is Mode.Pattern then
        ...
        if meth.isAllOf(DeferredInline) && !Inlines.inInlineMethod then
          errorTree(
            tree,
            em"Deferred inline ${meth.showLocated} cannot be invoked"
          )
        else if Inlines.needsInlining(tree) then
          tree.tpe <:< wildApprox(pt)
          val errorCount = ctx.reporter.errorCount
          val inlined = Inlines.inlineCall(tree)
          if (inlined ne tree) && errorCount == ctx.reporter.errorCount then
            readaptSimplified(inlined)
          else inlined
        ...
        then
          if ctx.settings.XignoreScala2Macros.value then
            report.warning(
              "Scala 2 macro cannot be used in Dotty, this call will crash at runtime. See https://docs.scala-lang.org/scala3/reference/dropped-features/macros.html",
              tree.srcPos.startPos
            )
            Throw(
              New(
                defn.MatchErrorClass.typeRef,
                Literal(
                  Constant(
                    s"Reached unexpanded Scala 2 macro call to ${tree.symbol.showFullName} compiled with -Xignore-scala2-macros."
                  )
                ) :: Nil
              )
            )
          ...
          TypeComparer.testSubType(tree.tpe.widenExpr, pt) match
            ...
            case CompareResult.OKwithGADTUsed
                if pt.isValueType
                  && !inContext(
                    ctx.fresh.setGadtState(GadtState(GadtConstraint.empty))
                  ) {
                    val res = (tree.tpe.widenExpr frozen_<:< pt)
                    if res then
                      // we overshot; a cast is not needed, after all.
                      gadts.println(
                        i"unnecessary GADTused for $tree: ${tree.tpe.widenExpr} vs $pt in ${ctx.source}"
                      )
                    res
                  } =>
    ...
    def sharpenedPt = pt match
      case pt: SelectionProto
          if pt.memberProto.revealIgnored.isExtensionApplyProto =>
        pt.deepenProto
      case _ => pt
      ...
      wtp match
        ...
        case wtp: MethodType
            if wtp.isImplicitMethod &&
              ({
                resMatch = constrainResult(tree.symbol, wtp, sharpenedPt);
                resMatch
              } || !functionExpected) =>
          if resMatch || ctx.mode.is(Mode.ImplicitsEnabled) then
        ...
        case wtp: MethodType if needsEta =>
          ...
          val arity =
            if funExpected then
              if !isFullyDefined(pt, ForceDegree.none) && isFullyDefined(
                  wtp,
                  ForceDegree.none
                )
              then
    ...
    def adaptConstant(tree: Tree, tpe: ConstantType): Tree =
      ...
      tree match
        ...
        case tree @ Block(stats, expr) =>
          tpd.cpy.Block(tree)(stats, adaptConstant(expr, tpe))
        case tree =>
          if isIdempotentExpr(tree) then
            lit // See discussion in phase Literalize why we demand isIdempotentExpr
    ...
    def adaptToSubType(wtp: Type): Tree =
      ...
      ConstFold(tree).tpe.widenTermRefExpr.normalized match
        case ConstantType(x) =>
          ...
          if converted != null && (converted ne x) then
            ...
            then
              report.warning(
                LossyWideningConstantConversion(x.tpe, pt),
                tree.srcPos
              )
      ...
      if captured `ne` wtp then return readapt(tree.cast(captured))
      ...
      if pt isRef defn.UnitClass then
        ...
        if !ctx.isAfterTyper && !tree.isInstanceOf[
            Inlined
          ] && ctx.settings.WvalueDiscard.value && !isThisTypeResult(tree)
      ...
      tree match
        case closure(Nil, id @ Ident(nme.ANON_FUN), _)
          ...
          pt match
            case SAMType(sam)
                if wtp <:< sam
                  .toFunctionType(isJava = pt.classSymbol.is(JavaDefined)) =>
      ...
      if pt.isMatchableBound && !wtp.derivesFrom(defn.MatchableClass) then
        ...
        if target <:< pt then return readapt(tree.cast(target))
    ...
    def adaptType(tp: Type): Tree =
      val tree1 =
        if (pt eq AnyTypeConstructorProto) || tp.typeParamSymbols.isEmpty then
          tree
        else
          val tp1 =
            if ctx.isJava then
              ...
              AppliedType(
                tree.tpe,
                tp.typeParams.map(Function.const(TypeBounds.empty))
              )
            else
              // Eta-expand higher-kinded type
              tree.tpe.EtaExpand(tp.typeParamSymbols)
          tree.withType(tp1)
      if ctx.mode.is(Mode.Pattern) || ctx.mode.is(
          Mode.QuotedPattern
        ) || tree1.tpe <:< pt
      then tree1
      else err.typeMismatch(tree1, pt)

    /** If tree has an error type but no errors are reported yet, issue the
      * error message stored in the type. One way this can happen is if implicit
      * search causes symbols and types to be completed. The types are stored by
      * `typedAhead` so that they can be retrieved later and thus avoid
      * duplication of typechecking work. But if the implicit search causing the
      * `typedAhead` fails locally but another alternative succeeds we can be
      * left with an ErrorType in the tree that went unreported. A scenario
      * where this happens is i1802.scala.
      */
    def ensureReported(tp: Type) = tp match
      case err: ErrorType if !ctx.reporter.errorsReported =>
        report.error(err.msg, tree.srcPos)
      case _ =>
    ...
    def newExpr =
      ...
      val tpt = qual match
        ...
        case _ =>
          errorTree(
            tree,
            em"cannot convert from $tree to an instance creation expression"
          )
      val tycon =
        tree.tpe.widen.finalResultType.underlyingClassRef(refinementOK = false)
      typed(
        untpd.Select(
    ...
    tree match
      case _: MemberDef | _: PackageDef | _: Import | _: WithoutTypeOrPos[?] |
          _: Closure =>
        tree
      case _ =>
        tree.tpe.widen match
            ...
            if isApplyProxy(tree) then newExpr
              ...
              var typeArgs = tree match
                case Select(qual, nme.CONSTRUCTOR) =>
                  qual.tpe.widenDealias.argTypesLo.map(TypeTree(_))
                case _ => Nil
          ...
          case wtp =>
            val isStructuralCall =
              wtp.isValueType && isStructuralTermSelectOrApply(tree)
            if isStructuralCall then readaptSimplified(handleStructural(tree))
            else
              pt match
                case pt: FunProto =>
                  if isApplyProxy(tree) then newExpr
                ...
                case pt: PolyProto if !wtp.isImplicitMethod =>
                  tryInsertApplyOrImplicit(tree, pt, locked)(
                    tree
                  ) // error will be reported in typedTypeApply
                case _ =>
                  if ctx.mode is Mode.Type then adaptType(tree.tpe)
  ...
  /** Does the "contextuality" of the method type `methType` match the one of
    * the prototype `pt`? This is the case if
    *   - both are contextual, or
    *   - neither is contextual, or
    *   - the prototype is contextual and the method type is implicit. The last
    *     rule is there for a transition period; it allows to mix `with`
    *     applications with old-style context functions. Overridden in
    *     `ReTyper`, where all applications are treated the same
    */
  protected def matchingApply(methType: MethodOrPoly, pt: FunProto)(using
      Context
  ): Boolean =
  ...
  /** Check that `tree == x: pt` is typeable. Used when checking a pattern
    ...
    */
  protected def checkEqualityEvidence(tree: tpd.Tree, pt: Type)(using
      Context
  ): Unit =
    tree match
        ...
        def approx = new ApproximatingTypeMap:
          ...
          def apply(tp: Type) = tp.dealias match
            case tp: TypeRef if !tp.symbol.isClass =>
              if alreadyExpanding contains tp then tp
              else
  ...
  private def checkInterestingResultInStatement(t: Tree)(using
      Context
  ): Boolean =
    ...
    def isJavaApplication(t: Tree): Boolean = t match
      case Apply(f, _) =>
        f.symbol.is(JavaDefined) && !defn.ObjectClass.isSubClass(f.symbol.owner)
      case _ => false
    def checkInterestingShapes(t: Tree): Boolean = t match
      case If(_, thenpart, elsepart) =>
        checkInterestingShapes(thenpart) || checkInterestingShapes(elsepart)
    ...
    if ctx.settings.WNonUnitStatement.value && !ctx.isAfterTyper && checkInterestingShapes(
        t
      )
    then
      val where = t match
  ...
  private def checkStatementPurity(
      tree: tpd.Tree
  )(original: untpd.Tree, exprOwner: Symbol)(using Context): Unit =
    ...
    then
      tree match
        case closureDef(meth)
            if meth.span == meth.rhs.span.toSynthetic && !untpd.isFunction(
              original
            ) =>
  ...
  protected def typedScala2MacroBody(call: untpd.Tree)(using Context): Tree =
    ...
    def typedPrefix(
        tree: untpd.RefTree
    )(splice: Context ?=> Tree => Tree)(using Context): Tree =
      tryAlternatively {
        ...
        val ref = tree match
          ...
          case Select(qual, name) =>
            untpd.Select(qual, name.toTypeName).withSpan(tree.span)
        val bundle = untpd
          .Apply(
            untpd.Select(untpd.New(ref), nme.CONSTRUCTOR),
            untpd.Literal(Constant(null))
          )
          .withSpan(call.span)
        val bundle1 = typedExpr(bundle, defn.AnyType)
        val bundleVal =
          SyntheticValDef(NameKinds.UniqueName.fresh(nme.bundle), bundle1)
            .withSpan(call.span)
        tpd
          .Block(List(bundleVal), splice(tpd.ref(bundleVal.symbol)))
          .withSpan(call.span)
      }
      ...
      call match
        ...
        case untpd.Select(qual: untpd.RefTree, name) =>
          typedPrefix(qual) { qual =>
            val call2 =
              untpd.Select(untpd.TypedSplice(qual), name).withSpan(call.span)
            typedSelect(call2, defn.AnyType)
          }
        case untpd.TypeApply(untpd.Select(qual: untpd.RefTree, name), targs) =>
          typedPrefix(qual) { qual =>
            val call2 = untpd
              .TypeApply(untpd.Select(untpd.TypedSplice(qual), name), targs)
              .withSpan(call.span)
  ...
  end typedScala2MacroBody

  /** Insert GADT cast to target type `pt` on the `tree` so that -Ycheck in
    * later phases succeeds. The check "safeToInstantiate" in `maximizeType`
    * works to prevent unsound GADT casts.
    */
  private def insertGadtCast(tree: Tree, wtp: Type, pt: Type)(using
      Context
  ): Tree =
    val target =
      if tree.tpe.isSingleton then
        ...
        if tree.tpe.isStable && !conj.isStable then
          ...
          AnnotatedType(
            conj,
            Annotation(defn.UncheckedStableAnnot, tree.symbol.span)
          )
        else conj
      else pt
