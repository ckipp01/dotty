...
import Contexts.*, Symbols.*, Types.*, SymDenotations.*, Names.*, NameOps.*,
  Flags.*
...
/** This class creates symbols from definitions and imports and gives them lazy
  * types.
  *
  * Timeline:
  *
  * During enter, trees are expanded as necessary, populating the expandedTree
  * map. Symbols are created, and the symOfTree map is set up.
  *
  * Symbol completion causes some trees to be already typechecked and typedTree
  * entries are created to associate the typed trees with the untyped expanded
  * originals.
  *
  * During typer, original trees are first expanded using expandedTree. For each
  * expanded member definition or import we extract and remove the corresponding
  * symbol from the symOfTree map and complete it. We then consult the typedTree
  * map to see whether a typed tree exists already. If yes, the typed tree is
  * returned as result. Otherwise, we proceed with regular type checking.
  *
  * The scheme is designed to allow sharing of nodes, as long as each duplicate
  * appears in a different method.
  */
class Namer:
  typer: Typer =>
  ...
  /** A partial map from unexpanded member and pattern defs and to their
    * expansions. Populated during enterSyms, emptied during typer.
    */
  ...
  /** A map from expanded MemberDef, PatDef or Import trees to their symbols.
    * Populated during enterSyms, emptied at the point a typed tree with the
    * same symbol is created (this can be when the symbol is completed or at the
    * latest when the tree is typechecked.
    */
  // lazy val symOfTree = new mutable.AnyRefMap[Tree, Symbol]

  /** A map from expanded trees to their typed versions. Populated when trees
    * are typechecked during completion (using method typedAhead).
    */
  // lazy val typedTree = new mutable.AnyRefMap[Tree, tpd.Tree]

  /** A map from method symbols to nested typers. Populated when methods are
    * completed. Emptied when they are typechecked. The nested typer contains
    * new versions of the four maps above including this one, so that trees that
    * are shared between different DefDefs can be independently used as indices.
    * It also contains a scope that contains nested parameters.
    */
    ...
    xtree.getAttachment(TypedAhead) match
      ...
      case none =>
        xtree.getAttachment(SymOfTree) match
          case Some(sym) => sym
          case _ =>
            throw IllegalArgumentException(i"$xtree does not have a symbol")
  ...
  def enclosingClassNamed(name: TypeName, span: Span)(using Context): Symbol =
    if name.isEmpty then NoSymbol
      ...
      if !cls.exists then
        report.error(
          UnknownNamedEnclosingClassOrObject(name),
  ...
  /** Check that a new definition with given name and privacy status in current
    * context would not conflict with existing currently compiled definitions.
    * The logic here is very subtle and fragile due to the fact that we are not
    * allowed to force anything.
    */
  def checkNoConflict(name: Name, isPrivate: Boolean, span: Span)(using
      Context
  ): Name =
    val owner = ctx.owner
    ...
    def checkNoConflictIn(owner: Symbol) =
      ...
      if (preExisting.isDefinedInCurrentRun || preExisting.lastKnownDenotation
          .is(Package))
        && (!preExisting.lastKnownDenotation.is(Private) || preExisting.owner
          .is(Package))
    ...
    if owner.is(PackageClass) then
      ...
      for pkgObj <- pkgObjs(owner) do checkNoConflictIn(pkgObj)
    else
      def preExisting = ctx.effectiveScope.lookup(name)
      if (!owner.isClass || name.isTypeName) && preExisting.exists then
        conflict(preExisting)
      else if owner.isPackageObject && !isPrivate && name != nme.CONSTRUCTOR
      then
  ...
  /** If this tree is a member def or an import, create a symbol of it and store
    * in symOfTree map.
    */
  def createSymbol(tree: Tree)(using Context): Symbol =
    ...
    /** Check that flags are OK for symbol. This is done early to avoid
      * catastrophic failure when we create a TermSymbol with TypeFlags, or vice
      * versa. A more complete check is done in checkWellFormed. Also,
      * speculatively add a Local flag to private members that can be Local if
      * referred to exclusively from their owner's this-type. The Local flag is
      * retracted in `isAccessibleFrom` if the access not from such a this-type.
      */
    def checkFlags(flags: FlagSet) =
      if flags.isEmpty then flags
        ...
        if !ok then
          report.error(
            em"modifier(s) `${flags.flagsString}` incompatible with $kind definition",
            tree.srcPos
          )
    ...
    def adjustIfModule(completer: LazyType, tree: MemberDef) =
      if tree.mods.is(Module) then adjustModuleCompleter(completer, tree.name)
      else completer
    ...
    def createOrRefine[S <: Symbol](
        ...
        owner: Symbol,
        infoFn: S => Type,
        symFn: (FlagSet, S => Type, Symbol) => S
    ): Symbol =
      val prev =
        if lateCompile && ctx.owner.is(Package) then
          ctx.effectiveScope.lookup(name)
        else NoSymbol

      var flags1 = flags
      var privateWithin = privateWithinClass(tree.mods)
      val effectiveOwner = owner.skipWeakOwner
      if flags.is(Private) && effectiveOwner.is(Package) then
    ...
    tree match
      case tree: TypeDef if tree.isClassDef =>
        ...
        val name =
          checkNoConflict(tree.name, flags.is(Private), tree.span).asTypeName
        val cls =
          createOrRefine[ClassSymbol](
            tree,
            name,
            flags,
            ctx.owner,
      ...
      case tree: MemberDef =>
        ...
        tree match
          case tree: ValOrDefDef =>
            if tree.isInstanceOf[ValDef] && !flags.is(Param) && name.endsWith(
                "_="
              )
            then
              report.error(
                "Names of vals or vars may not end in `_=`",
                tree.namePos
              )
            ...
            then flags |= Deferred
            if tree.isInstanceOf[DefDef] then flags |= Method
            else if flags.isAllOf(EnumValue) && ctx.owner.isStaticOwner then
              flags |= JavaStatic
          case tree: TypeDef =>
        ...
        val cctx =
          if tree.name == nme.CONSTRUCTOR && !flags.is(JavaDefined) then
            ctx.outer
          else ctx
        ...
        createOrRefine[Symbol](
          ...
          _ => info,
          (fs, _, pwithin) =>
            newSymbol(ctx.owner, name, fs, info, pwithin, tree.nameSpan)
        )
      case tree: Import =>
        recordSym(
          newImportSymbol(ctx.owner, Completer(tree)(ctx), tree.span),
          tree
        )
      case _ =>
        NoSymbol
  ...
  /** If `sym` exists, enter it in effective scope. Check that package members
    * are not entered twice in the same run.
    */
  ...
  private def createPackageSymbol(pid: RefTree)(using Context): Symbol =
    val pkgOwner = pid match
      case Ident(_) =>
        if ctx.owner eq defn.EmptyPackageClass then defn.RootClass
        else ctx.owner
      case Select(qual: RefTree, _) => createPackageSymbol(qual).moduleClass
    val existing = pkgOwner.info.decls.lookup(pid.name)

    if existing.is(Package) && (pkgOwner eq existing.owner) then
  ...
  def expanded(tree: Tree)(using Context): Tree = tree match
    case _: DefTree | _: PackageDef | _: ExtMethods =>
      tree.attachmentOrElse(ExpandedTree, tree)
    case _ => tree

  /** For all class definitions `stat` in `xstats`: If the companion class is
    * not also defined in `xstats`, invalidate it by setting its info to NoType.
    */
  def invalidateCompanions(pkg: Symbol, xstats: List[untpd.Tree])(using
      Context
  ): Unit =
    val definedNames = xstats collect { case stat: NameTree => stat.name }
    def invalidate(name: TypeName) =
      if !(definedNames contains name) then
        val member = pkg.info.decl(name).asSymDenotation
        if member.isClass && !(member.is(Package)) then member.markAbsent()
    xstats foreach {
  ...
  /** Expand tree and create top-level symbols for statement and enter them into
    * symbol table
    */
  ...
  /** Create top-level symbols for all statements in the expansion of this
    * statement and enter them into symbol table
    */
  ...
  /** Ensure that the first type in a list of parent types Ps points to a
    * non-trait class. If that's not already the case, add one. The added class
    * type CT is determined as follows. First, let C be the unique class such
    * that
    *   - there is a parent P_i such that P_i derives from C, and
    *   - for every class D: If some parent P_j, j <= i derives from D, then C
    *     derives from D. Then, let CT be the smallest type which
    *   - has C as its class symbol, and
    *   - for all parents P_i: If P_i derives from C then P_i <:< CT.
    *
    * Tweak: It could be that at the point where the method is called, some
    * superclass is still missing its parents. Parents are set to Nil when
    * completion starts and are set to the actual parents later. If a superclass
    * completes a subclass in one of its parents, the parents of the superclass
    * or some intervening class might not yet be set. This situation can be
    * detected by asking for the baseType of Any - if that type does not exist,
    * one of the base classes of this class misses its parents. If this
    * situation arises, the computation of the superclass might be imprecise.
    * For instance, in i12722.scala, the superclass of `IPersonalCoinOps` is
    * computed as `Object`, where `JsObject` would be correct. The problem
    * cannot be solved locally, but we detect the situaton and mark the
    * superclass with a `@ProvisionalSuperClass` annotation in this case. When
    * typechecking the class, we then run ensureFirstIsClass again and possibly
    * improve the computed super class. An alternatiev fix would compute
    * superclasses at typer instead at completion. But that breaks too many
    * invariants. For instance, we rely on correct @Child annotations after
    * completion, and these in turn need the superclass.
    */
  def ensureFirstIsClass(cls: ClassSymbol, parents: List[Type])(using
      Context
  ): List[Type] =
    ...
    parents match
      ...
      case _ =>
        ...
        typr.println(
          i"ensure first is class $parents%, % --> ${parents map (_ baseType pcls)}%, %"
        )
        ...
        if isProvisional then
          ...
          first = AnnotatedType(
            first,
            Annotation(defn.ProvisionalSuperClassAnnot, cls.span)
          )
        checkFeasibleParent(
          first,
          cls.srcPos,
          i" in inferred superclass $first"
        ) :: parents
  ...
  def addEnumConstants(mdef: DefTree, sym: Symbol)(using Context): Unit =
    mdef match
      case vdef: ValDef if (isEnumConstant(vdef)) =>
        ...
        if !enumClass.is(Sealed) then enumClass.setFlag(Flags.AbstractSealed)
  ...
  def index(stats: List[Tree])(using Context): Context =
    ...
    def mergeModuleClass(
        ...
        if tree == modCls then
          ...
          res = cpy.TypeDef(modCls)(
            rhs = cpy.Template(modTempl)(
              derived =
                if fromTempl.derived.nonEmpty then fromTempl.derived
                else modTempl.derived,
          ...
          if fromTempl.derived.nonEmpty then
            if modTempl.derived.nonEmpty then
              report.error(
                em"a class and its companion cannot both have `derives` clauses",
                mdef.srcPos
              )
            // `res` is inside a closure, so the flow-typing doesn't work here.
            res.uncheckedNN.putAttachment(
              desugar.DerivingCompanion,
              fromTempl.srcPos.startPos
            )
          res.uncheckedNN
        else tree
      }
      ...
      res.nn
    end mergeModuleClass

    /** Merge `fromCls` of `fromStat` into `toCls` of `toStat` if the former is
      * synthetic and the latter not.
      *
      * Note:
      *   1. `fromStat` and `toStat` could be the same stat 2. `fromCls` and
      *      `toCls` are necessarily different
      */
    def mergeIfSynthetic(
        fromStat: Tree,
        fromCls: TypeDef,
        toStat: Tree,
        toCls: TypeDef
    ): Unit =
      if fromCls.mods.is(Synthetic) && !toCls.mods.is(Synthetic) then
    ...
    def mergeCompanionDefs() =
      def valid(mdef: MemberDef): Boolean =
              ...
              case vdef @ ValDef(name, _, _) if valid(vdef) =>
                moduleValDef.get(name) match
                  case Some((stat1, vdef1)) =>
                    if vdef.mods.is(Synthetic) && !vdef1.mods.is(Synthetic) then
    ...
    end mergeCompanionDefs

    val classDef = mutable.Map[TypeName, TypeDef]()
    val moduleDef = mutable.Map[TypeName, TypeDef]()

    /** Create links between companion object and companion class. Populate
      * `moduleDef` and `classDef` as a side effect.
      */
    def createCompanionLinks()(using Context): Unit =

      def updateCache(cdef: TypeDef): Unit =
        if cdef.isClassDef && !cdef.mods.is(Package) then
          if cdef.mods.is(ModuleClass) then moduleDef(cdef.name) = cdef
      ...
      for cdef @ TypeDef(name, _) <- classDef.values do
        moduleDef.getOrElse(name.moduleClassName, EmptyTree) match
          case t: TypeDef =>
            createLinks(cdef, t)
          case EmptyTree =>
    end createCompanionLinks
    ...
    /** If a top-level object or class has no companion in the current run, we
      ...
      * proxy in scope. This ensures that we never use a companion from a
      * previous run or from thenclass path. See tests/pos/false-companion for
      * an example where this matters. Also: We add constructor proxies for
      * classes in some local scope, i.e. that are not members of other classes.
      * Constructor proxies for member classes are added in
      * addConstructorProxies.
      */
    def addAbsentCompanions()(using Context): Unit =
      if ctx.owner.isTerm then
        for case cdef @ TypeDef(className, _) <- classDef.values do
          ...
          if needsConstructorProxies(classSym) && ctx.effectiveScope
              .lookupEntry(moduleName) == null
          then enterSymbol(classConstructorCompanion(classSym.asClass))
        ...
        for case cdef @ TypeDef(moduleName, _) <- moduleDef.values do
          ...
          if moduleSym.isDefinedInCurrentRun then
            ...
            if !classSym.isDefinedInCurrentRun then
              val absentClassSymbol =
                newClassSymbol(ctx.owner, className, EmptyFlags, _ => NoType)
    ...
    mergeCompanionDefs()
    val ctxWithStats =
      stats.foldLeft(ctx)((ctx, stat) => indexExpanded(stat)(using ctx))
  ...
  /** Parse the source and index symbols in the compilation unit's untpdTree
    * while asserting the `lateCompile` flag. This will cause any old top-level
    * symbol with the same fully qualified name as a newly created symbol to be
    * replaced.
    *
    * Will call the callback with an implementation of type checking That will
    * set the tpdTree and root tree for the compilation unit.
    */
  def lateEnterUnit(typeCheckCB: (() => Unit) => Unit)(using Context) =
    ...
    def lateEnter()(using Context): Context =
      ...
      lateCompile = true
      try index(unit.untpdTree :: Nil)
      finally lateCompile = saved
    ...
    unit.untpdTree =
      if unit.isJava then new JavaParser(unit.source).parse()
  ...
  /** The type bound on wildcard imports of an import list, with special values
    * Nothing if no wildcard imports of this kind exist Any if there are
    * unbounded wildcard imports of this kind
    */
  def importBound(sels: List[untpd.ImportSelector], isGiven: Boolean)(using
      Context
  ): Type =
  ...
  def missingType(sym: Symbol, modifier: String)(using Context): Unit =
    report.error(
      em"${modifier}type of implicit definition needs to be given explicitly",
      sym.srcPos
    )
    sym.resetFlag(GivenOrImplicit)

  /** The completer of a symbol defined by a member def or import (except
    * ClassSymbols)
    */
  class Completer(val original: Tree)(ictx: Context)
    ...
    protected def typeSig(sym: Symbol): Type = original match
      case original: ValDef =>
        if sym.is(Module) then moduleValSig(sym)
        else
          valOrDefDefSig(original, sym, Nil, identity)(using
            localContext(sym).setNewScope
          )
      case original: DefDef =>
        val typer1 = ctx.typer.newLikeThis(ctx.nestingLevel + 1)
        nestedTyper(sym) = typer1
        typer1.defDefSig(original, sym, this)(using
          localContext(sym).setTyper(typer1)
        )
      case imp: Import =>
        try
          val expr1 = typedImportQualifier(
            imp,
            typedAheadExpr(_, _)(using ctx.withOwner(sym))
          )
          ImportType(expr1)
        catch
    ...
    final override def complete(denot: SymDenotation)(using Context): Unit =
      ...
      then
        def levels(c: Context): Int =
          if c.typerState eq creationContext.typerState then 0
          else if c.outer.typerState == c.typerState then levels(c.outer)
          else levels(c.outer) + 1
        println(
          s"!!!completing ${denot.symbol.showLocated} in buried typerState, gap = ${levels(ctx)}"
        )
      val creationRunId = creationContext.runId
      if ctx.runId > creationRunId then
        assert(
          ctx.mode.is(Mode.Interactive),
          s"completing $denot in wrong run ${ctx.runId}, was created in $creationRunId"
        )
        denot.info = UnspecifiedErrorType
      else
        try
          completeInCreationContext(denot)
          if denot.isCompleted then registerIfChild(denot)
        catch
    ...
    protected def addAnnotations(sym: Symbol): Unit = original match
      case original: untpd.MemberDef =>
        ...
        for annotTree <- original.mods.annotations do
      ...
      case original: untpd.DefDef if sym.isInlineMethod =>
        def rhsToInline(using Context): tpd.Tree =
          if !original.symbol.exists && !hasDefinedSymbol(original) then
            throw if sym.isCompleted then Inlines.MissingInlineInfo()
        ...
        PrepareInlineable.registerInlineInfo(sym, rhsToInline)(using
          localContext(sym)
        )
      case _ =>

    /** Invalidate `denot` by overwriting its info with `NoType` if `denot` is a
      * compiler generated case class method that clashes with a user-defined
      * method in the same scope with a matching type.
      */
    private def invalidateIfClashingSynthetic(denot: SymDenotation): Unit =
      def isCaseClassOrCompanion(owner: Symbol) =
        owner.isClass && {
          if owner.is(Module) then owner.linkedClass.is(CaseClass)
          else owner.is(CaseClass)
        }

      def definesMember =
        denot.owner.info.decls
          .lookupAll(denot.name)
          .exists(alt =>
      ...
      def inheritsConcreteMember =
        denot.owner.asClass.info.parents.exists(parent =>
          parent
            .member(denot.name)
            .hasAltWith(sd => !sd.symbol.is(Deferred) && sd.matches(denot))
        )
          ...
          (
            (desugar.isRetractableCaseClassMethodName(denot.name)
              ...
              ||
                ...
                (denot.isConstructor
                  && denot.owner.is(JavaDefined) && denot.owner.derivesFrom(
                    defn.JavaRecordClass
                  )
                  && denot.owner.unforcedDecls
                    .lookupAll(denot.name)
                    .exists(c =>
                      c != denot.symbol && c.info.matches(denot.info)
                    ))
          )
    ...
    /** If completed symbol is an enum value or a named class, register it as a
      * child in all direct parent classes which are sealed.
      */
    def registerIfChild(denot: SymDenotation)(using Context): Unit =
      val sym = denot.symbol

      def register(child: Symbol, parentCls: ClassSymbol) =
        if parentCls.is(Sealed) then
          if (child.isInaccessibleChildOf(
              parentCls
            ) || child.isAnonymousClass) && !sym.hasAnonymousChild
          then addChild(parentCls, parentCls)
          else if !parentCls.is(ChildrenQueried) then addChild(parentCls, child)
          else
            report.error(
              em"""children of $parentCls were already queried before $sym was discovered.
      ...
      if denot.isClass && !sym.isEnumAnonymClass && !sym.isRefinementClass then
        val child =
          if denot.is(Module) then denot.sourceModule else denot.symbol
        denot.info.parents.foreach { parent =>
          register(child, parent.classSymbol.asClass)
        }
      else if denot.is(CaseVal, butNot = MethodOrModule) then
        assert(denot.is(Enum), denot)
        denot.info.classSymbols.foreach { parent =>
          register(denot.symbol, parent)
        }
      end if
    end registerIfChild

    /** Intentionally left without `implicit ctx` parameter. We need to pick up
      * the context at the point where the completer was created.
      */
  ...
  class TypeDefCompleter(original: TypeDef)(ictx: Context)
      extends Completer(original)(ictx)
      with TypeParamsCompleter:
    private var myTypeParams: List[TypeSymbol] | Null = null
    private var nestedCtx: Context | Null = null
    assert(!original.isClassDef)

    /** If completion of the owner of the to be completed symbol has not yet
      * started, complete the owner first and check again. This prevents cyclic
      * references where we need to copmplete a type parameter that has an owner
      * that is not yet completed. Test case is pos/i10967.scala.
      */
    ...
    override def completerTypeParams(sym: Symbol)(using
        Context
    ): List[TypeSymbol] =
        ...
        def typeParamTrees(tdef: Tree): List[TypeDef] = tdef match
          case TypeDef(_, original) =>
            original match
              ...
              case original: DerivedFromParamTree =>
                typeParamTrees(original.watched)
              case _ => Nil
          case _ => Nil
    ...
    override final def typeSig(sym: Symbol): Type =
      ...
      val unsafeInfo =
        if isDerived then rhsBodyType else abstracted(rhsBodyType)

      def opaqueToBounds(info: Type): Type =
        if sym.isOpaqueAlias then
          if info.typeParams.nonEmpty && info.hkResult.typeParams.nonEmpty then
            report.error(
              em"opaque type alias cannot have multiple type parameter lists",
              rhs.srcPos
            )
      ...
      def ensureUpToDate(tref: TypeRef, outdated: Type) =
        if tref.info == outdated && sym.info != outdated then
          tref.recomputeDenot()
      ensureUpToDate(sym.typeRef, dummyInfo1)
      if dummyInfo2 `ne` dummyInfo1 then ensureUpToDate(sym.typeRef, dummyInfo2)

      sym.info
    end typeSig
  end TypeDefCompleter
  ...
  class ClassCompleter(cls: ClassSymbol, original: TypeDef)(ictx: Context)
    ...
    /** info to be used temporarily while completing the class, to avoid cyclic
      * references.
      */
    private var tempInfo: TempClassInfo | Null = null

    val TypeDef(name, impl @ Template(constr, _, self, _)) =
      original: @unchecked
    ...
    /** The forwarders defined by export `exp`
      * @param pathMethod
      *   If it exists, the symbol referenced by the path of an export in an
      *   extension clause. That symbol is always a companion extension method.
      */
    private def exportForwarders(exp: Export, pathMethod: Symbol)(using
        Context
    ): List[tpd.MemberDef] =
      ...
      def canForward(mbr: SingleDenotation, alias: TermName): CanForward =
        import CanForward.*
        val sym = mbr.symbol
        if !sym.isAccessibleFrom(pathType) then No("is not accessible")
        else if sym.isConstructor || sym.is(ModuleClass) || sym.is(
            Bridge
          ) || sym.is(ConstructorProxy) || sym.isAllOf(JavaModule)
        then Skip
        else if cls
            .derivesFrom(sym.owner) && (sym.owner == cls || !sym.is(Deferred))
        then No(i"is already a member of $cls")
          ...
          No(
            "is already an extension method, cannot be exported into another one"
          )
          ...
          sym.allOverriddenSymbols.find(other =>
            cls.derivesFrom(other.owner) && !other.is(Deferred)
          ) match
            case Some(other) =>
              No(
                i"overrides ${other.showLocated}, which is already a member of $cls"
              )
            case None => Yes
        else Yes

      def foreachDefaultGetterOf(
          sym: TermSymbol,
          op: TermSymbol => Unit
      ): Unit =
        var n = 0
        for params <- sym.paramSymss; param <- params do
          if param.isTerm then
            if param.is(HasDefault) then
              ...
              val getter =
                pathType.member(DefaultGetterName(sym.name, n)).symbol
              assert(
                getter.exists,
                i"$path does not have a default getter named $getterName"
              )
              op(getter.asTerm)
            n += 1

      /** Add a forwarder with name `alias` or its type name equivalent to
        * `mbr`, provided `mbr` is accessible and of the right
        * implicit/non-implicit kind.
        */
      def addForwarder(
          alias: TermName,
          mbr: SingleDenotation,
          span: Span
      ): Unit =

        def adaptForwarderParams(
            acc: List[List[tpd.Tree]],
            tp: Type,
            prefss: List[List[tpd.Tree]]
        ...
        if canForward(mbr, alias) == CanForward.Yes then
          val sym = mbr.symbol
          val hasDefaults =
            sym.hasDefaultParams // compute here to ensure HasDefaultParams and NoDefaultParams flags are set
          val forwarder =
            if mbr.isType then
              val forwarderName =
                checkNoConflict(alias.toTypeName, isPrivate = false, span)
              ...
              def refersToPrivate(tp: Type): Boolean = tp match
                case tp: TermRef =>
                  tp.termSymbol.is(Private) || refersToPrivate(tp.prefix)
                case _ => false
                    ...
                    def avoidNameClashes(info: Type): Type = info match
                      case info: MethodOrPoly =>
                        info.derivedLambdaType(
                          paramNames = info.paramNames.mapConserve { pname =>
                            if defines(pathType, pname) then pname.freshened
                            else pname
                          },
          ...
          if forwarder.isType then
            ...
            val ddef = tpd.DefDef(
              ...
              prefss =>
                ...
                val (pathRefss, methRefss) =
                  prefss.splitAt(extensionParamsCount(path.tpe.widen))
                val ref = path.appliedToArgss(pathRefss).select(sym.asTerm)
                val rhs = ref
                  .appliedToArgss(
                    adaptForwarderParams(Nil, sym.info, methRefss)
                  )
                ...
                if forwarder.isInlineMethod then
                  ...
                  // is already typed.
                  val inlinableRhs =
                    PrepareInlineable.makeInlineable(rhs)(using forwarderCtx)
                  PrepareInlineable.registerInlineInfo(forwarder, inlinableRhs)(
                    using forwarderCtx
                  )
                  inlinableRhs
                else rhs
            ...
            if hasDefaults then
              foreachDefaultGetterOf(
                sym.asTerm,
                getter =>
                  addForwarder(
                    getter.name.asTermName,
                    getter.asSeenFrom(path.tpe),
                    span
                  )
              )
          end if
      ...
      def addForwardersNamed(
        ...
        val size = buf.size
        val mbrs =
          List(name, name.toTypeName).flatMap(pathType.member(_).alternatives)
        mbrs.foreach(addForwarder(alias, _, span))
        if buf.size == size then
          val reason = mbrs
            .map(canForward(_, alias))
            .collect { case CanForward.No(whyNot) =>
              i"\n$path.$name cannot be exported because it $whyNot"
            }
            .headOption
            .getOrElse("")
          report.error(
            em"""no eligible member $name at $path$reason""",
            ctx.source.atSpan(span)
          )
        else targets += alias
        ...
        List(name, name.toTypeName)
          .flatMap(
            pathType
              .memberBasedOnFlags(
                _,
                excluded = Private | Given | ConstructorProxy
              )
              .alternatives
          )
      ...
      def addWildcardForwarders(seen: List[TermName], span: Span): Unit =
        ...
        for mbr <- pathType.membersBasedOnFlags(
            required = EmptyFlags,
            excluded = PrivateOrSynthetic
          )
        do
          ...
          then
            ...
            if mbr.symbol.is(Given) then
              if !seen.contains(alias) && mbr.matchesImportBound(givenBound)
              then addForwarder(alias, mbr, span)
            else if !nonContextual.contains(alias) && mbr.matchesImportBound(
                wildcardBound
              )
            then
              nonContextual += alias
              addWildcardForwardersNamed(alias, span)
      end addWildcardForwarders

      def addForwarders(
          sels: List[untpd.ImportSelector],
          seen: List[TermName]
      ): Unit = sels match
        case sel :: sels1 =>
          if sel.isWildcard then addWildcardForwarders(seen, sel.span)
          else
      ...
      /** Avoid a clash of export forwarder `forwarder` with other forwarders in
        * `forwarders`.
        * @return
        *   If `forwarder` clashes, a new leading forwarder and trailing
        *   forwarders list that avoids the clash according to the scheme
        *   described in `avoidClashes`. If there's no clash, the inputs as they
        *   are in a pair.
        */
      def avoidClashWith(
          forwarder: tpd.DefDef,
          forwarders: List[tpd.MemberDef]
      ): (tpd.DefDef, List[tpd.MemberDef]) =
        ...
        forwarders match
          case forwarders @ ((forwarder1: tpd.DefDef) :: forwarders1)
            ...
            if clashes(forwarder.symbol, forwarder1.symbol) then
              ...
              avoidClashWith(
                if cmp < 0 then forwarder1 else forwarder,
                forwarders1
              )
            else
              val (forwarder2, forwarders2) =
                avoidClashWith(forwarder, forwarders1)
      ...
      /** Avoid clashes of any two export forwarders in `forwarders`. A clash is
        * if two forwarders f1 and f2 have the same name and signatures after
        * erasure. We try to avoid a clash by dropping one of f1 and f2, keeping
        * the one whose right hand side reference would be preferred by
        * overloading resolution. If neither of f1 or f2 is preferred over the
        * other, report an error.
        *
        * The idea is that this simulates the hypothetical case where export
        * forwarders are not generated and we treat an export instead more like
        * an import where we expand the use site reference. Test cases in
        * {neg,pos}/i14966.scala.
        *
        * @pre
        *   Forwarders with the same name are consecutive in `forwarders`.
        */
      def avoidClashes(forwarders: List[tpd.MemberDef]): List[tpd.MemberDef] =
        forwarders match
          case forwarders @ (forwarder :: forwarders1) =>
            val (forwarder2, forwarders2) = forwarder match
              case forwarder: tpd.DefDef =>
                avoidClashWith(forwarder, forwarders1)
    ...
    private def processExports(using Context): Unit =
        ...
        path match
          case Ident(name) =>
            def matches(cand: Tree) = cand match
              case meth: DefDef =>
                meth.name == name && meth.paramss.hasSameLengthAs(ext.paramss)
              case _ => false
            expanded(ext).toList.filter(matches) match
              case cand :: Nil => symbolOfTree(cand)
              case Nil =>
                fail(i"$name is not a parameterless companion extension method")
      ...
      if hasExport(rest) then process(rest)
    end processExports

    /** Ensure constructor is completed so that any parameter accessors which
      * have type trees deriving from its parameters can be completed in turn.
      * Note that parent types access such parameter accessors, that's why the
      * constructor needs to be completed before the parent types are
      * elaborated.
      */
    def completeConstructor(denot: SymDenotation): Unit =
      if tempInfo != null then // Constructor has been completed already
        return

      addAnnotations(denot.symbol)

      val selfInfo: TypeOrSymbol =
        if self.isEmpty then NoType
        else if cls.is(Module) then
          val moduleType = cls.owner.thisType select sourceModule
          if self.name == nme.WILDCARD then moduleType
          else
            recordSym(
              newSymbol(
                cls,
      ...
      symbolOfTree(
        constr
      ).info.stripPoly match // Completes constr symbol as a side effect
      ...
      tempInfo = denot.asClass.classInfo.integrateOpaqueMembers
        .asInstanceOf[TempClassInfo]
      denot.info = savedInfo
    end completeConstructor
    ...
    override def completeInCreationContext(denot: SymDenotation): Unit =
      ...
      def parentType(parent: untpd.Tree)(using Context): Type =
        ...
        def typedParentApplication(parent: untpd.Tree): Type =
          ...
          core match
            case Select(New(tpt), nme.CONSTRUCTOR) =>
              ...
              if ptype.typeParams.isEmpty then ptype
              else
                if denot.is(ModuleClass) && denot.sourceModule.isOneOf(
                    GivenOrImplicit
                  )
                then missingType(denot.symbol, "parent ")(using creationContext)
                fullyDefinedType(
                  typedAheadExpr(parent).tpe,
                  "class parent",
                  parent.srcPos
                )
            case _ =>
        ...
        def typedParentType(tree: untpd.Tree): tpd.Tree =
          ...
          then
            ...
            val app = untpd.Apply(
              untpd.Select(untpd.New(parentTpt), nme.CONSTRUCTOR),
              Nil
            )
      ...
      /** Check parent type tree `parent` for the following well-formedness
        * conditions: (1) It must be a class type with a stable prefix (@see
        * checkClassTypeWithStablePrefix) (2) If may not derive from itself (3)
        * The class is not final (4) If the class is sealed, it is defined in
        * the same compilation unit as the current class
        */
      def checkedParentType(parent: untpd.Tree): Type =
        val ptype = parentType(parent)(using
        ...
        if cls.isRefinementClass then ptype
          ...
          if pt.derivesFrom(cls) then
            ...
            else if pclazz.isEffectivelySealed && pclazz.associatedFile != cls.associatedFile
            then
      ...
      /** If `parents` contains references to traits that have supertraits with
        * implicit parameters add those supertraits in linearization order
        * unless they are already covered by other parent types. For instance,
        * in
        *
        * class A trait B(using I) extends A trait C extends B class D extends
        * A, C
        *
        ...
        *
        * so that an implicit `I` can be passed to `B`. See i7613.scala for more
        * examples.
        */
      ...
      val parentTypes = defn.adjustForTuple(
        cls,
        cls.typeParams,
        defn.adjustForBoxedUnit(
          cls,
          addUsingTraits(
      ...
      typr.println(
        i"completing $denot, parents = $parents%, %, parentTypes = $parentTypes%, %"
      )

      if impl.derived.nonEmpty then
        val (derivingClass, derivePos) =
          original.removeAttachment(desugar.DerivingCompanion) match
      ...
      if isDerivedValueClass(cls) then cls.setFlag(Final)
      cls.info = avoidPrivateLeaks(cls)
      cls.baseClasses.foreach(
        _.invalidateBaseTypeCache()
  ...
  /** Typecheck `tree` during completion using `typed`, and remember result in
    * TypedAhead map
    */
  def typedAhead(tree: Tree, typed: untpd.Tree => tpd.Tree)(using
      Context
  ): tpd.Tree =
    val xtree = expanded(tree)
    xtree.getAttachment(TypedAhead) match
  ...
  def typedAheadType(tree: Tree, pt: Type = WildcardType)(using
      Context
  ): tpd.Tree =
    typedAhead(tree, typer.typedType(_, pt))

  def typedAheadExpr(tree: Tree, pt: Type = WildcardType)(using
      Context
  ): tpd.Tree =
  ...
  def completeParams(params: List[MemberDef])(using Context): Unit =
    index(params)
    for param <- params do typedAheadExpr(param)

  /** The signature of a module valdef. This will compute the corresponding
    * module class TypeRef immediately without going through the defined type of
    * the ValDef. This is necessary to avoid cyclic references involving imports
    * and module val defs.
    */
  def moduleValSig(sym: Symbol)(using Context): Type =
    val clsName = sym.name.moduleClassName
    val cls = ctx.effectiveScope
      .lookupAll(clsName)
  ...
  /** The type signature of a ValDef or DefDef
    ...
    *   Its symbol
    * @param paramFn
    *   A wrapping function that produces the type of the defined symbol, given
  ...
  def valOrDefDefSig(
    ...
    lazy val termParamss = paramss.collect { case TermSymbols(vparams) =>
      vparams
    }
    ...
    val tptProto = mdef.tpt match
      ...
      case DependentTypeTree(tpFun) =>
        ...
        if isFullyDefined(tpe, ForceDegree.none) then tpe
        else typedAheadExpr(mdef.rhs, tpe).tpe
      case TypedSplice(tpt: TypeTree)
          if !isFullyDefined(tpt.tpe, ForceDegree.none) =>
        mdef match
          case mdef: DefDef if mdef.name == nme.ANON_FUN =>
            ...
            if !Config.checkLevelsOnConstraints then
              ...
              if !hygienicType.isValueType || !(hygienicType <:< tpt.tpe) then
                report.error(
      ...
      case _ =>
        WildcardType
    val mbrTpe = paramFn(
      checkSimpleKinded(typedAheadType(mdef.tpt, tptProto)).tpe
    )
    if ctx.explicitNulls && mdef.mods.is(JavaDefined) then
      JavaNullInterop.nullifyMember(
        sym,
        mbrTpe,
        mdef.mods.isAllOf(JavaEnumValue)
      )
    else mbrTpe
  end valOrDefDefSig

  /** The type signature of a DefDef with given symbol */
  def defDefSig(ddef: DefDef, sym: Symbol, completer: Namer#Completer)(using
      Context
  ): Type =
    ...
    if isConstructor then sym.owner.typeParams.foreach(_.ensureCompleted())
    ...
    if completedTypeParams.forall(_.isType) then
      completer.setCompletedTypeParams(
        completedTypeParams.asInstanceOf[List[TypeSymbol]]
      )
    ddef.trailingParamss.foreach(completeParams)
    val paramSymss = normalizeIfConstructor(
      ddef.paramss.nestedMap(symbolOfTree),
      isConstructor
    )
  ...
  def inferredResultType(
    ...
    /** A type for this definition that might be inherited from elsewhere: If
      * this is a setter parameter, the corresponding getter type. If this is a
      * class member, the conjunction of all result types of overridden methods.
      * NoType if neither case holds.
      */
    val inherited =
      if sym.owner.isTerm then NoType
      else
        ...
        if bcs.isEmpty then
          ...
          bcs.tail.foldLeft(NoType: Type) { (tp, cls) =>
            def instantiatedResType(
                info: Type,
                paramss: List[List[Symbol]]
            ): Type = info match
              case info: PolyType =>
                paramss match
                  case TypeSymbols(tparams) :: paramss1
                      if info.paramNames.length == tparams.length =>
                    instantiatedResType(
                      info.instantiate(tparams.map(_.typeRef)),
                      paramss1
                    )
                  case _ =>
                    NoType
              case info: MethodType =>
                paramss match
                  case TermSymbols(vparams) :: paramss1
                      if info.paramNames.length == vparams.length =>
                    instantiatedResType(
                      info.instantiate(vparams.map(_.termRef)),
                      paramss1
                    )
                  case _ =>
                    NoType
            ...
            val iRawInfo =
              cls.info
                ...
                .matchingDenotation(site, schema, sym.targetName)
                .info
            val iResType =
              instantiatedResType(iRawInfo, paramss).asSeenFrom(site, cls)
            if iResType.exists then
              typr.println(
                i"using inherited type for ${mdef.name}; raw: $iRawInfo, inherited: $iResType"
              )
            tp & iResType
          }
        end if
    ...
    /** If this is a default getter, the type of the corresponding method
      * parameter, otherwise NoType.
      */
    def defaultParamType = sym.name match
      case DefaultGetterName(original, idx) =>
        val meth: Denotation =
          if original.isConstructorName && (sym.owner.is(ModuleClass)) then
            sym.owner.companionClass.info.decl(nme.CONSTRUCTOR)
          else ctx.defContext(sym).denotNamed(original)
        def paramProto(paramss: List[List[Type]], idx: Int): Type =
          paramss match
            case params :: paramss1 =>
              if idx < params.length then params(idx)
              else paramProto(paramss1, idx - params.length)
            case nil =>
              NoType
        val defaultAlts = meth.altsWith(_.hasDefaultParams)
        if defaultAlts.length == 1 then
          paramProto(defaultAlts.head.info.widen.paramInfoss, idx)
        else NoType
      case _ =>
        NoType

    /** The expected type for a default argument. This is normally the
      * `defaultParamType` with references to internal parameters replaced by
      * wildcards. This replacement makes it possible that the default argument
      * can have a more specific type than the parameter. For instance, we allow
      *
    ...
    if sym.is(ExtensionMethod) then
      rhsCtx = rhsCtx.addMode(Mode.InExtensionMethod)
    val typeParams = paramss.collect { case TypeSymbols(tparams) =>
      tparams
    }.flatten
    ...
    def rhsType =
      ...
      val pt =
        inherited.orElse(expectedDefaultArgType).orElse(fallbackProto).widenExpr
      ...
      if (defaultTp eq pt) && (tp frozen_<:< defaultTp) then
        ...
        AnnotatedType(
          defaultTp,
          Annotation(defn.UncheckedVarianceAnnot, sym.span)
        )
      else
    ...
    def dealiasIfUnit(tp: Type) =
      if tp.isRef(defn.UnitClass) then defn.UnitType else tp
