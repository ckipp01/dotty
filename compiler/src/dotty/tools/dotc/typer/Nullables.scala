...
object Nullables:
  ...
  def importUnsafeNulls(using Context): Import = Import(
    ...
    List(
      untpd.ImportSelector(untpd.Ident(nme.unsafeNulls), EmptyTree, EmptyTree)
    )
  )
  ...
  def createNullableTypeBounds(lo: Type, hi: Type)(using Context): TypeBounds =
    val newHi =
      if needNullifyHi(lo, hi) then OrType(hi, defn.NullType, soft = false)
      else hi
    TypeBounds(lo, newHi)

  /** Create a nullable type bound tree If lo is `Null`, `| Null` is added to hi
    */
  def createNullableTypeBoundsTree(lo: Tree, hi: Tree, alias: Tree = EmptyTree)(
      using Context
  ): TypeBoundsTree =
    val hiTpe = hi.typeOpt
    val newHi =
      if needNullifyHi(lo.typeOpt, hiTpe) then
        TypeTree(OrType(hiTpe, defn.NullType, soft = false))
      else hi
    TypeBoundsTree(lo, newHi, alias)

  /** A set of val or var references that are known to be not null, plus a set
    * of variable references that are not known (anymore) to be not null
    */
  case class NotNullInfo(asserted: Set[TermRef], retracted: Set[TermRef]):
    ...
    /** The alternative path combination with another not-null info. Used to
      * merge the nullability info of the two branches of an if.
      */
  ...
  /** A pair of not-null sets, depending on whether a condition is `true` or
    * `false`
    */
  ...
  /** An attachment that represents conditional flow facts established by this
    * tree, which represents a condition.
    */
  private[typer] val NNConditional = Property.StickyKey[NotNullConditional]

  /** An attachment that represents unconditional flow facts established by this
    * tree.
    */
  ...
  object CompareNull:
    ...
    /** Matches one of
      *
      * tree == null, tree eq null, null == tree, null eq tree tree != null,
      * tree ne null, null != tree, null ne tree
      *
      * The second boolean result is true for equality tests, false for
      * inequality tests
      */
    ...
    private def testSym(sym: Symbol, operand: Tree)(using Context) =
      ...
      else if sym == defn.Any_!= || sym == defn.Object_ne then
        Some((operand, false))
      else None
  ...
  /** Is the given reference tracked for nullability?
    *
    * This is the case if one of the following holds: 1) The reference is a path
    * to an immutable `val`. 2) The reference is to a mutable variable, in which
    * case all assignments to it must be reachable (in the sense of how it is
    * defined in assignmentSpans) _and_ the variable must not be used "out of
    * order" (in the sense specified by `usedOutOfOrder`).
    *
    * Whether to track a local mutable variable during flow typing? We track a
    * local mutable variable iff the variable is not assigned in a closure. For
    * example, in the following code `x` is assigned to by the closure `y`, so
    * we do not do flow typing on `x`.
    *
    * ```scala
    * var x: String | Null = ???
    * def y =
    ...
    *   val a: String =
    *     x // error: x is captured and mutated by the closure, not trackable
    * ```
    *
    * Check `usedOutOfOrder` to see the explaination and example of "out of
    * order". See more examples in
  ...
  /** The nullability context to be used after a case that matches pattern
    * `pat`. If `pat` is `null`, this will assert that the selector `sel` is not
    * null afterwards.
    */
  def afterPatternContext(sel: Tree, pat: Tree)(using Context) =
    (sel, pat) match
      case (TrackedRef(ref), Literal(Constant(null))) =>
        ctx.addNotNullRefs(Set(ref))
      case _ => ctx
  ...
  extension (infos: List[NotNullInfo])
    ...
    /** Do the current not-null infos imply that `ref` is not null? Not-null
      * infos are as a history where earlier assertions and retractions replace
      * later ones (i.e. it records the assignment history in reverse, with most
      * recent first)
      */
    ...
    /** Add `info` as the most recent entry to the list of null infos.
      * Assertions or retractions in `info` supersede infos in existing entries
      * of `infos`.
      */
  ...
  extension (ref: TermRef)
    /** Is the use of a mutable variable out of order
      *
      * Whether to generate and use flow typing on a specific _use_ of a local
      * mutable variable? We only want to do flow typing on a use that belongs
      * to the same method as the definition of the local variable. For example,
      * in the following code, even `x` is not assigned to by a closure, but we
      * can only use flow typing in one of the occurrences (because the other
      * occurrence happens within a nested closure).
      * ```scala
      * var x: String | Null = ???
      * def y =
      *   if x != null then
    ...
    def usedOutOfOrder(using Context): Boolean =
      ...
      @tailrec def recur(s: Symbol): Boolean =
        s != NoSymbol
          ...
          && (s.isOneOf(
            MethodOrLazy
          ) // not at the rhs of lazy ValDef or in a method (or lambda)
            || s.isClass // not in a class
            || recur(s.owner))

      refSym.is(
        Mutable
      ) // if it is immutable, we don't need to check the rest conditions
      && refOwner.isTerm
      && recur(ctx.owner)
  end extension
  ...
  extension (tree: Tree)
    ...
    def notNullInfoIf(c: Boolean)(using Context): NotNullInfo =
      ...
      if cond.isEmpty then tree.notNullInfo
        ...
        tree.notNullInfo.seq(
          NotNullInfo(if c then cond.ifTrue else cond.ifFalse, Set())
        )

    /** The paths that are known to be not null if the condition represented by
      * `tree` yields `true` or `false`. Two empty sets if `tree` is not a
      * condition.
      */
    ...
    /** The current context augmented with nullability information, assuming the
      * result of the condition represented by `tree` is the same as the value
      * of `c`.
      */
    ...
    /** The context to use for the arguments of the function represented by
      * `tree`. This is the current context, augmented with nullability
      * information of the left argument, if the application is a boolean `&&`
      * or `||`.
      */
    ...
    /** The `tree` augmented with nullability information in an attachment. The
      * following operations lead to nullability info being recorded:
      *
      *   1. Null tests using `==`, `!=`, `eq`, `ne`, if the compared entity is
      *      a path (i.e. a stable TermRef) 2. Boolean &&, ||, !
      */
    def computeNullable()(using Context): tree.type =
      ...
      if !ctx.erasedTypes && analyzedOps.contains(tree.symbol.name.toTermName)
      then
        tree match
          ...
          case Select(x, _) if tree.symbol == defn.Boolean_! =>
            val xc = x.notNullConditional
            if !xc.isEmpty then setConditional(xc.ifFalse, xc.ifTrue)
          case _ =>
      tree
  ...
  private val analyzedOps =
    Set(nme.EQ, nme.NE, nme.eq, nme.ne, nme.ZAND, nme.ZOR, nme.UNARY_!)
  ...
  /** A map from (name-) offsets of all local variables in this compilation unit
    ...
    * to these variables. A variable can be tracked if it has only reachable
    * assignments An assignment is reachable if the path of tree nodes between
    * the block enclosing the variable declaration to the assignment consists
    * only of if-expressions, while-expressions, block-expressions and
    * type-ascriptions. Only reachable assignments are handled correctly in the
    * nullability analysis. Therefore, variables with unreachable assignments
    * can be assumed to be not-null only if their type asserts it.
    *
    * Note: we track the local variables through their offset and not through
    * their name because of shadowing.
    */
  def assignmentSpans(using Context): Map[Int, List[Span]] =
    ...
    object populate extends UntypedTreeTraverser:
      ...
      /** Map the names of potentially trackable candidate variables in scope to
        * the spans of their reachable assignments
        */
      ...
      def traverse(tree: Tree)(using Context) =
        ...
        tree match
            ...
            candidates.get(name) match
              case Some(spans) =>
                if reachable.contains(name) then
                  candidates(name) = tree.span :: spans
          ...
          case _: (If | WhileDo | Typed) =>
            traverseChildren(
              tree
            ) // assignments to candidate variables are OK here ...
          case _ =>
  ...
  /** The initial context to be used for a while expression with given span. In
    * this context, all variables that are assigned within the while expression
    * have their nullability status retracted, i.e. are not known to be not
    * null. While necessary for soundness, this scheme loses precision: Even if
    * the initial state of the variable is not null and all assignments to the
    * variable in the while expression are also known to be not null, the
    * variable is still assumed to be potentially null. The loss of precision is
    * unavoidable during normal typing, since we can only do a linear traversal
    * which does not allow a fixpoint computation. But it could be mitigated as
    * follows:
    *
    *   - initially, use `whileContext` as computed here
    *   - when typechecking the while, delay all errors due to a variable being
    *     potentially null
    *   - afterwards, if there are such delayed errors, run the analysis again
    *     with as a fixpoint computation, reporting all previously delayed
    *     errors that remain.
    *
    * The following code would produce an error in the current analysis, but not
    * in the refined analysis:
    *
    * class Links(val elem: T, val next: Links | Null)
    *
    * var xs: Links | Null = Links(1, null) var ys: Links | Null = xs while xs
    * != null ys = Links(xs.elem, ys.next) // error in unrefined: ys is
    * potentially null here xs = xs.next
    */
  def whileContext(whileSpan: Span)(using Context): Context =
    ...
    def isRetracted(ref: TermRef): Boolean =
      ...
      sym.span.exists
      && assignmentSpans
        .getOrElse(sym.span.start, Nil)
        .exists(whileSpan.contains(_))
      && ctx.notNullInfos.impliesNotNull(ref)

    val retractedVars =
      ctx.notNullInfos.flatMap(_.asserted.filter(isRetracted)).toSet
  ...
  /** Post process all arguments to by-name parameters by removing any not-null
    * info that was used when typing them. Concretely: If an argument
    * corresponds to a call-by-name parameter, drop all embedded not-null
    * assertions of the form `x.$asInstanceOf[x.type & T]` where `x` is a
    * reference to a mutable variable. If the argument still typechecks with the
    * removed assertions and is still compatible with the formal parameter, keep
    * it. Otherwise issue an error that the call-by-name argument was typed
    * using flow assumptions about mutable variables and suggest that it is
    * enclosed in a `byName(...)` call instead.
    */
  def postProcessByNameArgs(fn: TermRef, app: Tree)(using Context): Tree =
    fn.widen match
      case mt: MethodType
          if mt.paramInfos
            .exists(_.isInstanceOf[ExprType]) && !fn.symbol.is(Inline) =>
        app match
          case Apply(fn, args) =>
            ...
            object retyper extends ReTyper:
              override def typedUnadapted(
                  t: untpd.Tree,
                  pt: Type,
                  locked: TypeVars
              )(using Context): Tree = t match
                case t: untpd.ValDef if !t.symbol.is(Lazy) =>
                  super.typedUnadapted(t, pt, locked)
            ...
            def postProcess(formal: Type, arg: Tree): Tree =
              ...
              if !dropNotNull.dropped then arg
                ...
                if nestedCtx.reporter.hasErrors || !(arg2.tpe <:< formal) then
                  report.error(
                    em"""This argument was typed using flow assumptions about mutable variables
            ...
            def recur(formals: List[Type], args: List[Tree]): List[Tree] =
              (formals, args) match
