...
trait QuotesAndSplices:
  ...
  /** Translate `'{ e }` into `scala.quoted.Expr.apply(e)` and `'[T]` into
    * `scala.quoted.Type.apply[T]` while tracking the quotation level in the
    * context.
    */
  def typedQuote(tree: untpd.Quote, pt: Type)(using Context): Tree =
    ...
    if quotes.tpe.isInstanceOf[SearchFailureType] then
      report.error(
        missingArgMsg(quotes, defn.QuotesClass.typeRef, ""),
        ctx.source.atSpan(tree.span)
      )
    else if !quotes.tpe.isStable then
      report.error(
        em"Quotes require stable Quotes, but found non stable $quotes",
        quotes.srcPos
      )
    ...
    if ctx.mode.is(Mode.Pattern) then
      ...
      val exprQuoteTree =
        untpd.Apply(untpd.ref(defn.QuotedRuntime_exprQuote.termRef), tree.body)
      val quotedExpr = typedApply(exprQuoteTree, pt)(using quoteContext) match
        case Apply(TypeApply(fn, tpt :: Nil), quotedExpr :: Nil) =>
          untpd.Quote(quotedExpr, Nil).withBodyType(tpt.tpe)
      makeInlineable(
        quotedExpr.select(nme.apply).appliedTo(quotes).withSpan(tree.span)
      )
  end typedQuote
  ...
  /** Translate `${ t: Expr[T] }` into expression `t.splice` while tracking the
    * quotation level in the context
    */
  def typedSplice(tree: untpd.Splice, pt: Type)(using Context): Tree =
    ...
    if level == 0 then
      ...
      def markAsMacro(c: Context): Unit =
        if c.owner eq c.outer.owner then markAsMacro(c.outer)
        else if c.owner.isInlineMethod then c.owner.setFlag(Macro)
        else if !c.outer.owner.is(Package) then markAsMacro(c.outer)
        else
          assert(
            ctx.reporter.hasErrors
          ) // Did not find inline def to mark as macro
      markAsMacro(ctx)

    // TODO typecheck directly (without `exprSplice`)
    val internalSplice =
      untpd.Apply(untpd.ref(defn.QuotedRuntime_exprSplice.termRef), tree.expr)
    typedApply(internalSplice, pt)(using spliceContext).withSpan(
      tree.span
    ) match
      case tree @ Apply(TypeApply(_, tpt :: Nil), spliced :: Nil)
          if tree.symbol == defn.QuotedRuntime_exprSplice =>
        cpy.Splice(tree)(spliced)
      case tree => tree
  end typedSplice

  def typedSplicePattern(tree: untpd.SplicePattern, pt: Type)(using
      Context
  ): Tree =
    record("typedSplicePattern")
    if isFullyDefined(pt, ForceDegree.flipBottom) then
      def patternOuterContext(ctx: Context): Context =
        if ctx.mode.is(Mode.QuotedPattern) then patternOuterContext(ctx.outer)
        else ctx
      ...
      for arg <- typedArgs if arg.symbol.is(Mutable)
      do // TODO support these patterns. Possibly using scala.quoted.util.Var
        report.error(
          "References to `var`s cannot be used in higher-order pattern",
          arg.srcPos
        )
      val argTypes = typedArgs.map(_.tpe.widenTermRefExpr)
      val patType =
        if tree.args.isEmpty then pt else defn.FunctionOf(argTypes, pt)
      val pat = typedPattern(
        tree.body,
        defn.QuotedExprClass.typeRef.appliedTo(patType)
      )(using
        spliceContext
          .retractMode(Mode.QuotedPattern)
          .addMode(Mode.Pattern)
          .withOwner(patternOuterContext(ctx).owner)
      )
      val baseType = pat.tpe.baseType(defn.QuotedExprClass)
      val argType =
        if baseType.exists then baseType.argTypesHi.head else defn.NothingType
      untpd.cpy.SplicePattern(tree)(pat, typedArgs).withType(pt)
    else
      errorTree(
        tree,
        em"Type must be fully defined.\nConsider annotating the splice using a type ascription:\n  ($tree: XYZ).",
        tree.body.srcPos
      )
    end if
  end typedSplicePattern
  ...
  /** Types a splice applied to some arguments `$f(arg1, ..., argn)` in a quote
    * pattern.
    *
    * The tree is desugared into `$f.apply(arg1, ..., argn)` where the
    * expression `$f` is expected to type as a function type `(T1, ..., Tn) =>
    * R`. `Ti` is the type of the argument `argi` and R if the type of the
    * prototype. The prototype must be fully defined to be able to infer the
    * type of `R`.
    */
  ...
  /** Type a pattern variable name `t` in quote pattern as `${given t$giveni:
    * Type[t @ _]}`. The resulting pattern is the split in `splitQuotePattern`.
    */
  def typedQuotedTypeVar(tree: untpd.Ident, pt: Type)(using Context): Tree =
    def spliceOwner(ctx: Context): Symbol =
      if ctx.mode.is(Mode.QuotedPattern) then spliceOwner(ctx.outer)
      else ctx.owner
    ...
    typeSym.addAnnotation(
      Annotation(
        New(ref(defn.QuotedRuntimePatterns_patternTypeAnnot.typeRef))
          .withSpan(tree.span)
      )
    )
    val pat = typedPattern(
      expr,
      defn.QuotedTypeClass.typeRef.appliedTo(typeSym.typeRef)
    )(using
      spliceContext.retractMode(Mode.QuotedPattern).withOwner(spliceOwner(ctx))
    )
    pat.select(tpnme.Underlying)
  end typedQuotedTypeVar

  private def checkSpliceOutsideQuote(tree: untpd.Tree)(using Context): Unit =
    if level == 0 && !ctx.owner.ownersIterator.exists(_.isInlineMethod) then
      report.error(
  ...
  /** Split a typed quoted pattern is split into its type bindings, pattern
    * expression and inner patterns. Type definitions with `@patternType` will
    * be inserted in the pattern expression for each type binding.
    *
    * A quote pattern
    * ```
    ...
    * ```
    */
  private def splitQuotePattern(quoted: Tree)(using
      Context
  ): (Map[Symbol, Bind], Tree, List[Tree]) =
    val ctx0 = ctx

    val typeBindings: collection.mutable.Map[Symbol, Bind] =
      collection.mutable.Map.empty
    def getBinding(sym: Symbol): Bind =
      typeBindings.getOrElseUpdate(
        sym, {
          val bindingBounds = sym.info
          val bsym = newPatternBoundSymbol(
            sym.name.toString.stripPrefix("$").toTypeName,
            bindingBounds,
            quoted.span
          )
          Bind(bsym, untpd.Ident(nme.WILDCARD).withType(bindingBounds))
            .withSpan(quoted.span)
        }
      )
    ...
    object splitter extends tpd.TreeMap:
      ...
      override def transform(tree: Tree)(using Context) = tree match
        case Typed(splice @ SplicePattern(pat, Nil), tpt)
            if !tpt.tpe.derivesFrom(defn.RepeatedParamClass) =>
        ...
        case SplicePattern(pat, args) =>
          ...
          val pat1 = if patType eq patType1 then pat else pat.withType(patType1)
          patBuf += pat1
          if args.isEmpty then
            ref(defn.QuotedRuntimePatterns_patternHole.termRef)
        ...
        case Select(pat: Bind, _) if tree.symbol.isTypeSplice =>
          ...
          if sym.exists then
            ...
            freshTypeBindingsBuff += transformTypeBindingTypeDef(
              nameOfSyntheticGiven,
              tdef,
              freshTypePatBuf
            )
        ...
        case tdef: TypeDef =>
          if tdef.symbol.hasAnnotation(
              defn.QuotedRuntimePatterns_patternTypeAnnot
            )
          then
            transformTypeBindingTypeDef(
              PatMatGivenVarName.fresh(tdef.name.toTermName),
              tdef,
              typePatBuf
            )
            ...
            report.error(
              em"Implementation restriction: cannot match $kind",
              tree.srcPos
            )
            EmptyTree
        ...
        case tree @ AppliedTypeTree(tpt, args) =>
          val args1: List[Tree] =
            args.zipWithConserve(tpt.tpe.typeParams.map(_.paramVarianceSign)) {
              (arg, v) =>
                arg.tpe match
        ...
        case tree: NamedDefTree =>
          ...
          if tree.name.isTermName && !tree.nameSpan.isSynthetic && tree.name
              .startsWith("$")
          then
            report.error(
              "Names cannot start with $ quote pattern ",
              tree.namePos
            )
          super.transform(tree)
        case _: Match =>
          report.error(
            "Implementation restriction: cannot match `match` expressions",
            tree.srcPos
          )
          EmptyTree
        case _: Try =>
          report.error(
            "Implementation restriction: cannot match `try` expressions",
            tree.srcPos
          )
          EmptyTree
        case _: Return =>
          report.error(
            "Implementation restriction: cannot match `return` statements",
            tree.srcPos
          )
          EmptyTree
        case _ =>
      ...
      private def transformTypeBindingTypeDef(
          nameOfSyntheticGiven: TermName,
          tdef: TypeDef,
          buff: mutable.Builder[Tree, List[Tree]]
      )(using Context): Tree =
        if ctx.mode.is(Mode.InPatternAlternative) then
          report.error(
            IllegalVariableInPatternAlternative(tdef.symbol.name),
            tdef.srcPos
          )
        if variance == -1 then
          tdef.symbol.addAnnotation(
            Annotation(
              New(ref(defn.QuotedRuntimePatterns_fromAboveAnnot.typeRef))
                .withSpan(tdef.span)
            )
          )
        val bindingType = getBinding(tdef.symbol).symbol.typeRef
        val bindingTypeTpe =
          AppliedType(defn.QuotedTypeClass.typeRef, bindingType :: Nil)
        val sym = newPatternBoundSymbol(
          nameOfSyntheticGiven,
          bindingTypeTpe,
          tdef.span,
          flags = ImplicitVal
        )(using ctx0)
        buff += Bind(sym, untpd.Ident(nme.WILDCARD).withType(bindingTypeTpe))
          .withSpan(tdef.span)
        super.transform(tdef)
      end transformTypeBindingTypeDef
    end splitter
    val shape0 = splitter.transform(quoted)
    val patterns =
      (splitter.freshTypePatBuf.iterator ++ splitter.typePatBuf.iterator ++ splitter.patBuf.iterator).toList
        ...
        val typeMap = new TypeMap():
          def apply(tp: Type): Type = tp match
            case tp: TypeRef if tp.symbol.isTypeSplice =>
              val tp1 = tp.dealias
              if isFreshTypeBindings(tp1.typeSymbol) then tp1
              else tp
            case tp => mapOver(tp)
  ...
  /** Type a quote pattern `case '{ <quoted> } =>` qiven the a current
    * prototype. Typing the pattern will also transform it into a call to
    * `scala.internal.quoted.Expr.unapply`.
    *
    * Code directly inside the quote is typed as an expression using
    * Mode.QuotedPattern. Splices within the quotes become patterns again and
    * typed accordingly.
    *
    * ```
    * case '{ ($ls: List[t]) } =>
    ...
    * ```
    *
    * For each type splice we will create a new type binding in the pattern
    * match (`t @ _` in this case) and a corresponding type in the quoted
    * pattern as a hole (`@patternType type t` in this case). All these
    * generated types are inserted at the start of the quoted code.
    *
    ...
    * ```
    *
    * Then the pattern is _split_ into the expression contained in the pattern
    * replacing the splices by holes, and the patterns in the splices. All these
    * are recombined into a call to `Matcher.unapply`.
    *
    * ```
    ...
    * ```
    */
  private def typedQuotePattern(tree: untpd.Quote, pt: Type, quotes: Tree)(using
      Context
  ): Tree =
    val quoted = tree.body
    if quoted.isTerm && !pt.derivesFrom(defn.QuotedExprClass) then
      report.error(
        "Quote pattern can only match scrutinees of type scala.quoted.Expr",
        tree.srcPos
      )
    ...
    val exprPt = pt.baseType(
      if quoted.isType then defn.QuotedTypeClass else defn.QuotedExprClass
    )
    val quotedPt = exprPt.argInfos.headOption match
      case Some(argPt: ValueType) => argPt // excludes TypeBounds
      case _                      => defn.AnyType
    val quoted0 =
      desugar.quotedPattern(quoted, untpd.TypedSplice(TypeTree(quotedPt)))
    val quoteCtx =
    ...
    class ReplaceBindings extends TypeMap():
      override def apply(tp: Type): Type = tp match
        case tp: TypeRef =>
          val tp1 = if tp.symbol.isTypeSplice then tp.dealias else tp
          mapOver(typeBindings.get(tp1.typeSymbol).fold(tp)(_.symbol.typeRef))
        case tp => mapOver(tp)
    val replaceBindings = new ReplaceBindings
    val patType =
      defn.tupleType(splices.tpes.map(tpe => replaceBindings(tpe.widen)))

    val typeBindingsTuple =
      tpd.hkNestedPairsTypeTree(typeBindings.values.toList)
    ...
    val replaceBindingsInTree = new TreeMap:
      ...
      private val replaceBindingsInType = new ReplaceBindings:
        override def apply(tp: Type): Type = tp match
          case tp: TermRef =>
            bindMap.get(tp.termSymbol).fold[Type](tp)(_.typeRef)
      ...
      if splices.isEmpty then ref(defn.EmptyTupleModule.termRef)
        ...
        typed(
          untpd
            .Tuple(
              splices.map(x =>
                untpd.TypedSplice(replaceBindingsInTree.transform(x))
              )
            )
            .withSpan(quoted.span),
          patType
    ...
    val quoteClass =
      if quoted.isTerm then defn.QuotedExprClass else defn.QuotedTypeClass
    val quotedPattern =
      if quoted.isTerm then
        tpd.Quote(shape, Nil).select(nme.apply).appliedTo(quotes)
      else
        ref(defn.QuotedTypeModule_of.termRef)
          .appliedToTypeTree(shape)
          .appliedTo(quotes)

    val matchModule =
      if quoted.isTerm then defn.QuoteMatching_ExprMatch
      else defn.QuoteMatching_TypeMatch
    val unapplyFun = quotes
      .asInstance(defn.QuoteMatchingClass.typeRef)
      .select(matchModule)
      .select(nme.unapply)

    UnApply(
      fun = unapplyFun.appliedToTypeTrees(
        typeBindingsTuple :: TypeTree(patType) :: Nil
      ),
      implicits = quotedPattern :: Nil,
      patterns = splicePat :: Nil,
      proto =
        quoteClass.typeRef.appliedTo(replaceBindings(quoted1.tpe) & quotedPt)
    )
  end typedQuotePattern
