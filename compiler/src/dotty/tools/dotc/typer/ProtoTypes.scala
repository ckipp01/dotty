...
import core.*
import ast.*
import Contexts.*, Types.*, Denotations.*, Names.*, StdNames.*, NameOps.*,
  Symbols.*
...
object ProtoTypes:
  ...
  trait Compatibility:
    ...
    /** A type `tp` is compatible with a type `pt` if one of the following
      * holds:
      *   1. `tp` is a subtype of `pt` 2. `pt` is by name parameter type, and
      *      `tp` is compatible with its underlying type 3. there is an implicit
      *      conversion from `tp` to `pt`. 4. `tp` is a numeric subtype of `pt`
      *      (this case applies even if implicit conversions are disabled) If
      *      `pt` is a by-name type, we compare against the underlying type
      *      instead.
      */
    def isCompatible(tp: Type, pt: Type)(using Context): Boolean =
      (tp.widenExpr relaxed_<:< pt.widenExpr) || viewExists(tp, pt)

    /** Like normalize and then isCompatible, but using a subtype comparison
      * with necessary eithers that does not unnecessarily truncate the
      * constraint space, returning false instead.
      */
    ...
    /** Test compatibility after normalization. If `keepConstraint` is false,
      * the current constraint set will not be modified by this call.
      */
    def normalizedCompatible(tp: Type, pt: Type, keepConstraint: Boolean)(using
        Context
    ): Boolean =

      def testCompat(using Context): Boolean =
        val normTp = normalize(tp, pt)
        isCompatible(normTp, pt) || pt.isRef(
          defn.UnitClass
        ) && normTp.isParameterless
      ...
      if keepConstraint then
        tp.widenSingleton match
          case poly: PolyType =>
            ...
            typr.println(i"""normalizedCompatible for $poly, $pt = $result
                   |constraint was: ${ctx.typerState.constraint}
                   |constraint now: ${newctx.typerState.constraint}""")
            if result && (ctx.typerState.constraint ne newctx.typerState.constraint)
    ...
    /** Check that the result type of the current method fits the given expected
      * result type.
      */
          ...
          mt match
            case mt: MethodType =>
              ...
              && {
                ...
                then
                  pt.args
                    .lazyZip(mt.paramInfos)
                    .forall((arg, paramInfo) =>
    ...
    /** Constrain result with special case if `meth` is an inlineable method in
      * an inlineable context. In that case, we should always succeed and not
      * constrain type parameters in the expected type, because the actual
      * return type can be a subtype of the currently known return type.
      * However, we should constrain parameters of the declared return type.
      * This distinction is achieved by replacing expected type parameters with
      * wildcards.
      */
    def constrainResult(meth: Symbol, mt: Type, pt: Type)(using
        Context
    ): Boolean =
  ...
  trait MatchAlways extends ProtoType:
    def isMatchedBy(tp1: Type, keepConstraint: Boolean)(using
        Context
    ): Boolean = true
  ...
  /** A class marking ignored prototypes that can be revealed by `deepenProto`
    */
  abstract case class IgnoredProto(ignored: Type)
    ...
    override def deepenProto(using Context): Type =
      myWasDeepened = true
      ignored
    override def deepenProtoTrans(using Context): Type =
      ignored.deepenProtoTrans

    /** Did someone look inside via deepenProto? Used for error deagniostics to
      * give a more extensive expected type.
      */
  ...
  abstract case class SelectionProto(
      ...
      with ProtoType
      with ValueTypeOrProto:

    /** Is the set of members of this type unknown, in the sense that we cannot
      * compute a non-trivial upper approximation? This is the case if:
      *   1. The type has Nothing or Wildcard as a prefix or underlying type 2.
      *      The type is an abstract type with a lower bound that has a unknown
      *      members and an upper bound that is both provisional and has unknown
      *      members. 3. The type is an uninstiated type var with a lower that
      *      has unknown members. 4. Type proxies have unknown members if their
      *      super types do
      */
    private def hasUnknownMembers(tp: Type)(using Context): Boolean = tp match
      ...
      case tp: AppliedType =>
        hasUnknownMembers(tp.tycon) || hasUnknownMembers(tp.superType)
    ...
    override def isMatchedBy(tp1: Type, keepConstraint: Boolean)(using
        Context
    ): Boolean =
      name == nme.WILDCARD
        ...
        || {
          try
            val mbr =
              if privateOK then tp1.member(name) else tp1.nonPrivateMember(name)
            def qualifies(m: SingleDenotation) =
              ...
              && (memberProto.isRef(defn.UnitClass)
                || tp1.isValueType && compat.normalizedCompatible(
                  NamedType(tp1, name, m),
                  memberProto,
                  keepConstraint
                ))
    ...
    def derivedSelectionProto(
        name: Name,
        memberProto: Type,
        compat: Compatibility
    )(using Context): SelectionProto =
      if (name eq this.name) && (memberProto eq this.memberProto) && (compat eq this.compat)
      then this
    ...
    def map(tm: TypeMap)(using Context): SelectionProto =
      derivedSelectionProto(name, tm(memberProto), compat)
    ...
    override def computeHash(bs: Hashable.Binders): Int =
      val delta =
        (if compat eq NoViewsAllowed then 1 else 0) | (if privateOK then 2
                                                       else 0)
      addDelta(doHash(bs, name, memberProto), delta)

    override def equals(that: Any): Boolean = that match
      case that: SelectionProto =>
        (name eq that.name) && memberProto.equals(
          that.memberProto
        ) && (compat eq that.compat) && (privateOK == that.privateOK)
      case _ =>
        false
  ...
  object SelectionProto:
    def apply(
      ...
      val selproto =
        new CachedSelectionProto(name, memberProto, compat, privateOK)
      if compat eq NoViewsAllowed then unique(selproto) else selproto

  /** Create a selection proto-type, but only one level deep; treat constructors
    * specially
    */
  def shallowSelectionProto(name: Name, tp: Type, typer: Typer)(using
      Context
  ): TermType =
    if name.isConstructorName then WildcardType
    else
      tp match
        case tp: UnapplyFunProto => new UnapplySelectionProto(name)
        case tp =>
          SelectionProto(name, IgnoredProto(tp), typer, privateOK = true)

  /** A prototype for expressions [] that are in some unspecified selection
    * operation
    *
    * [].?: ?
    *
    * Used to indicate that expression is in a context where the only valid
    * operation is further selection. In this case, the expression need not be a
    * value.
    * @see
    *   checkValue
    */
  @sharable object AnySelectionProto
      extends SelectionProto(nme.WILDCARD, WildcardType, NoViewsAllowed, true)

  @sharable object SingletonTypeProto
      extends SelectionProto(nme.WILDCARD, WildcardType, NoViewsAllowed, true)

  /** A prototype for selections in pattern constructors */
  class UnapplySelectionProto(name: Name)
      extends SelectionProto(name, WildcardType, NoViewsAllowed, true)

  trait ApplyingProto
      extends ProtoType // common trait of ViewProto and FunProto
  trait FunOrPolyProto
      extends ProtoType: // common trait of PolyProto and FunProto
  ...
  class FunProtoState:
    ...
    /** A map in which typed arguments can be stored to be later integrated in
      * `typedArgs`.
      */
    ...
    /** The tupled or untupled version of this prototype, if it has been
      * computed
      */
  ...
  /** A prototype for expressions that appear in function position
    ...
    *   A flag to indicate that constrainResult on this prototype should
    *   typecheck and compare the arguments.
    */
  case class FunProto(args: List[untpd.Tree], resType: Type)(
    ...
    def isMatchedBy(tp: Type, keepConstraint: Boolean)(using Context): Boolean =
      ...
      typer.isApplicableType(
        tp,
        args,
        resultType,
        keepConstraint && !args.exists(isPoly)
      )
    ...
    def derivedFunProto(
      ...
      then this
        ...
        new FunProto(args, resultType)(
          typer,
          applyKind,
          constrainResultDeep = constrainResultDeep
        )

    /** @return
      *   True if all arguments have types.
      */
    ...
    private def isUndefined(tp: Type): Boolean = tp match
      ...
      case defn.FunctionOf(args, result, _) =>
        args.exists(isUndefined) || isUndefined(result)
      case _ => false

    /** Did an argument produce an error when typing? This means: an error was
      * reported and a tree got an error type. Errors of adaptation whree a tree
      * has a good type but that type does not conform to the expected type are
      * not counted.
      */
    def hasErrorArg = !state.errorArgs.isEmpty

    /** Does tree have embedded error trees that are not at the outside. A
      * nested tree t1 is "at the outside" relative to a tree t2 if
    ...
    def hasInnerErrors(t: Tree)(using Context): Boolean = t match
      ...
      case _ =>
        t.existsSubTree { t1 =>
          if t1.typeOpt.isError && t1.span.toSynthetic != t.span.toSynthetic
          then
            typr.println(
              i"error subtree $t1 of $t with ${t1.typeOpt}, spans = ${t1.span}, ${t.span}"
            )
            true
          else false
        }

    private def cacheTypedArg(
        arg: untpd.Tree,
        typerFn: untpd.Tree => Tree,
        force: Boolean
    )(using Context): Tree =
        ...
        untpd.functionWithUnknownParamType(arg) match
          case Some(untpd.Function(args, _)) if !force =>
            ...
            val paramTypes = args map {
              case ValDef(_, tpt, _) if !tpt.isEmpty =>
                typer.typedType(tpt).typeOpt
              case _ => WildcardType
            }
          ...
          case _ =>
            ...
            if ctx.reporter.hasUnreportedErrors then
              if hasInnerErrors(targ.nn) then state.errorArgs += arg
            else
    ...
    /** The typed arguments. This takes any arguments already typed using
      ...
      *
      * Arguments are typechecked in the typerState where the FunProto was
      * created. However, any constraint changes are also propagated to the
      * currently passed context.
      *
      * @param norm
      *   a normalization function that is applied to an untyped argument tree
      *   before it is typed. The second Int parameter is the parameter index.
      */
    def typedArgs(
        norm: (untpd.Tree, Int) => untpd.Tree = sameTree
      ...
      if state.typedArgs.size == args.length then state.typedArgs
        ...
        inContext(protoCtx.withUncommittedTyperState) {
          ...
          val args1 = args.mapWithIndexConserve((arg, idx) =>
            cacheTypedArg(
              arg,
              arg => typer.typed(norm(arg, idx)),
              force = false
            )
          )
          val newConstraint = protoTyperState.constraint

          if !args1.exists(arg => isUndefined(arg.tpe)) then
            state.typedArgs = args1
          ...
          if (passedTyperState ne protoTyperState) && (oldConstraint ne newConstraint)
          then
            ...
            if protoTyperState.isCommittable then
              ...
              val newLambdas = newConstraint.domainLambdas.filter(tl =>
                !passedConstraint.contains(tl) || passedConstraint
                  .hasConflictingTypeVarsFor(tl, newConstraint)
              )
              val newTvars = newLambdas
                .flatMap(_.paramRefs)
                .map(newConstraint.typeVarOfParam)

              args1.foreach(arg =>
                Inferencing.instantiateSelected(arg.tpe, newTvars)
              )
            ...
            passedTyperState.mergeConstraintWith(protoTyperState)(using
              passedCtx
            )
          end if
          args1
    ...
    /** The type of the argument `arg`, or `NoType` if `arg` has not been typed
      * before or if `arg`'s typing produced a type error.
      */
    def typeOfArg(arg: untpd.Tree)(using Context): Type =
      val t = state.typedArg(arg)
      if t == null then NoType else t.tpe
    ...
    /** Cancel the application of this prototype. This can happen for a nullary
      * application `f()` if `f` refers to a symbol that exists both in
      * parameterless form `def f` and nullary method form `def f()`. A common
      * example for such a method is `toString`. If in that case the type in the
      * denotation is parameterless, we compensate by dropping the application.
      */
    ...
    override def toString: String =
      s"FunProto(${args mkString ","} => $resultType)"
    ...
    override def deepenProtoTrans(using Context): FunProto =
      derivedFunProto(
        args,
        resultType.deepenProtoTrans,
        constrainResultDeep = true
      )
  ...
  /** A prototype for expressions that appear in function position
    ...
    */
  class FunProtoTyped(args: List[tpd.Tree], resultType: Type)(
      typer: Typer,
      applyKind: ApplyKind
  )(using Context)
      extends FunProto(args, resultType)(typer, applyKind):
    override def typedArgs(norm: (untpd.Tree, Int) => untpd.Tree)(using
        Context
    ): List[tpd.Tree] = args
    override def typedArg(arg: untpd.Tree, formal: Type)(using
        Context
    ): tpd.Tree = arg.asInstanceOf[tpd.Tree]
  ...
  abstract case class ViewProto(argType: Type, resType: Type)
    ...
    def derivedViewProto(argType: Type, resultType: Type)(using
        Context
    ): ViewProto =
      if (argType eq this.argType) && (resultType eq this.resultType) then this
    ...
    def map(tm: TypeMap)(using Context): ViewProto =
      derivedViewProto(tm(argType), tm(resultType))
  ...
  class CachedViewProto(argType: Type, resultType: Type)
      extends ViewProto(argType, resultType):
    override def computeHash(bs: Hashable.Binders): Int =
      doHash(bs, argType, resultType)
    override def eql(that: Type): Boolean = that match
      case that: ViewProto =>
        (argType eq that.argType) && (resType eq that.resType)
      case _ => false
    // equals comes from case class; no need to redefine
  ...
  class UnapplyFunProto(argType: Type, typer: Typer)(using Context)
      extends FunProto(
        untpd.TypedSplice(dummyTreeOfType(argType)(ctx.source)) :: Nil,
        WildcardType
      )(typer, applyKind = ApplyKind.Regular)

  /** A prototype for expressions [] that are type-parameterized:
    *
    * [] [targs] resultType
    */
  case class PolyProto(targs: List[Tree], resType: Type)
      extends UncachedGroundType
      with FunOrPolyProto:
    ...
    override def isMatchedBy(tp: Type, keepConstraint: Boolean)(using
        Context
    ): Boolean =
      canInstantiate(tp) || tp
        .member(nme.apply)
        .hasAltWith(d => canInstantiate(d.info))

    def derivedPolyProto(targs: List[Tree], resType: Type): PolyProto =
      if (targs eq this.targs) && (resType eq this.resType) then this
  ...
  /** A prototype for type constructors that are followed by a type application
    */
  @sharable object AnyTypeConstructorProto
      extends UncachedGroundType
      with MatchAlways:
  ...
  extension (pt: Type)
    def isExtensionApplyProto: Boolean = pt match
      ...
      case FunProto((arg: untpd.TypedSplice) :: Nil, _) =>
        arg.isExtensionReceiver
      case _ => false
  ...
  /** Add all parameters of given type lambda `tl` to the constraint's domain.
    ...
    * Also, if `owningTree` is non-empty or `alwaysAddTypeVars` is true, add a
    * type variable for each parameter.
    * @return
    *   The added type lambda, and the list of created type variables.
    */
  def constrained(using Context)(
      tl: TypeLambda,
      owningTree: untpd.Tree,
    ...
    if tl.isInstanceOf[PolyType] then
      assert(
        !ctx.typerState.isCommittable || addTypeVars,
  ...
  def constrained(tl: TypeLambda, owningTree: untpd.Tree)(using
      Context
  ): (TypeLambda, List[TypeTree]) =
    constrained(
      tl,
      owningTree,
      alwaysAddTypeVars =
        tl.isInstanceOf[PolyType] && ctx.typerState.isCommittable
    )

  /** Same as `constrained(tl, EmptyTree)`, but returns just the created type
    * lambda
    */
  ...
  /** A fresh type variable added to the current constraint.
    ...
    * @param represents
    *   If it exists, a ParamRef that this TypeVar represents, to be retrieved
    *   using `representedParamRef`. in the substitution generated by
    *   `resultTypeApprox` If `represents` exists, it is stored in the result
    *   type of the PolyType that backs the TypeVar, to be retrieved by
    *   `representedParamRef`.
    */
  def newTypeVar(using Context)(
      bounds: TypeBounds,
      name: TypeName = DepParamName.fresh().toTypeName,
      nestingLevel: Int = ctx.nestingLevel,
      represents: Type = NoType
  ): TypeVar =
    ...
    constrained(
      poly,
      untpd.EmptyTree,
      alwaysAddTypeVars = true,
      nestingLevel
    )._2.head.tpe.asInstanceOf[TypeVar]

  /** If `param` was created using `newTypeVar(..., represents = X)`, returns X.
    * This is used in:
    *   - `Inferencing#constrainIfDependentParamRef` to retrieve the dependent
    *     function parameter for which the variable was substituted.
    *   - `ConstraintHandling#LevelAvoidMap#legalVar` to retrieve the type
    *     variable that was avoided in a previous call to `legalVar`.
    */
  ...
  /** Create a new TypeVar that represents a dependent method parameter
    * singleton `ref`
    */
  def newDepTypeVar(ref: TermParamRef)(using Context): TypeVar =
    newTypeVar(
      TypeBounds.upper(
        AndType(ref.underlying.widenExpr, defn.SingletonClass.typeRef)
      ),
      represents = ref
    )
  ...
  /** The result type of `mt`, where all references to parameters of `mt` are
    ...
    */
  def resultTypeApprox(mt: MethodType, wildcardOnly: Boolean = false)(using
      Context
  ): Type =
    if mt.isResultDependent then
      def replacement(ref: TermParamRef) =
        ...
        then
          WildcardType(
            ref.underlying
              .substParams(mt, mt.paramRefs.map(_ => WildcardType))
              .toBounds
          )
  ...
  /** The normalized form of a type
    *   - instantiate polymorphic types with fresh type variables in the current
    *     constraint
    *   - skips implicit parameters of methods and functions; if result type
    *     depends on implicit parameter, replace with wildcard.
    *   - converts non-dependent method types to the corresponding function
    *     types unless the expected type is an ApplyingProto or IgnoredProto.
    *   - dereferences parameterless method types
    *   - dereferences nullary method types provided the corresponding function
    *     type is not a subtype of the expected type. Note: We need to take
    *     account of the possibility of inserting a () argument list in
    *     normalization. Otherwise, a type with a def toString(): String member
    *     would not count as a valid solution for ?{toString: String}. This
    *     would then lead to an implicit insertion, with a nice explosion of
    *     inference search because of course every implicit result has some sort
    *     of toString method. The problem is solved by dereferencing nullary
    *     method types if the corresponding function type is not compatible with
    *     the prototype.
    */
  def normalize(tp: Type, pt: Type, followIFT: Boolean = true)(using
      Context
  ): Type =
    ...
    tp.widenSingleton match
        ...
        if mt.isImplicitMethod then
          ...
          pt match
            ...
            case pt: ApplyingProto =>
              if rt eq mt.resultType then tp
      ...
      case wtp =>
        ...
        if iftp.exists && followIFT then
          normalize(iftp.functionArgInfos.last, pt)
        else tp
  end normalize

  /** Approximate occurrences of parameter types and uninstantiated typevars by
    * wildcard types.
    */
  private def wildApprox(
      tp: Type,
      theMap: WildApproxMap | Null,
      seen: Set[TypeParamRef],
      internal: Set[TypeLambda]
  )(using Context): Type =
    tp match
      case tp: NamedType => // default case, inlined for speed
        val isPatternBoundTypeRef =
          tp.isInstanceOf[TypeRef] && tp.symbol.isPatternBound
        if isPatternBoundTypeRef then WildcardType(tp.underlying.bounds)
        else if tp.symbol.isStatic || (tp.prefix `eq` NoPrefix) then tp
      ...
      case tp @ AppliedType(tycon, args) =>
        ...
        wildApprox(tycon, theMap, seen, internal) match
          case WildcardType(TypeBounds(lo, hi))
              if hi.typeParams.hasSameLengthAs(args) =>
            val args1 = wildArgs
            val lo1 =
              if lo.typeParams.hasSameLengthAs(args) then lo.appliedTo(args1)
              else lo
      ...
      case tp @ TypeParamRef(poly, pnum) =>
        def wildApproxBounds(bounds: TypeBounds) =
          if seen.contains(tp) then WildcardType
          else
            WildcardType(wildApprox(bounds, theMap, seen + tp, internal).bounds)
        def unconstrainedApprox = wildApproxBounds(poly.paramInfos(pnum))
        def approxPoly =
          if ctx.mode.is(Mode.TypevarsMissContext) then unconstrainedApprox
          else
      ...
      case TermParamRef(mt, pnum) =>
        WildcardType(
          TypeBounds.upper(
            wildApprox(mt.paramInfos(pnum), theMap, seen, internal)
          )
        )
      case tp: TypeVar =>
      ...
      case tp: AndType =>
        def approxAnd =
          ...
          def wildBounds(tp: Type) =
            if tp.isInstanceOf[WildcardType] then tp.bounds
            else TypeBounds.upper(tp)
          if tp1a.isInstanceOf[WildcardType] || tp2a.isInstanceOf[WildcardType]
          then WildcardType(wildBounds(tp1a) & wildBounds(tp2a))
          else tp.derivedAndType(tp1a, tp2a)
        approxAnd
      case tp: OrType =>
        def approxOr =
          val tp1a = wildApprox(tp.tp1, theMap, seen, internal)
          val tp2a = wildApprox(tp.tp2, theMap, seen, internal)
          if tp1a.isInstanceOf[WildcardType] || tp2a.isInstanceOf[WildcardType]
          then WildcardType(tp1a.bounds | tp2a.bounds)
          else tp.derivedOrType(tp1a, tp2a)
        approxOr
      case tp: SelectionProto =>
        tp.derivedSelectionProto(
          tp.name,
          wildApprox(tp.memberProto, theMap, seen, internal),
          NoViewsAllowed
        )
      case tp: ViewProto =>
        tp.derivedViewProto(
      ...
      case tp: FunProto =>
        ...
        if (args eq tp.args) && (resTp eq tp.resultType) then tp
        else FunProtoTyped(args, resTp)(ctx.typer, tp.applyKind)
      ...
      case tl: TypeLambda =>
        ...
        tl.derivedLambdaType(
          paramInfos = tl.paramInfos.mapConserve(
            wildApprox(_, theMap, seen, internal1).bounds
          ),
          resType = wildApprox(tl.resType, theMap, seen, internal1)
        )
      case _ =>
        (if theMap != null && seen.eq(theMap.seen) then theMap
         else new WildApproxMap(seen, internal))
          .mapOver(tp)

  final def wildApprox(tp: Type)(using Context): Type =
    wildApprox(tp, null, Set.empty, Set.empty)

  @sharable object AssignProto extends UncachedGroundType with MatchAlways

  private[ProtoTypes] class WildApproxMap(
      val seen: Set[TypeParamRef],
      val internal: Set[TypeLambda]
  )(using Context)
      extends TypeMap:
