...
object Dynamic:
  ...
  /** Is `tree` a reference over `Dynamic` that should be expanded to a dyanmic
    * `applyDynamic`, `selectDynamic`, `updateDynamic`, or `applyDynamicNamed`
    * call?
    */
...
object DynamicUnapply:
  def unapply(tree: tpd.Tree): Option[List[tpd.Tree]] = tree match
    ...
    case Apply(
          Apply(
            Select(selectable, fname),
            Literal(Constant(name)) :: ctag :: Nil
          ),
          _ :: implicits
        )
...
/** Handles programmable member selections of `Dynamic` instances and values
  ...
  *
  *   1. Translates selection that does not typecheck according to the
  *      scala.Dynamic rules: foo.bar(baz) = quux ~~>
  *      foo.selectDynamic(bar).update(baz, quux) foo.bar = baz ~~>
  *      foo.updateDynamic("bar")(baz) foo.bar(x = bazX, y = bazY, baz, ...) ~~>
  *      foo.applyDynamicNamed("bar")(("x", bazX), ("y", bazY), ("", baz), ...)
  *      foo.bar(baz0, baz1, ...) ~~> foo.applyDynamic(bar)(baz0, baz1, ...)
  *      foo.bar ~~> foo.selectDynamic(bar)
  *
  * The first matching rule of is applied.
  *
  * 2. Translates member selections on structural types to calls of
  * `selectDynamic` or `applyDynamic` on a `Selectable` instance. @See
  * handleStructural.
  */
trait Dynamic:
  ...
  /** Translate selection that does not typecheck according to the normal rules
    * into a applyDynamic/applyDynamicNamed. foo.bar(baz0, baz1, ...) ~~>
    * foo.applyDynamic(bar)(baz0, baz1, ...) foo.bar[T0, ...](baz0, baz1, ...)
    * ~~> foo.applyDynamic[T0, ...](bar)(baz0, baz1, ...) foo.bar(x = bazX, y =
    * bazY, baz, ...) ~~> foo.applyDynamicNamed("bar")(("x", bazX), ("y", bazY),
    * ("", baz), ...) foo.bar[T0, ...](x = bazX, y = bazY, baz, ...) ~~>
    * foo.applyDynamicNamed[T0, ...]("bar")(("x", bazX), ("y", bazY), ("", baz),
    * ...)
    */
  def typedDynamicApply(tree: untpd.Apply, isInsertedApply: Boolean, pt: Type)(
      using Context
  ): Tree =
    def typedDynamicApply(
        qual: untpd.Tree,
        name: Name,
        selSpan: Span,
        targs: List[untpd.Tree]
    ): Tree =
      def isNamedArg(arg: untpd.Tree): Boolean = arg match
        case NamedArg(_, _) => true; case _ => false
      val args = tree.args
      val dynName =
        if args.exists(isNamedArg) then nme.applyDynamicNamed
      ...
      then
        ...
        val args1 = if dynName == nme.applyDynamic then args else namedArgs
        typedApply(
  ...
  /** Translate selection that does not typecheck according to the normal rules
    * into a selectDynamic. foo.bar ~~> foo.selectDynamic(bar) foo.bar[T0, ...]
    * ~~> foo.selectDynamic[T0, ...](bar)
    *
    * Note: inner part of translation foo.bar(baz) = quux ~~>
    * foo.selectDynamic(bar).update(baz, quux) is achieved through an existing
    * transformation of in typedAssign [foo.bar(baz) = quux ~~>
    * foo.bar.update(baz, quux)].
    */
  def typedDynamicSelect(tree: untpd.Select, targs: List[untpd.Tree], pt: Type)(
      using Context
  ): Tree =
    typedApply(
      coreDynamic(
  ...
  /** Translate selection that does not typecheck according to the normal rules
    * into a updateDynamic. foo.bar = baz ~~> foo.updateDynamic(bar)(baz)
    */
  def typedDynamicAssign(tree: untpd.Assign, pt: Type)(using Context): Tree =
    def typedDynamicAssign(
        qual: untpd.Tree,
        name: Name,
        selSpan: Span,
        targs: List[untpd.Tree]
      ...
      typedApply(
        untpd.Apply(
          coreDynamic(qual, nme.updateDynamic, name, selSpan, targs),
          tree.rhs
        ),
        pt
      )
  ...
  /** Handle reflection-based dispatch for members of structural types.
    *
    * Given `x.a`, where `x` is of (widened) type `T` (a value type or a nullary
    * method type), and `x.a` is of type `U`, map `x.a` to the equivalent of:
    *
    * ```scala
    ...
    *
    * Given `x.a(a11, ..., a1n)...(aN1, ..., aNn)`, where `x.a` is of (widened)
    * type `(T11, ..., T1n)...(TN1, ..., TNn): R`, it is desugared to:
    *
    * ```scala
  ...
  def handleStructural(tree: Tree)(using Context): Tree =
    ...
    def structuralCall(selectorName: TermName, classOfs: => List[Tree]) =
      val selectable =
        adapt(qual, defn.SelectableClass.typeRef | defn.DynamicClass.typeRef)

      // ($qual: Selectable).$selectorName("$name")
      val base =
        untpd.Apply(
          untpd
            .Select(untpd.TypedSplice(selectable), selectorName)
            .withSpan(fun.span),
          (Literal(Constant(name.encode.toString)) :: Nil)
            .map(untpd.TypedSplice(_))
        )

      val scall =
        if vargss.isEmpty then base
            ...
            case mt: MethodType =>
              mt.paramInfos match
                case _ :: classOfsParam :: Nil
                  ...
                  val jlClassType =
                    defn.ClassClass.typeRef.appliedTo(TypeBounds.empty)
                  cpy.Apply(tree)(
                    fn,
                    nameArg :: seqToRepeated(
                      SeqLiteral(classOfs, TypeTree(jlClassType))
                    ) :: Nil
                  )
                case _ => tree
            case other => tree
    ...
    def fail(reason: String): Tree =
      errorTree(
        tree,
        em"Structural access not allowed on method $name because it $reason"
      )

    extension (tree: Tree)
      /** The implementations of `selectDynamic` and `applyDynamic` in
        * `scala.reflect.SelectDynamic` have no information about the expected
        * return type of a value/method which was declared in the refinement,
        * only the JVM type after erasure can be obtained through reflection,
        * e.g.
        *
        * class Foo(val i: Int) extends AnyVal class Reflective extends
        * reflect.Selectable val reflective = new Reflective { def foo = Foo(1)
        * // Foo at compile time, java.lang.Integer in reflection }
        *
        * Because of that reflective access cannot be implemented properly in
        * `scala.reflect.SelectDynamic` itself because it's not known there if
        * the value should be wrapped in a value class constructor call or not.
        * Hence the logic of wrapping is performed here, relying on the fact
        * that the implementations of `selectDynamic` and `applyDynamic` in
        * `scala.reflect.SelectDynamic` are final.
        */
      def maybeBoxingCast(tpe: Type) =
        val maybeBoxed =
          if ValueClasses.isDerivedValueClass(
              tpe.classSymbol
            ) && qual.tpe <:< defn.ReflectSelectableTypeRef
          then
            val genericUnderlying =
              ValueClasses.valueClassUnbox(tpe.classSymbol.asClass)
