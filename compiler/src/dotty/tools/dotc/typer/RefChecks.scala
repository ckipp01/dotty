...
object RefChecks:
  ...
  private def checkOverloadedRestrictions(clazz: Symbol)(using Context): Unit =
    ...
    do
      {
        val defaultGetterNames =
          defaultGetterClass.asClass.memberNames(defaultMethodFilter)
      ...
      if clazz derivesFrom defn.DynamicClass then
        for case (
            _,
            m1 :: m2 :: _
          ) <- (clazz.info member nme.applyDynamic).alternatives groupBy (_.symbol.typeParams.length)
        do
          report.error(
  ...
  /** The this-type of `cls` which should be used when looking at the types of
    * inherited members. If `cls` has a non-trivial self type, this returns a
    * skolem with the class type instead of the `this-type` of the class as
    * usual. This is done because otherwise we want to understand inherited
    * infos as they are written, whereas with the this-type they could be more
    * special. A test where this makes a difference is pos/i1401.scala. This one
    * used to succeed only if forwarding parameters is on. (Forwarding tends to
    * hide problems by binding parameter names).
    */
  private def upwardsThisType(cls: Symbol)(using Context) = cls.info match
    case ClassInfo(_, _, _, _, tp: Type)
        if (tp ne cls.typeRef) && !cls.isOneOf(FinalOrModuleClass) =>
      SkolemType(cls.appliedRef).withName(nme.this_)
    case _ =>
      cls.thisType

  /**   - Check that self type of `cls` conforms to self types of all `parents`
    *     as seen from `cls.thisType`
    *     - If self type of `cls` is explicit, check that it conforms to the
    *       self types of all its class symbols.
    * @param deep
    *   If true and a self type of a parent is not given explicitly, recurse to
    *   check against the parents of the parent. This is needed when capture
    *   checking, since we assume (& check) that the capture set of an inferred
    *   self type is the intersection of the capture sets of all its parents
    */
  def checkSelfAgainstParents(cls: ClassSymbol, parents: List[Symbol])(using
      Context
  ): Unit =
    withMode(Mode.CheckBoundsOrSelfType) {
      ...
      for psym <- parents do checkSelfConforms(psym.asClass)
    }
  end checkSelfAgainstParents

  /** Check that self type of this class conforms to self types of parents and
    * required classes. Also check that only `enum` constructs extend
    ...
    cls.info match
      case cinfo: ClassInfo =>
        ...
        if isClassExtendingJavaEnum then
          if !migrateTo3
          then // always error, only traits or enum-syntax is possible under scala 3.x
            ...
            parentTrees.exists {
              case parent @ tpd.Apply(tpd.TypeApply(fn, _), _)
                  if fn.tpe.termSymbol eq javaEnumCtor =>
        ...
        /** Check that arguments passed to trait parameters conform to the
          * parameter types in the current class. This is necessary since
          * parameter types might be narrowed through intersection with other
          * parent traits. See neg/i11018.scala.
          */
        def checkParamInits(app: Apply): Unit =
          ...
          if parentCls.is(Trait) then
            ...
            for (param, arg) <- params.lazyZip(args) do
              if !param.is(Private)
              then // its type can be narrowed through intersection -> a check is needed
                ...
                if !(arg.tpe <:< paramType) then
                  ...
                  if !(arg.tpe.subst(params, argTypes) <:< paramType.subst(
                      params,
                      argTypes
                    ))
                  then
                    report.error(
                      IllegalParameterInit(arg.tpe, paramType, param, cls),
                      arg.srcPos
                    )
  ...
  /** Disallow using trait parameters as prefix for its parents.
    ...
    */
  private def checkParentPrefix(cls: Symbol, parent: Tree)(using
      Context
  ): Unit =
    parent.tpe.typeConstructor match
      case TypeRef(ref: TermRef, _) =>
        val paramRefs =
          ref.namedPartsWith(ntp => ntp.symbol.enclosingClass == cls)
        if paramRefs.nonEmpty then
          report.error(TraitParameterUsedAsParentPrefix(cls), parent.srcPos)
      case _ =>

  /** Check that a class and its companion object to not both define a class or
    * module with same name
    */
  private def checkCompanionNameClashes(cls: Symbol)(using Context): Unit =
    if !cls.owner.is(ModuleClass) then
  ...
  /** A class for checking all overriding pairs of `class` with a given check
    * function
    */
  class OverridingPairsChecker(clazz: ClassSymbol, self: Type)(using Context)
      extends OverridingPairs.Cursor(clazz):

    override def matches(sym1: Symbol, sym2: Symbol): Boolean =
      isOverridingPair(sym1, sym2, self)

    private def inLinearizationOrder(
        sym1: Symbol,
        sym2: Symbol,
        parent: Symbol
    ): Boolean =
    ...
    override def canBeHandledByParent(
        sym1: Symbol,
        sym2: Symbol,
        parent: Symbol
    ): Boolean =
      isOverridingPair(sym1, sym2, parent.thisType)
        .showing(
          i"already handled ${sym1.showLocated}: ${sym1
              .asSeenFrom(parent.thisType)
              .signature}, ${sym2.showLocated}: ${sym2.asSeenFrom(parent.thisType).signature} = $result",
          refcheck
        )
    ...
    private val subtypeChecker: (Type, Type) => Context ?=> Boolean =
      this.checkSubType

    def checkAll(
        checkOverride: (
            (Type, Type) => Context ?=> Boolean,
            Symbol,
            Symbol
        ) => Unit
    ) =
      ...
      for dcl <- clazz.info.decls.iterator do
        if dcl.is(Deferred) then
          for other <- dcl.allOverriddenSymbols do
            if !other.is(Deferred) then checkOverride(checkSubType, dcl, other)
    end checkAll
  end OverridingPairsChecker

  /**   1. Check all members of class `clazz` for overriding conditions. That is
    *      for overriding member M and overridden member O:
    *
    ...
    * 1.7.1 either M is an abstract type, and M's bounds are sharper than O's
    * bounds. or M is a type alias or class which conforms to O's bounds.
    * 1.7.2 higher-order type arguments must respect bounds on higher-order type
    * parameters -- @M (explicit bounds and those implied by variance
    * annotations) -- @see checkKindBounds
    * 1.8. If O and M are values, then
    * 1.8.1 M's type is a subtype of O's type, or
    * 1.8.2 M is of type []S, O is of type ()T and S <: T, or
    * 1.8.3 M is of type ()S, O is of type []T and S <: T, or
    * 1.9. If M is erased, O is erased. If O is erased, M is erased or inline.
    * 1.10. If O is inline (and deferred, otherwise O would be final), M must be
    * inline
    * 1.11. If O is a Scala-2 macro, M must be a Scala-2 macro.
    * 1.12. If O is non-experimental, M must be non-experimental.
    * 1.13 Under -source future, if O is a val parameter, M must be a val
    * parameter that passes its value on to O. 2. Check that only abstract
    * classes have deferred members 3. Check that concrete classes do not have
    * deferred definitions that are not implemented in a subclass. 4. Check that
    * every member with an `override` modifier overrides some other member. TODO
    * check that classes are not overridden TODO This still needs to be cleaned
    * up; the current version is a straight port of what was there before, but
    * it looks too complicated and method bodies are far too large.
    *
    * @param makeOverridingPairsChecker
    *   A function for creating a OverridePairsChecker instance from the class
    *   symbol and the self type
    */
  def checkAllOverrides(
      clazz: ClassSymbol,
      makeOverridingPairsChecker: (
          (ClassSymbol, Type) => Context ?=> OverridingPairsChecker
      ) | Null = null
  )(using Context): Unit =
    ...
    def printMixinOverrideErrors(): Unit =
      mixinOverrideErrors.toList match
        ...
        case MixinOverrideError(member, msg) :: others =>
          val others1 =
            others.map(_.member).filter(_.name != member.name).distinct
          def othersMsg =
            val others1 = others
              .map(_.member)
     ...
     */
    def checkOverride(
        checkSubType: (Type, Type) => Context ?=> Boolean,
        member: Symbol,
        other: Symbol
    ): Unit =
      def memberTp(self: Type) =
        if member.isClass then
          TypeAlias(member.typeRef.EtaExpand(member.typeParams))
        else self.memberInfo(member)
      def otherTp(self: Type) = self.memberInfo(other)

      refcheck.println(
        i"check override ${infoString(member)} overriding ${infoString(other)}"
      )

      def noErrorType =
        !memberTp(self).isErroneous && !otherTp(self).isErroneous

      def overrideErrorMsg(
          core: Context ?=> String,
          compareTypes: Boolean = false
      ): Message =
        val (mtp, otp) =
          if compareTypes then (memberTp(self), otherTp(self))
          else (NoType, NoType)
      ...
      /** Do types of term members `member` and `other` as seen from `self`
        * match? If not we treat them as not a real override and don't issue
        * override error messages. Also, bridges are not generated in this case.
      ...
      def emitOverrideError(fullmsg: Message) =
        if !(hasErrors && member.is(Synthetic) && member.is(Module)) then
          ...
          if member.owner == clazz then report.error(fullmsg, member.srcPos)
      ...
      def autoOverride(sym: Symbol) =
        sym.is(Synthetic) && (desugar.isDesugaredCaseClassMethodName(
          member.name
        ) || // such names are added automatically, can't have an override preset.
          sym.is(Module)) // synthetic companion

      def overrideAccessError() =
        report.log(
          i"member: ${member.showLocated} ${member.flagsString}"
        ) // DEBUG
        ...
        overrideError(
          "has weaker access privileges; it should be " +
            (if otherAccess == "" then "public" else "at least " + otherAccess)
        )
      ...
      def overrideTargetNameError() =
        ...
        if member.hasTargetName(member.name) then
          ...
          overrideError(
            i"should not have a @targetName annotation since the overridden member hasn't one either"
          )
        else
          overrideError(
            i"has a different target name annotation; it should be $otherTargetName"
          )

      // Console.println(infoString(member) + " overrides " + infoString(other) + " in " + clazz);//DEBUG

      /* Is the intersection between given two lists of overridden symbols empty? */
      def intersectionIsEmpty(
          syms1: Iterator[Symbol],
          syms2: Iterator[Symbol]
      ) =
      ...
      if member.is(Private) then // (1.1)
        overrideError("has weaker access privileges; it should not be private")

      // todo: align accessibility implication checking with isAccessible in Contexts
      def isOverrideAccessOK =
        val memberIsPublic =
          (member.flags & AccessFlags).isEmpty && !member.privateWithin.exists
        def protectedOK = !other.is(Protected) || member.is(
          Protected
        ) // if o is protected, so is m
        def accessBoundaryOK =
          val ob = other.accessBoundary(member.owner)
          val mb = member.accessBoundary(member.owner)
          // restriction isLocalToBlock because companionModule fails under -from-tasty (#14508)
          def companionBoundaryOK = ob.isClass && !ob.isLocalToBlock && mb.is(
            Module
          ) && (ob.companionModule eq mb.companionModule)
          ob.isContainedIn(
            mb
          ) || companionBoundaryOK // m relaxes o's access boundary,
        def otherIsJavaProtected = other.isAllOf(
          JavaProtected
      ...
      if !member.hasTargetName(other.targetName) then overrideTargetNameError()
      else if !isOverrideAccessOK then overrideAccessError()
      else if other.isClass then
      ...
      else if other.isOpaqueAlias then
        // direct overrides were already checked on completion (see Checking.chckWellFormed)
        // the test here catches indirect overriddes between two inherited base types.
        overrideError(
          "cannot be used here - opaque type aliases cannot be overridden"
        )
      else if !other.is(Deferred) && member.isClass then
        overrideError(
          "cannot be used here - classes can only override abstract types"
        )
      else if other.isEffectivelyFinal then // (1.2)
        overrideError(i"cannot override final member ${other.showLocated}")
      else if member.is(ExtensionMethod) && !other.is(ExtensionMethod)
      then // (1.3)
        overrideError("is an extension method, cannot override a normal method")
      else if other.is(ExtensionMethod) && !member.is(ExtensionMethod)
      then // (1.3)
      ...
      then
        ...
        then member.setFlag(Override)
        else if member.isType && self.memberInfo(member) =:= self.memberInfo(
            other
          )
        then () // OK, don't complain about type aliases which are equal
      ...
      else if other.is(AbsOverride) && other.isIncompleteIn(clazz) && !member
          .is(AbsOverride)
      then overrideError("needs `abstract override` modifiers")
      ...
      else if other.isStableMember && !member.isStableMember then // (1.5)
        overrideError("needs to be a stable, immutable value")
      else if member.is(ModuleVal) && !other.isRealMethod && !other.isOneOf(
          DeferredOrLazy
        )
      then overrideError("may not override a concrete non-lazy value")
      else if member.is(Lazy, butNot = Module) && !other.isRealMethod && !other
          .is(Lazy) &&
        !warnOnMigration(
          overrideErrorMsg("may not override a non-lazy value"),
          member.srcPos,
          version = `3.0`
        )
      then overrideError("may not override a non-lazy value")
      else if other.is(Lazy) && !other.isRealMethod && !member.is(Lazy) then
        overrideError("must be declared lazy to override a lazy value")
      else if member.is(Erased) && !other.is(Erased) then // (1.9)
        overrideError("is erased, cannot override non-erased member")
      else if other.is(Erased) && !member.isOneOf(Erased | Inline) then // (1.9)
      ...
      else if other.isScala2Macro && !member.isScala2Macro then // (1.11)
        overrideError(
          "cannot be used here - only Scala-2 macros can override Scala-2 macros"
        )
      else if !compatTypes(memberTp(self), otherTp(self)) &&
        !compatTypes(memberTp(upwardsSelf), otherTp(upwardsSelf))
      then overrideError("has incompatible type", compareTypes = true)
      else if member.targetName != other.targetName then
        if other.targetName != other.name then
          overrideError(
            i"needs to be declared with @targetName(${"\""}${other.targetName}${"\""}) so that external names match"
          )
        else
          overrideError(
            "cannot have a @targetName annotation since external names would be different"
          )
      ...
      else if !other.isExperimental && member.hasAnnotation(
          defn.ExperimentalAnnot
        )
      then // (1.12)
    ...
    if !clazz.isOneOf(AbstractOrTrait) then
      ...
      /** The term symbols in this class and its baseclasses that are abstract
        * in this class. We can't use memberNames for that since a concrete
        * member might have the same signature as an abstract member in a base
        * class, yet might not override it.
        */
      ...
      def checkNoAbstractMembers(): Unit =
        ...
        val missingMethods = grouped.toList flatMap { case (name, syms) =>
          ...
          if withoutSetters.nonEmpty then withoutSetters else syms
        }
        ...
        def stubImplementations: List[String] =
          ...
          def membersStrings(members: List[Symbol]) =
            members
              ...
              .map(_.asSeenFrom(clazz.thisType).showDcl + " = ???")

          if regrouped.tail.isEmpty then membersStrings(regrouped.head._2)
          else
            (regrouped.sortBy("" + _._1.name) flatMap { case (owner, members) =>
              ("// Members declared in " + owner.fullName) +: membersStrings(
                members
              ) :+ ""
            }).init
        ...
        if missingMethods.size > 1 then
          abstractClassError(
            false,
            "it has " + missingMethods.size + " unimplemented members."
          )
          val preface =
          ...
          abstractErrors += stubImplementations
            .map("  " + _ + "\n")
            .mkString(preface, "", "")
          return

        for member <- missing do
          def showDclAndLocation(sym: Symbol) =
            s"${sym.showDcl} in ${sym.owner.showLocated}"
          def undefined(msg: String) =
            abstractClassError(
              false,
              s"${showDclAndLocation(member)} is not defined $msg"
            )
          val underlying = member.underlyingSymbol

          // Give a specific error message for abstract vars based on why it fails:
          // It could be unimplemented, have only one accessor, or be uninitialized.
          if underlying.is(Mutable) then
            val isMultiple = grouped.getOrElse(underlying.name, Nil).size > 1

            // If both getter and setter are missing, squelch the setter error.
            if member.isSetter && isMultiple then ()
            else
              undefined(
                if member.isSetter then
                  "\n(Note that an abstract var requires a setter in addition to the getter)"
                else if member.isGetter && !isMultiple then
            ...
            val matchingName =
              clazz.info.nonPrivateMember(underlying.name).alternatives
            ...
            matchingArity match
              ...
              case concrete :: Nil =>
                val mismatches =
                  abstractParams
                    .zip(concrete.info.firstParamTypes)
                ...
                mismatches match
                  ...
                  case (pa, pc) :: Nil =>
                    ...
                    val addendum =
                      if abstractSym == concreteSym then
                        (pa.typeConstructor, pc.typeConstructor) match
                          case (TypeRef(pre1, _), TypeRef(pre2, _)) =>
                            if pre1 =:= pre2 then "their type parameters differ"
                            else
                    ...
                    undefined(s"""
                                 |(Note that
                                 | parameter ${pa.show} in ${showDclAndLocation(
                                  underlying
                                )} does not match
                                 | parameter ${pc.show} in ${showDclAndLocation(
                                  concrete.symbol
                                )}
              ...
              case concretes =>
                undefined(
                  s"\n(The class implements members with different types: ${concretes
                      .map(c => showDclAndLocation(c.symbol))}%\n  %)"
                )
            end match
          else undefined("")
          end if
      ...
      def checkNoAbstractDecls(bc: Symbol): Unit =
        for decl <- bc.info.decls do
          if decl.is(Deferred) then
            val impl =
              withMode(Mode.IgnoreCaptures)(decl.matchingMember(clazz.thisType))
            ...
            then
              ...
              abstractClassError(
                ...
                "there is a deferred declaration of " + infoString(decl) +
                  " which is not implemented in a subclass" + err
                    .abstractVarMessage(decl)
              )
        if bc.asClass.superClass.is(Abstract) then
      ...
      def checkMemberTypesOK() =
        ...
        def addDecls(cls: Symbol): Unit =
          if !seenClasses.contains(cls) then
            seenClasses += cls
            for mbr <- cls.info.decls do
              if mbr.isTerm && !mbr.isOneOf(
                  Synthetic | Bridge
                ) && mbr.memberCanMatchInheritedSymbols &&
                !membersToCheck.contains(mbr.name)
              then membersToCheck += mbr.name
            cls.info.parents
              .map(_.classSymbol)
        ...
        withMode(Mode.IgnoreCaptures) {
          for name <- membersToCheck do
            for mbrd <- self.member(name).alternatives do
              val mbr = mbrd.symbol
              val mbrType = mbr.info.asSeenFrom(self, mbr.owner)
              if !mbrType.overrides(
                  mbrd.info,
                  relaxedCheck = false,
                  matchLoosely = true
                )
              then
                report.errorOrMigrationWarning(
                  ...
                  (if mbr.owner == clazz then mbr else clazz).srcPos,
                  from = `3.0`
                )
        }
      end checkMemberTypesOK

      /** Check that inheriting a case class does not constitute a variant
        * refinement of a base type of the case class. It is because of this
        * restriction that we can assume invariant refinement for case classes
        * in `constrainPatternType`.
        */
      def checkCaseClassInheritanceInvariant() =
        for caseCls <- clazz.info.baseClasses.tail.find(_.is(Case)) do
          for baseCls <- caseCls.info.baseClasses.tail do
            if baseCls.typeParams.exists(_.paramVarianceSign != 0) then
              for problem <- variantInheritanceProblems(
                  baseCls,
                  caseCls,
                  "non-variant",
                  "case "
      ...
      if abstractErrors.isEmpty then checkNoAbstractDecls(clazz)
    ...
    if !clazz.is(Trait) then
      ...
      def checkParameterizedTraitsOK() =
        ...
        for
          ...
          problem <- variantInheritanceProblems(
            cls,
            clazz.asClass.superClass,
            "parameterized",
            "super"
          )
        do report.error(problem, clazz.srcPos)

      checkParameterizedTraitsOK()

    /** Check that `site` does not inherit conflicting generic instances of
      * `baseCls`, when doing a direct base type or going via intermediate class
      * `middle`. I.e, we require:
      *
    ...
    def variantInheritanceProblems(
      ...
      if combinedBT =:= thisBT then None // ok
      else
        Some(
    ...
    for member <- clazz.info.decls do
      if member.isAnyOverride && !(clazz.thisType.baseClasses exists (hasMatchingSym(
          _,
          member
        )))
      then
        if checks != noPrinter then
          for bc <- clazz.info.baseClasses.tail do
            val sym = bc.info.decl(member.name).symbol
            if sym.exists then
              checks.println(i"$bc has $sym: ${clazz.thisType.memberInfo(sym)}")

        val nonMatching =
          clazz.info.member(member.name).altsWith(alt => alt.owner != clazz)
        nonMatching match
          case Nil =>
          ...
          case ms =>
            ...
            then
              report.error(
                OverridesNothingButNameExists(member, ms),
                member.srcPos
              )
  ...
  /** Check that we do not "override" anything with a private method or
    * something that becomes a private method. According to the Scala modeling
    * this is non-sensical since private members don't override. But Java and
    * the JVM disagree, if the private member is a method. A test case is
    * neg/i7926b.scala. Note: The compiler could possibly silently rename the
    * offending private instead of flagging it as an error. But that might mean
    * we see some surprising names at runtime. E.g. in neg/i4564a.scala, a
    * private case class `apply` method would have to be renamed to something
    * else.
    */
  def checkNoPrivateOverrides(tree: Tree)(using Context): Unit =
    ...
    if sym.maybeOwner.isClass
      && sym.is(Private)
      && (sym.isOneOf(MethodOrLazyOrMutable) || !sym.is(
        Local
      )) // in these cases we'll produce a getter later
      && !sym.isConstructor
    then
      ...
      for bc <- cls.baseClasses.tail do
        ...
        if other.exists then
          report.error(
            em"private $sym cannot override ${other.showLocated}",
            sym.srcPos
          )
  end checkNoPrivateOverrides

  /** Check that unary method definition do not receive parameters. They can
    * only receive inferred parameters such as type parameters and implicit
    * parameters.
    */
  def checkUnaryMethods(sym: Symbol)(using Context): Unit =
      ...
      tpe match
        case tpe: MethodType =>
          if tpe.isImplicitMethod || tpe.isContextualMethod then
            ...
            val what =
              if tpe.paramNames.isEmpty then
                "empty parameter list.\n\nPossible fix: remove the `()` arguments."
              else "parameters"
            report.warning(
              s"unary_<op> method cannot take $what",
              sym.sourcePos
            )
        case tpe: PolyType =>
          checkParameters(tpe.resType)
    ...
    /** Skip leading type and contextual parameters, then skip the self
      * parameter, and finally check the parameter
      */
    ...
    if isUnaryPrefixName(sym.name) then
      ...
      else checkParameters(sym.info)

  end checkUnaryMethods

  /** Verify that references in the user-defined `@implicitNotFound` message are
    * valid. (i.e. they refer to a type variable that really occurs in the
    * signature of the annotated symbol.)
    */
  private object checkImplicitNotFoundAnnotation:
    /** Warns if the class or trait has an @implicitNotFound annotation with
      * invalid type variable references.
      */
    ...
    private def checkReferences(sd: SymDenotation)(using Context): Unit =
      lazy val substitutableTypesNames =
        ErrorReporting
          .substitutableTypeSymbolsInScope(sd.symbol)
          .map(_.denot.name.show)
      for
        annotation <- sd.getAnnotation(defn.ImplicitNotFoundAnnot)
        case PositionedStringLiteralArgument(msg, span) <- annotation.argument(
          0
        )
      do
        forEachTypeVariableReferenceIn(msg) { case (ref, start) =>
          if !substitutableTypesNames.contains(ref) then
            reportInvalidReference(span, ref, start, sd)
        }

    /** Reports an invalid reference to a type variable `typeRef` that was found
      * in `span`
      */
    private def reportInvalidReference(
      ...
      val ownerName =
        ...
        else if sd.owner.isConstructor then
          s"the constructor of `${sd.owner.owner.name.show}`"
        else s"method `${sd.owner.name.show}`"
      val msg =
        InvalidReferenceInImplicitNotFoundAnnotation(typeRefName, ownerName)
      val startPos =
        span
          .shift(variableOffsetinArgumentLiteral + 1)
          .startPos // +1 because of 0-based index
      val pos = ctx.source.atSpan(startPos)
      report.warning(msg, pos)

    /** Calls the supplied function for each quoted reference to a type variable
      * in <pre>s</pre>. The input
      *
      * ```scala
      ...
      * ```
      *
      * will lead to two invocations of `f`, once with `(T, 12)` and once with
      * `(F, 22)` as argument.
      *
      * @param s
      *   The string to query for type variable references.
      * @param f
      *   A function to apply to every pair of (\<type variable>, \<position in
      *   string>).
      */
    private def forEachTypeVariableReferenceIn(s: String)(
        f: (String, Int) => Unit
    ) =
      ...
      for reference <- matches do
        ...
        val variableOffset =
          referenceOffset + reference.length - prefixlessReference.length
  ...
  def checkAnyRefMethodCall(tree: Tree)(using Context) =
    if tree.symbol.exists
      ...
      && (!ctx.owner.enclosingClass.exists || ctx.owner.enclosingClass.isPackageObject)
    then report.warning(UnqualifiedCallToAnyRefMethod(tree, tree.symbol), tree)
end RefChecks

import RefChecks.*

/** Post-attribution checking and transformation, which fulfills the following
  * roles
  *
  ...
  *   - this(...) constructor calls do not forward reference other definitions
  *     in their block (not even lazy vals).
  *   - no forward reference in a local block jumps over a non-lazy val
  *     definition.
  *   - a class and its companion object do not both define a class or module
  *     with the same name.
  *
  ...
  *   - members accessed from an inner or companion class. All these members are
  *     marked as NotJavaPrivate. Unlike in Scala 2.x not-private members keep
  *     their name. It is up to the backend to find a unique expanded name for
  *     them. The rationale to do name changes that late is that they are very
  *     fragile.
  *
  * todo: But RefChecks is not done yet. It's still a somewhat dirty port from
  * the Scala 2 version. todo: move untrivial logic to their own mini-phases
  */
class RefChecks extends MiniPhase:
  ...
  override def transformValDef(tree: ValDef)(using Context): ValDef =
    if tree.symbol.exists then
      ...
      val sym = tree.symbol
      if sym.exists && sym.owner.isTerm then
        tree.rhs match
          case Ident(nme.WILDCARD) =>
            report.error(UnboundPlaceholderParameter(), sym.srcPos)
          case _ =>
    tree
  ...
  override def transformTemplate(tree: Template)(using Context): Tree = try
    ...
    if cls.is(Trait) then tree.parents.foreach(checkParentPrefix(cls, _))
