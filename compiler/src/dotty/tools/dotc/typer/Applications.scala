...
object Applications:
  ...
  def extractorMember(tp: Type, name: Name)(using Context): SingleDenotation =
    tp.member(name)
      .suchThat(sym =>
        sym.info.isParameterless && sym.info.widenExpr.isValueType
      )

  def extractorMemberType(tp: Type, name: Name, errorPos: SrcPos)(using
      Context
  ): Type =
    val ref = extractorMember(tp, name)
    if ref.isOverloaded then
      errorType(
        em"Overloaded reference to $ref is not allowed in extractor",
        errorPos
      )
    ref.info.widenExpr.annotatedToRepeated

  /** Does `tp` fit the "product match" conditions as an unapply result type for
    * a pattern with `numArgs` subpatterns? This is the case if `tp` has members
    * `_1` to `_N` where `N == numArgs`.
    */
  def isProductMatch(
      tp: Type,
      numArgs: Int,
      errorPos: SrcPos = NoSourcePosition
  )(using Context): Boolean =
    numArgs > 0 && productArity(tp, errorPos) == numArgs

  /** Does `tp` fit the "product-seq match" conditions as an unapply result type
    * for a pattern with `numArgs` subpatterns? This is the case if (1) `tp` has
    * members `_1` to `_N` where `N <= numArgs + 1`. (2) `tp._N` conforms to Seq
    * match
    */
  def isProductSeqMatch(
      tp: Type,
      numArgs: Int,
      errorPos: SrcPos = NoSourcePosition
  )(using Context): Boolean =
  ...
  /** Does `tp` fit the "get match" conditions as an unapply result type? This
    * is the case of `tp` has a `get` member as well as a parameterless
    * `isEmpty` member of result type `Boolean`.
    */
  def isGetMatch(tp: Type, errorPos: SrcPos = NoSourcePosition)(using
      Context
  ): Boolean =
    extractorMemberType(tp, nme.isEmpty, errorPos).widenSingleton
      .isRef(defn.BooleanClass) &&
  ...
  def unapplySeqTypeElemTp(getTp: Type)(using Context): Type =
    ...
    def dropTp(elemTp: Type) =
      MethodType(List(defn.IntType), defn.CollectionSeqType.appliedTo(elemTp))
    def toSeqTp(elemTp: Type) = ExprType(
      defn.CollectionSeqType.appliedTo(elemTp)
    )

    // the result type of `def apply(i: Int): T`
    val elemTp = getTp
      .member(nme.apply)
      .suchThat(_.info <:< applyTp(WildcardType))
      .info
      .resultType
    ...
    val isValid =
      elemTp.exists &&
        (hasMethod(nme.lengthCompare, lengthCompareTp) || hasMethod(
          nme.length,
          lengthTp
        )) &&
  ...
  end unapplySeqTypeElemTp

  def productSelectorTypes(tp: Type, errorPos: SrcPos)(using
      Context
  ): List[Type] =
    val sels =
      for (n <- Iterator.from(0))
        yield extractorMemberType(tp, nme.selectorName(n), errorPos)
  ...
  def productArity(tp: Type, errorPos: SrcPos = NoSourcePosition)(using
      Context
  ): Int =
    if defn.isProductSubType(tp) then productSelectorTypes(tp, errorPos).size
    else -1
  ...
  def productSelectors(tp: Type)(using Context): List[Symbol] =
    val sels =
      for (n <- Iterator.from(0))
        yield tp
          .member(nme.selectorName(n))
          .suchThat(_.info.isParameterless)
          .symbol
    sels.takeWhile(_.exists).toList

  def getUnapplySelectors(tp: Type, args: List[untpd.Tree], pos: SrcPos)(using
      Context
  ): List[Type] =
  ...
  def productSeqSelectors(tp: Type, argsNum: Int, pos: SrcPos)(using
      Context
  ): List[Type] =
    ...
    (0 until argsNum)
      .map(i => if i < arity - 1 then selTps(i) else elemTp)
      .toList
  ...
  def unapplyArgs(
    ...
    if unapplyName == nme.unapplySeq then
      ...
      else if unapplyResult.widenSingleton isRef defn.BooleanClass then Nil
      else if defn.isProductSubType(unapplyResult) && productArity(
          unapplyResult,
          pos
        ) != 0
      then
  ...
  def foldApplyTupleType(tp: Type)(using Context): List[Type] =
    object tupleFold extends TypeAccumulator[List[Type]]:
      override def apply(accum: List[Type], t: Type): List[Type] =
        t match
          case AppliedType(tycon, x :: x2 :: Nil)
              if tycon.typeSymbol == defn.PairClass =>
            apply(x :: accum, x2)
          case x => foldOver(accum, x)
    end tupleFold
    tupleFold(Nil, tp).reverse

  def wrapDefs(defs: mutable.ListBuffer[Tree] | Null, tree: Tree)(using
      Context
  ): Tree =
    if defs != null && defs.nonEmpty then tpd.Block(defs.toList, tree) else tree

  /** Optionally, if `sym` is a symbol created by `resolveMapped`, i.e.
    * representing a mapped alternative, the original prefix of the alternative
    * and the number of skipped term parameters.
    */
  private def mappedAltInfo(sym: Symbol)(using Context): Option[(Type, Int)] =
    for ann <- sym.getAnnotation(defn.MappedAlternativeAnnot) yield
      val AppliedType(_, pre :: ConstantType(c) :: Nil) =
        ann.tree.tpe: @unchecked
      (pre, c.intValue)
  ...
  /** Find reference to default parameter getter for parameter #n in current
    ...
    */
  def findDefaultGetter(fn: Tree, n: Int, testOnly: Boolean)(using
      Context
  ): Tree =
  ...
  /** Find reference to default parameter getter for method `meth` numbered
    * `idx` selected from given `receiver`, or EmptyTree if none was found.
    * @param meth
    *   the called method (can be mapped by resolveMapped)
    * @param receiver
    *   the receiver of the original method call, which determines where default
    *   getters are found
    * @param idx
    *   the index of the searched for default getter, as encoded in its name
    */
  def findDefaultGetter(meth: TermSymbol, receiver: Tree, idx: Int)(using
      Context
  ): Tree =
    val getterPrefix =
      if meth.is(Synthetic) && meth.name == nme.apply then nme.CONSTRUCTOR
      else meth.name
    ...
    if receiver.isEmpty then
      ...
      def selectGetter(qual: Tree): Tree =
        val getterDenot = qual.tpe
          .member(getterName)
          .accessibleFrom(
            qual.tpe.widenIfUnstable,
            superAccess = true
          ) // to reset Local
        if getterDenot.exists then
          qual.select(TermRef(qual.tpe, getterName, getterDenot))
        else EmptyTree
      if !meth.isClassConstructor then selectGetter(receiver)
      else
        ...
        if companion.isTerm then
          ...
          if prefix.exists then
            selectGetter(ref(TermRef(prefix, companion.asTerm)))
          else EmptyTree
        else EmptyTree
    end if
  end findDefaultGetter
  ...
  def defaultArgument(fn: Tree, n: Int, testOnly: Boolean)(using
      Context
  ): Tree =
  ...
  def retypeSignaturePolymorphicFn(fun: Tree, methType: Type)(using
      Context
  ): Tree =
    val sym1 = fun.symbol
    val flags2 =
      sym1.flags | NonMember // ensures Select typing doesn't let TermRef#withPrefix revert the type
    val sym2 = sym1.copy(
      info = methType,
      flags = flags2
    ) // symbol not entered, to avoid overload resolution problems
...
trait Applications extends Compatibility:
  ...
  abstract class Application[Arg](
    ...
    /** Check that argument corresponds to type `formal` and possibly add it to
      * the list of adapted arguments
      */
    ...
    /** If constructing trees, pull out all parts of the function which are not
      * idempotent into separate prefix definitions
      */
    protected def liftFun(): Unit = ()

    /** Whether `liftFun` is needed? It is the case if default arguments are
      * used.
      */
    protected def needLiftFun: Boolean =
      def requiredArgNum(tp: Type): Int = tp.widen match
        case funType: MethodType =>
          ...
          if argsNum >= 1 && paramInfos.last.isRepeatedParam then
    ...
    /** A flag signalling that the typechecking the application was so far
      * successful
      */
    ...
    /** The function's type after widening and instantiating polytypes with
      * TypeParamRefs in constraint set
      */
    ...
    protected def init(): Unit = methType match
      ...
      case _ =>
        if methType.isError then ok = false
    ...
    def reorder[T <: Untyped](args: List[Trees.Tree[T]]): List[Trees.Tree[T]] =
      ...
      /** @param pnames
        *   The list of parameter names that are missing arguments
        * @param args
        *   The list of arguments that are not yet passed, or that are waiting
        *   to be dropped
        * @param nameToArg
        *   A map from as yet unseen names to named arguments
        * @param toDrop
        *   A set of names that have already be passed as named arguments
        *
        * For a well-typed application we have the invariants
        *
        *   1. `(args diff toDrop)` can be reordered to match `pnames` 2. For
        *      every `(name -> arg)` in `nameToArg`, `arg` is an element of
        *      `args`
        */
      def handleNamed(
          pnames: List[Name],
          args: List[Trees.Tree[T]],
          nameToArg: Map[Name, Trees.NamedArg[T]],
          toDrop: Set[Name]
      ): List[Trees.Tree[T]] = pnames match
        case pname :: pnames1 if nameToArg contains pname =>
          // there is a named argument for this parameter; pick it
          nameToArg(pname) :: handleNamed(
            pnames1,
            args,
            nameToArg - pname,
            toDrop + pname
        ...
        case _ =>
          def pnamesRest = if pnames.isEmpty then pnames else pnames.tail
          args match
            case (arg @ NamedArg(aname, _)) :: args1 =>
              if toDrop contains aname then // argument is already passed
                handleNamed(pnames, args1, nameToArg, toDrop - aname)
              else if (nameToArg contains aname) && pnames.nonEmpty
              then // argument is missing, pass an empty tree
                genericEmptyTree :: handleNamed(
                  pnames.tail,
                  args,
                  nameToArg,
                  toDrop
      ...
      def handlePositional(
        ...
        args match
          case (arg: NamedArg @unchecked) :: _ =>
            val nameAssocs =
              for (case arg @ NamedArg(name, _) <- args) yield (name, arg)
            handleNamed(pnames, args, nameAssocs.toMap, Set())
          case arg :: args1 =>
            arg :: handlePositional(
              if pnames.isEmpty then Nil else pnames.tail,
              args1
            )
          case Nil => Nil
          ...
          case formal :: formals1 =>
            def checkNoVarArg(arg: Arg) =
              if !ctx.isAfterTyper && isVarArg(arg) then
                ...
                fail(
                  em"Sequence argument type annotation `*` cannot be used here:\n$addendum",
                  arg
                )

            /** Add result of typing argument `arg` against parameter type
              * `formal`.
              * @return
              *   The remaining formal parameter types. If the method is
              *   parameter-dependent this means substituting the actual
              *   argument type for the current formal parameter in the
            ...
            def tryDefault(n: Int, args1: List[Arg]): Unit =
              ...
              val defaultArg =
                if isJavaAnnotConstr(sym) then
                  val cinfo = sym.owner.asClass.classInfo
                  val pname = methodType.paramNames(n)
                  val hasDefault = cinfo
                    .member(pname)
                    .suchThat(d =>
                      d.is(Method) && d.hasAnnotation(
                        defn.AnnotationDefaultAnnot
                      )
                    )
                    .exists
                  ...
                  if hasDefault then tpd.Underscore(formal)
                  else EmptyTree
              ...
              if !defaultArg.isEmpty then
                defaultArg.tpe.widen match
                  case _: MethodOrPoly if testOnly =>
                    matchArgs(args1, formals1, n + 1)
                  case _ =>
                    matchArgs(args1, addTyped(treeToArg(defaultArg)), n + 1)
              else if methodType.isContextualMethod && ctx.mode.is(
                  Mode.ImplicitsEnabled
                )
              then matchArgs(args1, addTyped(treeToArg(implicitArg)), n + 1)
              else missingArg(n)
            end tryDefault
  ...
  /** Subclass of Application for the cases where we are interested only in a
    * "can/cannot apply" answer, without needing to construct trees or issue
    * error messages.
    */
  abstract class TestApplication[Arg](
        ...
        case argtpe =>
          ...
          def SAMargOK =
            defn.isFunctionType(argtpe1) && formal.match
              case SAMType(sam) =>
                argtpe <:< sam.toFunctionType(isJava =
                  formal.classSymbol.is(JavaDefined)
                )
              case _ => false
          ...
          || (!formal.isRef(defn.PartialFunctionClass) || ctx.mode.is(
            Mode.ImplicitsEnabled
          )) && SAMargOK
    ...
    final def addArg(arg: TypedArg, formal: Type): Unit = ok =
      ok & argOK(arg, formal)
  ...
  /** Subclass of Application for applicability tests with type arguments and
    * value argument trees.
    */
  class ApplicableToTrees(
    ...
    def argType(arg: Tree, formal: Type): Type =
      if untpd.isContextualClosure(arg) && defn.isContextFunctionType(formal)
      then arg.tpe
  ...
  /** Subclass of Application for type checking an Apply node, where types of
    * arguments are either known or unknown.
    */
  abstract class TypedApply[T <: Untyped](
    ...
    def harmonizeArgs(args: List[TypedArg]): List[Tree] =
      ...
      if isFullyDefined(methodType.resType, ForceDegree.none) then args
      else harmonize(args)
    ...
    private def lifter(using Context) =
      if methRef.symbol.hasDefaultParams then LiftComplex else LiftImpure

    override def liftFun(): Unit =
      if liftedDefs == null then
        liftedDefs = new mutable.ListBuffer[Tree]
        myNormalizedFun =
          lifter.liftApp(liftedDefs.uncheckedNN, myNormalizedFun)

    /** The index of the first difference between lists of trees `xs` and `ys`
      * -1 if there are no differences.
      */
    private def firstDiff[T <: Trees.Tree[?]](
        xs: List[T],
        ys: List[T],
        n: Int = 0
    ): Int = xs match
      case x :: xs1 =>
        ys match
          case y :: ys1 => if x ne y then n else firstDiff(xs1, ys1, n + 1)
          case nil      => n
      case nil =>
        ys match
          case y :: ys1 => n
          case nil      => -1
    private def sameSeq[T <: Trees.Tree[?]](xs: List[T], ys: List[T]): Boolean =
      firstDiff(xs, ys) < 0
    ...
    def isSafeArg(arg: Tree) =
      isPureExpr(arg)
        || arg.isInstanceOf[RefTree | Apply | TypeApply] && arg.symbol.name.is(
          DefaultGetterName
        )

    val result: Tree =
      var typedArgs = typedArgBuf.toList
      def app0 = cpy.Apply(app)(
        normalizedFun,
        typedArgs
          ...
          then
            ...
            val orderedArgDefs =
              ...
              def position(arg: Trees.Tree[T]) =
                ...
                if i >= 0 then i else orderedArgs.length
              ...
              def originalIndex(n: Int) =
                if n < originalIndices.length then originalIndices(n)
                else orderedArgs.length
              scala.util.Sorting
                .stableSort[(Tree, Int), Int](
                  argDefBuf.zip(impureArgIndices),
                  (arg, idx) => originalIndex(idx)
                )
                .map(_._1)
  ...
  /** Subclass of Application for type checking an Apply node with untyped
    * arguments.
    */
  ...
  /** Subclass of Application for type checking an Apply node with typed
    * arguments.
    */
  class ApplyToTyped(
      app: untpd.Apply,
  ...
  /** If `app` is a `this(...)` constructor call, the this-call argument
    * context, otherwise the current context.
    */
  def argCtx(app: untpd.Tree)(using Context): Context =
    if ctx.owner.isClassConstructor && untpd.isSelfConstrCall(app) then
      ctx.thisCallArgContext
    else ctx

  /** Typecheck application. Result could be an `Apply` node, or, if application
    * is an operator assignment, also an `Assign` or Block node.
    */
    ...
    def realApply(using Context): Tree =
      val resultProto = tree.fun match
        case Select(New(tpt), _) if pt.isInstanceOf[ValueType] =>
          if tpt.isType && typedAheadType(tpt).tpe.typeSymbol.typeParams.isEmpty
      ...
      val originalProto =
        new FunProto(tree.args, resultProto)(this, tree.applyKind)(using
          argCtx(tree)
        )
      record("typedApply")
      ...
      val proto =
        if originalProto.hasTupledDual && needsTupledDual(
            fun1.tpe,
            originalProto
          )
      ...
      /** Type application where arguments come from prototype, and no implicits
        * are inserted
        */
      def simpleApply(fun1: Tree, proto: FunProto)(using Context): Tree =
        methPart(fun1).tpe match
          case funRef: TermRef if funRef.symbol.isSignaturePolymorphic =>
            ...
            val resultType =
              if !originalResultType.isRef(defn.ObjectClass) then
                originalResultType
              else
                AvoidWildcardsMap()(proto.resultType.deepenProtoTrans) match
                  case SelectionProto(
                        nme.asInstanceOf_,
                        PolyProto(_, resTp),
                        _,
            ...
            val methType =
              MethodType(proto.typedArgs().map(_.tpe.widen), resultType)
      ...
      /** Try same application with an implicit inserted around the qualifier of
        * the function part. Return an optional value to indicate success.
        */
      def tryWithImplicitOnQualifier(fun1: Tree, proto: FunProto)(using
          Context
      ): Option[Tree] =
        if ctx.mode.is(Mode.SynthesizeExtMethodReceiver) || proto.hasErrorArg
        then
      ...
      fun1.tpe match
        case err: ErrorType =>
          cpy.Apply(tree)(fun1, proto.typedArgs()).withType(err)
        case TryDynamicCallType =>
          val isInsertedApply = fun1 match
            case Select(_, nme.apply) => fun1.span.isSynthetic
            case TypeApply(sel @ Select(_, nme.apply), _) =>
            ...
            tree.args match
              case (arg @ Match(EmptyTree, cases)) :: Nil =>
                cases.foreach {
                  ...
                  case CaseDef(Ident(name), _, _)
                      if name == nme.WILDCARD => // Ok
                  case CaseDef(pat, _, _) =>
                    report.error(
                      UnexpectedPatternForSummonFrom(pat),
                      pat.srcPos
                    )
                }
                typed(
                  untpd.InlineMatch(EmptyTree, cases).withSpan(tree.span),
                  pt
                )
              case _ =>
                errorTree(
                  tree,
                  em"argument to summonFrom must be a pattern matching closure"
                )
          else
            tryEither {
              ...
              tryWithImplicitOnQualifier(fun1, originalProto).getOrElse(
                if proto eq originalProto then fail
      ...
      if untpd.isOpAssign(tree) then
        ...
        app match
          case Apply(fn @ Select(left, _), right :: Nil) if fn.hasType =>
            val op = fn.symbol
            if op == defn.Any_== || op == defn.Any_!= then
  ...
  /** Typecheck an Apply node with a typed function and possibly-typed arguments
    * coming from `proto`
    */
  def ApplyTo(
      app: untpd.Apply,
    ...
    val typer = ctx.typer
    if proto.allArgTypesAreCurrent() then
      typer
        .ApplyToTyped(
          app,
          fun,
      ...
      typer
        .ApplyToUntyped(app, fun, methRef, proto, resultType)(using
          fun.nullableInArgContext(using argCtx(app))
        )
        .result

  /** Overridden in ReTyper to handle primitive operations that can be generated
    * after erasure
    */
  protected def handleUnexpectedFunType(tree: untpd.Apply, fun: Tree)(using
      Context
  ): Tree =
  ...
  def typedTypeApply(tree: untpd.TypeApply, pt: Type)(using Context): Tree =
    ...
    val typedArgs =
      if isNamed then typedNamedArgs(tree.args)
      else tree.args.mapconserve(typedType(_))
    ...
    typedExpr(tree.fun, PolyProto(typedArgs, pt)) match
      case fun: TypeApply if !ctx.isAfterTyper =>
        ...
        errorTree(
          tree,
          em"""illegal repeated type application
      ...
      case typedFn =>
        typedFn.tpe.widen match
          case pt: PolyType =>
            if typedArgs.length <= pt.paramInfos.length && !isNamed then
              if typedFn.symbol == defn.Predef_classOf && typedArgs.nonEmpty
              then
                val arg = typedArgs.head
                if !arg.symbol.is(Module)
                then // Allow `classOf[Foo.type]` if `Foo` is an object
                  checkClassType(
                    arg.tpe,
                    arg.srcPos,
                    traitReq = false,
                    stablePrefixReq = false
                  )
          case _ =>
        def tryDynamicTypeApply(): Tree = typedFn match
          case typedFn: Select if !pt.isInstanceOf[FunProto] =>
            typedDynamicSelect(typedFn, typedArgs.map(untpd.TypedSplice(_)), pt)
          case _ => tree.withType(TryDynamicCallType)
        if typedFn.tpe eq TryDynamicCallType then tryDynamicTypeApply()
        else
          assignType(
            cpy.TypeApply(tree)(typedFn, typedArgs),
            typedFn,
            typedArgs
          )
    end match
  ...
  /** Rewrite `new Array[T](....)` if T is an unbounded generic to calls to
    * newGenericArray. It is performed during typer as creation of generic
    * arrays needs a classTag. we rely on implicit search to find one.
    */
  def convertNewGenericArray(tree: Tree)(using Context): Tree = tree match
    case Apply(TypeApply(tycon, targs @ (targ :: Nil)), args)
        if tycon.symbol == defn.ArrayConstructor =>
      ...
      if TypeErasure.isGeneric(targ.tpe) then newGenericArrayCall
      else tree
    case _ =>
      tree

  /** Is `tp` a unary function type or an overloaded type with with only unary
    * function types as alternatives?
    */
  ...
  /** Should we tuple or untuple the argument before application? If
    * auto-tupling is enabled then
    *
    *   - we tuple n-ary arguments where n > 0 if the function consists only of
    *     unary alternatives
    *   - we untuple tuple arguments of infix operations if the function does
    *     not consist only of unary alternatives.
    */
  ...
  /** If `tree` is a complete application of a compiler-generated `apply` or
    * `copy` method of an enum case, widen its type to the underlying type by
    * means of a type ascription, as long as the widened type is still
    * compatible with the expected type. The underlying type is the intersection
    * of all class parents of the original type.
    */
  ...
  /** Does `state` contain a "NotAMember" or "MissingIdent" message as first
    * pending error message? That message would be `$memberName is not a member
    * of ...` or `Not found: $memberName`. If memberName is empty, any name will
    * do.
    */
  def saysNotFound(state: TyperState, memberName: Name)(using
      Context
  ): Boolean =
  ...
  def typedUnApply(tree: untpd.Apply, selType: Type)(using Context): Tree =
    ...
    def notAnExtractor(tree: Tree): Tree =
      ...
      if !tree.tpe.isError && tree.tpe.isErroneous then tree
      else errorTree(tree, NotAnExtractor(qual))

    /** Report errors buffered in state.
      * @pre
      *   state has errors to report If there is a single error stating that
      *   "unapply" is not a member, print the more informative "notAnExtractor"
      *   message instead.
      */
    ...
    /** If this is a term ref tree, try to typecheck with its type name. If this
      * refers to a type alias, follow the alias, and if one finds a class,
      * reference the class companion module.
      */
    def followTypeAlias(tree: untpd.Tree): untpd.Tree =
      tree match
        case tree: untpd.RefTree =>
          ...
          ttree.tpe match
            case alias: TypeRef
                if alias.info.isTypeAlias && !nestedCtx.reporter.hasErrors =>
              Inferencing.companionRef(alias) match
                case companion: TermRef =>
                  return untpd.ref(companion).withSpan(tree.span)
                case _ =>
            case _ =>
        case _ =>
      untpd.EmptyTree
    ...
    /** A typed qual.unapply or qual.unapplySeq tree, if this typechecks.
      * Otherwise fallBack with (maltyped) qual.unapply as argument Note:
      * requires special handling for overloaded occurrences of unapply or
      * unapplySeq. We first try to find a non-overloaded method which matches
      * any type. If that fails, we try to find an overloaded variant which
      * matches one of the argument types. In fact, overloaded unapply's are
      * problematic because a non- overloaded unapply does *not* need to be
      * applicable to its argument whereas overloaded variants need to have a
      * conforming variant.
      */
    def trySelectUnapply(qual: untpd.Tree)(
        fallBack: (Tree, TyperState) => Tree
    ): Tree =
      // try first for non-overloaded, then for overloaded occurrences
      def tryWithName(name: TermName)(fallBack: (Tree, TyperState) => Tree)(
          using Context
      ): Tree =

        def tryWithProto(qual: untpd.Tree, targs: List[Tree], pt: Type)(using
            Context
        ) =
          ...
          val result =
            if targs.isEmpty then typedExpr(unapp, proto)
              ...
              typedExpr(unapp, PolyProto(targs, proto)).appliedToTypeTrees(
                targs
              )
        ...
        def tryWithTypeArgs(qual: untpd.Tree, targs: List[Tree])(
            fallBack: (Tree, TyperState) => Tree
        ): Tree =
          tryEither {
      ...
      tryWithName(nme.unapply) { (sel, state) =>
        tryWithName(nme.unapplySeq) { (sel2, state2) =>
          // if both fail, return unapply error, unless that is simply a
          // "not a member", and the unapplySeq error is more refined.
          if saysNotFound(state, nme.unapply) && !saysNotFound(
              state2,
              nme.unapplySeq
            )
    ...
    end trySelectUnapply

    /** Produce a typed qual.unapply or qual.unapplySeq tree, or else if this
      * fails follow a type alias and try again.
      */
    var unapplyFn =
      trySelectUnapply(qual) { (sel, state) =>
        val qual1 = followTypeAlias(qual)
        if qual1.isEmpty then reportErrors(sel, state)
        else
          trySelectUnapply(qual1) { (_, state) =>
            reportErrors(sel, state)
          }
      }
    ...
    unapplyFn.tpe.widen match
      case mt: MethodType if mt.paramInfos.length == 1 =>
        ...
        val ownType =
          if selType <:< unapplyArgType then
            unapp.println(
              i"case 1 $unapplyArgType ${ctx.typerState.constraint}"
            )
            fullyDefinedType(unapplyArgType, "pattern selector", tree.srcPos)
            selType.dropAnnot(
              defn.UncheckedAnnot
            ) // need to drop @unchecked. Just because the selector is @unchecked, the pattern isn't.
          else
            ...
            withMode(Mode.GadtConstraintInference)(
              TypeComparer.constrainPatternType(unapplyArgType, selType)
            )
            val patternBound =
              maximizeType(unapplyArgType, unapplyFn.span.endPos)
            if patternBound.nonEmpty then
              unapplyFn = addBinders(unapplyFn, patternBound)
            unapp.println(
              i"case 2 $unapplyArgType ${ctx.typerState.constraint}"
            )
            unapplyArgType

        val dummyArg = dummyTreeOfType(ownType)
        val unapplyApp = typedExpr(
          untpd.TypedSplice(Apply(unapplyFn, dummyArg :: Nil))
        )
        def unapplyImplicits(unapp: Tree): List[Tree] =
          val res = List.newBuilder[Tree]
          def loop(unapp: Tree): Unit = unapp match
            case Apply(Apply(unapply, `dummyArg` :: Nil), args2) =>
              assert(args2.nonEmpty); res ++= args2
            case Apply(unapply, `dummyArg` :: Nil) =>
            case Inlined(u, _, _)                  => loop(u)
            case DynamicUnapply(_) =>
              report.error(
                em"Structural unapply is not supported",
                unapplyFn.srcPos
              )
            case Apply(fn, args) =>
              assert(args.nonEmpty); loop(fn); res ++= args
        ...
        for argType <- argTypes do
          assert(!isBounds(argType), unapplyApp.tpe.show)
        val bunchedArgs = argTypes match
          case argType :: Nil =>
            if args.lengthCompare(1) > 0 && Feature.autoTuplingEnabled && defn
                .isTupleNType(argType)
            then untpd.Tuple(args) :: Nil
            else args
          case _ => args
        if argTypes.length != bunchedArgs.length then
          report.error(
            UnapplyInvalidNumberOfArguments(qual, argTypes),
            tree.srcPos
          )
          argTypes = argTypes.take(args.length) ++
            List.fill(argTypes.length - args.length)(WildcardType)
        val unapplyPatterns = bunchedArgs.lazyZip(argTypes) map (typed(_, _))
        val result = assignType(
          cpy.UnApply(tree)(
            unapplyFn,
            unapplyImplicits(unapplyApp),
            unapplyPatterns
          ),
          ownType
        )
      ...
      case tp =>
        val unapplyErr =
          if tp.isError then unapplyFn else notAnExtractor(unapplyFn)
  ...
  /** Is given method reference applicable to argument trees `args`?
    ...
    *   The expected result type of the application
    */
  def isApplicableMethodRef(
      methRef: TermRef,
      args: List[Tree],
      resultType: Type,
      keepConstraint: Boolean,
      argMatch: ArgMatch
  )(using Context): Boolean =
    def isApp(using Context): Boolean =
      new ApplicableToTrees(methRef, args, resultType, argMatch).success
    if keepConstraint then isApp else explore(isApp)
  ...
  /** Is given type applicable to argument trees `args`, possibly after
    * inserting an `apply`?
    * @param resultType
    *   The expected result type of the application
    */
  def isApplicableType(
      tp: Type,
      args: List[Tree],
      resultType: Type,
      keepConstraint: Boolean
  ...
  /** Is given type applicable to argument types `args`, possibly after
    * inserting an `apply`?
    * @param resultType
    *   The expected result type of the application
    */
  def isApplicableType(tp: Type, args: List[Type], resultType: Type)(using
      Context
  ): Boolean =
    onMethod(tp, args.nonEmpty) {
  ...
  private def onMethod(tp: Type, followApply: Boolean)(p: TermRef => Boolean)(
      using Context
  ): Boolean = tp match
    case methRef: TermRef
        if methRef.widenSingleton.isInstanceOf[MethodicType] =>
      p(methRef)
    ...
    case _ =>
      followApply && tp
        .member(nme.apply)
        .hasAltWith(d => p(TermRef(tp, nme.apply, d)))

  /** Does `tp` have an extension method named `xname` with this-argument
    * `argType` and result matching `resultType`?
    */
  def hasExtensionMethodNamed(
      tp: Type,
      xname: TermName,
      argType: Type,
      resultType: Type
  ...
  def stripImplicit(tp: Type, wildcardOnly: Boolean = false)(using
      Context
  ): Type = tp match
    case mt: MethodType if mt.isImplicitMethod =>
  ...
  /** Compare owner inheritance level.
    ...
    *   The second owner
    * @return
    *   1 if `sym1` properly derives from `sym2`
    * -1 if `sym2` properly derives from `sym1` 0 otherwise Module classes also
    * inherit the relationship from their companions. This means, if no direct
    * derivation exists between `sym1` and `sym2` also perform the following
    * tests:
    ...
    *   - If sym1 is a module class with a companion, and sym2 is a normal class
    *     or trait, compare the companion with sym2.
    *
    * Condition (#) is necessary to make `compareOwner(_, _) > 0` a transitive
    * relation. For instance:
    *
    * class A extends B object A { given a ... } class B object B extends C {
    * given b ... } class C { given c }
    *
    * Then without (#), and taking A$ for the module class of A,
    * compareOwner(A$, B$) = 1 and compareOwner(B$, C) == 1, but
    * compareOwner(A$, C) == 0. Violating transitivity in this way is bad, since
    * it makes implicit search outcomes compilation order dependent. E.g. if we
    * compare `b` with `c` first, we pick `b`. Then, if we compare `a` and `b`,
    * we pick `a` as solution of the search. But if we start with comparing `a`
    * with `c`, we get an ambiguity.
    *
    * With the added condition (#), compareOwner(A$, B$) == 0. This means we get
    * an ambiguity between `a` and `b` in all cases.
    */
  def compareOwner(sym1: Symbol, sym2: Symbol)(using Context): Int =
    if sym1 == sym2 then 0
      ...
      if sym2.is(Module) then
        if sym2.thisType.implicitMembers.forall(_.symbol.owner == sym2)
        then // test for (#)
  ...
  /** Compare two alternatives of an overloaded call or an implicit search.
    ...
    * @return
    *   1 if 1st alternative is preferred over 2nd
    * -1 if 2nd alternative is preferred over 1st 0 if neither alternative is
    * preferred over the other
    *
    * Normal symbols are always preferred over constructor proxies. Otherwise,
    * an alternative A1 is preferred over an alternative A2 if it wins in a
    * tournament that awards one point for each of the following:
    *
    *   - A1's owner derives from A2's owner.
    *   - A1's type is more specific than A2's type.
    *
    * If that tournament yields a draw, a tiebreak is applied where an
    * alternative that takes more implicit parameters wins over one that takes
    * fewer.
    */
  def compare(alt1: TermRef, alt2: TermRef)(using Context): Int =
    trace(i"compare($alt1, $alt2)", overload) {
      ...
      /** Is alternative `alt1` with type `tp1` as specific as alternative
        * `alt2` with type `tp2` ?
        *
        *   1. A method `alt1` of type `(p1: T1, ..., pn: Tn)U` is as specific
        *      as `alt2` if `alt1` is nullary or `alt2` is applicable to
        *      arguments (p1, ..., pn) of types T1,...,Tn. If the last parameter
        *      `pn` has a vararg type T*, then `alt1` must be applicable to
        *      arbitrary numbers of `T` parameters (which implies that it must
        *      be a varargs method as well). 2. A polymorphic member of type [a1
        *      >: L1 <: U1, ..., an >: Ln <: Un]T is as specific as `alt2` of
        *      type `tp2` if T is as specific as `tp2` under the assumption that
        *      for i = 1,...,n each ai is an abstract type name bounded from
        *      below by Li and from above by Ui. 3. A member of any other type
        *      `tp1` is:
        *      a. always as specific as a method or a polymorphic method. b. as
        *         specific as a member of any other type `tp2` if `tp1` is
        *         compatible with `tp2`.
        */
      def isAsSpecific(
          alt1: TermRef,
          ...
          alt2: TermRef,
          tp2: Type
      ): Boolean = trace(i"isAsSpecific $tp1 $tp2", overload) {
        tp1 match
          case tp1: MethodType => // (1)
            ...
            || {
              if tp1.isVarArgsMethod then
                ...
                && isApplicableMethodRef(
                  alt2,
                  tp1.paramInfos.map(_.repeatedToSingle),
                  WildcardType,
                  ArgMatch.Compatible
                )
              else
          ...
          case tp1: PolyType => // (2)
            inContext(ctx.fresh.setExploreTyperState()) {
              ...
              // part of.
              val tp1Params =
                tp1.newLikeThis(tp1.paramNames, tp1.paramInfos, defn.AnyType)
              fullyDefinedType(
                tp1Params,
                "type parameters of alternative",
                alt1.symbol.srcPos
              )

              val tparams = newTypeParams(
                alt1.symbol,
                tp1.paramNames,
                EmptyFlags,
                tp1.instantiateParamInfos(_)
              )
              isAsSpecific(
                alt1,
                tp1.instantiate(tparams.map(_.typeRef)),
                alt2,
                tp2
              )
            }
          case _ => // (3)
            tp2 match
              ...
              case tp2: PolyType if tp2.resultType.isInstanceOf[MethodType] =>
                true // (3a)
              case tp2: PolyType => // (3b)
                explore(
                  isAsSpecificValueType(tp1, instantiateWithTypeVars(tp2))
                )
              case _ => // 3b)
      ...
      /** Test whether value type `tp1` is as specific as value type `tp2`.
        * Let's abbreviate this to `tp1 <:s tp2`. Previously, `<:s` was the same
        * as `<:`. This behavior is still available under mode
        * `Mode.OldOverloadingResolution`. The new behavior is different,
        * however. Here, `T <:s U` iff
        *
        * flip(T) <: flip(U)
        *
        * where `flip` changes covariant occurrences of contravariant type
        * parameters to covariant ones. Intuitively `<:s` means subtyping `<:`,
        * except that all arguments to contravariant parameters are compared as
        * if they were covariant. E.g. given class
        *
        * class Cmp[-X]
        *
        * `Cmp[T] <:s Cmp[U]` if `T <: U`. On the other hand, non-variant
        * occurrences of parameters are not affected. So `T <: U` would imply
        * `Set[Cmp[U]] <:s Set[Cmp[T]]`, as usual, because `Set` is non-variant.
        *
        * This relation might seem strange, but it models closely what happens
        * for methods. Indeed, if we integrate the existing rules for methods
        * into `<:s` we have now that
        *
        * (T)R <:s (U)R
        *
        * iff
            ...
            def apply(t: Type) = t match
              case t @ AppliedType(tycon, args) =>
                def mapArg(arg: Type, tparam: TypeParamInfo) =
                  if variance > 0 && tparam.paramVarianceSign < 0 then
                    defn.FunctionOf(arg :: Nil, defn.UnitType)
                  else arg
                mapOver(
                  t.derivedAppliedType(
                    tycon,
                    args.zipWithConserve(tycon.typeParams)(mapArg)
                  )
                )
              case _ => mapOver(t)
          def prepare(tp: Type) = tp.stripTypeVar match
            case tp: NamedType
                if tp.symbol.is(Module) && tp.symbol.sourceModule.is(Given) =>
      ...
      /** Widen the result type of synthetic given methods from the
        ...
        * This desugars to
        *
        * class I[X] extends T { ... } implicit def I[X]: I[X] = new I[X]
        *
        * To compare specificity we should compare with `T`, not with its
        * implementation `I[X]`. No such widening is performed for given
        * aliases, which are not synthetic. E.g.
        *
        * given J[X]: T = rhs
        *
        * already has the right result type `T`. Neither is widening performed
        * for given objects, since these are anyway taken to be more specific
        * than methods (by condition 3a above).
        */
      def widenGiven(tp: Type, alt: TermRef): Type = tp match
        case mt: MethodType if mt.isImplicitMethod =>
          mt.derivedLambdaType(
            mt.paramNames,
            mt.paramInfos,
            widenGiven(mt.resultType, alt)
          )
        case pt: PolyType =>
          pt.derivedLambdaType(
            pt.paramNames,
            pt.paramInfos,
            widenGiven(pt.resultType, alt)
          )
        case rt =>
      ...
      def compareWithTypes(tp1: Type, tp2: Type) =
        val ownerScore =
          compareOwner(alt1.symbol.maybeOwner, alt2.symbol.maybeOwner)
        def winsType1 = isAsSpecific(alt1, tp1, alt2, tp2)
        def winsType2 = isAsSpecific(alt2, tp2, alt1, tp1)

        overload.println(
          i"compare($alt1, $alt2)? $tp1 $tp2 $ownerScore $winsType1 $winsType2"
        )
        if ownerScore == 1 then if winsType1 || !winsType2 then 1 else 0
        else if ownerScore == -1 then if winsType2 || !winsType1 then -1 else 0
        else if winsType1 then if winsType2 then 0 else 1
        else if winsType2 then -1
        else 0

      if alt1.symbol.is(ConstructorProxy) && !alt2.symbol.is(ConstructorProxy)
      then -1
      else if alt2.symbol.is(ConstructorProxy) && !alt1.symbol.is(
          ConstructorProxy
        )
      then 1
      else
        ...
        if result != 0 then result
          ...
          if strippedType2 eq fullType2 then
            0 // no implicits either side: its' a draw
          else 1 // prefer 1st alternative with no implicits
        else if strippedType2 eq fullType2 then
          -1 // prefer 2nd alternative with no implicits
        else
          compareWithTypes(
            fullType1,
            fullType2
          ) // continue by comparing implicits parameters
    }
  end compare
  ...
  def narrowMostSpecific(alts: List[TermRef])(using Context): List[TermRef] =
    ...
    alts match
      ...
      case alt :: alts1 =>
        def survivors(
            previous: List[TermRef],
            alts: List[TermRef]
        ): List[TermRef] = alts match
          case alt :: alts1 =>
        ...
        def asGood(alts: List[TermRef]): List[TermRef] = alts match
          case alt :: alts1 =>
            if compare(alt, best) < 0 then asGood(alts1)
            else alt :: asGood(alts1)
          case nil =>
            Nil
        best :: asGood(rest)
  end narrowMostSpecific

  /** Resolve overloaded alternative `alts`, given expected type `pt`. Two
    * trials: First, without implicits or SAM conversions enabled. Then, if the
    * first finds no eligible candidates, with implicits and SAM conversions
    * enabled.
    */
  def resolveOverloaded(alts: List[TermRef], pt: Type)(using
      Context
  ): List[TermRef] =
    record("resolveOverloaded")

    /** Is `alt` a method or polytype whose result type after the first value
      * parameter section conforms to the expected type `resultType`? If
      * `resultType` is a `IgnoredProto`, pick the underlying type instead.
      */
    def resultConforms(altSym: Symbol, altType: Type, resultType: Type)(using
        Context
    ): Boolean =
      resultType.revealIgnored match
        case resultType: ValueType =>
          altType.widen match
            case tp: PolyType =>
              resultConforms(altSym, instantiateWithTypeVars(tp), resultType)
            case tp: MethodType =>
              constrainResult(altSym, tp.resultType, resultType)
            case _ => true
        case _ => true

    /** If the `chosen` alternative has a result type incompatible with the
      * expected result type `pt`, run overloading resolution again on all
      * alternatives that do match `pt`. If the latter succeeds with a single
      * alternative, return it, otherwise fallback to `chosen`.
      *
      * Note this order of events is done for speed. One might be tempted to
      * preselect alternatives by result type. But this is slower, because it
      * discriminates less. The idea is when searching for a best solution, as
      * is the case in overloading resolution, we should first try criteria
      * which are cheap and which have a high probability of pruning the search.
      * result type comparisons are neither cheap nor do they prune much, on
      * average.
      */
    def adaptByResult(chosen: TermRef, alts: List[TermRef]) = pt match
      case pt: FunProto
          if !explore(resultConforms(chosen.symbol, chosen, pt.resultType)) =>
        val conformingAlts = alts.filterConserve(alt =>
          (alt ne chosen) && explore(
            resultConforms(alt.symbol, alt, pt.resultType)
          )
        )
    ...
    def resolve(alts: List[TermRef]): List[TermRef] =
      ...
      var found =
        withoutMode(Mode.ImplicitsEnabled)(resolveOverloaded1(alts, pt))
    ...
    /** Try an apply method, if
      *   - the result is applied to value arguments and alternative is not a
      *     method, or
      *   - the result is applied to type arguments and alternative is not
      *     polymorphic
      */
    val tryApply: Type => Boolean = alt =>
      pt match
    ...
    def retract(alt: TermRef): TermRef =
      if alt.name == nme.apply && !alts.contains(alt) then
        alts.find(_.symbol == alt.prefix.termSymbol).getOrElse(alt)
      else alt

    if alts.exists(tryApply) then
  ...
  /** This private version of `resolveOverloaded` does the bulk of the work of
    * overloading resolution, but does neither result adaptation nor apply
    * insertion. It might be called twice from the public `resolveOverloaded`
    * method, once with implicits and SAM conversions enabled, and once without.
    */
  private def resolveOverloaded1(alts: List[TermRef], pt: Type)(using
      Context
  ): List[TermRef] =
    trace(i"resolve over $alts%, %, pt = $pt", typr, show = true) {
      ...
      def typeShape(tree: untpd.Tree): Type = tree match
        ...
        case Match(EmptyTree, _) =>
          defn.PartialFunctionClass.typeRef.appliedTo(
            defn.AnyType :: defn.NothingType :: Nil
          )
        case _ =>
          defn.NothingType

      /** The shape of given tree as a type; is more expensive than typeShape
        * but can can handle named arguments.
        */
      ...
      def narrowByTypes(
        ...
        alts.filterConserve(
          isApplicableMethodRef(_, argTypes, resultType, ArgMatch.CompatibleCAP)
        )

      /** Normalization steps before checking arguments:
        *
        * { expr } --> expr (x1, ..., xn) => expr --> ((x1, ..., xn)) => expr if
        * n != 1, no alternative has a corresponding formal parameter that is an
        * n-ary function, and at least one alternative has a corresponding
        * formal parameter that is a unary function.
        */
      def normArg(alts: List[TermRef], arg: untpd.Tree, idx: Int): untpd.Tree =
        arg match
      ...
      val candidates = pt match
        case pt @ FunProto(args, resultType) =>
            ...
            case tp: MethodType =>
              ...
              def numDefaultParams =
                if alt.symbol.hasDefaultParams then
                  ...
                  ptypes.indices.count(n =>
                    !findDefaultGetter(fn, n, testOnly = true).isEmpty
                  )
                else 0
          ...
          def narrowByShapes(alts: List[TermRef]): List[TermRef] =
            if args.exists(untpd.isFunctionWithUnknownParamType) then
              ...
              if hasNamedArg(args) then
                narrowByTrees(alts, normArgs.map(treeShape), resultType)
          ...
          overload.println(
            i"narrowed by size: ${alts1.map(_.symbol.showDcl)}%, %"
          )
          if isDetermined(alts1) then alts1
            ...
            overload.println(
              i"narrowed by shape: ${alts2.map(_.symbol.showDcl)}%, %"
            )
            if isDetermined(alts2) then alts2
              ...
              narrowByTrees(
                alts2,
                pt.typedArgs(normArg(alts2, _, _)),
                resultType
              )
        ...
        case pt =>
          val compat = alts.filterConserve(
            normalizedCompatible(_, pt, keepConstraint = false)
          )
            ...
            pt match
              ...
              case _ =>
                ...
                val convertible = alts.filterNot(alt =>
                  normalize(alt, IgnoredProto(pt)).widenSingleton
      ...
      /** The type of alternative `alt` after instantiating its first parameter
        * clause with `argTypes`. In addition, if the resulting type is a
        * PolyType and `typeArgs` matches its parameter list, instantiate the
        * result with `typeArgs`.
        */
      def skipParamClause(argTypes: List[Type], typeArgs: List[Type])(
          alt: TermRef
      ): Type =
      ...
      def resultIsMethod(tp: Type): Boolean = tp.widen.stripPoly match
        case tp: MethodType =>
          stripInferrable(tp.resultType).isInstanceOf[MethodType]
        case _ => false
        ...
        if found.length <= 1 then found
          ...
          deepPt match
            case pt @ FunProto(_, PolyProto(targs, resType)) =>
              ...
              resolveMapped(
                candidates,
                skipParamClause(pt.typedArgs().tpes, targs.tpes),
                resType
              )
            case pt @ FunProto(_, resType: FunOrPolyProto) =>
              ...
              resolveMapped(
                candidates,
                skipParamClause(pt.typedArgs().tpes, Nil),
                resType
              )
            case _ =>
              ...
              if noCurriedCount == 1 then noCurried
                ...
                if noDefaultsCount == 1 then noDefaults
                else if noDefaultsCount > 1 && noDefaultsCount < alts.length
                then resolveOverloaded1(noDefaults, pt)
  ...
  /** The largest suffix of `paramss` that has the same first parameter name as
    * `t`, plus the number of term parameters in `paramss` that come before that
    * suffix.
    */
  def trimParamss(t: Type, paramss: List[List[Symbol]])(using
      Context
  ): (List[List[Symbol]], Int) = t match
    case MethodType(Nil) => trimParamss(t.resultType, paramss)
    case t: MethodOrPoly =>
      val firstParamName = t.paramNames.head
      def recur(
          pss: List[List[Symbol]],
          skipped: Int
      ): (List[List[Symbol]], Int) =
        (pss: @unchecked) match
          case (ps @ (p :: _)) :: pss1 =>
            if p.name == firstParamName then (pss, skipped)
              ...
              recur(
                pss1,
                if p.name.isTermName then skipped + ps.length else skipped
              )
          case Nil =>
            (pss, skipped)
      recur(paramss, 0)
  ...
  /** Resolve overloading by mapping to a different problem where each
    * alternative's type is mapped with `f`, alternatives with non-existing
    * types are dropped, and the expected type is `pt`. Map the results back to
    * the original alternatives.
    */
  def resolveMapped(alts: List[TermRef], f: TermRef => Type, pt: Type)(using
      Context
  ): List[TermRef] =
    val reverseMapping = alts.flatMap { alt =>
      ...
      if t.exists then
        ...
        mappedSym.addAnnotation(
          Annotation(
            TypeTree(
              defn.MappedAlternativeAnnot.typeRef
                .appliedTo(pre, ConstantType(Constant(totalSkipped)))
            )
          )
        )
  ...
  /** Try to typecheck any arguments in `pt` that are function values missing a
    * parameter type. If the formal parameter types corresponding to a closure
    * argument all agree on their argument types, typecheck the argument with an
    * expected function or partial function type that contains these argument
    * types, The result of the typecheck is stored in `pt`, to be retrieved when
    * its `typedArgs` are selected. The benefit of doing this is to allow idioms
    * like this:
    *
    * def map(f: Char => Char): String = ??? def map[U](f: Char => U): Seq[U] =
    * ??? map(x => x.toUpper)
    *
    ...
    */
  private def pretypeArgs(alts: List[TermRef], pt: FunProto)(using
      Context
  ): Unit =
    def recur(altFormals: List[List[Type]], args: List[untpd.Tree]): Unit =
      args match
        case arg :: args1 if !altFormals.exists(_.isEmpty) =>
          def isUniform[T](xs: List[T])(p: (T, T) => Boolean) =
            xs.forall(p(_, xs.head))
          ...
          then
            val commonParamTypes = formalParamTypessForArg.transpose.map(ps =>
              ...
              if isUniform(ps)(_ frozen_=:= _) then ps.head
              else WildcardType
            )
            ...
            val commonFormal =
              if isPartial then
                defn.PartialFunctionOf(commonParamTypes.head, WildcardType)
              else
                defn.FunctionOf(
                  commonParamTypes,
                  WildcardType,
                  isContextual = untpd.isContextualClosure(arg)
                )
            overload.println(
              i"pretype arg $arg with expected type $commonFormal"
            )
            if commonParamTypes.forall(
                isFullyDefined(_, ForceDegree.flipBottom)
              )
            then
  ...
  private def harmonizeWith[T <: AnyRef](
      ts: List[T]
  )(tpe: T => Type, adapt: (T, Type) => T)(using Context): List[T] =
    def targetClass(ts: List[T], cls: Symbol, intLitSeen: Boolean): Symbol =
      ts match
        case t :: ts1 =>
          tpe(t).widenTermRefExpr match
            ...
            case t =>
              ...
              if !sym.isNumericValueClass || cls.exists && cls != sym then
                NoSymbol
              else targetClass(ts1, sym, intLitSeen)
        case Nil =>
          if cls != defn.IntClass && intLitSeen then cls else NoSymbol
    ...
    if cls.exists then
      ...
      val ts1 = ts.mapConserve { t =>
        tpe(t).widenTermRefExpr match
          case ConstantType(c: Constant) if c.tag == IntTag =>
            canAdapt &= c.convertTo(cls.typeRef) != null && !lossOfPrecision(
              c.intValue
            )
            if canAdapt then adapt(t, cls.typeRef) else t
          case _ => t
      }
      if canAdapt then ts1 else ts
    else ts
  end harmonizeWith

  /** If `trees` all have numeric value types, and they do not have all the same
    * type, pick a common numeric supertype and try to convert all constant Int
    * literals to this type. If the resulting trees all have the same type,
    * return them instead of the original ones.
    */
  def harmonize(trees: List[Tree])(using Context): List[Tree] =
    def adaptDeep(tree: Tree, pt: Type): Tree = tree match
      case cdef: CaseDef =>
        tpd.cpy.CaseDef(cdef)(body = adaptDeep(cdef.body, pt))
      case _ => adapt(tree, pt)
    if ctx.isAfterTyper then trees else harmonizeWith(trees)(_.tpe, adaptDeep)

  /** Apply a transformation `harmonize` on the results of operation `op`,
    * unless the expected type `pt` is fully defined. If the result is different
    * (wrt eq) from the original results of `op`, revert back to the constraint
    * in force before computing `op`. This reset is needed because otherwise the
    * original results might have added constraints to type parameters which are
    * no longer implied after harmonization. No essential constraints are lost
    * by this because the result of harmonization will be compared again with
    * the expected type. Test cases where this matters are in
    * neg/harmomize.scala and run/weak-conformance.scala.
    *
    * Note: this assumes that the internal typing of the arguments using `op`
    * does not leave any constraints, so the only info that is reset is the
    * relationship between the argument's types and the expected type. I am not
    * sure this will always be the case. If that property does not hold, we risk
    * forgetting constraints which could lead to unsoundness.
    */
  def harmonic[T](harmonize: List[T] => List[T], pt: Type)(
      op: => List[T]
  )(using Context): List[T] =
    if !isFullyDefined(pt, ForceDegree.none) then
      ...
      if harmonizedElems ne origElems then
        ctx.typerState.constraint = origConstraint
      harmonizedElems
    else op

  /** If all `types` are numeric value types, and they are not all the same
    * type, pick a common numeric supertype and widen any constant types in
    * `tpes` to it. If the resulting types are all the same, return them instead
  ...
  /** The typed application
    *
    * <methodRef>(<receiver>) or <methodRef>[<type-args>](<receiver>)
    *
    * where <type-args> comes from `pt` if it is a (possibly ignored) PolyProto.
    */
  def extMethodApply(methodRef: untpd.Tree, receiver: Tree, pt: Type)(using
      Context
  ): Tree =
    /** Integrate the type arguments (if any) from `currentPt` into `tree`, and
      * produce an expected type that hides the appropriate amount of
      * information through IgnoreProto.
      */
    def normalizePt(tree: untpd.Tree, currentPt: Type): (untpd.Tree, Type) =
      currentPt match
    ...
    withMode(Mode.SynthesizeExtMethodReceiver) {
      typed(
        untpd.Apply(
          core,
          untpd.TypedSplice(receiver, isExtensionReceiver = true) :: Nil
        ),
        pt1,
        ctx.typerState.ownedVars
      )
  ...
  /** Assuming methodRef is a reference to an extension method defined e.g. as
    *
    * extension [T1, T2](using A)(using B, C)(receiver: R)(using D) def
    * foo[T3](using E)(f: F): G = ???
    *
    * return the tree representing methodRef partially applied to the receiver
    * and all the implicit parameters preceding it (A, B, C) with the type
    * parameters of the extension (T1, T2) inferred. None is returned if the
    * implicit search fails for any of the leading implicit parameters or if the
    * receiver has a wrong type (note that in general the type of the receiver
    * might depend on the exact types of the found instances of the proceeding
    * implicits). No implicit search is tried for implicits following the
    * receiver or for parameters of the def (D, E).
    */
  def tryApplyingExtensionMethod(methodRef: TermRef, receiver: Tree)(using
      Context
  ): Option[Tree] =
    ...
    def truncateExtension(tp: Type)(using Context): Type = tp match
      case poly: PolyType =>
        poly.newLikeThis(
          poly.paramNames,
          poly.paramInfos,
          truncateExtension(poly.resType)
        )
      case meth: MethodType if meth.isContextualMethod =>
        meth.newLikeThis(
          meth.paramNames,
          meth.paramInfos,
          truncateExtension(meth.resType)
        )
      case meth: MethodType =>
        meth.newLikeThis(meth.paramNames, meth.paramInfos, defn.AnyType)

    def replaceCallee(inTree: Tree, replacement: Tree)(using Context): Tree =
      inTree match
        case Apply(fun, args) => Apply(replaceCallee(fun, replacement), args)
        case TypeApply(fun, args) =>
          TypeApply(replaceCallee(fun, replacement), args)
        case _ => replacement

    val methodRefTree = ref(methodRef, needLoad = false)
    val truncatedSym =
      methodRef.symbol.asTerm.copy(info = truncateExtension(methodRef.info))
    val truncatedRefTree =
      untpd.TypedSplice(ref(truncatedSym)).withSpan(receiver.span)
    val newCtx = ctx.fresh.setNewScope.setReporter(
      new reporting.ThrowingReporter(ctx.reporter)
    )
    ...
    catch case ex: UnhandledError => None
  end tryApplyingExtensionMethod

  def isApplicableExtensionMethod(methodRef: TermRef, receiverType: Type)(using
      Context
  ): Boolean =
    methodRef.symbol.is(ExtensionMethod) && !receiverType.isBottomType &&
      tryApplyingExtensionMethod(
        methodRef,
        nullLiteral.asInstance(receiverType)
      ).nonEmpty
