...
object ConstFold:
  ...
  private def foldUnop(op: Name, x: Constant): Constant | Null =
    (op, x.tag) match
  ...
  private def foldSubrangeOp(op: Name, x: Constant, y: Constant): Constant |
    ...
    case _       => null
  private def foldLongOp(op: Name, x: Constant, y: Constant): Constant | Null =
    op match
      case nme.OR  => Constant(x.longValue | y.longValue)
      case nme.XOR => Constant(x.longValue ^ y.longValue)
      case nme.AND => Constant(x.longValue & y.longValue)
      case nme.LSL =>
        if x.tag <= IntTag then Constant(x.intValue << y.longValue.toInt)
        else Constant(x.longValue << y.longValue)
      case nme.LSR =>
        if x.tag <= IntTag then Constant(x.intValue >>> y.longValue.toInt)
        else Constant(x.longValue >>> y.longValue)
      case nme.ASR =>
      ...
      case _       => null
  private def foldFloatOp(op: Name, x: Constant, y: Constant): Constant | Null =
    op match
  ...
  private def foldDoubleOp(op: Name, x: Constant, y: Constant): Constant |
    ...
    case _       => null
  private def foldStringOp(op: Name, x: Constant, y: Constant): Constant |
    Null = op match
  ...
  private def foldBinop(op: Name, x: Constant, y: Constant): Constant | Null =
    val optag =
      if x.tag == y.tag then x.tag
      else if x.isNumeric && y.isNumeric then math.max(x.tag, y.tag)
      else if x.tag == NullTag || y.tag == NullTag then NullTag
      else NoTag
    ...
    try
      ...
      case ex: ArithmeticException => null // the code will crash at runtime,
