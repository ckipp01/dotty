...
/** A class that handles argument lifting. Argument lifting is needed in the
  * following scenarios:
  *   - eta expansion
  *   - applications with default arguments
  *   - applications with out-of-order named arguments Lifting generally lifts
  *     impure expressions only, except in the case of possible default
  *     arguments, where we lift also complex pure expressions, since in that
  *     case arguments can be duplicated as arguments to default argument
  *     methods.
  */
abstract class Lifter:
  ...
  protected def liftedDef(sym: TermSymbol, rhs: Tree)(using
      Context
  ): MemberDef = ValDef(sym, rhs)
  ...
  private def lift(
    ...
    if noLift(expr) then expr
      ...
      if liftedFlags.is(Method) then liftedType = ExprType(liftedType)
      val lifted = newSymbol(
        ctx.owner,
        name,
        liftedFlags | Synthetic,
        liftedType,
  ...
  /** Lift out common part of lhs tree taking part in an operator assignment
    ...
    * lhs += expr
    */
  def liftAssigned(defs: mutable.ListBuffer[Tree], tree: Tree)(using
      Context
  ): Tree = tree match
    case Apply(MaybePoly(fn @ Select(pre, name), targs), args) =>
      cpy.Apply(tree)(
        cpy.Select(fn)(lift(defs, pre), name).appliedToTypeTrees(targs),
  ...
  private def liftArg(
    ...
    arg match
      case arg @ NamedArg(name, arg1) =>
        cpy.NamedArg(arg)(name, lift(defs, arg1, prefix))
      case arg => lift(defs, arg, prefix)

  /** Lift arguments that are not-idempotent into ValDefs in buffer `defs` and
    * replace by the idents of so created ValDefs.
    */
  def liftArgs(defs: mutable.ListBuffer[Tree], methRef: Type, args: List[Tree])(
      using Context
  ): List[Tree] =
    methRef.widen match
      case mt: MethodType =>
        args.lazyZip(mt.paramNames).lazyZip(mt.paramInfos).map {
          (arg, name, tp) =>
            if tp.hasAnnotation(defn.InlineParamAnnot) then arg
            else
              val lifter =
                if tp.isInstanceOf[ExprType] then exprLifter else this
              lifter.liftArg(
                defs,
                arg,
  ...
  /** Lift out function prefix and all arguments from application
    ...
    */
  def liftApp(defs: mutable.ListBuffer[Tree], tree: Tree)(using Context): Tree =
    tree match
      case Apply(fn, args) =>
        ...
        if isErased then
          untpd.cpy
            .Apply(tree)(fn1, args1)
            .withType(tree.tpe) // application may be partial
  ...
  /** Lift prefix `pre` of an application `pre.f(...)` to
    ...
    */
  def liftPrefix(defs: mutable.ListBuffer[Tree], tree: Tree)(using
      Context
  ): Tree =
    if isIdempotentExpr(tree) then tree else lift(defs, tree)
end Lifter
...
object LiftCoverage extends LiftImpure:
  ...
  /** Variant of `noLift` for the arguments of applications. To produce the
    * right coverage information (especially in case of exceptions), we must
    * lift:
  ...
  private def noLiftArg(arg: tpd.Tree)(using Context): Boolean =
    arg match
      case a: tpd.Apply =>
        a.symbol.is(
          Erased
        ) // don't lift erased applications, but lift all others
  ...
  def liftForCoverage(defs: mutable.ListBuffer[tpd.Tree], tree: tpd.Apply)(using
      Context
  ) =
    val liftedFun = liftApp(defs, tree.fun)
    val liftedArgs =
      liftArgs(defs, tree.fun.tpe, tree.args)(using liftingArgsContext)
...
object LiftToDefs extends LiftComplex:
  ...
  override def liftedDef(sym: TermSymbol, rhs: tpd.Tree)(using
      Context
  ): tpd.DefDef = tpd.DefDef(sym, rhs)

/** Lifter for eta expansion */
object EtaExpansion extends LiftImpure:
  import tpd.*

  /** Eta-expanding a tree means converting a method reference to a function
    * value.
    * @param tree
    *   The tree to expand
    * @param mt
    ...
    * @param xarity
    *   The arity of the expected function type and assume the lifted
    *   application of `tree` (@see liftApp) is
    *
    * { val xs = es; expr }
    *
    ...
    *
    * where `T1, ..., Tn` are the paremeter types of the expanded method. If
    * `expr` has implicit function type, the arguments are passed with `given`.
    * E.g. for (1):
    *
    * { val xs = es; (x1, ..., xn) => expr(using x1, ..., xn) }
    *
    * Case (3) applies if the method is curried, i.e. its result type is again a
    * method type. Case (2) applies if the expected arity of the function type
    * `xarity` differs from the number of parameters in `mt`. Case (1) applies
    * if `mt` is uncurried and its number of parameters equals `xarity`. In this
    * case we can always infer the parameter types later from the callee even if
    * parameter types could not be inferred from the expected type. Hence, we
    * lose nothing by omitting parameter types in the eta expansion. On the
    * other hand omitting these parameters keeps the possibility open that
    * different parameters are inferred from the expected type, so we keep more
    * options open.
    *
    * In each case, the result is an untyped tree, with `es` and `expr` as typed
    * splices.
    *
    * F[V](x) ==> (x => F[X])
    *
    ...
    * def m(x: => T): T
    *
    * to a value of type (=> T) => T. This type cannot be written in source,
    * since by-name types => T are not legal argument types.
    *
    * It would be simpler to not allow to eta expand by-name methods. That was
    * the rule initially, but at some point, the rule was dropped. Enforcing the
    * restriction again now would break existing code. Allowing by-name
    * parameters in function types seems to be OK. After elimByName they are all
    * converted to regular function types anyway. But see comment on the
    * `ExprType` case in function `prune` in class `ConstraintHandling`.
    */
  def etaExpand(tree: Tree, mt: MethodType, xarity: Int)(using
      Context
  ): untpd.Tree =
    import untpd.*
    assert(
      !ctx.isAfterTyper || (ctx.phase eq ctx.base.inliningPhase),
      ctx.phase
    )
    ...
    val paramTypes: List[Tree] =
      if isLastApplication && mt.paramInfos.length == xarity then
        mt.paramInfos map (_ => TypeTree())
      else mt.paramInfos map TypeTree
    var paramFlag = SyntheticParam
    if mt.isContextualMethod then paramFlag |= Given
    else if mt.isImplicitMethod then paramFlag |= Implicit
    val params = mt.paramNames
      .lazyZip(paramTypes)
      .map((name, tpe) =>
        ValDef(name, tpe, EmptyTree)
          .withFlags(paramFlag)
          .withSpan(tree.span.startPos)
      )
    var ids: List[Tree] =
      mt.paramNames map (name => Ident(name).withSpan(tree.span.startPos))
    if mt.paramInfos.nonEmpty && mt.paramInfos.last.isRepeatedParam then
      ids = ids.init :+ repeated(ids.last)
    val body = Apply(lifted, ids)
    if mt.isContextualMethod then body.setApplyKind(ApplyKind.Using)
    val fn =
      if mt.isContextualMethod then
        new untpd.FunctionWithMods(
          params,
          body,
          Modifiers(Given),
          mt.erasedParams
        )
      else if mt.isImplicitMethod then
        new untpd.FunctionWithMods(
          params,
          body,
          Modifiers(Implicit),
      ...
      else if mt.hasErasedParams then
        new untpd.FunctionWithMods(params, body, Modifiers(), mt.erasedParams)
      else untpd.Function(params, body)
    if defs.nonEmpty then
      untpd.Block(defs.toList map (untpd.TypedSplice(_)), fn)
    else fn
  end etaExpand
end EtaExpansion
