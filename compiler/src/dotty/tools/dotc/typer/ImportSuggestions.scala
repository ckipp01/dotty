...
/** This trait defines the method `importSuggestionAddendum` that adds an
  * addendum to error messages suggesting additional imports.
  */
trait ImportSuggestions:
  ...
  /** A list of TermRefs referring to the roots where suggestions for imports of
    * givens or extension methods that might fix a type error are searched.
    *
    ...
    *   - any object or package from which something is imported in an enclosing
    *     scope,
    *   - any package that is nested in a searched package, provided the package
    *     was accessed in some way previously.
    *
    * Excluded from the root set are:
    *
    *   - Objects that contain `$`s in their name. These have to be omitted
    *     since they might be inner Java class files which cannot be read by the
    *     ClassfileParser without crashing.
    *   - Any members of static parts of Java classes.
    *   - Any members of the empty package. These should be skipped since the
    *     empty package often contains unrelated junk files that should not be
    *     used for suggestions.
    *   - Any members of the java or java.lang packages. These are skipped as an
    *     optimization, since they won't contain implicits anyway.
    */
  private def suggestionRoots(using Context) =
      ...
      val nested =
        if refSym.is(Package) then
          ...
          if !refSym.is(Touched) then
            refSym
              .ensureCompleted() // JavaDefined is reliably known only after completion
    ...
    def rootsIn(ref: TermRef)(using Context): List[TermRef] =
      if seen.contains(ref) then Nil
        ...
        implicitsDetailed.println(
          i"search for suggestions in ${ref.symbol.fullName}"
        )
        seen += ref
      ...
      if ctx.owner.exists then
        ...
        val imported =
          if ctx.importInfo eqn ctx.outer.importInfo then Nil
            ...
            ctx.importInfo.nn.importSym.info match
  ...
  /** Given an expected type `pt`, return two lists of TermRefs:
    *
    *   1. The _fully matching_ given instances that can be completed to a full
    *      synthesized given term that matches the expected type `pt`.
    *
    * 2. The _head matching_ given instances, that conform to the expected type
    * `pt`, ignoring any dependent implicit arguments.
    *
    * If there are no fully matching given instances under (1), and `pt` is a
    * view prototype of a selection of the form `T ?=>? { name: ... }`, return
    * instead a list of all possible references to extension methods named
    * `name` that are applicable to `T`.
    */
  private def importSuggestions(pt: Type)(using
      Context
  ): (List[TermRef], List[TermRef]) =
    val timer = new Timer()
    val allotted = ctx.run.nn.importSuggestionBudget
    if allotted <= 1 then return (Nil, Nil)
    implicits.println(
      i"looking for import suggestions, timeout = ${allotted}ms"
    )
    ...
    /** Test whether the head of a given instance matches the expected type
      * `pt`, ignoring any dependent implicit arguments.
      */
    ...
    /** Test whether a full given term can be synthesized that matches the
      * expected type `pt`.
      */
      ...
      System.currentTimeMillis < deadLine
        && {
          val task = new TimerTask:
            def run() =
              println(
                i"Cancelling test of $ref when making suggestions for error in ${ctx.source}"
              )
          ...
          try
            ...
            typedImplicit(candidate, expectedType, argument, span)(using
              testContext()
            ).isSuccess
          finally
            val run = ctx.run.nn
            if task.cancel() then // timer task has not run yet
              ...
              while !run.isCancelled do
                (
              ) // wait until timer task has run to completion
              run.isCancelled = false
        }
    end deepTest

    /** Optionally, an extension method reference `site.name` that is applicable
      * to `argType`.
      */
    def extensionMethod(
        site: TermRef,
    ...
    try
      ...
      timer.cancel()
      reduceTimeBudget(
        ((System.currentTimeMillis() - start) min Int.MaxValue).toInt
      )
    end try
  end importSuggestions

  /** Reduce next timeout for import suggestions by the amount of time it took
    * for current search, but but never less than to half of the previous
    * budget.
    */
  ...
  extension (refs: List[(TermRef, String)])
    def distinctRefs(using Context): List[TermRef] =
  ...
  /** The best `n` references in `refs`, according to `compare` `compare` is a
    * partial order. If there's a tie, we take elements in the order thy appear
    * in the list.
    */
  ...
  /** An addendum to an error message where the error might be fixed by some
    * implicit value of type `pt` that is however not found. The addendum
    * suggests given imports that might fix the problem. If there's nothing to
    * suggest, an empty string is returned.
    */
  override def importSuggestionAddendum(pt: Type)(using Context): String =
    ...
    implicits.println(
      i"suggestions for $pt in ${ctx.owner} = ($fullMatches%, %, $headMatches%, %)"
    )
    ...
    def importString(ref: TermRef): String =
      val imported =
        ...
        else ctx.printer.toTextRef(ref).show
      s"  import $imported"
    val suggestions = suggestedRefs
      ...
      .best(
        MaxSuggestions
      ) // take MaxSuggestions best references according to specificity
      .map(importString)
