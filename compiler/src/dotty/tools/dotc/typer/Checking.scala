...
object Checking:
  ...
  /** Add further information for error messages involving applied types if the
    * type is inferred:
    *   1. the full inferred type is a TypeTree node 2. the applied type causing
    *      the error, if different from (1)
    */
  private def showInferred(msg: Message, app: Type, tpt: Tree)(using
      Context
  ): Message =
    if tpt.isInstanceOf[TypeTree] then
      def subPart = if app eq tpt.tpe then "" else i" subpart $app of"
      msg
        .append(i" in$subPart inferred type ${tpt}")
        .appendExplanation("\n\nTo fix the problem, provide an explicit type.")
    else msg

  /** A general checkBounds method that can be used for TypeApply nodes as well
    * as for AppliedTypeTree nodes. Also checks that type arguments to *-type
    * parameters are fully applied.
    * @param tpt
    *   If bounds are checked for an AppliedType, the type tree representing or
    *   (in case it is inferred) containing the type. See
    *   TypeOps.boundsViolations for an explanation of the first four
    *   parameters.
    */
  def checkBounds(
      args: List[tpd.Tree],
    ...
    for (arg, which, bound) <- TypeOps.boundsViolations(
        args,
        boundss,
        instantiate,
        app
  ...
  /** Check that type arguments `args` conform to corresponding bounds in `tl`
    * Note: This does not check the bounds of AppliedTypeTrees. These are
    * handled by method checkAppliedType below.
    */
  ...
  /** If `tp` dealiases to a typebounds L..H where not L <:< H return the
    * potentially conflicting bounds, otherwise return NoType.
    */
  ...
  /** Check applied type trees for well-formedness. This means
    ...
    *   - if type is a higher-kinded application with wildcard arguments, check
    *     that it or one of its supertypes can be reduced to a normal
    *     application. Unreducible applications correspond to general
    *     existentials, and we cannot handle those.
    * @param tree
    *   The applied type tree to check
    * @param tpt
    *   If `tree` is synthesized from a type in a TypeTree, the original
    *   TypeTree, or EmptyTree otherwise.
    */
  def checkAppliedType(tree: AppliedTypeTree, tpt: Tree = EmptyTree)(using
      Context
  ): Unit =
    ...
    then checkBounds(args, bounds, instantiate, tree.tpe, tpt)
      ...
      def traverse(tp: Type) =
        tp match
          case AppliedType(tycon, argTypes) =>
            checkAppliedType(
              untpd
                .AppliedTypeTree(TypeTree(tycon), argTypes.map(TypeTree(_)))
                .withType(tp)
                .withSpan(tpt.span.toSynthetic),
              tpt
            )
          case _ =>
        traverseChildren(tp)
  ...
  /** Check that kind of `arg` has the same outline as the kind of paramBounds.
    ...
    * and analogously for all other kinds. This kind checking does not take into
    * account variances or bounds. The more detailed kind checking is done as
    * part of checkBounds in PostTyper. The purpose of preCheckKind is to do a
    * rough test earlier in Typer, in order to prevent scenarios that lead to
    * self application of types. Self application needs to be avoided since it
    * can lead to stack overflows. Test cases are neg/i2771.scala and
    * neg/i2771b.scala. A NoType paramBounds is used as a sign that checking
  ...
  def preCheckKind(arg: Tree, paramBounds: Type)(using Context): Tree =
    ...
    then arg
      ...
      errorTree(
        arg,
        em"Type argument ${arg.tpe} does not have the same kind as its bound $paramBounds"
      )

  def preCheckKinds(args: List[Tree], paramBoundss: List[Type])(using
      Context
  ): List[Tree] =
  ...
  /** Check that `tp` refers to a nonAbstract class and that the instance
    * conforms to the self type of the created class.
    */
  def checkInstantiable(tp: Type, srcTp: Type, pos: SrcPos)(using
      Context
  ): Unit =
    tp.underlyingClassRef(refinementOK = false) match
      case tref: TypeRef =>
        val cls = tref.symbol
        if cls.isOneOf(AbstractOrTrait) then
          val srcCls = srcTp.underlyingClassRef(refinementOK = false).typeSymbol
          report.error(
            CantInstantiateAbstractClassOrTrait(
              srcCls,
              isTrait = srcCls.is(Trait)
            ),
            pos
          )
  ...
  def checkRealizable(tp: Type, pos: SrcPos, what: String = "path")(using
      Context
  ): Unit =
    val rstatus = realizability(tp)
    if rstatus ne Realizable then
      report.errorOrMigrationWarning(
  ...
  /** Given a parent `parent` of a class `cls`, if `parent` is a trait check
    * that the superclass of `cls` derived from the superclass of `parent`.
    *
    * An exception is made if `cls` extends `Any`, and `parent` is
    * `java.io.Serializable` or `java.lang.Comparable`. These two classes are
    * treated by Scala as universal traits. E.g. the following is OK:
    *
    * ... extends Any with java.io.Serializable
    *
    * The standard library relies on this idiom.
    */
  def checkTraitInheritance(parent: Symbol, cls: ClassSymbol, pos: SrcPos)(using
      Context
  ): Unit =
    parent match
      case parent: ClassSymbol if parent.is(Trait) =>
        ...
        if !ok then
          report.error(
            em"illegal trait inheritance: super$csuper does not derive from $parent's super$psuper",
            pos
          )
      case _ =>

  /** A type map which checks that the only cycles in a type are F-bounds and
    * that protects all F-bounded references by LazyRefs.
    */
  class CheckNonCyclicMap(sym: Symbol, reportErrors: Boolean)(using Context)
      extends TypeMap:
    ...
    /** A diagnostic output string that indicates the position of the last part
      * of a type bounds checked by checkInfo. Possible choices: alias, lower
      * bound, upper bound.
      */
    var where: String = ""
    ...
    def checkInfo(tp: Type): Type = tp match
      ...
      case tp @ TypeBounds(lo, hi) =>
        tp.derivedTypeBounds(
          checkPart(lo, "lower bound"),
          checkUpper(hi, "upper bound")
        )
      case _ =>
        tp
    ...
    def apply(tp: Type): Type = tp match
      ...
      case tp @ AppliedType(tycon, args) =>
        tp.derivedAppliedType(
          this(tycon),
          args.mapConserve(this(_, nestedCycleOK, nestedCycleOK))
        )
      case tp @ RefinedType(parent, name, rinfo) =>
        tp.derivedRefinedType(
          this(parent),
          name,
          this(rinfo, nestedCycleOK, nestedCycleOK)
        )
      case tp: RecType =>
        ...
        try
          ...
          def isInteresting(prefix: Type): Boolean = prefix.stripTypeVar match
            ...
            case prefix: ThisType =>
              sym.owner.isClass && (
                prefix.cls.isContainedIn(
                  sym.owner
                ) // sym reachable through outer references
                  || sym.owner.isContainedIn(
                    prefix.cls
                  ) // sym reachable through member references
              )
            case prefix: NamedType =>
              (!sym.is(Private) && prefix.derivesFrom(sym.owner)) ||
              (!prefix.symbol.moduleClass.isStaticOwner && isInteresting(
                prefix.prefix
              ))
          ...
          if isInteresting(pre) then
            ...
            if locked.contains(tp) || tp.symbol.infoOrCompleter
                .isInstanceOf[NoCompleter]
            then throw CyclicReference(tp.symbol)
            locked += tp
            try if !tp.symbol.isClass then checkInfo(tp.info)
            finally locked -= tp
            tp.withPrefix(pre1)
          else tp
        catch
          case ex: CyclicReference =>
            ...
            if cycleOK then LazyRef.of(tp)
            else if reportErrors then throw ex
            else tp
      case _ => mapOver(tp)
  end CheckNonCyclicMap

  /** Under -Yrequire-targetName, if `sym` has an operator name, check that it
    * has a
    * @targetName
    *   annotation.
    */
      ...
      sym.name.toTermName match
        case name: SimpleName
            if name.isOperatorName
              ...
              && !sym.is(Synthetic) =>
          report.warning(
            i"$sym has an operator name; it should come with an @targetName annotation",
            sym.srcPos
          )
        case _ =>
  ...
  /** Check that `info` of symbol `sym` is not cyclic.
    * @pre
    *   sym is not yet initialized (i.e. its type is a Completer).
    * @return
    *   `info` where every legal F-bounded reference is proctected by a
    *   `LazyRef`, or `ErrorType` if a cycle was detected and reported.
    */
  def checkNonCyclic(sym: Symbol, info: Type, reportErrors: Boolean)(using
      Context
  ): Type =
    val checker =
      withMode(Mode.CheckCyclic)(new CheckNonCyclicMap(sym, reportErrors))
    try checker.checkInfo(info)
    catch
      case ex: CyclicReference =>
        if reportErrors then
          errorType(
            IllegalCyclicTypeReference(sym, checker.where, checker.lastChecked),
            sym.srcPos
          )
        else info

  /** Check that refinement satisfies the following two conditions
    *   1. No part of it refers to a symbol that's defined in the same
    *      refinement at a textually later point. 2. All references to the
    *      refinement itself via `this` are followed by selections. Note: It's
    *      not yet clear what exactly we want to allow and what we want to rule
    *      out. This depends also on firming up the DOT calculus. For the moment
    *      we only issue deprecated warnings, not errors.
    */
  def checkRefinementNonCyclic(
      refinement: Tree,
      refineCls: ClassSymbol,
      seen: mutable.Set[Symbol]
  )(using Context): Unit =
    def flag(what: String, tree: Tree) =
      report.warning(
        i"$what reference in refinement is deprecated",
        tree.srcPos
      )
    ...
    val checkTree = new TreeAccumulator[Unit]:
      def checkRef(tree: Tree, sym: Symbol) =
        if sym.maybeOwner == refineCls && !seen(sym) then forwardRef(tree)
  ...
  end checkRefinementNonCyclic

  /** Check type members inherited from different `parents` of `joint` type for
    * cycles, unless a type with the same name already appears in `decls`.
    * @return
    *   true iff no cycles were detected
    */
  def checkNonCyclicInherited(
      joint: Type,
      parents: List[Type],
      decls: Scope,
      pos: SrcPos
    ...
    if parents.lengthCompare(1) <= 0 then return
    ...
    withMode(Mode.CheckCyclic) {
      ...
      for name <- abstractTypeNames do
        try
          ...
          case _: RecursionOverflow | _: CyclicReference =>
            report.error(em"cyclic reference involving type $name", pos)
            false
    }
  end checkNonCyclicInherited
  ...
  def checkWellFormed(sym: Symbol)(using Context): Unit =
    ...
    then fail(ParamsNoInline(sym.owner))
    if sym.isInlineMethod && !sym.is(
        Deferred
      ) && sym.allOverriddenSymbols.nonEmpty
    then checkInlineOverrideParameters(sym)
    if sym.is(Implicit) then
      assert(
        !sym.owner.is(Package),
        s"top-level implicit $sym should be wrapped by a package after typer"
      )
      if sym.isType && (!sym.isClass || sym.is(Trait)) then
        fail(TypesAndTraitsCantBeImplicit())
    if sym.is(Transparent) then
      if sym.isType then
        if !sym.isExtensibleClass then
          fail(
            em"`transparent` can only be used for extensible classes and traits"
          )
      else if !sym.isInlineMethod then
        fail(em"`transparent` can only be used for inline methods")
    if !sym.isClass && sym.is(Abstract) then
    ...
    if !sym.isClass && sym.isOneOf(ClassOnlyFlags) then
      ...
      if sym.is(
          TypeParam
        ) && illegal == Sealed && Feature.ccEnabled && cc.allowUniversalInBoxed
      then
    ...
    if sym.is(AbsOverride) && !sym.owner.is(Trait) then
      fail(AbstractOverrideOnlyInTraits(sym))
    if sym.is(Trait) then
      if sym.is(Final) then fail(TraitsMayNotBeFinal(sym))
      else if sym.is(Open) then warn(RedundantModifier(Open))
    if sym.isAllOf(Abstract | Open) then warn(RedundantModifier(Open))
    if sym.is(Open) && sym.isLocal then warn(RedundantModifier(Open))
    // Skip ModuleVal since the annotation will also be on the ModuleClass
    if sym.hasAnnotation(defn.TailrecAnnot) then
      if !sym.isOneOf(Method | ModuleVal) then fail(TailrecNotApplicable(sym))
      else if sym.is(Inline) then fail(em"Inline methods cannot be @tailrec")
    if sym.hasAnnotation(
        defn.TargetNameAnnot
      ) && sym.isClass && sym.isTopLevelClass
    then fail(TargetNameOnTopLevelClass(sym))
    if sym.hasAnnotation(defn.NativeAnnot) then
      if !sym.is(Deferred) then fail(NativeMembersMayNotHaveImplementation(sym))
      else if sym.owner.is(Trait) then fail(TraitMayNotDefineNativeMethod(sym))
    else if sym.is(Deferred, butNot = Param) && !sym.isType && !sym.isSelfSym
    then
      if !sym.owner.isClass || sym.owner.is(
          Module
        ) || sym.owner.isAnonymousClass
      then fail(OnlyClassesCanHaveDeclaredButUndefinedMembers(sym))
      checkWithDeferred(Private)
      checkWithDeferred(Final)
    if sym.isValueClass && sym.is(Trait) && !sym.isRefinementClass then
      fail(CannotExtendAnyVal(sym))
    if sym.isConstructor && !sym.isPrimaryConstructor && sym.owner.is(
        Trait,
        butNot = JavaDefined
      )
    then
      val addendum =
        if ctx.settings.Ydebug.value then s" ${sym.owner.flagsString}" else ""
    ...
    if sym.isType && !sym.isOneOf(Deferred | JavaDefined) then
      for cls <- sym.allOverriddenSymbols.filter(_.isClass) do
        fail(
          CannotHaveSameNameAs(
            sym,
            cls,
            CannotHaveSameNameAs.CannotBeOverridden
          )
        )
        sym.setFlag(
          Private
        ) // break the overriding relationship by making sym Private
    checkApplicable(
      Erased,
  ...
  /** Check for illegal or redundant modifiers on modules. This is done
    * separately from checkWellformed, since the original module modifiers don't
    * surivive desugaring
    */
  def checkWellFormedModule(mdef: untpd.ModuleDef)(using Context) =
    ...
    if mods.is(Abstract) then
      report.error(
        ModifierNotAllowedForDefinition(Abstract),
        flagSourcePos(Abstract)
      )
    if mods.is(Sealed) then
      report.error(
        ModifierNotAllowedForDefinition(Sealed),
        flagSourcePos(Sealed)
      )
    if mods.is(Final, butNot = Synthetic) then
      report.warning(RedundantModifier(Final), flagSourcePos(Final))

  /** Check the type signature of the symbol `M` defined by `tree` does not
    * refer to a private type or value which is invisible at a point where `M`
    * is still visible.
    *
  ...
  def checkNoPrivateLeaks(sym: Symbol)(using Context): Type =
    class NotPrivate extends TypeMap:
      ...
      def accessBoundary(sym: Symbol): Symbol =
        if sym.is(Private) || !sym.owner.isClass then sym.owner
        else if sym.privateWithin.exists then sym.privateWithin
        else if sym.is(Package) then sym
      ...
      /** Is `other` leaked outside its access boundary ?
        * @pre
        *   The signature of `sym` refers to `other`
        */
        ...
        other.is(Private, butNot = TypeParam)
          && {
            ...
            !(symBoundary.isContainedIn(otherBoundary) ||
              otherLinkedBoundary.exists && symBoundary.isContainedIn(
                otherLinkedBoundary
              ))
          }
    ...
    notPrivate.errors.foreach(
      report.errorOrMigrationWarning(_, sym.srcPos, from = `3.0`)
    )
    info
  end checkNoPrivateLeaks

  /** Verify classes extending AnyVal meet the requirements */
  def checkDerivedValueClass(clazz: Symbol, stats: List[Tree])(using
      Context
  ): Unit =
    def checkValueClassMember(stat: Tree) = stat match
      case _: TypeDef if stat.symbol.isClass =>
        report.error(
          ValueClassesMayNotDefineInner(clazz, stat.symbol),
          stat.srcPos
        )
      case _: ValDef if !stat.symbol.is(ParamAccessor) =>
        report.error(
          ValueClassesMayNotDefineNonParameterField(clazz, stat.symbol),
          stat.srcPos
        )
      case _: DefDef if stat.symbol.isConstructor =>
        report.error(
          ValueClassesMayNotDefineASecondaryConstructor(clazz, stat.symbol),
          stat.srcPos
        )
    ...
    if isDerivedValueClass(clazz) then
      ...
      then
        ...
        clParamAccessors match
          case param :: params =>
            if param.is(Mutable) then
              report.error(
                ValueClassParameterMayNotBeAVar(clazz, param),
                param.srcPos
              )
            if param.info.isInstanceOf[ExprType] then
              report.error(
                ValueClassParameterMayNotBeCallByName(clazz, param),
                param.srcPos
              )
            if param.is(Erased) then
              report.error(
                "value class first parameter cannot be `erased`",
                param.srcPos
              )
            else
              for p <- params if !p.is(Erased) do
                report.error(
                  "value class can only have one non `erased` parameter",
                  p.srcPos
                )
          case Nil =>
  ...
  end checkDerivedValueClass

  /** Check the inline override methods only use inline parameters if they
    * override an inline parameter.
    */
    ...
    do
      report.error(
        if p2.is(Inline) then
          "Cannot override inline parameter with a non-inline parameter"
  ...
  /** Check that experimental language imports in `trees` are done only in
    * experimental scopes, or in a top-level scope with only @experimental
    * definitions.
    */
  def checkExperimentalImports(trees: List[Tree])(using Context): Unit =
    ...
    def nonExperimentalStat(trees: List[Tree]): Tree = trees match
      ...
      case (tree @ TypeDef(_, impl: Template)) :: rest
    ...
    for case imp @ Import(qual, selectors) <- trees do
      ...
      languageImport(qual) match
        case Some(nme.experimental)
            if !ctx.owner.isInExperimentalScope && !selectors.forall(
              isAllowedImport
            ) =>
          def check(stable: => String) =
            Feature.checkExperimentalFeature(
              "features",
              imp.srcPos,
...
trait Checking:
  ...
  def checkNonCyclic(sym: Symbol, info: TypeBounds, reportErrors: Boolean)(using
      Context
  ): Type =
  ...
  def checkStable(tp: Type, pos: SrcPos, kind: String)(using Context): Unit =
    if !tp.isStable && !tp.isErroneous then
  ...
  /** Check that pattern `pat` is irrefutable for scrutinee type `sel.tpe`. This
    * means `sel` is either marked @unchecked or `sel.tpe` conforms to the
    * pattern's type. If pattern is an UnApply, also check that the extractor is
    * irrefutable, and do the check recursively.
    */
  def checkIrrefutable(sel: Tree, pat: Tree, isPatDef: Boolean)(using
      Context
  ): Boolean =
    val pt = sel.tpe
    ...
    def fail(pat: Tree, pt: Type, reason: Reason): Boolean =
      ...
      val message = reason match
        case NonConforming =>
          ...
          val problem =
            if pat.tpe <:< reportedPt then "is more specialized than"
            else "does not match"
      ...
      val pos =
        if isPatDef then
          reason match
            ...
            case RefutableExtractor =>
              pat.source.atSpan(pat.span union sel.span)
        else pat.srcPos
    ...
    def check(pat: Tree, pt: Type): Boolean =
      (pt <:< pat.tpe) || fail(pat, pt, Reason.NonConforming)
        ...
        || {
          ...
          pat match
            ...
            case UnApply(fn, implicits, pats) =>
              check(pat, pt) &&
              (isIrrefutable(fn, pats.length) || isIrrefutableQuotedPattern(
                fn,
                implicits,
                pt
              ) || fail(pat, pt, Reason.RefutableExtractor)) && {
                val argPts = unapplyArgs(
                  fn.tpe.widen.finalResultType,
                  fn,
                  pats,
                  pat.srcPos
                )
    ...
    recur(pat, pt)
  end checkIrrefutable

  private def checkLegalImportOrExportPath(path: Tree, kind: String)(using
      Context
  ): Unit =
    checkStable(path.tpe, path.srcPos, kind)
    if !ctx.isAfterTyper then Checking.checkRealizable(path.tpe, path.srcPos)
    ...
    languageImport(path) match
      case Some(prefix) =>
        ...
        if path.symbol != required then
          report.error(
            em"import looks like a language import, but refers to something else: ${path.symbol.showLocated}",
            path.srcPos
          )
      case None =>
        ...
        then
          report.error(
            em"no aliases can be used to refer to a language import",
            path.srcPos
          )

  /** Check that `path` is a legal prefix for an export clause */
  def checkLegalExportPath(path: Tree, selectors: List[untpd.ImportSelector])(
      using Context
  ): Unit =
  ...
  /** Check that module `sym` does not clash with a class of the same name that
    * is concurrently compiled in another source file.
    */
  def checkNoModuleClash(sym: Symbol)(using Context): Unit =
    ...
    then
      ...
      if conflicting.exists then
        report.error(
          AlreadyDefined(sym.name, effectiveOwner, conflicting),
          sym.srcPos
        )

  /** Check that `tp` is a class type. Also, if `traitReq` is true, check that
    * `tp` is a trait. Also, if `stablePrefixReq` is true and phase is not after
    * RefChecks, check that class prefix is stable.
    * @return
  ...
  def checkClassType(
      ...
      pos: SrcPos,
      traitReq: Boolean,
      stablePrefixReq: Boolean
  )(using Context): Type =
    tp.underlyingClassRef(refinementOK = false) match
      case tref: TypeRef =>
        if traitReq && !tref.symbol.is(Trait) then
          report.error(TraitIsExpected(tref.symbol), pos)
        if stablePrefixReq && ctx.phase <= refchecksPhase then
          checkStable(tref.prefix, pos, "class prefix")
        tp
      case _ =>
        report.error(NotClassType(tp), pos)
        defn.ObjectType

  /** If `sym` is an old-style implicit conversion, check that implicit
    * conversions are enabled.
    * @pre
    *   sym.is(GivenOrImplicit)
    */
  ...
  /** If `tree` is an application of a new-style implicit conversion (using the
    * apply method of a `scala.Conversion` instance), check that the expected
    * type is a convertible formal parameter type or that implicit conversions
    * are enabled.
    */
  def checkImplicitConversionUseOK(tree: Tree, expected: Type)(using
      Context
  ): Unit =
    val sym = tree.symbol
    ...
    then
      ...
      checkFeature(
        nme.implicitConversions,
        i"Use of implicit conversion ${conv.showLocated}",
        NoSymbol,
        tree.srcPos
      )
  ...
  /** Check that `tree` is a valid infix operation. That is, if the operator is
    * alphanumeric, it must be declared `infix`.
    */
        ...
        name.toTermName match
          case name: SimpleName
            ...
            val (kind, alternative) =
              ...
              else ("method", (n: Name) => s"method syntax .$n(...)")
            def rewriteMsg =
              Message.rewriteNotice("The latter", options = "-deprecation")
            report.deprecationWarning(
              em"""Alphanumeric $kind $name is not declared ${hlAsKeyword(
                  "infix"
                )}; it should not be used as infix operator.
                  |Instead, use ${alternative(
                  name
                )} or backticked identifier `$name`.$rewriteMsg""",
              tree.op.srcPos
            )
            if ctx.settings.deprecation.value then
  ...
  /** Check that `tp` is a class type and that any top-level type arguments in
    * this type are feasible, i.e. that their lower bound conforms to their
    * upper bound. If a type argument is infeasible, issue and error and
    * continue with upper bound.
    */
  def checkFeasibleParent(tp: Type, pos: SrcPos, where: => String = "")(using
      Context
  ): Type =
    def checkGoodBounds(tp: Type) = tp match
      case tp @ TypeBounds(lo, hi) if !(lo <:< hi) =>
        report.error(
          em"no type exists between low bound $lo and high bound $hi$where",
          pos
        )
        TypeBounds(hi, hi)
      case _ =>
    ...
    tp match
      ...
      case tp: RefinedType =>
        tp.derivedRefinedType(
          tp.parent,
          tp.refinedName,
          checkGoodBounds(tp.refinedInfo)
        )
      case _ =>
        tp
  ...
  def checkNoDoubleDeclaration(cls: Symbol)(using Context): Unit =
    ...
    def checkDecl(decl: Symbol): Unit =
      for other <- seen(decl.name) if !decl.isAbsent() && !other.isAbsent() do
        ...
        if decl.matches(other) && !javaFieldMethodPair then
          def doubleDefError(decl: Symbol, other: Symbol): Unit =
            if !decl.info.isErroneous && !other.info.isErroneous then
        ...
        if decl.hasDefaultParams && other.hasDefaultParams then
          report.error(
            em"two or more overloaded variants of $decl have default arguments",
            decl.srcPos
          )
  ...
  def checkParentCall(call: Tree, caller: ClassSymbol)(using Context): Unit =
    if !ctx.isAfterTyper then
      ...
      if called.is(JavaAnnotation) then
        report.error(
          em"${called.name} must appear without any argument to be a valid class parent because it is a Java annotation",
          call.srcPos
        )
      if caller.is(Trait) then
        report.error(
          em"$caller may not call constructor of $called",
          call.srcPos
        )
      else if called.is(Trait) && !caller.mixins.contains(called) then
        report.error(
          em"""$called is already implemented by super${caller.superClass},
      ...
      def checkLegalConstructorCall(
        ...
        case _ =>
          report.error(
            em"too many ${kind}arguments in parent constructor",
            encl.srcPos
          )
      call match
  ...
  def checkSimpleKinded(tpt: Tree)(using Context): Tree =
    if !tpt.tpe.hasSimpleKind && !ctx.isJava then
  ...
  /** Check that case classes are not inherited by case classes.
    */
  def checkCaseInheritance(parent: Symbol, caseCls: ClassSymbol, pos: SrcPos)(
      using Context
  ): Unit =
    parent match
      case parent: ClassSymbol =>
        if parent.is(Case) then
          report.error(
            em"""case $caseCls has case ancestor $parent, but case-to-case inheritance is prohibited.
  ...
  /** Check that method parameter types do not reference their own parameter or
    * later parameters in the same parameter section.
    */
      ...
      case vparam :: vparams1 =>
        vparam.tpt.foreachSubTree {
          case id: Ident if vparams.exists(_.symbol == id.symbol) =>
            report.error(
              em"illegal forward reference to method parameter",
              id.srcPos
            )
          case _ =>
        }
  ...
  /** Check that all named types that form part of this type have a denotation.
    * Called on inferred (result) types of ValDefs and DefDefs. This could fail
    * for types where the member was originally available as part of the self
    * type, yet is no longer visible once the `this` has been replaced by some
    * other prefix. See neg/i3083.scala
    */
  def checkMembersOK(tp: Type, pos: SrcPos)(using Context): Type =
    ...
    val check: Type => Unit = {
      case ref: NamedType =>
        val d =
          try ref.denot
          catch case ex: TypeError => NoDenotation
        if !d.exists then
    ...
    if ok then tp else UnspecifiedErrorType
  ...
  def checkRefsLegal(
      tree: tpd.Tree,
      badOwner: Symbol,
      allowed: (Name, Symbol) => Boolean,
      where: String
  )(using Context): Unit =
    val checker = new TreeTraverser:
      def traverse(t: Tree)(using Context) =
        def check(owner: Symbol, checkedSym: Symbol) =
          if t.span.isSourceDerived && owner == badOwner then
            t match
              case t: RefTree if allowed(t.name, checkedSym) =>
              case _ =>
                report.error(
                  em"illegal reference to $checkedSym from $where",
                  t.srcPos
                )
        val sym = t.symbol
        t match
  ...
  def checkFullyAppliedType(tree: Tree)(using Context): Unit = tree match
    ...
    case _ =>
      if tree.tpe.typeParams.nonEmpty then
        val what =
          if tree.symbol.exists then tree.symbol.show else i"type $tree"
        report.error(em"$what takes type parameters", tree.srcPos)

  /** Check that we are in an inline context (inside an inline method or in
    * inline code)
    */
  ...
  /** Check that the class corresponding to this tree is either a Scala or Java
    * annotation.
    *
    * @return
    *   The original tree or an error tree in case `tree` isn't a valid
    *   annotation or already an error tree.
    */
  def checkAnnotClass(tree: Tree)(using Context): Tree =
    if tree.tpe.isError then return tree
    val cls = Annotations.annotClass(tree)
    if cls.is(JavaDefined) then
      if !cls.is(JavaAnnotation) then
        errorTree(
          tree,
          em"$cls is not a valid Java annotation: it was not declared with `@interface`"
        )
      else tree
    else if !cls.derivesFrom(defn.AnnotationClass) then
      errorTree(
        tree,
        em"$cls is not a valid Scala annotation: it does not extend `scala.annotation.Annotation`"
      )
    else tree
    ...
    tree match
      case Apply(tycon, arg :: Nil) if cls == defn.TargetNameAnnot =>
        arg match
          ...
          case _ =>
            report.error(
              em"@${cls.name} needs a string literal as argument",
              arg.srcPos
            )
      case _ =>
    tree

  /**   1. Check that all case classes that extend `scala.reflect.Enum` are
    *      `enum` cases 2. Check that parameterised `enum` cases do not extend
    *      java.lang.Enum. 3. Check that only a static `enum` base class can
    *      extend java.lang.Enum. 4. Check that user does not implement an
    *      `ordinal` method in the body of an enum class.
    */
  def checkEnum(cdef: untpd.TypeDef, cls: Symbol, firstParent: Symbol)(using
      Context
  ): Unit =
    def existingDef(
        sym: Symbol,
        clazz: ClassSymbol
    )(using Context): Symbol = // adapted from SyntheticMembers
    ...
    def checkExistingOrdinal(using Context) =
      ...
      if decl.exists then
        if decl.owner == cls then
          report.error(
            em"the ordinal method of enum $cls can not be defined by the user",
            decl.srcPos
          )
        else
          report.error(
            em"enum $cls can not inherit the concrete ordinal method of ${decl.owner}",
            cdef.srcPos
          )
    def isEnumAnonCls =
      cls.isAnonymousClass
        ...
        && (cls.owner.flagsUNSAFE.isAllOf(EnumCase)
          || ((cls.owner.name eq nme.DOLLAR_NEW) && cls.owner.flagsUNSAFE
            .isAllOf(Private | Synthetic)))
    val isJavaEnum = cls.derivesFrom(defn.JavaEnumClass)
    if isJavaEnum && cdef.mods.isEnumClass && !cls.isStatic then
      report.error(
        em"An enum extending java.lang.Enum must be declared in a static scope",
        cdef.srcPos
      )
    if !isEnumAnonCls then
      if cdef.mods.isEnumCase then
        if isJavaEnum then
          report.error(
            em"paramerized case is not allowed in an enum that extends java.lang.Enum",
            cdef.srcPos
          )
    ...
    if cls.isEnumClass && !isJavaEnum then checkExistingOrdinal
  end checkEnum

  /** Check that the firstParent for an enum case derives from the declaring
    * enum class, if not, adds it as a parent after emitting an error.
    *
    * This check will have no effect on simple enum cases as their parents are
    * inferred by the compiler.
    */
  def checkEnumParent(cls: Symbol, firstParent: Symbol)(using Context): Unit =

    extension (sym: Symbol)
      def typeRefApplied(using Context): Type =
    ...
    def ensureParentDerivesFrom(enumCase: Symbol)(using Context) =
      ...
      if !firstParent.derivesFrom(enumCls) then
        report.error(
          em"enum case does not extend its enum $enumCls",
          enumCase.srcPos
        )
        cls.info match
          case info: ClassInfo =>
            cls.info = info.derivedClassInfo(declaredParents =
              enumCls.typeRefApplied :: info.declaredParents
            )
          case _ =>

    val enumCase =
      if cls.flagsUNSAFE.isAllOf(EnumCase) then cls
      else if cls.isAnonymousClass && cls.owner.flagsUNSAFE.isAllOf(EnumCase)
      then cls.owner
      else NoSymbol
    if enumCase.exists then ensureParentDerivesFrom(enumCase)

  end checkEnumParent

  /** Check that all references coming from enum cases in an enum companion
    * object are legal.
    * @param cdef
    ...
    */
  def checkEnumCaseRefsLegal(cdef: TypeDef, enumCtx: Context)(using
      Context
  ): Unit =
    ...
    def checkEnumCaseOrDefault(stat: Tree, caseCtx: Context) =
      ...
      def check(tree: Tree) =
        ...
        def allowAccess(name: Name, sym: Symbol): Boolean =
          ...
          val ref = ctx.typer.typedIdent(
            untpd.Ident(name).withSpan(stat.span),
            WildcardType
          )(using testCtx)
          ref.symbol == sym && !testCtx.reporter.hasErrors
        checkRefsLegal(tree, cdef.symbol, allowAccess, "enum case")

      if stat.symbol.isAllOf(EnumCase) then
        stat match
          ...
          .is(Module) && stat.symbol.linkedClass.isAllOf(EnumCase)
      then
        stat match
          case TypeDef(_, impl: Template) =>
            for case (defaultGetter @ DefDef(
                DefaultGetterName(nme.CONSTRUCTOR, _),
                _,
                _,
                _
              )) <- impl.body
            do check(defaultGetter.rhs)
          case _ =>
    end checkEnumCaseOrDefault
    ...
    cdef.rhs match
      case impl: Template =>
        ...
        for stat <- impl.body do checkEnumCaseOrDefault(stat, caseCtx)
      case _ =>
  end checkEnumCaseRefsLegal
  ...
  def checkAnnotApplicable(annot: Tree, sym: Symbol)(using Context): Boolean =
    !ctx.reporter.reportsErrorsFor {
      ...
      if annotCls == defn.MainAnnot || concreteAnnot.matches(
          defn.MainAnnotationClass
        )
      then
        if !sym.isRealMethod then
          report.error(em"main annotation cannot be applied to $sym", pos)
        if !sym.owner.is(Module) || !sym.owner.isStatic then
          report.error(
            em"$sym cannot be a main method since it cannot be accessed statically",
            pos
          )
      // TODO: Add more checks here
    }

  /** Check that symbol's external name does not clash with symbols defined in
    * the same scope
    */
    ...
    for stat <- stats do
      ...
      if tname != sym.name then
        ...
        if preExisting.exists || seen.contains(tname) then
          report.error(
            em"@targetName annotation ${'"'}$tname${'"'} clashes with other definition in same scope",
            stat.srcPos
          )
        if stat.isDef then seen += tname

  def checkMatchable(tp: Type, pos: SrcPos, pattern: Boolean)(using
      Context
  ): Unit =
    if !tp.derivesFrom(defn.MatchableClass) && sourceVersion.isAtLeast(
        `future-migration`
      )
    then report.warning(MatchableWarning(tp, pattern), pos)

  /** Check that there is an implicit capability to throw a checked exception if
    * the saferExceptions feature is turned on. Return that capability is it
    * exists, EmptyTree otherwise.
    */
  ...
  def checkCatch(pat: Tree, guard: Tree)(using Context): Unit = pat match
    case Typed(_: Ident, tpt)
        if isFullyDefined(tpt.tpe, ForceDegree.none) && guard.isEmpty =>
    // OK
    case Bind(_, pat1) =>
    ...
    case _ =>
      val req =
        if guard.isEmpty then
          "for cases of the form `ex: T` where `T` is fully defined"
  ...
  def checkNoContextFunctionType(tree: Tree)(using Context): Unit =
    def recur(tp: Type): Unit = tp.dealias match
      ...
      case tp if defn.isContextFunctionType(tp) =>
        report.error(
          em"context function type cannot have opaque aliases",
          tree.srcPos
        )
      case _ =>
    recur(tree.tpe)

  /** (1) Check that every named import selector refers to a type or value
    * member of the qualifier type. (2) Check that no import selector is renamed
    * more than once.
    */
  def checkImportSelectors(
      qualType: Type,
    ...
    def checkIdent(sel: untpd.ImportSelector): Unit =
      ...
      then
        report.error(
          NotAMember(qualType, sel.name, "value"),
          sel.imported.srcPos
        )
    ...
    if !ctx.compilationUnit.isJava then
      for sel <- selectors do if !sel.isWildcard then checkIdent(sel)
  end checkImportSelectors
end Checking

trait ReChecking extends Checking:
  import tpd.*
  override def checkEnumParent(cls: Symbol, firstParent: Symbol)(using
      Context
  ): Unit = ()
  override def checkEnum(cdef: untpd.TypeDef, cls: Symbol, firstParent: Symbol)(
      using Context
  ): Unit = ()
  override def checkRefsLegal(
      tree: tpd.Tree,
      badOwner: Symbol,
      allowed: (Name, Symbol) => Boolean,
      where: String
  ...
  override def checkEnumCaseRefsLegal(cdef: TypeDef, enumCtx: Context)(using
      Context
  ): Unit = ()
  override def checkAnnotApplicable(annot: Tree, sym: Symbol)(using
      Context
  ): Boolean = true
  override def checkMatchable(tp: Type, pos: SrcPos, pattern: Boolean)(using
      Context
  ): Unit = ()
  override def checkNoModuleClash(sym: Symbol)(using Context) = ()
  override def checkCanThrow(tp: Type, span: Span)(using Context): Tree =
    EmptyTree
...
trait NoChecking extends ReChecking:
  import tpd.*
  override def checkNonCyclic(
      sym: Symbol,
      info: TypeBounds,
      reportErrors: Boolean
  )(using Context): Type = info
  override def checkNonCyclicInherited(
      joint: Type,
      parents: List[Type],
      decls: Scope,
      pos: SrcPos
  )(using Context): Unit = ()
  override def checkStable(tp: Type, pos: SrcPos, kind: String)(using
      Context
  ): Unit = ()
  override def checkClassType(
      tp: Type,
      pos: SrcPos,
      traitReq: Boolean,
      stablePrefixReq: Boolean
  ...
  override def checkImplicitConversionDefOK(sym: Symbol)(using Context): Unit =
    ()
  override def checkImplicitConversionUseOK(tree: Tree, expected: Type)(using
      Context
  ): Unit = ()
  override def checkFeasibleParent(
      tp: Type,
      pos: SrcPos,
      where: => String = ""
  )(using Context): Type = tp
  override def checkAnnotArgs(tree: Tree)(using Context): tree.type = tree
  override def checkNoTargetNameConflict(stats: List[Tree])(using
      Context
  ): Unit = ()
  override def checkParentCall(call: Tree, caller: ClassSymbol)(using
      Context
  ): Unit = ()
  override def checkSimpleKinded(tpt: Tree)(using Context): Tree = tpt
  override def checkDerivedValueClass(clazz: Symbol, stats: List[Tree])(using
      Context
  ): Unit = ()
  override def checkTraitInheritance(
      parentSym: Symbol,
      cls: ClassSymbol,
      pos: SrcPos
  )(using Context): Unit = ()
  override def checkCaseInheritance(
      parentSym: Symbol,
      caseCls: ClassSymbol,
      pos: SrcPos
  )(using Context): Unit = ()
  override def checkNoForwardDependencies(vparams: List[ValDef])(using
      Context
  ): Unit = ()
  override def checkMembersOK(tp: Type, pos: SrcPos)(using Context): Type = tp
  override def checkInInlineContext(what: String, pos: SrcPos)(using
      Context
  ): Unit = ()
  override def checkValidInfix(tree: untpd.InfixOp, meth: Symbol)(using
      Context
  ): Unit = ()
  override def checkImportSelectors(
      qualType: Type,
      selectors: List[untpd.ImportSelector]
  )(using Context): Unit = ()
end NoChecking
