...
class Synthesizer(typer: Typer)(using @constructorOnly c: Context):
      ...
      case arg :: Nil =>
        instArg(arg) match
          case defn.ArrayOf(elemTp) =>
            val etag = typer.inferImplicitArg(
              defn.ClassTagClass.typeRef.appliedTo(elemTp),
              span
            )
    ...
    (formal, span) =>
      formal.argInfos match
        case arg1 :: arg2 :: Nil if !defn.isBottomClass(arg2.typeSymbol) =>
          ...
          val tp2 =
            fullyDefinedType(arg2, "TypeTest argument", srcPos).normalized
          val sym2 = tp2.typeSymbol
          if tp1 <:< tp2 then
            // optimization when we know the typetest will always succeed
            withNoErrors(
              ref(defn.TypeTestModule_identity)
                .appliedToType(tp2)
                .withSpan(span)
            )
          else if sym2 == defn.AnyValClass || sym2 == defn.AnyRefAlias || sym2 == defn.ObjectClass
          then EmptyTreeNoError
          else
            ...
            def body(args: List[Tree]): Tree =
              ...
              If(
                arg.isInstance(tp2),
                ref(defn.SomeClass.companionModule.termRef)
                  .select(nme.apply)
                  .appliedToType(t)
                  .appliedTo(arg.select(nme.asInstanceOf_).appliedToType(t)),
                ref(defn.NoneModule)
              )
            val tpe = MethodType(List(nme.s))(
              _ => List(tp1),
              mth =>
                defn.OptionClass.typeRef.appliedTo(mth.newParamRef(0) & tp2)
            )
            val meth = newAnonFun(ctx.owner, tpe, coord = span)
            val typeTestType =
              defn.TypeTestClass.typeRef.appliedTo(List(tp1, tp2))
            withNoErrors(
              Closure(
                meth,
                tss => body(tss.head).changeOwner(ctx.owner, meth),
                targetType = typeTestType
              ).withSpan(span)
            )
          end if
        case _ =>
  ...
  val synthesizedTupleFunction: SpecialHandler = (formal, span) =>
    formal match
      case AppliedType(_, funArgs @ fun :: tupled :: Nil) =>
        def functionTypeEqual(
            baseFun: Type,
            actualArgs: List[Type],
            actualRet: Type,
            expected: Type
        ) =
          expected =:= defn.FunctionOf(
            actualArgs,
            actualRet,
        ...
        val arity: Int =
          if defn.isErasedFunctionType(fun) then -1 // TODO support?
            ...
            fun.functionArgInfos match
              case funArgs :+ funRet
                  if functionTypeEqual(
                    fun,
                    defn.tupleType(funArgs) :: Nil,
                    funRet,
                    tupled
                  ) =>
            ...
            tupled.functionArgInfos match
              case tupledArgs :: funRet :: Nil =>
                defn.tupleTypes(tupledArgs.dealias) match
                  case Some(funArgs)
                      if functionTypeEqual(tupled, funArgs, funRet, fun) =>
                    // TupledFunction[?, ((...funArgs...)) => funRet]
                    funArgs.size
                  case _ => -1
              case _ => -1
            ...
            // TupledFunction[?, ?]
            -1
        if arity == -1 then EmptyTreeNoError
  ...
  val synthesizedCanEqual: SpecialHandler = (formal, span) =>
    ...
    def hasEq(tp: Type)(using Context): Boolean =
      val inst = typer.inferImplicitArg(
        defn.CanEqualClass.typeRef.appliedTo(tp, tp),
        span
      )
      !inst.isEmpty && !inst.tpe.isError

    /** Can we assume the canEqualAny instance for `tp1`, `tp2`? This is the
      * case if assumedCanEqual(tp1, tp2), or one of `tp1`, `tp2` has a
      * reflexive `CanEqual` instance.
      */
    ...
    /** Is an `CanEqual[cls1, cls2]` instance assumed for predefined classes
      * `cls1`, cls2`?
      */
    def canComparePredefinedClasses(
        cls1: ClassSymbol,
        cls2: ClassSymbol
    ): Boolean =
      ...
      if cls1.isPrimitiveValueClass then
        ...
        else cmpWithBoxed(cls1, cls2)
      else if cls2.isPrimitiveValueClass then cmpWithBoxed(cls2, cls1)
      ...
      else if cls2 == defn.NullClass then cls1.derivesFrom(defn.ObjectClass)
      else cls1 == defn.NothingClass || cls2 == defn.NothingClass
    end canComparePredefinedClasses

    /** Some simulated `CanEqual` instances for predefined types. It's more
      * efficient to do this directly instead of setting up a lot of `CanEqual`
      * instances to interpret.
      */
    ...
    formal.argTypes match
      case args @ (arg1 :: arg2 :: Nil) =>
        List(arg1, arg2).foreach(
          fullyDefinedType(_, "eq argument", ctx.source.atSpan(span))
        )
        if canComparePredefined(arg1, arg2)
          || !Implicits.strictEquality && explore(validEqAnyArgs(arg1, arg2))
        then
          withNoErrors(
            ref(defn.CanEqual_canEqualAny).appliedToTypes(args).withSpan(span)
          )
        else EmptyTreeNoError
      case _ => EmptyTreeNoError
  end synthesizedCanEqual

  /** Creates a tree that will produce a ValueOf instance for the requested
    * type. An EmptyTreeNoError is returned if materialization fails.
    */
  ...
  /** Create an anonymous class `new Object { type MirroredMonoType = ... }` and
    * mark it with given attachment so that it is made into a mirror at
    * PostTyper.
    */
  private def anonymousMirror(monoType: Type, impl: MirrorImpl, span: Span)(
      using Context
  ) =
    if ctx.isAfterTyper then ctx.compilationUnit.needsMirrorSupport = true
    val monoTypeDef =
      untpd.TypeDef(tpnme.MirroredMonoType, untpd.TypeTree(monoType))
    val newImpl = untpd
      .Template(
        constr = untpd.emptyConstructor,
        parents = untpd.TypeTree(defn.ObjectType) :: untpd.TypeTree(
          defn.JavaSerializableClass.typeRef
        ) :: Nil,
        derived = Nil,
        self = EmptyValDef,
  ...
  /** The mirror type
    *
    * <parent> { MirroredMonoType = <monoType> MirroredType = <mirroredType>
    * MirroredLabel = <label> }
    */
  private def mirrorCore(
    ...
    parentClass.typeRef
      ...
      .refinedWith(
        tpnme.MirroredLabel,
        TypeAlias(ConstantType(Constant(label.toString)))
      )
  ...
  private def companionPath(clsType: Type, span: Span)(using Context) =
    ...
    assert(
      ref.symbol.is(Module) && (clsType.classSymbol.is(
        ModuleClass
      ) || (ref.symbol.companionClass == clsType.classSymbol))
    )
    ref.withSpan(span)
  ...
  private def checkRefinement(
    ...
    then
      report.error(
        em"$name mismatch, expected: $expected, found: $actual.",
        ctx.source.atSpan(span)
      )
  ...
  private def mkMirroredMonoType(mirroredType: HKTypeLambda)(using
      Context
  ): Type =
    val monoMap = new TypeMap:
      def apply(t: Type) = t match
        case TypeParamRef(lambda, n) if lambda eq mirroredType =>
          mirroredType.paramInfos(n)
        case t => mapOver(t)
  ...
  private[Synthesizer] enum MirrorSource:
    ...
    infix def sameTuple(that: MirrorSource)(using Context): Boolean =
      def arity(msrc: MirrorSource): Int = msrc match
        ...
        case ClassSymbol(_, cls) if defn.isTupleClass(cls) =>
          cls.typeParams.size // tested in tests/pos/i13859.scala
        case _ => -1
      def equivalent(n: Int, m: Int) =
        n == m && n > 0
      equivalent(arity(this), arity(that))

    /** A comparison that chooses the most specific MirrorSource, this is guided
      * by what is necessary for `Mirror.Product.fromProduct`. i.e. its result
      * type should be compatible with the erasure of `mirroredType`.
      */
    def isSub(that: MirrorSource)(using Context): Boolean =
      (this, that) match
        case (Singleton(src, _), ClassSymbol(_, cls)) =>
          src.info.classSymbol.isSubClass(cls)
        case (ClassSymbol(_, cls1), ClassSymbol(_, cls2)) =>
          cls1.isSubClass(cls2)
  ...
  private[Synthesizer] object MirrorSource:
    ...
    /** Reduces a mirroredType to either its most specific ClassSymbol, or a
      * TermRef to a singleton value. These are the base elements required to
      * generate a mirror.
      */
    def reduce(mirroredType: Type)(using
        Context
    ): Either[String, MirrorSource] = mirroredType match
      case tp: TypeRef =>
        val sym = tp.symbol
        if sym.isClass then // direct ref to a class, not an alias
          if sym.isAllOf(Case | Module) then
            ...
            Right(
              MirrorSource.Singleton(singleton, TermRef(tp.prefix, singleton))
            )
          else Right(MirrorSource.ClassSymbol(tp.prefix, sym))
        else reduce(tp.superType)
      case tp: TermRef =>
        /** Dealias a path type to extract the underlying definition when it is
          * either a singleton enum case or a case object.
          */
        def reduceToEnumOrCaseObject(tp: Type)(using Context): Symbol = tp match
          case tp: TermRef =>
            val sym = tp.termSymbol
            if sym.isEnumCase || (sym.isClass && sym.isAllOf(Case | Module))
            then sym
            else if sym.exists && !tp.isOverloaded then
              reduceToEnumOrCaseObject(tp.underlying.widenExpr)
            else NoSymbol
          case _ => NoSymbol

        // capture enum singleton types. Tested in tests/run/i15234.scala
        val singleton = reduceToEnumOrCaseObject(tp)
        if singleton.exists then Right(MirrorSource.Singleton(singleton, tp))
        else reduce(tp.underlying)
        ...
        for
          ...
          res <- locally {
            if lsrc.isSub(rsrc) then Right(lsrc)
              ...
              Left(
                i"its subpart `$tp` is an intersection of unrelated definitions ${lsrc.show} and ${rsrc.show}."
              )
          }
        yield res
      case tp: OrType =>
  ...
  private def productMirror(mirroredType: Type, formal: Type, span: Span)(using
      Context
  ): TreeWithErrors =

    /** `new scala.runtime.TupleMirror(arity)` using TupleMirror avoids
      * generating anonymous classes for tuple mirrors.
      */
    ...
    def makeProductMirror(
        pre: Type,
        cls: Symbol,
        tps: Option[List[Type]]
    ): TreeWithErrors =
      val accessors = cls.caseAccessors.filterNot(_.isAllOf(PrivateLocal))
      val elemLabels =
        accessors.map(acc => ConstantType(Constant(acc.name.toString)))
      val typeElems = tps.getOrElse(
        accessors.map(mirroredType.resultType.memberInfo(_).widenExpr)
      )
      val nestedPairs = TypeOps.nestedPairs(typeElems)
      val (monoType, elemsType) = mirroredType match
        case mirroredType: HKTypeLambda =>
          (
            mkMirroredMonoType(mirroredType),
            mirroredType.derivedLambdaType(resType = nestedPairs)
          )
        case _ =>
      ...
      val mirrorRef =
        if cls.useCompanionAsProductMirror then
          companionPath(mirroredType, span)
        else if defn.isTupleClass(cls) then
          newTupleMirror(
            typeElems.size
          ) // TODO: cls == defn.PairClass when > 22
      ...
      case Right(msrc) =>
        msrc match
          case MirrorSource.Singleton(_, tref) =>
            val singleton =
              tref.termSymbol // prefer alias name over the orignal name
            val singletonPath = pathFor(tref).withSpan(span)
            if tref.classSymbol.is(Scala2x)
            then // could be Scala 3 alias of Scala 2 case object.
              val mirrorType = formal.constrained_& {
                mirrorCore(
                  defn.Mirror_SingletonProxyClass,
                  mirroredType,
                  mirroredType,
                  singleton.name
                )
              ...
              val mirrorRef = New(
                defn.Mirror_SingletonProxyClass.typeRef,
                singletonPath :: Nil
              )
              withNoErrors(mirrorRef.cast(mirrorType))
            else
          ...
          case MirrorSource.GenericTuple(tps) =>
            ...
            if tps.size <= maxArity then
              ...
              makeProductMirror(
                tupleCls.owner.reachableThisType,
                tupleCls,
                Some(tps)
              )
            else
              val reason =
                s"it reduces to a tuple with arity $arity, expected arity <= $maxArity"
              withErrors(
                i"${defn.PairClass} is not a generic product because $reason"
              )
          case MirrorSource.ClassSymbol(pre, cls) =>
            if cls.isGenericProduct then makeProductMirror(pre, cls, None)
            else
              withErrors(
                i"$cls is not a generic product because ${cls.whyNotGenericProduct}"
              )
      case Left(msg) =>
        withErrors(
          i"type `$mirroredType` is not a generic product because $msg"
        )
    end match
  end productMirror

  private def sumMirror(mirroredType: Type, formal: Type, span: Span)(using
      Context
  ): TreeWithErrors =

    val (acceptableMsg, pre, cls) = MirrorSource.reduce(mirroredType) match
      case Right(MirrorSource.Singleton(_, tp)) =>
        (i"its subpart `$tp` is a term reference", NoType, NoSymbol)
      ...
      case Right(MirrorSource.GenericTuple(tps)) =>
        ...
        val cls =
          if arity <= Definitions.MaxTupleArity then
            defn.TupleType(arity).nn.classSymbol
          else defn.PairClass
        ("", NoType, cls)
    ...
    if acceptableMsg.isEmpty && clsIsGenericSum then
      val elemLabels =
        cls.children.map(c => ConstantType(Constant(c.name.toString)))

      def internalError(msg: => String)(using Context): Unit =
        report.error(
          em"""Internal error when synthesizing sum mirror for $cls:
                         |$msg""",
          ctx.source.atSpan(span)
        )
      ...
      def refineAtPrefix(childPre: Type, child: Symbol, original: Type): Type =
        if childPre.exists then original.asSeenFrom(childPre, child.owner)
        else original
        ...
        case childClass: ClassSymbol =>
          ...
          if childClass.is(Module) then
            ...
            refineAtPrefix(
              childPre,
              childClass,
              childClass.primaryConstructor.info
            ) match
              case info: PolyType =>
                ...
                instantiate(using
                  ctx.fresh.setExploreTyperState().setOwner(childClass)
                )
              case methTpe =>
      ...
      val (childPres, childTypes) = cls.children
        .map(c =>
      ...
      withErrors(
        i"type `$mirroredType` is not a generic sum because $acceptableMsg"
      )
    else if !clsIsGenericSum then
      withErrors(
        i"$cls is not a generic sum because ${cls.whyNotGenericSum(pre)}"
      )
    else EmptyTreeNoError
    end if
  end sumMirror
  ...
  def makeMirror(
      synth: (Type, Type, Span) => Context ?=> TreeWithErrors,
      formal: Type,
      span: Span
  ...
  /** An implied instance for a type of the form `Mirror.Product { type
    * MirroredType = T }` where `T` is a generic product type or a case object
    * or an enum case.
    */
  val synthesizedProductMirror: SpecialHandler = (formal, span) =>
    makeMirror(productMirror, formal, span)

  /** An implied instance for a type of the form `Mirror.Sum { type MirroredType
    * \= T }` where `T` is a generic sum type.
    */
  ...
  private enum ManifestKind:
    ...
    /** The kind that should be used for an array element, if we are
      * `OptManifest` then this prevents wildcards arguments of Arrays being
      * converted to `NoManifest`
      */
  ...
  /** Manifest factory that does enough to satisfy the equality semantics for
    ...
    *   - `scala.reflect.Manifest` (runtime class of arguments are recorded,
    *     with wildcard upper bounds wrapped) however,`toString` may be
    *     different.
    *
    * There are some differences to `ClassTag`, e.g. in Scala 2 `manifest[Int
    * \@unchecked]` will fail, but `classTag[Int @unchecked]` succeeds.
    */
  private def manifestFactoryOf(kind: ManifestKind): SpecialHandler =
    (formal, span) =>
      import ManifestKind.*
      ...
      def factoryManifest(
        ...
        if args.contains(EmptyTree) then EmptyTree
        else
          val factory =
            if kind == Full then defn.ManifestFactoryModule
            else defn.ClassManifestFactoryModule
          applyOverloaded(
            ref(factory),
            constructor,
            args.toList,
      ...
      def synthArrayManifest(
        ...
        factoryManifest(
          nme.arrayType,
          elemTp,
          synthesize(elemTp, kind.arrayElem, topLevel)
        )

      /** manifests generated from wildcards can not equal
        * Int,Long,Any,AnyRef,AnyVal etc, so we wrap their upper bound.
        */
      def synthWildcardManifest(
          tp: Manifestable,
          hi: Type,
          topLevel: Boolean
      ): Tree =
        factoryManifest(
          nme.wildcardType,
          tp,
          singletonManifest(nme.Nothing),
          synthesize(hi, Full, topLevel)
        )
      ...
      /** This type contains all top-level types supported by Scala 2's
        * algorithm
        */
      type Manifestable =
        ThisType | TermRef | ConstantType | TypeRef | AppliedType | TypeBounds |
          RecType | RefinedType | AndType
      ...
      def synthManifest(
        ...
        case TypeBounds(_, hi) if kind == Full =>
          synthWildcardManifest(tp, hi, topLevel)

        case tp if canManifest(tp, topLevel) =>
          val sym = tp.typeSymbol
          if sym.isPrimitiveValueClass || defn.SpecialManifestClasses.contains(
              sym
            )
          then singletonManifest(sym.name.toTermName)
          else
            erasure(tp) match
              ...
              case etp =>
                val clsArg = clsOf(etp).asInstance(
                  defn.ClassType(tp)
                ) // cast needed to resolve overloading
                factoryManifest(
                  nme.classType,
                  tp,
                  (clsArg :: synthArgManifests(tp))*
                )
      ...
      def manifestOfType(
          ...
          kind: ManifestKind,
          topLevel: Boolean
      ): Tree = tp0.dealiasKeepAnnots match
      ...
      formal.argInfos match
        case arg :: Nil =>
          val manifest = synthesize(
            fullyDefinedType(arg, "Manifest argument", ctx.source.atSpan(span)),
            kind,
            topLevel = true
          )
  ...
  val synthesizedOptManifest: SpecialHandler = manifestFactoryOf(
    ManifestKind.Opt
  )
...
object Synthesizer:
  ...
  private def withErrors(errors: String*): TreeWithErrors =
    (EmptyTree, errors.toList)

  private val EmptyTreeNoError: TreeWithErrors = withNoErrors(EmptyTree)

  private def orElse(
      treeWithErrors1: TreeWithErrors,
      treeWithErrors2: => TreeWithErrors
  ): TreeWithErrors = treeWithErrors1 match
    case (tree, errors) if tree eq genericEmptyTree =>
      val (tree2, errors2) = treeWithErrors2
      (tree2, errors ::: errors2)
    case _ => treeWithErrors1

  private def clearErrorsIfNotEmpty(treeWithErrors: TreeWithErrors) =
    treeWithErrors match
