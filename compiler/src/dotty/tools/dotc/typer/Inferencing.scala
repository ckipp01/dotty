...
object Inferencing:
  ...
  /** Is type fully defined, meaning the type does not contain wildcard types or
    * uninstantiated type variables. As a side effect, this will minimize any
    * uninstantiated type variables, according to the given force degree, but
    * only if the overall result of `isFullyDefined` is `true`. Variables that
    * are successfully minimized do not count as uninstantiated.
    */
  def isFullyDefined(tp: Type, force: ForceDegree.Value)(using
      Context
  ): Boolean =
    withFreshTyperState(
  ...
  /** Try to fully define `tp`. Return whether constraint has changed. Any
    * changed constraint is kept.
    */
  ...
  /** The fully defined type, where all type variables are forced. Throws an
    * error if type contains wildcards.
    */
  def fullyDefinedType(tp: Type, what: String, pos: SrcPos)(using
      Context
  ): Type =
    try
      if isFullyDefined(tp, ForceDegree.all) then tp
        ...
        throw new Error(
          i"internal error: type of $what $tp is not fully defined, pos = $pos"
        )
    catch
  ...
  /** Instantiate selected type variables `tvars` in type `tp` in a special
    * mode:
    *   1. If a type variable is constrained from below (i.e. constraint bound
    *      != given lower bound) it is minimized. 2. Otherwise, if the type
    *      variable is constrained from above, it is maximized. 3. Otherwise, if
    *      the type variable has a lower bound != Nothing, it is minimized. 4.
    *      Otherwise, if the type variable has an upper bound != Any, it is
    *      maximized. If none of (1) - (4) applies, the type variable is left
    *      uninstantiated. The method is called to instantiate type variables
    *      before an implicit search.
    */
  def instantiateSelected(tp: Type, tvars: List[Type])(using Context): Unit =
    if tvars.nonEmpty then
      IsFullyDefinedAccumulator(
        ForceDegree.Value(tvars.contains, IfBottom.flip),
  ...
  /** Instantiate any type variables in `tp` whose bounds contain a reference to
    ...
    */
  def instantiateDependent(tp: Type, paramss: List[List[Symbol]])(using
      Context
  ): Unit =
    ...
    if depVars.nonEmpty then instantiateSelected(tp, depVars.toList)

  /** If `tp` is top-level type variable with a lower bound in the current
    * constraint, instantiate it from below. We also look for TypeVars in other
    * places where their instantiation could uncover new type members. However
    * that search is best effort only. It might miss type variables that appear
    * in structures involving alias types and type projections.
    * @param applied
    *   Test is done in a `tryInsertImplicitOnQualifier` application. In this
    *   case, we always try to instantiate TypeVars in type arguments. If
    *   `applied` is false, we only try that in arguments that may affect the
    *   result type.
    */
  def couldInstantiateTypeVar(tp: Type, applied: Boolean = false)(using
      Context
  ): Boolean = tp.dealias match
    case tvar: TypeVar
    ...
    case AppliedType(tycon, args) =>
      ...
      || (if applied then args else argsInResult).exists(
        couldInstantiateTypeVar(_, applied)
      )
    case RefinedType(parent, _, _) =>
      couldInstantiateTypeVar(parent, applied)
    case tp: AndOrType =>
      couldInstantiateTypeVar(tp.tp1, applied) || couldInstantiateTypeVar(
        tp.tp2,
        applied
      )
  ...
  /** The accumulator which forces type variables using the policy encoded in
    * `force` and returns whether the type is fully defined. The direction in
    * which a type variable is instantiated is determined as follows:
    *   1. T is minimized if the constraint over T is only from below (i.e.
    *      constrained lower bound != given lower bound and constrained upper
    *      bound == given upper bound). 2. T is maximized if the constraint over
    *      T is only from above (i.e. constrained upper bound != given upper
    *      bound and constrained lower bound == given lower bound).
    *
    * If (1) and (2) do not apply, and minimizeSelected is set: 3. T is
    * minimized if it has a lower bound (different from Nothing) in the current
    * constraint (the bound might come from T's declaration). 4. Otherwise, T is
    * maximized if it has an upper bound (different from Any) in the currented
    * constraint (the bound might come from T's declaration). 5. Otherwise, T is
    * not instantiated at all.
    *
    * If (1) and (2) do not apply, and minimizeSelected is not set: 6: T is
    * maximized if it appears only contravariantly in the given type, or if
    * forceDegree is `flipBottom` and T has no lower bound different from
    * Nothing. 7. Otherwise, T is minimized.
    *
    * The instantiation for (6) and (7) is done in two phases: 1st Phase: Try to
    * instantiate minimizable type variables to their lower bound. Record
    * whether successful. 2nd Phase: If first phase was successful, instantiate
    * all remaining type variables to their upper bound.
    */
  private class IsFullyDefinedAccumulator(
      force: ForceDegree.Value,
      minimizeSelected: Boolean = false
  )(using Context)
    ...
    def apply(x: Boolean, tp: Type): Boolean =
      trace(i"isFullyDefined($tp, $force)", typr) {
        try
          val tpd = tp.dealias
            ...
            tp match
              ...
              case tvar: TypeVar if !tvar.isInstantiated =>
                ...
                && {
                  ...
                  else if variance >= 0 && (force.ifBottom == IfBottom.ok && !tvar.hasUpperBound || tvar.hasLowerBound)
    ...
    def process(tp: Type): Boolean =
      ...
      def maximize(tvars: List[TypeVar]): Unit = tvars match
        case tvar :: tvars1 =>
          maximize(tvars1)
          if !tvar.isInstantiated then instantiate(tvar, fromBelow = false)
        case nil =>
      apply(true, tp)
      && (
        toMaximize.isEmpty
  ...
  /** Approximates a type to get rid of as many GADT-constrained abstract types
    * as possible.
    */
  private class ApproximateGadtAccumulator(using Context) extends TypeMap:
    ...
    /** GADT approximation proceeds differently from type variable
      * approximation.
      *
      ...
      *
      * Examples:
      *   - If we have GADT cstr A <: Int, then for all A <: Int, Option[A] <:
      *     Option[Int]. Therefore, we can approximate Option[A] ~~ Option[Int].
    ...
    def process(tp: Type): Type =
      apply(tp)
  end ApproximateGadtAccumulator

  /** For all type parameters occurring in `tp`: If the bounds of `tp` in the
    * current constraint are equal wrt =:=, instantiate the type parameter to
    * the lower bound's approximation (approximation because of possible
    * F-bounds).
    */
        ...
        tp match
          case param: TypeParamRef =>
            ...
            constraint.entry(param) match
              case TypeBounds(lo, hi) if (hi frozen_<:< lo) =>
  ...
  /** If `tree` has a type lambda type, infer its type parameters by comparing
  ...
  def inferTypeParams(tree: Tree, pt: Type)(using Context): Tree =
    tree.tpe match
      case tl: TypeLambda =>
        ...
        tree1.tpe <:< pt
        if isFullyDefined(tree1.tpe, force = ForceDegree.failBottom) then tree1
        else EmptyTree
      case _ =>
        tree
  ...
  /** The list of uninstantiated type variables bound by some prefix of type `T`
    * which occur in at least one formal parameter type of a prefix application.
    ...
    */
  def tvarsInParams(tree: Tree, locked: TypeVars)(using
      Context
  ): List[TypeVar] =
    @tailrec def boundVars(tree: Tree, acc: List[TypeVar]): List[TypeVar] =
      tree match
        case Apply(fn, _) => boundVars(fn, acc)
        case TypeApply(fn, targs) =>
          val tvars =
            targs.filter(_.isInstanceOf[InferredTypeTree]).tpes.collect {
              case tvar: TypeVar
                  if !tvar.isInstantiated &&
        ...
        case _              => acc
    @tailrec def occurring(
        tree: Tree,
        toTest: List[TypeVar],
        acc: List[TypeVar]
    ): List[TypeVar] =
      if toTest.isEmpty then acc
      else
        tree match
          case Apply(fn, _) =>
            fn.tpe.widen match
              case mtp: MethodType =>
                val (occ, nocc) =
                  toTest.partition(tvar => mtp.paramInfos.exists(tvar.occursIn))
  ...
  /** The instantiation direction for given poly param computed from the
    * constraint:
    * @return
    *   1 (maximize) if constraint is uniformly from above,
    * -1 (minimize) if constraint is uniformly from below, 0 if unconstrained,
    * or constraint is from below and above.
    */
  private def instDirection(param: TypeParamRef)(using Context): Int =
    ...
    val approxBelow =
      if !cmp.isSubTypeWhenFrozen(constrained.lo, original.lo) then 1 else 0
    val approxAbove =
      if !cmp.isSubTypeWhenFrozen(original.hi, constrained.hi) then 1 else 0
    approxAbove - approxBelow

  /** Following type aliases and stripping refinements and annotations, if one
    * arrives at a class type reference where the class has a companion module,
    * a reference to that companion module. Otherwise NoType
    */
  ...
  /** Instantiate undetermined type variables so that type `tp` is maximized.
    * @return
    *   The list of type symbols that were created to instantiate undetermined
    *   type variables that occur non-variantly
    */
  def maximizeType(tp: Type, span: Span)(using Context): List[Symbol] =
    ...
    vs foreachBinding { (tvar, v) =>
      if !tvar.isInstantiated then
        ...
        if safeToInstantiate then tvar.instantiate(fromBelow = v == -1)
          ...
          if (bounds.hi frozen_<:< bounds.lo) || bounds.hi.classSymbol.is(Final)
          then tvar.instantiate(fromBelow = false)
            ...
            val wildCard = newPatternBoundSymbol(
              UniqueName.fresh(tvar.origin.paramName),
              bounds,
              span,
              addToGadt = false
            )
    ...
    if res.nonEmpty then ctx.gadtState.addToConstraint(res)
    res
  end maximizeType

  type VarianceMap = SimpleIdentityMap[TypeVar, Integer]

  /** All occurrences of type vars in `tp` that satisfy predicate `include`
    * mapped to their variances (-1/0/1) in both `tp` and `pt.finalResultType`,
    * where
    * -1 means: only contravariant occurrences +1 means: only covariant
    * occurrences 0 means: mixed or non-variant occurrences
    *
    ...
    * a bound.
    *
    * Note: We intentionally use a relaxed version of variance here, where the
    * variance does not change under a prefix of a named type (the strict
    * version makes prefixes invariant). This turns out to be better for type
    * inference. In a nutshell, if a type variable occurs like this:
    *
    * (U? >: x.type) # T
    *
    * we want to instantiate U to x.type right away. No need to wait further.
    */
  private def variances(tp: Type, pt: Type = WildcardType)(using
      Context
  ): VarianceMap =
    ...
    object accu extends TypeAccumulator[VarianceMap]:
      ...
      def apply(vmap: VarianceMap, t: Type): VarianceMap = t match
        case t: TypeVar
          ...
          val v = vmap(t)
          if v == null then vmap.updated(t, variance)
          else if v == variance || v == 0 then vmap
          else vmap.updated(t, 0)
        case _ =>
          foldOver(vmap, t)

    /** Include in `vmap` type variables occurring in the constraints of type
      * variables already in `vmap`. Specifically:
      *   - if `tvar` is covariant in `vmap`, include all variables in its lower
      *     bound (because they influence the minimal solution of `tvar`),
      *   - if `tvar` is contravariant in `vmap`, include all variables in its
      *     upper bound at flipped variances (because they influence the maximal
      *     solution of `tvar`),
      *   - if `tvar` is nonvariant in `vmap`, include all variables in its
      *     upper and lower bounds as non-variant. Do this in a fixpoint
      *     iteration until `vmap` stabilizes.
      */
    def propagate(vmap: VarianceMap): VarianceMap =
        ...
        constraint.entry(param) match
          case TypeBounds(lo, hi) =>
            ...
            if v >= 0 then
              ...
              constraint
                .lower(param)
                .foreach(p => traverse(constraint.typeVarOfParam(p)))
            if v <= 0 then
              traverse(hi)
              constraint
                .upper(param)
      ...
      if vmap1 eq vmap then vmap else propagate(vmap1)
    end propagate

    propagate(accu(accu(SimpleIdentityMap.empty, tp), pt.finalResultType))
  end variances

  /** Run the transformation after dealiasing but return the original type if it
    * was a no-op.
    */
  private def derivedOnDealias(tp: Type)(transform: Type => Type)(using
      Context
  ) =
    val dealiased = tp.dealias
    val transformed = transform(dealiased)
    if transformed eq dealiased then
      tp // return the original type, not the result of dealiasing
    else transformed

  /** Replace every top-level occurrence of a wildcard type argument by a fresh
    * skolem type. The skolem types are of the form $i.CAP, where $i is a skolem
    * of type `scala.internal.TypeBox`, and `CAP` is its type member. See the
    * documentation of `TypeBox` for a rationale why we do this.
    */
  def captureWildcards(tp: Type)(using Context): Type = derivedOnDealias(tp) {
    case tp @ AppliedType(tycon, args) if tp.hasWildcardArg =>
      val tparams = tycon.typeParamSymbols
      val args1 = args.zipWithConserve(
        tparams.map(_.paramInfo.substApprox(tparams, args))
      ) {
        case (TypeBounds(lo, hi), bounds) =>
          val skolem = SkolemType(
            defn.TypeBoxClass.typeRef
              .appliedTo(lo | bounds.loBound, hi & bounds.hiBound)
          )
    ...
    case tp: AndOrType =>
      tp.derivedAndOrType(captureWildcards(tp.tp1), captureWildcards(tp.tp2))
    case tp: RefinedType =>
      tp.derivedRefinedType(
        captureWildcards(tp.parent),
        tp.refinedName,
        tp.refinedInfo
      )
    ...
    case tp: AnnotatedType =>
      tp.derivedAnnotatedType(captureWildcards(tp.parent), tp.annot)
    case _ => tp
  }
...
trait Inferencing:
  ...
  /** Interpolate undetermined type variables in the widened type of this tree.
    ...
    *   interpolated at the present time Eligible for interpolation are all type
    *   variables owned by the current typerstate that are not in `locked` and
    *   whose `nestingLevel` is `>= ctx.nestingLevel`. Type variables occurring
    *   co- (respectively, contra-) variantly in the tree type or expected type
    *   are minimized (respectvely, maximized). Non occurring type variables are
    *   minimized if they have a lower bound different from Nothing, maximized
    *   otherwise. Type variables appearing non-variantly in the type are left
    *   untouched.
    *
    * Note that even type variables that do not appear directly in a type, can
    * occur with some variance in the type, because of the constraints. E.g if
    * `X` occurs co-variantly in `T` and we have a constraint
    *
    * Y <: X
    *
    * Then `Y` also occurs co-variantly in `T` because it needs to be minimized
    * in order to constrain `T` the least. See `variances` for more detail.
    */
  def interpolateTypeVars(tree: Tree, pt: Type, locked: TypeVars)(using
      Context
  ): tree.type =
    ...
    if (ownedVars ne locked) && !ownedVars.isEmpty then
      ...
      if !qualifying.isEmpty then
        typr.println(
          i"interpolate $tree: ${tree.tpe.widen} in $state, pt = $pt, owned vars = ${state.ownedVars.toList}%, %, qualifying = ${qualifying.toList}%, %, previous = ${locked.toList}%, % / ${state.constraint}"
        )
        val resultAlreadyConstrained =
          tree.isInstanceOf[Apply] || tree.tpe.isInstanceOf[MethodOrPoly]
        if !resultAlreadyConstrained then
        ...
        /** Values of this type report type variables to instantiate with
          * variance indication: +1 variable appears covariantly, can be
          * instantiated from lower bound
          * -1 variable appears contravariantly, can be instantiated from upper
          * bound 0 variable does not appear at all, can be instantiated from
          * either bound
          */
        ...
        val toInstantiate: ToInstantiate =
          ...
          for tvar <- qualifying do
            if !tvar.isInstantiated && constraint.contains(
                tvar
              ) && tvar.nestingLevel >= ctx.nestingLevel
            then
                ...
                if v == null then buf += ((tvar, 0))
                  ...
                  comparing(cmp =>
                    if !cmp.levelOK(tvar.nestingLevel, ctx.nestingLevel) then
                      ...
                      typr.println(
                        i"instantiate nonvariant $tvar of level ${tvar.nestingLevel} to a type variable of level <= ${ctx.nestingLevel}, $constraint"
                      )
                      cmp.atLevel(ctx.nestingLevel, tvar.origin)
                    else
                      typr.println(
                        i"no interpolation for nonvariant $tvar in $state"
                      )
                  )
          buf.toList
        end toInstantiate
        ...
        def typeVarsIn(xs: ToInstantiate): TypeVars =
          xs.foldLeft(SimpleIdentitySet.empty: TypeVars)((tvs, tvi) =>
            tvs + tvi._1
          )

        /** Filter list of proposed instantiations so that they don't constrain
          * further the current constraint.
          */
        def filterByDeps(tvs0: ToInstantiate): ToInstantiate =
          ...
          def step(tvs: ToInstantiate): ToInstantiate = tvs match
            case tvs @ (hd @ (tvar, v)) :: tvs1 =>
              ...
              if v == 0 && !aboveOK then step((tvar, 1) :: tvs1)
              else if v == 0 && !belowOK then step((tvar, -1) :: tvs1)
              else if v == -1 && !aboveOK || v == 1 && !belowOK then
                typr.println(
                  i"drop $tvar, $v in $tp, $pt, qualifying = ${qualifying.toList}, tvs0 = ${tvs0.toList}%, %, excluded = ${excluded.toList}, $constraint"
                )
                step(tvs1)
        ...
        /** Instantiate all type variables in `tvs` in the indicated directions,
          * as described in the doc comment of `ToInstantiate`. If a type
          * variable A is instantiated from below, and there is another type
          * variable B in `buf` that is known to be smaller than A, wait and
          * instantiate all other type variables before trying to instantiate A
          * again. Dually, wait instantiating a type variable from above as long
          * as it has upper bounds in `buf`.
          *
          * This is done to avoid loss of precision when forming unions. An
          * example is in i7558.scala:
          *
          * type Tr[+V1, +O1 <: V1] extension [V2, O2 <: V2](tr: Tr[V2, O2]) def
          * sl: Tr[V2, O2] = ??? def as[V3, O3 <: V3](tr: Tr[V3, O3]) : Tr[V3,
          * O3] = tr.sl
          *
          ...
          * V2 >: V3, O2 >: O3, O2 <: V2
          *
          * where O3 and V3 are type refs with O3 <: V3. If we interpolate V2
          * first to V3 | O2, the widenUnion algorithm will instantiate O2 to
          * V3, leading to the final constraint
          *
          * V2 := V3, O2 := V3
          *
        ...
        def doInstantiate(tvs: ToInstantiate): Unit =
          ...
          def tryInstantiate(tvs: ToInstantiate): ToInstantiate = tvs match
            case (hd @ (tvar, v)) :: tvs1 =>
              ...
              typr.println(
                i"interpolate${
                    if v == 0 then " non-occurring" else ""
                  } $tvar in $state in $tree: $tp, fromBelow = $fromBelow, $constraint"
              )
              if tvar.isInstantiated then tryInstantiate(tvs1)
              else
  ...
  /** If `tvar` represents a parameter of a dependent method type in the current
    * `call` approximate it from below with the type of the actual argument.
    * Skolemize that type if necessary to make it a Singleton.
    */
  private def constrainIfDependentParamRef(tvar: TypeVar, call: Tree)(using
      Context
  ): Unit =
...
/** An enumeration controlling the degree of forcing in "is-fully-defined"
  * checks.
  */
