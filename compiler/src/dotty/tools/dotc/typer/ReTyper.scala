...
/** A version of Typer that keeps all symbols defined and referenced in a
  ...
  */
class ReTyper(nestingLevel: Int = 0)
    extends Typer(nestingLevel)
    with ReChecking:
  import tpd.*
  ...
  override def typedSelect(tree: untpd.Select, pt: Type)(using Context): Tree =
    assertTyped(tree)
    val qual1 =
      withoutMode(Mode.Pattern)(typed(tree.qualifier, AnySelectionProto))
  ...
  override def typedTyped(tree: untpd.Typed, pt: Type)(using Context): Tree =
    ...
    if ctx.mode.isExpr then result.withNotNullInfo(expr1.notNullInfo)
    else result
  ...
  override def typedRefinedTypeTree(tree: untpd.RefinedTypeTree)(using
      Context
  ): TypTree =
  ...
  override def typedUnApply(tree: untpd.UnApply, selType: Type)(using
      Context
  ): UnApply =
    val fun1 =
      // retract PatternOrTypeBits like in typedExpr
      withoutMode(Mode.PatternOrTypeBits)(
        typedUnadapted(tree.fun, AnyFunctionProto)
      )
  ...
  override def typedUnApply(tree: untpd.Apply, selType: Type)(using
      Context
  ): Tree =
  ...
  override def localDummy(cls: ClassSymbol, impl: untpd.Template)(using
      Context
  ): Symbol = impl.symbol

  override def retrieveSym(tree: untpd.Tree)(using Context): Symbol =
    tree.symbol
  override def symbolOfTree(tree: untpd.Tree)(using Context): Symbol =
    tree.symbol
  ...
  override def tryInsertApplyOrImplicit(
      tree: Tree,
      pt: ProtoType,
      locked: TypeVars
  )(fallBack: => Tree)(using Context): Tree =
    fallBack

  override def completeAnnotations(mdef: untpd.MemberDef, sym: Symbol)(using
      Context
  ): Unit = ()

  override def ensureConstrCall(cls: ClassSymbol, parent: Tree, psym: Symbol)(
      using Context
  ): Tree =
    parent

  override def handleUnexpectedFunType(tree: untpd.Apply, fun: Tree)(using
      Context
  ): Tree = fun.tpe match
    case mt: MethodType =>
  ...
  override def addCanThrowCapabilities(expr: untpd.Tree, cases: List[CaseDef])(
      using Context
  ): untpd.Tree =
    expr
  ...
  override def typedUnadapted(tree: untpd.Tree, pt: Type, locked: TypeVars)(
    ...
    try super.typedUnadapted(tree, pt, locked)
      ...
      case NonFatal(ex)
          if ctx.phase != Phases.typerPhase && ctx.phase != Phases.inliningPhase && !ctx.run.enrichedErrorMessage =>
        val treeStr = tree.show(using ctx.withPhase(ctx.phase.prevMega))
        println(
          ctx.run.enrichErrorMessage(
            s"exception while retyping $treeStr of class ${tree.className} # ${tree.uniqueId}"
          )
        )
        throw ex
  ...
  override def inferView(from: Tree, to: Type)(using
      Context
  ): Implicits.SearchResult =
    Implicits.NoMatchingImplicitsFailure
  override def checkCanEqual(ltp: Type, rtp: Type, span: Span)(using
      Context
  ): Unit = ()

  override def widenEnumCase(tree: Tree, pt: Type)(using Context): Tree = tree

  override protected def addAccessorDefs(cls: Symbol, body: List[Tree])(using
      Context
  ): List[Tree] = body
  override protected def checkEqualityEvidence(tree: tpd.Tree, pt: Type)(using
      Context
  ): Unit = ()
  override protected def matchingApply(methType: MethodOrPoly, pt: FunProto)(
      using Context
  ): Boolean = true
  override protected def typedScala2MacroBody(call: untpd.Tree)(using
      Context
  ): Tree = promote(call)
end ReTyper
