...
object FromSymbol:
  ...
  def definitionFromSym(sym: Symbol)(using Context): tpd.Tree =
    ...
    if sym.isClass then classDef(sym.asClass)
    else if sym.isType && sym.is(Case) then typeBindFromSym(sym.asType)
    else if sym.isType then typeDefFromSym(sym.asType)
    else if sym.is(Method) then defDefFromSym(sym.asTerm)
    else if sym.is(Case, butNot = ModuleVal | EnumVal) then
      bindFromSym(sym.asTerm)
  ...
  def classDef(cls: ClassSymbol)(using Context): tpd.TypeDef = cls.defTree match
    ...
    case tpd.EmptyTree =>
      val constrSym = cls.unforcedDecls
      ...
      val body = cls.unforcedDecls
        .filter(!_.isPrimaryConstructor)
        .map(s => definitionFromSym(s))
      tpd.ClassDefWithParents(cls, constr, parents, body)

  def typeDefFromSym(sym: TypeSymbol)(using Context): tpd.TypeDef =
    sym.defTree match
      case tree: tpd.TypeDef => tree
      case tpd.EmptyTree     => tpd.TypeDef(sym)

  def defDefFromSym(sym: TermSymbol)(using Context): tpd.DefDef =
    sym.defTree match
      case tree: tpd.DefDef => tree
      case tpd.EmptyTree    => tpd.DefDef(sym)

  def valDefFromSym(sym: TermSymbol)(using Context): tpd.ValDef =
    sym.defTree match
  ...
  def typeBindFromSym(sym: TypeSymbol)(using Context): tpd.Bind =
    sym.defTree match
      case tree: tpd.Bind => tree
      case tpd.EmptyTree =>
        tpd.Bind(sym, untpd.Ident(nme.WILDCARD).withType(sym.typeRef))
