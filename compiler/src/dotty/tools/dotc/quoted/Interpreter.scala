...
import java.lang.reflect.{InvocationTargetException, Method as JLRMethod}
...
class Interpreter(pos: SrcPos, classLoader0: ClassLoader)(using Context):
  ...
  /** Returns the result of interpreting the code in the tree. Return Some of
    * the result or None if the result type is not consistent with the expected
    * type. Throws a StopInterpretation if the tree could not be interpreted or
    * a runtime exception ocurred.
    */
  final def interpret[T](tree: Tree)(using ct: ClassTag[T]): Option[T] =
    interpretTree(tree)(using emptyEnv) match
      ...
      case obj    =>
        ...
        report.error(
          em"Interpreted tree returned a result of an unexpected type. Expected ${ct.runtimeClass} but was ${obj.getClass}",
          pos
        )
        None

  /** Returns the result of interpreting the code in the tree. Throws a
    * StopInterpretation if the tree could not be interpreted or a runtime
    * exception ocurred.
    */
      ...
      tree.tpe.widenTermRefExpr match
        ...
        case _ =>
          throw new StopInterpretation(
            em"${tree.symbol} could not be inlined",
            tree.srcPos
          )

    // TODO disallow interpreted method calls as arguments
    case Call(fn, args) =>
      if fn.symbol.isConstructor then
        interpretNew(fn.symbol, args.flatten.map(interpretTree))
      else if fn.symbol.is(Module) then interpretModuleAccess(fn.symbol)
      else if fn.symbol.is(Method) && fn.symbol.isStatic then
        interpretedStaticMethodCall(
          fn.symbol.owner,
          fn.symbol,
          interpretArgs(args, fn.symbol.info)
        )
      ...
      then
        if fn.name == nme.asInstanceOfPM then
          interpretModuleAccess(fn.qualifier.symbol)
        else
          interpretedStaticMethodCall(
            fn.qualifier.symbol.moduleClass,
            fn.symbol,
            interpretArgs(args, fn.symbol.info)
          )
      else if env.contains(fn.symbol) then env(fn.symbol)
      else if tree.symbol.is(InlineProxy) then
        interpretTree(tree.symbol.defTree.asInstanceOf[ValOrDefDef].rhs)
      else unexpectedTree(tree)
  ...
  private def interpretArgs(argss: List[List[Tree]], fnType: Type)(using
      Env
  ): List[Object] =
    def interpretArgsGroup(
        args: List[Tree],
        argTypes: List[Type]
    ): List[Object] =
      assert(args.size == argTypes.size)
      val view =
        for (arg, info) <- args.lazyZip(argTypes) yield info match
          case _: ExprType => () => interpretTree(arg) // by-name argument
          case _           => interpretTree(arg) // by-value argument
      view.toList

    fnType.dealias match
      case fnType: MethodType if fnType.hasErasedParams =>
      ...
      case fnType: MethodType =>
        ...
        interpretArgsGroup(argss.head, argTypes) ::: interpretArgs(
          argss.tail,
          fnType.resType
        )
  ...
  private def interpretBlock(stats: List[Tree], expr: Tree)(using Env) =
    ...
    val newEnv = stats.foldLeft(env)((accEnv, stat) =>
      stat match
        ...
        case stat =>
          if unexpected.isEmpty then unexpected = Some(unexpectedTree(stat))
          accEnv
    )
  ...
  private def interpretedStaticMethodCall(
      moduleClass: Symbol,
      fn: Symbol,
      args: List[Object]
  ): Object =
  ...
  private def unexpectedTree(tree: Tree): Object =
    throw new StopInterpretation(
      em"Unexpected tree could not be interpreted: ${tree.toString}",
      tree.srcPos
    )
  ...
  private def loadReplLineClass(moduleClass: Symbol): Class[?] =
    val lineClassloader =
      new AbstractFileClassLoader(ctx.settings.outputDir.value, classLoader)
  ...
  private def getMethod(
      clazz: Class[?],
      name: Name,
      paramClasses: List[Class[?]]
  ): JLRMethod =
    try clazz.getMethod(name.toString, paramClasses*)
    catch
      case _: NoSuchMethodException =>
        val msg =
          em"Could not find method ${clazz.getCanonicalName}.$name with parameters ($paramClasses%, %)"
  ...
  private def stopIfRuntimeException[T](thunk: => T, method: JLRMethod): T =
    try thunk
      ...
      case ex: RuntimeException =>
        ...
        sw.write(
          "A runtime exception occurred while executing macro expansion\n"
        )
          ...
          case targetException =>
            ...
            if !ctx.settings.Ydebug.value then
              ...
              targetException match
                ...
                case _ =>
            else targetException.printStackTrace(new PrintWriter(sw))
            sw.write("\n")
            throw new StopInterpretation(sw.toString.toMessage, pos)
  ...
  private def paramsSig(sym: Symbol): List[Class[?]] =
    def paramClass(param: Type): Class[?] =
      ...
      def javaSig(tpe: Type): String = tpe match
        ...
        case _                  =>
          ...
          val packageName =
            if pack == defn.EmptyPackageClass then "" else s"${pack.fullName}."
          packageName + tpe.classSymbol.fullNameSeparated(FlatName).toString

      val sym = param.classSymbol
      if sym == defn.BooleanClass then classOf[Boolean]
      else if sym == defn.ByteClass then classOf[Byte]
      else if sym == defn.CharClass then classOf[Char]
      else if sym == defn.ShortClass then classOf[Short]
      else if sym == defn.IntClass then classOf[Int]
      else if sym == defn.LongClass then classOf[Long]
      else if sym == defn.FloatClass then classOf[Float]
      else if sym == defn.DoubleClass then classOf[Double]
    ...
    def getExtraParams(tp: Type): List[Type] = tp.widenDealias match
      case tp: AppliedType if defn.isContextFunctionType(tp) =>
        ...
        tp.args.init.map(arg => TypeErasure.erasure(arg)) ::: getExtraParams(
          tp.args.last
        )
      case _ => Nil
...
object Interpreter:
  ...
  object Call:
    ...
    /** Matches an expression that is either a field access or an application It
      * retruns a TermRef containing field accessed or a method reference and
    ...
    private object Call0:
      def unapply(arg: Tree)(using
          Context
      ): Option[(RefTree, List[List[Tree]])] = arg match
        case Select(Call0(fn, args), nme.apply)
            if defn.isContextFunctionType(
              fn.tpe.widenDealias.finalResultType
            ) =>
          Some((fn, args))
        case fn: Ident  => Some((tpd.desugarIdent(fn).withSpan(fn.span), Nil))
        case fn: Select => Some((fn, Nil))
        case Apply(f @ Call0(fn, args1), args2) =>
          if f.tpe.widenDealias.hasErasedParams then Some((fn, args1))
        ...
        targetException match
          case _: NoClassDefFoundError | _: ClassNotFoundException =>
            ...
            if className eq null then None
              ...
              if sym.isDefinedInCurrentRun then Some(sym) else None
          case _ => None
