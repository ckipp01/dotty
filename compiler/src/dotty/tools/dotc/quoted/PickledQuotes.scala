...
import dotty.tools.dotc.core.tasty.{
  PositionPickler,
  TastyPickler,
  TastyPrinter,
  TreePickler
...
object PickledQuotes:
  ...
  def quotedExprToTree[T](expr: quoted.Expr[T])(using Context): Tree =
    ...
    ScopeException.checkInCorrectScope(
      expr1.scope,
      SpliceScope.getCurrent,
      expr1.tree,
      "Expr"
    )
  ...
  /** `typeHole`/`types` argument of
    * `QuoteUnpickler.{unpickleExpr,unpickleExprV2,unpickleType,unpickleTypeV2}`
    */
  enum TypeHole:
    /** `termHole` argument of `QuoteUnpickler.{unpickleExpr, unpickleType}`.
      * From code compiled with Scala 3.0.x and 3.1.x. Note: For `unpickleType`
      * it will always be `null`.
      */
    case V1(
        evalHole: Null |
          ((Int, Seq[scala.quoted.Type[?]]) => scala.quoted.Type[?])
    )

    /** `termHole` argument of `QuoteUnpickler.unpickleExprV2` From code
      * compiled with Scala 3.2.0+
      */
  ...
  def unpickleTerm(
    ...
    val unpickled =
      withMode(Mode.ReadPositions)(unpickle(pickled, isType = false))
    val Inlined(call, Nil, expansion) = unpickled: @unchecked
    val inlineCtx = inlineContext(call)
    val expansion1 = spliceTypes(expansion, typeHole)(using inlineCtx)
    val expansion2 =
      spliceTerms(expansion1, typeHole, termHole)(using inlineCtx)
    cpy.Inlined(unpickled)(call, Nil, expansion2)

  /** Unpickle the tree contained in the TastyType */
  def unpickleTypeTree(pickled: String | List[String], typeHole: TypeHole)(using
      Context
  ): Tree =
    val unpickled =
      withMode(Mode.ReadPositions)(unpickle(pickled, isType = true))
    spliceTypes(unpickled, typeHole)

  /** Replace all term holes with the spliced terms */
  private def spliceTerms(tree: Tree, typeHole: TypeHole, termHole: ExprHole)(
      using Context
  ): Tree =
    def evaluateHoles = new TreeMap:
      override def transform(tree: tpd.Tree)(using Context): tpd.Tree =
        tree match
              ...
              if isTerm then
                val quotedExpr = termHole match
                  case ExprHole.V1(evalHole) =>
                    evalHole.nn.apply(
                      idx,
                      reifyExprHoleV1Args(args),
                      QuotesImpl()
                    )
                  case ExprHole.V2(evalHole) =>
                    evalHole.nn.apply(
                      idx,
                      reifyExprHoleV2Args(args),
                      QuotesImpl()
                    )
          ...
          case tree =>
            if tree.isDef then
              tree.symbol.annotations = tree.symbol.annotations.map { annot =>
  ...
  private def spliceTypes(tree: Tree, typeHole: TypeHole)(using Context): Tree =
    if typeHole.isEmpty then tree
      ...
      tree match
        case Block(stat :: rest, expr1)
            if stat.symbol.hasAnnotation(defn.QuotedRuntime_SplicedTypeAnnot) =>
          val typeSpliceMap = (stat :: rest).iterator.map {
            case tdef: TypeDef =>
              assert(
                tdef.symbol.hasAnnotation(defn.QuotedRuntime_SplicedTypeAnnot)
              )
                ...
                case TypeHole.V1(evalHole) =>
                  tdef.rhs match
                    case TypeBoundsTree(_, Hole(_, idx, args, _), _) =>
                      ...
                      val quotedType =
                        evalHole.nn.apply(idx, reifyTypeHoleArgs(args))
                      PickledQuotes.quotedTypeToTree(quotedType)
          ...
          class ReplaceSplicedTyped extends TypeMap():
            override def apply(tp: Type): Type = tp match
              case tp: ClassInfo =>
                tp.derivedClassInfo(declaredParents =
                  tp.declaredParents.map(apply)
                )
              case tp: TypeRef =>
                typeSpliceMap.get(tp.symbol) match
                  case Some(t)
                      if tp.typeSymbol.hasAnnotation(
                        defn.QuotedRuntime_SplicedTypeAnnot
                      ) =>
                    mapOver(t)
                  case _ => mapOver(tp)
              case _ =>
                mapOver(tp)
          val expansion2 =
            new TreeTypeMap(new ReplaceSplicedTyped).transform(expr1)
          quotePickling.println(i"**** typed quote\n${expansion2.show}")
          expansion2
        case _ =>
          tree

  def reifyTypeHoleArgs(args: List[Tree])(using
      Context
  ): List[scala.quoted.Type[?]] =
    args.map(arg => new TypeImpl(arg, SpliceScope.getCurrent))

  def reifyExprHoleV1Args(
      args: List[Tree]
  )(using Context): List[ExprHole.ArgV1] =
    args.map { arg =>
      if arg.isTerm then
        (q: Quotes) ?=> new ExprImpl(arg, SpliceScope.getCurrent)
      else new TypeImpl(arg, SpliceScope.getCurrent)
    }

  def reifyExprHoleV2Args(
      args: List[Tree]
  )(using Context): List[ExprHole.ArgV2] =
    args.map { arg =>
  ...
  private def pickle(tree: Tree)(using Context): Array[Byte] =
    ...
    quotePickling.println(s"**** pickled quote\n${TastyPrinter
        .showContents(pickled, ctx.settings.color.value == "never")}")
    pickled

  /** Unpickle TASTY bytes into it's tree */
  private def unpickle(pickled: String | List[String], isType: Boolean)(using
      Context
  ): Tree =
    QuotesCache.getTree(pickled) match
      case Some(tree) =>
        ...
        treeOwner(tree) match
          case Some(owner) =>
            ...
            TreeTypeMap(oldOwners = List(owner), newOwners = List(owner))
              .apply(tree)
          case _ =>
            tree
      ...
      case _ =>
        ...
        inContext(unpicklingContext) {
          ...
          quotePickling.println(
            s"**** unpickling quote from TASTY\n${TastyPrinter
                .showContents(bytes, ctx.settings.color.value == "never")}"
          )

          val mode = if isType then UnpickleMode.TypeTree else UnpickleMode.Term
