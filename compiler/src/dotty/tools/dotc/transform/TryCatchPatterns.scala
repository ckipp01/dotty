...
/** Compiles the cases that can not be handled by primitive catch cases as a
...
class TryCatchPatterns extends MiniPhase:
  ...
  override def checkPostCondition(tree: Tree)(using Context): Unit = tree match
    case Try(_, cases, _) =>
      cases.foreach {
        case CaseDef(Typed(_, _), guard, _) =>
          assert(guard.isEmpty, "Try case should not contain a guard.")
        case CaseDef(Bind(_, _), guard, _) =>
          assert(guard.isEmpty, "Try case should not contain a guard.")
        case c =>
          assert(
            isDefaultCase(c),
            "Pattern in Try should be Bind, Typed or default case."
          )
      }
    case _ =>
  ...
  private def isSimpleThrowable(tp: Type)(using Context): Boolean =
    tp.stripped match
  ...
  private def mkFallbackPatterMatchCase(
      patternMatchCases: List[CaseDef],
      span: Span
    ...
    if patternMatchCases.isEmpty then None
      ...
      Some(
        CaseDef(
          Bind(
            fallbackSelector,
            Underscore(fallbackSelector.info).withSpan(span)
          ),
          EmptyTree,
