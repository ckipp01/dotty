...
class BeanProperties(thisPhase: DenotTransformer):
  ...
  def generateAccessors(valDef: ValDef)(using Context): List[Tree] =
    def generateGetter(valDef: ValDef, annot: Annotation)(using Context): Tree =
      val prefix =
        if annot matches defn.BooleanBeanPropertyAnnot then "is" else "get"
      val meth = newSymbol(
        owner = ctx.owner,
    ...
    def maybeGenerateSetter(valDef: ValDef, annot: Annotation)(using
        Context
    ): Option[Tree] =
      Option.when(valDef.denot.asSymDenotation.flags.is(Mutable)) {
        ...
        def body(params: List[List[Tree]]): Tree =
          Assign(ref(valDef.symbol), params.head.head)
        DefDef(meth, body).withSpan(meth.span)
      }
    ...
    symbol
      .getAnnotation(defn.BeanPropertyAnnot)
      .orElse(symbol.getAnnotation(defn.BooleanBeanPropertyAnnot))
      .toList
      .flatMap { annot =>
        generateGetter(valDef, annot) +: maybeGenerateSetter(
          valDef,
          annot
        ) ++: Nil
      }
