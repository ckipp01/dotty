...
import reporting.*
import config.Printers.{exhaustivity as debug}
...
sealed trait Space:
  ...
  def isSubspace(b: Space)(using Context): Boolean =
    ...
    if (a ne a2) || (b ne b2) then a2.isSubspace(b2)
      ...
      trace(s"isSubspace(${show(this)}, ${show(b)})", debug) {
...
object SpaceEngine:
  ...
  def computeSimplify(space: Space)(using Context): Space =
    trace(s"simplify ${show(space)} --> ", debug, show)(space match
      case Prod(tp, fun, spaces) =>
        ...
        else if sps eq spaces then space
      ...
      case Or(spaces) =>
        ...
        else if spaces2.corresponds(spaces)(_ eq _) then space
        else Or(spaces2)
      case typ: Typ =>
  ...
  /** Is `a` a subspace of `b`? Equivalent to `simplify(simplify(a) -
    * simplify(b)) == Empty`, but faster
    */
    ...
    if (a ne a2) || (b ne b2) then isSubspace(a2, b2)
      ...
      (a, b) match
        ...
        case (
              a @ Typ(tp1, _),
              Or(ss)
            ) => // optimization: don't go to subtraction too early
    ...
    trace(s"${show(a)} & ${show(b)}", debug, show) {
      (a, b) match
        ...
        case (a @ Typ(tp1, _), Prod(tp2, fun, ss)) =>
          ...
          else if isSubType(tp1, tp2) then
            a // problematic corner case: inheriting a case class
          else intersectUnrelatedAtomicTypes(tp1, tp2)(b)
        case (Prod(tp1, fun, ss), b @ Typ(tp2, _)) =>
          if isSubType(tp1, tp2) then a
          else if canDecompose(b) then intersect(a, Or(decompose(b)))
          else if isSubType(tp2, tp1) then
            a // problematic corner case: inheriting a case class
          else intersectUnrelatedAtomicTypes(tp1, tp2)(a)
        case (a @ Prod(tp1, fun1, ss1), Prod(tp2, fun2, ss2)) =>
          if !isSameUnapply(fun1, fun2) then
            intersectUnrelatedAtomicTypes(tp1, tp2)(a)
          else if ss1
              .lazyZip(ss2)
              .exists((a, b) => simplify(intersect(a, b)) == Empty)
          then Empty
      ...
      (a, b) match
        ...
        case (a @ Typ(tp1, _), Prod(tp2, fun, ss)) =>
          ...
          if isSubType(tp1, tp2) && covers(fun, tp1, ss.length) then
            minus(
              Prod(tp1, fun, signature(fun, tp1, ss.length).map(Typ(_, false))),
              b
            )
          else if canDecompose(a) then minus(Or(decompose(a)), b)
          else a
        ...
        case (Prod(tp1, fun1, ss1), Prod(tp2, fun2, ss2))
            if !isSameUnapply(fun1, fun2) =>
          a
        case (Prod(tp1, fun1, ss1), Prod(tp2, fun2, ss2))
            if fun1.symbol.name == nme.unapply && ss1.length != ss2.length =>
          a
        case (a @ Prod(tp1, fun1, ss1), Prod(tp2, fun2, ss2)) =>
          val range = ss1.indices.toList
          val cache = Array.fill[Space | Null](ss2.length)(null)
          def sub(i: Int) =
            if cache(i) == null then cache(i) = minus(ss1(i), ss2(i))
            cache(i).nn
  ...
  def isIrrefutable(unapp: TermRef, argLen: Int)(using Context): Boolean =
    ...
    || (unapp.symbol.is(Synthetic) && unapp.symbol.owner.linkedClass
      .is(Case)) // scala2 compatibility
    || unapplySeqTypeElemTp(unappResult).exists // only for unapplySeq
    || isProductMatch(unappResult, argLen)
    || {
      val isEmptyTp =
        extractorMemberType(unappResult, nme.isEmpty, NoSourcePosition)
  ...
  /** Is this an `'{..}` or `'[..]` irrefutable quoted patterns?
    ...
    *   The scrutinee type
    */
  def isIrrefutableQuotedPattern(
      unapp: tpd.Tree,
      implicits: List[tpd.Tree],
      pt: Type
    ...
    implicits.headOption match
      ...
      case Some(
            tpd.Apply(
              tpd.Select(tpd.Quote(tpd.TypeApply(fn, List(tpt)), _), nme.apply),
              _
            )
          )
  ...
  /** Return a space containing the values of both types.
    ...
    */
  def intersectUnrelatedAtomicTypes(tp1: Type, tp2: Type)(sp: Space)(using
      Context
  ): Space = trace(i"atomic intersection: ${AndType(tp1, tp2)}", debug) {
    ...
    if !ctx.mode.is(Mode.SafeNulls) && (tp1.isNullType || tp2.isNullType) then
      ...
      if tp1.classSymbol.isNumericValueClass && tp2.classSymbol.isNumericValueClass
      then intersection
  ...
  def project(pat: Tree)(using Context): Space =
    trace(i"project($pat ${pat.className} ${pat.tpe})", debug, show)(pat match
      case Literal(c) =>
        if c.value.isInstanceOf[Symbol] then
          Typ(c.value.asInstanceOf[Symbol].termRef, decomposed = false)
        else Typ(ConstantType(c), decomposed = false)
      ...
      case Ident(_) | Select(_, _) =>
        Typ(
          erase(pat.tpe.stripAnnots.widenSkolem, isValue = true),
          decomposed = false
        )
      ...
      case UnApply(fun, _, pats) =>
        ...
        if fun.symbol.name == nme.unapplySeq then
          val (arity, elemTp, resultTp) =
            unapplySeqInfo(fun.tpe.widen.finalResultType, fun.srcPos)
          if fun.symbol.owner == defn.SeqFactoryClass && defn.ListType
              .appliedTo(elemTp) <:< pat.tpe
          then
            ...
            if elemTp.exists then
              Prod(
                erase(pat.tpe.stripAnnots, isValue = false),
                funRef,
                projectSeq(pats) :: Nil
              )
            else
              Prod(
                erase(pat.tpe.stripAnnots, isValue = false),
                funRef,
                pats.take(arity - 1).map(project) :+ projectSeq(
                  pats.drop(arity - 1)
                )
              )
        else
  ...
  private def unapplySeqInfo(resTp: Type, pos: SrcPos)(using
      Context
  ): (Int, Type, Type) =
    var resultTp = resTp
  ...
  /** Erase pattern bound types with WildcardType
    *
    * For example, the type `C[T$1]` should match any `C[?]`, thus `v` should be
    * `WildcardType` instead of `T$1`:
    *
    * sealed trait B case class C[T](v: T) extends B (b: B) match { case C(v) =>
    * // case C.unapply[T$1 @ T$1](v @ _):C[T$1] }
    *
    * However, we cannot use WildcardType for Array[?], due to that
    * `Array[WildcardType] <: Array[Array[WildcardType]]`, which may cause false
    * unreachable warnings. See tests/patmat/t2425.scala
    *
    * We cannot use type erasure here, as it would lose the constraints
    * involving GADTs. For example, in the following code, type erasure would
    * loose the constraint that `x` and `y` must be the same type, resulting in
    * false inexhaustive warnings:
    *
    * sealed trait Expr[T] case class IntExpr(x: Int) extends Expr[Int] case
    * class BooleanExpr(b: Boolean) extends Expr[Boolean]
    *
    * def foo[T](x: Expr[T], y: Expr[T]) = (x, y) match { case (IntExpr(_),
    * IntExpr(_)) => case (BooleanExpr(_), BooleanExpr(_)) => }
    *
    * @param inArray
    *   whether `tp` is a type argument to `Array`
    * @param isValue
    *   whether `tp` is the type which match against values
    *
    * If `isValue` is true, then pattern-bound symbols are erased to its upper
    * bound. This is needed to avoid spurious unreachable warnings. See
  ...
  private def erase(
    ...
    trace(
      i"erase($tp${if inArray then " inArray" else ""}${
          if isValue then " isValue" else ""
        })",
      debug
    )(tp match
      ...
      case tp @ AppliedType(tycon, args) =>
        val args2 =
          if tycon.isRef(defn.ArrayClass) then
            ...
            tycon.typeParams.lazyZip(args).map { (tparam, arg) =>
      ...
      case tp @ OrType(tp1, tp2) =>
        OrType(
          erase(tp1, inArray, isValue),
          erase(tp2, inArray, isValue),
          tp.isSoft
        )
  ...
  def projectSeq(pats: List[Tree])(using Context): Space =
    if pats.isEmpty then return Typ(defn.NilType, false)

    val (items, zero) =
      if isWildcardStarArg(pats.last) then
        (pats.init, Typ(defn.ListType.appliedTo(pats.last.tpe.elemType), false))
      else (pats, Typ(defn.NilType, false))

    val unapplyTp =
      defn.ConsType.classSymbol.companionModule.termRef.select(nme.unapply)
  ...
  def isPrimToBox(tp: Type, pt: Type)(using Context): Boolean =
    tp.isPrimitiveValueType && (defn
      .boxedType(tp)
      .classSymbol eq pt.classSymbol)

  /** Is `tp1` a subtype of `tp2`? */
  def isSubType(tp1: Type, tp2: Type)(using Context): Boolean =
    trace(i"$tp1 <:< $tp2", debug, show = true) {
      if tp1 == ConstantType(Constant(null)) && !ctx.mode.is(Mode.SafeNulls)
      then tp2 == ConstantType(Constant(null))
      else tp1 <:< tp2
    }

  /** True if we can assume that the two unapply methods are the same. That is,
    * given the same parameter, they return the same result.
    *
    * We assume that unapply methods are pure, but the same method may be called
    * with different prefixes, thus behaving differently.
    */
  ...
  /** Return term parameter types of the extractor `unapp`. Parameter types of
    * the case class type `tp`. Adapted from `unapplyPlan` in patternMatcher
    */
  def signature(unapp: TermRef, scrutineeTp: Type, argLen: Int)(using
      Context
  ): List[Type] =
    ...
    val resTp = ctx.typeAssigner
      .safeSubstMethodParams(mt, scrutineeTp :: Nil)
      .finalResultType

    val sig =
      if resTp.isRef(defn.BooleanClass) then List()
      else {
        val isUnapplySeq = unappSym.name == nme.unapplySeq

        if isUnapplySeq then
          val (arity, elemTp, resultTp) = unapplySeqInfo(resTp, unappSym.srcPos)
          if elemTp.exists then defn.ListType.appliedTo(elemTp) :: Nil
          else {
          ...
          if arity > 0 then productSelectorTypes(resTp, unappSym.srcPos)
          else {
            val getTp = resTp.select(nme.get).finalResultType.widenTermRefExpr
            if argLen == 1 then getTp :: Nil
    ...
    debug.println(
      s"signature of ${unappSym.showFullName} ----> ${sig.map(_.show).mkString(", ")}"
    )
  ...
  def covers(unapp: TermRef, scrutineeTp: Type, argLen: Int)(using
      Context
  ): Boolean =
    SpaceEngine.isIrrefutable(unapp, argLen)
      || unapp.symbol == defn.TypeTest_unapply && {
        val AppliedType(_, _ :: tp :: Nil) =
          unapp.prefix.widen.dealias: @unchecked
        scrutineeTp <:< tp
      }
  ...
  def decompose(tp: Type)(using Context): List[Type] =
    trace(i"decompose($tp)", debug) {
      def rec(tp: Type, mixins: List[Type]): List[Type] = tp.dealias match
        case AndType(tp1, tp2) =>
          ...
          if parts == ListOfNoType then ListOfNoType
            ...
            parts.collect:
              ...
              case tp if !TypeComparer.provablyDisjoint(tp, tpB) =>
                AndType(tp, tpB)

        case OrType(tp1, tp2) => List(tp1, tp2)
        case tp if tp.isRef(defn.BooleanClass) =>
          List(ConstantType(Constant(true)), ConstantType(Constant(false)))
        ...
        case tp @ AppliedType(Parts(parts), targs)
            if tp.classSymbol.children.isEmpty =>
        ...
        case tp if tp.classSymbol.isDecomposableToChildren =>
          def getChildren(sym: Symbol): List[Symbol] =
            sym.children.flatMap { child =>
              if child eq sym then
                List(
                  sym
                ) // i3145: sealed trait Baz, val x = new Baz {}, Baz.children returns Baz...
              else if tp.classSymbol == defn.TupleClass || tp.classSymbol == defn.NonEmptyTupleClass
              then
                List(
                  child
                ) // TupleN and TupleXXL classes are used for Tuple, but they aren't Tuple's children
              else if (child.is(Private) || child.is(Sealed)) && child.isOneOf(
                  AbstractOrTrait
                )
              then getChildren(child)
              else List(child)
            }
          ...
          val parts = children
            .map { sym =>
              ...
              def inhabited(tp: Type): Boolean = tp.dealias match
                case AndType(tp1, tp2) =>
                  !TypeComparer.provablyDisjoint(tp1, tp2)
  ...
  extension (cls: Symbol)
    /** A type is decomposable to children if it's sealed, abstract (or a trait)
      * \- so its not a sealed concrete class that can be instantiated on its
      * own, has no anonymous children, which we wouldn't be able to name as
      * counter-examples, but does have children.
      *
      * A sealed trait with no subclasses is considered not decomposable and
      * thus is treated as an opaque type. A sealed trait with subclasses that
      * then get removed after `refineUsingParent`, decomposes to the empty
      * list. So that's why we consider whether a type has children.
      */
    def isDecomposableToChildren(using Context): Boolean =
      cls.is(Sealed) && cls.isOneOf(
        AbstractOrTrait
      ) && !cls.hasAnonymousChild && cls.children.nonEmpty

  val ListOfNoType = List(NoType)
  val ListOfTypNoType = ListOfNoType.map(Typ(_, decomposed = true))

  object Parts:
    def unapply(tp: Type)(using Context): PartsExtractor = PartsExtractor(
      decompose(tp)
    )
  ...
  /** Show friendly type name with current scope in mind
    *
    * E.g. C.this.B --> B if current owner is C C.this.x.T --> x.T if current
    * owner is C X[T] --> X C --> C if current owner is C !!!
    */
    ...
    def refine(tp: Type): String = tp.stripped match
      ...
      case tp: AppliedType =>
        refine(tp.typeConstructor) + (
          if showTypeArgs then tp.argInfos.map(refine).mkString("[", ",", "]")
          else ""
        )
  ...
  /** Whether the counterexample is satisfiable. The space is flattened and
    * non-empty.
    */
  def satisfiable(sp: Space)(using Context): Boolean =
    def impossible: Nothing = throw new AssertionError(
      "`satisfiable` only accepts flattened space."
    )
      ...
      val typeParamMap = new TypeMap():
        override def apply(tp: Type): Type = tp match
          case tref: TypeRef if tref.symbol.is(TypeParam) =>
            tvarMap.getOrElseUpdate(
              tref.symbol,
              newTypeVar(tref.underlying.bounds)
            )
          case tp => mapOver(tp)

      constrs.forall { case (tp1, tp2) =>
        typeParamMap(tp1) <:< typeParamMap(tp2)
      }

    checkConstraint(genConstraint(sp))(using ctx.fresh.setNewTyperState())
  end satisfiable

  def showSpaces(ss: Seq[Space])(using Context): String =
    ss.map(show).mkString(", ")

  /** Display spaces */
  def show(s: Space)(using Context): String =
    def params(tp: Type): List[Type] =
      tp.classSymbol.primaryConstructor.info.firstParamTypes
    ...
    def doShow(s: Space, flattenList: Boolean = false): String = s match
      ...
      case Typ(tp: TermRef, _) =>
        if flattenList && tp <:< defn.NilType then ""
      ...
      case Typ(tp, decomposed) =>
        ...
        else if defn.ListType.isRef(sym) then
          if flattenList then "_*" else "_: List"
        else if defn.ConsType.isRef(sym) then
          if flattenList then "_, _*" else "List(_, _*)"
        else if tp.classSymbol.is(Sealed) && tp.classSymbol.hasAnonymousChild
        then "_: " + showType(tp) + " (anonymous)"
        else if tp.classSymbol.is(CaseClass) && !hasCustomUnapply(
            tp.classSymbol
          )
        then
          // use constructor syntax for case class
          showType(tp) + params(tp).map(_ => "_").mkString("(", ", ", ")")
        else if decomposed then "_: " + showType(tp, showTypeArgs = true)
        else "_"
      case Prod(tp, fun, params) =>
        if ctx.definitions.isTupleNType(tp) then
          ...
          if flattenList then
            params.map(doShow(_, flattenList)).filter(_.nonEmpty).mkString(", ")
          else
            params
              .map(doShow(_, flattenList = true))
              .filter(!_.isEmpty)
              .mkString("List(", ", ", ")")
        else {
          val sym = fun.symbol
          ...
          val paramsStr = params
            .map(doShow(_, flattenList = isUnapplySeq))
            .mkString("(", ", ", ")")
  ...
  private def exhaustivityCheckable(sel: Tree)(using Context): Boolean =
    ...
    def isCheckable(tp: Type): Boolean =
      ...
      classSym.is(Case) && {
        if seen.add(tpw) then
          productSelectorTypes(tpw, sel.srcPos).exists(isCheckable(_))
        else
          true // recursive case class: return true and other members can still fail the check
      }
  ...
  /** Return the underlying type of non-module, non-constant, non-enum case
    * singleton types. Also widen ExprType to its result type, and rewrap any
    * annotation wrappers. For example, with `val opt = None`, widen `opt.type`
    * to `None.type`.
    */
  def toUnderlying(tp: Type)(using Context): Type =
    trace(i"toUnderlying($tp)", show = true)(tp match
  ...
  def checkExhaustivity(m: Match)(using Context): Unit =
    if exhaustivityCheckable(m.selector) then
      trace(i"checkExhaustivity($m)", debug) {
        val selTyp = toUnderlying(m.selector.tpe).dealias
        debug.println(i"selTyp = $selTyp")

        val patternSpace = Or(m.cases.foldLeft(List.empty[Space]) { (acc, x) =>
          val space = if x.guard.isEmpty then project(x.pat) else Empty
        ...
        if uncovered.nonEmpty then
          ...
          report.warning(
            PatternMatchExhaustivity(showSpaces(deduped), hasMore),
            m.selector
          )
      }
  ...
  def checkRedundancy(m: Match)(using Context): Unit = if redundancyCheckable(
      m.selector
    )
  then
    trace(i"checkRedundancy($m)", debug) {
      ...
      while i < len do
        ...
        if prev == Empty && covered == Empty
        then // defer until a case is reachable
          deferred ::= pat
        else
          for pat <- deferred.reverseIterator do
            report.warning(MatchCaseUnreachable(), pat.srcPos)
          if pat != EmptyTree // rethrow case of catch uses EmptyTree
            && !pat.symbol.isAllOf(
              SyntheticCase,
              butNot = Method
            ) // ExpandSAMs default cases use SyntheticCase
