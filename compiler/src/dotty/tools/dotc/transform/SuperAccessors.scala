...
/** This class adds super accessors for all super calls that either appear in a
  * trait or have as a target a member of some outer class.
  *
  * It also checks that:
  *
  * (1) Symbols accessed from super are not abstract, or are overridden by an
  * abstract override.
  *
  * (2) If a symbol accessed from super is defined in a real class (not a
  * trait), there are no abstract members which override this member in Java's
  * rules (see SI-4989; such an access would lead to illegal bytecode)
  *
  * (3) Super calls do not go to some synthetic members of Any (see
  * isDisallowed)
  *
...
class SuperAccessors(thisPhase: DenotTransformer):
  ...
  /** Some parts of trees will get a new owner in subsequent phases. These are
    * value class methods, which will become extension methods. (By-name
    * arguments used to be included also, but these don't get a new class
    * anymore, they are just wrapped in a new method).
    *
    * These regions will have to be treated specially for the purpose of adding
    * accessors. For instance, super calls from these regions always have to go
    * through an accessor.
    *
    * The `invalidEnclClass` field, if different from NoSymbol, contains the
    * symbol that is not a valid owner.
    */
  ...
  private def superAccessorCall(sel: Select, mixName: Name = nme.EMPTY)(using
      Context
  ) =
    ...
    val superAcc = clazz.info
      .decl(superName)
      .suchThat(_.signature == superInfo.signature)
      .symbol
      .orElse {
        report.debuglog(s"add super acc ${sym.showLocated} to $clazz")
        val maybeDeferred = if clazz.is(Trait) then Deferred else EmptyFlags
        val acc = newSymbol(
          clazz,
          superName,
        ...
        if !accDefs.contains(clazz) then
          report.error(
            em"Internal error: unable to store accessor definition in ${clazz}. clazz.hasPackageFlag=${clazz
                .is(Package)}. Accessor required for ${sel.toString} ($sel)",
            sel.srcPos
          )
  ...
  end superAccessorCall

  /** Check selection `super.f` for conforming to rules. If necessary, replace
    * by a super accessor call.
    */
  private def transformSuperSelect(sel: Select)(using Context): Tree =
    ...
    if sym.isTerm && !sym.is(Method, butNot = Accessor) && !ctx.owner.isAllOf(
        ParamForwarder
      )
    then
      // ParamForwaders as installed ParamForwarding.scala do use super calls to vals
      report.error(
        em"super may be not be used on ${sym.underlyingSymbol}",
        sel.srcPos
      )
    else if isDisallowed(sym) then
      report.error(
        em"super not allowed here: use this.${sel.name} instead",
        sel.srcPos
      )
      ...
      if !owner.is(Trait) then
        if mix.name.isEmpty then
          ...
          for intermediateClass <- clazz.info.baseClasses.tail
          ...
          def hasClassOverride(member: Symbol, subCls: ClassSymbol): Boolean =
            if subCls == defn.ObjectClass || subCls == member.owner then false
            else if member.overridingSymbol(subCls).exists then true
            else hasClassOverride(member, subCls.superClass.asClass)
          val superCls = clazz.asClass.superClass.asClass
          if owner != superCls && hasClassOverride(sym, superCls) then
            report.error(
    ...
    val needAccessor =
      name.isTermName && (clazz != currentClass || !validCurrentClass || mix.name.isEmpty && clazz
        .is(Trait))

    if needAccessor then
      atPhase(thisPhase.next)(superAccessorCall(sel, mix.name))
    else sel
  end transformSuperSelect

  /** Disallow some super.XX calls targeting Any methods which would otherwise
    * lead to either a compiler crash or runtime failure.
    */
      ...
      case _: This if needsSuperAccessor =>
        ...
        if ctx.owner.enclosingClass.derivesFrom(sym.owner) then
          if sym.is(JavaDefined) then
            report.error(
              em"${ctx.owner} accesses protected $sym inside a concrete trait method: use super.${sel.name} instead",
              sel.srcPos
            )
          sel
        else superAccessorCall(sel)
      case Super(_, mix) =>
  ...
  /** Wrap template to template transform `op` with needed initialization and
    * finalization
    */
  def wrapTemplate(tree: Template)(op: Template => Template)(using
      Context
  ): Template =
  ...
  def wrapDefDef(ddef: DefDef)(op: => DefDef)(using Context): DefDef =
    if isMethodWithExtension(ddef.symbol) then withInvalidCurrentClass(op)
    else op
end SuperAccessors
