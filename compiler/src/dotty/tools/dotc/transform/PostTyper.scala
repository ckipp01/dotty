...
/** A macro transform that runs immediately after typer and that performs the
  * following functions:
  *
  ...
  *
  * (4) Check that `New` nodes can be instantiated, and that annotations are
  * valid
  *
  ...
  *
  * (11) Minimizes `call` fields of `Inlined` nodes to just point to the
  * toplevel class from which code was inlined.
  *
  * The reason for making this a macro transform is that some functions (in
  * particular super and protected accessors and instantiation checks) are
  * naturally top-down and don't lend themselves to the bottom-up approach of a
  * mini phase. The other two functions (forwarding param accessors and
  * synthetic methods) only apply to templates and fit mini-phase or subfunction
  * of a macro phase equally well. But taken by themselves they do not warrant
  * their own group of miniphases before pickling.
  */
class PostTyper extends MacroTransform with IdentityDenotTransformer:
  ...
  override def changesMembers: Boolean =
    true // the phase adds super accessors and synthetic members
  ...
  class PostTyperTransformer extends Transformer:
    ...
    def withNoCheckNews[T](ts: List[New])(op: => T): T =
      ...
      noCheckNews ++= ts
      try op
      finally noCheckNews = saved

    def isCheckable(t: New): Boolean = !inJavaAnnot && !noCheckNews.contains(t)

    /** Mark parameter accessors that are aliases of like-named parameters in
      * their superclass with SuperParamAlias. This info is used in phase
      * ParamForwarding
      */
    private def forwardParamAccessors(impl: Template)(using Context): Unit =
      impl.parents match
            ...
            case MethodType(superParamNames) =>
              for case stat: ValDef <- impl.body do
                ...
                if sym.isAllOf(PrivateParamAccessor, butNot = Mutable)
                  && !sym.info
                    .isInstanceOf[ExprType] // val-parameters cannot be call-by name, so no need to try to forward to them
                then
    ...
    private def transformAnnot(annot: Tree)(using Context): Tree =
      ...
      if inJavaAnnot then checkValidJavaAnnotation(annot)
      try transform(annot)
      ...
      tree match
        case tree: ValOrDefDef if !sym.is(Synthetic) =>
          ...
          if sym.is(Method) then
            ...
            if sym.isOneOf(GivenOrImplicit) then
              ...
              if cls.isOneOf(GivenOrImplicit) then
                sym.updateAnnotationsAfter(
                  thisPhase,
                  atPhase(thisPhase)(
                    cls.annotationsCarrying(Set(defn.CompanionMethodMetaAnnot))
                  )
                    ++ sym.annotations
                )
          else if sym.is(Param) then
            sym.keepAnnotationsCarrying(
              thisPhase,
              Set(defn.ParamMetaAnnot),
              orNoneOf = defn.NonBeanMetaAnnots
            )
          else if sym.is(ParamAccessor) then
            sym.keepAnnotationsCarrying(
              thisPhase,
              Set(defn.GetterMetaAnnot, defn.FieldMetaAnnot)
            )
          else
            sym.keepAnnotationsCarrying(
              thisPhase,
          ...
          if sym.isScala2Macro && !ctx.settings.XignoreScala2Macros.value then
            if !sym.owner.unforcedDecls.exists(p =>
                !p.isScala2Macro && p.name == sym.name && p.signature == sym.signature
              )
            ...
            then
              report.error(
                "No Scala 3 implementation found for this Scala 2 macro.",
                tree.srcPos
              )
        case _ =>
      end match
      processMemberDef(tree)
    end processValOrDefDef

    private def checkInferredWellFormed(tree: Tree)(using Context): Unit =
      tree match
        case tree: TypeTree
    ...
    private def transformSelect(tree: Select, targs: List[Tree])(using
        Context
    ): Tree =
        ...
        case pkg: PackageClassDenotation =>
          ...
          if pobj.exists then
            return transformSelect(
              cpy
                .Select(tree)(qual.select(pobj).withSpan(qual.span), tree.name),
              targs
            )
        case _ =>
    ...
    private def normalizeTypeArgs(tree: TypeApply)(using Context): TypeApply =
      tree.tpe match
        ...
        case _ =>
          ...
          tycon.tpe.widen match
            case tp: PolyType if args.exists(isNamedArg) =>
              ...
              val args1 = reorderArgs(
                tp.paramNames,
                namedArgs.asInstanceOf[List[NamedArg]],
                otherArgs
              )
    ...
    def checkNoConstructorProxy(tree: Tree)(using Context): Unit =
      if tree.symbol.is(ConstructorProxy) then
        report.error(
          em"constructor proxy ${tree.symbol} cannot be used as a value",
          tree.srcPos
        )
    ...
    def checkStableSelection(tree: Tree)(using Context): Unit =
      def check(qual: Tree) =
        if !qual.tpe.isStable then
          report.error(
            em"Parameter untupling cannot be used for call-by-name parameters",
            tree.srcPos
          )
      tree match
        case Select(qual, _) => check(qual) // simple select _n
        case Apply(TypeApply(Select(qual, _), _), _) =>
          check(qual) // generic select .apply[T](n)
    ...
    override def transform(tree: Tree)(using Context): Tree =
      try
        tree match
          ...
          case tree: Ident =>
            if tree.isType then checkNotPackage(tree)
            else
          ...
          case tree: Apply =>
            ...
            val app =
              if methType.hasErasedParams then
                tpd.cpy.Apply(tree)(
                  tree.fun,
                  tree.args
                    .zip(methType.erasedParams)
                    .map((arg, isErased) =>
                      if !isErased then arg
                        ...
                        then
                          arg match
                            case _: RefTree | _: Apply | _: TypeApply
                                if arg.symbol.is(Erased) =>
          ...
          case tree: TypeApply =>
            ...
            for arg <- args do checkInferredWellFormed(arg)
          ...
          case Inlined(call, bindings, expansion) if !call.isEmpty =>
            ...
            val callTrace = Inlines.inlineCallTrace(call.symbol, pos)(using
              ctx.withSource(pos.source)
            )
            cpy.Inlined(tree)(
              callTrace,
          ...
          case tree: ValDef =>
            ...
            checkErasedDef(tree)
            val tree1 =
              cpy.ValDef(tree)(rhs = normalizeErasedRhs(tree.rhs, tree.symbol))
          ...
          case tree: DefDef =>
            ...
            val tree1 =
              cpy.DefDef(tree)(rhs = normalizeErasedRhs(tree.rhs, tree.symbol))
            processValOrDefDef(
              superAcc.wrapDefDef(tree1)(
          ...
          case tree: TypeDef =>
            ...
            if sym.isClass then
              ...
              if sym.isOneOf(GivenOrImplicit) then
                sym.keepAnnotationsCarrying(
                  thisPhase,
                  Set(defn.CompanionClassMetaAnnot),
                  orNoneOf = defn.MetaAnnots
                )
              tree.rhs match
                case impl: Template =>
                  for parent <- impl.parents do
                    Checking.checkTraitInheritance(
                      parent.tpe.classSymbol,
                      sym.asClass,
                      parent.srcPos
                    )
                    ...
                    // type (#16270).
                    val illegalRefs = parent.tpe.namedPartsWith(p =>
                      p.symbol.is(ParamAccessor) && (p.symbol.owner eq sym)
                    )
                    if illegalRefs.nonEmpty then
                      report.error(
                        em"The type of a class parent cannot refer to constructor parameters, but ${parent.tpe} refers to ${illegalRefs
                            .map(_.name.show)
                            .mkString(",")}",
                        parent.srcPos
                      )
              ...
              if sym.owner.is(Package) then
                if ctx.compilationUnit.source.exists && sym != defn.SourceFileAnnot
                then
                  val reference = ctx.settings.sourceroot.value
                  val relativePath = util.SourceFile.relativePath(
                    ctx.compilationUnit.source,
                    reference
                  )
                  sym.addAnnotation(
                    Annotation.makeSourceFile(relativePath, tree.span)
                  )
                if Feature.pureFunsEnabled && sym != defn.WithPureFunsAnnot then
                  sym.addAnnotation(
                    Annotation(defn.WithPureFunsAnnot, tree.span)
                  )
            else
                ...
                case (rhs: LambdaTypeTree, bounds: TypeBounds) =>
                  ...
                  if sym.isOpaqueAlias then
                    VarianceChecker.checkLambda(
                      rhs,
                      TypeBounds.upper(sym.opaqueAlias)
                    )
                case _ =>
            end if
            processMemberDef(super.transform(tree))
          case tree: Bind =>
            if tree.symbol.isType && !tree.symbol.name.is(WildcardParamName)
            then Checking.checkGoodBounds(tree.symbol)
          ...
          case tree: AppliedTypeTree =>
            if tree.tpt.symbol == defn.andType then
              Checking.checkNonCyclicInherited(
                tree.tpe,
                tree.args.tpes,
                EmptyScope,
                tree.srcPos
            ...
            else if tree.tpt.symbol == defn.orType then () // nothing to do
            else Checking.checkAppliedType(tree)
            super.transform(tree)
          ...
          case tree: TypeBoundsTree =>
            ...
            if !alias.isEmpty then
              ...
              if !bounds.contains(alias.tpe) then
                report.error(
                  em"type ${alias.tpe} outside bounds $bounds",
                  tree.srcPos
                )
            super.transform(tree)
          case tree: TypeTree =>
            tree.withType(
              tree.tpe match
                case AnnotatedType(tpe, annot) =>
                  AnnotatedType(tpe, transformAnnot(annot))
                case tpe => tpe
            )
          ...
          case Block(_, Closure(_, _, tpt))
              if ExpandSAMs.needsWrapperClass(tpt.tpe) =>
    ...
    /** Transforms the rhs tree into a its default tree if it is in an `erased`
      * val/def. Performed to shrink the tree that is known to be erased later.
      */
    private def normalizeErasedRhs(rhs: Tree, sym: Symbol)(using Context) =
      if sym.isEffectivelyErased then dropInlines.transform(rhs) else rhs

    private def registerNeedsInlining(tree: Tree)(using Context): Unit =
      if tree.symbol.is(Inline) && !Inlines.inInlineMethod && !ctx.mode.is(
          Mode.InlinedCall
        )
      then ctx.compilationUnit.needsInlining = true

    /** Check if the definition has macro annotation and sets
      * `compilationUnit.hasMacroAnnotations` if needed.
      */
    private def registerIfHasMacroAnnotations(tree: DefTree)(using Context) =
      if !Inlines.inInlineMethod && MacroAnnotations.hasMacroAnnotation(
          tree.symbol
        )
      then ctx.compilationUnit.hasMacroAnnotations = true
    ...
    private def checkMacroAnnotation(sym: Symbol)(using Context) =
      if sym.derivesFrom(defn.MacroAnnotationClass) && !sym.isStatic then
        report.error(
          "classes that extend MacroAnnotation must not be inner/local classes",
          sym.srcPos
        )
    ...
    private def checkErasedDef(tree: ValOrDefDef)(using Context): Unit =
      if tree.symbol.is(Erased, butNot = Macro) then
        ...
        if tpe.derivesFrom(defn.NothingClass) then
          report.error(
            "`erased` definition cannot be implemented with en expression of type Nothing",
            tree.srcPos
          )
        else if tpe.derivesFrom(defn.NullClass) then
          report.error(
            "`erased` definition cannot be implemented with en expression of type Null",
            tree.srcPos
          )

    private def annotateExperimental(sym: Symbol)(using Context): Unit =
      if sym.is(Module) && sym.companionClass.hasAnnotation(
          defn.ExperimentalAnnot
        )
      then
        sym.addAnnotation(Annotation(defn.ExperimentalAnnot, sym.span))
        sym.companionModule.addAnnotation(
          Annotation(defn.ExperimentalAnnot, sym.span)
        )
  end PostTyperTransformer
end PostTyper
