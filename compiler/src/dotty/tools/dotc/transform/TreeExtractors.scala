...
object TreeExtractors:
  ...
  /** Match new C(args) and extract (C, args). Also admit new C(args): T and
    * {new C(args)}.
    */
  object NewWithArgs:
  ...
  /** For an instance v of a value class like: class V(val underlying: X)
    * extends AnyVal Match v.underlying() and extract v
    */
  object ValueClassUnbox:
    def unapply(t: Tree)(using Context): Option[Tree] = t match
      case Apply(sel @ Select(ref, _), Nil) =>
        ...
        if isDerivedValueClass(sym) && (sel.symbol eq valueClassUnbox(
            sym.asClass
          ))
        then Some(ref)
        else None
