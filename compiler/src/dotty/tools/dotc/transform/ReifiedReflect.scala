...
/** Helper methods to construct trees calling methods in `Quotes.reflect` based
  * on the current `quotes` tree
  */
trait ReifiedReflect:
  ...
  def Apply(fn: Tree, args: List[Tree])(using Context) =
    ...
    self
      .select(defn.Quotes_reflect_Apply)
  ...
  def TypeApply(fn: Tree, args: List[Tree])(using Context) =
    ...
    self
      .select(defn.Quotes_reflect_TypeApply)
  ...
  def Assign(lhs: Tree, rhs: Tree)(using Context) =
    self
      .select(defn.Quotes_reflect_Assign)
  ...
  def Inferred(typeTree: Tree)(using Context) =
    self
      .select(defn.Quotes_reflect_Inferred)
  ...
  def Literal(constant: Tree)(using Context) =
    self
      .select(defn.Quotes_reflect_Literal)
  ...
  def TypeReprOf(tpe: Type)(using Context) =
    self
      .select(defn.Quotes_reflect_TypeRepr)
      .select(defn.Quotes_reflect_TypeRepr_of)
      .appliedToType(tpe)
      .appliedTo(
        tpd
          .Quote(TypeTree(tpe), Nil)
          .select(nme.apply)
