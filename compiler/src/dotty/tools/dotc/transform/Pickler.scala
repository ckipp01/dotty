...
object Pickler:
  ...
  /** If set, perform jump target compacting, position and comment pickling, as
    * well as final assembly in parallel with downstream phases; force only in
    * backend.
    */
...
class Pickler extends Phase:
  ...
  /** Drop any elements of this list that are linked module classes of other
    * elements in the list
    */
  private def dropCompanionModuleClasses(clss: List[ClassSymbol])(using
      Context
  ): List[ClassSymbol] =
  ...
  /** Runs given functions with a scratch data block in a serialized fashion
    * (i.e. inside a synchronized block). Scratch data is re-used between calls.
    * Used to conserve on memory usage by avoiding to create scratch data for
    * each pickled unit.
    */
  object serialized:
  ...
  override def run(using Context): Unit =
    ...
    do
        ...
        serialized.run { scratch =>
          ...
          if tree.span.exists then
            ...
            PositionPickler.picklePositions(
              ...
              tree :: Nil,
              positionWarnings,
              scratch.positionBuffer,
              scratch.pickledIndices
            )
      ...
      /** A function that returns the pickled bytes. Depending on
        * `Pickler.ParallelPickling` either computes the pickled data in a
        * future or eagerly before constructing the function value.
        */
  ...
  private def testUnpickler(using Context): Unit =
    ...
    for (cls, unpickler) <- unpicklers do
      val unpickled = unpickler.rootTrees
      testSame(i"$unpickled%\n%", beforePickling(cls), cls)

  private def testSame(unpickled: String, previous: String, cls: ClassSymbol)(
      using Context
  ) =
    import java.nio.charset.StandardCharsets.UTF_8
    def normal(s: String) = new String(s.getBytes(UTF_8), UTF_8)
    val unequal = unpickled.length() != previous.length() || normal(
      unpickled
    ) != normal(previous)
    if unequal then
      ...
      report.error(
        em"""pickling difference for $cls in ${cls.source}, for details:
                    |
