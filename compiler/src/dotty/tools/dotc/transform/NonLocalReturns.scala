...
class NonLocalReturns extends MiniPhase:
  ...
  private def ensureConforms(tree: Tree, pt: Type)(using Context) =
    if tree.tpe <:< pt then tree
    else Erasure.Boxing.adaptToType(tree, pt)

  private def nonLocalReturnControl(using Context) =
  ...
  /** Generate a non-local return throw with given return expression from given
    * method.
    * I.e. for the method's non-local return key, generate:
    *
    * throw new NonLocalReturnControl(key, expr) todo: maybe clone a
    * pre-existing exception instead? (but what to do about exceptions that miss
    * their targets?)
    */
  private def nonLocalReturnThrow(expr: Tree, meth: Symbol)(using Context) =
    Throw(
      New(
        ...
        ref(nonLocalReturnKey(meth)) :: expr.ensureConforms(
          defn.ObjectType
        ) :: Nil
      )
    )
  ...
  /** Transform (body, key) to:
    *
    * { val key = new Object() try { body } catch { case ex:
    * NonLocalReturnControl => if (ex.key().eq(key)) ex.value().asInstanceOf[T]
    * else throw ex } }
    */
  private def nonLocalReturnTry(body: Tree, key: TermSymbol, meth: Symbol)(using
      Context
  ) =
    val keyDef = ValDef(key, New(defn.ObjectType, Nil))
    val ex =
      newSymbol(meth, nme.ex, Case, nonLocalReturnControl, coord = body.span)
  ...
  override def transformDefDef(tree: DefDef)(using Context): Tree =
    nonLocalReturnKeys.remove(tree.symbol) match
      case key: TermSymbol =>
        cpy.DefDef(tree)(rhs = nonLocalReturnTry(tree.rhs, key, tree.symbol))
      case null => tree
