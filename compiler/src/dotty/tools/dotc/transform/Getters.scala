...
/** Performs the following rewritings for fields of a class:
  *
  * <mods> val x: T = e
  * --> <mods> <stable> <accessor> def x: T = e <mods> var x: T = e
  ...
  *
  * Also, generate setters for fields that are private but not private[this] The
  * form of a setter is
  *
  ...
  *
  * Furthermore, assignments to mutable vars with setters are replaced by setter
  * calls
  *
  * p.x = e
  * --> p.x_=(e)
  ...
  *
  * Also, drop the Local flag from all private[this] and protected[this]
  * members. This allows subsequent code motions in Flatten.
  */
  ...
  override def transformSym(d: SymDenotation)(using Context): SymDenotation =
    def noGetterNeeded =
      d.isOneOf(NoGetterNeededFlags) ||
        d.isAllOf(PrivateLocal) && !d.owner.is(Trait) && !isDerivedValueClass(
          d.owner
        ) && !d.is(Lazy) ||
    ...
    var d1 =
      if d.isTerm && (d.is(
          Lazy
        ) || d.owner.isClass) && d.info.isValueType && !noGetterNeeded
      then
        val maybeStable =
          if d.isStableMember then StableRealizable else EmptyFlags
        d.copySymDenotation(
    ...
    if d1.is(Local) then
      if d1 ne d then d1.resetFlag(Local)
