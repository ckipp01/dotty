...
/** This phase hoists complex arguments of supercalls and this-calls out of the
  ...
  * is translated to
  *
  * class B(y: Int) extends A(B#B$superArg$1(this.y)) { private <static> def
  * B$superArg$1(y: Int): Int = { def f(x: Int): Int = x.*(x); f(y) } }
  *
  * An argument is complex if it contains a method or template definition, a
  * this or a new, or it contains an identifier which needs a `this` prefix to
  * be accessed. This is the case if the identifier has neither a global
  * reference nor a reference to a parameter of the enclosing class.
  * @see
  *   needsHoist for an implementation.
  *
  * A hoisted argument definition gets the parameters of the class it is hoisted
  * from as method parameters. The definition is installed in the scope
  * enclosing the class, or, if that is a package, it is made a static method of
  * the class itself.
  */
class HoistSuperArgs extends MiniPhase with IdentityDenotTransformer:
  thisPhase =>
  import ast.tpd.*
  ...
  /** Defines methods for hoisting complex supercall arguments out of parent
    * super calls and constructor definitions. Hoisted superarg methods are
    * collected in `superArgDefs`
    */
  class Hoister(cls: Symbol)(using Context):
    val superArgDefs: mutable.ListBuffer[DefDef] = new mutable.ListBuffer

    /** If argument is complex, hoist it out into its own method and refer to
      * the method instead.
      * @param arg
      *   The argument that might be hoisted
      * @param cdef
      *   The definition of the constructor from which the call is made
      * @param lifted
      *   Argument definitions that were lifted out in a call prefix
      * @return
      *   The argument after possible hoisting
      */
    private def hoistSuperArg(
        arg: Tree,
        cdef: DefDef,
        lifted: List[Symbol]
    ): Tree =
      ...
      lazy val origParams = // The parameters that can be accessed in the supercall
        if constr == cls.primaryConstructor then
          cls.info.decls.filter(d =>
            d.is(TypeParam) || d.is(ParamAccessor) && !d.isSetter
          )
        else allParamSyms(cdef)
      ...
      def newSuperArgMethod(argType: Type) =
        ...
        val argTypeWrtConstr =
          argType.widenTermRefExpr.subst(origParams, allParamRefs(constr.info))
      ...
      /** Only rewire types that are owned by the current Hoister and is an
        * param or accessor
        */
      ...
      arg match
        case _ if arg.existsSubTree(needsHoist) =>
          ...
          val superArgDef = DefDef(
            ...
            prefss =>
              ...
              val tmap = new TreeTypeMap(
                typeMap = new TypeMap:
                  lazy val origToParam =
                    (origParams ::: lifted).zip(paramSyms).toMap
                  def apply(tp: Type) = tp match
                    case tp: NamedType if needsRewire(tp) =>
                      origToParam.get(tp.symbol) match
                        case Some(mappedSym) =>
                          if tp.symbol.isType then mappedSym.typeRef
                          else mappedSym.termRef
          ...
          def termParamRefs(tp: Type, params: List[Symbol]): List[List[Tree]] =
            tp match
              case tp: PolyType =>
                termParamRefs(tp.resultType, params)
              case tp: MethodType =>
                val (thisParams, otherParams) =
                  params.splitAt(tp.paramNames.length)
          ...
          if lifted.nonEmpty then res = res.appliedToArgs(lifted.map(ref))
        ...
        cpy.Block(superCall)(
          stats = defs.mapconserve {
            case vdef: ValDef =>
              try
                cpy.ValDef(vdef)(rhs =
                  hoistSuperArg(vdef.rhs, cdef, lifted.toList)
                )
              finally lifted += vdef.symbol
            case ddef: DefDef =>
              try
                cpy.DefDef(ddef)(rhs =
                  hoistSuperArg(ddef.rhs, cdef, lifted.toList)
                )
    ...
    /** Hoist complex arguments in this-constructor call of secondary
      * constructor out of the class.
      */
    def hoistSuperArgsFromConstr(stat: Tree): Tree = stat match
      case constr: DefDef if constr.symbol.isClassConstructor =>
        ...
        cpy.DefDef(constr)(rhs = constr.rhs match
          case Block(stats @ (superCall :: stats1), expr: Literal) =>
            cpy.Block(constr.rhs)(
              stats.derivedCons(
                hoistSuperArgsFromCall(superCall, constr, lifted),
                stats1
              ),
              expr
            )
