...
/** This phase synthesizes specialized methods for FunctionN, this is done since
  * there are no scala signatures in the bytecode for the specialized methods.
  *
...
class SpecializeApplyMethods extends MiniPhase with InfoTransformer:
  ...
  private def specApplySymbol(sym: Symbol, args: List[Type], ret: Type)(using
      Context
  ): Symbol =
  ...
  private inline def specFun0(inline op: Type => Unit)(using Context): Unit =
    for r <- defn.Function0SpecializedReturnTypes do op(r)

  private inline def specFun1(
      inline op: (Type, Type) => Unit
  )(using Context): Unit =
    for
  ...
  private inline def specFun2(
      inline op: (Type, Type, Type) => Unit
  )(using Context): Unit =
    for
  ...
  override def transformInfo(tp: Type, sym: Symbol)(using Context) = tp match
    case tp: ClassInfo =>
      if sym == defn.Function0 then
        ...
        val scope = tp.decls.cloneScope
        specFun2 { (t1, t2, r) =>
          scope.enter(specApplySymbol(sym, t1 :: t2 :: Nil, r))
        }
        tp.derivedClassInfo(decls = scope)
      else tp
