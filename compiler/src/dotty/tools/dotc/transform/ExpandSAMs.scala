...
import Contexts.*, Symbols.*, Types.*, Flags.*, Decorators.*, StdNames.*,
  Constants.*
import MegaPhase.*
import SymUtils.*
...
/** Expand SAM closures that cannot be represented by the JVM as lambdas to
  * anonymous classes. These fall into five categories
  *
  *   1. Partial function closures, we need to generate isDefinedAt and
  *      applyOrElse methods for these. 2. Closures implementing non-trait
  *      classes 3. Closures implementing classes that inherit from a class
  *      other than Object (a lambda cannot not be a run-time subtype of such a
  *      class) 4. Closures that implement traits which run initialization code.
  *      5. Closures that get synthesized abstract methods in the transformation
  *      pipeline. These methods can be (1) superaccessors, (2) outer
  ...
  def needsWrapperClass(tpe: Type)(using Context): Boolean =
    tpe.classSymbol match
      case cls: ClassSymbol =>
        !isPlatformSam(cls) || cls == defn.PartialFunctionClass
      case _ => false
...
class ExpandSAMs extends MiniPhase:
  ...
  override def transformBlock(tree: Block)(using Context): Tree = tree match
    case Block(stats @ (fn: DefDef) :: Nil, Closure(_, fnRef, tpt))
        if fnRef.symbol == fn.symbol =>
      tpt.tpe match
        case NoType =>
        ...
        case tpe @ SAMType(_)
            if ExpandSAMs.isPlatformSam(tpe.classSymbol.asClass) =>
        ...
        case tpe =>
          ...
          cpy.Block(tree)(
            ...
            AnonClass(
              tpe1 :: Nil,
              fn.symbol.asTerm :: Nil,
              samDenot.symbol.asTerm.name :: Nil
            )
          )
    case _ =>
      tree
  ...
  private def toPartialFunction(tree: Block, tpe: Type)(using Context): Tree =
    ...
    def partialFunRHS(tree: Tree): Match = tree match
      ...
      case _ =>
        Match(
          ref(param.symbol),
          CaseDef(
            untpd.Ident(nme.WILDCARD).withType(param.symbol.info),
            EmptyTree,
            tree
          ) :: Nil
        )
    ...
    val parents = List(
      defn.AbstractPartialFunctionClass.typeRef
        .appliedTo(anonTpe.firstParamTypes.head, anonTpe.resultType),
      defn.SerializableType
    )

    AnonClass(anonSym.owner, parents, tree.span) { pfSym =>
      def overrideSym(sym: Symbol) = sym
        .copy(
          owner = pfSym,
      ...
      def translateMatch(
        ...
        val cases1 =
          if cases.exists(isDefaultCase) then cases
          else
            val selectorTpe = selector.tpe.widen
            val defaultSym =
              newSymbol(pfParam.owner, nme.WILDCARD, SyntheticCase, selectorTpe)
      ...
      val isDefinedAtDef = transformFollowingDeep(
        DefDef(
          isDefinedAtFn,
          isDefinedAtRhs(_)(using ctx.withOwner(isDefinedAtFn))
        )
      )
      val applyOrElseDef = transformFollowingDeep(
        DefDef(
          applyOrElseFn,
          applyOrElseRhs(_)(using ctx.withOwner(applyOrElseFn))
        )
      )
  ...
  private def checkRefinements(tpe: Type, tree: Tree)(using Context): Type =
    tpe.dealias match
      case RefinedType(parent, name, _) =>
        if name.isTermName && tpe.member(name).symbol.ownersIterator.isEmpty
        then // if member defined in the refinement
