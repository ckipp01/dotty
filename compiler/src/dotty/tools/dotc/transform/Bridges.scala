...
import ContextFunctionResults.{
  contextResultCount,
  contextFunctionResultTypeAfter
}
import StdNames.nme
...
class Bridges(root: ClassSymbol, thisPhase: DenotTransformer)(using Context):
  ...
  private class BridgesCursor(using Context)
      extends OverridingPairs.Cursor(root):
    ...
    /** Only use the superclass of `root` as a parent class. This means
      * overriding pairs that have a common implementation in a trait parent are
      * also counted. This is necessary because we generate bridge methods only
      * in classes, never in traits.
      */
    override def parents = Array(root.superClass)

    override def exclude(sym: Symbol) =
      !sym.isOneOf(MethodOrModule) || sym.isAllOf(Module | JavaDefined) || super
        .exclude(sym)
  ...
  def bridgePosFor(member: Symbol): SrcPos =
    (if member.owner == root && member.span.exists then member else root).srcPos

  /** Add a bridge between `member` and `other`, where `member` overrides
    * `other` before erasure, if the following conditions are satisfied.
    *
    ...
    * Additionally, if `member` and `other` do have the same signature, but not
    * the same type after erasure and before elimErasedValueTypes issue an
    * error: A bridge would be needed yet it would clash with the member itself.
    * See neg/i1905.scala
    */
  private def addBridgeIfNeeded(member: Symbol, other: Symbol) =
    def bridgeExists =
      bridgesScope
        .lookupAll(member.name)
        .exists(bridge =>
    ...
    if member.signature == other.signature then
      if !member.info.matches(other.info) then
        report.error(
          em"""bridge generated for member ${desc(member)}
                      |which overrides ${desc(other)}
                      |clashes with definition of the member itself; both have erased type ${info(
              member
            )(using elimErasedCtx)}."""",
          bridgePosFor(member)
        )
    else if !inContext(preErasureCtx)(
        site.memberInfo(member).matches(site.memberInfo(other))
      )
    then
      ...
      report.log(
        i"suppress bridge in $root for ${member} in ${member.owner} and ${other.showLocated} since member infos ${site
            .memberInfo(member)} and ${site.memberInfo(other)} do not match"
      )
    else if !bridgeExists then addBridge(member, other)
  end addBridgeIfNeeded
  ...
  private def addBridge(member: Symbol, other: Symbol) =
    val bridge = other
      .copy(
        ...
        coord = bridgePosFor(member).span
      )
    ...
    if other.owner == root then
      root.delete(other)
      toBeRemoved += other

    val memberCount = contextResultCount(member)

    /** Eta expand application `ref(args)` as needed. To do this correctly, we
      * have to look at the member's original pre-erasure type and figure out
      * which context function types in its result are not yet instantiated.
      */
    def etaExpand(ref: Tree, args: List[Tree])(using Context): Tree =
      def expand(args: List[Tree], tp: Type, n: Int)(using Context): Tree =
        if n <= 0 then
          ...
          ctx.typer.typed(
            untpd.cpy.Apply(ref)(ref, args),
            member.info.finalResultType
          )
        else
          val defn.ContextFunctionType(argTypes, resType, erasedParams) =
            tp: @unchecked
          val anonFun = newAnonFun(
            ctx.owner,
            MethodType(
              argTypes
                .zip(erasedParams.padTo(argTypes.length, false))
          ...
          def lambdaBody(refss: List[List[Tree]]) =
            ...
            val expandedRefs = refs.map(_.withSpan(ctx.owner.span.endPos)) match
              ...
              case refs1 => refs1
            expand(args ::: expandedRefs, resType, n - 1)(using
              ctx.withOwner(anonFun)
            )
      ...
      val start =
        contextFunctionResultTypeAfter(member, otherCount)(using preErasureCtx)
    ...
    def bridgeRhs(argss: List[List[Tree]]) =
      ...
      if member.info.isParameterless then
        ref // can happen if `member` is a module
  ...
  /** Add all necessary bridges to template statements `stats`, and remove at
    * the same time deferred methods in `stats` that are replaced by a bridge
  ...
  def add(stats: List[untpd.Tree]): List[untpd.Tree] =
    ...
    if bridges.isEmpty then stats
      ...
      stats.filterNot(stat =>
        toBeRemoved contains stat.symbol
      ) ::: bridges.toList
end Bridges
