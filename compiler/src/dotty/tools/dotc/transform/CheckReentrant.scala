...
/** A no-op transform that checks whether the compiled sources are re-entrant.
  * If -Ycheck:reentrant is set, the phase makes sure that there are no
  * variables that are accessible from a global object. It excludes from
  * checking paths that are labeled with one of the annotations
  *
  * @sharable
  *   Indicating a class or val can be safely shared
  * @unshared
  *   Indicating an object will not be accessed from multiple threads
  *
  * Currently the analysis is only intended to check the dotty compiler itself.
  * To make it generally useful we'd need to add at least the following:
  *
  *   - Handle polymorphic instantiation: We might instantiate a generic class
  *     with a type that contains vars. If the class contains fields of the
  *     generic type, this may constitute a path to a shared var, which
  *     currently goes undetected.
  *   - Handle arrays: Array elements are currently ignored because they are
  *     often used in an immutable way anyway. To do better, it would be helpful
  *     to have a type for immutable array.
  */
class CheckReentrant extends MiniPhase:
  ...
  def addVars(cls: ClassSymbol)(using Context): Unit =
    if !seen.contains(cls) && !isIgnored(cls) then
      seen += cls
      scanning(cls) {
        for sym <- cls.classInfo.decls do
          if sym.isTerm && !sym.isSetter && !isIgnored(sym) then
        ...
        for parent <- cls.parentSyms do addVars(parent.asClass)
      }

  override def transformTemplate(tree: Template)(using Context): Tree =
    if ctx.settings.YcheckReentrant.value && tree.symbol.owner.isStaticOwner
    then addVars(tree.symbol.owner.asClass)
    tree
end CheckReentrant
