...
object ForwardDepChecks:
  ...
  class LevelInfo(
    ...
    override val levelAndIndex: LevelAndIndex =
      stats
        .foldLeft(outer.levelAndIndex, 0) { (mi, stat) =>
          val (m, idx) = mi
          val m1 = stat match
            case stat: MemberDef =>
              m.updated(stat.symbol, (this: @unchecked, idx))
            case _ => m
          (m1, idx + 1)
        }
        ._1
...
class ForwardDepChecks extends MiniPhase:
  ...
  override def prepareForStats(trees: List[Tree])(using Context): Context =
    if ctx.owner.isTerm then
      ctx.fresh.updateStore(
        LevelInfo,
        new LevelInfo(currentLevel, ctx.owner, trees)
      )
    else ctx
    ...
    if sym.exists && sym.owner.isTerm && !sym.is(Lazy) then
      currentLevel.levelAndIndex.get(sym) match
        case Some((level, symIdx)) if symIdx <= level.maxIndex =>
          report.error(
            ForwardReferenceExtendsOverDefinition(sym, level.refSym),
  ...
  /** Check that self constructor call does not contain references to vals or
    * defs defined later in the secondary constructor's right hand side. This is
    * tricky since the complete self constructor might itself be a block that
    * originated from expanding named and default parameters. In that case we
    * have to go outwards and find the enclosing expression that consists of
    * that block. Test cases in {pos,neg}/complex-self-call.scala.
    */
