...
/** A module that can produce a kind of iterator (`Cursor`), which yields all
  * pairs of overriding/overridden symbols that are visible in some baseclass,
  * unless there's a parent class that already contains the same pairs.
  *
  * Adapted from the 2.9 version of OverridingPairs. The 2.10 version is IMO way
  * too unwieldy to be maintained.
  */
object OverridingPairs:
  ...
  class Cursor(base: Symbol)(using Context):
    ...
    /** Symbols to exclude: Here these are constructors and private locals. But
      * it may be refined in subclasses.
      */
    protected def exclude(sym: Symbol): Boolean =
      !sym.memberCanMatchInheritedSymbols
        || ctx.phase == Phases.checkCapturesPhase && sym.is(
          Recheck.ResetPrivate
        )

    /** The parents of base that are checked when deciding whether an overriding
      * pair has already been treated in a parent class. This may be refined in
      * subclasses. @see Bridges for a use case.
      */
    protected def parents: Array[Symbol] =
      base.info.parents.toArray.map(_.classSymbol)

    /** Does `sym1` match `sym2` so that it qualifies as overriding when both
      * symbols are seen as members of `self`? Types always match. Term symbols
      * match if their membertypes relative to `self` do.
      */
    ...
    private val decls =
      ...
      def fillDecls(bcs: List[Symbol], deferred: Boolean): Unit = bcs match
        case bc :: bcs1 =>
          ...
          while e != null do
            if e.sym.is(Deferred) == deferred && !exclude(e.sym) then
              decls.enter(e.sym)
            e = e.prev
        case nil =>
    ...
    /** Is `parent` a qualified sub-parent of `bc`?
      * @pre
      *   `parent` is a parent class of `base` and it derives from `bc`.
      * @return
      *   true if the `bc`-basetype of the parent's type is the same as the
      *   `bc`-basetype of base. In that case, overriding checks relative to
      *   `parent` already subsume overriding checks relative to `base`. See
      *   neg/11719a.scala for where this makes a difference.
      */
    ...
    /** Is the override of `sym1` and `sym2` already handled when checking a
      * parent of `self`?
      */
    private def isHandledByParent(sym1: Symbol, sym2: Symbol): Boolean =
      val commonParents =
        subParents(sym1.owner).intersect(subParents(sym2.owner))
      commonParents.nonEmpty
      && commonParents.exists(i => canBeHandledByParent(sym1, sym2, parents(i)))

    /** Can pair `sym1`/`sym2` be handled by parent `parentType` which is a
      * common subtype of both symbol's owners? Assumed to be true by default,
      * but overridden in RefChecks.
      */
    protected def canBeHandledByParent(
        sym1: Symbol,
        sym2: Symbol,
        parent: Symbol
    ): Boolean =
    ...
    /** The scope entries that have already been visited as overridden (maybe
      * excluded because of already handled by a parent). These will not appear
      * as overriding
      */
    ...
    /** @post
      ...
      *   nextEntry = curEntry overriding = curEntry.sym
      */
    private def nextOverriding(): Unit =
      @tailrec def loop(): Unit =
        if curEntry != null then
    ...
    /** @post
      *   hasNext = there is another overriding pair overriding = overriding
      *   member of the pair, provided hasNext is true overridden = overridden
      *   member of the pair, provided hasNext is true
      */
      ...
      if nextEntry != null then
        ...
        if nextEntry != null then
          try
            ...
            if overriding.owner != overridden.owner && matches(
                overriding,
                overridden
              )
            then
  ...
  /** Is this `sym1` considered an override of `sym2` (or vice versa) if both
    * are seen as members of `site`? We declare a match if either we have a full
    * match including matching names or we have a loose match with different
    * target name but the types are the same. We leave out pairs of methods in
    * Java classes under the assumption since these have already been checked
    * and handled by javac. This leaves two possible sorts of discrepancies to
    * be reported as errors in `RefChecks`:
    *
    ...
    * OverridingPairs.
    */
  def isOverridingPair(sym1: Symbol, sym2: Symbol, self: Type)(using
      Context
  ): Boolean =
  ...
  /** Let `member` and `other` be members of some common class C with types
    * `memberTp` and `otherTp` in C. Are the two symbols considered an
    * overriding pair in C? We assume that names already match so we test only
    * the types here.
    * @param fallBack
    *   A function called if the initial test is false and `member` and `other`
    *   are term symbols.
    * @param isSubType
    *   A function to be used for checking subtype relationships between term
    *   fields.
    */
  def isOverridingPair(
      member: Symbol,
      memberTp: Type,
      other: Symbol,
      otherTp: Type,
    ...
    if member.isType
    then // intersection of bounds to refined types must be nonempty
