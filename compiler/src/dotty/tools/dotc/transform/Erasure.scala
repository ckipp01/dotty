...
class Erasure extends Phase with DenotTransformer:
  ...
  override def runsAfter: Set[String] =
    Set(InterceptedMethods.name, ElimRepeated.name)
    ...
    ref match
      case ref: SymDenotation =>
        ...
        def erasedName =
          if ref.is(Flags.Method)
            ...
            && (ref.owner.is(
              Flags.Trait
            ) || ref.symbol.allOverriddenSymbols.hasNext)
          then
        ...
        if ref.symbol eq defn.ObjectClass then
          ...
          val newOwner =
            if oldOwner == defn.AnyClass then defn.ObjectClass else oldOwner
          ...
          var newFlags =
            if oldSymbol.is(Flags.TermParam) && isCompacted(
                oldSymbol.owner.denot
              )
            then oldFlags &~ Flags.Param
            else oldFlags
          ...
          if oldSymbol.isRetainedInlineMethod then
            ...
            newAnnotations =
              newAnnotations.filterConserve(!_.isInstanceOf[BodyAnnotation])
          oldSymbol match
      ...
      case ref: JointRefDenotation =>
        new UniqueRefDenotation(
          ref.symbol,
          transformInfo(ref.symbol, ref.symbol.info),
          ref.validFor,
          ref.prefix
        )
      case _ =>
        ref.derivedSingleDenotation(
          ref.symbol,
          transformInfo(ref.symbol, ref.symbol.info)
        )
  ...
  def run(using Context): Unit =
    ...
    unit.tpdTree = eraser.typedExpr(unit.tpdTree)(using
      ctx.fresh.setTyper(eraser).setPhase(this.next)
    )

  /** erased classes get erased to empty traits with Object as parent and an
    * empty constructor
    */
  ...
  override def checkPostCondition(tree: tpd.Tree)(using Context): Unit =
    ...
    tree match
      ...
      case res: tpd.This =>
        assert(
          !ExplicitOuter
            .referencesOuter(ctx.owner.lexicallyEnclosingClass, res),
      ...
      case ret: tpd.Return =>
        ...
        val from =
          if ret.from.isEmpty then ctx.owner.enclosingMethod
          else ret.from.symbol
  ...
  /** Assert that tree type and its widened underlying type are erased. Also
    * assert that term refs have fixed symbols (so we are sure they need not be
    * reloaded using member; this would likely fail as signatures may change
    * after erasure).
    */
  def assertErased(tree: tpd.Tree)(using Context): Unit =
    assertErased(tree.typeOpt, tree)
    if !defn.isPolymorphicAfterErasure(tree.symbol) then
  ...
  def assertErased(tp: Type, tree: tpd.Tree = tpd.EmptyTree)(using
      Context
  ): Unit =
...
object Erasure:
  ...
  /** An attachment on Apply nodes indicating that multiple arguments are passed
    * in a single array. This occurs only if the function implements a
    * FunctionXXL apply.
    */
  ...
  private def applyResultType(mt: MethodType, args: List[Tree])(using
      Context
  ): Type =
    if mt.paramInfos.length <= args.length then mt.resultType
    else MethodType(mt.paramInfos.drop(args.length), mt.resultType)

  /** The method type corresponding to `mt`, except that bunched parameters are
    * expanded. The expansion is determined using the original function tree
    * `origFun` which has a type that is not yet converted to a type with
    * bunched arguments.
    */
  def expandedMethodType(mt: MethodType, origFun: Tree)(using
      Context
  ): MethodType =
    mt.paramInfos match
  ...
  object Boxing:
    ...
    /** Isf this tree is an unbox operation which can be safely removed when
      * enclosed in a box, the unboxed argument, otherwise EmptyTree. Note that
      * one can't always remove a Box(Unbox(x)) combination because the process
      * of unboxing x may lead to throwing an exception. This is important for
      * specialization: calls to the super constructor should not box/unbox
      * specialized fields (see TupleX). (ID)
      */
    private def safelyRemovableUnboxArg(tree: Tree)(using Context): Tree =
      tree match
        case Apply(fn, arg :: Nil)
            if isUnbox(fn.symbol) && defn
              .ScalaBoxedClasses()
              .contains(arg.tpe.typeSymbol) =>
          arg
        case _ =>
          EmptyTree

    def constant(tree: Tree, const: Tree)(using Context): Tree =
      (if isPureExpr(tree) then const else Block(tree :: Nil, const))
        .withSpan(tree.span)

    final def box(tree: Tree, target: => String = "")(using Context): Tree =
      trace(i"boxing ${tree.showSummary()}: ${tree.tpe} into $target") {
        tree.tpe.widen match
          case ErasedValueType(tycon, _) =>
            New(
              tycon,
              cast(tree, underlyingOfValueClass(tycon.symbol.asClass)) :: Nil
            ) // todo: use adaptToType?
          case tp =>
            val cls = tp.classSymbol
            if cls eq defn.UnitClass then
    ...
    def unbox(tree: Tree, pt: Type)(using Context): Tree =
      trace(i"unboxing ${tree.showSummary()}: ${tree.tpe} as a $pt") {
        pt match
          case ErasedValueType(tycon, underlying) =>
            ...
            val tree1 =
              if tree.tpe isRef defn.NullClass then
                ...
                evalOnce(tree) { t =>
                  If(
                    t.select(defn.Object_eq).appliedTo(nullTree),
                    unboxedNull,
                    unboxedTree(t)
                  )
                }
    ...
    /** Generate a synthetic cast operation from tree.tpe to pt. Does not do any
      * boxing/unboxing (this is handled upstream). Casts from and to
      * ErasedValueType are special, see the explanation in
      * ExtensionMethods#transform.
      */
    def cast(tree: Tree, pt: Type)(using Context): Tree =
      trace(i"cast ${tree.tpe.widen} --> $pt", show = true) {
        ...
        if pt isRef defn.UnitClass then unbox(tree, pt)
          ...
          (tree.tpe.widen, pt) match
            ...
            case (
                  tp1 @ ErasedValueType(tycon1, underlying1),
                  tp2 @ ErasedValueType(tycon2, underlying2)
                ) =>
            ...
            case _ =>
              if pt.isPrimitiveValueType then
                primitiveConversion(tree, pt.classSymbol)
              else tree.asInstance(pt)
        end if
      }

    /** Adaptation of an expression `e` to an expected type `PT`, applying the
      * following rewritings exhaustively as long as the type of `e` is not a
      * subtype of `PT`.
      *
      * e -> e() if `e` appears not as the function part of an application e ->
      * box(e) if `e` is of erased value type e -> unbox(e, PT) otherwise, if
      * `PT` is an erased value type e -> box(e) if `e` is of primitive type and
      * `PT` is not a primitive type e -> unbox(e, PT) if `PT` is a primitive
      * type and `e` is not of primitive type e -> cast(e, PT) otherwise
      */
    def adaptToType(tree: Tree, pt: Type)(using Context): Tree = pt match
      ...
      case _ =>
        tree.tpe.widen match
          ...
          case tpw =>
            if pt.isInstanceOf[ProtoType] || tree.tpe <:< pt then tree
            else if tpw.isErasedValueType then
              if pt.isErasedValueType then tree.asInstance(pt)
              else adaptToType(box(tree), pt)
            else if pt.isErasedValueType then adaptToType(unbox(tree, pt), pt)
            else if tpw.isPrimitiveValueType && !pt.isPrimitiveValueType then
              adaptToType(box(tree), pt)
            else if pt.isPrimitiveValueType && !tpw.isPrimitiveValueType then
    ...
    /** The following code:
      ...
      *
      * results in the creation of a closure and an implementation method in the
      * typer:
      *
      * def $anonfun(x: Int): Any = ... val f: Function1[Int, Any] =
      * closure($anonfun)
      *
      * Notice that `$anonfun` takes a primitive as argument, but the SAM
      * (Single Abstract Method) of `Function1` after erasure is:
      *
      ...
      * this would be:
      *
      * def $anonfun$adapted(x: Object): Object =
      * $anonfun(BoxesRunTime.unboxToInt(x)) val f: Function1 =
      * closure($anonfun$adapted)
      *
    ...
    def adaptClosure(tree: tpd.Closure)(using Context): Tree =
      ...
      val Seq(sam: MethodType) =
        lambdaType.possibleSamMethods.map(_.info): @unchecked
      val samParamTypes = sam.paramInfos
      val samResultType = sam.resultType

      /** Can the implementation parameter type `tp` be auto-adapted to a
        * different parameter type in the SAM?
        *
      ...
      /** Can the implementation result type be auto-adapted to a different
        * result type in the SAM?
        *
        * For derived value classes, it's the same story as for parameters. For
        * non-Unit primitives, we can actually rely on the `LambdaMetaFactory`
      ...
      val paramAdaptationNeeded =
        implParamTypes
          .lazyZip(samParamTypes)
          .exists((implType, samType) =>
      ...
      if paramAdaptationNeeded || resultAdaptationNeeded then
        ...
        if isFunction && !ctx.settings.scalajs.value then
          ...
          val specializedFunctionalInterface =
            if !implType.hasErasedParams && defn.isSpecializableFunctionSAM(
                implParamTypes,
                implResultType
              )
            then
              ...
              tpnme
                .JFunctionPrefix(arity)
                .specializedFunction(implResultType, implParamTypes)
          ...
          if !specializedFunctionalInterface.isEmpty then
            return cpy.Closure(tree)(tpt =
              TypeTree(requiredClass(specializedFunctionalInterface).typeRef)
            )

        // Otherwise, generate a new closure implemented with a bridge.
        val bridgeType =
          if paramAdaptationNeeded then
            if resultAdaptationNeeded then sam
            else implType.derivedLambdaType(paramInfos = samParamTypes)
          else implType.derivedLambdaType(resType = samResultType)
        val bridge = newSymbol(
          ctx.owner,
          AdaptedClosureName(meth.symbol.name.asTermName),
          Flags.Synthetic | Flags.Method | Flags.Bridge,
          bridgeType
        )
        Closure(
          bridge,
          bridgeParamss =>
            inContext(ctx.withOwner(bridge)) {
              ...
              val rhs = Apply(
                meth,
                bridgeParams.lazyZip(implParamTypes).map(ctx.typer.adapt(_, _))
              )
              ctx.typer.adapt(rhs, bridgeType.resultType)
            },
          targetType = functionalInterface
        ).withSpan(tree.span)
      else tree
      end if
    end adaptClosure
  ...
  class Typer(erasurePhase: DenotTransformer)
      extends typer.ReTyper
      with NoChecking:
    import Boxing.*
    ...
    private def checkNotErasedClass(tp: Type, tree: untpd.Tree)(using
        Context
    ): Unit = tp match
      ...
      case _ =>
        if tp.isErasedClass then
          ...
          report.error(
            em"illegal reference to erased ${tp.typeSymbol} in $kind that is not itself erased",
            tree1.srcPos
          )
    ...
    def erasedType(tree: untpd.Tree)(using Context): Type =
      val tp = tree.typeOpt
      if tree.isTerm then erasedRef(tp) else valueErasure(tp)
    ...
    /** When erasing most TypeTrees we should not semi-erase value types. This
      * is not the case for [[DefDef#tpt]], [[ValDef#tpt]] and [[Typed#tpt]],
      * they are handled separately by [[typedDefDef]], [[typedValDef]] and
      * [[typedTyped]].
      */
    override def typedTypeTree(tree: untpd.TypeTree, pt: Type)(using
        Context
    ): TypeTree =
    ...
    override def typedTyped(tree: untpd.Typed, pt: Type)(using Context): Tree =
      ...
      if tpt.typeOpt.typeSymbol == defn.UnitClass then
        ...
        val tpt1 = tpt match
          case Block(_, tpt) =>
            tpt // erase type aliases (statements) from type block
          case tpt => tpt
    ...
    override def typedLiteral(tree: untpd.Literal)(using Context): Tree =
      if tree.typeOpt.isRef(defn.UnitClass) then tree.withType(tree.typeOpt)
      else if tree.const.tag == Constants.ClazzTag then
    ...
    /** Type check select nodes, applying the following rewritings exhaustively
      ...
      *
      * e.m1 -> e.m2 if `m1` is a member of a class that erases to Object and
      * `m2` is the same-named member in Object. e.m -> box(e).m if `e` is
      * primitive and `m` is a member or a reference class or `e` has an erased
      * value class type. e.m -> unbox(e).m if `e` is not primitive and `m` is a
      * member of a primtive type. e.m -> cast(e, OT).m if the type of `e` does
      * not conform to OT and `m` is not an array operation.
      *
      * If `m` is an array operation, i.e. one of the members apply, update,
      * length, clone, and <init> of class Array, we additionally try the
      * following rewritings:
      *
      * e.m -> runtime.array_m(e) if ET is Object e.m -> cast(e, ET).m if the
      * type of `e` does not conform to ET e.clone -> e.clone' where clone' is
      * Object's clone method e.m -> e.[]m if `m` is an array operation other
      * than `clone`.
      */
    override def typedSelect(tree: untpd.Select, pt: Type)(using
        ...
        if !sym.exists && tree.name == nme.apply then
          ...
          inContext(preErasureCtx) {
            ...
            if qualTp.derivesFrom(defn.PolyFunctionClass) then
              ...
              eraseErasedFunctionApply(
                qualTp.select(nme.apply).widen.asInstanceOf[MethodType]
              ).classSymbol
            else NoSymbol
          }
        else
      ...
      val sym =
        (if owner eq origSym.maybeOwner then origSym
         else owner.info.decl(tree.name).symbol)
          .orElse {
      ...
      assert(
        sym.exists,
        i"no owner from $owner/${origSym.showLocated} in $tree"
      )
      ...
      def selectArrayMember(qual: Tree, erasedPre: Type): Tree =
        if erasedPre.isAnyRef then
          ...
          assignType(
            untpd.cpy.Select(tree)(qual, tree.name.primitiveArrayOp),
            qual
          )
      ...
      def adaptIfSuper(qual: Tree): Tree = qual match
        case Super(thisQual, untpd.EmptyTypeIdent) =>
          ...
          if sym.owner.is(Flags.Trait) then
            cpy
              .Super(qual)(thisQual, untpd.Ident(sym.owner.asClass.name))
              .withType(SuperType(thisType, sym.owner.typeRef))
          else
            qual.withType(
              SuperType(thisType, thisType.firstParent.typeConstructor)
            )
        case _ =>
          qual

      /** Can we safely use `cls` as a qualifier without getting a runtime error
        * on the JVM due to its accessibility checks?
        */
    ...
    override def typedTypeApply(tree: untpd.TypeApply, pt: Type)(using
      ...
      val ntree = atPhase(erasurePhase) {
        ...
        val interceptCtx =
          if ctx.explicitNulls then ctx.retractMode(Mode.SafeNulls) else ctx
        ...
        case TypeApply(fun, args) =>
          ...
          fun1.tpe.widen match
            case funTpe: PolyType =>
              ...
              untpd.cpy
                .TypeApply(tree)(fun1, args1)
                .withType(funTpe.instantiate(args1.tpes))
            case _ => fun1
        case _ => typedExpr(ntree, pt)

    /** Besides normal typing, this method does uncurrying and collects
      * parameters to anonymous functions of arity > 22.
      */
    override def typedApply(tree: untpd.Apply, pt: Type)(using Context): Tree =
      ...
      fun1.tpe.widen match
        case mt: MethodType =>
          val (
            xmt, // A method type like `mt` but with bunched arguments expanded to individual ones
            bunchArgs, // whether arguments are bunched
            outers
          ) = // the outer reference parameter(s)
          ...
          def app(fun1: Tree): Tree = fun1 match
            ...
            case _ if bunchArgs =>
              mkApply(
                fun1,
                JavaSeqLiteral(args1, TypeTree(defn.ObjectType)) :: Nil
              )
            case _ =>
              mkApply(fun1, args1)
        ...
        case t =>
          if ownArgs.isEmpty then fun1
            ...
            throw new MatchError(
              i"tree $tree has unexpected type of function $fun/$fun1: $t, was $origFunType, args = $ownArgs"
            )
      end match
    end typedApply
    ...
    override def typedSeqLiteral(tree: untpd.SeqLiteral, pt: Type)(using
        Context
    ): SeqLiteral =
    ...
    private def adaptProto(tree: untpd.Tree, pt: Type)(using Context) =
      if pt.isValueType then pt
      else if tree.typeOpt.derivesFrom(ctx.definitions.UnitClass) then
        tree.typeOpt
      else valueErasure(tree.typeOpt)

    override def typedInlined(tree: untpd.Inlined, pt: Type)(using
        Context
    ): Tree =
    ...
    override def typedValDef(vdef: untpd.ValDef, sym: Symbol)(using
      ...
      if sym.isEffectivelyErased then erasedDef(sym)
      else
        checkNotErasedClass(sym.info, vdef)
        super.typedValDef(
          untpd.cpy.ValDef(vdef)(
            tpt = untpd.TypedSplice(TypeTree(sym.info).withSpan(vdef.tpt.span))
          ),
          sym
        )
    ...
    /** Besides normal typing, this function also compacts anonymous functions
      ...
      */
    override def typedDefDef(ddef: untpd.DefDef, sym: Symbol)(using
        Context
    ): Tree =
      if sym.isEffectivelyErased || sym.name.is(BodyRetainerName) then
        ...
        val restpe =
          if sym.isConstructor then defn.UnitType else sym.info.resultType
        var vparams = outerParamDefs(sym)
          ::: ddef.paramss
            .collect { case untpd.ValDefs(vparams) =>
              vparams
            }
            .flatten
            .filterConserve(!_.symbol.is(Flags.Erased))
              ...
              case closureDef(meth) =>
                ...
                for param <- contextParams do
                  if !param.symbol.is(Flags.Erased) then
                    param.symbol
                      .copySymDenotation(owner = sym)
                      .installAfter(erasurePhase)
                    vparams = vparams :+ param
                if crCount == 1 then
                  meth.rhs.changeOwnerAfter(meth.symbol, sym, erasurePhase)
                else skipContextClosures(meth.rhs, crCount - 1)

        var rhs1 = skipContextClosures(
          ddef.rhs.asInstanceOf[Tree],
          contextResultCount(sym)
        )

        if sym.isAnonymousFunction && vparams.length > MaxImplementedFunctionArity
        then
          val bunchedParam = newSymbol(
            sym,
            nme.ALLARGS,
            Flags.TermParam,
          ...
          val paramDefs = vparams.zipWithIndex.map { case (paramDef, idx) =>
            assignType(
              untpd.cpy.ValDef(paramDef)(rhs = selector(idx)),
              paramDef.symbol
            )
          }
        ...
        constr.info match
          case MethodTpe(outerName :: _, outerType :: _, _) =>
            val outerSym = newSymbol(
              constr,
              outerName,
              Flags.Param | Flags.SyntheticArtifact,
              outerType
            )
            ValDef(outerSym) :: Nil
          case _ =>
    ...
    /** For all statements in stats: given a retained inline method and its
      * retainedBody method such as
      *
      * inline override def f(x: T) = body1 private def f$retainedBody(x: T) =
      * body2
      *
      ...
      *
      * Here, the owner of body2 is changed to f and all references to
      * parameters of f$retainedBody are changed to references of corresponding
      * parameters in f.
      *
      * `f$retainedBody` is subseqently mapped to the empty tree in
      * `typedDefDef` which is then dropped in `typedStats`.
      */
    private def addRetainedInlineBodies(stats: List[untpd.Tree])(using
        Context
    ): List[untpd.Tree] =
        ...
        case stat: DefDef @unchecked if stat.symbol.name.is(BodyRetainerName) =>
          ...
          val targetName =
            if retainer.hasAnnotation(defn.TargetNameAnnot) then
              retainer.targetName
                .unmangle(BodyRetainerName)
                .exclude(BodyRetainerName)
            else origName
          val inlineMeth = atPhase(typerPhase) {
            retainer.owner.info
              .decl(origName)
              .matchingDenotation(
                retainer.owner.thisType,
                stat.symbol.info,
                targetName
              )
              .symbol
    ...
    end addRetainedInlineBodies

    override def typedClosure(tree: untpd.Closure, pt: Type)(using
        Context
    ): Tree =
      val xxl = defn.isXXLFunctionClass(tree.typeOpt.typeSymbol)
      var implClosure = super.typedClosure(tree, pt).asInstanceOf[Closure]
      if xxl then
        implClosure = cpy.Closure(implClosure)(tpt =
          TypeTree(defn.FunctionXXLClass.typeRef)
        )
    ...
    override def typedTypeDef(tdef: untpd.TypeDef, sym: Symbol)(using
        Context
    ): Tree =
      EmptyTree

    override def typedClassDef(cdef: untpd.TypeDef, cls: ClassSymbol)(using
        Context
    ): Tree =
      if cls.is(Flags.Erased) then erasedDef(cls)
      else super.typedClassDef(cdef, cls)

    override def typedAnnotated(tree: untpd.Annotated, pt: Type)(using
        Context
    ): Tree =
      typed(tree.arg, pt)

    override def typedStats(stats: List[untpd.Tree], exprOwner: Symbol)(using
        Context
    ): (List[Tree], Context) =
      // discard Imports first, since Bridges will use tree's symbol
      val stats0 = addRetainedInlineBodies(
        stats.filter(!_.isInstanceOf[untpd.Import])
      )(using preErasureCtx)
      val stats1 =
        if takesBridges(ctx.owner) then
          new Bridges(ctx.owner.asClass, erasurePhase).add(stats0)
        else stats0
      val (stats2, finalCtx) = super.typedStats(stats1, exprOwner)
      (stats2.filterConserve(!_.isEmpty), finalCtx)

    /** Finally drops all (language-) imports in erasure. Since some of the
      * language imports change the subtyping, we cannot check the trees before
      * erasure.
      */
    override def typedImport(tree: untpd.Import)(using Context) = EmptyTree

    override def adapt(tree: Tree, pt: Type, locked: TypeVars)(using
        Context
    ): Tree =
      trace(
        i"adapting ${tree.showSummary()}: ${tree.tpe} to $pt",
        show = true
      ) {
        ...
        else if tree.isEmpty then tree
        else if ctx.mode is Mode.Pattern then
          tree // TODO: replace with assertion once pattern matcher is active
        else adaptToType(tree, pt)
      }

    override def simplify(tree: Tree, pt: Type, locked: TypeVars)(using
        Context
    ): tree.type = tree
  end Typer
