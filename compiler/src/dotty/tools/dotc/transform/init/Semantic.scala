...
// ----- Domain definitions --------------------------------
  ...
  /** Abstract values
    ...
    * `Warm` or `Fun`. The following ordering applies for RefSet:
    *
    * R_a ⊑ R_b if R_a ⊆ R_b
    *
    * V ⊑ R if V ∈ R
  ...
  /** An object with all fields initialized but reaches objects under
    * initialization
    *
  ...
  case class Warm(
    ...
    /** If a warm value is in the process of populating parameters, class bodies
      * are not executed.
      */
    ...
    /** Ensure that outers and class parameters are initialized.
      ...
      *     the last iteration have been remove due to heap reversion {@see
      ...
      */
    private def populateParams(): Contextual[this.type] =
      log("populating parameters", printer, (_: Warm).objekt.toString) {
        ...
        extendTrace(klass.defTree) {
          this.callConstructor(ctor, args.map(arg => new ArgInfo(arg, trace)))
        }
  ...
  /** The abstract object which stores value about its fields and immediate
    * outers.
    *
    * Semantically it suffices to store the outer for `klass`. We cache other
    * outers for performance reasons.
    *
    * Note: Object is NOT a value.
    */
  case class Objekt(
      val klass: ClassSymbol,
      val fields: Map[Symbol, Value],
      val outers: Map[ClassSymbol, Value]
  ):
  ...
  object Promoted:
    ...
    extension (promoted: Promoted)
      ...
      def promoteCurrent(thisRef: ThisRef): Unit =
        promoted.isCurrentObjectPromoted = true
  ...
  /** Cache used in fixed point computation
    ...
    * code is finite, we only need to carefully design the abstract domain to be
    * finitary.
    *
    ...
    * abstract interpreter, because when an error happens, we always return the
    * bottom value `Hot` for an expression. It is not a threat for termination
    * because when an error happens, we stop the fixed point computation at the
    * end of the iteration where the error happens. Care must be paid to tests
    * of errors, monotonicity will be broken if we simply ignore the test errors
    * (See `TryReporter`).
    *
    * Note: It's tempting to use location of trees as key. That should be
    * avoided as a template may have the same location as its single statement
    * body. Macros may also create incorrect locations.
    */
  object Cache:
    /** Cache for expressions
      ...
      * abstract values for object fields never change within one iteration. The
      * initial abstraction of a field is always a safe over-approximation
    ...
    /** The heap for abstract objects
      ...
      * object remembers the set of initialized fields. That information is
      * reset in each iteration thus is harmless.
      */
    ...
    class Data extends Cache[Value, Value]:
      /** Global cached values for expressions
        ...
        *
        * It is intended to improve performance for computation related to warm
        * values.
        */
      ...
      /** Abstract heap stores abstract objects
        *
        * The heap serves as cache of summaries for warm objects and is shared
        * for checking all classes.
        *
        * The fact that objects of `ThisRef` are stored in heap is just an
        * engineering convenience. Technically, we can also store the object
        * directly in `ThisRef`.
        *
        * The heap contains objects of two conceptually distinct kinds.
        *
        *   - Objects that are also in `heapStable` are flow-insensitive views
        *     of already initialized objects that are cached for reuse in
        *     analysis of later classes. These objects and their fields should
        *     never change; this is enforced using assertions.
        *
        *   - Objects that are not (yet) in `heapStable` are the flow-sensitive
        *     abstract state of objects being analyzed in the current iteration
        *     of the analysis of the current class. Their fields do change
        *     flow-sensitively: more fields are added as fields become
        *     initialized. These objects are valid only within the current
        *     iteration and are removed when moving to a new iteration of
        *     analyzing the current class. When the analysis of a class reaches
        *     a fixed point, these now stable flow-sensitive views of the object
        *     at the end of the constructor of the analyzed class now become the
        *     flow-insensitive views of already initialized objects and can
        *     therefore be added to `heapStable`.
        */
      ...
      private def commitToStableCache() =
        for
          ...
          (wrapper, res) <- m
        do
          this.stable = stable.updatedNestedWrapper(
            v,
            wrapper.asInstanceOf[ImmutableTreeWrapper],
            res
          )
      ...
      def prepareForNextClass()(using Context) =
        if this.hasChanged then this.heap = this.heapStable
        else
...
// ----- Error Handling -----------------------------------
  ...
  object Reporter:
    ...
    class TryBufferedReporter(backup: Cache.Data)
        extends BufferedReporter
        with TryReporter:
...
// ----- Operations on domains -----------------------------
  extension (a: Value)
    ...
    def widenArg: Contextual[Value] =
      a match
        case _: Ref | _: Fun =>
          val hasError = Reporter.hasErrors {
            a.promote("Argument is not provably transitively initialized (Hot)")
          }
  ...
  extension (ref: Ref)
    ...
    def ensureFresh()(using Cache.Data): ref.type =
      val obj = Objekt(
        ref.klass,
        fields = Map.empty,
        outers = Map(ref.klass -> ref.outer)
      )
    ...
    def updateField(field: Symbol, value: Value): Contextual[Unit] =
      log("set field " + field + " of " + ref + " to " + value) {
        val obj = objekt
        ...
        def isParamUpdate = field.isOneOf(
          Flags.ParamAccessor | Flags.Param
        ) && obj.field(field) == value
        assert(
          !obj.hasField(field) || isParamUpdate,
          field.show + " already init, new = " + value + ", old = " + obj.field(
            field
          ) + ", ref = " + ref
        )
    ...
    def updateOuter(klass: ClassSymbol, value: Value): Contextual[Unit] =
      log("set outer " + klass + " of " + ref + " to " + value) {
        val obj = objekt
        // See the comment in `updateField` for setting the value twice.
        assert(
          !obj.hasOuter(klass) || obj.outer(klass) == value,
          klass.show + " already has outer, new = " + value + ", old = " + obj
            .outer(klass) + ", ref = " + ref
        )
  ...
  extension (value: Value)
    ...
    def select(
        field: Symbol,
        receiver: Type,
        needResolve: Boolean = true
    ): Contextual[Value] = log(
      "select " + field.show + ", this = " + value,
      printer,
      (_: Value).show
    ) {
      if promoted.isCurrentObjectPromoted then Hot
        ...
        value match
          ...
          case ref: Ref =>
            val target =
              if needResolve then resolve(ref.klass, field) else field
            if target.is(Flags.Lazy) then
              val rhs = target.defTree.asInstanceOf[ValDef].rhs
              eval(rhs, ref, target.owner.asClass, cacheResult = true)
            else if target.exists then
              val obj = ref.objekt
              if obj.hasField(target) then obj.field(target)
                ...
                reporter.report(error)
                Hot
            else if ref.klass.isSubClass(receiver.widenSingleton.classSymbol)
            then
              report.error(
                "[Internal error] Unexpected resolution failure: ref.klass = " + ref.klass.show + ", field = " + field.show + Trace.show,
                Trace.position
              )
              Hot
            else
          ...
          case fun: Fun =>
            report.error(
              "[Internal error] unexpected tree in selecting a function, fun = " + fun.expr.show + Trace.show,
              fun.expr
            )
            Hot
    ...
    def call(
      ...
      "call " + meth.show + ", args = " + args.map(_.value.show),
      printer,
      (_: Value).show
    ) {
      ...
      def checkArgsWithParametricity() =
        ...
        var allArgsHot = true
        val allParamTypes =
          methodType.paramInfoss.flatten.map(_.repeatedToSingle)
          ...
          if tryReporter.errors.isEmpty then tryReporter.errors
            ...
            info match
              case typeParamRef: TypeParamRef =>
                ...
                val isWithinBounds =
                  bounds.lo <:< defn.NothingType && defn.AnyType <:< bounds.hi
                def otherParamContains = allParamTypes.exists { param =>
                  param != typeParamRef && param.typeSymbol != defn.ClassTagClass && typeParamRef
                    .occursIn(param)
                }
                ...
                if isWithinBounds && !otherParamContains then
                  tryReporter.abort()
                  Nil
                else tryReporter.errors
              case _ => tryReporter.errors
        }
        (errors, allArgsHot)
      end checkArgsWithParametricity
      ...
      if promoted.isCurrentObjectPromoted then Hot
        ...
        value match
          case Hot =>
            ...
            else if receiver.typeSymbol.isStaticOwner then
              val (errors, allArgsHot) = checkArgsWithParametricity()
              if allArgsHot then Hot: Value
          ...
          case ref: Ref =>
            ...
            val target =
              if !needResolve then meth
              else if superType.exists then meth
              else if meth.name.is(SuperAccessorName) then
                ResolveSuper.rebindSuper(ref.klass, meth)
              else resolve(ref.klass, meth)
            ...
            if target.isOneOf(Flags.Method) then
              if target.hasSource then
              ...
              else if ref.canIgnoreMethodCall(target) then Hot
              else
                ...
                val hasErrors = Reporter.hasErrors {
                  ref.promote(
                    ref.show + " has no source code and is not provably transitively initialized (Hot)."
                  )
                }
                if hasErrors then
              ...
              val obj = ref.objekt
              if obj.hasField(target) then obj.field(target)
              else value.select(target, receiver, needResolve = false)
            else if ref.klass.isSubClass(receiver.widenSingleton.classSymbol)
            then
              report.error(
                "[Internal error] Unexpected resolution failure: ref.klass = " + ref.klass.show + ", meth = " + meth.show + Trace.show,
                Trace.position
              )
              Hot
            else
    ...
    def callConstructor(ctor: Symbol, args: List[ArgInfo]): Contextual[Value] =
      log(
        "call " + ctor.show + ", args = " + args.map(_.value.show),
        printer,
        (_: Value).show
      ) {
        ...
        def addParamsAsFields(args: List[Value], ref: Ref, ctorDef: DefDef) =
          ...
          assert(
            ...
            "arguments = " + args.size + ", params = " + params.size + ", ctor = " + ctor.show
          )
        ...
        value match
          case Hot | Cold | _: RefSet | _: Fun =>
            report.error(
              "[Internal error] unexpected constructor call, meth = " + ctor + ", value = " + value + Trace.show,
              Trace.position
            )
            Hot
          ...
          case ref: Warm if ref.isPopulatingParams =>
            if ctor.hasSource then
              ...
              if ctor.isPrimaryConstructor then
                val tpl =
                  cls.defTree.asInstanceOf[TypeDef].rhs.asInstanceOf[Template]
          ...
          case ref: Ref =>
            if ctor.hasSource then
              ...
              if ctor.isPrimaryConstructor then
                ...
                extendTrace(cls.defTree) {
                  eval(tpl, ref, cls, cacheResult = true)
                }
                ref
              else
                extendTrace(ddef) {
            ...
            else if ref.canIgnoreMethodCall(ctor) then Hot
            else
              // no source code available
    ...
    def instantiate(
      ...
      "instantiating " + klass.show + ", value = " + value + ", args = " + args
        .map(_.value.show),
      printer,
      (_: Value).show
    ) {
      def tryLeak(
          ...
          argValues: List[Value]
      ): Contextual[Value] =
        val argInfos2 =
          args.zip(argValues).map { (argInfo, v) => argInfo.copy(value = v) }
        ...
        if errors.nonEmpty then
          ...
          val error =
            UnsafeLeaking(errors.head, nonHotOuterClass, indices)(trace)
          reporter.report(error)
          Hot
        else warm
      ...
      if promoted.isCurrentObjectPromoted then Hot
        ...
        value match
          ...
          case ref: Ref =>
            ...
            val warm =
              Warm(klass, outer, ctor, argsWidened).ensureObjectExists()
            if argsWidened.exists(_.isCold) then
              tryLeak(warm, klass.owner.lexicallyEnclosingClass, argsWidened)
            else
              val argInfos2 = args.zip(argsWidened).map { (argInfo, v) =>
                argInfo.copy(value = v)
              }
              warm.callConstructor(ctor, argInfos2)
              warm
          ...
          case Fun(body, thisV, klass) =>
            report.error(
              "[Internal error] unexpected tree in instantiating a function, fun = " + body.show + Trace.show,
              Trace.position
            )
            Hot
    ...
    def accessLocal(tmref: TermRef, klass: ClassSymbol): Contextual[Value] =
      ...
      if sym.is(Flags.Param) && sym.owner.isConstructor then
        ...
        thisValue2 match
          ...
          case _ =>
            report.error(
              "[Internal error] unexpected this value accessing local variable, sym = " + sym.show + ", thisValue = " + thisValue2.show + Trace.show,
              Trace.position
            )
            Hot
      else if sym.is(Flags.Param) then Hot
      else
        sym.defTree match
          case vdef: ValDef =>
            ...
            thisValue2 match
              ...
              case ref: Ref =>
                eval(
                  ...
                  ref,
                  enclosingClass,
                  cacheResult = sym.is(Flags.Lazy)
                )

              case _ =>
                report.error(
                  "[Internal error] unexpected this value when accessing local variable, sym = " + sym.show + ", thisValue = " + thisValue2.show + Trace.show,
                  Trace.position
                )
                Hot
            end match
...
// ----- Promotion ----------------------------------------------------
  extension (ref: Ref)
    /** Whether the object is fully assigned
      ...
      *
      * Note that `isFullyFilled = true` does not mean we can use the object
      * freely, as its fields or outers may still reach uninitialized objects.
      */
    def isFullyFilled: Contextual[Boolean] =
      log("isFullyFilled " + ref, printer) {
        val obj = ref.objekt
  ...
  extension (thisRef: ThisRef)
    def tryPromoteCurrentObject(): Contextual[Boolean] =
      log("tryPromoteCurrentObject ", printer) {
        if promoted.isCurrentObjectPromoted then true
  ...
  extension (value: Value)
    ...
    def promote(msg: String): Contextual[Unit] =
      log("promoting " + value + ", promoted = " + promoted, printer) {
        if !promoted.isCurrentObjectPromoted then
          ...
          value match
            ...
            case thisRef: ThisRef =>
              ...
              if emptyFields.isEmpty then promoted.promoteCurrent(thisRef)
              else
                val fields = "Non initialized field(s): " + emptyFields
                  .map(_.show)
                  .mkString(", ") + "."
            ...
            case fun @ Fun(body, thisV, klass) =>
              if !promoted.contains(fun) then
                val errors = Reporter.stopEarly {
                  ...
                  res.promote(
                    "Only transitively initialized (Hot) values can be returned by functions. The function " + fun.show + " returns " + res.show + "."
                  )
                }
                if errors.nonEmpty then
                  reporter.report(UnsafePromotion(msg, errors.head)(trace))
                else promoted.add(fun)
  ...
  extension (warm: Warm)
    /** Try early promotion of warm objects
      ...
      *
      *   1. for each concrete method `m` of the warm object: call the method
      *      and promote the result
      *
      * 2. for each concrete field `f` of the warm object: promote the field
      * value
      *
      * If the object contains nested classes as members, the checker simply
      * reports a warning to avoid expensive checks.
      */
    def tryPromote(msg: String): Contextual[List[Error]] =
      log("promote " + warm.show + ", promoted = " + promoted, printer) {
        val obj = warm.objekt

        def doPromote(
            klass: ClassSymbol,
            subClass: ClassSymbol,
            subClassSegmentHot: Boolean
        )(using Reporter): Unit =
          ...
          if subClassSegmentHot && !isHotSegment then
            report.error(
              "[Internal error] Expect current segment to be transitively initialized (Hot) in promotion, current klass = " + klass.show +
                ", subclass = " + subClass.show + Trace.show,
              Trace.position
            )
          ...
          if !isHotSegment then
            for member <- klass.info.decls do
              if member.isClass then
                val error = PromoteError(
                  "Promotion cancelled as the value contains inner " + member.show + "."
                )(Trace.empty)
                reporter.report(error)
              else if !member.isType && !member.isConstructor && !member.is(
                  Flags.Deferred
                )
              then
                given Trace = Trace.empty
                if member.is(Flags.Method, butNot = Flags.Accessor) then
                  val args = member.info.paramInfoss.flatten.map(_ =>
                    new ArgInfo(Hot: Value, Trace.empty)
                  )
                  val res = warm.call(
                    member,
                    args,
                    receiver = warm.klass.typeRef,
                    superType = NoType
                  )
                  withTrace(trace.add(member.defTree)) {
                    res.promote(
                      "Could not verify that the return value of " + member.show + " is transitively initialized (Hot). It was found to be " + res.show + "."
                    )
                  }
                else
                  ...
                  withTrace(trace.add(member.defTree)) {
                    res.promote(
                      "Could not verify that the field " + member.show + " is transitively initialized (Hot). It was found to be " + res.show + "."
                    )
                  }
            end for
          end if
          ...
          if !klass.is(Flags.Trait) then
            ...
            if superCls.hasSource then
              doPromote(superCls.asClass, klass, isHotSegment)
            val mixins = klass.baseClasses.tail.takeWhile(_ != superCls)
            for mixin <- mixins if mixin.hasSource do
              doPromote(mixin.asClass, klass, isHotSegment)
        end doPromote
...
// ----- API --------------------------------
  ...
  private def checkClass(
    ...
    def iterate(): Unit =
      ...
      if cache.hasChanged && reporter.errors.isEmpty && cache.isUsed then
        ...
        cache.prepareForNextIteration()
        iterate()
      else cache.prepareForNextClass()
  ...
  def checkClasses(classes: List[ClassSymbol])(using Context): Unit =
    given Cache.Data()
    for classSym <- classes if isConcreteClass(classSym) do checkClass(classSym)
...
// ----- Semantic definition --------------------------------
  ...
  extension (arg: ArgInfo)
    def promote: Contextual[Unit] = withTrace(arg.trace) {
      arg.value.promote(
        "Could not verify that the method argument is transitively initialized (Hot). It was found to be " + arg.value.show + ". Only transitively initialized arguments may be passed to methods (except constructors)."
      )
    }

  /** Evaluate an expression with the given value for `this` in a given class
    * `klass`
    *
    * Note that `klass` might be a super class of the object referred by
    * `thisV`. The parameter `klass` is needed for `this` resolution. Consider
    * the following code:
    *
    * class A { A.this class B extends A { A.this } }
    *
    * As can be seen above, the meaning of the expression `A.this` depends on
    * where it is located.
    *
    ...
    * @param thisV
    *   The value for `C.this` where `C` is represented by the parameter
    *   `klass`.
    * @param klass
  ...
  def eval(
    ...
    "evaluating " + expr.show + ", this = " + thisV.show + " in " + klass.show,
    printer,
    (_: Value).show
  ) {
    cache.cachedEval(thisV, expr, cacheResult, default = Hot) { expr =>
  ...
  def evalArgs(
      ...
      thisV: Ref,
      klass: ClassSymbol
  ): Contextual[List[ArgInfo]] =
    val argInfos = new mutable.ArrayBuffer[ArgInfo]
    args.foreach { arg =>
      val res =
        if arg.isByName then Fun(arg.tree, thisV, klass)
        else eval(arg.tree, thisV, klass)
  ...
  def cases(expr: Tree, thisV: Ref, klass: ClassSymbol): Contextual[Value] =
    ...
    expr match
        ...
        ref match
          case Select(supert: Super, _) =>
            ...
            val thisValue2 = extendTrace(ref) {
              resolveThis(thisTp.classSymbol.asClass, thisV, klass)
            }
            withTrace(trace2) {
          ...
          case id: Ident =>
            id.tpe match
              case TermRef(NoPrefix, _) =>
                ...
                val thisValue2 = extendTrace(ref) {
                  resolveThis(enclosingClass, thisV, klass)
                }
      ...
      case Typed(expr, tpt) =>
        if tpt.tpe.hasAnnotation(defn.UncheckedAnnot) then Hot
        else
      ...
      case Assign(lhs, rhs) =>
        lhs match
          case Select(qual, _) =>
            ...
            extendTrace(expr) {
              res.ensureHot(
                "The RHS of reassignment must be transitively initialized (Hot). It was found to be " + res.show + ". "
              )
            }
          case id: Ident =>
            val res = eval(rhs, thisV, klass)
      ...
      case Annotated(arg, annot) =>
        if expr.tpe.hasAnnotation(defn.UncheckedAnnot) then Hot
      ...
      case Match(selector, cases) =>
        ...
        extendTrace(selector) {
          res.ensureHot(
            "The value to be matched needs to be transitively initialized (Hot). It was found to be " + res.show + ". "
          )
        }
      ...
      case Return(expr, from) =>
        ...
        extendTrace(expr) {
          res.ensureHot(
            "return expression must be transitively initialized (Hot). It was found to be " + res.show + ". "
          )
        }
  ...
  /** Handle semantics of leaf nodes
    ...
    */
  def cases(tp: Type, thisV: Ref, klass: ClassSymbol): Contextual[Value] =
    log("evaluating " + tp.show, printer, (_: Value).show) {
      tp match
        case _: ConstantType =>
          Hot
        ...
        case tmref: TermRef =>
          ...
          if cls.exists && cls.isStaticOwner then
            if klass.isContainedIn(cls) then
              ...
              cases(tmref.prefix, thisV, klass)
                .select(tmref.symbol, receiver = tmref.prefix)
            else Hot
          else
            cases(tmref.prefix, thisV, klass)
              .select(tmref.symbol, receiver = tmref.prefix)
        ...
        case tp @ ThisType(tref) =>
          ...
          if cls.isStaticOwner && !klass.isContainedIn(cls) then
            ...
            Hot
          else resolveThis(cls, thisV, klass)
        ...
        case _ =>
          report.error(
            "[Internal error] unexpected type " + tp + Trace.show,
            Trace.position
          )
          Hot
    }
  ...
  /** Resolve C.this that appear in `klass`
    ...
    * @param thisV
    *   The value for `D.this` where `D` is represented by the parameter
    *   `klass`.
    * @param klass
  ...
  def resolveThis(
    ...
    "resolving " + target.show + ", this = " + thisV.show + " in " + klass.show,
    printer,
    (_: Value).show
  ) {
      ...
      thisV match
        ...
        case ref: Ref =>
          ...
          if !obj.hasOuter(klass) then
            val error =
              "[Internal error] outer not yet initialized, target = " + target + ", klass = " + klass + ", object = " + obj + Trace.show
            report.error(error, Trace.position)
            Hot
          else resolveThis(target, obj.outer(klass), outerCls)
        case RefSet(refs) =>
          refs.map(ref => resolveThis(target, ref, klass)).join
        case fun: Fun =>
          report.error(
            "[Internal error] unexpected thisV = " + thisV + ", target = " + target.show + ", klass = " + klass.show + Trace.show,
            Trace.position
          )
          Cold
        case Cold => Cold
  ...
  def outerValue(
    ...
    if tref.prefix == NoPrefix then
      ...
      outerV
    else if cls.isAllOf(Flags.JavaInterface) then Hot
  ...
  /** Initialize part of an abstract object in `klass` of the inheritance chain
    ...
    */
  def init(tpl: Template, thisV: Ref, klass: ClassSymbol): Contextual[Value] =
    log("init " + klass.show, printer, (_: Value).show) {
      ...
      def initParent(parent: Tree, tasks: Tasks) =
        parent match
          case tree @ Block(
                stats,
                NewExpr(tref, New(tpt), ctor, argss)
              ) => // can happen
            eval(stats, thisV, klass)
            val args = evalArgs(argss.flatten, thisV, klass)
        ...
        mixins.reverse.foreach { mixin =>
          parents.find(_.tpe.classSymbol == mixin) match
            ...
            case None =>
              ...
              val tref =
                typeRefOf(klass.typeRef.baseType(mixin).typeConstructor)
              ...
              if ctor.exists then
                ...
                val args: List[ArgInfo] = ctor.info.paramInfoss.flatten.map(_ =>
                  new ArgInfo(Hot, Trace.empty)
                )
      ...
      // class body
      if thisV.isThisRef || !thisV.asInstanceOf[Warm].isPopulatingParams then
        tpl.body.foreach {
          case vdef: ValDef
              if !vdef.symbol.is(Flags.Lazy) && !vdef.rhs.isEmpty =>
            ...
            then thisV.updateField(vdef.symbol, Hot)
            else thisV.updateField(vdef.symbol, res)
            fieldsChanged = true
