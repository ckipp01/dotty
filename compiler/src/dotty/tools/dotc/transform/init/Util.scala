...
object Util:
  ...
  def typeRefOf(tp: Type)(using Context): TypeRef =
    tp.dealias.typeConstructor match
  ...
  object Call:
    ...
    def unapply(tree: Tree)(using Context): Option[(Tree, List[List[Arg]])] =
      tree match
        ...
        case ref: RefTree
            if ref.tpe.widenSingleton.isInstanceOf[MethodicType] =>
          Some((ref, Nil))
  ...
  object NewExpr:
    def unapply(tree: Tree)(using
        Context
    ): Option[(TypeRef, New, Symbol, List[List[Arg]])] =
      tree match
        case Call(fn @ Select(newTree: New, init), argss)
            if init == nme.CONSTRUCTOR =>
  ...
  def resolve(cls: ClassSymbol, sym: Symbol)(using Context): Symbol =
    log("resove " + cls + ", " + sym, printer, (_: Symbol).show) {
      if sym.isEffectivelyFinal || sym.isConstructor then sym
