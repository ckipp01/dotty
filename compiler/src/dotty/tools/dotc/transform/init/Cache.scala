...
/** The co-inductive cache used for analysis
  ...
  *
  * def analysis(entryExp: Expr) = { def iterate(entryExp: Expr)(using Cache) =
  * eval(entryExp, initConfig) if cache.hasChanged && noErrors then cache.last =
  * cache.current cache.current = Empty cache.changed = false iterate(entryExp)
  * else reportErrors
  *
  * def eval(expr: Expr, config: Config)(using Cache) = cache.cachedEval(config,
  * expr) { // Actual recursive evaluation of expression. // // Only executed if
  * the entry `(exp, config)` is not in the output cache. }
  *
  * iterate(entryExp)(using new Cache) }
  *
  * See the documentation for the method `Cache.cachedEval` for more
  * information.
  *
  * What goes to the configuration (`Config`) and what goes to the result
  * (`Res`) need to be decided by the specific analysis and justified by
  * reasoning about soundness.
  *
  * @tparam Config
...
class Cache[Config, Res]:
  ...
  /** Whether any value in the output cache (this.current) was accessed after
    * being added. If no cached values are used after they are added for the
    * first time then another iteration of analysis is not needed.
    */
  ...
  /** Evaluate an expression with cache
    ...
    *
    * if don't cache result then return eval(expr) if
    * this.current.contains(config, expr) then return cached value else val
    * assumed = this.last(config, expr) or bottom value if absent
    * this.current(config, expr) = assumed val actual = eval(expr)
    *
    * if assumed != actual then this.changed = true this.current(config, expr) =
    * actual
    */
  def cachedEval(
      config: Config,
...
object Cache:
  ...
  /** A wrapper for trees for storage in maps based on referential equality of
    * trees.
    */
  ...
  /** For queries on the heap, reuse the same wrapper to avoid unnecessary
    * allocation.
    *
  ...
  extension [Config, Res](cache: ExprValueCache[Config, Res])
    def get(config: Config, expr: Tree)(using
        queryWrapper: MutableTreeWrapper
    ): Option[Res] =
      queryWrapper.queryTree = expr
      cache.get(config).flatMap(_.get(queryWrapper))

    def removed(config: Config, expr: Tree)(using
        queryWrapper: MutableTreeWrapper
    ) =
      queryWrapper.queryTree = expr
