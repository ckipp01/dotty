...
/** This transform normalizes type tests and type casts, also replacing type
  * tests with singleton argument type with reference equality check Any
  * remaining type tests
  ...
  *
  * Unfortunately this phase ended up being not Y-checkable unless types are
  * erased. A cast to an ConstantType(3) or x.type cannot be rewritten before
  * erasure. That's why TypeTestsCasts is called from Erasure.
  */
object TypeTestsCasts:
  ...
  /** Tests whether `(x: X).isInstanceOf[P]` is uncheckable at runtime,
    * returning the reason, or the empty string if it is checkable.
    *
    * First do the following substitution: (a) replace `T @unchecked` and
    * pattern binder types (e.g., `_$1`) in P with WildcardType
    *
    * Then check:
    *
    *   1. if `X <:< P`, "" 2. if `P` is a singleton type, "" 3. if `P` refers
    *      to an abstract type member or type parameter, "it refers to an
    *      abstract type member or type parameter" 4. if `P = Array[T]`,
    *      checkable(E, T) where `E` is the element type of `X`, defaults to
    *      `Any`. 5. if `P` is `pre.F[Ts]` and `pre.F` refers to a class which
    *      is not `Array`: (a) replace `Ts` with fresh type variables `Xs` (b)
    *      constrain `Xs` with `pre.F[Xs] <:< X` (c) maximize `pre.F[Xs]` (d) if
    *      !`pre.F[Xs] <:< P`, "its type arguments can't be determined from $X"
    *      6. if `P = T1 | T2` or `P = T1 & T2`, checkable(X, T1) &&
    *      checkable(X, T2). 7. if `P` is a refinement type, "it's a refinement
    *      type" 8. if `P` is a local class which is not statically reachable
    *      from the scope where `X` is defined, "it's a local class" 9. if `X`
    *      is `T1 | T2`, checkable(T1, P) && checkable(T2, P). 10. otherwise, ""
    */
  def whyUncheckable(X: Type, P: Type, span: Span)(using Context): String =
    atPhase(Phases.refchecksPhase.next) {
      extension (inline s1: String)
        inline def &&(inline s2: String): String = if s1 == "" then s2 else s1
      extension (inline b: Boolean)
        inline def |||(inline s: String): String = if b then "" else s
      ...
      def typeArgsTrivial(X: Type, P: AppliedType)(using Context) =
        inContext(ctx.fresh.setExploreTyperState().setFreshGADTBounds) {
          ...
          withMode(Mode.GadtConstraintInference) {
            ...
            TypeComparer.constrainPatternType(
              P1,
              X,
              forceInvariantRefinement = true
            )
            debug.println(
              TypeComparer.explained(
                _.constrainPatternType(P1, X, forceInvariantRefinement = true)
              )
            )
          }
      ...
      def recur(X: Type, P: Type): String =
        trace(s"recur(${X.show}, ${P.show})") {
          (X <:< P) ||| P.dealias.match
            case _: SingletonType => ""
            case _: TypeProxy if isAbstract(P) =>
              i"it refers to an abstract type member or type parameter"
            ...
            case tpe @ AppliedType(tycon, targs) =>
              X.widenDealias match
                ...
                case x =>
                  ...
                  TypeComparer.provablyDisjoint(
                    x,
                    tpe.derivedAppliedType(tycon, targs.map(_ => WildcardType))
                  )
            ...
            case tp2: RefinedType =>
              recur(X, tp2.parent)
              && (TypeComparer.hasMatchingMember(
                tp2.refinedName,
                X,
                tp2
              ) ||| i"it's a refinement type")
            case tp2: RecType => recur(X, tp2.parent)
            case _
                if P.classSymbol.isLocal && foundClasses(X).exists(
                  P.classSymbol.isInaccessibleChildOf
                ) => // 8
  ...
  def interceptTypeApply(tree: TypeApply)(using Context): Tree =
    trace(s"transforming ${tree.show}", show = true) {

      /** Intercept `expr.xyz[XYZ]` */
      def interceptWith(expr: Tree): Tree =
        if expr.isEmpty then tree
        else {
          val sym = tree.symbol
          ...
          def derivedTree(expr1: Tree, sym: Symbol, tp: Type) =
            cpy.TypeApply(tree)(
              expr1.select(sym).withSpan(expr.span),
              List(TypeTree(tp))
            )
          ...
          def transformIsInstanceOf(
              expr: Tree,
              testType: Type,
              unboxedTestType: Type,
              flagUnrelated: Boolean
          ): Tree =
            ...
            def unreachable(why: => String)(using Context): Boolean =
              if flagUnrelated then
                if inMatch then
                  report.error(
                    em"this case is unreachable since $why",
                    expr.srcPos
                  )
                else
                  report.warning(
                    em"this will always yield false since $why",
                    expr.srcPos
                  )
              false

            /** Are `foundCls` and `testCls` classes that allow checks whether a
              * test would be always false?
              */
            ...
            /** Check whether a runtime test that a value of `foundCls` can be a
              * `testCls` can be true in some cases. Issues a warning or an
              * error otherwise.
              */
            def checkSensical(foundClasses: List[Symbol])(using
                Context
            ): Boolean =
              def exprType = i"type ${expr.tpe.widen.stripped}"
              def check(foundCls: Symbol): Boolean =
                if !isCheckable(foundCls) then true
                else if !foundCls.derivesFrom(testCls) then
                  val unrelated =
                    !testCls.derivesFrom(foundCls)
                      && !unboxedTestCls.derivesFrom(foundCls)
                      && (testCls
                        .is(Final) || !testCls.is(Trait) && !foundCls.is(Trait))
                  if foundCls.is(Final) then
                    unreachable(i"$exprType is not a subclass of $testCls")
                  else if unrelated then
              ...
              then
                report.error(
                  em"cannot test if value of $exprType is a reference of $testCls",
                  tree.srcPos
                )
                false
            ...
            if (expr.tpe <:< testType) && inMatch then
              ...
              if expr.tpe.isBottomType then
                report.warning(
                  TypeTestAlwaysDiverges(expr.tpe, testType),
                  tree.srcPos
                )
              ...
              else if testCls.isPrimitiveValueClass then
                foundClsSyms match
                  case List(cls) if cls.isPrimitiveValueClass =>
                    constant(
                      expr,
                      Literal(Constant(foundClsSyms.head == testCls))
                    )
                  case _ =>
                    transformIsInstanceOf(
                      expr,
                      defn.boxedType(testCls.typeRef),
                      testCls.typeRef,
                      flagUnrelated
                    )
              else derivedTree(expr, defn.Any_isInstanceOf, testType)
            end if
          end transformIsInstanceOf
          ...
          def transformAsInstanceOf(testType: Type): Tree =
            ...
            if erasure(expr.tpe) <:< testType then
              Typed(
                expr,
                tree.args.head
              ) // Replace cast by type ascription (which does not generate any bytecode)
            else if testCls eq defn.BoxedUnitClass then
              // as a special case, casting to Unit always successfully returns Unit
              Block(expr :: Nil, Literal(Constant(()))).withSpan(expr.span)
            else if foundClsSymPrimitive then
              if testCls.isPrimitiveValueClass then
                primitiveConversion(expr, testCls)
              else derivedTree(box(expr), defn.Any_asInstanceOf, testType)
            else if testCls.isPrimitiveValueClass then
              unbox(expr.ensureConforms(defn.ObjectType), testType)
            else if isDerivedValueClass(testCls) then
              expr // adaptToType in Erasure will do the necessary type adaptation
            else if testCls eq defn.NothingClass then
              ...
              val throwCCE = Throw(
                New(
                  defn.ClassCastExceptionClass.typeRef,
                  defn.ClassCastExceptionClass_stringConstructor,
                  Literal(Constant("Cannot cast to scala.Nothing")) :: Nil
                )
              )
              Block(expr :: Nil, throwCCE).withSpan(expr.span)
            else derivedTree(expr, defn.Any_asInstanceOf, testType)
          end transformAsInstanceOf

          /** Transform isInstanceOf
            *
            * expr.isInstanceOf[A | B] ~~> expr.isInstanceOf[A] |
            * expr.isInstanceOf[B] expr.isInstanceOf[A & B] ~~>
            * expr.isInstanceOf[A] & expr.isInstanceOf[B]
            * expr.isInstanceOf[Tuple] ~~>
            * scala.runtime.Tuples.isInstanceOfTuple(expr)
            * expr.isInstanceOf[EmptyTuple] ~~>
            * scala.runtime.Tuples.isInstanceOfEmptyTuple(expr)
            * expr.isInstanceOf[NonEmptyTuple] ~~>
            * scala.runtime.Tuples.isInstanceOfNonEmptyTuple(expr)
            * expr.isInstanceOf[*:[_, _]] ~~>
            * scala.runtime.Tuples.isInstanceOfNonEmptyTuple(expr)
            *
            * The transform happens before erasure of `testType`, thus cannot be
            * merged with `transformIsInstanceOf`, which depends on erased type
            * of `testType`.
            */
          def transformTypeTest(
              expr: Tree,
              testType: Type,
              flagUnrelated: Boolean
          ): Tree = testType.dealias match
            ...
            case defn.MultiArrayOf(elem, ndims)
                if isGenericArrayElement(elem, isScala2 = false) =>
              def isArrayTest(arg: Tree) =
                ref(defn.runtimeMethodRef(nme.isArray))
                  .appliedTo(arg, Literal(Constant(ndims)))
              if ndims == 1 then isArrayTest(expr)
              else
                evalOnce(expr) { e =>
            ...
            case AppliedType(tref: TypeRef, _)
            ...
            case _ =>
              ...
              defn.untestableClasses.find(testWidened.isRef(_)) match
                case Some(untestable) =>
                  report.error(
                    em"$untestable cannot be used in runtime type tests",
                    tree.srcPos
                  )
                  constant(expr, Literal(Constant(false)))
                case _ =>
          ...
          if sym.isTypeTest then
            val argType = tree.args.head.tpe
            val isTrusted =
              tree.hasAttachment(PatternMatcher.TrustedTypeTestKey)
            val isUnchecked =
              expr.tpe.widenTermRefExpr.hasAnnotation(defn.UncheckedAnnot)
            if !isTrusted && !isUnchecked then
              ...
              if whyNot.nonEmpty then
                report.uncheckedWarning(
                  em"the type test for $argType cannot be checked at runtime because $whyNot",
                  expr.srcPos
                )
            transformTypeTest(
              expr,
              argType,
              flagUnrelated = enclosingInlineds.isEmpty
            ) // if test comes from inlined code, dont't flag it even if it always false
          else if sym.isTypeCast then
  ...
  private[transform] def foundClasses(tp: Type)(using Context): List[Symbol] =
    def go(tp: Type, acc: List[Type])(using Context): List[Type] =
      tp.dealias match
        case OrType(tp1, tp2) => go(tp2, go(tp1, acc))
        case AndType(tp1, tp2) =>
          (for t1 <- go(tp1, Nil); t2 <- go(tp2, Nil)
          yield AndType(t1, t2)) ::: acc
        case _ => tp :: acc
