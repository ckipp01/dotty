...
object LambdaLift:
  ...
  class Lifter(thisPhase: MiniPhase & DenotTransformer)(using Context):
    ...
    val deps =
      new Dependencies(ctx.compilationUnit.tpdTree, ctx.withPhase(thisPhase)):
    ...
    /** A map storing the free variable proxies of functions and classes. For
      * every function and class, this is a map from the free variables of that
      * function or class to the proxy symbols accessing them.
      */
    private val proxyMap = new LinkedHashMap[Symbol, Map[Symbol, Symbol]]

    def proxyOf(sym: Symbol, fv: Symbol): Symbol =
      proxyMap.getOrElse(sym, Map.empty)(fv)
    ...
    private def newName(sym: Symbol)(using Context): Name =
      if sym.isAnonymousFunction && sym.owner.is(Method) then
        sym.name.replace { case name: SimpleName =>
          ExpandPrefixName(sym.owner.name.asTermName, name)
        }.freshened
    ...
    private def generateProxies()(using Context): Unit =
      for owner <- deps.tracked do
        ...
        val newFlags =
          Synthetic | (if owner.isClass then PrivateParamAccessor else Param)
    ...
    private def liftedInfo(local: Symbol)(using Context): Type =
      local.info match
    ...
    private def liftLocals()(using Context): Unit =
      for (local, lOwner) <- deps.logicalOwner do
        ...
        local
          .copySymDenotation(
            ...
            name = newName(local),
            initFlags = initFlags,
            info = liftedInfo(local)
          )
          .installAfter(thisPhase)
      for local <- deps.tracked do
        if !deps.logicalOwner.contains(local) then
          local
            .copySymDenotation(info = liftedInfo(local))
            .installAfter(thisPhase)
    end liftLocals
    ...
    private def proxy(sym: Symbol)(using Context): Symbol =
      ...
      def searchIn(enclosure: Symbol): Symbol =
        if !enclosure.exists then
          def enclosures(encl: Symbol): List[Symbol] =
            if encl.exists then encl :: enclosures(liftedEnclosure(encl))
            else Nil
          throw new IllegalArgumentException(
            i"Could not find proxy for ${sym.showDcl} in ${sym.ownersIterator.toList}, encl = $currentEnclosure, owners = ${currentEnclosure.ownersIterator.toList}%, %; enclosures = ${enclosures(currentEnclosure)}%, %"
          )
      ...
      if inCurrentOwner(sym) then sym else searchIn(currentEnclosure)

    def memberRef(sym: Symbol)(using Context): Tree =
      val clazz = sym.enclosingClass
      val qual =
        if clazz.isStaticOwner || ctx.owner.enclosingClass == clazz then
          singleton(clazz.thisType)
        else if ctx.owner.isConstructor then
          outerParam.get(ctx.owner) match
            case Some(param) =>
              outer.path(start = Ident(param.termRef), toCls = clazz)
    ...
    def proxyRef(sym: Symbol)(using Context): Tree =
      ...
      thisPhase.transformFollowingDeep(
        if psym.owner.isTerm then ref(psym) else memberRef(psym)
      )
      ...
      proxies match
        ...
        case proxies =>
          ...
          val freeParamDefs = proxies.map(proxy =>
            thisPhase
              .transformFollowingDeep(ValDef(proxy.asTerm).withSpan(tree.span))
              .asInstanceOf[ValDef]
          )
          def proxyInit(field: Symbol, param: Symbol) =
            thisPhase.transformFollowingDeep(
              memberRef(field).becomes(ref(param))
            )

          /** Initialize proxy fields from proxy parameters and map `rhs` from
          ...
          def copyParams(rhs: Tree) =
            ...
            report.debuglog(
              i"copy params ${constrProxies.map(_.showLocated)}%, % to ${classProxies
                  .map(_.showLocated)}%, %}"
            )
          ...
          tree match
            case tree: DefDef =>
              cpy.DefDef(tree)(
                ...
                rhs =
                  if sym.isPrimaryConstructor && !sym.owner.is(Trait) then
                    copyParams(tree.rhs)
                  else tree.rhs
              )
            case tree: Template =>
    ...
    def liftDef(tree: MemberDef)(using Context): Tree =
      ...
      thisPhase
        .transformFollowing(rename(tree, tree.symbol.name))
        .foreachInThicket(buf += _)
      EmptyTree
...
/** This phase performs the necessary rewritings to eliminate classes and
  * methods nested in other methods. In detail:
  *   1. It adds all free variables of local functions as additional parameters
  *      (proxies). 2. It rebinds references to free variables to the
  *      corresponding proxies, 3. It lifts all local functions and classes out
  *      as far as possible, but at least to the enclosing class. 4. It stores
  *      free variables of non-trait classes as additional fields of the class.
  *      The fields serve as proxies for methods in the class, which avoids the
  *      need of passing additional parameters to these methods.
  *
  * A particularly tricky case are local traits. These cannot store free
  * variables as field proxies, because LambdaLift runs after Mixin, so the
  * fields cannot be expanded anymore. Instead, methods of local traits get free
  * variables of the trait as additional proxy parameters. The difference
  * between local classes and local traits is illustrated by the two rewritings
  * below.
  *
  * def f(x: Int) = { def f(x: Int) = new C(x).f2 class C { ==> class C(x$1:
  * Int) { def f2 = x def f2 = x$1 } } new C().f2 }
  *
  * def f(x: Int) = { def f(x: Int) = new C().f2(x) trait T { ==> trait T def f2
  * \= x def f2(x$1: Int) = x$1 } } class C extends T class C extends T new
  * C().f2 }
  */
class LambdaLift extends MiniPhase with IdentityDenotTransformer:
  thisPhase =>
  import LambdaLift.*
  import ast.tpd.*
  ...
  override def runsAfterGroupsOf: Set[String] =
    Set(Constructors.name, HoistSuperArgs.name)
      ...
      case tpe @ TermRef(prefix, _) =>
        ...
        if prefix eq NoPrefix then
          if sym.enclosure != lft.currentEnclosure && !sym.isStatic then
            (if sym is Method then lft.memberRef(sym) else lft.proxyRef(sym))
              .withSpan(tree.span)
  ...
  override def transformSelect(tree: Select)(using Context): Tree =
    ...
    if denot.isInstanceOf[NonSymSingleDenotation] && lifter.deps
        .freeVars(sym)
        .nonEmpty
    then tree.qualifier.select(sym).withSpan(tree.span)
    else tree

  override def transformApply(tree: Apply)(using Context): Apply =
    cpy
      .Apply(tree)(tree.fun, lifter.addFreeArgs(tree.symbol, tree.args))
      .withSpan(tree.span)
  ...
  override def transformDefDef(tree: DefDef)(using Context): Tree =
    ...
    val lft = lifter
    val paramsAdded =
      if lft.deps.freeVars(sym).nonEmpty then
        lft.addFreeParams(tree, lft.proxies(sym)).asInstanceOf[DefDef]
      else tree
    if lft.needsLifting(sym) then lft.liftDef(paramsAdded)
    else paramsAdded
  ...
  override def transformTypeDef(tree: TypeDef)(using Context): Tree =
    if lifter.needsLifting(tree.symbol) then lifter.liftDef(tree) else tree
end LambdaLift
