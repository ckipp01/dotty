...
import Flags.*, SymUtils.*, NameKinds.*,
  Denotations.{Denotation, SingleDenotation}
import ast.*
import Names.Name
import Phases.Phase
...
object Recheck:
  ...
  /** A flag used to indicate that a ParamAccessor has been temporarily made
    * not-private Only used at the start of the Recheck phase, reset at its end.
    * The flag repurposes the Scala2ModuleVar flag. No confusion is possible
    * since Scala2ModuleVar cannot be also ParamAccessors.
    */
  ...
  extension (sym: Symbol)
    ...
    /** Update symbol's info to newInfo from prevPhase.next to lastPhase. Reset
      * to previous info for phases after lastPhase.
      */
    def updateInfoBetween(
        prevPhase: DenotTransformer,
        lastPhase: DenotTransformer,
        newInfo: Type
    )(using Context): Unit =
    ...
    /** Does symbol have a new denotation valid from phase.next that is
      * different from the denotation it had before?
      */
  ...
  extension [T <: Tree](tree: T)
    ...
    /** Remember `tpe` as the type of `tree`, which might be different from the
      * type stored in the tree itself, unless a type was already remembered for
      * `tree`.
      */
    ...
    /** The remembered type of the tree, or if none was installed, the original
      * type
      */
    def knownType: Type =
  ...
  extension (tpe: Type)
    /** Map ExprType => T to () ?=> T (and analogously for pure versions). Even
      * though this phase runs after ElimByName, ExprTypes can still occur as
      * by-name arguments of applied types. See note in doc comment for
...
/** A base class that runs a simplified typer pass over an already re-typed
  * program. The pass does not transform trees but returns instead the re-typed
  * type of each tree as it is traversed. The Recheck phase must be directly
  * preceded by a phase of type PreRecheck.
  */
abstract class Recheck extends Phase, SymTransformer:
  ...
  def transformSym(sym: SymDenotation)(using Context): SymDenotation =
    if sym.isAllOf(Recheck.ResetPrivateParamAccessor) then
      sym.copySymDenotation(initFlags =
        sym.flags &~ Recheck.ResetPrivate | Private
      )
    else sym
  ...
  class Rechecker(@constructorOnly ictx: Context):
    ...
    /** Should type of `tree` be kept in an attachment so that it can be
      * retrieved with `knownType`? By default true only is `keepAllTypes` hold,
    ...
    def reset()(using Context): Unit =
      for (ref, mbr) <- prevSelDenots.iterator do ref.withDenot(mbr)
    ...
    /** When we select the `apply` of a function with type such as `(=> A) =>
      * B`, we need to convert the parameter type `=> A` to `() ?=> A`. See doc
      * comment of `mapExprType`.
      */
    def normalizeByName(mbr: SingleDenotation)(using
        Context
    ): SingleDenotation = mbr.info match
      case mt: MethodType if mt.paramInfos.exists(_.isInstanceOf[ExprType]) =>
        mbr.derivedSingleDenotation(
          mbr.symbol,
    ...
    def recheckSelection(
        tree: Select,
        qualType: Type,
        name: Name,
        sharpen: Denotation => Denotation
    )(using Context): Type =
      if name.is(OuterSelectName) then tree.tpe
        ...
        val newType = tree.tpe match
          case prevType: NamedType =>
            ...
            if (newType eq prevType) && (mbr.info ne prevDenot.info) && !prevSelDenots
                .contains(prevType)
            then prevSelDenots(prevType) = prevDenot
            newType
          case _ =>
    ...
    def recheckSelection(tree: Select, qualType: Type, name: Name, pt: Type)(
        using Context
    ): Type =
    ...
    def recheckDefDef(tree: DefDef, sym: Symbol)(using Context): Unit =
      ...
      then inContext(rhsCtx) { recheck(tree.rhs, recheck(tree.tpt)) }

    def recheckTypeDef(tree: TypeDef, sym: Symbol)(using Context): Type =
      recheck(tree.rhs)
      sym.typeRef

    def recheckClassDef(tree: TypeDef, impl: Template, sym: ClassSymbol)(using
        Context
    ): Type =
      recheck(impl.constr)
    ...
    private def mapJavaArgs(formals: List[Type])(using Context): List[Type] =
      val tm = new TypeMap with IdempotentCaptRefMap:
        def apply(t: Type) = t match
          case t: TypeRef if t.symbol == defn.ObjectClass =>
    ...
    def recheckApply(tree: Apply, pt: Type)(using Context): Type =
      ...
      val funtpe =
        if tree.fun.symbol.originalSignaturePolymorphic.exists then tree.fun.tpe
        else funTp
      funtpe.widen match
    ...
    def recheckBlock(stats: List[Tree], expr: Tree, pt: Type)(using
        Context
    ): Type =
      recheckStats(stats)
    ...
    def recheckInlined(tree: Inlined, pt: Type)(using Context): Type =
      recheckBlock(tree.bindings, tree.expansion, pt)(using
    ...
    def recheckCase(tree: CaseDef, selType: Type, pt: Type)(using
        Context
    ): Type =
    ...
    def recheckReturn(tree: Return)(using Context): Type =
      ...
      def widened(tp: Type): Type = tp match
        ...
        case tp: AndOrType =>
          tp.derivedAndOrType(widened(tp.tp1), widened(tp.tp2))
        case tp @ AnnotatedType(tp1, ann) =>
          tp.derivedAnnotatedType(widened(tp1), ann)
        case _ => tp
    ...
    def recheckStats(stats: List[Tree])(using Context): Unit =
      @tailrec def traverse(stats: List[Tree])(using Context): Unit =
        stats match
      ...
      def recheckNamed(tree: NameTree, pt: Type)(using Context): Type =
        ...
        tree match
          ...
          case tree: TypeDef =>
            ...
            tree.rhs match
              case impl: Template =>
                recheckClassDef(tree, impl, sym.asClass)(using
    ...
    /** Finish rechecking a tree node: check rechecked type against expected
      * type and remember rechecked type in a tree attachment if required.
      * @param tpe
    ...
    /** Typing and previous transforms sometiems leaves skolem types in prefixes
      * of NamedTypes in `expected` that do not match the `actual` Type. -Ycheck
      * does not complain (need to find out why), but a full recheck does. We
      * compensate by de-skolemizing everywhere in `expected` except when
      * variance is negative.
      * @return
      *   If `tp` contains SkolemTypes in covariant or invariant positions, the
      *   type where these SkolemTypes are mapped to their underlying type.
      *   Otherwise, `tp` itself
      */
    ...
    /** If true, print info for some successful checkConforms operations
      * (failing ones give an error message in any case).
      */
    ...
    def checkConformsExpr(actual: Type, expected: Type, tree: Tree)(using
        Context
    ): Unit =
      ...
      def isCompatible(expected: Type): Boolean =
        actual <:< expected
          ...
          && isCompatible(
            expected.translateFromRepeated(toArray =
              tree.tpe.isRef(defn.ArrayClass)
            )
          )
          || {
  ...
  /** Show tree with rechecked types instead of the types stored in the `.tpe`
    * field
    */
