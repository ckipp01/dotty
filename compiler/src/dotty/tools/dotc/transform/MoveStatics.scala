...
class MoveStatics extends MiniPhase with SymTransformer:
  ...
  def transformSym(sym: SymDenotation)(using Context): SymDenotation =
    if sym.hasAnnotation(defn.ScalaStaticAnnot) && sym.owner.is(
        Flags.Module
      ) && sym.owner.companionClass.exists &&
      (sym.is(Flags.Method) || !(sym.is(
        Flags.Mutable
      ) && sym.owner.companionClass.is(Flags.Trait)))
    then
  ...
  override def transformStats(trees: List[Tree])(using Context): List[Tree] =
    if ctx.owner.is(Flags.Package) then
      val (classes, others) =
        trees.partition(x => x.isInstanceOf[TypeDef] && x.symbol.isClass)
      val pairs = classes
        .groupBy(_.symbol.name.stripModuleClassSuffix)
        .asInstanceOf[Map[Name, List[TypeDef]]]

      def rebuild(orig: TypeDef, newBody: List[Tree]): Tree =
        val staticFields = newBody
          .filter(x =>
            x.isInstanceOf[ValDef] && x.symbol
              .hasAnnotation(defn.ScalaStaticAnnot)
          )
        ...
        val newBodyWithStaticConstr =
          if staticFields.nonEmpty then
            ...
            val staticCostructor = newSymbol(
              orig.symbol,
              nme.STATIC_CONSTRUCTOR,
              Flags.Synthetic | Flags.Method | Flags.Private,
              MethodType(Nil, defn.UnitType)
            )
            staticCostructor.addAnnotation(
              Annotation(defn.ScalaStaticAnnot, staticCostructor.span)
            )
            staticCostructor.entered

            val staticAssigns = staticFields.map(x =>
              Assign(
                ref(x.symbol),
                x.rhs.changeOwner(x.symbol, staticCostructor)
              )
            )
            tpd.DefDef(
              staticCostructor,
              Block(staticAssigns, tpd.unitLiteral)
            ) :: newBody
          else newBody

        val oldTemplate = orig.rhs.asInstanceOf[Template]
        cpy.TypeDef(orig)(rhs =
          cpy.Template(oldTemplate)(body = newBodyWithStaticConstr)
        )
      end rebuild
      ...
      def move(module: TypeDef, companion: TypeDef): List[Tree] =
        ...
        if !module.symbol.is(Flags.Module) then move(companion, module)
          ...
          rebuild(companion, companionTmpl.body ++ staticDefs) :: rebuild(
            module,
            remainingDefs
          ) :: Nil
      val newPairs =
