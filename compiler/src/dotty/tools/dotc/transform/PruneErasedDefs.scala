...
/** This phase makes all erased term members of classes private so that they
  * cannot conflict with non-erased members. This is needed so that subsequent
  * phases like ResolveSuper that inspect class members work correctly. The
  * phase also replaces all expressions that appear in an erased context by
  * default values. This is necessary so that subsequent checking phases such as
  * IsInstanceOfChecker don't give false negatives.
  */
class PruneErasedDefs extends MiniPhase with SymTransformer:
  ...
  override def runsAfterGroupsOf: Set[String] =
    Set(RefChecks.name, ExplicitOuter.name)

  override def transformSym(sym: SymDenotation)(using Context): SymDenotation =
    if !sym.isEffectivelyErased || !sym.isTerm || sym.is(
        Private
      ) || !sym.owner.isClass
    then sym
    ...
    tree.fun.tpe.widen match
      case mt: MethodType if mt.hasErasedParams =>
        cpy.Apply(tree)(
          ...
          tree.args
            .zip(mt.erasedParams)
            .map((a, e) => if e then trivialErasedTree(a) else a)
        )
      case _ =>
        tree
  ...
  def checkErasedInExperimental(sym: Symbol)(using Context): Unit =
    ...
    if sym.is(
        Erased,
        butNot = Macro
      ) && sym != defn.Compiletime_erasedValue && !sym.isInExperimentalScope
    then Feature.checkExperimentalFeature("erased", sym.sourcePos)
end PruneErasedDefs
...
object PruneErasedDefs:
  ...
  val description: String =
    "drop erased definitions and simplify erased expressions"

  def trivialErasedTree(tree: Tree)(using Context): Tree =
    ref(defn.Compiletime_erasedValue)
