...
/** The first tree transform
  ...
  *   - drops branches of ifs using the rules if (true) A else B ==> A if
  *     (false) A else B ==> B
  */
class FirstTransform extends MiniPhase with InfoTransformer:
  thisPhase =>
  import ast.tpd.*
  ...
  override def checkPostCondition(tree: Tree)(using Context): Unit =
    tree match
      case Select(qual, name)
          if !name.is(OuterSelectName) && tree.symbol.exists =>
        val qualTpe = qual.tpe
        assert(
          qualTpe.isErasedValueType || qualTpe.derivesFrom(tree.symbol.owner) ||
            tree.symbol.is(JavaStatic) && qualTpe.derivesFrom(
              tree.symbol.enclosingClass
            ),
  ...
  /** Reorder statements so that module classes always come after their
    * companion classes
    */
  private def reorderAndComplete(stats: List[Tree])(using Context): List[Tree] =
     ...
     */
    def reorder(stats: List[Tree], revPrefix: List[Tree]): List[Tree] =
      stats match
        case (stat: TypeDef) :: stats1 if stat.symbol.isClass =>
          if stat.symbol.is(Flags.Module) then
            ...
            pushOnTop(
              revPrefix,
              if moduleClassDefs contains stat.name then stat :: stats1r
              else stats1r
            )
          else
            reorder(
  ...
  /** Eliminate self in Template Under captureChecking, we keep the self type
    * `S` around in a type definition
    *
    * private[this] type $this = S
    *
    * This is so that the type can be checked for well-formedness in the
    * CaptureCheck phase.
    */
    ...
    impl.self match
      case self: ValDef if !self.tpt.isEmpty && Feature.ccEnabled =>
        val tsym = newSymbol(
          ...
          tpnme.SELF,
          PrivateLocal,
          TypeAlias(self.tpt.tpe)
        )
        val tdef =
          untpd.cpy.TypeDef(self)(tpnme.SELF, self.tpt).withType(tsym.typeRef)
  ...
  private object collectBinders extends TreeAccumulator[List[Ident]]:
    def apply(annots: List[Ident], t: Tree)(using Context): List[Ident] =
      t match
        case t @ Bind(_, body) =>
          val annot =
            untpd.Ident(tpnme.BOUNDTYPE_ANNOT).withType(t.symbol.typeRef)
          apply(annot :: annots, body)
        case _ =>
          foldOver(annots, t)

  /** Replace type tree `t` of type `T` with `TypeTree(T)`, but record all
    * nested Bind nodes in annotations. These are interpreted in TreeTypeMaps so
    * that bound symbols can be properly copied.
    */
  ...
  override def transformOther(tree: Tree)(using Context): Tree = tree match
    ...
    case tree           => if tree.isType then toTypeTree(tree) else tree

  override def transformIdent(tree: Ident)(using Context): Tree =
    if tree.isType then toTypeTree(tree)
    else if tree.name != nme.WILDCARD then
  ...
  override def transformSelect(tree: Select)(using Context): Tree =
    if tree.isType then toTypeTree(tree) else constToLiteral(tree)
  ...
  override def transformTyped(tree: Typed)(using Context): Tree =
    ...
    if !ctx.mode.is(Mode.Pattern) then constToLiteral(tree) else tree
  ...
  override def transformIf(tree: If)(using Context): Tree =
    tree.cond.tpe match
      case ConstantType(Constant(c: Boolean)) if isPureExpr(tree.cond) =>
        if c then tree.thenp else tree.elsep
      case _ => tree
    ...
    case Select(x @ Literal(Constant(c: Boolean)), op) =>
      tree.args match
        case y :: Nil if y.tpe.widen.isRef(defn.BooleanClass) =>
          op match
            case nme.ZAND => if c then y else x
            case nme.ZOR  => if c then x else y
            case _        => tree
        case _ => tree
    case _ => tree
