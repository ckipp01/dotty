...
/** A transformer that eliminates SeqLiteral's, transforming `SeqLiteral(elems)`
  ...
  *
  * Instead of `toSeq`, which takes an implicit, the appropriate "wrapArray"
  * method is called directly. The reason for this step is that JavaSeqLiterals,
  * being arrays keep a precise type after erasure, whereas SeqLiterals only get
  * the erased type `Seq`,
  */
