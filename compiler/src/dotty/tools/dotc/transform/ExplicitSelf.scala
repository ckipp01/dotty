...
import core.*
import Contexts.*, Types.*, MegaPhase.*, ast.Trees.*, Symbols.*, Decorators.*,
  Flags.*
import SymUtils.*
...
/** Transform references of the form
  ...
  * C.this.m
  *
  * where `C` is a class with explicit self type and `C` is not a subclass of
  * the owner of `m` to
  *
  * C.this.asInstanceOf[S & C.this.type].m
  *
  * where `S` is the self type of `C`. See run/i789.scala for a test case why
  * this is needed.
  *
...
class ExplicitSelf extends MiniPhase:
  ...
  private def castQualifier(tree: RefTree, cls: ClassSymbol, thiz: Tree)(using
      Context
  ) =
    val selfType = cls.classInfo.selfType
    if selfType.classSymbols.exists(_.isValueClass) && !cls.isUniversalTrait
    then
      report.error(
        em"self type $selfType of $cls may not be a value class",
        thiz.srcPos
      )
