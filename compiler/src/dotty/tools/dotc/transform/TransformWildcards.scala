...
/** This phase transforms wildcards in valdefs with their default value. In
  * particular for every valdef that is declared: `val x : T = _` to `val x : T
  * \= <zero of T>`
  */
class TransformWildcards extends MiniPhase with IdentityDenotTransformer:
  ...
  override def transformValDef(tree: ValDef)(using Context): Tree =
    if ctx.owner.isClass then tree
