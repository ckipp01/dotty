...
object DropBreaks:
  ...
  class LabelUsage(val goto: TermSymbol, val enclMeth: Symbol):
    /** The number of references to associated label that come from labeled
      * returns
      */
...
/** Rewrites local Break throws to labeled returns. Drops `try` statements on
  * breaks if no other uses of its label remain. A Break throw with a `Label`
...
class DropBreaks extends MiniPhase:
  ...
  private object LabelTry:
    ...
    object GuardedThrow:
      ...
      /** `(ex, local)` provided `expr` matches
        ...
        */
      def unapply(expr: Tree)(using Context): Option[(Symbol, Symbol)] =
        stripTyped(expr) match
          case If(
                Apply(
                  Select(Select(ex: Ident, label), eq),
                  (lbl @ Ident(local)) :: Nil
                ),
    ...
    /** `(local, body)` provided `tree` matches
      *
      * try body catch case ex: Break => if ex.label.eq(local) then ex.value
      * else throw ex
      */
    def unapply(tree: Tree)(using Context): Option[(Symbol, Tree)] = stripTyped(
      tree
    ) match
      case Try(
            body,
            CaseDef(
              pat @ Bind(_, Typed(_, tpt)),
              EmptyTree,
              GuardedThrow(exc, local)
            ) :: Nil,
            EmptyTree
  ...
  private object BreakBoundary:
    ...
    def unapply(tree: Tree)(using Context): Option[(Symbol, Tree)] = stripTyped(
      ...
      case Block((vd @ ValDef(nme.local, _, _)) :: Nil, LabelTry(caughtAndRhs))
          if vd.symbol.info.isRef(
            defn.LabelClass
          ) && vd.symbol == caughtAndRhs._1 =>
        Some(caughtAndRhs)
      case _ =>
        None
  end BreakBoundary
  ...
  private object Break:
    ...
    def unapply(tree: Tree)(using Context): Option[(Symbol, Tree)] = tree match
      case Apply(Apply(fn, args), id :: Nil) if isBreak(fn.symbol) =>
  ...
  /** If `tree` is a BreakBoundary, associate a fresh `LabelUsage` with its
    * label.
    */
  override def prepareForBlock(tree: Block)(using Context): Context = tree match
    case BreakBoundary(label, _) =>
      ...
      val goto =
        newSymbol(ctx.owner, BoundaryName.fresh(), Synthetic | Label, tree.tpe)
      ctx.fresh.setProperty(
        LabelUsages,
  ...
  override def transformBlock(tree: Block)(using Context): Tree = tree match
    case BreakBoundary(label, expr) =>
      ...
      report.log(
        i"trans boundary block $label // ${uses.returnRefs}, ${uses.otherRefs}"
      )
  ...
  private def transformBreak(tree: Tree, arg: Tree, lbl: Symbol)(using
      Context
  ): Tree =
  ...
  /** Rewrite a break call
    ...
    *
    * where `lbl` is a label defined in the current method and is not included
    * in ShadowedLabels to
    *
    * return[target] arg
    *
    * where `target` is the `goto` return label associated with `lbl`. Adjust
    * associated ref counts accordingly. The local refcount is increased and the
    * non-local refcount is decreased, since the `lbl` implicit argument to
    * `break` is dropped.
    */
  ...
  override def transformIdent(tree: Ident)(using Context): Tree =
    if enclosingBoundaries != 0 then
      for uses <- labelUsage(tree.symbol) do uses.otherRefs += 1
    tree
