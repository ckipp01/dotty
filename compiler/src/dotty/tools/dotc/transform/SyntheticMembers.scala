...
object SyntheticMembers:
  ...
  /** Attachment marking an anonymous class as a singleton case that will extend
    * from Mirror.Singleton
    */
  ...
  /** Attachment recording that an anonymous class should extend Mirror.Product
    */
  val ExtendsSumOrProductMirror: Property.StickyKey[MirrorImpl] =
    new Property.StickyKey

/** Synthetic method implementations for case classes, case objects, and value
  * classes.
  *
  * Selectively added to case classes/objects, unless a non-default
  * implementation already exists: def equals(other: Any): Boolean def
  * hashCode(): Int def canEqual(other: Any): Boolean def toString(): String def
  * productElement(i: Int): Any def productArity: Int def productPrefix: String
  *
  * Add to serializable static objects, unless an implementation already exists:
  * private def writeReplace(): AnyRef
  *
  * Selectively added to value classes, unless a non-default implementation
  * already exists: def equals(other: Any): Boolean def hashCode(): Int
  */
class SyntheticMembers(thisPhase: DenotTransformer):
  ...
  private def initSymbols(using Context) =
    if myValueSymbols.isEmpty then
      myValueSymbols = List(defn.Any_hashCode, defn.Any_equals)
      myCaseSymbols = defn.caseClassSynthesized
      myCaseModuleSymbols = myCaseSymbols.filter(_ ne defn.Any_equals)
      myEnumValueSymbols = List(defn.Product_productPrefix)
      myNonJavaEnumValueSymbols =
        myEnumValueSymbols :+ defn.Any_toString :+ defn.Enum_ordinal

  def valueSymbols(using Context): List[Symbol] =
    initSymbols; myValueSymbols
  def caseSymbols(using Context): List[Symbol] =
    initSymbols; myCaseSymbols
  def caseModuleSymbols(using Context): List[Symbol] =
    initSymbols; myCaseModuleSymbols
  def enumValueSymbols(using Context): List[Symbol] =
    initSymbols; myEnumValueSymbols
  def nonJavaEnumValueSymbols(using Context): List[Symbol] =
    initSymbols; myNonJavaEnumValueSymbols

  private def existingDef(sym: Symbol, clazz: ClassSymbol)(using
      Context
  ): Symbol =
    val existing = sym.matchingMember(clazz.thisType)
    if existing != sym && !existing.is(Deferred) then existing else NoSymbol

  private def synthesizeDef(
      sym: TermSymbol,
      rhsFn: List[List[Tree]] => Context ?=> Tree
  )(using Context): Tree =
    DefDef(sym, rhsFn(_)(using ctx.withOwner(sym)))
      .withSpan(ctx.owner.span.focus)

  /** If this is a case or value class, return the appropriate additional
    * methods, otherwise return nothing.
    */
  def caseAndValueMethods(clazz: ClassSymbol)(using Context): List[Tree] =
    ...
    lazy val accessors =
      if isDerivedValueClass(clazz) then
        clazz.paramAccessors.take(1) // Tail parameters can only be `erased`
      else clazz.caseAccessors
    val isEnumValue =
      clazz.isAnonymousClass && clazz.info.parents.head.classSymbol.is(Enum)
    ...
    val symbolsToSynthesize: List[Symbol] =
      ...
      else if isDerivedValueClass(clazz) then valueSymbols
      else Nil

    def syntheticDefIfMissing(sym: Symbol): List[Tree] =
      if existingDef(sym, clazz).exists then Nil else syntheticDef(sym) :: Nil
    ...
    def syntheticDef(sym: Symbol): Tree =
      ...
      def forwardToRuntime(vrefs: List[Tree]): Tree =
        ref(defn.runtimeMethodRef("_" + sym.name.toString))
          .appliedToTermArgs(This(clazz) :: vrefs)
      ...
      def identifierRef: Tree =
        if isSimpleEnumValue
        then // owner is `def $new(_$ordinal: Int, $name: String) = new MyEnum { ... }`
      ...
      def ordinalRef: Tree =
        ...
        then // owner is `def $new(_$ordinal: Int, $name: String) = new MyEnum { ... }`
          ref(
            clazz.owner.paramSymss.head.find(_.name == nme.ordinalDollar_).get
          )
          ...
          val candidate: Option[Int] = children.collectFirst {
            case (child, idx) if child == vdef => idx
          }
          assert(
            candidate.isDefined,
            i"could not find child for $vdef in ${parentEnum.children}%, % of $parentEnum"
          )
          Literal(Constant(candidate.get))

      def toStringBody(vrefss: List[List[Tree]]): Tree =
        if clazz.is(ModuleClass) then ownName
        else if isNonJavaEnumValue then identifierRef
        else forwardToRuntime(vrefss.head)

      def syntheticRHS(vrefss: List[List[Tree]])(using Context): Tree =
        synthetic.name match
          ...
          case nme.productElement =>
            productElementBody(accessors.length, vrefss.head.head)
          case nme.productElementName =>
            productElementNameBody(accessors.length, vrefss.head.head)
    ...
    def productElementBody(arity: Int, index: Tree)(using Context): Tree =
      ...
      val cases = 0.until(arity).map { i =>
        val sel =
          This(clazz).select(nme.selectorName(i), _.info.isParameterless)
    ...
    def productElementNameBody(arity: Int, index: Tree)(using Context): Tree =
      ...
      val cases = 0.until(arity).map { i =>
        CaseDef(
          Literal(Constant(i)),
          EmptyTree,
          Literal(Constant(accessors(i).name.toString))
        )
      }
    ...
    def generateIOBECase(index: Tree): CaseDef =
      ...
      def filterStringConstructor(s: Symbol): Boolean = s.info match
        ...
        case _ => false
      val constructor =
        ioob.typeSymbol.info.decls.find(filterStringConstructor _).asTerm
    ...
    /** The class
      ...
      * ```
      *
      * If `C` is a value class, the initial `eq` test is omitted. The
      * `canEqual` test can be omitted if it is known that `canEqual` return
      * always true.
      *
      * `@unchecked` is needed for parametric case classes.
      */
    def equalsBody(that: Tree)(using Context): Tree =
      val thatAsClazz = newSymbol(
        ctx.owner,
        nme.x_0,
        SyntheticCase,
        clazzType,
        coord = ctx.owner.span
      ) // x$0
      ...
      val pattern = Bind(
        ...
        wildcardAscription(
          AnnotatedType(
            clazzType,
            Annotation(defn.UncheckedAnnot, thatAsClazz.span)
          )
        )
      ) // x$0 @ (_: C @unchecked)
      // compare primitive fields first, slow equality checks of non-primitive fields can be skipped when primitives differ
      val sortedAccessors = accessors.sortBy(accessor =>
        if accessor.info.typeSymbol.isPrimitiveValueClass then 0 else 1
      )
      val comparisons = sortedAccessors.map { accessor =>
        This(clazz)
          .withSpan(ctx.owner.span.focus)
          .select(accessor)
          .equal(ref(thatAsClazz).select(accessor))
      }
      var rhs = // this.x == this$0.x && this.y == x$0.y && that.canEqual(this)
        if comparisons.isEmpty then Literal(Constant(true))
        else comparisons.reduceBalanced(_ and _)
      val canEqualMeth = existingDef(defn.Product_canEqual, clazz)
      if !clazz.is(Final) || canEqualMeth.exists && !canEqualMeth.is(Synthetic)
      then
        rhs = rhs.and(
          ref(thatAsClazz)
    ...
    /** The class
      ...
      * ```
      *
      * else if either `T` or `U` are primitive, gets the `hashCode` method
      * implemented by [[caseHashCodeBody]]
      */
    def chooseHashcode(using Context) =
      ...
      else if accessors.exists(
          _.info.finalResultType.classSymbol.isPrimitiveValueClass
        )
      then caseHashCodeBody
      else ref(defn.ScalaRuntime__hashCode).appliedTo(This(clazz))
    ...
    def caseHashCodeBody(using Context): Tree =
      ...
      val mixes =
        for (accessor <- accessors)
          yield Assign(
            ref(acc),
            ref(defn.staticsMethod("mix"))
              .appliedTo(ref(acc), hashImpl(accessor))
          )
      val finish = ref(defn.staticsMethod("finalizeHash"))
        .appliedTo(ref(acc), Literal(Constant(accessors.size)))
      Block(accDef :: mixPrefix :: mixes, finish)
    end caseHashCodeBody
    ...
    def hashImpl(sym: Symbol)(using Context): Tree =
      defn.scalaClassName(sym.info.finalResultType) match
        ...
        case tpnme.Boolean =>
          If(ref(sym), Literal(Constant(1231)), Literal(Constant(1237)))
        case tpnme.Int                             => ref(sym)
        case tpnme.Short | tpnme.Byte | tpnme.Char => ref(sym).select(nme.toInt)
        case tpnme.Long =>
          ref(defn.staticsMethod("longHash")).appliedTo(ref(sym))
        case tpnme.Double =>
          ref(defn.staticsMethod("doubleHash")).appliedTo(ref(sym))
        case tpnme.Float =>
          ref(defn.staticsMethod("floatHash")).appliedTo(ref(sym))
    ...
    /** The class
      ...
      * `@unchecked` is needed for parametric case classes.
      */
    def canEqualBody(that: Tree, span: Span): Tree = that.isInstance(
      AnnotatedType(clazzType, Annotation(defn.UncheckedAnnot, span))
    )

    symbolsToSynthesize.flatMap(syntheticDefIfMissing)
  end caseAndValueMethods

  private def hasWriteReplace(clazz: ClassSymbol)(using Context): Boolean =
    clazz
      .membersNamed(nme.writeReplace)
      .filterWithPredicate(s =>
  ...
  private def writeReplaceDef(clazz: ClassSymbol)(using Context): TermSymbol =
    newSymbol(
      clazz,
      nme.writeReplace,
      PrivateMethod | Synthetic,
      MethodType(Nil, defn.AnyRefType),
      coord = clazz.coord
    ).entered.asTerm
  ...
  /** If this is a static object `Foo`, add the method:
    *
    * private def writeReplace(): AnyRef = new
    * scala.runtime.ModuleSerializationProxy(classOf[Foo.type])
    *
    * unless an implementation already exists, otherwise do nothing.
    *
    * All static objects receive the `Serializable` flag in the back-end, so we
    * do that even for objects that are not serializable at this phase.
    */
  def serializableObjectMethod(clazz: ClassSymbol)(using Context): List[Tree] =
    ...
    then
      List(
        DefDef(
          writeReplaceDef(clazz),
          _ =>
            New(
              defn.ModuleSerializationProxyClass.typeRef,
  ...
  extension (cls: ClassSymbol)
    private def isEnumValueImplementation(using Context): Boolean =
      cls.isAnonymousClass && cls.info.parents.head.typeSymbol
        .is(Enum) // asserted in Typer

  /** If this is the class backing a serializable singleton enum value with base
    * class `MyEnum`, and not deriving from `java.lang.Enum` add the method:
    *
    * private def readResolve(): AnyRef = MyEnum.fromOrdinal(this.ordinal)
    *
    * unless an implementation already exists, otherwise do nothing.
    */
  def serializableEnumValueMethod(
      clazz: ClassSymbol
  ...
  def fromProductBody(
      caseClass: Symbol,
      param: Tree,
      optInfo: Option[MirrorImpl.OfProduct]
  )(using Context): Tree =
    ...
    def computeFromCaseClass: (Type, List[Type]) =
      val (baseRef, baseInfo) =
        ...
        optInfo match
          case Some(info) =>
            (
              rawRef.asSeenFrom(info.pre, caseClass.owner),
              rawInfo.asSeenFrom(info.pre, caseClass.owner)
            )
          case _ =>
            (rawRef, rawInfo)
      baseInfo match
        case tl: PolyType =>
          val (tl1, tpts) =
            constrained(tl, untpd.EmptyTree, alwaysAddTypeVars = true)
          val targs =
            for (tpt <- tpts) yield tpt.tpe match
    ...
    val elems =
      for ((formal, idx) <- paramInfos.zipWithIndex) yield
        val elem =
          param
            .select(defn.Product_productElement)
            .appliedTo(Literal(Constant(idx)))
            .ensureConforms(formal.translateFromRepeated(toArray = false))
        if formal.isRepeatedParam then ctx.typer.seqToRepeated(elem) else elem
  ...
  /** For an enum T:
    ...
    *
    * def ordinal(x: MirroredMonoType) = x match { case _: C_1 => 0 ... case _:
    * C_n => n - 1 }
    *
    * Here, the normalized type of a class C is C[?, ...., ?] with a wildcard
    * for each type parameter. The normalized type of an object O is O.type.
    */
  def ordinalBody(cls: Symbol, param: Tree, optInfo: Option[MirrorImpl.OfSum])(
      using Context
  ): Tree =
    if cls.is(Enum) then param.select(nme.ordinal).ensureApplied
    else
      def computeChildTypes: List[Type] =
        def rawRef(child: Symbol): Type =
          if child.isTerm then child.reachableTermRef
          else child.reachableRawTypeRef
        optInfo match
          case Some(info) =>
            info.childPres
      ...
      val cases =
        for (patType, idx) <- childTypes.zipWithIndex yield
          val pat = Typed(
            untpd.Ident(nme.WILDCARD).withType(patType),
            TypeTree(patType)
          )
  ...
  /**   - If `impl` is the companion of a generic sum, add `deriving.Mirror.Sum`
    *     parent and `MirroredMonoType` and `ordinal` members.
    *     - If `impl` is the companion of a generic product, add
    *       `deriving.Mirror.Product` parent and `MirroredMonoType` and
    *       `fromProduct` members.
    *     - If `impl` is marked with one of the attachments
    *       ExtendsSingletonMirror or ExtendsSumOfProductMirror, remove the
    *       attachment and generate the corresponding mirror support, On this
    *       case the represented class or object is referred to in a
    *       pre-existing `MirroredMonoType` member of the template.
    */
  def addMirrorSupport(impl: Template)(using Context): Template =
    ...
    def addMethod(
        ...
        info: Type,
        cls: Symbol,
        body: (Symbol, Tree) => Context ?=> Tree
    ): Unit =
      val meth =
        newSymbol(clazz, name, Synthetic | Method, info, coord = clazz.coord)
      if !existingDef(meth, clazz).exists then
    ...
    lazy val monoType =
      ...
      if existing.exists && !existing.is(Deferred) then existing
        ...
        val monoType =
          newSymbol(
            clazz,
            tpnme.MirroredMonoType,
            Synthetic,
            TypeAlias(linked.reachableRawTypeRef),
            coord = clazz.coord
          )
    ...
    def makeProductMirror(cls: Symbol, optInfo: Option[MirrorImpl.OfProduct]) =
      ...
      addMethod(
        nme.fromProduct,
        MethodType(defn.ProductClass.typeRef :: Nil, monoType.typeRef),
        cls,
        fromProductBody(_, _, optInfo).ensureConforms(monoType.typeRef)
      ) // t4758.scala or i3381.scala are examples where a cast is needed
    ...
    if clazz.is(Module) then
      if clazz.is(Case) then makeSingletonMirror()
      else if linked.isGenericProduct then makeProductMirror(linked, None)
      else if linked.isGenericSum(NoType) then makeSumMirror(linked, None)
      else if linked.is(Sealed) then
        derive.println(
          i"$linked is not a sum because ${linked.whyNotGenericSum(NoType)}"
        )
    else if impl.removeAttachment(ExtendsSingletonMirror).isDefined then
      makeSingletonMirror()
    else
      impl
        .removeAttachment(ExtendsSumOrProductMirror)
        .match
          case Some(prodImpl: MirrorImpl.OfProduct) =>
            makeProductMirror(
              monoType.typeRef.dealias.classSymbol,
              Some(prodImpl)
            )
  ...
  end addMirrorSupport

  def addSyntheticMembers(impl: Template)(using Context): Template =
    val clazz = ctx.owner.asClass
    val syntheticMembers = serializableObjectMethod(
      clazz
    ) ::: serializableEnumValueMethod(clazz) ::: caseAndValueMethods(clazz)
