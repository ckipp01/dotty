...
/** Replace member references as follows:
  ...
  *   - `x.##` for ## in Any becomes calls to ScalaRunTime.hash, using the most
  *     precise overload available
  */
class InterceptedMethods extends MiniPhase:
  ...
  private def transformRefTree(tree: RefTree)(using Context): Tree =
    if tree.symbol.isTerm && (defn.Any_## eq tree.symbol) then
      val qual = tree match
  ...
  private def poundPoundValue(tree: Tree)(using Context) =
    ...
    if s == defn.NullClass then Literal(Constant(0))
    else if s == defn.DoubleClass then staticsCall(nme.doubleHash)
    else if s == defn.LongClass then staticsCall(nme.longHash)
    else if s == defn.FloatClass then staticsCall(nme.floatHash)
  ...
  override def transformApply(tree: Apply)(using Context): Tree =
    ...
    if tree.fun.symbol == defn.Any_!= then
      qual
        .select(defn.Any_==)
        .appliedToTermArgs(tree.args)
        .select(defn.Boolean_!)
        .withSpan(tree.span)
    else tree
end InterceptedMethods
