...
/** The pattern matching transform. After this phase, the only Match nodes
  * remaining in the code are simple switches where every pattern is an integer
  * or string constant
  */
...
object PatternMatcher:
  ...
  inline val selfCheck =
    false // debug option, if on we check that no case gets generated twice
  ...
  /** The pattern matching translator. Its general structure is a pipeline:
    ...
    *
    * A plan represents the underlying decision graph. It consists of tests, let
    * bindings, labeled blocks, return from said labeled blocks and code blocks.
    * It's represented by its own data type. Plans are optimized by merging
    * common tests and eliminating dead code.
    */
  class Translator(resultType: Type, thisPhase: MiniPhase)(using Context):
    ...
    private val resultLabel =
      newSymbol(
        ctx.owner,
        PatMatResultName.fresh(),
        Synthetic | Label,
        resultType
      )

    /** A map from variable symbols to their defining trees and from labels to
      * their defining plans
      */
    ...
    private def letAbstract(rhs: Tree, tpe: Type = NoType)(
        body: Symbol => Plan
    ): Plan =
    ...
    case class TestPlan(
      ...
      override def equals(that: Any): Boolean = that match
        case that: TestPlan =>
          this.scrutinee === that.scrutinee && this.test == that.test
        case _ => false
    ...
    case class TypeTest(tpt: Tree, trusted: Boolean)
        extends Test: // scrutinee.isInstanceOf[tpt]
      override def equals(that: Any): Boolean = that match
    ...
    case class LengthTest(len: Int, exact: Boolean)
        extends Test // scrutinee (== | >=) len
    ...
    /** Rewrite (repeatedly) `x @ (p: T)` to `(x @ p): T` This brings out the
      * type tests to where they can be analyzed.
      */
    ...
    private def patternPlan(
      ...
      def matchArgsPlan(
         ...
         * ```
         */
        def matchArgsComponentsPlan(
            components: List[Tree],
            syms: List[Symbol]
        ): Plan =
          components match
            case component :: components1 =>
              letAbstract(component, component.avoidPatBoundType())(sym =>
                matchArgsComponentsPlan(components1, sym :: syms)
              )
      ...
      /** Plan for matching the sequence in `seqSym` against sequence elements
        * `args`. If `exact` is true, the sequence is not permitted to have any
        * elements following `args`.
        */
      def matchElemsPlan(
          seqSym: Symbol,
          args: List[Tree],
        ...
        val selectors = args.indices.toList.map(idx =>
          ref(seqSym)
            .select(defn.Seq_apply.matchingMember(seqSym.info))
            .appliedTo(Literal(Constant(idx)))
        )
        TestPlan(
          LengthTest(args.length, exact),
          seqSym,
          seqSym.span,
          matchArgsPlan(selectors, args, onSuccess)
        )

      /** Plan for matching the sequence in `getResult` against sequence
        * elements and a possible last varargs argument `args`.
        */
      ...
      /** Plan for matching the sequence in `getResult`
        *
        * `getResult` is a product, where the last element is a sequence of
        * elements.
        */
      def unapplyProductSeqPlan(
          getResult: Symbol,
          args: List[Tree],
          arity: Int
      ): Plan =
      ...
      /** Plan for matching the result of an unapply against argument patterns
        * `args`
        */
      def unapplyPlan(unapp: Tree, args: List[Tree]): Plan =
        ...
        if isSyntheticScala2Unapply(
            unapp.symbol
          ) && caseAccessors.length == args.length
        then
          def tupleSel(sym: Symbol) = ref(scrutinee).select(sym)
          val isGenericTuple = defn.isTupleClass(caseClass) &&
            !defn.isTupleNType(tree.tpe match
              case tp: OrType => tp.join
              case tp         => tp
            ) // widen even hard unions, to see if it's a union of tuples
          val components =
            if isGenericTuple then
              caseAccessors.indices.toList.map(tupleApp(_, ref(scrutinee)))
          ...
          letAbstract(unapp) { unappResult =>
            ...
            then
              val selectors = productSelectors(unapp.tpe)
                .take(args.length)
                .map(ref(unappResult).select(_))
              matchArgsPlan(selectors, args, onSuccess)
            else if isUnapplySeq && isProductSeqMatch(
                unapp.tpe.widen,
                args.length,
                unapp.srcPos
              )
            ...
            else if isUnapplySeq && unapplySeqTypeElemTp(
                unapp.tpe.widen.finalResultType
              ).exists
            then unapplySeqPlan(unappResult, args)
            else if unappResult.info <:< defn.NonEmptyTupleTypeRef then
              val components = (0 until foldApplyTupleType(
                unappResult.denot.info
              ).length).toList.map(tupleApp(_, ref(unappResult)))
              matchArgsPlan(components, args, onSuccess)
            else
              assert(isGetMatch(unapp.tpe))
              val argsPlan =
                val get =
                  ref(unappResult).select(nme.get, _.info.isParameterless)
                ...
                if isUnapplySeq then
                  ...
                  letAbstract(get) { getResult =>
                    val selectors =
                      if args.tail.isEmpty then ref(getResult) :: Nil
                      else
                        productSelectors(get.tpe).map(ref(getResult).select(_))
                    matchArgsPlan(selectors, args, onSuccess)
                  }
      ...
      swapBind(tree) match
        ...
        case UnApply(extractor, implicits, args) =>
          val unappPlan =
            if scrutinee.info.isBottomType then
              ...
              val unapp0 = extractor.appliedTo(
                ref(scrutinee).ensureConforms(mt.paramInfos.head)
              )
    ...
    private def matchPlan(tree: Match): Plan =
      letAbstract(tree.selector) { scrutinee =>
        val matchError: Plan = ResultPlan(
          Throw(New(defn.MatchErrorClass.typeRef, ref(scrutinee) :: Nil))
        )
      ...
      object refCounter extends RefCounter:
        override val treeMap = new TreeMap:
          override def transform(tree: Tree)(using Context) = tree match
            case tree: Ident =>
              if isPatmatGenerated(tree.symbol) then count(tree.symbol) += 1
              tree
            case _ =>
              super.transform(tree)
        override def apply(plan: LetPlan): Plan =
          apply(plan.body)
          if count(plan.sym) != 0 || !isPatmatGenerated(plan.sym) then
            apply(initializer(plan.sym))
          plan
        override def apply(plan: SeqPlan): Plan =
          apply(plan.head)
          if canFallThrough(plan.head) then apply(plan.tail)
          plan
      refCounter(plan)
      refCounter.count
    ...
    /** Merge identical consecutive tests.
      ...
      *
      * There are some tricks to "ignore" non-patmat-generated let bindings,
      * i.e., captures written in the source code, while identifying common
      * subplans.
      */
    def mergeTests(plan: Plan): Plan =
      class SubstituteIdent(from: TermSymbol, to: TermSymbol)
          extends PlanTransform:
        override val treeMap = new TreeMap:
          override def transform(tree: Tree)(using Context) = tree match
      ...
      class MergeTests extends PlanTransform:
        override def apply(plan: SeqPlan): Plan =
          def tryMerge(plan1: Plan, plan2: Plan): Option[Plan] =
            ...
            def transferNonPatmatGenedLets(
                originalPlan: Plan,
                newPlan: Plan
            ): Plan = originalPlan match
              case originalPlan: LetPlan
                  if !isPatmatGenerated(originalPlan.sym) =>
                originalPlan.body =
                  transferNonPatmatGenedLets(originalPlan.body, newPlan)
                originalPlan
              case _ =>
                newPlan

            (skipNonPatmatGenedLets(plan1), skipNonPatmatGenedLets(plan2)) match
              case (testPlan1: TestPlan, testPlan2: TestPlan)
                  if testPlan1 == testPlan2 =>
                /* Because testPlan2 is the same as testPlan1, it cannot possibly refer to
                 * the symbols defined by any of the skipped lets.
                 */
                testPlan1.onSuccess = SeqPlan(
                  testPlan1.onSuccess,
                  transferNonPatmatGenedLets(plan2, testPlan2.onSuccess)
                )
                Some(plan1) // that's the original plan1, on purpose

              case (letPlan1: LetPlan, letPlan2: LetPlan)
                  if initializer(letPlan1.sym) === initializer(letPlan2.sym) =>
                // By construction, letPlan1.sym and letPlan2.sym are patmat-generated
                val newPlan2Body =
                  new SubstituteIdent(letPlan2.sym, letPlan1.sym)(letPlan2.body)
                letPlan1.body = SeqPlan(
                  letPlan1.body,
    ...
    /** Inline let-bound trees that are referenced only once and eliminate dead
      * code.
      *
    ...
    private def inlineVars(plan: Plan): Plan =
      ...
      def toDrop(sym: Symbol) =
        ...
        if rhs != null then
          isPatmatGenerated(sym) && refCount(
            sym
          ) <= 1 && sym != topSym && isPureExpr(rhs)
        else false
      ...
      object Inliner extends PlanTransform:
        override val treeMap = new TreeMap:
          override def transform(tree: Tree)(using Context) = tree match
            case tree: Ident =>
              val sym = tree.symbol
              if toDrop(sym) then transform(initializer(sym))
              else tree
            case _ =>
              super.transform(tree)
        override def apply(plan: LetPlan): Plan =
    ...
    private def emitCondition(plan: TestPlan): Tree =
      ...
      (plan.test: @unchecked) match
        ...
        case LengthTest(len, exact) =>
          val lengthCompareSym =
            defn.Seq_lengthCompare.matchingMember(scrutinee.tpe)
          if lengthCompareSym.exists then
            scrutinee
              .select(defn.Seq_lengthCompare.matchingMember(scrutinee.tpe))
              .appliedTo(Literal(Constant(len)))
              .select(if exact then defn.Int_== else defn.Int_>=)
        ...
        case TypeTest(tpt, trusted) =>
          ...
          /** An outer test is needed in a situation like `case x: y.Inner =>
            * ... or like case x: O#Inner if the owner of Inner is not a
            * subclass of O. Outer tests are added here instead of in
            * TypeTestsCasts since they might cause outer accessors to be added
            * to inner classes (via ensureOuterAccessors) and therefore have to
            * run before ExplicitOuter.
            */
          def addOuterTest(tree: Tree, expected: Type): Tree =
            expected.dealias match
              case tref @ TypeRef(pre, _) =>
                tref.symbol match
                  case expectedCls: ClassSymbol
                      if ExplicitOuter.needsOuterIfReferenced(expectedCls) =>
                    def selectOuter =
                      ExplicitOuter.ensureOuterAccessors(expectedCls)
                      scrutinee
                        .ensureConforms(expected)
                        .outerSelect(1, expectedCls.owner.typeRef)
                    ...
                    then
                      val testPre =
                        if expected.hasAnnotation(defn.UncheckedAnnot) then
                          AnnotatedType(
                            pre,
                            Annotation(defn.UncheckedAnnot, tree.span)
                          )
                        else pre
          ...
          expectedTp.dealias match
            case expectedTp: SingletonType =>
              scrutinee.isInstance(
                expectedTp
              ) // will be translated to an equality test
            case _ =>
    ...
    /** Collect longest list of plans that represent possible cases of a switch,
      * including a last default case, by starting with this plan and following
      * onSuccess plans.
      */
    private def collectSwitchCases(
        scrutinee: Tree,
        plan: SeqPlan
    ): List[(List[Tree], Plan)] =
      ...
      def isNewSwitchableConst(tree: Tree) = tree match
        case Literal(const)
            if (const.isIntRange || const.tag == Constants.StringTag) && !seen
          ...
          plan.expr match
            case SeqPlan(LabeledPlan(innerLabel, innerPlan), ons)
              ...
              def rec(innerPlan: Plan): Boolean = innerPlan match
                ...
                case _ =>
                  false
              if rec(innerPlan) then Some((alts.result(), ons))
              else None
      ...
      def recur(plan: Plan): List[(List[Tree], Plan)] = plan match
        case SeqPlan(testPlan @ TestPlan(EqualTest(tree), scrut, _, ons), tail)
            if scrut === scrutinee && !canFallThrough(
              ons
            ) && isNewSwitchableConst(tree) =>
      ...
      if isSwitchableType(scrutinee.tpe.widen) then recur(plan)
      else Nil
    end collectSwitchCases

    private def hasEnoughSwitchCases(
        cases: List[(List[Tree], Plan)],
        required: Int
    ): Boolean =
      ...
      required <= 1 || {
        cases match
          case (alts, _) :: cases1 =>
            hasEnoughSwitchCases(cases1, required - alts.size)
          case _ => false
      }

    /** Emit a switch-match */
    private def emitSwitchMatch(
        scrutinee: Tree,
        cases: List[(List[Tree], Plan)]
    ): Match =
      ...
      val (primScrutinee, scrutineeTpe) =
        if scrutinee.tpe.widen.isRef(defn.IntClass) then
          (scrutinee, defn.IntType)
        else if scrutinee.tpe.widen.isRef(defn.StringClass) then
          (scrutinee, defn.StringType)
        else (scrutinee.select(nme.toInt), defn.IntType)

      def primLiteral(lit: Tree): Tree =
        val Literal(constant) = lit: @unchecked
        if constant.tag == Constants.IntTag then lit
        else if constant.tag == Constants.StringTag then lit
    ...
    /** If selfCheck is `true`, used to check whether a tree gets generated
      * twice
      */
    ...
    private def emit(plan: Plan): Tree =
      if selfCheck then
        assert(
          plan.isInstanceOf[ReturnPlan] || !emitted.contains(plan.id),
          plan.id
        )
        emitted += plan.id
      plan match
        case plan: TestPlan =>
          /** Merge nested `if`s that have the same `else` branch into a single
            * `if`. This optimization targets calls to label defs for case
            * failure jumps to next case.
            *
            * Plan for
            * ```
            * val x1: Int = ...
          ...
          def emitWithMashedConditions(plans: List[TestPlan]): Tree =
            ...
            plan.onSuccess match
              ...
              case _ =>
                def emitCondWithPos(plan: TestPlan) =
                  emitCondition(plan).withSpan(plan.span)
                val conditions =
                  plans.foldRight[Tree](EmptyTree) { (otherPlan, acc) =>
                    if acc.isEmpty then emitCondWithPos(otherPlan)
                    else
                      acc.select(nme.ZAND).appliedTo(emitCondWithPos(otherPlan))
                  }
                If(conditions, emit(plan.onSuccess), unitLiteral)
          end emitWithMashedConditions
          emitWithMashedConditions(plan :: Nil)
        case LetPlan(sym, body) =>
          val valDef = ValDef(
            sym,
            initializer(sym).ensureConforms(sym.info),
            inferred = true
          ).withSpan(sym.span)
        ...
        case plan: SeqPlan =>
          ...
          def maybeEmitSwitch(scrutinee: Tree): Tree =
            ...
            if hasEnoughSwitchCases(switchCases, MinSwitchCases)
            then // at least 3 cases + default
          ...
          plan.head match
            ...
            case LabeledPlan(
                  _,
                  SeqPlan(LabeledPlan(_, SeqPlan(testPlan: TestPlan, _)), _)
                ) =>
              maybeEmitSwitch(testPlan.scrutinee)
            case _ =>
              default
        case ResultPlan(tree) =>
          if tree.symbol == defn.throwMethod then tree // For example MatchError
    ...
    /** If match is switch annotated, check that it translates to a switch with
      * at least as many cases as the original match.
      */
    private def checkSwitch(original: Match, result: Tree) =
      original.selector match
        case Typed(_, tpt) if tpt.tpe.hasAnnotation(defn.SwitchAnnot) =>
          val resultCases = result match
            ...
            case Block((_: ValDef) :: Block(_, Match(_, cases)) :: Nil, _) =>
              cases
            case _ => Nil
          val caseThreshold =
          ...
          def numTypes(cdefs: List[CaseDef]): Int =
            typesInCases(
              cdefs
            ).toSet.size: Int // without the type ascription, testPickling fails because of #2840.
          val numTypesInOriginal = numTypes(original.cases)
          if numTypesInOriginal >= caseThreshold && numTypes(
              resultCases
            ) < numTypesInOriginal
          then
            patmatch.println(i"switch warning for ${ctx.compilationUnit}")
            patmatch.println(
              i"original types: ${typesInCases(original.cases)}%, %"
            )
            patmatch.println(
              i"switch types  : ${typesInCases(resultCases)}%, %"
            )
    ...
    def translateMatch(tree: Match): Tree =
      ...
      if !ctx.settings.YnoPatmatOpt.value then
        for (title, optimization) <- optimizations do
