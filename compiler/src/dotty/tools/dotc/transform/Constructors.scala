...
/** This transform
  ...
  *   - also moves private fields that are accessed only from constructor into
  *     the constructor if possible.
  */
class Constructors extends MiniPhase with IdentityDenotTransformer:
  thisPhase =>
  import tpd.*
  ...
  override def transformValDef(tree: tpd.ValDef)(using Context): tpd.Tree =
    if mightBeDropped(tree.symbol) then seenPrivateVals += tree.symbol
    tree

  /** All initializers for non-lazy fields should be moved into constructor. All
    * non-abstract methods should be implemented (this is assured for
    * constructors in this phase and for other methods in memoize).
    */
  override def checkPostCondition(tree: tpd.Tree)(using Context): Unit =
    def emptyRhsOK(sym: Symbol) =
      sym.isOneOf(DeferredOrLazy) || sym.isConstructor && sym.owner.isAllOf(
        NoInitsTrait
      )
    tree match
      case tree: ValDef
          if tree.symbol.exists && tree.symbol.owner.isClass && !tree.symbol.is(
            Lazy
          ) && !tree.symbol.hasAnnotation(defn.ScalaStaticAnnot) =>
        assert(
          tree.rhs.isEmpty,
          i"$tree: initializer should be moved to constructors"
        )
  ...
  /** @return
    *   true if after ExplicitOuter, all references from this tree go via an
  ...
  override def transformTemplate(tree: Template)(using Context): Tree =
    ...
    val constr @ DefDef(
      nme.CONSTRUCTOR,
      (vparams: List[ValDef] @unchecked) :: Nil,
      _,
      EmptyTree
    ) = tree.constr: @unchecked
    ...
    object intoConstr extends TreeMap:
      ...
      override def transform(tree: Tree)(using Context): Tree = tree match
        case Ident(_) | Select(This(_), _) =>
          var sym = tree.symbol
          def isOverridableSelect =
            tree.isInstanceOf[Select] && !sym.isEffectivelyFinal
          ...
          if sym.is(ParamAccessor) && (switchOutsideSupercall || inSuperCall)
          then sym = sym.subst(accessors, paramSyms)
          if sym.maybeOwner.isConstructor then ref(sym).withSpan(tree.span)
          else tree
        case Apply(fn, Nil) =>
          val fn1 = transform(fn)
          if (fn1 ne fn) && fn1.symbol.is(
              Param
        ...
        case _ =>
          if noDirectRefsFrom(tree) then tree else super.transform(tree)
    ...
    /** Map outer getters $outer and outer accessors $A$B$$$outer to the given
      * outer parameter.
      */
    def mapOuter(outerParam: Symbol) = new TreeMap:
      override def transform(tree: Tree)(using Context) = tree match
        case Apply(fn, Nil)
          ...
          ref(outerParam)
        case tree: RefTree
            if tree.symbol.is(ParamAccessor) && tree.symbol.name == nme.OUTER =>
          ref(outerParam)
        case _ =>
      ...
      case stat :: stats1 =>
        stat match
          case stat @ ValDef(name, tpt, _)
              if !stat.symbol.is(Lazy) && !stat.symbol.hasAnnotation(
                defn.ScalaStaticAnnot
              ) =>
            val sym = stat.symbol
            if isRetained(sym) then
              if !stat.rhs.isEmpty && !isWildcardArg(stat.rhs) then
                constrStats += Assign(ref(sym), intoConstr(stat.rhs, sym))
                  .withSpan(stat.span)
          ...
          case stat @ DefDef(name, _, tpt, _)
              if stat.symbol.isGetter && !stat.symbol.is(Lazy) =>
            val sym = stat.symbol
            assert(isRetained(sym), sym)
            if sym.isConstExprFinalVal then
              if stat.rhs.isInstanceOf[Literal] then clsStats += stat
              else
                constrStats += intoConstr(stat.rhs, sym)
                clsStats += cpy.DefDef(stat)(rhs =
                  Literal(sym.constExprFinalValConstantType.value)
                    .withSpan(stat.span)
                )
            else if !sym.owner.is(Trait) then clsStats += stat
            else
              if !stat.rhs.isEmpty && !isWildcardArg(stat.rhs) then
                /* !!! Work around #9390
                 ...
                 */
                val symSetter =
                  if sym.setter.exists then sym.setter
                  else
                    val setterName = sym.asTerm.name.setterName
                    sym.owner.info.decls.find(d =>
                      d.is(Accessor) && d.name == setterName
                    )
                val setter =
                  if symSetter.exists then symSetter
                  else sym.accessorNamed(Mixin.traitSetterName(sym.asTerm))
                constrStats += Apply(
                  ref(setter),
                  intoConstr(stat.rhs, sym).withSpan(stat.span) :: Nil
                )
              clsStats += cpy.DefDef(stat)(rhs = EmptyTree)
            end if
          case DefDef(
                nme.CONSTRUCTOR,
                ((outerParam @ ValDef(nme.OUTER, _, _)) :: _) :: Nil,
                _,
                _
              ) =>
    ...
    /** Check that we do not have both a private field with name `x` and a
      * private field with name `FieldName(x)`. These will map to the same JVM
      * name and therefore cause a duplicate field error. If that case arises
      * (as in i13862.scala), use an explicit name `x$field` instead of
      * `FieldName(x).
      */
      ...
      for case field: ValDef <- clsStats do
        field.symbol.name match
          case fldName @ FieldName(name) if fieldNames.contains(name) =>
            ...
            report.log(
              i"avoid field/field conflict by renaming $fldName to $newName"
            )
            field.symbol
              .copySymDenotation(name = newName)
              .installAfter(thisPhase)
          case _ =>
    ...
    val copyParams = accessors flatMap { acc =>
      if !isRetained(acc) then
        dropped += acc
        Nil
      else if !isRetained(acc.field)
      then // It may happen for unit fields, tests/run/i6987.scala
        ...
        if param.hasAnnotation(defn.ConstructorOnlyAnnot) then
          report.error(
            em"${acc.name} is marked `@constructorOnly` but it is retained as a field in ${acc.owner}",
            acc.srcPos
          )
        ...
        if !target.exists then
          ...
          if acc.name != nme.OUTER then assigns
            ...
            val nullTest =
              If(
                ref(param).select(defn.Object_eq).appliedTo(nullLiteral),
    ...
    val expandedConstr =
      if cls.isAllOf(NoInitsTrait) then
