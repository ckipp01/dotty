...
/** Generates JUnit bootstrapper objects for Scala.js.
  ...
  * methods. They are identified by annotations such as `@Test`, `@Before`, etc.
  * In Scala.js, there is no such reflection for methods and annotations, so a
  * different strategy is used: this phase performs the necessary inspections at
  * compile-time, and generates a so-called bootstrapper object where all those
  * metadata have been reified.
  *
...
class JUnitBootstrappers extends MiniPhase:
        ...
        val metadata = for (test <- tests) yield
          ...
          val mappedArguments = testAnnot.arguments.flatMap {
            ...
            case other =>
              ...
              report.error(
                em"$shownName is an unsupported argument for the JUnit @Test annotation in this position",
                other.sourcePos
              )
              None
          }
  ...
  private def genTestInvocation(
    ...
    if resultType.isRef(defn.UnitClass) then
      val newSuccess =
  ...
  private def annotatedMethods(owner: ClassSymbol, annot: Symbol)(using
    ...
    owner.info
      ...
      .map(_.symbol)
      .toList
end JUnitBootstrappers

object JUnitBootstrappers:
  val name: String = "junitBootstrappers"
  val description: String =
    "generate JUnit-specific bootstrapper classes for Scala.js"
