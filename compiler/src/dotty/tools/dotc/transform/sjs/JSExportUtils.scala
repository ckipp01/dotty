...
object JSExportUtils:
  ...
  def makeExportName(jsName: String, isProp: Boolean): TermName =
    val prefix = if isProp then PropExportPrefix else MethodExportPrefix
  ...
  /** Retrieves the originally assigned jsName of this export and whether it is
    * a property.
    */
  def exportNameInfo(name: Name): (String, Boolean) =
    val nameString = name.toString()
    if nameString.startsWith(MethodExportPrefix) then
      (nameString.substring(MethodExportPrefix.length), false)
    else if nameString.startsWith(PropExportPrefix) then
      (nameString.substring(PropExportPrefix.length), true)
    else
      throw new IllegalArgumentException(
        s"non-exported name passed to jsExportInfo: $name"
      )
