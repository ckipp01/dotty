...
object PrepJSExports:
  ...
  /** Checks a class or module class for export.
    *
    * Note that non-module Scala classes are never actually exported; their
    * constructors are. However, the checks are performed on the class when the
    * class is annotated.
    */
  def checkClassOrModuleExports(sym: Symbol)(using Context): Unit =
    val exports = exportsOf(sym)
    if exports.nonEmpty then checkClassOrModuleExports(sym, exports.head.pos)
  ...
  def genExportMember(baseSym: Symbol)(using Context): List[Tree] =
    ...
    def memType = if baseSym.isConstructor then "constructor" else "method"

    if exports.isEmpty then Nil
    else if !hasLegalExportVisibility(baseSym) then
      err(s"You may only export public and protected ${memType}s")
    else if baseSym.is(Inline) then err("You may not export an inline method")
    else if isJSAny(clsSym) then
      err(s"You may not export a $memType of a subclass of js.Any")
    else if baseSym.isLocalToBlock then
      err("You may not export a local definition")
    else if hasIllegalRepeatedParam(baseSym) then
      err(
        s"In an exported $memType, a *-parameter must come last (through all parameter lists)"
      )
    else if hasIllegalDefaultParam(baseSym) then
      err(
        s"In an exported $memType, all parameters with defaults must be at the end"
      )
    else if baseSym.isConstructor then
      ...
      val normalExports =
        exports.filter(_.destination == ExportDestination.Normal)
  ...
  /** Check a class or module for export.
    ...
    */
  private def checkClassOrModuleExports(sym: Symbol, errPos: SrcPos)(using
      Context
  ): Unit =
    ...
    def hasAnyNonPrivateCtor: Boolean =
      sym.info
        ...
        .hasAltWith(denot => !isPrivateMaybeWithin(denot.symbol))

    if sym.is(Trait) then err("You may not export a trait")
    else if sym.hasAnnotation(jsdefn.JSNativeAnnot) then
      err(
        "You may not export a native JS " + (if isMod then "object"
                                             else "class")
      )
    else if !hasLegalExportVisibility(sym) then
      err(
        "You may only export public and protected " + (if isMod then "objects"
                                                       else "classes")
      )
    else if isJSAny(sym.owner) then
      err(
        "You may not export a " + (if isMod then "object"
                                   else "class") + " in a subclass of js.Any"
      )
    else if sym.isLocalToBlock then
      err("You may not export a local " + (if isMod then "object" else "class"))
    else if !sym.isStatic then
      if isMod then err("You may not export a nested object")
      else
        err(
          "You may not export a nested class. Create an exported factory method in the outer class to work around this limitation."
        )
    else if sym.is(Abstract, butNot = Trait) && !isJSAny(sym) then
      err("You may not export an abstract class")
    else if !isMod && !hasAnyNonPrivateCtor then
  ...
  private def exportsOf(sym: Symbol)(using Context): List[ExportInfo] =
    val trgSym =
      ...
      if sym.isPrimaryConstructor && isOwnerScalaClass then sym.owner
      else sym
    ...
    val directMemberAnnots =
      Set[Symbol](JSExportAnnot, JSExportTopLevelAnnot, JSExportStaticAnnot)
    val directAnnots = trgSym.annotations.filter(annot =>
      directMemberAnnots.contains(annot.symbol)
    )
    ...
    val unitAnnots =
      if isMember && sym.isPublic && !sym.is(Synthetic) then
        sym.owner.annotations.filter(_.symbol == JSExportAllAnnot)
      else Nil

    val allExportInfos =
      for annot <- directAnnots ++ unitAnnots
      yield
        ...
        val exportPos: SrcPos = if isExportAll then sym else annot.tree

        assert(
          !isTopLevelExport || hasExplicitName,
        ...
        val name =
          ...
          else sym.defaultJSName

        val destination =
          if isTopLevelExport then
            val moduleID =
              if annot.arguments.size == 1 then DefaultModuleID
              else
         ...
         */
        if isMember && !hasExplicitName && sym.name == nme.apply then
          destination match
        ...
        val symOwner =
          if sym.isConstructor then sym.owner.owner
          else sym.owner
        ...
        destination match
          case ExportDestination.Normal =>
            ...
            if symOwner.is(Package) || symOwner.isPackageObject then
              report.error(
                "@JSExport is forbidden on top-level definitions. Use @JSExportTopLevel instead.",
                exportPos
              )
            else if !isMember && !sym.is(Trait) then
              report.error(
          ...
          case _: ExportDestination.TopLevel =>
            ...
            else if sym.is(Method, butNot = Accessor) && sym.isJSProperty then
              report.error(
            ...
            if !isValidTopLevelExportName(name) then
              report.error(
          ...
          case ExportDestination.Static =>
            ...
            if !symOwner.isStatic || !symOwner.is(
                ModuleClass
    ...
    allExportInfos
      ...
      .filter { case (jsName, group) =>
        if jsName == "apply" && group.size == 2 then
    ...
    if !sym.is(Method) then
      for firstStatic <- allExportInfos.find(
          _.destination == ExportDestination.Static
        )
      do
        for duplicate <- allExportInfos do
          duplicate.destination match
            case ExportDestination.Normal =>
            // OK
            case ExportDestination.Static =>
              if duplicate ne firstStatic then
                report.error(
  ...
  private def genExportDefs(defSym: Symbol, jsName: String, span: Span)(using
      Context
  ): List[Tree] =
    ...
    val info =
      if defSym.isConstructor then defSym.info
      else if defSym.is(Method) then finalResultTypeToAny(defSym.info)
      else ExprType(defn.AnyType)
    val expSym =
      newSymbol(clsSym, name, flags, info, defSym.privateWithin, span).entered
    ...
    val defaultGetters =
      if !defSym.hasDefaultParams then Nil
      else
        for
          (param, i) <- defSym.paramSymss.flatten.zipWithIndex
          if param.is(HasDefault)
        yield genExportDefaultGetter(clsSym, defSym, expSym, i, span)

    exporter :: defaultGetters

  private def genExportDefaultGetter(
      clsSym: ClassSymbol,
      trgMethod: Symbol,
      exporter: Symbol,
      paramPos: Int,
    ...
    assert(
      trgGetterDenot.exists,
      em"Cannot find default getter for param $paramPos of $trgMethod"
    )
  ...
  private def finalResultTypeToAny(tpe: Type)(using Context): Type = tpe match
    case tpe: MethodType =>
      MethodType(
        tpe.paramNames,
        tpe.paramInfos,
        finalResultTypeToAny(tpe.resultType)
      )
    case _: ExprType =>
  ...
  /** Checks whether this type has a repeated parameter elsewhere than at the
    * end of all the params.
    */
  ...
  /** Checks whether there are default parameters not at the end of the
    * flattened parameter list.
    */
  private def hasIllegalDefaultParam(sym: Symbol)(using Context): Boolean =
    sym.hasDefaultParams
      && sym.paramSymss.flatten.reverse
        .dropWhile(_.is(HasDefault))
        .exists(_.is(HasDefault))
end PrepJSExports
