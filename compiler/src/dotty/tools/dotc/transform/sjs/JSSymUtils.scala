...
object JSSymUtils:
  /** The result type for `sym.jsName`.
    *
    * It is either a literal string, or a computed name represented by a
    * reference to a static `Symbol` (a `dotc.core.Symbols.Symbol`, not a
    * `js.Symbol`).
    */
  enum JSName:
  ...
  object JSCallingConvention:
    def of(sym: Symbol)(using Context): JSCallingConvention =
      ...
      if isJSBracketAccess(sym) then BracketAccess
        ...
        if !sym.hasAnnotation(jsdefn.JSNameAnnot) then
  ...
  extension (sym: Symbol)
    ...
    def isJSExposed(using Context): Boolean =
      !sym.is(Bridge) && {
        ...
        || (sym.is(Accessor) && sym.field.hasAnnotation(
          jsdefn.ExposedJSMemberAnnot
        ))
      }
    ...
    def isJSGetter(using Context): Boolean =
      sym.is(Module)
        || !sym.is(Method)
        || (sym.info.firstParamTypes.isEmpty && atPhaseNoLater(erasurePhase)(
    ...
    def jsName(using Context): JSName =
      sym
        .getAnnotation(jsdefn.JSNameAnnot)
        .fold[JSName] {
    ...
    def defaultJSName(using Context): String =
      if sym.isTerm then sym.asTerm.name.unexpandedName.getterName.toString()
      else sym.name.unexpandedName.stripModuleClassSuffix.toString()

    def jsParamInfos(using Context): List[JSParamInfo] =
      assert(
        sym.is(Method),
        s"trying to take JS param info of non-method: $sym"
      )
      ...
      val paramInfosAtElimRepeated = atPhase(elimRepeatedPhase) {
        val list =
          for ((name, info) <- paramNamesAndTypes) yield
            val v =
              if info.isRepeatedParam then
                Some(info.repeatedToSingle.widenDealias)
              else None
            name -> v
        list.toMap
      }
      ...
      for ((paramName, paramInfoNow) <- paramNamesAndTypes)
        yield paramInfosAtElimRepeated.get(paramName) match
          case None =>
    ...
    /** Tests whether the semantics of Scala.js require a field for this symbol,
      ...
      *
      *   - it is a member of a JS type, since it needs to be visible as a
      *     JavaScript field
      *   - is is exported as static member of the companion class, since it
      *     needs to be visible as a JavaScript static field
      *   - it is exported to the top-level, since that can only be done as a
      *     true top-level variable, i.e., a field
      */
