...
/** This phase makes all JS classes explicit (their definitions and references
  * to them).
  *
  ...
  * field `Outer.Inner\$jsclass` to hold the JS class value of `Inner`. (B) For
  * every exposed object `Inner` in a static owner `Outer`, create an explicit
  * exposed getter `Outer.Inner\$jsobject`. (C) For every local JS class
  * `Local`, create a local val `Local\$jsclass` to hold the JS class value of
  * `Local`. (D) Desugar calls like `x.isInstanceOf[C]` into
  * `js.special.instanceof(x, js.constructorOf[C])` when `C` is a nested JS
  * class. (E) Wrap every `new C` call and `super[C]` reference of a nested JS
  * class `C` with `withContextualJSClassValue(js.constructorOf[C], ...)`. (F)
  * Desugar calls to `js.constructorOf[C]` (including those generated by the
  * previous transformations) into either `runtime.constructorOf` or access to
  * the `\$jsclass` fields/vals. (G) Adjust the `NoInits` flag of traits:
  ...
  * to `js.constructorOf[Inner]`, which becomes
  ...
  * }}}
  *
  * The `\$jsclass` fields must also be added to outer classes and traits coming
  ...
  *
  * Similarly to how step (A) creates explicit fields in the enclosing templates
  * of inner JS classes and traits to hold the JS class values, this phase
  ...
  *     `withContextualJSClassValue(js.constructorOf[C], new
  *     C[...Ts](...args))`, so that the back-end receives a reified reference
  *     to the JS class value.
  *   - In the same spirit, for `D extends C`, `D.super[C].m[...Ts](...args)`
  *     desugars into `withContextualJSClassValue(js.constructorOf[C],
  *     D.super[C].m[...Ts](...args))`.
  *
  * For any nested JS *object*, their (only) instantiation point of the form
  * `new O$()` is rewritten as
  * `withContextualJSClassValue(js.constructorOf[ParentClassOfO], new O$())`, so
  * that the back-end receives a reified reference to the parent class of `O`.
  *
  ...
  *     created by step (C).
  */
class ExplicitJSClasses extends MiniPhase with InfoTransformer:
  thisPhase =>
  ...
  override def runsAfter: Set[String] =
    Set(PatternMatcher.name, HoistSuperArgs.name)

  override def changesMembers: Boolean =
    true // the phase adds fields for inner JS classes

  /** Is the given symbol an owner that might receive `\$jsclass` and/or
    * `\$jsobject` fields?
    *
    ...
    */
  private def mayNeedJSClassOrJSObjectFields(
      sym: Symbol
  )(using Context): Boolean =
    !sym.isStaticOwner
      || (sym.is(ModuleClass) && sym.hasAnnotation(jsdefn.JSTypeAnnot) && !sym
        .hasAnnotation(jsdefn.JSNativeAnnot))
  ...
  private def isInnerOrLocalJSClassOrObject(sym: Symbol)(using
      Context
  ): Boolean =
  ...
  private def isLocalJSClassOrObject(clazz: Symbol)(using Context): Boolean =
    clazz.isLocalToBlock && !clazz.is(Trait) && clazz.hasAnnotation(
      jsdefn.JSTypeAnnot
    )
  ...
  /** Is the given clazz considered to be an object for the purposes of this
    * phase? This is true for module classes and for anonymous JS classes.
    */
  ...
  private def jsclassAccessorFor(clazz: Symbol)(using Context): TermSymbol =
    clazz.owner.info.decls
      .lookup(jsclassFieldName(clazz.name.asTypeName))
      .asTerm

  private def jsobjectGetterName(moduleName: TermName): TermName =
    moduleName ++ "$jsobject"

  private def jsobjectGetterNameFor(moduleSym: Symbol)(using
      Context
  ): TermName =
    jsobjectGetterName(moduleSym.name.asTermName)

  private def makeJSNameAnnotation(argument: String)(using
      Context
  ): Annotation =
    val annotClass = jsdefn.JSNameAnnot
    val stringCtor = annotClass.info
      .decl(nme.CONSTRUCTOR)
      .suchThat { ctor =>
        ctor.info match
          case mt: MethodType =>
            mt.paramInfos.nonEmpty && mt.paramInfos.head.derivesFrom(
              defn.StringClass
            )
          case _ => false
      }
      ...
      .asTerm
    Annotation(
      New(annotClass.typeRef, stringCtor, Literal(Constant(argument)) :: Nil)
    )

  override def transformInfo(tp: Type, sym: Symbol)(using Context): Type =
    tp match
      case tp @ ClassInfo(_, cls, _, decls, _)
          if !cls.is(JavaDefined) && mayNeedJSClassOrJSObjectFields(cls) =>
        val innerJSClasses = decls.filter(isJSClass)

        val innerObjectsForAdHocExposed =
          if !cls.isStaticOwner then Nil // those already have a module accessor
        ...
        if innerJSClasses.isEmpty && innerObjectsForAdHocExposed.isEmpty then tp
          ...
          for innerJSClass <- innerJSClasses do
            def addAnnotsIfInJSClass(sym: Symbol): Unit =
              if clsIsJSClass then addAnnots(sym, innerJSClass)
            ...
            val field = newSymbol(
              ...
              fieldFlags,
              defn.AnyRefType,
              coord = innerJSClass.coord
            )
            addAnnotsIfInJSClass(field)
            decls1.enter(field)

          // scala-js/scala-js#4086 Create exposed getters for exposed objects in static JS objects
          for innerObject <- innerObjectsForAdHocExposed do
            assert(
              clsIsJSClass && cls.is(ModuleClass) && cls.isStatic,
            ...
            val getter = newSymbol(
              cls,
              getterName,
              getterFlags,
              ExprType(defn.AnyRefType),
              coord = innerObject.coord
            )
  ...
  /** Adjust the `NoInits` flag of Scala traits containing a JS class and of JS
    * traits.
    */
  ...
  /** Populate `nestedObject2superTypeConstructor` for inner objects at the
    * start of a `Block` or `Template`, so that they are visible even before
    * their definition (in their enclosing scope).
    */
  private def populateNestedObject2superClassTpe(
      stats: List[Tree]
  )(using Context): Unit =
    for stat <- stats do
      stat match
        case cd @ TypeDef(_, rhs)
            if cd.isClassDef && isInnerOrLocalJSObject(cd.symbol) =>
          myState.nestedObject2superTypeConstructor(cd.symbol) =
            extractSuperTypeConstructor(rhs)
        case _ =>
  ...
  override def transformTemplate(tree: Template)(using Context): Tree =
     ...
     */
    val fixedParents =
      if !cls.isJSType then tree.parents // fast path
    ...
    if !mayNeedJSClassOrJSObjectFields(cls) then
      ...
      for stat <- tree.body do
        stat match
          ...
          case stat: ValDef
              if cls.isStaticOwner && isExposedModule(stat.symbol) =>
            // scala-js/scala-js#4086
            val moduleSym = stat.symbol
            val getter =
              cls.info.decls.lookup(jsobjectGetterNameFor(moduleSym)).asTerm
  ...
  override def transformTypeDef(tree: TypeDef)(using Context): Tree =
    ...
    if sym.isClass && isLocalJSClass(sym) then
      ...
      val rhs =
        ...
        val superClassCtor =
          genJSConstructorOf(tree, extractSuperTypeConstructor(tree.rhs))
        ref(jsdefn.Runtime_createLocalJSClass).appliedTo(
          clazzValue,
          superClassCtor,
  ...
  override def transformApply(tree: Apply)(using Context): Tree =
    if !isFullyApplied(tree) then tree
      ...
      if sym.isConstructor then
         ...
         */
        val cls = sym.owner
        if isInnerOrLocalJSClassOrObject(cls) then
          if !isConsideredAnObject(cls) then
            methPart(tree) match
            ...
            wrapWithContextualJSClassValue(
              myState.nestedObject2superTypeConstructor(cls)
            )(tree)
        else tree
  ...
  override def transformTypeApply(tree: TypeApply)(using Context): Tree =
    if !isFullyApplied(tree) then tree
      ...
      tree match
        ...
        case TypeApply(fun, tpt :: Nil)
            if sym == jsdefn.JSPackage_constructorOf =>
        ...
        case TypeApply(fun @ Select(obj, _), tpeArg :: Nil)
            if sym == defn.Any_isInstanceOf && isTypeTreeForInnerOrLocalJSClass(
              tpeArg
            ) =>
  ...
  /** Tests whether this tree is fully applied, i.e., it does not need any
    ...
    *
    * In this phase, `transformApply`, `transformTypeApply` and
    * `transformSelect` must only operate on fully applied selections and
    * applications.
    */
  ...
  /** Wraps `super` calls to inner and local JS classes with
    ...
    */
  private def maybeWrapSuperCallWithContextualJSClassValue(tree: Tree)(using
      Context
  ): Tree =
    methPart(tree) match
      case Select(sup: Super, _)
          if isInnerOrLocalJSClass(sup.symbol.asClass.superClass) =>
        wrapWithContextualJSClassValue(sup.symbol.asClass.superClass.typeRef)(
          tree
        )
      case _ =>
        tree
  ...
  end genJSConstructorOf

  private def wrapWithContextualJSClassValue(jsClassType: Type)(tree: Tree)(
      using Context
  ): Tree =
    wrapWithContextualJSClassValue(genJSConstructorOf(tree, jsClassType))(tree)

  private def wrapWithContextualJSClassValue(
      jsClassValue: Tree
  )(tree: Tree)(using Context): Tree =
    ref(jsdefn.Runtime_withContextualJSClassValue)
      .appliedToType(tree.tpe)
      .appliedTo(jsClassValue, tree)

  private def unwrapWithContextualJSClassValue(tree: Tree)(using
      Context
  ): Tree = tree match
  ...
  /** Extracts the super type constructor of a `Template`, without type
    ...
    * }}}
    * we extract the type constructor `pre.Parent`, without its type parameters.
    *
    * Since the result is not necessarily *-kinded, its applicability is
    * limited. It seems to be sufficient to put in a `classOf`, though, which is
    * what we care about.
    */
  private def extractSuperTypeConstructor(typeDefRhs: Tree)(using
      Context
  ): Type =
    typeDefRhs.asInstanceOf[Template].parents.head.tpe.dealias.typeConstructor
end ExplicitJSClasses
