...
/** Adds fake calls to the constructors of local JS classes in calls to
  ...
  * }}}
  * this phase fills in the `???` with an array of calls to the constructors of
  * `C`, like
  * {{{
  *  [ new C(), new C(???, ???) : Object ]
  * }}}
  ...
  */
class AddLocalJSFakeNews extends MiniPhase:
  thisPhase =>
  ...
  override def transformApply(tree: Apply)(using Context): Tree =
    if tree.symbol == jsdefn.Runtime_createLocalJSClass then
      val classValueArg :: superClassValueArg :: _ :: Nil =
        tree.args: @unchecked
      val cls = classValueArg match
        ...
        case _ =>
          ...
          report.error(
            em"unexpected $classValueArg for the first argument to `createLocalJSClass`",
            classValueArg
          )
      ...
      cpy.Apply(tree)(
        tree.fun,
        classValueArg :: superClassValueArg :: fakeNews :: Nil
      )
    else tree
  ...
  private def fakeNew(cls: ClassSymbol, ctor: TermSymbol)(using Context): Tree =
    ...
    val nonOuterArgs =
      List.fill(nonOuterArgCount)(ref(defn.Predef_undefined).appliedToNone)

    New(tycon, ctor, outerArgs ::: nonOuterArgs)
end AddLocalJSFakeNews
