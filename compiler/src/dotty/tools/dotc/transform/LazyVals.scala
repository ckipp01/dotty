...
class LazyVals extends MiniPhase with IdentityDenotTransformer:
  ...
  override def runsAfter: Set[String] =
    Set(Mixin.name, CollectNullableFields.name)

  override def changesMembers: Boolean =
    true // the phase adds lazy val accessors
  ...
  /** A map of lazy values to the fields they should null after initialization.
    */
  private var lazyValNullables
      : IdentityHashMap[Symbol, mutable.ListBuffer[Symbol]] | Null = _
  private def nullableFor(sym: Symbol)(using Context) =
    ...
    if nullables == null then Nil
    else nullables.toList

  private def needsBoxing(tp: Type)(using Context): Boolean =
    tp.classSymbol.isPrimitiveValueClass

  override def prepareForUnit(tree: Tree)(using Context): Context =
    if lazyValNullables == null then
      lazyValNullables = ctx.base.collectNullableFieldsPhase
        .asInstanceOf[CollectNullableFields]
        .lazyValNullables
    ctx
  ...
  def transformLazyVal(tree: ValOrDefDef)(using Context): Tree =
    ...
    if !sym.is(Lazy) ||
      sym.owner.is(
        Trait
      ) || // val is accessor, lazy field will be implemented in subclass
    ...
    then // static module vals are implemented in the JVM by lazy loading
      ...
      if isField then
        ...
        else if sym.isThreadUnsafe || ctx.settings.scalajs.value then
          if sym.is(Module) && !ctx.settings.scalajs.value then
            report.error(
              em"@threadUnsafe is only supported on lazy vals",
              sym.srcPos
            )
  ...
  override def transformTemplate(template: Template)(using Context): Tree =
    ...
    appendOffsetDefs.get(cls) match
      case None => template
      case Some(data) =>
        data.defs.foreach(defin =>
          defin.symbol.addAnnotation(
            Annotation(defn.ScalaStaticAnnot, defin.symbol.span)
          )
        )
  ...
  def transformSyntheticModule(tree: ValOrDefDef)(using Context): Thicket =
    val sym = tree.symbol
    val holderSymbol = newSymbol(
      sym.owner,
      LazyLocalName.fresh(sym.asTerm.name),
      Synthetic,
      sym.info.widen.resultType
    ).enteredAfter(this)
    val field =
      ValDef(holderSymbol, tree.rhs.changeOwnerAfter(sym, holderSymbol, this))
  ...
  def transformLocalDef(x: ValOrDefDef)(using Context): Thicket =
    ...
    val getValue =
      holderRef.select(lazyNme.value).ensureApplied.withSpan(x.span)
    ...
    // def x(): Int = if (x$lzy.initialized()) x$lzy.value() else x$lzycompute()
    val accessorBody = If(initialized, getValue, ref(initSymbol).ensureApplied)
      .ensureConforms(tpe)
    val accessor = DefDef(x.symbol.asTerm, accessorBody)

    report.debuglog(
      s"found a lazy val ${x.show},\nrewrote with ${holderTree.show}"
    )
    Thicket(holderTree, initTree, accessor)
  end transformLocalDef

  override def transformStats(trees: List[tpd.Tree])(using
      Context
  ): List[Tree] =
  ...
  /** Create thread-unsafe lazy accessor equivalent to such code
    ...
    * ```
    */
  def mkThreadUnsafeDef(sym: Symbol, flag: Symbol, target: Symbol, rhs: Tree)(
      using Context
  ): DefDef =
    val targetRef = ref(target)
    val flagRef = ref(flag)
    val stats = targetRef.becomes(rhs) :: flagRef.becomes(
      Literal(Constant(true))
    ) :: nullOut(nullableFor(sym))
    val init = If(
  ...
  /** Create thread-unsafe lazy accessor for not-nullable types equivalent to
    * such code
    * ```
    * def methodSymbol() = {
    *   if (target eq null) {
    ...
    * ```
    */
  def mkDefThreadUnsafeNonNullable(sym: Symbol, target: Symbol, rhs: Tree)(using
      Context
  ): DefDef =
    val targetRef = ref(target)
  ...
  def transformMemberDefThreadUnsafe(x: ValOrDefDef)(using Context): Thicket =
    ...
    val containerSymbol = newSymbol(
      claz,
      containerName,
      x.symbol.flags &~ containerFlagsMask | containerFlags | Private,
      tpe,
      coord = x.symbol.coord
    ).enteredAfter(this)

    val containerTree = ValDef(containerSymbol, defaultValue(tpe))
    if x.tpe.isNotNull && tpe <:< defn.ObjectType then
  ...
  /** Create a threadsafe lazy accessor and function that computes the field's
    * value. `Evaluating` and `NullValue` are represented by `object`s and
    * `Waiting` by a class that allows awaiting the completion of the
    * evaluation. Note that since tail-recursive functions are transformed
    * *before* lazy-vals, this implementation does involve explicit while loop.
    * `PatternMatcher` is coming before `LazyVals`, therefore the pattern
    * matching is implemented using if-s.
    *
    * \``` private @volatile var _x: AnyRef = null
    *
    * def x: A = val result = _x if result.isInstanceOf[A] then result //
    * possible unboxing applied here else if result.eq(NullValue) then null //
    * possible unboxing applied here else x_compute() // possible unboxing
    * applied here
    *
    * private def x_compute(): AnyRef = while <EmptyTree> do val current: AnyRef
    * \= _x if current.eq(null) then if CAS(_x, null, Evaluating) then var
    * resultNullable: AnyRef = null var result: AnyRef = null try resultNullable
    * \= rhs nullable = null // nulls out the nullable fields used only in
    * initialization if resultNullable.eq(null) then result = NullValue else
    * result = resultNullable finally if !CAS(_x, Evaluating, result) then val
    * lock = _x.asInstanceOf[Waiting] CAS(_x, lock, result) lock.release()
    * return resultNullable else if current.isInstanceOf[LazyValControlState]
    * then if current.eq(Evaluating) then // To avoid creating Waiting instance
    * CAS(current, current, new Waiting) else if current.isInstanceOf[Waiting]
    * then current.asInstanceOf[Waiting].await() else return null else return
    * current end while * ```
    *
    * @param memberDef
  ...
  def mkThreadSafeDef(
    ...
    val evaluating =
      Select(ref(defn.LazyValsModule), lazyNme.RLazyVals.evaluating)
    val nullValue =
      Select(ref(defn.LazyValsModule), lazyNme.RLazyVals.nullValue)
    ...
    val lazyInitMethodSymbol = newSymbol(
      claz,
      lazyInitMethodName,
      Synthetic | Method | Private,
      MethodType(Nil)(_ => Nil, _ => defn.ObjectType)
    )

    val rhs = memberDef.rhs
    val rhsMappedOwner =
      rhs.changeOwnerAfter(memberDef.symbol, lazyInitMethodSymbol, this)
    val valueSymbol = newSymbol(
    ...
    val immediateValueCondition =
      if defn.LazyValsControlState.isSubClass(tp.classSymbol) then
        ref(valueSymbol)
          .select(defn.Any_!=)
          .appliedTo(nullLiteral)
          .select(nme.And)
          .appliedTo(
            ref(valueSymbol)
              .select(defn.Any_isInstanceOf)
              .appliedToType(defn.LazyValsControlState.typeRef)
              .select(nme.UNARY_!)
              .appliedToNone
          )
      else ref(valueSymbol).select(defn.Any_isInstanceOf).appliedToType(tp)
      ...
      Block(
        ...
        If( // if _x != null && !_x.isInstanceOf[LazyValControlState] then
          ...
          If(
            ...
            ref(lazyInitMethodSymbol).ensureApplied.ensureConforms(
              tp
            ) // else return x_compute()
          )
        )
      )
    ...
    val initialize =
      // var result: AnyRef
      val resSymbNullable = newSymbol(
        lazyInitMethodSymbol,
        lazyNme.resultNullable,
        Synthetic | Mutable,
        defn.ObjectType
      )
      val resSymb = newSymbol(
        lazyInitMethodSymbol,
        lazyNme.result,
        Synthetic | Mutable,
        defn.ObjectType
      ...
      val lockRel =
        ...
        Block(
          ValDef(lockSymb, ref(target).cast(waiting.typeOpt))
            :: objCasFlag
              .appliedTo(thiz, offset, ref(lockSymb), ref(resSymb)) :: Nil,
          ref(lockSymb).select(lazyNme.RLazyVals.waitingRelease).ensureApplied
        )
      // finally block
      val fin = If(
        objCasFlag
          .appliedTo(thiz, offset, evaluating, ref(resSymb))
          .select(nme.UNARY_!)
          .appliedToNone,
        lockRel,
        unitLiteral
      ...
      val evaluate = Try(
        Block(
          (Assign(
            ref(resSymbNullable),
            if needsBoxing(tp) && rhsMappedOwner != EmptyTree then
              rhsMappedOwner.ensureConforms(defn.boxedType(tp))
            else rhsMappedOwner
          ) // try result = rhs
            :: If(
              ref(resSymbNullable)
      ...
      If(
        ...
        Block(
          ValDef(resSymb, nullLiteral) :: ValDef(
            resSymbNullable,
            nullLiteral
          ) :: evaluate :: Nil, // var result: AnyRef = null
    ...
    val ifNotUninitialized =
      If(
        ref(current)
          .select(defn.Any_isInstanceOf)
          .appliedToTypeTree(controlState),
        // if a control state
        If(
          ref(current).select(defn.Object_eq).appliedTo(evaluating),
          // if is Evaluating then CAS(_, Evaluating, new Waiting)
          Block(
            objCasFlag.appliedTo(
              thiz,
              offset,
              ref(current),
              Select(New(waiting), StdNames.nme.CONSTRUCTOR).ensureApplied
            ) :: Nil,
            unitLiteral
          ),
          // if not Evaluating
          If(
            ref(current)
              .select(defn.Any_isInstanceOf)
              .appliedToTypeTree(waiting),
            // if is waiting
            ref(current)
              .select(defn.Any_asInstanceOf)
              .appliedToTypeTree(waiting)
              .select(
                lazyNme.RLazyVals.waitingAwaitRelease,
                _.info.paramInfoss.exists(_.size == 0)
              )
              .ensureApplied,
    ...
    val initBody = Block(
      ValDef(current, ref(target)) :: Nil,
      If(
        ref(current).select(defn.Object_eq).appliedTo(nullLiteral),
        initialize,
        ifNotUninitialized
      ).withType(defn.UnitType)
    )
    val initMainLoop =
      WhileDo(EmptyTree, initBody) // becomes: while (true) do { body }
  ...
  def transformMemberDefThreadSafeNew(x: ValOrDefDef)(using Context): Thicket =
    ...
    def offsetName(id: Int) = s"${StdNames.nme.LAZY_FIELD_OFFSET}${
        if x.symbol.owner.is(Module) then "_m_" else ""
      }$id".toTermName
    val containerName = LazyLocalName.fresh(x.name.asTermName)
    val containerSymbol = newSymbol(
      claz,
    ...
    containerSymbol.addAnnotation(
      Annotation(defn.VolatileAnnot, containerSymbol.span)
    ) // private @volatile var _x: AnyRef
    containerSymbol.addAnnotations(
      x.symbol.annotations
    ) // pass annotations from original definition
    ...
    val offsetSymbol: TermSymbol = appendOffsetDefs.get(claz) match
      case Some(info) =>
        newSymbol(claz, offsetName(info.defs.size), Synthetic, defn.LongType)
          .enteredAfter(this)
      case None =>
        newSymbol(claz, offsetName(0), Synthetic, defn.LongType).enteredAfter(
          this
        )
    offsetSymbol.nn.addAnnotation(
      Annotation(defn.ScalaStaticAnnot, offsetSymbol.nn.span)
    )
    val fieldTree = thizClass
      .select(lazyNme.RLazyVals.getDeclaredField)
      .appliedTo(Literal(Constant(containerName.mangledString)))
    ...
    val (accessorDef, initMethodDef) =
      mkThreadSafeDef(x, claz, containerSymbol, offset, swapOver)
    Thicket(containerTree, accessorDef, initMethodDef)
  end transformMemberDefThreadSafeNew
  ...
  /** Create a threadsafe lazy accessor equivalent to such code
    ...
    * ```
    */
  def mkThreadSafeDefLegacy(
      methodSymbol: TermSymbol,
      claz: ClassSymbol,
      ord: Int,
      target: Symbol,
      rhs: Tree,
    ...
    val flagSymbol =
      newSymbol(methodSymbol, lazyNme.flag, Synthetic, defn.LongType)
    val flagDef = ValDef(flagSymbol, getFlag.appliedTo(thiz, offset))
    val flagRef = ref(flagSymbol)

    val stateSymbol =
      newSymbol(methodSymbol, lazyNme.state, Synthetic, defn.LongType)
    val stateDef = ValDef(
      stateSymbol,
      stateMask.appliedTo(ref(flagSymbol), Literal(Constant(ord)))
    )
    ...
    val retryCase =
      val caseSymbol = newSymbol(
        methodSymbol,
        nme.DEFAULT_EXCEPTION_NAME,
        SyntheticCase,
        defn.ThrowableType
      )
      val triggerRetry =
        setFlagState.appliedTo(thiz, offset, initState, fieldId)
      CaseDef(
  ...
  end mkThreadSafeDefLegacy

  def transformMemberDefThreadSafeLegacy(x: ValOrDefDef)(using
      Context
  ): Thicket =
    val tpe = x.tpe.widen.resultType.widen
    val claz = x.symbol.owner.asClass
    val thizClass = Literal(Constant(claz.info))
    val getOffset =
      Select(ref(defn.LazyValsModule), lazyNme.RLazyVals.getOffset)
    val getOffsetStatic =
      Select(ref(defn.LazyValsModule), lazyNme.RLazyVals.getOffsetStatic)
    ...
    appendOffsetDefs.get(claz) match
      case Some(info) =>
        ...
        if ord != 0 then // there are unused bits in already existing flag
          offsetSymbol = claz.info
            .decl(offsetById)
          ...
          val flagSymbol =
            newSymbol(claz, flagName, containerFlags, defn.LongType)
              .enteredAfter(this)
          flag = ValDef(flagSymbol, Literal(Constant(0L)))
          val fieldTree = thizClass
            .select(lazyNme.RLazyVals.getDeclaredField)
            .appliedTo(Literal(Constant(flagName.toString)))
          val offsetTree =
      ...
      case None =>
        ...
        val flagSymbol =
          newSymbol(claz, flagName, containerFlags, defn.LongType).enteredAfter(
            this
          )
        flag = ValDef(flagSymbol, Literal(Constant(0L)))
        val fieldTree = thizClass
    ...
    val containerSymbol = newSymbol(
      claz,
      containerName,
      x.symbol.flags &~ containerFlagsMask | containerFlags,
      tpe,
      coord = x.symbol.coord
    ).enteredAfter(this)
    val containerTree = ValDef(containerSymbol, defaultValue(tpe))
    ...
    val wait =
      Select(ref(defn.LazyValsModule), lazyNme.RLazyVals.wait4Notification)
    ...
    val accessor = mkThreadSafeDefLegacy(
      x.symbol.asTerm,
      claz,
      ord,
      containerSymbol,
    ...
    if flag eq EmptyTree then Thicket(containerTree, accessor)
    else Thicket(containerTree, flag, accessor)
  end transformMemberDefThreadSafeLegacy
end LazyVals
...
object LazyVals:
  ...
  object lazyNme:
    ...
    object RLazyVals:
      import scala.runtime.LazyVals.{Names as N}
