...
/** This phase adds outer accessors to classes and traits that need them.
  * Compared to Scala 2.x, it tries to minimize the set of classes that take
  * outer accessors by scanning class implementations for outer references.
  *
  * The following things are delayed until erasure and are performed by class
  * OuterOps:
  *
  *   - add outer parameters to constructors
  *   - pass outer arguments in constructor calls
  *
  * replacement of outer this by outer paths is done in Erasure. needs to run
  * after pattern matcher as it can add outer checks and force creation of
  * $outer
  */
class ExplicitOuter extends MiniPhase with InfoTransformer:
  thisPhase =>
  ...
  /** First, add outer accessors if a class does not have them yet and it
    * references an outer this. If the class has outer accessors, implement
    * them. Furthermore, if a parent trait might have an outer accessor, provide
    * an implementation for the outer accessor by computing the parent's outer
    * from the parent type prefix. If the trait ends up not having an outer
    * accessor after all, the implementation is redundant, but does not harm.
    * The same logic is not done for non-trait parent classes because for them
    * the outer pointer is passed in the super constructor, which will be
    * implemented later in a separate phase which needs to run after erasure.
    * However, we make sure here that the super class constructor is indeed a
    * New, and not just a type.
    */
  override def transformTemplate(impl: Template)(using Context): Tree =
    val cls = ctx.owner.asClass
    val isTrait = cls.is(Trait)
    if needsOuterIfReferenced(cls) && !needsOuterAlways(cls) && referencesOuter(
        cls,
        impl
      )
    then ensureOuterAccessors(cls)

    val clsHasOuter = hasOuter(cls)
    if clsHasOuter || cls.mixins.exists(needsOuterIfReferenced) then
      ...
      for parentTrait <- cls.mixins do
        if needsOuterIfReferenced(parentTrait) then
          ...
          val outerAccImpl =
            newOuterAccessor(cls, parentTrait).enteredAfter(thisPhase)
          newDefs += DefDef(
            outerAccImpl,
            singleton(fixThis(outerPrefix(parentTp)))
          )
        ...
        for (parent <- impl.parents) yield
          ...
          parent match
            ...
            case parent: TypeTree
                if !cls.is(Trait) && !parentCls
                  .is(Trait) && !defn.NotRuntimeClasses.contains(parentCls) =>
  ...
  override def transformClosure(tree: Closure)(using Context): tpd.Tree =
    if tree.tpt ne EmptyTree then
      val cls = tree.tpt.asInstanceOf[TypeTree].tpe.classSymbol
      if cls.exists && hasOuter(cls.asClass) then
        report.error(
          "Not a single abstract method type, requires an outer pointer",
          tree.srcPos
        )
    tree
end ExplicitOuter
...
object ExplicitOuter:
  ...
  def ensureOuterAccessors(cls: ClassSymbol)(using Context): Unit =
    atPhase(explicitOuterPhase.next) {
      if !hasOuter(cls) then
        newOuterAccessors(cls).foreach(
          _.enteredAfter(explicitOuterPhase.asInstanceOf[DenotTransformer])
        )
    }

  /** The outer accessor and potentially outer param accessor needed for class
    * `cls`
    */
  private def newOuterAccessors(cls: ClassSymbol)(using Context) =
    newOuterAccessor(cls, cls) :: (if cls.is(Trait) then Nil
                                   else newOuterParamAccessor(cls) :: Nil)

  /** Scala 2.x and Dotty don't always agree on what should be the type of the
    * outer parameter, so we replicate the old behavior when passing arguments
    * to methods coming from Scala 2.x.
    */
  ...
  /** A new outer accessor or param accessor.
    * @param owner
    *   The class where the outer accessor is located
    * @param cls
    *   The class relative to which the outer is computed (can be a base class
    *   of owner)
    * @param name
    *   The name of the outer access
    * @param flags
    *   The flags of the outer accessor
    *
    * The type of the outer accessor is computed as follows: Let O[X1, .., Xn]
    * be the class enclosing `cls`.
    *   - if owner == cls, O[X1, ..., Xn]
    *   - otherwise, if the class P enclosing `owner` derives from O, the base
    *     type of P.this wrt class O
    *   - otherwise O[?, ..., ?]
    */
  private def newOuterSym(
      owner: ClassSymbol,
      cls: ClassSymbol,
      name: TermName,
      flags: FlagSet
        ...
        outerThis
          ...
          .orElse(
            if prefix == NoPrefix then
              outerCls.typeRef
                .appliedTo(outerCls.typeParams.map(_ => TypeBounds.empty))
            else prefix.widen
          )
    val info = if flags.is(Method) then ExprType(target) else target
  ...
  private def newOuterAccessor(owner: ClassSymbol, cls: ClassSymbol)(using
      Context
  ) =
    val deferredIfTrait = if owner.is(Trait) then Deferred else EmptyFlags
    val outerAccIfOwn = if owner == cls then OuterAccessor else EmptyFlags
    newOuterSym(
      owner,
      cls,
      outerAccName(cls),
  ...
  /** Class needs an outer pointer, provided there is a reference to an outer
    * this in it.
    */
  ...
  /** Class unconditionally needs an outer pointer. This is the case if the
    * class needs an outer pointer if referenced and one of the following holds:
    *   - we might not know at all instantiation sites whether outer is
    *     referenced or not
  ...
  private def needsOuterAlways(cls: ClassSymbol)(using Context): Boolean =
    needsOuterIfReferenced(cls) &&
      (!hasLocalInstantiation(
        cls
      ) || // needs outer because we might not know whether outer is referenced or not
        cls.mixins.exists(
          needsOuterIfReferenced
        ) || // needs outer for parent traits
        cls.info.parents.exists(
          parent => // needs outer to potentially pass along to parent
  ...
  /** The outer accessor of class `cls`. To find it is a bit tricky. The class
    * might have been moved with new owners between ExplicitOuter and Erasure,
    * where the method is also called. For instance, it might have been part of
    * a by-name argument, and therefore be moved under a closure method by
    * ElimByName. In that case looking up the method again at Erasure with the
    * fully qualified name `outerAccName` will fail, because the
    * `outerAccName`'s result is phase dependent. In that case we use a backup
    * strategy where we search all definitions in the class to find the one with
    * the OuterAccessor flag.
    */
  def outerAccessor(cls: ClassSymbol)(using Context): Symbol =
    if cls.isStatic then NoSymbol // fast return to avoid scanning package decls
    else
      cls.info
        .member(outerAccName(cls))
        .suchThat(_.is(OuterAccessor))
        .symbol orElse
  ...
  /** Class constructor needs an outer argument. Can be called only after phase
    * ExplicitOuter.
    */
  def needsOuterParam(cls: ClassSymbol)(using Context): Boolean =
    !cls.is(Trait) && needsOuterIfReferenced(cls) && (cls.is(
      JavaDefined
    ) || // java inner class doesn't has outer accessor
  ...
  def referencesOuter(cls: Symbol, tree: Tree)(using Context): Boolean =
    ...
    val test = new TreeAccumulator[Boolean]:
          ...
          if ref.prefix ne NoPrefix then
            ...
            (
              ...
              ctx.owner.enclosingClass.owner.enclosingClass
                .isContainedIn(ref.symbol.owner)
          )
        case _ => false
      ...
      def containsOuterRefsAtTopLevel(tp: Type): Boolean = tp match
        ...
        case tp: AndOrType =>
          containsOuterRefsAtTopLevel(tp.tp1) || containsOuterRefsAtTopLevel(
            tp.tp2
          )
        case _ => false
    ...
    case tpe: TypeRef =>
      tpe.symbol match
        case cls: ClassSymbol =>
          if tpe.prefix eq NoPrefix then cls.owner.enclosingClass.thisType
          else tpe.prefix
        case _ =>
  ...
  /** It's possible (i1755.scala gives an example) that the type given by
    * outerPrefix contains a This-reference to a module outside the context
    * where that module is defined. This needs to be translated to an access to
    * the module object from the enclosing class or object.
    *
    * This solution is a bit of a hack; it would be better to avoid such
    * references to the This of a module from outside the module in the first
    * place. I was not yet able to find out how such references arise and how to
    * avoid them.
    */
  private def fixThis(tpe: Type)(using Context): Type = tpe match
    case tpe: ThisType
        if tpe.cls.is(Module) && !ctx.owner.isContainedIn(tpe.cls) =>
  ...
  extension (sym: Symbol)
    def isOuterParamAccessor(using Context): Boolean =
  ...
  /** The operations in this class
    ...
    *   - replace outer this references by outer paths. They are called from
    *     erasure. There are two constraints which suggest these operations
    *     should be done in erasure.
    *   - Replacing this references with outer paths loses aliasing information,
    *     so programs will not typecheck with unerased types unless a lot of
    *     type refinements are added. Therefore, outer paths should be computed
    *     no earlier than erasure.
    *   - outer parameters should not show up in signatures, so again they
    *     cannot be added before erasure.
    *   - outer arguments need access to outer parameters as well as to the
    *     original type prefixes of types in New expressions. These prefixes get
    *     erased during erasure. Therefore, outer arguments have to be passed no
    *     later than erasure.
    */
  class OuterOps(val ictx: Context) extends AnyVal:
    ...
    def addParam(cls: ClassSymbol, tp: Type): Type =
      if needsOuterParam(cls) then
        val mt @ MethodTpe(pnames, ptypes, restpe) = tp: @unchecked
        mt.derivedLambdaType(
          nme.OUTER :: pnames,
          outerClass(cls).typeRef :: ptypes,
          restpe
        )
      else tp
    ...
    def args(fun: Tree): List[Tree] =
      if fun.symbol.isConstructor then
        ...
        def outerArg(receiver: Tree): Tree = receiver match
          ...
          case This(_) =>
            ref(
              outerParamAccessor(cls)
            ) // will be rewired to outer argument of secondary constructor in phase Constructors
          case TypeApply(Select(r, nme.asInstanceOf_), args) =>
            outerArg(r) // cast was inserted, skip
        if needsOuterParam(cls) then
          methPart(fun) match
            case Select(receiver, _) =>
              outerArg(receiver).withSpan(fun.span) :: Nil
        else Nil
      else Nil
    ...
    def argsForNew(cls: ClassSymbol, tpe: Type): List[Tree] =
      if needsOuterParam(cls) then singleton(fixThis(outerPrefix(tpe))) :: Nil
      else Nil

    /** A path of outer accessors starting from node `start`. `start` defaults
      * to the context owner's this node. There are two alternative conditions
      * that determine where the path ends:
      *
      *   - if the initial `count` parameter is non-negative: where the number
      *     of outer accessors reaches count.
      *   - if the initial `count` parameter is negative: where the class symbol
      *     of the type of the reached tree matches `toCls`.
      */
    def path(
      ...
      try
        @tailrec def loop(tree: Tree, count: Int): Tree =
          ...
          report.log(
            i"outer to $toCls of $tree: ${tree.tpe}, looking for ${atPhaseNoLater(
                lambdaLiftPhase
              )(outerAccName(treeCls.asClass))} in $treeCls"
          )
          if count == 0 || count < 0 && treeCls == toCls then tree
          else
            ...
            val outerAcc = atPhaseNoLater(lambdaLiftPhase) {
              ...
              tree match
                case tree: This
                    if tree.symbol == enclClass && !enclClass.is(Trait) =>
        ...
        report.log(
          i"computing outerpath to $toCls from ${ctx.outersIterator.map(_.owner).toList}"
        )
        loop(start, count)
      catch
        case ex: ClassCastException =>
          throw new ClassCastException(
            i"no path exists from ${ctx.owner.enclosingClass} to $toCls"
          )
  end OuterOps
end ExplicitOuter
