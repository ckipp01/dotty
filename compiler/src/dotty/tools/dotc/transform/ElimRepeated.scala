...
/** A transformer that eliminates repeated parameters (T*) from all types,
  * replacing them with Seq or Array types and adapting repeated arguments to
  * conform to the transformed type if needed.
  */
class ElimRepeated extends MiniPhase with InfoTransformer:
  thisPhase =>
  import ast.tpd.*
  ...
  override def changesMembers: Boolean =
    true // the phase adds vararg forwarders

  def transformInfo(tp: Type, sym: Symbol)(using Context): Type =
    elimRepeated(tp, sym.is(JavaDefined))

  /** Create forwarder symbols for the methods that are annotated with
    * `@varargs` or that override java varargs.
    *
    ...
    def transformVarArgs(sym: Symbol, isJavaVarargsOverride: Boolean): Unit =
      ...
      if hasRepeatedParam then
        ...
        if isJavaVarargsOverride || hasAnnotation || parentHasAnnotation then
          ...
          if !validJava then
            report.error(
              em"""To generate java-compatible varargs:
            ...
            addVarArgsForwarder(
              sym,
              isJavaVarargsOverride,
              hasAnnotation,
              parentHasAnnotation
            )
      else if hasAnnotation then
        report.error(
          "A method without repeated parameters cannot be annotated with @varargs",
          sym.sourcePos
        )
    end
  ...
  /** Does `sym` override a symbol defined in a Java class? One might think that
    ...
    *
    * but that does not work, since `allOverriddenSymbols` gets confused because
    * the signatures of a Java varargs method and a Scala varargs override are
    * not the same.
    */
  ...
  private def hasVarargsAnnotation(sym: Symbol)(using Context) =
    sym.hasAnnotation(defn.VarargsAnnot)

  private def parentHasVarargsAnnotation(sym: Symbol)(using Context) =
  ...
  private def isVarargsMethod(sym: Symbol)(using Context) =
    hasVarargsAnnotation(sym)
      ...
      && (overridesJava(sym) || sym.allOverriddenSymbols.exists(
        hasVarargsAnnotation
      ))

  /** Eliminate repeated parameters from method types. */
  private def elimRepeated(tp: Type, isJava: Boolean)(using Context): Type =
    tp.stripTypeVar match
      ...
      case tp: PolyType =>
        tp.derivedLambdaType(
          tp.paramNames,
          tp.paramInfos,
          elimRepeated(tp.resultType, isJava)
        )
      case tp =>
        tp
    ...
    val args = tree.args.mapConserve { arg =>
      if isWildcardStarArg(arg) then
        val expr = arg match
          case t: Typed => t.expr
          case _ =>
            arg // if the argument has been lifted it's not a Typed (often it's an Ident)

        val isJavaDefined = tree.fun.symbol.is(JavaDefined)
        if isJavaDefined then adaptToArray(expr)
        else if expr.tpe.derivesFrom(defn.ArrayClass) then arrayToSeq(expr)
        else expr
      else arg
    }
  ...
  private def adaptToArray(tree: Tree)(implicit ctx: Context): Tree = tree match
    ...
    case _ =>
      ...
      val adapted =
        if tree.tpe.derivesFrom(defn.ArrayClass) then tree
        else
  ...
  /** Generate the method definitions for the varargs forwarders created in
    * transform
    */
  override def transformDefDef(tree: DefDef)(using Context): Tree =
    // If transform reported an error, don't go further
    if ctx.reporter.hasErrors then return tree
    ...
    if isVarArgs then
      ...
      val forwarderSym = currentClass.info
        .decl(sym.name)
        .alternatives
  ...
  /** Is this the type of a method that has a repeated parameter type as its
    * last parameter in the last parameter list?
    */
  private def isValidJavaVarArgs(tp: Type)(using Context): Boolean = tp match
    ...
    case _ =>
      throw new Exception(
        "Match error in @varargs checks. This should not happen, please open an issue " + tp
      )

  /** Add the symbol of a Java varargs forwarder to the scope. It retains all
    * the flags of the original method.
    *
    * @param original
    *   the original method symbol
    * @param isBridge
    *   true if we are generating a "bridge" (synthetic override forwarder)
    * @param hasAnnotation
    *   true if the method is annotated with `@varargs`
    * @param parentHasAnnotation
    *   true if the method overrides a method that is annotated with `@varargs`
    *
    * A forwarder is necessary because the following holds:
    *   - the varargs in `original` will change from `RepeatedParam[T]` to
    *     `Seq[T]` after this phase
    *   - _but_ the callers of the method expect its varargs to be changed to
    *     `Array[? <: T]` The solution is to add a method that converts its
    *     argument from `Array[? <: T]` to `Seq[T]` and forwards it to the
    *     original method.
    */
  private def addVarArgsForwarder(
      original: Symbol,
      isBridge: Boolean,
      hasAnnotation: Boolean,
      parentHasAnnotation: Boolean
  )(using Context): Unit =
    val owner = original.owner
    if !owner.isClass then
      report.error(
        "inner methods cannot be annotated with @varargs",
        original.sourcePos
      )
      return
    ...
    conflicts match
      case conflict :: _ =>
        ...
        report.error(
          s"$src produces a forwarder method that conflicts with ${conflict.showDcl}",
          original.srcPos
        )
      case Nil =>
  ...
  /** Translate a repeated type T* to an `Array[? <: Upper]` such that it is
    * compatible with java varargs.
    *
    * When necessary we set `Upper = T & AnyRef` to prevent the erasure of
    * `Array[? <: Upper]` to Object, which would break the varargs from Java.
    */
  private def varargArrayType(tp: Type)(using Context): Type =
    ...
    then array
      ...
      defn.ArrayOf(
        TypeBounds.upper(AndType(element, defn.AnyRefType))
      ) // Array[? <: T & AnyRef]
end ElimRepeated
