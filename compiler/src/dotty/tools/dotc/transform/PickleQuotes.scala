...
/** Translates quoted terms and types to `unpickleExprV2` or `unpickleType`
  * method calls.
  *
  ...
  *  )
  * ```
  * and then performs the same transformation on any quote contained in the
  * `content`s.
  */
class PickleQuotes extends MacroTransform:
  ...
  private def extractHolesContents(quote: tpd.Quote)(using
      Context
  ): (List[Tree], tpd.Quote) =
    class HoleContentExtractor extends Transformer:
      ...
      override def transform(tree: tpd.Tree)(using Context): tpd.Tree =
        tree match
          case tree @ Hole(isTerm, _, _, content) =>
            ...
            val hole =
              untpd.cpy.Hole(tree)(content = EmptyTree).withType(holeType)
            cpy.Inlined(tree)(EmptyTree, Nil, hole)
          case tree: DefTree =>
            if tree.symbol.isClass then stagedClasses += tree.symbol
            val newAnnotations = tree.symbol.annotations.mapconserve { annot =>
              annot.derivedAnnotation(
      ...
      private def mapAnnots =
        new TypeMap: // TODO factor out duplicated logic in Splicing
  ...
  private def encodeTypeArgs(quote: tpd.Quote)(using Context): tpd.Quote =
    if quote.tags.isEmpty then quote
      ...
      val typeMapping =
        quote.tags.map(_.tpe).zip(tdefs.map(_.symbol.typeRef)).toMap
      val typeMap = new TypeMap:
        override def apply(tp: Type): Type = tp match
          case TypeRef(tag: TermRef, _)
              if tp.typeSymbol == defn.QuotedType_splice =>
  ...
  private def mkTagSymbolAndAssignType(typeArg: Tree, idx: Int)(using
      Context
  ): TypeDef =
    val holeType =
      getPicklableHoleType(typeArg.tpe.select(tpnme.Underlying), _ => false)
    val hole = untpd.cpy
      .Hole(typeArg)(isTerm = false, idx, Nil, EmptyTree)
      .withType(holeType)
    val local = newSymbol(
      owner = ctx.owner,
    ...
    local.addAnnotation(
      Annotation(defn.QuotedRuntime_SplicedTypeAnnot, typeArg.span)
    )
    ctx.typeAssigner
      .assignType(untpd.TypeDef(local.name, hole), local)
      .withSpan(typeArg.span)

  /** Avoid all non-static types except those defined in the quote. */
  private def getPicklableHoleType(
      tpe: Type,
      isStagedClasses: Symbol => Boolean
  )(using Context) =
    new TypeOps.AvoidMap:
      def toAvoid(tp: NamedType) =
        !isStagedClasses(tp.typeSymbol) && !isStaticPrefix(tp)
      .apply(tpe)
end PickleQuotes
...
object PickleQuotes:
  ...
  def pickle(quote: Quote, quotes: Tree, holeContents: List[Tree])(using
      Context
  ) =
    val body = quote.body
    ...
    /** Encode quote using Reflection.Literal
      ...
      * ```
      * this closure is always applied directly to the actual context and the
      * BetaReduce phase removes it.
      */
    def pickleAsLiteral(lit: Literal) =
      val typeName = body.tpe.typeSymbol.name
      val literalValue =
        if lit.const.tag == Constants.NullTag || lit.const.tag == Constants.UnitTag
        then Nil
        else List(body)
    ...
    def pickleAsValue(lit: Literal) =
      ...
      lit.const.tag match
        ...
        case Constants.BooleanTag =>
          liftedValue(lit, defn.ToExprModule_BooleanToExpr)
        case Constants.ByteTag => liftedValue(lit, defn.ToExprModule_ByteToExpr)
        case Constants.ShortTag =>
          liftedValue(lit, defn.ToExprModule_ShortToExpr)
        ...
        case Constants.FloatTag =>
          liftedValue(lit, defn.ToExprModule_FloatToExpr)
        case Constants.DoubleTag =>
          liftedValue(lit, defn.ToExprModule_DoubleToExpr)
        case Constants.CharTag => liftedValue(lit, defn.ToExprModule_CharToExpr)
        case Constants.StringTag =>
          liftedValue(lit, defn.ToExprModule_StringToExpr)
    ...
    def pickleAsTasty() =
      val body1 =
        if body.isType then body
          ...
          Inlined(
            Inlines.inlineCallTrace(ctx.owner, quote.sourcePos),
            Nil,
            body
          )
      ...
      val pickledQuoteStrings = pickleQuote match
        ...
        case xs =>
          tpd.mkList(
            xs.map(x => Literal(Constant(x))),
            TypeTree(defn.StringType)
          )

      // This and all closures in typeSplices are removed by the BetaReduce phase
      val types =
        if quote.tags.isEmpty then
          Literal(
            Constant(null)
          ) // keep pickled quote without holeContents as small as possible
        else
          SeqLiteral(
            quote.tags,
            TypeTree(
              defn.QuotedTypeClass.typeRef.appliedTo(TypeBounds.emptyPolyKind)
            )
          )

      // This and all closures in termSplices are removed by the BetaReduce phase
      val termHoles =
        if holeContents.isEmpty then
          Literal(
            Constant(null)
          ) // keep pickled quote without holeContents as small as possible
        else
          Lambda(
            MethodType(
              List(nme.idx, nme.contents, nme.quotes).map(name =>
                UniqueName.fresh(name).toTermName
              ),
              List(
                defn.IntType,
                defn.SeqType.appliedTo(defn.AnyType),
                defn.QuotesClass.typeRef
              ),
            ...
            args =>
              val cases = holeContents.zipWithIndex.map { case (splice, idx) =>
                val defn.FunctionOf(
                  argTypes,
                  defn.FunctionOf(quotesType :: _, _, _),
                  _
                ) = splice.tpe: @unchecked
                val rhs =
                  ...
                  BetaReduce(
                    ...
                    .select(nme.apply)
                    .appliedTo(args(2).asInstance(quotesType))
              ...
              cases match
                ...
                case _ =>
                  Match(
                    args(0).annotated(New(ref(defn.UncheckedAnnot.typeRef))),
                    cases
                  )
          )

      val quoteClass =
        if quote.isTypeQuote then defn.QuotedTypeClass else defn.QuotedExprClass
    ...
    if body.isType then
      if holeContents.isEmpty && body.symbol.isPrimitiveValueClass then
        taggedType()
      else pickleAsTasty()
    else
