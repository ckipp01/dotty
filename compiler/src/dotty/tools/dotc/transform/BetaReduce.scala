...
/** Rewrite an application
  ...
  * [xi := yi]b
  *
  * This is more limited than beta reduction in inlining since it only works for
  * simple variables `yi`. It is more general since it also works for
  * type-ascripted closures.
  *
  * A typical use case is eliminating redundant closures for blackbox macros
  * that return context functions. See i6375.scala.
  */
...
object BetaReduce:
  ...
  /** Rewrite an application
    ...
    *
    * where `def` is used for call-by-name parameters. However, we shortcut any
    * NoPrefix refs among the ei's directly without creating an intermediate
    * binding.
    *
  ...
  def apply(tree: Tree)(using Context): Tree =
    ...
    def recur(fn: Tree, argss: List[List[Tree]]): Option[Tree] = fn match
      ...
      case Block(
            ...
            Typed(Apply(Select(New(_), _), _), _)
    ...
    tree match
      case Apply(Select(fn, nme.apply), args) if defn.isFunctionType(fn.tpe) =>
        recur(fn, List(args)) match
          ...
          case None =>
            tree
      case Apply(TypeApply(Select(fn, nme.apply), targs), args)
          if fn.tpe.typeSymbol eq dotc.core.Symbols.defn.PolyFunctionClass =>
  ...
  /** Beta-reduces a call to `ddef` with arguments `args` and registers new
    * bindings
    */
  def reduceApplication(
      ddef: DefDef,
      argss: List[List[Tree]],
      bindings: ListBuffer[DefTree]
  )(using Context): Tree =
    ...
    val targSyms =
      for (targ, tparam) <- targs.zip(tparams) yield targ.tpe.dealias match
    ...
    val argSyms =
      for (arg, param) <- args.zip(vparams) yield arg.tpe.dealias match
        ...
        case _ =>
          ...
          val tpe =
            ...
            else if arg.tpe.dealias.isInstanceOf[ConstantType] then
              arg.tpe.dealias
            else arg.tpe.widen
          val binding = ValDef(
            newSymbol(ctx.owner, param.name, flags, tpe, coord = arg.span),
            arg
          ).withSpan(arg.span)
    ...
    val expansion1 = new TreeMap:
      override def transform(tree: Tree)(using Context) =
        tree.tpe.widenTermRefExpr match
          case ConstantType(const) if isPureExpr(tree) =>
