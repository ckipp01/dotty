      ...
      case PackageDef(pid, _) if tree.symbol.owner == defn.RootClass =>
        val checker = new TreeTraverser:
          ...
          def traverse(tree: tpd.Tree)(using Context): Unit =
            ...
            if !tree.isEmpty && !tree.isInstanceOf[untpd.TypedSplice] && !tree
                .isInstanceOf[Inlined] && ctx.typerState.isGlobalCommittable
            then
              if !tree.isType // TODO also check types, currently we do not add Inlined(EmptyTree, _, _) for types. We should.
                && !tree.symbol.is(
                  InlineProxy
                ) // TODO check inline proxies (see tests/tun/lst)
              then
                val currentSource = sources.head
                assert(
                  tree.source == currentSource,
                  i"wrong source set for $tree # ${tree.uniqueId} of ${tree.getClass}, set to ${tree.source} but context had $currentSource\n ${tree.symbol.flagsString}"
                )

            // Recursivlely check children while keeping track of current source
            reporting.trace(
              i"check pos ${tree.getClass} ${tree.source} ${sources.head} $tree"
            ) {
              tree match
                case Inlined(EmptyTree, bindings, expansion) =>
                  ...
                  val old = sources
                  sources = old.tail
                  traverse(expansion)(using
                    inlineContext(EmptyTree).withSource(sources.head)
                  )
                  sources = old
                case Inlined(call, bindings, expansion) =>
                  ...
                  if !isMacro(call)
                  then // FIXME macro implementations can drop Inlined nodes. We should reinsert them after macro expansion based on the positions of the trees
                    traverse(expansion)(using
                      inlineContext(call).withSource(sources.head)
                    )
