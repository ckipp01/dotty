...
/** Make private term members that are accessed from another class non-private
  ...
  *
  * Also, make non-private any private parameter forwarders that forward to an
  * inherited public or protected parameter accessor with the same name as the
  * forwarder. This is necessary since private methods are not allowed to have
  * the same name as inherited public ones.
  *
  * See discussion in https://github.com/lampepfl/dotty/pull/784 and
  * https://github.com/lampepfl/dotty/issues/783
  */
class ExpandPrivate extends MiniPhase with IdentityDenotTransformer:
  thisPhase =>
  import ast.tpd.*
  ...
  override def changesMembers: Boolean =
    true // the phase introduces new members with mangled names
    ...
    tree match
      case t: DefDef =>
        ...
        def hasWeakerAccess(other: Symbol) =
          ...
          if sym.is(Private) then other.is(Private)
          else if sym.is(Protected) then other.isOneOf(Protected | Private)
          else true // sym is public
        val fail = sym.allOverriddenSymbols.findSymbol(x => !hasWeakerAccess(x))
        if fail.exists then
          assert(
            false,
            i"${sym.showFullName}: ${sym.info} has weaker access than superclass method ${fail.showFullName}: ${fail.info}"
          )
      case _ =>
  ...
  /** Make private terms accessed from different classes non-private. Note: this
    * happens also for accesses between class and linked module class. If we
    * change the scheme at one point to make static module class computations
    * static members of the companion class, we should tighten the condition
    * below.
    */
  private def ensurePrivateAccessible(d: SymDenotation)(using Context) =
    if isVCPrivateParamAccessor(d) then
      d.ensureNotPrivate.installAfter(thisPhase)
    else if d.is(PrivateTerm) && !d.owner.is(
        Package
      ) && d.owner != ctx.owner.lexicallyEnclosingClass
    then
      ...
      def isSimilar(p1: String, p2: String): Boolean =
        var i = p1.length - 1
        var j = p2.length - 1
        while i >= 0 && j >= 0 && p1(i) == p2(j) && p1(i) != separatorChar do
          i -= 1
          j -= 1
  ...
  override def transformDefDef(tree: DefDef)(using Context): DefDef =
    ...
    tree.rhs match
      case Apply(sel @ Select(_: Super, _), _)
          if sym.isAllOf(PrivateParamAccessor) && sel.symbol.is(
            ParamAccessor
          ) && sym.name == sel.symbol.name =>
        sym.ensureNotPrivate.installAfter(thisPhase)
      case _ =>
        if isVCPrivateParamAccessor(sym) then
          sym.ensureNotPrivate.installAfter(thisPhase)
    tree
end ExpandPrivate
