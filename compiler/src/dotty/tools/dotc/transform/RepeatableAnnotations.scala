...
class RepeatableAnnotations extends MiniPhase:
  ...
  private def aggregateAnnotations(annotations: Seq[Annotation])(using
      Context
  ): List[Annotation] =
    ...
    annsByType.flatMap {
      ...
      case (sym, anns) if sym.is(JavaDefined) =>
        sym
          .getAnnotation(defn.JavaRepeatableAnnot)
          .flatMap(_.argumentConstant(0)) match
          case Some(Constant(containerTpe: Type)) =>
            val clashingAnns =
              annsByType.getOrElse(containerTpe.classSymbol, Nil)
            if clashingAnns.nonEmpty then
              // this is the same error javac would raise in this case
              val pos = clashingAnns.head.tree.srcPos
              report.error(
                "Container must not be present at the same time as the element it contains",
                pos
              )
              Nil
            else
              val aggregated =
                JavaSeqLiteral(anns.map(_.tree).toList, TypeTree(sym.typeRef))
              Annotation(
                containerTpe,
                NamedArg("value".toTermName, aggregated),
                sym.span
              ) :: Nil
          case _ =>
  ...
  end aggregateAnnotations

  private def stableGroupBy[A, K](
      ins: Seq[A],
      f: A => K
  ): scala.collection.MapView[K, List[A]] =
