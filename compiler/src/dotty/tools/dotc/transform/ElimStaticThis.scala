...
/** Replace This references to module classes in static methods by global
...
class ElimStaticThis extends MiniPhase:
  ...
  override def transformThis(tree: This)(using Context): Tree =
    if !tree.symbol.is(Package) && ctx.owner.enclosingMethod.is(JavaStatic) then
  ...
  override def transformIdent(tree: tpd.Ident)(using Context): tpd.Tree =
    if ctx.owner.enclosingMethod.is(JavaStatic) then
      tree.tpe match
        case TermRef(thiz: ThisType, _)
            if thiz.cls.is(ModuleClass, JavaDefined) =>
