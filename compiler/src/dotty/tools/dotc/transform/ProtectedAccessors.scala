...
/** Add accessors for all protected accesses. An accessor is needed if according
  * to the rules of the JVM a protected class member is not accessible from the
  * point of access, but is accessible if the access is from an enclosing class.
  * In this point a public access method is placed in that enclosing class.
  */
object ProtectedAccessors:
  val name: String = "protectedAccessors"
  val description: String = "add accessors for protected members"

  /** Is the current context's owner inside the access boundary established by
    * `sym`?
    */
  ...
  def needsAccessorIfNotInSubclass(sym: Symbol)(using Context): Boolean =
    sym.isTerm && sym.is(Protected) &&
      !sym.owner.is(
        Trait
      ) && // trait methods need to be handled specially, are currently always public
      !insideBoundaryOf(sym)

  /** Do we need a protected accessor for accessing sym from the current
    * context's owner?
    */
  def needsAccessor(sym: Symbol)(using Context): Boolean =
    needsAccessorIfNotInSubclass(sym) &&
      !ctx.owner.enclosingClass.derivesFrom(sym.owner)
end ProtectedAccessors
...
class ProtectedAccessors extends MiniPhase:
  ...
  private class Accessors extends AccessProxies:
    val insert: Insert = new Insert:
      def accessorNameOf(name: TermName, site: Symbol)(using
          Context
      ): TermName = ProtectedAccessorName(name)
      def needsAccessor(sym: Symbol)(using Context) =
      ...
      override def ifNoHost(reference: RefTree)(using Context): Tree =
        ...
        report.error(
          em"illegal access to protected ${reference.symbol.showLocated} from $curCls",
          reference.srcPos
        )
        reference
  ...
  override def transformTemplate(tree: Template)(using Context): Tree =
    cpy.Template(tree)(body =
      accessors.addAccessorDefs(tree.symbol.owner, tree.body)
    )
end ProtectedAccessors
