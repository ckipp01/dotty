...
class TypedFormatChecker(
    partsElems: List[Tree],
    parts: List[String],
    args: List[Tree]
)(using Context):
  ...
  def argType(argi: Int, types: Type*): Type =
    ...
    types
      .find(t => argConformsTo(argi, tpe, t))
      .orElse(types.find(t => argConvertsTo(argi, tpe, t)))
      .getOrElse {
        report.argError(
          s"Found: ${tpe.show}, Required: ${types.map(_.show).mkString(", ")}",
          argi
        )
        actuals += args(argi)
        types.head
  ...
  /** For N part strings and N-1 args to interpolate, normalize parts and check
    * arg types.
    *
    * Returns normalized part strings and args, where args correcpond to
    * conversions in tail of parts.
    */
  def checked: (List[String], List[Tree]) =
    ...
    def loop(remaining: List[String], n: Int): Unit =
      remaining match
        case part0 :: more =>
          def badPart(t: Throwable): String = "".tap(_ =>
            report.partError(t.getMessage.nn, index = n, offset = 0)
          )
          val part =
            try StringContext.processEscapes(part0)
            catch badPart
          ...
          def insertStringConversion(): Unit =
            amended += "%s" + part
            convert += Conversion(
              formatPattern.findAllMatchIn("%s").next(),
              n
            ) // improve
    ...
    if reported then (Nil, Nil)
      ...
      assert(
        argc == actuals.size,
        s"Expected ${argc} args but got ${actuals.size} for [${parts.mkString(", ")}]"
      )
  ...
  enum Kind:
    case StringXn, HashXn, BooleanXn, CharacterXn, IntegralXn, FloatingPointXn,
      DateTimeXn, LiteralXn, ErrorXn
  import Kind.*

  /** A conversion specifier matched in the argi'th string part, with `argc`
  ...
  final class Conversion(val descriptor: Match, val argi: Int, val kind: Kind):
    ...
    val precision: Option[Int] =
      descriptor.group(Precision).map(_.drop(1).toInt)
    ...
    def verify: Boolean =
      ...
      def noPrecision =
        precision.isEmpty or errorAt(Precision)("precision not allowed")
      def only_-(msg: String) =
        val badFlags = flags.filterNot {
          case '-' | '<' => true
          case _         => false
        }
      ...
      def goodFlags =
        ...
        badFlags.isEmpty
      def goodIndex =
        if index.nonEmpty && hasFlag('<') then
          warningAt(Index)("Argument index ignored if '<' flag is present")
      ...
      kind match
        ...
        case IntegralXn =>
          def d_# = cc == 'd' && hasFlag('#') and badFlag(
            '#',
            "# not allowed for d conversion"
          )
          def x_comma = cc != 'd' && hasFlag(',') and badFlag(
            ',',
        ...
        case FloatingPointXn =>
          goodies && (cc match
            case 'a' | 'A' =>
              ...
              noPrecision && badFlags.isEmpty or badFlags.foreach(badf =>
                badFlag(badf, s"'$badf' not allowed for a, A")
              )
            case _ => true
          )
        case DateTimeXn =>
          def hasCC = op.length == 2 or errorAt(CC)(
            "Date/time conversion must have two characters"
          )
          def goodCC =
            "HIklMSLNpzZsQBbhAaCYyjmdeRTrDFc".contains(cc) or errorAt(CC, 1)(
              s"'$cc' doesn't seem to be a date or time conversion"
            )
          goodies && hasCC && goodCC && noPrecision && only_-(
            "date/time conversions"
          )
        case LiteralXn =>
          op match
            case "%" =>
              goodies && noPrecision and width.foreach(_ =>
                warningAt(Width)("width ignored on literal")
              )
    ...
    def accepts(arg: Type): Boolean =
      kind match
        case BooleanXn =>
          arg == defn.BooleanType orElse warningAt(CC)(
            ...
            case 'o' | 'x' | 'X' if hasAnyFlag("+ (") =>
              "+ ("
                ...
                .foreach(bad =>
                  badFlag(
                    bad,
                    s"only use '$bad' for BigInt conversions to o, x, X"
                  )
                ); true
          }
        case _ => true
    ...
    def acceptableVariants: List[Type] =
      kind match
        case StringXn =>
          if hasFlag('#') then FormattableType :: Nil else defn.AnyType :: Nil
        case BooleanXn => defn.BooleanType :: defn.NullType :: Nil
        case HashXn    => defn.AnyType :: Nil
        case CharacterXn =>
          defn.CharType :: defn.ByteType :: defn.ShortType :: defn.IntType :: Nil
        case IntegralXn =>
          defn.IntType :: defn.LongType :: defn.ByteType :: defn.ShortType :: BigIntType :: Nil
        case FloatingPointXn =>
          defn.DoubleType :: defn.FloatType :: BigDecimalType :: Nil
    ...
    def badFlag(f: Char, msg: String) =
      val i = flags.indexOf(f) match
        case -1 => 0
        case j  => j
      errorAt(Flags, i)(msg)

    def errorAt(g: SpecGroup, i: Int = 0)(msg: String) =
      report.partError(msg, argi, descriptor.offset(g, i), descriptor.end(g))
    def warningAt(g: SpecGroup, i: Int = 0)(msg: String) =
      report.partWarning(msg, argi, descriptor.offset(g, i), descriptor.end(g))
  end Conversion
  ...
  object Conversion:
    def apply(m: Match, i: Int): Conversion =
      def kindOf(cc: Char) = cc match
        ...
        case 'd' | 'o' | 'x' | 'X'                   => IntegralXn
        case 'e' | 'E' | 'f' | 'g' | 'G' | 'a' | 'A' => FloatingPointXn
        case 't' | 'T'                               => DateTimeXn
        case '%' | 'n'                               => LiteralXn
        case _                                       => ErrorXn
      end kindOf
      m.group(CC) match
        ...
        case None =>
          new Conversion(m, i, ErrorXn).tap(
            _.errorAt(Spec)(
              s"Missing conversion operator in '${m.matched}'; $literalHelp"
            )
          )
    end apply
  ...
  extension (r: report.type)
    def argError(message: String, index: Int): Unit =
      r.error(message, args(index).srcPos).tap(_ => reported = true)
    def partError(
        message: String,
        index: Int,
        offset: Int,
      ...
      r.error(message, partPosAt(index, offset, end)).tap(_ => reported = true)
    def partWarning(
        message: String,
        index: Int,
        offset: Int,
      ...
      .warning(message, partPosAt(index, offset, end))
      .tap(_ => reported = true)
