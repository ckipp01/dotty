...
/** For Scala enums that inherit from java.lang.Enum: Add constructor parameters
  * for `name` and `ordinal` to pass from each case to the java.lang.Enum class.
  */
class CompleteJavaEnums extends MiniPhase with InfoTransformer:
  thisPhase =>
  ...
  def transformInfo(tp: Type, sym: Symbol)(using Context): Type =
    if sym.isConstructor && (sym == defn.JavaEnumClass.primaryConstructor ||
        sym.owner.derivesFromJavaEnum)
    then addConstrParams(sym.info)
    else tp

  /** Add constructor parameters `$name: String` and `$ordinal: Int` to the end
    * of the last parameter list of (method- or poly-) type `tp`.
    */
  ...
  /** The list of parameter definitions `$name: String, $ordinal: Int`, in given
    * `owner` with given flags (either `Param` or `ParamAccessor`)
    */
  private def addedParams(owner: Symbol, isLocal: Boolean, flag: FlagSet)(using
      Context
  ): List[ValDef] =
    val flags =
      flag | Synthetic | (if isLocal then Private | Deferred else EmptyFlags)
    val nameParam = newSymbol(
      owner,
      nameParamName,
      flags,
      defn.StringType,
  ...
  /** Add arguments `args` to the parent constructor application in `parents`
    * that invokes a constructor of `targetCls`,
    */
  private def addEnumConstrArgs(
      targetCls: Symbol,
      parents: List[Tree],
      args: List[Tree]
    ...
    parents.map {
      case app @ Apply(fn, args0) if fn.symbol.owner == targetCls =>
        if args0.nonEmpty && targetCls == defn.JavaEnumClass then
          report.error(
            em"the constructor of java.lang.Enum cannot be called explicitly",
            app.sourcePos
          )
  ...
  /** If this is a constructor of a enum class that extends, add $name and
    * $ordinal parameters to it.
    */
  ...
  /** Return a list of forwarders for enum values defined in the companion
    * object for java interop.
    */
  private def addedEnumForwarders(clazz: Symbol)(using
      Context
  ): List[MemberDef] =
    ...
    yield
      def forwarderSym(
          flags: FlagSet,
          info: Type
      ): Symbol { type ThisName = TermName } =
        val sym = newSymbol(clazz, enumValue.name.asTermName, flags, info)
        sym.addAnnotation(
          Annotations.Annotation(defn.ScalaStaticAnnot, sym.span)
        )
        sym
      ...
      if ctx.settings.scalajs.value then
        ...
        DefDef(
          forwarderSym(
            EnumValue | Method | JavaStatic,
            MethodType(Nil, enumValue.info)
          ),
          body
        )
      else ValDef(forwarderSym(EnumValue | JavaStatic, enumValue.info), body)
  end addedEnumForwarders

  private def isJavaEnumValueImpl(cls: Symbol)(using Context): Boolean =
    cls.isAnonymousClass
      && (((cls.owner.name eq nme.DOLLAR_NEW) && cls.owner.isAllOf(
        Private | Synthetic
      )) || cls.owner.isAllOf(EnumCase))
  ...
  /**   1. If this is an enum class, add $name and $ordinal parameters to its
    *      parameter accessors and pass them on to the java.lang.Enum
    *      constructor.
    *
    * 2. If this is an anonymous class that implement a singleton enum case,
    * pass $name and $ordinal parameters to the enum superclass. The class looks
    * like this:
    *
    ...
    *
    * class $anon extends E(..., $name, _$ordinal) { // if class implements a
    * simple enum case "same as before" }
    *
    * class $anon extends E(..., "A", 0) { // if class implements a value enum
    * case `A` with ordinal 0 "same as before" }
    */
  override def transformTemplate(templ: Template)(using Context): Tree =
    ...
    if cls.derivesFromJavaEnum then
      registerEnumClass(
        cls
      ) // invariant: class is visited before cases: see tests/pos/enum-companion-first.scala
      ...
      val args =
        if cls.owner.isAllOf(EnumCase) then
          List(
            Literal(Constant(cls.owner.name.toString)),
            Literal(Constant(ordinalFor(cls.owner)))
          )
        else
          List(
            creatorParamRef(nme.nameDollar),
            creatorParamRef(nme.ordinalDollar_)
          )
      cpy.Template(templ)(
        parents =
          addEnumConstrArgs(cls.owner.owner.linkedClass, templ.parents, args)
      )
    else if cls.linkedClass.derivesFromJavaEnum then
      enumCaseOrdinals
        .clear() // remove simple cases // invariant: companion is visited after cases
      templ
    else templ
    end if
  ...
  override def checkPostCondition(tree: Tree)(using Context): Unit =
    assert(
      enumCaseOrdinals.isEmpty,
      "Java based enum ordinal cache was not cleared"
    )
end CompleteJavaEnums
