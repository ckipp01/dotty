...
/** The preceding lambda lift and flatten phases move symbols to different
  * scopes and rename them. This miniphase cleans up afterwards and makes sure
  * that all class scopes contain the symbols defined in them.
  */
class RestoreScopes extends MiniPhase with IdentityDenotTransformer:
  thisPhase =>
  import ast.tpd.*
  ...
  override def changesMembers: Boolean =
    true // the phase affects scopes, applying tree transformations of previous phases
   ...
   */
  override def transformPackageDef(pdef: PackageDef)(using
      Context
  ): PackageDef =
  ...
  private def restoreScope(tree: Tree)(using Context) = tree match
    case TypeDef(_, impl: Template) =>
      ...
      tree.symbol
        .copySymDenotation(
          info = cls.classInfo.derivedClassInfo(decls = restoredDecls: Scope)
        )
