...
/** Provides methods to produce fully parameterized versions of instance
  * methods, where the `this` of the enclosing class is abstracted out in an
  * extra leading `$this` parameter and type parameters of the class become
  * additional type parameters of the fully parameterized method.
  *
  ...
  *
  * Note that the methods lift out type parameters of the class containing the
  * instance method, but not type parameters of enclosing classes. The fully
  * instantiated method therefore needs to be put in a scope "close" to the
  * original method, i.e. they need to share the same outer pointer. Examples of
  * legal positions are: in the companion object, or as a local method inside
  * the original method.
  *
  ...
  *
  * which is not typable. Not clear yet what to do. Maybe allow PolyTypes to
  * follow method parameters and translate to the following:
  *
...
trait FullParameterization:
  ...
  /** If references to original symbol `referenced` from within fully
    * parameterized method `derived` should be rewired to some fully
    * parameterized method, the rewiring target symbol, otherwise NoSymbol.
    */
  protected def rewiredTarget(referenced: Symbol, derived: Symbol)(using
      Context
  ): Symbol

  /** If references to some original symbol from given tree node within fully
    * parameterized method `derived` should be rewired to some fully
    * parameterized method, the rewiring target symbol, otherwise NoSymbol. By
    ...
    */
  protected def rewiredTarget(tree: Tree, derived: Symbol)(using
      Context
  ): Symbol =
    rewiredTarget(tree.symbol, derived)

  /** Converts the type `info` of a member of class `clazz` to a method type
    * that takes the `this` of the class and any type parameters of the class as
    * additional parameters. Example:
    *
    * class Foo[+A <: AnyRef](val xs: List[A]) extends AnyVal { def baz[B >:
    * A](x: B): List[B] = ... }
    *
    * leads to:
    *
    * object Foo { def extension$baz[B >: A <: Any, A >: Nothing <:
    * AnyRef]($this: Foo[A])(x: B): List[B] }
    *
    * If a self type is present, $this has this self type as its type.
    *
    * @param abstractOverClass
    *   if true, include the type parameters of the class in the method's list
    *   of type parameters.
    * @param liftThisType
    *   if true, require created $this to be $this: (Foo[A] & Foo,this). This is
    *   needed if created member stays inside scope of Foo(as in tailrec)
    */
  def fullyParameterizedType(
      info: Type,
      clazz: ClassSymbol,
      abstractOverClass: Boolean = true,
      liftThisType: Boolean = false
  )(using Context): Type =
    val (mtparamCount, origResult) = info match
      ...
      case _              => (0, info)
    val ctparams = if abstractOverClass then clazz.typeParams else Nil
    ...
    def resultType(mapClassParams: Type => Type) =
      ...
      val firstArgType =
        if liftThisType then thisParamType & clazz.thisType else thisParamType
      MethodType(nme.SELF :: Nil)(
        mt => firstArgType :: Nil,
        mt =>
          mapClassParams(origResult)
            .substThisUnlessStatic(clazz, mt.newParamRef(0))
      )

    /** Replace class type parameters by the added type parameters of the
      * polytype `pt`
      */
    def mapClassParams(tp: Type, pt: PolyType): Type =
      val classParamsRange =
        (mtparamCount until mtparamCount + ctparams.length).toList
    ...
    info match
      ...
      case _ =>
        if ctparams.isEmpty then resultType(identity)
        else
          PolyType(ctnames)(
            mappedClassBounds,
            pt => resultType(mapClassParams(_, pt))
          )
  end fullyParameterizedType

  /** The type parameters (skolems) of the method definition `originalDef`,
    * followed by the class parameters of its enclosing class.
    */
  private def allInstanceTypeParams(
      originalDef: DefDef,
      abstractOverClass: Boolean
  )(using Context): List[Symbol] =
    if abstractOverClass then
      originalDef.leadingTypeParams.map(
        _.symbol
      ) ::: originalDef.symbol.enclosingClass.typeParams
    else originalDef.leadingTypeParams.map(_.symbol)

  /** Given an instance method definition `originalDef`, return a fully
    * parameterized method definition derived from `originalDef`, which has
    * `derived` as symbol and `fullyParameterizedType(originalDef.symbol.info)`
    * as info. `abstractOverClass` defines weather the DefDef should abstract
    * over type parameters of class that contained original defDef
    */
  def fullyParameterizedDef(
      derived: TermSymbol,
      originalDef: DefDef,
      abstractOverClass: Boolean = true
  )(using Context): Tree =
    DefDef(
      ...
      prefss =>
        ...
        val origLeadingTypeParamSyms =
          allInstanceTypeParams(originalDef, abstractOverClass)
        val origOtherParamSyms =
          originalDef.trailingParamss.flatten.map(_.symbol)
        val thisRef :: argRefs = vrefss.flatten: @unchecked

        /** If tree should be rewired, the rewired tree, otherwise EmptyTree.
          * @param targs
          *   Any type arguments passed to the rewired tree.
          */
        ...
        /** Type rewiring is needed because a previous reference to an instance
          ...
          *
          * but the type `this.imeth` still persists as the result type of the
          * `if`, because it is kept by the `cpy` operation of the tree
          * transformer. It needs to be rewritten to the common result type of
          * `imeth` and `xmeth`.
          */
        def rewireType(tpe: Type) = tpe match
          case tpe: TermRef if rewiredTarget(tpe.symbol, derived).exists =>
            tpe.widen
          case _ => tpe
        ...
        new TreeTypeMap(
          typeMap = rewireType(_)
            .subst(
              origLeadingTypeParamSyms ++ origOtherParamSyms,
              (trefs ++ argRefs).tpes
            )
  ...
  /** A forwarder expression which calls `derived`, passing along
    *   - if `abstractOverClass` the type parameters and enclosing class
    *     parameters of originalDef`,
  ...
  def forwarder(
    ...
    val fun: Tree =
      ref(derived.termRef)
        .appliedToTypes(
          allInstanceTypeParams(originalDef, abstractOverClass).map(_.typeRef)
        )
        .appliedTo(This(originalDef.symbol.enclosingClass.asClass))
    val fwd =
      if !liftThisType then
        fun.appliedToArgss(
          originalDef.trailingParamss.nestedMap(param => ref(param.symbol))
        )
      else
        ...
        originalDef.trailingParamss.foldLeft(fun)((acc, params) =>
          ...
          acc.appliedToArgs(
            params
              ...
              .map((param, paramType) =>
                assert(
                  param.tpe <:< paramType.widen
                ) // type should still conform to widened type
...
end FullParameterization

object FullParameterization:

  /** Assuming `info` is a result of a `fullyParameterizedType` call, the
    * signature of the original method type `X` after stripping its leading type
    * parameters section, such that: info.stripPoly.ensureMethodic =
    * fullyParameterizedType(X, ...).stripPoly.ensureMethodic
    *
