...
object ContextFunctionResults:
  ...
  /** Annotate methods that have context function result types directly matched
    * by context closures on their right-hand side. Parameters to such closures
    * will be integrated as additional method parameters in erasure.
    *
  ...
  def annotateContextResults(mdef: DefDef)(using Context): Unit =
    ...
    if Config.flattenContextFunctionResults && count != 0 && !disabled then
      val countAnnot = Annotation(
        defn.ContextResultCountAnnot,
        Literal(Constant(count)),
        mdef.symbol.span
      )
      mdef.symbol.addAnnotation(countAnnot)
  end annotateContextResults

  /** The argument of a ContextResultCount annotation, or 0 if none exists. See
    * PostTyper#annotateContextResults.
    */
  def contextResultCount(sym: Symbol)(using Context): Int =
    sym.getAnnotation(defn.ContextResultCountAnnot) match
      case Some(annot) =>
        val ast.Trees.Literal(Constant(crCount: Int)) :: Nil =
          annot.arguments: @unchecked
        crCount
      case none => 0

  /** True iff `ContextResultCount` is not zero and all context functions in the
    * result type are erased.
    */
  def contextResultsAreErased(sym: Symbol)(using Context): Boolean =
    def allErased(tp: Type): Boolean = tp.dealias match
      case defn.ContextFunctionType(_, resTpe, erasedParams) =>
        !erasedParams.contains(false) && allErased(resTpe)
      case _ => true
      ...
      tp match
        ...
        case tp: MethodOrPoly =>
          tp.derivedLambdaType(resType =
            integrateContextResults(tp.resType, crCount)
          )
        case defn.ContextFunctionType(argTypes, resType, erasedParams) =>
          MethodType(argTypes, integrateContextResults(resType, crCount - 1))

  /** The total number of parameters of method `sym`, not counting erased
    * parameters, but including context result parameters.
    */
  def totalParamCount(sym: Symbol)(using Context): Int =
    ...
    def contextParamCount(tp: Type, crCount: Int): Int =
      if crCount == 0 then 0
        ...
        val defn.ContextFunctionType(params, resTpe, erasedParams) =
          tp: @unchecked
        val rest = contextParamCount(resTpe, crCount - 1)
        if erasedParams.contains(true) then
          erasedParams.count(_ == false) + rest
        else params.length + rest
    ...
    def normalParamCount(tp: Type): Int = tp.widenExpr.stripPoly match
      case mt @ MethodType(pnames) =>
        ...
        if mt.hasErasedParams then mt.erasedParams.count(_ == false) + rest
  ...
  /** The `depth` levels nested context function type in the result type of
    * `meth`
    */
  ...
  /** Should selection `tree` be eliminated since it refers to an `apply` node
    * of a context function type whose parameters will end up being integrated
  ...
  def integrateSelect(tree: untpd.Tree, n: Int = 0)(using Context): Boolean =
    if ctx.erasedTypes then atPhase(erasurePhase)(integrateSelect(tree, n))
    else
      tree match
        case Select(qual, name) =>
          if name == nme.apply then
            qual.tpe match
              ...
              case _
                  if defn.isContextFunctionClass(
                    tree.symbol.maybeOwner
                  ) => // for TermRefs
                integrateSelect(qual, n + 1)
              case _ =>
                n > 0 && contextResultCount(tree.symbol) >= n
          else n > 0 && contextResultCount(tree.symbol) >= n
        case Ident(name) =>
          n > 0 && contextResultCount(tree.symbol) >= n
        case Apply(fn, args) =>
