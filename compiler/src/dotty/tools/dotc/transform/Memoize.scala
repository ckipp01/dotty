...
/** Provides the implementations of all getters and setters, introducing fields
  * to hold the value accessed by them. TODO: Make LazyVals a part of this
  * phase?
  *
  * <accessor> <stable> <mods> def x(): T = e
  * --> private val x: T = e <accessor> <stable> <mods> def x(): T = x
  *
  * <accessor> <mods> def x(): T = e
  * --> private[this] var x: T = e <accessor> <mods> def x(): T = x
  *
  * <accessor> <mods> def x_=(y: T): Unit = ()
  * --> <accessor> <mods> def x_=(y: T): Unit = x = y
  */
class Memoize extends MiniPhase with IdentityDenotTransformer:
  thisPhase =>
  ...
  override def checkPostCondition(tree: Tree)(using Context): Unit =
    ...
    tree match
      ...
      case tdef: TypeDef
          if tdef.symbol.isClass && !tdef.symbol.is(
            Deferred
          ) && tdef.rhs == EmptyTree =>
  ...
  /** Should run after mixin so that fields get generated in the class that
    * contains the concrete getter rather than the trait that defines it.
    */
  ...
  override def transformTemplate(tree: Template)(using Context): Tree =
    ...
    if myState.classesThatNeedReleaseFence.contains(cls) then
      val releaseFenceCall = ref(
        defn.staticsMethodRef(nme.releaseFence)
      ).appliedToNone
      cpy.Template(tree)(
        tree.constr,
        tree.parents,
  ...
  override def transformDefDef(tree: DefDef)(using Context): Tree =
    ...
    def newField =
      ...
      val fieldType =
        if sym.isGetter then sym.info.resultType
      ...
      newSymbol(
        ...
        flags =
          Private | (if sym.is(StableRealizable) then EmptyFlags else Mutable),
        info = fieldType,
        coord = tree.span
      ).withAnnotationsCarrying(
        sym,
        defn.FieldMetaAnnot,
    ...
    if sym.is(Accessor, butNot = NoFieldNeeded) then
      def adaptToField(field: Symbol, tree: Tree): Tree =
        if tree.isEmpty then tree else tree.ensureConforms(field.info.widen)
      ...
      if sym.isGetter then
        if sym.isConstExprFinalVal then
          ...
          if isWildcardArg(rhs) then rhs = EmptyTree
          val fieldDef = transformFollowing(
            ValDef(field, adaptToField(field, rhs))
          )
          val rhsClass = tree.tpt.tpe.widenDealias.classSymbol
          val getterRhs =
            if isErasableBottomField(field, rhsClass) then
              erasedBottomTree(rhsClass)
        ...
        if !sym.is(ParamAccessor) then
          val Literal(Constant(())) =
            tree.rhs: @unchecked // This is intended as an assertion
        val field = sym.field
        if !field.exists then
          ...
          val initializer =
            if isErasableBottomField(
                field,
                tree.termParamss.head.head.tpt.tpe.classSymbol
              )
            then Literal(Constant(()))
            else
              Assign(
                ref(field),
                adaptToField(field, ref(tree.termParamss.head.head.symbol))
              )
          val setterDef = cpy.DefDef(tree)(rhs =
            transformFollowingDeep(initializer)(using ctx.withOwner(sym))
          )
