...
class MacroAnnotations:
  ...
  /** Expands every macro annotation that is on this tree. Returns a list with
    * transformed definition and any added definitions.
    */
  def expandAnnotations(tree: MemberDef)(using Context): List[DefTree] =
    if !hasMacroAnnotation(tree.symbol) then List(tree)
      ...
      val macroInterpreter =
        new Interpreter(tree.srcPos, MacroClassLoader.fromContext)
      ...
      val transformedTree: DefTree = tree.symbol.annotations.foldLeft(tree) {
        (tree, annot) =>
          if isMacroAnnotation(annot) then
              ...
              try callMacro(macroInterpreter, tree, annot)
                ...
                case ex0: InvocationTargetException =>
                  ex0.getCause match
                    ...
                    case NonFatal(ex) =>
                      val stack0 = ex.getStackTrace.takeWhile(
                        _.getClassName != "dotty.tools.dotc.transform.MacroAnnotations"
                      )
                      val stack = stack0.take(
                        1 + stack0.lastIndexWhere(
                          _.getMethodName == "transform"
                        )
                      )
                      val msg =
                        em"""Failed to evaluate macro.
                          |  Caused by ${ex.getClass}: ${
                            if ex.getMessage == null then "" else ex.getMessage
                          }
                          |    ${stack.mkString("\n    ")}
                          |"""
            ...
            transformedTrees.span(_.symbol != tree.symbol) match
              ...
              case (Nil, Nil) =>
                report.error(
                  i"Unexpected `Nil` returned by `(${annot.tree}).transform(..)` during macro expansion",
                  annot.tree.srcPos
                )
                tree
              case (_, Nil) =>
                report.error(
                  i"Transformed tree for ${tree} was not return by `(${annot.tree}).transform(..)` during macro expansion",
                  annot.tree.srcPos
                )
                tree
          else tree
  ...
  private def callMacro(
    ...
    type MacroAnnotation = {
      def transform(using
          Quotes
      )(tree: Object /*Erased type of quotes.refelct.Definition*/ ): List[
        MemberDef /*quotes.refelct.Definition known to be MemberDef in QuotesImpl*/
      ]
    }
    ...
    // TODO: Remove when scala.annaotaion.MacroAnnotation is no longer experimental
    assert(
      annotInstance.getClass.getClassLoader
        .loadClass("scala.annotation.MacroAnnotation")
        .isInstance(annotInstance)
    )

    val quotes = QuotesImpl()(using
      SpliceScope
        .contextWithNewSpliceScope(tree.symbol.sourcePos)(using
          MacroExpansion.context(tree)
        )
        .withOwner(tree.symbol.owner)
    )
    annotInstance.transform(using quotes)(
      tree.asInstanceOf[quotes.reflect.Definition]
    )
  end callMacro

  /** Check that this tree can be added by the macro annotation */
  private def checkMacroDef(
      newTree: DefTree,
      annotatedTree: Tree,
      annot: Annotation
  )(using Context) =
    ...
    if sym.isType && !sym.isClass then
      report.error(
        i"macro annotation cannot return a `type`. $annot tried to add $sym",
        annot.tree
      )
    else if sym.owner != annotated.owner && !(annotated.owner.isPackageObject && (sym.isClass || sym
        .is(Module)) && sym.owner == annotated.owner.owner)
    then
      report.error(
        i"macro annotation $annot added $sym with an inconsistent owner. Expected it to be owned by ${annotated.owner} but was owned by ${sym.owner}.",
        annot.tree
      )
    else if annotated.isClass && annotated.owner.is(Package) /*&& !sym.isClass*/
    then
      report.error(
        i"macro annotation can not add top-level ${sym.showKind}. $annot tried to add $sym.",
        annot.tree
      )
...
object MacroAnnotations:
  ...
  /** Is this symbol annotated with an annotation that implements
    * `scala.annation.MacroAnnotation`
    */
