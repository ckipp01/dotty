...
/** This phase implements the following transformations:
  ...
  * e becomes () ?=> e
  *
  * An optimization is applied: If the argument `e` to a cbn parameter is
  * already of type `() ?=> T` and is a pure expression, we avoid (2) and (3),
  * i.e. we pass `e` directly instead of `() ?=> e.apply()`.
  *
  * Note that `() ?=> T` cannot be written in source since user-defined context
  * functions must have at least one parameter. We use the type here as a
  * convenient marker of something that will erase to Function0, and where we
  * know that it came from a by-name parameter.
  *
  * Note also that the transformation applies only to types of parameters, not
  * to other occurrences of ExprTypes. In particular, embedded occurrences in
  * function types such as `(=> T) => U` are left as-is here (they are
  * eliminated in erasure). Trying to convert these as well would mean
  * traversing all the types, and that leads to cyclic reference errors in many
  * cases. This can cause problems in that we might have sometimes a `() ?=> T`
  * where a `=> T` is expected. To compensate, there is a new clause in
  * TypeComparer#subArg that declares `() ?=> T` to be a subtype of `=> T` for
  * arguments of type applications at any point after this phase and up to
  * erasure.
  */
class ElimByName extends MiniPhase, InfoTransformer:
  ...
  override def runsAfterGroupsOf: Set[String] =
    Set(ExpandSAMs.name, ElimRepeated.name, RefChecks.name)
      ...
      def exprToFun(tp: Type, name: Name) = tp match
        case ExprType(rt) =>
          if rt.hasAnnotation(defn.ErasedParamAnnot) then
            report.error(
              em"By-name parameter cannot be erased: $name",
              sym.srcPos
            )
          defn.ByNameFunction(rt)
        case tp => tp
        ...
        case defn.ByNameFunction(formalResult) =>
          ...
          stripTyped(arg) match
            case Apply(Select(qual, nme.apply), Nil)
                if isByNameRef(qual) && (isPureExpr(qual) || qual.symbol
                  .isAllOf(InlineParam)) =>
              qual
            case _ =>
