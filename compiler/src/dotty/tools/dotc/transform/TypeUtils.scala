...
object TypeUtils:
  ...
  extension (self: Type)
    ...
    /** Is this type a checked exception? This is the case if the type derives
      * from Exception but not from RuntimeException. According to that
      * definition Throwable is unchecked. That makes sense since you should
      * neither throw nor catch `Throwable` anyway, so we should not define a
      * capability to do so.
      */
    ...
    def ensureMethodic(using Context): Type = self match
      ...
      case _ =>
        if ctx.erasedTypes then MethodType(Nil, self) else ExprType(self)

    def widenToParents(using Context): Type = self.parents match
      case Nil => self
      case ps  => ps.reduceLeft(AndType(_, _))

    /** The arity of this tuple type, which can be made up of EmptyTuple, TupleX
      * and `*:` pairs, or -1 if this is not a tuple type.
      */
    def tupleArity(using Context): Int =
      self /*.dealias*/ match // TODO: why does dealias cause a failure in tests/run-deep-subtype/Tuple-toArray.scala
        case AppliedType(tycon, _ :: tl :: Nil)
            if tycon.isRef(defn.PairClass) =>
    ...
    /** The element types of this tuple type, which can be made up of
      * EmptyTuple, TupleX and `*:` pairs
      */
    def tupleElementTypes(using Context): Option[List[Type]] =
      self.dealias match
        ...
        case AndType(tp1, tp2) =>
          ...
          tp1.tupleElementTypes.zip(tp2.tupleElementTypes).map {
            case (t1, t2) => t1.intersect(t2)
          }
        case OrType(tp1, tp2) =>
    ...
    def refinedWith(name: Name, info: Type)(using Context) =
      RefinedType(self, name, info)
    ...
    def mirrorCompanionRef(using Context): TermRef = self match
      case AndType(tp1, tp2) =>
        ...
        if c1.isSubClass(c2) then tp1.mirrorCompanionRef
          ...
          tp2.mirrorCompanionRef // precondition: the parts of the AndType have already been checked to be non-overlapping
    ...
    /** Is this type a methodic type that takes implicit parameters (both old
      * and new) at some point?
      */
    def takesImplicitParams(using Context): Boolean = self.stripPoly match
      case mt: MethodType =>
        mt.isImplicitMethod || mt.resType.takesImplicitParams
      case _ => false
  end extension
end TypeUtils
