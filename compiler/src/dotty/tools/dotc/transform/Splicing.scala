...
/** Transforms level 1 splices into holes. To do so it transforms the contents
  * of the splice into a lambda that receives all cross-quote references.
  *
  * Cross-quote reference is a reference to a definition that is not defined in
  * the current quote. Those references appear in quotes that are nested in a
  * splice.
  *
  * After this phase we have the invariant where all splices have the following
  * shape
  * ```
  * {{{ <holeIdx> | <holeType> | <captures>* | (<capturedTerms>*) => <spliceContent> }}}
  * ```
  * where `<spliceContent>` does not contain any free references to quoted
  * definitions and `<captures>*` contains the quotes with references to all
  * cross-quote references. There are some special rules for references in the
  * LHS of assignments and cross-quote method references.
  *
  ...
  * ```
  *
  * This phase identifies cross-quote references such as `x1` and replaces it
  * with an `${x1$}`. All cross-quote arguments are directly applied in the
  * lambda.
  *
  * ```
  * '{ ...
...
class Splicing extends MacroTransform:
  ...
  override def run(using Context): Unit =
    if ctx.compilationUnit.needsStaging then super.run

  protected def newTransformer(using Context): Transformer =
    Level0QuoteTransformer
  ...
  private object Level0QuoteTransformer extends Transformer:
    override def transform(tree: tpd.Tree)(using Context): tpd.Tree =
      ...
      tree match
        case tree: Quote =>
          val body1 =
            QuoteTransformer().transform(tree.body)(using quoteContext)
  ...
  /** Transforms all direct splices in the current quote and replace them with
    * holes.
    */
  private class QuoteTransformer() extends Transformer:
    ...
    /** Mapping from the term of a `Type[T]` to it's hole. Used to deduplicate
      * type holes.
      */
    ...
    override def transform(tree: tpd.Tree)(using Context): tpd.Tree =
      ...
      tree match
        case tree: Splice if level == 1 =>
          ...
          val newSplicedCode1 =
            splicer.transformSplice(tree.expr, tree.tpe, holeIdx)(using
              spliceContext
            )
          val newSplicedCode2 = Level0QuoteTransformer.transform(
            newSplicedCode1
          )(using spliceContext)
          newSplicedCode2
        case _: Template =>
          for sym <- tree.symbol.owner.info.decls do quotedDefs += sym
          super.transform(tree)
        case tree: DefTree =>
    ...
    private def transformAnnotations(tree: DefTree)(using Context): Unit =
      tree.symbol.annotations = tree.symbol.annotations.mapconserve { annot =>
        val newAnnotTree =
          transform(annot.tree)(using ctx.withOwner(tree.symbol))
        if annot.tree == newAnnotTree then annot
      ...
      override def apply(tp: Type): Type =
        tp match
          case tp @ AnnotatedType(underlying, annot) =>
            ...
            derivedAnnotatedType(
              tp,
              underlying1,
              annot.derivedAnnotation(transform(annot.tree))
            )
          case _ => mapOver(tp)
  ...
  /** Transforms a splice at level 1 into a hole
    *
    * Finds all terms and types that are defined in the current quote and used
    * within this splice. The resulting hole will contain all of these terms and
    * types as arguments. Note that these captured variables are stage correct.
    *
    ...
    * ```
    */
  private class SpliceTransformer(
      spliceOwner: Symbol,
      isCaptured: Symbol => Boolean
  ) extends Transformer:
    private var refBindingMap =
      mutable.LinkedHashMap.empty[Symbol, (Tree, Symbol)]

    /** Reference to the `Quotes` instance of the current level 1 splice */
    private var quotes: Tree | Null = null // TODO: add to the context

    def transformSplice(tree: tpd.Tree, tpe: Type, holeIdx: Int)(using
        Context
    ): tpd.Tree =
      assert(level == 0)
      ...
      val meth =
        newSymbol(spliceOwner, nme.ANON_FUN, Synthetic | Method, methType)
      val ddef = DefDef(
        meth,
        List(bindings),
        newTree.tpe,
        newTree.changeOwner(ctx.owner, meth)
      )
      val fnType = defn
        .FunctionType(bindings.size, isContextual = false)
        .appliedTo(bindingsTypes :+ newTree.tpe)
      val closure =
        Block(ddef :: Nil, Closure(Nil, ref(meth), TypeTree(fnType)))
      tpd.Hole(true, holeIdx, refs, closure, tpe)

    override def transform(tree: tpd.Tree)(using Context): tpd.Tree =
      tree match
        case tree: Select if tree.isTerm && isCaptured(tree.symbol) =>
          tree.symbol.allOverriddenSymbols.find(sym =>
            !isCaptured(sym.owner)
          ) match
            case Some(sym) =>
            ...
            case _ =>
              report.error(
                em"Can not use reference to staged local ${tree.symbol} defined in an outer quote.\n\nThis can work if ${tree.symbol.owner} would extend a top level interface that defines ${tree.symbol}.",
                tree
              )
              tree
        case tree: RefTree =>
        ...
        case tree: TypeTree =>
          if containsCapturedType(tree.tpe) && level >= 1 then
            getTagRefFor(tree)
          else tree
        ...
        case Apply(Select(Quote(body, _), nme.apply), quotes :: Nil)
            if level == 0 && body.isTerm =>
          body match
        ...
        case tree: Quote if level == 0 =>
          ...
          else if containsCapturedType(tree.body.tpe) then
            capturedPartTypes(tree)
          else tree
        case _ =>
          super.transform(tree)
    ...
    private object CapturedApplication:
      ...
      /** Matches and application `f(...)` (possibly with several argument
        * clauses) where `f` is captured
        */
      def unapply(tree: Tree)(using
          Context
      ): Option[(RefTree, List[ArgsClause])] = tree match
    ...
    private def containsCapturedType(tpe: Type)(using Context): Boolean =
      tpe.existsPart(
        t => isCaptured(t.typeSymbol) || isCaptured(t.termSymbol),
        StopAt.Static
      )
    ...
    /** Transform an assignment `x = e` with a captured `x` to `${
      ...
      *
      * Registers `x` as a captured variable in the hole and creates an `x$1`
      * `Expr` reference to it.
      */
    ...
    /** Transform an application `f(a1, a2, ...)` with a captured `f` to `${
      * Apply(f$1.asTerm, List('{a1$}.asTerm, '{a2$}.asTerm,
      * ...)).asExpr.asInstanceOf[Expr[T]] }`
      *
      * Registers `f` as a captured variable in the hole and creates an `f$1`
      * `Expr` reference to it.
      *
      * It also handles cases with multiple argument clauses using nested
      * `Apply`/`TypeApply`.
      */
    private def transformCapturedApplication(
        tree: Tree,
        fn: RefTree,
        argss: List[ArgsClause]
    )(using Context): Tree =
      spliced(tree.tpe) {
        def TermList(args: List[Tree]): List[Tree] =
          args.map(arg =>
            reflect.asTerm(quoted(transform(arg)(using spliceContext)))
          )
        def TypeTreeList(args: List[Tree]): List[Tree] =
          args.map(arg =>
            reflect.Inferred(
              reflect.TypeReprOf(transform(arg)(using spliceContext).tpe)
            )
          )
        reflect.asExpr(tree.tpe) {
          argss.foldLeft[Tree](reflect.asTerm(capturedTerm(fn, defn.AnyType))) {
            (acc, clause) =>
    ...
    private def capturedTerm(tree: Tree, tpe: Type)(using Context): Tree =
      ...
      val bindingSym =
        refBindingMap.getOrElseUpdate(tree.symbol, (tree, newBinding))._2
      ref(bindingSym)
    ...
    private def capturedType(tree: Tree)(using Context): Symbol =
      ...
      val bindingSym = refBindingMap
        .getOrElseUpdate(
          tree.symbol,
          (TypeTree(tree.tpe), newQuotedTypeClassBinding(tpe))
        )
        ._2
      bindingSym
          ...
          def apply(tp: Type) = tp match
            case typeRef: TypeRef if containsCapturedType(typeRef) =>
              val termRef = refBindingMap
                .getOrElseUpdate(
                  typeRef.symbol,
                  (TypeTree(typeRef), newQuotedTypeClassBinding(typeRef))
                )
                ._2
                .termRef
    ...
    private def withCurrentQuote[T](
        newQuotes: Tree
    )(body: => T)(using Context): T =
      if level == 0 then
    ...
    private def spliced(tpe: Type)(body: Context ?=> Tree)(using
        Context
    ): Tree =
      val exprTpe = defn.QuotedExprClass.typeRef.appliedTo(tpe)
      val closure =
        val methTpe =
          ContextualMethodType(List(defn.QuotesClass.typeRef), exprTpe)
        val meth =
          newSymbol(ctx.owner, nme.ANON_FUN, Synthetic | Method, methTpe)
        Closure(
          meth,
          argss =>
    ...
    /** Helper methods to construct trees calling methods in `Quotes.reflect`
      * based on the current `quotes` tree
      */
