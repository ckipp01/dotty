...
/** A utility class for generating access proxies. Currently used for inline
  * accessors and protected accessors.
  */
abstract class AccessProxies:
  ...
  /** Given the name of an accessor, is the receiver of the call to accessed
    * obtained as a parameterer?
    */
  protected def passReceiverAsArg(accessorName: Name)(using Context): Boolean =
    false
  ...
  private def accessorDefs(cls: Symbol)(using Context): Iterator[DefDef] =
    for
      accessor <- cls.info.decls.iterator; accessed <- accessedBy.get(accessor)
    yield DefDef(
      accessor.asTerm,
      prefss =>
        ...
        val (accessRef, forwardedTpts, forwardedArgss) =
          if passReceiverAsArg(accessor.name) then
            (
              argss.head.head.select(accessed),
              targs.takeRight(numTypeParams),
              argss.tail
            )
          else
            (
              if accessed.isStatic then ref(accessed)
              else ref(TermRef(cls.thisType, accessed)),
              targs,
              argss
            )
        val rhs =
          if accessor.name.isSetterName &&
            forwardedArgss.nonEmpty && forwardedArgss.head.nonEmpty
          then // defensive conditions
  ...
  def addAccessorDefs(cls: Symbol, body: List[Tree])(using
    ...
    if accDefs.isEmpty then body else body ++ accDefs

  trait Insert:
    import ast.tpd.*

    /** The name of the accessor for definition with given `name` in given
      * `site`
      */
    ...
    def ifNoHost(reference: RefTree)(using Context): Tree =
      assert(
        false,
        i"no host found for $reference with ${reference.symbol.showLocated} from ${ctx.owner}"
      )
      reference

    /** A fresh accessor symbol */
    private def newAccessorSymbol(
        owner: Symbol,
        name: TermName,
        info: Type,
        accessed: Symbol
      ...
      else if sym.allOverriddenSymbols.exists(!_.is(Deferred)) then
        sym.setFlag(Override)
    ...
    protected def accessorSymbol(
        owner: Symbol,
        accessorName: TermName,
        accessorInfo: Type,
        accessed: Symbol
    ...
    private def rewire(reference: RefTree, accessor: Symbol)(using
        Context
    ): Tree = {
      reference match
        case Select(qual, _) if qual.tpe.derivesFrom(accessor.owner) =>
          qual.select(accessor)
        case _ => ref(accessor)
    }.withSpan(reference.span)

    /** Given a reference to a getter accessor, the corresponding setter
      * reference
      */
      ...
      case getterRef: RefTree =>
        ...
        def toSetterInfo(getterInfo: Type): Type = getterInfo match
          case getterInfo: LambdaType =>
            getterInfo.derivedLambdaType(resType =
              toSetterInfo(getterInfo.resType)
            )
          case _ =>
            MethodType(getterInfo :: Nil, defn.UnitType)
        val setterInfo = toSetterInfo(getter.info.widenExpr)
        val setter =
          accessorSymbol(getter.owner, setterName, setterInfo, accessed)
    ...
    def useAccessor(reference: RefTree)(using Context): Tree =
      ...
      if accessorClass.exists then
        ...
        val accessorInfo =
          accessed.info.ensureMethodic
            .asSeenFrom(accessorClass.thisType, accessed.owner)
        val accessor =
          accessorSymbol(accessorClass, accessorName, accessorInfo, accessed)
    ...
    def accessorIfNeeded(tree: Tree)(using Context): Tree = tree match
      case tree: RefTree if needsAccessor(tree.symbol) =>
        if tree.symbol.isConstructor then
          report.error(
            "Implementation restriction: cannot use private constructors in inlineable methods",
            tree.srcPos
          )
...
object AccessProxies:
  /** Where an accessor for the `accessed` symbol should be placed. This is the
    * closest enclosing class that has `accessed` as a member.
    */
  def hostForAccessorOf(accessed: Symbol)(using Context): Symbol =
    def recur(cls: Symbol): Symbol =
      if !cls.exists then NoSymbol
