...
class CapturedVars extends MiniPhase with IdentityDenotTransformer:
  ...
  private class RefInfo(using Context):
    ...
    val refClassKeys: collection.Set[Symbol] =
      defn.ScalaNumericValueClasses() `union` Set(
        defn.BooleanClass,
        defn.ObjectClass
      )
    ...
    val refClass: Map[Symbol, Symbol] =
      refClassKeys
        .map(rc => rc -> requiredClass(s"scala.runtime.${rc.name}Ref"))
        .toMap

    val volatileRefClass: Map[Symbol, Symbol] =
      refClassKeys
        .map(rc => rc -> requiredClass(s"scala.runtime.Volatile${rc.name}Ref"))
        .toMap
  ...
  private def refInfo(using Context): RefInfo =
    if myRefInfo == null then myRefInfo = new RefInfo()
      ...
      case id: Ident =>
        ...
        if sym.is(Mutable, butNot = Method) && sym.owner.isTerm then
          ...
          if sym.enclosingMethod != enclMeth then
            report.log(
              i"capturing $sym in ${sym.enclosingMethod}, referenced from $enclMeth"
            )
            captured += sym
      case _ =>
  ...
  /** The {Volatile|}{Int|Double|...|Object}Ref class corresponding to the class
    * `cls`, depending on whether the reference should be @volatile
    */
  def refClass(cls: Symbol, isVolatile: Boolean)(using Context): Symbol =
    val refMap =
      if isVolatile then refInfo.volatileRefClass else refInfo.refClass
    if cls.isClass then refMap.getOrElse(cls, refMap(defn.ObjectClass))
  ...
  override def prepareForValDef(vdef: ValDef)(using Context): Context =
    ...
    if captured contains sym then
      val newd = atPhase(thisPhase)(sym.denot).copySymDenotation(
        info = refClass(
          sym.info.classSymbol,
          sym.hasAnnotation(defn.VolatileAnnot)
        ).typeRef,
  ...
  override def transformValDef(vdef: ValDef)(using Context): Tree =
    ...
    if captured.contains(vble) then
  ...
  /** If assignment is to a boxed ref type, e.g.
    ...
    *
    * That way, we avoid the problem that `expr` might contain a `try` that
    * would run on a non-empty stack (which is illegal under JVM rules). Note
    * that LiftTry has already run before, so such `try`s would not be
    * eliminated.
    *
    * If the ref type lhs is followed by a cast (can be an artifact of nested
    * translation), drop the cast.
    *
    * If the ref type is `ObjectRef` or `VolatileObjectRef`, immediately assign
    * `null` to the temporary to make the underlying target of the reference
    * available for garbage collection. Nullification is omitted if the `expr`
    * is already `null`.
    *
    * var ev$n: RHS = expr objRef.elem = ev$n ev$n = null.asInstanceOf[RHS]
    */
  override def transformAssign(tree: Assign)(using Context): Tree =
    ...
    def recur(lhs: Tree): Tree = lhs match
      case TypeApply(
            Select(qual @ Select(_, nme.elem), nme.asInstanceOf_),
            _
          ) =>
        recur(qual)
      case Select(_, nme.elem)
          if refInfo.boxedRefClasses.contains(lhs.symbol.maybeOwner) =>
        val tempDef = transformFollowing(
          SyntheticValDef(TempResultName.fresh(), tree.rhs, flags = Mutable)
        )
        val update = cpy.Assign(tree)(lhs, ref(tempDef.symbol))
        def reset = cpy.Assign(tree)(
          ref(tempDef.symbol),
          nullLiteral.cast(tempDef.symbol.info)
        )
        val res =
          if refInfo.objectRefClasses(lhs.symbol.maybeOwner) && !absolved then
            reset
          else unitLiteral
