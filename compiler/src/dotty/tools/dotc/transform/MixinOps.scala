...
class MixinOps(cls: ClassSymbol, thisPhase: DenotTransformer)(using Context):
  ...
  lazy val JUnit4Annotations: List[Symbol] =
    List("Test", "Ignore", "Before", "After", "BeforeClass", "AfterClass")
      .map(n => getClassIfDefined("org.junit." + n))
      .filter(_.exists)

  def mkForwarderSym(
      member: TermSymbol,
      extraFlags: FlagSet = EmptyFlags
  ): TermSymbol =
    val res = member
      .copy(
        owner = cls,
  ...
  def superRef(target: Symbol, span: Span = cls.span): Tree =
    val sup =
      if target.isConstructor && !target.owner.is(Trait) then
        Super(This(cls), tpnme.EMPTY)
      else Super(This(cls), target.owner.name.asTypeName, target.owner)
    // println(i"super ref $target on $sup")
    ast.untpd
      .Select(sup.withSpan(span), target.name)
      .withType(NamedType(sup.tpe, target))
    // sup.select(target)

  /** Is `sym` a member of implementing class `cls`? The test is performed at
    * phase `thisPhase`.
    */
  ...
  /** Does `method` need a forwarder to in class `cls` Method needs a forwarder
    * in those cases:
  ...
  def needsMixinForwarder(meth: Symbol): Boolean =
    ...
    def needsDisambiguation = competingMethods.exists(x =>
      !x.is(Deferred)
    ) // multiple implementations are available
    def hasNonInterfaceDefinition = competingMethods.exists(
      !_.owner.is(Trait)
    ) // there is a definition originating from class
    ...
    def generateJUnitForwarder: Boolean =
      meth.annotations.nonEmpty && JUnit4Annotations.exists(annot =>
        meth.hasAnnotation(annot)
      ) &&
    ...
    (ctx.settings.mixinForwarderChoices.isTruthy || meth.owner.is(
      Scala2x
    ) || needsDisambiguation || hasNonInterfaceDefinition ||
  ...
  def forwarderRhsFn(target: Symbol): List[List[Tree]] => Tree =
    prefss =>
      ...
      if ctx.explicitNulls && target.is(JavaDefined) && !ctx.phase.erasedTypes
      then
