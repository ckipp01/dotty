...
import Types.*, Contexts.*, Constants.*, Names.*, NameOps.*, Flags.*,
  DenotTransformers.*
...
/** This phase rewrite PolyFunction subclasses to FunctionN subclasses
  *
  * class Foo extends PolyFunction { def apply(x_1: P_1, ..., x_N: P_N): R = rhs
  * } becomes: class Foo extends FunctionN { def apply(x_1: P_1, ..., x_N: P_N):
  * R = rhs }
  */
class ElimPolyFunction extends MiniPhase with DenotTransformer:
  ...
  override def changesParents: Boolean =
    true // Replaces PolyFunction by FunctionN

  override def transform(ref: SingleDenotation)(using Context) = ref match
    case ref: ClassDenotation
        if ref.symbol != defn.PolyFunctionClass && ref.derivesFrom(
      ...
      val newParents = cinfo.declaredParents.map(parent =>
        if parent.typeSymbol == defn.PolyFunctionClass then newParent
        else parent
      )
      ref.copySymDenotation(info =
        cinfo.derivedClassInfo(declaredParents = newParents)
      )
    case _ =>
      ref
