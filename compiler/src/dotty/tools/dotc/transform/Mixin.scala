...
object Mixin:
  ...
  def traitSetterName(getter: TermSymbol)(using Context): TermName =
    extension (name: Name)
      def qualifiedToSimple = name.replace { case n @ AnyQualifiedName(_, _) =>
        n.toSimpleName
      }
    getter.ensureNotPrivate.name.qualifiedToSimple // TODO: Find out why TraitSetterNames can't be defined over QualifiedNames
...
/** This phase performs the following transformations:
  *
  *   1. (done in `traitDefs` and `transformSym`) For every concrete trait
  *      getter
  *
  * <mods> def x(): T = expr
  *
  ...
  * remove its Private flag (but do not expand its name), since it will have to
  * be instantiated in the classes that mix in the trait.
  *
  ...
  *
  * 4.1 (done in `traitInits`) For every parameter accessor `<mods> def x(): T`
  * in M, in order of textual occurrence, add
  *
  * <mods> def x() = e
  *
  * where `e` is the constructor argument in C that corresponds to `x`. Issue an
  * error if no such argument exists.
  *
  * 4.2 (done in `traitInits`) For every concrete trait getter `<mods> def x():
  * T` in M which is not a parameter accessor, in order of textual occurrence,
  ...
  * super[M].$init$()
  *
  * 4.4 (done in `setters`) For every concrete setter `<mods> def x_=(y: T)` in
  * M:
  *
  * <mods> def x_=(y: T) = ()
  *
  * 4.5 (done in `mixinForwarders`) For every method `<mods> def
  * f[Ts](ps1)...(psN): U` imn M` that needs to be disambiguated:
  *
  * <mods> def f[Ts](ps1)...(psN): U = super[M].f[Ts](ps1)...(psN)
  *
  * A method in M needs to be disambiguated if it is concrete, not overridden in
  * C, and if it overrides another concrete method.
  *
  * 5. (done in `transformTemplate` and `transformSym`) Drop all parameters from
  * trait constructors, and rename them to `nme.TRAIT_CONSTRUCTOR`.
  *
  * 6. (done in `transformSym`) Drop ParamAccessor flag from all parameter
  * accessors in traits.
  *
  * Conceptually, this is the second half of the previous mixin phase. It needs
  * to run after erasure because it copies references to possibly private inner
  * classes and objects into enclosing classes where they are not visible. This
  * can only be done if all references are symbolic.
  */
class Mixin extends MiniPhase with SymTransformer:
  ...
  override def changesMembers: Boolean =
    true // the phase adds implementions of mixin accessors

  override def transformSym(sym: SymDenotation)(using Context): SymDenotation =
    def ownerIsTrait: Boolean = was(sym.owner, Trait, butNot = JavaDefined)

    if sym.is(Accessor, butNot = Deferred) && ownerIsTrait then
      val sym1 =
        if sym.is(Lazy) || sym.symbol.isConstExprFinalVal then sym
        else
          sym.copySymDenotation(initFlags =
            sym.flags &~ (ParamAccessor | Inline) | Deferred
          )
    ...
    else if sym.isConstructor && ownerIsTrait then
      ...
      for decl <- classInfo.decls do
        ...
        if !decl.isClass && needsTraitSetter(decl) then
          ...
          setter.validFor =
            thisPhase.validFor // validity of setter = next phase up to next transformer afterwards
          decls1.enter(setter)
          modified = true
      if modified then
        sym.copySymDenotation(info = classInfo.derivedClassInfo(decls = decls1))
      else sym
    else sym
    end if
  end transformSym
  ...
  private def was(sym: Symbol, flag: Flag, butNot: FlagSet)(using
      Context
  ): Boolean =
  ...
  override def transformTemplate(impl: Template)(using Context): Template =
    ...
    /** Map constructor call to a triple of a supercall, and if the target is a
      * trait
      *   - a list of val defs used in arguments (these can arise due to
      *     reorderings with named and/or default parameters).
      *   - a list of arguments to be used as initializers of trait parameters
      */
    def transformConstructor(tree: Tree): (Tree, List[Tree], List[Tree]) =
      tree match
        case Block(stats, expr) =>
          val (scall, inits, args) = transformConstructor(expr)
          if args.isEmpty then (cpy.Block(tree)(stats, scall), inits, args)
        ...
        case _ =>
          val Apply(sel @ Select(New(_), nme.CONSTRUCTOR), args) =
            tree: @unchecked
          val (callArgs, initArgs) =
            if tree.symbol.owner.is(Trait) then (Nil, args) else (args, Nil)
          (
            superRef(tree.symbol, tree.span).appliedToTermArgs(callArgs),
            Nil,
            initArgs
          )
    ...
    val superCallsAndArgs: Map[Symbol, (Tree, List[Tree], List[Tree])] = (
      for (
        p <- impl.parents; constr = stripBlock(p).symbol if constr.isConstructor
      )
    ...
    def superCallOpt(baseCls: Symbol): List[Tree] =
      superCallsAndArgs.get(baseCls) match
        case Some((call, _, _)) =>
          if defn.NotRuntimeClasses.contains(baseCls) || baseCls.isAllOf(
              NoInitsTrait
            )
          then Nil
          else call :: Nil
        case None =>
          if baseCls.isAllOf(NoInitsTrait) || defn.NoInitClasses.contains(
              baseCls
            ) || defn.isFunctionClass(baseCls)
          then Nil
          else
            // println(i"synth super call ${baseCls.primaryConstructor}: ${baseCls.primaryConstructor.info}")
            transformFollowingDeep(
              superRef(baseCls.primaryConstructor).appliedToNone
            ) :: Nil
    ...
    def traitInits(mixin: ClassSymbol): List[Tree] =
      ...
      yield
        if isInImplementingClass(getter) || getter.name.is(ExpandedName) then
          val rhs =
            if wasOneOf(getter, ParamAccessor) then nextArgument()
    ...
    def setters(mixin: ClassSymbol): List[Tree] =
      val mixinSetters = mixin.info.decls.filter { sym =>
        sym.isSetter && (!wasOneOf(sym, Deferred) || sym.name.is(
          TraitSetterName
        ))
      }
      for (setter <- mixinSetters)
        yield transformFollowing(
          DefDef(mkForwarderSym(setter.asTerm), unitLiteral.withSpan(cls.span))
        )
    ...
    cpy.Template(impl)(
      constr =
        if cls.is(Trait) then cpy.DefDef(impl.constr)(paramss = Nil :: Nil)
        else impl.constr,
      parents = impl.parents.map(p => TypeTree(p.tpe).withSpan(p.span)),
      body =
        if cls.is(Trait) then traitDefs(impl.body)
        else if !cls.isPrimitiveValueClass then
