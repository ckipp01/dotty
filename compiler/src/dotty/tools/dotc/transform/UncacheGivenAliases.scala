...
/** This phase optimizes alias givens represented as lazy vals to be uncached if
  * that does not change runtime behavior. A definition does not need to be
...
class UncacheGivenAliases extends MiniPhase with IdentityDenotTransformer:
  ...
  private def needsCache(sym: Symbol, rhs: Tree)(using Context): Boolean =
    rhs.tpe match
      case rhsTpe @ TermRef(NoPrefix, _) if rhsTpe.isStable => false
      case rhsTpe @ TermRef(pre: ThisType, _)
          if rhsTpe.isStable && pre.cls == sym.owner.enclosingClass =>
        false
