...
object SymUtils:
  ...
  extension (self: Symbol)
    ...
    def directlyInheritedTraits(using Context): List[ClassSymbol] =
      ...
      if baseClasses.isEmpty then Nil
      else
        def recur(
          ...
          case bc :: bcs1 =>
            if bc eq superCls then acc else recur(bcs1, bc :: acc)
          case nil => acc
        recur(baseClasses.tail, Nil)

    /** All traits implemented by a class, except for those inherited through
      * the superclass. The empty list if `self` is a trait.
      */
    def mixins(using Context): List[ClassSymbol] =
      if self.is(Trait) then Nil
    ...
    def isThreadUnsafe(using Context): Boolean =
      self.hasAnnotation(defn.ThreadUnsafeAnnot)

    def isVolatile(using Context): Boolean =
      self.hasAnnotation(defn.VolatileAnnot)

    def isAnyOverride(using Context): Boolean =
      self.is(Override) || self.is(AbsOverride)
    ...
    def isSuperAccessor(using Context): Boolean =
      self.name.is(SuperAccessorName)

    def isNoValue(using Context): Boolean =
      self.is(Package) || self.isAllOf(JavaModule)
    ...
    /** Is this a case class for which a product mirror is generated? Excluded
      * are value classes, abstract classes and case classes with more than one
      * parameter section.
      */
    def whyNotGenericProduct(using Context): String =
      /** for a case class, if it will have an anonymous mirror, check that its
        * constructor can be accessed from the calling scope.
        */
      def canAccessCtor: Boolean =
        def isAccessible(sym: Symbol): Boolean = ctx.owner.isContainedIn(sym)
        def isSub(sym: Symbol): Boolean =
          ctx.owner.ownersIterator.exists(_.derivesFrom(sym))
        val ctor = self.primaryConstructor
        (!ctor.isOneOf(Private | Protected) || isSub(
          self
        )) // we cant access the ctor because we do not extend cls
        && (!ctor.privateWithin.exists || isAccessible(
          ctor.privateWithin
        )) // check scope is compatible

      def companionMirror = self.useCompanionAsProductMirror
      if !self.is(CaseClass) then "it is not a case class"
      else if self.is(Abstract) then "it is an abstract class"
      else if self.primaryConstructor.info.paramInfoss.length != 1 then
        "it takes more than one parameter list"
      else if isDerivedValueClass(self) then "it is a value class"
      else if !(companionMirror || canAccessCtor) then
        s"the constructor of $self is innaccessible from the calling scope."
      else ""
    ...
    /** Is this an old style implicit conversion?
      ...
      * @param forImplicitClassOnly
      *   only consider methods generated from implicit classes
      */
    def isOldStyleImplicitConversion(
        directOnly: Boolean = false,
        forImplicitClassOnly: Boolean = false
    ...
    /** Is this a sealed class or trait for which a sum mirror is generated? It
      * must satisfy the following conditions:
      *   - it has at least one child class or object
      *   - none of its children are anonymous classes
      *   - all of its children are addressable through a path from the parent
      *     class and also the location of the generated mirror.
      *   - all of its children are generic products, singletons, or generic
      *     sums themselves.
      */
    def whyNotGenericSum(pre: Type)(using Context): String =
      if !self.is(Sealed) then s"it is not a sealed ${self.kindString}"
        ...
        val ownerScope =
          if pre.isInstanceOf[SingletonType] then pre.classSymbol else NoSymbol
        def problem(child: Symbol) =

          def accessibleMessage(sym: Symbol): String =
            def inherits(sym: Symbol, scope: Symbol): Boolean =
              !scope.is(Package) && (scope
                .derivesFrom(sym) || inherits(sym, scope.owner))
            def isVisibleToParent(sym: Symbol): Boolean =
              self.isContainedIn(sym) || sym.is(Module) && isVisibleToParent(
                sym.owner
              )
            def isVisibleToScope(sym: Symbol): Boolean =
              ...
              isReachable || isMemberOfPrefix || sym.is(
                Module
              ) && isVisibleToScope(sym.owner)
            if !isVisibleToParent(sym) then i"to its parent $self"
            else if !companionMirror && !isVisibleToScope(sym) then
              i"to call site ${ctx.owner}"
            else ""
          end accessibleMessage

          val childAccessible = accessibleMessage(child.owner)

          if child == self then "it has anonymous or inaccessible subclasses"
    ...
    final def skipConstructor(using Context): Symbol =
      if self.isConstructor then self.owner else self
    ...
    @tailrec final def enclosingMethodOrClass(using Context): Symbol =
      ...
      else if self.exists then self.owner.enclosingMethodOrClass
      else NoSymbol

    /** Apply symbol/symbol substitution to this symbol */
    def subst(from: List[Symbol], to: List[Symbol]): Symbol =
      @tailrec def loop(from: List[Symbol], to: List[Symbol]): Symbol =
        if from.isEmpty then self
        else if self eq from.head then to.head
    ...
    /** Is this symbol a constant expression final val?
      ...
      *   - it does not need an explicit field because of Scala.js semantics
      *     (see `JSSymUtils.sjsNeedsField`).
      *
    ...
    def isConstExprFinalVal(using Context): Boolean =
      atPhaseNoLater(erasurePhase) {
        self.is(Final, butNot = Mutable) && self.info.resultType
          .isInstanceOf[ConstantType]
      } && !self.sjsNeedsField

    /** The `ConstantType` of a val known to be `isConstrExprFinalVal`.
      *
      * @pre
      *   `self.isConstantExprFinalVal` is true.
      */
    ...
    def withAnnotationsCarrying(
        from: Symbol,
        meta: Symbol,
        orNoneOf: Set[Symbol] = Set.empty
    )(using Context): self.type =
    ...
    /** Does this symbol refer to anonymous classes synthesized by enum
      * desugaring?
      */
    def isEnumAnonymClass(using Context): Boolean =
      self.isAnonymousClass && (self.owner.name.eq(nme.DOLLAR_NEW) || self.owner
        .is(CaseVal))

    /** Is this symbol defined locally (i.e. at some level owned by a term) so
      * that it cannot be seen from parent class `cls`?
      */
    def isInaccessibleChildOf(cls: Symbol)(using Context): Boolean =
      def isAccessible(sym: Symbol, cls: Symbol): Boolean =
        if cls.isType && !cls.is(Package) then isAccessible(sym, cls.owner)
        else
          sym == cls
          || sym.is(Package)
    ...
    /** Is this symbol directly owner by a term symbol, i.e., is it local to a
      * block?
      */
    ...
    @tailrec final def isLocal(using Context): Boolean =
      ...
      else if isLocalToBlock then true
      else if owner.is(Package) then false
      else owner.isLocal
    ...
    def isExperimental(using Context): Boolean =
      self.hasAnnotation(defn.ExperimentalAnnot)
        || (self.maybeOwner.isClass && self.owner.hasAnnotation(
    ...
    /** The declared self type of this class, as seen from `site`, stripping all
      * refinements for opaque types.
      */
    def declaredSelfTypeAsSeenFrom(site: Type)(using Context): Type =
      extension (tp: Type)
        def stripOpaques: Type = tp match
          case RefinedType(parent, name, _)
              if self.info.decl(name).symbol.isOpaqueAlias =>
            parent.stripOpaques
          case _ =>
            tp
      self.asClass.givenSelfType.stripOpaques.asSeenFrom(site, self)

    /** If `original` has a target name annotation, add one to this symbol as
      * well such that the new target name is `original`'s target name
      * transformed by `nameFn`.
      */
    def deriveTargetNameAnnotation(original: Symbol, nameFn: Name => Name)(using
        Context
    ): Unit =
      if original.hasAnnotation(defn.TargetNameAnnot) then
        self.addAnnotation(
          Annotation(
            ...
            Literal(Constant(nameFn(original.targetName).toString))
              .withSpan(original.span),
            original.span
          )
        )

    /** The return type as seen from the body of this definition. It is computed
      * from the symbol's type by replacing param refs by param symbols.
      */
    def localReturnType(using Context): Type =
      if self.isConstructor then defn.UnitType
      else
        def instantiateRT(info: Type, psymss: List[List[Symbol]]): Type =
          info match
            case info: PolyType =>
              instantiateRT(
                info.instantiate(psymss.head.map(_.typeRef)),
                psymss.tail
              )
            case info: MethodType =>
              instantiateRT(
                info.instantiate(psymss.head.map(_.termRef)),
                psymss.tail
              )
            case info =>
              info.widenExpr
    ...
    /** The expected type of a return to `self` at the place indicated by the
      * context. This is the local return type instantiated by the symbols of
      * any context function closures that enclose the site of the return
      */
    def returnProto(using Context): Type =

      /** If `pt` is a context function type, its return type. If the CFT is
        * dependent, instantiate with the parameters of the associated anonymous
        * function.
        * @param paramss
        *   the parameters of the anonymous functions enclosing the return
        *   expression
        */
        ...
        if ift.exists then
          ift.nonPrivateMember(nme.apply).info match
            case appType: MethodType =>
              instantiateCFT(
                appType.instantiate(paramss.head.map(_.termRef)),
                paramss.tail
              )
        else pt
