...
/** Helper object to generate generic java signatures, as defined in the Java
  * Virtual Machine Specification, §4.3.4
  */
object GenericSignatures:

  /** Generate the signature for `sym0`, with type `info`, as defined in the
    * Java Virtual Machine Specification, §4.3.4
    *
    * @param sym0
  ...
  def javaSig(sym0: Symbol, info: Type)(using Context): Option[String] =
    ...
    if sym0.isLocal && !sym0.isClass then None
    else atPhase(erasurePhase)(javaSig0(sym0, info))

  @noinline
  private final def javaSig0(sym0: Symbol, info: Type)(using
      Context
  ): Option[String] =
    ...
    def superSig(cls: Symbol, parents: List[Type]): Unit =
      def isInterfaceOrTrait(sym: Symbol) =
        sym.is(PureInterface) || sym.is(Trait)
    ...
    /** The signature of the upper-bound of a type parameter.
      *
      * @pre
      *   none of the bounds are themselves type parameters. TODO: Remove this
      *   restriction so we can support things like:
      *
      * class Foo[A]: def foo[S <: A & Object](...): ...
      *
      * Which should emit a signature `S <: A`. See the handling of `AndType` in
      * `jsig` which already supports `def foo(x: A & Object)`.
      */
    ...
    /** The parents of this intersection where type parameters that cannot
      * appear in the signature have been replaced by their upper-bound.
      */
    def flattenedIntersection(tp: AndType)(using Context): List[Type] =
      val parents = ListBuffer[Type]()

      def collect(parent: Type, parents: ListBuffer[Type]): Unit =
        parent.widenDealias match
          case AndType(tp1, tp2) =>
            collect(tp1, parents)
            collect(tp2, parents)
          case parent: TypeRef =>
            if parent.symbol.isClass || isTypeParameterInSig(
                parent.symbol,
                sym0
              )
            then parents += parent
            else collect(parent.superType, parents)
          case parent =>
            parents += parent
    ...
    /** Split the `parents` of an intersection into two subsets: those whose
      * individual erasure matches the overall erasure of the intersection and
      * the others.
      */
    def splitIntersection(parents: List[Type])(using
        Context
    ): (List[Type], List[Type]) =
      ...
      parents
        ...
        .partitionMap((parent, erasedParent) =>
          if erasedParent =:= erasedTp then Left(parent)
          else Right(parent)
        )
    ...
    def methodResultSig(restpe: Type): Unit =
      ...
      then builder.append(ClassfileConstants.VOID_TAG)
      else jsig(finalType)
    ...
    def fullNameInSig(sym: Symbol): Unit =
      assert(sym.isClass)
      val name = atPhase(genBCodePhase) {
        sanitizeName(sym.fullName).replace('.', '/')
      }
    ...
    def classSig(
      ...
      def argSig(tp: Type): Unit =
        tp match
          case bounds: TypeBounds =>
            if !(defn.AnyType <:< bounds.hi) then
              builder.append('+')
              boxedSig(bounds.hi)
            else if !(bounds.lo <:< defn.NothingType) then
              builder.append('-')
              boxedSig(bounds.lo)
            else builder.append('*')
      ...
      if pre.exists then
        ...
        if needsJavaSig(preRebound, Nil) then
          val i = builder.length()
          jsig(preRebound)
          if builder.charAt(i) == 'L' then
            ...
            if preRebound.typeSymbol.is(ModuleClass) then
            ...
            val delimiter =
              if builder.charAt(builder.length() - 1) == '>' then '.' else '$'
    ...
    def jsig(
      ...
      tp match
        ...
        case defn.ArrayOf(elemtp) =>
          if isGenericArrayElement(elemtp, isScala2 = false) then
        ...
        case RefOrAppliedType(sym, pre, args) =>
          ...
          then jsig(defn.TupleXXLClass.typeRef)
          else if isTypeParameterInSig(sym, sym0) then
            assert(!sym.isAliasType, "Unexpected alias type: " + sym)
            typeParamSig(sym.name.lastPart)
          else if defn.specialErasure.contains(sym) then
            jsig(defn.specialErasure(sym).nn.typeRef)
          else if sym == defn.UnitClass || sym == defn.BoxedUnitModule then
            jsig(defn.BoxedUnitClass.typeRef)
          else if sym == defn.NothingClass then
            builder.append("Lscala/runtime/Nothing$;")
          else if sym == defn.NullClass then
            builder.append("Lscala/runtime/Null$;")
          else if sym.isPrimitiveValueClass then
            if !primitiveOK then jsig(defn.ObjectType)
            else if sym == defn.UnitClass then jsig(defn.BoxedUnitClass.typeRef)
            else builder.append(defn.typeTag(sym.info))
          else if ValueClasses.isDerivedValueClass(sym) then
            val erasedUnderlying = fullErasure(tp)
            if erasedUnderlying.isPrimitiveValueType && !primitiveOK then
              classSig(sym, pre, args)
            else jsig(erasedUnderlying, toplevel, primitiveOK)
          else if defn.isSyntheticFunctionClass(sym) then
            val erasedSym = defn.functionTypeErasure(sym).typeSymbol
            classSig(
              erasedSym,
              pre,
              if erasedSym.typeParams.isEmpty then Nil else args
            )
          else if sym.isClass then classSig(sym, pre, args)
          else jsig(erasure(tp), toplevel, primitiveOK)
        ...
        case PolyType(tparams, mtpe: MethodType) =>
          ...
          if toplevel && !sym0.isConstructor then polyParamSig(tparams)
          jsig(mtpe)

        // Nullary polymorphic method
        case PolyType(tparams, restpe) =>
          assert(tparams.nonEmpty)
          if toplevel then polyParamSig(tparams)
          builder.append("()")
          methodResultSig(restpe)

        case mtpe: MethodType =>
          // erased method parameters do not make it to the bytecode.
          def effectiveParamInfoss(t: Type)(using Context): List[List[Type]] =
            t match
              case t: MethodType if t.hasErasedParams =>
                t.paramInfos.zip(t.erasedParams).collect { case (i, false) =>
                  i
                }
                  :: effectiveParamInfoss(t.resType)
              case t: MethodType =>
                t.paramInfos :: effectiveParamInfoss(t.resType)
              case _ => Nil
        ...
        case tp: AndType =>
          ...
          val repr =
            reprParents
              .find(_.typeSymbol.is(TypeParam))
              .getOrElse(reprParents.head)
    ...
    while rest.nonEmpty do
      ...
      if required then
        leaves = leaves filter { t =>
          val ts = t.typeSymbol
          !(ts.is(Trait) || ts.is(PureInterface)) || !candidateSym.isSubClass(
            ts
          )
  ...
  private def isTypeParameterInSig(sym: Symbol, initialSymbol: Symbol)(using
      Context
  ) =
  ...
  private def rebindInnerClass(pre: Type, cls: Symbol)(using Context): Type =
    val owner = cls.owner
    if owner.is(PackageClass) || owner.isTerm then pre
    else cls.owner.info /* .tpe_* */

  private object RefOrAppliedType:
    def unapply(tp: Type)(using Context): Option[(Symbol, Type, List[Type])] =
      tp match
  ...
  private def needsJavaSig(tp: Type, throwsArgs: List[Type])(using
      Context
  ): Boolean = !ctx.settings.YnoGenericSig.value && {
    def needs(tp: Type) = (new NeedsSigCollector).apply(false, tp)
    needs(tp) || throwsArgs.exists(needs)
  }

  private class NeedsSigCollector(using Context)
      ...
      if !x then
        tp.dealias match
          case RefinedType(parent, refinedName, refinedInfo) =>
            ...
            if sym == defn.ArrayClass then foldOver(x, refinedInfo)
            else true
          case tref @ TypeRef(pre, name) =>
            val sym = tref.typeSymbol
            if sym.is(TypeParam) || sym.typeParams.nonEmpty then true
            else if sym.isClass then
              foldOver(x, rebindInnerClass(pre, sym)) // #2585
            else foldOver(x, pre)
          case PolyType(_, _) =>
            true
