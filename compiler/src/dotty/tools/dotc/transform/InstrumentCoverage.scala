...
/** Implements code coverage by inserting calls to
  * scala.runtime.coverage.Invoker ("instruments" the source code). The result
  * can then be consumed by the Scoverage tool.
  */
class InstrumentCoverage extends MacroTransform with IdentityDenotTransformer:
  ...
  /** Transforms trees to insert calls to Invoker.invoked to compute the
    * coverage when the code is called
    */
  private class CoverageTransformer(outputPath: String) extends Transformer:
    ...
    /** Records information about a new coverable statement. Generates a unique
      * id for it.
      *
      * @param tree
      *   the tree to add to the coverage report
      * @param pos
      *   the position to save in the report
      * @param branch
      *   true if it's a branch (branches are considered differently by most
      ...
      *   the statement's id
      */
    private def recordStatement(
        tree: Tree,
        pos: SourcePosition,
        branch: Boolean
    )(using ctx: Context): Int =
      val id = statementId
      statementId += 1
    ...
    /** Adds a new statement to the current `Coverage` and creates a
      * corresponding call to `Invoker.invoke` with its id, and the given
      * position.
      *
      * Note that the entire tree won't be saved in the coverage analysis, only
      * some data related to the tree is recorded (e.g. its type, its parent
      * class, ...).
      *
      * @param tree
      *   the tree to add to the coverage report
      ...
      *   the position to save in the report
      * @param branch
      *   true if it's a branch
      * @return
      *   the tree corresponding to the call to `Invoker.invoke`
      */
    private def createInvokeCall(
        tree: Tree,
        pos: SourcePosition,
    ...
    /** Tries to instrument an `Apply`. These "tryInstrument" methods are useful
      * to tweak the generation of coverage instrumentation, in particular in
      * `case TypeApply` in the [[transform]] method.
      *
      * @param tree
      *   the tree to instrument
      * @return
      *   instrumentation result, with the preparation statement, coverage call
      *   and tree separated
      */
    private def tryInstrument(tree: Apply)(using Context): InstrumentedParts =
      if canInstrumentApply(tree) then
        ...
        if needsLift(tree) then
          ...
          val app =
            cpy.Apply(tree)(transform(tree.fun), tree.args.map(transform))
    ...
    private def tryInstrument(tree: Ident)(using Context): InstrumentedParts =
      ...
      else InstrumentedParts.notCovered(tree)
    ...
    private def tryInstrument(tree: Select)(using Context): InstrumentedParts =
      ...
      if canInstrumentParameterless(sym) then
        ...
        val coverageCall = createInvokeCall(tree, tree.sourcePos)
        InstrumentedParts.singleExpr(coverageCall, transformed)
      else InstrumentedParts.notCovered(transformed)
    ...
    /** Transforms and instruments a branch if it's non-empty. If the tree is
      * empty, return itself and don't instrument.
      */
    ...
    override def transform(tree: Tree)(using Context): Tree =
      inContext(transformCtx(tree)) { // necessary to position inlined code properly
        tree match
          ...
          case tree if tree.isEmpty || tree.isType =>
            tree // empty Thicket, Ident (referring to a type), TypeTree, ...
          case tree if !tree.span.exists || tree.span.isZeroExtent =>
            tree // no meaningful position
          ...
          case tree: Template =>
            ...
            cpy.Template(tree)(
              ...
              transformTemplateParents(tree.parents)(using
                ctx.superCallContext
              ),
              tree.derived,
              tree.self,
    ...
    /** Transforms a `def lhs = rhs` and instruments its body (rhs).
      ...
      * (never outside of the DefDef tree). Therefore, this method always
      * returns a `DefDef`. Thanks to this, it doesn't need to be wrapped in
      * an`InstrumentedParts`.
      */
    private def transformDefDef(tree: DefDef)(using Context): DefDef =
      ...
      if sym.isOneOf(Inline | Erased) then
        ...
        val transformedRhs =
          if tree.rhs.isEmpty then tree.rhs
          else if sym.isClassConstructor then instrumentSecondaryCtor(tree)
    ...
    private def transformCaseDef(tree: CaseDef)(using Context): CaseDef =
      ...
      val friendlyEnd =
        if guard.span.exists then guard.span.end else pat.span.end
      val pos = tree.sourcePos.withSpan(
        tree.span.withEnd(friendlyEnd)
      ) // user-friendly span
      ...
      val instrumentedBody =
        InstrumentedParts.singleExprTree(coverageCall, transformedBody)

      cpy.CaseDef(tree)(pat, transformedGuard, instrumentedBody)

    /** Transforms the parents of a Template. */
    private def transformTemplateParents(parents: List[Tree])(using
        Context
    ): List[Tree] =
    ...
    /** Instruments the body of a DefDef. Handles corner cases. Given a DefDef f
      * like this:
      * ```
      * def f(params) = rhs
      * ```
      ...
      * ```
      *
      * But in some cases (e.g. closures), this would be invalid (see the
      * comment below), and the call is inserted at another place.
      */
    private def instrumentBody(parent: DefDef, body: Tree)(using
        Context
    ): Tree =
      ...
      body match
        case b @ Block((meth: DefDef) :: Nil, closure: Closure)
            if meth.symbol == closure.meth.symbol && defn.isContextFunctionType(
              body.tpe
            ) =>
    ...
    /** Instruments the body of a secondary constructor DefDef.
      ...
      * the rhs Block, otherwise `HoistSuperArgs` will not be happy (see
      * #17042).
      */
    private def instrumentSecondaryCtor(ctorDef: DefDef)(using Context): Tree =
      ...
      ctorDef.rhs match
        case b @ Block(delegateCtorCall :: stats, expr: Literal) =>
          cpy.Block(b)(
            transform(delegateCtorCall) :: coverageCall :: stats.mapConserve(
              transform
            ),
            expr
          )
    ...
    /** Checks if the apply needs a lift in the coverage phase. In case of a
      * nested application, we have to lift all arguments Example:
      * ```
      * def T(x:Int)(y:Int)
      * T(f())(1)
      * ```
      * should not be changed to {val $x = f(); T($x)}(1) but to {val $x = f();
      * val $y = 1; T($x)($y)}
      */
    private def needsLift(tree: Apply)(using Context): Boolean =
      ...
      def isUnliftableFun(fun: Tree) =
         ...
         */
        val sym = fun.symbol
        sym.exists && (isShortCircuitedOp(sym) || StringInterpolatorOpt
          .isCompilerIntrinsic(sym))
      end
      ...
      !isUnliftableFun(fun) && !tree.args.isEmpty && !tree.args.forall(
        LiftCoverage.noLift
      )
    end needsLift

    /** Check if an Apply can be instrumented. Prevents this phase from
    ...
    end canInstrumentApply

    /** Is this the symbol of a parameterless method that we can instrument?
      * Note: it is crucial that `asInstanceOf` and `isInstanceOf`, among
      * others, do NOT get instrumented, because that would generate invalid
      * code and crash in post-erasure checking.
      */
    private def canInstrumentParameterless(sym: Symbol)(using
        Context
    ): Boolean =
    ...
    /** Does sym refer to a "compiler intrinsic" method, which only exist during
      * compilation, like Any.isInstanceOf? If this returns true, the call
      * souldn't be instrumented.
      */
...
object InstrumentCoverage:
  ...
  /** An instrumented Tree, in 3 parts.
    ...
    */
  case class InstrumentedParts(
      pre: List[Tree],
      invokeCall: Apply | EmptyTree.type,
      expr: Tree
  ):
    require(
      pre.isEmpty || (pre.nonEmpty && !invokeCall.isEmpty),
      "if pre isn't empty then invokeCall shouldn't be empty"
    )
  ...
  object InstrumentedParts:
    ...
    def singleExpr(invokeCall: Apply, expr: Tree) =
      InstrumentedParts(Nil, invokeCall, expr)
