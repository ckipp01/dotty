...
/** This phase inlines calls to methods of value classes.
  *
  * A value class V after [[ExtensionMethods]] will look like: class V[A, B,
  * ...](val underlying: U) extends AnyVal { def foo[T, S, ...](arg1: A1, arg2:
  * A2, ...) = V.foo$extension[T, S, ..., A, B, ...](this)(arg1, arg2, ...)
  *
  * ... }
  *
  * Let e have type V, if e is a stable prefix or if V does not have any class
  * type parameter, then we can rewrite: e.foo[X, Y, ...](args) as:
  * V.foo$extension[X, Y, ..., A', B', ...](e)(args) where A', B', ... are the
  * class type parameters A, B, ... as seen from `e`. Otherwise, we need to
  * evaluate e first: { val ev = e V.foo$extension[X, Y, ..., A', B',
  * ...](ev)(args) }
  *
...
class VCInlineMethods extends MiniPhase with IdentityDenotTransformer:
  ...
  /** Replace a value class method call by a call to the corresponding extension
    * method.
    *
    * @param tree
    *   The tree corresponding to the method call
    * @param mtArgs
    *   Type arguments for the method call not present in `tree`
    * @param mArgss
    *   Arguments for the method call not present in `tree`
    * @return
    *   A tree for the extension method call
    */
  private def rewire(
      tree: Tree,
      mtArgs: List[Tree] = Nil,
      mArgss: List[List[Tree]] = Nil
  )(using Context): Tree =
    tree match
  ...
  /** If this tree corresponds to a fully-applied value class method call,
    * replace it by a call to the corresponding extension method, otherwise
    * return it as is.
    */
