...
/** This phase rewrites calls to `Array.apply` to a direct instantiation of the
  * array in the bytecode.
  *
  * Transforms `scala.Array.apply([....])` and `scala.Array.apply(..., [....])`
  * into `[...]`
  */
class ArrayApply extends MiniPhase:
  ...
  override def transformApply(tree: tpd.Apply)(using Context): tpd.Tree =
    if isArrayModuleApply(tree.symbol) then
      tree.args match
        case StripAscription(
              Apply(wrapRefArrayMeth, (seqLit: tpd.JavaSeqLiteral) :: Nil)
            ) :: ct :: Nil
            if defn
              .WrapArrayMethods()
              .contains(wrapRefArrayMeth.symbol) && elideClassTag(ct) =>
          seqLit

        case elem0 :: StripAscription(
              Apply(wrapRefArrayMeth, (seqLit: tpd.JavaSeqLiteral) :: Nil)
            ) :: Nil
  ...
  private def isArrayModuleApply(sym: Symbol)(using Context): Boolean =
    sym.name == nme.apply
      && (sym.owner == defn.ArrayModuleClass || (sym.owner == defn.IArrayModuleClass && !sym
        .is(Extension)))
  ...
  private def elideClassTag(ct: Tree)(using Context): Boolean = ct match
    case Apply(_, rc :: Nil) if ct.symbol == defn.ClassTagModule_apply =>
      rc match
        ...
        case _ => false
    case Apply(ctm: RefTree, _)
        if ctm.symbol.maybeOwner.companionModule == defn.ClassTagModule =>
      // ClassTag.XYZ
