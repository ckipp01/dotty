    ...
    ref match
      ...
      case ref: SymDenotation if sym.containsOpaques =>
        def stripOpaqueRefinements(tp: Type): Type = tp match
          case RefinedType(parent, rname, TypeAlias(_))
              if ref.info.decl(rname).symbol.isOpaqueAlias =>
            stripOpaqueRefinements(parent)
          case _ => tp
      ...
      case ref: NonSymSingleDenotation if sym.isOpaqueAlias =>
        ref.derivedSingleDenotation(
          sym,
          TypeAlias(sym.opaqueAlias.asSeenFrom(ref.prefix, sym.owner))
        )
      case _ =>
        ref

  /** Resolve overloading of `==` and `!=` methods with the representation types
    ...
    if sym == defn.Any_== || sym == defn.Any_!= then
      tree match
        case Apply(Select(receiver, name: TermName), args)
            if atPhase(thisPhase)(
              receiver.tpe.widenDealias.typeSymbol.isOpaqueAlias
            ) =>
