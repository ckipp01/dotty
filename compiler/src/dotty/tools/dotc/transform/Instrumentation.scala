...
/** The phase is enabled if the -Yinstrument option is set. If enabled, it
  * counts the number of closures or allocations for each source position. It
  * does this by generating a call to dotty.tools.dotc.util.Stats.doRecord.
  */
class Instrumentation extends MiniPhase:
  ...
  private def recordSize(tree: Apply)(using Context): Tree = tree.fun match
    case sel @ Select(qual, name)
      ...
      val key = Literal(
        Constant(
          s"totalSize/${name} in ${qual.tpe.widen.classSymbol.name}@${tree.sourcePos.show}"
        )
      )
      val qual1 =
        ref(Stats_doRecordSize).appliedTo(key, qual).cast(qual.tpe.widen)
  ...
  override def transformDefDef(tree: DefDef)(using Context): Tree =
    ...
    then
      ...
      def rhs1 = tree.rhs match
        ...
        case _: Match | _: If | _: Try | _: Labeled =>
          cpy.Block(tree.rhs)(icall :: Nil, tree.rhs)
        case rhs => rhs
  ...
  override def transformApply(tree: Apply)(using Context): Tree = tree.fun match
    ...
    case ref: RefTree if namesToRecord.contains(ref.name) && ok =>
      cpy.Block(tree)(
        record(i"call/${ref.name}", tree) :: Nil,
        recordSize(tree)
      )
    case _ =>
      tree

  override def transformBlock(tree: Block)(using Context): Block =
    tree.expr match
      case _: Closure =>
