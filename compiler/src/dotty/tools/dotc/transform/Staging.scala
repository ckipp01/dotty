...
/** Checks that staging level consistency holds and heals types used in higher
  * levels.
  *
...
class Staging extends MacroTransform:
  ...
  override def checkPostCondition(tree: Tree)(using Context): Unit =
    if ctx.phase <= stagingPhase then
      ...
      tree match
        case PackageDef(pid, _) if tree.symbol.owner == defn.RootClass =>
          val checker = new CrossStageSafety:
            override protected def healType(pos: SrcPos)(tpe: Type)(using
                Context
            ) = new HealType(pos):
              override protected def tryHeal(tp: TypeRef): TypeRef =
                val sym = tp.symbol
                def symStr =
                  if sym.is(ModuleClass) then sym.sourceModule.show
                  else i"${sym.name}.this"
                val errMsg = s"\nin ${ctx.owner.fullName}"
                assert(
                  ctx.owner.hasAnnotation(
    ...
    if !Inlines.inInlineMethod then
      tree match
        case tree: RefTree =>
          assert(
            level != 0 || tree.symbol != defn.QuotedTypeModule_of,
      ...
      tree.tpe match
        case tpe @ TypeRef(prefix, _) if tpe.typeSymbol.isTypeSplice =>
          ...
          assert(
            prefix.isInstanceOf[TermRef] || prefix.isInstanceOf[ThisType],
            prefix
          )
        case _ =>
        // OK
