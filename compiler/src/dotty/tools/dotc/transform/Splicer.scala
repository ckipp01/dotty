...
object Splicer:
  ...
  /** Splice the Tree for a Quoted expression. `${'{xyz}}` becomes `xyz` and for
    * `$xyz` the tree of `xyz` is interpreted for which the resulting expression
  ...
  def splice(
      tree: Tree,
      splicePos: SrcPos,
      spliceExpansionPos: SrcPos,
      classLoader: ClassLoader
  )(using Context): Tree = tree match
    case Quote(quotedTree, Nil) => quotedTree
    case _ =>
      val macroOwner = newSymbol(
        ctx.owner,
        nme.MACROkw,
        Macro | Synthetic,
        defn.AnyType,
      ...
      try
        val sliceContext = SpliceScope
          .contextWithNewSpliceScope(splicePos.sourcePos)
          .withOwner(macroOwner)
        ...
        case NonFatal(ex) =>
          val msg =
            em"""Failed to evaluate macro.
                ...
                |    ${ex.getStackTrace
                .takeWhile(
                  _.getClassName != "dotty.tools.dotc.transform.Splicer$"
                )
                .drop(1)
                .mkString("\n    ")}
              """
  ...
  /** Checks that no symbol that was generated within the macro expansion has an
    * out of scope reference
    */
  def checkEscapedVariables(tree: Tree, expansionOwner: Symbol)(using
      Context
  ): tree.type =
    new TreeTraverser:
      ...
      def traverse(tree: Tree)(using Context): Unit =
        ...
        tree match
          case tree: Ident if isEscapedVariable(tree.symbol) =>
            val sym = tree.symbol
            report.error(
              em"While expanding a macro, a reference to $sym was used outside the scope where it was defined",
              tree.srcPos
            )
          case Block(stats, _) =>
            val last = locals
      ...
      .traverse(tree)
    tree
  end checkEscapedVariables

  /** Check that the Tree can be spliced. `${'{xyz}}` becomes `xyz` and for
    * `$xyz` the tree of `xyz` is interpreted for which the resulting expression
    * is returned as a `Tree`
    *
    * See: `Staging`
  ...
  def checkValidMacroBody(tree: Tree)(using Context): Unit = tree match
    ...
    case _ =>
      ...
      def checkIfValidArgument(tree: Tree)(using Env): Unit = tree match
        ...
        case Apply(Select(Quote(body, _), nme.apply), _) =>
          val noSpliceChecker = new TreeTraverser:
            def traverse(tree: Tree)(using Context): Unit = tree match
              case Splice(_) =>
                report.error(
                  "Quoted argument of macros may not have splices",
                  tree.srcPos
                )
              case _ =>
                traverseChildren(tree)
        ...
        case Apply(TypeApply(fn, List(quoted)), _)
            if fn.symbol == defn.QuotedTypeModule_of =>
        // OK
        ...
        case tree: Ident
            if summon[Env].contains(tree.symbol) || tree.symbol.is(
              Inline,
              butNot = Method
            ) =>
        // OK
      ...
      def checkIfValidStaticCall(tree: Tree)(using Env): Unit = tree match
        case closureDef(ddef @ DefDef(_, ValDefs(ev :: Nil) :: Nil, _, _))
            if ddef.symbol.info.isContextualMethod =>
          checkIfValidStaticCall(ddef.rhs)(using summon[Env] + ev.symbol)

        case Block(stats, expr) =>
          val newEnv = stats.foldLeft(summon[Env])((env, stat) =>
            checkValidStat(stat)(using env)
          )
        ...
        case Call(fn, args)
            if (fn.symbol.isConstructor && fn.symbol.owner.owner.is(Package)) ||
              ...
              (fn.qualifier.symbol.is(
                Module
              ) && fn.qualifier.symbol.isStatic) =>
          if fn.symbol.flags.is(Inline) then
            report.error(
  ...
  /** Tree interpreter that evaluates the tree. Interpreter is assumed to start
    * at quotation level -1.
    */
  private class SpliceInterpreter(pos: SrcPos, classLoader: ClassLoader)(using
      Context
  ) extends Interpreter(pos, classLoader):

    override protected def interpretTree(tree: Tree)(implicit
        env: Env
    ): Object = tree match
      ...
      case Apply(Select(Quote(body, _), nme.apply), _) =>
        ...
        new ExprImpl(
          Inlined(
            EmptyTree,
            Nil,
            QuoteUtils.changeOwnerOfTree(body1, ctx.owner)
          ).withSpan(body1.span),
          SpliceScope.getCurrent
        )

      // Interpret level -1 `Type.of[T]`
      case Apply(TypeApply(fn, quoted :: Nil), _)
          if fn.symbol == defn.QuotedTypeModule_of =>
        new TypeImpl(
          QuoteUtils.changeOwnerOfTree(quoted, ctx.owner),
          SpliceScope.getCurrent
        )

      case _ =>
        super.interpretTree(tree)
  end SpliceInterpreter
end Splicer
