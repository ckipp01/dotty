...
object CountOuterAccesses:
  ...
  /** Characterizes outer accessors and outer fields that can be dropped if
    * there are no references to them from within the toplevel class where they
    * are defined.
    */
...
/** Counts number of accesses to outer accessors and outer fields of classes
  * that are visible only within one source file. The info is collected in
  * `outerAccessCount` and used in the subsequent DropOuterAccessors phase
  */
