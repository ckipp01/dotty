...
/** This phase erases ErasedValueType to their underlying type. It also removes
  * the synthetic cast methods u2evt$ and evt2u$ which are no longer needed
  * afterwards. Finally, it checks that we don't introduce "double definitions"
  * of pairs of methods that now have the same signature but were not considered
  * matching before erasure.
  */
class ElimErasedValueType extends MiniPhase with InfoTransformer:
  ...
  override def transformApply(tree: Apply)(using Context): Tree =
    ...
    // has been eliminated.
    val t =
      if fun.symbol.isValueClassConvertMethod then args.head
      else tree
  ...
  private def checkNoClashes(root: Symbol)(using Context) =
    ...
    def checkNoConflict(sym1: Symbol, sym2: Symbol, info: Type)(using
        Context
    ): Unit =
      ...
      then report.error(DoubleDefinition(sym1, sym2, root), root.srcPos)
