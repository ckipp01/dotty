...
/** Perform Step 1 in the inline classes SIP: Creates extension methods for all
  * methods in a value class, except parameter or super accessors, or
  * constructors.
  *
  * Additionally, for a value class V, let U be the underlying type after
  * erasure. We add to the companion module of V two cast methods: def
  * u2evt$(x0: U): ErasedValueType(V, U) def evt2u$(x0: ErasedValueType(V, U)):
  * U The casts are used in [[Erasure]] to make it typecheck, they are then
  * removed in [[ElimErasedValueType]]. This is different from the
  * implementation of value classes in Scala 2 (see SIP-15) which uses
  * `asInstanceOf` which does not typecheck.
  *
  * Finally, if the constructor of a value class is private pr protected it is
  * widened to public.
  *
...
class ExtensionMethods
  ...
  override def runsAfterGroupsOf: Set[String] = Set(
    FirstTransform.name
  ) // need companion objects to exist

  override def changesMembers: Boolean =
    true // the phase adds extension methods

  override def transform(ref: SingleDenotation)(using
      Context
  ): SingleDenotation = ref match
    case moduleClassSym: ClassDenotation if moduleClassSym.is(ModuleClass) =>
      moduleClassSym.linkedClass match
        case valueClass: ClassSymbol if isDerivedValueClass(valueClass) =>
          ...
          if !(valueClass.is(Scala2x)) then
            for decl <- valueClass.classInfo.decls do
              if isMethodWithExtension(decl) then
                enterInModuleClass(
                  createExtensionMethod(decl, moduleClassSym.symbol)
                )
          ...
          val u2evtSym = newSymbol(
            moduleSym,
            nme.U2EVT,
            Synthetic | Method,
            MethodType(List(nme.x_0), List(underlying), evt)
          )
          val evt2uSym = newSymbol(
            moduleSym,
            nme.EVT2U,
            Synthetic | Method,
    ...
    case ref: SymDenotation =>
      var ref1 = ref
      if isMethodWithExtension(ref.symbol) && ref.hasAnnotation(
          defn.TailrecAnnot
        )
      then
        ref1 = ref.copySymDenotation()
        ref1.removeAnnotation(defn.TailrecAnnot)
      else if ref.isConstructor && isDerivedValueClass(ref.owner) && ref
          .isOneOf(AccessFlags)
      then
        ref1 = ref.copySymDenotation()
      ...
      // that will be moved to the companion object.
      if ref.is(Local) && isDerivedValueClass(ref.owner) then
        if ref1 ne ref then ref1.resetFlag(Local)
      ...
      ref.info match
        case ClassInfo(pre, cls, _, _, _) if cls is ModuleClass =>
          cls.linkedClass match
            case valueClass: ClassSymbol if isDerivedValueClass(valueClass) =>
              val info1 = atPhase(ctx.phase.next)(cls.denot).asClass.classInfo
                .derivedClassInfo(prefix = pre)
              ref.derivedSingleDenotation(ref.symbol, info1)
            case _ => ref
        case _ => ref

  protected def rewiredTarget(target: Symbol, derived: Symbol)(using
      Context
  ): Symbol =
    if isMethodWithExtension(target) &&
      target.owner.linkedClass == derived.owner
    then extensionMethod(target)
    else NoSymbol

  private def createExtensionMethod(imeth: Symbol, staticClass: Symbol)(using
      Context
  ): TermSymbol =
    val extensionMeth = newSymbol(
      staticClass,
      extensionName(imeth),
      ...
      extensionDefs.remove(tree.symbol.owner) match
        case defns: mutable.ListBuffer[Tree] if defns.nonEmpty =>
          cpy.Template(tree)(body =
            tree.body ++ defns.map(transformFollowing(_))
          )
        case _ =>
          tree
    else tree
  ...
  override def transformDefDef(tree: tpd.DefDef)(using Context): tpd.Tree =
    if isMethodWithExtension(tree.symbol) then
      ...
      assert(
        staticClass.exists,
        s"$origClass lacks companion, ${origClass.owner.definedPeriodsString} ${origClass.owner.info.decls} ${origClass.owner.info.decls}"
      )
      val extensionMeth = extensionMethod(origMeth)
      report.log(
        s"Value class $origClass spawns extension method.\n  Old: ${origMeth.showDcl}\n  New: ${extensionMeth.showDcl}"
      )
      val store =
        extensionDefs.getOrElseUpdate(staticClass, new mutable.ListBuffer[Tree])
...
object ExtensionMethods:
  ...
  /** Name of the extension method that corresponds to given instance method
    * `meth`.
    */
  def extensionName(imeth: Symbol)(using Context): TermName =
    ExtMethName(imeth.name.asTermName match
      case BodyRetainerName(name) => name
      case name                   => name
    )

  /** Return the extension method that corresponds to given instance method
    * `meth`.
    */
  def extensionMethod(imeth: Symbol)(using Context): TermSymbol =
    atPhase(extensionMethodsPhase.next) {
      ...
      def matches(candidate: SingleDenotation) =
        FullParameterization.memberSignature(
          candidate.info
        ) == imeth.info.stripPoly.ensureMethodic.signature
      ...
      assert(
        ...
        i"""no extension method found for:
          ...
          | ${candidates
            .map(c =>
              s"${c.name}:${c.info.signature}:${FullParameterization
                  .memberSignature(c.info)}"
            )
            .mkString("\n")}"""
      )
      if matching.tail.nonEmpty then
        // this case will report a "have the same erasure" error later at erasure pahse
        report.log(i"mutiple extension methods match $imeth: ${candidates
            .map(c => i"${c.name}:${c.info}")}")
