...
/** A MegaPhase combines a number of mini-phases which are all executed in a
  * single tree traversal.
  *
  * This is an evolution of the previous "TreeTransformers.scala", which was
  * written by @DarkDimius and is described in his thesis.
  */
object MegaPhase:
  import ast.tpd.*

  /** The base class of tree transforms. For each kind of tree K, there are two
    * methods which can be overridden:
    *
    * prepareForK: return a new Context which gets passed to the node and its
    * children transformK // transform node of type K
    *
    * There are also prepare/transform hooks for
    *
    *   - Stats: to prepare/transform a statement sequence in a block, template,
    *     or package def,
    *   - Unit : to prepare/transform a whole compilation unit
    *   - Other: to prepape/transform a tree that does not have a specific
    *     prepare/transform method pair.
    */
  abstract class MiniPhase extends Phase:
    ...
    /** List of names of phases that should have finished their processing of
      * all compilation units before this phase starts
      */
    ...
    /** Transform tree using all transforms of current group (including this
      * one)
      */
    def transformAllDeep(tree: Tree)(using Context): Tree =
      superPhase.transformTree(tree, 0)

    /** Transform tree using all transforms following the current one in this
      * group
      */
    def transformFollowingDeep(tree: Tree)(using Context): Tree =
      superPhase.transformTree(tree, idxInGroup + 1)

    /** Transform single node using all transforms following the current one in
      * this group
      */
...
class MegaPhase(val miniPhases: Array[MiniPhase]) extends Phase:
  ...
  override val phaseName: String =
    if miniPhases.length == 1 then miniPhases(0).phaseName
  ...
  /** Transform node using all phases in this group that have idxInGroup >=
    * start
    */
    ...
    def goUnnamed(tree: Tree, start: Int) =
      try
        ...
        case ex: TypeError =>
          report.error(ex, tree.srcPos)
          tree
    if tree.isInstanceOf[NameTree] then goNamed(tree, start)
    else goUnnamed(tree, start)
  end transformNode

  /** Transform full tree using all phases in this group that have idxInGroup >=
    * start
    */
  def transformTree(tree: Tree, start: Int)(using Context): Tree =
    ...
    inline def inLocalContext[T](inline op: Context ?=> T)(using Context): T =
      val sym = tree.symbol
      runWithOwner(if sym.is(PackageVal) then sym.moduleClass else sym)(op)

    def transformNamed(tree: Tree, start: Int, outerCtx: Context): Tree =
      tree match
        case tree: Ident =>
        ...
        case tree: ValDef =>
          inContext(prepValDef(tree, start)(using outerCtx)) {
            ...
            if tree.isEmpty then tree
              ...
              goValDef(
                if tree.symbol.exists then inLocalContext(mapValDef)
                else mapValDef,
                start
              )
          }
          ...
          inContext(prepDefDef(tree, start)(using outerCtx)) {
            def mapDefDef(using Context) =
              val paramss = tree.paramss
                .mapConserve(transformSpecificTrees(_, start))
    ...
    def transformUnnamed(tree: Tree, start: Int, outerCtx: Context): Tree =
      tree match
        case tree: Apply =>
        ...
        case tree: Template =>
          inContext(prepTemplate(tree, start)(using outerCtx)) {
            ...
            val parents =
              transformTrees(tree.parents, start)(using ctx.superCallContext)
        ...
        case tree: Inlined =>
          inContext(prepInlined(tree, start)(using outerCtx)) {
            ...
            val expansion = transformTree(tree.expansion, start)(using
              inlineContext(tree.call)
            )
    ...
    else if tree.isInstanceOf[NameTree] then transformNamed(tree, start, ctx)
  ...
  def transformStats(trees: List[Tree], exprOwner: Symbol, start: Int)(using
      Context
  ): List[Tree] =
  ...
  def transformBlock(tree: Block, start: Int)(using Context): Tree =
    ...
    val block1 = tree.stats.mapStatements(
      ctx.owner,
  ...
  def transformSpecificTrees[T <: Tree](trees: List[T], start: Int)(using
      Context
  ): List[T] =
  ...
  private val clsMethodsCache = new java.util.IdentityHashMap[Class[?], Array[
    java.lang.reflect.Method | Null
  ]]

  /** Does `phase` contain a redefinition of method `name`? (which is a method
    * of MiniPhase)
    */
  ...
  private def init(methName: String): Array[MiniPhase | Null] =
    ...
    for idx <- miniPhases.length - 1 to 0 by -1 do
      val subPhase = miniPhases(idx)
      if defines(subPhase, methName) then
        if nx eq emptyNxArray then nx = newNxArray
        nx(idx) = subPhase
      else if nx ne emptyNxArray then nx(idx) = nx(idx + 1)
    nx
  ...
  for (phase, idx) <- miniPhases.zipWithIndex do
  ...
  def prepIdent(tree: Ident, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
    else
      prepIdent(tree, phase.idxInGroup + 1)(using phase.prepareForIdent(tree))

  def goIdent(tree: Ident, start: Int)(using Context): Tree =
    val phase = nxIdentTransPhase(start)
    if phase == null then tree
    else
      phase.transformIdent(tree) match
  ...
  def prepSelect(tree: Select, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepSelect(tree, phase.idxInGroup + 1)(using phase.prepareForSelect(tree))

  def goSelect(tree: Select, start: Int)(using Context): Tree =
    val phase = nxSelectTransPhase(start)
    if phase == null then tree
    else
      phase.transformSelect(tree) match
  ...
  def goThis(tree: This, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformThis(tree) match
  ...
  def prepSuper(tree: Super, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepSuper(tree, phase.idxInGroup + 1)(using phase.prepareForSuper(tree))

  def goSuper(tree: Super, start: Int)(using Context): Tree =
    val phase = nxSuperTransPhase(start)
    if phase == null then tree
    else
      phase.transformSuper(tree) match
  ...
  def prepApply(tree: Apply, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepApply(tree, phase.idxInGroup + 1)(using phase.prepareForApply(tree))

  def goApply(tree: Apply, start: Int)(using Context): Tree =
    val phase = nxApplyTransPhase(start)
    if phase == null then tree
    else
      phase.transformApply(tree) match
  ...
  def prepTypeApply(tree: TypeApply, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepTypeApply(tree, phase.idxInGroup + 1)(using
        phase.prepareForTypeApply(tree)
      )
  ...
  def goTypeApply(tree: TypeApply, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformTypeApply(tree) match
  ...
  def prepLiteral(tree: Literal, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepLiteral(tree, phase.idxInGroup + 1)(using
        phase.prepareForLiteral(tree)
      )
  ...
  def goLiteral(tree: Literal, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformLiteral(tree) match
  ...
  def prepTyped(tree: Typed, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepTyped(tree, phase.idxInGroup + 1)(using phase.prepareForTyped(tree))

  def goTyped(tree: Typed, start: Int)(using Context): Tree =
    val phase = nxTypedTransPhase(start)
    if phase == null then tree
    else
      phase.transformTyped(tree) match
  ...
  def prepAssign(tree: Assign, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepAssign(tree, phase.idxInGroup + 1)(using phase.prepareForAssign(tree))

  def goAssign(tree: Assign, start: Int)(using Context): Tree =
    val phase = nxAssignTransPhase(start)
    if phase == null then tree
    else
      phase.transformAssign(tree) match
  ...
  def prepBlock(tree: Block, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepBlock(tree, phase.idxInGroup + 1)(using phase.prepareForBlock(tree))

  def goBlock(tree: Block, start: Int)(using Context): Tree =
    val phase = nxBlockTransPhase(start)
    if phase == null then tree
    else
      phase.transformBlock(tree) match
  ...
  def prepClosure(tree: Closure, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepClosure(tree, phase.idxInGroup + 1)(using
        phase.prepareForClosure(tree)
      )
  ...
  def goClosure(tree: Closure, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformClosure(tree) match
  ...
  def prepMatch(tree: Match, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepMatch(tree, phase.idxInGroup + 1)(using phase.prepareForMatch(tree))

  def goMatch(tree: Match, start: Int)(using Context): Tree =
    val phase = nxMatchTransPhase(start)
    if phase == null then tree
    else
      phase.transformMatch(tree) match
  ...
  def prepCaseDef(tree: CaseDef, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepCaseDef(tree, phase.idxInGroup + 1)(using
        phase.prepareForCaseDef(tree)
      )
  ...
  def goCaseDef(tree: CaseDef, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformCaseDef(tree) match
  ...
  def prepLabeled(tree: Labeled, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepLabeled(tree, phase.idxInGroup + 1)(using
        phase.prepareForLabeled(tree)
      )
  ...
  def goLabeled(tree: Labeled, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformLabeled(tree) match
  ...
  def prepReturn(tree: Return, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepReturn(tree, phase.idxInGroup + 1)(using phase.prepareForReturn(tree))

  def goReturn(tree: Return, start: Int)(using Context): Tree =
    val phase = nxReturnTransPhase(start)
    if phase == null then tree
    else
      phase.transformReturn(tree) match
  ...
  def prepWhileDo(tree: WhileDo, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepWhileDo(tree, phase.idxInGroup + 1)(using
        phase.prepareForWhileDo(tree)
      )
  ...
  def goWhileDo(tree: WhileDo, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformWhileDo(tree) match
  ...
  def prepSeqLiteral(tree: SeqLiteral, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepSeqLiteral(tree, phase.idxInGroup + 1)(using
        phase.prepareForSeqLiteral(tree)
      )
  ...
  def goSeqLiteral(tree: SeqLiteral, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformSeqLiteral(tree) match
  ...
  def prepInlined(tree: Inlined, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepInlined(tree, phase.idxInGroup + 1)(using
        phase.prepareForInlined(tree)
      )
  ...
  def goInlined(tree: Inlined, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformInlined(tree) match
  ...
  def prepQuote(tree: Quote, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepQuote(tree, phase.idxInGroup + 1)(using phase.prepareForQuote(tree))

  def goQuote(tree: Quote, start: Int)(using Context): Tree =
    val phase = nxQuoteTransPhase(start)
    if phase == null then tree
    else
      phase.transformQuote(tree) match
  ...
  def prepSplice(tree: Splice, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepSplice(tree, phase.idxInGroup + 1)(using phase.prepareForSplice(tree))

  def goSplice(tree: Splice, start: Int)(using Context): Tree =
    val phase = nxSpliceTransPhase(start)
    if phase == null then tree
    else
      phase.transformSplice(tree) match
  ...
  def prepTypeTree(tree: TypeTree, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepTypeTree(tree, phase.idxInGroup + 1)(using
        phase.prepareForTypeTree(tree)
      )
  ...
  def goTypeTree(tree: TypeTree, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformTypeTree(tree) match
  ...
  def goBind(tree: Bind, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformBind(tree) match
  ...
  def prepAlternative(tree: Alternative, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepAlternative(tree, phase.idxInGroup + 1)(using
        phase.prepareForAlternative(tree)
      )
  ...
  def goAlternative(tree: Alternative, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformAlternative(tree) match
  ...
  def prepUnApply(tree: UnApply, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepUnApply(tree, phase.idxInGroup + 1)(using
        phase.prepareForUnApply(tree)
      )
  ...
  def goUnApply(tree: UnApply, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformUnApply(tree) match
  ...
  def prepValDef(tree: ValDef, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepValDef(tree, phase.idxInGroup + 1)(using phase.prepareForValDef(tree))

  def goValDef(tree: ValDef, start: Int)(using Context): Tree =
    val phase = nxValDefTransPhase(start)
    if phase == null then tree
    else
      phase.transformValDef(tree) match
  ...
  def prepDefDef(tree: DefDef, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepDefDef(tree, phase.idxInGroup + 1)(using phase.prepareForDefDef(tree))

  def goDefDef(tree: DefDef, start: Int)(using Context): Tree =
    val phase = nxDefDefTransPhase(start)
    if phase == null then tree
    else
      phase.transformDefDef(tree) match
  ...
  def prepTypeDef(tree: TypeDef, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepTypeDef(tree, phase.idxInGroup + 1)(using
        phase.prepareForTypeDef(tree)
      )
  ...
  def goTypeDef(tree: TypeDef, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformTypeDef(tree) match
  ...
  def prepTemplate(tree: Template, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepTemplate(tree, phase.idxInGroup + 1)(using
        phase.prepareForTemplate(tree)
      )
  ...
  def goTemplate(tree: Template, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformTemplate(tree) match
  ...
  def prepPackageDef(tree: PackageDef, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepPackageDef(tree, phase.idxInGroup + 1)(using
        phase.prepareForPackageDef(tree)
      )
  ...
  def goPackageDef(tree: PackageDef, start: Int)(using Context): Tree =
    ...
    if phase == null then tree
      ...
      phase.transformPackageDef(tree) match
  ...
  def prepOther(tree: Tree, start: Int)(using Context): Context =
    ...
    if phase == null then ctx
      ...
      prepOther(tree, phase.idxInGroup + 1)(using phase.prepareForOther(tree))
