...
/** Removes `Select`s that would be compiled into `GetStatic`.
  ...
  * retained. As an exception, the qualifier is completely dropped if it is a
  * reference to a static owner (see `isStaticOwnerRef`). Concretely, this means
  * that in
  *
  * {{{
  *  object Foo {
...
class SelectStatic extends MiniPhase with IdentityDenotTransformer:
  ...
  override def transformSelect(tree: tpd.Select)(using Context): tpd.Tree =
    ...
    def isStaticMember =
      (sym is Flags.Module) && sym.initial.maybeOwner.initial.isStaticOwner ||
        ...
        sym.isScalaStatic
    val isStaticRef =
      !sym.is(Package) && !sym.maybeOwner.is(Package) && isStaticMember
    val tree1 =
      if isStaticRef && !tree.qualifier.symbol.isAllOf(
          JavaModule
        ) && !tree.qualifier.isType
      then
  ...
  private def isStaticOwnerRef(tree: Tree)(using Context): Boolean = tree match
    ...
    case Select(qual, _) =>
      isStaticOwnerRef(qual) && tree.symbol.is(
        Module
      ) && tree.symbol.moduleClass.isStaticOwner
    case _ =>
      false
  ...
  override def transformTypeApply(tree: tpd.TypeApply)(using
      Context
  ): tpd.Tree =
    normalize(tree)
