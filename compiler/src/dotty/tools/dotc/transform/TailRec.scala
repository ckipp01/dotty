...
/** A Tail Rec Transformer.
  ...
  * of a method. This includes being in tail-position of a `return` from a
  * `Labeled` block which is itself in tail-position (which is critical for
  * tail-recursive calls in the cases of a `match`). To identify tail positions,
  * we recurse over the trees that may contain calls in tail-position (trees
  * that can't contain such calls are not transformed).
  *
  * When a method contains at least one tail-recursive call, its rhs is wrapped
  ...
  * ```
  *
  * Self-recursive calls in tail-position are then replaced by (a) reassigning
  * the local `var`s substituting formal parameters and (b) a `return` from the
  * `tailResult` labeled block, which has the net effect of looping back to the
  * beginning of the method. If the receiver is modifed in a recursive call, an
  * additional `var` is used to replace `this`.
  *
  * As a complete example of the transformation, the classical `fact` function,
  * defined as:
  *
  * ```
  ...
  *
  * A method call is self-recursive if it calls the current method and the
  * method is final (otherwise, it could be a call to an overridden method in a
  * subclass). Recursive calls on a different instance are optimized.
  *
  * This phase has been moved after erasure to allow the use of vars for the
  * parameters combined with a `WhileDo`. This is also beneficial to support
  * polymorphic tail-recursive calls.
  *
  * In scalac, if the method had type parameters, the call must contain the same
  * parameters as type arguments. This is no longer the case in dotc thanks to
  * being located after erasure. In scalac, this is named tailCall but it does
  * only provide optimization for self recursive functions, that's why it's
  * renamed to tailrec
  *
  * @author
  *   Erik Stenman, Iulian Dragos, ported and heavily modified for dotty by
  *   Dmitry Petrashko moved after erasure and adapted to emit `Labeled` blocks
...
class TailRec extends MiniPhase:
  ...
  override def transformDefDef(tree: DefDef)(using Context): Tree =
    ...
    def noTailTransform(failureReported: Boolean) =
      ...
      if mandatory && !failureReported then
    ...
    if isCandidate then
      ...
      val transformer = new TailRecElimination(
        method,
        enclosingClass,
        tree.termParamss.head.map(_.symbol),
        mandatory
      )
      val rhsSemiTransformed = transformer.transform(tree.rhs)

      if transformer.rewrote then
        ...
        val initialVarDefs =
          val initialParamVarDefs =
            rewrittenParamSyms.lazyZip(varsForRewrittenParamSyms).map {
              (param, local) => ValDef(local.asTerm, ref(param))
            }
          varForRewrittenThis match
            case Some(local) =>
              ValDef(local.asTerm, This(enclosingClass)) :: initialParamVarDefs
          ...
          case Some(localThisSym) =>
            ...
            val substitute = new TreeTypeMap(
              typeMap = _.substThisUnlessStatic(enclosingClass, thisRef)
                .subst(
                  rewrittenParamSyms,
                  varsForRewrittenParamSyms.map(_.termRef)
                ),
              treeMap = {
            ...
            val cleanup = new TreeMap:
              override def transform(t: Tree)(using Context) = t match
                case Super(qual: Ident, mix)
                    if !qual.tpe.isInstanceOf[Types.ThisType] =>
        ...
        /** Is the RHS a direct recursive tailcall, possibly with swapped
          * arguments or modified pure arguments.
          * ```
          * def f(<params>): T = f(<args>)
          * ```
          * where `<args>` are pure arguments or references to parameters in
          * `<params>`.
          */
        def isInfiniteRecCall(tree: Tree): Boolean =
          def tailArgOrPureExpr(stat: Tree): Boolean = stat match
            case stat: ValDef
                if stat.name.is(TailTempName) || !stat.symbol.is(Mutable) =>
              tailArgOrPureExpr(stat.rhs)
            case Assign(lhs: Ident, rhs) if lhs.symbol.name.is(TailLocalName) =>
              tailArgOrPureExpr(rhs)
          ...
          tree match
            ...
            case Return(Literal(Constant(())), label) =>
              label.symbol == transformer.continueLabel
            case Block(stats, expr) =>
              stats.forall(tailArgOrPureExpr) && isInfiniteRecCall(expr)
            case _ => false
        ...
        cpy.DefDef(tree)(rhs =
          Block(
            ...
            WhileDo(
              EmptyTree, {
                Labeled(
                  transformer.continueLabel.asTerm, {
  ...
  class TailRecElimination(
    ...
    /** The `tailLabelN` label symbol, used to encode a `continue` from the
      * infinite `while` loop.
      */
    private var myContinueLabel: Symbol | Null = _
    def continueLabel(using Context): Symbol =
      if myContinueLabel == null then
        myContinueLabel =
          newSymbol(method, TailLabelName.fresh(), Label, defn.UnitType)
      myContinueLabel.uncheckedNN

    /** The local `var` that replaces `this`, if it is modified in at least one
      * recursive call.
      */
    var varForRewrittenThis: Option[Symbol] = None

    /** The subset of `paramSyms` that are modified in at least one recursive
      * call, and which therefore need a replacement `var`.
      */
    ...
    private def getVarForRewrittenThis()(using Context): Symbol =
      varForRewrittenThis match
        ...
        case none =>
          val tpe =
            if enclosingClass.is(Module) then enclosingClass.thisType
            else enclosingClass.classInfo.selfType
          val sym = newSymbol(
            method,
            TailLocalName.fresh(nme.SELF),
            Synthetic | Mutable,
            tpe
          )
    ...
    private def getVarForRewrittenParam(param: Symbol)(using Context): Symbol =
      rewrittenParamSyms.indexOf(param) match
        case -1 =>
          val sym = newSymbol(
            method,
            TailLocalName.fresh(param.name.toTermName),
            Synthetic | Mutable,
            param.info
          )
    ...
    override def transform(tree: Tree)(using Context): Tree =
      ...
      def rewriteApply(tree: Apply): Tree =
        ...
        def fail(reason: String) =
          if isMandatory then
            ...
            report.error(
              em"Cannot rewrite recursive call: $reason",
              tree.srcPos
            )
          else
            tailrec.println(
              "Cannot rewrite recursive call at: " + tree.span + " because: " + reason
            )
          continue
        ...
        if isRecursiveCall then
          if inTailPosition then
            ...
            yield (getVarForRewrittenParam(param), arg)
            ...
            val assignThisAndParamPairs = prefix match
              ...
              case _ =>
                (
                  getVarForRewrittenThis(),
                  noTailTransform(prefix)
                ) :: assignParamPairs
            ...
            val assignments = assignThisAndParamPairs match
              ...
              case _ :: _ =>
                val (tempValDefs, assigns) =
                  (for ((lhs, rhs) <- assignThisAndParamPairs) yield
                    val temp = newSymbol(
                      method,
                      TailTempName.fresh(lhs.name.toTermName),
                      Synthetic,
                      lhs.info
                    )
                    (
                      ValDef(temp, rhs),
                      Assign(ref(lhs), ref(temp)).withSpan(tree.span)
                    )
                  ).unzip
                tempValDefs ::: assigns
              case nil =>
            ...
            seq(
              assignments,
              Typed(Return(unitLiteral.withSpan(tree.span), continueLabel), tpt)
            )
          else fail("it is not in tail position")
        else if isRecursiveSuperCall then fail("it targets a supertype")
        else continue
        end if
      end rewriteApply
      ...
      tree match
        case tree @ Apply(fun, args) =>
          val meth = fun.symbol
          if meth == defn.Boolean_|| || meth == defn.Boolean_&& then
            cpy.Apply(tree)(noTailTransform(fun), transform(args))
          else rewriteApply(tree)
        ...
        case tree: Try =>
          ...
          if tree.finalizer eq EmptyTree then
        ...
        case tree: ValOrDefDef =>
          if isMandatory then noTailTransform(tree.rhs)
          tree

        case _: Super | _: This | _: Literal | _: TypeTree | _: TypeDef |
            EmptyTree =>
          tree

        case Labeled(bind, expr) =>
          if inTailPosition then tailPositionLabeledSyms += bind.symbol
        ...
        case Return(expr, from) =>
          ...
          val inTailPosition =
            !fromSym.is(Label) || tailPositionLabeledSyms.contains(fromSym)
