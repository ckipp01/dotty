...
class TupleOptimizations extends MiniPhase with IdentityDenotTransformer:
  ...
  override def transformApply(tree: tpd.Apply)(using Context): tpd.Tree =
    if !tree.symbol.exists || tree.symbol.owner != defn.RuntimeTuplesModuleClass
    then tree
    else if tree.symbol == defn.RuntimeTuples_cons then transformTupleCons(tree)
    else if tree.symbol == defn.RuntimeTuples_tail then transformTupleTail(tree)
    else if tree.symbol == defn.RuntimeTuples_size then transformTupleSize(tree)
    else if tree.symbol == defn.RuntimeTuples_concat then
      transformTupleConcat(tree)
    else if tree.symbol == defn.RuntimeTuples_apply then
      transformTupleApply(tree)
    else if tree.symbol == defn.RuntimeTuples_toArray then
      transformTupleToArray(tree)
    else tree
      ...
      case Some(tpes) =>
        ...
        if size <= 5 then
          ...
          val fullIterator = ref(defn.RuntimeTuples_consIterator)
            .appliedToTermArgs(head :: tail :: Nil)
  ...
  private def transformTupleTail(tree: tpd.Apply)(using Context): Tree =
    ...
    defn.tupleTypes(tup.tpe.widenTermRefExpr.dealias, MaxTupleArity + 1) match
      case Some(tpes) =>
        ...
        if size == 1 then
        ...
        else if size <= MaxTupleArity + 1 then
          // val it = this.asInstanceOf[Product].productIterator
          // it.next()
          // TupleN-1(it.next(), ..., it.next())
          evalOnce(
            tup
              .asInstance(defn.ProductClass.typeRef)
              .select(nme.productIterator)
          ) { it =>
            Block(
          ...
          tup
            .asInstance(defn.TupleXXLClass.typeRef)
            .select("tailXXL".toTermName)
      case None =>
        // No optimization, keep:
        // scala.runtime.Tuples.tail(tup)
        tree
    end match
  end transformTupleTail
  ...
  private def transformTupleConcat(tree: tpd.Apply)(using Context): Tree =
    ...
    (
      defn.tupleTypes(self.tpe.widenTermRefExpr.dealias),
      ...
      case (Some(tpes1), Some(tpes2)) =>
        ...
        if n == 0 then that
          ...
          val fullIterator =
            ref(defn.RuntimeTuples_concatIterator).appliedToTermArgs(tree.args)
          evalOnce(fullIterator) { it =>
  ...
  private def transformTupleApply(tree: tpd.Apply)(using Context): Tree =
    ...
    (defn.tupleTypes(tup.tpe.widenTermRefExpr.dealias), nTree.tpe) match
      case (Some(tpes), nTpe: ConstantType) =>
        ...
        if n < 0 || n >= size then
          report.error(
            em"index out of bounds: $n",
            nTree.underlyingArgument.srcPos
          )
          tree
        else if size <= MaxTupleArity then
          // tup._n
          ...
          tup
            .asInstance(defn.TupleXXLClass.typeRef)
            .select(nme.productElement)
            .appliedTo(Literal(nTpe.value))
      case (None, nTpe: ConstantType) if nTpe.value.intValue < 0 =>
        report.error(
          em"index out of bounds: ${nTpe.value.intValue}",
          nTree.srcPos
        )
        tree
      case _ =>
    ...
    defn.tupleTypes(tup.tpe.widen, MaxTupleArity) match
      case Some(tpes) =>
        ...
        if size == 0 then
          ...
          ref(defn.ArrayModule)
            .select("emptyObjectArray".toTermName)
            .ensureApplied
            .withSpan(tree.span)
        else if size <= MaxTupleArity then
          // scala.runtime.Tuples.productToArray(tup.asInstanceOf[Product])
          ref(defn.RuntimeTuples_productToArray).appliedTo(
            tup.asInstance(defn.ProductClass.typeRef)
          )
        else
  ...
  private def knownTupleFromElements(tpes: List[Type], elements: List[Tree])(
      using Context
  ) =
    ...
    ref(tupleModule)
      .select(nme.apply)
      .appliedToTypes(tpes)
      .appliedToTermArgs(elements)

  private def knownTupleFromIterator(size: Int, it: Tree)(using Context): Tree =
    if size == 0 then
      // EmptyTuple for empty tuple
      ref(
        defn.EmptyTupleModule.termRef
      ) // TODO should this code be here? Or assert(size > specializedSize)
    else if size <= MaxTupleArity then
