...
/** This phase adds forwarder for XXL functions `apply` methods that are
  * implemented with a method with explicit parameters (not in Array[Object]).
  *
  * In particular for every method `def apply(x1: T1, ... xn: Tn): R` in class
  * `M` subtype of `FunctionN[T1, ..., Tn, R]` with `N` > 22 a forwarder `def
  * apply(xs: Array[Object]): R = this.apply(xs(0).asInstanceOf[T1], ...,
  * xs(n-1).asInstanceOf[Tn]).asInstanceOf[R]` is generated.
  */
    ...
    val forwarders =
      for
        ...
        if ddef.name == nme.apply && ddef.symbol.is(Method) &&
          ...
          ddef.symbol.allOverriddenSymbols.exists(sym =>
            defn.isXXLFunctionClass(sym.owner)
          )
      yield
        val xsType = defn.ArrayType.appliedTo(List(defn.ObjectType))
        val methType =
          MethodType(List(nme.args))(_ => List(xsType), _ => defn.ObjectType)
        val meth =
          newSymbol(ddef.symbol.owner, nme.apply, Synthetic | Method, methType)
        DefDef(meth, paramss => forwarderRhs(ddef, paramss.head.head))

    cpy.Template(impl)(body = forwarders ::: impl.body)
  end transformTemplate
end FunctionXXLForwarders
