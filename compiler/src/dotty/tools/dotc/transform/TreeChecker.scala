...
/** Run by -Ycheck option after a given phase, this class retypes all syntax
  * trees and verifies that the type of each tree node so obtained conforms to
  * the type found in the tree node. It also performs the following checks:
  *
  *   - The owner of each definition is the same as the owner of the current
  *     typing context.
  *   - Ident nodes do not refer to a denotation that would need a select to be
  *     accessible (see tpd.needsSelect).
  *   - After typer, identifiers and select nodes refer to terms only (all types
  *     should be represented as TypeTrees then).
  */
class TreeChecker extends Phase with SymTransformer:
  ...
  def testDuplicate(
      sym: Symbol,
      registry: mutable.Map[String, Symbol],
      typ: String
  )(using Context): Unit =
    ...
    assert(
      !isDuplicate,
      s"$typ defined twice $sym ${sym.id} ${registry(name).id}"
    )
    registry(name) = sym
  ...
  def checkCompanion(symd: SymDenotation)(using Context): Unit =
    ...
    if prev.exists then
      assert(
        cur.exists || prev.is(ConstructorProxy),
        i"companion disappeared from $symd"
      )

  def transformSym(symd: SymDenotation)(using Context): SymDenotation =
    val sym = symd.symbol

    if sym.isClass && !sym.isAbsent() then
      val validSuperclass =
        sym.isPrimitiveValueClass || defn.syntheticCoreClasses.contains(sym) ||
          (sym eq defn.ObjectClass) || sym.isOneOf(
            NoSuperClassFlags
          ) || (sym.asClass.superClass.exists) ||
    ...
    if ctx.phaseId <= erasurePhase.id then
      val initial = symd.initial
      assert(
        symd == initial || symd.signature == initial.signature,
  ...
  def run(using Context): Unit =
    ...
    then
      report.echo(
        "Skipping Ycheck after pickling with -Ytest-pickler, the returned tree contains stale symbols"
      )
    else if ctx.phase.prev.isCheckable then
  ...
  def check(phasesToRun: Seq[Phase], ctx: Context): Tree =
    ...
    report.echo(s"checking ${ctx.compilationUnit} after phase ${fusedPhase}")(
      using ctx
    )

    inContext(ctx) {
      assert(
        ctx.typerState.constraint.domainLambdas.isEmpty,
    ...
    try checker.typedExpr(ctx.compilationUnit.tpdTree)(using checkingCtx)
      ...
      case NonFatal(ex) => // TODO CHECK. Check that we are bootstrapped
        inContext(checkingCtx) {
          println(
            i"*** error while checking ${ctx.compilationUnit} after phase ${ctx.phase
                .prevMega(using ctx)} ***"
          )
        }
        throw ex
        ...
        override def apply(parent: Tree, tree: Tree)(using Context): Tree =
          tree match
            case tree: New if !parent.isInstanceOf[tpd.Select] =>
              assert(
                assertion = false,
                i"`New` node must be wrapped in a `Select` of the constructor:\n  parent = ${parent.show}\n  child = ${tree.show}"
              )
            case _: Annotated =>
...
object TreeChecker:
  /**   - Check that TypeParamRefs and MethodParams refer to an enclosing type.
    ...
    */
  def checkNoOrphans(tp0: Type, tree: untpd.Tree = untpd.EmptyTree)(using
      Context
  ): Type = new TypeMap():
      ...
      tp match
        ...
        case tp: ParamRef =>
          assert(
            ...
            s"orphan param: ${tp.show}, hash of binder = ${System
                .identityHashCode(tp.binder)}, tree = ${tree.show}, type = $tp0"
          )
        case tp: TypeVar =>
          assert(
            tp.isInstantiated,
            s"Uninstantiated type variable: ${tp.show}, tree = ${tree.show}"
          )
          apply(tp.underlying)
        case _ =>
          mapOver(tp)
  ...
  object TreeNodeChecker extends untpd.TreeTraverser:
    ...
    def traverse(tree: Tree)(using Context) = tree match
      case t: TypeTree =>
        assert(assertion = false, i"TypeTree not expected: $t")
      ...
      case t @ DefDef(_, paramss, _tpt, _) =>
        for params <- paramss do traverse(params); traverse(t.rhs)
      case t @ TypeDef(_, _rhs) =>
      case t @ Template(constr, _, self, _) =>
        traverse(constr); traverse(t.parentsOrDerived); traverse(self);
        traverse(t.body)
      case t => traverseChildren(t)
    end traverse

  private[TreeChecker] def isValidJVMName(name: Name): Boolean =
    name.toString.forall(isValidJVMChar)

  private[TreeChecker] def isValidJVMMethodName(name: Name): Boolean =
    name.toString.forall(isValidJVMMethodChar)
  ...
  class Checker(phasesToCheck: Seq[Phase]) extends ReTyper with Checking:
    ...
    def withDefinedSyms[T](trees: List[untpd.Tree])(op: => T)(using
        Context
    ): T =
      ...
      for tree <- trees do
        ...
        tree match
          case tree: untpd.DefTree =>
            assert(
              isValidJVMName(sym.name.encode),
              s"${sym.name.debugString} name is invalid on jvm"
            )
            everDefinedSyms.get(sym) match
              case Some(t) =>
                if t ne tree then
                  report.warning(
                    i"symbol ${sym.fullName} is defined at least twice in different parts of AST"
                  )
              // should become an error
              case None =>
                everDefinedSyms(sym) = tree
            assert(
              !nowDefinedSyms.contains(sym),
              i"doubly defined symbol: ${sym.fullName} in $tree"
            )

            if ctx.settings.YcheckMods.value then
              tree match
                case t: untpd.MemberDef =>
                  if t.name ne sym.name then
                    report.warning(
                      s"symbol ${sym.fullName} name doesn't correspond to AST: ${t}"
                    )
    ...
    def assertDefined(tree: untpd.Tree)(using Context): Unit =
      if tree.symbol.maybeOwner.isTerm then
        val sym = tree.symbol
        assert(
        ...
        if !ctx.phase.patternTranslated then
          assert(
    ...
    def assertIdentNotJavaClass(tree: Tree)(using Context): Unit = tree match
      case _: untpd.Ident =>
        assert(
          !tree.symbol.isAllOf(JavaModule),
          "Java class can't be used as value: " + tree
        )
      case _ =>
    ...
    /** Exclude from double definition checks any erased symbols that were made
      * `private` in phase `UnlinkErasedDecls`. These symbols will be removed
      * completely in phase `Erasure` if they are defined in a currently
      * compiled unit.
      */
    override def excludeFromDoubleDeclCheck(sym: Symbol)(using
        Context
    ): Boolean =
      ...
      case _ =>
        tree.tpe match
          case tp: SuperType =>
            assert(
              false,
              s"The type of a non-Super tree must not be a SuperType, but $tree has type $tp"
            )
          case _ =>

    override def typed(tree: untpd.Tree, pt: Type = WildcardType)(using
        Context
    ): Tree =
        ...
        case _ =>
          ...
          def divergenceMsg(tp1: Type, tp2: Type) =
            s"""Types differ
               ...
               |Why different :
             """.stripMargin + core.TypeComparer.explained(
              _.isSubType(tp1, tp2)
            )
          if tree.hasType
          then // it might not be typed because Typer sometimes constructs new untyped trees and resubmits them to typedUnadapted
            assert(
              isSubType(tree1.tpe, tree.typeOpt),
              divergenceMsg(tree1.tpe, tree.typeOpt)
            )
          tree1
    ...
    def checkNotRepeated(tree: Tree)(using Context): tree.type =
      ...
      assert(
        !tree.tpe.widen.isRepeatedParam || allowedRepeated,
        i"repeated parameter type not allowed here: $tree"
      )
      tree
    ...
    override def typedIdent(tree: untpd.Ident, pt: Type)(using Context): Tree =
      ...
      assert(
        tree.isType || ctx.mode.is(Mode.Pattern) && untpd.isWildcardArg(
          tree
        ) || !needsSelect(tree.typeOpt),
        i"bad type ${tree.tpe} for $tree # ${tree.uniqueId}"
      )
      assertDefined(tree)
    ...
    /** Makes sure the symbol in the tree can be approximately reconstructed by
      * calling `member` on the qualifier type. Approximately means: The two
      * symbols might be different but one still overrides the other.
      */
    override def typedSelect(tree: untpd.Select, pt: Type)(using
      ...
      val isPolyFunctionApply =
        (tree.name eq nme.apply) && tree.qualifier.typeOpt.derivesFrom(
          defn.PolyFunctionClass
        )
      // Erased functions stay structural until Erasure
      val isErasedFunctionApply =
        (tree.name eq nme.apply) && tree.qualifier.typeOpt.derivesFrom(
          defn.ErasedFunctionClass
        )
      ...
      if !(tree.isType || isPolyFunctionApply || isErasedFunctionApply || isOuterSelect || isPrimitiveArrayOp)
      then
        val denot = tree.denot
        assert(
          denot.exists,
          i"Selection $tree with type $tpe does not have a denotation"
        )
        assert(
          denot.symbol.exists,
          i"Denotation $denot of selection $tree with type $tpe does not have a symbol, qualifier type = ${tree.qualifier.typeOpt}"
        )
      ...
      if sym.exists && !sym.is(Private) &&
        !symIsFixed &&
        !isOuterSelect
      then // outer selects have effectively fixed symbols
        val qualTpe = tree.qualifier.typeOpt
        val member =
          if sym.is(Private) then qualTpe.member(tree.name)
          else qualTpe.nonPrivateMember(tree.name)
        val memberSyms = member.alternatives.map(_.symbol)
        assert(
          memberSyms.exists(mbr =>
            sym == mbr ||
          ...
          i"""symbols differ for $tree
                  |was                 : $sym
                  |alternatives by type: $memberSyms%, % of types ${memberSyms
              .map(_.info)}%, %
    ...
    override def typedThis(tree: untpd.This)(using Context): Tree =
      ...
      val cls = res.symbol
      assert(
        cls.isStaticOwner || ctx.owner.isContainedIn(cls),
        i"error while typing $tree, ${ctx.owner} is not contained in $cls"
      )
      res

    override def typedSuper(tree: untpd.Super, pt: Type)(using Context): Tree =
      assert(
        tree.qual.typeOpt.isInstanceOf[ThisType],
        i"expect prefix of Super to be This, actual = ${tree.qual}"
      )
    ...
    override def typedNew(tree: untpd.New, pt: Type)(using Context): Tree =
      ...
      if postTyperPhase <= ctx.phase
      then // postTyper checks that `New` nodes can be instantiated
        assert(
          !tree1.tpe.isInstanceOf[TermRef],
          s"New should not have a TermRef type: ${tree1.tpe}"
        )
        assert(
          !sym.is(Module)
    ...
    override def typedApply(tree: untpd.Apply, pt: Type)(using Context): Tree =
      tree match
        case Apply(Select(qual, nme.CONSTRUCTOR), _)
          ...
          promote(
            tree
          ) // e.g. `new Tuple2$mcII$sp(7, 8)` should keep its `(7, 8)` type instead of `Tuple2$mcII$sp`
    ...
    override def typedTyped(tree: untpd.Typed, pt: Type)(using Context): Tree =
      ...
      val expr1 = tree.expr match
        case id: untpd.Ident
            if (ctx.mode is Mode.Pattern) && untpd.isVarPattern(
              id
            ) && (id.name == nme.WILDCARD || id.name == nme.WILDCARD_STAR) =>
          tree.expr.withType(tpt1.tpe)
        case _ =>
          var pt1 = tpt1.tpe
          if pt1.isRepeatedParam then
            pt1 = pt1.translateFromRepeated(toArray =
              tree.expr.typeOpt.derivesFrom(defn.ArrayClass)
            )
          val isAfterInlining =
    ...
    private def checkOwner(tree: untpd.Tree)(using Context): Unit =
      ...
      assert(
        ownerMatches(tree.symbol.owner, ctx.owner),
    ...
    override def typedTypeDef(tdef: untpd.TypeDef, sym: Symbol)(using
      ...
      assert(
        sym.info.isInstanceOf[ClassInfo | TypeBounds],
        i"wrong type, expect a template or type bounds for ${sym.fullName}, but found: ${sym.info}"
      )
    ...
    override def typedClassDef(cdef: untpd.TypeDef, cls: ClassSymbol)(using
      ...
      assert(
        cls.primaryConstructor == constr.symbol,
        i"mismatch, primary constructor ${cls.primaryConstructor}, in tree = ${constr.symbol}"
      )
      checkOwner(impl)
      ...
      val symbolsNotDefined =
        (decls -- defined - constr.symbol).filterNot(isAllowed)

      assert(
        symbolsNotDefined.isEmpty,
    ...
    override def typedDefDef(ddef: untpd.DefDef, sym: Symbol)(using
      ...
      assert(
        ctx.erasedTypes || sym.rawParamss == defParamss,
      ...
      withDefinedSyms(ddef.paramss.flatten) {
        if !sym.isClassConstructor && !(sym.name eq nme.STATIC_CONSTRUCTOR) then
          assert(
            isValidJVMMethodName(sym.name.encode),
            s"${sym.name.debugString} name is invalid on jvm"
          )

        ddef.termParamss.foreach(_.foreach { vparam =>
          assert(
            vparam.symbol.is(Param),
            s"Parameter ${vparam.symbol} of ${sym.fullName} does not have flag `Param` set"
          )
          assert(
            !vparam.symbol.isOneOf(AccessFlags),
        ...
        assert(
          isMethodType(sym.info),
          i"wrong type, expect a method type for ${sym.fullName}, but found: ${sym.info}"
        )
        tpdTree
      }
    end typedDefDef

    override def typedCase(
        tree: untpd.CaseDef,
        sel: Tree,
        selType: Type,
        pt: Type
    )(using Context): CaseDef =
    ...
    override def typedClosure(tree: untpd.Closure, pt: Type)(using
      ...
      if !ctx.phase.lambdaLifted then
        nestingBlock match
          case block @ Block(
                (meth: untpd.DefDef) :: Nil,
                closure: untpd.Closure
              ) =>
            assert(
              meth.symbol == closure.meth.symbol,
              "closure.meth symbol not equal to method symbol. Block: " + block.show
            )

          case block: untpd.Block =>
            assert(
              false,
              "function literal are not properly formed as a block of DefDef and Closure. Found: " + tree.show + " Nesting block: " + block.show
            )

          case null =>
            assert(
              false,
              "function literal are not properly formed as a block of DefDef and Closure. Found: " + tree.show + " Nesting block: null"
            )
    ...
    /** Check that all defined symbols have legal owners. An owner is legal if
      * it is either the same as the context's owner or there's an owner chain
      * of valdefs starting at the context's owner and reaching up to the
      * symbol's owner. The reason for this relaxed matching is that we should
      * be able to pull out an expression as an initializer of a helper value
      * without having to do a change owner traversal of the expression.
      */
    override def typedStats(trees: List[untpd.Tree], exprOwner: Symbol)(using
        Context
    ): (List[Tree], Context) =
      for tree <- trees do
        tree match
          case tree: untpd.DefTree => checkOwner(tree)
          case _: untpd.Thicket =>
            assert(
              false,
              i"unexpanded thicket $tree in statement sequence $trees%\n%"
            )
          case _ =>
    ...
    override def typedReturn(tree: untpd.Return)(using Context): Return =
      ...
      if fromSym.is(Label) then assertDefined(from)
      tree1

    override def typedWhileDo(tree: untpd.WhileDo)(using Context): Tree =
      assert(
        (tree.cond ne EmptyTree) || ctx.phase.refChecked,
        i"invalid empty condition in while at $tree"
      )
    ...
    override def typedPackageDef(tree: untpd.PackageDef)(using Context): Tree =
      if tree.symbol == defn.StdLibPatchesPackage then
        promote(
          tree
        ) // don't check stdlib patches, since their symbols were highjacked by stdlib classes
      else super.typedPackageDef(tree)

    override def typedQuote(tree: untpd.Quote, pt: Type)(using Context): Tree =
      if ctx.phase <= stagingPhase.prev then
        assert(
          tree.tags.isEmpty,
          i"unexpected tags in Quote before staging phase: ${tree.tags}"
        )
      else
        assert(
          !tree.body.isInstanceOf[untpd.Splice] || inInlineMethod,
          i"missed quote cancellation in $tree"
        )
        assert(
          !tree.body.isInstanceOf[untpd.Hole] || inInlineMethod,
          i"missed quote cancellation in $tree"
        )
        if StagingLevel.level != 0 then
          assert(
            tree.tags.isEmpty,
            i"unexpected tags in Quote at staging level ${StagingLevel.level}: ${tree.tags}"
          )

      for tag <- tree.tags do
        assert(
          tag.isInstanceOf[RefTree],
          i"expected RefTree in Quote but was: $tag"
        )

      val tree1 = super.typedQuote(tree, pt)
      for tag <- tree.tags do
        assert(
          tag.typeOpt.derivesFrom(defn.QuotedTypeClass),
          i"expected Quote tag to be of type `Type` but was: ${tag.tpe}"
        )

      tree1 match
        case Quote(body, targ :: Nil) if body.isType =>
          assert(
            !(body.tpe =:= targ.tpe.select(tpnme.Underlying)),
            i"missed quote cancellation in $tree1"
          )
        case _ =>
    ...
    override def typedSplice(tree: untpd.Splice, pt: Type)(using
      ...
      if stagingPhase <= ctx.phase then
        assert(
          !tree.expr.isInstanceOf[untpd.Quote] || inInlineMethod,
          i"missed quote cancellation in $tree"
        )
      super.typedSplice(tree, pt)

    override def typedHole(tree: untpd.Hole, pt: Type)(using Context): Tree =
      val tree1 @ Hole(isTerm, idx, args, content) =
        super.typedHole(tree, pt): @unchecked

      assert(idx >= 0, i"hole should not have negative index: $tree")
      assert(
        isTerm || tree.args.isEmpty,
        i"type hole should not have arguments: $tree"
      )
      ...
      for arg <- args do
        assert(
          arg.isTerm || arg.tpe.isInstanceOf[TypeRef],
          "Expected TypeRef in Hole type args but got: " + arg.tpe
        )
      ...
      assert(
        content.typeOpt =:= expectedContentType,
        i"unexpected content of hole\nexpected: ${expectedContentType}\nwas: ${content.typeOpt}"
      )

      tree1
    end typedHole

    override def ensureNoLocalRefs(
        tree: Tree,
        pt: Type,
    ...
    override def adapt(tree: Tree, pt: Type, locked: TypeVars)(using
      ...
      def isPrimaryConstructorReturn =
        ctx.owner.isPrimaryConstructor && pt.isRef(ctx.owner.owner) && tree.tpe
          .isRef(defn.UnitClass)
  ...
  class LocalChecker(phasesToCheck: Seq[Phase])
      extends Checker(phasesToCheck: Seq[Phase]):
  ...
  def checkMacroGeneratedTree(original: tpd.Tree, expansion: tpd.Tree)(using
      Context
  ): Unit =
          ...
          report.error(
            s"""Malformed tree was found while expanding macro with -Xcheck-macros.
               ...
               |${scala.quoted.runtime.impl.QuotesImpl
                .showDecompiledTree(original)}
               |
               |The macro returned:
               |${scala.quoted.runtime.impl.QuotesImpl
                .showDecompiledTree(expansion)}
               |
               |Error:
               |${err.getMessage}
               |$stack
  ...
  private[TreeChecker] def previousPhases(phases: List[Phase])(using
      Context
  ): List[Phase] = phases match
    case (phase: MegaPhase) :: phases1 =>
      val subPhases = phase.miniPhases
      val previousSubPhases = previousPhases(subPhases.toList)
      if previousSubPhases.length == subPhases.length then
        previousSubPhases ::: previousPhases(phases1)
