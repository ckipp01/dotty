...
sealed trait Plugin:
  ...
  /** A description of this plugin's options, suitable as a response to the
    * -help command-line option. Conventionally, the options should be listed
    * with the `-P:plugname:` part included.
    */
...
/** A research plugin may customize the compilation pipeline freely
  *
  * @note
  *   Research plugins are only supported by nightly or snapshot build of the
  *   compiler.
  */
trait ResearchPlugin extends Plugin:
  /** Research plugins should override this method to return the new phase plan
    *
    * @param options
    *   commandline options to the plugin, `-P:plugname:opt1,opt2` becomes
    *   List(opt1, opt2)
    * @param plan
    ...
    *   the new phase plan
    */
  def init(options: List[String], plan: List[List[Phase]])(using
      Context
  ): List[List[Phase]]
...
object Plugin:
  ...
  /** Create a class loader with the specified locations plus the loader that
    * loaded the Scala compiler.
    */
      ...
      case NonFatal(e) =>
        Failure(
          new PluginLoadException(
            classname,
            s"Error: unable to load class $classname: ${e.getMessage}"
          )
        )
  ...
  /** Load all plugins specified by the arguments. Each location of `paths` must
    * be a valid plugin archive or exploded archive. Each of `paths` must define
    * one plugin. Each of `dirs` may be a directory containing arbitrary plugin
    * archives. Skips all plugins named in `ignoring`. A classloader is created
    * to load each plugin.
    */
  def loadAllFrom(
    ...
    def fromFile(inputStream: InputStream, path: Path): String =
      ...
      if pluginClass == null then
        throw new RuntimeException("Bad plugin descriptor: " + path)
      else pluginClass
      ...
      def loop(qs: List[Path]): Try[String] = qs match
        ...
        case p :: rest =>
          if p.isDirectory then
            loadDescriptionFromDir(p.toDirectory) orElse loop(rest)
          else if p.isFile then
            loadDescriptionFromJar(p.toFile) orElse loop(rest)
          else loop(rest)
      loop(ps)

    val fromPaths: PDResults = paths map (p =>
      findDescriptor(p) match
    ...
    val enabled =
      (fromPaths ::: fromDirs) map (_.flatMap { case (classname, loader) =>
        Plugin.load(classname, loader).flatMap { clazz =>
          val plugin = instantiate(clazz)
          if seen.contains(classname)
          then // a nod to scala/bug#7494, take the plugin classes distinctly
            Failure(
              new PluginLoadException(
                plugin.name,
                s"Ignoring duplicate plugin ${plugin.name} (${classname})"
              )
            )
          else if ignoring contains plugin.name then
            Failure(
              new PluginLoadException(
                plugin.name,
  ...
  /** Instantiate a plugin class, given the class and the compiler it is to be
    * used in.
    */
  def instantiate(clazz: AnyClass): Plugin =
    clazz.getConstructor().newInstance().asInstanceOf[Plugin]
end Plugin
