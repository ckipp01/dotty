...
/** A tree traverser that prepares a compilation unit to be capture checked. It
  * does the following:
  *   - For every inferred type, drop any retains annotations, add capture sets
  *     to all its parts, add refinements to class types and function types.
  *     (c.f. mapInferred)
  *   - For explicit capturing types, expand throws aliases to the underlying
  *     (pure) function, and add some implied capture sets to curried functions
  *     (c.f. expandThrowsAlias, expandAbbreviations).
  *   - Add capture sets to self types of classes and objects, unless the self
  *     type was written explicitly.
  *   - Box the types of mutable variables and type arguments to methods (type
  *     arguments of types are boxed on access).
  *   - Link the external types of val and def symbols with the inferred types
  *     based on their parameter symbols.
  */
class Setup(
  ...
  /** Create dependent function with underlying function class `tycon` and given
    ...
    */
  private def depFun(tycon: Type, argTypes: List[Type], resType: Type)(using
      Context
  ): Type =
    MethodType
      .companion(
  ...
  /** If `tp` is an unboxed capturing type or a function returning an unboxed
    * capturing type, convert it to be boxed.
    */
  private def box(tp: Type)(using Context): Type =
    def recur(tp: Type): Type = tp.dealias match
      ...
      case tp1 @ RefinedType(_, _, rinfo: MethodType)
          if defn.isFunctionOrPolyType(tp1) =>
    ...
    tp match
      case tp: MethodOrPoly =>
        tp // don't box results of methods outside refinements
      case _ => recur(tp)

  /** Perform the following transformation steps everywhere in a type:
    *   1. Drop retains annotations 2. Turn plain function types into dependent
    *      function types, so that we can refer to their parameters in capture
    *      sets. Currently this is only done at the toplevel, i.e. for function
    *      types that are not themselves argument types of other function types.
    *      Without this restriction pos.../lists.scala and
    *      pos/...curried-shorthands.scala fail. Need to figure out why. 3.
    *      Refine other class types C by adding capture set variables to their
    *      parameter getters (see addCaptureRefinements) 4. Add capture set
    *      variables to all types that can be tracked
    *
    * Polytype bounds are only cleaned using step 1, but not otherwise
    * transformed.
    */
  private def mapInferred(using Context) = new TypeMap:
    ...
    /** Refine a possibly applied class type C where the class has tracked
      * parameters x_1: T_1, ..., x_n: T_n to C { val x_1: CV_1 T_1, ..., val
      * x_n: CV_n T_n } where CV_1, ..., CV_n are fresh capture sets.
      */
            ...
            cls.paramGetters.foldLeft(tp) { (core, getter) =>
              if getter.termRef.isTracked then
                ...
                RefinedType(
                  core,
                  getter.name,
                  CapturingType(getterType, CaptureSet.Var())
                )
      ...
      tp.typeParams.isEmpty && tp.match
        case tp: (TypeRef | AppliedType) =>
          ...
          if tp1 ne tp then needsVariable(tp1)
            ...
            if sym.isClass then !sym.isPureClass && sym != defn.AnyClass
    ...
    /** Add a capture set variable to `tp` if necessary, or maybe pull out an
      * embedded capture set variable from a part of `tp`.
      */
    def addVar(tp: Type) = tp match
      case tp @ RefinedType(
            parent @ CapturingType(parent1, refs),
            rname,
            rinfo
          ) =>
        CapturingType(
          tp.derivedRefinedType(parent1, rname, rinfo),
          refs,
          parent.isBoxed
        )
      case tp: RecType =>
      ...
      case tp @ OrType(
        ...
        assert(tp1.isBoxed == tp2.isBoxed)
        CapturingType(
          OrType(parent1, parent2, tp.isSoft),
          refs1 ++ refs2,
          tp1.isBoxed
        )
            ...
            if !defn.isFunctionSymbol(tp.typeSymbol) && (tp.dealias ne tp) then
              ...
              if isTopLevel then
                depFun(tycon1, args1, res1)
                  .showing(
                    i"add function refinement $tp ($tycon1, $args1, $res1) (${tp.dealias}) --> $result",
                    capt
                  )
              else if (tycon1 eq tycon) && (args1 eq args0) && (res1 eq res0)
              then tp
              else tp.derivedAppliedType(tycon1, args1 :+ res1)
          else tp.derivedAppliedType(tycon1, args.mapConserve(arg => this(arg)))
        case tp @ RefinedType(core, rname, rinfo: MethodType)
            if defn.isFunctionOrPolyType(tp) =>
          val rinfo1 = apply(rinfo)
          if rinfo1 ne rinfo then
  ...
  private def transformInferredType(tp: Type, boxed: Boolean)(using
      Context
  ): Type =
  ...
  /** Expand some aliases of function types to the underlying functions. Right
    * now, these are only $throws aliases, but this could be generalized.
    */
  private def expandThrowsAlias(tp: Type)(using Context) = tp match
    case AppliedType(tycon, res :: exc :: Nil)
        if tycon.typeSymbol == defn.throwsAlias =>
  ...
  /** Fill in capture sets of curried function types from left to right, using a
    * combination of the following two rules:
    *
    *   1. Expand `{c} (x: A) -> (y: B) -> C` to `{c} (x: A) -> {c} (y: B) -> C`
    *      2. Expand `(x: A) -> (y: B) -> C` where `x` is tracked to `(x: A) ->
    *      {x} (y: B) -> C`
    *
  ...
  private def expandAbbreviations(using Context) = new TypeMap:
    ...
    /** Propagate `outerCs` as well as all tracked parameters as capture set to
      * the result type of the dependent function type `tp`.
      */
    def propagateDepFunctionResult(tp: Type, outerCs: CaptureSet): Type =
      tp match
    ...
    /** If `tp` is a function type:
      ...
      *   - propagate `currentCs`, `outerCs`, and all tracked parameters of `tp`
      *     to the right.
      */
    def propagateEnclosing(
        tp: Type,
        currentCs: CaptureSet,
        outerCs: CaptureSet
    ): Type = tp match
      case tp @ AppliedType(tycon, args)
          if defn.isFunctionClass(tycon.typeSymbol) =>
        val tycon1 = this(tycon)
    ...
    def apply(tp: Type): Type = tp match
      case CapturingType(parent, cs) =>
        tp.derivedCapturingType(
          propagateEnclosing(parent, cs, CaptureSet.empty),
          cs
        )
      case _ =>
  ...
  private def transformExplicitType(tp: Type, boxed: Boolean)(using
      Context
  ): Type =
  ...
  /** Transform type of type tree, and remember the transformed type as the type
    * the tree
    */
  private def transformTT(tree: TypeTree, boxed: Boolean, exact: Boolean)(using
      Context
  ): Unit =
  ...
  /** Substitute parameter symbols in `from` to paramRefs in corresponding
    ...
    * @param to
    *   a list of method or poly types corresponding one-to-one to the parameter
    *   lists
    */
  private class SubstParams(from: List[List[Symbol]], to: List[LambdaType])(
      using Context
  ) extends DeepTypeMap,
        BiTypeMap:
  ...
  def traverse(tree: Tree)(using Context): Unit =
    tree match
      case tree: DefDef =>
        if isExcluded(tree.symbol) then return
        tree.tpt match
      ...
      case tree @ ValDef(_, tpt: TypeTree, _) =>
        transformTT(
          tpt,
          boxed =
            tree.symbol.is(Mutable), // types of mutable variables are boxed
          exact =
            tree.symbol.allOverriddenSymbols.hasNext // types of symbols that override a parent don't get a capture set
        )
        if allowUniversalInBoxed && tree.symbol.is(Mutable)
          && !tree.symbol.hasAnnotation(defn.UncheckedCapturesAnnot)
        then
          CheckCaptures.disallowRootCapabilitiesIn(
            tpt.knownType,
            i"Mutable variable ${tree.symbol.name}",
            "have type",
      ...
      case tree @ TypeApply(fn, args) =>
        ...
        for case arg: TypeTree <- args do
          transformTT(
            ...
            boxed = true,
            exact = false
          ) // type arguments in type applications are boxed

        if allowUniversalInBoxed then
          val polyType = fn.tpe.widen.asInstanceOf[TypeLambda]
          for case (arg: TypeTree, pinfo, pname) <- args
              .lazyZip(polyType.paramInfos)
              .lazyZip((polyType.paramNames))
          do
            if pinfo.bounds.hi.hasAnnotation(defn.Caps_SealedAnnot) then
              def where =
                if fn.symbol.exists then i" in the body of ${fn.symbol}" else ""
              CheckCaptures.disallowRootCapabilitiesIn(
                arg.knownType,
                i"Sealed type variable $pname",
                " be instantiated to",
    ...
    tree match
      ...
      case tree: ValOrDefDef =>
        ...
        def integrateRT(
            ...
            psymss: List[
              List[Symbol]
            ], // the local (type and term) parameter symbols corresponding to `info`
            prevPsymss: List[
              List[Symbol]
            ], // the local parameter symbols seen previously in reverse order
            prevLambdas: List[
              LambdaType
            ] // the outer method and polytypes generated previously in reverse order
        ): Type =
          info match
            case mt: MethodOrPoly =>
              ...
              mt.companion(mt.paramNames)(
                mt1 =>
                  if !psyms.exists(
                      _.isUpdatedAfter(preRecheckPhase)
                    ) && !mt.isParamDependent && prevLambdas.isEmpty
                  then mt.paramInfos
                  else
                    val subst =
                      SubstParams(psyms :: prevPsymss, mt1 :: prevLambdas)
      ...
      case tree: TypeDef =>
        tree.symbol match
          case cls: ClassSymbol =>
            val cinfo @ ClassInfo(prefix, _, ps, decls, selfInfo) =
              cls.classInfo
          ...
          case _ =>
            ...
            if newInfo ne info then
              ...
              capt.println(
                i"update info of ${tree.symbol} from $info to $newInfo"
              )
      case _ =>
    end match
  end traverse
