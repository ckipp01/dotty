...
/** A class for capture sets. Capture sets can be constants or variables.
  ...
  *
  * cs1 <:< cs2 cs1 = ∪ {f(x) | x ∈ cs2} where f is a function from capture
  * references to capture sets. cs1 = ∪ {x | x ∈ cs2, p(x)} where p is a
  * predicate on capture references cs1 = cs2 ∩ cs2
  *
  * We call the resulting constraint system "monadic set constraints". To
  * support capture propagation across maps, mappings are supported only if the
  * mapped function is either a bijection or if it is idempotent on capture
  * references (c.f. doc comment on `map` below).
  */
sealed abstract class CaptureSet extends Showable:
  import CaptureSet.*

  /** The elements of this capture set. For capture variables, the elements
    * known so far.
    */
  def elems: Refs
  ...
  /** Add new elements to this capture set if allowed.
    * @pre
    *   `newElems` is not empty and does not overlap with `this.elems`. Constant
    *   capture sets never allow to add new elements. Variables allow it if and
    *   only if the new elements can be included in all their dependent sets.
    * @param origin
    *   The set where the elements come from, or `empty` if not known.
    * @return
    *   CompareResult.OK if elements were added, or a conflicting capture set
    *   that prevents addition otherwise.
    */
  protected def addNewElems(newElems: Refs, origin: CaptureSet)(using
      Context,
      VarState
  ): CompareResult

  /** If this is a variable, add `cs` as a dependent set */
  protected def addDependent(
      cs: CaptureSet
  )(using Context, VarState): CompareResult
  ...
  /** Try to include all references of `elems` that are not yet accounted for by
    * this capture set. Inclusion is via `addNewElems`.
    * @param origin
    *   The set where the elements come from, or `empty` if not known.
    * @return
    *   CompareResult.OK if all unaccounted elements could be added, capture set
    *   that prevents addition otherwise.
    */
  protected final def tryInclude(elems: Refs, origin: CaptureSet)(using
      Context,
      VarState
  ): CompareResult =
  ...
  protected final def tryInclude(elem: CaptureRef, origin: CaptureSet)(using
      Context,
      VarState
  ): CompareResult =
  ...
  /** {x} <:< this where <:< is subcapturing, but treating all variables as
    * frozen.
    */
  def accountsFor(x: CaptureRef)(using Context): Boolean =
    reporting.trace(
      i"$this accountsFor $x, ${x.captureSetOfInfo}?",
      show = true
    ) {
      elems.exists(_.subsumes(x))
      || !x.isRootCapability && x.captureSetOfInfo
        .subCaptures(this, frozen = true)
        .isOK
    }

  /** A more optimistic version of accountsFor, which does not take variable
    * supersets of the `x` reference into account. A set might account for `x`
    * if it accounts for `x` in a state where we assume all supersets of `x`
    * have just the elements known at this point. On the other hand if x's
    * capture set has no known elements, a set `cs` might account for `x` only
    * if it subsumes `x` or it contains the root capability `cap`.
    */
  def mightAccountFor(x: CaptureRef)(using Context): Boolean =
    reporting.trace(
      i"$this mightAccountFor $x, ${x.captureSetOfInfo}?",
      show = true
    ) {
  ...
  /** A more optimistic version of subCaptures used to choose one of two typing
    * rules for selections and applications. `cs1 mightSubcapture cs2` if `cs2`
  ...
  /** The subcapturing test.
    * @param frozen
    *   if true, no new variables or dependent sets are allowed to be added when
    *   making this test. An attempt to add either will result in failure.
    */
  final def subCaptures(that: CaptureSet, frozen: Boolean)(using
      Context
  ): CompareResult =
    subCaptures(that)(using ctx, if frozen then FrozenState else VarState())

  /** The subcapturing test, using a given VarState */
  private def subCaptures(
      that: CaptureSet
  )(using Context, VarState): CompareResult =
    def recur(elems: List[CaptureRef]): CompareResult = elems match
      case elem :: elems1 =>
        var result = that.tryInclude(elem, this)
        if !result.isOK && !elem.isRootCapability && summon[
            VarState
          ] != FrozenState
        then result = elem.captureSetOfInfo.subCaptures(that)
        if result.isOK then recur(elems1)
        else
          varState.rollBack()
          result
      case Nil =>
  ...
  /** Two capture sets are considered =:= equal if they mutually subcapture each
    * other in a frozen state.
    */
  def =:=(that: CaptureSet)(using Context): Boolean =
    this.subCaptures(that, frozen = true).isOK
      && that.subCaptures(this, frozen = true).isOK

  /** The smallest capture set (via <:<) that is a superset of both `this` and
    * `that`
    */
  def ++(that: CaptureSet)(using Context): CaptureSet =
    if this.subCaptures(that, frozen = true).isOK then that
      ...
      Var(this.elems ++ that.elems)
        .addAsDependentTo(this)
        .addAsDependentTo(that)

  /** The smallest superset (via <:<) of this capture set that also contains
    * `ref`.
    */
  def +(ref: CaptureRef)(using Context): CaptureSet =
    this ++ ref.singletonCaptureSet

  /** The largest capture set (via <:<) that is a subset of both `this` and
    * `that`
    */
  def **(that: CaptureSet)(using Context): CaptureSet =
    if this.subCaptures(that, frozen = true).isOK then this
      ...
      Const(elemIntersection(this, that))
  ...
  /** The largest subset (via <:<) of this capture set that only contains
  ...
  /** Capture set obtained by applying `tm` to all elements of the current
    * capture set and joining the results. If the current capture set is a
    * variable, the same transformation is applied to all future additions of
    * new elements.
    *
    * Note: We have a problem how we handle the situation where we have a mapped
    * set
    *
    * cs2 = tm(cs1)
    *
    * and then the propagation solver adds a new element `x` to `cs2`. What do
    * we know in this case about `cs1`? We can answer this question in a sound
    * way only if `tm` is a bijection on capture references or it is idempotent
    * on capture references. (see definition in IdempotentCapRefMap). If `tm` is
    * a bijection we know that `tm^-1(x)` must be in `cs1`. If `tm` is
    * idempotent one possible solution is that `x` is in `cs1`, which is what we
    * assume in this case. That strategy is sound but not complete.
    *
  ...
  /** A mapping resulting from substituting parameters of a BindingType to a
    * list of types
    */
  ...
  /** Invoke handler if this set has (or later aquires) the root capability
    * `cap`
    */
  def disallowRootCapability(handler: () => Context ?=> Unit)(using
      Context
  ): this.type =
  ...
  /** Invoke handler on the elements to check wellformedness of the capture set
    */
  def ensureWellformed(handler: List[CaptureRef] => Context ?=> Unit)(using
      Context
  ): this.type =
  ...
  /** An upper approximation of this capture set, i.e. a constant set that is
    * subcaptured by this set. If the current set is a variable it is the
    * intersection of all upper approximations of known supersets of the
    * variable. The upper approximation is meaningful only if it is constant. If
    * not, `upperApprox` can return an arbitrary capture set variable.
    * `upperApprox` is used in `solve`.
    */
  protected def upperApprox(origin: CaptureSet)(using Context): CaptureSet

  /** Assuming set this set dependds on was just solved to be constant,
    * propagate this info to this set. This might result in the set being solved
    * to be constant itself.
    */
  ...
  /** The provided description (using `withDescription`) for this capture set or
    * else ""
    */
  def description: String

  /** A regular @retains or @retainsByName annotation with the elements of this
    * set as arguments.
    */
...
object CaptureSet:
  ...
  class Const private[CaptureSet] (
      val elems: Refs,
    ...
    def addNewElems(elems: Refs, origin: CaptureSet)(using
        Context,
        VarState
    ): CompareResult =
  ...
  class Var(initialElems: Refs = emptySet) extends CaptureSet:
    ...
    /** A variable is solved if it is aproximated to a from-then-on constant
      * set.
      */
    ...
    /** The sets currently known to be dependent sets (i.e. new additions to
      * this set are propagated to these dependent sets.)
      */
    ...
    def addNewElems(newElems: Refs, origin: CaptureSet)(using
        Context,
        VarState
    ): CompareResult =
    ...
    def addDependent(cs: CaptureSet)(using Context, VarState): CompareResult =
      if (cs eq this) || cs.isUniversal || isConst then CompareResult.OK
      else if recordDepsState() then
        deps += cs
        CompareResult.OK
      else CompareResult.fail(this)

    override def disallowRootCapability(handler: () => Context ?=> Unit)(using
        Context
    ): this.type =
      rootAddedHandler = handler
      super.disallowRootCapability(handler)

    override def ensureWellformed(
        handler: List[CaptureRef] => (Context) ?=> Unit
    ...
    /** Roughly: the intersection of all constant known supersets of this set.
      * The aim is to find an as-good-as-possible constant set that is a
      * superset of this set. The universal set {cap} is a sound fallback.
      */
    ...
    /** Widen the variable's elements to its upper approximation and mark it as
      * constant from now on. This is used for contra-variant type variables in
      * the results of defs and vals.
      */
    def solve()(using Context): Unit =
      if !isConst then
        ...
        if newElems.isEmpty || addNewElems(newElems, empty)(using
            ctx,
            VarState()
          ).isOK
        then markSolved()
    ...
    /** Used for diagnostics and debugging: A string that traces the creation
      ...
      * variable's class name. The path ends in a plain variable with letter `V`
      * that is not derived from some other variable.
      */
    ...
    override def toText(printer: Printer): Text =
      inContext(printer.printerContext) {
        ...
        super.toText(printer) ~ (Str(
          ids
        ) provided !isConst && ctx.settings.YccDebug.value)
      }
  ...
  /** A variable that is derived from some other variable via a map or filter.
    */
  abstract class DerivedVar(initialElems: Refs)(using
      @constructorOnly ctx: Context
    ...
    val stack =
      if debugSets && this.isInstanceOf[Mapped] then
        (new Throwable).getStackTrace().nn.take(20)
      else null
  ...
  /** A variable that changes when `source` changes, where all additional new
    * elements are mapped using ∪ { tm(x) | x <- source.elems }.
    * @param source
    *   the original set that is mapped
    * @param tm
    *   the type map, which is assumed to be idempotent on capture refs (except
    *   if ccUnsoundMaps is enabled)
    * @param variance
    *   the assumed variance with which types with capturesets of size >= 2 are
    *   approximated (i.e. co: full capture set, contra: empty set, nonvariant
    *   is not allowed.)
    * @param initial
    *   The initial mappings of source's elements at the point the Mapped set is
    *   created.
    */
  class Mapped private[CaptureSet] (
    ...
    override def addNewElems(newElems: Refs, origin: CaptureSet)(using
        Context,
        VarState
    ): CompareResult =
      val added =
        if origin eq source then // elements have to be mapped
          ...
          Const(newElems)
      super
        .addNewElems(added.elems, origin)
        .andAlso {
          if added.isConst then CompareResult.OK
          else if added.asVar.recordDepsState() then
            addAsDependentTo(added); CompareResult.OK
          else CompareResult.fail(this)
        }
        .andAlso {
          ...
          else if !mapIsIdempotent && variance <= 0 && !origin.isConst && (origin ne initial) && (origin ne source)
          then
            ...
            report.warning(
              em"trying to add elems ${CaptureSet(newElems)} from unrecognized source $origin of mapped set $this$whereCreated"
            )
            CompareResult.fail(this)
  ...
  /** A mapping where the type map is required to be a bijection. Parameters as
    * in Mapped.
    */
  final class BiMapped private[CaptureSet] (
    ...
    override def addNewElems(newElems: Refs, origin: CaptureSet)(using
      ...
      if origin eq source then
        ...
        super
          .addNewElems(newElems, origin)
          .andAlso {
            source
              .tryInclude(newElems.map(bimap.backward), this)
              .showing(
                i"propagating new elems ${CaptureSet(newElems)} backward from $this to $source",
                capt
              )
          }

    /** For a BiTypeMap, supertypes of the mapped type also constrain the source
      * via the inverse type mapping and vice versa. That is, if B = f(A) and B
      * <: C, then A <: f^-1(C), so C should flow into the upper approximation
      * of A. Conversely if A <: C2, then we also know that B <: f(C2). These
      * situations are modeled by the two branches of the conditional below.
      */
    override def computeApprox(origin: CaptureSet)(using Context): CaptureSet =
  ...
  class Filtered private[CaptureSet] (
    ...
    override def addNewElems(newElems: Refs, origin: CaptureSet)(using
      ...
      if origin eq source then super.addNewElems(filtered, origin)
      else
        ...
        super
          .addNewElems(newElems, origin)
          .andAlso {
            if filtered.size == newElems.size then
              source.tryInclude(newElems, this)
            else CompareResult.fail(this)
          }
    ...
    override def toString =
      s"${getClass.getSimpleName}$id($source, elems = $elems)"
  end Filtered

  /** A variable with elements given at any time as { x <- source.elems |
    * !other.accountsFor(x) }
    */
  ...
  class Intersected(cs1: CaptureSet, cs2: CaptureSet)(using Context)
    ...
    override def computeApprox(origin: CaptureSet)(using Context): CaptureSet =
      if (origin eq cs1) || (origin eq cs2) then
        ...
        CaptureSet(
          elemIntersection(cs1.upperApprox(this), cs2.upperApprox(this))
        )
  ...
  def elemIntersection(cs1: CaptureSet, cs2: CaptureSet)(using Context): Refs =
    cs1.elems.filter(cs2.mightAccountFor) ++ cs2.elems.filter(
      cs1.mightAccountFor
    )
  ...
  /** Extrapolate tm(r) according to `variance`. Let r1 be the result of tm(r).
    ...
    */
  def extrapolateCaptureRef(r: CaptureRef, tm: TypeMap, variance: Int)(using
      Context
  ): CaptureSet =
    val r1 = tm(r)
    val upper = r1.captureSet
    def isExact =
      upper.isAlwaysEmpty || upper.isConst && upper.elems.size == 1 && upper.elems
        .contains(r1)
    if variance > 0 || isExact then upper
      ...
      assert(
        false,
        i"trying to add $upper from $r via ${tm.getClass} in a non-variant setting"
      )

  /** Apply `f` to each element in `xs`, and join result sets with `++` */
  def mapRefs(xs: Refs, f: CaptureRef => CaptureSet)(using
      Context
  ): CaptureSet =
  ...
  /** Return true iff
    *   - arg1 is a TypeBounds >: CL T <: CH T of two capturing types with equal
    *     parents.
  ...
  def subCapturesRange(arg1: TypeBounds, arg2: Type)(using Context): Boolean =
    arg1 match
      case TypeBounds(CapturingType(lo, loRefs), CapturingType(hi, hiRefs))
          if lo =:= hi =>
  ...
  /** A TypeMap with the property that every capture reference in the image of
    * the map is mapped to itself. I.e. for all capture references r1, r2, if
    * M(r1) == r2 then M(r2) == r2.
    */
  ...
  /** The result of subcapturing comparisons is an opaque type
    * CompareResult.TYPE. This is either OK, indicating success, or another
    * capture set, indicating failure. The failure capture set is the one that
    * did not allow propagaton of elements into it.
    */
  ...
  /** A VarState serves as a snapshot mechanism that can undo additions of
    * elements or super sets if an operation fails
    */
  class VarState:

    /** A map from captureset variables to their elements at the time of the
      * snapshot.
      */
    private val elemsMap: util.EqHashMap[Var, Refs] = new util.EqHashMap

    /** A map from captureset variables to their dependent sets at the time of
      * the snapshot.
      */
    ...
    /** Optionally the recorded elements of `v`, None if nothing was recorded
      * for `v`
      */
    def getElems(v: Var): Option[Refs] = elemsMap.get(v)

    /** Record elements, return whether this was allowed. By default, recording
      * is allowed but the special state FrozenState overrides this.
      */
    def putElems(v: Var, elems: Refs): Boolean =
      elemsMap(v) = elems; true

    /** The recorded dependent sets of `v` (it's required that a recording was
      * made)
      */
    def deps(v: Var): Deps = depsMap(v)

    /** Optionally the recorded dependent sets of `v`, None if nothing was
      * recorded for `v`
      */
    def getDeps(v: Var): Option[Deps] = depsMap.get(v)

    /** Record dependent sets, return whether this was allowed. By default,
      * recording is allowed but the special state FrozenState overrides this.
      */
  ...
  @sharable
  /** A special state that turns off recording of elements. Used only in
    * `addSub` to prevent cycles in recordings.
    */
  ...
  def ofType(tp: Type)(using Context): CaptureSet =
    def recur(tp: Type): CaptureSet = tp.dealias match
      ...
      case tp: TermParamRef =>
        tp.captureSet
      case _: TypeRef =>
        if tp.classSymbol.hasAnnotation(defn.CapabilityAnnot) then universal
        else empty
      case _: TypeParamRef =>
        empty
  ...
  /** Perform `op`. Under -Ycc-debug, collect and print info about all variables
    * reachable via `(_.deps)*` from the variables that were shown in `op`.
    */
