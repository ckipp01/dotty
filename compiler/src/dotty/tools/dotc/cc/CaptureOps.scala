...
private[cc] def retainedElems(tree: Tree)(using Context): List[Tree] =
  tree match
...
extension (tree: Tree)
  ...
  /** Map tree with CaptureRef type to its type, throw IllegalCaptureRef
    * otherwise
    */
  ...
  /** Convert a @retains or @retainsByName annotation tree to the capture set it
    * represents. For efficience, the result is cached as an Attachment on the
    * tree.
    */
  ...
  /** Under pureFunctions, add a @retainsByName(*)` annotation to the argument
    * of a by name parameter type, turning the latter into an impure by name
    * parameter type.
    */
...
extension (tp: Type)
  ...
  def derivedCapturingType(parent: Type, refs: CaptureSet)(using
      Context
  ): Type = tp match
  ...
  /** If this is a unboxed capturing type with nonempty capture set, its boxed
    * version. Or, if type is a TypeBounds of capturing types, the version where
    * the bounds are boxed. The identity for all other types.
    */
  def boxed(using Context): Type = tp.dealias match
    case tp @ CapturingType(parent, refs)
        if !tp.isBoxed && !refs.isAlwaysEmpty =>
      tp.annot match
  ...
  def boxedUnlessFun(tycon: Type)(using Context) =
    if ctx.phase != Phases.checkCapturesPhase || defn.isFunctionSymbol(
        tycon.typeSymbol
      )
    then tp
    else tp.boxed

  /** The capture set consisting of all top-level captures of `tp` that appear
    * under a box. Unlike for `boxed` this also considers parents of capture
    * types, unions and intersections, and type proxies other than abstract
    * types.
    */
  ...
  /** If this type is a capturing type, the version with boxed statues as given
    * by `boxed`. If it is a TermRef of a capturing type, and the box status
    * flips, widen to a capturing type that captures the TermRef.
    */
  ...
  /** Map capturing type to their parents. Capturing types accessible via
    * dealising are also stripped.
    */
  ...
  def adaptFunctionTypeUnderPureFuns(using Context): Type = tp match
    case AppliedType(fn, args)
        if Feature.pureFunsEnabledSomewhere && defn.isFunctionClass(
          fn.typeSymbol
        ) =>
  ...
  def adaptByNameArgUnderPureFuns(using Context): Type =
    if Feature.pureFunsEnabledSomewhere then
      AnnotatedType(
        tp,
        CaptureAnnotation(CaptureSet.universal, boxed = false)(
          defn.RetainsByNameAnnot
        )
      )
    else tp
  ...
  /** Is type known to be always pure by its class structure, so that adding a
    * capture set to it would not make sense?
    */
...
extension (sym: Symbol)
  ...
  /** A class is pure if:
    *   - one its base types has an explicitly declared self type with an empty
    *     capture set
  ...
  /** Does this symbol allow results carrying the universal capability?
    * Currently this is true only for function type applies (since their results
    * are unboxed) and `erasedValue` since this function is magic in that is
    * allows to conjure global capabilies from nothing (aside: can we find a
    * more controlled way to achieve this?). But it could be generalized to
    * other functions that so that they can take capability classes as
    * arguments.
    */
  ...
  /** When applying `sym`, would the result type be unboxed? This is the case if
    * the result type contains a top-level reference to an enclosing class or
    * method type parameter and the method does not allow root capture. If the
    * type parameter is instantiated to a boxed type, that type would have to be
    * unboxed in the method's result.
    */
  def unboxesResult(using Context): Boolean =
    def containsEnclTypeParam(tp: Type): Boolean = tp.strippedDealias match
      ...
      case tp: AndOrType =>
        containsEnclTypeParam(tp.tp1) || containsEnclTypeParam(tp.tp2)
      case tp: RefinedType =>
        containsEnclTypeParam(tp.parent) || containsEnclTypeParam(
          tp.refinedInfo
        )
      case _ => false
...
extension (ts: List[Type])
  /** Equivalent to ts.mapconserve(_.boxedUnlessFun(tycon)) but more efficient
    * where it is the identity.
    */
  def boxedUnlessFun(tycon: Type)(using Context) =
    if ctx.phase != Phases.checkCapturesPhase || defn.isFunctionClass(
        tycon.typeSymbol
      )
    then ts
