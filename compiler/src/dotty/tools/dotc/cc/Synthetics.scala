...
/** Classification and transformation methods for synthetic case class methods
  * that need to be treated specially. In particular, compute capturing types
  * for some of these methods which have inferred (result-)types that need to be
  * established under separate compilation.
  */
object Synthetics:
  private def isSyntheticCopyMethod(sym: SymDenotation)(using Context) =
    sym.name == nme.copy && sym.is(Synthetic) && sym.owner.isClass && sym.owner
      .is(Case)

  private def isSyntheticCompanionMethod(sym: SymDenotation, names: Name*)(using
      Context
  ): Boolean =
    names.contains(sym.name) && sym.is(Synthetic) && sym.owner.is(
      Module
    ) && sym.owner.companionClass.is(Case)

  private def isSyntheticCopyDefaultGetterMethod(sym: SymDenotation)(using
      Context
  ) = sym.name match
    case DefaultGetterName(nme.copy, _) =>
      sym.is(Synthetic) && sym.owner.isClass && sym.owner.is(Case)
    case _ => false

  /** Is `sym` a synthetic apply, copy, or copy default getter method? The types
    * of these symbols are transformed in a special way without looking at the
    * definitions's RHS
    */
  ...
  /** Method is excluded from regular capture checking. Excluded are synthetic
    * class members
  ...
  def isExcluded(sym: Symbol)(using Context): Boolean =
    sym.is(Synthetic)
      && sym.owner.isClass
      && (defn.caseClassSynthesized.exists(ccsym =>
        sym.overriddenSymbol(ccsym.owner.asClass) == ccsym
      )
        || isSyntheticCompanionMethod(sym, nme.fromProduct)
        || needsTransform(sym))

  /** Add capture dependencies to the type of the `apply` or `copy` method of a
    * case class. An apply method in a case class like this: case class CC(a:
    * {d} A, b: B, {cap} c: C) would get type def apply(a': {d} A, b: B, {cap}
    * c': C): {a', c'} CC { val a = {a'} A, val c = {c'} C } where `'` is used
    * to indicate the difference between parameter symbol and refinement name.
    * Analogous for the copy method.
    */
  private def addCaptureDeps(info: Type)(using Context): Type = info match
    case info: MethodType =>
      val trackedParams =
        info.paramRefs.filter(atPhase(checkCapturesPhase)(_.isTracked))
      ...
      if trackedParams.isEmpty then info
        ...
        augmentResult(info).showing(
          i"augment apply/copy type $info to $result",
          capt
        )
    case info: PolyType =>
  ...
  /** Drop capture dependencies from the type of `apply` or `copy` method of a
    * case class
    */
  ...
  /** Add capture information to the type of the default getter of a case class
    * copy method
    */
  private def addDefaultGetterCapture(info: Type, owner: Symbol, idx: Int)(using
      Context
  ): Type = info match
    case info: MethodOrPoly =>
      info.derivedLambdaType(resType =
        addDefaultGetterCapture(info.resType, owner, idx)
      )
    case info: ExprType =>
  ...
  /** Drop capture information from the type of the default getter of a case
    * class copy method
    */
  private def dropDefaultGetterCapture(info: Type)(using Context): Type =
    info match
      case info: MethodOrPoly =>
  ...
  private def addUnapplyCaptures(info: Type)(using Context): Type = info match
    case info: MethodType =>
      ...
      info
        .derivedLambdaType(
          paramInfos = newParamInfo :: Nil,
          resType = newResult(info.resType)
        )
      ...
      info.paramInfos match
        case CapturingType(oldParamInfo, _) :: Nil =>
          ...
          info.derivedLambdaType(
            paramInfos = oldParamInfo :: Nil,
            resType = oldResult(info.resType)
          )
        case _ =>
          info
    case info: PolyType =>
  ...
  /** If `sym` refers to a synthetic apply, unapply, copy, or copy default
    * getter method of a case class, transform it to account for capture
    * information. The method is run in phase CheckCaptures.Pre
    * @pre
    *   needsTransform(sym)
    */
  def transformToCC(sym: SymDenotation)(using Context): SymDenotation =
    sym.name match
      case DefaultGetterName(nme.copy, n) =>
        sym.copySymDenotation(info =
          addDefaultGetterCapture(sym.info, sym.owner, n)
        )
      case nme.unapply =>
      ...
      case n if n == nme.eq || n == nme.ne =>
        sym.copySymDenotation(info =
          MethodType(
            defn.ObjectType.capturing(CaptureSet.universal) :: Nil,
            defn.BooleanType
          )
        )

  /** If `sym` refers to a synthetic apply, unapply, copy, or copy default
    * getter method of a case class, transform it back to what it was before the
    * CC phase.
    * @pre
    *   needsTransform(sym)
    */
