...
object CheckCaptures:
  ...
  class Pre extends PreRecheck, SymTransformer:
    ...
    def transformSym(sym: SymDenotation)(using Context): SymDenotation =
      if sym.isAllOf(PrivateParamAccessor) && !sym.hasAnnotation(
          defn.ConstructorOnlyAnnot
        )
      then
        sym.copySymDenotation(initFlags =
          sym.flags &~ Private | Recheck.ResetPrivate
        )
      else if Synthetics.needsTransform(sym) then Synthetics.transformToCC(sym)
      else sym
  end Pre
  ...
  /** A class describing environments.
    ...
    *   the current owner
    * @param nestedInOwner
    *   true if the environment is a temporary one nested in the owner's
    *   environment, and does not have a different actual owner symbol (this
    *   happens when doing box adaptation).
    * @param captured
    *   the caputure set containing all references to tracked free variables
    *   outside of boxes
    * @param isBoxed
    *   true if the environment is inside a box (in which case references are
    *   not counted)
    * @param outer0
  ...
  /** Similar normal substParams, but this is an approximating type map that
    * maps parameters in contravariant capture sets to the empty set. TODO:
    * check what happens with non-variant.
    */
  final class SubstParamsMap(from: BindingType, to: List[Type])(using Context)
    ...
    private val isExactSubstitution: Boolean =
      to.forall(_.isInstanceOf[CaptureRef])

    /** As long as this substitution is exact, there is no need to create
      * `Range`s when mapping invariant positions.
      */
    override protected def needsRangeIfInvariant(refs: CaptureSet): Boolean =
      !isExactSubstitution
  ...
  /** Check that a @retains annotation only mentions references that can be
    * tracked. This check is performed at Typer.
    */
      ...
      elem.tpe match
        case ref: CaptureRef =>
          if !ref.canBeTracked then
            report.error(
              em"$elem cannot be tracked since it is not a parameter or local value",
              elem.srcPos
            )
        case tpe =>
          report.error(
            em"$elem: $tpe is not a legal element of a capture set",
            elem.srcPos
          )

  /** If `tp` is a capturing type, check that all references it mentions have
    * non-empty capture sets. Also: warn about redundant capture annotations.
    ...
    case CapturingType(parent, refs) =>
      for ref <- refs.elems do
        if ref.captureSetOfInfo.elems.isEmpty then
          report.error(
            em"$ref cannot be tracked since its capture set is empty",
            pos
          )
        else if parent.captureSet.accountsFor(ref) then
          report.warning(
            em"redundant capture: $parent already accounts for $ref",
            pos
          )
    case _ =>

  /** Warn if `ann`, which is a tree of a @retains annotation, defines some
    * elements that are already accounted for by other elements of the same
    * annotation. Note: We need to perform the check on the original annotation
    * rather than its capture set since the conversion to a capture set already
  ...
  def warnIfRedundantCaptureSet(ann: Tree)(using Context): Unit =
    ...
    def choices(prev: List[Tree], elems: List[Tree]): List[List[Tree]] =
      elems match
        case Nil => Nil
        case elem :: elems =>
    ...
    for case first :: others <- choices(Nil, retainedElems(ann)) do
      ...
      if remaining.accountsFor(firstRef) then
        report.warning(
          em"redundant capture: $remaining already accounts for $firstRef",
          ann.srcPos
        )

  def disallowRootCapabilitiesIn(
      tp: Type,
      what: String,
      have: String,
      addendum: String,
...
class CheckCaptures extends Recheck, SymTransformer:
  ...
  override def run(using Context): Unit =
    if Feature.ccEnabled then super.run
  ...
  class CaptureChecker(ictx: Context) extends Rechecker(ictx):
    ...
    override def keepType(tree: Tree) =
      super.keepType(tree)
        || tree
          .isInstanceOf[Try] // type of `try` needs tp be checked for * escapes

    /** Instantiate capture set variables appearing contra-variantly to their
      * upper approximation.
      */
    private def interpolator(startingVariance: Int = 1)(using Context) =
      new TypeTraverser:
          ...
          t match
            case CapturingType(parent, refs: CaptureSet.Var) =>
              if variance < 0 then
                ...
                refs.solve()
              traverse(parent)
            case t @ RefinedType(_, nme.apply, rinfo)
                if defn.isFunctionOrPolyType(t) =>
              traverse(rinfo)
            case tp: TypeVar =>
            case tp: TypeRef =>
              traverse(tp.prefix)
    ...
    /** If `tpt` is an inferred type, interpolate capture set variables
      * appearing contra- variantly in it.
      */
    private def interpolateVarsIn(tpt: Tree)(using Context): Unit =
      if tpt.isInstanceOf[InferredTypeTree] then
        interpolator()
          .traverse(tpt.knownType)
    ...
    def assertSub(cs1: CaptureSet, cs2: CaptureSet)(using Context) =
      assert(
        cs1.subCaptures(cs2, frozen = false).isOK,
        i"$cs1 is not a subset of $cs2"
      )

    /** Check subcapturing `{elem} <: cs`, report error on failure */
    def checkElem(elem: CaptureRef, cs: CaptureSet, pos: SrcPos)(using
        Context
    ) =
      val res = elem.singletonCaptureSet.subCaptures(cs, frozen = false)
      if !res.isOK then
        report.error(
          em"$elem cannot be referenced here; it is not included in the allowed capture set ${res.blocking}",
          pos
        )

    /** Check subcapturing `cs1 <: cs2`, report error on failure */
    def checkSubset(cs1: CaptureSet, cs2: CaptureSet, pos: SrcPos)(using
        Context
    ) =
      ...
      if !res.isOK then
        def header =
          if cs1.elems.size == 1 then
            i"reference ${cs1.elems.toList}%, % is not"
          else i"references $cs1 are not all"
        report.error(
          em"$header included in allowed capture set ${res.blocking}",
          pos
        )

    /** The current environment */
    private var curEnv: Env = Env(
      NoSymbol,
      nestedInOwner = false,
      CaptureSet.empty,
      isBoxed = false,
    ...
    /** If `sym` is a class or method nested inside a term, a capture set
      * variable representing the captured variables of the environment
      * associated with `sym`.
      */
    def capturedVars(sym: Symbol)(using Context) =
      myCapturedVars.getOrElseUpdate(
        sym,
    ...
    def forallOuterEnvsUpTo(limit: Symbol)(op: Env => Unit)(using
        Context
    ): Unit =
    ...
    /** Include `sym` in the capture sets of all enclosing environments nested
      * in the the environment in which `sym` is defined.
      */
      ...
      if sym.exists then
        ...
        if ref.isTracked then
          forallOuterEnvsUpTo(sym.enclosure) { env =>
            capt.println(
              i"Mark $sym with cs ${ref.captureSet} free in ${env.owner}"
            )
            checkElem(ref, env.captured, pos)
          }

    /** Make sure (projected) `cs` is a subset of the capture sets of all
      * enclosing environments. At each stage, only include references from `cs`
    ...
    /** Include references captured by the called method in the current
      * environment stack
      */
    ...
    /** A specialized implementation of the selection rule.
      ...
      * outcome of a `mightSubcapture` test. It picks `{f}` if this might
      * subcapture Cr and Cr otherwise.
      */
    override def recheckSelection(
        tree: Select,
        qualType: Type,
        name: Name,
        pt: Type
      ...
      if selCs.isAlwaysEmpty || selType.widen.isBoxedCapturing || qualType.isBoxedCapturing
      then selType
      else
        val qualCs = qualType.captureSet
        capt.println(
          i"intersect $qualType, ${selType.widen}, $qualCs, $selCs in $tree"
        )
        if qualCs.mightSubcapture(selCs)
          && !selCs.mightSubcapture(qualCs)
          && !pt.stripCapturing.isInstanceOf[SingletonType]
        then
          selType.widen.stripCapturing
            .capturing(qualCs)
            .showing(
              i"alternate type for select $tree: $selType --> $result, $qualCs / $selCs",
              capt
            )
        else selType
    ...
    /** A specialized implementation of the apply rule.
      ...
      * ------------------------ E |- f a: C Rr
      *
      * The implementation picks as `C` one of `{f, a}` or `Cr`, depending on
      * the outcome of a `mightSubcapture` test. It picks `{f, a}` if this might
      * subcapture Cr and Cr otherwise.
      */
    override def recheckApply(tree: Apply, pt: Type)(using Context): Type =
      ...
      if meth == defn.Caps_unsafeBox then mapArgUsing(_.forceBoxStatus(true))
        ...
        mapArgUsing {
          case defn.FunctionOf(paramtpe :: Nil, restpe, isContectual) =>
            defn.FunctionOf(
              paramtpe.forceBoxStatus(true) :: Nil,
              restpe,
              isContectual
            )
        }
            ...
            tree.fun match
              case Select(qual, _)
                  if !tree.fun.symbol.isConstructor
                    ...
                    && tree.args.forall(
                      _.tpe.captureSet.mightSubcapture(refs)
                    ) =>
                val callCaptures =
                  tree.args.foldLeft(qual.tpe.captureSet)((cs, arg) =>
                    cs ++ arg.tpe.captureSet
                  )
                appType
                  .derivedCapturingType(appType1, callCaptures)
                  .showing(
                    i"narrow $tree: $appType, refs = $refs, qual = ${qual.tpe.captureSet} --> $result",
                    capt
                  )
              case _ => appType
    ...
    /** Handle an application of method `sym` with type `mt` to arguments of
      * types `argTypes`. This means:
      *   - Instantiate result type with actual arguments
      *   - If call is to a constructor:
      *     - remember types of arguments corresponding to tracked parameters in
      *       refinements.
      *     - add capture set of instantiated class to capture set of result
      *       type.
      */
    override def instantiate(mt: MethodType, argTypes: List[Type], sym: Symbol)(
        using Context
    ): Type =
      val ownType =
      ...
      if sym.isConstructor then
        ...
        /** First half of result pair: Refine the type of a constructor call
          * `new C(t_1, ..., t_n)` to C{val x_1: T_1, ..., x_m: T_m} where x_1,
          * ..., x_m are the tracked parameters of C and T_1, ..., T_m are the
          * types of the corresponding arguments.
          *
          * Second half: union of all capture sets of arguments to tracked
          * parameters.
          */
        def addParamArgRefinements(
            core: Type,
            initCs: CaptureSet
        ): (Type, CaptureSet) =
          mt.paramNames.lazyZip(argTypes).foldLeft((core, initCs)) {
            (acc, refine) =>
              val (core, allCaptures) = acc
              val (getterName, argType) = refine
              val getter =
                cls.info.member(getterName).suchThat(_.is(ParamAccessor)).symbol
              if getter.termRef.isTracked && !getter.is(Private)
              then
                (
                  RefinedType(core, getterName, argType),
                  allCaptures ++ argType.captureSet
                )
              else (core, allCaptures)
          }

        def augmentConstructorType(core: Type, initCs: CaptureSet): Type =
          core match
            case core: MethodType =>
              // more parameters to follow; augment result type
              core.derivedLambdaType(resType =
                augmentConstructorType(core.resType, initCs)
              )
            case CapturingType(parent, refs) =>
        ...
        augmentConstructorType(ownType, CaptureSet.empty) match
          ...
          case augmented =>
            ...
            augmented
              .capturing(capturedVars(cls) ++ capturedVars(sym))
              .showing(
                i"constr type $mt with $argTypes%, % in $cls = $result",
                capt
              )
      else ownType
      end if
    ...
    override def recheckClosure(tree: Closure, pt: Type)(using Context): Type =
      ...
      super
        .recheckClosure(tree, pt)
        .capturing(cs)
        .showing(i"rechecked $tree / $pt = $result", capt)

    /** Additionally to normal processing, update types of closures if the
      * expected type is a function with only pure parameters. In that case,
      * make the anonymous function also have the same parameters as the
      * prototype. TODO: Develop a clearer rationale for this. TODO: Can we
      * generalize this to arbitrary parameters? Currently some tests fail if we
      * do this. (e.g. neg.../stackAlloc.scala, others)
      */
        ...
        case closureDef(mdef) =>
          pt.dealias match
            case defn.FunctionOf(ptformals, _, _)
                if ptformals.nonEmpty && ptformals.forall(
                  _.captureSet.isAlwaysEmpty
                ) =>
              ...
              meth.paramSymss.head.lazyZip(ptformals).foreach {
                (psym, pformal) =>
                  psym.updateInfoBetween(
                    preRecheckPhase,
                    thisPhase,
                    pformal.mapExprType
                  )
              ...
              val completer = new LazyType:
                def complete(denot: SymDenotation)(using Context) =
                  denot.info = mt
                    .companion(ptformals, mdef.tpt.knownType)
    ...
    override def recheckValDef(tree: ValDef, sym: Symbol)(using Context): Unit =
      try
        if !sym.is(Module)
        then // Modules are checked by checking the module class
    ...
    override def recheckDefDef(tree: DefDef, sym: Symbol)(using Context): Unit =
      if !Synthetics.isExcluded(sym) then
        ...
        if !localSet.isAlwaysEmpty then
          curEnv =
            Env(sym, nestedInOwner = false, localSet, isBoxed = false, curEnv)
    ...
    /** Class-specific capture set relations:
      *   1. The capture set of a class includes the capture sets of its
      *      parents. 2. The capture set of the self type of a class includes
      *      the capture set of the class. 3. The capture set of the self type
      *      of a class includes the capture set of every class parameter,
      *      unless the parameter is marked @constructorOnly.
      */
    override def recheckClassDef(
        tree: TypeDef,
        impl: Template,
        cls: ClassSymbol
    )(using Context): Type =
      ...
      for parent <- impl.parents do // (1)
        checkSubset(
          capturedVars(parent.tpe.classSymbol),
          localSet,
          parent.srcPos
        )
      if !localSet.isAlwaysEmpty then
        curEnv =
          Env(cls, nestedInOwner = false, localSet, isBoxed = false, curEnv)
      try
        val thisSet = cls.classInfo.selfType.captureSet
          .withDescription(i"of the self type of $cls")
    ...
    /** If type is of the form `T @requiresCapability(x)`, mark `x` as free in
      * the current environment. This is used to require the correct `CanThrow`
      * capability when encountering a `throw`.
      */
    override def recheckTyped(tree: Typed)(using Context): Type =
      tree.tpt.tpe match
        case AnnotatedType(_, annot)
            if annot.symbol == defn.RequiresCapabilityAnnot =>
          annot.tree match
    ...
    /** If expected type `pt` is boxed and the tree is a function or a
      * reference, don't propagate free variables. Otherwise, if the result type
      * is boxed, simulate an unboxing by adding all references in the boxed
      * capture set to the current environment.
      */
    override def recheck(tree: Tree, pt: Type = WildcardType)(using
        Context
    ): Type =
    ...
    /** If `tree` is a reference or an application where the result type refers
      * to an enclosing class or method parameter of the reference, check that
      * the result type does not capture the universal capability. This is
      * justified since the result type would have to be implicitly unboxed.
      * TODO: Can we find a cleaner way to achieve this? Logically, this should
      * be part of simulated boxing and unboxing.
      */
    override def recheckFinish(tpe: Type, tree: Tree, pt: Type)(using
        Context
    ): Type =
      val typeToCheck = tree match
        case _: Ident | _: Select | _: Apply | _: TypeApply
            if tree.symbol.unboxesResult =>
          tpe
        case _: Try =>
          tpe
        case _ =>
      ...
      def checkNotUniversal(tp: Type): Unit = tp.widenDealias match
        case wtp @ CapturingType(parent, refs) =>
          refs.disallowRootCapability { () =>
            val kind =
              if tree.isInstanceOf[ValDef] then "mutable variable"
              else "expression"
            report.error(
    ...
    /** Massage `actual` and `expected` types using the methods below before
      * checking conformance
      */
    override def checkConformsExpr(actual: Type, expected: Type, tree: Tree)(
        using Context
    ): Unit =
      val expected1 = alignDependentFunction(
        addOuterRefs(expected, actual),
    ...
    private def toDepFun(
        args: List[Type],
        resultType: Type,
        isContextual: Boolean
    )(using Context): Type =
      MethodType
        .companion(isContextual = isContextual)(args, resultType)
        .toFunctionType(isJava = false, alwaysDependent = true)

    /** Turn `expected` into a dependent function when `actual` is dependent. */
    private def alignDependentFunction(expected: Type, actual: Type)(using
        Context
    ): Type =
      def recur(expected: Type): Type = expected.dealias match
        ...
        case expected @ defn.FunctionOf(args, resultType, isContextual)
            if defn.isNonRefinedFunction(expected) && defn.isFunctionType(
              actual
            ) && !defn.isNonRefinedFunction(actual) =>
    ...
    /** For the expected type, implement the rule outlined in #14390:
      *   - when checking an expression `a: Ca Ta` against an expected type `Ce
      *     Te`,
      *   - where the capture set `Ce` contains Cls.this,
      *   - and where and all method definitions enclosing `a` inside class
      *     `Cls` have only pure parameters,
      ...
      */
    private def addOuterRefs(expected: Type, actual: Type)(using
        Context
    ): Type =
      def isPure(info: Type): Boolean = info match
        case info: PolyType => isPure(info.resType)
        case info: MethodType =>
          info.paramInfos.forall(_.captureSet.isAlwaysEmpty) && isPure(
            info.resType
          )
        case _ => true
        ...
        (erefs /: erefs.elems) { (erefs, eref) =>
          eref match
            case eref: ThisType if isPureContext(ctx.owner, eref.cls) =>
              erefs ++ arefs.filter {
                case aref: TermRef =>
                  eref.cls.isProperlyContainedIn(aref.symbol.owner)
      ...
      expected match
        case CapturingType(ecore, erefs) =>
          ...
          if erefs1 ne erefs then
            capt.println(
              i"augmented $expected from ${actual.captureSet} --> $erefs1"
            )
    ...
    /** Adapt `actual` type to `expected` type by inserting boxing and unboxing
      * conversions
      *
      * @param alwaysConst
    ...
    def adaptBoxed(
        ...
        expected: Type,
        pos: SrcPos,
        alwaysConst: Boolean = false
    )(using Context): Type =

      /** Adapt function type `actual`, which is `aargs -> ares` (possibly with
        * dependencies) to `expected` type. It returns the adapted type along
        * with the additionally captured variable during adaptation.
        * @param reconstruct
        *   how to rebuild the adapted function type
        */
      def adaptFun(
          actual: Type,
          aargs: List[Type],
          ...
          covariant: Boolean,
          boxed: Boolean,
          reconstruct: (List[Type], Type) => Type
      ): (Type, CaptureSet) =
        val saved = curEnv
        curEnv = Env(
          curEnv.owner,
        ...
        try
          val (eargs, eres) = expected.dealias.stripCapturing match
            ...
            case expected @ RefinedType(_, _, rinfo: MethodType)
                if defn.isFunctionType(expected) =>
              (rinfo.paramInfos, rinfo.resType)
            case _ => (aargs.map(_ => WildcardType), WildcardType)
          val aargs1 = aargs.zipWithConserve(eargs) { (aarg, earg) =>
            adapt(aarg, earg, !covariant)
          }
      ...
      def adapt(actual: Type, expected: Type, covariant: Boolean): Type =
        trace(adaptInfo(actual, expected, covariant), recheckr, show = true) {
          if expected.isInstanceOf[WildcardType] then actual
            ...
            val styp =
              if actual.isFromJavaObject then actual else actual.stripCapturing
            ...
            val (styp1, leaked) = styp match
              case actual @ AppliedType(tycon, args)
                  if defn.isNonRefinedFunction(actual) =>
                adaptFun(
                  actual,
                  args.init,
                  args.last,
                  ...
                  insertBox,
                  (aargs1, ares1) =>
                    actual.derivedAppliedType(tycon, aargs1 :+ ares1)
                )
              case actual @ RefinedType(_, _, rinfo: MethodType)
                  if defn.isFunctionOrPolyType(actual) =>
                ...
                adaptFun(
                  ...
                  (aargs1, ares1) =>
                    rinfo
                      .derivedLambdaType(paramInfos = aargs1, resType = ares1)
                      .toFunctionType(isJava = false, alwaysDependent = true)
                )
              case actual: MethodType =>
                adaptFun(
                  ...
                  insertBox,
                  (aargs1, ares1) =>
                    actual.derivedLambdaType(
                      paramInfos = aargs1,
                      resType = ares1
                    )
                )
              case actual @ RefinedType(p, nme, rinfo: PolyType)
                  if defn.isFunctionOrPolyType(actual) =>
                adaptTypeFun(
                  actual,
                  rinfo.resType,
            ...
            def adaptedType(resultBoxed: Boolean) =
              styp1
                .capturing(if alwaysConst then CaptureSet(cs1.elems) else cs1)
            ...
            if needsAdaptation then
              ...
              if criticalSet.isUniversal && expected.isValueType && !allowUniversalInBoxed
              then
    ...
    /** Check overrides again, taking capture sets into account. TODO: Can we
      * avoid doing overrides checks twice? We need to do them here since only
      * at this phase CaptureTypes are relevant But maybe we can then elide the
      * check during the RefChecks phase under captureChecking?
      */
    def checkOverrides = new TreeTraverser:
      class OverridingPairsCheckerCC(
          clazz: ClassSymbol,
          self: Type,
          srcPos: SrcPos
      )(using Context)
          extends OverridingPairsChecker(clazz, self):
        /** Check subtype with box adaptation. This function is passed to
          * RefChecks to check the compatibility of overriding pairs.
          * @param sym
          *   symbol of the field definition that is being checked
          */
        override def checkSubType(actual: Type, expected: Type)(using
            Context
        ): Boolean =
          val expected1 = alignDependentFunction(
            addOuterRefs(expected, actual),
          ...
          val actual1 =
            ...
            try
              ...
              val adapted =
                adaptBoxed(actual, expected1, srcPos, alwaysConst = true)
              actual match
                case _: MethodType =>
      ...
      def traverse(t: Tree)(using Context) =
        t match
          case t: Template =>
            checkAllOverrides(
              ctx.owner.asClass,
              OverridingPairsCheckerCC(_, _, t)
            )
          case _ =>
        traverseChildren(t)
    ...
    override def checkUnit(unit: CompilationUnit)(using Context): Unit =
      ...
      withCaptureSetsExplained {
        ...
        if ctx.settings.YccDebug.value then
          show(
            unit.tpdTree
          ) // this does not print tree, but makes its variables visible for dependency printing
      }

    /** Check that self types of subclasses conform to self types of super
      * classes. (See comment below how this is achieved). The check assumes
      * that classes without an explicit self type have the universal capture
      * set `{cap}` on the self type. If a class without explicit self type is
      * not `effectivelyFinal` it is checked that the inferred self type is
      * universal, in order to assure that joint and separate compilation give
      * the same result.
      */
    def checkSelfTypes(unit: tpd.Tree)(using Context): Unit =
      val parentTrees = mutable.HashMap[Symbol, List[Tree]]()
      unit.foreachSubTree {
        case cdef @ TypeDef(_, impl: Template) =>
          parentTrees(cdef.symbol) = impl.parents
        case _ =>
      }
      ...
      while parentTrees.nonEmpty do
        val roots = parentTrees.keysIterator.filter { cls =>
          !parentTrees(cls).exists(ptree =>
            parentTrees.contains(ptree.tpe.classSymbol)
          )
        }
          ...
          if !root.isEffectivelySealed then
            ...
            selfType match
              case CapturingType(_, refs: CaptureSet.Var)
                  if !refs.isUniversal && !matchesExplicitRefsInBaseClass(
                    refs,
                    root
                  ) =>
    ...
    /** Heal ill-formed capture sets in the type parameter.
      *
      * We can push parameter refs into a capture set in type parameters that
      * this type parameter can't see. For example, when capture checking the
      ...
      * } }
      *
      * We may propagate `f` into ?1, making ?1 ill-formed. This also causes
      * soundness issues, since `f` in ?1 should be widened to `cap`, giving
      * rise to an error that `cap` cannot be included in a boxed capture set.
      *
      * To solve this, we still allow ?1 to capture parameter refs like `f`, but
      * compensate this by pushing the widened capture set of `f` into ?1. This
      * solves the soundness issue caused by the ill-formness of ?1.
      */
    ...
    /** Perform the following kinds of checks
      *   - Check all explicitly written capturing types for well-formedness
      *     using `checkWellFormedPost`.
      *   - Check that externally visible `val`s or `def`s have empty capture
      *     sets. If not, suggest an explicit type. This is so that separate
      *     compilation (where external symbols have empty capture sets) gives
      *     the same results as joint compilation.
      *   - Check that arguments of TypeApplys and AppliedTypes conform to their
      *     bounds.
    ...
    def postCheck(unit: tpd.Tree)(using Context): Unit =
      val checker = new TreeTraverser:
        ...
        def check(tree: Tree) = tree match
          ...
          case t: ValOrDefDef
              if t.tpt.isInstanceOf[InferredTypeTree] && !Synthetics.isExcluded(
                t.symbol
              ) =>
            val sym = t.symbol
            val isLocal =
              sym.owner.ownersIterator.exists(_.isTerm)
                || sym
                  .accessBoundary(defn.RootClass)
                  .isContainedIn(sym.topLevelClass)
            ...
            if !canUseInferred then
              ...
              def checkPure(tp: Type) = tp match
                case CapturingType(_, refs)
                  ...
                  val resultStr =
                    if t.isInstanceOf[DefDef] then " result" else ""
                  report.error(
          ...
          case t @ TypeApply(fun, args) =>
            fun.knownType.widen match
              case tl: PolyType =>
                val normArgs =
                  args.lazyZip(tl.paramInfos).map { (arg, bounds) =>
                    arg.withType(
                      arg.knownType.forceBoxStatus(
