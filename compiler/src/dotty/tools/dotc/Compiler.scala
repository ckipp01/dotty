...
class Compiler:
  ...
  /** Meta-ordering constraint:
    *
    * DenotTransformers that change the signature of their denotation's info
    * must go after erasure. The reason is that denotations are permanently
    * referred to by TermRefs which contain a signature. If the signature of a
    * symbol would change, all refs to it would become outdated - they could not
    * be dereferenced in the new phase.
    *
    * After erasure, signature changing denot-transformers are OK because
    * signatures are never recomputed later than erasure.
    */
  ...
  protected def transformPhases: List[List[Phase]] =
    List(
      new InstrumentCoverage
    ) :: // Perform instrumentation for code coverage (if -coverage-out is set)
      List(
        new CrossVersionChecks, // Check issues related to deprecated and experimental
      ...
      List(
        new TestRecheck.Pre
      ) :: // Test only: run rechecker, enabled under -Yrecheck-test
      List(
        new TestRecheck
      ) :: // Test only: run rechecker, enabled under -Yrecheck-test
      List(
        new CheckCaptures.Pre
      ) :: // Preparations for check captures phase, enabled under captureChecking
      List(
        new CheckCaptures
      ) :: // Check captures, enabled under captureChecking
      List(
        ...
        new DropBreaks
      ) :: // Optimize local Break throws by rewriting them
      List(
  ...
  def reset()(using Context): Unit =
    ctx.base.reset()
    val run = ctx.run
    if run != null then run.reset()

  def newRun(using Context): Run =
    reset()
    val rctx =
      if ctx.settings.Xsemanticdb.value then ctx.addMode(Mode.ReadPositions)
      else ctx
    new Run(this, rctx)
end Compiler
