...
class SJSPlatform()(using Context) extends JavaPlatform:
  ...
  /** Is the given class symbol eligible for Java serialization-specific
    * methods?
    *
    * This is not simply false because we still want to add them to Scala
    * classes and objects. They might be transitively used by macros and other
    * compile-time code. It feels safer to have them be somewhat equivalent to
    * the ones we would get in a JVM project. The JVM back-end will slap an
    * extends `java.io.Serializable` to them, so we should be consistent and
    * also emit the proper serialization methods.
    */
  override def shouldReceiveJavaSerializationMethods(sym: ClassSymbol)(using
      Context
  ): Boolean =
