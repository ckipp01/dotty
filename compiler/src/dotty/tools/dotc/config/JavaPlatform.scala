...
class JavaPlatform extends Platform:
  ...
  def classPath(using Context): ClassPath =
    if currentClassPath.isEmpty then
  ...
  def isMainMethod(sym: Symbol)(using Context): Boolean =
    (sym.name == nme.main) && (sym.info match
      case MethodTpe(_, defn.ArrayOf(el) :: Nil, restpe) =>
        el =:= defn.StringType && (restpe isRef defn.UnitClass)
      case _ => false
    )

  /** Update classpath with a substituted subentry */
  def updateClassPath(subst: Map[ClassPath, ClassPath]): Unit =
    currentClassPath.get match
      case AggregateClassPath(entries) =>
        currentClassPath = Some(
          AggregateClassPath(entries map (e => subst.getOrElse(e, e)))
        )
      case cp: ClassPath =>
  ...
  /** We could get away with excluding BoxedBooleanClass for the purpose of
    * equality testing since it need not compare equal to anything but other
    * booleans, but it should be present in case this is put to other uses.
    */
  ...
  def shouldReceiveJavaSerializationMethods(sym: ClassSymbol)(using
      Context
  ): Boolean =
    true
