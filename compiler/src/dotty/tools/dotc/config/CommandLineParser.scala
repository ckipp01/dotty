...
object CommandLineParser:
  ...
  /** Split the line into tokens separated by whitespace.
    ...
    * echo hello,' 'world! """` => "echo" :: "hello, world!" :: Nil `""" echo
    * \"hello, world!\" """` => "echo" :: "\"hello," :: "world!\"" :: Nil
    *
  ...
  def tokenize(line: String, errorFn: String => Unit): List[String] =
    ...
    var start = 0
    val qpos =
      new ArrayBuffer[Int](16) // positions of paired quotes in current token
    ...
    def skipToDelim(): Boolean =
      ...
      @tailrec def advance(): Boolean = cur match
        ...
        case q @ (DQ | SQ) =>
          { quote(); skipToEndQuote(q) } && { quote(); advance() }
        case EOF                  => true
    ...
    def text(): String =
      val res =
        ...
        else if qpos(0) == start && qpos(1) == pos then
          line.substring(start + 1, pos - 1)
        else copyText()
      qpos.clear()
      res.nn

    inline def badquote() = errorFn(
      s"Unmatched quote [${qpos.last}](${line.charAt(qpos.last)})"
    )
  ...
  /** Expands all arguments starting with @ to the contents of the file named
    * like each argument.
    */
  def expandArg(arg: String): List[String] =
    ...
    if !Files.exists(path) then
      System.err.nn.println(
        s"Argument file ${path.nn.getFileName} could not be found"
      )
      Nil
    else
      def stripComment(s: String) = s.indexOf('#') match
        case -1 => s
        case i  => s.substring(0, i)
      val lines = Files.readAllLines(path).nn
      val params =
        lines.asScala.map(stripComment).filter(!_.nn.isEmpty).mkString(" ")
      tokenize(params)
