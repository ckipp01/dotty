...
object PathResolver:
  ...
  /** Map all classpath elements to absolute paths and reconstruct the
    * classpath.
    */
  ...
  object Environment:
    ...
    /** Environment variables which java pays attention to so it seems we do as
      * well.
      */
  ...
  /** Default values based on those in Environment as interpreted according to
    * the path resolution specification.
    */
  object Defaults:
    ...
    def scalaLibDirFound: Option[Directory] =
      if scalaLibAsJar.isFile then Some(scalaLibDir)
      else if scalaLibAsDir.isDirectory then Some(scalaClassesDir)
      else None

    def scalaLibFound: String =
      if scalaLibAsJar.isFile then scalaLibAsJar.path
      else if scalaLibAsDir.isDirectory then scalaLibAsDir.path
      else ""
  ...
  /** Show values in Environment and Defaults when no argument is provided.
    * Otherwise, show values in Calculated as if those options had been given to
    * a scala runner.
    */
  def main(args: Array[String]): Unit =
    if args.isEmpty then
      ...
      inContext(ContextBase().initialCtx) {
...
class PathResolver(using c: Context):
  ...
  object Calculated:
    ...
    def javaUserClassPath: String =
      if useJavaClassPath then Defaults.javaUserClassPath else ""
    def scalaBootClassPath: String =
    ...
    /** Scaladoc doesn't need any bootstrapping, otherwise will create errors
      * such as: [scaladoc]
      * ../scala-trunk/src/reflect/scala/reflect/macros/Reifiers.scala:89:
      * error: object api is not a member of package reflect [scaladoc] case
      * class ReificationException(val pos: reflect.api.PositionApi, val msg:
      * String) extends Throwable(msg) [scaladoc] ^ Because bootstrapping looks
      * at the sourcepath and creates the package "reflect" in "<root>" it will
      * cause the typedIdentifier to pick <root>.reflect instead of the
      * <root>.scala.reflect package. Thus, no bootstrapping for scaladoc!
      */
    ...
    def basis: List[Iterable[ClassPath]] =
      ...
      List(
        JrtClassPath(
          release
        ), // 1. The Java 9+ classpath (backed by the jrt:/ virtual system, if available)
        ...
        classesInExpandedPath(
          javaUserClassPath
        ), // 4. The Java application class path.
        classesInPath(scalaBootClassPath), // 5. The Scala boot class path.
        contentsOfDirsInPath(
          scalaExtDirs
        ), // 6. The Scala extension class path.
        classesInExpandedPath(
          userClassPath
        ), // 7. The Scala application class path.
  ...
  lazy val result: ClassPath =
    ...
    if settings.YlogClasspath.value then
      Console.println(
        "Classpath built from " + settings.toConciseString(ctx.settingsState)
      )
      ...
      println(
        "After java boot/extdirs classpath has %d entries:" format xs.size
      )
