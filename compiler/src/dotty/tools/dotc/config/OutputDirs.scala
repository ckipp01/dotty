...
/** A class for holding mappings from source directories to their output
  * location. This functionality can be accessed only programmatically. The
  * command line compiler uses a single output location, but tools may use this
  * functionality to set output location per source directory.
  */
class OutputDirs:
  /** Pairs of source directory - destination directory. */
  private var outputDirs: List[(AbstractFile, AbstractFile)] = Nil

  /** If this is not None, the output location where all classes should go.
    */
  private var singleOutDir: Option[AbstractFile] = None

  /** Add a destination directory for sources found under srcdir. Both
    * directories should exits.
    */
  ...
  private def checkDir(
      dir: AbstractFile,
      name: String,
      allowJar: Boolean = false
  ): AbstractFile = (if dir != null && dir.isDirectory then dir
                     // was:      else if (allowJar && dir == null && Path.isJarOrZip(name, false))
                     else if allowJar && dir == null && Jar.isJarOrZip(
                         File(name),
                         false
                       )
                     then new PlainFile(Path(name))
                     else
  ...
  /** Set the single output directory. From now on, all files will be dumped in
    * there, regardless of previous calls to 'add'.
    */
  ...
  /** Return the source file path(s) which correspond to the given classfile
    * path and SourceFile attribute value, subject to the condition that source
    * files are arranged in the filesystem according to Java package layout
    * conventions.
    *
    * The given classfile path must be contained in at least one of the
    * specified output directories. If it does not then this method returns Nil.
    *
    * Note that the source file is not required to exist, so assuming a valid
    * classfile path this method will always return a list containing at least
    * one element.
    *
    * Also that if two or more source path elements target the same output
    * directory there will be two or more candidate source file paths.
    */
  def srcFilesFor(
