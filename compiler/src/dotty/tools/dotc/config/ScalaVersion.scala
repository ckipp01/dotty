...
/** Represents a single Scala version in a manner that supports easy comparison
  * and sorting.
  */
...
/** A specific Scala version, not one of the magic min/max versions. An
  * SpecificScalaVersion may or may not be a released version - i.e. this same
  * class is used to represent final, release candidate, milestone, and
  * development builds. The build argument is used to segregate builds
  */
case class SpecificScalaVersion(
  ...
  def compare(that: ScalaVersion): Int = that match
    case SpecificScalaVersion(thatMajor, thatMinor, thatRev, thatBuild) =>
      ...
      if major < thatMajor then -1
      else if major > thatMajor then 1
      else if minor < thatMinor then -1
      else if minor > thatMinor then 1
      else if rev < thatRev then -1
      else if rev > thatRev then 1
...
@sharable object ScalaVersion:
  ...
  private val R =
    s"((${not(dot)}*)(${dot}(${not(dot)}*)(${dot}(${not(dash)}*)(${dash}(.*))?)?)?)".r
  ...
  def parse(versionString: String): Try[ScalaVersion] =
    ...
    def toBuild(s: String) = s match
      ...
      case s if (s.toUpperCase.startsWith("RC") && isInt(s.substring(2))) =>
        RC(toInt(s.substring(2)))
      case s if (s.toUpperCase.startsWith("M") && isInt(s.substring(1))) =>
        Milestone(toInt(s.substring(1)))
    ...
    try
      versionString match
        ...
        case _ => failure
    catch case e: NumberFormatException => failure
  end parse
...
abstract class ScalaBuild extends Ordered[ScalaBuild]:
  /** Return a version of this build information that can be parsed back into
    * the same ScalaBuild
    */
  def unparse: String
