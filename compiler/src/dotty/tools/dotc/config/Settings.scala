...
object Settings:
  ...
  case class Setting[T: ClassTag] private[Settings] (
    ...
    def updateIn(state: SettingsState, x: Any): SettingsState = x match
      ...
      case _ =>
        throw IllegalArgumentException(
          s"found: $x of type ${x.getClass.getName}, required: ${summon[ClassTag[T]]}"
        )
    ...
    def tryToSet(state: ArgsSummary): ArgsSummary =
        ...
        val value1 =
          if changed && isMultivalue then
            ...
            value0
              ...
              .foreach(s => dangers :+= s"Setting $name set to $s redundantly")
            current ++ value0
          else
      ...
      def setBoolean(argValue: String, args: List[String]) =
        if argValue.equalsIgnoreCase("true") || argValue.isEmpty then
          update(true, args)
        else if argValue.equalsIgnoreCase("false") then update(false, args)
        else
          fail(
            s"$argValue is not a valid choice for boolean setting $name",
            args
          )
        ...
        try
          ...
          choices match
            case Some(r: Range) if x < r.head || r.last < x =>
              fail(
                s"$argValue is out of legal range ${r.head}..${r.last} for $name",
                args
              )
      ...
      def doSet(argRest: String) =
        ((summon[ClassTag[T]], args): @unchecked) match
          ...
          case (ListTag, _) =>
            if argRest.isEmpty then missingArg
            else
              val strings = argRest.split(",").toList
              choices match
                case Some(valid) =>
                  strings.filterNot(valid.contains) match
                    case Nil => update(strings, args)
                    case invalid =>
                      fail(
                        s"invalid choice(s) for $name: ${invalid.mkString(",")}",
                        args
                      )
                case _ => update(strings, args)
          case (StringTag, _)
              if argRest.nonEmpty || choices.exists(_.contains("")) =>
            setString(argRest, args)
          case (StringTag, arg2 :: args2) =>
            if arg2 startsWith "-" then missingArg
      ...
      if prefix != "" && arg.startsWith(prefix) then
        doSet(arg drop prefix.length)
      else if prefix == "" && matches(arg.takeWhile(_ != ':')) then
  ...
  object Setting:
    extension [T](setting: Setting[T])
      ...
      def update(x: T)(using Context): SettingsState =
        setting.updateIn(ctx.settingsState, x)
      def isDefault(using Context): Boolean =
        setting.isDefaultIn(ctx.settingsState)
  ...
  class SettingGroup:
    ...
    def defaultState: SettingsState = new SettingsState(
      allSettings map (_.default)
    )
    ...
    private def checkDependenciesOfSetting(
        state: ArgsSummary,
        setting: Setting[?]
    ) =
      setting.depends.foldLeft(state) { (s, dep) =>
        val (depSetting, reqValue) = dep
        if depSetting.valueIn(state.sstate) == reqValue then s
        else
          s.fail(
            s"incomplete option ${setting.name} (requires ${depSetting.name})"
          )
      }
    ...
    /** Iterates over the arguments applying them to settings where applicable.
      ...
      *
      * This takes a boolean indicating whether to keep processing if an
      * argument is seen which is not a command line option. This is an
      * expedience for the moment so that you can say
      *
      ...
      */
    @tailrec
    final def processArguments(
        state: ArgsSummary,
        processAll: Boolean,
        skipped: List[String]
    ): ArgsSummary =
      def stateWithArgs(args: List[String]) =
        ArgsSummary(state.sstate, args, state.errors, state.warnings)
      state.arguments match
        ...
        case x :: _ if x startsWith "-" =>
          @tailrec def loop(settings: List[Setting[?]]): ArgsSummary =
            settings match
        ...
        case arg :: args =>
          if processAll then
            processArguments(stateWithArgs(args), processAll, skipped :+ arg)
          else state
    ...
    def processArguments(
        arguments: List[String],
        processAll: Boolean,
        settingsState: SettingsState = defaultState
    ): ArgsSummary =
      processArguments(
        ArgsSummary(settingsState, arguments, Nil, Nil),
        processAll,
        Nil
      )
    ...
    def ChoiceSetting(
      ...
      publish(
        Setting(name, descr, default, helpArg, Some(choices), aliases = aliases)
      )
    ...
    def MultiChoiceHelpSetting(
        name: String,
        helpArg: String,
        descr: String,
        choices: List[ChoiceWithHelp[String]],
        default: List[ChoiceWithHelp[String]],
        aliases: List[String] = Nil
    ): Setting[List[ChoiceWithHelp[String]]] =
      publish(
        Setting(name, descr, default, helpArg, Some(choices), aliases = aliases)
      )
    ...
    def OutputSetting(
        ...
        helpArg: String,
        descr: String,
        default: AbstractFile
    ): Setting[AbstractFile] =
    ...
    def PhasesSetting(
      ...
      publish(
        Setting(
          name,
          descr,
          if default.isEmpty then Nil else List(default),
          aliases = aliases
        )
      )
    ...
    def VersionSetting(
        name: String,
        descr: String,
        default: ScalaVersion = NoScalaVersion
    ): Setting[ScalaVersion] =
      publish(Setting(name, descr, default))

    def OptionSetting[T: ClassTag](
        name: String,
        descr: String,
        aliases: List[String] = Nil
    ): Setting[Option[T]] =
      publish(
        Setting(
          ...
          descr,
          None,
          propertyClass = Some(summon[ClassTag[T]].runtimeClass),
          aliases = aliases
        )
      )
  end SettingGroup
