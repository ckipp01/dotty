...
/** For placing a wrapper function around property functions. Motivated by
  * places like google app engine throwing exceptions on property lookups.
  */
trait WrappedProperties extends PropertiesTrait:
  ...
  override def setProp(name: String, value: String): String = wrap(
    super.setProp(name, value)
  ).orNull
  override def clearProp(name: String): String = wrap(
    super.clearProp(name)
  ).orNull
  override def envOrElse(name: String, alt: String): String =
    wrap(super.envOrElse(name, alt)) getOrElse alt
  override def envOrNone(name: String): Option[String] = wrap(
    super.envOrNone(name)
  ).flatten
