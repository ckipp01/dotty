...
object Feature:
  ...
  /** Is `feature` enabled by by an import? This is the case if the feature is
    * imported by a named import
    *
  ...
  /** Is `feature` enabled by either a command line setting or an import?
    ...
    * @param owner
    *   The prefix symbol (nested in `scala.language`) where the feature is
    *   defined.
    */
  ...
  def genericNumberLiteralsEnabled(using Context) = enabled(
  ...
  /** Is pureFunctions enabled for any of the currently compiled compilation
    * units?
    */
  ...
  /** Is captureChecking enabled for any of the currently compiled compilation
    * units?
    */
  ...
  /** If current source migrates to `version`, issue given warning message and
    * return `true`, otherwise return `false`.
    */
  def warnOnMigration(msg: Message, pos: SrcPos, version: SourceVersion)(using
      Context
  ): Boolean =
  ...
  def checkExperimentalFeature(
    ...
    if !isExperimentalEnabled then
      report.error(
        em"Experimental $which may only be used with a nightly or snapshot version of the compiler$note",
        srcPos
      )
  ...
  def checkExperimentalDef(sym: Symbol, srcPos: SrcPos)(using Context) =
    if !isExperimentalEnabled then
      ...
      report.error(
        em"$symMsg and therefore may only be used in an experimental scope.",
        srcPos
      )

  /** Check that experimental compiler options are only set for snapshot or
    * nightly compiler versions.
    */
  def checkExperimentalSettings(using Context): Unit =
    for
      setting <- ctx.settings.language.value
  ...
  /** Handle language import `import language.<prefix>.<imported>` if it is one
    ...
    */
  def handleGlobalLanguageImport(prefix: TermName, imported: Name)(using
      Context
  ): Boolean =
