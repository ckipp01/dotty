...
object Properties extends PropertiesTrait:
  ...
  protected def pickJarBasedOn: Class[PropertiesTrait] =
    classOf[PropertiesTrait]

  /** Scala manifest attributes.
    */
  @sharable val ScalaCompilerVersion: AttributeName = new AttributeName(
...
trait PropertiesTrait:
  ...
  protected def pickJarBasedOn
      : Class[?] // props file comes from jar containing this
  ...
  @sharable protected lazy val scalaProps: java.util.Properties =
    ...
    if stream ne null then quietlyDispose(props load stream, stream.close)
  ...
  def propOrFalse(name: String): Boolean = propOrNone(name) exists (x =>
  ...
  def scalaPropOrElse(name: String, alt: String): String =
    scalaProps.getProperty(name, alt)
  def scalaPropOrEmpty(name: String): String = scalaPropOrElse(name, "")
  def scalaPropOrNone(name: String): Option[String] = Option(
    scalaProps.getProperty(name)
  )

  /** Either the development or release version if known, otherwise the empty
    * string.
    */
  def versionNumberString: String = scalaPropOrEmpty("version.number")

  /** The version number of the jar this was loaded from, or `"(unknown)"` if it
  ...
  /** The version number of the jar this was loaded from plus `"version "`
    * prefix, or `"version (unknown)"` if it cannot be determined.
    */
  val versionString: String = "version " + simpleVersionString

  /** Whether the current version of compiler is experimental
    *
    *   1. Snapshot, nightly releases and non-bootstrapped compiler are
    *      experimental. 2. Features supported by experimental versions of the
    *      compiler:
    *      - research plugins
    */
  val experimental: Boolean =
    versionString.contains("SNAPSHOT") || versionString.contains(
      "NIGHTLY"
    ) || versionString.contains("nonbootstrapped")

  val copyrightString: String =
    scalaPropOrElse("copyright.string", "(c) 2002-2017 LAMP/EPFL")

  /** This is the encoding to use reading in source files, overridden with
    * -encoding Note that it uses "prop" i.e. looks in the scala jar, not the
    * system properties.
    */
  def sourceEncoding: String =
    scalaPropOrElse("file.encoding", StandardCharsets.UTF_8.name)
  def sourceReader: String =
    scalaPropOrElse("source.reader", "scala.tools.nsc.io.SourceReader")

  /** This is the default text encoding, overridden (unreliably) with
    * `JAVA_OPTS="-Dfile.encoding=Foo"`
    */
  def encodingString: String =
    propOrElse("file.encoding", StandardCharsets.UTF_8.name)
  ...
  def versionMsg: String =
    "Scala %s %s -- %s".format(propCategory, versionString, copyrightString)
