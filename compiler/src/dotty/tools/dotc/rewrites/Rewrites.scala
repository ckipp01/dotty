...
object Rewrites:
    ...
    def apply(cs: Array[Char]): Array[Char] =
      ...
      if patches.nonEmpty then
        patches.reduceLeft { (p1, p2) =>
          assert(
            p1.span.end <= p2.span.start,
            s"overlapping patches in $source: $p1 and $p2"
          )
          p2
        }
  ...
  /** If -rewrite is set, record a patch that replaces the range given by `span`
    * in `source` by `replacement`
    */
  def patch(source: SourceFile, span: Span, replacement: String)(using
      Context
  ): Unit =
    if ctx.reporter != Reporter.NoReporter // NoReporter is used for syntax highlighting
    then
      ctx.settings.rewrite.value.foreach(
        _.patched
...
/** A completely encapsulated class representing rewrite state, used as an
  * optional setting.
  */
class Rewrites:
  import Rewrites.*
