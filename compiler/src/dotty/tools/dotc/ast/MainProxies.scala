...
import core.*
import Symbols.*, Types.*, Contexts.*, Decorators.*, util.Spans.*, Flags.*,
  Constants.*
...
object MainProxies:
  ...
  /** Generate proxy classes for @main functions and @myMain functions where
    * myMain <:< MainAnnotation
    */
  def proxies(stats: List[tpd.Tree])(using Context): List[untpd.Tree] =
    mainAnnotationProxies(stats) ++ mainProxies(stats)

  /** Generate proxy classes for @main functions. A function like
    *
    * @main
    *   def f(x: S, ys: T*) = ...
    *
    ...
    *
    * import CommandLineParser._ class f {
    * @static
    *   def main(args: Array[String]): Unit = try f( parseArgument[S](args, 0),
    *   parseRemainingArguments[T](args, 1): _* ) catch case err: ParseError =>
    *   showError(err) }
    */
  private def mainProxies(stats: List[tpd.Tree])(using
      Context
  ): List[untpd.Tree] =
    import tpd.*
  ...
  private def mainProxy(mainFun: Symbol)(using Context): List[TypeDef] =
        ...
        val args = mt.paramInfos.zipWithIndex map { (formal, n) =>
          val (parserSym, formalElem) =
            if formal.isRepeatedParam then
              (defn.CLP_parseRemainingArguments, formal.argTypes.head)
        ...
        mt.resType match
          case restpe: MethodType =>
            if mt.paramInfos.lastOption.getOrElse(NoType).isRepeatedParam then
              report.error(
                em"varargs parameter of @main method must come last",
                pos
              )
    ...
    if !mainFun.owner.isStaticOwner then
      ...
      val mainArg = ValDef(
        nme.args,
        TypeTree(defn.ArrayType.appliedTo(defn.StringType)),
        EmptyTree
      )
        .withFlags(Param)

      /** Replace typed `Ident`s that have been typed with a TypeSplice with the
        * reference to the symbol. The annotations will be retype-checked in
        * another scope that may not have the same imports.
        */
      ...
      val mainMeth =
        DefDef(nme.main, (mainArg :: Nil) :: Nil, TypeTree(defn.UnitType), body)
          .withFlags(JavaStatic | Synthetic)
          .withAnnotations(annots)
      val mainTempl =
        Template(emptyConstructor, Nil, Nil, EmptyValDef, mainMeth :: Nil)
  ...
  /** Generate proxy classes for main functions. A function like
    *
    * /** * Lorem ipsum dolor sit amet * consectetur adipiscing elit. * * @param
    * x my param x * @param ys all my params y */
    * @myMain(80)
    *   def f(
    * @myMain.Alias("myX")
    *   x: S, y: S, ys: T* ) = ...
    ...
    * would be translated to something like
    *
    * final class f { static def main(args: Array[String]): Unit = { val
    * annotation = new myMain(80) val info = new Info( name = "f", documentation
    * \= "Lorem ipsum dolor sit amet consectetur adipiscing elit.", parameters =
    * Seq( new scala.annotation.MainAnnotation.Parameter("x", "S", false, false,
    * "my param x", Seq(new scala.main.Alias("myX"))), new
    * scala.annotation.MainAnnotation.Parameter("y", "S", true, false, "",
    * Seq()), new scala.annotation.MainAnnotation.Parameter("ys", "T", false,
    * true, "all my params y", Seq()) ) ), val command =
    * annotation.command(info, args) if command.isDefined then val cmd =
    * command.get val args0: () => S =
    * annotation.argGetter[S](info.parameters(0), cmd(0), None) val args1: () =>
    * S = annotation.argGetter[S](info.parameters(1), mainArgs(1), Some(() =>
    * sum$default$1())) val args2: () => Seq[T] =
    * annotation.varargGetter[T](info.parameters(2), cmd.drop(2))
    * annotation.run(() => f(args0(), args1(), args2()*)) } }
    */
  private def mainAnnotationProxies(stats: List[tpd.Tree])(using
      Context
  ): List[untpd.Tree] =
    import tpd.*

    /** Computes the symbols of the default values of the function. Since they
      * cannot be inferred anymore at this point of the compilation, they must
      * be explicitly passed by [[mainProxy]].
      */
    def defaultValueSymbols(
        scope: Tree,
        funSymbol: Symbol
    ): DefaultValueSymbols =
      scope match
          ...
          template.body
            .flatMap((_: Tree) match
              case dd: DefDef
                  if dd.name.is(
                    DefaultGetterName
                  ) && dd.name.firstPart == funSymbol.name =>
                val DefaultGetterName.NumberedInfo(index) =
                  dd.name.info: @unchecked
    ...
    def mainMethods(scope: Tree, stats: List[Tree]): List[
      (Symbol, ParameterAnnotationss, DefaultValueSymbols, Option[Comment])
    ] = stats.flatMap {
      case stat: DefDef =>
        val sym = stat.symbol
        sym.annotations.filter(_.matches(defn.MainAnnotationClass)) match
          case Nil =>
            Nil
          case _ :: Nil =>
            val paramAnnotations = stat.paramss.flatMap(
              _.map(valdef =>
                valdef.symbol.annotations.filter(
                  _.matches(defn.MainAnnotationParameterAnnotation)
                )
              )
            )
            (
              sym,
              paramAnnotations.toVector,
              defaultValueSymbols(scope, sym),
              stat.rawComment
            ) :: Nil
          case mainAnnot :: others =>
            report.error(
              em"method cannot have multiple main annotations",
              mainAnnot.tree
            )
            Nil
  ...
  end mainAnnotationProxies

  private def mainAnnotationProxy(
      mainFun: Symbol,
      paramAnnotations: ParameterAnnotationss,
      defaultValueSymbols: DefaultValueSymbols,
      docComment: Option[Comment]
    ...
    def parameterInfos(mt: MethodType): List[Tree] =
      extension (tree: Tree)
        def withProperty(sym: Symbol, args: List[Tree]) =
          Apply(Select(tree, sym.name), args)

      for ((formal, paramName), idx) <- mt.paramInfos
          .zip(mt.paramNames)
          .zipWithIndex
      yield
        val param = paramName.toString
        val paramType0 =
          if formal.isRepeatedParam then formal.argTypes.head.dealias
          else formal.dealias
        ...
        val paramAnnots =
          val annotationTrees =
            paramAnnotations(idx).map(instantiateAnnotation).toList
          Apply(ref(defn.SeqModule.termRef), annotationTrees)

        val constructorArgs =
          List(param, paramTypeStr, hasDefault, isRepeated, paramDoc)
            .map(value => Literal(Constant(value)))

        New(
          TypeTree(defn.MainAnnotationParameter.typeRef),
          List(constructorArgs :+ paramAnnots)
        )
      end for

    end parameterInfos

    /** Creates a list of references and definitions of arguments. The goal is
      * to create the `val args0: () => S = annotation.argGetter[S](0, cmd(0),
      * None)` part of the code.
      */
    def argValDefs(mt: MethodType): List[ValDef] =
      ...
      yield
        ...
        val argGetter0 = TypeApply(
          Select(Ident(nme.annotation), getterName),
          TypeTree(formalType) :: Nil
        )
        ...
        val argGetter =
          if isRepeated then
            Apply(
              ...
              List(
                paramInfo,
                Apply(Select(Ident(nme.cmd), nme.drop), List(index))
              )
            )
          else
            Apply(
              ...
              List(
                paramInfo,
                Apply(Ident(nme.cmd), List(index)),
                defaultValueGetterOpt
              )
            )
        ValDef(argName, TypeTree(), argGetter)
    end argValDefs

    /** Create a list of argument references that will be passed as argument to
      * the main method. `args0`, ...`argn*`
      */
    ...
    /** Turns an annotation (e.g. `@main(40)`) into an instance of the class
      * (e.g. `new scala.main(40)`).
      */
    def instantiateAnnotation(annot: Annotation): Tree =
      val argss =
        def recurse(t: tpd.Tree, acc: List[List[Tree]]): List[List[Tree]] =
          t match
            case Apply(t, args: List[tpd.Tree]) =>
              recurse(t, extractArgs(args) :: acc)
            case _ => acc

        def extractArgs(args: List[tpd.Tree]): List[Tree] =
          args.flatMap {
            case Typed(SeqLiteral(varargs, _), _) =>
              varargs.map(arg => TypedSplice(arg))
            case arg: Select if arg.name.is(DefaultGetterName) =>
              Nil // Ignore default values, they will be added later by the compiler
    ...
    def generateMainClass(
        mainCall: Tree,
        args: List[Tree],
        parameterInfos: List[Tree]
    ): TypeDef =
      val cmdInfo =
        ...
        New(
          TypeTree(defn.MainAnnotationInfo.typeRef),
      ...
      val body = Block(
        List(annotVal, infoVal, command),
        body0
      ) // TODO add `if (cmd.nonEmpty)`
          ...
          if mt.isImplicitMethod then
            ...
            mt.resType match
              ...
              case _ =>
                Some(
                  generateMainClass(
                    unitToValue(Apply(ref(mainFun.termRef), argRefs(mt))),
                    argValDefs(mt),
                    parameterInfos(mt)
                  )
                )
  ...
  private class Documentation(docComment: Option[Comment]):
    ...
    /** The parameters identified by @param. Maps from parameter name to its
      * documentation.
      */
    ...
    docComment match
      case Some(comment) =>
        if comment.isDocComment then parseDocComment(comment.raw)
        else _mainDoc = comment.raw
      case None =>

    private def cleanComment(raw: String): String =
      var lines: Seq[String] = raw.trim.nn.split('\n').nn.toSeq
      lines =
        lines.map(l => l.substring(skipLineLead(l, -1), l.length).nn.trim.nn)
    ...
    private def parseDocComment(raw: String): Unit =
      ...
      var mainComment: String =
        raw.substring(skipLineLead(raw, 0), startTag(raw, tidx)).nn
      _mainDoc = cleanComment(mainComment)

      // Parse arguments comments
      val argsCommentsSpans: Map[String, (Int, Int)] =
        paramDocs(raw, "@param", tidx)
      val argsCommentsTextSpans =
        argsCommentsSpans.view.mapValues(extractSectionText(raw, _))
      val argsCommentsTexts = argsCommentsTextSpans.mapValues({
        case (beg, end) => raw.substring(beg, end).nn
      })
