...
import core.*
import Types.*, Names.*, NameOps.*, Flags.*, util.Spans.*, Contexts.*,
  Constants.*
...
object Trees:
  ...
  /** Trees take a parameter indicating what the type of their `tpe` field is.
    * Two choices: `Type` or `Untyped`. Untyped trees have type `Tree[Untyped]`.
    *
    * Tree typing uses a copy-on-write implementation:
    *
    *   - You can never observe a `tpe` which is `null` (throws an exception)
    *   - So when creating a typed tree with `withType` we can re-use the
    *     existing tree transparently, assigning its `tpe` field.
  ...
  abstract class Tree[+T <: Untyped](implicit @constructorOnly src: SourceFile)
    ...
    if Stats.enabled then ntrees += 1
    ...
    /** Destructively set the type of the tree. This should be called only when
      * it is known that it is safe under sharing to do so. One use-case is in
      * the withType method below which implements copy-on-write. Another
      * use-case is in method interpolateAndAdapt in Typer, where we overwrite
      * with a simplified version of the type itself.
      */
    private[dotc] def overwriteType(tpe: T @uncheckedVariance): Unit =
      myTpe = tpe

    /** The type of the tree. In case of an untyped tree, an
      * UnAssignedTypeException is thrown. (Overridden by empty trees)
      */
    final def tpe: T =
    ...
    /** Copy `tpe` attribute from tree `from` into this tree, independently
      * whether it is null or not. final def copyAttr[U <: Untyped](from:
      * Tree[U]): ThisTree[T] = { val t1 = this.withSpan(from.span) val t2 = if
      * (from.myTpe != null) t1.withType(from.myTpe.asInstanceOf[Type]) else t1
      * t2.asInstanceOf[ThisTree[T]] }
      */

    /** Return a typed tree that's isomorphic to this tree, but has given type.
      * (Overridden by empty trees)
      */
    def withType(tpe: Type)(using Context): ThisTree[Type] =
      if tpe.isInstanceOf[ErrorType] then
        assert(
          !Config.checkUnreportedErrors ||
      ...
      else if Config.checkTreesConsistent then
    ...
    /** Does the tree have its type field set? Note: this operation is not
      ...
      * modifications. Should be used only in special circumstances (we need it
      * for printing trees with optional type info).
      */
    ...
    /** The denotation referred to by this tree. Defined for `DenotingTree`s and
      * `ProxyTree`s, NoDenotation for other kinds of trees
      */
    ...
    /** Is this tree either the empty tree or the empty ValDef or an empty type
      * ident?
      */
    def isEmpty: Boolean = false

    /** Convert tree to a list. Gives a singleton list, except for thickets
      * which return their element trees.
      */
    ...
    inline def orElse[U >: T <: Untyped](inline that: Tree[U]): Tree[U] =
      if this eq genericEmptyTree then that else this
    ...
    /** If this is a thicket, perform `op` on each of its trees otherwise,
      * perform `op` ion tree itself.
      */
  ...
  class UnAssignedTypeException[T <: Untyped](tree: Tree[T])
  ...
  /** Instances of this class are trees for which isType is definitely true.
    * Note that some trees have isType = true without being TypTrees (e.g.
    * Ident, Annotated)
    */
  ...
  /** Instances of this class are trees for which isTerm is definitely true.
    * Note that some trees have isTerm = true without being TermTrees (e.g.
    * Ident, Annotated)
    */
  ...
  /** Instances of this class are trees which are not terms but are legal parts
    * of patterns.
    */
  ...
  abstract class DenotingTree[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends Tree[T]:
  ...
  /** Tree's denot/isType/isTerm properties come from a subtree identified by
    * `forwardTo`.
    */
  abstract class ProxyTree[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends Tree[T]:
  ...
  /** Tree has a name */
  abstract class NameTree[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ...
  abstract class RefTree[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends NameTree[T]:
  ...
  trait DefTree[+T <: Untyped] extends DenotingTree[T]:
    ...
    private[dotc] def rawMods: untpd.Modifiers =
      if myMods == null then untpd.EmptyModifiers else myMods.uncheckedNN

    def withAnnotations(annots: List[untpd.Tree]): ThisTree[Untyped] = withMods(
      rawMods.withAnnotations(annots)
    )

    def withMods(mods: untpd.Modifiers): ThisTree[Untyped] =
      val tree =
        if myMods == null || (myMods == mods) then this else cloneIn(source)
      tree.setMods(mods)
      tree.asInstanceOf[ThisTree[Untyped]]

    def withFlags(flags: FlagSet): ThisTree[Untyped] = withMods(
      untpd.Modifiers(flags)
    )
    def withAddedFlags(flags: FlagSet): ThisTree[Untyped] = withMods(
      rawMods | flags
    )
  ...
  sealed trait WithEndMarker[+T <: Untyped]:
    ...
    /** The name in source code that represents this construct, and is the name
      * that the user must write to create a valid end marker. e.g. a
      * constructor definition is terminated in the source code by `end this`,
      * so it's `srcName` should return `this`.
      */
    ...
    final def withEndMarker(copyFrom: WithEndMarker[?]): self.type =
      if copyFrom.hasEndMarker then this.withEndMarker()
      else this
  ...
  object WithEndMarker:
    /** Property key that signals the tree was terminated with an `end` marker
      * in the source code
      */
  ...
  abstract class NamedDefTree[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends NameTree[T]
      with DefTree[T]
      with WithEndMarker[T]:
    ...
    /** The position of the name defined by this definition. This is a point
      * position if the definition is synthetic, or a range position if the
      * definition comes from source. It might also be that the definition does
      * not have a position (for instance when synthesized by a calling chain
      * from `viewExists`), in that case the return position is NoSpan.
      * Overridden in Bind
      */
    ...
    /** The source position of the name defined by this definition. This is a
      * point position if the definition is synthetic, or a range position if
      * the definition comes from source.
      */
    def namePos(using Context): SourcePosition = source.atSpan(nameSpan)
  end NamedDefTree

  /** Tree defines a new symbol and carries modifiers. The position of a
    * MemberDef contains only the defined identifier or pattern. The envelope of
    * a MemberDef contains the whole definition and has its point on the opening
    * keyword (or the next token after that if keyword is missing).
    */
  abstract class MemberDef[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends NamedDefTree[T]:
  ...
  abstract class ValOrDefDef[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends MemberDef[T],
        WithLazyFields:
  ...
  /** name */
  case class Ident[+T <: Untyped] private[ast] (name: Name)(implicit
      @constructorOnly src: SourceFile
  ) extends RefTree[T]:
  ...
  class SearchFailureIdent[+T <: Untyped] private[ast] (
      name: Name,
      expl: => String
  ...
  class SelectWithSig[+T <: Untyped] private[ast] (
      qualifier: Tree[T],
      name: Name,
      val sig: Signature
  )(implicit @constructorOnly src: SourceFile)
  ...
  /** qual.this */
  case class This[+T <: Untyped] private[ast] (qual: untpd.Ident)(implicit
      @constructorOnly src: SourceFile
  ) extends DenotingTree[T]
      with TermTree[T]:
  ...
  case class Super[+T <: Untyped] private[ast] (
      qual: Tree[T],
      mix: untpd.Ident
  )(implicit @constructorOnly src: SourceFile)
      extends ProxyTree[T]
  ...
  abstract class GenericApply[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends ProxyTree[T]
      with TermTree[T]:
  ...
  case class Apply[+T <: Untyped] private[ast] (
    ...
    /** The kind of this application. Works reliably only for untyped trees;
      * typed trees are under no obligation to update it correctly.
      */
    def applyKind: ApplyKind =
      attachmentOrElse(untpd.KindOfApply, ApplyKind.Regular)

  /** fun[args] */
  case class TypeApply[+T <: Untyped] private[ast] (
      fun: Tree[T],
      args: List[Tree[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends GenericApply[T]:
  ...
  /** const */
  case class Literal[+T <: Untyped] private[ast] (const: Constant)(implicit
      @constructorOnly src: SourceFile
  ) extends Tree[T]
      with TermTree[T]:
  ...
  case class New[+T <: Untyped] private[ast] (tpt: Tree[T])(implicit
      @constructorOnly src: SourceFile
  ) extends Tree[T]
      with TermTree[T]:
  ...
  /** { stats; expr } */
  case class Block[+T <: Untyped] private[ast] (
      stats: List[Tree[T]],
      expr: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
      extends Tree[T]:
    ...
    override def isTerm: Boolean =
      !isType // this will classify empty trees as terms, which is necessary
  ...
  case class If[+T <: Untyped] private[ast] (
    ...
    def isInline = false
  class InlineIf[+T <: Untyped] private[ast] (
      cond: Tree[T],
      thenp: Tree[T],
      elsep: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
  ...
  /** A closure with an environment and a reference to a method.
    ...
    * @param meth
    *   A ref tree that refers to the method of the closure. The first
    *   (env.length) parameters of that method are filled with env values.
    * @param tpt
    *   Either EmptyTree or a TypeTree. If tpt is EmptyTree the type of the
    *   closure is a function type, otherwise it is the type given in `tpt`,
    *   which must be a SAM type.
    */
  case class Closure[+T <: Untyped] private[ast] (
      env: List[Tree[T]],
      meth: Tree[T],
      tpt: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
  ...
  case class Match[+T <: Untyped] private[ast] (
      selector: Tree[T],
      cases: List[CaseDef[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends TermTree[T]:
    type ThisTree[+T <: Untyped] = Match[T]
    def isInline = false
  class InlineMatch[+T <: Untyped] private[ast] (
      selector: Tree[T],
      cases: List[CaseDef[T]]
  )(implicit @constructorOnly src: SourceFile)
  ...
  case class CaseDef[+T <: Untyped] private[ast] (
      pat: Tree[T],
      guard: Tree[T],
      body: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
  ...
  /** return expr where `from` refers to the method or label from which the
    * return takes place After program transformations this is not necessarily
    * the enclosing method, because closures can intervene.
    */
  case class Return[+T <: Untyped] private[ast] (
      expr: Tree[T],
      from: Tree[T] = genericEmptyTree
  )(implicit @constructorOnly src: SourceFile)
      extends TermTree[T]:
  ...
  case class Try[+T <: Untyped] private[ast] (
      expr: Tree[T],
      cases: List[CaseDef[T]],
      finalizer: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
  ...
  /** Seq(elems)
    ...
    */
  case class SeqLiteral[+T <: Untyped] private[ast] (
      elems: List[Tree[T]],
      elemtpt: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
      extends Tree[T]:
  ...
  /** Array(elems) */
  class JavaSeqLiteral[+T <: Untyped] private[ast] (
      elems: List[Tree[T]],
      elemtpt: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
  ...
  /** A tree representing inlined code.
    *
    * @param call
    *   Info about the original call that was inlined Until PostTyper, this is
    *   the full call, afterwards only a reference to the toplevel class from
    ...
    * different context: `bindings` represent the arguments to the inlined call,
    * whereas `expansion` represents the body of the inlined function.
    */
  case class Inlined[+T <: Untyped] private[ast] (
      call: tpd.Tree,
      bindings: List[MemberDef[T]],
      expansion: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
      extends Tree[T]:
  ...
  /** A tree representing a quote `'{ body }` or `'[ body ]`. `Quote`s are
    * created by the `Parser`. In typer they can be typed as a `Quote` with a
    * known `tpt` or desugared and typed as a quote pattern.
    *
    * `Quotes` are checked and transformed in the `staging`, `splicing` and
    * `pickleQuotes` phases. After `pickleQuotes` phase, the only quotes that
    * exist are in `inline` methods. These are dropped when we remove the inline
    * method implementations.
    *
    * Type quotes `'[body]` from the parser are desugared into quote patterns
    * (using a `Type.of[T]]`) when type checking. TASTy files will not contain
    * type quotes. Type quotes are used again in the `staging` phase to
    * represent the reification of `Type.of[T]]`.
    *
    * Type tags `tags` are always empty before the `staging` phase. Tags for
    * stage inconsistent types are added in the `staging` phase to level 0
    * quotes. Tags for types that refer to definitions in an outer quote are
    * added in the `splicing` phase
    *
    * @param body
    *   The tree that was quoted
    * @param tags
    *   Term references to instances of `Type[T]` for `T`s that are used in the
    *   quote
    */
  case class Quote[+T <: Untyped] private[ast] (
      body: Tree[T],
      tags: List[Tree[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends TermTree[T]:
    ...
    def withBodyType(tpe: Type)(using Context): Quote[Type] =
      ...
      val quoteType = // `Quotes ?=> Expr[T]` or `Quotes ?=> Type[T]`
        defn
          .FunctionType(1, isContextual = true)
  ...
  /** A tree representing a splice `${ expr }`
    ...
    * `Splice` with a known `tpt` or desugared and typed as a quote pattern
    * holes.
    *
    * `Splice` are checked and transformed in the `staging` and `splicing`
    * phases. After `splicing` phase, the only splices that exist are in
    * `inline` methods. These are dropped when we remove the inline method
    * implementations.
    *
    * @param expr
    *   The tree that was spliced
    */
  case class Splice[+T <: Untyped] private[ast] (expr: Tree[T])(implicit
      @constructorOnly src: SourceFile
  ) extends TermTree[T]:
    type ThisTree[+T <: Untyped] = Splice[T]

  /** A tree representing a pattern splice `${ pattern }`, `$ident` or
    * `$ident(args*)` in a quote pattern.
    *
    * Parser will only create `${ pattern }` and `$ident`, hence they will not
    * have args. While typing, the `$ident(args*)` the args are identified and
    * desugared into a `SplicePattern` containing them.
    *
    * SplicePattern are removed after typing the pattern and are not present in
    * TASTy.
    *
    * @param body
    *   The tree that was spliced
    ...
    */
  case class SplicePattern[+T <: Untyped] private[ast] (
      body: Tree[T],
      args: List[Tree[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends TermTree[T]:
  ...
  case class TypeTree[+T <: Untyped]()(implicit
      @constructorOnly src: SourceFile
  ) extends DenotingTree[T]
      with TypTree[T]:
    ...
    override def toString: String =
      s"TypeTree${if hasType then s"[$typeOpt]" else ""}"

  /** Tree that replaces a level 1 splices in pickled (level 0) quotes. It is
    * only used when picking quotes (will never be in a TASTy file).
    *
    * @param isTerm
    ...
    * @param content
    *   Lambda that computes the content of the hole. This tree is empty when in
    *   a quote pickle.
    */
  case class Hole[+T <: Untyped](
      override val isTerm: Boolean,
      idx: Int,
      args: List[Tree[T]],
      content: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
  ...
  /** A type tree whose type is inferred. These trees appear in two contexts
    *   - as an argument of a TypeApply. In that case its type is always a
    *     TypeVar
    *   - as a (result-)type of an inferred ValDef or DefDef. Every TypeVar is
    *     created as the type of one InferredTypeTree.
    */
  class InferredTypeTree[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends TypeTree[T]
  ...
  /** tpt { refinements } */
  case class RefinedTypeTree[+T <: Untyped] private[ast] (
      tpt: Tree[T],
      refinements: List[Tree[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends ProxyTree[T]
      with TypTree[T]:
  ...
  /** tpt[args] */
  case class AppliedTypeTree[+T <: Untyped] private[ast] (
      tpt: Tree[T],
      args: List[Tree[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends ProxyTree[T]
  ...
  /** [typeparams] -> tpt
    ...
    * lower bound is either a reference to `Nothing` or an `HKTypeLambda`, this
    * happens because these trees are typed by `HKTypeLambda#fromParams` which
    * makes sure to move bounds outside of the type lambda itself to simplify
    * their handling in the compiler.
    *
    ...
    */
  case class LambdaTypeTree[+T <: Untyped] private[ast] (
      tparams: List[TypeDef[T]],
      body: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
      extends TypTree[T]:
    type ThisTree[+T <: Untyped] = LambdaTypeTree[T]

  case class TermLambdaTypeTree[+T <: Untyped] private[ast] (
      params: List[ValDef[T]],
      body: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
      extends TypTree[T]:
  ...
  case class MatchTypeTree[+T <: Untyped] private[ast] (
      bound: Tree[T],
      selector: Tree[T],
      cases: List[CaseDef[T]]
  )(implicit @constructorOnly src: SourceFile)
  ...
  /** >: lo <: hi >: lo <: hi = alias for RHS of bounded opaque type
    */
  case class TypeBoundsTree[+T <: Untyped] private[ast] (
      lo: Tree[T],
      hi: Tree[T],
      alias: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
  ...
  case class Bind[+T <: Untyped] private[ast] (name: Name, body: Tree[T])(
    ...
    override def nameSpan(using Context): Span =
      if span.exists then Span(span.start, span.start + name.toString.length)
      else span
  ...
  /** The typed translation of `extractor(patterns)` in a pattern. The
    * translation has the following components:
    *
    * @param fun
    *   is `extractor.unapply` (or, for backwards compatibility,
    *   `extractor.unapplySeq`) possibly with type parameters
    * @param implicits
    *   Any implicit parameters passed to the unapply after the selector
    * @param patterns
    *   The argument patterns in the pattern match.
    *
    * It is typed with same type as first `fun` argument Given a match selector
    * `sel` a pattern UnApply(fun, implicits, patterns) is roughly translated as
    * follows
    *
    * val result = fun(sel)(implicits) if (result.isDefined) "match patterns
    * against result"
    */
  case class UnApply[+T <: Untyped] private[ast] (
      fun: Tree[T],
      implicits: List[Tree[T]],
      patterns: List[Tree[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends ProxyTree[T]
      with PatternTree[T]:
  ...
  case class ValDef[+T <: Untyped] private[ast] (
      name: TermName,
      tpt: Tree[T],
      private var preRhs: LazyTree[T]
  )(implicit @constructorOnly src: SourceFile)
    ...
    def rhs(using Context): Tree[T] =
      forceFields(); preRhs.asInstanceOf[Tree[T]]

  /** mods def name[tparams](vparams_1)...(vparams_n): tpt = rhs */
  case class DefDef[+T <: Untyped] private[ast] (
      name: TermName,
      paramss: List[ParamClause[T]],
      tpt: Tree[T],
      private var preRhs: LazyTree[T]
  )(implicit @constructorOnly src: SourceFile)
      extends ValOrDefDef[T]:
    ...
    def leadingTypeParams(using Context): List[TypeDef[T]] = paramss match
      case (tparams @ (tparam: TypeDef[?]) :: _) :: _ =>
        tparams.asInstanceOf[List[TypeDef[T]]]
      case _ => Nil
  ...
  /** mods class name template or mods trait name template or mods type name =
    * rhs or mods type name >: lo <: hi, if rhs = TypeBoundsTree(lo, hi) or mods
    * type name >: lo <: hi = rhs if rhs = TypeBoundsTree(lo, hi, alias) and
    * opaque in mods
    */
  ...
  /** extends parents { self => body }
    * @param preParentsOrDerived
    *   A list of parents followed by a list of derived classes, if this is of
    *   class untpd.DerivingTemplate. Typed templates only have parents.
    */
  case class Template[+T <: Untyped] private[ast] (
      constr: DefDef[T],
      private var preParentsOrDerived: LazyTreeList[T],
      self: ValDef[T],
      private var preBody: LazyTreeList[T]
  )(implicit @constructorOnly src: SourceFile)
      extends DefTree[T]
      with WithLazyFields:
    ...
    def body(using Context): List[Tree[T]] =
      forceFields(); preBody.asInstanceOf[List[Tree[T]]]
    def parentsOrDerived(using Context): List[Tree[T]] =
      forceFields(); preParentsOrDerived.asInstanceOf[List[Tree[T]]]

    def parents(using Context): List[Tree[T]] =
      parentsOrDerived // overridden by DerivingTemplate
    def derived: List[untpd.Tree] = Nil // overridden by DerivingTemplate

  abstract class ImportOrExport[+T <: Untyped](implicit
      @constructorOnly src: SourceFile
  ) extends DenotingTree[T]:
  ...
  /** import expr.selectors where a selector is either an untyped `Ident`,
    * `name` or an untyped thicket consisting of `name` and `rename`.
    */
  case class Import[+T <: Untyped] private[ast] (
      expr: Tree[T],
  ...
  /** export expr.selectors where a selector is either an untyped `Ident`,
    * `name` or an untyped thicket consisting of `name` and `rename`.
    */
  case class Export[+T <: Untyped] private[ast] (
      expr: Tree[T],
      selectors: List[untpd.ImportSelector]
  )(implicit @constructorOnly src: SourceFile)
      extends ImportOrExport[T]:
  ...
  /** package pid { stats } */
  case class PackageDef[+T <: Untyped] private[ast] (
      pid: RefTree[T],
      stats: List[Tree[T]]
  )(implicit @constructorOnly src: SourceFile)
      extends ProxyTree[T]
  ...
  /** arg @annot */
  case class Annotated[+T <: Untyped] private[ast] (
      arg: Tree[T],
      annot: Tree[T]
  )(implicit @constructorOnly src: SourceFile)
      extends ProxyTree[T]:
  ...
  /** Temporary class that results from translation of ModuleDefs (and possibly
    * other statements). The contained trees will be integrated when transformed
    * with a `transform(List[Tree])` call.
    */
  case class Thicket[+T <: Untyped](trees: List[Tree[T]])(implicit
      @constructorOnly src: SourceFile
  ) extends Tree[T]
      with WithoutTypeOrPos[T]:
    ...
    def mapElems[U >: T <: Untyped](op: Tree[T] => Tree[U]): Thicket[U] =
      ...
      else Thicket[U](newTrees)(source).asInstanceOf[this.type]
    ...
    override def toString: String =
      if isEmpty then "EmptyTree" else "Thicket(" + trees.mkString(", ") + ")"
  ...
  class EmptyTree[T <: Untyped] extends Thicket(Nil)(NoSource):
    ...
    override def withSpan(span: Span) = throw AssertionError(
      "Cannot change span of EmptyTree"
    )

  class EmptyValDef[T <: Untyped]
      extends ValDef[T](nme.WILDCARD, genericEmptyTree[T], genericEmptyTree[T])(
        NoSource
      )
      with WithoutTypeOrPos[T]:
    myTpe = NoType.asInstanceOf[T]
    ...
    override def withSpan(span: Span) = throw AssertionError(
      "Cannot change span of EmptyValDef"
    )

  @sharable val theEmptyTree = new EmptyTree[Type]()
  @sharable val theEmptyValDef = new EmptyValDef[Type]()

  def genericEmptyValDef[T <: Untyped]: ValDef[T] =
    theEmptyValDef.asInstanceOf[ValDef[T]]
  def genericEmptyTree[T <: Untyped]: Thicket[T] =
    theEmptyTree.asInstanceOf[Thicket[T]]

  def flatten[T <: Untyped](trees: List[Tree[T]]): List[Tree[T]] =
    def recur(
        buf: ListBuffer[Tree[T]] | Null,
        remaining: List[Tree[T]]
    ): ListBuffer[Tree[T]] | Null =
      remaining match
  ...
  /** A base trait for lazy tree fields. These can be instantiated with Lazy
    * instances which can delay tree construction until the field is first
    * demanded.
    */
  trait Lazy[+T <: AnyRef]:
    def complete(using Context): T

  /** A tree that can have a lazy fields. Such fields are variables of type `T |
    * Lazy[T]`, for some tyope `T`.
    */
  ...
  abstract class Instance[T <: Untyped]:
    ...
    @sharable val ContextualEmptyTree: Thicket =
      new EmptyTree() // an empty tree marking a contextual closure

    // ----- Auxiliary creation methods ------------------

    def Thicket(): Thicket = EmptyTree
    def Thicket(x1: Tree, x2: Tree)(implicit src: SourceFile): Thicket =
      new Thicket(x1 :: x2 :: Nil)
    def Thicket(x1: Tree, x2: Tree, x3: Tree)(implicit
        src: SourceFile
    ): Thicket = new Thicket(x1 :: x2 :: x3 :: Nil)
    def Thicket(xs: List[Tree])(implicit src: SourceFile) = new Thicket(xs)

    def flatTree(xs: List[Tree])(implicit src: SourceFile): Tree = flatten(
      xs
    ) match
      case x :: Nil => x
      case ys       => Thicket(ys)
    ...
    /** A class for copying trees. The copy methods avoid creating a new tree If
      * all arguments stay the same.
      *
      * Note: Some of the copy methods take a context. These are exactly those
      * methods that are overridden in TypedTreeCopier so that they selectively
      * retype themselves. Retyping needs a context.
      */
    abstract class TreeCopier:
      ...
      def Select(tree: Tree)(qualifier: Tree, name: Name)(using
          Context
      ): Select = tree match
        case tree: SelectWithSig =>
          if (qualifier eq tree.qualifier) && (name == tree.name) then tree
          else
            finalize(
              tree,
              SelectWithSig(qualifier, name, tree.sig)(sourceFile(tree))
            )
        case tree: Select
            if (qualifier eq tree.qualifier) && (name == tree.name) =>
          tree
        case _ =>
          finalize(tree, untpd.Select(qualifier, name)(sourceFile(tree)))
      ...
      def Super(tree: Tree)(qual: Tree, mix: untpd.Ident)(using
          Context
      ): Super = tree match
      ...
      def TypeApply(tree: Tree)(fun: Tree, args: List[Tree])(using
          Context
      ): TypeApply = tree match
      ...
      def NamedArg(tree: Tree)(name: Name, arg: Tree)(using Context): NamedArg =
        tree match
          case tree: NamedArg if (name == tree.name) && (arg eq tree.arg) =>
            tree
      ...
      def Block(tree: Tree)(stats: List[Tree], expr: Tree)(using
          Context
      ): Block = tree match
        case tree: Block if (stats eq tree.stats) && (expr eq tree.expr) => tree
        case _ => finalize(tree, untpd.Block(stats, expr)(sourceFile(tree)))
      def If(tree: Tree)(cond: Tree, thenp: Tree, elsep: Tree)(using
          Context
      ): If = tree match
        case tree: If
            if (cond eq tree.cond) && (thenp eq tree.thenp) && (elsep eq tree.elsep) =>
          tree
        case tree: InlineIf =>
          finalize(tree, untpd.InlineIf(cond, thenp, elsep)(sourceFile(tree)))
        case _ => finalize(tree, untpd.If(cond, thenp, elsep)(sourceFile(tree)))
      def Closure(tree: Tree)(env: List[Tree], meth: Tree, tpt: Tree)(using
          Context
      ): Closure = tree match
        case tree: Closure
            if (env eq tree.env) && (meth eq tree.meth) && (tpt eq tree.tpt) =>
          tree
        case _ =>
          finalize(tree, untpd.Closure(env, meth, tpt)(sourceFile(tree)))
      def Match(tree: Tree)(selector: Tree, cases: List[CaseDef])(using
          Context
      ): Match = tree match
        case tree: Match
            if (selector eq tree.selector) && (cases eq tree.cases) =>
          tree
        case tree: InlineMatch =>
          finalize(tree, untpd.InlineMatch(selector, cases)(sourceFile(tree)))
        case _ => finalize(tree, untpd.Match(selector, cases)(sourceFile(tree)))
      def CaseDef(tree: Tree)(pat: Tree, guard: Tree, body: Tree)(using
          Context
      ): CaseDef = tree match
        case tree: CaseDef
            if (pat eq tree.pat) && (guard eq tree.guard) && (body eq tree.body) =>
          tree
        case _ =>
          finalize(tree, untpd.CaseDef(pat, guard, body)(sourceFile(tree)))
      def Labeled(tree: Tree)(bind: Bind, expr: Tree)(using Context): Labeled =
        tree match
          case tree: Labeled if (bind eq tree.bind) && (expr eq tree.expr) =>
            tree
      ...
      def Return(tree: Tree)(expr: Tree, from: Tree)(using Context): Return =
        tree match
          case tree: Return if (expr eq tree.expr) && (from eq tree.from) =>
            tree
      ...
      def WhileDo(tree: Tree)(cond: Tree, body: Tree)(using Context): WhileDo =
        tree match
          case tree: WhileDo if (cond eq tree.cond) && (body eq tree.body) =>
            tree
          case _ => finalize(tree, untpd.WhileDo(cond, body)(sourceFile(tree)))
      def Try(tree: Tree)(expr: Tree, cases: List[CaseDef], finalizer: Tree)(
          using Context
      ): Try = tree match
        case tree: Try
            if (expr eq tree.expr) && (cases eq tree.cases) && (finalizer eq tree.finalizer) =>
          tree
        case _ =>
          finalize(tree, untpd.Try(expr, cases, finalizer)(sourceFile(tree)))
      def SeqLiteral(tree: Tree)(elems: List[Tree], elemtpt: Tree)(using
          Context
      ): SeqLiteral = tree match
        case tree: JavaSeqLiteral =>
          if (elems eq tree.elems) && (elemtpt eq tree.elemtpt) then tree
          else finalize(tree, untpd.JavaSeqLiteral(elems, elemtpt))
        case tree: SeqLiteral
            if (elems eq tree.elems) && (elemtpt eq tree.elemtpt) =>
          tree
        case _ =>
          finalize(tree, untpd.SeqLiteral(elems, elemtpt)(sourceFile(tree)))
      def Inlined(
          tree: Tree
      )(call: tpd.Tree, bindings: List[MemberDef], expansion: Tree)(using
          Context
      ): Inlined = tree match
        case tree: Inlined
            if (call eq tree.call) && (bindings eq tree.bindings) && (expansion eq tree.expansion) =>
          tree
        case _ =>
          finalize(
            tree,
            untpd.Inlined(call, bindings, expansion)(sourceFile(tree))
          )
      def Quote(tree: Tree)(body: Tree, tags: List[Tree])(using
          Context
      ): Quote = tree match
      ...
      def SplicePattern(tree: Tree)(body: Tree, args: List[Tree])(using
          Context
      ): SplicePattern = tree match
        case tree: SplicePattern
            if (body eq tree.body) && (args eq tree.args) =>
          tree
        case _ =>
          finalize(tree, untpd.SplicePattern(body, args)(sourceFile(tree)))
      def SingletonTypeTree(tree: Tree)(ref: Tree)(using
          Context
      ): SingletonTypeTree = tree match
        case tree: SingletonTypeTree if (ref eq tree.ref) => tree
        case _ => finalize(tree, untpd.SingletonTypeTree(ref)(sourceFile(tree)))
      def RefinedTypeTree(tree: Tree)(tpt: Tree, refinements: List[Tree])(using
          Context
      ): RefinedTypeTree = tree match
        case tree: RefinedTypeTree
            if (tpt eq tree.tpt) && (refinements eq tree.refinements) =>
          tree
        case _ =>
          finalize(
            tree,
            untpd.RefinedTypeTree(tpt, refinements)(sourceFile(tree))
          )
      def AppliedTypeTree(tree: Tree)(tpt: Tree, args: List[Tree])(using
          Context
      ): AppliedTypeTree = tree match
        case tree: AppliedTypeTree
            if (tpt eq tree.tpt) && (args eq tree.args) =>
          tree
        case _ =>
          finalize(tree, untpd.AppliedTypeTree(tpt, args)(sourceFile(tree)))
      def LambdaTypeTree(tree: Tree)(tparams: List[TypeDef], body: Tree)(using
          Context
      ): LambdaTypeTree = tree match
        case tree: LambdaTypeTree
            if (tparams eq tree.tparams) && (body eq tree.body) =>
          tree
        case _ =>
          finalize(tree, untpd.LambdaTypeTree(tparams, body)(sourceFile(tree)))
      def TermLambdaTypeTree(tree: Tree)(params: List[ValDef], body: Tree)(using
          Context
      ): TermLambdaTypeTree = tree match
        case tree: TermLambdaTypeTree
            if (params eq tree.params) && (body eq tree.body) =>
          tree
        case _ =>
          finalize(
            tree,
            untpd.TermLambdaTypeTree(params, body)(sourceFile(tree))
          )
      def MatchTypeTree(
          tree: Tree
      )(bound: Tree, selector: Tree, cases: List[CaseDef])(using
          Context
      ): MatchTypeTree = tree match
        case tree: MatchTypeTree
            if (bound eq tree.bound) && (selector eq tree.selector) && (cases eq tree.cases) =>
          tree
        case _ =>
          finalize(
            tree,
            untpd.MatchTypeTree(bound, selector, cases)(sourceFile(tree))
          )
      def ByNameTypeTree(tree: Tree)(result: Tree)(using
          Context
      ): ByNameTypeTree = tree match
        case tree: ByNameTypeTree if (result eq tree.result) => tree
        case _ => finalize(tree, untpd.ByNameTypeTree(result)(sourceFile(tree)))
      def TypeBoundsTree(tree: Tree)(lo: Tree, hi: Tree, alias: Tree)(using
          Context
      ): TypeBoundsTree = tree match
        case tree: TypeBoundsTree
            if (lo eq tree.lo) && (hi eq tree.hi) && (alias eq tree.alias) =>
          tree
        case _ =>
          finalize(tree, untpd.TypeBoundsTree(lo, hi, alias)(sourceFile(tree)))
      ...
      def Alternative(tree: Tree)(trees: List[Tree])(using
          Context
      ): Alternative = tree match
        case tree: Alternative if (trees eq tree.trees) => tree
        case _ => finalize(tree, untpd.Alternative(trees)(sourceFile(tree)))
      def UnApply(
          tree: Tree
      )(fun: Tree, implicits: List[Tree], patterns: List[Tree])(using
          Context
      ): UnApply = tree match
        case tree: UnApply
            if (fun eq tree.fun) && (implicits eq tree.implicits) && (patterns eq tree.patterns) =>
          tree
        case _ =>
          finalize(
            tree,
            untpd.UnApply(fun, implicits, patterns)(sourceFile(tree))
          )
      def ValDef(tree: Tree)(name: TermName, tpt: Tree, rhs: LazyTree)(using
          Context
      ): ValDef = tree match
        case tree: ValDef
            if (name == tree.name) && (tpt eq tree.tpt) && (rhs eq tree.unforcedRhs) =>
          tree
        case _ => finalize(tree, untpd.ValDef(name, tpt, rhs)(sourceFile(tree)))
      def DefDef(
          tree: Tree
      )(name: TermName, paramss: List[ParamClause], tpt: Tree, rhs: LazyTree)(
          using Context
      ): DefDef = tree match
        case tree: DefDef
            if (name == tree.name) && (paramss eq tree.paramss) && (tpt eq tree.tpt) && (rhs eq tree.unforcedRhs) =>
          tree
        case _ =>
          finalize(
            tree,
            untpd.DefDef(name, paramss, tpt, rhs)(sourceFile(tree))
          )
      def TypeDef(tree: Tree)(name: TypeName, rhs: Tree)(using
          Context
      ): TypeDef = tree match
      ...
      def Template(tree: Tree)(
          constr: DefDef,
          parents: List[Tree],
          derived: List[untpd.Tree],
          self: ValDef,
          body: LazyTreeList
      )(using Context): Template = tree match
        case tree: Template
            if (constr eq tree.constr) && (parents eq tree.parents) && (derived eq tree.derived) && (self eq tree.self) && (body eq tree.unforcedBody) =>
          tree
        case tree =>
          finalize(
            tree,
            untpd.Template(constr, parents, derived, self, body)(
              sourceFile(tree)
            )
          )
      def Import(tree: Tree)(expr: Tree, selectors: List[untpd.ImportSelector])(
          using Context
      ): Import = tree match
        case tree: Import
            if (expr eq tree.expr) && (selectors eq tree.selectors) =>
          tree
        case _ =>
          finalize(tree, untpd.Import(expr, selectors)(sourceFile(tree)))
      def Export(tree: Tree)(expr: Tree, selectors: List[untpd.ImportSelector])(
          using Context
      ): Export = tree match
        case tree: Export
            if (expr eq tree.expr) && (selectors eq tree.selectors) =>
          tree
        case _ =>
          finalize(tree, untpd.Export(expr, selectors)(sourceFile(tree)))
      def PackageDef(tree: Tree)(pid: RefTree, stats: List[Tree])(using
          Context
      ): PackageDef = tree match
        case tree: PackageDef if (pid eq tree.pid) && (stats eq tree.stats) =>
          tree
        case _ => finalize(tree, untpd.PackageDef(pid, stats)(sourceFile(tree)))
      def Annotated(tree: Tree)(arg: Tree, annot: Tree)(using
          Context
      ): Annotated = tree match
        case tree: Annotated if (arg eq tree.arg) && (annot eq tree.annot) =>
          tree
      ...
      def Hole(
          tree: Tree
      )(isTerm: Boolean, idx: Int, args: List[Tree], content: Tree)(using
          Context
      ): Hole = tree match
        case tree: Hole
            if isTerm == tree.isTerm && idx == tree.idx && args.eq(
              tree.args
            ) && content.eq(tree.content) && content.eq(tree.content) =>
          tree
        case _ =>
          finalize(
            tree,
            untpd.Hole(isTerm, idx, args, content)(sourceFile(tree))
          )
      ...
      def If(tree: If)(
          cond: Tree = tree.cond,
          thenp: Tree = tree.thenp,
          elsep: Tree = tree.elsep
      )(using Context): If =
        If(tree: Tree)(cond, thenp, elsep)
      def Closure(tree: Closure)(
          env: List[Tree] = tree.env,
          meth: Tree = tree.meth,
          tpt: Tree = tree.tpt
      )(using Context): Closure =
        Closure(tree: Tree)(env, meth, tpt)
      def CaseDef(
          tree: CaseDef
      )(pat: Tree = tree.pat, guard: Tree = tree.guard, body: Tree = tree.body)(
          using Context
      ): CaseDef =
        CaseDef(tree: Tree)(pat, guard, body)
      def Try(tree: Try)(
          expr: Tree = tree.expr,
          cases: List[CaseDef] = tree.cases,
          finalizer: Tree = tree.finalizer
      )(using Context): Try =
        Try(tree: Tree)(expr, cases, finalizer)
      def UnApply(tree: UnApply)(
          fun: Tree = tree.fun,
          implicits: List[Tree] = tree.implicits,
          patterns: List[Tree] = tree.patterns
      )(using Context): UnApply =
        UnApply(tree: Tree)(fun, implicits, patterns)
      def ValDef(tree: ValDef)(
          name: TermName = tree.name,
          tpt: Tree = tree.tpt,
          rhs: LazyTree = tree.unforcedRhs
      )(using Context): ValDef =
        ValDef(tree: Tree)(name, tpt, rhs)
      def DefDef(tree: DefDef)(
          name: TermName = tree.name,
          paramss: List[ParamClause] = tree.paramss,
          tpt: Tree = tree.tpt,
          rhs: LazyTree = tree.unforcedRhs
      ...
      def TypeDef(
          tree: TypeDef
      )(name: TypeName = tree.name, rhs: Tree = tree.rhs)(using
          Context
      ): TypeDef =
        TypeDef(tree: Tree)(name, rhs)
      def Template(tree: Template)(using Context)(
          constr: DefDef = tree.constr,
          parents: List[Tree] = tree.parents,
          derived: List[untpd.Tree] = tree.derived,
          self: ValDef = tree.self,
          body: LazyTreeList = tree.unforcedBody
      ): Template =
        Template(tree: Tree)(constr, parents, derived, self, body)
      def Hole(tree: Hole)(
          isTerm: Boolean = tree.isTerm,
          idx: Int = tree.idx,
          args: List[Tree] = tree.args,
    ...
    /** For untyped trees, this is just the identity. For typed trees, a context
      * derived form `ctx` that records `call` as the innermost enclosing call
      * for which the inlined version is currently processed.
      */
    ...
    /** The context to use when transforming a tree. It ensures that the source
      * is correct, and that the local context is used if that's necessary for
      * transforming the whole tree. TODO: ensure transform is always called
      * with the correct context as argument
      * @see
      *   https://github.com/lampepfl/dotty/pull/13880#discussion_r836395977
      */
    def transformCtx(tree: Tree)(using Context): Context =
      ...
      tree match
        case t: (MemberDef | PackageDef | LambdaTypeTree |
              TermLambdaTypeTree) =>
    ...
    abstract class TreeMap(val cpy: TreeCopier = inst.cpy):
          ...
          if skipTransform(tree) then tree
            ...
            tree match
              ...
              case Inlined(call, bindings, expansion) =>
                cpy.Inlined(tree)(
                  call,
                  transformSub(bindings),
                  transform(expansion)(using inlineContext(call))
                )
              case TypeTree() =>
                tree
              ...
              case tree @ Template(constr, parents, self, _)
                  if tree.derived.isEmpty =>
                cpy.Template(tree)(
                  transformSub(constr),
                  transform(tree.parents),
                  Nil,
                  transformSub(self),
              ...
              case PackageDef(pid, stats) =>
                cpy.PackageDef(tree)(
                  transformSub(pid),
                  transformStats(stats, ctx.owner)
                )
              case Annotated(arg, annot) =>
                cpy.Annotated(tree)(transform(arg), transform(annot))
              case Thicket(trees) =>
                val trees1 = transform(trees)
                if trees1 eq trees then tree else Thicket(trees1)
              ...
              case tree @ SplicePattern(body, args) =>
                cpy.SplicePattern(tree)(
                  transform(body)(using spliceContext),
                  transform(args)
                )
      ...
      def transformStats(trees: List[Tree], exprOwner: Symbol)(using
          Context
      ): List[Tree] =
        transform(trees)
      ...
      def transformParamss(paramss: List[ParamClause])(using
          Context
      ): List[ParamClause] =
    ...
    abstract class TreeAccumulator[X]:
      ...
      def foldMoreCases(x: X, tree: Tree)(using Context): X =
        assert(
          ctx.reporter.hasUnreportedErrors
            || ctx.reporter.errorsReported
    ...
    /** Fold `f` over all tree nodes, in depth-first, prefix order, but don't
      * visit subtrees where `f` returns a different result for the root, i.e.
      * `f(x, root) ne x`.
      */
    class ShallowFolder[X](f: (X, Tree) => X) extends TreeAccumulator[X]:
      def apply(x: X, tree: Tree)(using Context): X =
        val x1 = f(x, tree)
        if x1.asInstanceOf[AnyRef] ne x.asInstanceOf[AnyRef] then x1
        else foldOver(x1, tree)

    def rename(
        tree: NameTree,
        newName: Name
    )(using Context): tree.ThisTree[T] = {
      tree match
    ...
    def termParamssIn(paramss: List[ParamClause]): List[List[ValDef]] =
      paramss match
    ...
    /** If `tparams` is non-empty, add it to the left `paramss`, merging it with
      * a leading type parameter list of `paramss`, if one exists.
      */
    def joinParams(
      ...
      if tparams.isEmpty then paramss
        ...
        paramss match
          case TypeDefs(tparams1) :: paramss1 =>
            (tparams ++ tparams1) :: paramss1
    ...
    /** Delegate to FunProto or FunProtoTyped depending on whether the prefix is
      * `untpd` or `tpd`.
      */
    protected def FunProto(args: List[Tree], resType: Type)(using
        Context
    ): ProtoTypes.FunProto

    /** Construct the application `$receiver.$method[$targs]($args)` using
      * overloading resolution to find a matching overload of `$method` if
      * necessary. This is useful when overloading resolution needs to be
      * performed in a phase after typer. Note that this will not perform any
      * kind of implicit search.
      *
      * @param expectedType
      *   An expected type of the application used to guide overloading
      *   resolution
      */
    def applyOverloaded(
        receiver: tpd.Tree,
        ...
        targs: List[Type],
        expectedType: Type
    )(using parentCtx: Context): tpd.Tree =
      ...
      val selected =
        if denot.isOverloaded then
          ...
          assert(
            alternatives.size == 1,
            i"${if alternatives.isEmpty then "no" else "multiple"} overloads available for " +
          ...
          ) // this is parsed from bytecode tree. there's nothing user can do about it
          alternatives.head
    ...
    def resolveConstructor(atp: Type, args: List[Tree])(using
        Context
    ): tpd.Tree =
      val targs = atp.argTypes
      withoutMode(Mode.PatternOrTypeBits) {
        applyOverloaded(
          tpd.New(atp.typeConstructor),
          nme.CONSTRUCTOR,
          args,
          targs,
          atp
