...
object tpd extends Trees.Instance[Type] with TypedTreeInfo:
  ...
  def Super(qual: Tree, mix: untpd.Ident, mixinClass: Symbol)(using
      Context
  ): Super =
    ta.assignType(untpd.Super(qual, mix), qual, mixinClass)

  def Super(qual: Tree, mixName: TypeName, mixinClass: Symbol = NoSymbol)(using
      Context
  ): Super =
    Super(
      qual,
      if mixName.isEmpty then untpd.EmptyTypeIdent else untpd.Ident(mixName),
      mixinClass
    )
    ...
    if stats.isEmpty then expr
      ...
      expr match
        ...
        case Block(estats, eexpr) =>
          cpy
            .Block(expr)(stats ::: estats, eexpr)
            .withType(ta.avoidingType(eexpr, stats))
        case _ =>
          Block(stats, expr)
  ...
  /** A function def
    ...
    *
    * where the closure's type is the target type of the expression (FunctionN,
    * unless otherwise specified).
    */
  def Closure(
      meth: TermSymbol,
      rhsFn: List[List[Tree]] => Tree,
      targs: List[Tree] = Nil,
      targetType: Type = NoType
  )(using Context): Block =
    val targetTpt =
      if targetType.exists then TypeTree(targetType) else EmptyTree
    val call =
      if targs.isEmpty then Ident(TermRef(NoPrefix, meth))
      else TypeApply(Ident(TermRef(NoPrefix, meth)), targs)
    Block(DefDef(meth, rhsFn) :: Nil, Closure(Nil, call, targetTpt))
  ...
  def Try(block: Tree, cases: List[CaseDef], finalizer: Tree)(using
      Context
  ): Try =
  ...
  def JavaSeqLiteral(elems: List[Tree], elemtpt: Tree)(using
      Context
  ): JavaSeqLiteral =
    ta.assignType(untpd.JavaSeqLiteral(elems, elemtpt), elems, elemtpt)
      .asInstanceOf[JavaSeqLiteral]

  def Inlined(call: Tree, bindings: List[MemberDef], expansion: Tree)(using
      Context
  ): Inlined =
  ...
  def TypeTree(tp: Type, inferred: Boolean = false)(using Context): TypeTree =
    (if inferred then untpd.InferredTypeTree() else untpd.TypeTree())
      .withType(tp)
  ...
  def RefinedTypeTree(
      parent: Tree,
      refinements: List[Tree],
      refineCls: ClassSymbol
  )(using Context): Tree =
    ta.assignType(
      untpd.RefinedTypeTree(parent, refinements),
      parent,
      refinements,
      refineCls
    )

  def AppliedTypeTree(tycon: Tree, args: List[Tree])(using
      Context
  ): AppliedTypeTree =
  ...
  def LambdaTypeTree(tparams: List[TypeDef], body: Tree)(using
      Context
  ): LambdaTypeTree =
    ta.assignType(untpd.LambdaTypeTree(tparams, body), tparams, body)

  def MatchTypeTree(bound: Tree, selector: Tree, cases: List[CaseDef])(using
      Context
  ): MatchTypeTree =
    ta.assignType(
      untpd.MatchTypeTree(bound, selector, cases),
      bound,
      selector,
      cases
    )

  def TypeBoundsTree(lo: Tree, hi: Tree, alias: Tree = EmptyTree)(using
      Context
  ): TypeBoundsTree =
  ...
  def ValDef(
      sym: TermSymbol,
      rhs: LazyTree = EmptyTree,
      inferred: Boolean = false
  )(using Context): ValDef =
    ta.assignType(
      untpd.ValDef(sym.name, TypeTree(sym.info, inferred), rhs),
      sym
    )

  def SyntheticValDef(name: TermName, rhs: Tree, flags: FlagSet = EmptyFlags)(
      using Context
  ): ValDef =
    ValDef(
      newSymbol(
  ...
  def DefDef(
    ...
    ta.assignType(
      untpd.DefDef(
        sym.name,
        paramss.map {
          case TypeSymbols(params) =>
            params.map(param => TypeDef(param).withSpan(param.span))
          case TermSymbols(params) =>
            params.map(param => ValDef(param).withSpan(param.span))
          case _ => unreachable()
        },
        TypeTree(resultType),
        rhs
  ...
  /** A DefDef with given method symbol `sym`.
    * @rhsFn
    *   A function from parameter references to the method's right-hand side.
    *   Parameter symbols are taken from the `rawParamss` field of `sym`, or are
    *   freshly generated if `rawParamss` is empty.
    */
  def DefDef(sym: TermSymbol, rhsFn: List[List[Tree]] => Tree)(using
      Context
  ): DefDef =
    ...
    def recur(
        tp: Type,
        remaining: List[List[Symbol]]
    ): (Type, List[List[Symbol]]) = tp match
      case tp: PolyType =>
        val (tparams: List[TypeSymbol], remaining1) = remaining match
          case tparams :: remaining1 =>
            assert(
              tparams.hasSameLengthAs(tp.paramNames) && tparams.head.isType
            )
          ...
          case nil =>
            (
              newTypeParams(
                ...
                tp.paramNames,
                EmptyFlags,
                tp.instantiateParamInfos(_)
              ),
              Nil
            )
        val (rtp, paramss) =
          recur(tp.instantiate(tparams.map(_.typeRef)), remaining1)
        (rtp, tparams :: paramss)
      case tp: MethodType =>
        ...
        val previousParamRefs: ListBuffer[TermRef] =
          ...
          if isParamDependent then mutable.ListBuffer[TermRef]()
          else (null: ListBuffer[TermRef] | Null).uncheckedNN
        ...
        def valueParam(
          ...
          def makeSym(info: Type) = newSymbol(
            sym,
            name,
            TermParam | maybeImplicit | maybeErased,
            info,
            coord = sym.coord
          )
          ...
          if isParamDependent then
            val sym = makeSym(
              origInfo.substParams(tp, previousParamRefs.toList)
            )
          ...
          if tp.paramNames.isEmpty then (Nil, remaining)
            ...
            remaining match
              case vparams :: remaining1 =>
                assert(
                  vparams.hasSameLengthAs(tp.paramNames) && vparams.head.isTerm
                )
                (vparams.asInstanceOf[List[TermSymbol]], remaining1)
              case nil =>
                (
                  tp.paramNames
                    .lazyZip(tp.paramInfos)
                    .lazyZip(tp.erasedParams)
                    .map(valueParam),
                  Nil
                )
        val (rtp, paramss) =
          recur(tp.instantiate(vparams.map(_.termRef)), remaining1)
        (rtp, vparams :: paramss)
      case _ =>
  ...
  def ClassDef(
    ...
    ClassDefWithParents(
      cls,
      constr,
      superRef :: otherParents.map(TypeTree(_)),
      body
    )

  def ClassDefWithParents(
      cls: ClassSymbol,
      constr: DefDef,
      parents: List[Tree],
      body: List[Tree]
    ...
    val selfType =
      if cls.classInfo.selfInfo ne NoType then ValDef(newSelfSym(cls))
      else EmptyValDef
    ...
    val localDummy = body
      .foldLeft(NoSymbol: Symbol)(findLocalDummy.apply)
      .orElse(newLocalDummy(cls))
    val impl = untpd
      .Template(constr, parents, Nil, selfType, newTypeParams ++ body)
  ...
  /** An anonymous class
    ...
    * @param fns
    *   a non-empty of functions for which forwarders should be defined in the
    *   class. The class has the same owner as the first function in `fns`. Its
    *   position is the union of all functions in `fns`.
    */
  def AnonClass(
      parents: List[Type],
      fns: List[TermSymbol],
      methNames: List[TermName]
  )(using Context): Block =
    AnonClass(fns.head.owner, parents, fns.map(_.span).reduceLeft(_ union _)) {
      cls =>
  ...
  /** An anonymous class
    ...
    */
  def AnonClass(owner: Symbol, parents: List[Type], coord: Coord)(
      body: ClassSymbol => List[Tree]
  )(using Context): Block =
    val parents1 =
      if parents.head.classSymbol.is(Trait) then
        val head = parents.head.parents.head
        if head.isRef(defn.AnyClass) then defn.AnyRefType :: parents
        else head :: parents
      else parents
  ...
  def Import(expr: Tree, selectors: List[untpd.ImportSelector])(using
      Context
  ): Import =
    ta.assignType(
      untpd.Import(expr, selectors),
      newImportSymbol(ctx.owner, expr)
    )

  def Export(expr: Tree, selectors: List[untpd.ImportSelector])(using
      Context
  ): Export =
  ...
  def prefixIsElidable(tp: NamedType)(using Context): Boolean =
    val typeIsElidable = tp.prefix match
      case pre: ThisType =>
        ...
        tp.symbol.isParamOrAccessor && !pre.cls.is(
          Trait
        ) && ctx.owner.enclosingClass == pre.cls
  ...
  def ref(tp: NamedType, needLoad: Boolean = true)(using Context): Tree =
    ...
    else if tp.symbol.is(Module) && ctx.owner.isContainedIn(
        tp.symbol.moduleClass
      )
    then followOuterLinks(This(tp.symbol.moduleClass.asClass))
    else if tp.symbol hasAnnotation defn.ScalaStaticAnnot then Ident(tp)
    else
      val pre = tp.prefix
      if pre.isSingleton then
        followOuterLinks(singleton(pre.dealias, needLoad)).select(tp)
      else
  ...
  private def followOuterLinks(t: Tree)(using Context) = t match
    case t: This
        if ctx.erasedTypes && !(t.symbol == ctx.owner.enclosingClass || t.symbol.isStaticOwner) =>
      // after erasure outer paths should be respected
      ExplicitOuter.OuterOps(ctx).path(toCls = t.tpe.classSymbol)
    case t =>
      t

  def singleton(tp: Type, needLoad: Boolean = true)(using Context): Tree =
    tp.dealias match
  ...
  /** A path that corresponds to the given type `tp`. Error if `tp` is not a
    * refinement of an addressable singleton type.
    */
  ...
  /** A tree representing a `newXYZArray` operation of the right kind for the
    * given element type in `elemTpe`. No type arguments or `length` arguments
    * are given.
    */
  def newArray(
      elemTpe: Type,
    ...
    if !ctx.erasedTypes then
      assert(
        !TypeErasure.isGeneric(elemTpe),
        elemTpe
      ) // needs to be done during typer. See Applications.convertNewGenericArray
      newArr
        .appliedToTypeTrees(TypeTree(returnTpe) :: Nil)
        .appliedToTermArgs(clsOf(elemTpe) :: clsOf(returnTpe) :: dims :: Nil)
        .withSpan(span)
    else // after erasure
      newArr
        .appliedToTermArgs(clsOf(elemTpe) :: clsOf(returnTpe) :: dims :: Nil)
  ...
  def wrapArrayMethodName(elemtp: Type)(using Context): TermName =
    ...
    if elemCls.isPrimitiveValueClass then nme.wrapXArray(elemCls.name)
    else if elemCls.derivesFrom(defn.ObjectClass) && !elemCls.isNotRuntimeClass
    then nme.wrapRefArray
    else nme.genericWrapArray

  /** A tree representing a `wrapXYZArray(tree)` operation of the right kind for
    * the given element type in `elemTpe`.
    */
  def wrapArray(tree: Tree, elemtp: Type)(using Context): Tree =
    val wrapper = ref(defn.getWrapVarargsArrayModule)
      ...
      .appliedToTypes(
        if elemtp.isPrimitiveValueType then Nil else elemtp :: Nil
      )
  ...
  def New(tp: Type, constr: TermSymbol, args: List[Tree])(using
      Context
  ): Apply =
  ...
  /** An object def
    ...
    * gets expanded to
    *
    * <module> val obj = new obj$ <module> class obj$ extends parents { this:
    * obj.type => decls }
    *
    * (The following no longer applies: What's interesting here is that the
    * block is well typed (because class obj$ is hoistable), but the type of the
    * `obj` val is not expressible. What needs to happen in general when
    * inferring the type of a val from its RHS, is: if the type contains a class
    * that has the val itself as owner, then that class is remapped to have the
    * val's owner as owner. Remapping could be done by cloning the class with
    * the new owner and substituting everywhere in the tree. We know that
    * remapping is safe because the only way a local class can appear in the RHS
    * of a val is by being hoisted outside of a block, and the necessary checks
    * are done at this point already.
    *
    * On the other hand, for method result type inference, if the type of the
    * RHS of a method contains a class owned by the method, this would be an
    * error.)
    */
  def ModuleDef(sym: TermSymbol, body: List[Tree])(using Context): tpd.Thicket =
    val modcls = sym.moduleClass.asClass
    val constrSym =
      modcls.primaryConstructor orElse newDefaultConstructor(modcls).entered
    val constr = DefDef(constrSym.asTerm, EmptyTree)
    val clsdef = ClassDef(modcls, constr, body)
    val valdef =
      ValDef(sym, New(modcls.typeRef).select(constrSym).appliedToNone)
    Thicket(valdef, clsdef)
  end ModuleDef

  /** A `_` with given type */
  def Underscore(tp: Type)(using Context): Ident =
    untpd.Ident(nme.WILDCARD).withType(tp)
  ...
  private class FindLocalDummyAccumulator(cls: ClassSymbol)(using Context)
      extends TreeAccumulator[Symbol]:
    def apply(sym: Symbol, tree: Tree)(using Context) =
      if sym.exists then sym
      else if tree.isDef then
        val owner = tree.symbol.owner
        if owner.isLocalDummy && owner.owner == cls then owner
        else if owner == cls then foldOver(sym, tree)
        else sym
  ...
  def localCtx(tree: Tree)(using Context): Context =
    ctx.withOwner(localOwner(tree))

  override val cpy
      : TypedTreeCopier = // Type ascription needed to pick up any new members in TreeCopier (currently there are none)
    TypedTreeCopier()
  ...
  class TypedTreeCopier extends TreeCopier:
    ...
    ): copied.ThisTree[Type] =
      copied.withTypeUnchecked(tree.tpe)

    protected val untpdCpy = untpd.cpy

    override def Select(tree: Tree)(qualifier: Tree, name: Name)(using
        Context
    ): Select =
        ...
        case _ =>
          val tree2: Select = tree.tpe match
            case tpe: NamedType =>
              ...
              if qualType.isExactlyNothing then
                tree1.withTypeUnchecked(tree.tpe)
              else tree1.withType(tpe.derivedSelect(qualType))
            case _ => tree1.withTypeUnchecked(tree.tpe)
          ConstFold.Select(tree2)

    override def Apply(tree: Tree)(fun: Tree, args: List[Tree])(using
        Context
    ): Apply =
    ...
    override def TypeApply(tree: Tree)(fun: Tree, args: List[Tree])(using
        Context
    ): TypeApply =
    ...
    override def Typed(tree: Tree)(expr: Tree, tpt: Tree)(using
        Context
    ): Typed =
      ta.assignType(untpdCpy.Typed(tree)(expr, tpt), tpt)

    override def NamedArg(tree: Tree)(name: Name, arg: Tree)(using
        Context
    ): NamedArg =
      ta.assignType(untpdCpy.NamedArg(tree)(name, arg), arg)

    override def Assign(tree: Tree)(lhs: Tree, rhs: Tree)(using
        Context
    ): Assign =
      ta.assignType(untpdCpy.Assign(tree)(lhs, rhs))

    override def Block(tree: Tree)(stats: List[Tree], expr: Tree)(using
        Context
    ): Block =
      val tree1 = untpdCpy.Block(tree)(stats, expr)
      tree match
        case tree: Block
            if (expr.tpe eq tree.expr.tpe) && (expr.tpe eq tree.tpe) =>
    ...
    override def If(tree: Tree)(cond: Tree, thenp: Tree, elsep: Tree)(using
        Context
    ): If =
      val tree1 = untpdCpy.If(tree)(cond, thenp, elsep)
      tree match
        case tree: If
            if (thenp.tpe eq tree.thenp.tpe) && (elsep.tpe eq tree.elsep.tpe) &&
    ...
    override def Closure(tree: Tree)(env: List[Tree], meth: Tree, tpt: Tree)(
        using Context
    ): Closure =
      ...
      tree match
        case tree: Closure
            if sameTypes(
              env,
              tree.env
            ) && (meth.tpe eq tree.meth.tpe) && (tpt.tpe eq tree.tpt.tpe) =>
          tree1.withTypeUnchecked(tree.tpe)
        case _ => ta.assignType(tree1, meth, tpt)

    override def Match(tree: Tree)(selector: Tree, cases: List[CaseDef])(using
        Context
    ): Match =
      val tree1 = untpdCpy.Match(tree)(selector, cases)
      tree match
        case tree: Match if sameTypes(cases, tree.cases) =>
          tree1.withTypeUnchecked(tree.tpe)
        case _ => ta.assignType(tree1, selector, cases)

    override def CaseDef(tree: Tree)(pat: Tree, guard: Tree, body: Tree)(using
        Context
    ): CaseDef =
      val tree1 = untpdCpy.CaseDef(tree)(pat, guard, body)
      tree match
        case tree: CaseDef if body.tpe eq tree.body.tpe =>
          tree1.withTypeUnchecked(tree.tpe)
        case _ => ta.assignType(tree1, pat, body)

    override def Labeled(tree: Tree)(bind: Bind, expr: Tree)(using
        Context
    ): Labeled =
      ta.assignType(untpdCpy.Labeled(tree)(bind, expr))

    override def Return(tree: Tree)(expr: Tree, from: Tree)(using
        Context
    ): Return =
      ta.assignType(untpdCpy.Return(tree)(expr, from))

    override def WhileDo(tree: Tree)(cond: Tree, body: Tree)(using
        Context
    ): WhileDo =
    ...
    override def Try(
        tree: Tree
    )(expr: Tree, cases: List[CaseDef], finalizer: Tree)(using Context): Try =
      val tree1 = untpdCpy.Try(tree)(expr, cases, finalizer)
      tree match
        case tree: Try
            if (expr.tpe eq tree.expr.tpe) && sameTypes(cases, tree.cases) =>
          tree1.withTypeUnchecked(tree.tpe)
        case _ => ta.assignType(tree1, expr, cases)

    override def Inlined(
        tree: Tree
    )(call: Tree, bindings: List[MemberDef], expansion: Tree)(using
        Context
    ): Inlined =
      ...
      tree match
        case tree: Inlined
            if sameTypes(
              bindings,
              tree.bindings
            ) && (expansion.tpe eq tree.expansion.tpe) =>
          tree1.withTypeUnchecked(tree.tpe)
        case _ => ta.assignType(tree1, bindings, expansion)

    override def SeqLiteral(tree: Tree)(elems: List[Tree], elemtpt: Tree)(using
        Context
    ): SeqLiteral =
      ...
      tree match
        case tree: SeqLiteral
            if sameTypes(
              elems,
              tree.elems
            ) && (elemtpt.tpe eq tree.elemtpt.tpe) =>
          tree1.withTypeUnchecked(tree.tpe)
        case _ =>
          ta.assignType(tree1, elems, elemtpt)

    override def Annotated(tree: Tree)(arg: Tree, annot: Tree)(using
        Context
    ): Annotated =
      val tree1 = untpdCpy.Annotated(tree)(arg, annot)
      tree match
        case tree: Annotated
            if (arg.tpe eq tree.arg.tpe) && (annot eq tree.annot) =>
          tree1.withTypeUnchecked(tree.tpe)
        case _ => ta.assignType(tree1, arg, annot)

    override def If(tree: If)(
        cond: Tree = tree.cond,
    ...
    override def Closure(
        tree: Closure
    )(env: List[Tree] = tree.env, meth: Tree = tree.meth, tpt: Tree = tree.tpt)(
        using Context
    ): Closure =
      Closure(tree: Tree)(env, meth, tpt)
    override def CaseDef(
        tree: CaseDef
    )(pat: Tree = tree.pat, guard: Tree = tree.guard, body: Tree = tree.body)(
        using Context
    ): CaseDef =
      CaseDef(tree: Tree)(pat, guard, body)
    override def Try(tree: Try)(
        expr: Tree = tree.expr,
  ...
  class TimeTravellingTreeCopier extends TypedTreeCopier:
    ...
    override def Closure(
      ...
      Closure(tree: Tree)(env, meth, tpt)
  end TimeTravellingTreeCopier

  override def skipTransform(tree: Tree)(using Context): Boolean =
    tree.tpe.isError

  implicit class TreeOps[ThisTree <: tpd.Tree](private val tree: ThisTree)
      extends AnyVal:
    ...
    def find[T](pred: (tpd.Tree) => Boolean)(using Context): Option[tpd.Tree] =
      shallowFold[Option[tpd.Tree]](None)((accum, tree) =>
        if pred(tree) then Some(tree) else accum
      )

    def subst(from: List[Symbol], to: List[Symbol])(using Context): ThisTree =
      TreeTypeMap(substFrom = from, substTo = to).apply(tree)

    /** Change owner from `from` to `to`. If `from` is a weak owner, also change
      * its owner to `to`, and continue until a non-weak owner is reached.
      */
    def changeOwner(from: Symbol, to: Symbol)(using Context): ThisTree =
      ...
      if from == to then tree else loop(from, Nil, to :: Nil)

    /** Set the owner of every definition in this tree which is not itself
      * contained in this tree to be `newowner`
      */
    def changeNonLocalOwners(newOwner: Symbol)(using Context): Tree =
      val ownerAcc = new TreeAccumulator[immutable.Set[Symbol]]:
        def apply(ss: immutable.Set[Symbol], tree: Tree)(using Context) =
          tree match
            case tree: DefTree =>
              val sym = tree.symbol
              if sym.exists && !sym.owner.is(Package) then ss + sym.owner
              else ss
            case _ =>
              foldOver(ss, tree)
    ...
    /** After phase `trans`, set the owner of every definition in this tree that
      * was formerly owned by `from` to `to`.
      */
    def changeOwnerAfter(from: Symbol, to: Symbol, trans: DenotTransformer)(
        using Context
    ): ThisTree =
      if ctx.phase == trans.next then
        val traverser = new TreeTraverser:
          def traverse(tree: Tree)(using Context) = tree match
            case tree: DefTree =>
              val sym = tree.symbol
              val prevDenot = atPhase(trans)(sym.denot)
              if prevDenot.effectiveOwner == from.skipWeakOwner then
                val d = sym.copySymDenotation(owner = to)
                d.installAfter(trans)
                d.transformAfter(
                  trans,
                  d =>
                    if d.owner eq from then d.copySymDenotation(owner = to)
                    else d
                )
              if sym.isWeakOwner then traverseChildren(tree)
            case _ =>
    ...
    /** A select node with the given selector name such that the designated
      * member satisfies predicate `p`. Useful for disambiguating overloaded
      * members.
      */
    ...
    /** A select node that selects the given symbol. Note: Need to make sure
      * this is in fact the symbol you would get when you select with the
      * symbol's name, otherwise a data race may occur which would be flagged by
      * -Yno-double-bindings.
      */
    ...
    /** A select node with the given selector name and signature and a computed
      * type
      */
    def selectWithSig(name: Name, sig: Signature, target: Name)(using
        Context
    ): Tree =
      untpd
        .SelectWithSig(tree, name, sig)
        .withType(tree.tpe.select(name.asTermName, sig, target))

    /** A select node with selector name and signature taken from `sym`. Note:
      * Use this method instead of select(sym) if the referenced symbol might be
      * overridden in the type of the qualifier prefix. See note on select(sym:
      * Symbol).
      */
    ...
    def appliedToArgs(args: List[Tree])(using Context): GenericApply =
      args match
    ...
    /** The current tree applied to given type argument list: `tree[targs(0),
      * ..., targs(targs.length - 1)]`
      */
    ...
    def ensureApplied(using Context): Tree =
      if tree.tpe.widen.isParameterless then tree else tree.appliedToNone

    /** `tree == that` */
    def equal(that: Tree)(using Context): Tree =
      if that.tpe.widen.isRef(defn.NothingClass) then Literal(Constant(false))
      else applyOverloaded(tree, nme.EQ, that :: Nil, Nil, defn.BooleanType)
    ...
    def isInstance(tp: Type)(using Context): Tree = tp.dealias match
      ...
      case tp: SingletonType =>
        if tp.widen.derivesFrom(defn.ObjectClass) then
          tree
            .ensureConforms(defn.ObjectType)
            .select(defn.Object_eq)
            .appliedTo(singleton(tp))
        else singleton(tp).equal(tree)
      case _ =>
    ...
    /** cast tree to `tp`, assuming no exception is raised, i.e the operation is
      * pure
      */
    ...
    def cast(tpt: TypeTree)(using Context): Tree =
      ...
      tree
        .select(
          if ctx.erasedTypes then defn.Any_asInstanceOf else defn.Any_typeCast
        )
        .appliedToTypeTree(tpt)

    /** cast `tree` to `tp` (or its box/unbox/cast equivalent when after erasure
      * and value and non-value types are mixed), unless tree's type already
      * conforms to `tp`.
      */
    def ensureConforms(tp: Type)(using Context): Tree =
      if tree.tpe <:< tp then tree
      else if !ctx.erasedTypes then cast(tp)
    ...
    def testNotNull(using Context): Tree =
      ...
      val nullLit =
        if ctx.explicitNulls then nullLiteral.cast(defn.AnyRefType)
        else nullLiteral
      receiver.select(defn.Object_ne).appliedTo(nullLit).withSpan(tree.span)

    /** If inititializer tree is `_`, the default value of its type, otherwise
      * the tree itself.
      */
    def wildcardToDefault(using Context): Tree =
      if isWildcardArg(tree) then defaultValue(tree.tpe) else tree
    ...
    /** The translation of `tree = rhs`. This is either the tree as an
      * assignment, or a setter call.
      */
    def becomes(rhs: Tree)(using Context): Tree =
      ...
      if sym.is(Method) then
        val setter = sym.setter.orElse {
          assert(
            sym.name.isSetterName && sym.info.firstParamTypes.nonEmpty,
            sym
          )
          sym
        }
    ...
    def annotated(annot: Tree)(using Context): Tree =
      if tree.isTerm then
        Typed(
          tree,
          TypeTree(AnnotatedType(tree.tpe.widenIfUnstable, Annotation(annot)))
        )
      else Annotated(tree, annot)

    /** A synthetic select with that will be turned into an outer path by
      * ExplicitOuter.
      * @param levels
    ...
    def outerSelect(levels: Int, tp: Type)(using Context): Tree =
      untpd
        .Select(tree, OuterSelectName(EmptyTermName, levels))
        .withType(SkolemType(tp))

    /** Replace Inlined nodes and InlineProxy references to underlying arguments
      */
    def underlyingArgument(using Context): Tree =
      val mapToUnderlying = new MapToUnderlying:
        /** Should get the rhs of this binding Returns true if the symbol is a
          * val or def generated by eta-expansion/inline
          */
  ...
  extension (trees: List[Tree])
    ...
    inline def flattenedMapConserve(inline f: Tree => Tree): List[Tree] =
      ...
      def loop(
        ...
        if pending.isEmpty then
          ...
          if head1 eq head0 then loop(mapped, unchanged, pending.tail)
          else
    ...
    end flattenedMapConserve

    /** Transform statements while maintaining import contexts and expression
      * contexts in the same way as Typer does. The code addresses additional
      * concerns:
    ...
    inline def mapStatements[T](
        ...
        inline wrapResult: List[Tree] => Context ?=> T
    )(using Context): T =
      @tailrec
      def loop(
          mapped: mutable.ListBuffer[Tree] | Null,
          unchanged: List[Tree],
          pending: List[Tree]
      )(using Context): T =
        pending match
          case stat :: rest =>
            ...
            val restCtx = stat match
              ...
              case _            => ctx
            if stat1 eq stat then loop(mapped, unchanged, rest)(using restCtx)
            else
              val buf =
                if mapped == null then new mutable.ListBuffer[Tree] else mapped
              var xc = unchanged
              while xc ne pending do
                buf += xc.head
                xc = xc.tail
  ...
  /** A treemap that generates the same contexts as the original typer for
    ...
    */
  class TreeMapWithPreciseStatContexts(cpy: TreeCopier = tpd.cpy)
      extends TreeMap(cpy):
    def transformStats[T](
        trees: List[Tree],
        exprOwner: Symbol,
        wrapResult: List[Tree] => Context ?=> T
    )(using Context): T =
      trees.mapStatements(exprOwner, transform(_), wrapResult)
    final override def transformStats(trees: List[Tree], exprOwner: Symbol)(
        using Context
    ): List[Tree] =
  ...
  /** Map Inlined nodes, NamedArgs, Blocks with no statements and local
    * references to underlying arguments. Also drops Inline and Block with no
    * statements.
    */
  private class MapToUnderlying extends TreeMap:
    ...
    /** Is this a symbol that of a local val or parameterless def for which we
      * could get the rhs
      */
    private def isBinding(sym: Symbol)(using Context): Boolean =
      sym.isTerm && !sym.is(Param) && !sym.owner.isClass &&
        !(sym.is(Method) && sym.info
          .isInstanceOf[MethodOrPoly]) // if is a method it is parameterless
  ...
  /** A trait for loaders that compute trees. Currently implemented just by
    * DottyUnpickler.
    */
  trait TreeProvider:
    ...
    def rootTrees(using Context): List[Tree] =
      if ctx.settings.YretainTrees.value then
        if myTrees == null then myTrees = computeRootTrees
        myTrees.uncheckedNN
    ...
    /** Is it possible that the tree to load contains a definition of or
      * reference to `id`?
      */
  ...
  def sameTypes(trees: List[tpd.Tree], trees1: List[tpd.Tree]): Boolean =
    if trees.isEmpty then trees.isEmpty
    else if trees1.isEmpty then trees.isEmpty
    else
      (trees.head.tpe eq trees1.head.tpe) && sameTypes(trees.tail, trees1.tail)

  /** If `tree`'s purity level is less than `level`, let-bind it so that it gets
    * evaluated only once. I.e. produce a
    *
    * { val x = 'tree ; ~within('x) }
    *
    ...
    * ~within('tree)
    */
  def letBindUnless(level: TreeInfo.PurityLevel, tree: Tree)(
      within: Tree => Tree
  )(using Context): Tree =
  ...
  def runtimeCall(name: TermName, args: List[Tree])(using Context): Tree =
    Ident(defn.ScalaRuntimeModule.requiredMethod(name).termRef)
      .appliedToTermArgs(args)
  ...
  /** Split argument clauses into a leading type argument clause if it exists
  ...
  /** A key to be used in a context property that tracks the number of inlined
    * trees
    */
  ...
  /** Record an enclosing inlined call. EmptyTree calls (for parameters) cancel
    * the next-enclosing call in the list instead of being added to it. We
    * assume parameters are never nested inside parameters.
    */
  ...
  /** All enclosing calls that are currently inlined, from innermost to
    * outermost.
    */
  ...
  /** The source file where the symbol of the `inline` method referred to by
    * `call` is defined
    */
  def sourceFile(call: Tree)(using Context): SourceFile = call.symbol.source

  /** Desugar identifier into a select node. Return the tree itself if not
    * possible
    */
  def desugarIdent(tree: Ident)(using Context): RefTree =
    val qual = desugarIdentPrefix(tree)
    if qual.isEmpty then tree
  ...
  def desugarIdentPrefix(tree: Ident)(using Context): Tree = tree.tpe match
    case TermRef(prefix: TermRef, _) =>
      prefix.info match
        case mt: MethodType
            if mt.paramInfos.isEmpty && mt.resultType.typeSymbol.is(Module) =>
  ...
  /** All the symbols that are imported by the first selector of `imp` that
  ...
  def importedSymbols(
      imp: Import,
      selectorPredicate: untpd.ImportSelector => Boolean =
        util.common.alwaysTrue
  )(using Context): List[Symbol] =
    imp.selectors.find(selectorPredicate) match
      case Some(sel) => importedSymbols(imp.expr, sel.name)
      case _         => Nil

  /** The list of select trees that resolve to the same symbols as the ones that
    * are imported by `imp`.
    */
    ...
    def imported(
      ...
      rename match
        ...
        case Some(rename) =>
          ...
          val name =
            if sym.name.isTypeName then rename.name.toTypeName else rename.name
          val actual = Select(noPosExpr, sym.name)
          val renameTree =
            Select(noPosExpr, name).withSpan(rename.span).withType(actual.tpe)
  ...
  def tupleTypeTree(elems: List[Tree])(using Context): Tree =
    ...
    if arity <= Definitions.MaxTupleArity then
      ...
      if tupleTp != null then AppliedTypeTree(TypeTree(tupleTp), elems)
      else nestedPairsTypeTree(elems)
    else nestedPairsTypeTree(elems)

  /** Creates the nested pairs type tree representation of the type trees in
    * `ts`
    */
  def nestedPairsTypeTree(ts: List[Tree])(using Context): Tree =
    ts.foldRight[Tree](TypeTree(defn.EmptyTupleModule.termRef))((x, acc) =>
      AppliedTypeTree(TypeTree(defn.PairClass.typeRef), x :: acc :: Nil)
    )

  /** Creates the nested higher-kinded pairs type tree representation of the
    * type trees in `ts`
    */
  def hkNestedPairsTypeTree(ts: List[Tree])(using Context): Tree =
    ts.foldRight[Tree](TypeTree(defn.QuoteMatching_KNil.typeRef))((x, acc) =>
      AppliedTypeTree(
        TypeTree(defn.QuoteMatching_KCons.typeRef),
        x :: acc :: Nil
      )
    )
  ...
  /** Convert a list of trees to a vararg-compatible tree. Used to make
    * arguments for methods that accept varargs.
    */
  def repeated(trees: List[Tree], tpt: Tree)(using Context): Tree =
    ctx.typeAssigner.arrayToRepeated(JavaSeqLiteral(trees, tpt))

  /** Create a tree representing a list containing all the elements of the
    * argument list. A "list of tree to tree of list" conversion.
    *
    * @param trees
