...
trait TreeInfo[T <: Untyped]:
  ...
  class MatchingArgs(params: List[Symbol], args: List[Tree])(using Context):
    def foreach(f: (Symbol, Tree) => Unit): Boolean =
      def recur(params: List[Symbol], args: List[Tree]): Boolean = params match
        ...
        case param :: params1 =>
          if param.info.isRepeatedParam then
            for arg <- args do f(param, arg)
            true
          else
            args match
              case Nil => false
  ...
  /** The method part of an application node, possibly enclosed in a block with
    * only valdefs as statements. the reason for also considering blocks is that
    * named arguments can transform a call into a block, e.g. <init>(b = foo, a
    * \= bar) is transformed to { val x$1 = foo val x$2 = bar <init>(x$2, x$1) }
    */
  ...
  /** If this is an application, its function part, stripping all Apply nodes
    * (but leaving TypeApply nodes in). Otherwise the tree itself.
    */
  ...
  def hasExplicitTypeArgs(tree: Tree): Boolean = tree match
    case TypeApply(tycon, args) =>
      args.exists(arg =>
        !arg.span.isZeroExtent && !tycon.span.contains(arg.span)
      )
    case _ => false
  ...
  /** Is tree a self constructor call this(...)? I.e. a call to a constructor of
    * the same object?
    */
  ...
  def isSelfOrSuperConstrCall(tree: Tree): Boolean = methPart(tree) match
    case Ident(nme.CONSTRUCTOR) | Select(This(_), nme.CONSTRUCTOR) |
        Select(Super(_, _), nme.CONSTRUCTOR) =>
      true
    case _ => false
  ...
  def isRepeatedParamType(tpt: Tree)(using Context): Boolean = stripByNameType(
    tpt
  ) match
  ...
  /** Is this argument node of the form <expr> *, or is it a reference to such
    * an argument ? The latter case can happen when an argument is lifted.
    */
  ...
  /** Is tree a type tree of the form `=> T` or (under pureFunctions) `{refs}->
    * T`?
    */
  ...
  val isNamedArg: Any => Boolean = (arg: Any) =>
    arg.isInstanceOf[Trees.NamedArg[?]]
  ...
  def isUsingOrTypeParamClause(params: ParamClause)(using Context): Boolean =
    params match
      case TypeDefs(_) => true
      case _           => isUsingClause(params)

  def isTypeParamClause(params: ParamClause)(using Context): Boolean =
    params match
      case TypeDefs(_) => true
      case _           => false
  ...
  /** If `path` looks like a language import, `Some(name)` where name is
    * `experimental` if that sub-module is imported, and the empty term name
    * otherwise.
    */
  ...
  /** The largest subset of {NoInits, PureInterface} that a trait or class with
    * these parents can have as flags.
    */
  ...
  /** Checks whether predicate `p` is true for all result parts of this
    * expression, where we zoom into Ifs, Matches, and Blocks.
    */
  def forallResults(tree: Tree, p: Tree => Boolean): Boolean = tree match
    case If(_, thenp, elsep) =>
      forallResults(thenp, p) && forallResults(elsep, p)
  ...
  /** Is tree an application with result `this.type`? Accept `b.addOne(x)` and
    * also `xs(i) += x` where the op is an assignment operator.
    */
  def isThisTypeResult(tree: Tree)(using Context): Boolean = appliedCore(
    tree
  ) match
    case fun @ Select(receiver, op) =>
      ...
      tree.tpe match
        ...
        case tref: TermRef =>
          tref.symbol == receiver.symbol || argss.exists(
            _.exists(tref.symbol == _.symbol)
          )
        case _ =>
...
trait UntypedTreeInfo extends TreeInfo[Untyped]:
  ...
  def isContextualClosure(tree: Tree)(using Context): Boolean = unsplice(
    tree
  ) match
    ...
    case Block(DefDef(nme.ANON_FUN, params :: _, _, _) :: Nil, cl: Closure) =>
      if params.isEmpty then
        cl.tpt.eq(untpd.ContextualEmptyTree) || defn.isContextFunctionType(
          cl.tpt.typeOpt
        )
      else isUsingClause(params)
    case _ => false

  /** The largest subset of {NoInits, PureInterface} that a trait or class
    * enclosing this statement can have as flags.
    */
  private def defKind(tree: Tree)(using Context): FlagSet = unsplice(tree) match
    case EmptyTree | _: Import => NoInitsInterface
    case tree: TypeDef => if tree.isClassDef then NoInits else NoInitsInterface
    case tree: DefDef =>
    ...
    case tree: ValDef =>
      if tree.unforcedRhs == EmptyTree then NoInitsInterface else EmptyFlags
    case _ => EmptyFlags

  /** The largest subset of {NoInits, PureInterface} that a trait or class with
    * this body can have as flags.
    */
  ...
  /** Under pureFunctions: A builder and extractor for `=> T`, which is an alias
    * for `->{cap} T`. Only trees of the form `=> T` are matched; trees written
    * directly as `->{cap} T` are ignored by the extractor.
    */
  object ImpureByNameTypeTree:
    ...
    def apply(tp: Tree)(using Context): untpd.ByNameTypeTree =
      untpd.ByNameTypeTree(
        untpd.CapturesAndResult(
          untpd.captureRoot.withSpan(tp.span.startPos) :: Nil,
          tp
        )
      )
    ...
    def unapply(tp: Tree)(using Context): Option[Tree] = tp match
      case untpd.ByNameTypeTree(
            untpd.CapturesAndResult(
              id @ Select(_, nme.CAPTURE_ROOT) :: Nil,
              result
            )
          ) if id.span == result.span.startPos =>
        Some(result)
      case _ => None
...
trait TypedTreeInfo extends TreeInfo[Type]:
  self: Trees.Instance[Type] =>
  import TreeInfo.*
  import tpd.*

  /** The purity level of this statement.
    * @return
    *   Pure if statement has no side effects Idempotent if running the
    *   statement a second time has no side effects Impure otherwise
    */
  ...
  /** The purity level of this expression. See docs for PurityLevel for what
    * that means
    *
    * Note that purity and idempotency are treated differently. References to
    * modules and lazy vals are impure (side-effecting) both because
    * side-effecting code may be executed and because the first reference takes
    * a different code path than all to follow; but they are idempotent because
    * running the expression a second time gives the cached result.
    */
  def exprPurity(tree: Tree)(using Context): PurityLevel = unsplice(tree) match
    ...
    case Ident(_) =>
      refPurity(tree)
    case Select(qual, _) =>
      if tree.symbol.is(Erased) then Pure
    ...
    case TypeApply(fn, _) =>
      if fn.symbol.is(
          Erased
        ) || fn.symbol == defn.QuotedTypeModule_of || fn.symbol == defn.Predef_classOf
      then Pure
      else exprPurity(fn)
    case Apply(fn, args) =>
      if isPureApply(tree, fn) then
        minOf(exprPurity(fn), args.map(exprPurity)) `min` Pure
      else if fn.symbol.is(Erased) then Pure
  ...
  private def minOf(l0: PurityLevel, ls: List[PurityLevel]) =
    ls.foldLeft(l0)(_ `min` _)
  ...
  def isPureApply(tree: Tree, fn: Tree)(using Context): Boolean =
    ...
    tree.tpe
      .isInstanceOf[ConstantType] && tree.symbol != NoSymbol && isKnownPureOp(
      tree.symbol
    ) // A constant expression with pure arguments is pure.
    || fn.symbol.isStableMember && !fn.symbol.is(
      Lazy
    ) // constructors of no-inits classes are stable

  /** The purity level of this reference.
    * @return
    *   PurePath if reference is (nonlazy and stable) or to a parameterized
    *   function or its type is a constant type IdempotentPath if reference is
    *   lazy and stable Impure otherwise
    * @DarkDimius:
    *   need to make sure that lazy accessor methods have Lazy and Stable flags
    *   set.
    */
  def refPurity(tree: Tree)(using Context): PurityLevel =
    val sym = tree.symbol
    if !tree.hasType then Impure
    ...
    else if !sym.isStableMember then Impure
    else if sym.is(Module) then
      if sym.moduleClass.isNoInitsRealClass then PurePath else IdempotentPath
    else if sym.is(Lazy) then IdempotentPath
  ...
  /** (1) If `tree` is a constant expression, its value as a Literal, or `tree`
    * itself otherwise.
    *
    * Note: Demanding idempotency instead of purity in literalize is strictly
    * speaking too loose. Example
    *
    ...
    * kind of eliding behavior. Or else enforce true purity in the compiler. The
    * choice will be affected by what we will do with `inline` and with
    * Singleton type bounds (see SIP 23). Presumably
    *
    * object O1 { val x: Singleton = 42; println("43") } object O2 { inline val
    * x = 42; println("43") }
    *
    ...
    * O2.x = 42
    *
    * Revisit this issue once we have standardized on `inline`. Then we can
    * demand purity of the prefix unless the selection goes to a inline val.
    *
    * Note: This method should be applied to all term tree nodes that are not
    * literals, that can be idempotent, and that can have constant types. So
    * far, only nodes of the following classes qualify:
    *
    * Ident Select TypeApply
    *
    * (2) A primitive unary operator expression `pre.op` where `op` is one of
    * `+`, `-`, `~`, `!` that has a constant type `ConstantType(v)` but that is
    * not a constant expression (i.e. `pre` has side-effects) is translated to
    *
    * { pre; v }
    *
    * (3) An expression `pre.getClass[..]()` that has a constant type
    * `ConstantType(v)` but where `pre` has side-effects is translated to:
    *
    * { pre; v }
    *
    * This avoids the situation where we have a Select node that does not have a
    * symbol.
    */
  ...
  def isExtMethodApply(tree: Tree)(using Context): Boolean = methPart(
    tree
  ) match
    case Inlined(call, _, _) => isExtMethodApply(call)
    case tree @ Select(qual, nme.apply) =>
      tree.symbol.is(ExtensionMethod) || isExtMethodApply(qual)
  ...
  /** Is tree a reference to a mutable variable, or to a potential getter that
    * has a setter in the same class?
    */
  def isVariableOrGetter(tree: Tree)(using Context): Boolean =
    def sym = tree.symbol
    def isVar = sym.is(Mutable)
    def isGetter =
      mayBeVarGetter(sym) && sym.owner.info
        .member(sym.name.asTermName.setterName)
        .exists
  ...
  def isSelf(tree: Tree, enclClass: Symbol)(using Context): Boolean = unsplice(
    tree
  ) match
    case This(_) => tree.symbol == enclClass
    case _       => false

  /** Strips layers of `.asInstanceOf[T]` / `_.$asInstanceOf[T]()` from an
    * expression
    */
  ...
  /** The type and term arguments of a possibly curried call, in the order they
    * are given
    */
  ...
  /** The function part of a possibly curried call. Unlike `methPart` this one
    * does not decompose blocks
    */
  ...
  def decomposeTemplateBody(
      body: List[Tree]
  )(using Context): (List[Tree], List[Tree]) =
    body.partition {
  ...
  def patVars(tree: Tree)(using Context): List[Symbol] =
    val acc = new TreeAccumulator[List[Symbol]]:
      def apply(syms: List[Symbol], tree: Tree)(using Context) = tree match
        ...
        case Annotated(tree, id @ Ident(tpnme.BOUNDTYPE_ANNOT)) =>
          apply(id.symbol :: syms, tree)
  ...
  def definedSym(tree: Tree)(using Context): Symbol =
    if tree.isDef then tree.symbol else NoSymbol

  /** Going from child to parent, the path of tree nodes that starts with a
    * definition of symbol `sym` and ends with `root`, or Nil if no such path
    * exists. Pre: `sym` must have a position.
    */
  def defPath(sym: Symbol, root: Tree)(using Context): List[Tree] =
    trace.onDebug(s"defpath($sym with position ${sym.span}, ${root.show})") {
  ...
  /** The top level classes in this tree, including only those module classes
    * that are not a linked class of some other class in the result.
    */
  ...
  /** The tree containing only the top-level classes and objects matching either
    * `cls` or its companion object
    */
  def sliceTopLevel(tree: Tree, cls: ClassSymbol)(using Context): List[Tree] =
    tree match
      ...
      case tdef: TypeDef =>
        val sym = tdef.symbol
        assert(sym.isClass)
        if cls == sym || cls == sym.linkedClass then tdef :: Nil
        else Nil
      case vdef: ValDef =>
        val sym = vdef.symbol
        assert(sym.is(Module))
        if cls == sym.companionClass || cls == sym.moduleClass then vdef :: Nil
        else Nil
      case tree =>
        tree :: Nil

  /** The statement sequence that contains a definition of `sym`, or Nil if none
    * was found. For a tree to be found, The symbol must have a position and its
    * definition tree must be reachable from come tree stored in an enclosing
    * context.
    */
  def definingStats(sym: Symbol)(using Context): List[Tree] =
    if !sym.span.exists || (ctx eq NoContext) || (ctx.compilationUnit eq NoCompilationUnit)
    then Nil
    else
      defPath(sym, ctx.compilationUnit.tpdTree) match
        case defn :: encl :: _ =>
          def verify(stats: List[Tree]) =
            if stats exists (definedSym(_) == sym) then stats else Nil
          encl match
  ...
  /** If `tree` is an instance of `TupleN[...](e1, ..., eN)`, the arguments `e1,
    * ..., eN` otherwise the empty list.
    */
  def tupleArgs(tree: Tree)(using Context): List[Tree] = tree match
    ...
    case Apply(fn: NameTree, args)
        if fn.name == nme.apply &&
          ...
          defn.isTupleClass(fn.symbol.owner.companionClass) =>
      args
    case _ => Nil
  ...
  def isStructuralTermSelectOrApply(tree: Tree)(using Context): Boolean =
    ...
    def loop(tree: Tree): Boolean = tree match
      ...
      case _ =>
        false
    loop(tree)
  end isStructuralTermSelectOrApply
  ...
  /** Return a pair consisting of (supercall, rest)
    ...
    */
  final def splitAtSuper(constrStats: List[Tree])(implicit
      ctx: Context
  ): (List[Tree], List[Tree]) =
    constrStats.toList match
      case (sc: Apply) :: rest if sc.symbol.isConstructor => (sc :: Nil, rest)
      case (block @ Block(_, sc: Apply)) :: rest if sc.symbol.isConstructor =>
        (block :: Nil, rest)
      case stats => (Nil, stats)

  /** Structural tree comparison (since == on trees is reference equality). For
    * the moment, only Ident, Select, Literal, Apply and TypeApply are supported
    */
  extension (t1: Tree)
    ...
    def hash(using Context): Int =
      t1.getClass.hashCode * 37 + {
        t1 match
          ...
          case Apply(f1, as1) =>
            as1.foldLeft(f1.hash)((h, arg) => h * 41 + arg.hash)
          case TypeApply(f1, ts1) =>
            ts1.foldLeft(f1.hash)((h, arg) => h * 41 + arg.tpe.hash)
          case _ => t1.hashCode
      }
  ...
  object QuotedTypeOf:
    /** Extracts the content of a quoted tree. The result can be the contents of
      * a term or type quote, which will return a term or type tree
      * respectively.
      */
  ...
  object SplicedType:
    /** Extracts the content of a spliced type tree. The result can be the
      * contents of a type splice, which will return a type tree.
      */
  ...
  object AssertNotNull:
    ...
    def unapply(tree: tpd.TypeApply)(using Context): Option[tpd.Tree] =
      tree match
...
object TreeInfo:
  ...
  /** An expression is a stable path. Requires that expression is at least
    * idempotent
    */
  ...
  /** Running the expression a second time has no side effects. Implied by
    * `Pure`.
    */
