...
object NavigateAST:
  ...
  def toUntyped(tree: tpd.Tree)(using Context): untpd.Tree =
    untypedPath(tree, exactMatch = true) match
      ...
      case _ =>
        ...
        throw new Error(i"""no untyped tree for $tree, pos = ${tree.sourcePos}
                   |best matching path =\n$loosePath%\n====\n%
                   |path positions = ${loosePath.map(_.sourcePos)}""")

  /** The reverse path of untyped trees starting with a tree that closest
    * matches `tree` and ending in the untyped tree at the root of the
    * compilation unit specified by `ctx`.
    * @param exactMatch
    *   If `true`, the path must start with a node that exactly matches `tree`,
    *   or `Nil` is returned. If `false` the path might start with a node
    *   enclosing the logical position of `tree`. Note: A complication concerns
    *   member definitions. ValDefs and DefDefs have after desugaring a position
    *   that spans just the name of the symbol being defined and nothing else.
    *   So we look instead for an untyped tree approximating the envelope of the
    *   definition, and declare success if we find another DefTree.
    */
  def untypedPath(tree: tpd.Tree, exactMatch: Boolean = false)(using
      Context
  ): List[Positioned] =
    tree match
      case tree: MemberDef[?] =>
      ...
      case _ =>
        untypedPath(tree.span) match
          case (path @ last :: _) if last.span == tree.span || !exactMatch =>
            path
          case _ => Nil
  ...
  /** The reverse path from any node in `from` to the node that closest encloses
    * `span`, or `Nil` if no such path exists. If a non-empty path is returned
    * it starts with the node closest enclosing `span` and ends with one of the
    * nodes in `from`.
    *
    * @param skipZeroExtent
    *   If true, skip over zero-extent nodes in the search. These nodes do not
    *   correspond to code the user wrote since their start and end point are
    *   the same, so this is useful when trying to reconcile nodes with source
    *   code.
    */
  def pathTo(
      span: Span,
      from: List[Positioned],
      skipZeroExtent: Boolean = false
  )(using Context): List[Positioned] =
    ...
    def singlePath(p: Positioned, path: List[Positioned]): List[Positioned] =
      if p.span.exists && !(skipZeroExtent && p.span.isZeroExtent) && p.span
          .contains(span)
      then
