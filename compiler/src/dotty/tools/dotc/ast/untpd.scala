...
object untpd extends Trees.Instance[Untyped] with UntypedTreeInfo:
  ...
  /** A typed subtree of an untyped tree needs to be wrapped in a TypedSplice
    ...
    * @param isExtensionReceiver
    *   The splice was created from the receiver `e` in an extension method call
    *   `e.f(...)`
    */
  abstract case class TypedSplice(splice: tpd.Tree)(
      val owner: Symbol,
    ...
    override def toString =
      def ext =
        if isExtensionReceiver then ", isExtensionReceiver = true" else ""
      s"TypedSplice($splice$ext)"

  object TypedSplice:
    def apply(tree: tpd.Tree, isExtensionReceiver: Boolean = false)(using
        Context
    ): TypedSplice =
      new TypedSplice(tree)(ctx.owner, isExtensionReceiver) {}

  /** mods object name impl */
  case class ModuleDef(name: TermName, impl: Template)(implicit
      @constructorOnly src: SourceFile
  ) extends MemberDef:
    type ThisTree[+T <: Untyped] <: Trees.NameTree[T] with Trees.MemberDef[
      T
    ] with ModuleDef
    def withName(name: Name)(using Context): ModuleDef =
      cpy.ModuleDef(this)(name.toTermName, impl)

  /** An untyped template with a derives clause. Derived parents are added to
    * the end of the `parents` list. `derivedCount` keeps track of how many
    * there are. This representation was chosen because it balances two
    * concerns:
    *   - maximize overlap between DerivingTemplate and Template for code
    *     streamlining
    *   - keep invariant that elements of untyped trees align with source
    *     positions
    */
  class DerivingTemplate(
      constr: DefDef,
      parentsOrDerived: List[Tree],
      self: ValDef,
      preBody: LazyTreeList,
      derivedCount: Int
  )(implicit @constructorOnly src: SourceFile)
  ...
  case class ParsedTry(expr: Tree, handler: Tree, finalizer: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends TermTree

  case class SymbolLit(str: String)(implicit @constructorOnly src: SourceFile)
      extends TermTree

  /** An interpolated string
    * @param segments
    *   a list of two element tickets consisting of string literal and argument
    *   tree, possibly with a simple string literal as last element of the list
    */
  case class InterpolatedString(id: TermName, segments: List[Tree])(implicit
      @constructorOnly src: SourceFile
  ) extends TermTree

  /** A function type or closure */
  case class Function(args: List[Tree], body: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends Tree:
    override def isTerm: Boolean = body.isTerm
    override def isType: Boolean = body.isType

  /** A function type or closure with `implicit` or `given` modifiers and
    * information on which parameters are `erased`
    */
  class FunctionWithMods(
      args: List[Tree],
      body: Tree,
      val mods: Modifiers,
      val erasedParams: List[Boolean]
  )(implicit @constructorOnly src: SourceFile)
  ...
  case class PolyFunction(targs: List[Tree], body: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends Tree:
  ...
  /** A function created from a wildcard expression
    ...
    * @param body
    *   the function body where wildcards are replaced by references to
    *   synthetic parameters. This is equivalent to Function, except that forms
    *   a special case for the overlapping positions tests.
    */
  class WildcardFunction(placeholderParams: List[ValDef], body: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends Function(placeholderParams, body)

  case class InfixOp(left: Tree, op: Ident, right: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends OpTree
  case class PostfixOp(od: Tree, op: Ident)(implicit
      @constructorOnly src: SourceFile
  ) extends OpTree
  case class PrefixOp(op: Ident, od: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends OpTree
  case class Parens(t: Tree)(implicit @constructorOnly src: SourceFile)
      extends ProxyTree:
    def forwardTo: Tree = t
  case class Tuple(trees: List[Tree])(implicit @constructorOnly src: SourceFile)
      extends Tree:
    override def isTerm: Boolean = trees.isEmpty || trees.head.isTerm
    override def isType: Boolean = !isTerm
  case class Throw(expr: Tree)(implicit @constructorOnly src: SourceFile)
      extends TermTree
  case class ForYield(enums: List[Tree], expr: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends TermTree
  case class ForDo(enums: List[Tree], body: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends TermTree
  case class GenFrom(pat: Tree, expr: Tree, checkMode: GenCheckMode)(implicit
      @constructorOnly src: SourceFile
  ) extends Tree
  case class GenAlias(pat: Tree, expr: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends Tree
  case class ContextBounds(bounds: TypeBoundsTree, cxBounds: List[Tree])(
      implicit @constructorOnly src: SourceFile
  ) extends TypTree
  case class PatDef(mods: Modifiers, pats: List[Tree], tpt: Tree, rhs: Tree)(
      implicit @constructorOnly src: SourceFile
  ) extends DefTree
  case class ExtMethods(paramss: List[ParamClause], methods: List[Tree])(
      implicit @constructorOnly src: SourceFile
  ) extends Tree
  case class Into(tpt: Tree)(implicit @constructorOnly src: SourceFile)
      extends Tree
  case class MacroTree(expr: Tree)(implicit @constructorOnly src: SourceFile)
      extends Tree

  case class ImportSelector(
      imported: Ident,
      renamed: Tree = EmptyTree,
  ...
  case class Number(digits: String, kind: NumberKind)(implicit
      @constructorOnly src: SourceFile
  ) extends TermTree
  ...
  case class CapturesAndResult(refs: List[Tree], parent: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends TypTree

  /** Short-lived usage in typer, does not need copy/transform/fold
    * infrastructure
    */
  case class DependentTypeTree(tp: List[Symbol] => Type)(implicit
      @constructorOnly src: SourceFile
  ) extends Tree

  @sharable object EmptyTypeIdent
      extends Ident(tpnme.EMPTY)(NoSource)
      with WithoutTypeOrPos[Untyped]:
    override def isEmpty: Boolean = true

  def WildcardTypeBoundsTree()(using src: SourceFile): TypeBoundsTree =
    TypeBoundsTree(EmptyTree, EmptyTree, EmptyTree)
  ...
  /** A block generated by the XML parser, only treated specially by
    * `Positioned#checkPos`
    */
  class XMLBlock(stats: List[Tree], expr: Tree)(implicit
      @constructorOnly src: SourceFile
  ) extends Block(stats, expr)
  ...
  // ----- Modifiers -----------------------------------------------------
  /** Mod is intended to record syntactic information about modifiers, it's NOT
    * a replacement of FlagSet.
    *
    * For any query about semantic information, check `flags` instead.
    */
  sealed abstract class Mod(val flags: FlagSet)(implicit
      @constructorOnly src: SourceFile
  ) extends Positioned

  object Mod:
    case class Private()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Private)

    case class Protected()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Protected)

    case class Var()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Mutable)

    case class Implicit()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Implicit)

    case class Given()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Given)

    case class Erased()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Erased)

    case class Final()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Final)

    case class Sealed()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Sealed)

    case class Opaque()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Opaque)

    case class Open()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Open)

    case class Override()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Override)

    case class Abstract()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Abstract)

    case class Lazy()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Lazy)

    case class Inline()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Inline)

    case class Transparent()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Transparent)

    case class Infix()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Infix)

    /** Used under pureFunctions to mark impure function types `A => B` in
      * `FunctionWithMods`
      */
    case class Impure()(implicit @constructorOnly src: SourceFile)
        extends Mod(Flags.Impure)
  end Mod
  ...
  /** Modifiers and annotations for definitions
    ...
    *   The set flags
    * @param privateWithin
    *   If a private or protected has is followed by a qualifier [q], the name
    *   q, "" as a typename otherwise.
    * @param annotations
  ...
  case class Modifiers(
    ...
    def withFlags(flags: FlagSet): Modifiers =
      if this.flags == flags then this
      else copy(flags = flags)

    def withoutFlags(flags: FlagSet): Modifiers =
      if this.isOneOf(flags) then
        Modifiers(
          this.flags &~ flags,
          this.privateWithin,
          this.annotations,
          this.mods.filterNot(_.flags.isOneOf(flags))
        )
      else this

    def withAddedMod(mod: Mod): Modifiers =
      if mods.exists(_ eq mod) then this
    ...
    /** Add `flags` to thos modifier set, checking that there are no type/term
      * conflicts. If there are conflicts, issue an error and return the
      * modifiers consisting of the added flags only. The reason to do it this
      * way is that the added flags usually describe the core of a construct
    ...
    def withAddedFlags(flags: FlagSet, span: Span)(using Context): Modifiers =
      if this.flags.isAllOf(flags) then this
        ...
        report.error(
          em"${(flags & ModifierFlags).flagsString} $what cannot be ${this.flags.flagsString}",
          ctx.source.atSpan(span)
        )
        Modifiers(flags)

    /** Modifiers with given list of Mods. It is checked that all modifiers are
      * already accounted for in `flags` and `privateWithin`.
      */
    def withMods(ms: List[Mod]): Modifiers =
      if mods eq ms then this
        ...
        if ms.nonEmpty then
          for m <- ms do
            assert(
              flags.isAllOf(m.flags)
                || m.isInstanceOf[Mod.Private] && !privateWithin.isEmpty
                || (m.isInstanceOf[Mod.Abstract] || m
                  .isInstanceOf[Mod.Override]) && flags.is(AbsOverride),
    ...
    def withAnnotations(annots: List[Tree]): Modifiers =
      if annots eq annotations then this
      else copy(annotations = annots)

    def withPrivateWithin(pw: TypeName): Modifiers =
      if pw.isEmpty then this
  ...
  /** A type tree that gets its type from some other tree's symbol. Enters the
    * type tree in the References attachment of the `from` tree as a side
    * effect.
    */
  abstract class DerivedTypeTree(implicit @constructorOnly src: SourceFile)
      extends TypeTree:
  ...
  /** Property key containing TypeTrees whose type is computed from the symbol
    * in this type. These type trees have marker trees TypeRefOfSym or InfoOfSym
    * as their originals.
    */
  val References: Property.Key[List[DerivedTypeTree]] = Property.Key()

  /** Property key for TypeTrees marked with TypeRefOfSym or InfoOfSym which
    * contains the symbol of the original tree from which this TypeTree is
    * derived.
    */
  ...
  def SearchFailureIdent(name: Name, explanation: => String)(implicit
      src: SourceFile
  ): SearchFailureIdent = new SearchFailureIdent(name, explanation)
  def Select(qualifier: Tree, name: Name)(implicit src: SourceFile): Select =
    new Select(qualifier, name)
  def SelectWithSig(qualifier: Tree, name: Name, sig: Signature)(implicit
      src: SourceFile
  ): Select = new SelectWithSig(qualifier, name, sig)
  ...
  def Apply(fun: Tree, args: List[Tree])(implicit src: SourceFile): Apply =
    new Apply(fun, args)
  def TypeApply(fun: Tree, args: List[Tree])(implicit
      src: SourceFile
  ): TypeApply = new TypeApply(fun, args)
  def Literal(const: Constant)(implicit src: SourceFile): Literal = new Literal(
    const
  )
  ...
  def InlineIf(cond: Tree, thenp: Tree, elsep: Tree)(implicit
      src: SourceFile
  ): If = new InlineIf(cond, thenp, elsep)
  def Closure(env: List[Tree], meth: Tree, tpt: Tree)(implicit
      src: SourceFile
  ): Closure = new Closure(env, meth, tpt)
  def Match(selector: Tree, cases: List[CaseDef])(implicit
      src: SourceFile
  ): Match = new Match(selector, cases)
  def InlineMatch(selector: Tree, cases: List[CaseDef])(implicit
      src: SourceFile
  ): Match = new InlineMatch(selector, cases)
  def CaseDef(pat: Tree, guard: Tree, body: Tree)(implicit
      src: SourceFile
  ): CaseDef = new CaseDef(pat, guard, body)
  def Labeled(bind: Bind, expr: Tree)(implicit src: SourceFile): Labeled =
    new Labeled(bind, expr)
  ...
  def WhileDo(cond: Tree, body: Tree)(implicit src: SourceFile): WhileDo =
    new WhileDo(cond, body)
  def Try(expr: Tree, cases: List[CaseDef], finalizer: Tree)(implicit
      src: SourceFile
  ): Try = new Try(expr, cases, finalizer)
  def SeqLiteral(elems: List[Tree], elemtpt: Tree)(implicit
      src: SourceFile
  ): SeqLiteral = new SeqLiteral(elems, elemtpt)
  def JavaSeqLiteral(elems: List[Tree], elemtpt: Tree)(implicit
      src: SourceFile
  ): JavaSeqLiteral = new JavaSeqLiteral(elems, elemtpt)
  def Inlined(call: tpd.Tree, bindings: List[MemberDef], expansion: Tree)(
      implicit src: SourceFile
  ): Inlined = new Inlined(call, bindings, expansion)
  def Quote(body: Tree, tags: List[Tree])(implicit src: SourceFile): Quote =
    new Quote(body, tags)
  def Splice(expr: Tree)(implicit src: SourceFile): Splice = new Splice(expr)
  def SplicePattern(body: Tree, args: List[Tree])(implicit
      src: SourceFile
  ): SplicePattern = new SplicePattern(body, args)
  def TypeTree()(implicit src: SourceFile): TypeTree = new TypeTree()
  def InferredTypeTree()(implicit src: SourceFile): TypeTree =
    new InferredTypeTree()
  def SingletonTypeTree(ref: Tree)(implicit
      src: SourceFile
  ): SingletonTypeTree = new SingletonTypeTree(ref)
  def RefinedTypeTree(tpt: Tree, refinements: List[Tree])(implicit
      src: SourceFile
  ): RefinedTypeTree = new RefinedTypeTree(tpt, refinements)
  def AppliedTypeTree(tpt: Tree, args: List[Tree])(implicit
      src: SourceFile
  ): AppliedTypeTree = new AppliedTypeTree(tpt, args)
  def LambdaTypeTree(tparams: List[TypeDef], body: Tree)(implicit
      src: SourceFile
  ): LambdaTypeTree = new LambdaTypeTree(tparams, body)
  def TermLambdaTypeTree(params: List[ValDef], body: Tree)(implicit
      src: SourceFile
  ): TermLambdaTypeTree = new TermLambdaTypeTree(params, body)
  def MatchTypeTree(bound: Tree, selector: Tree, cases: List[CaseDef])(implicit
      src: SourceFile
  ): MatchTypeTree = new MatchTypeTree(bound, selector, cases)
  def ByNameTypeTree(result: Tree)(implicit src: SourceFile): ByNameTypeTree =
    new ByNameTypeTree(result)
  def TypeBoundsTree(lo: Tree, hi: Tree, alias: Tree = EmptyTree)(implicit
      src: SourceFile
  ): TypeBoundsTree = new TypeBoundsTree(lo, hi, alias)
  ...
  def UnApply(fun: Tree, implicits: List[Tree], patterns: List[Tree])(implicit
      src: SourceFile
  ): UnApply = new UnApply(fun, implicits, patterns)
  def ValDef(name: TermName, tpt: Tree, rhs: LazyTree)(implicit
      src: SourceFile
  ): ValDef = new ValDef(name, tpt, rhs)
  def DefDef(
      name: TermName,
      paramss: List[ParamClause],
      tpt: Tree,
      rhs: LazyTree
  )(implicit src: SourceFile): DefDef = new DefDef(name, paramss, tpt, rhs)
  def TypeDef(name: TypeName, rhs: Tree)(implicit src: SourceFile): TypeDef =
    new TypeDef(name, rhs)
  def Template(
      constr: DefDef,
      parents: List[Tree],
      derived: List[Tree],
      self: ValDef,
      body: LazyTreeList
  )(implicit src: SourceFile): Template =
    if derived.isEmpty then new Template(constr, parents, self, body)
    else
      new DerivingTemplate(
        constr,
        parents ++ derived,
        self,
        body,
  ...
  def Template(
      constr: DefDef,
      parents: LazyTreeList,
      self: ValDef,
      body: LazyTreeList
  )(implicit src: SourceFile): Template =
    new Template(constr, parents, self, body)
  def Import(expr: Tree, selectors: List[ImportSelector])(implicit
      src: SourceFile
  ): Import = new Import(expr, selectors)
  def Export(expr: Tree, selectors: List[ImportSelector])(implicit
      src: SourceFile
  ): Export = new Export(expr, selectors)
  def PackageDef(pid: RefTree, stats: List[Tree])(implicit
      src: SourceFile
  ): PackageDef = new PackageDef(pid, stats)
  def Annotated(arg: Tree, annot: Tree)(implicit src: SourceFile): Annotated =
    new Annotated(arg, annot)
  def Hole(isTerm: Boolean, idx: Int, args: List[Tree], content: Tree)(implicit
      src: SourceFile
  ): Hole = new Hole(isTerm, idx, args, content)
  ...
  /** new T(args1)...(args_n) \==> new T.<init>[Ts](args1)...(args_n)
    ...
    * to allow navigation into these arguments from the IDE, and to do the right
    * thing in PrepareInlineable.
    */
  ...
  def AppliedTypeTree(tpt: Tree, arg: Tree)(implicit
      src: SourceFile
  ): AppliedTypeTree =
  ...
  def rawRef(tp: NamedType)(using Context): Tree =
    if tp.typeParams.isEmpty then ref(tp)
      ...
      AppliedTypeTree(ref(tp), tp.typeParams.map(_ => WildcardTypeBoundsTree()))
  ...
  def scalaDot(name: Name)(implicit src: SourceFile): Select =
    Select(rootDot(nme.scala), name)
  def scalaAnnotationDot(name: Name)(using SourceFile): Select =
    Select(scalaDot(nme.annotation), name)
  ...
  def javaDotLangDot(name: Name)(implicit src: SourceFile): Select =
    Select(Select(Ident(nme.java), nme.lang), name)

  def captureRoot(using Context): Select =
    Select(scalaDot(nme.caps), nme.CAPTURE_ROOT)

  def makeRetaining(parent: Tree, refs: List[Tree], annotName: TypeName)(using
      Context
  ): Annotated =
  ...
  def makeParameter(
      ...
      tpe: Tree,
      mods: Modifiers,
      isBackquoted: Boolean = false
  )(using Context): ValDef =
    val vdef = ValDef(pname, tpe, EmptyTree)
    if isBackquoted then vdef.pushAttachment(Backquoted, ())
    vdef.withMods(mods | Param)

  def makeSyntheticParameter(
      n: Int = 1,
      tpt: Tree | Null = null,
      flags: FlagSet = SyntheticTermParam
  )(using Context): ValDef =
    ValDef(
      nme.syntheticParamName(n),
      if tpt == null then TypeTree() else tpt,
      EmptyTree
    )
      .withFlags(flags)

  def lambdaAbstract(params: List[ValDef] | List[TypeDef], tpt: Tree)(using
      Context
  ): Tree =
    params match
      case Nil => tpt
      case (vd: ValDef) :: _ =>
        TermLambdaTypeTree(params.asInstanceOf[List[ValDef]], tpt)
      case _ => LambdaTypeTree(params.asInstanceOf[List[TypeDef]], tpt)

  def lambdaAbstractAll(paramss: List[List[ValDef] | List[TypeDef]], tpt: Tree)(
      using Context
  ): Tree =
    paramss.foldRight(tpt)(lambdaAbstract)

  /** A reference to given definition. If definition is a repeated parameter,
    * the reference will be a repeated argument.
    */
  def refOfDef(tree: MemberDef)(using Context): Tree = tree match
    case ValDef(_, PostfixOp(_, Ident(tpnme.raw.STAR)), _) =>
      repeated(Ident(tree.name))
    case _ => Ident(tree.name)

  /** A repeated argument such as `arg: _*` */
  def repeated(arg: Tree)(using Context): Typed =
    Typed(arg, Ident(tpnme.WILDCARD_STAR))
...
// --------- Copier/Transformer/Accumulator classes for untyped trees -----
  ...
  class UntypedTreeCopier extends TreeCopier:
    ...
    def ModuleDef(tree: Tree)(name: TermName, impl: Template)(using
        Context
    ...
    def ParsedTry(tree: Tree)(expr: Tree, handler: Tree, finalizer: Tree)(using
        Context
    ): TermTree = tree match
      case tree: ParsedTry
          if (expr eq tree.expr) && (handler eq tree.handler) && (finalizer eq tree.finalizer) =>
        tree
      case _ =>
        finalize(tree, untpd.ParsedTry(expr, handler, finalizer)(tree.source))
    ...
    def InterpolatedString(tree: Tree)(id: TermName, segments: List[Tree])(using
        Context
    ): TermTree = tree match
      case tree: InterpolatedString
          if (id eq tree.id) && (segments eq tree.segments) =>
        tree
      case _ =>
        finalize(tree, untpd.InterpolatedString(id, segments)(tree.source))
    def Function(tree: Tree)(args: List[Tree], body: Tree)(using
        Context
    ): Tree = tree match
      case tree: Function if (args eq tree.args) && (body eq tree.body) => tree
      case _ => finalize(tree, untpd.Function(args, body)(tree.source))
    def PolyFunction(tree: Tree)(targs: List[Tree], body: Tree)(using
        Context
    ): Tree = tree match
      case tree: PolyFunction if (targs eq tree.targs) && (body eq tree.body) =>
        tree
      case _ => finalize(tree, untpd.PolyFunction(targs, body)(tree.source))
    def InfixOp(tree: Tree)(left: Tree, op: Ident, right: Tree)(using
        Context
    ): Tree = tree match
      case tree: InfixOp
          if (left eq tree.left) && (op eq tree.op) && (right eq tree.right) =>
        tree
    ...
    def ForYield(tree: Tree)(enums: List[Tree], expr: Tree)(using
        Context
    ): TermTree = tree match
      case tree: ForYield if (enums eq tree.enums) && (expr eq tree.expr) =>
        tree
      case _ => finalize(tree, untpd.ForYield(enums, expr)(tree.source))
    def ForDo(tree: Tree)(enums: List[Tree], body: Tree)(using
        Context
    ): TermTree = tree match
      case tree: ForDo if (enums eq tree.enums) && (body eq tree.body) => tree
      case _ => finalize(tree, untpd.ForDo(enums, body)(tree.source))
    def GenFrom(tree: Tree)(pat: Tree, expr: Tree, checkMode: GenCheckMode)(
        using Context
    ): Tree = tree match
      case tree: GenFrom
          if (pat eq tree.pat) && (expr eq tree.expr) && (checkMode == tree.checkMode) =>
        tree
    ...
    def ContextBounds(tree: Tree)(bounds: TypeBoundsTree, cxBounds: List[Tree])(
        using Context
    ): TypTree = tree match
      case tree: ContextBounds
          if (bounds eq tree.bounds) && (cxBounds eq tree.cxBounds) =>
        tree
      case _ =>
        finalize(tree, untpd.ContextBounds(bounds, cxBounds)(tree.source))
    def PatDef(
        tree: Tree
    )(mods: Modifiers, pats: List[Tree], tpt: Tree, rhs: Tree)(using
        Context
    ): Tree = tree match
      case tree: PatDef
          if (mods eq tree.mods) && (pats eq tree.pats) && (tpt eq tree.tpt) && (rhs eq tree.rhs) =>
        tree
      case _ => finalize(tree, untpd.PatDef(mods, pats, tpt, rhs)(tree.source))
    def ExtMethods(tree: Tree)(paramss: List[ParamClause], methods: List[Tree])(
        using Context
    ): Tree = tree match
      case tree: ExtMethods
          if (paramss eq tree.paramss) && (methods == tree.methods) =>
        tree
    ...
    def ImportSelector(tree: Tree)(imported: Ident, renamed: Tree, bound: Tree)(
        using Context
    ): Tree = tree match
      case tree: ImportSelector
          if (imported eq tree.imported) && (renamed eq tree.renamed) && (bound eq tree.bound) =>
        tree
      case _ =>
        finalize(
          tree,
          untpd.ImportSelector(imported, renamed, bound)(tree.source)
        )
    def Number(tree: Tree)(digits: String, kind: NumberKind)(using
        Context
    ): Tree = tree match
      case tree: Number if (digits == tree.digits) && (kind == tree.kind) =>
        tree
      case _ => finalize(tree, untpd.Number(digits, kind))
    def CapturesAndResult(tree: Tree)(refs: List[Tree], parent: Tree)(using
        Context
    ): Tree = tree match
      case tree: CapturesAndResult
          if (refs eq tree.refs) && (parent eq tree.parent) =>
        tree
      case _ => finalize(tree, untpd.CapturesAndResult(refs, parent))

    def TypedSplice(tree: Tree)(splice: tpd.Tree)(using Context): ProxyTree =
      tree match
  ...
  end UntypedTreeCopier

  abstract class UntypedTreeMap(cpy: UntypedTreeCopier = untpd.cpy)
      extends TreeMap(cpy):
      ...
      tree match
        ...
        case tree: DerivingTemplate =>
          cpy.Template(tree)(
            ...
            transform(tree.derived),
            transformSub(tree.self),
        ...
        case ImportSelector(imported, renamed, bound) =>
          cpy.ImportSelector(tree)(
            transformSub(imported),
            transform(renamed),
            transform(bound)
          )
  ...
  abstract class UntypedTreeAccumulator[X] extends TreeAccumulator[X]:
    ...
    override def foldMoreCases(x: X, tree: Tree)(using Context): X = tree match
      ...
      case tree: DerivingTemplate =>
        this(
          this(
            this(this(this(x, tree.constr), tree.parents), tree.derived),
            tree.self
          ),
          tree.body
        )
  ...
  class UntypedDeepFolder[X](f: (X, Tree) => X)
      extends UntypedTreeAccumulator[X]:
