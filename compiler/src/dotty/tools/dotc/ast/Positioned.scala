...
/** A base class for things that have positions (currently: modifiers and trees)
  */
abstract class Positioned(implicit @constructorOnly src: SourceFile)
    extends SrcPos,
      Product,
      Cloneable:
  ...
  /** A unique identifier in case -Yshow-tree-ids, or -Ydebug-tree-with-id is
    * set, -1 otherwise.
    */
  def uniqueId: Int =
  ...
  /** This positioned item, widened to `SrcPos`. Used to make clear we only need
    * the position, typically for error reporting.
    */
  final def srcPos: SrcPos = this

  /** A positioned item like this one with given `span`. If the positioned item
    * is source-derived, a clone is returned. If the positioned item is
    * synthetic, the position is updated destructively and the item itself is
    * returned.
    */
  ...
  /** The union of startSpan and the spans of all positioned children that have
    ...
    * their `call` child.
    *
    * Side effect: Any descendants without spans have but with the same source
    * as this node have their span set to the end position of the envelope of
    * all children to the left, or, if that one does not exist, to the start
    * position of the envelope of all children to the right.
    */
    ...
    (this: @unchecked) match
      ...
      case _ =>
        def include(span: Span, x: Any): Span = x match
          case p: Positioned =>
            if p.source != src then span
            else if p.span.exists then span.union(p.span)
            else if span.exists then
              if span.end != MaxOffset then
        ...
        def includeChildren(span: Span, n: Int): Span =
          if n < limit then
            includeChildren(include(span, productElement(n): @unchecked), n + 1)
          else span
        val span1 = includeChildren(startSpan, 0)
        val span2 =
          if !span1.exists || span1.end != MaxOffset then span1
          else if span1.start == MaxOffset then
  ...
  /** Clone this node but assign it a fresh id which marks it as a node in
    * `file`.
    */
  ...
  /** Check that all positioned items in this tree satisfy the following
    * conditions:
  ...
  def checkPos(nonOverlapping: Boolean)(using Context): Unit = try
        ...
        if nonOverlapping then
          this match
            ...
            case _: WildcardFunction
                if last.positioned.isInstanceOf[ValDef] && !p
                  .isInstanceOf[ValDef] =>
            // ignore transition from last wildcard parameter to body
            case _ =>
              assert(
                !last.span.exists || !p.span.exists || last.span.end <= p.span.start,
    ...
    this match
      case tree: DefDef
          if tree.name == nme.CONSTRUCTOR && tree.mods.is(JavaDefined) =>
      ...
      case tree: DefDef if tree.mods.is(ExtensionMethod) =>
        tree.paramss match
          case vparams1 :: vparams2 :: rest
              if tree.name.isRightAssocOperatorName =>
...
object Positioned:
  ...
  def init(using Context): Unit =
    ...
    then ids = java.util.WeakHashMap()
