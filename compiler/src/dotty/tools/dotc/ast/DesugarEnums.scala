...
object DesugarEnums:
  ...
  final case class EnumConstraints(
      minKind: CaseKind,
      maxKind: CaseKind,
      enumCases: List[(Int, RefTree)]
  ):
    require(
      minKind.ordinal <= maxKind.ordinal && !(cached && enumCases.isEmpty)
    )
  ...
  /** Attachment containing the number of enum cases, the smallest kind that was
    * seen so far, and a list of all the value cases with their ordinals.
    */
  val EnumCaseCount
      : Property.Key[(Int, CaseKind, CaseKind, List[(Int, TermName)])] =
    Property.Key()

  /** Attachment signalling that when this definition is desugared, it should
    * add any additional lookup methods for enums.
    */
  ...
  /** The enumeration class that belongs to an enum case. This works no matter
    * whether the case is still in the enum class or it has been transferred to
    * the companion object.
    */
  def enumClass(using Context): Symbol =
    val cls = ctx.owner
    if cls.is(Module) then cls.linkedClass else cls

  def enumCompanion(using Context): Symbol =
    val cls = ctx.owner
    if cls.is(Module) then cls.sourceModule else cls.linkedClass.sourceModule
  ...
  /** A reference to the enum class `E`, possibly followed by type arguments.
    * Each covariant type parameter is approximated by its lower bound. Each
    * contravariant type parameter is approximated by its upper bound. It is an
    * error if a type parameter is non-variant, or if its approximation refers
    * to pther type parameters.
    */
  def interpolatedEnumParent(span: Span)(using Context): Tree =
    ...
    val targs = tparams map { tparam =>
      if tparam.is(Covariant) && isGround(tparam.info.bounds.lo) then
        ...
        def problem =
          if !tparam.isOneOf(VarianceFlags) then "is invariant"
            ...
            "has bounds that depend on a type parameter in the same parameter list"
        errorType(
          em"""cannot determine type argument for enum parent $enumClass,
  ...
  def enumClassRef(using Context): Tree =
    if enumClass.exists then TypeTree(enumClass.typeRef) else TypeTree()

  /** Add implied flags to an enum class or an enum case */
  def addEnumFlags(cdef: TypeDef)(using Context): TypeDef =
    if cdef.mods.isEnumClass then
      cdef.withMods(cdef.mods.withAddedFlags(Abstract | Sealed, cdef.span))
    else if isEnumCase(cdef) then
      cdef.withMods(cdef.mods.withAddedFlags(Final, cdef.span))
    else cdef
  ...
  private def ArrayLiteral(values: List[Tree], tpt: Tree)(using Context): Tree =
    ...
    val ctag = Apply(
      TypeApply(ref(defn.ClassTagModule_apply.termRef), tpt :: Nil),
      clazzOf :: Nil
    )
    val apply = Select(ref(defn.ArrayModule.termRef), nme.apply)
    Apply(Apply(TypeApply(apply, tpt :: Nil), values), ctag :: Nil)

  /** The following lists of definitions for an enum type E and known value
    * cases e_0, ..., e_n:
    *
    * private val $values =
    * Array[E](this.e_0,...,this.e_n)(ClassTag[E](classOf[E])) def values =
    * $values.clone def valueOf($name: String) = $name match { case "e_0" =>
    * this.e_0 ... case "e_n" => this.e_n case _ => throw new
    * IllegalArgumentException("case not found: " + $name) }
    */
  private def enumScaffolding(enumValues: List[RefTree])(using
      Context
  ): List[Tree] =
    val rawEnumClassRef = rawRef(enumClass.typeRef)
    extension (tpe: NamedType)
      def ofRawEnum = AppliedTypeTree(ref(tpe), rawEnumClassRef)
    ...
    val valuesOfBody: Tree =
      val defaultCase =
        val msg = Apply(
          Select(Literal(Constant("enum case not found: ")), nme.PLUS),
        ...
        CaseDef(
          ...
          EmptyTree,
          Throw(
            New(TypeTree(defn.IllegalArgumentExceptionType), List(msg :: Nil))
          )
        )
      val stringCases = enumValues.map(enumValue =>
        CaseDef(
          Literal(Constant(enumValue.name.toString)),
          EmptyTree,
          enumValue
        )
      ) ::: defaultCase :: Nil
      Match(Ident(nme.nameDollar), stringCases)
    val valueOfDef = DefDef(
      nme.valueOf,
      List(param(nme.nameDollar, defn.StringType) :: Nil),
      TypeTree(),
      valuesOfBody
    )
      .withFlags(Synthetic)
  ...
  private def enumLookupMethods(constraints: EnumConstraints)(using
      Context
  ): List[Tree] =
    ...
    def valueCtor: List[Tree] =
      if constraints.requiresCreator then enumValueCreator :: Nil else Nil
    def fromOrdinal: Tree =
      def throwArg(ordinal: Tree) =
        Throw(
          New(
            TypeTree(defn.NoSuchElementExceptionType),
            List(Select(ordinal, nme.toString_) :: Nil)
          )
        )
      if !constraints.cached then fromOrdinalMeth(throwArg)
      else
        ...
        if constraints.isEnumeration then
          fromOrdinalMeth(ordinal =>
            Try(
              Apply(valuesDot(nme.apply), ordinal),
              default(ordinal) :: Nil,
              EmptyTree
            )
          )
          ...
          fromOrdinalMeth(ordinal =>
            Match(
              ordinal,
              constraints.enumCases.map((i, enumValue) =>
  ...
  /** A creation method for a value of enum type `E`, which is defined as
    * follows:
    *
    * private def $new(_$ordinal: Int, $name: String) = new E with
    * scala.runtime.EnumValue { def ordinal = _$ordinal // if `E` does not
    * derive from `java.lang.Enum` }
    */
  private def enumValueCreator(using Context) =
    ...
    DefDef(
      ...
      List(
        List(
          param(nme.ordinalDollar_, defn.IntType),
          param(nme.nameDollar, defn.StringType)
        )
      ),
      TypeTree(),
      creator
  ...
  /** Is a type parameter in `enumTypeParams` referenced from an enum class case
    * that has given type parameters `caseTypeParams`, value parameters
    * `vparamss` and parents `parents`? Issues an error if that is the case but
    * the reference is illegal. The reference could be illegal for two reasons:
      ...
      def apply(x: Boolean, tree: Tree)(using Context): Boolean = x || {
        tree match
          case Ident(name) =>
            ...
            if matches && (caseTypeParams.nonEmpty || vparamss.isEmpty) then
              report.error(
                em"illegal reference to type parameter $name from enum case",
                tree.srcPos
              )
            matches
  ...
  end typeParamIsReferenced

  /** A pair consisting of
    *   - the next enum tag
    *   - scaffolding containing the necessary definitions for singleton enum
    *     cases unless that scaffolding was already generated by a previous call
    *     to `nextEnumKind`.
    */
  def nextOrdinal(name: Name, kind: CaseKind, definesLookups: Boolean)(using
      Context
  ): (Int, List[Tree]) =
    val (ordinal, seenMinKind, seenMaxKind, seenCases) =
      ctx.tree
        .removeAttachment(EnumCaseCount)
        .getOrElse((0, CaseKind.Class, CaseKind.Simple, Nil))
    val minKind =
      if kind.ordinal < seenMinKind.ordinal then kind else seenMinKind
    val maxKind =
      if kind.ordinal > seenMaxKind.ordinal then kind else seenMaxKind
    ...
    if definesLookups then
      ...
      val cachedValues =
        cases.reverse.map((i, name) => (i, Select(thisRef, name)))
      (
        ordinal,
        enumLookupMethods(EnumConstraints(minKind, maxKind, cachedValues))
      )
    else
      ctx.tree.pushAttachment(
        EnumCaseCount,
        (ordinal + 1, minKind, maxKind, cases)
      )
      (ordinal, Nil)
  end nextOrdinal

  def param(name: TermName, typ: Type)(using Context): ValDef =
    param(name, TypeTree(typ))
  def param(name: TermName, tpt: Tree)(using Context): ValDef =
    ValDef(name, tpt, EmptyTree).withFlags(Param)

  def ordinalMeth(body: Tree)(using Context): DefDef =
    DefDef(nme.ordinal, Nil, TypeTree(defn.IntType), body)
      .withAddedFlags(Synthetic)
  ...
  def fromOrdinalMeth(body: Tree => Tree)(using Context): DefDef =
    DefDef(
      nme.fromOrdinal,
      (param(nme.ordinal, defn.IntType) :: Nil) :: Nil,
      rawRef(enumClass.typeRef),
      body(Ident(nme.ordinal))
    ).withFlags(Synthetic)

  /** Expand a module definition representing a parameterless enum case */
  def expandEnumModule(
      name: TermName,
      impl: Template,
      mods: Modifiers,
      definesLookups: Boolean,
      span: Span
  )(using Context): Tree =
    ...
    if !enumClass.exists then EmptyTree
      ...
      val impl1 = cpy
        ...
        .withAttachment(ExtendsSingletonMirror, ())
      val vdef = ValDef(name, TypeTree(), New(impl1)).withMods(
  ...
  def expandSimpleEnumCase(
    ...
    if !enumClass.exists then EmptyTree
    else if enumClass.typeParams.nonEmpty then
      ...
      val creator = Apply(
        Ident(nme.DOLLAR_NEW),
        List(Literal(Constant(tag)), Literal(Constant(name.toString)))
      )
      val vdef = ValDef(name, enumClassRef, creator).withMods(
        mods.withAddedFlags(EnumValue, span)
      )
      flatTree(vdef :: scaffolding).withSpan(span)
    }
