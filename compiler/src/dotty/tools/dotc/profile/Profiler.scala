      ...
      val reporter =
        if ctx.settings.YprofileDestination.value != "" then
          new StreamProfileReporter(
            new PrintWriter(
              new FileWriter(ctx.settings.YprofileDestination.value, true)
            )
          )
...
private[profile] object RealProfiler:
  ...
  val gcMx: List[GarbageCollectorMXBean] =
    ManagementFactory.getGarbageCollectorMXBeans.asScala.toList
  val classLoaderMx: ClassLoadingMXBean =
  ...
  if threadMx.isThreadCpuTimeSupported then
    threadMx.setThreadCpuTimeEnabled(true)
  private val idGen = new AtomicInteger()

private[profile] class RealProfiler(reporter: ProfileReporter)(using Context)
    extends Profiler
  ...
  override def afterPhase(phase: Phase, snapBefore: ProfileSnap): Unit =
    ...
    if ctx.settings.YprofileExternalTool.value.contains(phase.toString) then
    ...
    reporter.reportForeground(
      this,
      ProfileRange(snapBefore, finalSnap, phase, "", 0, Thread.currentThread)
    )
...
object ConsoleProfileReporter extends ProfileReporter:
  ...
  private def reportCommon(
    ...
    println(
      s"${threadRange.phase.phaseName.replace(',', ' ')},run ns = ${threadRange.runNs},idle ns = ${threadRange.idleNs},cpu ns = ${threadRange.cpuNs},user ns = ${threadRange.userNs},allocated = ${threadRange.allocatedBytes},heap at end = ${threadRange.end.heapBytes}, total allocated = $totalAlloc "
    )
...
class StreamProfileReporter(out: PrintWriter) extends ProfileReporter:
  override def header(profiler: RealProfiler): Unit =
    ...
    out.println(
      s"header(main/background),startNs,endNs,runId,phaseId,phaseName,purpose,task-count,threadId,threadName,runNs,idleNs,cpuTimeNs,userTimeNs,allocatedByte,heapSize"
    )
    out.println(
      s"header(GC),startNs,endNs,startMs,endMs,name,action,cause,threads"
    )
  ...
  private def reportCommon(
    ...
    out.println(
      s"$tpe,${threadRange.start.snapTimeNanos},${threadRange.end.snapTimeNanos},${profiler.id},${threadRange.phase.id},${threadRange.phase.phaseName
          .replace(',', ' ')},${threadRange.purpose},${threadRange.taskCount},${threadRange.thread.getId},${threadRange.thread.getName},${threadRange.runNs},${threadRange.idleNs},${threadRange.cpuNs},${threadRange.userNs},${threadRange.allocatedBytes},${threadRange.end.heapBytes} "
    )

  override def reportGc(data: GcEventData): Unit =
    val duration =
      TimeUnit.MILLISECONDS.toNanos(data.gcEndMillis - data.gcStartMillis + 1)
    val start = data.reportTimeNs - duration
    out.println(
      s"${EventType.GC},$start,${data.reportTimeNs},${data.gcStartMillis}, ${data.gcEndMillis},${data.name},${data.action},${data.cause},${data.threads}"
    )

  override def close(profiler: RealProfiler): Unit =
    out.flush
    out.close
end StreamProfileReporter
