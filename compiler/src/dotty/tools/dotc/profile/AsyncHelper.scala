...
object AsyncHelper:
  ...
  private abstract class BaseAsyncHelper(phase: Phase)(using Context)
      extends AsyncHelper:
    ...
    protected class CommonThreadFactory(
      ...
      override def newThread(r: Runnable): Thread =
        ...
        if t.isDaemon != daemon then t.setDaemon(daemon)
        if t.getPriority != priority then t.setPriority(priority)
        t

  private final class BasicAsyncHelper(phase: Phase)(using Context)
      extends BaseAsyncHelper(phase):
    ...
    override protected def wrapRunnable(
        r: Runnable,
        shortId: String
    ): Runnable = r
  end BasicAsyncHelper
  ...
  private class ProfilingAsyncHelper(
    ...
    override def newUnboundedQueueFixedThreadPool(
      ...
      val threadFactory = new CommonThreadFactory(shortId, priority = priority)
      // like Executors.newFixedThreadPool
      new SinglePhaseInstrumentedThreadPoolExecutor(
        nThreads,
        nThreads,
        0L,
        TimeUnit.MILLISECONDS,
    ...
    private class SinglePhaseInstrumentedThreadPoolExecutor(
      ...
      override def beforeExecute(t: Thread, r: Runnable): Unit =
        ...
        if data.firstStartNs == 0 then data.firstStartNs = now
    ...
    end SinglePhaseInstrumentedThreadPoolExecutor
  end ProfilingAsyncHelper
end AsyncHelper
