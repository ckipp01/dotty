...
class ReplCompiler extends Compiler:
    ...
    val run = new Run(this, initCtx):
      ...
      override protected def rootContext(using Context): Context =
        ...
        def importPreviousRun(id: Int)(using Context) =
          ...
          val ctx0 = ctx.fresh.setNewScope
          ...
          if imports.isEmpty then ctx0
            ...
            imports.foldLeft(ctx0.fresh.setNewScope)((ctx, imp) =>
              importContext(imp)(using ctx)
            )

        val rootCtx = super.rootContext.fresh.withRootImports.fresh
          .setOwner(defn.EmptyPackageClass): Context
  ...
  private def packaged(stats: List[untpd.Tree])(using
      Context
  ): untpd.PackageDef =
    import untpd.*
    PackageDef(Ident(nme.EMPTY_PACKAGE), stats)

  final def compile(parsed: Parsed)(using
      state: State
  ): Result[(CompilationUnit, State)] =
  ...
  def docOf(expr: String)(using state: State): Result[String] =
    inContext(state.context) {
      ...
      /** Extract the "selected" symbol from `tree`.
        *
        * Because the REPL typechecks an expression, special syntax is needed to
        * get the documentation of certain symbols:
        *
        *   - To select the documentation of classes, the user needs to pass a
        *     call to the class' constructor (e.g. `new Foo` to select `class
        *     Foo`)
        *   - When methods are overloaded, the user needs to enter a lambda to
        *     specify which functions he wants (e.g. `foo(_: Int)` to select
        *     `def foo(x: Int)` instead of `def foo(x: String)`
        *
        * This function returns the right symbol for the received expression,
        * and all the symbols that are overridden.
        */
  ...
  final def typeCheck(expr: String, errorsAllowed: Boolean = false)(using
      state: State
  ): Result[tpd.ValDef] =

    def wrapped(expr: String, sourceFile: SourceFile, state: State)(using
        Context
    ): Result[untpd.PackageDef] =
      def wrap(trees: List[untpd.Tree]): untpd.PackageDef =
        ...
        val valdef = ValDef(
          "expr".toTermName,
          TypeTree(),
          Block(trees, unitLiteral).withSpan(Span(0, expr.length))
        )
        val tmpl =
          Template(emptyConstructor, Nil, Nil, EmptyValDef, List(valdef))
      ...
      ParseResult(sourceFile) match
        ...
        case SyntaxErrors(_, reported, trees) =>
          if errorsAllowed then wrap(trees).result
    ...
    def unwrapped(tree: tpd.Tree, sourceFile: SourceFile)(using
        Context
    ): Result[tpd.ValDef] =
      def error: Result[tpd.ValDef] =
        List(
          new Diagnostic.Error(
            s"Invalid scala expression",
            sourceFile.atSpan(Span(0, sourceFile.content.length))
          )
        ).errors
      ...
      tree match
        case PackageDef(_, List(TypeDef(_, tmpl: Template))) =>
          tmpl.body
            .collectFirst {
              case dd: ValDef if dd.name.show == "expr" => dd.result
            }
            .getOrElse(error)
        case _ =>
          error
    ...
    inContext(
      ...
      wrapped(expr, src, state).flatMap { pkg =>
        ...
        if errorsAllowed || !ctx.reporter.hasErrors then
...
/** A phase that assembles wrapped parse trees from user input.
  ...
  *
  * This enables -Vprint:repl so that users can see how their code snippet was
  * wrapped.
  */
class ReplPhase extends Phase:
  ...
  def run(using Context): Unit =
    ctx.compilationUnit.untpdTree match
      case pkg @ PackageDef(_, stats) =>
        pkg.getAttachment(ReplCompiler.ReplState).foreach { case given State =>
  ...
  private def definitions(
    ...
    /** If the user inputs a definition whose name is of the form
      * REPL_RES_PREFIX and a number, such as `val res9 = 1`, we bump `valIdx`
      * to avoid name clashes. lampepfl/dotty#3536
      */
    def maybeBumpValIdx(tree: Tree): Unit = tree match
      ...
      case tree: NameTree =>
        tree.name.show.stripPrefix(str.REPL_RES_PREFIX).toIntOption match
    ...
    Definitions(
      defs.toList,
      state.copy(objectIndex = state.objectIndex + 1, valIndex = valIdx)
    )
  end definitions
  ...
  /** Wrap trees in an object and add imports from the previous compilations.
    ...
    * ```
    */
  private def wrapped(defs: Definitions, span: Span)(using
      Context
  ): untpd.PackageDef =
    import untpd.*
