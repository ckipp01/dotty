...
/** The state of the REPL contains necessary bindings instead of having to have
  * mutation
  *
  * The compiler in the REPL needs to do some wrapping in order to compile valid
  * code. This wrapping occurs when a single `MemberDef` that cannot be
...
class ReplDriver(
      ...
      case Some((files, ictx)) =>
        inContext(ictx) {
          ...
          if files.nonEmpty then
            out.println(i"Ignoring spurious arguments: $files%, %")
    ...
    if rootCtx.settings.outputDir.isDefault(using rootCtx) then
      rootCtx = rootCtx.fresh
        .setSetting(
          ...
          new VirtualDirectory("<REPL compilation output>")
        )
  ...
  /** Try to run REPL if there is nothing that prevents us doing so.
    *
    * Possible reason for unsuccessful run are raised flags in CLI like --help
    * or --version
    */
  ...
  /** Controls whether the `System.out` and `System.err` streams are set to the
    * provided constructor parameter instance of [[java.io.PrintStream]] during
    * the execution of the repl. On by default.
    *
    * Disabling this can be beneficial when executing a repl instance inside a
    * concurrent environment, for example a thread pool (such as the Scala
    * compile server in the Scala Plugin for IntelliJ IDEA).
    *
    * In such environments, indepently executing `System.setOut` and
    * `System.setErr` without any synchronization can lead to unpredictable
    * results when restoring the original streams (dependent on the order of
    * execution), leaving the Java process in an inconsistent state.
    */
  ...
  protected final def completions(
      cursor: Int,
      expr: String,
      state0: State
  ): List[Candidate] =
    ...
    if expr.startsWith(":") then
      ...
      compiler
        ...
        .map { tree =>
          val file =
            SourceFile.virtual("<completions>", expr, maybeIncomplete = true)
          val unit = CompilationUnit(file)(using state.context)
          unit.tpdTree = tree
          given Context = state.context.fresh.setCompilationUnit(unit)
          val srcPos = SourcePosition(file, Span(cursor))
          val completions =
            try Completion.completions(srcPos)._2
  ...
  protected def interpret(res: ParseResult, quiet: Boolean = false)(using
      state: State
  ): State =
    res match
  ...
  private def compile(
    ...
    def extractNewestWrapper(tree: untpd.Tree): Name = tree match
      case PackageDef(_, (obj: untpd.ModuleDef) :: Nil) =>
        obj.name.moduleClassName
      case _ => nme.NO_NAME

    def extractTopLevelImports(ctx: Context): List[tpd.Import] =
      unfusedPhases(using ctx).collectFirst {
        case phase: CollectTopLevelImports => phase.imports
      }.get

    def contextWithNewImports(
        ctx: Context,
        ...
        { case (unit: CompilationUnit, newState: State) =>
          ...
          inContext(newState.context) {
            val (updatedState, definitions) =
              if !ctx.settings.XreplDisableDisplay.value then
                renderDefinitions(unit.tpdTree, newestWrapper)(using
                  newStateWithImports
                )
              else (newStateWithImports, Seq.empty)
            ...
            given Ordering[Diagnostic] =
              Ordering[(Int, Int, Int)]
                .on(d => (d.pos.line, -d.level, d.pos.column))

            if !quiet then
              (definitions ++ warnings).sorted
  ...
  private def renderDefinitions(tree: tpd.Tree, newestWrapper: Name)(using
      state: State
  ): (State, Seq[Diagnostic]) =
    given Context = state.context
    ...
    def resAndUnit(denot: Denotation) =
      ...
      name.startsWith(
        str.REPL_RES_PREFIX
      ) && hasValidNumber && sym.info == defn.UnitType
      ...
      if tree.symbol.info.exists then
        ...
        val defs =
          info.bounds.hi.finalResultType
            .membersBasedOnFlags(
              required = Method,
              excluded = Accessor | ParamAccessor | Synthetic | Private
            )
            .filterNot { denot =>
              defn.topClasses.contains(
                denot.symbol.owner
        ...
        val vals =
          info.fields
            .filterNot(
              _.symbol.isOneOf(
                ParamAccessor | Private | Synthetic | Artifact | Module
              )
            )
        ...
        if failedInit then
          ...
          (
            state.copy(invalidObjectIndexes =
              state.invalidObjectIndexes + state.objectIndex
            ),
            renderedVals
          )
        else
          val formattedMembers =
            typeAliases.map(rendering.renderTypeAlias)
              ...
              ++ renderedVals
          val diagnostics =
            if formattedMembers.isEmpty then rendering.forceModule(symbol)
      ...
      .collect {
        case x
            if !isSyntheticCompanion(
              x.symbol
            ) && !x.symbol.name.isReplWrapperName =>
      ...
      tree.symbol.info.memberClasses
        ...
        .map { wrapperModule =>
          ...
          val formattedTypeDefs = // don't render type defs if wrapper initialization failed
            if newState.invalidObjectIndexes.contains(state.objectIndex) then
              Seq.empty
            else typeDefs(wrapperModule.symbol)
          val highlighted = (formattedTypeDefs ++ formattedMembers)
            .map(d =>
              new Diagnostic(
                d.msg.mapMsg(SyntaxHighlighting.highlight),
                d.pos,
                d.level
              )
            )
  ...
  private def interpretCommand(cmd: Command)(using state: State): State =
    cmd match
      case UnknownCommand(cmd) =>
        out.println(
          s"""Unknown command: "$cmd", run ":help" for a list of commands"""
        )
        state

      case AmbiguousCommand(cmd, matching) =>
        out.println(
          s""""$cmd" matches ${matching.mkString(
              ", "
            )}. Try typing a few more characters. Run ":help" for a list of commands"""
        )
        state
      ...
      case Reset(arg) =>
        ...
        if tokens.nonEmpty then
          out.println(s"""|Resetting REPL state with the following settings:
                        |  ${tokens.mkString("\n  ")}
                        |""".stripMargin)
        else out.println("Resetting REPL state.")
      ...
      case Imports =>
        ...
        do out.println(imp.show(using state.context))
        state
      ...
      case TypeOf(expr) =>
        expr match
          ...
          case _ =>
            compiler
              .typeOf(expr)(using newRun(state))
              .fold(
                displayErrors,
      ...
      case Settings(arg) =>
        arg match
          case "" =>
            ...
            for s <- ctx.settings
                .userSetSettings(ctx.settingsState)
                .sortBy(_.name)
            do
              out.println(
                s"${s.name} = ${if s.value == "" then "\"\"" else s.value}"
              )
            state
          case _ =>
  ...
  /** Like ConsoleReporter, but without file paths, -Xprompt displaying, and
    * using a PrintStream rather than a PrintWriter so messages aren't
    * re-encoded.
    */
  private object ReplConsoleReporter
  ...
  /** Print warnings & errors using ReplConsoleReporter, and info straight to
    * out
    */
  private def printDiagnostic(dia: Diagnostic)(using state: State) =
    dia.level match
      case interfaces.Diagnostic.INFO =>
        out.println(
          dia.msg
        ) // print REPL's special info diagnostics directly to out
