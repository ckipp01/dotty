...
/** This rendering object uses `ClassLoader`s to accomplish crossing the 4th
  ...
  *
  * @pre
  *   this object should be paired with a compiler session, i.e. when
...
private[repl] class Rendering(parentClassLoader: Option[ClassLoader] = None):
  ...
  private[repl] def classLoader()(using Context) =
    ...
    then myClassLoader
      ...
      myClassLoader =
        new AbstractFileClassLoader(ctx.settings.outputDir.value, parent)
      myReplStringOf =
        ...
        val scalaRuntime =
          Class.forName("scala.runtime.ScalaRunTime", true, myClassLoader)
        val renderer = "stringOf"
        def stringOfMaybeTruncated(value: Object, maxElements: Int): String =
          try
            val meth = scalaRuntime.getMethod(
              renderer,
              classOf[Object],
              classOf[Int],
              classOf[Boolean]
            ...
            case _: NoSuchMethodException =>
              val meth =
                scalaRuntime.getMethod(renderer, classOf[Object], classOf[Int])
        ...
        (value: Object, maxElements: Int, maxCharacters: Int) =>
          ...
          val maybeTruncatedByElementCount =
            stringOfMaybeTruncated(value, maxElements)
          val maybeTruncated =
  ...
  private[repl] def replStringOf(value: Object)(using Context): String =
    ...
    val maxPrintElements =
      ctx.settings.VreplMaxPrintElements.valueIn(ctx.settingsState)
    val maxPrintCharacters =
      ctx.settings.VreplMaxPrintCharacters.valueIn(ctx.settingsState)
    val res = myReplStringOf(value, maxPrintElements, maxPrintCharacters)
    if res == null then "null // non-null reference has null-valued toString"
    else res
  ...
  private def valueOf(sym: Symbol)(using Context): Option[String] =
    ...
    val symValue = resObj.getDeclaredMethods
      .find(_.getName == sym.name.encode.toString)
      .flatMap(result =>
        rewrapValueClass(sym.info.classSymbol, result.invoke(null))
      )
    ...
    if !sym.is(Flags.Method) && sym.info == defn.UnitType then None
      ...
      valueString.map { s =>
        if s.startsWith(REPL_WRAPPER_NAME_PREFIX) then
          s.drop(REPL_WRAPPER_NAME_PREFIX.length)
            .dropWhile(c => c.isDigit || c == '$')
        else s
      }
  ...
  /** Rewrap value class to their Wrapper class
    ...
    *   underlying value
    */
  private def rewrapValueClass(sym: Symbol, value: Object)(using
      Context
  ): Option[Object] =
  ...
  def renderVal(d: Denotation)(using
      Context
  ...
  def renderError(thr: Throwable, d: Denotation)(using Context): Diagnostic =
    ...
    def isWrapperInitialization(ste: StackTraceElement) =
      ste.getClassName.startsWith(
        REPL_WRAPPER_NAME_PREFIX
      ) // d.symbol.owner.name.show is simple name
  ...
  private def infoDiagnostic(msg: String, d: Denotation)(using
      Context
  ): Diagnostic =
...
object Rendering:
  ...
  def rootCause(x: Throwable): Throwable = x match
    case _: ExceptionInInitializerError |
        ...
        _: java.util.concurrent.ExecutionException if x.getCause != null =>
