...
/** A JSR 223 (Scripting API) compatible wrapper around the REPL for improved
  ...
  *
  * It works by instantiating a new script engine through the script engine
  * manager. The script engine provides a eval method to evaluate scripts in
  * string form. Example use:
  *
  * val m = new javax.script.ScriptEngineManager() val e =
  * m.getEngineByName("scala") println(e.eval("42"))
  */
class ScriptEngine extends AbstractScriptEngine:
  ...
  def eval(script: String, context: ScriptContext): Object =
    ...
    Class
      .forName(
        s"${Rendering.REPL_WRAPPER_NAME_PREFIX}$oid",
        true,
  ...
  def eval(reader: Reader, context: ScriptContext): Object =
    eval(stringFromReader(reader), context)
...
object ScriptEngine:
  ...
  class Factory extends ScriptEngineFactory:
    ...
    def getMethodCallSyntax(obj: String, m: String, args: String*) =
