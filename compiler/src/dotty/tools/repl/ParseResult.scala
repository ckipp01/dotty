...
case class AmbiguousCommand(cmd: String, matchingCommands: List[String])
    extends Command

/** `:load <path>` interprets a scala file as if entered line-by-line into the
  * REPL
  */
...
/** A command that is used to display the documentation associated with the
  * given expression.
  */
...
object ParseResult:
    ...
    sourceCode match
      ...
      case CommandExtract(cmd, arg) =>
        val matchingCommands =
          commands.filter((command, _) => command.startsWith(cmd))
      ...
      case _ =>
        inContext(state.context) {
          ...
          val stats = parseStats(using
            state.context.fresh.setReporter(reporter).withSource(source)
          )

          if reporter.hasErrors then
            SyntaxErrors(sourceCode, reporter.removeBufferedMessages, stats)
          else Parsed(source, stats, reporter)
        }
  ...
  private def maybeIncomplete(sourceCode: String, maybeIncomplete: Boolean)(
      using state: State
  ): ParseResult =
    apply(
      SourceFile.virtual(
        str.REPL_SESSION_LINE + (state.objectIndex + 1),
        sourceCode,
