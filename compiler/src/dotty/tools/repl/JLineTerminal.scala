...
class JLineTerminal extends java.io.Closeable:
  ...
  def readLine(
    ...
    val lineReader = LineReaderBuilder
      ...
      .variable(
        HISTORY_FILE,
        s"$userHome/.dotty_history"
      ) // Save history to file
      .variable(
        SECONDARY_PROMPT_PATTERN,
        "%M"
      ) // A short word explaining what is "missing",
  ...
  private class Parser(using Context) extends reader.Parser:
    ...
    def parse(
      ...
      def currentToken: TokenData /* | Null */ =
        ...
        val scanner = new Scanner(source)(using
          ctx.fresh.setReporter(Reporter.NoReporter)
        )
        ...
        while scanner.token != EOF do
          ...
          if isCurrentToken then
            return TokenData(
              token,
              lastBacktickErrorStart.getOrElse(start),
              end
            )
          ...
          else lastBacktickErrorStart = None
        null
      ...
      context match
        ...
        case ParseContext.COMPLETE
            if ParseResult.commands
              .exists(command => command._1.startsWith(input)) =>
