...
object Main:
  /** All arguments before -script <target_script> are compiler arguments. All
    * arguments afterwards are script arguments.
    */
  private def distinguishArgs(
      args: Array[String]
  ): (Array[String], File, Array[String], Boolean, Boolean) =
    val (leftArgs, rest) = args.splitAt(args.indexOf("-script"))
    assert(
      rest.size >= 2,
      s"internal error: rest == Array(${rest.mkString(",")})"
    )
  ...
  def process(args: Array[String]): Option[Throwable] =
    val (compilerArgs, scriptFile, scriptArgs, saveJar, invokeFlag) =
      distinguishArgs(args)
    val driver = ScriptingDriver(compilerArgs, scriptFile, scriptArgs)
    driver.compileAndRun {
      (outDir: Path, classpathEntries: Seq[Path], mainClass: String) =>
  ...
  private def writeJarfile(
    ...
    val jarTargetDir: Path = Option(
      scriptFile.toPath.toAbsolutePath.getParent
    ) match
      case None =>
        sys.error(s"no parent directory for script file [$scriptFile]")
  ...
  extension (path: String)
    ...
    def isAbsolute =
      path.norm.startsWith("/") || (isWin && path.secondChar == ":")
