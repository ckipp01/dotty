...
trait BCodeSkelBuilder extends BCodeHelpers:
  ...
  enum LoadDestination:
    /** The value is put on the stack, and control flows through to the next
      * opcode.
      */
    case FallThrough

    /** The value is put on the stack, and control flow is transferred to the
      * given `label`.
      */
  ...
  abstract class PlainSkelBuilder(cunit: CompilationUnit)
    ...
    def symInfoTK(sym: Symbol): BType =
      toTypeKind(
        sym.info
      ) // this tracks mentioned inner classes (in innerClassBufferASM)
    ...
    def genPlainClass(cd0: TypeDef) = cd0 match
      case TypeDef(_, impl: Template) =>
        ...
        val cd = if isCZStaticModule then
          ...
          claszSymbol.info.decls.foreach { f =>
            if f.isField && !f.name.is(LazyBitMapName) && !f.name.is(
                LazyLocalName
              )
            then f.setFlag(JavaStatic)
          }

          val (clinits, body) = impl.body.partition(stat =>
            stat.isInstanceOf[DefDef] && stat.symbol.isStaticConstructor
          )

          val (uptoSuperStats, remainingConstrStats) = splitAtSuper(
            impl.constr.rhs.asInstanceOf[Block].stats
          )
          val clInitSymbol: TermSymbol =
            if clinits.nonEmpty then clinits.head.symbol.asTerm
            else
              newSymbol(
                claszSymbol,
          ...
          val thisMap = new TreeMap:
            override def transform(tree: Tree)(using Context) =
              val tp = tree.tpe.substThis(
                claszSymbol.asClass,
                claszSymbol.sourceModule.termRef
              )
          ...
          def rewire(stat: Tree) = thisMap
            .transform(stat)
            .changeOwner(claszSymbol.primaryConstructor, clInitSymbol)

          val callConstructor = New(claszSymbol.typeRef)
            .select(claszSymbol.primaryConstructor)
            .appliedToTermArgs(Nil)
          ...
          val clinit = clinits match
            case (ddef: DefDef) :: _ =>
              cpy.DefDef(ddef)(rhs =
                Block(
                  ddef.rhs :: assignModuleField :: remainingConstrStatsSubst,
                  unitLiteral
                )
              )
            case _ =>
              DefDef(
                clInitSymbol,
                Block(
                  assignModuleField :: remainingConstrStatsSubst,
                  unitLiteral
                )
              )
          ...
          val constr2 =
            val rhs =
              Block(uptoSuperStats, impl.constr.rhs.asInstanceOf[Block].expr)
            cpy.DefDef(impl.constr)(rhs = rhs)

          val impl2 =
            cpy.Template(impl)(constr = constr2, body = clinit :: body)
          cpy.TypeDef(cd0)(rhs = impl2)
        else cd0

        val hasStaticCtor =
          isCZStaticModule || cd.symbol.info.decls.exists(_.isStaticConstructor)
        if !hasStaticCtor && isCZParcelable then fabricateStaticInitAndroid()

        val optSerial: Option[Long] =
          claszSymbol.getAnnotation(defn.SerialVersionUIDAnnot).flatMap {
            annot =>
              if claszSymbol.is(Trait) then
                report.warning(
                ...
                if vuid.isEmpty then
                  report.error(
                    "The argument passed to @SerialVersionUID must be a constant",
        ...
        then AsmUtils.traceClass(cnode)

        cnode.innerClasses
        assert(
          cd.symbol == claszSymbol,
          "Someone messed up BCodePhase.claszSymbol during genPlainClass()."
        )
    ...
    private def initJClass(jclass: asm.ClassVisitor): Unit =
      ...
      val superClass: String =
        if ps.isEmpty then ObjectRef.internalName
        else internalName(ps.head.typeSymbol)
      val interfaceNames0 =
        classBTypeFromSymbol(claszSymbol).info.interfaces.map(_.internalName)
       ...
       */
      val interfaceNames =
        if claszSymbol.is(ModuleClass) && !interfaceNames0.contains(
            "java/io/Serializable"
          )
        then interfaceNames0 :+ "java/io/Serializable"
        else interfaceNames0
      ...
      val ssa =
        None // TODO: inlined form `getAnnotPickle(thisName, claszSymbol)`. Should something be done on Dotty?
      cnode.visitAttribute(
        if ssa.isDefined then pickleMarkerLocal else pickleMarkerForeign
      )
      emitAnnotations(cnode, claszSymbol.annotations ++ ssa)

      if !isCZStaticModule && !isCZParcelable then
        val skipStaticForwarders =
          (claszSymbol.is(Module) || ctx.settings.XnoForwarders.value)
        if !skipStaticForwarders then
          val lmoc = claszSymbol.companionModule
          // add static forwarders if there are no name conflicts; see bugs #363 and #1735
          if lmoc != NoSymbol then
            // it must be a top level class (name contains no $s)
            val isCandidateForForwarders = (lmoc.is(Module)) && lmoc.isStatic
            if isCandidateForForwarders then
              report.log(
    ...
    def addClassFields(): Unit =
       ...
       */
      for f <- claszSymbol.info.decls.filter(p => p.isTerm && !p.is(Method)) do
        val javagensig = getGenericSignature(f, claszSymbol)
        val flags = javaFieldFlags(f)

        assert(
          !f.isStaticMember || !claszSymbol.isInterface || !f.is(Mutable),
     ...
     */
    var jumpDest
        : immutable.Map[ /* Labeled */ Symbol, (BType, LoadDestination)] = null
    def registerJumpDest(
        labelSym: Symbol,
        expectedType: BType,
      ...
      assert(
        labelSym.is(Label),
        s"trying to register a jump-dest for a non-label symbol, at: ${labelSym.span}"
      )
      assert(
        dest != LoadDestination.FallThrough,
        s"trying to register a FallThrough dest for label, at: ${labelSym.span}"
      )
      assert(
        !jumpDest.contains(labelSym),
        s"trying to register a second jump-dest for label, at: ${labelSym.span}"
      )
      jumpDest += (labelSym -> (expectedType, dest))
    def findJumpDest(labelSym: Symbol): (BType, LoadDestination) =
      assert(
        labelSym.is(Label),
        s"trying to map a non-label symbol to an asm.Label, at: ${labelSym.span}"
      )
      jumpDest.getOrElse(
        labelSym, {
    ...
    def registerCleanup(finCleanup: asm.Label): Unit =
      if finCleanup != null then cleanups = finCleanup :: cleanups
    def unregisterCleanup(finCleanup: asm.Label): Unit =
      if finCleanup != null then
        assert(
          cleanups.head eq finCleanup,
    ...
    object locals:
      ...
      def reset(isStaticMethod: Boolean): Unit =
        slots.clear()
        nxtIdx = if isStaticMethod then 0 else 1
      ...
      def makeLocal(tk: BType, name: String, tpe: Type, pos: Span): Symbol =
        ...
        val locSym =
          newSymbol(methSymbol, name.toTermName, Synthetic, tpe, NoSymbol, pos)
        makeLocal(locSym, tk)
        locSym
      ...
      def reuseLocal(sym: Symbol, loc: Local): Unit =
        ...
        if existing.isDefined then
          report.error(
            "attempt to create duplicate local var.",
            ctx.source.atSpan(sym.span)
          )

      def reuseThisSlot(sym: Symbol): Unit =
        reuseLocal(
          sym,
          Local(symInfoTK(sym), sym.javaSimpleName, 0, sym.is(Synthetic))
        )
    ...
    var varsInScope: List[(Symbol, asm.Label)] =
    ...
    def markProgramPoint(lbl: asm.Label): Unit =
      ...
      if !skip then mnode visitLabel lbl
    def isAtProgramPoint(lbl: asm.Label): Boolean =
      def getNonLineNumberNode(
          a: asm.tree.AbstractInsnNode
      ): asm.tree.AbstractInsnNode = a match
        case a: asm.tree.LineNumberNode =>
          getNonLineNumberNode(
            a.getPrevious
          ) // line numbers aren't part of code itself
        case _ => a
    ...
    def resetMethodBookkeeping(dd: DefDef) =
      ...
      assert(
        varsInScope == null,
        "Unbalanced entering/exiting of GenBCode's genBlock()."
      )
      // check previous invocation of genDefDef unregistered as many cleanups as it registered.
      assert(
        cleanups == Nil,
        "Previous invocation of genDefDef didn't unregister as many cleanups as it registered."
      )
      earlyReturnVar = null
    ...
    def gen(tree: Tree): Unit =
      tree match
        ...
        case tree: Template =>
          val body =
            if tree.constr.rhs.isEmpty then tree.body
    ...
    def initJMethod(flags: Int, params: List[Symbol]): Unit =
      ...
      val (excs, others) =
        methSymbol.annotations.partition(_.symbol eq defn.ThrowsAnnot)
      val thrownExceptions: List[String] = getExceptions(excs)

      val bytecodeName =
        if isMethSymStaticCtor then CLASS_CONSTRUCTOR_NAME
        else jMethodName
        ...
        { paramRefss =>
          ...
          new TreeTypeMap(
            typeMap = _.substThis(enclosingClass, selfParamRef.symbol.termRef)
              .subst(
                dd.termParamss.head.map(_.symbol),
                regularParamRefs.map(_.symbol.termRef)
              ),
            treeMap = {
    ...
    private def makeStaticForwarder(dd: DefDef): DefDef =
      ...
      tpd.DefDef(
        ...
        { paramss =>
          ...
          tpd
            .Apply(params.head.select(origSym), params.tail)
            .withAttachment(BCodeHelpers.UseInvokeSpecial, ())
        }
      )

    private def makeStatifiedDefSymbol(
        origSym: TermSymbol,
        name: TermName
    ): TermSymbol =
      val info = origSym.info match
        case mt: MethodType =>
          MethodType(
            nme.SELF :: mt.paramNames,
            origSym.owner.typeRef :: mt.paramInfos,
            mt.resType
          )
      origSym
        .copy(
    ...
    def genDefDef(dd: DefDef): Unit =
      ...
      if dd.symbol eq defn.Any_getClass then return
      ...
      val params = if vparamss.isEmpty then Nil else vparamss.head
      for p <- params do locals.makeLocal(p.symbol)
      // debug assert((params.map(p => locals(p.symbol).tk)) == asmMethodType(methSymbol).getArgumentTypes.toList, "debug")

      if params.size > MaximumJvmParameters then
        // SI-7324
        report.error(
          em"Platform restriction: a parameter list's length cannot exceed $MaximumJvmParameters.",
          ctx.source.atSpan(methSymbol.span)
        )
        return

      val isNative = methSymbol.hasAnnotation(NativeAttr)
      val isAbstractMethod = (methSymbol.is(
        Deferred
      ) || (methSymbol.owner.isInterface && ((methSymbol
        .is(Deferred)) || methSymbol.isClassConstructor)))
      val flags =
      ...
      if !isAbstractMethod && !isNative then
        // #14773 Reuse locals slots for tailrec-generated mutable vars
        val trimmedRhs: Tree =
          @tailrec def loop(stats: List[Tree]): List[Tree] =
            stats match
              case (tree @ ValDef(TailLocalName(_, _), _, _)) :: rest
                  if tree.symbol.isAllOf(Mutable | Synthetic) =>
                tree.rhs match
                  case This(_) =>
          ...
          rhs match
            case Block(stats, expr) =>
              ...
              if trimmedStats eq stats then rhs
              else Block(trimmedStats, expr)
            case _ =>
              rhs
        end trimmedRhs
        ...
        def emitNormalMethodBody(): Unit =
          ...
          else genLoadTo(trimmedRhs, returnType, LoadDestination.Return)

          if emitVars then
            // add entries to LocalVariableTable JVM attribute
            val onePastLastProgramPoint = currProgramPoint()
            val hasStaticBitSet = ((flags & asm.Opcodes.ACC_STATIC) != 0)
            if !hasStaticBitSet then
          ...
          if isMethSymStaticCtor then appendToStaticCtor(dd)
        end emitNormalMethodBody
      ...
      then AsmUtils.traceMethod(mnode)
    ...
    private def appendToStaticCtor(dd: DefDef): Unit =
      ...
      if isCZParcelable then
        ...
        val callee =
          claszSymbol.companionModule.info.member(androidFieldName).symbol
      ...
      for r <- rets do insertBefore(r, insnParcA, insnParcB)
    end appendToStaticCtor

    def emitLocalVarScope(
        sym: Symbol,
        start: asm.Label,
        end: asm.Label,
        force: Boolean = false
  ...
  end PlainSkelBuilder

  // end of class PlainSkelBuilder
end BCodeSkelBuilder
