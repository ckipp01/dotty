...
/** This component hosts tools and utilities used in the backend that require
  * access to a `BTypes` instance.
  */
class BackendUtils(val postProcessor: PostProcessor):
   ...
   * methods.
   */
  def addLambdaDeserialize(
      classNode: ClassNode,
      implMethodsArray: Array[Handle]
  ): Unit =
    import asm.Opcodes.*
    import bTypes.*
    import coreBTypes.*
    ...
    def emitLambdaDeserializeIndy(targetMethods: Seq[Handle]): Unit =
      ...
      mv.visitInvokeDynamicInsn(
        "lambdaDeserialize",
        serlamObjDesc,
        jliLambdaDeserializeBootstrapHandle,
        targetMethods*
      )

    val targetMethodGroupLimit =
      255 - 1 - 3 // JVM limit. See See MAX_MH_ARITY in CallSite.java
    val groups: Array[Array[Handle]] =
      implMethodsArray.grouped(targetMethodGroupLimit).toArray
    ...
    def nextLabel(i: Int) =
      if i == numGroups - 2 then terminalLabel else initialLabels(i + 1)

    for (label, i) <- initialLabels.iterator.zipWithIndex do
      mv.visitTryCatchBlock(
        label,
        nextLabel(i),
        nextLabel(i),
        jlIllegalArgExceptionRef.internalName
      )
    for (label, i) <- initialLabels.iterator.zipWithIndex do
      mv.visitLabel(label)
    ...
    mv.visitInsn(ARETURN)
  end addLambdaDeserialize

  /** Visit the class node and collect all referenced nested classes.
    */
  def collectNestedClasses(
      classNode: ClassNode
  ): (List[ClassBType], List[ClassBType]) =
  ...
  final def addInnerClasses(
    ...
    val allNestedClasses =
      new mutable.TreeSet[ClassBType]()(Ordering.by(_.internalName))
    allNestedClasses ++= declaredInnerClasses
    refedInnerClasses.foreach(
      allNestedClasses ++= _.enclosingNestedClassesChain
    )
