...
class DottyBackendInterface(
    val superCallsMap: ReadOnlyMap[Symbol, Set[ClassSymbol]]
)(using val ctx: Context):
  ...
  object ArrayValue extends DeconstructorCommon[tpd.JavaSeqLiteral]:
    def _1: Type = field.tpe match
      ...
      case _ =>
        report.error(
          em"JavaSeqArray with type ${field.tpe} reached backend: $field",
          ctx.source.atSpan(field.span)
        )
  ...
  abstract class DeconstructorCommon[T >: Null <: AnyRef]:
    ...
    def unapply(s: T): this.type =
      field = s
      this
end DottyBackendInterface

object DottyBackendInterface:

  private def erasureString(clazz: Class[?]): String =
    if clazz.isArray then "Array[" + erasureString(clazz.getComponentType) + "]"
    else clazz.getName
  ...
  def requiredModule[T](using evidence: ClassTag[T], ctx: Context): Symbol =
    ...
    val className =
      if moduleName.endsWith("$") then moduleName.dropRight(1) else moduleName
  ...
  given symExtensions: AnyRef with
    extension (sym: Symbol)
      ...
      def isStaticConstructor(using Context): Boolean =
        (sym.isStaticMember && sym.isClassConstructor) || (sym.name eq nme.STATIC_CONSTRUCTOR)

      /** Fields of static modules will be static at backend
        *
        * Note that lazy val encoding assumes bitmap fields are non-static. See
        * also `genPlainClass` in `BCodeSkelBuilder.scala`.
        *
        * TODO: remove the special handing of `LazyBitMapName` once we swtich to
        * the new lazy val encoding:
        * https://github.com/lampepfl/dotty/issues/7140
        */
      def isStaticModuleField(using Context): Boolean =
        sym.owner.isStaticModuleClass && sym.isField && !sym.name.is(
          LazyBitMapName
        ) && !sym.name.is(LazyLocalName)
      ...
      /** True for module classes of modules that are top-level or owned only by
        * objects. Module classes for such objects will get a MODULE$ flag and a
        * corresponding static initializer.
        */
      ...
      /** True for module classes of package level objects. The backend will
        * generate a mirror class for such objects.
        */
      ...
      def javaSimpleName(using Context): String = toDenot(
        sym
      ).name.mangledString
      def javaClassName(using Context): String = toDenot(
        sym
      ).fullName.mangledString
  ...
  /** True if the current compilation unit is of a primitive class
    * (scala.Boolean et al). Used only in assertions.
    */
