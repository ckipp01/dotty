...
trait BCodeSyncAndTry extends BCodeBodyBuilder:
  ...
  abstract class SyncAndTryBuilder(cunit: CompilationUnit)
    ...
    def genSynchronized(tree: Apply, expectedType: BType): BType =
      (tree: @unchecked) match
        case Apply(TypeApply(fun, _), args) =>
          val monitor =
            locals.makeLocal(ObjectRef, "monitor", defn.ObjectType, tree.span)
          ...
          genLoad(
            args.head,
            expectedType /* toTypeKind(tree.tpe.resultType) */
          )
          unregisterCleanup(monCleanup)
          if hasResult then locals.store(monitorResult)
          ...
          if hasResult then locals.load(monitorResult)
    ...
    def nopIfNeeded(lbl: asm.Label): Unit =
      ...
      if noInstructionEmitted then emit(asm.Opcodes.NOP)
    ...
    def genLoadTry(tree: Try): BType = tree match
      case Try(block, catches, finalizer) =>
         ...
         */
        val guardResult =
          hasFinally && (kind != UNIT) && mayCleanStack(finalizer)
        ...
        val tmp =
          if guardResult then
            locals.makeLocal(tpeTK(tree), "tmp", tree.tpe, tree.span)
          else null
         ...
         */
        val finCleanup = if hasFinally then new asm.Label else null
        ...
        /** A return within a `try` or `catch` block where a `finally` is
          * present ("early return") emits a store of the result to a local,
          * jump to a "cleanup" version of the `finally` block, and sets
          * `shouldEmitCleanup = true` (see [[PlainBodyBuilder.genReturn]]).
          *
          * If the try-catch is nested, outer `finally` blocks need to be
          * emitted in a cleanup version as well, so the `shouldEmitCleanup`
          * variable remains `true` until the outermost `finally`. Nested
          * cleanup `finally` blocks jump to the next enclosing one. For the
          * outermost, we emit a read of the local variable, a return, and we
          * set `shouldEmitCleanup = false` (see [[pendingCleanups]]).
          *
          * Now, assume we have
          *
          * try { return 1 } finally { try { println() } finally { println() } }
          *
          * Here, the outer `finally` needs a cleanup version, but the inner one
          * does not. The method here makes sure that `shouldEmitCleanup` is
          * only propagated outwards, not inwards to nested `finally` blocks.
          */
        ...
        for ch <- caseHandlers do
          withFreshCleanupScope {
            ...
            ch match
              ...
              case BoundEH(patSymbol, caseBody) =>
                ...
                val Local(patTK, _, patIdx, _) =
                  locals.getOrMakeLocal(patSymbol)
        ...
        if hasFinally && currentFinallyBlockNeedsCleanup then
    ...
    def emitFinalizer(
      ...
      var saved
          : immutable.Map[ /* Labeled */ Symbol, (BType, LoadDestination)] =
        null
      if isDuplicate then saved = jumpDest
      ...
      if tmp != null then locals.store(tmp)
      genLoad(finalizer, UNIT)
      if tmp != null then locals.load(tmp)
      if isDuplicate then jumpDest = saved

    /* Does this tree have a try-catch block? */
    def mayCleanStack(tree: Tree): Boolean = tree.find { t =>
      t match // TODO: use existsSubTree
        case Try(_, _, _) => true
        case _            => false
    }.isDefined
