...
trait BCodeHelpers extends BCodeIdiomatic:
  ...
  protected lazy val backendUtils =
    genBCodePhase.asInstanceOf[GenBCode].postProcessor.backendUtils

  def ScalaATTRName: String = "Scala"
  def ScalaSignatureATTRName: String = "ScalaSig"

  @threadUnsafe lazy val AnnotationRetentionAttr: ClassSymbol = requiredClass(
  ...
  @threadUnsafe lazy val AnnotationRetentionClassAttr: TermSymbol =
    requiredClass("java.lang.annotation.RetentionPolicy").linkedClass
      .requiredValue("CLASS")
  @threadUnsafe lazy val AnnotationRetentionRuntimeAttr: TermSymbol =
    requiredClass("java.lang.annotation.RetentionPolicy").linkedClass
      .requiredValue("RUNTIME")
  ...
  final def traitSuperAccessorName(sym: Symbol): String =
    ...
    if sym.name == nme.TRAIT_CONSTRUCTOR then nameString
    else nameString + "$"

  /*
   * can-multi-thread
   */
  def createJAttribute(
      name: String,
      b: Array[Byte],
      offset: Int,
      len: Int
  ...
  trait BCInnerClassGen:
    ...
    def debugLevel =
      3 // 0 -> no debug info; 1-> filename; 2-> lines; 3-> varnames
    ...
    final def internalName(sym: Symbol): String =
      ...
      val classSym =
        if sym.is(JavaDefined) && sym.is(ModuleClass) then sym.linkedClass
        else sym
    ...
    /** The ClassBType for a class symbol.
      *
      * The class symbol scala.Nothing is mapped to the class
      * scala.runtime.Nothing$. Similarly, scala.Null is mapped to
      * scala.runtime.Null$. This is because there exist no class files for the
      * Nothing / Null. If used for example as a parameter type, we use the
      * runtime classes in the classfile method signature.
      *
      * Note that the referenced class symbol may be an implementation class.
      * For example when compiling a mixed-in method that forwards to the static
      * method in the implementation class, the class descriptor of the receiver
      * (the implementation class) is obtained by creating the ClassBType.
      */
    final def getClassBType(sym: Symbol): ClassBType =
      assertClassNotArrayNotPrimitive(sym)

      if sym == defn.NothingClass then srNothingRef
      else if sym == defn.NullClass then srNullRef
    ...
    final def asmMethodType(msym: Symbol): MethodBType =
      ...
      val resT: BType =
        if msym.isClassConstructor || msym.isConstructor then UNIT
    ...
    final def toTypeKind(tp: Type): BType =
      typeToTypeKind(tp)(BCodeHelpers.this)(this)
  end BCInnerClassGen
  ...
  trait BCAnnotGen extends BCInnerClassGen:
    ...
    def emitAnnotations(
      ...
      for annot <- annotations; if shouldEmitAnnotation(annot) do
        val typ = annot.tree.tpe
        val assocs = assocsFromApply(annot.tree)
        val av =
          cw.visitAnnotation(typeDescriptor(typ), isRuntimeVisible(annot))
    ...
    def emitAnnotations(
      ...
      for annot <- annotations; if shouldEmitAnnotation(annot) do
        ...
        val assocs = assocsFromApply(annot.tree)
        val av =
          mw.visitAnnotation(typeDescriptor(typ), isRuntimeVisible(annot))
        emitAssocs(av, assocs, BCodeHelpers.this)(this)
    ...
    def emitAnnotations(
      ...
      for annot <- annotations; if shouldEmitAnnotation(annot) do
        ...
        val assocs = assocsFromApply(annot.tree)
        val av =
          fw.visitAnnotation(typeDescriptor(typ), isRuntimeVisible(annot))
        emitAssocs(av, assocs, BCodeHelpers.this)(this)
     ...
     * must-single-thread
     */
    def emitParamAnnotations(
        jmethod: asm.MethodVisitor,
        pannotss: List[List[Annotation]]
    ): Unit =
      val annotationss = pannotss map (_ filter shouldEmitAnnotation)
      if annotationss forall (_.isEmpty) then return
      for
      ...
      do
        ...
        val pannVisitor: asm.AnnotationVisitor =
          jmethod.visitParameterAnnotation(
            idx,
            typeDescriptor(typ.asInstanceOf[Type]),
    ...
    private def emitAssocs(
        av: asm.AnnotationVisitor,
        assocs: List[(Name, Object)],
        bcodeStore: BCodeHelpers
    )(innerClasesStore: bcodeStore.BCInnerClassGen) =
      for (name, value) <- assocs do
        emitArgument(
          av,
          name.mangledString,
    ...
    private def emitArgument(
      ...
      t match
        case Literal(const @ Constant(_)) =>
          const.tag match
            case BooleanTag | ByteTag | ShortTag | CharTag | IntTag | LongTag |
                FloatTag | DoubleTag =>
            ...
            case ClazzTag =>
              av.visit(
                name,
                typeToTypeKind(TypeErasure.erasure(const.typeValue))(
                  bcodeStore
                )(innerClasesStore).toASMType
              )
        case Ident(nme.WILDCARD) =>
        // An underscore argument indicates that we want to use the default value for this parameter, so do not emit anything
        case t: tpd.RefTree
            if t.symbol.owner.linkedClass.isAllOf(JavaEnumTrait) =>
          val edesc = innerClasesStore.typeDescriptor(
            t.tpe
          ) // the class descriptor of the enumeration class.
          val evalue =
            t.symbol.javaSimpleName // value the actual enumeration value.
        ...
        case Apply(fun, args)
            if fun.symbol == defn.ArrayClass.primaryConstructor ||
              toDenot(
                fun.symbol
              ).owner == defn.ArrayClass.linkedClass && fun.symbol.name == nme.apply =>
          val arrAnnotV: AnnotationVisitor = av.visitArray(name)

          var actualArgs =
            if fun.tpe.isImplicitMethod then
        ...
        case t @ Apply(constr, args) if t.tpe.classSymbol.is(JavaAnnotation) =>
          ...
          val desc = innerClasesStore.typeDescriptor(
            typ
    ...
    private def isRuntimeVisible(annot: Annotation): Boolean =
      if toDenot(annot.tree.tpe.typeSymbol)
          .hasAnnotation(AnnotationRetentionAttr)
      then retentionPolicyOf(annot) == AnnotationRetentionRuntimeAttr
      else
    ...
    private def retentionPolicyOf(annot: Annotation): Symbol =
      annot.tree.tpe.typeSymbol
        .getAnnotation(AnnotationRetentionAttr)
        .flatMap(_.argument(0).map(_.tpe.termSymbol))
        .getOrElse(AnnotationRetentionClassAttr)
          ...
          fun.tpe.widen match
            case MethodType(names) =>
              (names zip args).filter {
                case (_, t: tpd.Ident)
                    if (t.tpe.normalizedPrefix eq NoPrefix) =>
                  false
                case _ => true
              }
  end BCAnnotGen
  ...
  trait BCJGenSigGen:
    ...
    /** Generates the generic signature for `sym` before erasure.
      ...
      *   The owner of `sym`.
      * @return
      *   The generic signature of `sym` before erasure, as specified in the
      *   Java Virtual Machine Specification, §4.3.4, or `null` if `sym` doesn't
      *   need a generic signature.
      * @see
    ...
    def getGenericSignature(sym: Symbol, owner: Symbol): String =
      atPhase(erasurePhase) {
        val memberTpe =
          if sym.is(Method) then sym.denot.info
  ...
  trait BCForwardersGen extends BCAnnotGen with BCJGenSigGen:
    ...
    private def addForwarder(
      ...
      val paramJavaTypes: List[BType] =
        methodInfo.firstParamTypes map toTypeKind
      ...
      val flags = GenBCodeOps.PublicStatic | (
        if m.is(JavaVarargs) then asm.Opcodes.ACC_VARARGS else 0
      ) | (
        if isSynthetic then asm.Opcodes.ACC_SYNTHETIC else 0
      )

      // TODO needed? for(ann <- m.annotations) { ann.symbol.initialize }
      val jgensig = getStaticForwarderGenericSignature(m, module)
      val (throws, others) =
        m.annotations.partition(_.symbol eq defn.ThrowsAnnot)
      ...
      mirrorMethod.visitFieldInsn(
        ...
        symDescriptor(module)
      )

      var index = 0
      for jparamType <- paramJavaTypes do
    ...
    def addForwarders(
      ...
      lazy val conflictingNames: Set[Name] =
        (linkedClass.info.allMembers.collect {
          case d if d.name.isTermName => d.name
        }).toSet
      report.debuglog(
        s"Potentially conflicting names for forwarders: $conflictingNames"
      )

      for m0 <- sortedMembersBasedOnFlags(
          moduleClass.info,
          required = Method,
          excluded = ExcludedForwarder
        )
      do
        val m = if m0.is(Bridge) then m0.nextOverriddenSymbol else m0
        if m == NoSymbol then
          report.log(
            s"$m0 is a bridge method that overrides nothing, something went wrong in a previous phase."
          )
        else if m.isType || m.is(
            Deferred
          ) || (m.owner eq defn.ObjectClass) || m.isConstructor || m.name.is(
            ExpandedName
          )
        then
          report.debuglog(
            s"No forwarder for '$m' from $jclassName to '$moduleClass'"
          )
        else if conflictingNames(m.name) then
          report.log(
    ...
    /** The members of this type that have all of `required` flags but none of
      * `excluded` flags set. The members are sorted by name and signature to
      * guarantee a stable ordering.
      */
    private def sortedMembersBasedOnFlags(
        tp: Type,
        required: Flag,
        excluded: FlagSet
    ): List[Symbol] =
      ...
      names.foreach { name =>
        buffer ++= tp
          ...
          .alternatives
          .sortBy(_.signature)(Signature.lexicographicOrdering)
          .map(_.symbol)
      }
      buffer.toList
    ...
    def genMirrorClass(
      ...
      if emitSource then
        mirrorClass.visitSource(
          "" + cunit.source.file.name,
          null /* SourceDebugExtension */
        )

      val ssa =
        None // getAnnotPickle(mirrorName, if (moduleClass.is(Module)) moduleClass.companionClass else moduleClass.companionModule)
      mirrorClass.visitAttribute(
        if ssa.isDefined then pickleMarkerLocal else pickleMarkerForeign
      )
  ...
  trait JAndroidBuilder:
    ...
    lazy val AndroidParcelableInterface: Symbol =
      NoSymbol // getClassIfDefined("android.os.Parcelable")
    lazy val AndroidCreatorClass: Symbol =
      NoSymbol // getClassIfDefined("android.os.Parcelable$Creator")
     ...
     * must-single-thread
     */
    def legacyAddCreatorCode(
        clinit: asm.MethodVisitor,
    ...
    end legacyAddCreatorCode
  end JAndroidBuilder

  // end of trait JAndroidBuilder

  /** This method returns the BType for a type reference, for example a
    * parameter type.
    *
    * If the result is a ClassBType for a nested class, it is added to the
    * innerClassBufferASM.
    *
    * If `t` references a class, toTypeKind ensures that the class is not an
    * implementation class. See also comment on
    * getClassBTypeAndRegisterInnerClass, which is invoked for implementation
    * classes.
    */
  private def typeToTypeKind(tp: Type)(ct: BCodeHelpers)(
      storage: ct.BCInnerClassGen
  ): ct.bTypes.BType =
    import ct.bTypes.*
    ...
    /** Primitive types are represented as TypeRefs to the class symbol of, for
      * example, scala.Int. The `primitiveTypeMap` maps those class symbols to
      * the corresponding PrimitiveBType.
      */
    def primitiveOrClassToBType(sym: Symbol): BType =
      assert(sym.isClass, sym)
      assert(sym != defn.ArrayClass || compilingArray, sym)
      primitiveTypeMap
        .getOrElse(sym, storage.getClassBType(sym))
        .asInstanceOf[BType]

    /** When compiling Array.scala, the type parameter T is not erased and shows
      * up in method signatures, e.g. `def apply(i: Int): T`. A TyperRef to T is
      * replaced by ObjectReference.
      */
    ...
    tp.widenDealias match
        ...
        t.info match
          ...
          case _ =>
            if !t.symbol.isClass then
              nonClassTypeRefToBType(
                t.symbol
              ) // See comment on nonClassTypeRefToBType
            else
      ...
      case Types.ClassInfo(_, sym, _, _, _) =>
        primitiveOrClassToBType(
          sym
        ) // We get here, for example, for genLoadModule, which invokes toTypeKind(moduleClassSymbol.info)
      ...
      case tp =>
        ...
        tp match
          case tp: ThisType if tp.cls == defn.ArrayClass =>
            ObjectRef
              .asInstanceOf[ct.bTypes.ClassBType] // was introduced in 9b17332f11 to fix SI-999, but this code is not reached in its test, or any other test
          ...
          case t: RefinedType =>
            typeToTypeKind(t.parent)(ct)(
              storage
            ) // parents.map(_.toTypeKind(ct)(storage).asClassBType).reduceLeft((a, b) => a.jvmWiseLUB(b))
    end match
  end typeToTypeKind

  private def getGenericSignatureHelper(
      sym: Symbol,
      owner: Symbol,
      memberTpe: Type
  )(using Context): Option[String] =
    if needsGenericSignature(sym) then
      val erasedTypeSym = TypeErasure.fullErasure(sym.denot.info).typeSymbol
      if erasedTypeSym.isPrimitiveValueClass then
  ...
  private def verifySignature(sym: Symbol, sig: String)(using Context): Unit =
    ...
    wrap {
      if sym.is(Method) then CheckClassAdapter.checkMethodSignature(sig)
      else if sym.isTerm then CheckClassAdapter.checkFieldSignature(sig)
  ...
  private def getStaticForwarderGenericSignature(
      sym: Symbol,
      moduleClass: Symbol
  ): String =
    ...
    val memberTpe = atPhase(erasurePhase) {
      moduleClass.denot.thisType.memberInfo(sym)
    }
    val erasedMemberType =
      ElimErasedValueType.elimEVT(TypeErasure.transformInfo(sym, memberTpe))
    if erasedMemberType =:= sym.denot.info then
...
object BCodeHelpers:
  ...
  /** An attachment on Apply nodes indicating that it should be compiled with
    ...
    * forwarders. See BCodeSkelBuilder.makeStaticForwarder for more details.
    */
