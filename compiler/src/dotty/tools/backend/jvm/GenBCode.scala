...
class GenBCode extends Phase:
  ...
  def codeGen(using Context): CodeGen =
    if _codeGen eq null then
      ...
      _codeGen = new CodeGen(int, dottyPrimitives)(
        bTypes.asInstanceOf[BTypesFromSymbols[int.type]]
      )
    _codeGen
  ...
  def frontendAccess(using Context): PostProcessorFrontendAccess =
    if _frontendAccess eq null then
      _frontendAccess =
        PostProcessorFrontendAccess.Impl(backendInterface, entryPoints)
    _frontendAccess.nn
    ...
    try super.runOn(units)
      ...
      if _frontendAccess ne null then
        frontendAccess.compilerSettings.outputDirectory match
          case jar: JarArchive =>
            if ctx.run.nn.suspendedUnits.nonEmpty then
              ...
              report.error(
                "Can not suspend and output to a jar at the same time. See suspension with -Xprint-suspension."
              )

            jar.close()
          case _ => ()
      if _postProcessor ne null then postProcessor.classfileWriter.close()
end GenBCode
