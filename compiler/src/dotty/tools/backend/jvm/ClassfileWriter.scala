...
class ClassfileWriter(frontendAccess: PostProcessorFrontendAccess):
  ...
  private val dumpOutputDir: AbstractFile = getDirectoryOrNull(
  ...
  private val jarWriter: JarWriter | Null =
    compilerSettings.outputDirectory match
      case jar: JarArchive =>
        val mainClass = compilerSettings.mainClass.orElse {
          ...
          frontendAccess.getEntryPoints match
            case name :: Nil =>
              backendReporting.log(
                i"Unique entry point: setting Main-Class to $name"
              )
              Some(name)
            case names =>
              if names.isEmpty then
                backendReporting.warning(
                  em"No Main-Class designated or discovered."
                )
              else
                backendReporting.warning(
                  em"No Main-Class due to multiple entry points:\n  ${names.mkString("\n  ")}"
                )
              None
        }
        jar.underlyingSource.map { source =>
          if jar.isEmpty then
            ...
            new Jar(source.file).jarWriter(jarMainAttrs*)
          else
            ...
            backendReporting.warning(
              em"Tried to write to non-empty JAR: $source"
            )
            null
        }.orNull
  ...
  def writeClass(
    ...
    case e: java.nio.file.FileSystemException =>
      ...
      backendReporting.error(
        em"error writing $className: ${e.getClass.getName} ${e.getMessage}"
      )
      null
    ...
    if jarWriter == null then
      ...
      try writeBytes(outFile, bytes)
        ...
        case ex: ClosedByInterruptException =>
          try
            outFile
              .delete() // don't leave an empty or half-written files around after an interrupt
  ...
  def close(): Unit =
    if jarWriter != null then jarWriter.close()
end ClassfileWriter
