...
/** Functionality needed in the post-processor whose implementation depends on
  * the compiler frontend. All methods are synchronized.
  */
sealed abstract class PostProcessorFrontendAccess:
  ...
  inline final def frontendSynch[T](inline x: => T): T =
    frontendLock.synchronized(x)
...
object PostProcessorFrontendAccess:
  ...
  class Impl[I <: DottyBackendInterface](
      val int: I,
    ...
    private def buildCompilerSettings(): CompilerSettings =
      new CompilerSettings:
        ...
        lazy val target =
          val releaseValue =
            Option(s.javaOutputVersion.value).filter(_.nonEmpty)
          val targetValue =
            Option(s.XuncheckedJavaOutputVersion.value).filter(_.nonEmpty)
          (releaseValue, targetValue) match
            case (Some(release), None) => release
            case (None, Some(target))  => target
            case (Some(release), Some(_)) =>
              report.warning(
                s"The value of ${s.XuncheckedJavaOutputVersion.name} was overridden by ${ctx.settings.javaOutputVersion.name}"
              )
              release
            case (None, None) => "8" // least supported version by default

        lazy val debug: Boolean = ctx.debug
        lazy val dumpClassesDirectory: Option[String] =
          s.Ydumpclasses.valueSetByUser
    ...
    object backendReporting extends BackendReporting:
      def error(message: Context ?=> Message): Unit = frontendSynch(
        report.error(message)
      )
      def warning(message: Context ?=> Message): Unit = frontendSynch(
        report.warning(message)
      )
      def log(message: Context ?=> String): Unit = frontendSynch(
        report.log(message)
      )

    def getEntryPoints: List[String] = frontendSynch(entryPoints.toList)
  end Impl
end PostProcessorFrontendAccess
