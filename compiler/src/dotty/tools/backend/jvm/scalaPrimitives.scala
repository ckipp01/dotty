...
/** Scala primitive operations are represented as methods in `Any` and `AnyVal`
  * subclasses. Here we demultiplex them by providing a mapping from their
  * symbols to integers. Different methods exist for different value types, but
  * with the same meaning (like plus, minus, etc.). They will all be mapped to
  * the same int.
  *
  * Note: The three equal methods have the following semantics:
  *   - `"=="` checks for `null`, and if non-null, calls
  *     `java.lang.Object.equals` `(class: Any; modifier: final)`. Primitive:
  *     `EQ`
  *   - `"eq"` usual reference comparison `(class: AnyRef; modifier: final)`.
  *     Primitive: `ID`
  *   - `"equals"` user-defined equality (Java semantics) `(class: Object;
  *     modifier: none)`. Primitive: `EQUALS`
  *
...
class DottyPrimitives(ictx: Context):
  ...
  /** Return the primitive code of the given operation. If the operation is an
    * array get/set, we inspect the type of the receiver to demux the operation.
    *
    * @param fun
    *   The method symbol
    * @param tpe
        ...
        alts foreach (s =>
          addPrimitive(
            ...
            s.info.paramInfoss match
              case List(tp :: _)
                  if code == ADD && tp =:= ctx.definitions.StringType =>
                CONCAT
              case _ => code
          )
        )
  ...
  def isPrimitive(fun: Tree): Boolean =
    ...
    || (fun.symbol == NoSymbol // the only trees that do not have a symbol assigned are array.{update,select,length,clone}}
      && {
        fun match
          case Select(_, StdNames.nme.clone_) =>
            false // but array.clone is NOT a primitive op.
          case _ => true
      })
end DottyPrimitives
