...
class CodeGen(val int: DottyBackendInterface, val primitives: DottyPrimitives)(
      ...
      try
        ...
        def registerGeneratedClass(
          ...
          generatedClasses += GeneratedClass(
            ...
            sourceFile,
            isArtifact,
            onFileCreated(classNode, sym, unit.source)
          )
        ...
        if sym.isClass then genTastyAndSetAttributes(sym, attrNode)
      catch
        case ex: Throwable =>
          ex.printStackTrace()
          report.error(
            s"Error while emitting ${unit.source}\n${ex.getMessage}",
            NoSourcePosition
          )

    def genTastyAndSetAttributes(claszSymbol: Symbol, store: ClassNode): Unit =
      import Impl.createJAttribute
      for binary <- unit.pickled.get(claszSymbol.asClass) do
        ...
        val dataAttr =
          createJAttribute(nme.TASTYATTR.mangledString, tasty, 0, tasty.length)
  ...
  private def onFileCreated(
      cls: ClassNode,
      claszSymbol: Symbol,
      sourceFile: interfaces.SourceFile
  ): AbstractFile => Unit = clsFile =>
    val (fullClassName, isLocal) = atPhase(sbtExtractDependenciesPhase) {
      (ExtractDependencies.classNameAsString(claszSymbol), claszSymbol.isLocal)
    }

    val className = cls.name.replace('/', '.')
    if ctx.compilerCallback != null then
      ctx.compilerCallback.onClassGenerated(
        sourceFile,
        convertAbstractFile(clsFile),
        className
      )
    ...
    if ctx.sbtCallback != null then
      ...
      if isLocal then cb.generatedLocalClass(jSourceFile, clsFile.file)
      else
        cb.generatedNonLocalClass(
          jSourceFile,
          clsFile.file,
          className,
          fullClassName
  ...
  /** Convert a `dotty.tools.io.AbstractFile` into a
    ...
    */
  private def convertAbstractFile(
      absfile: dotty.tools.io.AbstractFile
  ): interfaces.AbstractFile =
  ...
  private def checkForCaseConflict(javaClassName: String, classSymbol: Symbol) =
    ...
    lowerCaseNames.get(lowerCaseName) match
      ...
      case Some(dupClassSym) =>
        ...
        val (cl1, cl2) =
          if classSymbol.effectiveName.toString < dupClassSym.effectiveName.toString
          then (classSymbol, dupClassSym)
          else (dupClassSym, classSymbol)
        val same =
          classSymbol.effectiveName.toString == dupClassSym.effectiveName.toString
        atPhase(typerPhase) {
          if same then
            // FIXME: This should really be an error, but then FromTasty tests fail
            report.warning(
              s"${cl1.show} and ${cl2.showLocated} produce classes that overwrite one another",
              cl1.sourcePos
            )
          else
            report.warning(
              s"${cl1.show} differs only in case from ${cl2.showLocated}. " +
  ...
  object Impl extends ImplEarlyInit with BCodeSyncAndTry:
    class PlainClassBuilder(unit: CompilationUnit)
        extends SyncAndTryBuilder(unit)
end CodeGen
