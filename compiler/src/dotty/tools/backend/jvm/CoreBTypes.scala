...
abstract class CoreBTypesFromSymbols[I <: DottyBackendInterface]
    extends CoreBTypes:
  ...
  /** Maps primitive types to their corresponding PrimitiveBType. The map is
    * defined lexically above the first use of `classBTypeFromSymbol` because
    * that method looks at the map.
    */
  ...
  /** Map from primitive types to their boxed class type. Useful when pushing
    * class literals onto the operand stack (ldc instruction taking a class
    * literal), see genConstant.
    */
  ...
  /** Maps the method symbol for a box method to the boxed type of the result.
    * For example, the method symbol for `Byte.box()` is mapped to the
    * ClassBType `java/lang/Byte`.
    */
  lazy val boxResultType: Map[Symbol, ClassBType] =
    val boxMethods = defn
      .ScalaValueClasses()
      .map { x => // @darkdimius Are you sure this should be a def?
        (x, Erasure.Boxing.boxMethod(x.asClass))
      }
      .toMap
    for ((valueClassSym, boxMethodSym) <- boxMethods)
      yield boxMethodSym -> boxedClassOfPrimitive(
        primitiveTypeMap(valueClassSym)
      )

  /** Maps the method symbol for an unbox method to the primitive type of the
    * result. For example, the method symbol for `Byte.unbox()`) is mapped to
    * the PrimitiveBType BYTE.
    */
  lazy val unboxResultType: Map[Symbol, PrimitiveBType] =
    val unboxMethods: Map[Symbol, Symbol] =
      defn
        .ScalaValueClasses()
        .map(x => (x, Erasure.Boxing.unboxMethod(x.asClass)))
        .toMap
  ...
  lazy val jlStringBuilderRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.StringBuilder]
  )
  lazy val jlStringBufferRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.StringBuffer]
  )
  lazy val jlCharSequenceRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.CharSequence]
  )
  lazy val jlClassRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.Class[?]]
  )
  lazy val jlThrowableRef: ClassBType = classBTypeFromSymbol(
    defn.ThrowableClass
  )
  lazy val jlCloneableRef: ClassBType = classBTypeFromSymbol(
    defn.JavaCloneableClass
  )
  lazy val jiSerializableRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.io.Serializable]
  )
  lazy val jlClassCastExceptionRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.ClassCastException]
  )
  lazy val jlIllegalArgExceptionRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.IllegalArgumentException]
  )
  ...
  private lazy val jliCallSiteRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.invoke.CallSite]
  )
  private lazy val jliLambdaMetafactoryRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.invoke.LambdaMetafactory]
  )
  private lazy val jliMethodHandleRef: ClassBType = classBTypeFromSymbol(
    defn.MethodHandleClass
  )
  private lazy val jliMethodHandlesLookupRef: ClassBType = classBTypeFromSymbol(
    defn.MethodHandlesLookupClass
  )
  private lazy val jliMethodTypeRef: ClassBType = classBTypeFromSymbol(
    requiredClass[java.lang.invoke.MethodType]
  )
  private lazy val jliStringConcatFactoryRef: ClassBType = classBTypeFromSymbol(
    requiredClass("java.lang.invoke.StringConcatFactory")
  ) // since JDK 9
  ...
  lazy val asmBoxTo: Map[BType, MethodNameAndType] = Map(
    BOOL -> MethodNameAndType(
      "boxToBoolean",
      MethodBType(List(BOOL), boxedClassOfPrimitive(BOOL))
    ),
    BYTE -> MethodNameAndType(
      "boxToByte",
      MethodBType(List(BYTE), boxedClassOfPrimitive(BYTE))
    ),
    CHAR -> MethodNameAndType(
      "boxToCharacter",
      MethodBType(List(CHAR), boxedClassOfPrimitive(CHAR))
    ),
    SHORT -> MethodNameAndType(
      "boxToShort",
      MethodBType(List(SHORT), boxedClassOfPrimitive(SHORT))
    ),
    INT -> MethodNameAndType(
      "boxToInteger",
      MethodBType(List(INT), boxedClassOfPrimitive(INT))
    ),
    LONG -> MethodNameAndType(
      "boxToLong",
      MethodBType(List(LONG), boxedClassOfPrimitive(LONG))
    ),
    FLOAT -> MethodNameAndType(
      "boxToFloat",
      MethodBType(List(FLOAT), boxedClassOfPrimitive(FLOAT))
    ),
    DOUBLE -> MethodNameAndType(
      "boxToDouble",
      MethodBType(List(DOUBLE), boxedClassOfPrimitive(DOUBLE))
    )
  )
  ...
  lazy val asmUnboxTo: Map[BType, MethodNameAndType] = Map(
    BOOL -> MethodNameAndType(
      "unboxToBoolean",
      MethodBType(List(ObjectRef), BOOL)
    ),
    BYTE -> MethodNameAndType(
      "unboxToByte",
      MethodBType(List(ObjectRef), BYTE)
    ),
    CHAR -> MethodNameAndType(
      "unboxToChar",
      MethodBType(List(ObjectRef), CHAR)
    ),
    SHORT -> MethodNameAndType(
      "unboxToShort",
      MethodBType(List(ObjectRef), SHORT)
    ),
    INT -> MethodNameAndType("unboxToInt", MethodBType(List(ObjectRef), INT)),
    LONG -> MethodNameAndType(
      "unboxToLong",
      MethodBType(List(ObjectRef), LONG)
    ),
    FLOAT -> MethodNameAndType(
      "unboxToFloat",
      MethodBType(List(ObjectRef), FLOAT)
    ),
    DOUBLE -> MethodNameAndType(
      "unboxToDouble",
      MethodBType(List(ObjectRef), DOUBLE)
    )
  )
