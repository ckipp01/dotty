...
trait BCodeIdiomatic:
  ...
  final def mkArrayReverse(xs: List[String]): Array[String] =
    val len = xs.size
    if len == 0 then return EMPTY_STRING_ARRAY
  ...
  final def mkArrayReverse(xs: List[Int]): Array[Int] =
    val len = xs.size
    if len == 0 then return EMPTY_INT_ARRAY
  ...
  abstract class JCodeMethodN:
    ...
    final def genPrimitiveArithmetic(op: ArithmeticOp, kind: BType): Unit =
      ...
      op match
        ...
        case NOT =>
          if kind.isIntSizedType then
            emit(Opcodes.ICONST_M1)
            emit(Opcodes.IXOR)
          else if kind == LONG then
            jmethod.visitLdcInsn(java.lang.Long.valueOf(-1))
            jmethod.visitInsn(Opcodes.LXOR)
          else abort(s"Impossible to negate an $kind")
    ...
    final def genPrimitiveLogical(op: /* LogicalOp */ Int, kind: BType): Unit =
      ...
      ((op, kind): @unchecked) match
        ...
        case (AND, _) =>
          emit(Opcodes.IAND)
          if kind != BOOL then emitT2T(INT, kind)
    ...
    final def genStringBuilderAppend(elemType: BType): Unit =
      val paramType = elemType match
        ...
        case ct: ClassBType if ct.isSubtypeOf(jlStringBufferRef) =>
          jlStringBufferRef
        case ct: ClassBType if ct.isSubtypeOf(jlCharSequenceRef) =>
          jlCharSequenceRef
    ...
    // can-multi-thread
    final def boolconst(b: Boolean): Unit = iconst(if b then 1 else 0)

    // can-multi-thread
    final def iconst(cst: Int): Unit =
      if cst >= -1 && cst <= 5 then emit(Opcodes.ICONST_0 + cst)
      else if cst >= java.lang.Byte.MIN_VALUE && cst <= java.lang.Byte.MAX_VALUE
      then jmethod.visitIntInsn(Opcodes.BIPUSH, cst)
      else if cst >= java.lang.Short.MIN_VALUE && cst <= java.lang.Short.MAX_VALUE
      then jmethod.visitIntInsn(Opcodes.SIPUSH, cst)
      else jmethod.visitLdcInsn(Integer.valueOf(cst))

    // can-multi-thread
    final def lconst(cst: Long): Unit =
      if cst == 0L || cst == 1L then
        emit(Opcodes.LCONST_0 + cst.asInstanceOf[Int])
      else jmethod.visitLdcInsn(java.lang.Long.valueOf(cst))

    // can-multi-thread
    final def fconst(cst: Float): Unit =
      val bits: Int = java.lang.Float.floatToIntBits(cst)
      if bits == 0L || bits == 0x3f800000 || bits == 0x40000000 then // 0..2
        emit(Opcodes.FCONST_0 + cst.asInstanceOf[Int])
      else jmethod.visitLdcInsn(java.lang.Float.valueOf(cst))

    // can-multi-thread
    final def dconst(cst: Double): Unit =
      val bits: Long = java.lang.Double.doubleToLongBits(cst)
      if bits == 0L || bits == 0x3ff0000000000000L then // +0.0d and 1.0d
        emit(Opcodes.DCONST_0 + cst.asInstanceOf[Int])
      else jmethod.visitLdcInsn(java.lang.Double.valueOf(cst))
    ...
    final def load(idx: Int, tk: BType): Unit =
      emitVarInsn(Opcodes.ILOAD, idx, tk) // can-multi-thread
    final def store(idx: Int, tk: BType): Unit =
      emitVarInsn(Opcodes.ISTORE, idx, tk) // can-multi-thread
    final def iinc(idx: Int, increment: Int): Unit =
      jmethod.visitIincInsn(idx, increment) // can-multi-thread

    final def aload(tk: BType): Unit =
      emitTypeBased(JCodeMethodN.aloadOpcodes, tk) // can-multi-thread
    final def astore(tk: BType): Unit =
      emitTypeBased(JCodeMethodN.astoreOpcodes, tk) // can-multi-thread

    final def neg(tk: BType): Unit =
      emitPrimitive(JCodeMethodN.negOpcodes, tk) // can-multi-thread
    final def add(tk: BType): Unit =
      emitPrimitive(JCodeMethodN.addOpcodes, tk) // can-multi-thread
    final def sub(tk: BType): Unit =
      emitPrimitive(JCodeMethodN.subOpcodes, tk) // can-multi-thread
    final def mul(tk: BType): Unit =
      emitPrimitive(JCodeMethodN.mulOpcodes, tk) // can-multi-thread
    final def div(tk: BType): Unit =
      emitPrimitive(JCodeMethodN.divOpcodes, tk) // can-multi-thread
    final def rem(tk: BType): Unit =
      emitPrimitive(JCodeMethodN.remOpcodes, tk) // can-multi-thread
    ...
    // can-multi-thread
    final def goTo(label: asm.Label): Unit =
      jmethod.visitJumpInsn(Opcodes.GOTO, label)
    // can-multi-thread
    final def emitIF(cond: TestOp, label: asm.Label): Unit =
      jmethod.visitJumpInsn(cond.opcodeIF(), label)
    // can-multi-thread
    final def emitIF_ICMP(cond: TestOp, label: asm.Label): Unit =
    ...
    final def emitIF_ACMP(cond: TestOp, label: asm.Label): Unit =
      ...
      val opc = (if cond == EQ then Opcodes.IF_ACMPEQ else Opcodes.IF_ACMPNE)
      jmethod.visitJumpInsn(opc, label)
    // can-multi-thread
    final def emitIFNONNULL(label: asm.Label): Unit =
      jmethod.visitJumpInsn(Opcodes.IFNONNULL, label)
    // can-multi-thread
    final def emitIFNULL(label: asm.Label): Unit =
      jmethod.visitJumpInsn(Opcodes.IFNULL, label)

    // can-multi-thread
    final def emitRETURN(tk: BType): Unit =
      if tk == UNIT then emit(Opcodes.RETURN)
     ...
     * can-multi-thread
     */
    final def emitSWITCH(
        keys: Array[Int],
        branches: Array[asm.Label],
        defaultBranch: asm.Label,
        minDensity: Double
    ): Unit =
    ...
    final def emitTypeBased(opcs: Array[Int], tk: BType): Unit =
      assert(tk != UNIT, tk)
      val opc =
        if tk.isRef then opcs(0)
        else if tk.isIntSizedType then
          (tk: @unchecked) match
    ...
    // can-multi-thread
    final def drop(tk: BType): Unit = emit(
      if tk.isWideType then Opcodes.POP2 else Opcodes.POP
    )
    ...
    final def dropMany(size: Int): Unit =
      ...
      while s >= 2 do
        emit(Opcodes.POP2)
        s -= 2
      if s > 0 then emit(Opcodes.POP)

    // can-multi-thread
    final def dup(tk: BType): Unit = emit(
      if tk.isWideType then Opcodes.DUP2 else Opcodes.DUP
    )
  ...
  object JCodeMethodN:
    ...
    val fromCharT2T =
      Array(I2B, I2S, -1, -1, I2L, I2F, I2D) // for (CHAR  -> INT) do nothing
    val fromShortT2T =
      Array(I2B, -1, I2C, -1, I2L, I2F, I2D) // for (SHORT -> INT) do nothing
  ...
  implicit class InsnIterMethodNode(mnode: asm.tree.MethodNode):
    @`inline` final def foreachInsn(
        f: (asm.tree.AbstractInsnNode) => Unit
    ): Unit = mnode.instructions.foreachInsn(f)
  ...
  implicit class InsnIterInsnList(lst: asm.tree.InsnList):
    ...
    @`inline` final def foreachInsn(
      ...
      while insnIter.hasNext do f(insnIter.next())
end BCodeIdiomatic
