...
/** Implements late stages of the backend that don't depend on a Global
  * instance, i.e., optimizations, post-processing and classfile serialization
  * and writing.
  */
class PostProcessor(
  ...
  def postProcessAndSendToDisk(generatedDefs: GeneratedDefs): Unit =
    ...
    for GeneratedClass(classNode, sourceFile, isArtifact, onFileCreated) <-
        classes
    do
      val bytes =
        try
          ...
          case e: java.lang.RuntimeException
              if e.getMessage != null && e.getMessage.nn.contains(
                "too large!"
              ) =>
            backendReporting.error(
              em"Could not write class ${classNode.name} because it exceeds JVM code size limits. ${e.getMessage}"
            )
            null
          case ex: Throwable =>
            ex.printStackTrace()
            backendReporting.error(
              em"Error while emitting ${classNode.name}\n${ex.getMessage}"
            )
            null

      if bytes != null then
        if AsmUtils.traceSerializedClassEnabled && classNode.name.nn.contains(
            AsmUtils.traceSerializedClassPattern
          )
        then AsmUtils.traceClass(bytes)

        val clsFile =
          classfileWriter.writeClass(classNode.name.nn, bytes, sourceFile)
        if clsFile != null then onFileCreated(clsFile)
    end for

    for GeneratedTasty(classNode, binaryGen) <- tasty do
      classfileWriter.writeTasty(classNode.name.nn, binaryGen())
  end postProcessAndSendToDisk
  ...
  final class ClassWriterWithBTypeLub(flags: Int) extends ClassWriter(flags):
    ...
    /** This method is used by asm when computing stack map frames. It is
      * thread-safe: it depends only on the BTypes component, which does not
      * depend on global. TODO @lry move to a different place where no global is
      * in scope, on bTypes.
      */
...
case class GeneratedDefs(
    classes: List[GeneratedClass],
    tasty: List[GeneratedTasty]
)
