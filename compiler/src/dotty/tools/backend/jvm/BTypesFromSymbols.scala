...
/** This class mainly contains the method classBTypeFromSymbol, which extracts
  * the necessary information from a symbol and its type to create the
  * corresponding ClassBType. It requires access to the compiler (global
  * parameter).
  */
class BTypesFromSymbols[I <: DottyBackendInterface](
    val int: I,
  ...
  /** Cache for the method classBTypeFromSymbol.
    */
  @threadUnsafe private lazy val convertedClasses =
    collection.mutable.HashMap.empty[Symbol, ClassBType]
  ...
  final def mirrorClassBTypeFromSymbol(moduleClassSym: Symbol): ClassBType =
    assert(
      moduleClassSym.isTopLevelModuleClass,
      s"not a top-level module class: $moduleClassSym"
    )
    val internalName =
      moduleClassSym.javaBinaryName.stripSuffix(StdNames.str.MODULE_SUFFIX)
    ...
    bType.info = ClassInfo(
      ...
      memberClasses =
        getMemberClasses(moduleClassSym).map(classBTypeFromSymbol),
      nestedInfo = None
    )
    bType
  ...
  private def setClassInfo(
    ...
    val superClassSym: Symbol =
      ...
      if t.exists then t
      else if classSym.is(ModuleClass) then
        // workaround #371
    ...
    assert(
      if classSym == defn.ObjectClass then superClassSym == NoSymbol
      else if classSym.isInterface then superClassSym == defn.ObjectClass
      else
        // A ClassBType for a primitive class (scala.Boolean et al) is only created when compiling these classes.
        ((superClassSym != NoSymbol) && !superClassSym.isInterface) || (isCompilingPrimitive && primitiveTypeMap
          .contains(classSym))
      ,
      s"Bad superClass for $classSym: $superClassSym"
    )
    val superClass =
      if superClassSym == NoSymbol then None
      else Some(classBTypeFromSymbol(superClassSym))

    /** All interfaces implemented by a class, except for those inherited
      * through the superclass. Redundant interfaces are removed unless there is
      * a super call to them.
      */
    extension (sym: Symbol)
      def superInterfaces: List[Symbol] =
        val directlyInheritedTraits = sym.directlyInheritedTraits
        val directlyInheritedTraitsSet = directlyInheritedTraits.toSet
        val allBaseClasses = directlyInheritedTraits.iterator
          .flatMap(_.asClass.baseClasses.drop(1))
          .toSet
        val superCalls = superCallsMap.getOrElse(sym, Set.empty)
        val additional =
          (superCalls -- directlyInheritedTraitsSet).filter(_.is(Trait))
//      if (additional.nonEmpty)
//        println(s"$fullName: adding supertraits $additional")
        directlyInheritedTraits.filter(t =>
          !allBaseClasses(t) || superCalls(t)
        ) ++ additional
    ...
    val nestedClassSymbols =
      ...
      val companionModuleMembers =
        if classSym.linkedClass.isTopLevelModuleClass then
          getMemberClasses(classSym.linkedClass)
        else Nil

      nestedClasses ++ companionModuleMembers

    /** For nested java classes, the scala compiler creates both a class and a
      * module (and therefore a module class) symbol. For example, in `class A {
      * class B {} }`, the nestedClassSymbols for A contain both the class B and
      * the module class B. Here we get rid of the module class B, making sure
      * that the class B is present.
      */
    val nestedClassSymbolsNoJavaModuleClasses = nestedClassSymbols.filter(s =>
      if s.is(JavaDefined) && s.is(ModuleClass) then
  ...
  /** For currently compiled classes: All locally defined classes including
    * local classes. The empty list for classes that are not currently compiled.
    */
  private def getNestedClasses(sym: Symbol): List[Symbol] =
    definedClasses(sym, flattenPhase)

  /** For currently compiled classes: All classes that are declared as members
    * of this class (but not inherited ones). The empty list for classes that
    * are not currently compiled.
    */
  private def getMemberClasses(sym: Symbol): List[Symbol] =
    definedClasses(sym, lambdaLiftPhase)

  private def definedClasses(sym: Symbol, phase: Phase) =
    if sym.isDefinedInCurrentRun then
      atPhase(phase) {
        toDenot(sym).info.decls.filter(sym =>
          sym.isClass && !sym.isEffectivelyErased
        )
      }
    else Nil
      ...
      val outerName: Option[String] =
        if isAnonymousOrLocalClass(innerClassSym) then None
          ...
          def dropModule(str: String): String =
            if !str.isEmpty && str.last == '$' then str.take(str.length - 1)
            else str
          // Java compatibility. See the big comment in BTypes that summarizes the InnerClass spec.
          val outerNameModule =
            if innerClassSym.originalOwner.originalLexicallyEnclosingClass.isTopLevelModuleClass
  ...
  /** This is basically a re-implementation of sym.isStaticOwner, but using the
    * originalOwner chain.
    *
    * The problem is that we are interested in a source-level property. Various
    * phases changed the symbol's properties in the meantime, mostly lambdalift
    * modified (destructively) the owner. Therefore, `sym.isStatic` is not what
    * we want. For example, in object T { def f { object U } } the owner of U is
    * T, so UModuleClass.isStatic is true. Phase travel does not help here.
    */
  extension (sym: Symbol)
    private def isOriginallyStaticOwner: Boolean =
      sym.is(PackageClass) || sym.is(
        ModuleClass
      ) && sym.originalOwner.originalLexicallyEnclosingClass.isOriginallyStaticOwner

  /** Return the Java modifiers for the given symbol. Java modifiers for
    * classes:
    ...
    *   - abstract, synchronized (not used), strictfp (not used), native (not
    *     used) for all:
    *   - deprecated
    *
  ...
  final def javaFlags(sym: Symbol): Int =
    ...
    val privateFlag = !sym.isClass && (sym.is(
      Private
    ) || (sym.isPrimaryConstructor && sym.owner.isTopLevelModuleClass))

    val finalFlag = sym.is(Final) && !toDenot(sym).isClassConstructor && !sym
      .is(Mutable, butNot = Accessor) && !sym.enclosingClass.is(Trait)
