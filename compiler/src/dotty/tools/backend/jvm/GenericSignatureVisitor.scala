...
abstract class GenericSignatureVisitor(nestedOnly: Boolean):
  ...
  private trait CharBooleanFunction:
    def apply(c: Char): Boolean

  final def visitInternalName(internalName: String): Unit = visitInternalName(
    internalName,
    0,
    if internalName eq null then 0 else internalName.length
  )
  ...
  def visitClassSignature(sig: String): Unit = if sig != null then
    val p = new Parser(sig, nestedOnly)
    p.safely { p.classSignature() }

  def visitMethodSignature(sig: String): Unit = if sig != null then
    val p = new Parser(sig, nestedOnly)
    p.safely { p.methodSignature() }

  def visitFieldSignature(sig: String): Unit = if sig != null then
    val p = new Parser(sig, nestedOnly)
  ...
  private final class Parser(sig: String, nestedOnly: Boolean):
    ...
    @inline def safely(f: => Unit): Unit = try f
      ...
      case Aborted =>
      case NonFatal(e) =>
        raiseError(s"Exception thrown during signature parsing", sig, Some(e))
    ...
    private def getCurrentAndSkip(): Char =
      val c = current; skip(); c

    private def skipUntil(isDelimiter: CharBooleanFunction): Unit =
      while !isDelimiter(current) do index += 1
    ...
    private def appendUntil(
        builder: java.lang.StringBuilder,
        isDelimiter: CharBooleanFunction
    ): Unit =
      val start = index
    ...
    private val isClassNameEnd: CharBooleanFunction = (c: Char) =>
      c == '<' || c == '.' || c == ';'

    private def typeArguments(): Unit = if current == '<' then
      skip()
      while current != '>' do
        current match
          case '*' | '+' | '-' =>
    ...
    @tailrec private def referenceTypeSignature(): Unit =
      getCurrentAndSkip() match
        case 'L' =>
          var names: java.lang.StringBuilder = null

          val start = index
          var seenDollar = false
          while !isClassNameEnd(current) do
            seenDollar ||= current == '$'
            index += 1
          if (current == '.' || seenDollar) || !nestedOnly then
        ...
        case '[' =>
          if isBaseType(current) then skip()
          else referenceTypeSignature()

    private def typeParameters(): Unit = if current == '<' then
      skip()
      while current != '>' do
        ...
        if c != ':' && c != '>' then referenceTypeSignature()
        while current == ':' do
          skip(); referenceTypeSignature()
      accept('>')

    def classSignature(): Unit =
      typeParameters()
      while index < end do referenceTypeSignature()
    ...
    def methodSignature(): Unit =
      ...
      if current == 'V' || isBaseType(current) then skip()
      else referenceTypeSignature()
    ...
    def fieldSignature(): Unit = if sig != null then
      safely {
        referenceTypeSignature()
      }
  end Parser
end GenericSignatureVisitor
...
abstract class NestedClassesCollector[T](nestedOnly: Boolean)
    extends GenericSignatureVisitor(nestedOnly):
  ...
  def innerClasses: collection.Set[T] =
  ...
  def visit(classNode: ClassNode): Unit =
    ...
    for f <- classNode.fields.asScala do
    ...
    for m <- classNode.methods.asScala do
      ...
      for tcb <- m.tryCatchBlocks.asScala do visitInternalName(tcb.`type`)
  ...
  def visitInternalName(internalName: String, offset: Int, length: Int): Unit =
    if internalName != null && containsChar(internalName, offset, length, '$')
    then
      for c <- getClassIfNested(internalName.substring(offset, length)) do
        if !declaredInnerClasses.contains(c) then referredInnerClasses += c
  ...
  // `[Linternal/Name;`.
  def visitInternalNameOrArrayReference(ref: String): Unit = if ref != null then
    val bracket = ref.lastIndexOf('[')
    if bracket == -1 then visitInternalName(ref)
    else if ref.charAt(bracket + 1) == 'L' then
      visitInternalName(ref, bracket + 2, ref.length - 1)
  ...
  def visitDescriptor(desc: String): Unit = (desc.charAt(0): @switch) match
    case '(' =>
      var i = 1
      while i < desc.length do
        if desc.charAt(i) == 'L' then
          val start = i + 1 // skip the L
          var seenDollar = false
          while {
            val ch = desc.charAt(i); seenDollar ||= (ch == '$'); ch != ';'
          } do i += 1
          if seenDollar then visitInternalName(desc, start, i)
  ...
  def visitAnnotation(annot: AnnotationNode): Unit =
    ...
    if annot.values != null then annot.values.asScala foreach visitConstant

  def visitAnnotations(annots: java.util.List[? <: AnnotationNode]) =
    if annots != null then annots.asScala foreach visitAnnotation
  def visitAnnotationss(annotss: Array[java.util.List[AnnotationNode]]) =
    if annotss != null then annotss foreach visitAnnotations

  def visitHandle(handle: Handle): Unit =
    visitInternalNameOrArrayReference(handle.getOwner)
    visitDescriptor(handle.getDesc)
end NestedClassesCollector
