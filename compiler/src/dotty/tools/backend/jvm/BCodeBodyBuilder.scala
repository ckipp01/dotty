...
trait BCodeBodyBuilder extends BCodeSkelBuilder:
  ...
  abstract class PlainBodyBuilder(cunit: CompilationUnit)
        ...
        case Assign(lhs, rhs) =>
          ...
          rhs match
            ...
            case Apply(Select(larg: Ident, nme.SUB), Literal(x) :: Nil)
                if larg.symbol == s && tk.isIntSizedType && Constant(
                  -x.intValue
                ).isShortRange =>
              lineNumber(tree)
              bc.iinc(idx, -x.intValue)
        ...
        args match
          ...
          case Nil =>
            ...
            code match
              ...
              case _ =>
                abort(
                  s"Unknown unary operation: ${fun.symbol.showFullName} code: $code"
                )

          // binary operation
          case rarg :: Nil =>
            val isShift = isShiftOp(code)
            resKind = tpeTK(larg).maxType(if isShift then INT else tpeTK(rarg))

            if isShift || isBitwiseOp(code) then
              assert(
            ...
            genLoad(rarg, if isShift then INT else resKind)
            stackHeight -= resKind.size
    ...
    def genArrayOp(tree: Tree, code: Int, expectedType: BType): BType =
      tree match
        ...
        case Apply(DesugaredSelect(arrayObj, _), args) =>
          ...
          genLoad(arrayObj, k)
          val elementType = typeOfArrayOp.getOrElse[bTypes.BType](
            code,
      ...
      case If(condp, thenp, elsep) =>
        ...
        if dest == LoadDestination.FallThrough then
          if hasElse then
            ...
            def hasUnitBranch =
              (thenKind == UNIT || elseKind == UNIT) && expectedType == UNIT
            val resKind = if hasUnitBranch then UNIT else tpeTK(tree)
          ...
          markProgramPoint(failure)
          if hasElse then genLoadTo(elsep, expectedType, dest)
          else genAdaptAndSendToDest(UNIT, expectedType, dest)
          expectedType
        end if
    ...
    def genPrimitiveOp(tree: Apply, expectedType: BType): BType =
      (tree: @unchecked) match
        case Apply(fun @ DesugaredSelect(receiver, _), _) =>
          ...
          if isArithmeticOp(code) then genArithmeticOp(tree, code)
          else if code == CONCAT then genStringConcat(tree)
          else if code == HASH then genScalaHash(receiver)
          else if isArrayOp(code) then genArrayOp(tree, code, expectedType)
          else if isLogicalOp(code) || isComparisonOp(code) then
    ...
    def genLoadTo(
      ...
      tree match
        case tree @ ValDef(_, _, _) =>
          ...
          if tree.rhs == tpd.EmptyTree then emitZeroOf(tk)
          else genLoad(tree.rhs, tk)
          bc.store(idx, tk)
          val localVarStart = currProgramPoint()
          if !isSynth
          then // there are case <synthetic> ValDef's emitted by patmat
        ...
        case New(tpt) =>
          abort(
            s"Unexpected New(${tpt.tpe.showSummary()}/$tpt) reached GenBCode.\n" +
        ...
        case t @ Closure(env, call, tpt) =>
          ...
          if !fun.symbol.isStaticMember then
          ...
          generatedType = genInvokeDynamicLambda(
            NoSymbol,
            fun.symbol,
            env.size,
            functionalInterface
        ...
        case DesugaredSelect(Ident(nme.EMPTY_PACKAGE), module) =>
          assert(
            ...
            s"Selection of non-module from empty package: $tree sym: ${tree.symbol} at: ${tree.span}"
          )
          genLoadModule(tree)
        ...
        case blck @ Block(stats, expr) =>
          if stats.isEmpty then genLoadTo(expr, expectedType, dest)
          else genBlockTo(blck, expectedType, dest)
          generatedDest = dest
        ...
        case tpd.EmptyTree =>
          if expectedType != UNIT then emitZeroOf(expectedType)
    ...
    def genAdaptAndSendToDest(
        generatedType: BType,
        expectedType: BType,
        dest: LoadDestination
    ): Unit =
      if generatedType != expectedType then adapt(generatedType, expectedType)
      ...
      dest match
        ...
        case LoadDestination.Jump(label, targetStackHeight) =>
          if targetStackHeight < stackHeight then
            ...
            if expectedType == UNIT then bc dropMany stackDiff
            else
        ...
        case LoadDestination.Throw =>
          ...
          assert(
            thrownType.isNullType || thrownType.isNothingType || thrownType.asClassBType
    ...
    def genConstant(const: Constant): Unit =
      (const.tag /*: @switch*/ ) match
        ...
        case ClazzTag =>
          ...
          else mnode.visitLdcInsn(tp.toASMType)

        case _ => abort(s"Unknown constant value: $const")

    private def genLabeledTo(
        tree: Labeled,
        expectedType: BType,
        dest: LoadDestination
    ): BType = tree match
      case Labeled(bind, expr) =>
        ...
        if dest == LoadDestination.FallThrough then
          ...
          registerJumpDest(
            labelSym,
            resKind,
            LoadDestination.Jump(jumpTarget, stackHeight)
          )
    ...
    private def genReturn(r: Return): Unit =
      ...
      val fromSym: Symbol =
        if r.from.symbol.is(LabelFlag) then r.from.symbol else NoSymbol

      if NoSymbol == fromSym then
        ...
        cleanups match
          ...
          case nextCleanup :: rest =>
            ...
            if saveReturnValue then
              // regarding return value, the protocol is: in place of a `return-stmt`, a sequence of `adapt, store, jump` are inserted.
              if earlyReturnVar == null then
                earlyReturnVar = locals.makeLocal(
                  returnType,
                  "earlyReturnVar",
                  expr.tpe,
                  expr.span
    ...
    def genTypeApply(t: TypeApply): BType = (t: @unchecked) match
      case TypeApply(fun @ DesugaredSelect(obj, _), targs) =>
        val sym = fun.symbol
        val cast =
          if sym == defn.Any_isInstanceOf then false
          else if sym == defn.Any_asInstanceOf then true
          else
            abort(
              s"Unexpected type application $fun[sym: ${sym.showFullName}] in: $t"
            )
        val l = tpeTK(obj)
        ...
        if l.isPrimitive && r.isPrimitive then genConversion(l, r, cast)
        else if l.isPrimitive then
          bc drop l
          if cast then
            mnode.visitTypeInsn(
              asm.Opcodes.NEW,
        ...
        else if r.isPrimitive && cast then
          abort(
            s"Erasure should have added an unboxing operation to prevent this cast. Tree: $t"
          )
    ...
    private def mkArrayConstructorCall(
        arr: ArrayBType,
        app: Apply,
        args: List[Tree]
    ) =
      ...
      if argsSize > dims then
        report.error(
          em"too many arguments for array constructor: found ${args.length} but array has only $dims dimension(s)",
          ctx.source.atSpan(app.span)
        )
      if argsSize < dims then
        /* In one step:
         ...
         */
        for i <- args.length until dims do elemKind = ArrayBType(elemKind)
    ...
    private def genApply(app: Apply, expectedType: BType): BType =
      ...
      app match
        case Apply(_, args) if app.symbol eq defn.newArrayMethod =>
          val List(elemClaz, Literal(c: Constant), ArrayValue(_, dims)) =
            args: @unchecked
        ...
        case Apply(fun @ DesugaredSelect(New(tpt), nme.CONSTRUCTOR), args) =>
          ...
          generatedType match
            ...
            case rt: ClassBType =>
              assert(
                classBTypeFromSymbol(ctor.owner) == rt,
                s"Symbol ${ctor.owner.showFullName} is different from $rt"
              )
        ...
        case Apply(fun, List(expr))
            if Erasure.Boxing.isBox(
              fun.symbol
            ) && fun.symbol.denot.owner != defn.UnitModuleClass =>
          ...
          generatedType = boxResultType(
            fun.symbol
          ) // was toTypeKind(fun.symbol.tpe.resultType)

        case Apply(fun, List(expr))
            if Erasure.Boxing.isUnbox(
              fun.symbol
            ) && fun.symbol.denot.owner != defn.UnitModuleClass =>
          genLoad(expr)
          val boxType = unboxResultType(
            fun.symbol
          ) // was toTypeKind(fun.symbol.owner.linkedClassOfClass.tpe)
          ...
          if isPrimitive(fun) then // primitive method call
            ...
            val invokeStyle =
              if sym.isStaticMember then InvokeStyle.Static
                ...
                InvokeStyle.Special
              else if app.hasAttachment(BCodeHelpers.UseInvokeSpecial) then
                InvokeStyle.Special
            ...
            val DesugaredSelect(qual, name) =
              fun: @unchecked // fun is a Select, also checked in genLoadQualifier
            val isArrayClone =
              name == nme.clone_ && qual.tpe.widen.isInstanceOf[JavaArrayType]
        ...
        val (generatedType, postMatch, postMatchDest) =
          if dest == LoadDestination.FallThrough then
            ...
            (
              tpeTK(tree),
              postMatch,
              LoadDestination.Jump(postMatch, stackHeight)
            )
          else (expectedType, null, dest)

        // Only two possible selector types exist in `Match` trees at this point: Int and String
        if tpeTK(selector) == INT then
          ...
          for caze @ CaseDef(pat, guard, body) <- cases do
            ...
            pat match
              ...
              case Ident(nme.WILDCARD) =>
                assert(
                  default == null,
                  s"multiple default targets in a Match node, at ${tree.span}"
                )
              ...
              case Alternative(alts) =>
                alts foreach {
                  ...
                  case _ =>
                    abort(
                      s"Invalid alternative in alternative pattern in Match node: $tree at: ${tree.span}"
                    )
                }
              case _ =>
          ...
          bc.emitSWITCH(
            mkArrayReverse(flatKeys),
            mkArrayL(targets.reverse),
            default,
            MIN_SWITCH_DENSITY
          )

          // emit switch-blocks.
          for sb <- switchBlocks.reverse do
          ...
          val casesByHash =
            SortedMap.empty[Int, List[(String, Either[asm.Label, Tree])]]
          ...
          var targets: List[asm.Label] = Nil
          var hashBlocks
              : List[(asm.Label, List[(String, Either[asm.Label, Tree])])] = Nil
          for (hashValue, hashCases) <- casesByHash do
            val switchBlockPoint = new asm.Label
          ...
          for (hashLabel, caseAlternatives) <- hashBlocks.reverse do
            markProgramPoint(hashLabel)
            for (caseString, indirectLblOrBody) <- caseAlternatives do
              val comparison =
                if caseString == null then defn.Any_== else defn.Any_equals
              val condp = Literal(Constant(caseString))
                .select(defn.Any_==)
                .appliedTo(tree.selector)
              ...
              indirectLblOrBody match
                ...
                case Right(caseBody) =>
                  ...
                  genCond(
                    ...
                    thisCaseMatches,
                    keepGoing,
                    targetIfNoJump = thisCaseMatches
                  )
    ...
    /** Add entries to the `LocalVariableTable` JVM attribute for all the vars
      * in `varsInScope`, ending at the current program point.
      */
    def emitLocalVarScopes(): Unit =
      if emitVars then
        val end = currProgramPoint()
        for (sym, start) <- varsInScope.reverse do
          emitLocalVarScope(sym, start, end)
    end emitLocalVarScopes

    def adapt(from: BType, to: BType): Unit =
      if !from.conformsTo(to) then
        to match
          case UNIT => bc drop from
          case _    => bc.emitT2T(from, to)
      else if from.isNothingType then
         ...
         */
        if lastInsn.getOpcode != asm.Opcodes.ATHROW then
          emit(asm.Opcodes.ATHROW)
      else if from.isNullType then
         ...
         */
        if lastInsn.getOpcode != asm.Opcodes.ACONST_NULL then
          bc drop from
    ...
    def genLoadModule(tree: Tree): BType =
      val module =
        (
          if !tree.symbol.is(PackageClass) then tree.symbol
          else
            tree.symbol.info.member(nme.PACKAGE).symbol match
              case NoSymbol =>
    ...
    def genStringConcat(tree: Tree): BType =
      ...
      liftStringConcat(tree) match
        ...
        case concatenations =>
          val concatArguments = concatenations.view
            .filter {
              case Literal(Constant("")) =>
                false // empty strings are no-ops in concatenation
              case _ => true
            }
            .map {
          ...
          if backendUtils.classfileVersion < asm.Opcodes.V9 then
            ...
            val TagArg =
              '\u0001' // indicates a hole (in the recipe string) for an argument
            val TagConst =
              '\u0002' // indicates a hole (in the recipe string) for a constant
            ...
            for elem <- concatArguments do
              ...
              // Unlikely spill case
              if totalArgSlots + elemSlots >= MaxIndySlots then
              ...
              elem match
                case Literal(Constant(s: String)) =>
                  if s.contains(TagArg) || s.contains(TagConst) then
                    totalArgSlots += elemSlots
                    recipe.append(TagConst)
                    constVals += s
                  else recipe.append(s)
    ...
    /** Generate a method invocation. If `specificReceiver != null`, it is used
      * as receiver in the invocation instruction, otherwise `method.owner`. A
      * specific receiver class is needed to prevent an IllegalAccessError,
      * (aladdin bug 455).
      */
    def genCallMethod(
        method: Symbol,
        style: InvokeStyle,
        pos: Span = NoSpan,
        specificReceiver: Symbol = null
    ): BType =
      val methodOwner = method.owner

      // the class used in the invocation's method descriptor in the classfile
      val receiverClass =
        if specificReceiver != null then
          assert(
            style.isVirtual || specificReceiver == methodOwner,
            s"specificReceiver can only be specified for virtual calls. $method - $specificReceiver"
          )

        val useSpecificReceiver =
          specificReceiver != null && !defn.isBottomClass(
            specificReceiver
          ) && !method.isScalaStatic
        val receiver =
          if useSpecificReceiver then specificReceiver else methodOwner
        ...
        val isTraitMethodOverridingObjectMember =
          receiver != methodOwner && // fast path - the boolean is used to pick either of these two, if they are the same it does not matter
            ...
            defn.ObjectType
              .decl(method.name)
              .symbol
              .exists && { // fast path - compute overrideChain on the next line only if necessary
              val syms = method.allOverriddenSymbols.toList
              !syms.isEmpty && syms.last.owner == defn.ObjectClass
            }
        if isTraitMethodOverridingObjectMember then methodOwner else receiver
      end receiverClass
      ...
      import InvokeStyle.*
      if style == Super then
        if isInterface && !method.is(JavaDefined) then
          ...
          val staticDesc = MethodBType(
            ownerBType :: bmType.argumentTypes,
            bmType.returnType
          ).descriptor
    ...
    def genScalaHash(tree: Tree): BType =
      ...
      genCallMethod(
        NoSymbol,
        InvokeStyle.Static
      ) // used to dispatch ## on primitives to ScalaRuntime.hash. Should be implemented by a miniphase
    ...
    def liftStringConcat(tree: Tree): List[Tree] = tree match
      case tree @ Apply(fun @ DesugaredSelect(larg, method), rarg) =>
        ...
        then liftStringConcat(larg) ::: rarg
        else tree :: Nil
      case _ =>
        tree :: Nil
    ...
    private def genCJUMP(
      ...
      if targetIfNoJump == success then
        genCJUMP(
          failure,
          success,
          op.negate(),
        ...
        if tk.isIntSizedType then // BOOL, BYTE, CHAR, SHORT, or INT
          ...
          (tk: @unchecked) match
            ...
            case FLOAT =>
              emit(if useCmpG then asm.Opcodes.FCMPG else asm.Opcodes.FCMPL)
            case DOUBLE =>
    ...
    private def genCond(
      ...
      def genComparisonOp(l: Tree, r: Tree, code: Int): Unit =
        ...
        def ifOneIsNull(l: Tree, r: Tree): Tree =
          if isNull(l) then r else if isNull(r) then l else null
        val nonNullSide =
          if ScalaPrimitivesOps.isReferenceEqualityOp(code) then
            ifOneIsNull(l, r)
          else null
        if nonNullSide != null then
      ...
      tree match
        ...
        case tree @ Apply(fun, args) if primitives.isPrimitive(fun.symbol) =>
          ...
          val rhs =
            if args.isEmpty then tpd.EmptyTree
          ...
          primitives.getPrimitive(fun.symbol) match
            ...
            case code =>
              if ScalaPrimitivesOps.isUniversalEqualityOp(code) && tpeTK(
                  lhs
                ).isClass
              then
                // rewrite `==` to null tests and `equals`. not needed for arrays (`equals` is reference equality).
                if code == EQ then
                  genEqEqPrimitive(lhs, rhs, success, failure, targetIfNoJump)
                else
                  genEqEqPrimitive(lhs, rhs, failure, success, targetIfNoJump)
              else if ScalaPrimitivesOps.isComparisonOp(code) then
    ...
    def genEqEqPrimitive(
      ...
      val mustUseAnyComparator: Boolean =
        val areSameFinals = l.tpe.typeSymbol.is(Final) && r.tpe.typeSymbol.is(
          Final
        ) && (l.tpe =:= r.tpe)
        // todo: remove
        ...
        !areSameFinals && isMaybeBoxed(l.tpe.typeSymbol) && isMaybeBoxed(
          r.tpe.typeSymbol
        )
      def isNull(t: Tree): Boolean = t match
        case Literal(Constant(null)) => true
      ...
      if mustUseAnyComparator then
        val equalsMethod: Symbol =
          if l.tpe <:< defn.BoxedNumberClass.info then
            if r.tpe <:< defn.BoxedNumberClass.info then
              defn.BoxesRunTimeModule.requiredMethod(nme.equalsNumNum)
            else if r.tpe <:< defn.BoxedCharClass.info then
    ...
    def genInvokeDynamicLambda(
      ...
      val isSerializable =
        functionalInterface.isSerializable || defn.isFunctionClass(
          functionalInterface
        )
      val isInterface = isEmittedInterface(lambdaTarget.owner)
      val invokeStyle =
        if lambdaTarget.isStaticMember then asm.Opcodes.H_INVOKESTATIC
        else if lambdaTarget.is(Private) || lambdaTarget.isClassConstructor then
          asm.Opcodes.H_INVOKESPECIAL
        else if isInterface then asm.Opcodes.H_INVOKEINTERFACE
      ...
      if invokeStyle != asm.Opcodes.H_INVOKESTATIC then
        capturedParamsTypes = lambdaTarget.owner.info :: capturedParamsTypes
      ...
      val desc = capturedParamsTypes
        .map(tpe => toTypeKind(tpe))
        .mkString(("("), "", ")") + functionalInterfaceDesc
      // TODO specialization
      val instantiatedMethodType = new MethodBType(
        lambdaParamTypes.map(p => toTypeKind(p)),
        toTypeKind(lambdaTarget.info.resultType)
      ).toASMType

      val samMethod = atPhase(erasurePhase) {
        val samMethods =
          toDenot(functionalInterface).info.possibleSamMethods.toList
        samMethods match
          case x :: Nil => x.symbol
          case Nil =>
            abort(
              s"${functionalInterface.show} is not a functional interface. It doesn't have abstract methods"
            )
          case xs =>
            abort(
              s"${functionalInterface.show} is not a functional interface. " +
      ...
      val overriddenMethodTypes =
        bridgeMethods.map(b => asmMethodType(b).toASMType)
      ...
      def flagIf(b: Boolean, flag: Int): Int = if b then flag else 0
      val flags = flagIf(isSerializable, FLAG_SERIALIZABLE) | flagIf(
        needsBridges,
        FLAG_BRIDGES
      )

      val bsmArgs0 = Seq(samMethodType, targetHandle, instantiatedMethodType)
      val bsmArgs1 = if flags != 0 then Seq(Int.box(flags)) else Seq.empty
      val bsmArgs2 =
        if needsBridges then bridgeTypes.length +: bridgeTypes else Seq.empty
      ...
      val metafactory =
        ...
        else jliLambdaMetaFactoryMetafactoryHandle

      bc.jmethod.visitInvokeDynamicInsn(methodName, desc, metafactory, bsmArgs*)

      generatedType
    end genInvokeDynamicLambda
  end PlainBodyBuilder
  ...
  /** Does this symbol actually correspond to an interface that will be emitted?
    ...
    * also returns true for the symbols of the fake companion objects we create
    * for Java-defined classes as well as for Java annotations which we
    * represent as classes.
    */
  private def isEmittedInterface(sym: Symbol): Boolean = sym.isInterface ||
    sym.is(JavaDefined) && (toDenot(sym).isAnnotation || sym.is(
      ModuleClass
    ) && (sym.companionClass.is(PureInterface)) || sym.companionClass.is(Trait))
end BCodeBodyBuilder
