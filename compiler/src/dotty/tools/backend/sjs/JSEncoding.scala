...
import org.scalajs.ir.{Trees as js, Types as jstpe}
...
/** Encoding of symbol names for JavaScript
  *
  * Some issues that this encoding solves: * Overloading: encode the full
  * signature in the JS name * Same scope for fields and methods of a class *
...
object JSEncoding:
  ...
  private val dynamicImportForwarderSimpleName = SimpleMethodName(
    "dynamicImport$"
  )
  ...
  class LocalNameGenerator:
    ...
    def freshLocalIdent(base: LocalName)(implicit
        pos: ir.Position
    ): js.LocalIdent =
      js.LocalIdent(freshName(base))

    def freshLocalIdent(base: String)(implicit
        pos: ir.Position
    ): js.LocalIdent =
      freshLocalIdent(LocalName(base))

    def freshLocalIdent(base: TermName)(implicit
        pos: ir.Position
    ): js.LocalIdent =
    ...
    def freshLabelIdent(base: String)(implicit
        pos: ir.Position
    ): js.LabelIdent =
    ...
    def getEnclosingReturnLabel()(implicit pos: ir.Position): js.LabelIdent =
      if returnLabelName.isEmpty then
     ...
     */
    def makeLabeledIfRequiresEnclosingReturn(tpe: jstpe.Type)(body: js.Tree)(
  ...
  def encodeLabelSym(sym: Symbol)(implicit
      ctx: Context,
      pos: ir.Position,
    ...
    require(
      sym.is(Flags.Label),
      "encodeLabelSym called with non-label symbol: " + sym
    )
  ...
  def encodeFieldSymAsStringLiteral(
      sym: Symbol
  )(implicit ctx: Context, pos: ir.Position): js.StringLiteral =
    js.StringLiteral(encodeFieldSymAsString(sym))

  private def encodeFieldSymAsString(sym: Symbol)(using Context): String =
    require(
      sym.owner.isClass && sym.isTerm && !sym.isOneOf(MethodOrModule),
      "encodeFieldSym called with non-field symbol: " + sym
    )

    val name0 = sym.javaSimpleName
    if name0.charAt(name0.length() - 1) != ' ' then name0
  ...
  def encodeMethodSym(sym: Symbol, reflProxy: Boolean = false)(implicit
    ...
    val hasExplicitThisParameter =
      !sym.is(JavaStatic) && sym.owner.isNonNativeJSClass
    val paramTypeRefs =
      if !hasExplicitThisParameter then paramTypeRefs0
    ...
    val methodName =
      if sym.isClassConstructor then MethodName.constructor(paramTypeRefs)
      else if reflProxy then
        MethodName.reflectiveProxy(simpleName, paramTypeRefs)
      else
        MethodName(
          simpleName,
          paramTypeRefs,
          paramOrResultTypeRef(patchedResultType(sym))
        )

    js.MethodIdent(methodName)
  end encodeMethodSym

  def encodeJSNativeMemberSym(
      sym: Symbol
  )(using Context, ir.Position): js.MethodIdent =
    require(
      sym.hasAnnotation(jsdefn.JSNativeAnnot),
      "encodeJSNativeMemberSym called with non-native symbol: " + sym
    )
    if sym.is(Method) then encodeMethodSym(sym)
    else encodeFieldSymAsMethod(sym)

  def encodeStaticMemberSym(
      sym: Symbol
  )(using Context, ir.Position): js.MethodIdent =
    require(
      sym.is(Flags.JavaStaticTerm),
      "encodeStaticMemberSym called with non-static symbol: " + sym
    )
    encodeFieldSymAsMethod(sym)

  private def encodeFieldSymAsMethod(
      sym: Symbol
  )(using Context, ir.Position): js.MethodIdent =
    val name = sym.name
  ...
  def encodeDynamicImportForwarderIdent(
      params: List[Symbol]
  )(using Context, ir.Position): js.MethodIdent =
  ...
  def encodeLocalSym(sym: Symbol)(implicit
      ctx: Context,
      pos: ir.Position,
      localNames: LocalNameGenerator
  ): js.LocalIdent =
    require(
      !sym.owner.isClass && sym.isTerm && !sym.is(Flags.Method) && !sym.is(
        Flags.Module
      ),
  ...
  def encodeClassName(sym: Symbol)(using Context): ClassName =
     ...
     */
    if sym1 == defn.BoxedUnitClass then ir.Names.BoxedUnitClass
    else if sym1 == defn.NothingClass then ScalaRuntimeNothingClassName
    else if sym1 == defn.NullClass then ScalaRuntimeNullClassName
    else ClassName(sym1.javaClassName)
  ...
  private def toIRTypeInternal(typeRefInternal: (jstpe.TypeRef, Symbol))(using
      Context
  ): jstpe.Type =
    typeRefInternal._1 match
      ...
      case typeRef: jstpe.ClassRef =>
        ...
        if sym == defn.ObjectClass || sym.isJSType then jstpe.AnyType
        else if sym == defn.NothingClass then jstpe.NothingType
        else if sym == defn.NullClass then jstpe.NullType
        else jstpe.ClassType(typeRef.className)
  ...
  private def toTypeRefInternal(tp: Type)(using
      Context
  ): (jstpe.TypeRef, Symbol) =
    def primitiveOrClassToTypeRef(sym: Symbol): (jstpe.TypeRef, Symbol) =
      ...
      val typeRef =
        if sym.isPrimitiveValueClass then
          if sym == defn.UnitClass then jstpe.VoidRef
          else if sym == defn.BooleanClass then jstpe.BooleanRef
          else if sym == defn.CharClass then jstpe.CharRef
          else if sym == defn.ByteClass then jstpe.ByteRef
          else if sym == defn.ShortClass then jstpe.ShortRef
          else if sym == defn.IntClass then jstpe.IntRef
          else if sym == defn.LongClass then jstpe.LongRef
          else if sym == defn.FloatClass then jstpe.FloatRef
          else if sym == defn.DoubleClass then jstpe.DoubleRef
          else throw new Exception(s"unknown primitive value class $sym")
        else encodeClassRef(sym)
      (typeRef, sym)
    ...
    tp.widenDealias match
      ...
      case t: TypeRef =>
        if !t.symbol.isClass then
          nonClassTypeRefToTypeRef(
            t.symbol
          ) // See comment on nonClassTypeRefToBType
        else
          primitiveOrClassToTypeRef(
            t.symbol
  ...
  def originalNameOfLocal(
    ...
    if UTF8String.equals(originalName, irName.encoded) then NoOriginalName
  ...
  private def originalNameOf(name: Name): OriginalName =
    ...
    if originalName == name.mangledString then NoOriginalName
    else OriginalName(originalName)
end JSEncoding
