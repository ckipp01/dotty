...
/** Main codegen for Scala.js IR.
  *
  * [[GenSJSIR]] creates one instance of `JSCodeGen` per compilation unit. The
  * `run()` method processes the whole compilation unit and generates `.sjsir`
  * files for it.
  *
...
class JSCodeGen()(using genCtx: Context):
  ...
  private val generatedStaticForwarderClasses =
    mutable.ListBuffer.empty[(Symbol, js.ClassDef)]
  ...
  /** Returns a new fresh local identifier. */
  private def freshLocalIdent(base: TermName)(implicit
      pos: Position
  ): js.LocalIdent =
    localNames.get.freshLocalIdent(base)
  ...
  /** Generates the Scala.js IR for a compilation unit This method iterates over
    * all the class and interface definitions found in the compilation unit and
    * emits their IR (.sjsir).
    *
    * Some classes are never actually emitted:
    *   - Classes representing primitive types
    *   - The scala.Array class
    *
    * TODO Some classes representing anonymous functions are not actually
    * emitted. Instead, a temporary representation of their `apply` method is
    * built and recorded, so that it can be inlined as a JavaScript anonymous
    ...
    * `genRawJSClassData()` * Interface -> `genInterface()` * Normal class ->
    * `genClass()`
    */
  private def genCompilationUnit(cunit: CompilationUnit): Unit =
     ...
     */
    for typeDef <- allTypeDefs do
      if typeDef.symbol.is(ModuleClass) then
        typeDef.symbol.info.decls.foreach { f =>
          if f.isField && f.is(Module) then f.setFlag(JavaStatic)
        }

    val (anonJSClassTypeDefs, otherTypeDefs) =
      allTypeDefs.partition(td =>
        td.symbol.isAnonymousClass && td.symbol.isJSType
      )

    // Record the TypeDefs of anonymous JS classes to be lazily generated
    for td <- anonJSClassTypeDefs do lazilyGeneratedAnonClasses(td.symbol) = td

    /* Finally, we emit true code for the remaining class defs. */
    for td <- otherTypeDefs do
      val sym = td.symbol
      ...
      if !isPrimitive then
        withScopedVars(
          ...
          val tree =
            if sym.isJSType then
              if !sym.is(Trait) && sym.isNonNativeJSClass then
                genNonNativeJSClass(td)
              else genRawJSClassData(td)
            else if sym.is(Trait) then genInterface(td)
            else genScalaClass(td)

          generatedClasses += tree
        }

    for tree <- generatedClasses do genIRFile(cunit, tree)

    if generatedStaticForwarderClasses.nonEmpty then
      ...
      for (site, classDef) <- generatedStaticForwarderClasses do
        if !generatedCaseInsensitiveNames.contains(
            caseInsensitiveNameOf(classDef)
          )
        then genIRFile(cunit, classDef)
        else
          report.warning(
  ...
  private def genScalaClass(td: TypeDef): js.ClassDef =
    ...
    if hasDefaultCtorArgsAndJSModule(sym) then
      report.error(
    ...
    val shouldMarkInline = (isDynamicImportThunk ||
    ...
    for tree <- tpl.constr :: tpl.body do
      tree match
        case EmptyTree => ()
        ...
        case vd: ValDef =>
          ...
          val sym = vd.symbol
          if !sym.is(Module) && sym.hasAnnotation(jsdefn.JSNativeAnnot) then
        ...
        case dd: DefDef =>
          ...
          else generatedNonFieldMembers ++= genMethod(dd)

        case _ =>
          throw new FatalError(
            "Illegal tree in body of genScalaClass(): " + tree
          )
    ...
    val optStaticInitializer =
      ...
      val staticModuleInit =
        if !needsStaticModuleInit then Nil
        else List(genLoadModule(sym))

      val staticInitializerStats = reflectInit ::: staticModuleInit
      if staticInitializerStats.nonEmpty then
        List(
          genStaticConstructorWithStats(
    ...
    val optDynamicImportForwarder =
      if isDynamicImportThunk then List(genDynamicImportForwarder(sym))
      else Nil
    ...
    val allMemberDefs =
      if !isCandidateForForwarders(sym) then allMemberDefsExceptStaticForwarders
      else
        if isStaticModule(sym) then
          ...
          if !sym.linkedClass.exists then
            ...
            if forwarders.nonEmpty then
              val forwardersClassDef = js.ClassDef(
                js.ClassIdent(
                  ClassName(classIdent.name.nameString.stripSuffix("$"))
                ),
                originalName,
                ClassKind.Class,
                None,
    ...
    val kind =
      if isStaticModule(sym) then ClassKind.ModuleClass
      else if isHijacked then ClassKind.HijackedClass
  ...
  private def genNonNativeJSClass(td: TypeDef): js.ClassDef =
      ...
      tree match
        ...
        case dd: DefDef =>
          ...
          if sym.isClassConstructor then constructorTrees += dd
            ...
            if exposed && !sym.is(Deferred) then
        ...
        case _ =>
          throw new FatalError(
            "Illegal tree in gen of genNonNativeJSClass(): " + tree
          )
    end for
    ...
    val staticMembers =
      ...
      if !module.exists then Nil
        ...
        if exports.exists(_.isInstanceOf[js.JSFieldDef]) then
    ...
    val (generatedConstructor, jsClassCaptures) = withNewLocalNameScope {
      ...
      if isNested then localNames.reserveLocalName(JSSuperClassParamName)

      val (captures, ctor) =
        genJSClassCapturesAndConstructor(constructorTrees.toList)

      val jsClassCaptures = if isNested then
    ...
    val generatedMembers =
      genClassFields(td) :::
        ...
        jsExportsGen.genJSClassDispatchers(
          sym,
    ...
    // The complete class definition
    val kind =
      if isStaticModule(sym) then ClassKind.JSModuleClass
  ...
  private def genRawJSClassData(td: TypeDef): js.ClassDef =
    ...
    val classIdent = encodeClassNameIdent(sym)
    val kind =
      if sym.is(Trait) then ClassKind.AbstractJSType
      else if sym.is(ModuleClass) then ClassKind.NativeJSModuleClass
      else ClassKind.NativeJSClass
    val superClass =
      if sym.is(Trait) then None
  ...
  private def genInterface(td: TypeDef): js.ClassDef =
    ...
    for tree <- tpl.constr :: tpl.body do
      tree match
        ...
        case _ =>
          throw new FatalError(i"""Illegal tree in gen of genInterface(): $tree
               |class = $td
    ...
    val allMemberDefs =
      if !isCandidateForForwarders(sym) then genMethodsList
      else
        genMethodsList ::: genStaticForwardersForClassOrInterface(
          genMethodsList,
          sym
        )
  ...
  private def genClassInterfaces(sym: ClassSymbol, forJSClass: Boolean)(implicit
      pos: Position
  ): List[js.ClassIdent] =
    for
      intf <- sym.directlyInheritedTraits
      if !(forJSClass && intf == defn.DynamicClass)
    yield encodeClassNameIdent(intf)
  ...
  /** Is the given Scala class, interface or module class a candidate for static
    ...
    *     - the flag `-scalajsGenStaticForwardersForNonTopLevelObjects` is set
    *       to true, or
  ...
  def genStaticForwardersForClassOrInterface(
      existingMembers: List[js.MemberDef],
      sym: Symbol
  ...
  /** Gen the static forwarders for the methods of a module class.
    ...
    */
  def genStaticForwardersFromModuleClass(
      existingMembers: List[js.MemberDef],
      moduleClass: Symbol
  )(implicit pos: SourcePosition): List[js.MemberDef] =
    ...
    val members =
      moduleClass.info
        .membersBasedOnFlags(
          ...
          excluded = Flags.ExcludedForwarder
        )
        .map(_.symbol)
    ...
    def isExcluded(m: Symbol): Boolean =
      ...
      def isDefaultParamOfJSNativeDef: Boolean =
        m.name.is(DefaultGetterName) && {
          ...
          !info.isForConstructor && info.attachedMethod.hasAnnotation(
            jsdefn.JSNativeAnnot
          )
        }
      ...
      || m.hasAnnotation(
        jsdefn.JSNativeAnnot
      ) || isDefaultParamOfJSNativeDef // #4557

    val forwarders = for
      m <- members
      if !isExcluded(m)
    yield withNewLocalNameScope {
      val flags =
      ...
      val jsParams =
        for (paramName, paramInfo) <- m.info.paramNamess.flatten
            .zip(m.info.paramInfoss.flatten)
        yield js.ParamDef(
          freshLocalIdent(paramName),
          NoOriginalName,
          toIRType(paramInfo),
          mutable = false
        )
      ...
      if existingPublicStaticMethodNames.contains(methodIdent.name) then
        report.error(
    ...
    forwarders.toList
  end genStaticForwardersFromModuleClass
  ...
  private def genClassFields(td: TypeDef): List[js.MemberDef] =
    ...
    classSym.info.decls
      ...
      .flatMap({ f =>
        ...
        assert(
          !(isTopLevelExport && isJavaStatic),
          em"found ${f.fullName} which is both a top-level export and a Java static"
        )
        val isStaticField = isTopLevelExport || isJavaStatic

        val namespace =
          if isStaticField then js.MemberNamespace.PublicStatic
          else js.MemberNamespace.Public
        val mutable = isStaticField || f.is(Mutable)

        val flags =
          js.MemberFlags.empty.withMutable(mutable).withNamespace(namespace)

        val irTpe0 =
          if isJSClass then genExposedFieldIRType(f)
          else if isTopLevelExport then jstpe.AnyType
          else toIRType(f.info)

        // scala-js/#4370 Fields cannot have type NothingType
        val irTpe =
          if irTpe0 == jstpe.NothingType then encodeClassType(defn.NothingClass)
          else irTpe0

        if isJSClass && f.isJSExposed then
  ...
  private def genRegisterReflectiveInstantiation(
      sym: Symbol
  )(implicit pos: SourcePosition): Option[js.Tree] =
    if isStaticModule(sym) then
      genRegisterReflectiveInstantiationForModuleClass(sym)
    else if sym.is(ModuleClass) then None // scala-js#3228
    else if sym.is(Lifted) && !sym.originalOwner.isClass then
      None // scala-js#3227
    else genRegisterReflectiveInstantiationForNormalClass(sym)
  ...
  private def genRegisterReflectiveInstantiationForNormalClass(sym: Symbol)(
    ...
    val ctors =
      if sym.is(Abstract) then Nil
      else
        sym.info
          .member(nme.CONSTRUCTOR)
          .alternatives
          .map(_.symbol)
          .filter(m => !m.isOneOf(Private | Protected))

    if ctors.isEmpty then None
    else {
      val constructorsInfos =
        for ctor <- ctors
        yield withNewLocalNameScope {
          val (parameterTypes, formalParams, actualParams) =
            (for (paramName, paramInfo) <- ctor.info.paramNamess.flatten.zip(
                ctor.info.paramInfoss.flatten
              )
            yield
              val paramType = js.ClassOf(toTypeRef(paramInfo))
              val paramDef = js.ParamDef(
                freshLocalIdent(paramName),
                NoOriginalName,
                jstpe.AnyType,
                mutable = false
  ...
  end genRegisterReflectiveInstantiationForNormalClass

  // Constructor of a non-native JS class ------------------------------------

  def genJSClassCapturesAndConstructor(constructorTrees: List[DefDef])(implicit
      pos: SourcePosition
  ): (List[js.ParamDef], js.JSConstructorDef) =
    ...
    val paramVarDefs =
      for vparam <- constructorTrees.flatMap(_.paramss.flatten)
      yield
        val sym = vparam.symbol
        val tpe = toIRType(sym.info)
        js.VarDef(
          encodeLocalSym(sym),
          originalNameOfLocal(sym),
          tpe,
          mutable = true,
          jstpe.zeroOf(tpe)
        )(vparam.span)
    ...
    val (exports, jsClassCaptures) =
      ...
      def add(tree: ConstructorTree[? <: JSCtor]): Unit =
        val (e, c) = genJSClassCtorDispatch(
          tree.ctor.sym,
          tree.ctor.paramsAndInfo,
          tree.overloadNum
        )
        exports += e
        jsClassCaptures ++= c
    ...
    val (formalArgs, restParam, overloadDispatchBody) =
      jsExportsGen.genOverloadDispatch(
        JSName.Literal("constructor"),
        exports,
        jstpe.IntType
      )

    val overloadVar = js.VarDef(
      freshLocalIdent("overload"),
      NoOriginalName,
      jstpe.IntType,
      mutable = false,
      overloadDispatchBody
    )
  ...
  end genJSClassCapturesAndConstructor
    ...
    withPerMethodBodyState(sym) {
      stats.foreach {
        case tree @ Apply(fun @ Select(Super(This(_), _), _), args)
          ...
          assert(
            jsSuperCall.isEmpty,
            s"Found 2 JS Super calls at ${dd.sourcePos}"
          )
          implicit val pos: Position = tree.span
          jsSuperCall =
            Some(js.JSSuperConstructorCall(genActualJSArgs(fun.symbol, args)))

        case stat =>
          val jsStat = genStat(stat)
          assert(
            jsSuperCall.isDefined || !jsStat.isInstanceOf[js.VarDef],
  ...
  private def genSecondaryJSClassCtor(dd: DefDef): SplitSecondaryJSCtor =
    ...
    withPerMethodBodyState(sym) {
      stats.foreach {
        ...
        case stat =>
          val jsStat = genStat(stat)
          if thisCall.isEmpty then beforeThisCall += jsStat
          else afterThisCall += jsStat
      }
    }
    ...
    new SplitSecondaryJSCtor(
      sym,
      genParamsAndInfo(sym, dd.paramss),
      beforeThisCall.result(),
      targetCtor,
      ctorArgs,
      afterThisCall.result()
    )
  end genSecondaryJSClassCtor

  private def genParamsAndInfo(
      ctorSym: Symbol,
      vparamss: List[ParamClause]
  ): List[(Symbol, JSParamInfo)] =
    implicit val pos: SourcePosition = ctorSym.sourcePos

    val paramSyms =
      if vparamss.isEmpty then Nil else vparamss.head.map(_.symbol)
    paramSyms.zip(ctorSym.jsParamInfos)

  private def genJSClassCtorDispatch(
      ctorSym: Symbol,
      allParamsAndInfos: List[(Symbol, JSParamInfo)],
      overloadNum: Int
  ): (jsExportsGen.Exported, List[js.ParamDef]) =
     ...
     */
    val (jsClassCaptures, captureAssigns) =
      (for (param, info) <- captureParamsAndInfos
      yield
        val ident = freshLocalIdent(param.name.toTermName)
        val jsClassCapture =
          js.ParamDef(
            ident,
            originalNameOfLocal(param),
            toIRType(info.info),
            mutable = false
          )
        val captureAssign =
    ...
    val jsExport = new jsExportsGen.Exported(ctorSym, normalInfos):
      def genBody(
          formalArgsRegistry: jsExportsGen.FormalArgsRegistry
      ): js.Tree =
        val paramAssigns =
          for ((param, info), i) <- normalParamsAndInfos.zipWithIndex
          yield
            val rhs = jsExportsGen.genScalaArg(
              this,
              i,
              formalArgsRegistry,
              info,
              ...
              normalParamsAndInfos
                .take(prevArgsCount)
                .map(pi => genVarRef(pi._1))
            )
  ...
  end genJSClassCtorDispatch

  /** Generates a JS constructor body based on a constructor tree. */
  private def genJSClassCtorBody(
      overloadVar: js.VarRef,
      ctorTree: ConstructorTree[PrimaryJSCtor]
  )(implicit pos: Position): js.JSConstructorBody =
     ...
     */
    def ifOverload(tree: ConstructorTree[?], body: js.Tree): js.Tree =
      body match
    ...
    def preStats(
        tree: ConstructorTree[SplitSecondaryJSCtor],
        nextParamsAndInfo: List[(Symbol, JSParamInfo)]
    ): js.Tree =
      val inner = tree.subCtors.map(preStats(_, tree.ctor.paramsAndInfo))

      assert(
        tree.ctor.ctorArgs.size == nextParamsAndInfo.size,
      ...
      val captureAssigns =
        for ((param, _), arg) <- captureParamsInfosAndArgs
        yield js.Assign(genVarRef(param), arg)

      val normalAssigns =
        for (((param, info), arg), i) <- normalParamsInfosAndArgs.zipWithIndex
        yield
          val newArg = arg match
            case js.Transient(UndefinedParam) =>
              ...
              jsExportsGen.genCallDefaultGetter(
                tree.ctor.sym,
                i,
                static = false,
                captures =
                  captureParamsInfosAndArgs.map(p => genVarRef(p._1._1))
              )(prevArgsCount =>
                normalParamsInfosAndArgs
                  .take(prevArgsCount)
                  .map(p => genVarRef(p._1._1))
              )
  ...
  end genJSClassCtorBody

  private def wrapJSCtorBody(
      before: List[js.Tree],
      body: js.JSConstructorBody,
      after: List[js.Tree]
  ): js.JSConstructorBody =
    js.JSConstructorBody(
      before ::: body.beforeSuper,
      body.superCall,
      body.afterSuper ::: after
    )(body.pos)
  ...
  private class PrimaryJSCtor(
      val sym: Symbol,
      val paramsAndInfo: List[(Symbol, JSParamInfo)],
      val body: js.JSConstructorBody
  ) extends JSCtor

  private class SplitSecondaryJSCtor(
      val sym: Symbol,
      val paramsAndInfo: List[(Symbol, JSParamInfo)],
      val beforeCall: List[js.Tree],
      val targetCtor: Symbol,
      val ctorArgs: List[js.Tree],
  ...
  /** Gen JS code for a method definition in a class or in an impl class. On the
    * JS side, method names are mangled to encode the full signature of the
    * Scala method, as described in `JSEncoding`, to support overloading.
    *
    ...
    */
  private def genMethodWithCurrentLocalNameScope(
      dd: DefDef
  ): Option[js.MethodDef] =
    ...
    withPerMethodBodyState(sym) {
      ...
      val params = if vparamss.isEmpty then Nil else vparamss.head.map(_.symbol)
      ...
      if primitives.isPrimitive(sym) then None
      else if sym.is(Deferred) && currentClassSym.isNonNativeJSClass then
        ...
        Some(
          js.MethodDef(
            ...
            originalName,
            jsParams,
            toIRType(patchedResultType(sym)),
            None
          )(OptimizerHints.empty, None)
        )
      else if isIgnorableDefaultParam then
        // #11592
        None
      else if sym.is(Bridge) && sym.name.is(
          DefaultGetterName
        ) && currentClassSym.isNonNativeJSClass
      then
      ...
      } else*/
        {
          ...
          val methodDef =
            if sym.isClassConstructor then
              ...
              val namespace =
                if isMethodStaticInIR(sym) then
                  if sym.isPrivate then js.MemberNamespace.PrivateStatic
                  else js.MemberNamespace.PublicStatic
                else
                  if sym.isPrivate then js.MemberNamespace.Private
                  else js.MemberNamespace.Public
  ...
  end genMethodWithCurrentLocalNameScope
  ...
  /** Generates the MethodDef of a (non-constructor) method
    ...
    * Unit, then the body is emitted as a statement. Otherwise, it is emitted as
    * an expression.
    *
  ...
  private def genMethodDef(
      namespace: js.MemberNamespace,
      methodName: js.MethodIdent,
    ...
    def genBody() =
      localNames.makeLabeledIfRequiresEnclosingReturn(resultIRType) {
        if resultIRType == jstpe.NoType then genStat(tree)
        else genExpr(tree)
      }

    if namespace.isStatic || !currentClassSym.isNonNativeJSClass then
      ...
      withScopedVars(
        ...
        val staticNamespace =
          if namespace.isPrivate then js.MemberNamespace.PrivateStatic
          else js.MemberNamespace.PublicStatic
        val flags =
  ...
  private def genParamDef(
      sym: Symbol,
      ptpe: jstpe.Type,
      pos: Position
  ): js.ParamDef =
    js.ParamDef(
      encodeLocalSym(sym)(implicitly, pos, implicitly),
      originalNameOfLocal(sym),
      ptpe,
      mutable = false
    )(pos)
  ...
  private def genComputedJSName(sym: Symbol)(implicit
      pos: SourcePosition
  ): js.Tree =
    ...
    if sym.owner.isJSType then
      if !sym.owner.isNonNativeJSClass || sym.isJSExposed then
        genApplyJSMethodGeneric(
          sym,
          moduleOrGlobalScope,
          args = Nil,
          isStat = false
        )
      else genApplyJSClassMethod(module, sym, arguments = Nil)
    else genApplyMethod(module, sym, arguments = Nil)
  ...
  def genExprOrGlobalScope(tree: Tree): MaybeGlobalScope =
    ...
    tree match
      case _: This =>
        val sym = tree.symbol
        if sym != currentClassSym.get && sym.is(Module) then
          genLoadModuleOrGlobalScope(sym)
        else MaybeGlobalScope.NotGlobalScope(genExpr(tree))
      ...
      case Apply(fun, _) =>
        if fun.symbol == jsdefn.JSDynamic_global then
  ...
  private def genStatOrExpr(tree: Tree, isStat: Boolean): js.Tree =
    ...
    tree match
      ...
      case Return(expr, from) =>
        ...
        val label =
          if fromSym.is(Label) then encodeLabelSym(fromSym)
      ...
      case WhileDo(cond, body) =>
        val genCond =
          if cond == EmptyTree then js.BooleanLiteral(true)
          else genExpr(cond)
      ...
      case tree: This =>
        ...
        if symIsModuleClass && tree.symbol != currentClass then
      ...
      case Select(qualifier, _) =>
        ...
        if sym.is(Module) then
          ...
          genLoadModule(sym)
        else if sym.is(JavaStatic) then genLoadStaticField(sym)
        else if sym.hasAnnotation(jsdefn.JSNativeAnnot) then
          genJSNativeMemberSelect(tree)
        else {
          val (field, boxed) = genAssignableField(sym, qualifier)
          if boxed then
            unbox(field, atPhase(elimErasedValueTypePhase)(sym.info))
          else field
        }
      ...
      case tree: Ident =>
        desugarIdent(tree).fold[js.Tree] {
          val sym = tree.symbol
          assert(!sym.is(Package), "Cannot use package as value: " + tree)
          if sym.is(Module) then genLoadModule(sym)
          else if undefinedDefaultParams.contains(sym) then
      ...
      case Assign(lhs0, rhs) =>
        ...
        then
          throw new FatalError(
            s"Assignment to static member ${sym.fullName} not supported"
          )
        ...
        lhs match
          case lhs: Select =>
            ...
            if sym.hasAnnotation(jsdefn.JSNativeAnnot) then
              ...
              if boxed then
                val genBoxedRhs =
                  box(genRhs, atPhase(elimErasedValueTypePhase)(sym.info))
  ...
  /** Gen JS this of the current class. Normally encoded straightforwardly as a
    * JS this. But must be replaced by the `thisLocalVarIdent` local variable if
    * there is one.
    */
  ...
  /** Gen IR code for a `try..catch` or `try..finally` block.
    ...
    * type-based selection of exceptions to catch. We thus encode explicitly the
    * type tests, like in:
    *
    * ```
    * try { ... }
    ...
    * ```
    *
    * In addition, there are provisions to handle catching JavaScript exceptions
    * (which do not extend `Throwable`) as wrapped in a
  ...
  private def genTry(tree: Try, isStat: Boolean): js.Tree =
    ...
    val handled =
      if catches.isEmpty then blockAST
  ...
  private def genTryCatch(
    ...
    val (exceptValDef, exceptVar) = if mightCatchJavaScriptException then
      val valDef = js.VarDef(
        freshLocalIdent("e"),
        NoOriginalName,
        encodeClassType(defn.ThrowableClass),
        mutable = false,
        js.WrapAsThrowable(origExceptVar)
      )
      (valDef, valDef.ref)
    else (js.Skip(), origExceptVar)
    ...
    val handler = catches.foldRight(elseHandler) { (caseDef, elsep) =>
      ...
      // Generate the test
      if tpe =:= defn.ThrowableType then bodyWithBoundVar
      else {
  ...
  /** Gen JS code for an Apply node (method call)
    ...
    * super calls, constructor calls, isInstanceOf/asInstanceOf, primitives, JS
    * calls, etc. They are further dispatched in here.
    */
  private def genApply(tree: Apply, isStat: Boolean): js.Tree =
      ...
      sym.name.is(DefaultGetterName) && {
        ...
        if info.isForConstructor then
          ...
          if sym.owner.isJSType then
             ...
             */
            sym.owner.is(ModuleClass) && info.attachedMethod.hasAnnotation(
              jsdefn.JSNativeAnnot
            )
      }
    ...
    tree.fun match
      ...
      case _ =>
        if primitives.isPrimitive(tree) then genPrimitiveOp(tree, isStat)
        ...
        else genNormalApply(tree, isStat)
  end genApply

  /** Gen JS code for a super call, of the form Class.super[mix].fun(args).
    *
    * This does not include calls defined in mixin traits, as these are already
    * desugared by the 'mixin' phase. Only calls to super classes remain.
    *
    * Since a class has exactly one direct superclass, and calling a method two
    * classes above the current one is invalid in Scala, the `mix` item is
    * irrelevant.
    */
  private def genSuperCall(tree: Apply, isStat: Boolean): js.Tree =
    ...
    if sym == defn.Any_getClass then
      // The only primitive that is also callable as super call
      js.GetClass(genThis())
    else if currentClassSym.isNonNativeJSClass then genJSSuperCall(tree, isStat)
    else {
      /* #3013 `qual` can be `this.$outer()` in some cases since Scala 2.12,
       * so we call `genExpr(qual)`, not just `genThis()`.
       */
      val superCall =
        genApplyMethodStatically(genExpr(qual), sym, genActualArgs(sym, args))
  ...
  /** Gen JS code for a constructor call (new). Further refined into: * new
    * String(...) * new of a hijacked boxed class * new of an anonymous function
    * class that was recorded as JS function * new of a raw JS class * new Array
    * * regular new
    */
  private def genApplyNew(tree: Apply): js.Tree =
    ...
    if isHijackedClass(clsSym) then
    ...
    } else*/ if clsSym.isJSType then genNewJSClass(tree)
    else
      toTypeRef(tpe) match
  ...
  /** Gen JS code for a call to a constructor of a hijacked class. Reroute them
    * to the `new` method with the same signature in the companion object.
    */
  private def genNewHijackedClass(
      clazz: Symbol,
      ctor: Symbol,
      args: List[js.Tree]
  )(implicit pos: SourcePosition): js.Tree =
  ...
  private def genNewJSClass(tree: Apply): js.Tree =
    acquireContextualJSClassValue { jsClassValue =>
      ...
      assert(
        jsClassValue.isDefined == nestedJSClass,
      ...
      jsClassValue.fold {
        ...
        if cls == jsdefn.JSObjectClass && args.isEmpty then
          js.JSObjectConstr(Nil)
        else if cls == jsdefn.JSArrayClass && args.isEmpty then
          js.JSArrayConstr(Nil)
        else js.JSNew(genLoadJSConstructor(cls), genArgs)
      } { jsClassVal =>
        // Nested JS class
        if cls.isAnonymousClass then
          genNewAnonJSClass(cls, jsClassVal, genArgsAsClassCaptures)(fun.span)
        else if atPhase(erasurePhase)(cls.is(ModuleClass))
        then // LambdaLift removes the ModuleClass flag of lifted classes
          js.JSNew(
            js.CreateJSClass(
              encodeClassName(cls),
              jsClassVal :: genArgsAsClassCaptures
            ),
            Nil
          )
        else js.JSNew(jsClassVal, genArgs)
      }
    }
  ...
  /** Generate an instance of an anonymous (non-lambda) JS class inline
    ...
    */
  private def genNewAnonJSClass(
      sym: Symbol,
      jsSuperClassValue: js.Tree,
      args: List[js.Tree]
    ...
    originalClassDef.memberDefs.foreach {
      ...
      case nativeMemberDef: js.JSNativeMemberDef =>
        throw new FatalError(
          "illegal native JS member in JS class at " + nativeMemberDef.pos
        )
    }

    assert(
      originalClassDef.topLevelExportDefs.isEmpty,
    ...
    val newClassDef =
      ...
      js.ClassDef(
        originalClassDef.name,
        originalClassDef.originalName,
    ...
    assert(
      ctorParams.isEmpty && ctorRestParam.isEmpty,
    ...
    def memberLambda(
        ...
        restParam: Option[js.ParamDef],
        body: js.Tree
    )(implicit pos: ir.Position): js.Closure =
      js.Closure(
        arrow = false,
        captureParams = Nil,
        params,
    ...
    val memberDefinitions0 = instanceMembers.toList.map {
      ...
      case pdef: js.JSPropertyDef =>
        ...
        val optGetter = pdef.getterBody.map { body =>
          js.StringLiteral("get") -> memberLambda(
            params = Nil,
            restParam = None,
            body
          )
        ...
        val optSetter = pdef.setterArgAndBody.map { case (arg, body) =>
          js.StringLiteral("set") -> memberLambda(
            params = arg :: Nil,
            restParam = None,
            body
          )
    ...
    val memberDefinitions =
      if privateFieldDefs.isEmpty then memberDefinitions0
      else {
        ...
        val fieldsObjValue =
          js.JSObjectConstr(privateFieldDefs.toList.map { fdef =>
            implicit val pos = fdef.pos
            js.StringLiteral(fdef.name.name.nameString) -> jstpe.zeroOf(
              fdef.ftpe
            )
          })
    ...
    val inlinedCtorStats: List[js.Tree] =
      ...
      val superCall =
        ...
        val newTree =
          val ident = originalClassDef.superClass.getOrElse(
            throw new FatalError("No superclass")
          )
          if args.isEmpty && ident.name == JSObjectClassName then
            js.JSObjectConstr(Nil)
          else js.JSNew(jsSuperClassRef, args)

        val selfVarDef = js.VarDef(
          selfName,
          thisOriginalName,
          jstpe.AnyType,
          mutable = false,
    ...
    val closure = js.Closure(
      ...
      Nil,
      None,
      js.Block(inlinedCtorStats, selfRef),
      jsSuperClassValue :: args
    )
    js.JSFunctionApply(closure, Nil)
  end genNewAnonJSClass
  ...
  private def genPrimitiveOp(tree: Apply, isStat: Boolean): js.Tree =
    ...
    else if code == CONCAT then genStringConcat(tree, receiver, args)
    else if code == HASH then genScalaHash(tree, receiver)
    else if isArrayOp(code) then genArrayOp(tree, code)
    else if code == SYNCHRONIZED then genSynchronized(tree, isStat)
    else if isCoercion(code) then genCoercion(tree, receiver, code)
    else if code == JSPrimitives.THROW then genThrow(tree, args)
    else if JSPrimitives.isJSPrimitive(code) then
      genJSPrimitive(tree, args, code, isStat)
    else
      throw new FatalError(
        s"Unknown primitive: ${tree.symbol.fullName} at: $pos"
      )
  ...
  end genSimpleUnaryOp

  /** Gen JS code for a simple binary operation. */
  private def genSimpleBinaryOp(
      tree: Apply,
      lhs: Tree,
      rhs: Tree,
      code: Int
    ...
    val opType =
      if isShift then
        if lhsIRType == jstpe.LongType then jstpe.LongType
        else jstpe.IntType
      else
        (lhsIRType, rhsIRType) match
          ...
          case (
                jstpe.IntType | jstpe.ByteType | jstpe.ShortType |
                jstpe.CharType,
                _
              ) =>
            jstpe.IntType
    ...
    val lsrc =
      if opType == jstpe.AnyType then genExpr(lhs)
      else adaptPrimitive(genExpr(lhs), opType)
    val rsrc =
      if opType == jstpe.AnyType then genExpr(rhs)
      else
        adaptPrimitive(genExpr(rhs), if isShift then jstpe.IntType else opType)

    if opType == jstpe.AnyType && isUniversalEqualityOp(code) then
      genUniversalEqualityOp(lhs.tpe, rhs.tpe, lsrc, rsrc, code)
    else if code == ZOR then
      js.If(lsrc, js.BooleanLiteral(true), rsrc)(jstpe.BooleanType)
    else if code == ZAND then
  ...
  end genSimpleBinaryOp

  private def adaptPrimitive(value: js.Tree, to: jstpe.Type)(implicit
      pos: Position
  ): js.Tree =
  ...
  private def genConversion(from: jstpe.Type, to: jstpe.Type, value: js.Tree)(
    ...
    if from == to || from == jstpe.NothingType then value
    else if from == jstpe.BooleanType || to == jstpe.BooleanType then
      ...
      def intValue = (from: @unchecked) match
        ...
        case jstpe.FloatType =>
          js.UnaryOp(DoubleToInt, js.UnaryOp(FloatToDouble, value))
      ...
      (to: @unchecked) match
        ...
        case jstpe.FloatType =>
          if from == jstpe.LongType then
            js.UnaryOp(js.UnaryOp.LongToFloat, value)
          else js.UnaryOp(js.UnaryOp.DoubleToFloat, doubleValue)
        case jstpe.DoubleType =>
          doubleValue
    }
    end if
  ...
  private def genUniversalEqualityOp(
      ltpe: Type,
      rtpe: Type,
      lhs: js.Tree,
      rhs: js.Tree,
    ...
    if bypassEqEq then
      js.BinaryOp(
        if code == EQ then js.BinaryOp.=== else js.BinaryOp.!==,
        lhs,
        rhs
      )
    else {
      val body = genEqEqPrimitive(ltpe, rtpe, lhs, rhs)
      if code == EQ then body
  ...
  private lazy val externalEquals: Symbol =
    defn.BoxesRunTimeModule.info
      .decl(nme.equals_)
      .suchThat(toDenot(_).info.firstParamTypes.size == 2)
      .symbol

  /** Gen JS code for a call to Any.== */
  private def genEqEqPrimitive(
      ltpe: Type,
      rtpe: Type,
      lsrc: js.Tree,
      rsrc: js.Tree
    ...
    val mustUseAnyComparator: Boolean =
      lsym.isJSType || rsym.isJSType || {
        ...
        p.isMaybeBoxed(lsym) && p.isMaybeBoxed(rsym) && {
          val areSameFinals =
            lsym.is(Final) && rsym.is(Final) && (ltpe =:= rtpe)
          !areSameFinals || lsym == defn.BoxedFloatClass || lsym == defn.BoxedDoubleClass
        }
      }

    if mustUseAnyComparator then
      val equalsMethod: Symbol =
        val ptfm = ctx.platform
        if lsym.derivesFrom(defn.BoxedNumberClass) then
          if rsym.derivesFrom(defn.BoxedNumberClass) then externalEqualsNumNum
          else if rsym.derivesFrom(defn.BoxedCharClass) then
            externalEqualsNumObject // will be externalEqualsNumChar in 2.12, SI-9030
          else externalEqualsNumObject
        else externalEquals
      genApplyStatic(equalsMethod, List(lsrc, rsrc))
    else
      // if (lsrc eq null) rsrc eq null else lsrc.equals(rsrc)
      if lsym == defn.StringClass then
  ...
  private def genScalaHash(tree: Apply, receiver: Tree): js.Tree =
    ...
    genModuleApplyMethod(
      defn.ScalaRuntimeModule.requiredMethod(nme.hash_),
  ...
  private def genSynchronized(tree: Apply, isStat: Boolean): js.Tree =
    ...
    genReceiver match
      ...
      case _ =>
        ...
        js.Block(
          js.If(
            js.BinaryOp(js.BinaryOp.===, genReceiver, js.Null()),
            js.Throw(
              js.New(
                NullPointerExceptionClass,
                js.MethodIdent(jsNames.NoArgConstructorName),
                Nil
              )
            ),
            js.Skip()
  ...
  /** Gen a "normal" apply (to a true method).
    *
    * But even these are further refined into: * Methods of java.lang.String,
    * which are redirected to the RuntimeString trait implementation. * Calls to
    * methods of raw JS types (Scala.js -> JS interop) * Calls to methods in
    * impl classes of Scala2 traits. * Regular method call
    */
  private def genNormalApply(tree: Apply, isStat: Boolean): js.Tree =
    ...
    else if sym.owner.isJSType then
      if !sym.owner.isNonNativeJSClass || sym.isJSExposed then
        genApplyJSMethodGeneric(
          sym,
          genExprOrGlobalScope(receiver),
          genActualJSArgs(sym, args),
          isStat
        )(tree.sourcePos)
      else
  ...
  /** Gen JS code for a call to a JS method (of a subclass of `js.Any`).
    ...
    *   - `apply` is translated as a function call, i.e., `o()` instead of
    *     `o.apply()`
    ...
    */
  private def genApplyJSMethodGeneric(
      sym: Symbol,
      receiver: MaybeGlobalScope,
    ...
    def argsNoSpread: List[js.Tree] =
      assert(
        !args.exists(_.isInstanceOf[js.JSSpread]),
    ...
    def requireNotSuper(): Unit =
      if jsSuperClassValue.isDefined then
    ...
    val boxedResult = sym.jsCallingConvention match
      ...
      case JSCallingConvention.BinaryOp(code) =>
        ...
        js.JSBinaryOp(
          code,
          ruleOutGlobalScope(receiver),
          requireNotSpread(args.head)
        )

      case JSCallingConvention.Call =>
        requireNotSuper()
        if sym.owner.isSubClass(jsdefn.JSThisFunctionClass) then
          js.JSMethodApply(
            ruleOutGlobalScope(receiver),
            js.StringLiteral("call"),
            args
          )
        else js.JSFunctionApply(ruleOutGlobalScope(receiver), args)
    ...
    if isStat then boxedResult
    else {
      val tpe = atPhase(elimErasedValueTypePhase) {
        sym.info.finalResultType
      }
      if tpe.isRef(defn.BoxedUnitClass) && sym.isGetter then
      ...
      else unbox(boxedResult, tpe)
    }
  end genApplyJSMethodGeneric

  /** Extract the first argument in a list of actual arguments.
    *
    * This is nothing else than decomposing into head and tail, except that we
    * assert that the first element is not a JSSpread.
    */
  private def extractFirstArg(
      args: List[js.TreeOrJSSpread]
  ): (js.Tree, List[js.TreeOrJSSpread]) =
    assert(
      args.nonEmpty,
  ...
  /** Gen JS code for a call to a native JS def or val. */
  private def genJSNativeMemberSelectOrCall(
      tree: Tree,
      args: List[Tree]
  ): js.Tree =
    val sym = tree.symbol
    ...
    val jsNativeMemberValue =
      js.SelectJSNativeMember(
        encodeClassName(sym.owner),
        encodeJSNativeMemberSym(sym)
      )

    val boxedResult =
      if sym.isJSGetter then jsNativeMemberValue
      else js.JSFunctionApply(jsNativeMemberValue, genActualJSArgs(sym, args))

    unbox(
      boxedResult,
      atPhase(elimErasedValueTypePhase) {
        sym.info.resultType
      }
    )
  ...
  private def genJSSuperCall(tree: Apply, isStat: Boolean): js.Tree =
    acquireContextualJSClassValue { explicitJSSuperClassValue =>
      ...
      if sym.owner == defn.ObjectClass then
        // Normal call anyway
        assert(
      ...
      else if sym.owner.isNonNativeJSClass && !sym.isJSExposed then
        ...
        genApplyJSMethodGeneric(
          sym,
          MaybeGlobalScope.NotGlobalScope(genReceiver),
          genJSArgs,
          isStat,
          jsSuperClassValue
        )(tree.sourcePos)
  ...
  /** Gen JS code for a call to a polymorphic method.
    *
    * The only methods that reach the back-end as polymorphic are `isInstanceOf`
    * and `asInstanceOf`.
    *
  ...
  private def genTypeApply(tree: TypeApply): js.Tree =
    ...
    assert(
      !isPrimitiveValueType(receiver.tpe),
      s"Found receiver of type test with primitive type ${receiver.tpe} at $pos"
    )
    assert(
      !isPrimitiveValueType(to),
      s"Found target type of type test with primitive type ${receiver.tpe} at $pos"
    )

    val genReceiver = genExpr(receiver)

    if sym == defn.Any_asInstanceOf then genAsInstanceOf(genReceiver, to)
    else if sym == defn.Any_isInstanceOf then genIsInstanceOf(genReceiver, to)
    else
      throw new FatalError(
  ...
  /** Gen JS code for a switch-`Match`, which is translated into an IR
    * `js.Match`.
    */
  def genMatch(tree: Tree, isStat: Boolean): js.Tree =
    ...
    def abortMatch(msg: String): Nothing =
      throw new FatalError(
        s"$msg in switch-like pattern match at ${tree.span}: $tree"
      )
    ...
    genSelector.tpe match
      case jstpe.IntType | jstpe.ClassType(jsNames.BoxedStringClass) |
          jstpe.NullType | jstpe.NothingType =>
      // ok
      case _ =>
    ...
    val resultType = toIRType(tree.tpe) match
      case jstpe.NothingType =>
        jstpe.NothingType // must take priority over NoType below
    ...
    for caze @ CaseDef(pat, guard, body) <- cases do
      if guard != EmptyTree then abortMatch("Found a case guard")
    ...
    clauses match
      ...
      case (uniqueAlt :: Nil, caseRhs) :: Nil =>
         ...
         */
        val op =
          if isInt(genSelector) && isInt(uniqueAlt) then js.BinaryOp.Int_==
          else js.BinaryOp.===
        js.If(js.BinaryOp(op, genSelector, uniqueAlt), caseRhs, defaultClause)(
          resultType
        )
  ...
  /** Gen JS code for a closure.
    ...
    * results of those evaluations is "stored" in the environment of the closure
    * as the corresponding `formalCapture`.
    *
    ...
    * `formalParams` of the created closure receive their values from the actual
    * arguments at the call-site of the closure, and they are also available in
    * the `body`.
    */
  private def genClosure(tree: Closure): js.Tree =
    ...
    val formalAndActualCaptures = allCaptureValues.map { value =>
      ...
      val (formalIdent, originalName) = value match
        case Ident(name) =>
          (freshLocalIdent(name.toTermName), OriginalName(name.toString))
        case This(_) => (freshLocalIdent("this"), thisOriginalName)
        case _       => (freshLocalIdent(), NoOriginalName)
      val formalCapture = js.ParamDef(
        formalIdent,
        originalName,
        toIRType(value.tpe),
        mutable = false
    ...
    val hasRepeatedParam =
      funInterfaceSym.exists && {
        ...
        atPhase(elimRepeatedPhase)(
          samMethod.info.paramInfoss.flatten.exists(_.isRepeatedParam)
        )
      }
    ...
    val formalParamRepeateds =
      if hasRepeatedParam then
        (0 until (formalParamTypes.size - 1)).map(_ => false) :+ true
      else (0 until formalParamTypes.size).map(_ => false)

    val formalAndActualParams = formalParamNames
      .lazyZip(formalParamTypes)
      .lazyZip(formalParamRepeateds)
      .map { (name, tpe, repeated) =>
        val formalParam = js.ParamDef(
          freshLocalIdent(name),
          OriginalName(name.toString),
          jstpe.AnyType,
          mutable = false
        )
        val actualParam =
          if repeated then genJSArrayToVarArgs(formalParam.ref)(tree.sourcePos)
    ...
    val (formalParams, restParam) =
      if hasRepeatedParam then
        (formalAndRestParams.init, Some(formalAndRestParams.last))
    ...
    val genBody =
      val call =
        if isStaticCall then
          ...
          val thisCaptureRef :: argCaptureRefs =
            formalCaptures.map(_.ref): @unchecked
          if !sym.owner.isNonNativeJSClass || sym.isJSExposed then
            genApplyMethodMaybeStatically(
              thisCaptureRef,
              sym,
    ...
    val isThisFunction =
      funInterfaceSym.isSubClass(jsdefn.JSThisFunctionClass) && {
        val ok = formalParams.nonEmpty
        if !ok then
          report.error(
            "The SAM or apply method for a js.ThisFunction must have a leading non-varargs parameter",
            tree
          )
        ok
      }

    if isThisFunction then
      ...
      if !funInterfaceSym.exists || defn.isFunctionClass(funInterfaceSym) then
        ...
        val ctorName = MethodName.constructor(
          jstpe.ClassRef(
            ClassName("scala.scalajs.js.Function" + formalCount)
          ) :: Nil
        )
  ...
  /** Generates a static method instantiating and calling this
    ...
    * }}}
    */
  private def genDynamicImportForwarder(
      clsSym: Symbol
  )(using Position): js.MethodDef =
  ...
  /** Boxes a value of the given type before `elimErasedValueType`.
    *
    * This should be used when sending values to a JavaScript context, which is
    * erased/boxed at the IR level, although it is not erased at the dotty/JVM
    * level.
    *
    * @param expr
    ...
    */
  def box(expr: js.Tree, tpeEnteringElimErasedValueType: Type)(implicit
      pos: Position
  ): js.Tree =
  ...
  /** Unboxes a value typed as Any to the given type before
    * `elimErasedValueType`.
    *
    * This should be used when receiving values from a JavaScript context, which
    ...
    *   The type of `expr` as it was entering the `elimErasedValueType` phase.
    */
  def unbox(expr: js.Tree, tpeEnteringElimErasedValueType: Type)(implicit
      pos: Position
  ): js.Tree =
    tpeEnteringElimErasedValueType match
      case tpe if isPrimitiveValueType(tpe) =>
      ...
      case tpe: ErasedValueType =>
        ...
        val content = genApplyMethod(
          js.AsInstanceOf(expr, encodeClassType(boxedClass)),
          unboxMethod,
          Nil
        )
        if unboxMethod.info.resultType <:< tpe.erasedUnderlying then content
        else unbox(content, tpe.erasedUnderlying)

      case tpe =>
        genAsInstanceOf(expr, tpe)

  /** Gen JS code for an asInstanceOf cast (for reference types only) */
  private def genAsInstanceOf(value: js.Tree, to: Type)(implicit
      pos: Position
  ): js.Tree =
    genAsInstanceOf(value, toIRType(to))

  /** Gen JS code for an asInstanceOf cast (for reference types only) */
  private def genAsInstanceOf(value: js.Tree, to: jstpe.Type)(implicit
      pos: Position
  ): js.Tree =
    to match
      case jstpe.AnyType =>
  ...
  private def genThrowClassCastException()(implicit pos: Position): js.Tree =
    js.Throw(
      js.New(
        jsNames.ClassCastExceptionClass,
        js.MethodIdent(jsNames.NoArgConstructorName),
        Nil
      )
    )
  ...
  def genIsInstanceOf(value: js.Tree, to: Type)(implicit
    ...
    if sym == defn.ObjectClass then
      js.BinaryOp(js.BinaryOp.!==, value, js.Null())
    else if sym.isJSType then
      ...
      assert(
        sym != defn.NullClass && sym != defn.NothingClass,
  ...
  def genApplyMethodMaybeStatically(
      receiver: js.Tree,
      method: Symbol,
      arguments: List[js.Tree]
  )(implicit pos: Position): js.Tree =
    if method.isPrivate || method.isClassConstructor then
      genApplyMethodStatically(receiver, method, arguments)
    else genApplyMethod(receiver, method, arguments)
  ...
  def genApplyMethodStatically(
    ...
    js.ApplyStatically(
      flags,
      receiver,
      encodeClassName(method.owner),
      encodeMethodSym(method),
      arguments
    )(toIRType(patchedResultType(method)))

  /** Gen a call to a static method. */
  private def genApplyStatic(method: Symbol, arguments: List[js.Tree])(implicit
      pos: Position
  ): js.Tree =
    js.ApplyStatic(
      js.ApplyFlags.empty.withPrivate(method.isPrivate),
      encodeClassName(method.owner),
      encodeMethodSym(method),
      arguments
    )(toIRType(patchedResultType(method)))
  ...
  private def makePrimitiveBox(expr: js.Tree, tpe: Type)(implicit
      pos: Position
  ): js.Tree =
    toIRType(tpe) match
  ...
  private def makePrimitiveUnbox(expr: js.Tree, tpe: Type)(implicit
      pos: Position
  ): js.Tree =
    toIRType(tpe) match
    ...
    def resolveReifiedJSClassSym(arg: Tree): Symbol =
      ...
      arg match
        case Literal(value) if value.tag == Constants.ClazzTag =>
          ...
          if classSym.isJSType && !classSym.is(Trait) && !classSym.is(
              ModuleClass
            )
          then classSym
          else fail()
    ...
    (code: @switch) match
      ...
      case CREATE_INNER_JS_CLASS | CREATE_LOCAL_JS_CLASS =>
        ...
        if classSym == NoSymbol then
          ...
          val captureValues =
            if code == CREATE_INNER_JS_CLASS then
              ...
              val requiredThisParams =
                classSym.info.decls
                  .lookupAll(nme.CONSTRUCTOR)
                  .count(_.info.paramInfoss.head.nonEmpty)
              val outer = genThis()
      ...
      case JS_NEW_TARGET =>
        // js.new.target
        val valid =
          currentMethodSym.get.isClassConstructor && currentClassSym.isNonNativeJSClass
        if !valid then
          report.error(
      ...
      case DYNAMIC_IMPORT =>
        ...
        args.head match
          case Block(
                stats,
                expr @ Typed(Apply(fun @ Select(New(tpt), _), args), _)
              ) =>
            ...
            assert(
              clsSym.isSubClass(jsdefn.DynamicImportThunkClass),
              s"expected subclass of DynamicImportThunk, got: $clsSym at: ${expr.sourcePos}"
            )
            assert(
              ctor.isPrimaryConstructor,
      ...
      case IN =>
        ...
        val (arg1, arg2) = genArgs2
        js.AsInstanceOf(
          js.JSBinaryOp(js.JSBinaryOp.in, arg1, arg2),
          jstpe.BooleanType
        )
      ...
      case INSTANCEOF =>
        ...
        val (arg1, arg2) = genArgs2
        js.AsInstanceOf(
          js.JSBinaryOp(js.JSBinaryOp.instanceof, arg1, arg2),
          jstpe.BooleanType
        )
      ...
      case JS_TRY_CATCH =>
         ...
         */
        val (arg1, arg2) = genArgs2
        val bodyVarDef = js.VarDef(
          freshLocalIdent("body"),
          NoOriginalName,
          jstpe.AnyType,
          mutable = false,
          arg1
        )
        val handlerVarDef = js.VarDef(
          freshLocalIdent("handler"),
          NoOriginalName,
          jstpe.AnyType,
          mutable = false,
          arg2
  ...
  /** Gen the SJSIR for a reflective call.
    ...
    *     `asInstanceOf`s and `jstpe.TypeRef`s for the
    *     `MethodName.reflectiveProxy`
  ...
  private def genReflectiveCall(
    ...
    val selectedValueTree = js.Apply(
      js.ApplyFlags.empty,
      genExpr(receiver),
      js.MethodIdent(selectedValueMethodName),
      Nil
    )(jstpe.AnyType)
    ...
    val (formalParamTypeRefs, actualArgs) =
      if isSelectDynamic then (Nil, Nil)
      else
        // Extract the param type refs and actual args from the 2nd and 3rd argument to applyDynamic
        args.tail match
          case WrapArray(classOfsArray: JavaSeqLiteral) :: WrapArray(
                actualArgsAnyArray: JavaSeqLiteral
              ) :: Nil =>
            ...
            val actualArgs =
              actualArgsAnyArray.elems.zip(formalParamTypesAndTypeRefs).map {
                (actualArgAny, formalParamTypeAndTypeRef) =>
                  val genActualArgAny = genExpr(actualArgAny)
                  genAsInstanceOf(
                    genActualArgAny,
                    formalParamTypeAndTypeRef._1
                  )(genActualArgAny.pos)
              }

            (
              formalParamTypesAndTypeRefs.map(pair =>
                toParamOrResultTypeRef(pair._2)
              ),
              actualArgs
            )
    ...
    val methodName =
      MethodName.reflectiveProxy(methodNameStr, formalParamTypeRefs)
  ...
  end genReflectiveCall

  /** Gen actual actual arguments to Scala method call. Returns a list of the
    * transformed arguments.
    *
    * This tries to optimize repeated arguments (varargs) by turning them into
    * js.WrappedArray instead of Scala wrapped arrays.
    */
  ...
  /** Gen actual actual arguments to a JS method call. Returns a list of the
    * transformed arguments.
    *
    ...
    *
    * Repeated arguments that cannot be expanded at compile time (i.e., if a Seq
    * is passed to a varargs parameter with the syntax `seq: _*`) will be
    * wrapped in a [[js.JSSpread]] node to be expanded at runtime.
    */
  private def genActualJSArgs(sym: Symbol, args: List[Tree])(implicit
      pos: Position
  ): List[js.TreeOrJSSpread] =

    var reversedArgs: List[js.TreeOrJSSpread] = Nil

    for (arg, info) <- args.zip(sym.jsParamInfos) do
      if info.repeated then
        reversedArgs = genJSRepeatedParam(arg) reverse_::: reversedArgs
      else if info.capture then
        // Ignore captures
        assert(
          sym.isClassConstructor,
  ...
  /** Gen JS code for a repeated param of a JS method.
    ...
    * be an expanded list of the elements in the sequence. So this method takes
    * care of the conversion.
    *
    * It is specialized for the shapes of tree generated by the desugaring of
    * repeated params in Scala, so that these are actually expanded at
    * compile-time.
    *
  ...
  private def genJSRepeatedParam(arg: Tree): List[js.TreeOrJSSpread] =
    tryGenRepeatedParamAsJSArray(arg, handleNil = true).getOrElse {
      ...
      val jsArrayArg =
        genModuleApplyMethod(jsdefn.Runtime_toJSVarArgs, List(genExpr(arg)))
      List(js.JSSpread(jsArrayArg))
    }

  /** Try and expand an actual argument to a repeated param `(xs: T*)`.
    *
    * This method recognizes the shapes of tree generated by the desugaring of
    * repeated params in Scala, and expands them. If `arg` does not have the
    * shape of a generated repeated param, this method returns `None`.
    */
  private def tryGenRepeatedParamAsJSArray(
    ...
    arg match
      ...
      case MaybeAsInstanceOf(
            WrapArray(MaybeAsInstanceOf(array: JavaSeqLiteral))
          ) =>
      ...
      case _ =>
        None
  end tryGenRepeatedParamAsJSArray
  ...
  private object WrapArray:
    lazy val isWrapArray: Set[Symbol] =
      ...
      val symsInScalaRunTime =
        names1.map(defn.ScalaRuntimeModule.requiredMethod(_))
  ...
  def genJSArrayToVarArgs(arrayRef: js.Tree)(implicit
      pos: SourcePosition
  ): js.Tree =
    genModuleApplyMethod(jsdefn.Runtime_toScalaVarArgs, List(arrayRef))

  /** Gen the actual capture values for a JS constructor based on its fake `new`
    * invocation.
    */
  ...
  private def genAssignableField(sym: Symbol, qualifier: Tree)(implicit
      pos: SourcePosition
  ): (js.AssignLhs, Boolean) =
    def qual = genExpr(qualifier)

    if sym.owner.isNonNativeJSClass then
      val f =
        if sym.isJSExposed then js.JSSelect(qual, genExpr(sym.jsName))
        else if sym.owner.isAnonymousClass then
          js.JSSelect(
      ...
      (f, true)
    else if sym.hasAnnotation(jsdefn.JSExportTopLevelAnnot) then
      val f = js.SelectStatic(encodeClassName(sym.owner), encodeFieldSym(sym))(
        jstpe.AnyType
      )
      (f, true)
    else if sym.hasAnnotation(jsdefn.JSExportStaticAnnot) then
      val jsName = sym
        .getAnnotation(jsdefn.JSExportStaticAnnot)
        .get
      ...
      val f =
        ...
        else js.Select(qual, className, fieldIdent)(irType)

      (f, boxed)
    }
    end if
  end genAssignableField

  /** Gen JS code for loading a Java static field.
    */
  private def genLoadStaticField(sym: Symbol)(implicit
      pos: SourcePosition
  ): js.Tree =
    ...
    if sym == defn.BoxedUnit_UNIT then js.Undefined()
    else if sym == defn.BoxedUnit_TYPE then js.ClassOf(jstpe.VoidRef)
    else {
      val className = encodeClassName(sym.owner)
      val method = encodeStaticMemberSym(sym)
      js.ApplyStatic(js.ApplyFlags.empty, className, method, Nil)(
        toIRType(sym.info)
      )
    }
  ...
  /** Generate loading of a module value or the global scope.
    ...
    */
  def genLoadModuleOrGlobalScope(sym0: Symbol)(implicit
      pos: SourcePosition
  ): MaybeGlobalScope =
    ...
    val sym = if sym0.isTerm then sym0.moduleClass else sym0

    // Does that module refer to the global scope?
    if sym.hasAnnotation(jsdefn.JSGlobalScopeAnnot) then
      MaybeGlobalScope.GlobalScope(pos)
    else {
      val cls = encodeClassName(sym)
      val tree =
        if sym.isJSType then js.LoadJSModule(cls)
        else js.LoadModule(cls)
      MaybeGlobalScope.NotGlobalScope(tree)
    }

  /** Gen JS code representing the constructor of a JS class. */
  private def genLoadJSConstructor(sym: Symbol)(implicit
      pos: Position
  ): js.Tree =
    assert(
      !isStaticModule(sym) && !sym.is(Trait),
  ...
  /** Report a compile error specifying that the global scope cannot be loaded
    * as a value.
    */
  private def reportErrorLoadGlobalScope()(implicit
      pos: SourcePosition
  ): js.Tree =
    report.error(
  ...
  /** Gen a JS bracket select or a `JSGlobalRef`.
    *
    * If the receiver is a normal value, i.e., not the global scope, then emit a
    * `JSSelect`.
    *
      ...
      case MaybeGlobalScope.GlobalScope(_) =>
        item match
          case js.StringLiteral(value) =>
            if js.JSGlobalRef.isValidJSGlobalRefName(value) then
  ...
  /** Gen a JS bracket method apply or an apply of a `GlobalRef`.
    ...
    * `JSMethodApply`.
    *
    * Otherwise, if the `method` is a constant string that is a valid JavaScript
    * identifier, emit a `JSFunctionApply(JSGlobalRef(...), ...)`.
    *
    * Otherwise, report a compile error.
    */
  ...
  private def computeJSNativeLoadSpecOfValDef(
      sym: Symbol
  ): js.JSNativeLoadSpec =
  ...
  private def computeJSNativeLoadSpecOfClass(
      sym: Symbol
  ): Option[js.JSNativeLoadSpec] =
    if sym.is(Trait) || sym.hasAnnotation(jsdefn.JSGlobalScopeAnnot) then None
    else
      atPhaseBeforeTransforms {
        if sym.owner.isStaticOwner then
          Some(computeJSNativeLoadSpecOfInPhase(sym))
        else None
      }

  private def computeJSNativeLoadSpecOfInPhase(sym: Symbol)(using
      Context
  ): js.JSNativeLoadSpec =
    ...
    if symOwner.hasAnnotation(jsdefn.JSNativeAnnot) then
      val jsName = sym.jsName match
        case JSName.Literal(jsName) => jsName
        case JSName.Computed(_) =>
          unexpected("could not read the simple JS name as a string literal")

      if symOwner.hasAnnotation(jsdefn.JSGlobalScopeAnnot) then
        Global(jsName, Nil)
      else {
      ...
      if annot.symbol == jsdefn.JSGlobalAnnot then
        ...
        val path = annot
          ...
          .fold {
            if annot.arguments.sizeIs < 2 then parsePath(sym.defaultJSName)
            else Nil
          } { pathName =>
            parsePath(pathName)
          }
        ...
        annot
          .argumentConstantString(2)
          .fold[js.JSNativeLoadSpec] {
            importSpec
          } { globalPathName =>
            ImportWithGlobalFallback(
              importSpec,
  ...
  end computeJSNativeLoadSpecOfInPhase
  ...
  private def hasDefaultCtorArgsAndJSModule(classSym: Symbol): Boolean =
    ...
    def hasDefaultParameters =
      classSym.info.decls.exists(sym =>
        sym.isClassConstructor && sym.hasDefaultParams
      )
...
object JSCodeGen:
  ...
  private val NullPointerExceptionClass = ClassName(
    "java.lang.NullPointerException"
  )
  private val JSObjectClassName = ClassName("scala.scalajs.js.Object")
  private val JavaScriptExceptionClassName = ClassName(
  ...
  private val selectedValueMethodName =
    MethodName("selectedValue", Nil, ObjectClassRef)

  private val ObjectArgConstructorName =
    MethodName.constructor(List(ObjectClassRef))
  ...
  private class DefaultParamInfo(sym: Symbol)(using Context):
    ...
    def attachedMethod: Symbol =
      ...
      if !overloads.isOverloaded then overloads.symbol
      else overloads.suchThat(_.is(HasDefaultParams, butNot = Bridge)).symbol
end JSCodeGen
