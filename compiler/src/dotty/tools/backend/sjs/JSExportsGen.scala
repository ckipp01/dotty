...
import org.scalajs.ir.{Position, Names as jsNames, Trees as js, Types as jstpe}
...
final class JSExportsGen(jsCodeGen: JSCodeGen)(using Context):
  ...
  final case class TopLevelExportInfo(moduleID: String, jsName: String)(
      val pos: SourcePosition
  ) extends ExportInfo
  final case class StaticExportInfo(jsName: String)(val pos: SourcePosition)
      extends ExportInfo
  ...
  private object ExportKind:
    ...
    def apply(sym: Symbol): ExportKind =
      if sym.is(Flags.Module) && sym.isStatic then Module
      else if sym.isClass then JSClass
      else if sym.isConstructor then Constructor
      else if !sym.is(Flags.Method) then Field
      else if sym.isJSProperty then Property
      else Method
  ...
  private def topLevelExportsOf(sym: Symbol): List[TopLevelExportInfo] =
    ...
    if isScalaClass(sym) then
      // Scala classes are never exported; their constructors are
      Nil
    else if sym.is(Accessor) || sym.is(Module, butNot = ModuleClass) then
  ...
  private def checkSameKind(
      tups: List[(ExportInfo, Symbol)]
  ): Option[ExportKind] =
    ...
    for (info, sym) <- tups.tail do
      val kind = ExportKind(sym)

      if kind != overallKind then
        bad = true
        report.error(
  ...
  def genMemberExports(classSym: ClassSymbol): List[js.MemberDef] =
    ...
    val allExports = classInfo.memberDenots(
      takeAllFilter,
      { (name, buf) =>
        if isExportName(name) then buf ++= classInfo.member(name).alternatives
      }
    )

    val newlyDeclaredExports =
      if classSym.superClass == NoSymbol then allExports
      else
        allExports.filterNot { denot =>
          classSym.superClass.info
            .member(denot.name)
            .hasAltWith(_.info =:= denot.info)
        }

    val newlyDeclaredExportNames =
      newlyDeclaredExports.map(_.name.toTermName).toList.distinct

    newlyDeclaredExportNames.map(genMemberExport(classSym, _))

  private def genMemberExport(
     ...
     */
    val alts = classSym
      .findMemberNoShadowingBasedOnFlags(
        name,
        classSym.appliedRef,
        required = Method,
        excluded = Bridge | MixedIn
      )
      .alternatives
    ...
    if conflicting.exists then
      val kind = if isProp then "property" else "method"
      val conflictingMember = conflicting.alternatives.head.symbol.fullName
      val errorPos: SrcPos =
        alts.map(_.symbol).filter(_.owner == classSym) match
          case Nil         => classSym
          case altsInClass => altsInClass.minBy(_.span.point)
      report.error(
        em"Exported $kind $jsName conflicts with $conflictingMember",
        errorPos
      )

    genMemberExportOrDispatcher(
      JSName.Literal(jsName),
      isProp,
      alts.map(_.symbol),
      static = false
    )
  ...
  def genJSClassDispatchers(
      classSym: Symbol,
      dispatchMethodsNames: List[JSName]
  ): List[js.MemberDef] =
    dispatchMethodsNames.map(genJSClassDispatcher(classSym, _))

  private def genJSClassDispatcher(
      classSym: Symbol,
      name: JSName
  ): js.MemberDef =
    val alts = classSym.info
      .membersBasedOnFlags(required = Method, excluded = Bridge)
      .map(_.symbol)
      .filter { sym =>
    ...
    if isProp && methodSyms.nonEmpty then
      ...
      js.JSPropertyDef(
        ...
        genExpr(name)(firstAlt.sourcePos),
        None,
        None
      )
    else genMemberExportOrDispatcher(name, isProp, alts, static = false)
  end genJSClassDispatcher

  private def genMemberExportOrDispatcher(
      jsName: JSName,
      isProp: Boolean,
      alts: List[Symbol],
      static: Boolean
    ...
    withNewLocalNameScope {
      if isProp then genExportProperty(alts, jsName, static)
      else genExportMethod(alts, jsName, static)
    }

  private def genExportProperty(
      alts: List[Symbol],
      jsName: JSName,
      static: Boolean
  ): js.JSPropertyDef =
    ...
    val namespace =
      if static then js.MemberNamespace.PublicStatic
    ...
    if getter.sizeIs > 1 then reportCannotDisambiguateError(jsName, alts)

    val getterBody = getter.headOption.map { getterSym =>
      genApplyForSingleExported(
        new FormalArgsRegistry(0, false),
        new ExportedSymbol(getterSym, static),
        static
      )
    }
    ...
    val setterArgAndBody =
      if setters.isEmpty then None
        ...
        val body = genOverloadDispatchSameArgc(
          jsName,
          formalArgsRegistry,
          setters.map(new ExportedSymbol(_, static)),
          jstpe.AnyType,
          None
        )
        Some((arg, body))
    ...
    js.JSPropertyDef(
      flags,
      genExpr(jsName)(alts.head.sourcePos),
      getterBody,
      setterArgAndBody
    )
  end genExportProperty

  private def genExportMethod(
      alts0: List[Symbol],
      jsName: JSName,
      static: Boolean
  )(using Context): js.JSMethodDef =
    assert(
      alts0.nonEmpty,
    ...
    val alts = jsName match
      case JSName.Literal("toString")
          if alts0.forall(_.info.paramInfoss.exists(_.nonEmpty)) =>
  ...
  def genOverloadDispatch(
      jsName: JSName,
      alts: List[Exported],
      tpe: jstpe.Type
  )(using
      pos: SourcePosition
  ): (List[js.ParamDef], Option[js.ParamDef], js.Tree) =
     ...
     */
    val body =
      if alts.tail.isEmpty then alts.head.genBody(formalArgsRegistry)
      else
        genExportMethodMultiAlts(
          formalArgsRegistry,
          maxNonRepeatedArgc,
          alts,
          tpe,
  ...
  end genOverloadDispatch

  private def genExportMethodMultiAlts(
      formalArgsRegistry: FormalArgsRegistry,
      maxNonRepeatedArgc: Int,
      alts: List[Exported],
      tpe: jstpe.Type,
      jsName: JSName
    ...
    val methodArgCounts = for
      alt <- alts
      argc <- alt.minArgc to (if alt.hasRepeatedParam then maxNonRepeatedArgc
                              else alt.maxNonRepeatedArgc)
    yield (argc, alt)
    ...
    val methodsByArgCount: List[(Int, List[Exported])] =
      methodArgCounts
        .groupMap(_._1)(_._2)
        .toList
        .sortBy(_._1) // sort for determinism
    ...
    yield
      ...
      val caseBody = genOverloadDispatchSameArgc(
        jsName,
        formalArgsRegistry,
        methods,
        tpe,
    ...
    def defaultCase =
      if altsWithVarArgs.isEmpty then genThrowTypeError()
      else
        genOverloadDispatchSameArgc(
          jsName,
          formalArgsRegistry,
          altsWithVarArgs,
          tpe,
    ...
    val body =
      if cases.isEmpty then defaultCase
        ...
        js.Match(
          js.AsInstanceOf(
            js.JSSelect(restArgRef, js.StringLiteral("length")),
            jstpe.IntType
          ),
          cases,
          defaultCase
  ...
  /** Resolves method calls to [[alts]] while assuming they have the same
    * parameter count.
    *
    * @param jsName
    ...
    */
  private def genOverloadDispatchSameArgc(
      jsName: JSName,
      formalArgsRegistry: FormalArgsRegistry,
      alts: List[Exported],
      tpe: jstpe.Type,
    ...
    genOverloadDispatchSameArgcRec(
      jsName,
      formalArgsRegistry,
      alts,
      tpe,
  ...
  /** Resolves method calls to [[alts]] while assuming they have the same
    ...
    *   Result type
    * @param paramIndex
    *   Index where to start disambiguation (starts at 0, increases through
    *   recursion)
    * @param maxArgc
    *   Maximum number of arguments to use for disambiguation
    */
  private def genOverloadDispatchSameArgcRec(
      jsName: JSName,
      formalArgsRegistry: FormalArgsRegistry,
      alts: List[Exported],
      tpe: jstpe.Type,
    ...
    if alts.sizeIs == 1 then alts.head.genBody(formalArgsRegistry)
    else if maxArgc.exists(_ <= paramIndex) || !alts.exists(
        _.params.size > paramIndex
      )
    then
      ...
      if altsByTypeTest.size == 1 then
        ...
        sortedAltsByTypeTest.foldRight[js.Tree](defaultCase) { (elem, elsep) =>
          ...
          val genSubAlts = genOverloadDispatchSameArgcRec(
            jsName,
            formalArgsRegistry,
            subAlts,
            tpe,
  ...
  private def reportCannotDisambiguateError(
      jsName: JSName,
      alts: List[Symbol]
  ): Unit =
    ...
    val pos: SourcePosition =
      if validPositions.isEmpty then currentClass.sourcePos
      else validPositions.maxBy(_.point)

    val kind =
      if alts.head.isJSGetter then "getter"
      else if alts.head.isJSSetter then "setter"
      else "method"

    val fullKind =
      if currentClass.isJSType then kind
      else "exported " + kind
  ...
  end reportCannotDisambiguateError

  /** Generates a call to the method represented by the given `exported` while
    * using the formalArguments and potentially the argument array.
    *
    * Also inserts default parameters if required.
    */
  private def genApplyForSingleExported(
      formalArgsRegistry: FormalArgsRegistry,
      exported: Exported,
      static: Boolean
  ): js.Tree =
    ...
    then
      ...
      genApplyForSingleExportedNonJSSuperCall(
        formalArgsRegistry,
        exported,
        static
      )
  ...
  private def genApplyForSingleExportedJSSuperCall(
    ...
    else js.JSSuperMethodCall(superClass, receiver, nameTree, allArgs)
  end genApplyForSingleExportedJSSuperCall
  ...
  private def genApplyForSingleExportedNonJSSuperCall(
    ...
    for (param, i) <- exported.params.zipWithIndex do
      val rhs = genScalaArg(
        exported,
        i,
        formalArgsRegistry,
  ...
  end genApplyForSingleExportedNonJSSuperCall

  /** Generates a Scala argument from dispatched JavaScript arguments (unboxing
  ...
  def genCallDefaultGetter(
      ...
      static: Boolean,
      captures: List[js.Tree]
  )(previousArgsValues: Int => List[js.Tree])(implicit
      pos: SourcePosition
  ): js.Tree =
    ...
    assert(
      defaultGetterDenot.exists,
      s"need default getter for method ${sym.fullName}"
    )
    assert(
      !defaultGetterDenot.isOverloaded,
      i"found overloaded default getter $defaultGetterDenot"
    )
    val defaultGetter = defaultGetterDenot.symbol

    val targetTree =
      if sym.isClassConstructor || static then
        if targetSym.isStatic then
          assert(
            captures.isEmpty,
          ...
          val name =
            atPhase(typerPhase)(targetSym.name.unexpandedName).sourceModuleName
          val modAccessor = outer.info.allMembers
            .find { denot =>
              denot.symbol.is(Module) && denot.name.unexpandedName == name
            }
            .getOrElse {
              throw new AssertionError(
                i"could not find module accessor for ${targetSym.fullName} at $pos"
              )
            }
            .symbol
    ...
    val defaultGetterArgs = previousArgsValues(
      defaultGetter.info.paramInfoss.head.size
    )

    val callGetter =
      if targetSym.isJSType then
        if defaultGetter.owner.isNonNativeJSClass then
          if defaultGetter.hasAnnotation(jsdefn.JSOptionalAnnot) then
            js.Undefined()
          else
            genApplyJSClassMethod(targetTree, defaultGetter, defaultGetterArgs)
        else if defaultGetter.owner == targetSym then
    ...
    if callGetter.tpe == jstpe.NoType then js.Block(callGetter, js.Undefined())
    else callGetter
  end genCallDefaultGetter

  private def targetSymForDefaultGetter(sym: Symbol): Symbol =
    if sym.isClassConstructor then sym.owner.companionModule.moduleClass
    else sym.owner

  private def defaultGetterDenot(
      targetSym: Symbol,
      sym: Symbol,
      paramIndex: Int
  ): Denotation =
    targetSym.info.memberBasedOnFlags(
      DefaultGetterName(sym.name.asTermName, paramIndex),
      excluded = Bridge
    )
  ...
  private def genResult(
    ...
    def receiver =
      if static then genLoadModule(sym.owner)
      else js.This()(currentThisType)
  ...
  private def genThrowTypeError(
    ...
    js.Throw(
      js.JSNew(js.JSGlobalRef("TypeError"), js.StringLiteral(msg) :: Nil)
    )
  ...
  abstract class Exported(
    ...
    val maxNonRepeatedArgc =
      if hasRepeatedParam then params.size - 1 else params.size
    ...
    tpe match
      ...
      case _ =>
        ...
        (toIRType(tpe): @unchecked) match
          ...
          case jstpe.ClassType(Names.BoxedUnitClass) =>
            PrimitiveTypeTest(jstpe.UndefType, 0)
          case jstpe.ClassType(Names.BoxedStringClass) =>
            PrimitiveTypeTest(jstpe.StringType, 9)
  ...
  class FormalArgsRegistry(val minArgc: Int, needsRestParam: Boolean):
    ...
    def genFormalArgs()(implicit
      ...
      val restParam =
        if needsRestParam then
          Some(
            js.ParamDef(
              js.LocalIdent(restParamName),
              NoOriginalName,
              jstpe.AnyType,
              mutable = false
            )
    ...
    def genAllArgsRefsForForwarder()(implicit
        pos: Position
