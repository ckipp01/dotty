...
class JSPositions()(using Context):
  ...
  private val sourceURIMaps: List[URIMap] =
    ctx.settings.scalajsMapSourceURI.value.flatMap { option =>
      ...
      if uris.length != 1 && uris.length != 2 then
        report.error(
          "-scalajs-mapSourceURI needs one or two URIs as argument (separated by '->')."
        )
        Nil
      else
        try
  ...
  private def sourceAndSpan2irPos(source: SourceFile, span: Span): ir.Position =
    if !span.exists then ir.Position.NoPosition
    else
  ...
  /** Implicitly materializes an ir.Position from an implicit dotty
    * SourcePosition.
    */
  implicit def implicitSourcePos2irPos(implicit
