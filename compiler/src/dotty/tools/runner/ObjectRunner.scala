...
/** This is a copy implementation from scala/scala scala.tools.nsc.CommonRunner
  * trait
  */
trait CommonRunner:
  /** Run a given object, specified by name, using a specified classpath and
    * argument list.
    *
  ...
  /** Catches any non-fatal exception thrown by run (in the case of
    * InvocationTargetException, unwrapping it) and returns it in an Option.
    */
  def runAndCatch(
      urls: Seq[URL],
      objectName: String,
    ...
    try
      run(urls, objectName, arguments); None
