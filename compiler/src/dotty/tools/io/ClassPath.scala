...
trait ClassPath:
  ...
  final def packages(inPackage: String): Seq[PackageEntry] = packages(
    PackageName(inPackage)
  )
  final def classes(inPackage: String): Seq[ClassFileEntry] = classes(
    PackageName(inPackage)
  )
  final def sources(inPackage: String): Seq[SourceFileEntry] = sources(
    PackageName(inPackage)
  )
  final def list(inPackage: String): ClassPathEntries = list(
    PackageName(inPackage)
  )
  ...
  /** Returns packages and classes (source or classfile) that are members of
    * `inPackage` (not recursively). The `inPackage` contains a full package
    * name, e.g., "scala.collection".
    *
    * This is the main method uses to find classes, see class `PackageLoader`.
    * The `rootMirror.rootLoader` is created with `inPackage = ""`.
    */
  private[dotty] def list(inPackage: PackageName): ClassPathEntries

  /** Returns the class file and / or source file for a given external name,
    * e.g., "java.lang.String". If there is both a class file and source file,
    * the compiler can decide whether to read the class file or compile the
    * source file.
    *
    * Internally this seems to be used only by `ScriptRunner`, but only to call
    * `.isDefined`. That could probably be implemented differently.
    *
    ...
    * Jason has some improvements for that in the works
    * (https://github.com/scala/bug/issues/10289#issuecomment-310022699)
    */
  def findClass(className: String): Option[ClassRepresentation] =
    ...
    val foundClassFromClassFiles =
      classes(packageName).find(_.name == simpleClassName)
    def findClassInSources =
      sources(packageName).find(_.name == simpleClassName)

    foundClassFromClassFiles orElse findClassInSources

  /** Returns the classfile for an external name, e.g., "java.lang.String". This
    * method does not return source files.
    *
    * This method is used by the classfile parser. When parsing a Java class,
    * its own inner classes are entered with a `ClassfileLoader` that parses the
    * classfile returned by this method. It is also used in the backend, by the
    * inliner, to obtain the bytecode when inlining from the classpath. It's
    * also used by scalap.
    */
...
trait EfficientClassPath extends ClassPath:
  ...
  override def list(inPackage: PackageName): ClassPathEntries =
    ...
    if packageBuf.isEmpty && classRepBuf.isEmpty then ClassPathEntries.empty
...
object ClassPath:
  ...
  private def expandS(pattern: String): List[String] =
    ...
    if pattern == "*" then lsDir(Directory("."))
    // On Windows the JDK supports forward slash or backslash in classpath entries
    else if pattern.endsWith(wildSuffix) || pattern.endsWith("/*") then
  ...
  def expandPath(path: String, expandStar: Boolean = true): List[String] =
    if expandStar then split(path).flatMap(expandS)
    else split(path)
  ...
  def expandManifestPath(jarPath: String): List[URL] =
    ...
    if !file.isFile then return Nil
...
trait ClassRepresentation:
  ...
  final def nameLength: Int =
    ...
    if ix < 0 then fileName.length else ix
