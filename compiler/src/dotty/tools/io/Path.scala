...
/** An abstraction for filesystem paths. The differences between Path, File, and
  * Directory are primarily to communicate intent. Since the filesystem can
  * change at any time, there is no way to reliably associate Files only with
  * files and so on. Any Path can be converted to a File or Directory (and thus
  * gain access to the additional entity specific methods) by calling toFile or
  * toDirectory, which has no effect on the filesystem.
  *
  * Also available are createFile and createDirectory, which attempt to create
  ...
  */
object Path:
  def isExtensionJarOrZip(jpath: JPath): Boolean = isExtensionJarOrZip(
  ...
  def extension(name: String): String =
    var i = name.length - 1
    while i >= 0 && name.charAt(i) != '.' do i -= 1
...
/** The Path constructor is private so we can enforce some semantics regarding
  * how a Path might relate to the world.
  *
  * ''Note: This library is considered experimental and should not be used
  * unless you know what you are doing.''
  */
class Path private[io] (val jpath: JPath):
  ...
  def toAbsolute: Path =
    if isAbsolute then this else new Path(jpath.toAbsolutePath)
  def toCanonical: Path = normalize.toAbsolute
  def toURI: URI = jpath.toUri()
  def toURL: URL = toURI.toURL()

  /** If this path is absolute, returns it: otherwise, returns an absolute path
    * made up of root / this.
    */
  def toAbsoluteWithRoot(root: Path): Path =
    if isAbsolute then this else root.toAbsolute / this

  /** Creates a new Path with the specified path appended. Assumes the type of
    * the new component implies the type of the result.
    */
  ...
  /** If this path is a directory, recursively iterate over its contents. The
    * supplied condition is a filter which is applied to each element, with that
    * branch of the tree being closed off if it is false. So for example if the
    * condition is false for some subdirectory, nothing under that directory
    * will be in the Iterator. If it's true, all files for which the condition
    * holds and are directly in that subdirectory are in the Iterator, and all
    * sub-subdirectories are recursively evaluated
    */
  ...
  def addExtension(ext: String): Path = new Path(
    jpath.resolveSibling(name + ext)
  )
  ...
  def createDirectory(
    ...
    val res = tryCreate(
      if force then Files.createDirectories(jpath)
      else Files.createDirectory(jpath)
    )
    if !res && failIfExists && exists then
      fail("Directory '%s' already exists." format name)
    else if isDirectory then toDirectory
  ...
  // deletions
  def delete(): Unit =
    try Files.deleteIfExists(jpath)
