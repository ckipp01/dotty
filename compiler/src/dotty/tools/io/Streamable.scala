...
import java.io.{BufferedReader, InputStreamReader, Closeable as JCloseable}
...
/** Traits for objects which can be represented as Streams.
  ...
  */
object Streamable:
  /** Traits which can be viewed as a sequence of bytes. Source types which know
    * their length should override def length: Long for more efficient method
    * implementations.
    *
    * ''Note: This library is considered experimental and should not be used
    * unless you know what you are doing.''
  ...
  trait Bytes:
    ...
    def bufferedInput(): BufferedInputStream = new BufferedInputStream(
      inputStream()
    )
    ...
    /** This method aspires to be the fastest way to read a stream of known
      * length into memory.
      */
    def toByteArray(): Array[Byte] =
      // if we don't know the length, fall back on relative inefficiency
      if length == -1L then return (new ArrayBuffer[Byte]() ++= bytes()).toArray
  ...
  trait Chars extends Bytes:
    /** Calls to methods requiring byte<->char transformations should be offered
      ...
      * the one discovered at creation time will be used, which will always find
      * the one in scala.io.Codec if no other is available. This can be
      * overridden to use a different default.
      */
    ...
    def chars(codec: Codec): BufferedSource =
      Source.fromInputStream(inputStream())(codec)

    /** Beware! Leaks an InputStream which will not be closed until it gets
      * finalized.
      */
    ...
    def bufferedReader(codec: Codec): BufferedReader = new BufferedReader(
      reader(codec)
    )

    /** Creates a BufferedReader and applies the closure, automatically closing
      * it on completion.
      */
