...
class Directory(jpath: JPath) extends Path(jpath):
  override def toAbsolute: Directory =
    if isAbsolute then this else super.toAbsolute.toDirectory
  ...
  /** If optional depth argument is not given, will recurse until it runs out of
    * contents.
    */
  def deepList(depth: Int = -1): Iterator[Path] =
    if depth < 0 then list ++ dirs.flatMap(_.deepList(depth))
    else if depth == 0 then Iterator.empty
