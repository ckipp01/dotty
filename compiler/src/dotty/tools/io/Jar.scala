...
class JarWriter(val file: File, val manifest: Manifest):
  ...
  /** Adds a jar entry for the given path and returns an output stream to which
    * the data should immediately be written. This unusual interface exists to
    * work with fjbg.
    */
  ...
  def addEntry(entry: Path, prefix: String): Unit =
    if entry.isFile then addFile(entry.toFile, prefix)
...
object Jar:
  ...
  object WManifest:
    def apply(mainAttrs: (Attributes.Name, String)*): WManifest =
      ...
      for (k, v) <- mainAttrs do m(k) = v

      m
  implicit class WManifest(val manifest: JManifest):
    for (k, v) <- initialMainAttrs do this(k) = v

    def underlying: JManifest = manifest
    def attrs: mutable.Map[Name, String] = manifest
      .getMainAttributes()
    ...
    def update(key: Attributes.Name, value: String): Option[String] =
      attrs.put(key, value)
    ...
    def mainClass_=(value: String): Option[String] =
      update(Name.MAIN_CLASS, value)
