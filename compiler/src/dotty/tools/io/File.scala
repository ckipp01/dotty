...
/** An abstraction for files. For character data, a Codec can be supplied at
  * either creation time or when a method involving character data is called
  * (with the latter taking precedence if supplied.) If neither is available,
  * the value of scala.io.Codec.default is used.
  *
  * @author
  *   Paul Phillips
  * @since 2.8
  *
  * ''Note: This is library is considered experimental and should not be used
  * unless you know what you are doing.''
  */
class File(jpath: JPath)(implicit constructorCodec: Codec)
    extends Path(jpath)
  ...
  override def toAbsolute: File =
    if isAbsolute then this else super.toAbsolute.toFile
  ...
  /** Obtains an OutputStreamWriter wrapped around a FileOutputStream. This
    * should behave like a less broken version of java.io.FileWriter, in that
    * unlike the java version you can specify the encoding.
    */
  ...
  def bufferedWriter(append: Boolean): BufferedWriter =
    bufferedWriter(append, creationCodec)
  ...
  /** Calls println on each string (so it adds a newline in the PrintWriter
    * fashion.)
    */
  ...
  def setExecutable(executable: Boolean, ownerOnly: Boolean = true): Boolean =
    ...
    try
      method
        .invoke(jpath.toFile, executable: JBoolean, ownerOnly: JBoolean)
