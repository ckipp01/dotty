...
/** An abstraction for zip files and streams. Everything is written the way it
  * is for performance: we come through here a lot on every run. Be careful
  * about changing it.
  *
  * @author
  ...
  */
object ZipArchive:
  private[io] val closeZipFile: Boolean =
    sys.props.get("scala.classpath.closeZip").exists(_.toBoolean)
  ...
  def fromPath(jpath: JPath): FileZipArchive =
    new FileZipArchive(jpath, release = None)
...
/** ''Note: This library is considered experimental and should not be used
  * unless you know what you are doing.''
  */
abstract class ZipArchive(override val jpath: JPath, release: Option[String])
  ...
  /** ''Note: This library is considered experimental and should not be used
    * unless you know what you are doing.''
    */
  sealed abstract class Entry(path: String, val parent: Entry)
      extends VirtualFile(baseName(path), path):
  ...
  protected def getDir(
    ...
    if entry.isDirectory then ensureDir(dirs, entry.getName)
...
/** ''Note: This library is considered experimental and should not be used
  * unless you know what you are doing.''
  */
final class FileZipArchive(jpath: JPath, release: Option[String])
  ...
  private class LazyEntry(
    ...
    override def input: InputStream =
      ...
      val entry = zipFile.getEntry(
        name
      ) // with `-release`, returns the correct version under META-INF/versions
  ...
  @volatile lazy val (root, allDirs)
      : (DirEntry, collection.Map[String, DirEntry]) =
    ...
    try
      while entries.hasMoreElements do
        ...
        if !zipEntry.getName.startsWith("META-INF/versions/") then
          val zipEntryVersioned =
            if release.isDefined then
          ...
          if !zipEntry.isDirectory then
            ...
            val f =
              if ZipArchive.closeZipFile then
                new LazyEntry(
                  zipEntry.getName(),
                  zipEntry.getTime(),
              ...
              else new LeakyEntry(zipFile, zipEntryVersioned, dir)

            dir.entries(f.name) = f
    finally
      if ZipArchive.closeZipFile then zipFile.close()
  ...
  override def canEqual(other: Any): Boolean =
    other.isInstanceOf[FileZipArchive]
...
final class ManifestResources(val url: URL) extends ZipArchive(null, None):
  ...
  override def canEqual(other: Any): Boolean =
    other.isInstanceOf[ManifestResources]
