...
class CoursierScalaTests:
  ...
  extension (str: String)
    private def dropExtension =
      str.reverse.dropWhile(_ != '.').drop(1).reverse

  // classpath tests are managed by scripting.ClasspathTests.scala
  def testFiles = scripts("/scripting").filter {
    !_.getName.startsWith("classpath")
  }
  ...
  @Test def allTests =
    def scriptArgs() =
      val scriptPath =
        scripts("/scripting").find(_.getName == "showArgs.sc").get.absPath
      val testScriptArgs =
        Seq("a", "b", "c", "-repl", "-run", "-script", "-debug")
    ...
    def scriptPath() =
      val scriptPath =
        scripts("/scripting").find(_.getName == "scriptPath.sc").get.absPath
      val args = scriptPath
      val output = CoursierScalaTests.csScalaCmd(args)
    ...
    def scriptEnvDashJDashD() =
      val scriptPath =
        scripts("/scripting").find(_.getName == "envtest.sc").get.absPath
      val args = scriptPath
    ...
    def emptyArgsEqualsRepl() =
      ...
      assertTrue(
        output.mkString("\n").contains("Unable to create a system terminal")
      ) // Scala attempted to create REPL so we can assume it is working
    emptyArgsEqualsRepl()

    def run() =
      val output = CoursierScalaTests.csScalaCmd(
        "-classpath",
        scripts("/run").head.getParentFile.getParent,
        "-run",
        "run.myfile"
      )
      assertEquals(output.mkString("\n"), "Hello")
    ...
    def jar() =
      ...
      assertEquals(output.mkString("\n"), "Hello")
      assertTrue(
        source.getParentFile.listFiles.find(_.getName == "myfile.jar").isDefined
      )
    jar()
    ...
    def compileFilesToJarAndRun() =
      ...
      val output1 =
        CoursierScalaTests.csScalaCompilerCmd("-d", o1source, source.absPath)
      assertEquals(output1.mkString("\n"), "")

      val o2source = Paths.get(prefix, "custommain.jar").toAbsolutePath.toString
      val output2 = CoursierScalaTests.csScalaCompilerCmd(
        "-d",
        o2source,
        "-Xmain-class",
        "run.myfile",
...
object CoursierScalaTests:
  ...
  private def csCmd(entry: String, options: String*): List[String] =
    ...
    val newOptions = args match
      ...
      case _   => "--" +: args
    val newJOpts =
      jOpts.map(s => s"--java-opt ${s.stripPrefix("-J")}").mkString(" ")
    execCmd(
      "./cs",
      (s"""launch "org.scala-lang:scala3-compiler_3:${sys.env(
          "DOTTY_BOOTSTRAPPED_VERSION"
        )}" $newJOpts --main-class "$entry" --property "scala.usejavacp=true"""" +: newOptions)*
    )._2
  ...
  @BeforeClass def setup(): Unit =
    ...
    def runAndCheckCmd(cmd: String, options: String*): Unit =
      ...
      if code != 0 then
        fail(s"Failed to run $cmd ${options
            .mkString(" ")}, exit code: $code, output: ${out.mkString("\n")}")
