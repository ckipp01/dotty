...
/** Makes using ASM from tests more convenient.
  *
  * Wraps ASM instructions in case classes so that equals and toString work for
  * the purpose of bytecode diffing and pretty printing.
  */
object ASMConverters:
  import asm.{tree as t}

  /** Transform the instructions of an ASM Method into a list of
    * [[Instruction]]s.
    */
  def instructionsFromMethod(meth: t.MethodNode): List[Instruction] =
    new AsmToScala(meth).instructions
  ...
  extension (self: List[Instruction])
    ...
    def dropLinesFrames = self.filterNot(i =>
    ...
    def dropStaleLabels: List[Instruction] =
      ...
      val usedLabels: Set[Instruction] =
        self.iterator.flatMap(self.referencedLabels(_)).toSet
  ...
  def opcodeToString(op: Int, default: Any = "?"): String =
    ...
    if OPCODES.isDefinedAt(op) then OPCODES(op) else default.toString
  ...
  case class Field(opcode: Int, owner: String, name: String, desc: String)
      extends Instruction
  ...
  class AsmToScala(asmMethod: t.MethodNode):
    ...
    def method: Method = Method(
      instructions,
      convertHandlers(asmMethod),
      convertLocalVars(asmMethod)
    )

    private def labelIndex(l: t.LabelNode): Int =
      asmMethod.instructions.indexOf(l)

    private def op(i: t.AbstractInsnNode): Int = i.getOpcode

    private def lst[T](xs: java.util.List[T]): List[T] =
      if xs == null then Nil else xs.asScala.toList
    ...
    private def convertBsmArgs(a: Array[Object]): List[Object] = a.iterator
      .map({
    ...
    private def convertMethodHandle(h: asm.Handle): MethodHandle =
      MethodHandle(h.getTag, h.getOwner, h.getName, h.getDesc, h.isInterface)

    private def convertHandlers(method: t.MethodNode): List[ExceptionHandler] =
      method.tryCatchBlocks.asScala
        .map(h =>
          ExceptionHandler(
            applyLabel(h.start),
            applyLabel(h.end),
  ...
  /** Bytecode is equal modulo local variable numbering and label numbering.
    */
  def equivalentBytecode(
      as: List[Instruction],
      bs: List[Instruction],
      varMap: MMap[Int, Int] = MMap(),
      labelMap: MMap[Int, Int] = MMap()
  ): Boolean =
    ...
    if as.isEmpty then bs.isEmpty
    else if bs.isEmpty then false
    else
  ...
  /** Convert back a [[Method]] to ASM land. The code is emitted into the
    * parameter `asmMethod`.
    */
  ...
  def unconvertMethodHandle(h: MethodHandle): asm.Handle =
    new asm.Handle(h.tag, h.owner, h.name, h.desc, h.itf)
  def unconvertBsmArgs(a: List[Object]): Array[Object] = a
    .map({
