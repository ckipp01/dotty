...
class ArrayApplyOptTest extends DottyBytecodeTest:
  ...
  @Test def testArrayEmptyGenericApply =
    test(
      ...
      List(
        Op(ICONST_0),
        TypeOp(ANEWARRAY, "java/lang/String"),
        Op(POP),
        Op(RETURN)
      )
    )
    test(
      ...
      List(
        Op(ICONST_0),
        TypeOp(ANEWARRAY, "scala/runtime/BoxedUnit"),
        Op(POP),
        Op(RETURN)
      )
    )
    test(
      ...
      List(
        Op(ICONST_0),
        TypeOp(ANEWARRAY, "java/lang/Object"),
        Op(POP),
        Op(RETURN)
      )
    )
  ...
  @Test def testArrayApplyInt =
    ...
    val init2 = List(
      ...
      Invoke(INVOKEVIRTUAL, "Foo$", "t", "()I", false),
      Op(IASTORE),
      Op(DUP),
      Op(ICONST_1),
      Field(GETSTATIC, "Foo$", "MODULE$", "LFoo$;"),
  ...
  private def newArray0Opcodes(tpe: Int, init: List[Any] = Nil): List[Any] =
    Op(ICONST_0) :: IntOp(NEWARRAY, tpe) :: init ::: Op(POP) :: Op(
      RETURN
    ) :: Nil

  private def newArray2Opcodes(tpe: Int, init: List[Any] = Nil): List[Any] =
    Op(ICONST_2) :: IntOp(NEWARRAY, tpe) :: init ::: Op(POP) :: Op(
      RETURN
    ) :: Nil
