...
class DottyBytecodeTests extends DottyBytecodeTest:
  ...
  /** This test verifies that simple matches are transformed if possible despite
    * no annotation
    */
  ...
  @Test def multidimArraysFromOfDim =
    ...
    checkBCode(source) { dir =>
      ...
      val hadCorrectInstr =
        instructionsFromMethod(method).collect {
          case x @ NewArray(op, _, dims)
              if op == Opcode.multianewarray && dims == 2 =>
            x
        }.length > 0
  ...
  @Test def arraysFromOfDim =
    ...
    checkBCode(source) { dir =>
      ...
      val arr1CorrectInstr =
        instructionsFromMethod(arr1).collect {
          case x @ IntOp(op, oprnd)
              if op == Opcode.newarray && oprnd == Opcode.int =>
            x
        }.length > 0
      ...
      val arr2CorrectInstr =
        instructionsFromMethod(arr2).collect {
          case x @ TypeOp(op, oprnd)
              if op == Opcode.anewarray && oprnd == Opcode.boxedUnit =>
            x
        }.length > 0
      ...
      val arr3CorrectInstr =
        instructionsFromMethod(arr3).collect {
          case x @ TypeOp(op, oprnd)
              if op == Opcode.anewarray && oprnd == Opcode.javaString =>
            x
        }.length > 0
  ...
  /** Verifies that arrays are not unnecessarily wrapped when passed to Java
    * varargs methods
    */
  @Test def dontWrapArraysInJavaVarargs =
    ...
    checkBCode(source) { dir =>
      ...
      assert(
        !arrayWrapped,
        "Arrays should not be wrapped when passed to a Java varargs method\n"
      )
    }
  ...
  @Test def noBoxingInSyntheticEquals =
    ...
    checkBCode(source) { dir =>
      ...
      do
        ...
        assert(
          !callsEquals,
          s"equals method should not be called in the definition of $clsName#$methodName\n"
        )
    }
  ...
  @Test def i5090 =
    ...
    checkBCode(source) { dir =>
      ...
      assert(
        testInstructions == refInstructions,
        "`T[Int]` was not properly dealias" +
          diffInstructions(testInstructions, refInstructions)
      )
    }

  /** Test that the receiver of a call to a method with varargs is not
  ...
  @Test def i5191 =
    ...
    checkBCode(source) { dir =>
      ...
      assertFalse(
        "Receiver of a call to a method with varargs is unnecessarily lifted",
        liftReceiver
      )
    }

  /** Test that the size of the lazy val initialiazer is under a certain
    * threshold
    *
  ...
  private def checkReleaseFence(
      releaseFenceExpected: Boolean,
  ...
  @Test def freshNames =
    ...
    checkBCode(List(sourceA, sourceB)) { dir =>
      val clsNodeA = loadClassNode(
        dir.lookupName("A.class", directory = false).input,
        skipDebugInfo = false
      )
      val clsNodeB = loadClassNode(
        dir.lookupName("B.class", directory = false).input,
        skipDebugInfo = false
      )
  ...
  def t9179(): Unit =
    ...
    checkBCode(code) { dir =>
      val c = loadClassNode(
        dir.lookupName("C.class", directory = false).input,
        skipDebugInfo = false
      )
      val t = getMethod(c, "t")
      ...
      assertSameCode(
        instructions.filterNot(isFrameLine),
        List(
          Label(0),
          Ldc(LDC, ""),
          VarOp(ASTORE, 1),
  ...
  @Test def i14773TailRecReuseParamSlots(): Unit =
    ...
    checkBCode(source) { dir =>
      ...
      val fooClass =
        loadClassNode(dir.lookupName("Foo.class", directory = false).input)
      ...
      val intListClass =
        loadClassNode(dir.lookupName("IntList.class", directory = false).input)
  ...
  end i14773TailRecReuseParamSlots
      ...
      assertSameCode(
        ...
        List(
          ...
          LookupSwitch(
            ...
            Label(20),
            List(1, 7, 8, 9),
            List(Label(4), Label(8), Label(12), Label(16))
          ),
          Label(4),
          IntOp(BIPUSH, 10),
          Op(IRETURN),
      ...
      assertSameCode(
        ...
        List(
          ...
          LookupSwitch(
            ...
            Label(16),
            List(1, 2, 7, 8),
            List(Label(4), Label(4), Label(8), Label(12))
          ),
          Label(4),
          IntOp(BIPUSH, 10),
          Op(IRETURN),
  ...
  @Test def ifThenElseControlFlow(): Unit =
    ...
    checkBCode(source) { dir =>
      val sourceMapWriterClass = loadClassNode(
        dir.lookupName("SourceMapWriter.class", directory = false).input
      )
      ...
      val writeBase64VLQSlowPathMeth =
        getMethod(sourceMapWriterClass, "writeBase64VLQSlowPath$1")

      assertSameCode(
        writeBase64VLQSlowPathMeth,
        List(
          VarOp(ILOAD, 1),
          VarOp(ISTORE, 2),
          Label(2),
  ...
  def invocationReceivers(): Unit =
    ...
    checkBCode(List(invocationReceiversTestCode.definitions("Object"))) { dir =>
      val c1 =
        loadClassNode(dir.lookupName("C1.class", directory = false).input)
      val c2 =
        loadClassNode(dir.lookupName("C2.class", directory = false).input)
      assertSameCode(
        getMethod(c1, "clone"),
        List(
          VarOp(ALOAD, 0),
          Invoke(INVOKESTATIC, "T", "clone$", "(LT;)Ljava/lang/Object;", true),
          Op(ARETURN)
        )
      )
    ...
    checkBCode(List(invocationReceiversTestCode.definitions("String"))) { dir =>
      ...
      def ms(c: ClassNode, n: String) =
        c.methods.asScala.toList.filter(_.name == n)
      assert(ms(tb, "clone").length == 1)
      assert(ms(ub, "clone").isEmpty)
      val List(c1Clone) =
        ms(c1b, "clone").filter(_.desc == "()Ljava/lang/Object;")
      assert((c1Clone.access | Opcodes.ACC_BRIDGE) != 0)
      assertSameCode(
        c1Clone,
        List(
          VarOp(ALOAD, 0),
          Invoke(INVOKEVIRTUAL, "C1", "clone", "()Ljava/lang/String;", false),
          Op(ARETURN)
        )
      )

      def iv(m: MethodNode) = getInstructions(c1b, "f1").collect({
        case i: Invoke => i
      })
      assertSameCode(
        iv(getMethod(c1b, "f1")),
        List(
          Invoke(INVOKEINTERFACE, "T", "clone", "()Ljava/lang/String;", true)
        )
      )
      assertSameCode(
        iv(getMethod(c1b, "f2")),
  ...
  def invocationReceiversProtected(): Unit =
    ...
    checkBCode(scalaSources = List(cC), javaSources = List(aC, bC, iC, jC)) {
      dir =>
        val clsIn = dir
          .subdirectoryNamed("b")
          .lookupName("C.class", directory = false)
          .input
        ...
        assertInvoke(
          ...
          "a/B",
          "f"
        ) // receiver needs to be B (A is not accessible in class C, package b)
        assertInvoke(getMethod(c, "f3"), "a/J", "f") // receiver needs to be J
    }
  end invocationReceiversProtected
  ...
  def deprecation(): Unit =
    ...
    checkBCode(code) { dir =>
      val c =
        loadClassNode(dir.lookupName("Test.class", directory = false).input)
  ...
  @Test def vcElideAllocations =
    ...
    checkBCode(source) { dir =>
      ...
      assertSameCode(
        instructions1.filterNot(isFrameLine),
        List(
          VarOp(ALOAD, 0),
          Invoke(INVOKEVIRTUAL, "Foo", "approx", "()I", false),
          VarOp(ALOAD, 0),
          Invoke(INVOKEVIRTUAL, "Foo", "FreshApprox", "()I", false),
          Jump(IF_ICMPNE, Label(7)),
          Op(ICONST_1),
          Jump(GOTO, Label(10)),
          Label(7),
      ...
      assertSameCode(
        instructions2.filterNot(isFrameLine),
        List(
          Op(ICONST_4),
          VarOp(ALOAD, 0),
          Invoke(INVOKEVIRTUAL, "Foo", "FreshApprox", "()I", false),
          Jump(IF_ICMPNE, Label(6)),
          Op(ICONST_1),
          Jump(GOTO, Label(9)),
          Label(6),
          Op(ICONST_0),
  ...
  @Test def i10835 =
    ...
    checkBCode(source) { dir =>
      ...
      assertTrue(
        "field is private",
        field.exists(acc => (acc & Opcodes.ACC_PRIVATE) != 0)
      )

      val methods = clsNode.methods.asScala
      def methodAccess(name: String) =
        methods.find(_.name == name).map(_.access)
  ...
  @Test def i14970 =
    ...
    checkBCode(source) { dir =>
      ...
      def testSig(methodName: String, expectedSignature: String) =
        val signature =
          clsNode.methods.asScala.filter(_.name == methodName).map(_.signature)
  ...
  @Test def i15535 =
    ...
    checkBCode(source) { dir =>
      ...
      val instructions =
        instructionsFromMethod(method).filter(_.isInstanceOf[LineNumber])
...
object invocationReceiversTestCode:
  ...
  def definitions(cloneType: String): String =
    s"""trait T { override def clone(): $cloneType = "hi" }
        ...
        |}
    """.stripMargin
end invocationReceiversTestCode
