...
sealed trait AsmNode[+T]:
  ...
  def characteristics =
    "%15s %-30s%s%s".format(name, desc, accessString, sigString)
  def erasedCharacteristics = "%15s %-30s%s".format(name, desc, accessString)

  private def accessString =
    if access == 0 then "" else " " + Modifier.toString(access)
  private def sigString = if signature == null then "" else " " + signature
