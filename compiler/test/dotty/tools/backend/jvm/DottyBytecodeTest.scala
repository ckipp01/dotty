...
import java.io.{File as JFile, InputStream}
...
trait DottyBytecodeTest:
  ...
  def checkBCode(scalaSources: List[String], javaSources: List[String] = Nil)(
      checkOutput: AbstractFile => Unit
  ): Unit =
  ...
  def getGeneratedClassfiles(
    ...
    def files(dir: AbstractFile): List[(String, Array[Byte])] =
      ...
      for f <- dir.iterator do
        if !f.isDirectory then res += ((f.name, f.toByteArray))
        else if f.name != "." && f.name != ".." then res ++= files(f)
      res.toList
    files(outDir)

  protected def loadClassNode(
      input: InputStream,
      skipDebugInfo: Boolean = true
  ): ClassNode =
    val cr = new ClassReader(input)
    val cn = new ClassNode()
    cr.accept(cn, if skipDebugInfo then ClassReader.SKIP_DEBUG else 0)
    cn
  ...
  def diffInstructions(isa: List[Instruction], isb: List[Instruction]): String =
    ...
    if len > 0 then
      ...
      (1 to len) foreach { line =>
        ...
        sb append (s"""$line${" " * (lineWidth - line.toString.length)} ${
            if a == b then "==" else "<>"
          } $a${" " * (width - a.length)} | $b\n""")
      }
    sb.toString
  ...
  def verifySwitch(
    ...
    val succ = instructions.collect {
      case x: TableSwitch  => x
      case x: LookupSwitch => x
    }.length > 0

    if debug || !succ && !shouldFail || succ && shouldFail then
  ...
  /** Same as sameMethodAndFieldSignatures, but ignoring generic signatures.
    ...
    * values does not.
    */
  def sameMethodAndFieldDescriptors(
      clazzA: ClassNode,
      clazzB: ClassNode
  ): Unit =
    val (succ, msg) =
  ...
  private def sameCharacteristics(clazzA: ClassNode, clazzB: ClassNode)(
    ...
    if ms1.length != ms2.length then
      ...
      val success = ms1.lazyZip(ms2) forall { (m1, m2) =>
        ...
        if c1 == c2 then msg append (s"[ok] $m1")
        else msg append (s"[fail]\n  in $name1: $c1\n  in $name2: $c2")
  ...
  def correctNumberOfNullChecks(expectedChecks: Int, insnList: InsnList) =
    /** Is given instruction a null check?
      *
      * This will detect direct null comparison as in if (x == null) ... and not
      * indirect as in val foo = null if (x == foo) ...
      */
  ...
  def assertBoxing(nodeName: String, methods: java.lang.Iterable[MethodNode])(
      implicit source: String
  ): Unit =
    methods.asScala
      .find(_.name == nodeName)
      .map { node =>
        val (ins, boxed) = boxingInstructions(node)
        if !boxed then fail("No boxing in:\n" + boxingError(ins, source))
      }
  ...
  protected def assertNoBoxing(
      nodeName: String,
      methods: java.lang.Iterable[MethodNode]
  )(implicit source: String): Unit =
    methods.asScala
      .find(_.name == nodeName)
      .map { node =>
        val (ins, boxed) = boxingInstructions(node)
        if boxed then fail(boxingError(ins, source))
      }
      .getOrElse(fail("Could not find constructor for object `Test`"))
