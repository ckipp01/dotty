...
class DocTests extends ReplTest:
  ...
  @Test def docOfDef =
    eval("/** doc */ def foo = 0") andThen assertEquals("doc", doc("foo"))

  @Test def docOfVal =
    eval("/** doc */ val foo = 0") andThen assertEquals("doc", doc("foo"))

  @Test def docOfObject =
    eval("/** doc */ object Foo") andThen assertEquals("doc", doc("Foo"))

  @Test def docOfClass =
    eval("/** doc */ class Foo") andThen assertEquals("doc", doc("new Foo"))

  @Test def docOfTrait =
    eval("/** doc */ trait Foo") andThen assertEquals("doc", doc("new Foo"))

  /*@Test*/
  def docOfExtension1 =
    eval("/** doc */ extension (x: Int) def foo = 0") andThen assertEquals(
      "doc",
      doc("extension_foo")
    )
  ...
  /*@Test*/
  def docOfExtension2 =
    eval("extension (x: Int) /** doc */ def foo = 0") andThen assertEquals(
      "doc",
      doc("extension_foo")
    )
  ...
  def docOfExtension3 =
    eval(
      "/** doc0 */ extension (x: Int) { /** doc1 */ def foo = 0; /** doc2 */ def bar = 0; def baz = 0 }"
    ) andThen {
  ...
  @Test def docOfDefInObject = eval(
    "object O { /** doc */ def foo = 0 }"
  ) andThen assertEquals("doc", doc("O.foo"))

  @Test def docOfValInObject = eval(
    "object O { /** doc */ val foo = 0 }"
  ) andThen assertEquals("doc", doc("O.foo"))

  @Test def docOfObjectInObject = eval(
    "object O { /** doc */ object Foo }"
  ) andThen assertEquals("doc", doc("O.Foo"))

  @Test def docOfClassInObject = eval(
    "object O { /** doc */ class Foo }"
  ) andThen assertEquals("doc", doc("new O.Foo"))

  @Test def docOfTraitInObject = eval(
    "object O { /** doc */ trait Foo }"
  ) andThen assertEquals("doc", doc("new O.Foo"))

  @Test def docOfDefInClass =
    eval("""class C { /** doc */ def foo = 0 }
        |val c = new C
      """.stripMargin) andThen {
