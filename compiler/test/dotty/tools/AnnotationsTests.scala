...
class AnnotationsTest:
  ...
  @Test def annotTreeNotErased: Unit =
    withJavaCompiled(
      ...
      VirtualJavaSource("A.java", "@Annot(values = {}) public class A {}")
    ) { javaOutputDir =>
      inCompilerContext(
        javaOutputDir.toString + File.pathSeparator + TestConfiguration.basicClasspath
      ) {
        ...
        atPhase(erasurePhase.next) {
          ...
          assert(
            arg.tpe.isInstanceOf[AppliedType] && res,
  ...
  @Test def surviveMissingAnnot: Unit =
    withJavaCompiled(
      VirtualJavaSource("Annot1.java", "public @interface Annot1 {}"),
      VirtualJavaSource("Annot2.java", "public @interface Annot2 {}"),
      VirtualJavaSource("A.java", "@Annot1() @Annot2() public class A {}")
    ) { javaOutputDir =>
      ...
      inCompilerContext(
        ...
        assert(
          !ctx.reporter.hasErrors && !ctx.reporter.hasWarnings,
  ...
  @Test def hasNativeAnnot: Unit =
    inCompilerContext(TestConfiguration.basicClasspath) {
      val term: TermSymbol =
        requiredClass("java.lang.invoke.MethodHandle").requiredMethod(
          "invokeExact"
        )
