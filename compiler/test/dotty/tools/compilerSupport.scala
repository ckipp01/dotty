...
/** Initialize a compiler context with the given `classpath`, compile all
  ...
  *
  * If `separateRun` is true , then `op` will be run in a new run different from
  * the one used to compile the sources, this makes it easier to test for
  * potential issues involving retrieving symbols defined in a previous run.
  */
def inCompilerContext[T](
    classpath: String,
    separateRun: Boolean = true,
    scalaSources: String*
)(op: Context ?=> T): T =
...
def withJavaCompiled[T](javaSources: JavaFileObject*)(op: Path => T): T =
  ...
  try
    ...
    javac
      .getTask(null, null, null, options.asJava, null, javaSources.asJava)
      .call();
    op(javaOutputDir)
  finally deleteDirectory(javaOutputDir.toFile)

/** Recursively delete a directory. */
def deleteDirectory(directory: File): Unit =
  directory.listFiles.toList.foreach { file =>
    if file.isDirectory then deleteDirectory(file)
    file.delete()
  }

class VirtualJavaSource(fileName: String, code: String)
