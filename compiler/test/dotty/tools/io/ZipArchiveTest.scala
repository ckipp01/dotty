...
class ZipArchiveTest:
  ...
  private val bootClassLoader: ClassLoader =
    if !util.Properties.isJavaAtLeast("9") then null
    else
      try
        MethodHandles
          .lookup()
  ...
  @Test def `manifest resources just works`(): Unit =
    ...
    try
      ...
      try Files.delete(jar)
      catch case _: IOException => ()

  private def createTestJar(): Path =
    Files.createTempFile("junit", ".jar").tap { f =>
      val man = new Manifest()
      man.getMainAttributes().put(Attributes.Name.MANIFEST_VERSION, "1.0")
      man.getEntries().put("foo.class", new Attributes(0))
      Using.resource(new JarOutputStream(Files.newOutputStream(f), man)) {
        jout =>
  ...
  private def createTestZip(): Path =
    Files.createTempFile("junit", ".zip").tap { f =>
