...
def assertThrows[T <: Throwable: ClassTag](p: T => Boolean)(
    body: => Any
): Unit =
  try
    body
    ...
    case e: T if p(e) => ()
    case failed: T =>
      throw AssertionError(s"Exception failed check: $failed").tap(
        _.addSuppressed(failed)
      )
    case NonFatal(other) =>
      throw AssertionError(
        s"Wrong exception: expected ${implicitly[ClassTag[T]]} but was ${other.getClass.getName}"
      ).tap(_.addSuppressed(other))
end assertThrows
...
object ToolName:
  def named(s: String): ToolName = values
    .find(_.toString.equalsIgnoreCase(s))
    .getOrElse(throw IllegalArgumentException(s))

type ToolArgs = Map[ToolName, List[String]]

/** Take a prefix of each file, extract tool args, parse, and combine. Arg
  * parsing respects quotation marks. Result is a map from ToolName to the
  * combined tokens.
  */
def toolArgsFor(files: List[JPath], charset: Charset = UTF_8): ToolArgs =
  files.foldLeft(Map.empty[ToolName, List[String]]) { (res, path) =>
    val toolargs = toolArgsParse(
      resource(Files.lines(path, charset))(_.limit(10).toScala(List))
    )
    toolargs.foldLeft(res) { case (acc, (tool, args)) =>
...
def toolArgsFor(tool: ToolName)(lines: List[String]): List[String] =
  toolArgsParse(lines)
    .collectFirst {
      case (name, args) if tool eq ToolName.named(name) =>
        CommandLineParser.tokenize(args)
    }
    .getOrElse(Nil)
...
private val toolArg =
  raw"(?://|/\*| \*) ?(?i:(${ToolName.values.mkString("|")})):((?:[^*]|\*(?!/))*)".r.unanchored
...
class ToolArgsTest:
  @Test def `missing toolarg is absent`: Unit =
    assertEquals(Nil, toolArgsParse(List("")))
  @Test def `toolarg is present`: Unit = assertEquals(
    ("test", " -hey") :: Nil,
    toolArgsParse("// test: -hey" :: Nil)
  )
  @Test def `tool is present`: Unit = assertEquals(
    "-hey" :: Nil,
    toolArgsFor(ToolName.Test)("// test: -hey" :: Nil)
  )
  @Test def `missing tool is absent`: Unit =
    assertEquals(Nil, toolArgsFor(ToolName.Javac)("// test: -hey" :: Nil))
  @Test def `multitool is present`: Unit =
    assertEquals(
      "-hey" :: Nil,
      toolArgsFor(ToolName.Test)("// test: -hey" :: "// javac: -d /tmp" :: Nil)
    )
    assertEquals(
      "-d" :: "/tmp" :: Nil,
      toolArgsFor(ToolName.Javac)("// test: -hey" :: "// javac: -d /tmp" :: Nil)
    )
end ToolArgsTest
