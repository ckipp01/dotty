...
import java.io.{File as JFile, IOException, PrintStream, ByteArrayOutputStream}
...
trait ParallelTesting extends RunnerOrchestration:
  ...
  /** A list of strings which is used to filter which tests to run, if `Nil`
    * will run all tests. All absolute paths that contain any of the substrings
    * in `testFilter` will be run
    */
  ...
  /** A test source whose files or directory of files is to be compiled in a
    * specific way defined by the `Test`
    */
  sealed trait TestSource:
    self =>
    def name: String
    def outDir: JFile
    def flags: TestFlags
    ...
    def runClassPath: String =
      outDir.getPath + JFile.pathSeparator + flags.runClassPath

    def title: String = self match
      case self: JointCompilationSource =>
        if self.files.length > 1 then name
    ...
    def withFlags(newFlags0: String*) =
      ...
      if !flags.options.containsSlice(newFlags) then
        self match
          case self: JointCompilationSource =>
    ...
    lazy val allToolArgs: ToolArgs =
      toolArgsFor(
        sourceFiles.toList.map(_.toPath),
        getCharsetFromEncodingOpt(flags)
      )
  ...
  private case class JointCompilationSource(
    ...
    override def toString() = sourceFiles match
      case Array(f) => f.getPath
      case _        => outDir.getPath
  ...
  case class SeparateCompilationSource(
    ...
    lazy val compilationGroups: List[(Group, Array[JFile])] =
      ...
      def groupFor(file: JFile): Group =
        val groupSuffix = file.getName
          .dropWhile(_ != '_')
          .stripSuffix(".scala")
          .stripSuffix(".java")
        val groupSuffixParts = groupSuffix.split("_")
        val ordinal = groupSuffixParts
          .collectFirst { case Ordinal(n) => n.toInt }
          .getOrElse(Int.MinValue)
        val compiler =
          groupSuffixParts.collectFirst { case Compiler(c) => c }.getOrElse("")
    ...
    failedTests.forall(rerun =>
      testSource match
        case JointCompilationSource(_, files, _, _, _, _) =>
          rerun.exists(filter =>
            files.exists(file => file.getPath.contains(filter))
          )
        case SeparateCompilationSource(_, dir, _, _) =>
          rerun.exists(dir.getPath.contains)
    )

  private trait CompilationLogic:
    this: Test =>
    def suppressErrors = false

    /** Compiles the test source.
      * @return
      *   The reporters containing the results of all the compilation runs for
      *   this test source.
      */
    private final def compileTestSource(
        testSource: TestSource
    ): Try[List[TestReporter]] =
      Try(testSource match
        case testSource @ JointCompilationSource(
              name,
              files,
              flags,
              outDir,
          ...
          val reporter =
            if fromTasty then compileFromTasty(flags, suppressErrors, outDir)
        ...
        case testSource @ SeparateCompilationSource(_, dir, flags, outDir) =>
          testSource.compilationGroups.map { (group, files) =>
            ...
            else compileWithOtherCompiler(group.compiler, files, flags, outDir)
          }
      )

    final def countErrorsAndWarnings(reporters: Seq[TestReporter]): (Int, Int) =
      reporters.foldLeft((0, 0)) { case ((err, warn), r) =>
        (err + r.errorCount, warn + r.warningCount)
      }

    final def countErrors(reporters: Seq[TestReporter]) =
      countErrorsAndWarnings(reporters)._1
    final def countWarnings(reporters: Seq[TestReporter]) =
      countErrorsAndWarnings(reporters)._2
    final def reporterFailed(r: TestReporter) =
      r.compilerCrashed || r.errorCount > 0

    /** For a given test source, returns a check file against which the result
      * of the test run should be compared. Is used by implementations of this
      * trait.
      */
    final def checkFile(testSource: TestSource): Option[JFile] =
      (testSource match
    ...
    /** Checks if the given actual lines are the same as the ones in the check
      * file. If not, fails the test.
      */
    final def diffTest(
        testSource: TestSource,
        checkFile: JFile,
        actual: List[String],
        reporters: Seq[TestReporter],
        logger: LoggedRunnable
    ) =
      for msg <- FileDiff.check(testSource.title, actual, checkFile.getPath) do
        ...
        if updateCheckFiles then
          ...
          val outFile = checkFile.toPath
            .resolveSibling(s"${checkFile.toPath.getFileName}.out")
            .toString
    ...
    final def encapsulatedCompilation(testSource: TestSource) =
      new LoggedRunnable:
        self =>
    ...
    /** This callback is executed once the compilation of this test source
      * finished
      */
    private final def onComplete(
        testSource: TestSource,
        reportersOrCrash: Try[Seq[TestReporter]],
        logger: LoggedRunnable
    ): Unit =
      reportersOrCrash match
        case TryFailure(exn) =>
          onFailure(
            ...
            logger,
            Some(
              s"Fatal compiler crash when compiling: ${testSource.title}:\n${exn.getMessage}${exn.getStackTrace.map("\n\tat " + _).mkString}"
            )
          )
          ...
          maybeFailureMessage(testSource, reporters) match
            case Some(msg) =>
              onFailure(
                ...
                reporters,
                logger,
                Option(msg).filter(_.nonEmpty)
              )
            case None => onSuccess(testSource, reporters, logger)
        case _ =>

    /** Based on the reporters obtained after the compilation, determines if
      * this test has failed. If it has, returns a Some with an error message.
      * Otherwise, returns None. As the conditions of failure are different for
      * different test types, this method should be overridden by the concrete
      * implementations of this trait.
      */
    def maybeFailureMessage(
        testSource: TestSource,
        reporters: Seq[TestReporter]
    ): Option[String] =
      if reporters.exists(reporterFailed) then
        Some(s"Compilation failed for: '${testSource.title}'")
      else None

    /** If the test has compiled successfully, this callback will be called. You
      * can still fail the test from this callback.
      */
    def onSuccess(
        testSource: TestSource,
        reporters: Seq[TestReporter],
    ...
    /** If the test failed to compile or the compiler crashed, this callback
      * will be called.
      */
    def onFailure(
        testSource: TestSource,
  ...
  /** Each `Test` takes the `testSources` and performs the compilation and
    * assertions according to the implementing class "neg", "run" or "pos".
    */
  private class Test(
  ...
  )(implicit val summaryReport: SummaryReporting)
      extends CompilationLogic:
    test =>
    ...
    private val filteredSources =
      val filteredByName =
        if testFilter.isEmpty then testSources
        else
          testSources.filter {
            case JointCompilationSource(_, files, _, _, _, _) =>
              testFilter.exists(filter =>
                files.exists(file => file.getPath.contains(filter))
              )
            case SeparateCompilationSource(_, dir, _, _) =>
    ...
    protected def logBuildInstructions(
        testSource: TestSource,
        reporters: Seq[TestReporter]
    ) =
    ...
    protected final def failTestSource(
        testSource: TestSource,
        reason: Failure = Generic
    ) = synchronized {
      val extra = reason match
        case TimeoutFailure(title) => s", test '$title' timed out"
        case JavaCompilationFailure(msg) =>
          s", java test sources failed to compile with: \n$msg"
        case Generic => ""
      failedTestSources.append(
        FailedTestInfo(testSource.title, s" failed" + extra)
      )
      fail(reason)
    }

    /** Prints to `System.err` if we're not suppressing all output */
    protected def echo(msg: String): Unit = if !suppressAllOutput then
    ...
    protected def compile(
      ...
      def flattenFiles(f: JFile): Array[JFile] =
        if f.isDirectory then f.listFiles.flatMap(flattenFiles)
        else Array(f)

      val files: Array[JFile] = files0.flatMap(flattenFiles)

      val toolArgs = toolArgsFor(
        files.toList.map(_.toPath),
        getCharsetFromEncodingOpt(flags0)
      )
      ...
      val testIsFiltered = toolArgs.get(ToolName.Test) match
        ...
        case Some(args) =>
          throw new IllegalStateException(
            args.mkString("unknown test option: ", ", ", "")
          )
        case None => true
      ...
      def waitForJudiciously(process: Process): Int =
        try process.waitFor()
          ...
          case _: InterruptedException =>
            try
              if process.waitFor(5L, TimeUnit.MINUTES) then process.exitValue()
              else -2
            finally Thread.currentThread.interrupt()

      def compileWithJavac(fs: Array[String]) = if fs.nonEmpty then
        val fullArgs = Array(
          "javac",
          "-encoding",
      ...
      if testIsFiltered then
        ...
        val dottyFiles =
          files.filterNot(_.getName.contains("JAVA_ONLY")).map(_.getPath)
        ...
        val javaFiles = files
          ...
          .filterNot(_.getName.contains("SCALA_ONLY"))
          .map(_.getPath)
        val javaErrors = compileWithJavac(javaFiles)

        if javaErrors.isDefined then
    ...
    private def parseErrors(
      ...
      def errorLine(line: String) =
        s"$indent┆${String.format(s"%-${pageWidth}s", stripAnsi(line))}┆\n"
      def stripAnsi(str: String): String = str.replaceAll("\u001b\\[\\d+m", "")
      def addToLast(str: String): Unit =
        diagnostics match
          case head :: tail =>
            diagnostics =
              Diagnostic.Error(s"${head.msg.message}$str", head.pos) :: tail
          case Nil =>
      var inError = false
        ...
        line match
          ...
          case error @ errorPattern(filePath, line, column) =>
            ...
            diagnostics ::= Diagnostic.Error(
              s"Compilation of $filePath with Scala $compilerVersion failed at line: $line, column: $column.\nFull error output:\n${barLine(start = true)}${errorLine(error)}",
              sourcePos
            )
    ...
    protected def compileWithOtherCompiler(
        compiler: String,
        files: Array[JFile],
        flags: TestFlags,
        targetDir: JFile
      ...
      val stdlibClasspath =
        artifactClasspath("org.scala-lang", "scala3-library_3")
      val scalacClasspath =
        artifactClasspath("org.scala-lang", "scala3-compiler_3")
      ...
      val scalacCommand =
        Array("java", "-cp", scalacClasspath, "dotty.tools.dotc.Main")
    ...
    protected def compileFromTasty(
        flags0: TestFlags,
        suppressErrors: Boolean,
        targetDir: JFile
    ): TestReporter =
    ...
    private def mkLogLevel =
      if suppressErrors || suppressAllOutput then ERROR + 1 else ERROR
    private def mkReporter =
      TestReporter.reporter(realStdout, logLevel = mkLogLevel)
    ...
    private[ParallelTesting] def executeTestSuite(): this.type =
      ...
      if filteredSources.nonEmpty then
        val pool = JExecutors.newWorkStealingPool(
          threadLimit.getOrElse(Runtime.getRuntime.availableProcessors())
        )
        val timer = new Timer()
        ...
        if logProgress then
          timer.schedule(
            (() => updateProgressMonitor(start)): TimerTask,
            100 /*ms*/,
            200 /*ms*/
          )

        val eventualResults =
          for target <- filteredSources
          yield pool.submit(encapsulatedCompilation(target))
        ...
        if !pool.awaitTermination(20, TimeUnit.MINUTES) then
          ...
          for (src, res) <- filteredSources.lazyZip(eventualResults) do
            if !res.isDone then remaining += src

          pool.shutdownNow()
          System.setOut(realStdout)
          System.setErr(realStderr)
          throw new TimeoutException(
            s"Compiling targets timed out, remaining targets: ${remaining.mkString(", ")}"
          )
        end if
        ...
        for fut <- eventualResults do
          try fut.get()
            ...
            case ee: ExecutionException
                if ee.getCause.isInstanceOf[InterruptedException] =>
              System.err.println(
        ...
        echo {
          testFilter match
            case _ :: _ =>
              s"""No files matched "${testFilter.mkString(",")}" in test"""
            case _ => "No tests available under target - erroneous test?"
        }
      end if

      this
    end executeTestSuite
  ...
  private final class RewriteTest(
      testSources: List[TestSource],
      checkFiles: Map[JFile, JFile],
      times: Int,
      threadLimit: Option[Int],
      suppressAllOutput: Boolean
  )(implicit summaryReport: SummaryReporting)
      extends Test(testSources, times, threadLimit, suppressAllOutput):
    private def verifyOutput(
        testSource: TestSource,
        reporters: Seq[TestReporter],
        logger: LoggedRunnable
    ) =
  ...
  private final class RunTest(
    ...
    private def addNoRunWarning() = if !didAddNoRunWarning then
    ...
    private def verifyOutput(
      ...
      if Properties.testsNoRun then addNoRunWarning()
        ...
        runMain(testSource.runClassPath, testSource.allToolArgs) match
          case Success(output) =>
            checkFile match
    ...
    override def onSuccess(
      ...
      verifyOutput(
        checkFile(testSource),
        testSource.outDir,
        testSource,
        countWarnings(reporters),
        reporters,
        logger
      )
  end RunTest

  private final class NegTest(
      testSources: List[TestSource],
      times: Int,
      threadLimit: Option[Int],
      suppressAllOutput: Boolean
    ...
    override def maybeFailureMessage(
        testSource: TestSource,
        reporters: Seq[TestReporter]
    ): Option[String] =
      def compilerCrashed = reporters.exists(_.compilerCrashed)
      lazy val (errorMap, expectedErrors) = getErrorMapAndExpectedCount(
        testSource.sourceFiles.toIndexedSeq
      )
      lazy val actualErrors = reporters.foldLeft(0)(_ + _.errorCount)
      lazy val (expected, unexpected) =
        getMissingExpectedErrors(errorMap, reporters.iterator.flatMap(_.errors))
      ...
      def showErrors = "-> following the errors:\n" +
        reporters
          .flatMap(
            _.allErrors
              .sortBy(_.pos.line)
              .map(e => s"${e.pos.line + 1}: ${e.message}")
          )
          .mkString(" at ", "\n at ", "")

      Option {
        if compilerCrashed then
          s"Compiler crashed when compiling: ${testSource.title}"
        else if actualErrors == 0 then
    ...
    override def onSuccess(
      ...
      checkFile(testSource).foreach(
        diffTest(
          testSource,
          _,
          reporterOutputLines(reporters),
          reporters,
          logger
        )
      )
    ...
    def getErrorMapAndExpectedCount(
        files: Seq[JFile]
    ): (HashMap[String, Integer], Int) =
            ...
            comment.findAllMatchIn(line).foreach { m =>
              m.group(2) match
                case prefix if m.group(1).isEmpty =>
                  ...
                  echo(
                    s"Warning: ${file.getCanonicalPath}:${lineNbr}: found `//${what}error` but expected `// ${what}error`, skipping comment"
                  )
    ...
    def getMissingExpectedErrors(
        errorMap: HashMap[String, Integer],
        reporterErrors: Iterator[Diagnostic]
    ): (List[String], List[String]) =
      val unexpected, unpositioned = ListBuffer.empty[String]
      // For some reason, absolute paths leak from the compiler itself...
      def relativize(path: String): String = path
        .split(JFile.separatorChar)
        .dropWhile(_ != "tests")
        .mkString(JFile.separator)
  ...
  private final class NoCrashTest(
      testSources: List[TestSource],
      times: Int,
      threadLimit: Option[Int],
      suppressAllOutput: Boolean
  ...
  /** The `CompilationTest` is the main interface to `ParallelTesting`, it can
    * be instantiated via one of the following methods:
    *
    *   - `compileFile`
    *   - `compileDir`
    *   - `compileList`
    ...
    * ```
    *
    * These tests can be customized before calling one of the execution methods,
    * for instance:
    *
    * ```
    ...
    *
    * neg tests
    * \========= Neg tests are expected to generate a certain amount of errors -
    * but not crash the compiler. In each `.scala` file, you specify the line on
    * which the error will be generated, e.g:
    *
    * ```
    ...
    *
    * run tests
    * \========= Run tests are a superset of pos tests, they both verify
    * compilation and that the compiler does not crash. In addition, run tests
    * verify that the tests are able to run as expected.
    *
    ...
    * ```
    *
    * If you are testing a directory under separate compilation, you would have:
    *
    * ```none
    * .
    * └── tests
    ...
    * ```
    *
    * In the above example, `i1513.scala` and one of the files `T_X.scala` would
    * contain a `Test` object with a main method.
    *
    * Composing tests
    * \=============== Since this is a parallel test suite, it is essential to
    * be able to compose tests to take advantage of the concurrency. This is
    * done using `aggregateTests` in the companion, which will ensure that
  ...
  final class CompilationTest private (
    ...
    def checkCompile()(implicit summaryReport: SummaryReporting): this.type =
      val test = new PosTest(
        targets,
        times,
        threadLimit,
        shouldFail || shouldSuppressOutput
      ).executeTestSuite()
      ...
      if !shouldFail && test.didFail then
        fail(
          s"Expected no errors when compiling, failed for the following reason(s):\n${reasonsForFailure(test)}\n"
        )
    ...
    /** Creates a "neg" test run, which makes sure that each test generates the
      ...
      */
    def checkExpectedErrors()(implicit
        summaryReport: SummaryReporting
    ): this.type =
      val test = new NegTest(
        targets,
        times,
        threadLimit,
        shouldFail || shouldSuppressOutput
      ...
      if shouldFail && !test.didFail && test.skipCount == 0 then
        fail(
          s"Neg test shouldn't have failed, but did. Reasons:\n${reasonsForFailure(test)}"
        )
    ...
    /** Creates a "fuzzy" test run, which makes sure that each test compiles (or
      * not) without crashing
      */
    def checkNoCrash()(implicit summaryReport: SummaryReporting): this.type =
      val test =
        new NoCrashTest(targets, times, threadLimit, shouldSuppressOutput)
          .executeTestSuite()

      cleanup()

      if test.didFail then fail("Fuzzy test shouldn't have crashed, but did")
    ...
    def checkRuns()(implicit summaryReport: SummaryReporting): this.type =
      ...
      if !shouldFail && test.didFail then
        fail(
          s"Run test failed, but should not, reasons:\n${reasonsForFailure(test)}"
        )
      else if shouldFail && !test.didFail && test.skipCount == 0 then
    ...
    /** Tests `-rewrite`, which makes sure that the rewritten files still
      * compile and agree with the expected result (if specified).
      *
    ...
    def checkRewrites()(implicit summaryReport: SummaryReporting): this.type =
      ...
      val copiedTargets = targets.map {
        case target @ JointCompilationSource(_, files, _, outDir, _, _) =>
          val files2 = files.map { f =>
            ...
            val checkFile =
              new JFile(f.getPath.replaceFirst("\\.scala$", ".check"))
            if checkFile.exists then
              checkFileMap = checkFileMap.updated(dest, checkFile)
            dest
          }
      ...
      if test.didFail then fail("Rewrite test failed")
    ...
    private def reasonsForFailure(test: Test): String =
      ...
      failureReport + test.failureReasons
        .collect {
    ...
    private def copyToDir(dir: JFile, file: JFile): JFile =
      ...
      if file.isDirectory then file.listFiles.map(copyToDir(target.toFile, _))
      target.toFile
    ...
    /** Builds a `CompilationTest` which keeps the generated output files
      ...
      * compiling a certain part of the project and then compiling a second part
  ...
  object CompilationTest:
    ...
    /** Compose test targets from `tests`
      *
      * It does this, only if all the tests are mutally compatible. Otherwise it
      ...
      * method (`pos()` / `checkExpectedErrors()`/ `run()`) will spin up a
      * thread pool with the maximum allowed level of concurrency. Doing this
      * for only a few targets does not yield any real benefit over sequential
      * compilation.
      *
    ...
    def aggregateTests(tests: CompilationTest*): CompilationTest =
      ...
      def aggregate(test1: CompilationTest, test2: CompilationTest) =
        require(
          test1.times == test2.times,
          "can't combine tests that are meant to be benchmark compiled"
        )
        require(
          test1.shouldDelete == test2.shouldDelete,
          "can't combine tests that differ on deleting output"
        )
        require(
          test1.shouldFail == test2.shouldFail,
          "can't combine tests that have different expectations on outcome"
        )
        require(
          test1.shouldSuppressOutput == test2.shouldSuppressOutput,
          "can't combine tests that both suppress and don't suppress output"
        )
        test1.copy(
          test1.targets ++ test2.targets
        ) // what if thread limit differs? currently threads are limited on aggregate only
      tests.reduce(aggregate)
    end aggregateTests
  end CompilationTest

  /** Create out directory for directory `d` */
  def createOutputDirsForDir(
      d: JFile,
      sourceDir: JFile,
      outDir: String
  ): JFile =
  ...
  private def createOutputDirsForFile(
      file: JFile,
      sourceDir: JFile,
      outDir: String
  ): JFile =
    val uniqueSubdir = file.getName.substring(0, file.getName.lastIndexOf('.'))
    val targetDir = new JFile(
      outDir + s"${sourceDir.getName}${JFile.separatorChar}$uniqueSubdir"
    )
    targetDir.mkdirs()
    targetDir
  ...
  private def checkRequirements(
    ...
    require(
      sourceDir.isDirectory && sourceDir.exists,
      "passed non-directory to `compileFilesInDir`: " + sourceDir
    )
    require(
      outDir.last == JFile.separatorChar,
      "please specify an `outDir` with a trailing file separator"
    )

  /** Separates directories from files and returns them as `(dirs, files)` */
  private def compilationTargets(
      sourceDir: JFile,
      fileFilter: FileFilter = FileFilter.NoFilter
  ): (List[JFile], List[JFile]) =
    sourceDir.listFiles.foldLeft((List.empty[JFile], List.empty[JFile])) {
      case ((dirs, files), f) =>
        if !fileFilter.accept(f.getName) then (dirs, files)
        else if f.isDirectory then (f :: dirs, files)
        else if isSourceFile(f) then (dirs, f :: files)
        else (dirs, files)
    }

  /** Compiles a single file from the string path `f` using the supplied flags
    */
  def compileFile(f: String, flags: TestFlags)(implicit
      testGroup: TestGroup
  ): CompilationTest =
  ...
  /** Compiles a directory `f` using the supplied `flags`. This method does deep
    * compilation, that is - it compiles all files and subdirectories contained
    * within the directory `f`.
    *
    * By default, files are compiled in alphabetical order. An optional seed can
    * be used for randomization.
    */
  def compileDir(
      f: String,
      flags: TestFlags,
      randomOrder: Option[Int] = None,
      recursive: Boolean = true
  )(implicit testGroup: TestGroup): CompilationTest =
    ...
    def flatten(f: JFile): Array[JFile] =
      if f.isDirectory then
        ...
        if recursive then files.flatMap(flatten) else files
      else Array(f)
    ...
    val targetDir = new JFile(
      outDir + JFile.separator + sourceDir.getName + JFile.separator
    )
    targetDir.mkdirs()

    val target = JointCompilationSource(
      s"compiling '$f' in test '$testGroup'",
      randomized,
      flags,
      targetDir
    )
  ...
  /** Compiles all `files` together as a single compilation run. It is given a
    ...
    * dissociated
    */
  def compileList(testName: String, files: List[String], flags: TestFlags)(
      implicit testGroup: TestGroup
  ): CompilationTest =
    val outDir =
      defaultOutputDir + testGroup + JFile.separator + testName + JFile.separator
    ...
    val target = JointCompilationSource(
      s"$testName from $testGroup",
      files.map(new JFile(_)).toArray,
      flags,
      targetDir
    )
  ...
  /** This function compiles the files and folders contained within directory
    ...
    *     target all files are grouped according to the file suffix `_X` where
    *     `X` is a number. These groups are then ordered in ascending order
    *     based on the value of `X` and each group is compiled one after the
    *     other.
    *
    ...
    *   - Directories can have an associated check-file, where the check file
    *     has the same name as the directory (with the file extension `.check`)
    */
    ...
    val targets =
      files.map(f =>
        JointCompilationSource(
          ...
          Array(f),
          flags,
          createOutputDirsForFile(f, sourceDir, outDir)
        )
      ) ++
        dirs
          .map { dir =>
            SeparateCompilationSource(
              ...
              dir,
              flags,
              createOutputDirsForDir(dir, sourceDir, outDir)
            )
          }
  ...
  end compileFilesInDir

  /** This function compiles the files and folders contained within directory
    * `f` in a specific way. Once compiled, they are recompiled/run from tasty
    * as sources.
    *
    *   - Each file is compiled separately as a single compilation run
    *   - Each directory is compiled as a `SeparateCompilationTarget`, in this
    ...
    */
  def compileTastyInDir(
      f: String,
      flags0: TestFlags,
      fromTastyFilter: FileFilter
    ...
    class JointCompilationSourceFromTasty(
        ...
        outDir: JFile,
        fromTasty: Boolean = false
    ) extends JointCompilationSource(
          name,
          Array(file),
          flags,
          outDir,
      ...
      override def buildInstructions(errors: Int, warnings: Int): String =
        val runOrPos =
          if file.getPath.startsWith(
              s"tests${JFile.separator}run${JFile.separator}"
            )
          then "run"
          else "pos"
        val listName = if fromTasty then "from-tasty" else "decompilation"
        s"""|
            |Test '$title' compiled with $errors error(s) and $warnings warning(s),
            ...
            |
            |""".stripMargin
    end JointCompilationSourceFromTasty

    val targets = filteredFiles.map { f =>
      val classpath = createOutputDirsForFile(f, sourceDir, outDir)
      new JointCompilationSourceFromTasty(
        testGroup.name,
        f,
        flags.withClasspath(classpath.getPath),
        classpath,
        fromTasty = true
      )
    }
  ...
  end compileTastyInDir

  class TastyCompilationTest(
      step1: CompilationTest,
      step2: CompilationTest,
    ...
    def checkCompile()(implicit summaryReport: SummaryReporting): this.type =
      ...
      if shouldDelete then CompilationTest.aggregateTests(step1, step2).delete()
  ...
  /** This function behaves similar to `compileFilesInDir` but it ignores
    ...
    * tests.
    */
  def compileShallowFilesInDir(f: String, flags: TestFlags)(implicit
      testGroup: TestGroup
  ): CompilationTest =
    val outDir = defaultOutputDir + testGroup + JFile.separator
    val sourceDir = new JFile(f)
    ...
    val targets = files.map { file =>
      JointCompilationSource(
        ...
        Array(file),
        flags,
        createOutputDirsForFile(file, sourceDir, outDir)
      )
    }
  ...
  private def getCharsetFromEncodingOpt(flags: TestFlags) =
    flags.options
      .sliding(2)
      .collectFirst { case Array("-encoding", encoding) =>
