...
object FileDiff:
  ...
  def check(
    ...
    val checkLines =
      if !(new File(checkFile)).exists then Nil
      else
        Using(Source.fromFile(checkFile, StandardCharsets.UTF_8.name))(
          _.getLines().toList
        ).get

    if !matches(outputLines, checkLines) then
      Some(
  ...
  def matches(actual: String, expect: String): Boolean =
    ...
    actual1 == expect1 || File.separatorChar == '\\' && actual1.replace(
      '\\',
      '/'
    ) == expect1
