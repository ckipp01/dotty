...
final case class TestFlags(
  ...
  def withClasspath(classPath: String): TestFlags =
    TestFlags(
      s"$defaultClassPath${JFile.pathSeparator}$classPath",
      runClassPath,
      options,
      javacOptions
    )
  ...
  def withRunClasspath(classPath: String): TestFlags =
    TestFlags(
      defaultClassPath,
      s"$runClassPath${JFile.pathSeparator}$classPath",
      options,
      javacOptions
    )
  ...
  def withoutLanguageFeatures: TestFlags =
    copy(options = withoutLanguageFeaturesOptions)

  private val languageFeatureFlag = "-language:"
  private def withoutLanguageFeaturesOptions =
    options.filterNot(_.startsWith(languageFeatureFlag))
  ...
  def withoutLanguageFeature(feature: String) =
    val (languageFeatures, rest) =
      options.partition(_.startsWith(languageFeatureFlag))
    val existingFeatures =
      languageFeatures.flatMap(_.stripPrefix(languageFeatureFlag).split(","))
...
object TestFlags:
  def apply(classPath: String, flags: Array[String]): TestFlags =
    TestFlags(classPath, classPath, flags, Array.empty)
