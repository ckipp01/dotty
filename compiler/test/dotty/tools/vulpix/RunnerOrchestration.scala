...
import java.io.{File as JFile, InputStreamReader, BufferedReader, PrintStream}
...
trait RunnerOrchestration:
  ...
  def runMain(classPath: String, toolArgs: ToolArgs)(implicit
  ...
  /** The runner monitor object keeps track of child JVM processes by keeping
    ...
    * cleanup by returning the used JVM to the free list, or respawning it if it
    * died
    */
  private class RunnerMonitor:

    def runMain(classPath: String)(implicit
        summaryReport: SummaryReporting
    ): Status =
    ...
    private class Runner(private var process: Process):
      ...
      def runMain(classPath: String)(implicit
        ...
        if childStdin eq null then
          childStdin =
            new PrintStream(process.getOutputStream, /* autoFlush = */ true)
        ...
        val readOutput = Future {
          ...
          if childStdout eq null then
            childStdout = new BufferedReader(
          ...
          do childOutput = childStdout.readLine()
    ...
    private def createProcess: Process =
      val url =
        classOf[ChildJVMMain].getProtectionDomain.getCodeSource.getLocation
      val cp = Paths
        .get(url.toURI)
        .toString + JFile.pathSeparator + Properties.scalaLibrary
      ...
      new ProcessBuilder(
        ...
        "-cp",
        cp,
        "dotty.tools.vulpix.ChildJVMMain"
      )
    ...
    private def getRunner(): Runner = synchronized {
      while freeRunners.isEmpty && busyRunners.size >= numberOfSlaves do wait()

      val runner =
        if freeRunners.isEmpty then new Runner(createProcess)
