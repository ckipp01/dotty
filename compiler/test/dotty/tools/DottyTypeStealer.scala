...
/** Pass a string representing a Scala source file, and then some type
  * signatures referencing prior definitions.
  *
  * The type signatures will then be printed as raw data structures.
  *
  * @param source
  *   top level Scala definitions, e.g. `"class O { type X }"`
  * @param kind
  *   the kind of type we are inspecting [`rhs`, `method`, `class`, `type`]
  * @param typeStrings
  *   Scala type signatures, e.g. `"O#X"`
  *
  * @syntax
  *   markdown
  */
...
/** Pass a string representing a Scala source file, and then some type
  * signatures referencing prior definitions.
  *
  * The type signatures will then be printed comparing between phase `typer`
  * where types are as Scala understands them and phase `erasure`, which models
  * the JVM types.
  *
  * @param source
  *   top level Scala definitions, e.g. `"class O { type X }"`
  ...
  *   markdown
  */
@main def printTypesAndErasure(
    source: String,
    kind: DottyTypeStealer.Kind,
    typeStrings: String*
): Unit =
  val (ictx, vdefs) =
    DottyTypeStealer.stealMember("erasure", source, kind, typeStrings*)
...
object DottyTypeStealer extends DottyTest:
  ...
  def stealMember(
    ...
    val dummyName = "x_x_x"
    val vals = typeStrings.zipWithIndex
      .map { case (s, x) => kind.format(dummyName + x, s) }
      .mkString("\n")
    val gatheredSource = s" ${source}\n object A$dummyName {$vals}"
    var scontext: Context = null
    var members: List[Symbol] = null
    checkCompile(lastPhase, gatheredSource) { (tree, context) =>
