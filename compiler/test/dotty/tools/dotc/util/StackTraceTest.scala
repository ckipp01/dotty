...
class StackTraceTest:
  ...
  catch case e: Throwable => throw new RuntimeException(e).tap(e.initCause)
  def insaner: String = insane

  def repressed: String = try sample
  catch
    case e: Throwable =>
      throw new RuntimeException("My problem").tap(_.addSuppressed(e))
  ...
  @Test def showsOnlyPrefix() =
    probe(sample)(_.getMethodName == "sample")(s =>
      assertEquals(3, s.linesIterator.length)
    )
  ...
  @Test def showsWrappedExceptions() =
    probe(rewrapperer)(_.getMethodName != "rewrapperer") { s =>
      ...
      assertEquals(
        2,
        res.collect { case s if s.startsWith(CausedBy) => s }.size
      )
    }
