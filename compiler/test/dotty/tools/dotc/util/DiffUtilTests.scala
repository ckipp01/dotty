...
class DiffUtilTests:
  ...
  def testExpected(
    ...
    if fnd != foundColoring then
      fail(s"expected(found):\n$foundColoring but was: \n$fnd")
    if exp != expectedColoring then
  ...
  def seqSeq(): Unit =
    testExpected(
      "Seq[Seq[Seq[Foo]]]",
      "Seq[List[Seq[(Bar, Foo)]]]",
      "Seq[<R|Seq>[Seq[Foo]]]",
      "Seq[<G|List>[Seq[<G|(Bar, >Foo<G|)>]]]"
    )
    testExpected(
      "Seq[List[Seq[(Bar, Foo)]]]",
      "Seq[Seq[Seq[Foo]]]",
      "Seq[<R|List>[Seq[<R|(Bar, >Foo<R|)>]]]",
      "Seq[<G|Seq>[Seq[Foo]]]"
    )
end DiffUtilTests
