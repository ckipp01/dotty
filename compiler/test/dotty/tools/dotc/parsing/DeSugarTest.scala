...
class DeSugarTest extends ParserTest:
  ...
  object DeSugar extends UntypedTreeMap:
    ...
    def transform(tree: Tree, mode: Mode)(using Context): Tree =
      withMode(mode) { transform(tree) }
    def transform(trees: List[Tree], mode: Mode)(using Context): List[Tree] =
      withMode(mode) { transform(trees) }
    ...
    override def transform(tree: Tree)(using Context): Tree =
      ...
      tree1 match
        ...
        case impl @ Template(constr, _, self, _) =>
          cpy.Template(tree1)(
            transformSub(constr),
            transform(impl.parentsOrDerived),
            Nil,
            transformSub(self),
            transform(impl.body, Expr)
          )
        case Thicket(trees) =>
