...
trait DocstringTest extends DottyTest:
  ...
  def checkDocString(actual: Option[String], expected: String): Unit =
    actual match
      case Some(str) =>
        assert(
          str == expected,
          s"""Docstring: "$str" didn't match expected "$expected""""
        )
      case None =>
  ...
  def expectNoDocString(doc: Option[String]): Unit =
    doc.fold(()) { d =>
      assert(false, s"""Expected not to find a docstring, but found: "$d"""")
    }
  ...
  def defaultAssertion: PartialFunction[Any, Unit] = {
    case t: Tree[Untyped] =>
      assert(
        false,
        s"Couldn't match resulting AST to expected AST in: ${t.show}"
      )
    case x =>
      assert(false, s"Couldn't match resulting AST to expected AST in: $x")
  }

  def checkFrontend(source: String)(
      docAssert: PartialFunction[Tree[Untyped], Unit]
  ): Unit =
