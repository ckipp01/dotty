...
class ParserEdgeTest extends ParserTest:
  ...
  @Test def `t9944 respect line separator`: Unit =
    ...
    val triple = "\"" * 3
    val text =
      s"""class C { def g = "X" ; def f = s${triple}123${CR}${NL}$${g}${CR}${NL}456${triple} }"""
    def isStripped(s: String) = s.contains(NL) && !s.contains(CR)

    // sanity check
    assert(
      text.linesIterator.size == 3,
      s"line count ${text.linesIterator.size}"
    )
    ...
    assert(!t.existsSubTree {
      case Literal(const @ Constant(_)) if const.tag == StringTag =>
        isStripped(const.stringValue)
      case st => false
    })
  /* was:
  package <empty> {
