...
class ConstraintsTest:
  ...
  @Test def mergeParamsTransitivity: Unit =
    inCompilerContext(
      ...
      val tvars = constrained(
        requiredClass("A").typeRef
          .select("foo".toTermName)
          .info
          .asInstanceOf[TypeLambda],
        EmptyTree,
  ...
  @Test def validBoundsInit: Unit = inCompilerContext(
    ...
    val TypeBounds(lo, hi) = ctx.typerState.constraint.entry(
      t.asInstanceOf[TypeVar].origin
    ): @unchecked
    assert(
      lo =:= defn.NothingType,
      i"Unexpected lower bound $lo for $t: ${ctx.typerState.constraint}"
    )
    assert(
      hi =:= defn.StringType,
      i"Unexpected upper bound $hi for $t: ${ctx.typerState.constraint}"
    ) // used to be Any
  }
  ...
  @Test def validBoundsUnify: Unit = inCompilerContext(
    TestConfiguration.basicClasspath,
    scalaSources =
      "trait A { def foo[S >: T <: T | Int, T <: String | Int]: Any  }"
  ) {
    val tvars = constrained(
      requiredClass("A").typeRef
        .select("foo".toTermName)
    ...
    assert(
      hi =:= (defn.StringType | defn.IntType),
      i"Unexpected upper bound $hi for $t: ${ctx.typerState.constraint}"
    )
  }
  ...
  @Test def validBoundsReplace: Unit = inCompilerContext(
    ...
    for tvar <- tvars do
      ...
      assert(
        !ctx.typerState.constraint.occursAtToplevel(tvar.origin, entry),
