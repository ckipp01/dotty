...
class ScalaSettingsTests:
  ...
  @Test def `A setting with aliases is accepted`: Unit =
    class MySettings extends SettingGroup:
      val classpath: Setting[String] = PathSetting(
        ...
        ".",
        aliases = List("--class-path", "-cp")
      )
    val settings = MySettings()
    val args = tokenize("-cp path/to/classes1:other/path/to/classes2")
    val summary =
      ArgsSummary(settings.defaultState, args, errors = Nil, warnings = Nil)
    val res =
    ...
    assertTrue(
      "wrong classpath",
      classpath == "path/to/classes1:other/path/to/classes2"
    )
  ...
  @Test def `Warn if multistring element is supplied multiply`: Unit =
    ...
    val sut = SUT()
    val args = tokenize(
      "-language:dynamics -language:implicitConversions -language:dynamics"
    )
    val sumy = ArgsSummary(sut.defaultState, args, errors = Nil, warnings = Nil)
    val res = sut.processArguments(sumy, processAll = true, skipped = Nil)
    val set = sut.language.valueIn(res.sstate)
  ...
  @Test def `WConf setting is parsed`: Unit =
    ...
    val args = List(
      "-Wconf:cat=deprecation:s,cat=feature:e",
      "-Wconf:msg=a problem\\.:s"
    )
    val sumy =
      ArgsSummary(sets.defaultState, args, errors = Nil, warnings = Nil)
    ...
    val nowr = new Diagnostic.Warning(
      "This is a problem.".toMessage,
