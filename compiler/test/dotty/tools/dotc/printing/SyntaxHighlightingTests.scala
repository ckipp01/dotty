...
class SyntaxHighlightingTests extends DottyTest:
  ...
  private def test(source: String, expected: String): Unit =
    val highlighted = SyntaxHighlighting
      .highlight(source)(using ctx.withColors)
      .replace(NoColor, ">")
  ...
  def annotations =
    ...
    test(
      "@Test(\"Hello\")(\"World\") class Foo",
      "<T|@Test(\"Hello\")(\"World\")> <K|class> <T|Foo>"
    )
    test(
      "@annotation.tailrec def foo = 1",
      "<T|@annotation.tailrec> <K|def> <V|foo> = <L|1>"
    )
  ...
  def valOrDefDef =
    ...
    test(
      "def foo(bar: Int): Int =",
      "<K|def> <V|foo>(<V|bar>: <T|Int>): <T|Int> ="
    )
    test(
      "def foo(bar: Int): Int = 123",
      "<K|def> <V|foo>(<V|bar>: <T|Int>): <T|Int> = <L|123>"
    )

    test("def f1(x: Int) = 123", "<K|def> <V|f1>(<V|x>: <T|Int>) = <L|123>")
    test(
      "def f2[T](x: T) = { 123 }",
      "<K|def> <V|f2>[<T|T>](<V|x>: <T|T>) = { <L|123> }"
    )
  ...
  def softKeywords =
    ...
    test(
      "def foo(inline: Int) = 2",
      "<K|def> <V|foo>(<V|inline>: <T|Int>) = <L|2>"
    )
    test(
      """enum Foo:
        |  case foo
