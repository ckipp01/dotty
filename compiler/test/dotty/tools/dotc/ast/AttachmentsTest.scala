  ...
  def attachmentsAreNotCopiedOver: Unit =
    checkCompile("typer", "class A") {
      case (PackageDef(_, (clazz: tpd.TypeDef) :: Nil), context) =>
        assertTrue(
          "Attachment shouldn't be present",
          clazz.getAttachment(TestKey).isEmpty
        )
        ...
        assertTrue(
          "Attachment shouldn't be present",
          copy.getAttachment(TestKey).isEmpty
        )
  ...
  def stickyAttachmentsAreCopiedOver: Unit =
    checkCompile("typer", "class A") {
      case (PackageDef(_, (clazz: tpd.TypeDef) :: Nil), context) =>
        assertTrue(
          "Attachment shouldn't be present",
          clazz.getAttachment(StickyTestKey).isEmpty
        )
        assertTrue(
          "Attachment shouldn't be present",
          clazz.getAttachment(StickyTestKey2).isEmpty
        )
        assertTrue(
          "Attachment shouldn't be present",
          clazz.getAttachment(TestKey).isEmpty
        ...
        assertTrue(
          ...
          copy.hasAttachment(StickyTestKey)
        )
        assertTrue(
          "Attachment shouldn't be present",
          !copy.hasAttachment(TestKey)
        )
        assertTrue(
          "Attachment should be present",
          copy.hasAttachment(StickyTestKey2)
        )

      case _ =>
        fail
    }
end AttachmentsTests
