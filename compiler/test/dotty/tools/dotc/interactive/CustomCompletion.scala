...
object CustomCompletion:
  ...
  def completions(
    ...
    computeCompletions(pos, path, dependencyCompleteOpt, enableDeep)(using
      Interactive.contextOfPath(path)
    )
    ...
    val completions = path match
      ...
      case _ =>
        completer.scopeCompletions ++ {
          if enableDeep then completer.deepCompletions
          else Nil
        }
  ...
  private def describeCompletions(
  ...
  class DeepCompleter(
    ...
    def deepCompletions(using Context): Map[Name, Seq[SingleDenotation]] =
      ...
      def allMembers(s: Symbol) =
        ...
        catch case _: dotty.tools.dotc.core.TypeError => Nil
      def rec(t: Symbol): Seq[Symbol] =
        val children =
          if t.is(Package) || t.is(PackageVal) || t.is(PackageClass) then
