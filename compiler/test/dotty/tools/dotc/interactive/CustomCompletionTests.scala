...
class CustomCompletionTests extends DottyTest:
  ...
  private def completions(
    ...
    val file =
      SourceFile.virtual("<completions>", allCode, maybeIncomplete = true)
    ...
    unit.tpdTree =
      ...
      unit.tpdTree match
        case PackageDef(_, p) =>
          p.reverseIterator
            .collectFirst { case TypeDef(_, tmpl: Template) =>
              tmpl.body
                .collectFirst {
                  case dd: ValDef if dd.name.show == "expr" => dd
                }
    ...
    val (offset0, completions) =
      if deep || dependencyCompleter.nonEmpty then
        CustomCompletion.completions(
          srcPos,
          dependencyCompleteOpt = dependencyCompleter,
          enableDeep = deep
        )(using ctx1)
      else Completion.completions(srcPos)(using ctx1)
