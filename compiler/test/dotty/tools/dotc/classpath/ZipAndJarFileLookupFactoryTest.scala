...
class ZipAndJarFileLookupFactoryTest:
  @Test def cacheInvalidation(): Unit =
    if scala.util.Properties.isWin then
      return // can't overwrite an open file on windows.
    ...
    assert(
      !ctx.settings.YdisableFlatCpCaching.value
    ) // we're testing with our JAR metadata caching enabled.
  ...
  def createZip(
    ...
    try
      ...
      zipfs.close()
end ZipAndJarFileLookupFactoryTest
