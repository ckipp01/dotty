...
class MultiReleaseJarTest extends dotty.tools.backend.jvm.DottyBytecodeTest:
  ...
  @Test
  def mrJar(): Unit =
    if !Properties.isJavaAtLeast("9") then
      println("skipping mrJar() on old JDK"); return
    ...
    val java9FooDef =
      "package p1; abstract class Foo { def foo1: Int; def foo2: Int }"
    val java10BarDef =
      "package p2; abstract class Bar { def bar1: Int; def bar2: Int }"
  ...
  private def createZip(
      zipLocation: Path,
      content: List[(String, Array[Byte])]
  ): Unit =
    ...
    try
      try
        for (internalPath, contentBytes) <- content do
