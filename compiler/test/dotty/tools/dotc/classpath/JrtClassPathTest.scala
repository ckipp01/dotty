...
class JrtClassPathTest:
  ...
  @Test def lookupJavaClasses(): Unit =
    ...
    val cp: ClassPath =
      if specVersion == "" || specVersion == "1.8" then
        val resolver = new PathResolver
        val elements = (new ClassPathFactory).classesInPath(
          resolver.Calculated.javaBootClassPath
        )
    ...
    assertTrue(
      cp.packages("java").toString,
      cp.packages("java").exists(_.name == "java.lang")
    )
    assertTrue(cp.classes("java.lang").exists(_.name == "Object"))
    val jl_Object = cp.classes("java.lang").find(_.name == "Object").get
    assertEquals(
      "java/lang/Object",
      AsmUtils.readClass(jl_Object.file.toByteArray).name
    )
    assertTrue(
      cp.list("java.lang").packages.exists(_.name == "java.lang.annotation")
    )
    assertTrue(
      cp.list("java.lang").classesAndSources.exists(_.name == "Object")
    )
    assertTrue(cp.findClass("java.lang.Object").isDefined)
    assertTrue(cp.findClassFile("java.lang.Object").isDefined)
  end lookupJavaClasses
end JrtClassPathTest
