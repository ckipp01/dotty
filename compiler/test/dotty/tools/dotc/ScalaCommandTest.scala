...
class ScalaCommandTest:
  ...
  @Test def `Simple one parameter`: Unit = inContext {
    ...
    assertEquals(
      "path/to/classes1:other/path/to/classes2",
  ...
  @Test def `Unfold @file`: Unit = inContext {
    ...
    writer.close();
    val args =
      s"-cp path/to/classes1:other/path/to/classes2 @${file} someFiles".split(
        " "
      )
    val summary = ScalacCommand.distill(args, settings)()
  ...
  private def inContext(f: Context ?=> Unit) = f(using
    (new ContextBase).initialCtx.fresh
  )

  extension [T](setting: Setting[T])
    private def value(using ss: SettingsState): T = setting.valueIn(ss)
end ScalaCommandTest
