...
class TestReporter protected (
    outWriter: PrintWriter,
    filePrintln: String => Unit,
    logLevel: Int
) extends Reporter
  ...
  protected final val _consoleReporter =
    new ConsoleReporter(null, new PrintWriter(_consoleBuf))
  ...
  def printMessageAndPos(dia: Diagnostic, extra: String)(using Context): Unit =
    ...
    if dia.level >= logLevel then
      outWriter.println(msg)
      if extraInfo.nonEmpty then outWriter.println(extraInfo)

    _messageBuf.append(msg)
    if extraInfo.nonEmpty then _messageBuf.append(extraInfo)
...
object TestReporter:
  ...
  private val failedTestsFile: JFile = new JFile(
    s"$testLogsDirName/$failedTestsFileName"
  )
  ...
  private def initLog() = if logWriter eq null then
    val date = new Date
    ...
    failedTestsWriter = new PrintWriter(
      new FileOutputStream(failedTestsFile, false)
    )
  ...
  def logFlush(): Unit =
    if logWriter ne null then logWriter.flush()
  ...
  def simplifiedReporter(writer: PrintWriter): TestReporter =
    val rep = new TestReporter(writer, logPrintln, WARNING):
      /** Prints the message with the given position indication in a simplified
        * manner
        */
      override def printMessageAndPos(dia: Diagnostic, extra: String)(using
          Context
      ): Unit =
        def report() =
