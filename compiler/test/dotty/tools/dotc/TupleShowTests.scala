...
class TupleShowTests extends DottyTest:
  ...
  val tup23 =
    defn.tupleType(Types_20 ::: IntType :: LongType :: ShortType :: Nil)
  val tup24 = defn.tupleType(
    Types_20 ::: IntType :: LongType :: ShortType :: ShortType :: Nil
  )
  ...
  @Test def tup3_show10 = chkEq(
    "(Int,\n  Long,\n  Short)".normEOL,
    tup3.toText(ctx.printer).mkString(10, false)
  )
  ...
  def chkEq[A](expected: A, obtained: A) =
    assert(expected == obtained, diff(s"$expected", s"$obtained"))
  ...
  /** On Windows the string literal in this test source file will be read with
    ...
    * as the line separator. So we align the expected result to faithfully
  ...
  def diff(exp: String, obt: String) =
    ...
    val suf =
      ...
      var i = 0
      while i <= max && exp(exp.length - 1 - i) == obt(obt.length - 1 - i) do
        i += 1
    ...
    val compactPre =
      if pre.length <= 20 then pre else ellip + pre.drop(pre.length - 20)
