...
/** Test that demonstrates how to use dotty-interfaces
  ...
  *
  * @see
  *   [[OtherEntryPointsTest]]
  */
@Category(Array(classOf[BootstrappedOnlyTests]))
class InterfaceEntryPointTest:
  @Test def runCompilerFromInterface =
    val sources =
      List("tests/pos/HelloWorld.scala").map(p =>
        Paths.get(p).toAbsolutePath().toString
      )
    ...
    val process = mainClass.getMethod(
      "process",
  ...
  private class CustomSimpleReporter extends SimpleReporter:
    ...
    def report(diag: Diagnostic): Unit =
      if diag.level == Diagnostic.ERROR then errorCount += 1
      if diag.level == Diagnostic.WARNING then warningCount += 1
  ...
  private class CustomCompilerCallback extends CompilerCallback:
    ...
    override def onSourceCompiled(source: SourceFile): Unit =
      if source.jfile.isPresent then pathsBuffer += source.jfile.get.getPath
end InterfaceEntryPointTest
