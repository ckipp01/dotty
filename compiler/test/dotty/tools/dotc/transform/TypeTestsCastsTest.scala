...
class TypeTestsCastsTest extends DottyTest:
  ...
  @Test def orL =
    checkFound(List(StringType, LongType), OrType(LongType, StringType, false))
  @Test def orR =
    checkFound(List(LongType, StringType), OrType(StringType, LongType, false))

  @Test def annot = checkFound(
    List(StringType, LongType),
    AnnotatedType(
      OrType(LongType, StringType, false),
      Annotation(defn.UncheckedAnnot, Span(0))
    )
  )
  ...
  @Test def orInAndL = checkFound(
    List(StringType, LongType),
    AndType(OrType(LongType, StringType, false), AnyType)
  )
  @Test def orInAndR = checkFound(
    List(StringType, LongType),
    AndType(AnyType, OrType(LongType, StringType, false))
  )
  @Test def orInAndZ =
    ...
    val tp = AndType(
      OrType(ThrowableType, ExceptionType, false),
      OrType(RuntimeExceptionType, AnyType, false)
    )
    val exp = List(
      ExceptionType,
