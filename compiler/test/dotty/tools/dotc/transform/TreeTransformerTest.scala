...
class TreeTransformerTest extends DottyTest:
  ...
  @Test
  def shouldReturnSameTreeIfUnchanged: Unit =
    checkCompile("typer", "class A{ val d = 1}") { (tree, context) =>
      ...
      Assert.assertTrue("returns same tree if unmodified", tree eq transformed)
    }
  ...
  def canReplaceConstant: Unit = checkCompile("typer", "class A{ val d = 1}") {
    (tree, context) =>
      ...
      class ConstantTransform extends MiniPhase:
        ...
        override def transformLiteral(tree: tpd.Literal)(using
            Context
        ): tpd.Tree = tpd.Literal(Constant(2))
      ...
      Assert.assertTrue(
        "returns same tree if unmodified",
        transformed.toString.contains(
          "List(ValDef(Modifiers(,,List()),d,TypeTree[TypeRef(ThisType(module class scala),Int)],Literal(Constant(2)))"
        )
      )
  }
  ...
  def canOverwrite: Unit = checkCompile("typer", "class A{ val d = 1}") {
    (tree, context) =>
      ...
      class Transformation extends MiniPhase:
        ...
        override def transformLiteral(tree: tpd.Literal)(using
            Context
        ): tpd.Tree = tpd.Literal(Constant(-1))
        override def phaseName: String = "canOverwrite"

        override def transformValDef(tree: tpd.ValDef)(using
            Context
        ): tpd.ValDef =
          Assert.assertTrue(
            "transformation of children succeeded",
      ...
      Assert.assertTrue(
        "node can rewrite children",
        tr.contains("Literal(Constant(2))") && !tr.contains(
          "Literal(Constant(-1))"
        )
      )
  }
  ...
  def transformationOrder: Unit = checkCompile("typer", "class A{ val d = 1}") {
    (tree, context) =>
      ...
      class Transformation2 extends MiniPhase:
        ...
        init(ctx.base, ctx.period.firstPhaseId, ctx.period.lastPhaseId)
      val transformer =
        new MegaPhase(Array(new Transformation1, new Transformation2))
      val tr = transformer.transformUnit(tree).toString
    ...
    (tree, context) =>
      ...
      class Transformation1 extends MiniPhase:
        ...
        override def transformValDef(tree: tpd.ValDef)(using Context) =
          ...
          tpd.cpy.ValDef(tree)(rhs =
            transformFollowing(tpd.Literal(Constant(2)))
          )
      ...
      val tr = transformer.transformUnit(tree).toString
      Assert.assertTrue(
        "transformations aren't invoked multiple times",
        transformed1 == 2 && transformed2 == 3
      )
  }
end TreeTransformerTest
