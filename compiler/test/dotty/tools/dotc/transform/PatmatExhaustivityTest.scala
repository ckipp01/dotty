...
class PatmatExhaustivityTest:
  ...
  private def compile(files: List[JPath]): Seq[String] =
    ...
    try
      Main.process(
        (options ::: opts ::: files.map(_.toString)).toArray,
        reporter,
        null
      )
    catch
  ...
  def patmatExhaustivity: Unit =
    val res = Directory(testsDir).list.toList
      ...
      .map(f =>
        if f.isDirectory then compileDir(f.jpath) else compileFile(f.jpath)
      )

    val failed = res.filter(!_)
    val ignored =
      Directory(testsDir).list.toList.filter(_.extension == "ignore")

    val msg =
      s"Total: ${res.length + ignored.length}, Failed: ${failed.length}, Ignored: ${ignored.length}"

    assert(failed.length == 0, msg)

    println(msg)
end PatmatExhaustivityTest
