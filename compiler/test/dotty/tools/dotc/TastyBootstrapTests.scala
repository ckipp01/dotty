...
class TastyBootstrapTests:
  ...
  /** The purpose of this test is three-fold, being able to compile dotty
    ...
    * version of Dotty, and compiling the compiler using the SemanticDB
    * generation
    */
  @Test def tastyBootstrap: Unit =
    ...
    dotty.tools.io
      .Directory(defaultOutputDir + "tastyBootstrap")
      .deleteRecursively()
    ...
    val libraryDirs =
      List(Paths.get("library/src"), Paths.get("library/src-bootstrapped"))
    ...
    val tastyCore =
      compileList("tastyCore", tastyCoreSources, opt)(tastyCoreGroup)

    val compilerSources = sources(Paths.get("compiler/src")) ++ sources(
      Paths.get("compiler/src-bootstrapped")
    )
    ...
    val dotty1 =
      compileList("dotty1", compilerSources ++ compilerManagedSources, opt)(
        dotty1Group
      )
    val dotty2 =
      compileList("dotty2", compilerSources ++ compilerManagedSources, opt)(
        dotty2Group
      )
    ...
    compileList(
      "idempotency",
      List(
        "tests/idempotency/BootstrapChecker.scala",
