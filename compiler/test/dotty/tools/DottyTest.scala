...
trait DottyTest extends ContextEscapeDetection:
  ...
  private def compilerWithChecker(phase: String)(
      assertion: (tpd.Tree, Context) => Unit
  ) = new Compiler:
    override def phases =
  ...
  def checkTypes(source: String, typeStrings: String*)(
      assertion: (List[Type], Context) => Unit
  ): Unit =
  ...
  def checkTypes(source: String, typeStringss: List[List[String]])(
      assertion: (List[List[Type]], Context) => Unit
  ): Unit =
    val dummyName = "x_x_x"
    val vals = typeStringss.flatten.zipWithIndex
      ...
      val findValDef: (List[tpd.ValDef], tpd.Tree) => List[tpd.ValDef] =
        (acc, tree) =>
          tree match
            ...
            case _                                             => acc
      val d =
        new tpd.DeepFolder[List[tpd.ValDef]](findValDef).foldOver(Nil, tree)
  ...
  def methType(names: String*)(paramTypes: Type*)(
