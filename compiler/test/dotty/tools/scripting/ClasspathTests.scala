...
class ClasspathTests:
  ...
  @Test def hashbangClasspathVerifyTest =
    ...
    val testScript = scripts("/scripting").find {
      _.getName.matches(testScriptName)
    } match
    ...
    if packBinScalaExists then
      ...
      val scriptCwd: String =
        findTaggedLine("cwd", scriptOutput) // the value tagged "cwd: "
      printf("script ran in directory [%s]\n", scriptCwd)
      val scriptCp = findTaggedLine(
        "classpath",
        scriptOutput
      ) // the value tagged "classpath: "

      // convert scriptCp to a list of files
      val hashbangJars: List[File] =
        scriptCp.split(psep).map { _.toFile }.toList
      ...
      val packlibDir =
        s"$scriptCwd/$packLibDir" // classpathReport.sc specifies a wildcard classpath in this directory
      val packlibJars: List[File] = listJars(
        packlibDir
      ) // classpath entries expected to have been reported by the script
  ...
  end hashbangClasspathVerifyTest
  /*
