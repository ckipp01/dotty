...
class ScriptingTests:
  ...
  def testFiles = scripts("/scripting").filter {
    !_.getName.toLowerCase.contains("classpath")
  }
  ...
  @Test def scriptingDriverTests =
    for (scriptFile, scriptArgs) <- scalaFilesWithArgs(".scala") do
      ...
      assert(
        !unexpectedJar.exists,
        s"not expecting jar file: ${unexpectedJar.absPath}"
      )
  ...
  @Test def scriptingMainTests =
    for (scriptFile, scriptArgs) <- scalaFilesWithArgs(".sc") do
      ...
      val mainArgs: Array[String] = Array(
        "-classpath",
        TestConfiguration.basicClasspath.toString,
        "-script",
        scriptFile.toString
      ) ++ scriptArgs
  ...
  @Test def scriptingNoCompileJar: Unit =
    ...
    assert(
      expectedJar.exists,
      s"unable to create executable jar [$expectedJar]"
    )
...
///////////////////////////////////
  ...
  def argss: Map[String, Array[String]] = (for
    argFile <- testFiles
  ...
  def scalaFilesWithArgs(extension: String) = (for
