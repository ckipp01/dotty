...
class BashExitCodeTests:
  ...
  private lazy val tmpDir =
    myTmpDir = Files.createTempDirectory("exit-code-tests").toFile.absPath;
    myTmpDir
  ...
  def verifyExit(cmd: String, args: String*)(expectedExitCode: Int): Unit =
    ...
    if verifyValid(validTest) then
      assertEquals(
        {
          def pp(n: String, ss: Seq[String]) =
            if ss.isEmpty then "" else s"\nstd$n:${ss.map("\n  " + _).mkString}"
  ...
  def f(body: String, suffix: String = ".scala"): String =
    Files
      .write(
        Files.createTempFile(tmpDir.toPath, getClass.getSimpleName, suffix),
        body.getBytes(UTF_8)
      )
      .absPath
  ...
  @Test def run =
    scalac(f("@main def Test = ???"))(0) & scala("-classpath", tmpDir, "Test")(
      1
    )
  @Test def pos =
    scalac(f("@main def Test = ()"))(0) & scala("-classpath", tmpDir, "Test")(0)
  ...
  @Test def decompilePos =
    scalac(f("class Test"))(0) & scalacRaw("-decompile", s"$tmpDir/Test.tasty")(
      0
    )

  @Test def printTastyNeg = scalac("-print-tasty", "non-existing-file.tasty")(1)
  @Test def printTastyPos = scalac(f("class Test"))(0) & scalacRaw(
    "-print-tasty",
    s"$tmpDir/Test.tasty"
  )(0)
  ...
  extension (inline u1: Unit)
    inline def &(inline u2: Unit): Unit =
      u1; u2
