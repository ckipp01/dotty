...
/** Common Code for supporting scripting tests. To override the path to the bash
  * executable, set TEST_BASH=<path-to-bash.exe> To specify where
  * `dist/target/pack/bin` resides, set TEST_CWD=<working-directory> Test
  * scripts run in a bash env, so paths are converted to forward slash via
  * .norm.
  */
object ScriptTestEnv:
  ...
  def testCwd =
    envOrElse("TEST_CWD", "").norm // optional working directory TEST_CWD
  ...
  lazy val workingDirectory: String =
    val dirstr = if testCwd.nonEmpty then
      ...
      testCwd
    else userDir // userDir, if TEST_CWD not set
  ...
  def supplementedPath: String =
    s"dist/target/pack/bin$psep$envJavaHome/bin$psep$envScalaHome/bin$psep$envPath".norm
  def adjustedPathEntries: List[String] =
    supplementedPath.norm.split(psep).toList.distinct
  ...
  def ostype = ostypeFull.toLowerCase.takeWhile { cc =>
    cc >= 'a' && cc <= 'z' || cc >= 'A' && cc <= 'Z'
  }
  ...
  def whichExe(basename: String): String =
    val exeName =
      if osname.toLowerCase.startsWith("windows") then s"$basename.exe"
      else basename
    which(exeName)
  ...
  def bashCommand(
      cmdstr: String,
      additionalEnvPairs: List[(String, String)] = Nil
  ): (Boolean, Int, Seq[String], Seq[String]) =
  ...
  def callExecutableJar(
      script: File,
      jar: File,
      scriptArgs: Array[String] = Array.empty[String]
  ) =
    import scala.sys.process.*
    val cmd =
      Array("java", s"-Dscript.path=${script.getName}", "-jar", jar.absPath)
        ++ scriptArgs
  ...
  extension (s: String)
    ...
    def parent(up: Int): String =
      s.norm.split("/").reverse.drop(up).reverse.mkString("/")
  ...
  extension (p: Path)
    ...
    def absPath: String =
      if p.isAbsolute then p.norm else Paths.get(userDir, p.norm).norm
    ...
    def isAbsolute =
      p.norm.startsWith("/") || (isWin && p.norm.secondChar == ":")
  ...
  lazy val (scalacPath: String, scalaPath: String) =
    val scalac =
      s"$workingDirectory/dist/target/pack/bin/scalac".toPath.normalize
  ...
  lazy val shellopts: String = {
    val value: String =
      envOrElse("SHELLOPTS", "braceexpand:hashall:igncr:ignoreeof:monitor:vi")
    val list: List[String] = value.split(":").toList
    val minlist = list.filter {
      case "igncr" | "xtrace" => false
      case _                  => true
    }
    if isWin then "igncr" :: minlist
    else minlist
  }.mkString(":")
