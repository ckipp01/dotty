...
object testnames:
  ...
  val n = termName(
    "hello"
  ) // > n  : dotty.tools.dotc.core.Names.TermName = hello
  val tn = n.toTypeName // > tn  : dotty.tools.dotc.core.Names.TypeName = hello
  val ln =
    n.toLocalName // > ln  : dotty.tools.dotc.core.Names.LocalName = hello
  ...
  val foo = encodedTermName(
    "++"
  ) // > foo  : dotty.tools.dotc.core.Names.TermName = $plus$plus
  foo.hashCode // > res5: Int = 5
  foo.toTypeName.hashCode // > res6: Int = -5

  val nfoo =
    n ++ foo // > nfoo  : dotty.tools.dotc.core.testnames.n.ThisName = hello$plus$plus
  nfoo contains '$' // > res7: Boolean = true
  nfoo.replace(
    '$',
    '.'
  ) // > res8: dotty.tools.dotc.core.testnames.nfoo.ThisName = hello.plus.plus
  ...
  nfoo.filter(
    _ >= 'l'
  ) // > res19: dotty.tools.dotc.core.Names.Name = lloplusplus
  nfoo map (_.toUpper) // > res20: dotty.tools.dotc.core.Names.Name = HELLO$PLUS$PLUS

  import Decorators.*

  val local =
    "local".toTermName.toLocalName // > local  : dotty.tools.dotc.core.Names.LocalName = local
  val local1 =
    local ++ "!" // > local1  : dotty.tools.dotc.core.testnames.local.ThisName = local!
  local1 // > res21: dotty.tools.dotc.core.testnames.local.ThisName = local!
  val local2 =
    "Foo.".toTermName ++: local1 // > local2  : dotty.tools.dotc.core.Names.Name = Foo.local!
  local2 // > res22: dotty.tools.dotc.core.Names.Name = Foo.local!
  local1.dropRight(2) // > res23: dotty.tools.dotc.core.Names.Name = loca
  local1.fromName(
    "Foo.".toTermName ++ local1
  ) // > res24: dotty.tools.dotc.core.testnames.local1.ThisName = Foo.local!
end testnames
