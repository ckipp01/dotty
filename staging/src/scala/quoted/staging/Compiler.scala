...
object Compiler:
  ...
  /** Create a new instance of the compiler using the the classloader of the
    * application.
    *
    * Usage:
    * ```
  ...
  def make(appClassloader: ClassLoader)(implicit settings: Settings): Compiler =
    new Compiler:
      ...
      def run[T](exprBuilder: Quotes => Expr[T]): T = synchronized {
        try
          if running then // detected nested run
            throw new ScopeException(
              "Cannot call `scala.quoted.staging.run(...)` within a another `run(...)`"
            )
          running = true
  ...
  object Settings:
    ...
    /** Make compiler settings
      * @param outDir
      *   Output directory for the compiled quote. If set to None the output
      *   will be in memory
      * @param compilerArgs
      *   Compiler arguments. Use only if you know what you are doing.
      */
