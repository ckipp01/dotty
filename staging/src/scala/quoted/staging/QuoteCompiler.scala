...
private class QuoteCompiler extends Compiler:
  ...
  /** Either `Left` with name of the classfile generated or `Right` with the
    * value contained in the expression
    */
  ...
  class QuotedFrontend extends Phase:
    ...
    override def runOn(units: List[CompilationUnit])(implicit
        ctx: Context
    ): List[CompilationUnit] =
      units.flatMap { case exprUnit: ExprCompilationUnit =>
        val ctx1 = ctx.fresh.setPhase(this.start).setCompilationUnit(exprUnit)
        implicit val unitCtx: Context =
          SpliceScope.setSpliceScope(new RunScope)(using ctx1)
        ...
        val quoted =
          ...
          val qctx = QuotesImpl()
          val quoted =
            PickledQuotes.quotedExprToTree(exprUnit.exprBuilder.apply(qctx))
        ...
        getLiteral(quoted) match
          ...
          case None =>
            ...
            val classTree =
              ClassDef(cls, DefDef(cls.primaryConstructor.asTerm), run :: Nil)
            val tree = PackageDef(
              ref(defn.RootPackage).asInstanceOf[Ident],
              classTree :: Nil
            ).withSpan(pos)
  ...
  class ExprRun(comp: QuoteCompiler, ictx: Context) extends Run(comp, ictx):
    /** Unpickle and optionally compile the expression. Returns either `Left`
      * with name of the classfile generated or `Right` with the value contained
      * in the expression.
      */
