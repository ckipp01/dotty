...
import java.io.{File as JFile, FileNotFoundException}

class TestScripts:
  private val lineSep = util.Properties.lineSeparator
  private def doUnlessWindows(op: => Unit) =
    if !System.getProperty("os.name").toLowerCase.contains("windows") then op
    else
      Console.err.println(
        "[warn] Could not perform test, windows batch-scripts not available"
      )

  private def executeScript(script: String): (Int, String) =
    val sb = new StringBuilder
    val ret = Process(script) ! ProcessLogger { line =>
      println(line); sb.append(line + "\n")
    }
  ...
  @Test def buildAndRunHelloWorld = doUnlessWindows {
    val (retDotc, dotcOutput) =
      executeScript("./bin/scalac ./tests/pos/HelloWorld.scala")
  ...
  @Test def reCreatesPackagesIfNecessary = doUnlessWindows {
    ...
    val contents =
      """|/Users/fixel/Projects/dotty/interfaces/target/dotty-interfaces-0.1.1-bin-SNAPSHOT-X.jar
         ...
         |/Users/fixel/Projects/dotty/doc-tool/target/scala-2.11/dotty-doc_2.1X-0.1.1-bin-SNAPSHOT-tests.jar""".stripMargin
