...
class CodeSnippets:
  ...
  private def getButtonsSection(snippet: html.Element): Option[html.Div] =
    snippet.querySelector("div.buttons") match
      case div: html.Div => Some(div)
      case _             => None

  def enrichSnippets() =
    document.querySelectorAll("div.snippet[scala-snippet]").foreach {
      case snippet: html.Element =>
        snippet.addEventListener(
          "click",
          (e: MouseEvent) => e.asInstanceOf[js.Dynamic].fromSnippet = true
        )
  ...
  private def handleHideableCode(snippet: html.Element): Unit =
    def toggleHide(e: html.Element | html.Document) =
      e.querySelectorAll(".hideable").foreach {
    ...
    def createShowHideButton(toggleRoot: html.Element) =
      div(cls := "snippet-showhide-container")(
        label(cls := "snippet-showhide-button")(
          input("type" := "checkbox", cls := "snippet-showhide").tap(
            _.addEventListener("change", _ => toggleHide(toggleRoot))
          )
        )
      )
  ...
  private def snippetAnchor(snippet: html.Element): Unit =
    snippet.querySelector(".snippet-meta .snippet-label") match
      case e: html.Element =>
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M5 6C5 5.44772 5.44772 5 6 5H14C14.5523 5 15 5.44772 15 6V14C15 14.5523 14.5523 15 14 15H6C5.44772 15 5 14.5523 5 14V6ZM6.5 13.5V6.5H13.5V13.5H6.5Z" fill="#A09FA6"/>
 ...
 """
    def copyButton =
      div(
        button(cls := "copy-button icon-button").tap(
          _.addEventListener(
            "click",
            _ =>
              val code = snippet
                .querySelectorAll("code>span:not(.hidden)")
                .map(_.textContent)
                .mkString
        ...
        e =>
          val popup = div(cls := "snippet-popup")(
            div(cls := "snippet-popup-content body-small")(
              ...
              _.addEventListener(
                "click",
                e => e.asInstanceOf[js.Dynamic].fromPopup = true
              )
            )
          )
          ...
          popup
            .querySelector("li.btn.run-button")
    ...
    buttonsSection.foreach(s =>
      ...
      if snippet.hasAttribute("runnable") then s.appendChild(runButton)
    )
  end copyRunButtons

  window.addEventListener("dynamicPageLoad", (e: Event) => enrichSnippets())
end CodeSnippets
