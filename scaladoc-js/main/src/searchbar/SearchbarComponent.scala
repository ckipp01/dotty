...
import scala.concurrent.duration.{span as dspan, *}
import scala.util.chaining.*

import java.net.URI

class SearchbarComponent(
    engine: PageSearchEngine,
    inkuireEngine: InkuireJSSearchEngine,
    parser: QueryParser
):
  val initialChunkSize = 5
  val resultsChunkSize = 20
  def pathToRoot() =
    window.document.documentElement.getAttribute("data-pathToRoot")
  extension (p: PageEntry)
    def toHTML(boldChars: Set[Int]) =
      val location =
        if p.isLocationExternal then p.location
        else pathToRoot() + p.location

      val extensionTargetMessage =
        if p.extensionTarget.isEmpty() then ""
        else " extension on " + p.extensionTarget
  ...
  extension (m: InkuireMatch)
    def toHTML =
      val location =
        if m.pageLocation(0) == 'e' then m.pageLocation.substring(1)
        else pathToRoot() + m.pageLocation.substring(1)

      div(
        cls := "scaladoc-searchbar-row mono-small-inline",
        "result" := "",
        "inkuire-result" := "",
        "mq" := m.mq.toString
      )(
        a(href := location)(
          m.functionName,
          span(cls := "pull-right scaladoc-searchbar-inkuire-signature")(
            m.prettifiedSignature
          )
        ).tap {
          _.onclick = (event: Event) =>
            if document.body.contains(rootDiv) then
    ...
    searchTask.map { result =>
      if result.isEmpty then
        val noResultsDiv = div(id := "no-results-container")(
          ...
          h2(cls := "h200 no-result-header")(
            "No results match your filter criteria."
          ),
          p(cls := "body-small no-result-content")(
            "Try adjusting or clearing your filters",
            p("to display better result")
          ),
          button(
            id := "searchbar-clear-button",
            cls := "clearButton label-only-button"
          )("Clear all filters").tap(
            _.addEventListener(
              "click",
              _ =>
                inputElem.value = ""
        ...
        val resultWithDocBonus = result
          .map(entry =>
            ...
            if entry.pageEntry.kind == "static" && !window.location.href
                .contains("api")
            then entry.copy(score = entry.score + 7)
            else entry
          )
        ...
        val groupedResultsSortedByScore = groupedResults
          .map { case (kind, results) =>
            (kind, results.maxByOption(_.score).map(_.score), results)
          }
          .toList
          .sortBy { case (_, topScore, _) =>
            -topScore.getOrElse(0)
          }
          .map { case (kind, _, results) =>
            (kind, results.take(40)) // limit to 40 results per category
          }

        groupedResultsSortedByScore.map { case (kind, results) =>
          val kindSeparator = createKindSeparator(kind)
          val htmlEntries =
            results.map(result => result.pageEntry.toHTML(result.indices))
          ...
          def loadMoreResults(entries: List[raw.HTMLElement]): Unit =
            loadMoreElement.onclick = (event: Event) =>
              entries
                .take(resultsChunkSize)
                .foreach(_.classList.remove("hidden"))
              val nextElems = entries.drop(resultsChunkSize)
              if nextElems.nonEmpty then loadMoreResults(nextElems)
              else loadMoreElement.classList.add("hidden")
  ...
  end handleNewFluffQuery

  def handleRecentQueries(query: String) =
    val recentQueries = RecentQueryStorage.getData
    if query != "" then
      RecentQueryStorage.addEntry(RecentQuery(query, Date.now()))
    val matching = recentQueries
      .filterNot(rq =>
        rq.query.equalsIgnoreCase(query)
      ) // Don't show recent query that is equal to provided query
      .filter { rq => // Fuzzy search
        rq.query
          .foldLeft(query) { (pattern, nextChar) =>
            if !pattern.isEmpty then
              if pattern.head.toString.equalsIgnoreCase(nextChar.toString) then
                pattern.tail
              else pattern
            else ""
          }
    ...
    if matching.nonEmpty then
      resultsDiv.appendChild(
        createKindSeparator("Recently searched", "fas fa-clock re-icon")
      )
        ...
        case SignatureQuery(signature) =>
          ...
          inkuireEngine.query(query) { (m: InkuireMatch) =>
            val next = resultsDiv.children
              .find(child =>
                child.hasAttribute("mq") && Integer.parseInt(
                  child.getAttribute("mq")
                ) > m.mq
              )
            next.fold {
  ...
  private val searchIcon: html.Button =
    ...
    document.body.addEventListener(
      "keydown",
      (e: KeyboardEvent) => handleGlobalKeyDown(e)
    )

    icon

  private val mobileSearch =
    val mobileSearch = document
      .getElementById("mobile-scaladoc-searchbar-input")
      ...
      element.addEventListener(
        ...
        { e =>
          ...
          if inputValue.isEmpty then
            clearResults()
            if RecentQueryStorage.isEmpty then showHints()
            else handleRecentQueries("")
  ...
  def clearResults() = while resultsDiv.hasChildNodes() do
  ...
  private def generateRootDiv(): html.Div =
    val cancelButton =
      span(cls := "scaladoc-searchbar-cancel-button body-small")("Cancel")
    cancelButton.onclick = (event: Event) => document.body.removeChild(rootDiv)
    ...
    val element = div(id := "scaladoc-searchbar")(
      ...
      elem.addEventListener(
        ...
        (e: Event) =>
          ...
          if evTargetId != "scaladoc-searchbar" then e.stopPropagation()
      )
      elem.addEventListener(
        "keydown",
    ...
    document.body.addEventListener(
      "mousedown",
      (e: Event) => if document.body.contains(element) then handleEscape()
    )
  ...
  private def handleArrowUp() =
    ...
    if selectedElement != null then
      ...
      if sibling != null then
        ...
        resultsDiv.scrollTop = sibling
          .asInstanceOf[html.Element]
          .offsetTop - (2 * sibling.asInstanceOf[html.Element].clientHeight)
  private def handleArrowDown() =
    ...
    if selectedElement != null then
      ...
      if firstResult != null then
        val toSelect =
          if firstResult.classList.contains(
              "scaladoc-searchbar-row"
            ) && firstResult.hasAttribute("result")
          then firstResult
          else recur(firstResult)
        toSelect.setAttribute("selected", "")
        resultsDiv.scrollTop = toSelect
          .asInstanceOf[html.Element]
          .offsetTop - (2 * toSelect.asInstanceOf[html.Element].clientHeight)
  end handleArrowDown
  private def handleEnter() =
    val selectedElement =
      resultsDiv.querySelector("[selected] a").asInstanceOf[html.Element]
    if selectedElement != null then selectedElement.click()
  ...
  private def handleHover(elem: html.Element) =
    val selectedElement = resultsDiv.querySelector("[selected]")
    if selectedElement != null then selectedElement.removeAttribute("selected")
    ...
    val hintsDiv = div(cls := "searchbar-hints")(
      ...
      ul(cls := "searchbar-hints-list")(
        ...
        li(cls := "mono-small-inline")(
          "String => Int",
          b(" to find"),
          " String.size, String.toInt"
        ),
        div(cls := "divider"),
        li(cls := "mono-small-inline")(
          "String => String => String",
          b(" to find "),
          "String.mkString, String.stripPrefix"
        ),
        div(cls := "divider"),
        li(cls := "mono-small-inline")(
          "Some[A] => A",
        ...
        li(cls := "link body-small")(
          "Availability of searching by inkuire depends on the configuration of Scaladoc. For more info, ",
          a(
            href := "https://docs.scala-lang.org/scala3/guides/scaladoc/search-engine.html"
          )("the documentation")
        )
      )
    )
