...
class PageSearchEngine(pages: List[PageEntry]):
  ...
  def query(query: NameAndKindQuery): List[MatchResult] =
    matchPages(query)
      .filter { case MatchResult(score, _, _) =>
        score >= 0
      }
      .sortBy { case MatchResult(score, _, _) =>
        -score
      }
  ...
  private val positionScores =
    List(8, 4, 2, 1).orElse(PartialFunction.fromFunction(_ => 0))

  private def matchCompletnessBonus(
      nameCharacters: Int,
      matchCharacters: Int
  ): Int =
  ...
  private def matchPages(query: NameAndKindQuery): List[MatchResult] =
    query match
        ...
        if nameSearch.length > 1 then
          prematchedPages.map { prematched =>
            ...
            val bonusScore = kindScoreBonus(prematched.pageEntry.kind)
              + matchCompletnessBonus(
                prematched.pageEntry.shortName.length,
                nameSearch.length
              )
  ...
  private def prematchPage(page: PageEntry, search: String): MatchResult =
    ...
    def prematchPageAcc(
      ...
      if searchIndex >= search.length then MatchResult(scoreAcc, page, acc)
      else if nameIndex >= pageName.length then MatchResult(0, page, Set.empty)
      else if pageName(nameIndex).toLower == search(searchIndex).toLower then
        val score =
          (if consecutiveMatches > 0 then 1 else 0) + positionScores(nameIndex)
        prematchPageAcc(
          nameIndex + 1,
          searchIndex + 1,
          acc + nameIndex,
      ...
      else prematchPageAcc(nameIndex + 1, searchIndex, acc, scoreAcc, 0)

    val result = prematchPageAcc(0, 0, Set.empty, 0, 0)
    result.copy(score = result.score + kindScoreBonus(page.kind))

  private def matchPage(
      prematched: MatchResult,
      nameSearch: String
  ): MatchResult =
    val searchTokens: List[List[Char]] = StringUtils
      .createCamelCaseTokens(nameSearch)
      .map(_.toList) // todo extract
    ...
    def matchTokens(
      ...
      (
        searchTokensLifted(searchTokenIndex).map(_.toList),
        pageTokensLifted(pageTokenIndex).map(_.toList)
      ) match
        ...
        case (Some(searchHead :: _), Some(pageHead :: _)) =>
          if searchHead == pageHead then
            matchTokens(
              searchTokenIndex + 1,
              pageTokenIndex + 1,
              acc + ((searchTokenIndex, pageTokenIndex))
            )
          else matchTokens(searchTokenIndex, pageTokenIndex + 1, acc)
        // empty tokens edge cases
        case (Some(_), Some(_ :: _)) =>
          matchTokens(searchTokenIndex + 1, pageTokenIndex, acc)
    ...
    def findHighScoreMatch(
      ...
      if searchPosition >= nameSearch.length then
      ...
      else if pagePosition >= pageName.length then None
      else
        val currentSearchTokenStart = searchTokenPositions(searchTokenIndex)
        val matchingPageToken =
          matchedTokens.find(_._1 == searchTokenIndex).map(_._2)
        val searchChar = nameSearch.charAt(searchPosition).toLower
        val pageChar = pageName.charAt(pagePosition).toLower

        def recalculateTokenIndex(
            tokenPositions: Seq[Int],
            previousIndex: Int,
            position: Int
        ): Int =
          if tokenPositions.length <= previousIndex + 1 || tokenPositions(
              previousIndex + 1
            ) > position
          then previousIndex
        ...
        def getMatchScore(
            matchedPagePosition: Int,
            matchedPageTokenStart: Int
        ): Int =
          val consecutiveMatchesScore = if consecutiveMatches > 0 then 1 else 0
          val matchPositionScore = positionScores(
            matchedPagePosition - matchedPageTokenStart
          )
          val firstTokenScore =
            if matchPositionScore > 0 && matchedPageTokenStart == 0 then 3
            else 0
          consecutiveMatchesScore + matchPositionScore + firstTokenScore

        matchingPageToken match
          case Some(matchingToken)
              if searchPosition == currentSearchTokenStart =>
            val matchedTokenPosition = pageTokensPositions(matchingToken)
            findHighScoreMatch(
              recalculateTokenIndex(
                searchTokenPositions,
                searchTokenIndex,
                searchPosition + 1
              ),
              searchPosition + 1,
              ...
              scoreAcc + getMatchScore(
                matchedTokenPosition,
                matchedTokenPosition
              ),
              consecutiveMatches + 1
            )
          case _ if searchChar == pageChar =>
            val matchedTokenPosition =
              matchingPageToken.map(pageTokensPositions).getOrElse(0)
            findHighScoreMatch(
              recalculateTokenIndex(
                searchTokenPositions,
                searchTokenIndex,
